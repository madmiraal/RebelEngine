...
XcursorImage* create_xcursor_image(
    ...
    XcursorUInt pixels     = cursor_size.x * cursor_size.y;
...
Error LinuxOS::initialize(
    ...
    if (xkb_dar == True) {
        ...
        // Xutf8LookupString will be used later instead of XmbLookupString
        // before the multibyte sequences can be converted to unicode string.
    ...
    if (!xrandr_handle) {
        err           = dlerror();
        // For some arcane reason, NetBSD now ships libXrandr.so.3 while the
        // rest of the world has libXrandr.so.2... In case this happens for
        // other X11 platforms in the future, let's give it a try too before
        // failing.
        ...
        if (((xrandr_major << 8) | xrandr_minor) >= 0x0105) {
            xrr_get_monitors =
                (xrr_get_monitors_t)dlsym(xrandr_handle, "XRRGetMonitors");
            if (!xrr_get_monitors) {
                err = dlerror();
                fprintf(
                    stderr,
                    "could not find symbol XRRGetMonitors\nError: %s\n",
                    err
                );
            } else {
                xrr_free_monitors = (xrr_free_monitors_t
                )dlsym(xrandr_handle, "XRRFreeMonitors");
                if (!xrr_free_monitors) {
                    err = dlerror();
                    fprintf(
                        stderr,
                        "could not find XRRFreeMonitors\nError: %s\n",
                        err
                    );
                    xrr_get_monitors = nullptr;
                }
    ...
    if (xim == nullptr) {
        ...
        if (XSetIMValues(xim, XNDestroyCallback, &im_destroy_callback, NULL)
            != nullptr) {
...
// maybe contextgl wants to be in charge of creating the window
    ...
    if (getenv("DRI_PRIME") == nullptr) {
        ...
        if (getenv("PRIMUS_DISPLAY") || getenv("PRIMUS_libGLd")
            || getenv("PRIMUS_libGLa") || getenv("PRIMUS_libGL")
            || getenv("PRIMUS_LOAD_GLOBAL") || getenv("BUMBLEBEE_SOCKET")) {
            print_verbose("Optirun/primusrun detected. Skipping GPU detection");
            use_prime = 0;
        }

        // Some tools use fake libGL libraries and have them override the real
        // one using LD_LIBRARY_PATH, so we skip them. *But* Steam also sets
        // LD_LIBRARY_PATH for its runtime and includes system `/lib` and
        // `/lib64`... so ignore Steam.
        if (use_prime == -1 && getenv("LD_LIBRARY_PATH")
            && !getenv("STEAM_RUNTIME_LIBRARY_PATH")) {
        ...
        if (use_prime == -1) {
            print_verbose(
                "Detecting GPUs, set DRI_PRIME in the environment to override "
                "GPU detection logic."
            );
    ...
    LinuxGLContext::ContextType opengl_api_type =
        LinuxGLContext::GLES_3_0_COMPATIBLE;

    if (p_video_driver == VIDEO_DRIVER_GLES2) {
        opengl_api_type = LinuxGLContext::GLES_2_0_COMPATIBLE;
    }
    ...
    gl_context = nullptr;
    while (!gl_context) {
        gl_context = memnew(LinuxGLContext(
            x11_display,
            x11_window,
            current_videomode,
            opengl_api_type
    ...
    while (true) {
        if (opengl_api_type == LinuxGLContext::GLES_3_0_COMPATIBLE) {
        ...
        if (opengl_api_type == LinuxGLContext::GLES_2_0_COMPATIBLE) {
    ...
    if (xim && xim_style) {
        ...
        if (XGetICValues(xic, XNFilterEvents, &im_event_mask, NULL)
            != nullptr) {
...

    {
        ...
        Pixmap cursormask = XCreatePixmap(
            x11_display,
            RootWindow(x11_display, DefaultScreen(x11_display)),
            1,
            1,
            1
        );
        ...
        // Color value doesn't matter. Mask zero means no foreground or
        // background will be drawn
        XColor col = {};

        Cursor cursor = XCreatePixmapCursor(
            x11_display,
            cursormask, // source (using cursor mask as placeholder, since it'll
                        // all be ignored)
            cursormask, // mask
            &col,
            &col,
    ...
    joypad = memnew(LinuxJoypad(input));
#endif
...
void LinuxOS::create_cursors() {
    create_cursor(CURSOR_ARROW, "left_ptr");
    create_cursor(CURSOR_IBEAM, "xterm");
    create_cursor(CURSOR_POINTING_HAND, "hand2");
    create_cursor(CURSOR_CROSS, "cross");
    create_cursor(CURSOR_WAIT, "watch");
    create_cursor(CURSOR_BUSY, "left_ptr_watch");
    create_cursor(CURSOR_DRAG, "fleur");
    create_cursor(CURSOR_CAN_DROP, "hand1");
    create_cursor(CURSOR_FORBIDDEN, "X_cursor");
    create_cursor(CURSOR_VSIZE, "sb_v_double_arrow");
    create_cursor(CURSOR_HSIZE, "sb_h_double_arrow");
    create_cursor(CURSOR_BDIAGSIZE, "size_bdiag");
    create_cursor(CURSOR_FDIAGSIZE, "size_fdiag");
    create_cursor(CURSOR_MOVE, "hand1");
    create_cursor(CURSOR_VSPLIT, "sb_v_double_arrow");
    create_cursor(CURSOR_HSPLIT, "sb_h_double_arrow");
    create_cursor(CURSOR_HELP, "question_arrow");
}
...
void LinuxOS::free_cursor(CursorShape cursor_shape) {
...
bool LinuxOS::refresh_device_info() {
    ...
    if (XIQueryVersion(x11_display, &xi_major_query, &xi_minor_query)
        != Success) {
        print_verbose(vformat(
            "XInput 2 not available (server supports %d.%d).",
            xi_major_query,
            xi_minor_query
        ));
        xi.opcode = 0;
    ...
    if (xi_major_query < XINPUT_CLIENT_VERSION_MAJOR
        || (xi_major_query == XINPUT_CLIENT_VERSION_MAJOR
            && xi_minor_query < XINPUT_CLIENT_VERSION_MINOR)) {
        print_verbose(vformat(
            "XInput %d.%d not available (server supports %d.%d). Touch input "
            "unavailable.",
            XINPUT_CLIENT_VERSION_MAJOR,
            XINPUT_CLIENT_VERSION_MINOR,
            xi_major_query,
            xi_minor_query
        ));
    }
    ...
    for (int i = 0; i < dev_count; i++) {
        ...
        for (int j = 0; j < dev->num_classes; j++) {
            ...
            if (dev->classes[j]->type == XITouchClass
                && ((XITouchClassInfo*)dev->classes[j])->mode
                       == XIDirectTouch) {
                direct_touch = true;
            }
#endif
            if (dev->classes[j]->type == XIValuatorClass) {
                XIValuatorClassInfo* class_info =
                    (XIValuatorClassInfo*)dev->classes[j];

                if (class_info->number == VALUATOR_ABSX
        ...
        if (absolute_mode) {
            ...
            if (resolution_x <= 0) {
                resolution_x =
                    (abs_x_max - abs_x_min) * abs_resolution_range_mult;
            }
            if (resolution_y <= 0) {
                resolution_y =
                    (abs_y_max - abs_y_min) * abs_resolution_range_mult;
            }
            xi.absolute_devices[dev->deviceid] = Vector2(
                abs_resolution_mult / resolution_x,
                abs_resolution_mult / resolution_y
            );
            print_verbose(
                "XInput: Absolute pointing device: " + String(dev->name)
            );
        }

        xi.pressure = 0;
        xi.pen_pressure_range[dev->deviceid] =
            Vector2(pressure_min, pressure_max);
...
void LinuxOS::xim_destroy_callback(
    ::XIM im,
    ::XPointer client_data,
    ::XPointer call_data
) {
    WARN_PRINT("Input method stopped");
    LinuxOS* os = reinterpret_cast<LinuxOS*>(client_data);
    os->xim     = nullptr;
    os->xic     = nullptr;
}

void LinuxOS::set_ime_active(const bool p_active) {
    im_active = p_active;
...
void LinuxOS::set_ime_position(const Point2& p_pos) {
    im_position = p_pos;
...
String LinuxOS::get_unique_id() const {
    static String machine_id;
    if (machine_id.empty()) {
        if (FileAccess* f =
                FileAccess::open("/etc/machine-id", FileAccess::READ)) {
...
void LinuxOS::set_mouse_mode(MouseMode p_mode) {
...
void LinuxOS::warp_mouse_position(const Point2& p_to) {
    if (mouse_mode == MOUSE_MODE_CAPTURED) {
        last_mouse_pos = p_to;
    } else {
        /*XWindowAttributes xwa;
...
void LinuxOS::flush_mouse_motion() {
    // Block events polling while flushing motion events.
    MutexLock mutex_lock(events_mutex);

    for (uint32_t event_index = 0; event_index < polled_events.size();
         ++event_index) {
...
OS::MouseMode LinuxOS::get_mouse_mode() const {
    return mouse_mode;
}

int LinuxOS::get_mouse_button_state() const {
    return last_button_state;
}

Point2 LinuxOS::get_mouse_position() const {
    return last_mouse_pos;
}

bool LinuxOS::get_window_per_pixel_transparency_enabled() const {
...
void LinuxOS::set_window_per_pixel_transparency_enabled(bool p_enabled) {
    if (!is_layered_allowed()) {
        return;
    }
...
void LinuxOS::set_window_title(const String& p_title) {
...
void LinuxOS::set_window_mouse_passthrough(const PoolVector2Array& p_region) {
    int event_base, error_base;
    const Bool ext_okay =
        XShapeQueryExtension(x11_display, &event_base, &error_base);
    if (ext_okay) {
        Region region;
        if (p_region.size() == 0) {
            ...
            XPoint* points =
                (XPoint*)memalloc(sizeof(XPoint) * p_region.size());
...
void LinuxOS::set_video_mode(const VideoMode& p_video_mode, int p_screen) {}

OS::VideoMode LinuxOS::get_video_mode(int p_screen) const {
    return current_videomode;
}

void LinuxOS::get_fullscreen_mode_list(List<VideoMode>* p_list, int p_screen)
    const {}

void LinuxOS::set_wm_fullscreen(bool p_enabled) {
    ...
    if (p_enabled && !is_window_resizable()) {
        // Set the window as resizable to prevent window managers to ignore the
    ...
    Atom wm_fullscreen =
        XInternAtom(x11_display, "_NET_WM_STATE_FULLSCREEN", False);
    ...
    xev.xclient.data.l[0] =
        p_enabled ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE;
    ...
    Atom bypass_compositor =
        XInternAtom(x11_display, "_NET_WM_BYPASS_COMPOSITOR", False);
    ...
    if (!p_enabled) {
        ...
        // put back or remove decorations according to the last set borderless
        // state
        Hints hints;
        Atom property;
        hints.flags       = 2;
...
void LinuxOS::set_wm_above(bool p_enabled) {
    Atom wm_state = XInternAtom(x11_display, "_NET_WM_STATE", False);
...
int LinuxOS::get_screen_count() const {
    // Using Xinerama Extension
    int event_base, error_base;
    const Bool ext_okay =
        XineramaQueryExtension(x11_display, &event_base, &error_base);
    if (!ext_okay) {
        return 0;
    }
...
int LinuxOS::get_current_screen() const {
    int x, y;
    Window child;
...
void LinuxOS::set_current_screen(int p_screen) {
    if (p_screen == -1) {
...
Point2 LinuxOS::get_screen_position(int p_screen) const {
    if (p_screen == -1) {
        p_screen = get_current_screen();
    }
...
Size2 LinuxOS::get_screen_size(int p_screen) const {
    if (p_screen == -1) {
        p_screen = get_current_screen();
    }
...
int LinuxOS::get_screen_dpi(int p_screen) const {
    if (p_screen == -1) {
        p_screen = get_current_screen();
    }
    ...
    if (xrandr_ext_ok) {
        int count = 0;
        if (xrr_get_monitors) {
            xrr_monitor_info* monitors =
                xrr_get_monitors(x11_display, x11_window, true, &count);
            if (p_screen < count) {
                double xdpi =
                    sc.width / (double)monitors[p_screen].mwidth * 25.4;
                double ydpi =
                    sc.height / (double)monitors[p_screen].mheight * 25.4;
...
Point2 LinuxOS::get_window_position() const {
    int x, y;
    Window child;
    XTranslateCoordinates(
        x11_display,
...
void LinuxOS::set_window_position(const Point2& p_position) {
    int x = 0;
    int y = 0;
...
Size2 LinuxOS::get_window_size() const {
...
Size2 LinuxOS::get_real_window_size() const {
    XWindowAttributes xwa;
    XSync(x11_display, False);
    XGetWindowAttributes(x11_display, x11_window, &xwa);
    int w     = xwa.width;
...
Size2 LinuxOS::get_max_window_size() const {
    return max_size;
}

Size2 LinuxOS::get_min_window_size() const {
    return min_size;
}

void LinuxOS::set_min_window_size(const Size2 p_size) {
    if ((p_size != Size2()) && (max_size != Size2())
        && ((p_size.x > max_size.x) || (p_size.y > max_size.y))) {
        ERR_PRINT(
            "Minimum window size can't be larger than maximum window size!"
        );
        return;
    }
    min_size = p_size;
...
void LinuxOS::set_max_window_size(const Size2 p_size) {
    if ((p_size != Size2())
        && ((p_size.x < min_size.x) || (p_size.y < min_size.y))) {
        ERR_PRINT(
            "Maximum window size can't be smaller than minimum window size!"
        );
        return;
    }
    max_size = p_size;
...
void LinuxOS::set_window_size(const Size2 p_size) {
    if (current_videomode.width == p_size.width
...
void LinuxOS::set_window_fullscreen(bool p_enabled) {
    ...
    if (p_enabled && current_videomode.always_on_top) {
        // Fullscreen + Always-on-top requires a maximized window on some window
        // managers (Metacity)
        set_window_maximized(true);
    }
...
bool LinuxOS::is_window_fullscreen() const {
    return current_videomode.fullscreen;
}

void LinuxOS::set_window_resizable(bool p_enabled) {
    XSizeHints* xsh;
    xsh = XAllocSizeHints();
    if (!p_enabled) {
        Size2 size = get_window_size();
...
bool LinuxOS::is_window_resizable() const {
    return current_videomode.resizable;
}

void LinuxOS::set_window_minimized(bool p_enabled) {
...
bool LinuxOS::is_window_minimized() const {
    // Using ICCCM -- Inter-Client Communication Conventions Manual
...
void LinuxOS::set_window_maximized(bool p_enabled) {
    if (is_no_window_mode_enabled()) {
        return;
    }
    ...
    Atom wm_state = XInternAtom(x11_display, "_NET_WM_STATE", False);
    Atom wm_max_horz =
        XInternAtom(x11_display, "_NET_WM_STATE_MAXIMIZED_HORZ", False);
    Atom wm_max_vert =
        XInternAtom(x11_display, "_NET_WM_STATE_MAXIMIZED_VERT", False);
    ...
    if (p_enabled && is_window_maximize_allowed()) {
        ...
        for (int attempt = 0; !is_window_maximized() && attempt < 50;
             attempt++) {
            usleep(10000);
        }
    }
...
bool LinuxOS::window_maximize_check(const char* p_atom_name) const {
    ...
    if (result == Success) {
        ...
        if (strcmp(p_atom_name, "_NET_WM_STATE") == 0) {
            ...
            wm_act_max_horz =
                XInternAtom(x11_display, "_NET_WM_ACTION_MAXIMIZE_HORZ", False);
            wm_act_max_vert =
                XInternAtom(x11_display, "_NET_WM_ACTION_MAXIMIZE_VERT", False);
        }
...
bool LinuxOS::window_fullscreen_check() const {
    // Using EWMH -- Extended Window Manager Hints
...
bool LinuxOS::is_window_maximize_allowed() const {
    return window_maximize_check("_NET_WM_ALLOWED_ACTIONS");
}

bool LinuxOS::is_window_maximized() const {
    // Using EWMH -- Extended Window Manager Hints
    return window_maximize_check("_NET_WM_STATE");
}

void LinuxOS::set_window_always_on_top(bool p_enabled) {
...
bool LinuxOS::is_window_always_on_top() const {
    return current_videomode.always_on_top;
}

bool LinuxOS::is_window_focused() const {
    return window_focused;
}

void LinuxOS::set_borderless_window(bool p_borderless) {
...
bool LinuxOS::get_borderless_window() {
...
void LinuxOS::request_attention() {
    // Using EWMH -- Extended Window Manager Hints
    //
    // Sets the _NET_WM_STATE_DEMANDS_ATTENTION atom for WM_STATE
    // Will be unset by the window manager after user react on the request for
    // attention

    XEvent xev;
    Atom wm_state = XInternAtom(x11_display, "_NET_WM_STATE", False);
    Atom wm_attention =
        XInternAtom(x11_display, "_NET_WM_STATE_DEMANDS_ATTENTION", False);
...
void* LinuxOS::get_native_handle(int p_handle_type) {
    switch (p_handle_type) {
        ...
        case WINDOW_VIEW:
            return nullptr; // Do we have a value to return here?
        case OPENGL_CONTEXT:
            return gl_context->get_glx_context();
        default:
            return nullptr;
    }
}

void LinuxOS::get_key_modifier_state(
    ...
    state->set_alt((p_x11_state & Mod1Mask /*|| p_x11_state&Mod5Mask*/)
    ); // altgr should not count as alt
    state->set_metakey((p_x11_state & Mod4Mask));
}

unsigned int LinuxOS::get_mouse_button_state(
...
void LinuxOS::_handle_key_event(
    XKeyEvent* p_event,
    ...
    if (xkeyevent->type == KeyPress && xic) {
        ...
        int utf8bytes    = Xutf8LookupString(
            xic,
            xkeyevent,
            utf8string,
            utf8len - 1,
        ...
        if (status == XLookupChars) {
            ...
            unsigned int keycode = LinuxKeyMapping::get_keycode(keysym_keycode);
            unsigned int physical_keycode =
                LinuxKeyMapping::get_scancode(xkeyevent->keycode);
    ...
    // LinuxKeyMapping just translated the X11 keysym to a PIGUI
    ...
    // LinuxKeyMapping also translates keysym to unicode.
    // It does a binary search on a table to translate
    // most properly.
    unsigned int unicode =
        keysym_unicode > 0
            ? LinuxKeyMapping::get_unicode_from_keysym(keysym_unicode)
            : 0;
    ...
    // print_verbose("mod1: "+itos(xkeyevent->state&Mod1Mask)+" mod 5:
    // "+itos(xkeyevent->state&Mod5Mask));
    ...
    bool last_is_pressed =
        Input::get_singleton()->is_key_pressed(k->get_scancode());
    if (k->is_pressed()) {
        if (last_is_pressed) {
            k->set_echo(true);
        }
...
Atom LinuxOS::_process_selection_request_target(
    Atom p_target,
    Window p_requestor,
    Atom p_property
) const {
...
void LinuxOS::_handle_selection_request_event(XSelectionRequestEvent* p_event
) const {
    XEvent respond;
    if (p_event->target == XInternAtom(x11_display, "MULTIPLE", 0)) {
        ...
        if (XGetWindowProperty(
            ...
            if ((len >= 2) && data) {
                ...
                for (uint64_t i = 0; i < len; i += 2) {
                    ...
                    property       = _process_selection_request_target(
                        target,
                        p_event->requestor,
                        property
                    );
        ...
        respond.xselection.property = _process_selection_request_target(
            p_event->target,
            p_event->requestor,
            p_event->property
        );
...
static Atom pick_target_from_list(
    Display* p_display,
    Atom* p_list,
    int p_count
) {
    ...
    for (int i = 0; i < p_count; i++) {
        ...
        if (atom != None
            && String(XGetAtomName(p_display, atom)) == target_type) {
            return atom;
        }
    }
    return None;
...
static Atom pick_target_from_atoms(
    Display* p_disp,
    Atom p_t1,
    Atom p_t2,
    Atom p_t3
...
void LinuxOS::_window_changed(XEvent* event) {
    if (xic) {
        // Not portable.
...
void LinuxOS::_poll_events_thread(void* ud) {
    LinuxOS* os = (LinuxOS*)ud;
    os->_poll_events();
}

Bool LinuxOS::_predicate_all_events(
    Display* display,
    XEvent* event,
    XPointer arg
) {
...
bool LinuxOS::_wait_for_events() const {
...
void LinuxOS::_check_pending_events(LocalVector<XEvent>& r_events) {
    ...
    while (XCheckIfEvent(x11_display, &ev, _predicate_all_events, nullptr)) {
        ...
        // communication through the x server takes several events sent back and
        // forth and we don't want to block other programs while processing only
        // one each frame.
...
void LinuxOS::process_xevents() {
    // printf("checking events %i\n", XPending(x11_display));

    do_mouse_warp = false;

    // Is the current mouse mode one where it needs to be grabbed.
    bool mouse_mode_grab =
        mouse_mode == MOUSE_MODE_CAPTURED || mouse_mode == MOUSE_MODE_CONFINED;
    ...
    for (uint32_t event_index = 0; event_index < events.size(); ++event_index) {
            ...
            if (event.xcookie.type == GenericEvent
                ...
                switch (event_data->evtype) {
                    ...
                    case XI_RawMotion: {
                        ...
                        // Determine the axis used (called valuators in XInput
                        // for some forsaken reason)
                        //  Mask is a bitmask indicating which axes are
                        //  involved. We are interested in the values of axes 0
                        //  and 1.
                        ...
                        if (XIMaskIsSet(
                            ...
                            Map<int, Vector2>::Element* pen_pressure =
                                xi.pen_pressure_range.find(device_id);
                            if (pen_pressure) {
                                Vector2 pen_pressure_range =
                                    pen_pressure->value();
                                if (pen_pressure_range != Vector2()) {
                                    xi.pressure_supported = true;
                                    xi.pressure =
                                        (*values - pen_pressure_range[0])
                                        / (pen_pressure_range[1]
                                           - pen_pressure_range[0]);
                                }
                            }
                        ...
                        if (XIMaskIsSet(
                            ...
                            Map<int, Vector2>::Element* pen_tilt_x =
                                xi.pen_tilt_x_range.find(device_id);
                            if (pen_tilt_x) {
                        ...
                        if (XIMaskIsSet(
                            ...
                            Map<int, Vector2>::Element* pen_tilt_y =
                                xi.pen_tilt_y_range.find(device_id);
                            if (pen_tilt_y) {
                        ...
                        if (raw_event->time == xi.last_relative_time
                            && rel_x == xi.relative_motion.x
                            && rel_y == xi.relative_motion.y) {
                            break; // Flush duplicate to avoid overly fast
                                   // motion
                        }
                        ...
                        Map<int, Vector2>::Element* abs_info =
                            xi.absolute_devices.find(device_id);

                        if (abs_info) {
                            // Absolute mode device
                            Vector2 mult = abs_info->value();

                            xi.relative_motion.x +=
                                (xi.raw_pos.x - xi.old_raw_pos.x) * mult.x;
                            xi.relative_motion.y +=
                                (xi.raw_pos.y - xi.old_raw_pos.y) * mult.y;
                        } else {
                    ...
                    case XI_TouchUpdate: {
                        Map<int, Vector2>::Element* curr_pos_elem =
                            xi.state.find(index);
        ...
        switch (event.type) {
            ...
            case FocusIn:
                ...
                if (mouse_mode_grab) {
                    // Show and update the cursor if confined and the window
                    // regained focus.
                    if (mouse_mode == MOUSE_MODE_CONFINED) {
                        XUndefineCursor(x11_display, x11_window);
                    } else if (mouse_mode == MOUSE_MODE_CAPTURED) { // or
                                                                    // re-hide
                                                                    // it in
                                                                    // captured
                                                                    // mode
                ...
                /*for (int i = 0; i < xi.touch_devices.size(); ++i) {
                    XIGrabDevice(x11_display, xi.touch_devices[i], x11_window,
                CurrentTime, None, XIGrabModeAsync, XIGrabModeAsync, False,
                &xi.touch_event_mask);
                }*/
#endif
                if (xic) {
            ...
            case FocusOut:
                ...
                if (mouse_mode_grab) {
                    // dear X11, I try, I really try, but you never work, you do
                    // whatever you want.
                    if (mouse_mode == MOUSE_MODE_CAPTURED) {
                        // Show the cursor if we're in captured mode so it
                        // doesn't look weird.
                        XUndefineCursor(x11_display, x11_window);
                    }
                    XUngrabPointer(x11_display, CurrentTime);
...
#ifdef TOUCH_ENABLED
                // Ungrab touch devices so input works as usual while we are
                // unfocused
                /*for (int i = 0; i < xi.touch_devices.size(); ++i) {
                    XIUngrabDevice(x11_display, xi.touch_devices[i],
                CurrentTime);
                }*/

                // Release every pointer to avoid sticky points
                for (Map<int, Vector2>::Element* E = xi.state.front(); E;
                     E                             = E->next()) {
                    Ref<InputEventScreenTouch> st;
                    st.instance();
                    st->set_index(E->key());
            ...
            case ButtonRelease: {
                ...
                mb->set_button_mask(get_mouse_button_state(
                    mb->get_button_index(),
                    event.xbutton.type
                ));
                ...
                if (event.type == ButtonPress) {
                    ...
                    if (!mb->is_doubleclick()) {
                        last_click_ms += diff;
                        last_click_pos =
                            Point2(event.xbutton.x, event.xbutton.y);
                    }
                }
            ...
            case MotionNotify: {
                ...
                while (true) {
                    if (mouse_mode == MOUSE_MODE_CAPTURED
                        && event.xmotion.x == current_videomode.width / 2
                        && event.xmotion.y == current_videomode.height / 2) {
                        // this is likely the warp event since it was warped
                        // here
                ...
                if (xi.state.size()
                    && Vector2(pos).distance_squared_to(xi.mouse_pos_to_filter)
                           < 2) {
                    filter = true;
                }
                // Invalidate to avoid filtering a possible legitimate similar
                // event coming later
                ...
                if (mouse_mode == MOUSE_MODE_CAPTURED) {
                    ...
                    center             = new_center;
                    do_mouse_warp =
                        window_has_focus; // warp the cursor if we're focused in
                }
                ...
                // Hackish but relative mouse motion is already handled in the
                // RawMotion event.
                //  RawMotion does not provide the absolute mouse position
                //  (whereas MotionNotify does). Therefore, RawMotion cannot be
                //  the authority on absolute mouse position. RawMotion provides
                //  more precision than MotionNotify, which doesn't sense
                //  subpixel motion. Therefore, MotionNotify cannot be the
                //  authority on relative mouse motion. This means we need to
                //  take a combined approach...
                Point2 rel;

                // Only use raw input if in capture mode. Otherwise use the
                // classic behavior.
                if (mouse_mode == MOUSE_MODE_CAPTURED) {
                ...
                if (mouse_mode == MOUSE_MODE_CAPTURED) {
                    pos = Point2i(
                        current_videomode.width / 2,
                        current_videomode.height / 2
                    );
                }
                ...
                if (xi.pressure_supported) {
                    ...
                    mm->set_pressure(
                        (get_mouse_button_state() & (1 << (BUTTON_LEFT - 1)))
                            ? 1.0f
                            : 0.0f
                    );
                }
                ...
                // Make the absolute position integral so it doesn't look _too_
                // weird :)
                Point2i posi(pos);
            ...
            case SelectionNotify:
                ...
                if (event.xselection.target == requested) {
                    Property p = read_property(
                        x11_display,
                        x11_window,
                        XInternAtom(x11_display, "PRIMARY", 0)
                    );

                    Vector<String> files =
                        String((char*)p.data).split("\n", false);
                    for (int i = 0; i < files.size(); i++) {
                        files.write[i] = files[i]
                                             .replace("file://", "")
                                             .http_unescape()
                                             .strip_edges();
                    }
            ...
            case ClientMessage:
                ...
                if ((unsigned int)event.xclient.data.l[0]
                    ...
                    if (more_than_3) {
                        ...
                        requested = pick_target_from_list(
                            x11_display,
                            (Atom*)p.data,
                            p.nitems
                        );
    ...
    if (do_mouse_warp) {
        ...
        int win_x, win_y;
        unsigned int mask;
        XQueryPointer( x11_display, x11_window, &root, &child, &root_x, &root_y,
        &win_x, &win_y, &mask );
...
MainLoop* LinuxOS::get_main_loop() const {
    return main_loop;
}

void LinuxOS::delete_main_loop() {
    // Send owned clipboard data to clipboard manager before exit.
    // This has to be done here because the clipboard data is cleared before
    // finalize().
    _clipboard_transfer_ownership(XA_PRIMARY, x11_window);
    _clipboard_transfer_ownership(
...
void LinuxOS::set_main_loop(MainLoop* p_main_loop) {
    main_loop = p_main_loop;
    input->set_main_loop(p_main_loop);
}

bool LinuxOS::can_draw() const {
    return !minimized;
};

void LinuxOS::set_clipboard(const String& p_text) {
    {
...
Bool LinuxOS::_predicate_clipboard_selection(
    Display* display,
    XEvent* event,
    XPointer arg
) {
...
Bool LinuxOS::_predicate_clipboard_incr(
    Display* display,
    XEvent* event,
    XPointer arg
) {
...
String LinuxOS::_get_clipboard_impl(
    Atom p_source,
    Window x11_window,
    Atom target
) const {
    ...
    if (selection_owner != None) {
        ...
        // Blocking wait for predicate to be True and remove the event from the
        // queue.
        XEvent event;
        XIfEvent(
            x11_display,
            &event,
            _predicate_clipboard_selection,
            (XPointer)&x11_window
        );
        ...
        if (type == XInternAtom(x11_display, "INCR", 0)) {
            ...
            while (!done) {
                ...
                // Non-blocking wait for next event and remove it from the
                // queue.
                XEvent ev;
                while (XCheckIfEvent(
                    x11_display,
                    &ev,
                    _predicate_clipboard_incr,
                    nullptr
                )) {
...
String LinuxOS::_get_clipboard(Atom p_source, Window x11_window) const {
    String ret;
...
String LinuxOS::get_clipboard() const {
    String ret;
...
Bool LinuxOS::_predicate_clipboard_save_targets(
    Display* display,
    XEvent* event,
    XPointer arg
) {
...
void LinuxOS::_clipboard_transfer_ownership(Atom p_source, Window x11_window)
    const {
    Window selection_owner = XGetSelectionOwner(x11_display, p_source);
    ...
    Atom clipboard_manager =
        XInternAtom(x11_display, "CLIPBOARD_MANAGER", False);
    ...
    while (true) {
        ...
        while (XCheckIfEvent(
            x11_display,
            &ev,
            _predicate_clipboard_save_targets,
            (XPointer)&x11_window
        )) {
            switch (ev.type) {
                ...
                case SelectionNotify: {
                    if (ev.xselection.target == save_targets) {
                        // Once SelectionNotify is received, we're done whether
                        // it succeeded or not.
                        return;
                    }
...
String LinuxOS::get_name() const {
    return "Linux";
}

Error LinuxOS::shell_open(String p_uri) {
    Error ok;
    int err_code;
    List<String> args;
    ...
    // GNOME
    args.push_front("open"
    ); // The command is `gio open`, so we need to add it to args
...
bool LinuxOS::_check_internal_feature_support(const String& p_feature) {
    return p_feature == "pc";
}

String LinuxOS::get_config_path() const {
    if (has_environment("XDG_CONFIG_HOME")) {
        if (get_environment("XDG_CONFIG_HOME").is_abs_path()) {
            return get_environment("XDG_CONFIG_HOME");
        } else {
            WARN_PRINT_ONCE(
                "`XDG_CONFIG_HOME` is a relative path. Ignoring its value and "
                "falling back to `$HOME/.config` or `.` per the XDG Base "
...
String LinuxOS::get_data_path() const {
    if (has_environment("XDG_DATA_HOME")) {
        if (get_environment("XDG_DATA_HOME").is_abs_path()) {
            return get_environment("XDG_DATA_HOME");
        } else {
            WARN_PRINT_ONCE(
                "`XDG_DATA_HOME` is a relative path. Ignoring its value and "
                "falling back to `$HOME/.local/share` or `get_config_path()` "
...
String LinuxOS::get_cache_path() const {
    if (has_environment("XDG_CACHE_HOME")) {
        if (get_environment("XDG_CACHE_HOME").is_abs_path()) {
            return get_environment("XDG_CACHE_HOME");
        } else {
            WARN_PRINT_ONCE(
                "`XDG_CACHE_HOME` is a relative path. Ignoring its value and "
                "falling back to `$HOME/.cache` or `get_config_path()` per the "
...
String LinuxOS::get_system_dir(SystemDir p_dir, bool p_shared_storage) const {
    String xdgparam;
    ...
    Error err = const_cast<LinuxOS*>(this)
                    ->execute("xdg-user-dir", arg, true, nullptr, &pipe);
    if (err != OK) {
        return ".";
    }
...
void LinuxOS::move_window_to_foreground() {
    XEvent xev;
    Atom net_active_window =
        XInternAtom(x11_display, "_NET_ACTIVE_WINDOW", False);
...
void LinuxOS::set_cursor_shape(CursorShape cursor_shape) {
...
void LinuxOS::set_custom_mouse_cursor(
    ...
    if (cursor_image.is_null()) {
        ...
        create_cursor(cursor_shape, xcursor_names[cursor_shape]);
        update_cursor();
        return;
    }
    ...
    XcursorImage* xcursor_image =
        create_xcursor_image(cursor_texture, cursor_hotspot);
...
void LinuxOS::make_rendering_thread() {
#if defined(OPENGL_ENABLED)
    gl_context->make_current();
#endif
}

void LinuxOS::swap_buffers() {
#if defined(OPENGL_ENABLED)
    gl_context->swap_buffers();
#endif
}

void LinuxOS::alert(const String& p_alert, const String& p_title) {
    if (is_no_window_mode_enabled()) {
        print_line("ALERT: " + p_title + ": " + p_alert);
        return;
    }

    const char* message_programs[] =
        {"zenity", "kdialog", "Xdialog", "xmessage"};
    ...
    for (int i = 0; i < path_elems.size(); i++) {
        for (uint64_t k = 0; k < sizeof(message_programs) / sizeof(char*);
             k++) {
...
void LinuxOS::set_icon(const Ref<Image>& p_icon) {
    int (*oldHandler)(Display*, XErrorEvent*) =
        XSetErrorHandler(&set_icon_errorhandler);
    ...
    if (p_icon.is_valid()) {
        ...
        while (true) {
            ...
            // We're using long to have wordsize (32Bit build -> 32 Bits, 64 Bit
            // build -> 64 Bits
            Vector<long> pd;
...
void LinuxOS::force_process_input() {
...
bool LinuxOS::is_joy_known(int p_device) {
    return input->is_joy_mapped(p_device);
}

String LinuxOS::get_joy_guid(int p_device) const {
    return input->get_joy_guid_remapped(p_device);
}

void LinuxOS::_set_use_vsync(bool p_enable) {
#if defined(OPENGL_ENABLED)
    if (gl_context) {
...
/*
bool LinuxOS::is_vsync_enabled() const {
...
*/
void LinuxOS::set_context(int p_context) {
...
OS::PowerState LinuxOS::get_power_state() {
    return power_manager->get_power_state();
}

int LinuxOS::get_power_seconds_left() {
    return power_manager->get_power_seconds_left();
}

int LinuxOS::get_power_percent_left() {
    return power_manager->get_power_percent_left();
}

void LinuxOS::disable_crash_handler() {
    crash_handler.disable();
}

bool LinuxOS::is_disable_crash_handler() const {
...
Error LinuxOS::move_to_trash(const String& p_path) {
    String path = p_path.rstrip("/"
    ); // Strip trailing slash when path points to a directory

    int err_code;
    List<String> args;
    args.push_back(path);
    args.push_front("trash"
    ); // The command is `gio trash <file_name>` so we need to add it to args.
    ...
    args.push_back("trash:/"
    ); // The command is `kioclient5 move <file_name> trash:/`.
    result = execute(
        "kioclient5",
        args,
        true,
    ...
    // If the commands `kioclient5`, `gio` or `gvfs-trash` don't exist on the
    // system we do it manually.
    String trash_path = "";
    String mnt        = get_mountpoint(path);

    // If there is a directory "[Mountpoint]/.Trash-[UID], use it as the trash
    // can.
    if (mnt != "") {
    ...
    // Otherwise, if ${XDG_DATA_HOME} is defined, use "${XDG_DATA_HOME}/Trash"
    // as the trash can.
    if (trash_path == "") {
    ...
    // Otherwise, if ${HOME} is defined, use "${HOME}/.local/share/Trash" as the
    // trash can.
    if (trash_path == "") {
    ...
    // Issue an error if none of the previous locations is appropriate for the
    // trash can.
    ERR_FAIL_COND_V_MSG(
        trash_path == "",
        FAILED,
        "Could not determine the trash can location"
    );
    ...
    // Create needed directories for decided trash can location.
    {
        ...
        ERR_FAIL_COND_V_MSG(
            err != OK,
            err,
            "Could not create the trash path \"" + trash_path + "\""
        );
        err = dir_access->make_dir_recursive(trash_path + "/files");
        ERR_FAIL_COND_V_MSG(
            err != OK,
            err,
            "Could not create the trash path \"" + trash_path + "/files\""
        );
        err = dir_access->make_dir_recursive(trash_path + "/info");
        ERR_FAIL_COND_V_MSG(
            err != OK,
            err,
            "Could not create the trash path \"" + trash_path + "/info\""
        );
    }

    // The trash can is successfully created, now we check that we don't exceed
    // our file name length limit. If the file name is too long trim it so we
    // can add the identifying number and ".trashinfo". Assumes that the file
    // name length limit is 255 characters.
    ...
    while (stat(dest_path.utf8().get_data(), &buff) == 0) {
        ...
        // Added a limit to check for identically named files already on the
        // trash can if there are too many it could make the editor
        // unresponsive.
        ERR_FAIL_COND_V_MSG(
            id_number > 99,
            FAILED,
            "Too many identically named resources already in the trash can."
        );
    ...
    timestamp = vformat(
        ...
        timestamp,
        time.sec
    ); // vformat only supports up to 6 arguments.
    String trash_info = "[Trash Info]\nPath=" + path.http_escape()
                      + "\nDeletionDate=" + timestamp + "\n";
    {
        Error err;
        FileAccessRef file = FileAccess::open(
            trash_path + "/info/" + file_name + ".trashinfo",
            FileAccess::WRITE,
            &err
        );
        ERR_FAIL_COND_V_MSG(
            err != OK,
            err,
            "Can't create trashinfo file: \"" + trash_path + "/info/"
                + file_name + ".trashinfo\""
        );
        ...
        ERR_FAIL_COND_V_MSG(
            err != OK,
            err,
            "Can't rename file \"" + path + "\" to \"" + renamed_path + "\""
        );
    }

    // Move the given resource to the trash can.
    // Do not use DirAccess:rename() because it can't move files across multiple
    // mountpoints.
    List<String> mv_args;
    ...
    mv_args.push_back(trash_path + "/files");
    {
        ...
        // Issue an error if "mv" failed to move the given resource to the trash
        // can.
        if (err != OK || retval != 0) {
            ERR_PRINT(
                "move_to_trash: Could not move the resource \"" + path
            ...
            ERR_FAIL_COND_V_MSG(
                err != OK,
                err,
                "Could not rename \"" + renamed_path
                    + "\" back to its original name: \"" + path + "\""
            );
            return FAILED;
        }
    }
...
OS::LatinKeyboardVariant LinuxOS::get_latin_keyboard_variant() const {
...
int LinuxOS::keyboard_get_layout_count() const {
    int _group_count = 0;
    ...
    if (kbd) {
        ...
        if (kbd->ctrls != nullptr) {
            ...
            while (_group_count < XkbNumKbdGroups
                   && groups[_group_count] != None) {
                _group_count++;
            }
        }
...
int LinuxOS::keyboard_get_current_layout() const {
    XkbStateRec state;
    XkbGetState(x11_display, XkbUseCoreKbd, &state);
    return state.group;
}

void LinuxOS::keyboard_set_current_layout(int p_index) {
    ERR_FAIL_INDEX(p_index, keyboard_get_layout_count());
    XkbLockGroup(x11_display, XkbUseCoreKbd, p_index);
}

String LinuxOS::keyboard_get_layout_language(int p_index) const {
    String ret;
    XkbDescRec* kbd = XkbAllocKeyboard();
    if (kbd) {
        kbd->dpy = x11_display;
        ...
        if (names != None) {
            ...
            if (p_index >= 0 && p_index < _group_count) {
                if (p_index + 1 < info.size()) {
                    ret = info[p_index + 1]; // Skip "pc" at the start and
                                             // "inet"/"group" at the end of
                                             // symbols.
                } else {
                ...
                ERR_PRINT(
                    "Index " + itos(p_index) + "is out of bounds ("
                    + itos(_group_count) + ")."
                );
            }
            XFree(name);
...
String LinuxOS::keyboard_get_layout_name(int p_index) const {
    String ret;
    XkbDescRec* kbd = XkbAllocKeyboard();
    if (kbd) {
        kbd->dpy = x11_display;
...
void LinuxOS::update_real_mouse_position() {
    ...
    if (xquerypointer_result) {
        if (win_x > 0 && win_y > 0 && win_x <= current_videomode.width
            && win_y <= current_videomode.height) {
