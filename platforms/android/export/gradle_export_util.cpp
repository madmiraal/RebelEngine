...
Error create_directory(const String& p_dir) {
    if (!DirAccess::exists(p_dir)) {
        DirAccess* filesystem_da =
            DirAccess::create(DirAccess::ACCESS_RESOURCES);
        ERR_FAIL_COND_V_MSG(
            !filesystem_da,
            ERR_CANT_CREATE,
            "Cannot create directory '" + p_dir + "'."
        );
...
Error rename_and_store_file_in_gradle_project(
    ...
    String dst_path =
        p_path.replace_first("res://", export_data->assets_directory + "/");
...
String _android_xml_escape(const String& p_string) {
    // Android XML requires strings to be both valid XML (`xml_escape()`) but
    // also to escape characters which are valid XML but have special meaning in
    // Android XML.
...
Error _create_project_name_strings_files(
    ...
    print_verbose(
        "Creating strings resources for supported locales for project "
        + project_name
    );
    ...
    while (true) {
        ...
        if (!file.begins_with("values-")) {
            // NOTE: This assumes all directories that start with "values-" are
            // for localization.
            continue;
        }
        ...
        if (Global::ProjectSettings().has_setting(property_name)) {
            ...
            print_verbose(
                "Storing project name for locale " + locale + " under "
                + locale_directory
            );
            store_string_at_path(locale_directory, processed_xml_string);
        } else {
            // TODO: Once the legacy build system is deprecated we don't need to
            // have xml files for this else branch
            store_string_at_path(
                locale_directory,
                processed_default_xml_string
            );
...
String _get_xr_features_tag(const Ref<EditorExportPreset>& p_preset) {
    ...
    bool uses_xr =
        xr_mode_index == XR_MODE_OVR || xr_mode_index == XR_MODE_OPENXR;
    if (uses_xr) {
        manifest_xr_features +=
            "    <uses-feature tools:node=\"replace\" "
...
String _get_activity_tag(const Ref<EditorExportPreset>& p_preset) {
    ...
    String manifest_activity_text = vformat(
        ...
        "tools:replace=\"android:screenOrientation,android:"
        "excludeFromRecents\" "
...
String _get_application_tag(
    ...
    String manifest_application_text = vformat(
        ...
        "        "
        "tools:replace=\"android:allowBackup,android:isGame,android:"
        "hasFragileUserData,android:requestLegacyExternalStorage\"\n"
    ...
    if (uses_xr) {
        ...
        bool hand_tracking_enabled =
            (int)(p_preset->get("xr_features/hand_tracking"))
            > XR_HAND_TRACKING_NONE;
        if (hand_tracking_enabled) {
            int hand_tracking_frequency_index =
                p_preset->get("xr_features/hand_tracking_frequency");
            String hand_tracking_frequency =
                hand_tracking_frequency_index == XR_HAND_TRACKING_FREQUENCY_LOW
                    ? "LOW"
                    : "HIGH";
