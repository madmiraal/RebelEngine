...
static const char* uwp_device_capabilities[] =
    {"bluetooth", "location", "microphone", "proximity", "webcam", nullptr};
...
class AppxPackager {
    ...
    void store_central_dir_header(
        const FileMeta& p_file,
        bool p_do_hash = true
    );
...
String AppxPackager::hash_block(
    const uint8_t* p_block_data,
    size_t p_block_len
) {
...
void AppxPackager::make_block_map(const String& p_path) {
    ...
    for (int i = 0; i < file_metadata.size(); i++) {
        ...
        tmp_file->store_string(
            "<File Name=\"" + file.name.replace("/", "\\") + "\" Size=\""
            + itos(file.uncompressed_size) + "\" LfhSize=\""
            + itos(file.lfh_size) + "\">"
        );

        for (int j = 0; j < file.hashes.size(); j++) {
            tmp_file->store_string(
                "<Block Hash=\"" + file.hashes[j].base64_hash + "\" "
            );
            if (file.compressed) {
                tmp_file->store_string(
                    "Size=\"" + itos(file.hashes[j].compressed_size) + "\" "
                );
            }
            tmp_file->store_string("/>");
        }
...
Vector<uint8_t> AppxPackager::make_file_header(FileMeta p_file_meta) {
    ...
    // Compression
    offs += buf_put_int16(
        p_file_meta.compressed ? Z_DEFLATED : 0,
        &buf.write[offs]
    );
...
void AppxPackager::store_central_dir_header(
    const FileMeta& p_file,
    bool p_do_hash
) {
...
Vector<uint8_t> AppxPackager::make_end_of_central_record() {
    Vector<uint8_t> buf;
    buf.resize(
        ZIP64_END_OF_CENTRAL_DIR_SIZE + 12 + END_OF_CENTRAL_DIR_SIZE
    ); // Size plus magic
...
Error AppxPackager::add_file(
    ...
    if (p_file_no >= 1 && p_total_files >= 1) {
        if (EditorNode::progress_task_step(
                progress_task,
                "File: " + p_file_name,
                (p_file_no * 100) / p_total_files
            )) {
            return ERR_SKIP;
        }
    }
    ...
    while (p_len - step > 0) {
        size_t block_size =
            (p_len - step) > BLOCK_SIZE ? (size_t)BLOCK_SIZE : (p_len - step);
    ...
    if (p_compress) {
        ...
        int start = file_buffer.size();
        file_buffer.resize(
            file_buffer.size() + (strm.total_out - total_out_before)
        );
...
void AppxPackager::finish() {
    ...
    const String& tmp_blockmap_file_path =
        EditorSettings::get_singleton()->get_cache_dir().plus_file(
            "tmpblockmap.xml"
        );
    make_block_map(tmp_blockmap_file_path);

    FileAccess* blockmap_file =
        FileAccess::open(tmp_blockmap_file_path, FileAccess::READ);
    ...
    const String& tmp_content_types_file_path =
        EditorSettings::get_singleton()->get_cache_dir().plus_file(
            "tmpcontenttypes.xml"
        );
    ...
    FileAccess* types_file =
        FileAccess::open(tmp_content_types_file_path, FileAccess::READ);
...
class EditorExportPlatformUWP : public EditorExportPlatform {
    ...
    bool _valid_image(const StreamTexture* p_image, int p_width, int p_height)
        const {
        if (!p_image) {
            return false;
        }

        // TODO: Add resource creation or image rescaling to enable other
        // scales: 1.25, 1.5, 2.0
        return p_width == p_image->get_width()
            && p_height == p_image->get_height();
    }

    Vector<uint8_t> _fix_manifest(
        const Ref<EditorExportPreset>& p_preset,
        const Vector<uint8_t>& p_template,
        ...
        result = result.replace(
            "$identity_name$",
            p_preset->get("package/unique_name")
        );
        result =
            result.replace("$publisher$", p_preset->get("package/publisher"));

        result = result.replace(
            "$product_guid$",
            p_preset->get("identity/product_guid")
        );
        result = result.replace(
            "$publisher_guid$",
            p_preset->get("identity/publisher_guid")
        );

        String version = itos(p_preset->get("version/major")) + "."
                       + itos(p_preset->get("version/minor")) + "."
                       + itos(p_preset->get("version/build")) + "."
                       + itos(p_preset->get("version/revision"));
        result = result.replace("$version_string$", version);

        Platform arch = (Platform)(int)p_preset->get("architecture/target");
        String architecture =
            arch == ARM ? "arm" : (arch == X86 ? "x86" : "x64");
        ...
        result = result.replace(
            ...
            p_preset->get("package/publisher_display_name")
        );
        result = result.replace(
            "$app_description$",
            p_preset->get("package/description")
        );
        result = result.replace(
            "$bg_color$",
            p_preset->get("images/background_color")
        );
        result =
            result.replace("$short_name$", p_preset->get("package/short_name"));
        ...
        while (*uap) {
            if ((bool)p_preset->get("capabilities/" + String(*uap))) {
                capabilities_elements +=
                    "    <uap:Capability Name=\"" + String(*uap) + "\" />\n";
            }
            uap++;
        }
        ...
        while (*device) {
            if ((bool)p_preset->get("capabilities/" + String(*device))) {
                capabilities_elements += "    <DeviceCapability Name=\""
                                       + String(*device) + "\" />\n";
            }
            device++;
        }

        if (!((bool)p_preset->get("capabilities/internetClient"))
            && p_give_internet) {
            capabilities_elements +=
    ...
    Vector<uint8_t> _get_image_data(
        const Ref<EditorExportPreset>& p_preset,
        const String& p_path
    ) {
        Vector<uint8_t> data;
    ...
    static Error save_appx_file(
        ...
        return packager->add_file(
            ...
            p_file,
            p_total,
            _should_compress_asset(p_path, p_data)
        );
    }
...
public:
    ...
    virtual void get_export_options(List<ExportOption>* r_options) {
        ...
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::STRING, "package/description"),
            ""
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(
        ...
        r_options->push_back(
            ExportOption(PropertyInfo(Variant::STRING, "signing/password"), "")
        );
        r_options->push_back(ExportOption(
            PropertyInfo(
                Variant::INT,
        ...
        r_options->push_back(
            ExportOption(PropertyInfo(Variant::INT, "version/major"), 1)
        );
        r_options->push_back(
            ExportOption(PropertyInfo(Variant::INT, "version/minor"), 0)
        );
        r_options->push_back(
            ExportOption(PropertyInfo(Variant::INT, "version/build"), 0)
        );
        r_options->push_back(
            ExportOption(PropertyInfo(Variant::INT, "version/revision"), 0)
        );

        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "orientation/landscape"),
            true
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "orientation/portrait"),
            true
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "orientation/landscape_flipped"),
            true
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "orientation/portrait_flipped"),
            true
        ));

        r_options->push_back(ExportOption(
            PropertyInfo(Variant::STRING, "images/background_color"),
            "transparent"
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(
        ...
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "tiles/show_name_on_square150x150"),
            false
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "tiles/show_name_on_wide310x150"),
            false
        ));
        r_options->push_back(ExportOption(
            PropertyInfo(Variant::BOOL, "tiles/show_name_on_square310x310"),
            false
        ));
        ...
        while (*basic) {
            r_options->push_back(ExportOption(
                PropertyInfo(Variant::BOOL, "capabilities/" + String(*basic)),
                false
            ));
            basic++;
        }
        ...
        while (*uap) {
            r_options->push_back(ExportOption(
                PropertyInfo(Variant::BOOL, "capabilities/" + String(*uap)),
                false
            ));
            uap++;
        }
        ...
        while (*device) {
            r_options->push_back(ExportOption(
                PropertyInfo(Variant::BOOL, "capabilities/" + String(*device)),
                false
            ));
            device++;
        }
    ...
    virtual bool can_export(
        ...
        bool dvalid = exists_export_template(
            "uwp_" + platform_infix + "_debug.zip",
            &err
        );
        bool rvalid = exists_export_template(
            "uwp_" + platform_infix + "_release.zip",
            &err
        );
        ...
        if (!p_preset->get("images/square44x44_logo").is_zero()
            && !_valid_image(
                (Object::cast_to<StreamTexture>(
                    (Object*)p_preset->get("images/square44x44_logo")
                )),
            ...
            valid = false;
            err += TTR("Invalid square 44x44 logo image dimensions (should be "
                       "44x44).")
                 + "\n";
        }

        if (!p_preset->get("images/square71x71_logo").is_zero()
            && !_valid_image(
                (Object::cast_to<StreamTexture>(
                    (Object*)p_preset->get("images/square71x71_logo")
                )),
            ...
            valid = false;
            err += TTR("Invalid square 71x71 logo image dimensions (should be "
                       "71x71).")
                 + "\n";
        }

        if (!p_preset->get("images/square150x150_logo").is_zero()
            && !_valid_image(
                (Object::cast_to<StreamTexture>(
                    (Object*)p_preset->get("images/square150x150_logo")
                )),
        ...
        if (!p_preset->get("images/square310x310_logo").is_zero()
            && !_valid_image(
                (Object::cast_to<StreamTexture>(
                    (Object*)p_preset->get("images/square310x310_logo")
                )),
        ...
        if (!p_preset->get("images/wide310x150_logo").is_zero()
            && !_valid_image(
                (Object::cast_to<StreamTexture>(
                    (Object*)p_preset->get("images/wide310x150_logo")
                )),
            ...
            valid = false;
            err += TTR("Invalid wide 310x150 logo image dimensions (should be "
                       "310x150).")
                 + "\n";
        }

        if (!p_preset->get("images/splash_screen").is_zero()
            && !_valid_image(
                (Object::cast_to<StreamTexture>(
                    (Object*)p_preset->get("images/splash_screen")
                )),
    ...
    virtual Error export_project(
        ...
        if (src_appx == "") {
            ...
            if (p_debug) {
                src_appx =
                    find_export_template("uwp" + infix + "debug.zip", &err);
            } else {
                src_appx =
                    find_export_template("uwp" + infix + "release.zip", &err);
            }
            if (src_appx == "") {
        ...
        if (!pkg) {
            EditorNode::add_io_error(
                "Could not find template appx to export:\n" + src_appx
            );
            return ERR_FILE_NOT_FOUND;
        }
        ...
        while (ret == UNZ_OK) {
            ...
            err = packager.add_file(
                path,
                data.ptr(),
                data.size(),
                template_file_no++,
                template_files_amount,
                _should_compress_asset(path, data)
            );
            if (err != OK) {
                return err;
            }
        ...
        Vector<String> cl = ((String)p_preset->get("command_line/extra_args"))
                                .strip_edges()
                                .split(" ");
        ...
        err = packager
                  .add_file("__cl__.cl", clf.ptr(), clf.size(), -1, -1, false);
        if (err != OK) {
            return err;
        }
        ...
        String signtool_path =
            EditorSettings::get_singleton()->get("export/uwp/signtool");
        ...
        if (!FileAccess::exists(signtool_path)) {
            ERR_PRINT(
                "Could not find signtool executable at " + signtool_path
                + ", aborting."
            );
            return ERR_FILE_NOT_FOUND;
        }
        ...
        String cert_path =
            EditorSettings::get_singleton()->get("export/uwp/debug_certificate"
            );
        String cert_pass =
            EditorSettings::get_singleton()->get("export/uwp/debug_password");
        int cert_alg =
            EditorSettings::get_singleton()->get("export/uwp/debug_algorithm");
        ...
        if (!FileAccess::exists(cert_path)) {
            ERR_PRINT(
                "Could not find certificate file at " + cert_path
                + ", aborting."
            );
            return ERR_FILE_NOT_FOUND;
        }
        ...
        if (cert_alg < 0 || cert_alg > 2) {
            ERR_PRINT(
                "Invalid certificate algorithm " + itos(cert_alg)
                + ", aborting."
            );
    ...
    static Error copy_shared_objects(
        void* p_userdata,
        const SharedObject& p_so
    ) {
        return save_appx_file(
            p_userdata,
            p_so.path,
            FileAccess::get_file_as_array(p_so.path),
            0,
            1
        );
    }
