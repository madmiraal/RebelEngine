...
#include "platforms/windows/logo.gen.h"
...
Error EditorExportPlatformWindows::sign_shared_object(
    const Ref<EditorExportPreset>& p_preset,
    bool p_debug,
    const String& p_path
) {
...
Error EditorExportPlatformWindows::export_project(
    const Ref<EditorExportPreset>& p_preset,
    bool p_debug,
    const String& p_path,
    int p_flags
...
void EditorExportPlatformWindows::get_export_options(
    List<ExportOption>* r_options
) {
    EditorExportPlatformPC::get_export_options(r_options);

    r_options->push_back(
        ExportOption(PropertyInfo(Variant::BOOL, "codesign/enable"), false)
    );
    ...
    r_options->push_back(ExportOption(
        PropertyInfo(
            ...
            PROPERTY_HINT_ENUM,
            "Select automatically,Use PKCS12 file (specify *.PFX/*.P12 "
            "file),Use certificate store (specify SHA1 hash)"
        ),
        0
    ));
#endif
    ...
    r_options->push_back(
        ExportOption(PropertyInfo(Variant::STRING, "codesign/password"), "")
    );
    r_options->push_back(
        ExportOption(PropertyInfo(Variant::BOOL, "codesign/timestamp"), true)
    );
    r_options->push_back(ExportOption(
        PropertyInfo(Variant::STRING, "codesign/timestamp_server_url"),
        ""
    ));
    r_options->push_back(ExportOption(
        PropertyInfo(
    ...
    r_options->push_back(
        ExportOption(PropertyInfo(Variant::STRING, "codesign/description"), "")
    );
    r_options->push_back(ExportOption(
    ...
    r_options->push_back(ExportOption(
        PropertyInfo(Variant::STRING, "application/file_description"),
        ""
    ));
    r_options->push_back(
    ...
    r_options->push_back(ExportOption(
        PropertyInfo(Variant::STRING, "application/trademarks"),
        ""
    ));
}
...
void EditorExportPlatformWindows::_rcedit_add_data(
    ...
    String rcedit_path =
        EditorSettings::get_singleton()->get("export/windows/rcedit");
    ...
    if (!FileAccess::exists(rcedit_path)) {
        ERR_PRINT(
            "Could not find rcedit executable at " + rcedit_path
            + ", no icon or app information data will be included."
        );
        return;
    }
    ...
    String wine_path =
        EditorSettings::get_singleton()->get("export/windows/wine");

    if (wine_path != String() && !FileAccess::exists(wine_path)) {
        ERR_PRINT(
            "Could not find wine executable at " + wine_path
            + ", no icon or app information data will be included."
        );
        return;
    }
...
Error EditorExportPlatformWindows::_code_sign(
    const Ref<EditorExportPreset>& p_preset,
    const String& p_path
) {
    List<String> args;
    ...
    String signtool_path =
        EditorSettings::get_singleton()->get("export/windows/signtool");
...
#else
    String signtool_path =
        EditorSettings::get_singleton()->get("export/windows/osslsigncode");
    if (signtool_path != String() && !FileAccess::exists(signtool_path)) {
        ERR_PRINT(
            "Could not find osslsigncode executable at " + signtool_path
            + ", aborting."
        );
        return ERR_FILE_NOT_FOUND;
    }
    ...
    Error err =
        OS::get_singleton()
            ->execute(signtool_path, args, true, nullptr, &str, nullptr, true);
...
static Error fixup_embedded_pck(
    ...
    // Patch the header of the "pck" section in the PE file so that it
    ...
    for (int i = 0; i < num_sections; ++i) {
        ...
        if (strcmp((char*)section_name, "pck") == 0) {
            ...
            // Set virtual size to a little to avoid it taking memory (zero
            // would give issues)
