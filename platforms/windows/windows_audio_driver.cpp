...
#include <initguid.h>
// <initguid.h> must be included before <functiondiscoverykeys_devpkey.h>
...
Error WindowsAudioDriver::audio_device_init(
    ...
    if (reinit) {
        // In case we're trying to re-initialize the device prevent throwing
        // this error on the console, otherwise if there is currently no device
        // available this will spam the console.
        if (hr != S_OK) {
    ...
    hr                    = p_device->audio_client
             ->IsFormatSupported(AUDCLNT_SHAREMODE_SHARED, pwfex, &closest);
    if (hr == S_FALSE) {
        WARN_PRINT("WASAPI: Mix format is not supported by the Device");
        if (closest) {
            print_verbose(
                "WASAPI: closest->wFormatTag = " + itos(closest->wFormatTag)
            );
            print_verbose(
                "WASAPI: closest->nChannels = " + itos(closest->nChannels)
            );
            print_verbose(
                "WASAPI: closest->nSamplesPerSec = "
                + itos(closest->nSamplesPerSec)
            );
            print_verbose(
                "WASAPI: closest->nAvgBytesPerSec = "
                + itos(closest->nAvgBytesPerSec)
            );
            print_verbose(
                "WASAPI: closest->nBlockAlign = " + itos(closest->nBlockAlign)
            );
            print_verbose(
                "WASAPI: closest->wBitsPerSample = "
                + itos(closest->wBitsPerSample)
            );
    ...
    // Since we're using WASAPI Shared Mode we can't control any of these, we
    // just tag along
    ...
    if (p_device->format_tag == WAVE_FORMAT_EXTENSIBLE) {
        ...
        if (p_device->format_tag != WAVE_FORMAT_PCM
            && p_device->format_tag != WAVE_FORMAT_IEEE_FLOAT) {
            ERR_PRINT("WASAPI: Format not supported");
            ERR_FAIL_V(ERR_CANT_OPEN);
        }
    ...
    if ((DWORD)mix_rate != pwfex->nSamplesPerSec) {
        ...
        pwfex->nAvgBytesPerSec  = pwfex->nSamplesPerSec * pwfex->nChannels
                               * (pwfex->wBitsPerSample / 8);
    }

    hr = p_device->audio_client->Initialize(
        AUDCLNT_SHAREMODE_SHARED,
        streamflags,
        p_capture ? REFTIMES_PER_SEC : 0,
        0,
        pwfex,
        NULL
    );
    ERR_FAIL_COND_V_MSG(
        hr != S_OK,
        ERR_CANT_OPEN,
        "WASAPI: Initialize failed with error 0x" + String::num_uint64(hr, 16)
            + "."
    );

    if (p_capture) {
        hr = p_device->audio_client->GetService(
            IID_IAudioCaptureClient,
            (void**)&p_device->capture_client
        );
    } else {
        hr = p_device->audio_client->GetService(
            IID_IAudioRenderClient,
            (void**)&p_device->render_client
        );
    }
    ERR_FAIL_COND_V(hr != S_OK, ERR_CANT_OPEN);
...
Error WindowsAudioDriver::init_render_device(bool reinit) {
    ...
    switch (audio_output.channels) {
        ...
        default:
            WARN_PRINT(
                "WASAPI: Unsupported number of channels: "
                + itos(audio_output.channels)
            );
            channels = 2;
            break;
    }
    ...
    // Sample rate is independent of channels (ref:
    ...
    print_verbose(
        "WASAPI: audio buffer frames: " + itos(buffer_frames)
        + " calculated latency: " + itos(buffer_frames * 1000 / mix_rate) + "ms"
    );
...
void WindowsAudioDriver::thread_func(void* p_udata) {
    ...
    while (!ad->exit_thread) {
        ...
        if (avail_frames > 0 && ad->audio_output.audio_client) {
            ...
            HRESULT hr =
                ad->audio_output.audio_client->GetCurrentPadding(&cur_frames);
            if (hr == S_OK) {
                // Check how much frames are available on the WASAPI buffer
                UINT32 write_frames =
                    MIN(ad->buffer_frames - cur_frames, avail_frames);
                if (write_frames > 0) {
                    BYTE* buffer = NULL;
                    hr           = ad->audio_output.render_client->GetBuffer(
                        write_frames,
                        &buffer
                    );
                    if (hr == S_OK) {
                        // We're using WASAPI Shared Mode so we must convert the
                        // buffer
                        if (ad->channels == ad->audio_output.channels) {
                            for (unsigned int i = 0;
                                 i < write_frames * ad->channels;
                                 i++) {
                                ad->write_sample(
                                    ad->audio_output.format_tag,
                                    ad->audio_output.bits_per_sample,
                                    buffer,
                                    i,
                                    ad->samples_in.write[write_ofs++]
                                );
                            }
                        } else {
                            for (unsigned int i = 0; i < write_frames; i++) {
                                for (unsigned int j = 0;
                                    ...
                                    ad->write_sample(
                                        ...
                                        ad->audio_output.bits_per_sample,
                                        buffer,
                                        i * ad->audio_output.channels + j,
                                        ad->samples_in.write[write_ofs++]
                                    );
                                }
                                if (ad->audio_output.channels > ad->channels) {
                                    for (unsigned int j = ad->channels;
                                         j < ad->audio_output.channels;
                                         j++) {
                                        ad->write_sample(
                                            ad->audio_output.format_tag,
                                            ad->audio_output.bits_per_sample,
                        ...
                        hr = ad->audio_output.render_client->ReleaseBuffer(
                            write_frames,
                            0
                        );
                        if (hr != S_OK) {
                        ...
                        if (err != OK) {
                            ...
                            // We reopened the device and samples_in may have
                            // resized, so invalidate the current avail_frames
                            avail_frames = 0;
                        }
                    } else {
        ...
        // If we're using the Default device and it changed finish it so we'll
        // re-init the device
        if (ad->audio_output.device_name == "Default"
        ...
        if (ad->audio_input.active) {
            ...
            if (hr == S_OK) {
                while (packet_length != 0) {
                    hr = ad->audio_input.capture_client->GetBuffer(
                        &data,
                    ...
                    for (UINT32 j = 0; j < num_frames_available; j++) {
                        ...
                        if (flags & AUDCLNT_BUFFERFLAGS_SILENT) {
                            ...
                            if (ad->audio_input.channels == 2) {
                                l = read_sample(
                                    ad->audio_input.format_tag,
                                    ad->audio_input.bits_per_sample,
                                    data,
                                    j * 2
                                );
                                r = read_sample(
                                ...
                                l = r = 0;
                                ERR_PRINT(
                                    "WASAPI: unsupported channel count in "
                                    "microphone!"
                                );
                            }
                        }
                    ...
                    hr = ad->audio_input.capture_client->ReleaseBuffer(
                        num_frames_available
                    );
                    ERR_BREAK(hr != S_OK);

                    hr = ad->audio_input.capture_client->GetNextPacketSize(
                        &packet_length
                    );
                    ERR_BREAK(hr != S_OK);
                }
            ...
            // If we're using the Default device and it changed finish it so
            // we'll re-init the device
            if (ad->audio_input.device_name == "Default"
            ...
            // User selected a new device, finish the current one so we'll init
            // the new device
