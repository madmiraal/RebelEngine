...
def save_active_platforms(apnames, ap):
    for x in ap:
        ...
        for name in names:
            ...
            str += " static const unsigned char _" + x[10:] + "_" + name + "[]={"
            while len(b) == 1:
                str += hex(ord(b))
                b = pngf.read(1)
                if len(b) == 1:
...
def generate_vs_project(env, num_jobs):
    ...
    # build_options, targets and variants muat all be the same length.
    build_options = [
        "dev=yes",
        "production=yes target=release_debug",
        "production=yes tools=no target=release",
        "dev=yes bits=32",
        "production=yes target=release_debug bits=32",
        "production=yes tools=no target=release bits=32",
    ]
    build_options = [
        "-j" + str(num_jobs) + " " + build_option for build_option in build_options
    ]
    targets = [
        "bin\\rebel.windows.tools.64.msvc.exe",
        "bin\\rebel.windows.opt.tools.64.msvc.exe",
        "bin\\rebel.windows.opt.64.msvc.exe",
        "bin\\rebel.windows.tools.32.msvc.exe",
        "bin\\rebel.windows.opt.tools.32.msvc.exe",
        "bin\\rebel.windows.opt.32.msvc.exe",
    ]
    variants = [
        "debug|x64",
        "release_debug|x64",
