...
void EditorExportPreset::set_export_path(const String& p_path) {
    export_path = p_path;
    /* NOTE(SonerSound): if there is a need to implement a PropertyHint that
...
void EditorExportPlatform::gen_debug_flags(
    Vector<String>& r_flags,
    int p_flags
) {
    String host =
    ...
    if (p_flags & DEBUG_FLAG_DUMB_CLIENT) {
        int port =
            EditorSettings::get_singleton()->get("filesystem/file_server/port");
        String passwd = EditorSettings::get_singleton()->get(
...
Error EditorExportPlatform::_save_pack_file(
    ...
    ERR_FAIL_COND_V_MSG(
        ...
        "Must select at least one file to export."
    );
    ...
    if (pd->ep->step(
            TTR("Storing File:") + " " + p_path,
            2 + p_file * 100 / p_total,
            false
        )) {
        return ERR_SKIP;
    }
...
void EditorExportPlatform::_export_find_dependencies(
    const String& p_path,
    Set<String>& p_paths
) {
        ...
        if (da->current_is_dir()) {
            ...
            for (int i = 0; i < p_filters.size(); ++i) {
                if (fullpath.matchn(p_filters[i])
                    || fullpath_no_prefix.matchn(p_filters[i])) {
                    if (!exclude) {
...
void EditorExportPlugin::add_file(
    const String& p_path,
    const Vector<uint8_t>& p_file,
    bool p_remap
) {
    ExtraFile ef;
    ef.data  = p_file;
...
void EditorExportPlugin::_export_file_script(
    const String& p_path,
    const String& p_type,
    const PoolVector<String>& p_features
) {
...
void EditorExportPlugin::_export_begin_script(
    ...
    if (get_script_instance()) {
        get_script_instance()
            ->call("_export_begin", p_features, p_debug, p_path, p_flags);
    }
}
...
void EditorExportPlugin::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("add_shared_object", "path", "tags"),
        &EditorExportPlugin::add_shared_object
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_project_static_lib", "path"),
        &EditorExportPlugin::add_ios_project_static_lib
    );
    ClassDB::bind_method(
        D_METHOD("add_file", "path", "file", "remap"),
        &EditorExportPlugin::add_file
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_framework", "path"),
        &EditorExportPlugin::add_ios_framework
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_embedded_framework", "path"),
        &EditorExportPlugin::add_ios_embedded_framework
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_plist_content", "plist_content"),
        &EditorExportPlugin::add_ios_plist_content
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_linker_flags", "flags"),
        &EditorExportPlugin::add_ios_linker_flags
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_bundle_file", "path"),
        &EditorExportPlugin::add_ios_bundle_file
    );
    ClassDB::bind_method(
        D_METHOD("add_ios_cpp_code", "code"),
    ...
    BIND_VMETHOD(MethodInfo(
        ...
        PropertyInfo(Variant::POOL_STRING_ARRAY, "features")
    ));
    BIND_VMETHOD(MethodInfo(
        "_export_begin",
        PropertyInfo(Variant::POOL_STRING_ARRAY, "features"),
...
EditorExportPlatform::FeatureContainers EditorExportPlatform::
    get_feature_containers(const Ref<EditorExportPreset>& p_preset) {
    ...
    if (p_preset->get_custom_features() != String()) {
        Vector<String> tmp_custom_list =
            p_preset->get_custom_features().split(",");
...
EditorExportPlatform::ExportNotifier::ExportNotifier(
    EditorExportPlatform& p_platform,
    const Ref<EditorExportPreset>& p_preset,
    bool p_debug,
    const String& p_path,
    int p_flags
    ...
    for (int i = 0; i < export_plugins.size(); i++) {
        if (export_plugins[i]->get_script_instance()) { // script based
            export_plugins.write[i]->_export_begin_script(
                features.features_pv,
                p_debug,
                p_path,
                p_flags
            );
        } else {
            export_plugins.write[i]
                ->_export_begin(features.features, p_debug, p_path, p_flags);
        }
    }
}
...
Error EditorExportPlatform::export_project_files(
    const Ref<EditorExportPreset>& p_preset,
    EditorExportSaveFunction p_func,
    void* p_udata,
    EditorExportSaveSharedObject p_so_func
) {
    // figure out paths of files that will be exported
    Set<String> paths;
    Vector<String> path_remaps;

    if (p_preset->get_export_filter()
        == EditorExportPreset::EXPORT_ALL_RESOURCES) {
        // find stuff
        _export_find_resources(
            EditorFileSystem::get_singleton()->get_filesystem(),
            paths
        );
    } else {
        bool scenes_only = p_preset->get_export_filter()
                        == EditorExportPreset::EXPORT_SELECTED_SCENES;

        Vector<String> files = p_preset->get_files_to_export();
        for (int i = 0; i < files.size(); i++) {
            if (scenes_only
                && ResourceLoader::get_resource_type(files[i])
                       != "PackedScene") {
                continue;
            }
    ...
    // Ignore import files, since these are automatically added to the jar later
    // with the resources
    ...
    for (int i = 0; i < export_plugins.size(); i++) {
        ...
        for (int j = 0; j < export_plugins[i]->extra_files.size(); j++) {
            err = p_func(
                p_udata,
                export_plugins[i]->extra_files[j].path,
                export_plugins[i]->extra_files[j].data,
                0,
                paths.size()
            );
            if (err != OK) {
    ...
    for (Set<String>::Element* E = paths.front(); E; E = E->next()) {
        ...
        if (FileAccess::exists(path + ".import")) {
            ...
            if (remap_features.size() > 1) {
                this->resolve_platform_feature_priorities(
                    p_preset,
                    remap_features
                );
            }
            ...
            for (List<String>::Element* F = remaps.front(); F; F = F->next()) {
                ...
                if (remap == "path") {
                    ...
                    Vector<uint8_t> array =
                        FileAccess::get_file_as_array(remapped_path);
            ...
            Vector<uint8_t> array =
                FileAccess::get_file_as_array(path + ".import");
            ...
            for (int i = 0; i < export_plugins.size(); i++) {
                if (export_plugins[i]->get_script_instance()) { // script based
                    export_plugins.write[i]
                        ->_export_file_script(path, type, features_pv);
                } else {
                    export_plugins.write[i]->_export_file(path, type, features);
                }
                if (p_so_func) {
                    for (int j = 0;
                         j < export_plugins[i]->shared_objects.size();
                         j++) {
                        err = p_so_func(
                            p_udata,
                            export_plugins[i]->shared_objects[j]
                        );
                        if (err != OK) {
                            return err;
                        }
                ...
                for (int j = 0; j < export_plugins[i]->extra_files.size();
                     j++) {
                    err = p_func(
                        p_udata,
                        export_plugins[i]->extra_files[j].path,
...
Error EditorExportPlatform::_add_shared_object(
    void* p_userdata,
...
Error EditorExportPlatform::save_pack(
    const Ref<EditorExportPreset>& p_preset,
    const String& p_path,
    Vector<SharedObject>* p_so_files,
    bool p_embed,
    int64_t* r_embedded_start,
    ...
    String tmppath =
        EditorSettings::get_singleton()->get_cache_dir().plus_file("packtmp");
    FileAccess* ftmp = FileAccess::open(tmppath, FileAccess::WRITE);
    ERR_FAIL_COND_V_MSG(
        !ftmp,
        ERR_CANT_CREATE,
        "Cannot create file '" + tmppath + "'."
    );
    ...
    for (int i = 0; i < pd.file_ofs.size(); i++) {
        ...
        f->store_buffer(
            (const uint8_t*)pd.file_ofs[i].path_utf8.get_data(),
            string_len
        );
    ...
    if (!ftmp) {
        ...
        ERR_FAIL_V_MSG(
            ERR_CANT_CREATE,
            "Can't open file to read from path '" + String(tmppath) + "'."
        );
    }
...
Error EditorExportPlatform::save_zip(
    const Ref<EditorExportPreset>& p_preset,
    const String& p_path
) {
...
void EditorExportPlatform::gen_export_flags(
    Vector<String>& r_flags,
    int p_flags
) {
    String host =
...
void EditorExport::_save() {
    ...
    for (int i = 0; i < export_presets.size(); i++) {
        ...
        config->set_value(
            section,
            "platform",
            preset->get_platform()->get_name()
        );
        config->set_value(section, "runnable", preset->is_runnable());
        config->set_value(
            section,
            "custom_features",
            preset->get_custom_features()
        );
        ...
        config->set_value(
            section,
            "include_filter",
            preset->get_include_filter()
        );
        config->set_value(
            section,
            "exclude_filter",
            preset->get_exclude_filter()
        );
        config->set_value(section, "export_path", preset->get_export_path());
        config->set_value(
            section,
            "script_export_mode",
            preset->get_script_export_mode()
        );
        config->set_value(
            section,
            "script_encryption_key",
            preset->get_script_encryption_key()
        );

        String option_section = "preset." + itos(i) + ".options";

        for (const List<PropertyInfo>::Element* E =
                 preset->get_properties().front();
             E;
             E = E->next()) {
            config->set_value(
                option_section,
                E->get().name,
                preset->get(E->get().name)
            );
        }
    }
...
void EditorExport::add_export_platform(
...
Ref<EditorExportPlatform> EditorExport::get_export_platform(int p_idx) {
    ERR_FAIL_INDEX_V(
        p_idx,
        export_platforms.size(),
        Ref<EditorExportPlatform>()
    );

    return export_platforms[p_idx];
}

void EditorExport::add_export_preset(
    const Ref<EditorExportPreset>& p_preset,
    int p_at_pos
) {
    if (p_at_pos < 0) {
...
String EditorExportPlatform::test_etc2() const {
    ...
    if (driver == "GLES2" && !etc_supported) {
        return TTR(
            "Target platform requires 'ETC' texture compression for GLES2. "
            "Enable 'Import Etc' in Project Settings."
        );
    } else if (driver == "GLES3") {
        String err;
        if (!etc2_supported) {
            err +=
                TTR("Target platform requires 'ETC2' texture compression for "
                    "GLES3. Enable 'Import Etc 2' in Project Settings.");
        }
        if (driver_fallback && !etc_supported) {
            ...
            err +=
                TTR("Target platform requires 'ETC' texture compression for "
                    "the driver fallback to GLES2.\nEnable 'Import Etc' in "
                    "Project Settings, or disable 'Driver Fallback Enabled'.");
        }
        return err;
    }
    return String();
...
String EditorExportPlatform::test_etc2_or_pvrtc() const {
    ...
    if (driver == "GLES2" && !pvrtc_supported) {
        return TTR(
            "Target platform requires 'PVRTC' texture compression for GLES2. "
            "Enable 'Import Pvrtc' in Project Settings."
        );
    } else if (driver == "GLES3") {
        String err;
        if (!etc2_supported && !pvrtc_supported) {
            err +=
                TTR("Target platform requires 'ETC2' or 'PVRTC' texture "
                    "compression for GLES3. Enable 'Import Etc 2' or 'Import "
                    "Pvrtc' in Project Settings.");
        }
        if (driver_fallback && !pvrtc_supported) {
            ...
            err +=
                TTR("Target platform requires 'PVRTC' texture compression for "
                    "the driver fallback to GLES2.\nEnable 'Import Pvrtc' in "
                    "Project Settings, or disable 'Driver Fallback Enabled'.");
        }
        return err;
    }
...
void EditorExport::load_config() {
    ...
    while (true) {
        ...
        if (config->has_section_key(section, "custom_features")) {
            preset->set_custom_features(
                config->get_value(section, "custom_features")
            );
        }
        ...
        if (export_filter == "all_resources") {
            ...
            preset->set_export_filter(
                EditorExportPreset::EXPORT_SELECTED_RESOURCES
            );
            get_files = true;
        }
        ...
        if (config->has_section_key(section, "script_export_mode")) {
            preset->set_script_export_mode(
                config->get_value(section, "script_export_mode")
            );
        }
        if (config->has_section_key(section, "script_encryption_key")) {
            preset->set_script_encryption_key(
                config->get_value(section, "script_encryption_key")
            );
        }
...
void EditorExport::update_export_presets() {
    ...
    for (int i = 0; i < export_presets.size(); i++) {
        ...
        if (platform_options.has(preset->get_platform()->get_name())) {
            ...
            List<EditorExportPlatform::ExportOption> options =
                platform_options[preset->get_platform()->get_name()];
            ...
            for (List<EditorExportPlatform::ExportOption>::Element* E =
                     options.front();
                 E;
                 E = E->next()) {
                preset->properties.push_back(E->get().option);
...
void EditorExportPlatformPC::get_export_options(List<ExportOption>* r_options) {
    ...
    r_options->push_back(
        ExportOption(PropertyInfo(Variant::BOOL, "binary_format/64_bits"), true)
    );
    r_options->push_back(ExportOption(
        PropertyInfo(Variant::BOOL, "binary_format/embed_pck"),
        false
    ));

    r_options->push_back(
        ExportOption(PropertyInfo(Variant::BOOL, "texture_format/bptc"), false)
    );
    r_options->push_back(
    ...
    r_options->push_back(ExportOption(
        PropertyInfo(Variant::BOOL, "texture_format/no_bptc_fallbacks"),
        true
    ));
}
...
bool EditorExportPlatformPC::can_export(
    const Ref<EditorExportPreset>& p_preset,
    String& r_error,
    ...
    bool dvalid =
        exists_export_template(use64 ? debug_file_64 : debug_file_32, &err);
    bool rvalid =
        exists_export_template(use64 ? release_file_64 : release_file_32, &err);
...
List<String> EditorExportPlatformPC::get_binary_extensions(
    const Ref<EditorExportPreset>& p_preset
) const {
    List<String> list;
...
Error EditorExportPlatformPC::export_project(
    const Ref<EditorExportPreset>& p_preset,
    bool p_debug,
    const String& p_path,
    int p_flags
    ...
    if (err == OK) {
        ...
        err = save_pack(
            ...
            pck_path,
            &so_files,
            p_preset->get("binary_format/embed_pck"),
            &embedded_pos,
            &embedded_size
        );
        if (err == OK && p_preset->get("binary_format/embed_pck")) {
            if (embedded_size >= 0x100000000
                && !p_preset->get("binary_format/64_bits")) {
                EditorNode::get_singleton()->show_warning(
                    TTR("On 32-bit exports the embedded PCK cannot be bigger "
                        "than 4 GiB.")
                );
        ...
        if (err == OK && !so_files.empty()) {
            ...
            for (int i = 0; i < so_files.size() && err == OK; i++) {
                err = da->copy(
                    so_files[i].path,
                    p_path.get_base_dir().plus_file(so_files[i].path.get_file())
                );
                if (err == OK) {
                    err = sign_shared_object(
                        p_preset,
                        p_debug,
                        p_path.get_base_dir().plus_file(
                            so_files[i].path.get_file()
                        )
                    );
                }
            }
...
void EditorExportPlatformPC::set_extension(
    const String& p_extension,
    const String& p_feature_key
) {
...
EditorExportPlatformPC::FixUpEmbeddedPckFunc EditorExportPlatformPC::
    get_fixup_embedded_pck_func() const {
    return fixup_embedded_pck_func;
}

void EditorExportPlatformPC::set_fixup_embedded_pck_func(
    FixUpEmbeddedPckFunc p_fixup_embedded_pck_func
) {
...
void EditorExportTextSceneToBinaryPlugin::_export_file(
    ...
    bool convert =
        GLOBAL_GET("editor/convert_text_resources_to_binary_on_export");
    if (!convert) {
        return;
    }
    String tmp_path =
        EditorSettings::get_singleton()->get_cache_dir().plus_file("tmpfile.res"
        );
    Error err =
        ResourceFormatLoaderText::convert_file_to_binary(p_path, tmp_path);
    if (err != OK) {
