...
void EditorHistory::_add_object(
    ObjectID p_object,
...
EditorPlugin* EditorData::get_editor(Object* p_object) {
    for (int i = 0; i < editor_plugins.size(); i++) {
        if (editor_plugins[i]->has_main_screen()
            && editor_plugins[i]->handles(p_object)) {
...
void EditorData::copy_object_params(Object* p_object) {
    ...
    for (List<PropertyInfo>::Element* E = pinfo.front(); E; E = E->next()) {
        if (!(E->get().usage & PROPERTY_USAGE_EDITOR)
            || E->get().name == "script" || E->get().name == "scripts") {
            continue;
        }
...
void EditorData::add_custom_type(
    const String& p_type,
...
Variant EditorData::instance_custom_type(
    const String& p_type,
    const String& p_inherits
) {
...
void EditorData::remove_custom_type(const String& p_type) {
    for (Map<String, Vector<CustomType>>::Element* E = custom_types.front(); E;
         E                                           = E->next()) {
...
void EditorData::remove_scene(int p_idx) {
    ...
    if (edited_scene[p_idx].root) {
        for (int i = 0; i < editor_plugins.size(); i++) {
            editor_plugins[i]->notify_scene_closed(
                edited_scene[p_idx].root->get_filename()
            );
        }
    ...
    if (edited_scene[p_idx].path != String()) {
        ScriptEditor::get_singleton()->close_builtin_scripts_from_scene(
            edited_scene[p_idx].path
        );
    }
...
bool EditorData::_find_updated_instances(
    Node* p_root,
    Node* p_node,
    Set<String>& checked_paths
) {
    /*
    if (p_root!=p_node && p_node->get_owner()!=p_root &&
    !p_root->is_editable_instance(p_node->get_owner())) return false;
    */
    ...
    for (int i = 0; i < p_node->get_child_count(); i++) {
        bool found = _find_updated_instances(
            p_root,
            p_node->get_child(i),
            checked_paths
        );
...
bool EditorData::check_and_update_scene(int p_idx) {
    ...
    bool must_reload = _find_updated_instances(
        edited_scene[p_idx].root,
        edited_scene[p_idx].root,
        checked_scenes
    );
    ...
    if (must_reload) {
        ...
        for (List<Node*>::Element* E =
                 edited_scene.write[p_idx].selection.front();
             E;
             E = E->next()) {
...
void EditorData::set_edited_scene_root(Node* p_root) {
    ...
    if (p_root) {
        if (p_root->get_filename() != "") {
            edited_scene.write[current_edited_scene].path =
                p_root->get_filename();
        } else {
    ...
    if (edited_scene[current_edited_scene].path != "") {
        edited_scene.write[current_edited_scene].file_modified_time =
            FileAccess::get_modified_time(
                edited_scene[current_edited_scene].path
            );
    }
}
...
String EditorData::get_scene_title(int p_idx, bool p_always_strip_extension)
    const {
    ERR_FAIL_INDEX_V(p_idx, edited_scene.size(), String());
    if (!edited_scene[p_idx].root) {
        return TTR("[empty]");
    ...
    // Return the filename including the extension if there's ambiguity (e.g.
    // both `foo.tscn` and `foo.scn` are being edited).
    for (int i = 0; i < edited_scene.size(); i++) {
        if (i == p_idx) {
...
void EditorData::save_edited_scene_state(
...
bool EditorData::script_class_is_parent(
...
Variant EditorData::script_class_instance(const String& p_class) {
    if (ScriptServer::is_global_class(p_class)) {
        Variant obj =
            ClassDB::instance(ScriptServer::get_global_class_native_base(p_class
            ));
        if (obj) {
            Ref<Script> script = script_class_load_script(p_class);
            if (script.is_valid()) {
...
void EditorData::script_class_set_icon_path(
    const String& p_class,
...
String EditorData::script_class_get_icon_path(const String& p_class) const {
    ...
    while (ret.empty()) {
        ...
        ret = _script_class_icon_paths.has(current)
                ? _script_class_icon_paths[current]
                : String();
    }
...
StringName EditorData::script_class_get_name(const String& p_path) const {
    return _script_class_file_to_path.has(p_path)
             ? _script_class_file_to_path[p_path]
             : StringName();
}

void EditorData::script_class_set_name(
    const String& p_path,
...
Array EditorSelection::_get_transformable_selected_nodes() {
    ...
    for (List<Node*>::Element* E = selected_node_list.front(); E;
         E                       = E->next()) {
        ret.push_back(E->get());
    }
...
void EditorSelection::_bind_methods() {
    ClassDB::bind_method(
        ...
        &EditorSelection::_node_removed
    );
    ClassDB::bind_method(D_METHOD("clear"), &EditorSelection::clear);
    ClassDB::bind_method(
        D_METHOD("add_node", "node"),
        &EditorSelection::add_node
    );
    ClassDB::bind_method(
        D_METHOD("remove_node", "node"),
        &EditorSelection::remove_node
    );
    ClassDB::bind_method(
        D_METHOD("get_selected_nodes"),
        &EditorSelection::get_selected_nodes
    );
    ClassDB::bind_method(
        D_METHOD("get_transformable_selected_nodes"),
        &EditorSelection::_get_transformable_selected_nodes
    );
    ClassDB::bind_method(
        D_METHOD("_emit_change"),
        &EditorSelection::_emit_change
    );
