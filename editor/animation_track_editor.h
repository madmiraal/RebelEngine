...
public:
    ...
    void draw_texture_region_clipped(
        const Ref<Texture>& p_texture,
        const Rect2& p_rect,
        const Rect2& p_region
    );
    void draw_rect_clipped(
        const Rect2& p_rect,
        const Color& p_color,
        bool p_filled = true
    );
...
class AnimationTrackEditor : public VBoxContainer {
    ...
    PropertyInfo _find_hint_for_track(
        int p_idx,
        NodePath& r_base_path,
        Variant* r_current_val = nullptr
    );
...
public:
    ...
    void insert_node_value_key(
        Node* p_node,
        const String& p_property,
        const Variant& p_value,
        bool p_only_if_exists = false
    );
    void insert_value_key(
        const String& p_property,
        const Variant& p_value,
        bool p_advance
    );
    void insert_transform_key(
        Spatial* p_node,
        const String& p_sub,
    ...
    /** If `p_from_mouse_event` is `true`, handle Shift key presses for precise
     * snapping. */
