...
public:
    ...
    Vector<Ref<Texture>> make_mesh_previews(
...
public:
    ...
    ToolButton* add_control_to_bottom_panel(
        Control* p_control,
    ...
    } // notify that it was raised by the user, not the editor

    virtual void edit(Object* p_object);
    virtual bool handles(Object* p_object) const;
    virtual Dictionary get_state(
    ) const; // save editor state so it can't be reloaded when reloading scene
    virtual void set_state(const Dictionary& p_state
    ); // restore editor state (likely was saved with the scene)
    virtual void clear(); // clear any temporary data in the editor, reset it
                          // (likely new scene or load another scene)
    ...
    virtual bool build(); // builds with external tools. Returns true if safe to
                          // continue running scene.
    ...
    void add_spatial_gizmo_plugin(
        const Ref<EditorSpatialGizmoPlugin>& p_gizmo_plugin
    );
    void remove_spatial_gizmo_plugin(
        const Ref<EditorSpatialGizmoPlugin>& p_gizmo_plugin
    );
