...
void AnimationTrackEditColor::draw_key_link(
    int p_index,
    float p_pixels_sec,
    int p_x,
    int p_next_x,
    ...
    color_samples.push_back(
        get_animation()->track_get_key_value(get_track(), p_index)
    );
    ...
    if (get_animation()->track_get_type(get_track()) == Animation::TYPE_VALUE) {
        if (get_animation()->track_get_interpolation_type(get_track())
            ...
            && !Math::is_zero_approx(
                get_animation()->track_get_key_transition(get_track(), p_index)
            )) {
            float start_time =
                get_animation()->track_get_key_time(get_track(), p_index);
            float end_time =
                get_animation()->track_get_key_time(get_track(), p_index + 1);

            Color color_next =
                get_animation()->value_track_interpolate(get_track(), end_time);

            if (!color_samples[0].is_equal_approx(color_next)) {
                color_samples.resize(
                    1 + (x_to - x_from) / 64
                ); // Make a color sample every 64 px.
                for (int i = 1; i < color_samples.size(); i++) {
                    float j = i;
                    color_samples.write[i] =
                        get_animation()->value_track_interpolate(
                            get_track(),
                            Math::lerp(
                                start_time,
                                end_time,
                                j / color_samples.size()
                            )
                        );
                }
            }
        ...
        color_samples.push_back(
            get_animation()->track_get_key_value(get_track(), p_index + 1)
        );
    }
    ...
    for (int i = 0; i < color_samples.size() - 1; i++) {
        ...
        points.push_back(Vector2(
            Math::lerp(x_from, x_to, float(i) / (color_samples.size() - 1)),
            y_from
        ));
        colors.push_back(color_samples[i]);

        points.push_back(Vector2(
            Math::lerp(x_from, x_to, float(i + 1) / (color_samples.size() - 1)),
            y_from
        ));
...
void AnimationTrackEditColor::draw_key(
    ...
    Rect2 rect(
        Vector2(p_x - fh / 2, int(get_size().height - fh) / 2),
        Size2(fh, fh)
    );
    ...
    if (play) {
        ...
        if (len == 0) {
            Ref<AudioStreamPreview> preview =
                AudioStreamPreviewGenerator::get_singleton()->generate_preview(
                    stream
                );
...
void AnimationTrackEditAudio::draw_key(
    ...
    if (play) {
        ...
        Rect2 rect =
            Rect2(from_x, (get_size().height - fh) / 2, to_x - from_x, fh);
        ...
        for (int i = from_x; i < to_x; i++) {
            ...
            int idx = i - from_x;
            lines.write[idx * 2 + 0] =
                Vector2(i, rect.position.y + min * rect.size.y);
            lines.write[idx * 2 + 1] =
                Vector2(i, rect.position.y + max * rect.size.y);
        }
        ...
        int fh         = font->get_height() * 0.8;
        Rect2 rect(
            Vector2(p_x, int(get_size().height - fh) / 2),
            Size2(fh, fh)
        );
...
void AnimationTrackEditAudio::_bind_methods() {
    ClassDB::bind_method(
        "_preview_changed",
        &AnimationTrackEditAudio::_preview_changed
    );
}
...
Rect2 AnimationTrackEditSpriteFrame::get_key_rect(
    int p_index,
    float p_pixels_sec
) {
    Object* object = ObjectDB::get_instance(id);
    ...
    if (Object::cast_to<Sprite>(object) || Object::cast_to<Sprite3D>(object)) {
        ...
        if (animations.size() == 1) {
            ...
            String animation_path =
                get_animation()->track_get_path(get_track());
            animation_path = animation_path.replace(":frame", ":animation");
            int animation_track = get_animation()->find_track(animation_path);
            float track_time =
                get_animation()->track_get_key_time(get_track(), p_index);
            int animaiton_index =
                get_animation()->track_find_key(animation_track, track_time);
            animation = get_animation()->track_get_key_value(
                animation_track,
                animaiton_index
            );
        }
...
void AnimationTrackEditSubAnim::draw_key(
    ...
    if (anim != "[stop]" && ap->has_animation(anim)) {
        ...
        if (lines.size() > 2) {
            VS::get_singleton()
                ->canvas_item_add_multiline(get_canvas_item(), lines, colorv);
        }
...
void AnimationTrackEditVolumeDB::draw_key_link(
    int p_index,
    float p_pixels_sec,
    int p_x,
    int p_next_x,
    ...
    if (from_x < p_clip_left) {
        h = Math::lerp(
            h,
            h_n,
            float(p_clip_left - from_x) / float(to_x - from_x)
        );
        from_x = p_clip_left;
    }
    ...
    if (to_x > p_clip_right) {
        h_n = Math::lerp(
            h,
            h_n,
            float(p_clip_right - from_x) / float(to_x - from_x)
        );
        to_x = p_clip_right;
    }
...
void AnimationTrackEditTypeAudio::_preview_changed(ObjectID p_which) {
    for (int i = 0; i < get_animation()->track_get_key_count(get_track());
         i++) {
        Ref<AudioStream> stream =
            get_animation()->audio_track_get_key_stream(get_track(), i);
        if (stream.is_valid() && stream->get_instance_id() == p_which) {
            update();
            return;
        }
...
Rect2 AnimationTrackEditTypeAudio::get_key_rect(
    int p_index,
    float p_pixels_sec
) {
    Ref<AudioStream> stream =
        get_animation()->audio_track_get_key_stream(get_track(), p_index);
    ...
    float start_ofs =
        get_animation()->audio_track_get_key_start_offset(get_track(), p_index);
    float end_ofs =
        get_animation()->audio_track_get_key_end_offset(get_track(), p_index);
...
void AnimationTrackEditTypeAudio::_bind_methods() {
    ClassDB::bind_method(
        "_preview_changed",
        &AnimationTrackEditTypeAudio::_preview_changed
    );
}
...
bool AnimationTrackEditTypeAudio::can_drop_data(
...
void AnimationTrackEditTypeAudio::_gui_input(const Ref<InputEvent>& p_event) {
    ...
    if (!len_resizing && mm.is_valid()) {
        ...
        for (int i = 0; i < get_animation()->track_get_key_count(get_track());
            ...
            float start_ofs = get_animation()->audio_track_get_key_start_offset(
                get_track(),
                i
            );
            float end_ofs =
                get_animation()->audio_track_get_key_end_offset(get_track(), i);
            float len = stream->get_length();

            if (len == 0) {
                Ref<AudioStreamPreview> preview =
                    AudioStreamPreviewGenerator::get_singleton()
                        ->generate_preview(stream);
                float preview_len = preview->get_length();
                len               = preview_len;
            }
            ...
            if (get_animation()->track_get_key_count(get_track()) > i + 1) {
                len = MIN(
                    len,
                    get_animation()->track_get_key_time(get_track(), i + 1)
                        - get_animation()->track_get_key_time(get_track(), i)
                );
            }
            ...
            if (end >= get_timeline()->get_name_limit()
                && end <= get_size().width - get_timeline()->get_buttons_width()
                && ABS(mm->get_position().x - end) < 5 * EDSCALE) {
        ...
        if (use_hsize_cursor) {
            set_default_cursor_type(CURSOR_HSIZE);
        } else {
    ...
    if (mb.is_valid() && mb->is_pressed()
        ...
        && get_default_cursor_type() == CURSOR_HSIZE) {
        len_resizing         = true;
    ...
    if (len_resizing && mb.is_valid() && !mb->is_pressed()
        && mb->get_button_index() == BUTTON_LEFT) {
        float ofs_local = -len_resizing_rel / get_timeline()->get_zoom_scale();
        if (len_resizing_start) {
            float prev_ofs = get_animation()->audio_track_get_key_start_offset(
                get_track(),
                len_resizing_index
            );
            get_undo_redo()->create_action(
                TTR("Change Audio Track Clip Start Offset")
            );
            get_undo_redo()->add_do_method(
                get_animation().ptr(),
            ...
            float prev_ofs = get_animation()->audio_track_get_key_end_offset(
                get_track(),
                len_resizing_index
            );
            get_undo_redo()->create_action(
                TTR("Change Audio Track Clip End Offset")
            );
            get_undo_redo()->add_do_method(
                get_animation().ptr(),
...
Rect2 AnimationTrackEditTypeAnimation::get_key_rect(
    int p_index,
    float p_pixels_sec
) {
    Object* object = ObjectDB::get_instance(id);
    ...
    String anim = get_animation()->animation_track_get_key_animation(
        get_track(),
        p_index
    );
...
/////////
AnimationTrackEdit* AnimationTrackEditDefaultPlugin::create_value_track_edit(
    Object* p_object,
    Variant::Type p_type,
    ...
    if (p_property == "playing"
        && (p_object->is_class("AudioStreamPlayer")
            || p_object->is_class("AudioStreamPlayer2D")
            || p_object->is_class("AudioStreamPlayer3D"))) {
    ...
    if (p_property == "frame"
        && (p_object->is_class("Sprite") || p_object->is_class("Sprite3D")
            || p_object->is_class("AnimatedSprite")
            || p_object->is_class("AnimatedSprite3D"))) {
        AnimationTrackEditSpriteFrame* sprite =
            memnew(AnimationTrackEditSpriteFrame);
        sprite->set_node(p_object);
        return sprite;
    }

    if (p_property == "frame_coords"
        && (p_object->is_class("Sprite") || p_object->is_class("Sprite3D"))) {
        AnimationTrackEditSpriteFrame* sprite =
            memnew(AnimationTrackEditSpriteFrame);
    ...
    if (p_property == "current_animation"
        && (p_object->is_class("AnimationPlayer"))) {
...
AnimationTrackEdit* AnimationTrackEditDefaultPlugin::
    create_animation_track_edit(Object* p_object) {
    AnimationTrackEditTypeAnimation* an =
        memnew(AnimationTrackEditTypeAnimation);
