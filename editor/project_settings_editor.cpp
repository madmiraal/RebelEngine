    ...
    if (k.is_valid() && is_window_modal_on_top() && k->is_pressed()) {
        if (k->get_scancode_with_modifiers() == (KEY_MASK_CMD | KEY_F)) {
            if (search_button->is_pressed()) {
                ...
                // This toggles the search bar display while giving the button
                // its "pressed" appearance
...
void ProjectSettingsEditor::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_ENTER_TREE: {
            globals_editor->edit(&(Global::ProjectSettings()));

            search_button->set_icon(get_icon("Search", "EditorIcons"));
            search_box->set_right_icon(get_icon("Search", "EditorIcons"));
            search_box->set_clear_button_enabled(true);

            action_add_error->add_color_override(
                "font_color",
                get_color("error_color", "Editor")
            );

            translation_list
                ->connect("button_pressed", this, "_translation_delete");
            _update_actions();

            // List Physical Key before Key to encourage its use.
            // Physical Key should be used for most game inputs as it allows
            // keys to work on non-QWERTY layouts out of the box. This is
            // especially important for WASD movement layouts.
            popup_add->add_icon_item(
                get_icon("KeyboardPhysical", "EditorIcons"),
                TTR("Physical Key"),
                INPUT_KEY_PHYSICAL
            );
            popup_add->add_icon_item(
                get_icon("Keyboard", "EditorIcons"),
                TTR("Key "),
                INPUT_KEY
            ); //"Key " - because the word 'key' has already been used as a key
               // animation
            popup_add->add_icon_item(
                get_icon("JoyButton", "EditorIcons"),
                TTR("Joy Button"),
                INPUT_JOY_BUTTON
            );
            popup_add->add_icon_item(
                get_icon("JoyAxis", "EditorIcons"),
                TTR("Joy Axis"),
                INPUT_JOY_MOTION
            );
            popup_add->add_icon_item(
                get_icon("Mouse", "EditorIcons"),
                TTR("Mouse Button"),
                INPUT_MOUSE_BUTTON
            );
        ...
        case EditorSettings::NOTIFICATION_EDITOR_SETTINGS_CHANGED: {
            ...
            popup_add->set_item_icon(
                popup_add->get_item_index(INPUT_KEY_PHYSICAL),
                get_icon("KeyboardPhysical", "EditorIcons")
            );
            popup_add->set_item_icon(
                popup_add->get_item_index(INPUT_KEY),
                get_icon("Keyboard", "EditorIcons")
            );
            popup_add->set_item_icon(
                popup_add->get_item_index(INPUT_JOY_BUTTON),
                get_icon("JoyButton", "EditorIcons")
            );
            popup_add->set_item_icon(
                popup_add->get_item_index(INPUT_JOY_MOTION),
                get_icon("JoyAxis", "EditorIcons")
            );
            popup_add->set_item_icon(
                popup_add->get_item_index(INPUT_MOUSE_BUTTON),
                get_icon("Mouse", "EditorIcons")
            );
            _update_actions();
        } break;
...
void ProjectSettingsEditor::_action_edited() {
    ...
    if (input_editor->get_selected_column() == 0) {
        ...
        if (new_name == "" || !_validate_action_name(new_name)) {
            ...
            message->set_text(
                TTR("Invalid action name. It cannot be empty nor contain '/', "
                    "':', '=', '\\' or '\"'")
            );
        ...
        if (Global::ProjectSettings().has_setting(action_prop)) {
            ti->set_text(0, old_name);
            add_at = "input/" + old_name;

            message->set_text(vformat(
                TTR("An action with the name '%s' already exists."),
                new_name
            ));
            message->popup_centered(Size2(300, 100) * EDSCALE);
            return;
        ...
        int order         = Global::ProjectSettings().get_order(add_at);
        Dictionary action = GLOBAL_GET(add_at);

        setting = true;
        undo_redo->create_action(TTR("Rename Input Action Event"));
        undo_redo->add_do_method(&(Global::ProjectSettings()), "clear", add_at);
        ...
        Dictionary old_action  = GLOBAL_GET(name);
...
void ProjectSettingsEditor::_device_input_add() {
    ...
    String name        = add_at;
    int idx            = edit_idx;
    Dictionary old_val = GLOBAL_GET(name);
    ...
    switch (add_type) {
        case INPUT_MOUSE_BUTTON: {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                if (aie->get_device() == mb->get_device()
                    && aie->get_button_index() == mb->get_button_index()) {
                    return;
                }
            }
        ...
        case INPUT_JOY_MOTION: {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                if (aie->get_device() == jm->get_device()
                    && aie->get_axis() == jm->get_axis()
                    && aie->get_axis_value() == jm->get_axis_value()) {
                    return;
                }
            }
        ...
        case INPUT_JOY_BUTTON: {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                if (aie->get_device() == jb->get_device()
                    && aie->get_button_index() == jb->get_button_index()) {
                    return;
                }
            }
            ie = jb;
    ...
    undo_redo->add_do_method(&(Global::ProjectSettings()), "set", name, action);
    undo_redo
        ->add_undo_method(&(Global::ProjectSettings()), "set", name, old_val);
    undo_redo->add_do_method(this, "_update_actions");
    undo_redo->add_undo_method(this, "_update_actions");
    undo_redo->add_do_method(this, "_settings_changed");
    undo_redo->add_undo_method(this, "_settings_changed");
...
void ProjectSettingsEditor::_press_a_key_confirm() {
    ...
    for (int i = 0; i < events.size(); i++) {
        ...
        if (!press_a_key_physical) {
            if (aie->get_scancode_with_modifiers()
                == ie->get_scancode_with_modifiers()) {
                return;
            }
        } else {
            if (aie->get_physical_scancode_with_modifiers()
                == ie->get_physical_scancode_with_modifiers()) {
                return;
            }
        }
    }
...
void ProjectSettingsEditor::_show_last_added(
...
void ProjectSettingsEditor::_wait_for_key(const Ref<InputEvent>& p_event) {
    ...
    if (k.is_valid() && k->is_pressed() && k->get_scancode() != 0) {
        ...
        const String str =
            (press_a_key_physical)
                ? keycode_get_string(k->get_physical_scancode_with_modifiers())
                      + TTR(" (Physical)")
                : keycode_get_string(k->get_scancode_with_modifiers());
...
void ProjectSettingsEditor::_add_item(
    int p_item,
    Ref<InputEvent> p_exiting_event
) {
    ...
    switch (add_type) {
        ...
        case INPUT_JOY_MOTION: {
            ...
            if (jm.is_valid()) {
                device_index->select(
                    jm->get_axis() * 2 + (jm->get_axis_value() > 0 ? 1 : 0)
                );
...
void ProjectSettingsEditor::_action_activated() {
    ...
    Dictionary action = GLOBAL_GET(name);
...
void ProjectSettingsEditor::_action_button_pressed(
    Object* p_obj,
    int p_column,
    int p_id
) {
    ...
    if (p_id == 1) {
        ...
        if (ti->get_parent() == input_editor->get_root()) {
            ...
            String name        = "input/" + ti->get_text(0);
            Dictionary old_val = GLOBAL_GET(name);
            int order          = Global::ProjectSettings().get_order(name);

            undo_redo->create_action(TTR("Erase Input Action"));
            undo_redo
                ->add_do_method(&(Global::ProjectSettings()), "clear", name);
            undo_redo->add_undo_method(
                &(Global::ProjectSettings()),
                "set",
                name,
...
void ProjectSettingsEditor::_update_actions() {
    ...
    for (List<PropertyInfo>::Element* E = props.front(); E; E = E->next()) {
        ...
        if (!Global::ProjectSettings().get_input_presets().find(pi.name)) {
            item->add_button(
                2,
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            if (k.is_valid()) {
                const String str =
                    (k->get_scancode() == 0)
                        ? keycode_get_string(
                              k->get_physical_scancode_with_modifiers()
                          ) + TTR(" (Physical)")
                        : keycode_get_string(k->get_scancode_with_modifiers());
            ...
            if (jb.is_valid()) {
                ...
                if (jb->get_button_index() >= 0
                    && jb->get_button_index() < JOY_BUTTON_MAX) {
                    str += String() + " ("
                         + _button_names[jb->get_button_index()] + ").";
                } else {
                    str += ".";
                }
...
void ProjectSettingsEditor::_item_selected(const String& p_path) {
    ...
    property->set_text(
        globals_editor->get_current_section().plus_file(selected_path)
    );
...
void ProjectSettingsEditor::_item_add() {
    ...
    const Variant value = Variant::construct(
        Variant::Type(type->get_selected_id()),
        nullptr,
        0,
        ce
    );
    ...
    undo_redo->add_do_property(&(Global::ProjectSettings()), name, value);

    if (Global::ProjectSettings().has_setting(name)) {
        ...
        undo_redo
            ->add_undo_property(&(Global::ProjectSettings()), name, Variant());
    }
...
void ProjectSettingsEditor::_item_del() {
    ...
    if (path == String()) {
        EditorNode::get_singleton()->show_warning(
            TTR("Select a setting item first!")
        );
        return;
    }
    ...
    if (!Global::ProjectSettings().has_setting(property)) {
        EditorNode::get_singleton()->show_warning(
            vformat(TTR("No property '%s' exists."), property)
        );
        return;
    }

    if (Global::ProjectSettings().get_order(property)
    ...
    Variant value = GLOBAL_GET(property);
    int order     = Global::ProjectSettings().get_order(property);

    undo_redo->add_do_method(&(Global::ProjectSettings()), "clear", property);
    undo_redo
        ->add_undo_method(&(Global::ProjectSettings()), "set", property, value);
    undo_redo->add_undo_method(
        &(Global::ProjectSettings()),
        "set_order",
        property,
...
void ProjectSettingsEditor::_action_check(String p_action) {
    if (p_action == "") {
        ...
        if (Global::ProjectSettings().has_setting("input/" + p_action)) {
            action_add_error->set_text(vformat(
                TTR("An action with the name '%s' already exists."),
                p_action
            ));
            action_add_error->show();
...
void ProjectSettingsEditor::_action_add() {
    ...
    undo_redo->add_do_method(&(Global::ProjectSettings()), "set", name, action);
    undo_redo->add_undo_method(&(Global::ProjectSettings()), "clear", name);
    undo_redo->add_do_method(this, "_update_actions");
    undo_redo->add_undo_method(this, "_update_actions");
    undo_redo->add_do_method(this, "_settings_changed");
    undo_redo->add_undo_method(this, "_settings_changed");
...
void ProjectSettingsEditor::_save() {
    Error err = Global::ProjectSettings().save();
    message->set_text(
        err != OK ? TTR("Error saving settings.") : TTR("Settings saved OK.")
    );
    message->popup_centered(Size2(300, 100) * EDSCALE);
}

void ProjectSettingsEditor::_settings_prop_edited(const String& p_name) {
    // Method needed to discard the mandatory argument of the property_edited
    // signal
    _settings_changed();
}
...
void ProjectSettingsEditor::_copy_to_platform_about_to_show() {
    ...
    for (int i = 0;
        ...
        EditorExport::get_singleton()
            ->get_export_platform(i)
            ->get_platform_features(&p);
    ...
    for (int i = 0;
        ...
        String custom = EditorExport::get_singleton()
                            ->get_export_preset(i)
                            ->get_custom_features();
...
Variant ProjectSettingsEditor::get_drag_data_fw(
...
bool ProjectSettingsEditor::can_drop_data_fw(
    const Point2& p_point,
...
void ProjectSettingsEditor::drop_data_fw(
    ...
    TreeItem* target = item->get_parent() == input_editor->get_root()
                         ? item
                         : item->get_parent();

    String selected_name = "input/" + selected->get_text(0);
    int old_order        = Global::ProjectSettings().get_order(selected_name);
    String target_name   = "input/" + target->get_text(0);
    int target_order     = Global::ProjectSettings().get_order(target_name);
    ...
    while (iterator != target) {
        ...
        int iterator_order = Global::ProjectSettings().get_order(iterator_name);
        undo_redo->add_do_method(
            &(Global::ProjectSettings()),
            "set_order",
            iterator_name,
...
void ProjectSettingsEditor::_copy_to_platform(int p_which) {
    ...
    if (property.find(".") != -1) { // overwriting overwrite, keep overwrite
        undo_redo
            ->add_do_method(&(Global::ProjectSettings()), "clear", property);
        undo_redo->add_undo_method(
            &(Global::ProjectSettings()),
            "set",
            property,
    ...
    undo_redo
        ->add_do_method(&(Global::ProjectSettings()), "set", new_path, value);
    if (Global::ProjectSettings().has_setting(new_path)) {
        undo_redo->add_undo_method(
            &(Global::ProjectSettings()),
            "set",
            new_path,
...
void ProjectSettingsEditor::_translation_add(const PoolStringArray& p_paths) {
    PoolStringArray translations = GLOBAL_GET("locale/translations");
    ...
    undo_redo->add_undo_property(
        &(Global::ProjectSettings()),
        "locale/translations",
        GLOBAL_GET("locale/translations")
    );
...
void ProjectSettingsEditor::_translation_delete(
    Object* p_item,
    int p_column,
    int p_button
) {
...
void ProjectSettingsEditor::_translation_res_add(const PoolStringArray& p_paths
    ...
    if (Global::ProjectSettings().has_setting("locale/translation_remaps")) {
        remaps = GLOBAL_GET("locale/translation_remaps");
        prev   = remaps;
    }

    for (int i = 0; i < p_paths.size(); i++) {
        if (!remaps.has(p_paths[i])) {
            // Don't overwrite with an empty remap array if an array already
...
void ProjectSettingsEditor::_translation_res_option_add(
    const PoolStringArray& p_paths
) {
    ERR_FAIL_COND(
        !Global::ProjectSettings().has_setting("locale/translation_remaps")
    );
    ...
    undo_redo->add_undo_property(
        &(Global::ProjectSettings()),
        "locale/translation_remaps",
        GLOBAL_GET("locale/translation_remaps")
    );
    undo_redo->add_do_method(this, "_update_translations");
    undo_redo->add_undo_method(this, "_update_translations");
    undo_redo->add_do_method(this, "_settings_changed");
...
void ProjectSettingsEditor::_translation_res_option_delete(
    Object* p_item,
    int p_column,
    int p_button
) {
...
void ProjectSettingsEditor::_translation_filter_option_changed() {
    ...
    if (Global::ProjectSettings().has_setting("locale/locale_filter")) {
        f_locales_all = GLOBAL_GET("locale/locale_filter");
        prev          = f_locales_all;
...
void ProjectSettingsEditor::_update_translations() {
    ...
    if (Global::ProjectSettings().has_setting("locale/translations")) {
        PoolStringArray translations = GLOBAL_GET("locale/translations");
    ...
    if (Global::ProjectSettings().has_setting("locale/locale_filter")) {
        l_filter_all = GLOBAL_GET("locale/locale_filter");
    ...
    bool is_short_list_when_show_all_selected =
        filter_mode == SHOW_ALL_LOCALES
        && translation_filter_treeitems.size() < s;
    bool is_full_list_when_show_only_selected =
        filter_mode == SHOW_ONLY_SELECTED_LOCALES
        && translation_filter_treeitems.size() == s;
    bool should_recreate_locales_list = is_short_list_when_show_all_selected
                                     || is_full_list_when_show_only_selected;
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            if (keys[i] == remap_selected) {
                ...
                for (int j = 0; j < selected.size(); j++) {
                    ...
                    TreeItem* t2 =
                        translation_remap_options->create_item(root2);
...
void ProjectSettingsEditor::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_unhandled_input"),
        &ProjectSettingsEditor::_unhandled_input
    );
    ClassDB::bind_method(
        D_METHOD("_item_selected"),
        &ProjectSettingsEditor::_item_selected
    );
    ClassDB::bind_method(
        D_METHOD("_item_add"),
        &ProjectSettingsEditor::_item_add
    );
    ClassDB::bind_method(
        D_METHOD("_item_adds"),
        &ProjectSettingsEditor::_item_adds
    );
    ClassDB::bind_method(
        D_METHOD("_item_del"),
        &ProjectSettingsEditor::_item_del
    );
    ClassDB::bind_method(
        D_METHOD("_item_checked"),
        &ProjectSettingsEditor::_item_checked
    );
    ClassDB::bind_method(D_METHOD("_save"), &ProjectSettingsEditor::_save);
    ClassDB::bind_method(
        D_METHOD("_action_add"),
        &ProjectSettingsEditor::_action_add
    );
    ClassDB::bind_method(
        D_METHOD("_action_adds"),
        &ProjectSettingsEditor::_action_adds
    );
    ClassDB::bind_method(
        D_METHOD("_action_check"),
        &ProjectSettingsEditor::_action_check
    );
    ClassDB::bind_method(
        D_METHOD("_action_selected"),
        &ProjectSettingsEditor::_action_selected
    );
    ClassDB::bind_method(
        D_METHOD("_action_edited"),
        &ProjectSettingsEditor::_action_edited
    );
    ClassDB::bind_method(
        D_METHOD("_action_activated"),
        &ProjectSettingsEditor::_action_activated
    );
    ClassDB::bind_method(
        D_METHOD("_action_button_pressed"),
        &ProjectSettingsEditor::_action_button_pressed
    );
    ClassDB::bind_method(
        D_METHOD("_update_actions"),
        &ProjectSettingsEditor::_update_actions
    );
    ClassDB::bind_method(
        D_METHOD("_wait_for_key"),
        &ProjectSettingsEditor::_wait_for_key
    );
    ClassDB::bind_method(
        D_METHOD("_add_item"),
        &ProjectSettingsEditor::_add_item,
        DEFVAL(Variant())
    );
    ClassDB::bind_method(
        D_METHOD("_device_input_add"),
        &ProjectSettingsEditor::_device_input_add
    );
    ClassDB::bind_method(
        D_METHOD("_press_a_key_confirm"),
        &ProjectSettingsEditor::_press_a_key_confirm
    );
    ClassDB::bind_method(
        D_METHOD("_settings_prop_edited"),
        &ProjectSettingsEditor::_settings_prop_edited
    );
    ClassDB::bind_method(
        D_METHOD("_copy_to_platform"),
        &ProjectSettingsEditor::_copy_to_platform
    );
    ClassDB::bind_method(
        D_METHOD("_update_translations"),
        &ProjectSettingsEditor::_update_translations
    );
    ClassDB::bind_method(
        D_METHOD("_translation_delete"),
        &ProjectSettingsEditor::_translation_delete
    );
    ClassDB::bind_method(
        D_METHOD("_settings_changed"),
        &ProjectSettingsEditor::_settings_changed
    );
    ClassDB::bind_method(
        D_METHOD("_translation_add"),
        &ProjectSettingsEditor::_translation_add
    );
    ClassDB::bind_method(
        D_METHOD("_translation_file_open"),
        &ProjectSettingsEditor::_translation_file_open
    );

    ClassDB::bind_method(
        D_METHOD("_translation_res_add"),
        &ProjectSettingsEditor::_translation_res_add
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_file_open"),
        &ProjectSettingsEditor::_translation_res_file_open
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_option_add"),
        &ProjectSettingsEditor::_translation_res_option_add
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_option_file_open"),
        &ProjectSettingsEditor::_translation_res_option_file_open
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_select"),
        &ProjectSettingsEditor::_translation_res_select
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_option_changed"),
        &ProjectSettingsEditor::_translation_res_option_changed
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_delete"),
        &ProjectSettingsEditor::_translation_res_delete
    );
    ClassDB::bind_method(
        D_METHOD("_translation_res_option_delete"),
        &ProjectSettingsEditor::_translation_res_option_delete
    );

    ClassDB::bind_method(
        D_METHOD("_translation_filter_option_changed"),
        &ProjectSettingsEditor::_translation_filter_option_changed
    );
    ClassDB::bind_method(
        D_METHOD("_translation_filter_mode_changed"),
        &ProjectSettingsEditor::_translation_filter_mode_changed
    );

    ClassDB::bind_method(
        D_METHOD("_toggle_search_bar"),
        &ProjectSettingsEditor::_toggle_search_bar
    );

    ClassDB::bind_method(
        D_METHOD("_copy_to_platform_about_to_show"),
        &ProjectSettingsEditor::_copy_to_platform_about_to_show
    );

    ClassDB::bind_method(
        D_METHOD("_editor_restart_request"),
        &ProjectSettingsEditor::_editor_restart_request
    );
    ClassDB::bind_method(
        D_METHOD("_editor_restart"),
        &ProjectSettingsEditor::_editor_restart
    );
    ClassDB::bind_method(
        D_METHOD("_editor_restart_close"),
        &ProjectSettingsEditor::_editor_restart_close
    );

    ClassDB::bind_method(
        D_METHOD("get_tabs"),
        &ProjectSettingsEditor::get_tabs
    );

    ClassDB::bind_method(
        D_METHOD("get_drag_data_fw"),
        &ProjectSettingsEditor::get_drag_data_fw
    );
    ClassDB::bind_method(
        D_METHOD("can_drop_data_fw"),
        &ProjectSettingsEditor::can_drop_data_fw
    );
    ClassDB::bind_method(
        D_METHOD("drop_data_fw"),
        &ProjectSettingsEditor::drop_data_fw
    );
}
...
ProjectSettingsEditor::ProjectSettingsEditor(EditorData* p_data) {
    ...
    globals_editor->get_inspector()->set_undo_redo(
        EditorNode::get_singleton()->get_undo_redo()
    );
    globals_editor->set_v_size_flags(Control::SIZE_EXPAND_FILL);
    globals_editor->register_search_box(search_box);
    globals_editor->get_inspector()
        ->connect("property_selected", this, "_item_selected");
    globals_editor->get_inspector()
        ->connect("property_edited", this, "_settings_prop_edited");
    globals_editor->get_inspector()
        ->connect("restart_requested", this, "_editor_restart_request");
    ...
    popup_copy_to_feature->get_popup()
        ->connect("id_pressed", this, "_copy_to_platform");
    popup_copy_to_feature->get_popup()
        ->connect("about_to_show", this, "_copy_to_platform_about_to_show");
    ...
    restart_label->set_text(
        TTR("The editor must be restarted for changes to take effect.")
    );
...

    {
        ...
        translation_file_open
            ->connect("files_selected", this, "_translation_add");
    }

    {
        ...
        translation_remap
            ->connect("cell_selected", this, "_translation_res_select");
        tmc->add_child(translation_remap);
        translation_remap
            ->connect("button_pressed", this, "_translation_res_delete");
        ...
        translation_res_file_open
            ->connect("files_selected", this, "_translation_res_add");
        ...
        translation_remap_options
            ->connect("item_edited", this, "_translation_res_option_changed");
        translation_remap_options
            ->connect("button_pressed", this, "_translation_res_option_delete");

        translation_res_option_file_open = memnew(EditorFileDialog);
        add_child(translation_res_option_file_open);
        translation_res_option_file_open->set_mode(
            EditorFileDialog::MODE_OPEN_FILES
        );
        translation_res_option_file_open
...

    {
        ...
        translation_locale_filter_mode->add_item(
            TTR("Show All Locales"),
            SHOW_ALL_LOCALES
        );
        translation_locale_filter_mode->add_item(
            TTR("Show Selected Locales Only"),
            SHOW_ONLY_SELECTED_LOCALES
        );
        translation_locale_filter_mode->select(0);
        tmc->add_margin_child(
            TTR("Filter mode:"),
            translation_locale_filter_mode
        );
        translation_locale_filter_mode->connect(
            "item_selected",
            this,
            "_translation_filter_mode_changed"
        );
        ...
        translation_filter->connect(
            "item_edited",
            this,
            "_translation_filter_option_changed"
        );
    }
    ...
    import_defaults_editor
        ->connect("project_settings_changed", this, "_settings_changed");

    timer = memnew(Timer);
    timer->set_wait_time(1.5);
    timer->connect("timeout", &(Global::ProjectSettings()), "save");
