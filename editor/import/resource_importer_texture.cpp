...
void ResourceImporterTexture::_texture_reimport_srgb(
    const Ref<StreamTexture>& p_tex
) {
...
void ResourceImporterTexture::_texture_reimport_3d(
    const Ref<StreamTexture>& p_tex
) {
    singleton->mutex.lock();
    StringName path = p_tex->get_path();
...
void ResourceImporterTexture::_texture_reimport_normal(
    const Ref<StreamTexture>& p_tex
) {
    singleton->mutex.lock();
    StringName path = p_tex->get_path();
...
void ResourceImporterTexture::update_imports() {
    if (EditorFileSystem::get_singleton()->is_scanning()
        || EditorFileSystem::get_singleton()->is_importing()) {
    ...
    for (Map<StringName, int>::Element* E = make_flags.front(); E;
        ...
        bool changed = false;
        if (E->get() & MAKE_SRGB_FLAG
            && int(cf->get_value("params", "flags/srgb")) == 2) {
            cf->set_value("params", "flags/srgb", 1);
            changed = true;
        }

        if (E->get() & MAKE_NORMAL_FLAG
            && int(cf->get_value("params", "compress/normal_map")) == 0) {
            cf->set_value("params", "compress/normal_map", 1);
            changed = true;
        }

        if (E->get() & MAKE_3D_FLAG
            && bool(cf->get_value("params", "detect_3d"))) {
...
void ResourceImporterTexture::get_recognized_extensions(
...
bool ResourceImporterTexture::get_option_visibility(
    ...
    if (p_option == "compress/lossy_quality") {
        ...
        if (compress_mode != COMPRESS_LOSSY
            && compress_mode != COMPRESS_VIDEO_RAM) {
            return false;
        }
...
void ResourceImporterTexture::get_import_options(
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(
            ...
            "compress/mode",
            PROPERTY_HINT_ENUM,
            "Lossless,Lossy,Video RAM,Uncompressed",
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(
            ...
            "flags/repeat",
            PROPERTY_HINT_ENUM,
            "Disabled,Enabled,Mirrored"
        ),
        p_preset == PRESET_3D ? 1 : 0
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "flags/filter"),
        p_preset != PRESET_2D_PIXEL
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "flags/mipmaps"),
        p_preset == PRESET_3D
    ));
    r_options->push_back(
        ImportOption(PropertyInfo(Variant::BOOL, "flags/anisotropic"), false)
    );
    r_options->push_back(ImportOption(
        PropertyInfo(
            Variant::INT,
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "process/fix_alpha_border"),
        p_preset != PRESET_3D
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "process/premult_alpha"),
        false
    ));
    r_options->push_back(
        ImportOption(PropertyInfo(Variant::BOOL, "process/HDR_as_SRGB"), false)
    );
    r_options->push_back(
        ImportOption(PropertyInfo(Variant::BOOL, "process/invert_color"), false)
    );
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "process/normal_map_invert_y"),
        false
    ));
    r_options->push_back(
        ImportOption(PropertyInfo(Variant::INT, "stream"), false)
    );
    r_options->push_back(ImportOption(
        PropertyInfo(
            Variant::INT,
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "detect_3d"),
        p_preset == PRESET_DETECT
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
            Variant::REAL,
...
void ResourceImporterTexture::_save_stex(
    ...
    if (p_compress_mode == COMPRESS_VIDEO_RAM && p_force_po2_for_compressed
        && (p_mipmaps || p_texture_flags & Texture::FLAG_REPEAT)) {
        resize_to_po2 = true;
    ...
    if ((p_compress_mode == COMPRESS_LOSSLESS
         || p_compress_mode == COMPRESS_LOSSY)
        && p_image->get_format() > Image::FORMAT_RGBA8) {
    ...
    switch (p_compress_mode) {
        case COMPRESS_LOSSLESS: {
            ...
            bool use_webp =
                ...
                && p_image->get_height() <= 16383; // WebP has a size limit
        ...
        case COMPRESS_LOSSY: {
            ...
            for (int i = 0; i < mmc; i++) {
                ...
                PoolVector<uint8_t> data =
                    Image::webp_lossy_packer(image, p_lossy_quality);
                int data_len = data.size();
                f->store_32(data_len);
        ...
        case COMPRESS_VIDEO_RAM: {
            ...
            if (p_force_rgbe && image->get_format() >= Image::FORMAT_R8
                && image->get_format() <= Image::FORMAT_RGBE9995) {
...
Error ResourceImporterTexture::import(
    ...
    Ref<Image> image;
    image.instance();
    Error err = ImageLoader::load_image(
        p_source_file,
        image,
        nullptr,
        hdr_as_srgb,
    ...
    if (size_limit > 0
        && (image->get_width() > size_limit || image->get_height() > size_limit
        )) {
        // limit size
        if (image->get_width() >= image->get_height()) {
            int new_width = size_limit;
            int new_height =
                image->get_height() * new_width / image->get_width();

            image->resize(new_width, new_height, Image::INTERPOLATE_CUBIC);
        } else {
            int new_height = size_limit;
            int new_width =
                image->get_width() * new_height / image->get_height();
    ...
    if (normal_map_invert_y) {
        // Inverting the green channel can be used to flip a normal map's
        // direction. There's no standard when it comes to normal map Y
        // direction, so this is sometimes needed when using a normal map
    ...
    if (compress_mode == COMPRESS_VIDEO_RAM) {
        ...
        bool ok_on_pc = false;
        bool is_hdr =
            (image->get_format() >= Image::FORMAT_RF
             && image->get_format() <= Image::FORMAT_RGBE9995);
        bool is_ldr =
        ...
        if (can_bptc) {
            ...
            if (is_hdr) {
                if (channels == Image::DETECTED_LA
                    || channels == Image::DETECTED_RGBA) {
                    can_bptc = false;
                }
            } else if (is_ldr) {
                // handle "RGBA Only" setting
                if (bptc_ldr == 1 && channels != Image::DETECTED_LA
                    && channels != Image::DETECTED_RGBA) {
                    can_bptc = false;
                }
            }
        ...
        if (can_bptc || can_s3tc) {
            _save_stex(
                ...
                compress_mode,
                lossy,
                can_bptc ? Image::COMPRESS_BPTC : Image::COMPRESS_S3TC,
                mipmaps,
                tex_flags,
                stream,
                detect_3d,
        ...
        if (!ok_on_pc) {
            EditorNode::add_io_error(
                "Warning, no suitable PC VRAM compression enabled in Project "
                "Settings. This texture will not display correctly on PC."
            );
        }
    } else {
        // import normally
...
const char* ResourceImporterTexture::compression_formats[] =
    {"bptc", "s3tc", "etc", "etc2", "pvrtc", nullptr};
...
bool ResourceImporterTexture::are_import_settings_valid(const String& p_path
) const {
    // will become invalid if formats are missing to import
    Dictionary metadata =
        ResourceFormatImporter::get_singleton()->get_resource_metadata(p_path);
