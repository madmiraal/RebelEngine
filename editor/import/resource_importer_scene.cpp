...
Node* EditorSceneImporter::import_scene(
    ...
    if (get_script_instance()) {
        return get_script_instance()
            ->call("_import_scene", p_path, p_flags, p_bake_fps);
    }

    ERR_FAIL_V(nullptr);
}

Ref<Animation> EditorSceneImporter::import_animation(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps
) {
    if (get_script_instance()) {
        return get_script_instance()
            ->call("_import_animation", p_path, p_flags);
    }

    ERR_FAIL_V(nullptr);
}

// for documenters, these functions are useful when an importer calls an
// external conversion helper (like, fbx2gltf), and you want to load the
// resulting file

Node* EditorSceneImporter::import_scene_from_other_importer(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps,
    uint32_t p_compress_flags
) {
    return ResourceImporterScene::get_singleton()
...
void EditorSceneImporter::_bind_methods() {
    ...
    MethodInfo mi = MethodInfo(
        ...
        PropertyInfo(Variant::INT, "bake_fps")
    );
...
void EditorScenePostImport::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("get_source_folder"),
        &EditorScenePostImport::get_source_folder
    );
    ClassDB::bind_method(
        D_METHOD("get_source_file"),
...
void EditorScenePostImport::init(
    const String& p_source_folder,
...
void ResourceImporterScene::get_recognized_extensions(List<String>* p_extensions
) const {
...
bool ResourceImporterScene::get_option_visibility(
    ...
    if (p_option.begins_with("animation/")) {
        ...
        if (p_option == "animation/keep_custom_tracks"
            && int(p_options["animation/storage"]) == 0) {
            return false;
        }

        if (p_option.begins_with("animation/optimizer/")
            && p_option != "animation/optimizer/enabled"
            && !bool(p_options["animation/optimizer/enabled"])) {
            return false;
        }
        ...
        if (p_option.begins_with("animation/clip_")) {
            ...
            int clip =
                p_option.get_slice("/", 1).get_slice("_", 1).to_int() - 1;
            if (clip >= max_clip) {
                return false;
            }
        }
    ...
    if (p_option == "materials/keep_on_reimport"
        && int(p_options["materials/storage"]) == 0) {
        return false;
    }

    if (p_option == "meshes/lightmap_texel_size"
        && int(p_options["meshes/light_baking"]) < 2) {
        return false;
    }
...
static bool _teststr(const String& p_what, const String& p_str) {
    ...
    // remove trailing spaces and numbers, some apps like blender add ".number"
    // to duplicates so also compensate for this
    while (
        what.length()
        && ((what[what.length() - 1] >= '0' && what[what.length() - 1] <= '9')
    ...
    if (what.to_lower().ends_with(
            "-" + p_str
        )) { // collada only supports "_" and "-" besides letters
        return true;
    }
    if (what.to_lower().ends_with(
            "_" + p_str
...
static void _gen_shape_list(
    const Ref<Mesh>& mesh,
    List<Ref<Shape>>& r_shape_list,
    bool p_convex
) {
    if (!p_convex) {
...
Node* ResourceImporterScene::_fix_node(
    ...
    LightBakeMode p_light_bake_mode
) {
    // children first
        ...
        if (m.is_valid()) {
            for (int i = 0; i < m->get_surface_count(); i++) {
                ...
                if (_teststr(mat->get_name(), "vcol")) {
                    ...
                    mat->set_flag(
                        SpatialMaterial::FLAG_SRGB_VERTEX_COLOR,
                        true
                    );
    ...
    if (_teststr(name, "colonly") || _teststr(name, "convcolonly")) {
        ...
        if (mi) {
            ...
            String empty_draw_type =
                String(p_node->get_meta("empty_draw_type"));
            StaticBody* sb = memnew(StaticBody);
            sb->set_name(_fixstr(name, "colonly"));
            Object::cast_to<Spatial>(sb)->set_transform(
                Object::cast_to<Spatial>(p_node)->get_transform()
            );
        ...
        if (mesh.is_valid()) {
            ...
            if (fixed_name != String()) {
                if (mi->get_parent()
                    && !mi->get_parent()->has_node(fixed_name)) {
...
void ResourceImporterScene::_create_clips(
    Node* scene,
    const Array& p_clips,
    bool p_bake_all
) {
        ...
        for (int j = 0; j < default_anim->get_track_count(); j++) {
            ...
            for (int k = 0; k < kc; k++) {
                ...
                if (kt >= from && kt < to) {
                    ...
                    if (dtrack == -1) {
                        ...
                        new_anim->track_set_path(
                            dtrack,
                            default_anim->track_get_path(j)
                        );

                        if (kt > (from + 0.01) && k > 0) {
                            if (default_anim->track_get_type(j)
                                == Animation::TYPE_TRANSFORM) {
                                Quat q;
                                Vector3 p;
                                Vector3 s;
                                default_anim->transform_track_interpolate(
                                    j,
                                    from,
                                    &p,
                                    &q,
                                ...
                                new_anim->transform_track_insert_key(
                                    dtrack,
                                    0,
                                    p,
                                    q,
                            ...
                            if (default_anim->track_get_type(j)
                                ...
                                Variant var =
                                    default_anim->value_track_interpolate(
                                        j,
                                        from
                                    );
                ...
                if (dtrack != -1 && kt >= to) {
                    if (default_anim->track_get_type(j)
                        ...
                        Vector3 s;
                        default_anim
                            ->transform_track_interpolate(j, to, &p, &q, &s);
                        new_anim->transform_track_insert_key(
                            dtrack,
                            to - from,
                            p,
                    ...
                    if (default_anim->track_get_type(j)
                        == Animation::TYPE_VALUE) {
                        Variant var =
                            default_anim->value_track_interpolate(j, to);
            ...
            if (dtrack == -1 && p_bake_all) {
                ...
                if (default_anim->track_get_type(j)
                    ...
                    Vector3 s;
                    default_anim
                        ->transform_track_interpolate(j, from, &p, &q, &s);
                ...
                if (default_anim->track_get_type(j) == Animation::TYPE_VALUE) {
                    Variant var =
                        default_anim->value_track_interpolate(j, from);
                    new_anim->track_insert_key(dtrack, 0, var);
                    Variant to_var =
                        default_anim->value_track_interpolate(j, to);
...
void ResourceImporterScene::_filter_anim_tracks(
    Ref<Animation> anim,
    Set<String>& keep
) {
    ...
    for (List<StringName>::Element* E = anim_names.front(); E; E = E->next()) {
        ...
        for (int i = 0; i < strings.size(); i++) {
            if (strings[i].begins_with("@")) {
                ...
                for (Set<String>::Element* F = keep_local.front(); F;
                     F                       = F->next()) {
                    keep.insert(F->get());
                }
                keep_local.clear();
                ...
                Vector<String> filters =
                    strings[i].substr(1, strings[i].length()).split(",");
                for (int j = 0; j < filters.size(); j++) {
                    ...
                    String filter =
                        fname.substr(1, fname.length()).strip_edges();
                ...
                for (int j = 0; j < a->get_track_count(); j++) {
                    ...
                    String filter =
                        tname.substr(1, tname.length()).strip_edges();
...
void ResourceImporterScene::_optimize_animations(
    Node* scene,
    float p_max_lin_error,
    float p_max_ang_error,
    float p_max_angle
) {
    if (!scene->has_node(String("AnimationPlayer"))) {
        return;
...
void ResourceImporterScene::_find_meshes(
    Node* p_node,
...
void ResourceImporterScene::_make_external_resources(
    Node* p_node,
    const String& p_base_path,
    bool p_make_animations,
    bool p_animations_as_text,
    bool p_keep_animations,
    bool p_make_materials,
    bool p_materials_as_text,
    bool p_keep_materials,
    bool p_make_meshes,
    bool p_meshes_as_text,
    Map<Ref<Animation>, Ref<Animation>>& p_animations,
    Map<Ref<Material>, Ref<Material>>& p_materials,
        ...
        if (Object::cast_to<AnimationPlayer>(p_node)) {
            ...
            for (List<StringName>::Element* E = anims.front(); E;
                ...
                if (!p_animations.has(anim)) {
                    // Tracks from source file should be set as imported,
                    // anything else is a custom track.
                    for (int i = 0; i < anim->get_track_count(); i++) {
                    ...
                    if (p_animations_as_text) {
                        ext_name = p_base_path.plus_file(
                            _make_extname(E->get()) + ".tres"
                        );
                    } else {
                        ext_name = p_base_path.plus_file(
                            _make_extname(E->get()) + ".anim"
                        );
                    }

                    if (FileAccess::exists(ext_name) && p_keep_animations) {
                        // Copy custom animation tracks from previously imported
                        // files.
                        Ref<Animation> old_anim =
                            ResourceLoader::load(ext_name, "Animation", true);
                        if (old_anim.is_valid()) {
                            for (int i = 0; i < old_anim->get_track_count();
                                 i++) {
            ...
            if (p_make_materials && mat.is_valid() && mat->get_name() != "") {
                if (!p_materials.has(mat)) {
                    ...
                    if (p_materials_as_text) {
                        ext_name = p_base_path.plus_file(
                            _make_extname(mat->get_name()) + ".tres"
                        );
                    } else {
                        ext_name = p_base_path.plus_file(
                            _make_extname(mat->get_name()) + ".material"
                        );
                    }
                    ...
                    if (p_keep_materials && FileAccess::exists(ext_name)) {
                        ...
                        p_materials[mat] = ResourceLoader::load(
                            ext_name,
                            "",
                            true
                ...
                if (mesh.is_valid()) {
                    ...
                    if (p_make_meshes) {
                        if (!p_meshes.has(mesh)) {
                            // meshes are always overwritten, keeping them is
                            // not practical
                            String ext_name;

                            if (p_meshes_as_text) {
                                ext_name = p_base_path.plus_file(
                                    _make_extname(mesh->get_name()) + ".tres"
                                );
                            } else {
                                ext_name = p_base_path.plus_file(
                                    _make_extname(mesh->get_name()) + ".mesh"
                                );
                            }
                        ...
                        if (mesh_just_added || !p_meshes.has(mesh)) {
                            for (int i = 0; i < mesh->get_surface_count();
                                ...
                                if (!p_materials.has(mat)) {
                                    ...
                                    if (p_keep_materials
                                        && FileAccess::exists(ext_name)) {
                                        // if exists, use it
                                        p_materials[mat] =
                                            ResourceLoader::load(ext_name);
                                    } else {
                                        ResourceSaver::save(
                                            ext_name,
                                            mat,
                                ...
                                if (p_materials[mat] != mat) {
                                    ...
                                    // re-save the mesh since a material is now
                                    // assigned
                                    if (p_make_meshes) {
                                        String ext_name;
                            ...
                            if (!p_make_meshes) {
                                p_meshes[mesh] =
                                    Ref<ArrayMesh>(); // save it anyway, so it
    ...
    for (int i = 0; i < p_node->get_child_count(); i++) {
        _make_external_resources(
            p_node->get_child(i),
            p_base_path,
            p_make_animations,
            p_animations_as_text,
            p_keep_animations,
            p_make_materials,
            p_materials_as_text,
            p_keep_materials,
...
void ResourceImporterScene::get_import_options(
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::STRING, "nodes/root_name"),
        "Scene Root"
    ));
    ...
    for (List<String>::Element* E = script_extentions.front(); E;
         E                        = E->next()) {
        if (script_ext_hint != "") {
            script_ext_hint += ",";
        }
    ...
    bool materials_out =
        p_preset == PRESET_SEPARATE_MATERIALS
    ...
    bool meshes_out =
        p_preset == PRESET_SEPARATE_MESHES
        || p_preset == PRESET_SEPARATE_MESHES_AND_MATERIALS
        || p_preset == PRESET_SEPARATE_MESHES_AND_ANIMATIONS
        || p_preset == PRESET_SEPARATE_MESHES_MATERIALS_AND_ANIMATIONS;
    bool scenes_out = p_preset == PRESET_MULTIPLE_SCENES
                   || p_preset == PRESET_MULTIPLE_SCENES_AND_MATERIALS;
    bool animations_out =
        p_preset == PRESET_SEPARATE_ANIMATIONS
        || p_preset == PRESET_SEPARATE_MESHES_AND_ANIMATIONS
        || p_preset == PRESET_SEPARATE_MATERIALS_AND_ANIMATIONS
        || p_preset == PRESET_SEPARATE_MESHES_MATERIALS_AND_ANIMATIONS;
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(
            ...
            "nodes/storage",
            PROPERTY_HINT_ENUM,
            "Single Scene,Instanced Sub-Scenes"
        ),
        scenes_out ? 1 : 0
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "nodes/use_legacy_names"),
        true
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "materials/keep_on_reimport"),
        materials_out
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "meshes/octahedral_compression"),
        true
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
        ...
        VS::ARRAY_COMPRESS_DEFAULT >> VS::ARRAY_COMPRESS_BASE
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "meshes/ensure_tangents"),
        true
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
    ...
    r_options->push_back(
        ImportOption(PropertyInfo(Variant::BOOL, "skins/use_named_skins"), true)
    );
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "external_files/store_in_subdir"),
        false
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "animation/keep_custom_tracks"),
        animations_out
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
            Variant::BOOL,
            "animation/optimizer/enabled",
            PROPERTY_HINT_NONE,
            "",
            PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_UPDATE_ALL_IF_MODIFIED
        ),
    ...
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::REAL, "animation/optimizer/max_linear_error"),
        0.05
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::REAL, "animation/optimizer/max_angular_error"),
        0.01
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::REAL, "animation/optimizer/max_angle"),
        22
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "animation/optimizer/remove_unused_tracks"),
        true
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
            Variant::INT,
            "animation/clips/amount",
            PROPERTY_HINT_RANGE,
            "0,256,1",
            PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_UPDATE_ALL_IF_MODIFIED
        ),
    ...
    for (int i = 0; i < 256; i++) {
        ...
        r_options->push_back(ImportOption(
            PropertyInfo(
                Variant::INT,
                "animation/clip_" + itos(i + 1) + "/start_frame"
            ),
            0
        ));
        r_options->push_back(ImportOption(
            PropertyInfo(
                Variant::INT,
                "animation/clip_" + itos(i + 1) + "/end_frame"
            ),
            0
        ));
        r_options->push_back(ImportOption(
            PropertyInfo(
                Variant::BOOL,
                "animation/clip_" + itos(i + 1) + "/loops"
            ),
            false
        ));
    }
...
void ResourceImporterScene::_add_shapes(
    Node* p_node,
    const List<Ref<Shape>>& p_shapes
) {
    int idx = 0;
    for (const List<Ref<Shape>>::Element* E = p_shapes.front(); E;
         E                                  = E->next()) {
...
Error ResourceImporterScene::import(
    ...
    if (!bool(p_options["animation/optimizer/remove_unused_tracks"])) {
        import_flags |=
            EditorSceneImporter::IMPORT_ANIMATION_FORCE_ALL_TRACKS_IN_ALL_CLIPS;
    }
    ...
    uint32_t compress_flags = int(p_options["meshes/compress"])
                           << VS::ARRAY_COMPRESS_BASE;
    ...
    Node* scene = importer->import_scene(
        src_path,
        import_flags,
        fps,
        compress_flags,
    ...
    root_type =
        root_type.split(" ")[0]; // full root_type is "ClassName (filename.gd)"
                                 // for a script global class.

    Ref<Script> root_script = nullptr;
    if (ScriptServer::is_global_class(root_type)) {
        root_script =
            ResourceLoader::load(ScriptServer::get_global_class_path(root_type)
            );
    ...
    if (Object::cast_to<Spatial>(scene)) {
        ...
        Object::cast_to<Spatial>(scene)->scale(
            Vector3(root_scale, root_scale, root_scale)
        );
    }
    ...
    String animation_filter =
        String(p_options["animation/filter_script"]).strip_edges();

    bool use_optimizer = p_options["animation/optimizer/enabled"];
    float anim_optimizer_linerr =
        p_options["animation/optimizer/max_linear_error"];
    float anim_optimizer_angerr =
    ...
    scene =
        _fix_node(scene, scene, collision_map, LightBakeMode(light_bake_mode));

    if (use_optimizer) {
        _optimize_animations(
            scene,
            anim_optimizer_linerr,
            anim_optimizer_angerr,
            anim_optimizer_maxang
        );
    }
    ...
    Array animation_clips;
    {
        ...
        for (int i = 0; i < clip_count; i++) {
            ...
            int from_frame =
                p_options["animation/clip_" + itos(i + 1) + "/start_frame"];
            int end_frame =
                p_options["animation/clip_" + itos(i + 1) + "/end_frame"];
    ...
    if (animation_clips.size()) {
        _create_clips(
            scene,
            animation_clips,
            !bool(p_options["animation/optimizer/remove_unused_tracks"])
        );
    }
    ...
    bool external_animations = int(p_options["animation/storage"]) == 1
                            || int(p_options["animation/storage"]) == 2;
    ...
    bool external_materials = int(p_options["materials/storage"]) == 1
                           || int(p_options["materials/storage"]) == 2;
    bool external_materials_as_text = int(p_options["materials/storage"]) == 2;
    bool external_meshes            = int(p_options["meshes/storage"]) == 1
                        || int(p_options["meshes/storage"]) == 2;
    ...
    if (external_animations || external_materials || external_meshes
        ...
        if (bool(p_options["external_files/store_in_subdir"])) {
            ...
            memdelete(da);
            ERR_FAIL_COND_V_MSG(
                err2 != OK && err2 != ERR_ALREADY_EXISTS,
                err2,
                "Cannot make directory '" + subdir_name + "'."
            );
    ...
    if (light_bake_mode == 2 /* || generate LOD */) {
        ...
        if (FileAccess::exists(cache_file_path)) {
            Error err2;
            FileAccess* file =
                FileAccess::open(cache_file_path, FileAccess::READ, &err2);
        ...
        EditorProgress progress2(
            "gen_lightmaps",
            TTR("Generating Lightmaps"),
            meshes.size()
        );
        int step = 0;
        for (Map<Ref<ArrayMesh>, Transform>::Element* E = meshes.front(); E;
             E                                          = E->next()) {
            ...
            progress2.step(
                TTR("Generating for Mesh: ") + name + " (" + itos(step) + "/"
                    + itos(meshes.size()) + ")",
                step
            );
            ...
            Error err2          = mesh->lightmap_unwrap_cached(
                ret_cache_data,
                ret_cache_size,
                ret_used_cache,
                E->get(),
            ...
            if (err2 != OK) {
                EditorNode::add_io_error(
                    "Mesh '" + name
                    + "' failed lightmap generation. Please fix geometry."
                );
            } else {
                ...
                if (!ret_used_cache) {
                    // Cache was not used, add the generated entry to the
                    // current cache

                    unsigned int new_cache_size =
                        cache_size + ret_cache_size + (cache_size == 0 ? 4 : 0);
                    ...
                    memcpy(
                        &new_cache_data[cache_size / sizeof(int)],
                        ret_cache_data,
                        ret_cache_size
                    );
        ...
        Error err2;
        FileAccess* file =
            FileAccess::open(cache_file_path, FileAccess::WRITE, &err2);
    ...
    if (external_animations || external_materials || external_meshes) {
        ...
        _make_external_resources(
            ...
            base_path,
            external_animations,
            external_animations_as_text,
            keep_custom_tracks,
            external_materials,
            external_materials_as_text,
            keep_materials,
            external_meshes,
            external_meshes_as_text,
            anim_map,
            mat_map,
            mesh_map
        );
    ...
    if (post_import_script_path != "") {
        ...
        if (!scr.is_valid()) {
            EditorNode::add_io_error(
                TTR("Couldn't load post-import script:") + " "
                + post_import_script_path
            );
        } else {
            post_import_script =
                Ref<EditorScenePostImport>(memnew(EditorScenePostImport));
    ...
    if (post_import_script.is_valid()) {
        ...
        if (!scene) {
            EditorNode::add_io_error(
                TTR("Error running post-import script:") + " "
                + post_import_script_path + "\n"
                + TTR("Did you return a Node-derived object in the "
                      "`post_import()` method?")
            );
            return err;
        }
    ...
    if (external_scenes) {
        ...
        for (int i = 0; i < scene->get_child_count(); i++) {
            ...
            if (child->get_owner() != scene) {
                continue; // not a real child probably created by scene type
                          // (ig, a scrollbar)
            }
            _replace_owner(child, scene, child);

            String cn = String(child->get_name())
                            .strip_edges()
                            .replace(".", "_")
                            .replace(":", "_");
            if (cn == String()) {
            ...
            err = ResourceSaver::save(
                path,
                packer
            ); // do not take over, let the changed files reload themselves
            ERR_FAIL_COND_V_MSG(
                err != OK,
                err,
                "Cannot save scene to file '" + path + "'."
            );
        }
    }
    ...
    // this is not the time to reimport, wait until import process is done,
    // import file is saved, etc.
...
Node* EditorSceneImporterESCN::import_scene(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps,
    uint32_t p_compress_flags,
    ...
    Error error;
    Ref<PackedScene> ps =
        ResourceFormatLoaderText::singleton->load(p_path, p_path, &error);
    ERR_FAIL_COND_V_MSG(
        !ps.is_valid(),
        nullptr,
        "Cannot load scene as text resource from path '" + p_path + "'."
    );
...
Ref<Animation> EditorSceneImporterESCN::import_animation(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps
) {
