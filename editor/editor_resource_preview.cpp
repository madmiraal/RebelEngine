...
bool EditorResourcePreviewGenerator::handles(const String& p_type) const {
    ...
    ERR_FAIL_V_MSG(
        false,
        "EditorResourcePreviewGenerator::handles needs to be overridden."
    );
}

Ref<Texture> EditorResourcePreviewGenerator::generate(
    const RES& p_from,
    const Size2& p_size
) const {
    if (get_script_instance()
        && get_script_instance()->has_method("generate")) {
        return get_script_instance()->call("generate", p_from, p_size);
    }
    ERR_FAIL_V_MSG(
        Ref<Texture>(),
        "EditorResourcePreviewGenerator::generate needs to be overridden."
    );
}

Ref<Texture> EditorResourcePreviewGenerator::generate_from_path(
    const String& p_path,
    const Size2& p_size
) const {
    if (get_script_instance()
        && get_script_instance()->has_method("generate_from_path")) {
        return get_script_instance()
            ->call("generate_from_path", p_path, p_size);
    }
...
bool EditorResourcePreviewGenerator::can_generate_small_preview() const {
    if (get_script_instance()
        && get_script_instance()->has_method("can_generate_small_preview")) {
...
void EditorResourcePreviewGenerator::_bind_methods() {
    ...
    ClassDB::add_virtual_method(
        get_class_static(),
        MethodInfo(Variant::BOOL, "generate_small_preview_automatically")
    );
    ClassDB::add_virtual_method(
        get_class_static(),
        MethodInfo(Variant::BOOL, "can_generate_small_preview")
    );
}
...
void EditorResourcePreview::_preview_ready(
    const String& p_str,
    ...
    MessageQueue::get_singleton()
        ->push_call(id, p_func, path, p_texture, p_small_texture, p_ud);
}

void EditorResourcePreview::_generate_preview(
    ...
    for (int i = 0; i < preview_generators.size(); i++) {
        ...
        if (p_item.resource.is_valid()) {
            generated = preview_generators[i]->generate(
                p_item.resource,
                Vector2(thumbnail_size, thumbnail_size)
            );
        } else {
            generated = preview_generators[i]->generate_from_path(
                p_item.path,
                Vector2(thumbnail_size, thumbnail_size)
            );
        }
        ...
        if (preview_generators[i]->can_generate_small_preview()) {
            ...
            if (p_item.resource.is_valid()) {
                generated_small = preview_generators[i]->generate(
                    p_item.resource,
                    Vector2(small_thumbnail_size, small_thumbnail_size)
                );
            } else {
                generated_small = preview_generators[i]->generate_from_path(
                    p_item.path,
                    Vector2(small_thumbnail_size, small_thumbnail_size)
                );
            }
    ...
    if (!p_item.resource.is_valid()) {
        ...
        if (r_texture.is_valid()) {
            ...
            FileAccess* f =
                FileAccess::open(cache_base + ".txt", FileAccess::WRITE);
            ERR_FAIL_COND_MSG(
                !f,
                "Cannot create file '" + cache_base
                    + ".txt'. Check user write permissions."
            );
    ...
    while (!exit.is_set()) {
        ...
        if (queue.size()) {
            ...
            if (cache.has(item.path)) {
                // already has it because someone loaded it, just let it know
                // it's ready
                String path = item.path;
                if (item.resource.is_valid()) {
                    path += ":"
                          + itos(cache[item.path].last_hash
                          ); // keep last hash (see description of what this is
                             // in condition below)
                }

                _preview_ready(
                    path,
                    cache[item.path].preview,
                    cache[item.path].small_preview,
                    item.id,
                    item.function,
                    item.userdata
                );
                ...
                if (item.resource.is_valid()) {
                    ...
                    // adding hash to the end of path (should be
                    // ID:<objid>:<hash>) because of 5 argument limit to
                    // call_deferred
                    _preview_ready(
                        item.path + ":"
                            + itos(item.resource->hash_edited_version()),
                        texture,
                        small_texture,
                        item.id,
                        item.function,
                    ...
                    String cache_base = Global::ProjectSettings()
                                            .globalize_path(item.path)
                                            .md5_text();
                    ...
                    if (!f) {
                        ...
                        if (tsize != thumbnail_size) {
                            ...
                            if (last_md5 != md5) {
                                ...
                                if (!f) {
                                    // Not returning as this would leave the
                                    // thread hanging and would require some
                                    // proper cleanup/disabling of resource
                                    // preview generation.
                                    ERR_PRINT(
                        ...
                        if (cache_valid) {
                            ...
                            if (img->load(cache_base + ".png") != OK) {
                                ...
                                if (has_small_texture) {
                                    if (small_img->load(
                                            cache_base + "_small.png"
                                        )
                                        != OK) {
                                        cache_valid = false;
                                    } else {
                                        small_texture.instance();
                                        small_texture->create_from_image(
                                            small_img,
                                            Texture::FLAG_FILTER
                                        );
                                    }
...
void EditorResourcePreview::queue_edited_resource_preview(
    ...
    if (cache.has(path_id)
        && cache[path_id].last_hash == p_res->hash_edited_version()) {
        cache[path_id].order = order++;
        p_receiver->call(
            p_receiver_func,
            path_id,
            cache[path_id].preview,
            cache[path_id].small_preview,
            p_userdata
        );
        preview_mutex.unlock();
        return;
...
void EditorResourcePreview::queue_resource_preview(
    const String& p_path,
    Object* p_receiver,
    const StringName& p_receiver_func,
    const Variant& p_userdata
    ...
    if (cache.has(p_path)) {
        ...
        p_receiver->call(
            p_receiver_func,
            p_path,
            cache[p_path].preview,
            cache[p_path].small_preview,
            p_userdata
        );
        preview_mutex.unlock();
        return;
...
void EditorResourcePreview::_bind_methods() {
    ClassDB::bind_method(
        "_preview_ready",
        &EditorResourcePreview::_preview_ready
    );
    ...
    ClassDB::bind_method(
        D_METHOD("add_preview_generator", "generator"),
        &EditorResourcePreview::add_preview_generator
    );
    ClassDB::bind_method(
        D_METHOD("remove_preview_generator", "generator"),
        &EditorResourcePreview::remove_preview_generator
    );
    ClassDB::bind_method(
        D_METHOD("check_for_invalidation", "path"),
        &EditorResourcePreview::check_for_invalidation
    );

    ADD_SIGNAL(
        MethodInfo("preview_invalidated", PropertyInfo(Variant::STRING, "path"))
    );
}
...
void EditorResourcePreview::stop() {
    if (thread.is_started()) {
        ...
        while (!exited.is_set()) {
            ...
            ); // sync pending stuff, as thread may be blocked on visual server
        }
