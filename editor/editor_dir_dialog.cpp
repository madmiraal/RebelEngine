...
void EditorDirDialog::_update_dir(
    TreeItem* p_item,
    EditorDirectory* p_dir,
    const String& p_select_path
) {
    updating = true;
    ...
    if (!p_item->get_parent()) {
        ...
        if (!opened_paths.has(path)
            && (p_select_path == String() || !p_select_path.begins_with(path)
            )) {
...
void EditorDirDialog::_notification(int p_what) {
    if (p_what == NOTIFICATION_ENTER_TREE) {
        EditorFileSystem::get_singleton()
            ->connect("filesystem_changed", this, "reload");
        reload();
        ...
        if (!EditorFileSystem::get_singleton()
                 ->is_connected("filesystem_changed", this, "reload")) {
            EditorFileSystem::get_singleton()
                ->connect("filesystem_changed", this, "reload");
        }
    }
    ...
    if (p_what == NOTIFICATION_EXIT_TREE) {
        if (EditorFileSystem::get_singleton()
                ->is_connected("filesystem_changed", this, "reload")) {
            EditorFileSystem::get_singleton()
                ->disconnect("filesystem_changed", this, "reload");
        }
    }
...
void EditorDirDialog::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_item_collapsed"),
        &EditorDirDialog::_item_collapsed
    );
    ClassDB::bind_method(D_METHOD("_make_dir"), &EditorDirDialog::_make_dir);
    ClassDB::bind_method(
        D_METHOD("_make_dir_confirm"),
        &EditorDirDialog::_make_dir_confirm
    );
    ClassDB::bind_method(
        D_METHOD("reload"),
        &EditorDirDialog::reload,
        DEFVAL("")
    );
