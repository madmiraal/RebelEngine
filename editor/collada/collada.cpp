...
String Collada::Effect::get_texture_path(const String& p_source, Collada& state)
    const {
...
Transform Collada::get_root_transform() const {
    ...
    unit_scale_transform.scale(
        Vector3(state.unit_scale, state.unit_scale, state.unit_scale)
    );
#endif
...
static Transform _read_transform_from_array(
    const Vector<float>& array,
    int ofs = 0
) {
    Transform tr;
    // i wonder why collada matrices are transposed, given that's opposed to
    // opengl..
    ...
    for (int i = 0; i < xform_list.size(); i++) {
        ...
        switch (xf.op) {
            case XForm::OP_ROTATE: {
                if (xf.data.size() >= 4) {
                    xform_step.rotate(
                        Vector3(xf.data[0], xf.data[1], xf.data[2]),
                        Math::deg2rad(xf.data[3])
                    );
                }
            } break;
            case XForm::OP_SCALE: {
...
Vector<float> Collada::AnimationTrack::get_value_at_time(float p_time) const {
    ...
    switch (keys[i].interp_type) {
        ...
        case INTERP_LINEAR: {
            float c =
                (p_time - keys[i - 1].time) / (keys[i].time - keys[i - 1].time);
            ...
            if (keys[i].data.size() == 16) {
                ...
                Transform interp =
                    c < 0.001 ? src : src.interpolate_with(dst, c);

                Vector<float> ret;
                ret.resize(16);
                Transform tr;
                // i wonder why collada matrices are transposed, given that's
                // opposed to opengl..
                ...
                for (int j = 0; j < dest.size(); j++) {
                    dest.write[j] =
                        keys[i].data[j] * c + keys[i - 1].data[j] * (1.0 - c);
                }
                return dest;
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (name == "up_axis") {
                ...
                state.unit_scale =
                    parser.get_attribute_value("meter").to_double();
...
void Collada::_parse_image(XMLParser& parser) {
    ...
    if (state.version < State::Version(1, 4, 0)) {
        ...
        if (path.find("://") == -1 && path.is_rel_path()) {
            // path is relative to file being loaded, so convert to a resource
            // path
            image.path = Global::ProjectSettings().localize_path(
                state.local_path.get_base_dir().plus_file(path.percent_decode())
            );
        }
    } else {
        while (parser.read() == OK) {
            if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
                ...
                if (name == "init_from") {
                    parser.read();
                    String path =
                        parser.get_node_data().strip_edges().percent_decode();

                    if (path.find("://") == -1 && path.is_rel_path()) {
                        // path is relative to file being loaded, so convert to
                        // a resource path
                        path = Global::ProjectSettings().localize_path(
                            state.local_path.get_base_dir().plus_file(path)
                        );

                    } else if (path.find("file:///") == 0) {
                        path = path.replace_first("file:///", "");
                        path = Global::ProjectSettings().localize_path(path);
                    }
...
void Collada::_parse_effect_material(
    XMLParser& parser,
    Effect& effect,
    String& id
) {
            ...
            if (!parser.is_empty()
                ...
                while (parser.read() == OK) {
                    if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
                        ...
                        if (what == "emission" || what == "diffuse"
                            || what == "specular" || what == "reflective") {
                            // color or texture types
                            while (parser.read() == OK) {
                                if (parser.get_node_type()
                                    == XMLParser::NODE_ELEMENT) {
                                    if (parser.get_node_name() == "color") {
                                        Vector<float> colorarr =
                                            _read_float_array(parser);
                                        COLLADA_PRINT(
                                            "colorarr size: "
                                            + rtos(colorarr.size())
                                        );
                                        ...
                                        if (colorarr.size() >= 3) {
                                            // alpha strangely not alright?
                                            // maybe it needs to be multiplied
                                            // by value as a channel intensity
                                            Color color(
                                                colorarr[0],
                                                colorarr[1],
                                                colorarr[2],
                ...
                COLLADA_PRINT(
                    "double sided: " + itos(parser.get_node_data().to_int())
                );
                        ...
                        if (parser.get_node_name() == "texture") {
                            ...
                            if (!effect.params.has(sampler)) {
                                ...
                                if (!effect.params.has(surface)) {
                                    ...
                                    if (parser.has_attribute("bumptype")
                                        && parser.get_attribute_value("bumptype"
                                           ) != "NORMALMAP") {
                                        WARN_PRINT(
                                            "'bump' texture type is not "
                                            "NORMALMAP, only NORMALMAP is "
                                            "supported."
                                        );
                                    }
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (name == "ambient") {
                ...
                if (colorarr.size() >= 4) {
                    // alpha strangely not alright? maybe it needs to be
                    // multiplied by value as a channel intensity
        ...
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (section == "source") {
                ...
                if (curvedata.sources.has(current_source)) {
                    curvedata.sources[current_source].array =
                        _read_float_array(parser);
                    COLLADA_PRINT(
                        "section: " + current_source + " read "
                        + itos(curvedata.sources[current_source].array.size())
                        + " values."
                    );
                }
                ...
                // create a new array and read it.
                if (curvedata.sources.has(current_source)) {
                    curvedata.sources[current_source].sarray =
                        _read_string_array(parser);
                    COLLADA_PRINT(
                        "section: " + current_source + " read "
                        + itos(curvedata.sources[current_source].array.size())
                        + " values."
                ...
                // skip it
            } else if (section == "accessor") { // child of source (below a
                                                // technique tag)

                if (curvedata.sources.has(current_source)) {
                    curvedata.sources[current_source].stride =
                        parser.get_attribute_value("stride").to_int();
                    COLLADA_PRINT(
                        "section: " + current_source + " stride "
                        + itos(curvedata.sources[current_source].stride)
                    );
                }
                ...
                while (parser.read() == OK) {
                    if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
                        if (parser.get_node_name() == "input") {
                            String semantic =
                                parser.get_attribute_value("semantic");
                            String source =
                                _uri_to_id(parser.get_attribute_value("source")
                                );

                            curvedata.control_vertices[semantic] = source;

                            COLLADA_PRINT(
                                section + " input semantic: " + semantic
                                + " source: " + source
                            );
                        }
...
void Collada::_parse_mesh_geometry(
    XMLParser& parser,
    String p_id,
    String p_name
) {
        ...
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (section == "source") {
                ...
                if (meshdata.sources.has(current_source)) {
                    meshdata.sources[current_source].array =
                        _read_float_array(parser);
                    COLLADA_PRINT(
                        "section: " + current_source + " read "
                        + itos(meshdata.sources[current_source].array.size())
                        + " values."
                    );
                }
            } else if (section == "technique_common") {
                ...
                if (meshdata.sources.has(current_source)) {
                    meshdata.sources[current_source].stride =
                        parser.get_attribute_value("stride").to_int();
                    COLLADA_PRINT(
                        "section: " + current_source + " stride "
                        + itos(meshdata.sources[current_source].stride)
                    );
                }
                ...
                if (polygons) {
                    WARN_PRINT(
                        "Primitive type \"polygons\" is not well supported "
                        "(concave shapes may fail). To ensure that the "
                        "geometry is properly imported, please re-export using "
                ...
                while (parser.read() == OK) {
                    if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
                        if (parser.get_node_name() == "input") {
                            ...
                            int offset =
                                parser.get_attribute_value("offset").to_int();
                            ...
                            if (polygons) {
                                ...
                                prim.polygons.push_back(
                                    values.size() / prim.vertex_size
                                );
                ...
                ERR_PRINT(
                    "Primitive type \"polygons\" not supported, re-export "
                    "using \"polylist\" or \"triangles\"."
                );
            } else if (!parser.is_empty()) {
                parser.skip_section();
            }
...
void Collada::_parse_skin_controller(XMLParser& parser, String p_id) {
        ...
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (section == "bind_shape_matrix") {
...
#endif
                COLLADA_PRINT(
                    "skeleton bind shape transform: " + skindata.bind_shape
                );
                ...
                if (skindata.sources.has(current_source)) {
                    skindata.sources[current_source].array =
                        _read_float_array(parser);
                    COLLADA_PRINT(
                        "section: " + current_source + " read "
                        + itos(skindata.sources[current_source].array.size())
                        + " values."
                    );
                }
                ...
                if (skindata.sources.has(current_source)) {
                    skindata.sources[current_source].sarray =
                        _read_string_array(parser);
                    if (section == "IDREF_array") {
                        Vector<String> sa =
                            skindata.sources[current_source].sarray;
                ...
                if (skindata.sources.has(current_source)) {
                    ...
                    COLLADA_PRINT(
                        "section: " + current_source + " stride "
                        + itos(skindata.sources[current_source].stride)
                    );
                }
    ...
    for (int i = 0; i < joint_source.sarray.size(); i++) {
        ...
        Transform xform = _read_transform_from_array(
            ibm_source.array,
            i * 16
        ); //<- this is a mistake, it must be applied to vertices
        ...
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (section == "source") {
                ...
                if (morphdata.sources.has(current_source)) {
                    morphdata.sources[current_source].array =
                        _read_float_array(parser);
                    COLLADA_PRINT(
                        "section: " + current_source + " read "
                        + itos(morphdata.sources[current_source].array.size())
                        + " values."
                    );
                }
            } else if (section == "Name_array" || section == "IDREF_array") {
                ...
                */
                if (morphdata.sources.has(current_source)) {
                    morphdata.sources[current_source].sarray =
                        _read_string_array(parser);
                    /*
                    if (section=="IDREF_array") {
                        Vector<String> sa =
                    morphdata.sources[current_source].sarray; for(int
                    i=0;i<sa.size();i++) state.idref_joints.insert(sa[i]);
                    }*/
                    COLLADA_PRINT(
                        "section: " + current_source + " read "
                        + itos(morphdata.sources[current_source].array.size())
                ...
                if (morphdata.sources.has(current_source)) {
                    ...
                    COLLADA_PRINT(
                        "section: " + current_source + " stride "
                        + itos(morphdata.sources[current_source].stride)
                    );
                }
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            if (parser.get_node_name() == "instance_material") {
                ...
                String target =
                    _uri_to_id(parser.get_attribute_value("target"));

                NodeGeometry::Material mat;
                mat.target                 = target;
                geom->material_map[symbol] = mat;
                COLLADA_PRINT(
                    "uses material: '" + target + "' on primitive'" + symbol
                    + "'"
                );
    ...
    if (geom->controller) {
        if (geom->skeletons.empty()) {
            ...
            if (state.skin_controller_data_map.has(geom->source)) {
                SkinControllerData* skin =
                    &state.skin_controller_data_map[geom->source];
                ...
                Collada::SkinControllerData::Source& joint_source =
                    skin->sources[joint_arr];
                geom->skeletons =
                    joint_source.sarray; // quite crazy, but should work.
            }
        }
    }
...
Collada::Node* Collada::_parse_visual_instance_camera(XMLParser& parser) {
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT_END
            && parser.get_node_name() == "instance_camera") {
            break;
        }
    }
...
Collada::Node* Collada::_parse_visual_instance_light(XMLParser& parser) {
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT_END
            && parser.get_node_name() == "instance_light") {
            break;
        }
    }
...
Collada::Node* Collada::_parse_visual_node_instance_data(XMLParser& parser) {
    ...
    if (instance_type == "instance_geometry"
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT_END
            && parser.get_node_name() == instance_type) {
            break;
        }
    }
...
Collada::Node* Collada::_parse_visual_scene_node(XMLParser& parser) {
    ...
    name = parser.has_attribute("name")
             ? parser.get_attribute_value_safe("name")
             : parser.get_attribute_value_safe("id");
    if (name == "") {
        name = id;
    } else {
        found_name = true;
    ...
    if ((parser.has_attribute("type")
         && parser.get_attribute_value("type") == "JOINT")
        || state.idref_joints.has(name)) {
        // handle a bone
            ...
            if (name == "source") {
                ...
                if (parser.has_attribute("name")) {
                    source_param_names[current_source].push_back(
                        parser.get_attribute_value("name")
                    );
                } else {
                    source_param_names[current_source].push_back("");
                }
                ...
                if (parser.has_attribute("type")) {
                    source_param_types[current_source].push_back(
                        parser.get_attribute_value("type")
                    );
                } else {
                    source_param_types[current_source].push_back("");
                }
                ...
                if (current_sampler != "") {
                    samplers[current_sampler]
                            [parser.get_attribute_value("semantic")] =
                                parser.get_attribute_value("source");
                }
    ...
    for (int i = 0; i < channel_sources.size(); i++) {
        ...
        for (int l = 0; l < names.size(); l++) {
            ...
            for (int j = 0; j < key_count; j++) {
                ...
                state.animation_length =
                    MAX(state.animation_length, time_keys[j]);
            }
            ...
            ERR_CONTINUE_MSG(
                (output.size() / stride) != key_count,
                "Wrong number of keys in output."
            );

            for (int j = 0; j < key_count; j++) {
                track.keys.write[j].data.resize(output_len);
                for (int k = 0; k < output_len; k++) {
                    track.keys.write[j].data.write[k] =
                        output[l + j * stride + k]; // super weird but should
                                                    // work:
                }
            }
            ...
            if (sampler.has("INTERPOLATION")) {
                ...
                for (int j = 0; j < key_count; j++) {
                    if (interps[j] == "BEZIER") {
                        track.keys.write[j].interp_type =
                            AnimationTrack::INTERP_BEZIER;
                    } else {
                        track.keys.write[j].interp_type =
                            AnimationTrack::INTERP_LINEAR;
                    }
                }
            }
            ...
            if (sampler.has("IN_TANGENT") && sampler.has("OUT_TANGENT")) {
                ...
                for (int j = 0; j < key_count; j++) {
                    track.keys.write[j].in_tangent = Vector2(
                        intangents[j * 2 * names.size() + 0 + l * 2],
                        intangents[j * 2 * names.size() + 1 + l * 2]
                    );
                    track.keys.write[j].out_tangent = Vector2(
                        outangents[j * 2 * names.size() + 0 + l * 2],
                        outangents[j * 2 * names.size() + 1 + l * 2]
                    );
                }
            }
            ...
            if (target.find("/") != -1) { // transform component
                ...
                if (names.size() > 1 && track.component == "") {
                    // this is a guess because the collada spec is ambiguous
                    // here... i suppose if you have many names (outputs) you
                    // can't use a component and i should abide to that.
            ...
            state.referenced_tracks[target].push_back(
                state.animation_tracks.size() - 1
            );
            ...
            if (id != "") {
                ...
                state.by_id_tracks[id].push_back(
                    state.animation_tracks.size() - 1
                );
            }
    ...
    while (parser.read() == OK) {
        if (parser.get_node_type() == XMLParser::NODE_ELEMENT) {
            ...
            if (name == "instance_visual_scene") {
                state.root_visual_scene =
                    _uri_to_id(parser.get_attribute_value("url"));
            } else if (name == "instance_physics_scene") {
                state.root_physics_scene =
                    _uri_to_id(parser.get_attribute_value("url"));
            }
...
void Collada::_create_skeletons(
    Collada::Node** p_node,
    NodeSkeleton* p_skeleton
) {
    Node* node = *p_node;
    ...
    if (p_node->type == Node::TYPE_GEOMETRY) {
        ...
        if (gnode->controller) {
            ...
            for (int i = 0; i < gnode->skeletons.size(); i++) {
                ...
                ERR_CONTINUE(!state.scene_map.has(nodeid)
                ); // weird, it should have it...

                NodeJoint* nj = SAFE_CAST<NodeJoint*>(state.scene_map[nodeid]);
                ERR_CONTINUE(!nj); // broken collada
                ERR_CONTINUE(!nj->owner
                ); // weird, node should have a skeleton owner
...
void Collada::_merge_skeletons2(VisualScene* p_vscene) {
    for (Map<String, SkinControllerData>::Element* E =
             state.skin_controller_data_map.front();
         E;
         E = E->next()) {
        ...
        for (Map<String, Transform>::Element* F = cd.bone_rest_map.front(); F;
             F                                  = F->next()) {
            String name;
...
bool Collada::_optimize_skeletons(VisualScene* p_vscene, Node* p_node) {
    ...
    if (node->type == Node::TYPE_SKELETON && node->parent
        && node->parent->type == Node::TYPE_NODE
        && node->parent->children.size() == 1) {
        // replace parent by this...
        Node* parent = node->parent;

        // i wonder if this is alright.. i think it is since created skeleton
        // (first joint) is already animated by bone..
        node->id                = parent->id;
...
bool Collada::_move_geometry_to_skeletons(
    VisualScene* p_vscene,
    Node* p_node,
    List<Node*>* p_mgeom
) {
    // Bind Shape Matrix scales the bones and makes them gigantic, so the matrix
    // then shrinks the model? Solution: apply the Bind Shape Matrix to the
    // VERTICES, and if the object comes scaled, it seems to be left alone!
    ...
    if (p_node->type == Node::TYPE_GEOMETRY) {
        ...
        if (ng->controller && ng->skeletons.size()) {
            ...
            ERR_FAIL_COND_V(
                !state.skin_controller_data_map.has(ng->source),
                false
            );
            SkinControllerData& skin =
                state.skin_controller_data_map[ng->source];
            Transform skel_inv = sk->get_global_transform().affine_inverse();
            p_node->default_transform =
                skel_inv
                * (skin.bind_shape /* p_node->get_global_transform()*/
                ); // i honestly have no idea what to do with a previous model
                   // xform.. most exporters ignore it

            // make rests relative to the skeleton (they seem to be always
            // relative to world)
            for (Map<String, Transform>::Element* E =
                     skin.bone_rest_map.front();
                 E;
                 E = E->next()) {
                E->get() = skel_inv
                         * E->get(); // make the bone rest local to the skeleton
                state.bone_rest_map[E->key()] =
                    E->get(); // make it remember where the bone is globally,
                              // now that it's relative
            }

            // but most exporters seem to work only if i do this..
            // p_node->default_transform = p_node->get_global_transform();

            // p_node->default_transform=Transform(); //this seems to be
            // correct, because bind shape makes the object local to the
            // skeleton
            p_node->ignore_anim =
                true; // collada may animate this later, if it does, then this
                      // is not supported (redo your original asset and don't
                      // animate the base mesh)
            p_node->parent = sk;
    ...
    for (int i = 0; i < p_node->children.size(); i++) {
        if (_move_geometry_to_skeletons(
                p_vscene,
                p_node->children[i],
                p_mgeom
            )) {
        ...
        if (nj->controller) {
            ...
            while (base != "" && !state.mesh_data_map.has(base)) {
                if (state.skin_controller_data_map.has(base)) {
                    SkinControllerData& sk =
                        state.skin_controller_data_map[base];
                    base = sk.base;
...
void Collada::_optimize() {
    for (Map<String, VisualScene>::Element* E = state.visual_scene_map.front();
         E;
         E = E->next()) {
...
Error Collada::load(const String& p_path, int p_flags) {
    ...
    state.local_path   = Global::ProjectSettings().localize_path(p_path);
