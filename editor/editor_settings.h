...
private:
    ...
    bool optimize_save; // do not save stuff that came from config but was not
                        // set from engine
...
public:
    ...
    Vector<String> get_script_templates(
        ...
        const String& p_custom_path = String()
    );
    String get_editor_layouts_config() const;
    float get_auto_display_scale() const;

    void add_shortcut(const String& p_name, Ref<ShortCut>& p_shortcut);
    bool is_shortcut(const String& p_name, const Ref<InputEvent>& p_event)
        const;
...
#define ED_IS_SHORTCUT(p_name, p_ev)                                           \
    (EditorSettings::get_singleton()->is_shortcut(p_name, p_ev))
Ref<ShortCut> ED_SHORTCUT(
    const String& p_path,
    const String& p_name,
    uint32_t p_keycode = 0
);
