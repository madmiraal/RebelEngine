...
namespace {
bool is_using_fat32_or_exfat() {
    DirAccess* dir_access = DirAccess::create(DirAccess::ACCESS_RESOURCES);
    bool using_fat32      = dir_access->get_filesystem_type() == "FAT32";
    bool using_exfat      = dir_access->get_filesystem_type() == "exFAT";
    memdelete(dir_access);
    return using_fat32 || using_exfat;
}
} // namespace
...
EditorFileSystem::EditorFileSystem() {
    ...
    using_fat32_or_exfat   = is_using_fat32_or_exfat();
...
EditorDirectory* EditorFileSystem::get_filesystem_path(const String& path) {
    ...
    resource_path = resource_path.substr(6, resource_path.length());
    resource_path = resource_path.replace("\\", "/");
    if (resource_path.ends_with("/")) {
        resource_path = resource_path.substr(0, resource_path.length() - 1);
    }
    Vector<String> folder_names = resource_path.split("/");

    EditorDirectory* directory = root_directory;
    for (int folder_name_index = 0; folder_name_index < folder_names.size();
         folder_name_index++) {
        String folder_name           = folder_names[folder_name_index];
        int found_subdirectory_index = -1;
        for (int subdirectory_index = 0;
             subdirectory_index < directory->get_subdir_count();
             subdirectory_index++) {
            if (directory->get_subdir(subdirectory_index)->get_name()
                == folder_name) {
                found_subdirectory_index = subdirectory_index;
                break;
            }
        }
        if (found_subdirectory_index == -1) {
            return nullptr;
        }
        directory = directory->get_subdir(found_subdirectory_index);
    }
    return directory;
}
...
void EditorFileSystem::_thread_func(void* userdata) {
    EditorFileSystem* sd = (EditorFileSystem*)userdata;
    sd->_scan_filesystem();
}

void EditorFileSystem::_thread_func_sources(void* userdata) {
    EditorFileSystem* efs = (EditorFileSystem*)userdata;
