...
// TODO Copied from TextEdit private, would be nice to extract it in a single
// place
...
static bool find_next(
    ...
    while (true) {
        int begin =
            match_case ? line.find(pattern, end) : line.findn(pattern, end);
...
void FindInFiles::_iterate() {
    if (_folders_stack.size() != 0) {
        // Scan folders first so we can build a list of files and have progress
        // info later

        PoolStringArray& folders_to_scan =
            _folders_stack.write[_folders_stack.size() - 1];
...
float FindInFiles::get_progress() const {
    if (_initial_files_count != 0) {
        return static_cast<float>(_initial_files_count - _files_to_scan.size())
             / static_cast<float>(_initial_files_count);
    }
    return 0;
}
...
void FindInFiles::_scan_dir(String path, PoolStringArray& out_folders) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        // If there is a .gdignore file in the directory, don't bother searching
        // it
        if (file == ".gdignore") {
            break;
        }
...
FindInFilesDialog::FindInFilesDialog() {
    ...
    _search_text_line_edit
        ->connect("text_changed", this, "_on_search_text_modified");
    _search_text_line_edit
        ->connect("text_entered", this, "_on_search_text_entered");
    ...
    filter_label->set_tooltip(
        TTR("Include the files with the following extensions. Add or remove "
            "them in ProjectSettings.")
    );
...
Set<String> FindInFilesDialog::get_filter() const {
    // could check the _filters_preferences but it might not have been generated
    // yet.
    Set<String> filters;
...
void FindInFilesDialog::_notification(int p_what) {
    if (p_what == NOTIFICATION_VISIBILITY_CHANGED) {
        if (is_visible()) {
            ...
            // Extensions might have changed in the meantime, we clean them and
            // instance them again.
            for (int i = 0; i < _filters_container->get_child_count(); i++) {
...
void FindInFilesDialog::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_on_folder_selected",
        &FindInFilesDialog::_on_folder_selected
    );
    ClassDB::bind_method(
        "_on_search_text_modified",
        &FindInFilesDialog::_on_search_text_modified
    );
    ClassDB::bind_method(
        "_on_search_text_entered",
        &FindInFilesDialog::_on_search_text_entered
    );
...
FindInFilesPanel::FindInFilesPanel() {
    ...
    _finder
        ->connect(FindInFiles::SIGNAL_RESULT_FOUND, this, "_on_result_found");
...

    {
        ...
        _search_text_label->add_font_override(
            "font",
    ...
    _results_display->add_font_override(
        "font",
        EditorNode::get_singleton()->get_gui_base()->get_font(
            "source",
            "EditorFonts"
...

    {
        ...
        _replace_line_edit
            ->connect("text_changed", this, "_on_replace_text_changed");
        ...
        _replace_all_button
            ->connect("pressed", this, "_on_replace_all_clicked");
...
void FindInFilesPanel::_on_result_found(
    String fpath,
    int line_number,
    int begin,
    int end,
    ...
    if (E == nullptr) {
        ...
        // The width of this column is restrained to checkboxes, but that
        // doesn't make sense for the parent items, so we override their width
        // so they can expand to full width
...
void FindInFilesPanel::draw_result_text(Object* item_obj, Rect2 rect) {
    ...
    _results_display->draw_rect(
        match_rect,
        get_color("accent_color", "Editor").inverted() * Color(1, 1, 1, 0.5)
    );
...
void FindInFilesPanel::_on_finished() {
    ...
    if (result_count == 1 && file_count == 1) {
        results_text =
            vformat(TTR("%d match in %d file."), result_count, file_count);
    } else if (result_count != 1 && file_count == 1) {
        results_text =
            vformat(TTR("%d matches in %d file."), result_count, file_count);
    } else {
        results_text =
            vformat(TTR("%d matches in %d files."), result_count, file_count);
    }
...
void FindInFilesPanel::_on_replace_all_clicked() {
    ...
    // Hide replace bar so we can't trigger the action twice without doing a new
    // search
    _replace_container->hide();
...
void FindInFilesPanel::apply_replaces_in_file(
    String fpath,
    const Vector<Result>& locations,
    String new_text
) {
    ...
    for (int i = 0; i < locations.size(); ++i) {
        ...
        if (!find_next(
                ...
                search_text,
                repl_begin,
                _finder->is_match_case(),
                _finder->is_whole_words(),
                _,
                _
            )) {
            // Make sure the replace is still valid in case the file was
            // tampered with.
            print_verbose(
                String("Occurrence no longer matches, replace will be ignored "
                       "in {0}: line {1}, col {2}")
                    .format(varray(fpath, repl_line_number, repl_begin))
            );
            continue;
        }
    ...
    // Now the modified contents are in the buffer, rewrite the file with our
    // changes
...
void FindInFilesPanel::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &FindInFilesPanel::_on_replace_text_changed
    );
    ClassDB::bind_method(
        "_on_replace_all_clicked",
        &FindInFilesPanel::_on_replace_all_clicked
    );
    ClassDB::bind_method(
        "_draw_result_text",
        &FindInFilesPanel::draw_result_text
    );

    ADD_SIGNAL(MethodInfo(
        SIGNAL_RESULT_SELECTED,
        PropertyInfo(Variant::STRING, "path"),
        PropertyInfo(Variant::INT, "line_number"),
        PropertyInfo(Variant::INT, "begin"),
        PropertyInfo(Variant::INT, "end")
    ...
    ADD_SIGNAL(MethodInfo(
        SIGNAL_FILES_MODIFIED,
        PropertyInfo(Variant::STRING, "paths")
    ));
}
