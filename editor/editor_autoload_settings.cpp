...
void EditorAutoloadSettings::_notification(int p_what) {
    if (p_what == NOTIFICATION_ENTER_TREE) {
        ...
        for (List<AutoLoadInfo>::Element* E = autoload_cache.front(); E;
             E                              = E->next()) {
...
bool EditorAutoloadSettings::_autoload_name_is_valid(
    const String& p_name,
    String* r_error
) {
    if (!p_name.is_valid_identifier()) {
        if (r_error) {
            *r_error = TTR("Invalid name.") + "\n" + TTR("Valid characters:")
                     + " a-z, A-Z, 0-9 or _";
        }
        ...
        if (Variant::get_type_name(Variant::Type(i)) == p_name) {
            if (r_error) {
                *r_error =
                    TTR("Invalid name.") + "\n"
                    + TTR(
                        "Must not collide with an existing built-in type name."
                    );
            }
        ...
        if (GlobalConstants::get_global_constant_name(i) == p_name) {
            if (r_error) {
                *r_error =
                    TTR("Invalid name.") + "\n"
                    + TTR(
                        "Must not collide with an existing global constant "
                        "name."
                    );
            }
...
void EditorAutoloadSettings::_autoload_add() {
    if (autoload_add(
            ...
            autoload_add_path->get_line_edit()->get_text()
        )) {
...
void EditorAutoloadSettings::_autoload_edited() {
    ...
    if (column == 0) {
        ...
        if (Global::ProjectSettings().has_setting("autoload/" + name)) {
            ti->set_text(0, old_name);
            EditorNode::get_singleton()->show_warning(
                vformat(TTR("Autoload '%s' already exists!"), name)
            );
            return;
        }
        ...
        int order   = Global::ProjectSettings().get_order(selected_autoload);
        String path = GLOBAL_GET(selected_autoload);

        undo_redo->create_action(TTR("Rename Autoload"));

        undo_redo->add_do_property(&(Global::ProjectSettings()), name, path);
        ...
        int order   = Global::ProjectSettings().get_order(base);
        String path = GLOBAL_GET(base);
        ...
        undo_redo->add_do_property(&(Global::ProjectSettings()), base, path);
        undo_redo->add_undo_property(
            &(Global::ProjectSettings()),
            base,
            GLOBAL_GET(base)
...
void EditorAutoloadSettings::_autoload_button_pressed(
    Object* p_item,
    int p_column,
    int p_button
) {
    ...
    switch (p_button) {
        ...
        case BUTTON_MOVE_DOWN: {
            ...
            int swap_order = Global::ProjectSettings().get_order(swap_name);
...
void EditorAutoloadSettings::_autoload_file_callback(const String& p_path) {
    ...
    // If the name collides with a built-in class, prefix the name to make it
    // possible to add without having to edit the name. The prefix is
    // subjective, but it provides better UX than leaving the Add button
    // disabled :)
...
void EditorAutoloadSettings::_autoload_text_entered(const String p_name) {
    if (autoload_add_path->get_line_edit()->get_text() != ""
        && _autoload_name_is_valid(p_name, nullptr)) {
        _autoload_add();
    }
}
...
void EditorAutoloadSettings::_autoload_path_text_changed(const String p_path) {
    add_autoload->set_disabled(
        p_path == ""
        || !_autoload_name_is_valid(autoload_add_name->get_text(), nullptr)
    );
}
...
Node* EditorAutoloadSettings::_create_autoload(const String& p_path) {
    ...
    if (res->is_class("PackedScene")) {
        ...
        ERR_FAIL_COND_V_MSG(
            !valid_type,
            nullptr,
            "Script does not inherit a Node: " + p_path + "."
        );
        ...
        ERR_FAIL_COND_V_MSG(
            obj == nullptr,
            nullptr,
            "Cannot instance script for autoload, expected 'Node' inheritance, "
            "got: "
                + String(ibt) + "."
        );
    ...
    ERR_FAIL_COND_V_MSG(
        !n,
        nullptr,
        "Path in autoload not a node or script: " + p_path + "."
    );
...
void EditorAutoloadSettings::update_autoload() {
    ...
    for (List<PropertyInfo>::Element* E = props.front(); E; E = E->next()) {
        ...
        String path = GLOBAL_GET(pi.name);
        ...
        info.name  = name;
        info.path  = path;
        info.order = Global::ProjectSettings().get_order(pi.name);
        ...
        if (to_remove.has(name)) {
            ...
            if (old_info.path == info.path) {
                ...
                if (info.node) {
                    ...
                    if (info.is_singleton == old_info.is_singleton
                        && info.in_editor == old_info.in_editor) {
    ...
    for (Map<String, AutoLoadInfo>::Element* E = to_remove.front(); E;
         E                                     = E->next()) {
        AutoLoadInfo& info = E->get();
        if (info.is_singleton) {
            for (int i = 0; i < ScriptServer::get_language_count(); i++) {
                ScriptServer::get_language(i)->remove_named_global_constant(
                    info.name
                );
            }
        }
    ...
    for (List<AutoLoadInfo*>::Element* E = to_add.front(); E; E = E->next()) {
        ...
        if (info->is_singleton) {
            for (int i = 0; i < ScriptServer::get_language_count(); i++) {
                ScriptServer::get_language(i)->add_named_global_constant(
                    info->name,
                    info->node
                );
            }
...
Variant EditorAutoloadSettings::get_drag_data_fw(
    ...
    for (int i = 0; i < max_size; i++) {
        ...
        label->set_self_modulate(
            Color(1, 1, 1, Math::lerp(1, 0, float(i) / PREVIEW_LIST_MAX_SIZE))
        );
...
void EditorAutoloadSettings::drop_data_fw(
    const Point2& p_point,
    const Variant& p_data,
    Control* p_control
) {
    ...
    int order = Global::ProjectSettings().get_order("autoload/" + name);
    ...
    for (int i = 0; i < autoloads.size(); i++) {
        aux.order =
            Global::ProjectSettings().get_order("autoload/" + autoloads[i]);
    ...
    for (List<AutoLoadInfo>::Element* F = autoload_cache.front(); F;
         F                              = F->next()) {
...
bool EditorAutoloadSettings::autoload_add(
    const String& p_name,
    const String& p_path
) {
    String name = p_name;
    ...
    if (!path.begins_with("res://")) {
        EditorNode::get_singleton()->show_warning(
            TTR("Can't add autoload:") + "\n"
            + vformat(
                TTR("%s is an invalid path. Not in resource path (res://)."),
                path
            )
        );
        return false;
...
void EditorAutoloadSettings::autoload_remove(const String& p_name) {
    ...
    undo_redo->add_do_property(&(Global::ProjectSettings()), name, Variant());
    ...
    undo_redo
        ->add_undo_method(&(Global::ProjectSettings()), "set_order", order);
...
void EditorAutoloadSettings::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &EditorAutoloadSettings::_autoload_path_text_changed
    );
    ClassDB::bind_method(
        "_autoload_text_entered",
        &EditorAutoloadSettings::_autoload_text_entered
    );
    ClassDB::bind_method(
        "_autoload_text_changed",
        &EditorAutoloadSettings::_autoload_text_changed
    );
    ClassDB::bind_method(
        "_autoload_open",
        &EditorAutoloadSettings::_autoload_open
    );
    ClassDB::bind_method(
        "_autoload_file_callback",
        &EditorAutoloadSettings::_autoload_file_callback
    );

    ClassDB::bind_method(
        "get_drag_data_fw",
        &EditorAutoloadSettings::get_drag_data_fw
    );
    ClassDB::bind_method(
        "can_drop_data_fw",
    ...
    ClassDB::bind_method(
        "autoload_remove",
        &EditorAutoloadSettings::autoload_remove
    );
...
EditorAutoloadSettings::EditorAutoloadSettings() {
    ...
    autoload_add_path->get_file_dialog()->set_mode(
        EditorFileDialog::MODE_OPEN_FILE
    );
    autoload_add_path->get_file_dialog()
        ->connect("file_selected", this, "_autoload_file_callback");
    autoload_add_path->get_line_edit()
        ->connect("text_changed", this, "_autoload_path_text_changed");
    ...
    // The button will be enabled once a valid name is entered (either
    ...
    // Reserve enough space for translations of "Global Variable" which may be
    // longer.
