...
void DependencyEditor::edit(const String& p_path) {
    ...
    if (EditorNode::get_singleton()->is_scene_open(p_path)) {
        EditorNode::get_singleton()->show_warning(vformat(
            TTR("Scene '%s' is currently being edited.\nChanges will only take "
                "effect when reloaded."),
            p_path.get_file()
        ));
    } else if (ResourceCache::has(p_path)) {
        EditorNode::get_singleton()->show_warning(vformat(
            TTR("Resource '%s' is in use.\nChanges will only take effect when "
                "reloaded."),
            p_path.get_file()
        ));
    }
...
/////////////////////////////////////
void DependencyEditorOwners::_list_rmb_select(
    int p_item,
    const Vector2& p_pos
) {
    file_options->clear();
...
void DependencyEditorOwners::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_list_rmb_select"),
        &DependencyEditorOwners::_list_rmb_select
    );
    ClassDB::bind_method(
        D_METHOD("_file_option"),
        &DependencyEditorOwners::_file_option
    );
    ClassDB::bind_method(
        D_METHOD("_select_file"),
        &DependencyEditorOwners::_select_file
    );
}
...
void DependencyRemoveDialog::_find_files_in_removed_folder(
    ...
    for (int i = 0; i < efsd->get_file_count(); i++) {
        ...
        all_remove_files[file] =
            p_folder; // Point the file to the ancestor directory we are
                      // deleting so we know what to parent it under in the
                      // tree.
    }
}
...
void DependencyRemoveDialog::_build_removed_dependency_tree(
    ...
    for (int i = 0; i < p_removed.size(); i++) {
            ...
            if (rd.dependency_folder.length() > 0) {
                ...
                if (!tree_items.has(rd.dependency_folder)) {
                    TreeItem* folder_item =
                        owners->create_item(owners->get_root());
                ...
                TreeItem* dependency_item =
                    owners->create_item(tree_items[rd.dependency_folder]);
                dependency_item->set_text(0, rd.dependency);
                dependency_item->set_icon(
                    0,
        ...
        Ref<Texture> icon =
            EditorNode::get_singleton()->get_class_icon(rd.file_type);
...
void DependencyRemoveDialog::show(
    const Vector<String>& p_folders,
    ...
    for (int i = 0; i < p_folders.size(); ++i) {
        String folder =
            p_folders[i].ends_with("/") ? p_folders[i] : (p_folders[i] + "/");
        _find_files_in_removed_folder(
            EditorFileSystem::get_singleton()->get_filesystem_path(folder),
            folder
        );
    ...
    if (removed_deps.empty()) {
        owners->hide();
        text->set_text(TTR(
            "Remove the selected files from the project? (Cannot be "
            "undone.)\nDepending on your filesystem configuration, the files "
            "will either be moved to the system trash or deleted permanently."
        ));
        set_size(Size2());
        popup_centered();
    } else {
        ...
        owners->show();
        text->set_text(TTR(
            "The files being removed are required by other resources in order "
            "for them to work.\nRemove them anyway? (Cannot be "
            "undone.)\nDepending on your filesystem configuration, the files "
            "will either be moved to the system trash or deleted permanently."
        ));
...
void DependencyRemoveDialog::ok_pressed() {
    ...
    for (int i = 0; i < files_to_delete.size(); ++i) {
        ...
        // If the file we are deleting for e.g. the main scene, default
        // environment, or audio bus layout, we must clear its definition in
        // Project Settings.
        if (files_to_delete[i] == settings.get("application/config/icon")) {
            settings.set("application/config/icon", "");
        }
        if (files_to_delete[i] == settings.get("application/run/main_scene")) {
            settings.set("application/run/main_scene", "");
        }
        if (files_to_delete[i]
            == settings.get("application/boot_splash/image")) {
            settings.set("application/boot_splash/image", "");
        }
        if (files_to_delete[i]
            == settings.get("rendering/environment/default_environment")) {
            settings.set("rendering/environment/default_environment", "");
        }
        if (files_to_delete[i]
            == settings.get("display/mouse_cursor/custom_image")) {
            settings.set("display/mouse_cursor/custom_image", "");
        }
        if (files_to_delete[i] == settings.get("gui/theme/custom")) {
            settings.set("gui/theme/custom", "");
        }
        if (files_to_delete[i] == settings.get("gui/theme/custom_font")) {
            settings.set("gui/theme/custom_font", "");
        }
        if (files_to_delete[i] == settings.get("audio/default_bus_layout")) {
            settings.set("audio/default_bus_layout", "");
        }

        String path = OS::get_singleton()->get_resource_dir()
                    + files_to_delete[i].replace_first("res://", "/");
    ...
    if (dirs_to_delete.size() == 0) {
        // If we only deleted files we should only need to tell the file system
        // about the files we touched.
        for (int i = 0; i < files_to_delete.size(); ++i) {
        ...
        for (int i = 0; i < dirs_to_delete.size(); ++i) {
            String path = OS::get_singleton()->get_resource_dir()
                        + dirs_to_delete[i].replace_first("res://", "/");
            print_verbose("Moving to trash: " + path);
            Error err = OS::get_singleton()->move_to_trash(path);
            if (err != OK) {
                EditorNode::get_singleton()->add_io_error(
    ...
    Vector<String> previous_favorites =
...
void DependencyErrorDialog::show(
    Mode p_mode,
...
DependencyErrorDialog::DependencyErrorDialog() {
    ...
    vb->add_margin_child(
        TTR("Load failed due to missing dependencies:"),
        files,
        true
    );
...
void OrphanResourcesDialog::_find_to_delete(
    TreeItem* p_item,
    List<String>& paths
) {
    while (p_item) {
...
void OrphanResourcesDialog::_button_pressed(
    Object* p_item,
    int p_column,
    int p_id
) {
...
OrphanResourcesDialog::OrphanResourcesDialog() {
    ...
    files->set_hide_root(true);
    vbc->add_margin_child(
        TTR("Resources Without Explicit Ownership:"),
        files,
        true
    );
