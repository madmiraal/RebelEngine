...
bool EditorPropertyArrayObject::_set(
...
bool EditorPropertyArrayObject::_get(const StringName& p_name, Variant& r_ret)
    const {
    String name = p_name;
...
bool EditorPropertyDictionaryObject::_set(
    const StringName& p_name,
    const Variant& p_value
) {
    String name = p_name;
...
bool EditorPropertyDictionaryObject::_get(
    const StringName& p_name,
    Variant& r_ret
) const {
    String name = p_name;
...
void EditorPropertyDictionaryObject::set_new_item_value(
    const Variant& p_new_item
) {
...
void EditorPropertyArray::_property_changed(
    ...
    if (p_property.begins_with("indices")) {
        ...
        if (array.get_type() == Variant::ARRAY) {
            ...
            ); // Duplicate, so undo/redo works better.
        }
...
void EditorPropertyArray::_change_type(Object* p_button, int p_index) {
    ...
    change_type->set_global_position(
        ...
        - Vector2(change_type->get_combined_minimum_size().x, 0)
    );
    change_type->popup();
}
...
void EditorPropertyArray::_change_type_menu(int p_index) {
    ...
    if (array.get_type() == Variant::ARRAY) {
        array =
            array.call("duplicate"); // Duplicate, so undo/redo works better.
    }
...
void EditorPropertyArray::_object_id_selected(
    const String& p_property,
    ObjectID p_id
) {
...
void EditorPropertyArray::update_property() {
    ...
    if (unfolded) {
        ...
        if (!vbox) {
            ...
            for (int i = vbox->get_child_count() - 1; i >= 2; i--) {
                ...
                if (child == reorder_selected_element_hbox) {
                    continue; // Don't remove the property that the user is
                              // moving.
                }

                child->queue_delete(
                ); // Button still needed after pressed is called.
        ...
        for (int i = 0; i < amount; i++) {
            bool reorder_is_from_current_page =
                reorder_from_index / page_length == page_index;
            if (reorder_is_from_current_page
                && i == reorder_from_index % page_length) {
                // Don't duplicate the property that the user is moving.
                continue;
            }
            if (!reorder_is_from_current_page
                && i == reorder_to_index % page_length) {
                // Don't create the property the moving property will take the
                // place of, e.g. (if page_length == 20) don't create element 20
                // if dragging an item from the first page to the second page
                // because element 20 would become element 19.
                continue;
            }
            ...
            reorder_button->set_default_cursor_type(CursorType::MOVE);
            reorder_button
                ->connect("gui_input", this, "_reorder_button_gui_input");
            reorder_button->connect(
                "button_down",
                this,
                "_reorder_button_down",
                varray(i + offset)
            );
            ...
            if (value_type == Variant::OBJECT
                && Object::cast_to<EncodedObjectAsID>(value)) {
                EditorPropertyObjectID* editor = memnew(EditorPropertyObjectID);
                editor->setup("Object");
                prop = editor;
            } else {
                prop = EditorInspector::instantiate_property_editor(
                    nullptr,
                    value_type,
                    "",
                    subtype_hint,
            ...
            bool is_untyped_array =
                array.get_type() == Variant::ARRAY && subtype == Variant::NIL;
        ...
        if (reorder_to_index % page_length > 0) {
            vbox->move_child(
                vbox->get_child(2),
                reorder_to_index % page_length + 2
            );
        }
...
bool EditorPropertyArray::_is_drop_valid(const Dictionary& p_drag_data) const {
    ...
    // When the subtype is of type Object, an additional subtype may be
    // specified in the hint string (e.g. Resource, Texture2D, ShaderMaterial,
    // etc). We want the allowed type to be that, not just "Object".
...
bool EditorPropertyArray::can_drop_data_fw(
...
void EditorPropertyArray::drop_data_fw(
    const Point2& p_point,
    const Variant& p_data,
    Control* p_from
) {
...
void EditorPropertyArray::_length_changed(double p_page) {
    ...
    if (array.get_type() == Variant::ARRAY) {
        ...
        for (int i = previous_size; i < size; i++) {
            ...
            array.set(
                i,
                Variant::construct(array.get(i).get_type(), nullptr, 0, ce)
            );
        }
    }
...
void EditorPropertyArray::setup(
    Variant::Type p_array_type,
    ...
    if (array_type == Variant::ARRAY && !p_hint_string.empty()) {
        ...
        if (hint_subtype_separator >= 0) {
            String subtype_string =
                p_hint_string.substr(0, hint_subtype_separator);
            int slash_pos = subtype_string.find("/");
            if (slash_pos >= 0) {
                subtype_hint =
                    PropertyHint(subtype_string
                                     .substr(
                                         slash_pos + 1,
                                         subtype_string.size() - slash_pos - 1
                                     )
                                     .to_int());
                subtype_string = subtype_string.substr(0, slash_pos);
            }

            subtype_hint_string = p_hint_string.substr(
                hint_subtype_separator + 1,
                p_hint_string.size() - hint_subtype_separator - 1
            );
...
void EditorPropertyArray::_reorder_button_gui_input(
    ...
    if (mm.is_valid()) {
        ...
        // Reordering is done by moving the dragged element by +1/-1 index at a
        // time based on the cumulated mouse delta so if already at the array
        // bounds make sure to ignore the remaining out of bounds drag (by
        // resetting the cumulated delta).
        if ((reorder_to_index == 0 && reorder_mouse_y_delta < 0.0f)
            || (reorder_to_index == size - 1 && reorder_mouse_y_delta > 0.0f)) {
        ...
        if (ABS(reorder_mouse_y_delta) > required_y_distance) {
            ...
            if ((direction < 0
                 && reorder_to_index % page_length == page_length - 1)
                || (direction > 0 && reorder_to_index % page_length == 0)) {
                // Automatically move to the next/previous page.
                page_slider->set_value(page_index + direction);
            }
            vbox->move_child(
                reorder_selected_element_hbox,
                reorder_to_index % page_length + 2
            );
            // Ensure the moving element is visible.
            EditorNode::get_singleton()
                ->get_inspector()
                ->ensure_control_visible(reorder_selected_element_hbox);
        }
    }
}
...
void EditorPropertyArray::_reorder_button_down(int p_index) {
    ...
    reorder_selected_button =
        Object::cast_to<Button>(reorder_selected_element_hbox->get_child(0));
...
void EditorPropertyArray::_reorder_button_up() {
    ...
    reorder_selected_button->warp_mouse(
        reorder_selected_button->get_size() / 2.0f
    );
...
void EditorPropertyArray::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_change_type_menu",
        &EditorPropertyArray::_change_type_menu
    );
    ClassDB::bind_method(
        "_object_id_selected",
    ...
    ClassDB::bind_method(
        "_remove_pressed",
        &EditorPropertyArray::_remove_pressed
    );
    ClassDB::bind_method(
        "_reorder_button_gui_input",
        &EditorPropertyArray::_reorder_button_gui_input
    );
    ClassDB::bind_method(
        "_reorder_button_down",
    ...
    ClassDB::bind_method(
        ...
        &EditorPropertyArray::can_drop_data_fw
    );
    ClassDB::bind_method(
        D_METHOD("drop_data_fw"),
        &EditorPropertyArray::drop_data_fw
    );
}
...
EditorPropertyArray::EditorPropertyArray() {
    object.instance();
    page_length =
        int(EDITOR_GET("interface/inspector/max_array_dictionary_items_per_page"
        ));
    edit = memnew(Button);
    edit->set_flat(true);
...
void EditorPropertyDictionary::update_property() {
    ...
    if (updated_val.get_type() == Variant::NIL) {
        edit->set_text("Dictionary (Nil)"
        ); // This provides symmetry with the array property.
        edit->set_pressed(false);
        if (vbox) {
            set_bottom_editor(nullptr);
            memdelete(vbox);
    ...
    if (unfolded) {
        ...
        if (!vbox) {
            ...
            // Queue children for deletion, deleting immediately might cause
            // errors.
        ...
        for (int i = 0; i < amount + 2; i++) {
            ...
            switch (value.get_type()) {
                ...
                case Variant::INT: {
                    EditorPropertyInteger* editor =
                        memnew(EditorPropertyInteger);
                ...
                case Variant::VECTOR2: {
                    EditorPropertyVector2* editor =
                        memnew(EditorPropertyVector2);
                ...
                case Variant::VECTOR3: {
                    EditorPropertyVector3* editor =
                        memnew(EditorPropertyVector3);
                    editor->setup(-100000, 100000, 0.001, true);
                    prop = editor;
                ...
                case Variant::TRANSFORM: {
                    EditorPropertyTransform* editor =
                ...
                case Variant::OBJECT: {
                    if (Object::cast_to<EncodedObjectAsID>(value)) {
                        EditorPropertyObjectID* editor =
                            memnew(EditorPropertyObjectID);
                        editor->setup("Object");
                        prop = editor;

                    } else {
                        EditorPropertyResource* editor =
                            memnew(EditorPropertyResource);
...
void EditorPropertyDictionary::_object_id_selected(
    const String& p_property,
    ObjectID p_id
) {
    emit_signal("object_id_selected", p_property, p_id);
...
void EditorPropertyDictionary::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_page_changed",
        &EditorPropertyDictionary::_page_changed
    );
    ClassDB::bind_method(
        "_property_changed",
        &EditorPropertyDictionary::_property_changed,
        DEFVAL(String()),
        DEFVAL(false)
    );
    ClassDB::bind_method(
        "_change_type",
        &EditorPropertyDictionary::_change_type
    );
    ClassDB::bind_method(
        "_change_type_menu",
        &EditorPropertyDictionary::_change_type_menu
    );
    ClassDB::bind_method(
        "_add_key_value",
