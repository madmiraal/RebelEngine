...
void ScriptCreateDialog::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_ENTER_TREE: {
            ...
            status_panel->add_style_override(
                "panel",
...
void ScriptCreateDialog::config(
    const String& p_base_name,
    const String& p_base_path,
    bool p_built_in_enabled,
    bool p_load_enabled
) {
    class_name->set_text("");
    class_name->deselect();
    ...
    if (p_base_path != "") {
        ...
        file_path->set_text(
            initial_bp + "."
            + ScriptServer::get_language(language_menu->get_selected())
                  ->get_extension()
        );
...
bool ScriptCreateDialog::_validate_parent(const String& p_string) {
    ...
    return ClassDB::class_exists(p_string)
        || ScriptServer::is_global_class(p_string);
}
...
bool ScriptCreateDialog::_validate_class(const String& p_string) {
    ...
    for (int i = 0; i < p_string.length(); i++) {
        ...
        bool valid_char = (p_string[i] >= '0' && p_string[i] <= '9')
                       || (p_string[i] >= 'a' && p_string[i] <= 'z')
                       || (p_string[i] >= 'A' && p_string[i] <= 'Z')
...
String ScriptCreateDialog::_validate_path(
    const String& p_path,
    bool p_file_must_exist
) {
...
void ScriptCreateDialog::_template_changed(int p_template) {
    String selected_template =
        p_template == 0 ? "" : template_menu->get_item_text(p_template);
    ...
    for (int i = 0; i < template_list.size(); i++) {
        ...
        if (sinfo.id == selected_id) {
            script_template =
                sinfo.dir.plus_file(sinfo.name + "." + sinfo.extension);
            break;
        }
    }
}
...
void ScriptCreateDialog::_create_new() {
    ...
    if (script_template != "") {
        ...
        if (scr.is_null()) {
            alert->set_text(
                vformat(TTR("Error loading template '%s'"), script_template)
            );
        ...
        scr = scr->duplicate();
        ScriptServer::get_language(language_menu->get_selected())
            ->make_template(cname_param, parent_name->get_text(), scr);
    } else {
        scr = ScriptServer::get_language(language_menu->get_selected())
                  ->get_template(cname_param, parent_name->get_text());
    }
...
void ScriptCreateDialog::_lang_changed(int l) {
    ...
    if (path != "") {
        ...
        if (extension.length() == 0) {
            ...
            for (int m = 0; m < language_menu->get_item_count(); m++) {
                ScriptServer::get_language(m)->get_recognized_extensions(
                    &extensions
                );
            }
    ...
    if (use_templates) {
        ...
        // Populate script template items previously sorted and now grouped by
        // origin
        for (int i = 0; i < template_list.size(); i++) {
        ...
        // Disable overridden
        for (Map<String, Vector<int>>::Element* E = template_overrides.front();
             E;
             E = E->next()) {
            ...
            for (int i = 1; i < overrides.size(); i++) {
                const ScriptTemplateInfo& overridden =
                    template_list[overrides[i]];

                int disable_index =
                    template_menu->get_item_index(overridden.id);
            ...
            template_menu->set_item_icon(
                extended.id,
                get_icon("Override", "EditorIcons")
            );
            template_menu->get_popup()->set_item_tooltip(
                extended.id,
                override_info.as_string()
            );
        }
        // Reselect last selected template
        for (int i = 0; i < template_menu->get_item_count(); i++) {
            const String& ti = template_menu->get_item_text(i);
            if (language_menu->get_item_text(language_menu->get_selected())
                    == last_lang
                && last_template == ti) {
    ...
    EditorSettings::get_singleton()->set_project_metadata(
        "script_setup",
        "last_selected_language",
        language_menu->get_item_text(language_menu->get_selected())
    );
...
void ScriptCreateDialog::_update_script_templates(const String& p_extension) {
    ...
    dirs.push_back(
        EditorSettings::get_singleton()->get_project_script_templates_dir()
    );
    dirs.push_back(EditorSettings::get_singleton()->get_script_templates_dir());

    for (int i = 0; i < dirs.size(); i++) {
        Vector<String> list = EditorSettings::get_singleton()
                                  ->get_script_templates(p_extension, dirs[i]);
...
void ScriptCreateDialog::_msg_path_valid(bool valid, const String& p_msg) {
    ...
    if (valid) {
        path_error_label->add_color_override(
            "font_color",
            get_color("success_color", "Editor")
        );
    } else {
...
void ScriptCreateDialog::_update_dialog() {
    ...
    script_name_warning_label->set_visible(
        !is_built_in && _get_class_name() == parent_name->get_text()
    );
...
void ScriptCreateDialog::_bind_methods() {
    ClassDB::bind_method(
        "_path_hbox_sorted",
        &ScriptCreateDialog::_path_hbox_sorted
    );
    ClassDB::bind_method(
        "_class_name_changed",
        &ScriptCreateDialog::_class_name_changed
    );
    ClassDB::bind_method(
        "_parent_name_changed",
        &ScriptCreateDialog::_parent_name_changed
    );
    ...
    ClassDB::bind_method(
        "_template_changed",
        &ScriptCreateDialog::_template_changed
    );
    ClassDB::bind_method("_create", &ScriptCreateDialog::_create);
    ClassDB::bind_method(
        "_browse_class_in_tree",
        &ScriptCreateDialog::_browse_class_in_tree
    );
...
ScriptCreateDialog::ScriptCreateDialog() {
    ...
    builtin_warning_label->set_text(
        TTR("Note: Built-in scripts have some limitations and can't be edited "
    ...
    script_name_warning_label->set_custom_minimum_size(
        Size2(340, 10) * EDSCALE
    );
    script_name_warning_label->set_text(
        TTR("Warning: Having the script name be the same as a built-in type is "
            "usually not desired.")
    );
    ...
    parent_browse_button
        ->connect("pressed", this, "_browse_path", varray(true, false));
