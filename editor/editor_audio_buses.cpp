    ...
    switch (p_what) {
        ...
        case NOTIFICATION_THEME_CHANGED: {
            for (int i = 0; i < CHANNELS_MAX; i++) {
                channel[i].vu_l->set_under_texture(
                    get_icon("BusVuEmpty", "EditorIcons")
                );
                channel[i].vu_l->set_progress_texture(
                    get_icon("BusVuFull", "EditorIcons")
                );
                channel[i].vu_r->set_under_texture(
                    get_icon("BusVuEmpty", "EditorIcons")
                );
                channel[i].vu_r->set_progress_texture(
                    get_icon("BusVuFull", "EditorIcons")
                );
            ...
            audio_value_preview_label->add_color_override(
                "font_color",
                get_color("font_color", "TooltipLabel")
            );
            audio_value_preview_label->add_color_override(
                "font_color_shadow",
                get_color("font_color_shadow", "TooltipLabel")
            );
            audio_value_preview_box->add_style_override(
                "panel",
                get_stylebox("panel", "TooltipPanel")
            );
        } break;
        ...
        case NOTIFICATION_DRAW: {
            if (is_master) {
                draw_style_box(
                    get_stylebox("disabled", "Button"),
                    Rect2(Vector2(), get_size())
                );
            } else if (has_focus()) {
                draw_style_box(
                    get_stylebox("focus", "Button"),
                    Rect2(Vector2(), get_size())
                );
            } else {
                draw_style_box(
        ...
        case NOTIFICATION_PROCESS: {
            if (cc
                != AudioServer::get_singleton()->get_bus_channels(get_index()
                )) {
                cc =
                    AudioServer::get_singleton()->get_bus_channels(get_index());
            ...
            for (int i = 0; i < cc; i++) {
                ...
                if (AudioServer::get_singleton()
                        ->is_bus_channel_active(get_index(), i)) {
                    activity_found = true;
                    real_peak[0] =
                        MAX(real_peak[0],
                            AudioServer::get_singleton()
                                ->get_bus_peak_volume_left_db(get_index(), i));
                    real_peak[1] =
                        MAX(real_peak[1],
                            AudioServer::get_singleton()
                                ->get_bus_peak_volume_right_db(get_index(), i));
                }
...
void EditorAudioBus::update_bus() {
    ...
    for (int i = 0;
         i < AudioServer::get_singleton()->get_bus_effect_count(index);
         i++) {
        Ref<AudioEffect> afx =
...
void EditorAudioBus::_name_changed(const String& p_new_name) {
    ...
    while (true) {
        bool name_free = true;
        for (int i = 0; i < AudioServer::get_singleton()->get_bus_count();
             i++) {
...
void EditorAudioBus::_volume_changed(float p_normalized) {
    ...
    if (Input::get_singleton()->is_key_pressed(KEY_CONTROL)) {
        ...
        // To do so, it needs to be converted back to normalized volume (as the
        // slider uses that unit).
...
float EditorAudioBus::_scaled_db_to_normalized_volume(float db) {
    ...
    if (db > -2.88) {
        ...
        if (db < 0.0) {
            /* To accommodate for NaN on negative numbers for root, we will
             * mirror the results of the positive db range in order to get the
             * desired numerical value on the negative side. */
            float positive_x =
                Math::pow(Math::abs(db) / 45.0f, 1.0f / 3.0f) + 1.0f;
            Vector2 translation =
                Vector2(1.0f, 0.0f) - Vector2(positive_x, Math::abs(db));
...
void EditorAudioBus::_show_value(float slider_value) {
    ...
    if (Math::is_zero_approx(Math::stepify(db, 0.1))) {
        ...
        // The leading space makes the text visually line up with its
    ...
    // This way, it can be seen when the slider is merely hovered (instead of
    // dragged).
    ...
    const Vector2 box_position    = Vector2(
        slider_size.x,
        (slider_size.y - vert_padding) * (1.0f - slider->get_value())
            - vert_padding
    );
...
void EditorAudioBus::_effect_selected() {
    ...
    if (effect->get_metadata(0) != Variant()) {
        ...
        Ref<AudioEffect> effect2 =
            AudioServer::get_singleton()->get_bus_effect(get_index(), index);
        if (effect2.is_valid()) {
...
void EditorAudioBus::_effect_edited() {
    ...
    if (effect->get_metadata(0) == Variant()) {
        ...
        effect_options->set_position(
            effects->get_global_position() + area.position
            + Vector2(0, area.size.y)
        );
        ...
        ur->add_undo_method(
            ...
            index,
            AudioServer::get_singleton()
                ->is_bus_effect_enabled(get_index(), index)
        );
        ur->add_do_method(buses, "_update_bus", get_index());
        ur->add_undo_method(buses, "_update_bus", get_index());
        ur->commit_action();
...
void EditorAudioBus::_unhandled_key_input(Ref<InputEvent> p_event) {
    ...
    if (k.is_valid() && k->is_pressed() && !k->is_echo()
        && k->get_scancode() == KEY_DELETE) {
        TreeItem* current_effect = effects->get_selected();
        if (current_effect
            && current_effect->get_metadata(0).get_type() == Variant::INT) {
...
bool EditorAudioBus::can_drop_data(const Point2& p_point, const Variant& p_data)
    const {
    if (get_index() == 0) {
        return false;
    }
...
Variant EditorAudioBus::get_drag_data_fw(
...
bool EditorAudioBus::can_drop_data_fw(
    const Point2& p_point,
...
void EditorAudioBus::drop_data_fw(
    ...
    bool enabled =
        AudioServer::get_singleton()->is_bus_effect_enabled(bus, effect);
    ...
    ur->add_do_method(
        ...
        "add_bus_effect",
        get_index(),
        AudioServer::get_singleton()->get_bus_effect(bus, effect),
        paste_at
    );
...
void EditorAudioBus::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_hide_value_preview",
        &EditorAudioBus::_hide_value_preview
    );
    ...
    ClassDB::bind_method(
        ...
        &EditorAudioBus::_unhandled_key_input
    );
    ClassDB::bind_method(
        "_bus_popup_pressed",
        &EditorAudioBus::_bus_popup_pressed
    );
    ...
    ClassDB::bind_method(
        "_delete_effect_pressed",
        &EditorAudioBus::_delete_effect_pressed
    );
...
EditorAudioBus::EditorAudioBus(EditorAudioBuses* p_buses, bool p_is_master) {
    ...
    for (List<StringName>::Element* E = effects.front(); E; E = E->next()) {
        ...
        Ref<Texture> icon =
            EditorNode::get_singleton()->get_class_icon(E->get());
        String name = E->get().operator String().replace("AudioEffect", "");
        effect_options->add_item(name);
        effect_options->set_item_metadata(
            effect_options->get_item_count() - 1,
            E->get()
        );
        effect_options->set_item_icon(
            effect_options->get_item_count() - 1,
            icon
        );
    }
    ...
    delete_effect_popup
        ->connect("index_pressed", this, "_delete_effect_pressed");
}
...
bool EditorAudioBusDrop::can_drop_data(
    const Point2& p_point,
    const Variant& p_data
) const {
    Dictionary d = p_data;
...
void EditorAudioBusDrop::drop_data(
    const Point2& p_point,
    const Variant& p_data
) {
    Dictionary d = p_data;
...
EditorAudioBuses* EditorAudioBuses::register_editor() {
    ...
    EditorNode::get_singleton()->add_bottom_panel_item(
        TTR("Audio"),
        audio_buses
    );
    return audio_buses;
    ...
    switch (p_what) {
        ...
        case NOTIFICATION_PROCESS: {
            ...
            for (int i = 0; i < AudioServer::get_singleton()->get_bus_count();
                 i++) {
                for (int j = 0;
                     j < AudioServer::get_singleton()->get_bus_effect_count(i);
                     j++) {
                    Ref<AudioEffect> effect =
                        AudioServer::get_singleton()->get_bus_effect(i, j);
                    if (effect->is_edited()) {
                        edited = true;
...
void EditorAudioBuses::_delete_bus(Object* p_which) {
    ...
    if (index == 0) {
        EditorNode::get_singleton()->show_warning(
            TTR("Master bus can't be deleted!")
        );
        return;
    }
...
void EditorAudioBuses::_duplicate_bus(int p_which) {
    ...
    for (int i = 0;
         i < AudioServer::get_singleton()->get_bus_effect_count(p_which);
         i++) {
        ur->add_do_method(
            AudioServer::get_singleton(),
            "add_bus_effect",
...
void EditorAudioBuses::_server_save() {
    Ref<AudioBusLayout> state =
        AudioServer::get_singleton()->generate_bus_layout();
...
void EditorAudioBuses::_load_default_layout() {
    ...
    if (state.is_null()) {
        EditorNode::get_singleton()->show_warning(
            vformat(TTR("There is no '%s' file."), layout_path)
        );
        return;
    }
...
void EditorAudioBuses::_file_dialog_callback(const String& p_string) {
    if (file_dialog->get_mode() == EditorFileDialog::MODE_OPEN_FILE) {
        ...
        if (state.is_null()) {
            EditorNode::get_singleton()->show_warning(
                TTR("Invalid file, not an audio bus layout.")
            );
            return;
        }
...
void EditorAudioBuses::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_request_drop_end",
        &EditorAudioBuses::_request_drop_end
    );
    ...
    ClassDB::bind_method(
        ...
        &EditorAudioBuses::_load_default_layout
    );
    ...
    ClassDB::bind_method(
        "_reset_bus_volume",
        &EditorAudioBuses::_reset_bus_volume
    );

    ClassDB::bind_method(
        "_file_dialog_callback",
        &EditorAudioBuses::_file_dialog_callback
    );
}
...
EditorAudioBuses::EditorAudioBuses() {
    ...
    edited_path = GLOBAL_GET("audio/default_bus_layout");
...
void EditorAudioMeterNotches::add_notch(
    ...
    notches.push_back(
        AudioNotch(p_normalized_offset, p_db_value, p_render_value)
    );
}
            ...
            width =
                MAX(width,
                    font->get_string_size(
                            String::num(Math::abs(notches[i].db_value)) + "dB"
                    )
                        .x);
            height += font_height;
        }
...
void EditorAudioMeterNotches::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_draw_audio_notches",
        &EditorAudioMeterNotches::_draw_audio_notches
    );
}
...
void EditorAudioMeterNotches::_draw_audio_notches() {
    ...
    for (int i = 0; i < notches.size(); i++) {
        ...
        draw_line(
            Vector2(
                0,
                (1.0f - n.relative_position)
                        * (get_size().y - btm_padding - top_padding)
                    + top_padding
            ),
            Vector2(
                line_length * EDSCALE,
        ...
        if (n.render_db_value) {
            draw_string(
                font,
                Vector2(
                    (line_length + label_space) * EDSCALE,
                    (1.0f - n.relative_position)
                            * (get_size().y - btm_padding - top_padding)
