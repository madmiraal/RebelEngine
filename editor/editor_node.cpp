...
void EditorNode::disambiguate_filenames(
    const Vector<String> p_full_paths,
    Vector<String>& r_filenames
) {
    ...
    for (int i = 0; i < r_filenames.size(); i++) {
        ...
        if (!scene_name_to_set_index.has(scene_name)) {
            ...
            scene_name_to_set_index.insert(
                r_filenames[i],
                index_sets.size() - 1
            );
        }
    ...
    for (int i = 0; i < index_sets.size(); i++) {
        ...
        while (iset.size() > 1) {
            ...
            for (Set<int>::Element* E = iset.front(); E; E = E->next()) {
                ...
                // Find just the parent folder of the current path and append
                // it. If the current name is foo.tscn, and the full path is
                // /some/folder/foo.tscn then slash_idx is the second '/', so
                // that we select just "folder", and append that to yield
                // "folder/foo.tscn".
                if (difference > 0) {
                    ...
                    parent =
                        slash_idx >= 0 ? parent.substr(slash_idx + 1) : parent;
            ...
            while (E) {
                ...
                // We can proceed iff the full path is longer than the scene
                // name, meaning that there is at least one more parent folder
                // we can tack onto the name.
                can_proceed =
                    can_proceed || (path.size() - scene_name.size()) >= 1;
...
void EditorNode::_update_scene_tabs() {
    bool show_rb = EditorSettings::get_singleton()->get(
    ...
    for (int i = 0; i < editor_data.get_edited_scene_count(); i++) {
        ...
        if (type_node) {
            icon =
                EditorNode::get_singleton()->get_object_icon(type_node, "Node");
        }
        ...
        scene_tabs->add_tab(
            disambiguated_scene_names[i] + (unsaved ? "(*)" : ""),
            icon
        );

        OS::get_singleton()->global_menu_add_item(
            "_dock",
            editor_data.get_scene_title(i) + (unsaved ? "(*)" : ""),
            GLOBAL_SCENE,
            i
        );
    ...
    if (scene_tabs->get_offset_buttons_visible()) {
        ...
        if (scene_tabs->get_tab_count() != 0) {
            last_tab =
                scene_tabs->get_tab_rect(scene_tabs->get_tab_count() - 1);
        }
        scene_tab_add->set_position(Point2(
            last_tab.get_position().x + last_tab.get_size().x + 3,
            last_tab.get_position().y
        ));
    }
}
    ...
    switch (vcs_actions_menu->get_item_id(p_idx)) {
        case RUN_VCS_SETTINGS: {
            VersionControlEditorPlugin::get_singleton()
                ->popup_vcs_set_up_dialog(gui_base);
        } break;
...
void EditorNode::_update_title() {
    ...
    const String edited =
        editor_data.get_edited_scene_root()
            ? editor_data.get_edited_scene_root()->get_filename()
            : String();
    if (!edited.empty()) {
        // Display the edited scene name before the program name so that it can
        // be seen in the OS task bar.
        title = vformat("%s - %s", edited.get_file(), title);
    }
    if (unsaved_cache) {
        // Display the "modified" mark before anything else so that it can
        // always be seen in the OS task bar.
...
void EditorNode::_unhandled_input(const Ref<InputEvent>& p_event) {
    ...
    if (k.is_valid() && k->is_pressed() && !k->is_echo()
        && !gui_base->get_viewport()->gui_has_modal_stack()) {
        ...
        if (ED_IS_SHORTCUT("editor/prev_tab", p_event)) {
            int next_tab = editor_data.get_edited_scene() - 1;
            next_tab     = next_tab >= 0 ? next_tab
                                         : editor_data.get_edited_scene_count() - 1;
            _scene_tab_changed(next_tab);
        }
...
void EditorNode::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_PROCESS: {
            ...
            if (frame != update_spinner_step_frame
                && (tick - update_spinner_step_msec) > (1000 / 8)) {
                update_spinner_step++;
                ...
                if (EditorSettings::get_singleton()->get(
                    ...
                    update_spinner->set_icon(gui_base->get_icon(
                        "Progress" + itos(update_spinner_step + 1),
                        "EditorIcons"
                    ));
                }
            }
        ...
        case NOTIFICATION_ENTER_TREE: {
            ...
            OS::get_singleton()->set_low_processor_usage_mode_sleep_usec(int(
                EDITOR_GET("interface/editor/low_processor_mode_sleep_usec")
            ));
        ...
        case NOTIFICATION_READY: {
            ...
            VisualServer::get_singleton()->viewport_set_hide_scenario(
                get_scene_root()->get_viewport_rid(),
                true
            );
            ...
            if (!main_editor_buttons[EDITOR_3D]->is_visible(
                )) { // may be hidden due to feature profile
                _editor_select(EDITOR_2D);
            } else {
                _editor_select(EDITOR_3D);
            }
        ...
        case MainLoop::NOTIFICATION_WM_FOCUS_OUT: {
            // Save on focus loss before applying the FPS limit to avoid slowing
            // down the saving process.
            if (EDITOR_GET("interface/editor/save_on_focus_loss")) {
                _menu_option_confirm(FILE_SAVE_SCENE, false);
            }

            // Set a low FPS cap to decrease CPU/GPU usage while the editor is
            // unfocused.
        ...
        case EditorSettings::NOTIFICATION_EDITOR_SETTINGS_CHANGED: {
            ...
            gui_base->add_style_override(
                "panel",
                gui_base->get_stylebox("Background", "EditorStyles")
            );
            scene_root_parent->add_style_override(
                "panel",
                gui_base->get_stylebox("Content", "EditorStyles")
            );
            bottom_panel->add_style_override(
                "panel",
                gui_base->get_stylebox("panel", "TabContainer")
            );
            scene_tabs->add_style_override(
                "tab_fg",
                gui_base->get_stylebox("SceneTabFG", "EditorStyles")
            );
            scene_tabs->add_style_override(
                "tab_bg",
                gui_base->get_stylebox("SceneTabBG", "EditorStyles")
            );

            file_menu->add_style_override(
                "hover",
                gui_base->get_stylebox("MenuHover", "EditorStyles")
            );
            project_menu->add_style_override(
                "hover",
                gui_base->get_stylebox("MenuHover", "EditorStyles")
            );
            debug_menu->add_style_override(
                "hover",
                gui_base->get_stylebox("MenuHover", "EditorStyles")
            );
            settings_menu->add_style_override(
                "hover",
                gui_base->get_stylebox("MenuHover", "EditorStyles")
            );
            help_menu->add_style_override(
                "hover",
                gui_base->get_stylebox("MenuHover", "EditorStyles")
            );

            if (EDITOR_GET("interface/scene_tabs/resize_if_many_tabs")) {
                scene_tabs->set_min_width(
                    int(EDITOR_GET("interface/scene_tabs/minimum_width"))
                    * EDSCALE
                );
            } else {
            ...
            for (int i = 0; i < singleton->main_editor_buttons.size(); i++) {
                ...
                if (icon.is_valid()) {
                    tb->set_icon(icon);
                } else if (singleton->gui_base->has_icon(
                               p_editor->get_name(),
                               "EditorIcons"
                           )) {
                    tb->set_icon(singleton->gui_base->get_icon(
                        p_editor->get_name(),
                        "EditorIcons"
                    ));
                }
            ...
            play_scene_button->set_icon(
                gui_base->get_icon("PlayScene", "EditorIcons")
            );
            play_custom_scene_button->set_icon(
                gui_base->get_icon("PlayCustom", "EditorIcons")
            );
            ...
            distraction_free->set_icon(
                gui_base->get_icon("DistractionFree", "EditorIcons")
            );
            scene_tab_add->set_icon(gui_base->get_icon("Add", "EditorIcons"));

            bottom_panel_raise->set_icon(
                gui_base->get_icon("ExpandBottomDock", "EditorIcons")
            );

            // clear_button->set_icon(gui_base->get_icon("Close",
            // "EditorIcons")); don't have access to that node. needs to become
            // a class property
            dock_tab_move_left->set_icon(theme->get_icon("Back", "EditorIcons")
            );
            dock_tab_move_right->set_icon(
                theme->get_icon("Forward", "EditorIcons")
            );

            PopupMenu* p = help_menu->get_popup();
            p->set_item_icon(
                p->get_item_index(HELP_SEARCH),
                gui_base->get_icon("HelpSearch", "EditorIcons")
            );
            p->set_item_icon(
                p->get_item_index(HELP_DOCS),
                gui_base->get_icon("Instance", "EditorIcons")
            );
            p->set_item_icon(
                p->get_item_index(HELP_REPORT_A_BUG),
                gui_base->get_icon("Instance", "EditorIcons")
            );
            p->set_item_icon(
                p->get_item_index(HELP_ABOUT),
                gui_base->get_icon("Rebel", "EditorIcons")
            );
            p->set_item_icon(
                p->get_item_index(HELP_SUPPORT_DEVELOPMENT),
                gui_base->get_icon("Heart", "EditorIcons")
            );
...
void EditorNode::_update_update_spinner() {
    update_spinner->set_visible(EditorSettings::get_singleton()->get(
        "interface/editor/show_update_spinner"
    ));

    const bool update_continuously = EditorSettings::get_singleton()->get(
        "interface/editor/update_continuously"
    );
    PopupMenu* update_popup = update_spinner->get_popup();
    update_popup->set_item_checked(
        update_popup->get_item_index(SETTINGS_UPDATE_CONTINUOUSLY),
        update_continuously
    );
    update_popup->set_item_checked(
        update_popup->get_item_index(SETTINGS_UPDATE_WHEN_CHANGED),
        !update_continuously
    );

    if (update_continuously) {
        update_spinner->set_tooltip(
            TTR("Spins when the editor window redraws.\nUpdate Continuously is "
                "enabled, which can increase power usage. Click to disable it.")
        );

        // Use a different color for the update spinner when Update Continuously
        // is enabled, as this feature should only be enabled for
        // troubleshooting purposes. Make the icon modulate color overbright
        // because icons are not completely white on a dark theme. On a light
        // theme, icons are dark, so we need to modulate them with an even
        // brighter color.
...
void EditorNode::_on_plugin_ready(
    Object* p_script,
    const String& p_activate_name
) {
...
void EditorNode::_resources_changed(const PoolVector<String>& p_resources) {
    ...
    for (int i = 0; i < rc; i++) {
        ...
        if (res->get_import_path() != String()) {
            // this is an imported resource, will be reloaded if reimported via
            // the _resources_reimported() callback
            continue;
        }
    ...
    if (changed.size()) {
        for (List<Ref<Resource>>::Element* E = changed.front(); E;
             E                               = E->next()) {
...
void EditorNode::_fs_changed() {
    for (Set<FileDialog*>::Element* E = file_dialogs.front(); E;
         E                            = E->next()) {
        E->get()->invalidate();
    }

    for (Set<EditorFileDialog*>::Element* E = editor_file_dialogs.front(); E;
         E                                  = E->next()) {
        E->get()->invalidate();
    }
    ...
    // FIXME: Move this to a cleaner location, it's hacky to do this is
    // _fs_changed.
    String export_error;
    if (export_defer.preset != ""
        && !EditorFileSystem::get_singleton()->is_scanning()) {
        String preset_name  = export_defer.preset;
        // Ensures export_project does not loop infinitely, because
        // notifications may come during the export.
        export_defer.preset = "";
        ...
        if (preset.is_null()) {
            ...
            if (da->file_exists("res://export_presets.cfg")) {
                ...
                for (int i = 0;
                     i
                     < EditorExport::get_singleton()->get_export_preset_count();
                     ++i) {
                    // Write the preset name between double quotes since it
                    // needs to be written between quotes on the command line if
                    // it contains spaces.
                    export_error += vformat(
                        "        \"%s\"\n",
                        EditorExport::get_singleton()
                            ->get_export_preset(i)
                            ->get_name()
                    );
                }
                ...
                export_error =
                    "This project doesn't have an `export_presets.cfg` file at "
                    "its root.\nCreate an export preset from the \"Project > "
                    "Export\" dialog and try again.";
            }
        } else {
            Ref<EditorExportPlatform> platform = preset->get_platform();
            const String export_path           = export_defer.path.empty()
                                                   ? preset->get_export_path()
                                                   : export_defer.path;
            if (export_path.empty()) {
                export_error = vformat(
                    "Export preset \"%s\" doesn't have a default export path, "
                    "and none was specified.",
                    preset_name
                );
            } else if (platform.is_null()) {
                export_error = vformat(
                    "Export preset \"%s\" doesn't have a matching platform.",
                    preset_name
                );
            } else {
                Error err = OK;
                if (export_defer
                        .pack_only) { // Only export .pck or .zip data pack.
                    ...
                    if (!platform->can_export(
                        ...
                        ERR_PRINT(vformat(
                            "Cannot export project with preset \"%s\" due to "
                            "configuration errors:\n%s",
                            preset_name,
                            config_error
                        ));
                        err = missing_templates ? ERR_FILE_NOT_FOUND
                                                : ERR_UNCONFIGURED;
                    } else {
                ...
                switch (err) {
                    ...
                    case ERR_FILE_NOT_FOUND:
                        export_error = vformat(
                            "Project export failed for preset \"%s\". The "
                            "export template appears to be missing.",
                            preset_name
                        );
                        break;
                    case ERR_FILE_BAD_PATH:
                        export_error = vformat(
                            "Project export failed for preset \"%s\". The "
                            "target path \"%s\" appears to be invalid.",
                            preset_name,
                            export_path
                        );
                        break;
                    default:
                        export_error = vformat(
                            "Project export failed with error code %d for "
                            "preset \"%s\".",
                            (int)err,
                            preset_name
                        );
...
void EditorNode::_scan_external_changes() {
    ...
    for (int i = 0; i < editor_data.get_edited_scene_count(); i++) {
        ...
        if (editor_data.get_scene_path(i) == ""
            || !da->file_exists(editor_data.get_scene_path(i))) {
            continue;
        }

        uint64_t last_date = editor_data.get_scene_modified_time(i);
        uint64_t date =
            FileAccess::get_modified_time(editor_data.get_scene_path(i));
...
Error EditorNode::load_resource(
    const String& p_resource,
    bool p_ignore_broken_deps
) {
    ...
    if (!p_ignore_broken_deps && dependency_errors.has(p_resource)) {
        ...
        for (Set<String>::Element* E = dependency_errors[p_resource].front(); E;
             E                       = E->next()) {
            errors.push_back(E->get());
        }
        dependency_error
            ->show(DependencyErrorDialog::MODE_RESOURCE, p_resource, errors);
...
void EditorNode::save_resource_in_path(
...
void EditorNode::save_resource_as(
    const Ref<Resource>& p_resource,
    const String& p_at_path
) {
    {
        ...
        if (srpos != -1) {
            ...
            if (!get_edited_scene()
                || get_edited_scene()->get_filename() != base) {
                show_warning(
                    TTR("This resource can't be saved because it does not "
                        "belong to the edited scene. Make it unique first.")
                );
                return;
            }
        }
    ...
    if (p_at_path != String()) {
        ...
        if (p_resource->get_path().is_resource_file()) {
            ...
            if (extensions.size()) {
                file->set_current_file(
                    "new_" + p_resource->get_class().to_lower() + "."
                    + preferred.front()->get().to_lower()
                );
            } else {
        ...
        if (extensions.size()) {
            ...
            if (extensions.find(ext) == nullptr) {
                file->set_current_path(p_resource->get_path().replacen(
                    "." + ext,
                    "." + extensions.front()->get()
                ));
            }
        }
        ...
        String existing;
        if (extensions.size()) {
            existing = "new_" + p_resource->get_class().to_lower() + "."
                     + preferred.front()->get().to_lower();
        }
    ...
    if (p_error) {
        switch (p_error) {
            case ERR_FILE_CANT_WRITE: {
                show_accept(
                    TTR("Can't open file for writing:") + " "
            ...
            case ERR_FILE_UNRECOGNIZED: {
                show_accept(
                    TTR("Requested file format unknown:") + " "
                        + p_file.get_extension(),
                    TTR("OK")
                );
            } break;
    ...
    if (p_error) {
        switch (p_error) {
            case ERR_CANT_OPEN: {
                show_accept(
                    vformat(
                        TTR("Can't open '%s'. The file could have been moved "
                            "or deleted."),
                        p_file.get_file()
                    ),
                    TTR("OK")
            ...
            case ERR_FILE_NOT_FOUND: {
                show_accept(
                    vformat(
                        TTR("Missing '%s' or its dependencies."),
                        p_file.get_file()
                    ),
                    TTR("OK")
                );
...
void EditorNode::_get_scene_metadata(const String& p_file) {
    ...
    String path =
        EditorSettings::get_singleton()->get_project_settings_dir().plus_file(
            p_file.get_file() + "-editstate-" + p_file.md5_text() + ".cfg"
        );
...
void EditorNode::_set_scene_metadata(const String& p_file, int p_idx) {
    ...
    scene->set_meta(
        ...
        Variant()
    ); // clear it (no point in keeping it)
...
bool EditorNode::_find_and_save_resource(
    ...
    bool subchanged =
        _find_and_save_edited_subresources(p_res.ptr(), processed, flags);
    ...
    for (List<PropertyInfo>::Element* E = pi.front(); E; E = E->next()) {
        ...
        switch (E->get().type) {
            ...
            case Variant::DICTIONARY: {
                ...
                List<Variant> keys;
                d.get_key_list(&keys);
                for (List<Variant>::Element* F = keys.front(); F;
                     F                         = F->next()) {
                    Variant v = d[F->get()];
                    RES res   = v;
                    if (_find_and_save_resource(res, processed, flags)) {
...
void EditorNode::_save_scene_with_preview(String p_file, int p_idx) {
    ...
    if (editor_data.get_edited_scene_root() != nullptr) {
        ...
        // We cannot fallback on the 2D editor, because it may not have been
        // used yet, which would result in an invalid texture.
        if (c3d == 0 && c2d == 0) {
            img.instance();
            img->create(1, 1, false, Image::FORMAT_RGB8);
        } else if (c3d < c2d) {
            Ref<ViewportTexture> viewport_texture = scene_root->get_texture();
            if (viewport_texture->get_width() > 0
                && viewport_texture->get_height() > 0) {
                img = viewport_texture->get_data();
            }
        } else {
            // The 3D editor may be disabled as a feature, but scenes can still
            // be opened. This check prevents the preview from regenerating in
            // case those scenes are then saved. The preview will be generated
            // if no feature profile is set (as the 3D editor is enabled by
            // default).
            Ref<EditorFeatureProfile> profile =
                feature_profile_manager->get_current_profile();
            if (!profile.is_valid()
                || !profile->is_feature_disabled(
                    EditorFeatureProfile::FEATURE_3D
                )) {
        ...
        if (img.is_valid() && img->get_width() > 0 && img->get_height() > 0) {
            ...
            int preview_size = EditorSettings::get_singleton()->get(
...
bool EditorNode::_validate_scene_recursive(
...
static bool _find_edited_resources(
    const Ref<Resource>& p_resource,
    Set<Ref<Resource>>& edited_resources
) {
    ...
    for (List<PropertyInfo>::Element* E = plist.front(); E; E = E->next()) {
        if (E->get().type == Variant::OBJECT
            && E->get().usage & PROPERTY_USAGE_STORAGE
            && !(E->get().usage & PROPERTY_USAGE_RESOURCE_NOT_PERSISTENT)) {
            RES res = p_resource->get(E->get().name);
            if (res.is_null()) {
                continue;
            }
            if (res->get_path().is_resource_file(
                )) { // not a subresource, continue
                continue;
            }
...
int EditorNode::_save_external_resources() {
    ...
    for (List<Ref<Resource>>::Element* E = cached.front(); E; E = E->next()) {
        ...
        // not only check if this resourec is edited, check contained
        // subresources too
    ...
    // clear later, because user may have put the same subresource in two
    // different resources, which will be shared until the next reload

    for (Set<Ref<Resource>>::Element* E = edited_subresources.front(); E;
         E                              = E->next()) {
        Ref<Resource> res = E->get();
...
static void _reset_animation_players(
    Node* p_node,
    List<Ref<AnimatedValuesBackup>>* r_anim_backups
) {
    for (int i = 0; i < p_node->get_child_count(); i++) {
        AnimationPlayer* player =
            Object::cast_to<AnimationPlayer>(p_node->get_child(i));
        if (player && player->is_reset_on_save_enabled()
            && player->can_apply_reset()) {
...
void EditorNode::_save_scene(String p_file, int idx) {
    ...
    if (!scene) {
        show_accept(
            TTR("This operation can't be done without a tree root."),
            TTR("OK")
        );
        return;
    }

    if (scene->get_filename() != String()
        && _validate_scene_recursive(scene->get_filename(), scene)) {
        show_accept(
            TTR("This scene can't be saved because there is a cyclic "
                "instancing inclusion.\nPlease resolve it and then attempt to "
                "save again."),
            TTR("OK")
        );
        return;
    ...
    if (ResourceCache::has(p_file)) {
        ...
        // old version still work for referencing changes in instanced or
        // inherited scenes

        sdata = Ref<PackedScene>(
            Object::cast_to<PackedScene>(ResourceCache::get(p_file))
        );
        if (sdata.is_valid()) {
    ...
    for (List<Ref<AnimatedValuesBackup>>::Element* E = anim_backups.front(); E;
         E                                           = E->next()) {
        E->get()->restore();
    }
    ...
    if (err == OK) {
        ...
        editor_data.set_scene_modified_time(
            idx,
            FileAccess::get_modified_time(p_file)
        );
...
void EditorNode::_save_all_scenes() {
    ...
    for (int i = 0; i < editor_data.get_edited_scene_count(); i++) {
        ...
        if (scene) {
            if (scene->get_filename() != ""
                && DirAccess::exists(scene->get_filename().get_base_dir())) {
    ...
    if (!all_saved) {
        show_warning(
            TTR("Could not save one or more scenes!"),
            TTR("Save All Scenes")
        );
    }
    _save_default_environment();
...
void EditorNode::_dialog_action(String p_file) {
    switch (current_option) {
        case FILE_NEW_INHERITED_SCENE: {
            Node* scene = editor_data.get_edited_scene_root();
            // If the previous scene is rootless, just close it in favor of the
            // new one.
            if (!scene) {
        ...
        case SETTINGS_PICK_MAIN_SCENE: {
            ...
            if (pick_main_scene->has_meta("from_native")
                && (bool)pick_main_scene->get_meta("from_native")) {
        ...
        case FILE_SAVE_AS_SCENE: {
            int scene_idx = (current_option == FILE_SAVE_SCENE
                             || current_option == FILE_SAVE_AS_SCENE)
                              ? -1
                              : tab_closing;
            ...
            if (file->get_mode() == EditorFileDialog::MODE_SAVE_FILE) {
                ...
                for (int i = 0; i < editor_data.get_edited_scene_count(); i++) {
                    if (editor_data.get_scene_path(i) == p_file
                        && i != scene_idx) {
        ...
        case FILE_EXPORT_MESH_LIBRARY: {
            ...
            if (file_export_lib_merge->is_pressed()
                && FileAccess::exists(p_file)) {
                ml = ResourceLoader::load(p_file, "MeshLibrary");

                if (ml.is_null()) {
                    show_accept(
                        TTR("Can't load MeshLibrary for merging!"),
                        TTR("OK")
                    );
                    return;
                }
            ...
            MeshLibraryEditor::update_library_file(
                ...
                ml,
                true,
                file_export_lib_apply_xforms->is_pressed()
            );
        ...
        case FILE_EXPORT_TILESET: {
            ...
            if (FileAccess::exists(p_file)
                && file_export_lib_merge->is_pressed()) {
                tileset = ResourceLoader::load(p_file, "TileSet");

                if (tileset.is_null()) {
                    show_accept(
                        TTR("Can't load TileSet for merging!"),
                        TTR("OK")
                    );
                    return;
                }
            ...
            TileSetEditor::update_library_file(
                editor_data.get_edited_scene_root(),
                tileset,
                true
            );
        ...
        case RESOURCE_SAVE_AS: {
            ...
            Object* current_obj =
                current > 0 ? ObjectDB::get_instance(current) : nullptr;
        ...
        case SETTINGS_LAYOUT_SAVE: {
            ...
            if (err == ERR_FILE_CANT_OPEN || err == ERR_FILE_NOT_FOUND) {
                ...
                show_warning(
                    TTR("An error occurred while trying to save the editor "
                        "layout.\nMake sure the editor's user data path is "
                        "writable.")
                );
                return;
            }
            ...
            if (p_file == "Default") {
                show_warning(
                    TTR("Default editor layout overridden.\nTo restore the "
                        "Default layout to its base settings, use the Delete "
                        "Layout option and delete the Default layout.")
                );
            }
        ...
        case SETTINGS_LAYOUT_DELETE: {
            ...
            if (p_file == "Default") {
                show_warning(
                    TTR("Restored the Default layout to its base settings.")
                );
            }
...
bool EditorNode::_is_class_editor_disabled_by_feature_profile(
...
void EditorNode::edit_item(Object* p_object) {
    ...
    if (!sub_plugins.empty()) {
        bool same = true;
        if (sub_plugins.size()
            == editor_plugins_over->get_plugins_list().size()) {
            for (int i = 0; i < sub_plugins.size(); i++) {
                if (sub_plugins[i]
                    != editor_plugins_over->get_plugins_list()[i]) {
                    same = false;
                }
            }
        } else {
...
void EditorNode::push_item(
    Object* p_object,
    const String& p_property,
    bool p_inspector_only
) {
    if (!p_object) {
...
void EditorNode::_save_default_environment() {
    Ref<Environment> fallback =
        get_tree()->get_root()->get_world()->get_fallback_environment();
...
void EditorNode::_edit_current() {
    ...
    bool disable_folding =
        bool(EDITOR_GET("interface/inspector/disable_folding"));
    ...
    if (is_resource) {
        ...
        node_dock->set_node(nullptr);
        inspector_dock->update(nullptr);
        EditorNode::get_singleton()->get_import_dock()->set_edit_path(
            current_res->get_path()
        );
        ...
        if (subr_idx != -1) {
            ...
            if (FileAccess::exists(base_path + ".import")) {
                editable_warning =
                    TTR("This resource belongs to a scene that was imported, "
                        "so it's not editable.\nPlease read the documentation "
                        "relevant to importing scenes to better understand "
                        "this workflow.");
            } else {
                if ((!get_edited_scene()
                     || get_edited_scene()->get_filename() != base_path)
                    && ResourceLoader::get_resource_type(base_path)
                           == "PackedScene") {
                    editable_warning =
                        TTR("This resource belongs to a scene that was "
                            "instanced or inherited.\nChanges to it won't be "
                            "kept when saving the current scene.");
                }
            }
        } else if (current_res->get_path().is_resource_file()) {
            if (FileAccess::exists(current_res->get_path() + ".import")) {
                editable_warning = TTR(
                    "This resource was imported, so it's not editable. Change "
                    "its settings in the import panel and then re-import."
                );
            }
        }
    } else if (is_node) {
        ...
        if (get_edited_scene()
            && get_edited_scene()->get_filename() != String()) {
            String source_scene = get_edited_scene()->get_filename();
            if (FileAccess::exists(source_scene + ".import")) {
                editable_warning = TTR(
                    "This scene was imported, so changes to it won't be "
                    "kept.\nInstancing it or inheriting will allow making "
                    "changes to it.\nPlease read the documentation relevant to "
                    "importing scenes to better understand this workflow."
                );
            }
        }
        ...
        if (current_obj->is_class("ScriptEditorDebuggerInspectedObject")) {
            editable_warning =
                TTR("This is a remote object, so changes to it won't be "
                    "kept.\nPlease read the documentation relevant to "
                    "debugging to better understand this workflow.");
            disable_folding = true;
        } else if (current_obj->is_class("MultiNodeEdit")) {
            Node* scene = get_edited_scene();
            if (scene) {
                MultiNodeEdit* multi_node_edit =
                    Object::cast_to<MultiNodeEdit>(current_obj);
                int node_count = multi_node_edit->get_node_count();
                if (node_count > 0) {
                    List<Node*> multi_nodes;
                    for (int node_index = 0; node_index < node_count;
                         ++node_index) {
                        Node* node =
                            scene->get_node(multi_node_edit->get_node(node_index
                            ));
                        if (node) {
    ...
    if (!inspector_only) {
        ...
        for (; plugin_index < editor_table.size(); plugin_index++) {
            if (editor_table[plugin_index] == main_plugin) {
                if (!main_editor_buttons[plugin_index]->is_visible()) {
                    main_plugin = nullptr; // if button is not visible, then no
                                           // plugin active
                }
        ...
        if (main_plugin) {
            ...
            if (main_plugin->get_name() == "Script"
        ...
        if (!_is_class_editor_disabled_by_feature_profile(
                current_obj->get_class()
            )) {
...
void EditorNode::_run(bool p_current, const String& p_custom) {
    ...
    if (p_current
        || (editor_data.get_edited_scene_root() && p_custom != String()
    ...
    if (bool(EDITOR_GET("run/auto_save/save_before_running"))) {
        if (unsaved_cache) {
            ...
            if (scene
                && scene->get_filename() != "") { // Only autosave if there is a
                                                  // scene and if it has a path.
                _save_scene_with_preview(scene->get_filename());
            }
        }
    ...
    Error error =
        editor_run.run(run_filename, args, breakpoints, skip_breakpoints);
    ...
    if (p_current) {
        ...
        play_custom_scene_button->set_icon(
            gui_base->get_icon("Reload", "EditorIcons")
        );
    } else {
    ...
    switch (p_option) {
        ...
        case FILE_OPEN_SCENE: {
            ...
            for (int i = 0; i < extensions.size(); i++) {
                file->add_filter(
                    "*." + extensions[i] + " ; " + extensions[i].to_upper()
                );
            }
            ...
            file->set_title(
                p_option == FILE_OPEN_SCENE ? TTR("Open Scene")
                                            : TTR("Open Base Scene")
            );
            file->popup_centered_ratio();
        ...
        case FILE_CLOSE: {
            if (!p_confirmed) {
                tab_closing = p_option == FILE_CLOSE
                                ? editor_data.get_edited_scene()
                                : _next_unsaved_scene(false);
                _scene_tab_changed(tab_closing);

                if (unsaved_cache || p_option == FILE_CLOSE_ALL_AND_QUIT
                    || p_option == FILE_CLOSE_ALL_AND_RUN_PROJECT_MANAGER) {
                    Node* scene_root =
                        editor_data.get_edited_scene_root(tab_closing);
                    if (scene_root) {
                        String scene_filename = scene_root->get_filename();
                        save_confirmation->get_ok()->set_text(TTR("Save & Close"
                        ));
                        save_confirmation->set_text(vformat(
                            TTR("Save changes to '%s' before closing?"),
        ...
        case FILE_SAVE_SCENE: {
            ...
            if (scene && scene->get_filename() != "") {
                if (DirAccess::exists(scene->get_filename().get_base_dir())) {
                    ...
                    show_save_accept(
                        vformat(
                            TTR("%s no longer exists! Please specify a new "
                                "save location."),
                            scene->get_filename().get_base_dir()
                        ),
                        TTR("OK")
                    );
                }
        ...
        case FILE_SAVE_AS_SCENE: {
            int scene_idx =
                (p_option == FILE_SAVE_SCENE || p_option == FILE_SAVE_AS_SCENE)
                    ? -1
                    : tab_closing;
            ...
            if (!scene) {
                if (p_option == FILE_SAVE_SCENE) {
                    // Pressing Ctrl + S saves the current script if a scene is
                    // currently open, but it won't if the scene has no root
                    // node. Work around this by explicitly saving the script in
                    // this case (similar to pressing Ctrl + Alt + S).
                ...
                if (saved > 0) {
                    ...
                    // Don't show this dialog when pressing Ctrl + S to avoid
                    // interfering with script saving.
                    show_accept(
                        TTR("A root node is required to save the scene. You "
                            "can add a root node using the Scene tree dock."),
                        TTR("OK")
                    );
                }
            ...
            if (scene->get_filename() != "") {
                ...
                if (extensions.size()) {
                    ...
                    if (extensions.find(ext) == nullptr) {
                        file->set_current_path(path.replacen(
                            "." + ext,
                            "." + extensions.front()->get()
                        ));
                    }
                ...
                if (extensions.size()) {
                    ...
                    existing =
                        root_name + "." + extensions.front()->get().to_lower();
                }
                file->set_current_path(existing);
            }
            file->popup_centered_ratio();
        ...
        case FILE_EXPORT_TILESET: {
            // Make sure that the scene has a root before trying to convert to
            // tileset
            if (!editor_data.get_edited_scene_root()) {
                show_accept(
                    TTR("This operation can't be done without a root node."),
                    TTR("OK")
                );
                break;
            }
        ...
        case FILE_IMPORT_SUBSCENE: {
            if (!editor_data.get_edited_scene_root()) {
                show_accept(
                    TTR("This operation can't be done without a selected node."
                    ),
                    TTR("OK")
                );
                break;
        ...
        case FILE_EXTERNAL_OPEN_SCENE: {
            if (unsaved_cache && !p_confirmed) {
                ...
                confirmation->set_text(
                    TTR("Current scene not saved. Open anyway?")
                );
        ...
        case EDIT_UNDO: {
            if (Input::get_singleton()->get_mouse_button_mask() & 0x7) {
                log->add_message(
                    TTR("Can't undo while mouse buttons are pressed."),
                    EditorLog::MSG_TYPE_EDITOR
                );
            } else {
                String action =
                    editor_data.get_undo_redo().get_current_action_name();
        ...
        case EDIT_REDO: {
            if (Input::get_singleton()->get_mouse_button_mask() & 0x7) {
                log->add_message(
                    TTR("Can't redo while mouse buttons are pressed."),
                    EditorLog::MSG_TYPE_EDITOR
                );
            } else {
            ...
            if (unsaved_cache && !p_confirmed) {
                ...
                confirmation->set_text(
                    TTR("The current scene has unsaved changes.\nReload the "
                        "saved scene anyway? This action cannot be undone.")
                );
                confirmation->popup_centered_minsize();
                break;
            }
        ...
        case FILE_SHOW_IN_FILESYSTEM: {
            String path =
                editor_data.get_scene_path(editor_data.get_edited_scene());
            if (path != String()) {
        ...
        case RUN_PROJECT_DATA_FOLDER: {
            // ensure_user_data_dir() to prevent the edge case: "Open Project
            // Data Folder" won't work after the project was renamed in
            // ProjectSettingsEditor unless the project is saved
        ...
        case RUN_PROJECT_MANAGER: {
            if (!p_confirmed) {
                bool save_each =
                    EDITOR_GET("interface/editor/save_each_scene_on_quit");
                if (_next_unsaved_scene(!save_each) == -1) {
                    bool confirm =
                        EDITOR_GET("interface/editor/quit_confirmation");
                    if (confirm) {
                        confirmation->get_ok()->set_text(
                    ...
                    if (save_each) {
                        ...
                        while (i != -1) {
                            unsaved_scenes +=
                                "\n            "
                                + editor_data.get_edited_scene_root(i)
                                      ->get_filename();
        ...
        case RUN_FILE_SERVER: {
            bool ischecked = debug_menu->get_popup()->is_item_checked(
                debug_menu->get_popup()->get_item_index(RUN_FILE_SERVER)
            );
        ...
        case RUN_LIVE_DEBUG: {
            bool ischecked = debug_menu->get_popup()->is_item_checked(
                debug_menu->get_popup()->get_item_index(RUN_LIVE_DEBUG)
            );
            ...
            ScriptEditor::get_singleton()->get_debugger()->set_live_debugging(
                !ischecked
            );
            EditorSettings::get_singleton()->set_project_metadata(
                "debug_options",
        ...
        case RUN_RELOAD_SCRIPTS: {
            ...
            ScriptEditor::get_singleton()->set_live_auto_reload_running_scripts(
                !ischecked
            );
            EditorSettings::get_singleton()->set_project_metadata(
                "debug_options",
        ...
        case SETTINGS_UPDATE_CONTINUOUSLY: {
            ...
            _update_update_spinner();
            show_accept(
                TTR("This option is deprecated. Situations where refresh must "
                    "be forced are now considered a bug. Please report."),
                TTR("OK")
            );
        } break;
        ...
        case SETTINGS_EDITOR_DATA_FOLDER: {
            OS::get_singleton()->shell_open(
                String("file://")
                + EditorSettings::get_singleton()->get_data_dir()
            );
        } break;
        ...
        case SETTINGS_TOGGLE_FULLSCREEN: {
            OS::get_singleton()->set_window_fullscreen(
                !OS::get_singleton()->is_window_fullscreen()
            );
        ...
        case HELP_REPORT_A_BUG: {
            OS::get_singleton()->shell_open(
                "https://github.com/RebelToolbox/RebelEngine/issues/new/choose"
            );
        } break;
        case HELP_ABOUT: {
            about->popup_centered_minsize(Size2(780, 500) * EDSCALE);
        } break;
        case HELP_SUPPORT_DEVELOPMENT: {
            OS::get_singleton()->shell_open(
                "mailto:info@rebeltoolbox.com?subject=I%20want%20to%20donate"
            );
        } break;
...
void EditorNode::_screenshot(bool p_use_utc) {
    String name =
        "editor_screenshot_"
        + OS::get_singleton()->get_iso_date_time(p_use_utc).replace(":", "")
        + ".png";
...
void EditorNode::_save_screenshot(NodePath p_path) {
    Viewport* viewport =
        EditorInterface::get_singleton()->get_editor_viewport()->get_viewport();
    ...
    ERR_FAIL_COND_MSG(
        error != OK,
        "Cannot save screenshot to file '" + p_path + "'."
    );
}
...
void EditorNode::_exit_editor() {
    ...
    // Dim the editor window while it's quitting to make it clearer that it's
    // busy
    ...
    switch (current_option) {
        ...
        case SCENE_TAB_CLOSE: {
            ...
            if (current_option == FILE_CLOSE_ALL_AND_QUIT
                || current_option == FILE_CLOSE_ALL_AND_RUN_PROJECT_MANAGER) {
                // If restore tabs is enabled, reopen the scene that has just
                // been closed, so it's remembered properly.
                if (bool(EDITOR_GET(
                ...
                if (_next_unsaved_scene(false) == -1) {
                    current_option = current_option == FILE_CLOSE_ALL_AND_QUIT
                                       ? FILE_QUIT
                                       : RUN_PROJECT_MANAGER;
                    _discard_changes();
                } else {
                ...
                if (editor_data.get_edited_scene_count() == 1
                    || (current_option == FILE_CLOSE_RIGHT
                        && editor_data.get_edited_scene_count()
                               <= editor_data.get_edited_scene() + 1)) {
                    current_option = -1;
...
void EditorNode::_update_debug_options() {
    ...
    bool check_file_server =
        EditorSettings::get_singleton()
            ->get_project_metadata("debug_options", "run_file_server", false);
    ...
    bool check_live_debug =
        EditorSettings::get_singleton()
            ->get_project_metadata("debug_options", "run_live_debug", true);
    bool check_reload_scripts =
        EditorSettings::get_singleton()
            ->get_project_metadata("debug_options", "run_reload_scripts", true);
...
void EditorNode::_update_file_menu_opened() {
    ...
    Ref<ShortCut> reopen_closed_scene_sc =
        ED_GET_SHORTCUT("editor/reopen_closed_scene");
    reopen_closed_scene_sc->set_name(TTR("Reopen Closed Scene"));

    PopupMenu* pop = file_menu->get_popup();
    pop->set_item_disabled(
        pop->get_item_index(FILE_OPEN_PREV),
        previous_scenes.empty()
    );

    const UndoRedo& undo_redo = editor_data.get_undo_redo();
    pop->set_item_disabled(
        pop->get_item_index(EDIT_UNDO),
        !undo_redo.has_undo()
    );
    pop->set_item_disabled(
        pop->get_item_index(EDIT_REDO),
        !undo_redo.has_redo()
    );
}
...
void EditorNode::_editor_select(int p_which) {
    ...
    if (!main_editor_buttons[p_which]->is_visible(
        )) { // button hidden, no editor
        return;
    }
    ...
    for (int i = 0; i < plugin_count; i++) {
        editor_data.get_editor_plugin(i)->notify_main_screen_changed(
            editor_plugin_screen->get_name()
        );
    }
...
void EditorNode::add_editor_plugin(
    EditorPlugin* p_editor,
    ...
    if (p_editor->has_main_screen()) {
        ...
        tb->connect(
            ...
            singleton,
            "_editor_select",
            varray(singleton->main_editor_buttons.size())
        );
        ...
        if (icon.is_valid()) {
            tb->set_icon(icon);
        } else if (singleton->gui_base
                       ->has_icon(p_editor->get_name(), "EditorIcons")) {
            tb->set_icon(singleton->gui_base
                             ->get_icon(p_editor->get_name(), "EditorIcons"));
        }
    ...
    if (p_editor->has_main_screen()) {
        for (int i = 0; i < singleton->main_editor_buttons.size(); i++) {
            if (p_editor->get_name()
                == singleton->main_editor_buttons[i]->get_text()) {
...
void EditorNode::_update_addon_config() {
    ...
    for (Map<String, EditorPlugin*>::Element* E = plugin_addons.front(); E;
         E                                      = E->next()) {
...
void EditorNode::set_addon_plugin_enabled(
    String p_addon,
    bool p_enabled,
    bool p_config_changed
) {
    ...
    if (err != OK) {
        show_warning(vformat(
            TTR("Unable to enable addon plugin at: '%s' parsing of config "
                "failed."),
            p_addon
        ));
        return;
    ...
    if (!cf->has_section_key("plugin", "script")) {
        show_warning(vformat(
            TTR("Unable to find script field for addon plugin at: '%s'."),
            p_addon
        ));
        return;
    }
    ...
    // Only try to load the script if it has a name. Else, the plugin has no
    // init script.
    if (script_path.length() > 0) {
        ...
        if (script.is_null()) {
            show_warning(vformat(
                TTR("Unable to load addon script from path: '%s'."),
                script_path
            ));
            return;
        }
        ...
        if (String(script->get_instance_base_type()) == "") {
            show_warning(vformat(
                TTR("Unable to load addon script from path: '%s'. This might "
                    "be due to a code error in that script.\nDisabling the "
                    "addon at '%s' to prevent further errors."),
                script_path,
                p_addon
            ));
            _remove_plugin_from_enabled(p_addon);
        ...
        if (String(script->get_instance_base_type()) != "EditorPlugin") {
            show_warning(vformat(
                TTR("Unable to load addon script from path: '%s' Base type is "
                    "not EditorPlugin."),
                script_path
            ));
            return;
        ...
        if (!script->is_tool()) {
            show_warning(vformat(
                TTR("Unable to load addon script from path: '%s' Script is not "
                    "in tool mode."),
                script_path
            ));
            return;
...
void EditorNode::set_edited_scene(Node* p_scene) {
    if (get_editor_data().get_edited_scene_root()) {
        if (get_editor_data().get_edited_scene_root()->get_parent()
            == scene_root) {
...
Dictionary EditorNode::_get_main_scene_state() {
    ...
    state["scene_tree_offset"] = scene_tree_dock->get_tree_editor()
                                     ->get_scene_tree()
                                     ->get_vscroll_bar()
                                     ->get_value();
...
void EditorNode::_set_main_scene_state(Dictionary p_state, Node* p_for_scene) {
    ...
    if (p_state.has("editor_index")) {
        ...
        if (current < 2) { // if currently in spatial/2d, only switch to
                           // spatial/2d. if currently in script, stay there
    ...
    if (p_state.has("scene_tree_offset")) {
        scene_tree_dock->get_tree_editor()
            ->get_scene_tree()
            ->get_vscroll_bar()
            ->set_value(p_state["scene_tree_offset"]);
    }
...
void EditorNode::set_current_scene(int p_idx) {
    ...
    if (editor_data.get_scene_path(p_idx) != ""
        && editor_data.get_edited_scene_root(p_idx)) {
        editor_folding.save_scene_folding(
            editor_data.get_edited_scene_root(p_idx),
            editor_data.get_scene_path(p_idx)
        );
    }
    ...
    changing_scene = true;
    editor_data.save_edited_scene_state(
        editor_selection,
        &editor_history,
    ...
    Dictionary state = editor_data.restore_edited_scene_state(
        editor_selection,
        &editor_history
    );
    _edit_current();
    ...
    call_deferred(
        ...
        state,
        get_edited_scene()
    ); // do after everything else is done setting up
}
...
Error EditorNode::load_scene(
    const String& p_scene,
    bool p_ignore_broken_deps,
    bool p_set_inherited,
    bool p_clear_errors,
    bool p_force_open_imported,
    bool p_silent_change_tab
) {
    if (!is_inside_tree()) {
    ...
    if (!p_set_inherited) {
        ...
        if (!p_force_open_imported && FileAccess::exists(p_scene + ".import")) {
            open_imported->set_text(vformat(
                TTR("Scene '%s' was automatically imported, so it can't be "
                    "modified.\nTo make changes to it, a new inherited scene "
                    "can be created."),
                p_scene.get_file()
            ));
    ...
    if (!lpath.begins_with("res://")) {
        show_accept(
            TTR("Error loading scene, it must be inside the project path. Use "
                "'Import' to open the scene, then save it inside the project "
                "path."),
            TTR("OK")
        );
        opening_prev = false;
    ...
    if (!editor_data.get_edited_scene_root()
        && editor_data.get_edited_scene_count() == 2) {
        _remove_edited_scene();
    ...
    if (!p_ignore_broken_deps && dependency_errors.has(lpath)) {
        current_option = -1;
        Vector<String> errors;
        for (Set<String>::Element* E = dependency_errors[lpath].front(); E;
             E                       = E->next()) {
            errors.push_back(E->get());
        }
        dependency_error
            ->show(DependencyErrorDialog::MODE_SCENE, lpath, errors);
        opening_prev = false;
    ...
    for (Map<String, Set<String>>::Element* E = dependency_errors.front(); E;
         E                                    = E->next()) {
        String txt =
            vformat(TTR("Scene '%s' has broken dependencies:"), E->key())
            + "\n";
    ...
    if (ResourceCache::has(lpath)) {
        ...
        Ref<PackedScene> ps = Ref<PackedScene>(
            Object::cast_to<PackedScene>(ResourceCache::get(lpath))
        );
        if (ps.is_valid()) {
...
void EditorNode::_add_to_recent_scenes(const String& p_scene) {
    Array rc = EditorSettings::get_singleton()
                   ->get_project_metadata("recent_files", "scenes", Array());
    ...
    EditorSettings::get_singleton()
        ->set_project_metadata("recent_files", "scenes", rc);
...
void EditorNode::_open_recent_scene(int p_idx) {
    if (p_idx == recent_scenes->get_item_count() - 1) {
        EditorSettings::get_singleton()
            ->set_project_metadata("recent_files", "scenes", Array());
...
void EditorNode::_update_recent_scenes() {
    ...
    recent_scenes->add_shortcut(
        ED_SHORTCUT("editor/clear_recent", TTR("Clear Recent Scenes"))
    );
    recent_scenes->set_as_minsize();
}
...
Ref<Script> EditorNode::get_object_custom_type_base(const Object* p_object
) const {
    ...
    if (script.is_valid()) {
        // Uncommenting would break things! Consider adding a parameter if you
        // need it. StringName name =
        // EditorNode::get_editor_data().script_class_get_name(base_script->get_path());
        // if (name != StringName())
        // 	return name;
        ...
        if (base != StringName()
            && EditorNode::get_editor_data().get_custom_types().has(base)) {
            const Vector<EditorData::CustomType>& types =
                EditorNode::get_editor_data().get_custom_types()[base];
...
StringName EditorNode::get_object_custom_type_name(const Object* p_object
) const {
    ...
    if (script.is_valid()) {
        ...
        while (base_script.is_valid()) {
            StringName name =
                EditorNode::get_editor_data().script_class_get_name(
                    base_script->get_path()
                );
            if (name != StringName()) {
                return name;
            }
...
void EditorNode::_pick_main_scene_custom_action(
    const String& p_custom_action_name
) {
...
Ref<Texture> EditorNode::get_object_icon(
    ...
    if (script.is_valid()) {
        ...
        while (base_script.is_valid()) {
            ...
            String icon_path =
                EditorNode::get_editor_data().script_class_get_icon_path(name);
...
Ref<Texture> EditorNode::get_class_icon(
    ...
    if (ScriptServer::is_global_class(p_class)) {
        ...
        Ref<Script> script =
            EditorNode::get_editor_data().script_class_load_script(class_name);

        while (true) {
            String icon_path =
                EditorNode::get_editor_data().script_class_get_icon_path(
                    class_name
                );
            ...
            do {
                ...
                if (base_script.is_null()) {
                    ...
                    String base_type;
                    script->get_language()->get_global_class_name(
                        script->get_path(),
                        &base_type
                    );
    ...
    const Map<String, Vector<EditorData::CustomType>>& p_map =
        EditorNode::get_editor_data().get_custom_types();
    for (const Map<String, Vector<EditorData::CustomType>>::Element* E =
             p_map.front();
         E;
         E = E->next()) {
...
void EditorNode::progress_add_task(
    const String& p_task,
    ...
    if (singleton->cmdline_export_mode) {
        ...
        singleton->progress_dialog
            ->add_task(p_task, p_label, p_steps, p_can_cancel);
    }
}

bool EditorNode::progress_task_step(
    const String& p_task,
    ...
    if (singleton->cmdline_export_mode) {
        ...
        return singleton->progress_dialog
            ->task_step(p_task, p_state, p_step, p_force_refresh);
    }
}
...
void EditorNode::progress_add_task_bg(
    const String& p_task,
    const String& p_label,
    int p_steps
) {
...
void EditorNode::_build_icon_type_cache() {
    ...
    for (List<StringName>::Element* E = tl.front(); E; E = E->next()) {
        ...
        icon_type_cache[E->get()] =
            theme_base->get_theme()->get_icon(E->get(), ei);
    }
}
...
void EditorNode::_dock_select_input(const Ref<InputEvent>& p_input) {
    ...
    if (me.is_valid()) {
        ...
        if (mb.is_valid() && mb->get_button_index() == 1 && mb->is_pressed()
            ...
            Control* dock =
                dock_slot[dock_popup_selected]->get_current_tab_control();
            if (dock) {
            ...
            dock_popup_selected = nrect;
            dock_slot[nrect]->set_current_tab(
                dock_slot[nrect]->get_tab_count() - 1
            );
            dock_slot[nrect]->show();
            dock_select->update();

            for (int i = 0; i < vsplits.size(); i++) {
                bool in_use = dock_slot[i * 2 + 0]->get_tab_count()
                           || dock_slot[i * 2 + 1]->get_tab_count();
                if (in_use) {
                    vsplits[i]->show();
                } else {
                    vsplits[i]->hide();
...
void EditorNode::_dock_move_left() {
    ...
    Control* current = dock_slot[dock_popup_selected]->get_tab_control(
        dock_slot[dock_popup_selected]->get_current_tab()
    );
    Control* prev = dock_slot[dock_popup_selected]->get_tab_control(
        dock_slot[dock_popup_selected]->get_current_tab() - 1
    );
    if (!current || !prev) {
        return;
    }
    dock_slot[dock_popup_selected]->move_child(current, prev->get_index());
    dock_slot[dock_popup_selected]->set_current_tab(
        dock_slot[dock_popup_selected]->get_current_tab() - 1
    );
    dock_select->update();
    _edit_current();
...
void EditorNode::_dock_move_right() {
    ...
    Control* next = dock_slot[dock_popup_selected]->get_tab_control(
        dock_slot[dock_popup_selected]->get_current_tab() + 1
    );
    if (!current || !next) {
        return;
...
void EditorNode::_dock_select_draw() {
    ...
    if (dock_popup_selected != -1
        && dock_slot[dock_popup_selected]->get_tab_count()) {
        dock_tab_move_left->set_disabled(
            dock_slot[dock_popup_selected]->get_current_tab() == 0
        );
        dock_tab_move_right->set_disabled(
            dock_slot[dock_popup_selected]->get_current_tab()
    ...
    for (int i = 0; i < DOCK_SLOT_MAX; i++) {
        ...
        for (int j = 0; j < MIN(3, dock_slot[i]->get_child_count()); j++) {
            ...
            if (i == dock_popup_selected
                && (dock_slot[i]->get_current_tab() > 3
                    || dock_slot[i]->get_current_tab() == j)) {
                c = tab_selected;
            }
            dock_select->draw_rect(
                Rect2(2 + ofs.x + xofs, ofs.y, r.size.width / 3 - 1, 3),
                c
            );
        }
    }
...
void EditorNode::_save_docks_to_config(
    ...
    p_layout->set_value(
        p_section,
        "dock_filesystem_split",
        filesystem_dock->get_split_offset()
    );
    p_layout->set_value(
        p_section,
        "dock_filesystem_display_mode",
        filesystem_dock->get_display_mode()
    );
    p_layout->set_value(
        p_section,
        "dock_filesystem_file_list_display_mode",
        filesystem_dock->get_file_list_display_mode()
    );
    ...
    for (int i = 0; i < hsplits.size(); i++) {
        p_layout->set_value(
            p_section,
            "dock_hsplit_" + itos(i + 1),
            hsplits[i]->get_split_offset()
        );
    }
}

void EditorNode::_save_open_scenes_to_config(
    Ref<ConfigFile> p_layout,
    const String& p_section
) {
    Array scenes;
...
void EditorNode::_load_docks_from_config(
    Ref<ConfigFile> p_layout,
    const String& p_section
) {
    for (int i = 0; i < DOCK_SLOT_MAX; i++) {
        ...
        Vector<String> names =
            String(p_layout->get_value(p_section, "dock_" + itos(i + 1)))
                .split(",");
    ...
    if (p_layout->has_section_key(p_section, "dock_filesystem_split")) {
        int fs_split_ofs =
            p_layout->get_value(p_section, "dock_filesystem_split");
    ...
    if (p_layout->has_section_key(p_section, "dock_filesystem_display_mode")) {
        FileSystemDock::DisplayMode dock_filesystem_display_mode =
            FileSystemDock::DisplayMode(int(
                p_layout->get_value(p_section, "dock_filesystem_display_mode")
            ));
        filesystem_dock->set_display_mode(dock_filesystem_display_mode);
    }

    if (p_layout->has_section_key(
            p_section,
            "dock_filesystem_file_list_display_mode"
        )) {
        FileSystemDock::FileListDisplayMode
            dock_filesystem_file_list_display_mode =
                FileSystemDock::FileListDisplayMode(int(p_layout->get_value(
                    p_section,
                    "dock_filesystem_file_list_display_mode"
                )));
        filesystem_dock->set_file_list_display_mode(
            dock_filesystem_file_list_display_mode
        );
    }
    ...
    for (int i = 0; i < vsplits.size(); i++) {
        if (!p_layout
                 ->has_section_key(p_section, "dock_split_" + itos(i + 1))) {
            continue;
        }
    ...
    for (int i = 0; i < hsplits.size(); i++) {
        if (!p_layout
                 ->has_section_key(p_section, "dock_hsplit_" + itos(i + 1))) {
            continue;
        }
...
void EditorNode::_load_open_scenes_from_config(
    Ref<ConfigFile> p_layout,
    const String& p_section
) {
    ...
    if (!p_layout->has_section(p_section)
        || !p_layout->has_section_key(p_section, "open_scenes")) {
        return;
    }
...
bool EditorNode::has_scenes_in_session() {
    ...
    if (!config->has_section("EditorNode")
        || !config->has_section_key("EditorNode", "open_scenes")) {
        return false;
    }
...
bool EditorNode::ensure_main_scene(bool p_from_native) {
    pick_main_scene->set_meta(
        "from_native",
        p_from_native
    ); // whether from play button or native run
    String main_scene = GLOBAL_DEF("application/run/main_scene", "");

    if (main_scene == "") {
        current_option = -1;
        pick_main_scene->set_text(TTR(
            "No main scene has ever been defined, select one?\nYou can change "
            "it later in \"Project Settings\" under the 'application' category."
        ));
    ...
    if (!FileAccess::exists(main_scene)) {
        current_option = -1;
        pick_main_scene->set_text(vformat(
            TTR("Selected scene '%s' does not exist, select a valid one?\nYou "
                "can change it later in \"Project Settings\" under the "
    ...
    if (ResourceLoader::get_resource_type(main_scene) != "PackedScene") {
        current_option = -1;
        pick_main_scene->set_text(vformat(
            TTR("Selected scene '%s' is not a scene file, select a valid "
                "one?\nYou can change it later in \"Project Settings\" under "
...
bool EditorNode::is_run_playing() const {
    ...
    return (
        status == EditorRun::STATUS_PLAY || status == EditorRun::STATUS_PAUSED
    );
}
...
String EditorNode::get_run_playing_scene() const {
    ...
    if (run_filename == "" && is_run_playing()) {
        run_filename = GLOBAL_DEF(
            "application/run/main_scene",
            ""
        ); // Must be the main scene then.
    }
...
void EditorNode::_update_layouts_menu() {
    ...
    editor_layouts->add_shortcut(
        ED_SHORTCUT("layout/save", TTR("Save Layout")),
        SETTINGS_LAYOUT_SAVE
    );
    editor_layouts->add_shortcut(
        ED_SHORTCUT("layout/delete", TTR("Delete Layout")),
        SETTINGS_LAYOUT_DELETE
    );
    editor_layouts->add_separator();
    editor_layouts->add_shortcut(
        ED_SHORTCUT("layout/default", TTR("Default")),
    ...
    for (List<String>::Element* E = layouts.front(); E; E = E->next()) {
        ...
        if (layout == TTR("Default")) {
            editor_layouts->remove_item(
                editor_layouts->get_item_index(SETTINGS_LAYOUT_DEFAULT)
            );
...
void EditorNode::_layout_menu_option(int p_id) {
    switch (p_id) {
        ...
        default: {
            ...
            _load_docks_from_config(
                config,
                editor_layouts->get_item_text(p_id)
            );
            _save_docks();
        }
    }
...
void EditorNode::_scene_tab_closed(int p_tab, int option) {
    ...
    bool unsaved =
        (p_tab == editor_data.get_edited_scene())
            ? saved_version != editor_data.get_undo_redo().get_version()
    ...
    if (unsaved) {
        ...
        save_confirmation->set_text(vformat(
            TTR("Save changes to '%s' before closing?"),
            scene->get_filename() != "" ? scene->get_filename()
                                        : "unsaved scene"
        ));
        save_confirmation->popup_centered_minsize();
    } else {
    ...
    if (p_tab == current_tab || p_tab < 0) {
        ...
        if (path != String()) {
            EditorResourcePreview::get_singleton()
                ->queue_resource_preview(path, this, "_thumbnail_done", p_tab);
        }
    }
}
...
void EditorNode::_scene_tab_input(const Ref<InputEvent>& p_input) {
    ...
    if (mb.is_valid()) {
        ...
        if (mb->get_button_index() == BUTTON_RIGHT && mb->is_pressed()) {
            ...
            scene_tabs_context_menu->add_shortcut(
                ED_GET_SHORTCUT("editor/new_scene"),
                FILE_NEW_SCENE
            );
            if (scene_tabs->get_hovered_tab() >= 0) {
                scene_tabs_context_menu->add_shortcut(
                    ED_GET_SHORTCUT("editor/save_scene"),
                    FILE_SAVE_SCENE
                );
                scene_tabs_context_menu->add_shortcut(
                    ED_GET_SHORTCUT("editor/save_scene_as"),
                    FILE_SAVE_AS_SCENE
                );
            }
            scene_tabs_context_menu->add_shortcut(
                ED_GET_SHORTCUT("editor/save_all_scenes"),
                FILE_SAVE_ALL_SCENES
            );
            if (scene_tabs->get_hovered_tab() >= 0) {
                scene_tabs_context_menu->add_separator();
                scene_tabs_context_menu->add_item(
                    TTR("Show in FileSystem"),
                ...
                if (previous_scenes.empty()) {
                    scene_tabs_context_menu->set_item_disabled(
                        scene_tabs_context_menu->get_item_index(FILE_OPEN_PREV),
                        true
                    );
                }
                scene_tabs_context_menu->add_item(
                    TTR("Close Other Tabs"),
                    FILE_CLOSE_OTHERS
                );
                scene_tabs_context_menu->add_item(
                    TTR("Close Tabs to the Right"),
                    FILE_CLOSE_RIGHT
                );
                scene_tabs_context_menu->add_item(
                    TTR("Close All Tabs"),
...
void EditorNode::_thumbnail_done(
    const String& p_path,
    ...
    if (p_preview.is_valid()) {
        ...
        tab_preview_panel->set_position(
...
void EditorNode::_scene_tab_changed(int p_tab) {
    ...
    editor_data.get_undo_redo().add_do_method(
        this,
        "set_current_version",
        unsaved ? saved_version : 0
    );
    editor_data.get_undo_redo().add_do_method(this, "set_current_scene", p_tab);
    editor_data.get_undo_redo().add_do_method(
        this,
        "set_current_version",
        next_scene_version == 0 ? editor_data.get_undo_redo().get_version() + 1
                                : next_scene_version
    );

    editor_data.get_undo_redo()
        .add_undo_method(this, "set_current_version", next_scene_version);
    ...
    editor_data.get_undo_redo()
        .add_undo_method(this, "set_current_version", saved_version);
...
ToolButton* EditorNode::add_bottom_panel_item(String p_text, Control* p_item) {
    ...
    tb->connect(
        "toggled",
        this,
        "_bottom_panel_switch",
        varray(bottom_panel_items.size())
    );
    tb->set_text(p_text);
    tb->set_toggle_mode(true);
...
void EditorNode::raise_bottom_panel_item(Control* p_item) {
    for (int i = 0; i < bottom_panel_items.size(); i++) {
        if (bottom_panel_items[i].control == p_item) {
            ...
            SWAP(
                bottom_panel_items.write[i],
                bottom_panel_items.write[bottom_panel_items.size() - 1]
            );
            break;
        }
    }
    ...
    for (int i = 0; i < bottom_panel_items.size(); i++) {
        bottom_panel_items[i]
            .button->disconnect("toggled", this, "_bottom_panel_switch");
        bottom_panel_items[i].button->connect(
            "toggled",
            this,
            "_bottom_panel_switch",
            varray(i)
    ...
    if (p_enable) {
        ...
        if (ScriptEditor::get_singleton()->get_debugger()
            == bottom_panel_items[p_idx]
                   .control) { // this is the debug panel which uses tabs, so
                               // the top section should be smaller
            bottom_panel->add_style_override(
                "panel",
                gui_base->get_stylebox(
                    "BottomPanelDebuggerOverride",
...
Variant EditorNode::drag_resource(const Ref<Resource>& p_res, Control* p_from) {
...

    {
        ...
        Ref<ImageTexture> pic =
            gui_base->get_icon("FileBigThumb", "EditorIcons");
    ...
    label->set_position(Point2(
        (preview->get_width() - label->get_minimum_size().width) / 2,
        preview->get_height()
    ));
...
Variant EditorNode::drag_files_and_dirs(
    ...
    for (int i = 0; i < num_rows; i++) {
        ...
        if (p_paths[i].ends_with("/")) {
            label->set_text(
                p_paths[i].substr(0, p_paths[i].length() - 1).get_file()
            );
    ...
    if (p_paths.size() > num_rows) {
        ...
        if (has_file && has_folder) {
            label->set_text(vformat(
                TTR("%d more files or folders"),
                p_paths.size() - num_rows
            ));
        } else if (has_folder) {
            label->set_text(
                vformat(TTR("%d more folders"), p_paths.size() - num_rows)
            );
        } else {
            label->set_text(
                vformat(TTR("%d more files"), p_paths.size() - num_rows)
            );
        }
...
void EditorNode::add_tool_submenu_item(
    const String& p_name,
    PopupMenu* p_submenu
) {
...
void EditorNode::_global_menu_action(
    const Variant& p_id,
...
void EditorNode::_add_dropped_files_recursive(
...
void EditorNode::_file_access_close_error_notify(const String& p_str) {
    add_io_error(
        "Unable to write to file '" + p_str
        + "', file in use, locked or lacking permissions."
    );
}

void EditorNode::reload_scene(const String& p_path) {
    // first of all, reload internal textures, materials, meshes, etc. as they
    // might have changed on disk

    List<Ref<Resource>> cached;
    ResourceCache::get_cached_resources(&cached);
    List<Ref<Resource>> to_clear; // clear internal resources from previous
                                  // scene from being used
    for (List<Ref<Resource>>::Element* E = cached.front(); E; E = E->next()) {
        if (E->get()->get_path().begins_with(
                p_path + "::"
            )) { // subresources of existing scene
    ...
    if (scene_idx == -1) {
        if (get_edited_scene()) {
            // scene is not open, so at it might be instanced. We'll refresh the
            // whole scene later.
...
void EditorNode::add_plugin_init_callback(
    EditorPluginInitializeCallback p_callback
) {
...
EditorPluginInitializeCallback
    EditorNode::plugin_init_callbacks[EditorNode::MAX_INIT_CALLBACKS];
...
EditorBuildCallback
    EditorNode::build_callbacks[EditorNode::MAX_BUILD_CALLBACKS];
...
void EditorNode::dim_editor(bool p_dimming, bool p_force_dim) {
    // Dimming can be forced regardless of the editor setting, which is useful
    // when quitting the editor.
    if ((p_force_dim
         || EditorSettings::get_singleton()->get(
             "interface/editor/dim_editor_on_dialog_popup"
         ))
        && p_dimming) {
        dimmed = true;
...
void EditorNode::add_resource_conversion_plugin(
...
Vector<Ref<EditorResourceConversionPlugin>> EditorNode::
    find_resource_conversion_plugin(const Ref<Resource>& p_for_resource) {
    Vector<Ref<EditorResourceConversionPlugin>> ret;

    for (int i = 0; i < resource_conversion_plugins.size(); i++) {
        if (resource_conversion_plugins[i].is_valid()
            && resource_conversion_plugins[i]->handles(p_for_resource)) {
...
void EditorNode::_video_driver_selected(int p_which) {
    ...
    String current = OS::get_singleton()->get_video_driver_name(
...
void EditorNode::_feature_profile_changed() {
    Ref<EditorFeatureProfile> profile =
        feature_profile_manager->get_current_profile();
    TabContainer* import_tabs =
        cast_to<TabContainer>(import_dock->get_parent());
    TabContainer* node_tabs = cast_to<TabContainer>(node_dock->get_parent());
    TabContainer* fs_tabs =
        cast_to<TabContainer>(filesystem_dock->get_parent());
    if (profile.is_valid()) {
        node_tabs->set_tab_hidden(
            node_dock->get_index(),
            profile->is_feature_disabled(EditorFeatureProfile::FEATURE_NODE_DOCK
            )
        );
        // The Import dock is useless without the FileSystem dock. Ensure the
        // configuration is valid.
        bool fs_dock_disabled = profile->is_feature_disabled(
            EditorFeatureProfile::FEATURE_FILESYSTEM_DOCK
        );
        fs_tabs->set_tab_hidden(filesystem_dock->get_index(), fs_dock_disabled);
        import_tabs->set_tab_hidden(
            import_dock->get_index(),
            fs_dock_disabled
                || profile->is_feature_disabled(
                    EditorFeatureProfile::FEATURE_IMPORT_DOCK
                )
        );

        main_editor_buttons[EDITOR_3D]->set_visible(
        ...
        if (StreamPeerSSL::is_available()) {
            main_editor_buttons[EDITOR_ASSETLIB]->set_visible(
                !profile->is_feature_disabled(
                    EditorFeatureProfile::FEATURE_ASSET_LIB
                )
            );
        }
        if ((profile->is_feature_disabled(EditorFeatureProfile::FEATURE_3D)
            ...
            || (profile->is_feature_disabled(
                    EditorFeatureProfile::FEATURE_SCRIPT
                )
...
void EditorNode::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &EditorNode::_menu_confirm_current
    );
    ...
    ClassDB::bind_method(
        "_update_file_menu_opened",
        &EditorNode::_update_file_menu_opened
    );
    ClassDB::bind_method(
        "_update_file_menu_closed",
    ...
    ClassDB::bind_method(
        D_METHOD("push_item", "object", "property", "inspector_only"),
    ...
    ClassDB::bind_method(
        "_get_scene_metadata",
        &EditorNode::_get_scene_metadata
    );
    ...
    ClassDB::bind_method(
        ...
        &EditorNode::_version_button_pressed
    );
    ...
    ClassDB::bind_method(
        ...
        &EditorNode::get_script_create_dialog
    );
    ...
    ClassDB::bind_method(
        "_dock_split_dragged",
        &EditorNode::_dock_split_dragged
    );
    ...
    ClassDB::bind_method(
        "_reposition_active_tab",
        &EditorNode::_reposition_active_tab
    );
    ClassDB::bind_method("_thumbnail_done", &EditorNode::_thumbnail_done);
    ClassDB::bind_method(
        "_scene_tab_script_edited",
        &EditorNode::_scene_tab_script_edited
    );
    ClassDB::bind_method(
        "_set_main_scene_state",
        &EditorNode::_set_main_scene_state
    );
    ...
    ClassDB::bind_method(
        "_update_recent_scenes",
        &EditorNode::_update_recent_scenes
    );
    ClassDB::bind_method(
        "_pick_main_scene_custom_action",
        &EditorNode::_pick_main_scene_custom_action
    );

    ClassDB::bind_method(
        "_clear_undo_history",
        &EditorNode::_clear_undo_history
    );
    ClassDB::bind_method("_dropped_files", &EditorNode::_dropped_files);
    ClassDB::bind_method(
        D_METHOD("_global_menu_action"),
        &EditorNode::_global_menu_action,
        DEFVAL(Variant())
    );
    ClassDB::bind_method(
        "_toggle_distraction_free_mode",
        &EditorNode::_toggle_distraction_free_mode
    );
    ClassDB::bind_method(
        "_version_control_menu_option",
        &EditorNode::_version_control_menu_option
    );
    ClassDB::bind_method("edit_item_resource", &EditorNode::edit_item_resource);

    ClassDB::bind_method(D_METHOD("get_gui_base"), &EditorNode::get_gui_base);
    ClassDB::bind_method(
        D_METHOD("_bottom_panel_switch"),
        &EditorNode::_bottom_panel_switch
    );

    ClassDB::bind_method(
        D_METHOD("_open_imported"),
        &EditorNode::_open_imported
    );
    ClassDB::bind_method(
        D_METHOD("_inherit_imported"),
        &EditorNode::_inherit_imported
    );

    ClassDB::bind_method("_copy_warning", &EditorNode::_copy_warning);

    ClassDB::bind_method(
        D_METHOD("_resources_reimported"),
        &EditorNode::_resources_reimported
    );
    ClassDB::bind_method(
        D_METHOD("_bottom_panel_raise_toggled"),
        &EditorNode::_bottom_panel_raise_toggled
    );

    ClassDB::bind_method(
        D_METHOD("_on_plugin_ready"),
        &EditorNode::_on_plugin_ready
    );

    ClassDB::bind_method(
        D_METHOD("_video_driver_selected"),
        &EditorNode::_video_driver_selected
    );

    ClassDB::bind_method(
        D_METHOD("_resources_changed"),
        &EditorNode::_resources_changed
    );
    ClassDB::bind_method(
        D_METHOD("_reload_modified_scenes"),
        &EditorNode::_reload_modified_scenes
    );
    ClassDB::bind_method(
        D_METHOD("_reload_project_settings"),
        &EditorNode::_reload_project_settings
    );
    ClassDB::bind_method(
        D_METHOD("_resave_scenes"),
        &EditorNode::_resave_scenes
    );
    ClassDB::bind_method(
        D_METHOD("_feature_profile_changed"),
        &EditorNode::_feature_profile_changed
    );

    ClassDB::bind_method("_screenshot", &EditorNode::_screenshot);
    ClassDB::bind_method(
        "_request_screenshot",
        &EditorNode::_request_screenshot
    );
    ...
    ADD_SIGNAL(MethodInfo(
        ...
        PropertyInfo(Variant::OBJECT, "obj"),
        PropertyInfo(Variant::STRING, "function"),
        PropertyInfo(Variant::POOL_STRING_ARRAY, "args")
    ));
    ADD_SIGNAL(
        MethodInfo("resource_saved", PropertyInfo(Variant::OBJECT, "obj"))
    );
}
...
void EditorNode::_print_handler(
    void* p_this,
    ...
    en->log->add_message(
        p_string,
        p_error ? EditorLog::MSG_TYPE_ERROR : EditorLog::MSG_TYPE_STD
    );
}
...
int EditorNode::execute_and_show_output(
    const String& p_title,
    const String& p_path,
    const List<String>& p_arguments,
    bool p_close_on_ok,
    bool p_close_on_errors
) {
...
EditorNode::EditorNode() {
    ...
    ); // no physics by default if editor
    ...
    FileAccess::set_backup_save(
        EDITOR_GET("filesystem/on_save/safe_save_on_backup_then_rename")
    );

    {
        ...
        switch (display_scale) {
            ...
            case 6:
                ...
                break;
            default:
                editor_set_scale(EditorSettings::get_singleton()->get(
    ...
    // Define a minimum window size to prevent UI elements from overlapping or
    // being cut off
    ...
    ResourceLoader::set_dependency_error_notify_func(
        this,
        _dependency_error_report
    );

    { // register importers at the beginning, so dialogs are created with the
      // right extensions
        ...
        ResourceFormatImporter::get_singleton()->add_importer(
    ...
    main_vbox->set_anchors_and_margins_preset(
        Control::PRESET_WIDE,
    ...
    for (int i = 0; i < DOCK_SLOT_MAX; i++) {
        ...
        dock_slot[i]
            ->connect("pre_popup_pressed", this, "_dock_pre_popup", varray(i));
    ...
    scene_tabs->set_tab_close_display_policy((
        bool(EDITOR_DEF("interface/scene_tabs/always_show_close_button", false))
            ? Tabs::CLOSE_BUTTON_SHOW_ALWAYS
            : Tabs::CLOSE_BUTTON_SHOW_ACTIVE_ONLY
    ));
    scene_tabs->set_min_width(
        int(EDITOR_DEF("interface/scene_tabs/minimum_width", 50)) * EDSCALE
    );
    scene_tabs->set_drag_to_rearrange_enabled(true);
    scene_tabs->connect("tab_changed", this, "_scene_tab_changed");
    scene_tabs
        ->connect("right_button_pressed", this, "_scene_tab_script_edited");
    scene_tabs->connect(
        "tab_close",
        this,
        "_scene_tab_closed",
        varray(SCENE_TAB_CLOSE)
    );
    ...
    distraction_free->set_shortcut(ED_SHORTCUT(
        ...
        KEY_MASK_CMD | KEY_MASK_CTRL | KEY_D
    ));
#else
    distraction_free->set_shortcut(ED_SHORTCUT(
        "editor/distraction_free_mode",
        TTR("Distraction Free Mode"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_F11
    ));
#endif
    ...
    scene_tab_add->set_icon(gui_base->get_icon("Add", "EditorIcons"));
    scene_tab_add->add_color_override(
        "icon_color_normal",
        Color(0.6f, 0.6f, 0.6f, 0.8f)
    );
    scene_tab_add
        ->connect("pressed", this, "_menu_option", make_binds(FILE_NEW_SCENE));
    ...
    // scene_root->set_usage(Viewport::USAGE_2D); canvas BG mode prevents usage
    // of this as 2D
    scene_root->set_disable_3d(true);

    VisualServer::get_singleton()->viewport_set_hide_scenario(
        scene_root->get_viewport_rid(),
        true
    );
    ...
    prev_scene
        ->connect("pressed", this, "_menu_option", make_binds(FILE_OPEN_PREV));
    ...
    save_accept->connect(
        ...
        this,
        "_menu_option",
        make_binds((int)MenuOptions::FILE_SAVE_AS_SCENE)
    );
    ...
    feature_profile_manager->connect(
        "current_feature_profile_changed",
        this,
    ...
    ED_SHORTCUT(
        "editor/prev_tab",
        TTR("Previous tab"),
        KEY_MASK_CMD + KEY_MASK_SHIFT + KEY_TAB
    );
    ED_SHORTCUT(
        "editor/filter_files",
        TTR("Filter Files…"),
    ...
    p->add_shortcut(
        ED_SHORTCUT("editor/new_scene", TTR("New Scene")),
        FILE_NEW_SCENE
    );
    p->add_shortcut(
        ED_SHORTCUT("editor/new_inherited_scene", TTR("New Inherited Scene…")),
    ...
    p->add_separator();
    p->add_shortcut(
        ED_SHORTCUT("editor/undo", TTR("Undo"), KEY_MASK_CMD + KEY_Z),
        EDIT_UNDO,
        true
    );
    p->add_shortcut(
        ED_SHORTCUT(
            "editor/redo",
            TTR("Redo"),
            KEY_MASK_SHIFT + KEY_MASK_CMD + KEY_Z
        ),
        EDIT_REDO,
        true
    );
    ...
    p->add_separator();
    p->add_shortcut(
        ED_SHORTCUT("editor/reload_saved_scene", TTR("Reload Saved Scene")),
    ...
    p->add_shortcut(
        ED_SHORTCUT(
            "editor/close_scene",
            TTR("Close Scene"),
            KEY_MASK_SHIFT + KEY_MASK_CMD + KEY_W
        ),
        FILE_CLOSE
    );
    ...
    p->add_separator();
    p->add_shortcut(
        ED_SHORTCUT("editor/file_quit", TTR("Quit"), KEY_MASK_CMD + KEY_Q),
        FILE_QUIT,
        true
    );
    ...
    vcs_actions_menu = VersionControlEditorPlugin::get_singleton()
                           ->get_version_control_actions_panel();
    vcs_actions_menu->set_name("Version Control");
    vcs_actions_menu
        ->connect("index_pressed", this, "_version_control_menu_option");
    p->add_separator();
    ...
    vcs_actions_menu->add_item(
        TTR("Shut Down Version Control"),
        RUN_VCS_SHUT_DOWN
    );
    ...
    p->add_check_shortcut(
        ED_SHORTCUT(
            "editor/deploy_with_remote_debug",
            TTR("Deploy with Remote Debug")
        ),
        RUN_DEPLOY_REMOTE_DEBUG
    );
    p->set_item_tooltip(
        p->get_item_count() - 1,
        TTR("When this option is enabled, using one-click deploy will make the "
    ...
    p->add_check_shortcut(
        ED_SHORTCUT(
            "editor/small_deploy_with_network_fs",
            TTR("Small Deploy with Network Filesystem")
        ),
        RUN_FILE_SERVER
    );
    p->set_item_tooltip(
        p->get_item_count() - 1,
        TTR("When this option is enabled, using one-click deploy for Android "
            "will only export an executable without the project data.\nThe "
            "filesystem will be provided from the project by the editor over "
            "the network.\nOn Android, deploying will use the USB cable for "
            "faster performance. This option speeds up testing for projects "
            "with large assets.")
    );
    p->add_separator();
    p->add_check_shortcut(
        ED_SHORTCUT(
            "editor/visible_collision_shapes",
            TTR("Visible Collision Shapes")
        ),
        RUN_DEBUG_COLLISONS
    );
    p->set_item_tooltip(
        p->get_item_count() - 1,
        TTR("When this option is enabled, collision shapes and raycast nodes "
            "(for 2D and 3D) will be visible in the running project.")
    );
    p->add_check_shortcut(
        ED_SHORTCUT("editor/visible_navigation", TTR("Visible Navigation")),
        RUN_DEBUG_NAVIGATION
    );
    p->set_item_tooltip(
        p->get_item_count() - 1,
        TTR("When this option is enabled, navigation meshes and polygons will "
            "be visible in the running project.")
    );
    p->add_separator();
    p->add_check_shortcut(
        ED_SHORTCUT(
            "editor/sync_scene_changes",
            TTR("Synchronize Scene Changes")
        ),
        RUN_LIVE_DEBUG
    );
    p->set_item_tooltip(
        p->get_item_count() - 1,
        TTR("When this option is enabled, any changes made to the scene in the "
            "editor will be replicated in the running project.\nWhen used "
            "remotely on a device, this is more efficient when the network "
            "filesystem option is enabled.")
    );
    p->add_check_shortcut(
        ED_SHORTCUT(
            "editor/sync_script_changes",
            TTR("Synchronize Script Changes")
        ),
        RUN_RELOAD_SCRIPTS
    );
    p->set_item_tooltip(
        p->get_item_count() - 1,
        TTR("When this option is enabled, any script that is saved will be "
            "reloaded in the running project.\nWhen used remotely on a device, "
            "this is more efficient when the network filesystem option is "
            "enabled.")
    );
    p->connect("id_pressed", this, "_menu_option");
    ...
    p->add_shortcut(
        ED_SHORTCUT(
            ...
            TTR("Toggle Fullscreen"),
            KEY_MASK_CMD | KEY_MASK_CTRL | KEY_F
        ),
    ...
    if (OS::get_singleton()->get_data_path()
        == OS::get_singleton()->get_config_path()) {
        // Configuration and data folders are located in the same place
        // (Windows/macOS)
        p->add_item(
            TTR("Open Editor Data/Settings Folder"),
            SETTINGS_EDITOR_DATA_FOLDER
        );
    } else {
        // Separate configuration and data folders (Linux)
        p->add_item(
            TTR("Open Editor Data Folder"),
            SETTINGS_EDITOR_DATA_FOLDER
        );
        p->add_item(
            TTR("Open Editor Settings Folder"),
    ...
    p->add_icon_shortcut(
        gui_base->get_icon("Instance", "EditorIcons"),
        ED_SHORTCUT("editor/online_docs", TTR("Online Documentation")),
        HELP_DOCS
    );
    p->add_icon_shortcut(
        gui_base->get_icon("Instance", "EditorIcons"),
        ED_SHORTCUT("editor/report_a_bug", TTR("Report a Bug")),
        HELP_REPORT_A_BUG
    );
    p->add_separator();
    p->add_icon_shortcut(
        gui_base->get_icon("Rebel", "EditorIcons"),
        ED_SHORTCUT("editor/about", TTR("About Rebel")),
        HELP_ABOUT
    );
    p->add_icon_shortcut(
        gui_base->get_icon("Heart", "EditorIcons"),
        ED_SHORTCUT(
            "editor/support_development",
            TTR("Support Rebel Development")
        ),
        HELP_SUPPORT_DEVELOPMENT
    );
    ...
    pause_button->set_shortcut(ED_SHORTCUT(
        "editor/pause_scene",
        TTR("Pause Scene"),
        KEY_MASK_CMD | KEY_MASK_CTRL | KEY_Y
    ));
#else
    ...
    run_native
        ->connect("native_run", this, "_menu_option", varray(RUN_PLAY_NATIVE));
...
#else
    play_scene_button->set_shortcut(
        ED_SHORTCUT("editor/play_scene", TTR("Play Scene"), KEY_F6)
    );
#endif
    ...
    play_custom_scene_button->connect(
        "pressed",
        this,
        "_menu_option",
    ...
    play_custom_scene_button->set_shortcut(ED_SHORTCUT(
        ...
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_R
    ));
#else
    play_custom_scene_button->set_shortcut(ED_SHORTCUT(
        "editor/play_custom_scene",
        TTR("Play Custom Scene"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_F5
    ));
#endif
    ...
    video_driver->add_font_override(
        "font",
        gui_base->get_font("bold", "EditorFonts")
    );
    ...
    video_restart_dialog->set_text(
        TTR("Changing the video driver requires restarting the editor.")
    );
    video_restart_dialog->get_ok()->set_text(TTR("Save & Restart"));
    video_restart_dialog->connect(
        "confirmed",
        this,
        "_menu_option",
        varray(SET_VIDEO_DRIVER_SAVE_AND_RESTART)
    );
    ...
    p->add_radio_check_item(
        TTR("Update Continuously"),
        SETTINGS_UPDATE_CONTINUOUSLY
    );
    p->add_radio_check_item(
        TTR("Update When Changed"),
    ...
    dock_slot[DOCK_SLOT_LEFT_UR]->set_tab_title(
        scene_tree_dock->get_index(),
        TTR("Scene")
    );
    ...
    dock_slot[DOCK_SLOT_LEFT_BR]->set_tab_title(
        filesystem_dock->get_index(),
        TTR("FileSystem")
    );

    // Inspector: Full height right
    dock_slot[DOCK_SLOT_RIGHT_UL]->add_child(inspector_dock);
    dock_slot[DOCK_SLOT_RIGHT_UL]->set_tab_title(
        inspector_dock->get_index(),
        TTR("Inspector")
    );
    ...
    // Add some offsets to left_r and main hsplits to make LEFT_R and RIGHT_L
    // docks wider than minsize
    ...
    for (int i = 0; i < vsplits.size(); i++) {
        default_layout
            ->set_value(docks_section, "dock_split_" + itos(i + 1), 0);
    }
    ...
    bottom_panel_hb->set_custom_minimum_size(Size2(0, 24 * EDSCALE)
    ); // Adjust for the height of the "Expand Bottom Dock" icon.
    ...
    bottom_panel_raise->set_shortcut(ED_SHORTCUT(
        "editor/bottom_panel_expand",
        TTR("Expand Bottom Panel"),
        KEY_MASK_SHIFT | KEY_F12
    ));
    ...
    save_confirmation->add_button(
        TTR("Don't Save"),
        OS::get_singleton()->get_swap_ok_cancel(),
        "discard"
    );
    ...
    file_menu->get_popup()
        ->connect("popup_hide", this, "_update_file_menu_closed");
    ...
    disk_changed = memnew(ConfirmationDialog);
    {
        ...
        dl->set_text(
            TTR("The following files are newer on disk.\nWhat action should be "
                "taken?")
        );
        vbc->add_child(dl);
    ...
    resource_preview->add_preview_generator(Ref<EditorPackedScenePreviewPlugin>(
        memnew(EditorPackedScenePreviewPlugin)
    ));
    resource_preview->add_preview_generator(
    ...
    resource_preview->add_preview_generator(Ref<EditorAudioStreamPreviewPlugin>(
        memnew(EditorAudioStreamPreviewPlugin)
    ));
    resource_preview->add_preview_generator(
    ...
    EditorExport::get_singleton()->add_export_plugin(
        export_text_to_binary_plugin
    );
    ...
    EditorFileSystem::get_singleton()
        ->connect("sources_changed", this, "_sources_changed");
    EditorFileSystem::get_singleton()
        ->connect("filesystem_changed", this, "_fs_changed");
    EditorFileSystem::get_singleton()
        ->connect("resources_reimported", this, "_resources_reimported");
    EditorFileSystem::get_singleton()
        ->connect("resources_reload", this, "_resources_changed");
    ...
    pick_main_scene->connect(
        "confirmed",
        this,
        "_menu_option",
        varray(SETTINGS_PICK_MAIN_SCENE)
    );
    select_current_scene_button = pick_main_scene->add_button(
        TTR("Select Current"),
        true,
        "select_current"
    );
    pick_main_scene
        ->connect("custom_action", this, "_pick_main_scene_custom_action");
    ...
    ED_SHORTCUT(
        ...
        TTR("Open Script Editor"),
        KEY_MASK_ALT | KEY_3
    );
    ED_SHORTCUT(
        "editor/editor_assetlib",
        TTR("Open Asset Library"),
        KEY_MASK_ALT | KEY_4
    );
#else
    // Use the Ctrl modifier so F2 can be used to rename nodes in the scene tree
    // dock.
    ED_SHORTCUT(
        "editor/editor_2d",
        TTR("Open 2D Editor"),
        KEY_MASK_CTRL | KEY_F1
    );
    ED_SHORTCUT(
        "editor/editor_3d",
        TTR("Open 3D Editor"),
        KEY_MASK_CTRL | KEY_F2
    );
    ED_SHORTCUT(
        "editor/editor_script",
        TTR("Open Script Editor"),
        KEY_MASK_CTRL | KEY_F3
    );
    ED_SHORTCUT(
        "editor/editor_assetlib",
        TTR("Open Asset Library"),
        KEY_MASK_CTRL | KEY_F4
    );
#endif
    ...
    screenshot_timer->set_wait_time(
        settings_menu->get_popup()->get_submenu_popup_delay() + 0.1f
    );
...
bool EditorPluginList::forward_spatial_gui_input(
    ...
    bool serve_when_force_input_enabled
) {
    bool discard = false;

    for (int i = 0; i < plugins_list.size(); i++) {
        if ((!serve_when_force_input_enabled)
            && plugins_list[i]->is_input_event_forwarding_always_enabled()) {
            continue;
        }
...
void EditorPluginList::forward_canvas_force_draw_over_viewport(
    Control* p_overlay
) {
    for (int i = 0; i < plugins_list.size(); i++) {
...
void EditorPluginList::forward_spatial_force_draw_over_viewport(
    Control* p_overlay
) {
    for (int i = 0; i < plugins_list.size(); i++) {
