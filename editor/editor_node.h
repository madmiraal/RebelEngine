...
private:
    ...
    void _add_dropped_files_recursive(
    ...
    static void _dependency_error_report(
        void* ud,
        const String& p_path,
        const String& p_dep,
        const String& p_type
    ...
    void _save_open_scenes_to_config(
        Ref<ConfigFile> p_layout,
        const String& p_section
    );
    void _load_open_scenes_from_config(
        Ref<ConfigFile> p_layout,
        const String& p_section
    );
...
public:
    ...
    static void add_plugin_init_callback(
    ...
    static void add_editor_plugin(
        EditorPlugin* p_editor,
        bool p_config_changed = false
    );
    static void remove_editor_plugin(
        EditorPlugin* p_editor,
        bool p_config_changed = false
    );
    ...
    void save_resource_in_path(
        const Ref<Resource>& p_resource,
        const String& p_path
    );
    void save_resource(const Ref<Resource>& p_resource);
    void save_resource_as(
        const Ref<Resource>& p_resource,
        const String& p_at_path = String()
    );
    ...
    OS::ProcessID get_child_process_id() const {
        return editor_run.get_pid();
    }
    ...
    Ref<Texture> get_object_icon(
        const Object* p_object,
        const String& p_fallback = "Object"
    ) const;
    Ref<Texture> get_class_icon(
    ...
    static void progress_add_task_bg(
        const String& p_task,
        const String& p_label,
        int p_steps
    );
    ...
    void add_tool_menu_item(
        const String& p_name,
        Object* p_handler,
        const String& p_callback,
        const Variant& p_ud = Variant()
    );
    ...
    void remove_resource_conversion_plugin(
        const Ref<EditorResourceConversionPlugin>& p_plugin
    );
    Vector<Ref<EditorResourceConversionPlugin>> find_resource_conversion_plugin(
