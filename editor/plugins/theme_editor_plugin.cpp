...
void ThemeItemImportTree::_update_items_tree() {
    ...
    for (List<StringName>::Element* E = types.front(); E; E = E->next()) {
        ...
        bool is_matching_filter =
            (filter_text.empty() || type_name.findn(filter_text) > -1);
        ...
        for (int i = 0; i < Theme::DATA_TYPE_MAX; i++) {
            ...
            for (List<StringName>::Element* F = names.front(); F;
                 F                            = F->next()) {
                String item_name = (String)F->get();
                bool is_item_matching_filter =
                    (item_name.findn(filter_text) > -1);
                if (!filter_text.empty() && !is_matching_filter
                    && !is_item_matching_filter) {
                    continue;
                }

                // Only mark this if actual items match the filter and not just
                // the type group.
            ...
            TreeItem* data_type_node =
                import_items_tree->create_item(type_node);
            ...
            switch (dt) {
                case Theme::DATA_TYPE_COLOR:
                    data_type_node->set_icon(
                        0,
                        get_icon("Color", "EditorIcons")
                    );
                ...
                case Theme::DATA_TYPE_CONSTANT:
                    data_type_node->set_icon(
                        0,
                        get_icon("MemberConstant", "EditorIcons")
                    );
                ...
                case Theme::DATA_TYPE_STYLEBOX:
                    data_type_node->set_icon(
                        0,
                        get_icon("StyleBoxFlat", "EditorIcons")
                    );
            ...
            for (List<StringName>::Element* F = filtered_names.front(); F;
                 F                            = F->next()) {
                TreeItem* item_node =
                    import_items_tree->create_item(data_type_node);
                item_node->set_meta("_can_be_imported", true);
                item_node->set_text(0, F->get());
                item_node->set_cell_mode(
                    IMPORT_ITEM,
                    TreeItem::CELL_MODE_CHECK
                );
        ...
        type_node->set_checked(
            IMPORT_ITEM_DATA,
            any_checked && any_checked_with_data
        );
    }
...
void ThemeItemImportTree::_update_total_selected(Theme::DataType p_data_type) {
    ERR_FAIL_INDEX_MSG(
        p_data_type,
        Theme::DATA_TYPE_MAX,
        "Theme item data type is out of bounds."
    );
    ...
    int count = 0;
    for (Map<ThemeItem, ItemCheckedState>::Element* E = selected_items.front();
         E;
         E = E->next()) {
        ThemeItem ti = E->key();
    ...
    if (count == 0) {
        ...
        Array arr;
        arr.push_back(count);
        total_selected_items_label->set_text(
            TTR("{num} currently selected").format(arr, "{num}")
        );
...
void ThemeItemImportTree::_select_all_subitems(
    TreeItem* p_root_item,
    bool p_select_with_data
) {
...
void ThemeItemImportTree::_deselect_all_subitems(
    TreeItem* p_root_item,
    bool p_deselect_completely
) {
    TreeItem* child_item = p_root_item->get_children();
    while (child_item) {
...
void ThemeItemImportTree::_import_selected() {
    if (selected_items.size() == 0) {
        EditorNode::get_singleton()->show_accept(
            TTR("Nothing was selected for the import."),
            TTR("OK")
        );
        return;
    }

    // Prevent changes from immediately being reported while the operation is
    // still ongoing.
    edited_theme->_freeze_change_propagation();
    ProgressDialog::get_singleton()->add_task(
        "import_theme_items",
        TTR("Importing Theme Items"),
        selected_items.size() + 2
    );
    ...
    for (Map<ThemeItem, ItemCheckedState>::Element* E = selected_items.front();
        ...
        // Reduces the number of UI updates that this causes when copying large
        // themes.
        if (idx % 10 == 0) {
            Array arr;
            arr.push_back(idx + 1);
            arr.push_back(selected_items.size());
            ProgressDialog::get_singleton()->task_step(
                "import_theme_items",
                TTR("Importing items {n}/{n}").format(arr, "{n}"),
                idx
            );
        }
        ...
        if (cs == SELECT_IMPORT_DEFINITION || cs == SELECT_IMPORT_FULL) {
            ...
            if (cs == SELECT_IMPORT_FULL) {
                item_value = base_theme->get_theme_item(
                    ti.data_type,
                    ti.item_name,
                    ti.type_name
                );
    ...
    ProgressDialog::get_singleton()
        ->task_step("import_theme_items", TTR("Updating the editor"), idx++);
    edited_theme->_unfreeze_and_propagate_changes();
    // Make sure the task is not ended before the editor freezes to update the
    // Inspector.
    ProgressDialog::get_singleton()
        ->task_step("import_theme_items", TTR("Finalizing"), idx++);
...
void ThemeItemImportTree::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_THEME_CHANGED: {
            select_icons_warning_icon->set_texture(
                get_icon("StatusWarning", "EditorIcons")
            );
            select_icons_warning->add_color_override(
                "font_color",
                get_color("disabled_font_color", "Editor")
            );

            // Bottom panel buttons.
            import_collapse_types_button->set_icon(
                get_icon("CollapseTree", "EditorIcons")
            );
            import_expand_types_button->set_icon(
                get_icon("ExpandTree", "EditorIcons")
            );

            import_select_all_button->set_icon(
                get_icon("ThemeSelectAll", "EditorIcons")
            );
            import_select_full_button->set_icon(
                get_icon("ThemeSelectFull", "EditorIcons")
            );
            import_deselect_all_button->set_icon(
                get_icon("ThemeDeselectAll", "EditorIcons")
            );

            // Side panel buttons.
            select_colors_icon->set_texture(get_icon("Color", "EditorIcons"));
            deselect_all_colors_button->set_icon(
                get_icon("ThemeDeselectAll", "EditorIcons")
            );
            select_all_colors_button->set_icon(
                get_icon("ThemeSelectAll", "EditorIcons")
            );
            select_full_colors_button->set_icon(
                get_icon("ThemeSelectFull", "EditorIcons")
            );

            select_constants_icon->set_texture(
                get_icon("MemberConstant", "EditorIcons")
            );
            deselect_all_constants_button->set_icon(
                get_icon("ThemeDeselectAll", "EditorIcons")
            );
            select_all_constants_button->set_icon(
                get_icon("ThemeSelectAll", "EditorIcons")
            );
            select_full_constants_button->set_icon(
                get_icon("ThemeSelectFull", "EditorIcons")
            );

            select_fonts_icon->set_texture(get_icon("Font", "EditorIcons"));
            deselect_all_fonts_button->set_icon(
                get_icon("ThemeDeselectAll", "EditorIcons")
            );
            select_all_fonts_button->set_icon(
                get_icon("ThemeSelectAll", "EditorIcons")
            );
            select_full_fonts_button->set_icon(
                get_icon("ThemeSelectFull", "EditorIcons")
            );

            select_icons_icon->set_texture(
                get_icon("ImageTexture", "EditorIcons")
            );
            deselect_all_icons_button->set_icon(
                get_icon("ThemeDeselectAll", "EditorIcons")
            );
            select_all_icons_button->set_icon(
                get_icon("ThemeSelectAll", "EditorIcons")
            );
            select_full_icons_button->set_icon(
                get_icon("ThemeSelectFull", "EditorIcons")
            );

            select_styleboxes_icon->set_texture(
                get_icon("StyleBoxFlat", "EditorIcons")
            );
            deselect_all_styleboxes_button->set_icon(
                get_icon("ThemeDeselectAll", "EditorIcons")
            );
            select_all_styleboxes_button->set_icon(
                get_icon("ThemeSelectAll", "EditorIcons")
            );
            select_full_styleboxes_button->set_icon(
                get_icon("ThemeSelectFull", "EditorIcons")
            );
        } break;
    }
...
void ThemeItemImportTree::_bind_methods() {
    // Internal binds.
    ClassDB::bind_method(
        "_filter_text_changed",
        &ThemeItemImportTree::_filter_text_changed
    );
    ClassDB::bind_method(
        "_tree_item_edited",
        &ThemeItemImportTree::_tree_item_edited
    );
    ClassDB::bind_method(
        "_select_all_data_type_pressed",
        &ThemeItemImportTree::_select_all_data_type_pressed
    );
    ClassDB::bind_method(
        "_select_full_data_type_pressed",
        &ThemeItemImportTree::_select_full_data_type_pressed
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        "_toggle_type_items",
        &ThemeItemImportTree::_toggle_type_items
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        "_select_full_items_pressed",
        &ThemeItemImportTree::_select_full_items_pressed
    );
    ClassDB::bind_method(
        "_deselect_all_items_pressed",
        &ThemeItemImportTree::_deselect_all_items_pressed
    );
    ClassDB::bind_method(
        "_import_selected",
        &ThemeItemImportTree::_import_selected
    );
...
ThemeItemImportTree::ThemeItemImportTree() {
    ...
    for (int i = 0; i < Theme::DATA_TYPE_MAX; i++) {
        ...
        switch (dt) {
            case Theme::DATA_TYPE_COLOR:
                ...
                select_all_items_tooltip =
                    TTR("Select all visible color items.");
                select_full_items_tooltip =
                    TTR("Select all visible color items and their data.");
                deselect_all_items_tooltip =
                    TTR("Deselect all visible color items.");
                break;
            ...
            case Theme::DATA_TYPE_CONSTANT:
                ...
                select_all_items_tooltip =
                    TTR("Select all visible constant items.");
                select_full_items_tooltip =
                    TTR("Select all visible constant items and their data.");
                deselect_all_items_tooltip =
                    TTR("Deselect all visible constant items.");
                break;
            ...
            case Theme::DATA_TYPE_FONT:
                ...
                select_all_items_tooltip =
                    TTR("Select all visible font items.");
                select_full_items_tooltip =
                    TTR("Select all visible font items and their data.");
                deselect_all_items_tooltip =
                    TTR("Deselect all visible font items.");
                break;
            ...
            case Theme::DATA_TYPE_ICON:
                ...
                select_all_items_tooltip =
                    TTR("Select all visible icon items.");
                select_full_items_tooltip =
                    TTR("Select all visible icon items and their data.");
                deselect_all_items_tooltip =
                    TTR("Deselect all visible icon items.");
                break;
            ...
            case Theme::DATA_TYPE_STYLEBOX:
                ...
                select_all_items_tooltip =
                    TTR("Select all visible stylebox items.");
                select_full_items_tooltip =
                    TTR("Select all visible stylebox items and their data.");
                deselect_all_items_tooltip =
                    TTR("Deselect all visible stylebox items.");
                break;
        ...
        select_all_items_button->connect(
            "pressed",
            this,
            "_select_all_data_type_pressed",
            varray(i)
        ...
        select_full_items_button->connect(
            "pressed",
            this,
            "_select_full_data_type_pressed",
            varray(i)
        ...
        deselect_all_items_button->connect(
            "pressed",
            this,
            "_deselect_all_data_type_pressed",
            varray(i)
        ...
        if (dt == Theme::DATA_TYPE_ICON) {
            ...
            select_icons_warning_icon->set_v_size_flags(
                Control::SIZE_SHRINK_CENTER
            );
            select_icons_warning_hb->add_child(select_icons_warning_icon);

            select_icons_warning = memnew(Label);
            select_icons_warning->set_text(
                TTR("Caution: Adding icon data may considerably increase the "
                    "size of your Theme resource.")
            );
    ...
    import_collapse_types_button
        ->connect("pressed", this, "_toggle_type_items", varray(true));
    ...
    import_expand_types_button
        ->connect("pressed", this, "_toggle_type_items", varray(false));
    ...
    import_select_all_button
        ->connect("pressed", this, "_select_all_items_pressed");
    ...
    import_select_full_button->set_tooltip(
        TTR("Select all Theme items with item data.")
    );
    import_buttons->add_child(import_select_full_button);
    import_select_full_button
        ->connect("pressed", this, "_select_full_items_pressed");
    ...
    import_deselect_all_button
        ->connect("pressed", this, "_deselect_all_items_pressed");
...
void ThemeItemEditorDialog::ok_pressed() {
    if (import_default_theme_items->has_selected_items()
        ...
        confirm_closing_dialog->set_text(
            TTR("Import Items tab has some items selected. Selection will be "
                "lost upon closing this window.\nClose anyway?")
        );
...
void ThemeItemEditorDialog::_dialog_about_to_show() {
    ...
    import_editor_theme_items->set_base_theme(
...
void ThemeItemEditorDialog::_update_edit_types() {
    ...
    for (List<StringName>::Element* E = theme_types.front(); E; E = E->next()) {
        ...
        if (E->get() == "") {
            ...
            item_icon = EditorNode::get_singleton()->get_class_icon(
                E->get(),
                "NodeDisabled"
            );
        }
    ...
    if (selected_ids.size() > 0) {
        ...
        edit_items_message->set_text(TTR(
            "Select a theme type from the list to edit its items.\nYou can add "
            "a custom type or import a type with its items from another theme."
        ));
...
void ThemeItemEditorDialog::_update_edit_item_tree(String p_item_type) {
    ...
    { // Constants.
        ...
        if (names.size() > 0) {
            ...
            constant_root->set_icon(
                0,
                get_icon("MemberConstant", "EditorIcons")
            );
            constant_root->set_text(0, TTR("Constants"));
            constant_root->add_button(
                0,
                get_icon("Clear", "EditorIcons"),
                ITEMS_TREE_REMOVE_DATA_TYPE,
                false,
                TTR("Remove All Constant Items")
            );
    ...
    { // Fonts.
        ...
        if (names.size() > 0) {
            ...
            font_root->add_button(
                0,
                get_icon("Clear", "EditorIcons"),
                ITEMS_TREE_REMOVE_DATA_TYPE,
                false,
                TTR("Remove All Font Items")
            );
    ...
    { // Icons.
        ...
        if (names.size() > 0) {
            ...
            icon_root->add_button(
                0,
                get_icon("Clear", "EditorIcons"),
                ITEMS_TREE_REMOVE_DATA_TYPE,
                false,
                TTR("Remove All Icon Items")
            );
    ...
    { // Styleboxes.
        ...
        if (names.size() > 0) {
            ...
            stylebox_root->add_button(
                0,
                get_icon("Clear", "EditorIcons"),
                ITEMS_TREE_REMOVE_DATA_TYPE,
                false,
                TTR("Remove All StyleBox Items")
            );
    ...
    // If some type is selected, but it doesn't seem to have any items, show a
    // guiding message.
    Vector<int> selected_ids = edit_type_list->get_selected_items();
    if (selected_ids.size() > 0) {
        if (!has_any_items) {
            edit_items_message->set_text(
                TTR("This theme type is empty.\nAdd more items to it manually "
                    "or by importing from another theme.")
            );
            edit_items_message->show();
        } else {
            edit_items_message->set_text("");
...
void ThemeItemEditorDialog::_item_tree_button_pressed(
    Object* p_item,
    int p_column,
    int p_id
) {
    ...
    switch (p_id) {
        case ITEMS_TREE_RENAME_ITEM: {
            ...
            _open_rename_theme_item_dialog(
...
void ThemeItemEditorDialog::_add_theme_item(
    Theme::DataType p_data_type,
    String p_item_name,
    String p_item_type
) {
    switch (p_data_type) {
        ...
        case Theme::DATA_TYPE_STYLEBOX:
            edited_theme
                ->set_stylebox(p_item_name, p_item_type, Ref<StyleBox>());
            break;
        case Theme::DATA_TYPE_FONT:
...
void ThemeItemEditorDialog::_remove_data_type_items(
    Theme::DataType p_data_type,
    String p_item_type
) {
    List<StringName> names;
...
void ThemeItemEditorDialog::_remove_class_items() {
    ...
    for (int dt = 0; dt < Theme::DATA_TYPE_MAX; dt++) {
        ...
        names.clear();
        Theme::get_default()
            ->get_theme_item_list(data_type, edited_item_type, &names);
        for (List<StringName>::Element* E = names.front(); E; E = E->next()) {
            if (edited_theme->has_theme_item_nocheck(
                    data_type,
                    E->get(),
                    edited_item_type
                )) {
                edited_theme
                    ->clear_theme_item(data_type, E->get(), edited_item_type);
            }
        }
    }
...
void ThemeItemEditorDialog::_remove_custom_items() {
    ...
    for (int dt = 0; dt < Theme::DATA_TYPE_MAX; dt++) {
        ...
        for (List<StringName>::Element* E = names.front(); E; E = E->next()) {
            if (!Theme::get_default()->has_theme_item_nocheck(
                    data_type,
                    E->get(),
                    edited_item_type
                )) {
...
void ThemeItemEditorDialog::_confirm_edit_theme_item() {
    if (item_popup_mode == CREATE_THEME_ITEM) {
        _add_theme_item(
            edit_item_data_type,
            theme_item_name->get_text(),
            edited_item_type
        );
    } else if (item_popup_mode == RENAME_THEME_ITEM) {
        edited_theme->rename_theme_item(
            edit_item_data_type,
            edit_item_old_name,
            theme_item_name->get_text(),
            edited_item_type
...
void ThemeItemEditorDialog::_edit_theme_item_gui_input(
...
void ThemeItemEditorDialog::_select_another_theme_cbk(const String& p_path) {
    ...
    if (loaded_theme.is_null()) {
        EditorNode::get_singleton()->show_warning(
            TTR("Invalid file, not a Theme resource.")
        );
        return;
    }
...
void ThemeItemEditorDialog::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_THEME_CHANGED: {
            ...
            edit_items_add_constant->set_icon(
                get_icon("MemberConstant", "EditorIcons")
            );
            edit_items_add_font->set_icon(get_icon("Font", "EditorIcons"));
            edit_items_add_icon->set_icon(
                get_icon("ImageTexture", "EditorIcons")
            );
            edit_items_add_stylebox->set_icon(
                get_icon("StyleBoxFlat", "EditorIcons")
            );

            edit_items_remove_class->set_icon(get_icon("Control", "EditorIcons")
            );
            edit_items_remove_custom->set_icon(
                get_icon("ThemeRemoveCustomItems", "EditorIcons")
            );
            edit_items_remove_all->set_icon(
                get_icon("ThemeRemoveAllItems", "EditorIcons")
            );

            import_another_theme_button->set_icon(
                get_icon("Folder", "EditorIcons")
            );

            tc->add_style_override(
                "tab_selected",
                get_stylebox("tab_selected_odd", "TabContainer")
            );
            tc->add_style_override(
                "panel",
                get_stylebox("panel_odd", "TabContainer")
            );
        } break;
    }
}
...
void ThemeItemEditorDialog::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &ThemeItemEditorDialog::_edited_type_selected
    );
    ClassDB::bind_method(
        "_add_theme_type",
        &ThemeItemEditorDialog::_add_theme_type
    );
    ClassDB::bind_method(
        "_open_add_theme_item_dialog",
        &ThemeItemEditorDialog::_open_add_theme_item_dialog
    );
    ClassDB::bind_method(
        "_remove_class_items",
        &ThemeItemEditorDialog::_remove_class_items
    );
    ClassDB::bind_method(
        "_remove_custom_items",
        &ThemeItemEditorDialog::_remove_custom_items
    );
    ClassDB::bind_method(
        "_remove_all_items",
        &ThemeItemEditorDialog::_remove_all_items
    );
    ClassDB::bind_method(
        "_item_tree_button_pressed",
        &ThemeItemEditorDialog::_item_tree_button_pressed
    );
    ClassDB::bind_method(
        "_edit_theme_item_gui_input",
        &ThemeItemEditorDialog::_edit_theme_item_gui_input
    );
    ClassDB::bind_method(
        "_confirm_edit_theme_item",
        &ThemeItemEditorDialog::_confirm_edit_theme_item
    );
    ClassDB::bind_method(
        "_update_edit_types",
        &ThemeItemEditorDialog::_update_edit_types
    );
    ClassDB::bind_method(
        "_open_select_another_theme",
        &ThemeItemEditorDialog::_open_select_another_theme
    );
    ClassDB::bind_method(
        "_select_another_theme_cbk",
        &ThemeItemEditorDialog::_select_another_theme_cbk
    );
    ClassDB::bind_method(
        "_close_dialog",
        &ThemeItemEditorDialog::_close_dialog
    );
    ClassDB::bind_method(
        "_dialog_about_to_show",
        &ThemeItemEditorDialog::_dialog_about_to_show
    );
}
...
ThemeItemEditorDialog::ThemeItemEditorDialog() {
    ...
    edit_add_type_button
        ->connect("pressed", this, "_add_theme_type", varray(""));
    ...
    edit_items_add_color->connect(
        "pressed",
        this,
        "_open_add_theme_item_dialog",
        varray(Theme::DATA_TYPE_COLOR)
    );
    ...
    edit_items_add_constant->connect(
        "pressed",
        this,
        "_open_add_theme_item_dialog",
        varray(Theme::DATA_TYPE_CONSTANT)
    );
    ...
    edit_items_add_font->connect(
        "pressed",
        this,
        "_open_add_theme_item_dialog",
        varray(Theme::DATA_TYPE_FONT)
    );
    ...
    edit_items_add_icon->connect(
        "pressed",
        this,
        "_open_add_theme_item_dialog",
        varray(Theme::DATA_TYPE_ICON)
    );
    ...
    edit_items_add_stylebox->connect(
        "pressed",
        this,
        "_open_add_theme_item_dialog",
        varray(Theme::DATA_TYPE_STYLEBOX)
    );
    ...
    edit_items_tree
        ->connect("button_pressed", this, "_item_tree_button_pressed");
    ...
    edit_theme_item_dialog
        ->connect("confirmed", this, "_confirm_edit_theme_item");
    ...
    import_default_theme_items
        ->connect("items_imported", this, "_update_edit_types");
    ...
    import_another_theme_button
        ->connect("pressed", this, "_open_select_another_theme");
    ...
    for (List<String>::Element* E = ext.front(); E; E = E->next()) {
        import_another_theme_dialog->add_filter(
            "*." + E->get() + "; Theme Resource"
        );
    }
    import_another_file_hb->add_child(import_another_theme_dialog);
    import_another_theme_dialog
        ->connect("file_selected", this, "_select_another_theme_cbk");
...
void ThemeTypeEditor::_update_type_list() {
    ...
    if (focused) {
        if (focusables.find(focused, 0) != -1) {
            // If focus is currently on one of the internal property editors,
            // don't update.
            updating = false;
            return;
        }
        ...
        while (focus_parent) {
            ...
            if (c && focusables.find(c, 0) != -1) {
                // If focus is currently on one of the internal property
                // editors, don't update.
                updating = false;
                return;
            }
    ...
    if (theme_types.size() > 0) {
        ...
        bool item_reselected = false;
        int e_idx            = 0;
        for (List<StringName>::Element* E = theme_types.front(); E;
             E                            = E->next()) {
            Ref<Texture> item_icon;
            if (E->get() == "") {
                item_icon = get_icon("NodeDisabled", "EditorIcons");
...
OrderedHashMap<StringName, bool> ThemeTypeEditor::_get_type_items(
    String p_type_name,
    void (Theme::*get_list_func)(StringName, List<StringName>*) const,
    ...
    if (include_default) {
        names.clear();
        (Theme::get_default().operator->()->*get_list_func)(
            p_type_name,
            &names
        );
        names.sort_custom<StringName::AlphCompare>();
    ...
    for (OrderedHashMap<StringName, bool>::Element E = items.front(); E;
         E                                           = E.next()) {
...
HBoxContainer* ThemeTypeEditor::_create_property_control(
    Theme::DataType p_data_type,
    String p_item_name,
    bool p_editable
) {
    ...
    if (p_editable) {
        ...
        item_remove_button->connect(
            "pressed",
            this,
            "_item_remove_cbk",
            varray(p_data_type, p_item_name)
        );

        Button* item_rename_confirm_button = memnew(Button);
        item_rename_confirm_button->set_icon(
            get_icon("ImportCheck", "EditorIcons")
        );
        ...
        item_rename_confirm_button->connect(
            "pressed",
            this,
            "_item_rename_confirmed",
            varray(p_data_type, p_item_name, item_name_container)
        );
        item_rename_confirm_button->hide();

        Button* item_rename_cancel_button = memnew(Button);
        item_rename_cancel_button->set_icon(
            get_icon("ImportFail", "EditorIcons")
        );
        ...
        item_rename_cancel_button->connect(
            "pressed",
            this,
            "_item_rename_canceled",
            varray(p_data_type, p_item_name, item_name_container)
        );
        item_rename_cancel_button->hide();
    } else {
        item_name->add_color_override(
            "font_color",
            get_color("disabled_font_color", "Editor")
        );
...
void ThemeTypeEditor::_update_type_items() {
    ...
    // Colors.
    {
        ...
        OrderedHashMap<StringName, bool> color_items =
            _get_type_items(edited_type, &Theme::get_color_list, show_default);
        for (OrderedHashMap<StringName, bool>::Element E = color_items.front();
             E;
             E = E.next()) {
            HBoxContainer* item_control = _create_property_control(
            ...
            if (E.get()) {
                item_editor->set_pick_color(
                    edited_theme->get_color(E.key(), edited_type)
                );
                item_editor->connect(
                    "color_changed",
                    this,
                ...
                item_editor->set_pick_color(
                    Theme::get_default()->get_color(E.key(), edited_type)
                );
    ...
    // Constants.
    {
        ...
        OrderedHashMap<StringName, bool> constant_items = _get_type_items(
            edited_type,
            &Theme::get_constant_list,
            show_default
        );
        for (OrderedHashMap<StringName, bool>::Element E =
                 constant_items.front();
             E;
             E = E.next()) {
            HBoxContainer* item_control = _create_property_control(
            ...
            if (E.get()) {
                item_editor->set_value(
                    edited_theme->get_constant(E.key(), edited_type)
                );
                item_editor->connect(
                    "value_changed",
                    this,
                    "_constant_item_changed",
                    varray(E.key())
                );
            } else {
                item_editor->set_value(
                    Theme::get_default()->get_constant(E.key(), edited_type)
                );
    ...
    // Fonts.
    {
        ...
        OrderedHashMap<StringName, bool> font_items =
            _get_type_items(edited_type, &Theme::get_font_list, show_default);
        for (OrderedHashMap<StringName, bool>::Element E = font_items.front();
             E;
             E = E.next()) {
            HBoxContainer* item_control = _create_property_control(
            ...
            if (E.get()) {
                if (edited_theme->has_font(E.key(), edited_type)) {
                    item_editor->set_edited_resource(
                        edited_theme->get_font(E.key(), edited_type)
                    );
                } else {
                    item_editor->set_edited_resource(RES());
                }
                item_editor
                    ->connect("resource_selected", this, "_edit_resource_item");
                item_editor->connect(
                    "resource_changed",
                    this,
                    "_font_item_changed",
                ...
                if (Theme::get_default()->has_font(E.key(), edited_type)) {
                    item_editor->set_edited_resource(
                        Theme::get_default()->get_font(E.key(), edited_type)
                    );
                } else {
                    item_editor->set_edited_resource(RES());
                }
    ...
    // Icons.
    {
        ...
        OrderedHashMap<StringName, bool> icon_items =
            _get_type_items(edited_type, &Theme::get_icon_list, show_default);
        for (OrderedHashMap<StringName, bool>::Element E = icon_items.front();
             E;
             E = E.next()) {
            HBoxContainer* item_control = _create_property_control(
            ...
            if (E.get()) {
                if (edited_theme->has_icon(E.key(), edited_type)) {
                    item_editor->set_edited_resource(
                        edited_theme->get_icon(E.key(), edited_type)
                    );
                } else {
                    item_editor->set_edited_resource(RES());
                }
                ...
                if (Theme::get_default()->has_icon(E.key(), edited_type)) {
                    item_editor->set_edited_resource(
                        Theme::get_default()->get_icon(E.key(), edited_type)
                    );
                } else {
                    item_editor->set_edited_resource(RES());
                }
    ...
    // Styleboxes.
    {
        ...
        if (leading_stylebox.pinned) {
            ...
            pin_leader_button->set_tooltip(
                TTR("Unpin this StyleBox as a main style.")
            );
            item_control->add_child(pin_leader_button);
            pin_leader_button
                ->connect("pressed", this, "_unpin_leading_stylebox");
            ...
            item_editor->connect(
                "resource_changed",
                this,
                "_stylebox_item_changed",
                varray(leading_stylebox.item_name)
            );
        ...
        OrderedHashMap<StringName, bool> stylebox_items = _get_type_items(
            edited_type,
            &Theme::get_stylebox_list,
            show_default
        );
        for (OrderedHashMap<StringName, bool>::Element E =
                 stylebox_items.front();
             E;
             E = E.next()) {
            if (leading_stylebox.pinned
                && leading_stylebox.item_name == E.key()) {
                continue;
            }
            ...
            if (E.get()) {
                ...
                if (edited_theme->has_stylebox(E.key(), edited_type)) {
                    stylebox_value =
                        edited_theme->get_stylebox(E.key(), edited_type);
                ...
                pin_leader_button->set_icon(get_icon("Pin", "EditorIcons"));
                pin_leader_button->set_tooltip(
                    TTR("Pin this StyleBox as a main style. Editing its "
                        "properties will update the same properties in all "
                        "other StyleBoxes of this type.")
                );
                item_control->add_child(pin_leader_button);
                pin_leader_button->connect(
                    "pressed",
                    this,
                    "_pin_leading_stylebox",
                    varray(item_editor, E.key())
                );
            } else {
                if (Theme::get_default()->has_stylebox(E.key(), edited_type)) {
                    item_editor->set_edited_resource(
                        Theme::get_default()->get_stylebox(E.key(), edited_type)
                    );
                } else {
                    item_editor->set_edited_resource(RES());
                }
    ...
    }
    {
        ...
        for (List<StringName>::Element* E = names.front(); E; E = E->next()) {
            if (!edited_theme->has_stylebox(E->get(), edited_type)) {
                edited_theme
                    ->set_stylebox(E->get(), edited_type, Ref<StyleBox>());
            }
        }
    }
    {
...
void ThemeTypeEditor::_item_add_lineedit_cbk(
    String p_value,
    int p_data_type,
    Control* p_control
) {
    ...
    switch (p_data_type) {
        case Theme::DATA_TYPE_COLOR: {
            edited_theme->set_color(
                p_item_name,
                edited_type,
                Theme::get_default()->get_color(p_item_name, edited_type)
            );
        } break;
        case Theme::DATA_TYPE_CONSTANT: {
            edited_theme->set_constant(
                p_item_name,
                edited_type,
                Theme::get_default()->get_constant(p_item_name, edited_type)
            );
        } break;
        case Theme::DATA_TYPE_FONT: {
        ...
        case Theme::DATA_TYPE_STYLEBOX: {
            edited_theme
                ->set_stylebox(p_item_name, edited_type, Ref<StyleBox>());
        } break;
    }
}
...
void ThemeTypeEditor::_item_remove_cbk(int p_data_type, String p_item_name) {
    switch (p_data_type) {
        ...
        case Theme::DATA_TYPE_STYLEBOX: {
            ...
            if (leading_stylebox.pinned
                && leading_stylebox.item_name == p_item_name) {
...
void ThemeTypeEditor::_item_rename_cbk(
    int p_data_type,
    String p_item_name,
    Control* p_control
) {
...
void ThemeTypeEditor::_item_rename_confirmed(
    int p_data_type,
    String p_item_name,
    Control* p_control
) {
...
void ThemeTypeEditor::_item_rename_canceled(
    int p_data_type,
    String p_item_name,
    Control* p_control
) {
...
void ThemeTypeEditor::_constant_item_changed(
    float p_value,
    String p_item_name
) {
...
void ThemeTypeEditor::_font_item_changed(
    Ref<Font> p_value,
    String p_item_name
) {
...
void ThemeTypeEditor::_icon_item_changed(
    Ref<Texture> p_value,
    String p_item_name
) {
...
void ThemeTypeEditor::_stylebox_item_changed(
    Ref<StyleBox> p_value,
    String p_item_name
) {
    ...
    if (leading_stylebox.pinned && leading_stylebox.item_name == p_item_name) {
        if (leading_stylebox.stylebox.is_valid()) {
            leading_stylebox.stylebox
                ->disconnect("changed", this, "_update_stylebox_from_leading");
        }

        leading_stylebox.stylebox = p_value;
        leading_stylebox.ref_stylebox =
            (p_value.is_valid() ? p_value->duplicate() : RES());
        if (p_value.is_valid()) {
            leading_stylebox.stylebox
                ->connect("changed", this, "_update_stylebox_from_leading");
        }
    }
}
...
void ThemeTypeEditor::_pin_leading_stylebox(
    ...
    if (Object::cast_to<EditorResourcePicker>(p_editor)) {
        stylebox = Object::cast_to<EditorResourcePicker>(p_editor)
                       ->get_edited_resource();
    }
...
void ThemeTypeEditor::_update_stylebox_from_leading() {
    ...
    for (List<PropertyInfo>::Element* E = props.front(); E; E = E->next()) {
        ...
        for (List<Ref<StyleBox>>::Element* F = styleboxes.front(); F;
             F                               = F->next()) {
...
void ThemeTypeEditor::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_THEME_CHANGED: {
            ...
            data_type_tabs->add_style_override(
                "tab_selected",
                get_stylebox("tab_selected_odd", "TabContainer")
            );
            data_type_tabs->add_style_override(
...
void ThemeTypeEditor::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_update_type_list",
        &ThemeTypeEditor::_update_type_list
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        "_update_type_items",
        &ThemeTypeEditor::_update_type_items
    );
    ClassDB::bind_method(
        "_list_type_selected",
        &ThemeTypeEditor::_list_type_selected
    );

    ClassDB::bind_method(
        "_add_type_button_cbk",
        &ThemeTypeEditor::_add_type_button_cbk
    );
    ClassDB::bind_method(
        "_add_type_dialog_confirmed",
        &ThemeTypeEditor::_add_type_dialog_confirmed
    );
    ClassDB::bind_method(
        "_add_type_filter_cbk",
        &ThemeTypeEditor::_add_type_filter_cbk
    );
    ClassDB::bind_method(
        "_add_type_dialog_entered",
        &ThemeTypeEditor::_add_type_dialog_entered
    );
    ClassDB::bind_method(
        "_add_type_options_cbk",
        &ThemeTypeEditor::_add_type_options_cbk
    );
    ClassDB::bind_method(
        "_add_type_dialog_activated",
        &ThemeTypeEditor::_add_type_dialog_activated
    );
    ClassDB::bind_method(
        "_add_default_type_items",
        &ThemeTypeEditor::_add_default_type_items
    );

    ClassDB::bind_method(
        "_item_add_lineedit_cbk",
        &ThemeTypeEditor::_item_add_lineedit_cbk
    );
    ClassDB::bind_method("_item_add_cbk", &ThemeTypeEditor::_item_add_cbk);
    ClassDB::bind_method(
        "_item_rename_cbk",
        &ThemeTypeEditor::_item_rename_cbk
    );
    ClassDB::bind_method(
        "_item_rename_entered",
    ...
    ClassDB::bind_method(
        "_item_rename_confirmed",
        &ThemeTypeEditor::_item_rename_confirmed
    );
    ClassDB::bind_method(
        "_item_rename_canceled",
        &ThemeTypeEditor::_item_rename_canceled
    );
    ClassDB::bind_method(
        "_item_remove_cbk",
        &ThemeTypeEditor::_item_remove_cbk
    );
    ClassDB::bind_method(
        "_item_override_cbk",
        &ThemeTypeEditor::_item_override_cbk
    );

    ClassDB::bind_method(
        "_color_item_changed",
        &ThemeTypeEditor::_color_item_changed
    );
    ClassDB::bind_method(
        "_constant_item_changed",
        &ThemeTypeEditor::_constant_item_changed
    );
    ClassDB::bind_method(
        "_edit_resource_item",
    ...
    ClassDB::bind_method(
        "_font_item_changed",
        &ThemeTypeEditor::_font_item_changed
    );
    ClassDB::bind_method(
        "_icon_item_changed",
        &ThemeTypeEditor::_icon_item_changed
    );
    ClassDB::bind_method(
        "_stylebox_item_changed",
    ...
    ClassDB::bind_method(
        "_unpin_leading_stylebox",
        &ThemeTypeEditor::_unpin_leading_stylebox
    );
    ClassDB::bind_method(
        "_update_stylebox_from_leading",
...
void ThemeTypeEditor::set_edited_theme(const Ref<Theme>& p_theme) {
    if (edited_theme.is_valid()) {
        edited_theme
            ->disconnect("changed", this, "_update_type_list_debounced");
    }
...
ThemeTypeEditor::ThemeTypeEditor() {
    ...
    add_type_options
        ->connect("item_activated", this, "_add_type_dialog_activated");
    ...
    show_default_items_button->set_tooltip(TTR(
        "Show default type items alongside items that have been overridden."
    ));
    ...
    add_default_items_button
        ->connect("pressed", this, "_add_default_type_items");
...
void ThemeEditor::edit(const Ref<Theme>& p_theme) {
    ...
    for (int i = 0; i < preview_tabs_content->get_child_count(); i++) {
        ThemeEditorPreview* preview_tab = Object::cast_to<ThemeEditorPreview>(
            preview_tabs_content->get_child(i)
        );
        if (!preview_tab) {
            continue;
        }
...
void ThemeEditor::_add_preview_tab(
    ThemeEditorPreview* p_preview_tab,
    const String& p_preview_name,
    ...
    preview_tabs->set_tab_right_button(
        preview_tabs->get_tab_count() - 1,
        EditorNode::get_singleton()->get_gui_base()->get_icon("close", "Tabs")
    );
...
void ThemeEditor::_change_preview_tab(int p_tab) {
    ERR_FAIL_INDEX_MSG(
        p_tab,
        preview_tabs_content->get_child_count(),
        "Attempting to open a preview tab that doesn't exist."
    );

    for (int i = 0; i < preview_tabs_content->get_child_count(); i++) {
        Control* c =
            Object::cast_to<Control>(preview_tabs_content->get_child(i));
        if (!c) {
            continue;
        }
...
void ThemeEditor::_remove_preview_tab(int p_tab) {
    ERR_FAIL_INDEX_MSG(
        p_tab,
        preview_tabs_content->get_child_count(),
        "Attempting to remove a preview tab that doesn't exist."
    );

    ThemeEditorPreview* preview_tab = Object::cast_to<ThemeEditorPreview>(
        preview_tabs_content->get_child(p_tab)
    );
    ERR_FAIL_COND_MSG(
        Object::cast_to<DefaultThemeEditorPreview>(preview_tab),
        "Attemptying to remove the default preview tab."
    );

    if (preview_tab) {
        preview_tab
            ->disconnect("control_picked", this, "_preview_control_picked");
        if (preview_tab->is_connected(
                "scene_invalidated",
                this,
                "_remove_preview_tab_invalid"
            )) {
        ...
        if (preview_tab
                ->is_connected("scene_reloaded", this, "_update_preview_tab")) {
            preview_tab
                ->disconnect("scene_reloaded", this, "_update_preview_tab");
        }
...
void ThemeEditor::_update_preview_tab(Node* p_tab_control) {
    ...
    SceneThemeEditorPreview* scene_preview =
        Object::cast_to<SceneThemeEditorPreview>(p_tab_control);
    preview_tabs->set_tab_title(
        tab_index,
        scene_preview->get_preview_scene_path().get_file()
    );
}
...
void ThemeEditor::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_ENTER_TREE:
        case NOTIFICATION_THEME_CHANGED: {
            preview_tabs->add_style_override(
                "tab_fg",
                get_stylebox("ThemeEditorPreviewFG", "EditorStyles")
            );
            preview_tabs->add_style_override(
                "tab_bg",
                get_stylebox("ThemeEditorPreviewBG", "EditorStyles")
            );
            preview_tabs_content->add_style_override(
                "panel",
                get_stylebox("panel_odd", "TabContainer")
            );
...
void ThemeEditor::_bind_methods() {
    // Internal binds.
    ClassDB::bind_method(
        "_theme_save_button_cbk",
        &ThemeEditor::_theme_save_button_cbk
    );
    ClassDB::bind_method(
        "_theme_edit_button_cbk",
        &ThemeEditor::_theme_edit_button_cbk
    );
    ClassDB::bind_method(
        "_change_preview_tab",
        &ThemeEditor::_change_preview_tab
    );
    ClassDB::bind_method(
        "_remove_preview_tab",
        &ThemeEditor::_remove_preview_tab
    );
    ClassDB::bind_method(
        "_add_preview_button_cbk",
        &ThemeEditor::_add_preview_button_cbk
    );
    ClassDB::bind_method(
        "_preview_control_picked",
        &ThemeEditor::_preview_control_picked
    );
    ClassDB::bind_method(
        "_preview_scene_dialog_cbk",
        &ThemeEditor::_preview_scene_dialog_cbk
    );
    ClassDB::bind_method(
        "_remove_preview_tab_invalid",
    ...
    ClassDB::bind_method(
        "_update_preview_tab",
        &ThemeEditor::_update_preview_tab
    );
}
...
ThemeEditor::ThemeEditor() {
    ...
    theme_save_button
        ->connect("pressed", this, "_theme_save_button_cbk", varray(false));
    ...
    DefaultThemeEditorPreview* default_preview_tab =
        memnew(DefaultThemeEditorPreview);
    preview_tabs_content->add_child(default_preview_tab);
    default_preview_tab
        ->connect("control_picked", this, "_preview_control_picked");
...
bool ThemeEditorPlugin::handles(Object* p_node) const {
    ...
    // If we are editing a theme already and this particular resource happens to
    // belong to it, then we just keep editing it, despite not being able to
    // directly handle it. This only goes one layer deep, but if required this
    // can be extended to support, say, FontData inside of Font.
    ...
    if (Object::cast_to<Font>(p_node)) {
        ...
        for (List<StringName>::Element* E = types.front(); E; E = E->next()) {
            ...
            for (List<StringName>::Element* F = names.front(); F;
                 F                            = F->next()) {
                if (stylebox_item
                    == edited_theme->get_stylebox(F->get(), E->get())) {
                    belongs_to_theme = true;
                    break;
                }
            }
