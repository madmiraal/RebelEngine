...
String TextEditor::get_name() {
    ...
    if (text_file->get_path().find("local://") == -1
        && text_file->get_path().find("::") == -1) {
...
void TextEditor::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &TextEditor::_bookmark_item_pressed
    );
    ClassDB::bind_method(
        "_load_theme_settings",
        &TextEditor::_load_theme_settings
    );
    ClassDB::bind_method("_edit_option", &TextEditor::_edit_option);
    ClassDB::bind_method(
        "_change_syntax_highlighter",
        &TextEditor::_change_syntax_highlighter
    );
    ClassDB::bind_method(
        "_text_edit_gui_input",
        &TextEditor::_text_edit_gui_input
    );
...
void TextEditor::_text_edit_gui_input(const Ref<InputEvent>& ev) {
    ...
    if (k.is_valid() && k->is_pressed() && k->get_scancode() == KEY_MENU) {
        ...
        _make_context_menu(
            ...
            tx->can_fold(line),
            tx->is_folded(line),
            (get_global_transform().inverse() * tx->get_global_transform())
                .xform(tx->_get_cursor_pixel_pos())
        );
...
void TextEditor::_make_context_menu(
    bool p_selection,
    bool p_can_fold,
    bool p_is_folded,
    Vector2 p_position
...
TextEditor::TextEditor() {
    ...
    convert_case->add_shortcut(
        ED_SHORTCUT("script_text_editor/capitalize", TTR("Capitalize")),
        EDIT_CAPITALIZE
    );
