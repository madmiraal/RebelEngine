...
void AnimationNodeStateMachineEditor::_state_machine_gui_input(
    ...
    if (mb.is_valid() && mb->is_pressed()
        ...
        menu->set_global_position(
            state_machine_draw->get_global_transform().xform(mb->get_position())
        );
        menu->popup();
        add_node_pos =
            mb->get_position() / EDSCALE + state_machine->get_graph_offset();
    }

    // select node or push a field inside
    if (mb.is_valid() && !mb->get_shift() && mb->is_pressed()
        && tool_select->is_pressed() && mb->get_button_index() == BUTTON_LEFT) {
        selected_transition_from = StringName();
        selected_transition_to   = StringName();
        selected_node            = StringName();

        for (int i = node_rects.size() - 1; i >= 0;
             i--) { // inverse to draw order
            ...
            if (node_rects[i].name.has_point(mb->get_position())) { // edit name
                ...
                name_edit->set_global_position(
                    state_machine_draw->get_global_transform().xform(
                        edit_rect.position
                    )
                );
            ...
            if (node_rects[i].node.has_point(mb->get_position()
                )) { // select node since nothing else was selected
                selected_node = node_rects[i].node_name;

                Ref<AnimationNode> anode =
                    state_machine->get_node(selected_node);
        ...
        if (closest >= 0) {
            ...
            Ref<AnimationNodeStateMachineTransition> tr =
                state_machine->get_transition(closest);
    ...
    // end moving node
    if (mb.is_valid() && dragging_selected_attempt
        ...
        if (dragging_selected) {
            ...
            undo_redo->add_do_method(
                ...
                "set_node_position",
                selected_node,
                state_machine->get_node_position(selected_node)
                    + drag_ofs / EDSCALE
            );
    ...
    // connect nodes
    if (mb.is_valid()
        && ((tool_select->is_pressed() && mb->get_shift())
            || tool_connect->is_pressed())
        && mb->get_button_index() == BUTTON_LEFT && mb->is_pressed()) {
        for (int i = node_rects.size() - 1; i >= 0;
             i--) { // inverse to draw order
            if (node_rects[i].node.has_point(mb->get_position()
    ...
    if (mb.is_valid() && connecting && mb->get_button_index() == BUTTON_LEFT
        && !mb->is_pressed()) {
        if (connecting_to_node != StringName()) {
            if (state_machine
                    ->has_transition(connecting_from, connecting_to_node)) {
                EditorNode::get_singleton()->show_warning(
                    TTR("Transition exists!")
                );
    ...
    if (mm.is_valid() && connecting) {
        ...
        for (int i = node_rects.size() - 1; i >= 0;
            ...
            if (node_rects[i].node_name != connecting_from
                && node_rects[i].node.has_point(connecting_to
                )) { // select node since nothing else was selected
    ...
    if (mm.is_valid() && dragging_selected_attempt) {
        ...
        {
            // snap
            Vector2 cpos = state_machine->get_node_position(selected_node)
                         + drag_ofs / EDSCALE;
    ...
    // put ibeam (text cursor) over names to make it clearer that they are
    // editable
    if (mm.is_valid()) {
        ...
        if (new_over_node != over_node
            || new_over_node_what != over_node_what) {
        ...
        if (over_text != over_text_now) {
            if (over_text_now) {
                state_machine_draw->set_default_cursor_type(CURSOR_IBEAM);
            } else {
                state_machine_draw->set_default_cursor_type(CURSOR_ARROW);
            }
...
void AnimationNodeStateMachineEditor::_connection_draw(
    ...
    xf.elements[2] = (p_from + p_to) * 0.5
                   - xf.elements[1] * icon->get_height() * 0.5
                   - xf.elements[0] * icon->get_height() * 0.5;
...
void AnimationNodeStateMachineEditor::_state_machine_draw() {
    ...
    Ref<StyleBox> style_selected =
        get_stylebox("state_machine_selectedframe", "GraphNode");
    ...
    if (state_machine_draw->has_focus()) {
        state_machine_draw->draw_rect(
            Rect2(Point2(), state_machine_draw->get_size()),
            accent,
            false
        );
    }
    ...
    if (dragging_selected) {
        Vector2 from =
            (state_machine->get_node_position(selected_node) * EDSCALE)
            + drag_ofs - state_machine->get_graph_offset() * EDSCALE;
        if (snap_x != StringName()) {
            Vector2 to = (state_machine->get_node_position(snap_x) * EDSCALE)
                       - state_machine->get_graph_offset() * EDSCALE;
            state_machine_draw->draw_line(from, to, linecolor, 2);
        }
        if (snap_y != StringName()) {
            Vector2 to = (state_machine->get_node_position(snap_y) * EDSCALE)
                       - state_machine->get_graph_offset() * EDSCALE;
            state_machine_draw->draw_line(from, to, linecolor, 2);
        }
    }
    ...
    for (List<StringName>::Element* E = nodes.front(); E; E = E->next()) {
        ...
        String name              = E->get();
        bool needs_editor =
            EditorNode::get_singleton()->item_has_editor(anode.ptr());
    ...
    if (connecting) {
        Vector2 from =
            (state_machine->get_node_position(connecting_from) * EDSCALE)
            - state_machine->get_graph_offset() * EDSCALE;
        Vector2 to;
        if (connecting_to_node != StringName()) {
            to =
                (state_machine->get_node_position(connecting_to_node) * EDSCALE)
                - state_machine->get_graph_offset() * EDSCALE;
        } else {
            to = connecting_to;
        }
    ...
    for (int i = 0; i < state_machine->get_transition_count(); i++) {
        ...
        Vector2 ofs_from = (dragging_selected && tl.from_node == selected_node)
                             ? drag_ofs
                             : Vector2();
        tl.from = (state_machine->get_node_position(tl.from_node) * EDSCALE)
                + ofs_from - state_machine->get_graph_offset() * EDSCALE;

        tl.to_node     = state_machine->get_transition_to(i);
        Vector2 ofs_to = (dragging_selected && tl.to_node == selected_node)
                           ? drag_ofs
                           : Vector2();
        tl.to = (state_machine->get_node_position(tl.to_node) * EDSCALE)
              + ofs_to - state_machine->get_graph_offset() * EDSCALE;

        Ref<AnimationNodeStateMachineTransition> tr =
            state_machine->get_transition(i);
        ...
        if (state_machine->has_transition(
                tl.to_node,
                tl.from_node
            )) { // offset if same exists
            Vector2 offset =
                -(tl.from - tl.to).normalized().tangent() * tr_bidi_offset;
            tl.from += offset;
            tl.to   += offset;
        }
        ...
        bool selected = selected_transition_from == tl.from_node
                     && selected_transition_to == tl.to_node;
        ...
        if (travel_path.size()) {
            if (current == tl.from_node && travel_path[0] == tl.to_node) {
                ...
                for (int j = 0; j < travel_path.size() - 1; j++) {
                    if (travel_path[j] == tl.from_node
                        && travel_path[j + 1] == tl.to_node) {
                        travel = true;
                        break;
                    }
                }
        ...
        StringName fullpath =
            AnimationTreeEditor::get_singleton()->get_base_path()
            + String(tl.advance_condition_name);
        if (tl.advance_condition_name != StringName()
            && bool(
                AnimationTreeEditor::get_singleton()->get_tree()->get(fullpath)
            )) {
    ...
    for (int i = 0; i < node_rects.size(); i++) {
        ...
        bool needs_editor =
            AnimationTreeEditor::get_singleton()->can_edit(anode);
        ...
        if (playing
            && (blend_from == name || current == name
                || travel_path.find(name) != -1)) {
            state_machine_draw->draw_style_box(playing_overlay, nr.node);
        }

        bool onstart = state_machine->get_start_node() == name;
        if (onstart) {
            state_machine_draw->draw_string(
                font,
                offset
                    + Vector2(
                        0,
                        -font->get_height() - 3 * EDSCALE + font->get_ascent()
                    ),
                TTR("Start"),
                font_color
            );
        ...
        nr.play.position =
            offset + Vector2(0, (h - play->get_height()) / 2).floor();
        ...
        if (over_node == name && over_node_what == 0) {
            state_machine_draw
                ->draw_texture(play_tex, nr.play.position, accent);
        } else {
        ...
        nr.name.position =
            offset + Vector2(0, (h - font->get_height()) / 2).floor();
        nr.name.size = Vector2(strsize, font->get_height());

        state_machine_draw->draw_string(
            font,
            nr.name.position + Vector2(0, font->get_ascent()),
            name,
            font_color
        );
        ...
        if (needs_editor) {
            nr.edit.position =
                offset + Vector2(0, (h - edit->get_height()) / 2).floor();
            nr.edit.size = edit->get_size();

            if (over_node == name && over_node_what == 1) {
                state_machine_draw
                    ->draw_texture(edit, nr.edit.position, accent);
            } else {
...
void AnimationNodeStateMachineEditor::_state_machine_pos_draw() {
    ...
    from.y = (nr.play.position.y + nr.play.size.y + nr.node.position.y
              + nr.node.size.y)
           * 0.5;
...
void AnimationNodeStateMachineEditor::_notification(int p_what) {
    if (p_what == NOTIFICATION_ENTER_TREE
        ...
        transition_mode->add_icon_item(
            get_icon("TransitionImmediate", "EditorIcons"),
            TTR("Immediate")
        );
        transition_mode->add_icon_item(
            get_icon("TransitionSync", "EditorIcons"),
            TTR("Sync")
        );
        transition_mode->add_icon_item(
            get_icon("TransitionEnd", "EditorIcons"),
            TTR("At End")
        );

        // force filter on those, so they deform better
        get_icon("TransitionImmediateBig", "EditorIcons")
            ->set_flags(Texture::FLAG_FILTER);
        get_icon("TransitionEndBig", "EditorIcons")
            ->set_flags(Texture::FLAG_FILTER);
        get_icon("TransitionSyncBig", "EditorIcons")
            ->set_flags(Texture::FLAG_FILTER);
        ...
        play_mode->clear();
        play_mode->add_icon_item(
            get_icon("PlayTravel", "EditorIcons"),
            TTR("Travel")
        );
        play_mode->add_icon_item(
            get_icon("Play", "EditorIcons"),
            TTR("Immediate")
        );
    }
    ...
    if (p_what == NOTIFICATION_PROCESS) {
        ...
        for (int i = 0; i < transition_lines.size(); i++) {
            ...
            for (int j = 0; j < state_machine->get_transition_count(); j++) {
                if (transition_lines[i].from_node
                        == state_machine->get_transition_from(j)
                    && transition_lines[i].to_node
                           == state_machine->get_transition_to(j)) {
                    tidx = j;
                    break;
                }
            }
            ...
            if (transition_lines[i].disabled
                != state_machine->get_transition(tidx)->is_disabled()) {
                state_machine_draw->update();
                break;
            }

            if (transition_lines[i].auto_advance
                != state_machine->get_transition(tidx)->has_auto_advance()) {
                state_machine_draw->update();
                break;
            }

            if (transition_lines[i].advance_condition_name
                != state_machine->get_transition(tidx)
                       ->get_advance_condition_name()) {
                state_machine_draw->update();
                break;
            }

            if (transition_lines[i].mode
                != state_machine->get_transition(tidx)->get_switch_mode()) {
                state_machine_draw->update();
                break;
            }

            bool acstate =
                transition_lines[i].advance_condition_name != StringName()
                && bool(AnimationTreeEditor::get_singleton()->get_tree()->get(
                    AnimationTreeEditor::get_singleton()->get_base_path()
                    + String(transition_lines[i].advance_condition_name)
                ));
        ...
        if (!same_travel_path || last_active != is_playing
...

        {
            if (current_node != StringName()
                ...
                while (anodesm.is_valid()) {
                    ...
                    anodesm  = anodesm->get_node(
                        current_node_playback->get_current_node()
                    );
                }

                // when current_node is a state machine, use playback of
                // current_node to set play_pos
                if (current_node_playback.is_valid()) {
                    play_pos = current_node_playback->get_current_play_pos();
                    current_length =
                        current_node_playback->get_current_length();
                }
            }
        }
...
void AnimationNodeStateMachineEditor::_name_edited(const String& p_text) {
    ...
    ERR_FAIL_COND(
        new_name == "" || new_name.find(".") != -1 || new_name.find("/") != -1
    );
    ...
    undo_redo
        ->add_do_method(state_machine.ptr(), "rename_node", prev_name, name);
    undo_redo
        ->add_undo_method(state_machine.ptr(), "rename_node", name, prev_name);
    undo_redo->add_do_method(this, "_update_graph");
    undo_redo->add_undo_method(this, "_update_graph");
    undo_redo->commit_action();
    name_edit->hide();
...
void AnimationNodeStateMachineEditor::_scroll_changed(double) {
    ...
    state_machine->set_graph_offset(
        Vector2(h_scroll->get_value(), v_scroll->get_value())
    );
    state_machine_draw->update();
}
...
void AnimationNodeStateMachineEditor::_erase_selected() {
    if (selected_node != StringName()
        && state_machine->has_node(selected_node)) {
        updating = true;
        undo_redo->create_action(TTR("Node Removed"));
        undo_redo
            ->add_do_method(state_machine.ptr(), "remove_node", selected_node);
        undo_redo->add_undo_method(
            state_machine.ptr(),
            "add_node",
            selected_node,
    ...
    if (selected_transition_to != StringName()
        && selected_transition_from != StringName()
        && state_machine->has_transition(
            selected_transition_from,
            selected_transition_to
        )) {
        Ref<AnimationNodeStateMachineTransition> tr =
            state_machine->get_transition(state_machine->find_transition(
                selected_transition_from,
                selected_transition_to
            ));
        updating = true;
...
void AnimationNodeStateMachineEditor::_end_selected() {
    if (selected_node != StringName()
        ...
        undo_redo->create_action(TTR("Set Start Node (Autoplay)"));
        undo_redo
            ->add_do_method(state_machine.ptr(), "set_end_node", new_end_node);
        undo_redo->add_undo_method(
            state_machine.ptr(),
            "set_end_node",
            state_machine->get_end_node()
        );
        undo_redo->add_do_method(this, "_update_graph");
        undo_redo->add_undo_method(this, "_update_graph");
        undo_redo->commit_action();
...
void AnimationNodeStateMachineEditor::_bind_methods() {
    ClassDB::bind_method(
        "_state_machine_gui_input",
        &AnimationNodeStateMachineEditor::_state_machine_gui_input
    );
    ClassDB::bind_method(
        "_state_machine_draw",
    ...
    ClassDB::bind_method(
        "_state_machine_pos_draw",
        &AnimationNodeStateMachineEditor::_state_machine_pos_draw
    );
    ClassDB::bind_method(
        "_update_graph",
        &AnimationNodeStateMachineEditor::_update_graph
    );

    ClassDB::bind_method(
        "_add_menu_type",
        &AnimationNodeStateMachineEditor::_add_menu_type
    );
    ClassDB::bind_method(
        "_add_animation_type",
        &AnimationNodeStateMachineEditor::_add_animation_type
    );
    ...
    ClassDB::bind_method(
        "_name_edited_focus_out",
        &AnimationNodeStateMachineEditor::_name_edited_focus_out
    );

    ClassDB::bind_method(
        "_removed_from_graph",
        &AnimationNodeStateMachineEditor::_removed_from_graph
    );

    ClassDB::bind_method(
        "_open_editor",
        &AnimationNodeStateMachineEditor::_open_editor
    );
    ClassDB::bind_method(
        "_scroll_changed",
    ...
    ClassDB::bind_method(
        "_erase_selected",
        &AnimationNodeStateMachineEditor::_erase_selected
    );
    ClassDB::bind_method(
        "_autoplay_selected",
        &AnimationNodeStateMachineEditor::_autoplay_selected
    );
    ClassDB::bind_method(
        "_end_selected",
        &AnimationNodeStateMachineEditor::_end_selected
    );
    ClassDB::bind_method(
        "_update_mode",
        &AnimationNodeStateMachineEditor::_update_mode
    );
    ClassDB::bind_method(
        "_file_opened",
        &AnimationNodeStateMachineEditor::_file_opened
    );
}
...
AnimationNodeStateMachineEditor::AnimationNodeStateMachineEditor() {
    ...
    tool_select
        ->connect("pressed", this, "_update_mode", varray(), CONNECT_DEFERRED);
    ...
    tool_autoplay->set_tooltip(
        TTR("Toggle autoplay this animation on start, restart or seek to zero.")
    );
    ...
    tool_end->set_tooltip(
        TTR("Set the end animation. This is useful for sub-transitions.")
    );
    ...
    state_machine_play_pos->set_mouse_filter(MOUSE_FILTER_PASS
    ); // pass all to parent
