...
public:
    ...
    CanvasItemEditorSelectedItem() : prev_anchors() {
        prev_rot = 0;
    }
};
...
private:
    ...
    struct BoneList {
        ...
        BoneList() : length(0.f), last_pass(0) {}
    };
    ...
    void _find_canvas_items_at_pos(
        const Point2& p_pos,
        Node* p_node,
    ...
    void _find_canvas_items_in_rect(
        const Rect2& p_rect,
        Node* p_node,
    ...
    void _save_canvas_item_state(
        List<CanvasItem*> p_canvas_items,
        bool save_bones = false
    );
    void _restore_canvas_item_ik_chain(
        CanvasItem* p_canvas_item,
        const List<Dictionary>* p_bones_state
    );
    void _restore_canvas_item_state(
        List<CanvasItem*> p_canvas_items,
        bool restore_bones = false
    );
    void _commit_canvas_item_state(
        List<CanvasItem*> p_canvas_items,
        String action_name,
        bool commit_bones = false
    );
    ...
    void _expand_encompassing_rect_using_children(
        Rect2& r_rect,
        const Node* p_node,
        bool& r_first,
        const Transform2D& p_parent_xform = Transform2D(),
    ...
    void _draw_text_at_position(
        Point2 p_position,
        String p_string,
        Margin p_side
    );
    void _draw_margin_at_position(
        int p_value,
        Point2 p_position,
        Margin p_side
    );
    ...
    float _get_next_zoom_value(
        int p_increment_count,
        bool p_integer_only = false
    ) const;
...
protected:
    ...
    struct compare_items_x {
        bool operator()(const CanvasItem* a, const CanvasItem* b) const {
            return a->get_global_transform().elements[2].x
                 < b->get_global_transform().elements[2].x;
        }
    };

    struct compare_items_y {
        bool operator()(const CanvasItem* a, const CanvasItem* b) const {
            return a->get_global_transform().elements[2].y
                 < b->get_global_transform().elements[2].y;
        }
    };
