...
void AnimationNodeBlendTreeEditor::add_custom_type(
    const String& p_name,
...
void AnimationNodeBlendTreeEditor::remove_custom_type(
    const Ref<Script>& p_script
) {
    for (int i = 0; i < add_options.size(); i++) {
...
void AnimationNodeBlendTreeEditor::_update_graph() {
    ...
    for (List<StringName>::Element* E = nodes.front(); E; E = E->next()) {
        ...
        for (List<PropertyInfo>::Element* F = pinfo.front(); F; F = F->next()) {
            ...
            String base_path =
                ...
                + String(E->get()) + "/" + F->get().name;
            EditorProperty* prop = EditorInspector::instantiate_property_editor(
                AnimationTreeEditor::get_singleton()->get_tree(),
                F->get().type,
                base_path,
                F->get().hint,
                F->get().hint_string,
        ...
        if (anim.is_valid()) {
            ...
            if (player->has_node(player->get_animation_player())) {
                AnimationPlayer* ap = Object::cast_to<AnimationPlayer>(
                    player->get_node(player->get_animation_player())
                );
                if (ap) {
                    ...
                    if (ap->has_animation(anim->get_animation())) {
                        pb->set_max(ap->get_animation(anim->get_animation())
                                        ->get_length());
                    }
                }
            }
            ...
            mb->get_popup()->connect(
                "index_pressed",
                this,
                "_anim_selected",
                varray(options, E->get()),
                CONNECT_DEFERRED
            );
        }
    ...
    for (List<AnimationNodeBlendTree::NodeConnection>::Element* E =
             connections.front();
         E;
         E = E->next()) {
...
void AnimationNodeBlendTreeEditor::_add_node(int p_idx) {
    ...
    if (p_idx == MENU_LOAD_FILE) {
        ...
        AnimationNode* an = Object::cast_to<AnimationNode>(
        ...
        AnimationNode* an =
            Object::cast_to<AnimationNode>(ClassDB::instance(base_type));
        ERR_FAIL_COND(!an);
        anode = Ref<AnimationNode>(an);
    ...
    if (out.is_valid()) {
        EditorNode::get_singleton()->show_warning(
            TTR("Output node can't be added to the blend tree.")
        );
        return;
    }
...
void AnimationNodeBlendTreeEditor::_popup(
    bool p_has_input_ports,
    const Vector2& p_popup_position,
    const Vector2& p_node_position
) {
...
void AnimationNodeBlendTreeEditor::_node_dragged(
...
void AnimationNodeBlendTreeEditor::_connection_request(
    ...
    AnimationNodeBlendTree::ConnectionError err =
        blend_tree->can_connect_node(p_to, p_to_index, p_from);

    if (err != AnimationNodeBlendTree::CONNECTION_OK) {
        EditorNode::get_singleton()->show_warning(
            TTR("Unable to connect, port may be in use or connection may be "
                "invalid.")
        );
        return;
    }
...
void AnimationNodeBlendTreeEditor::_disconnection_request(
    const String& p_from,
    int p_from_index,
    const String& p_to,
    int p_to_index
    ...
    undo_redo
        ->add_do_method(blend_tree.ptr(), "disconnect_node", p_to, p_to_index);
    undo_redo->add_undo_method(
        blend_tree.ptr(),
        "connect_node",
        p_to,
...
void AnimationNodeBlendTreeEditor::_anim_selected(
    int p_index,
    Array p_options,
    const String& p_node
) {
    ...
    undo_redo
        ->add_undo_method(anim.ptr(), "set_animation", anim->get_animation());
    undo_redo->add_do_method(this, "_update_graph");
    undo_redo->add_undo_method(this, "_update_graph");
    undo_redo->commit_action();
}
...
void AnimationNodeBlendTreeEditor::_filter_toggled() {
    ...
    undo_redo->add_do_method(
        _filter_edit.ptr(),
        "set_filter_enabled",
        filter_enabled->is_pressed()
    );
    undo_redo->add_undo_method(
        _filter_edit.ptr(),
        "set_filter_enabled",
        _filter_edit->is_filter_enabled()
    );
...
bool AnimationNodeBlendTreeEditor::_update_filters(
    const Ref<AnimationNode>& anode
) {
    ...
    if (!player) {
        EditorNode::get_singleton()->show_warning(TTR(
            "Player path set is invalid, so unable to retrieve track names."
        ));
        return false;
    }
            ...
            if (skeleton && skeleton->find_bone(concat) != -1) {
                ...
                for (List<String>::Element* F = bone_path.front(); F;
                    ...
                    if (!parenthood.has(accum)) {
                        ...
                        ti->set_icon(
                            0,
                            get_icon("BoneAttachment", "EditorIcons")
                        );
                    } else {
                        ti = parenthood[accum];
                    }
    ...
    if (p_what == NOTIFICATION_PROCESS) {
        ...
        for (List<AnimationNodeBlendTree::NodeConnection>::Element* E =
            ...
            if (AnimationTreeEditor::get_singleton()->get_tree()
                && !AnimationTreeEditor::get_singleton()
                        ->get_tree()
                        ->is_state_invalid()) {
                activity =
                    AnimationTreeEditor::get_singleton()
                        ->get_tree()
                        ->get_connection_activity(path, E->get().input_index);
            }
            graph->set_connection_activity(
                E->get().output_node,
                0,
                E->get().input_node,
                E->get().input_index,
        ...
        AnimationTree* graph_player =
            AnimationTreeEditor::get_singleton()->get_tree();
        AnimationPlayer* player = nullptr;
        if (graph_player->has_node(graph_player->get_animation_player())) {
            player = Object::cast_to<AnimationPlayer>(
                graph_player->get_node(graph_player->get_animation_player())
            );
        }

        if (player) {
            for (Map<StringName, ProgressBar*>::Element* E = animations.front();
                 E;
                 E = E->next()) {
                ...
                if (an.is_valid()) {
                    if (player->has_animation(an->get_animation())) {
                        Ref<Animation> anim =
                            player->get_animation(an->get_animation());
                        if (anim.is_valid()) {
...
void AnimationNodeBlendTreeEditor::_bind_methods() {
    ClassDB::bind_method(
        "_update_graph",
        &AnimationNodeBlendTreeEditor::_update_graph
    );
    ...
    ClassDB::bind_method(
        "_node_renamed",
        &AnimationNodeBlendTreeEditor::_node_renamed
    );
    ClassDB::bind_method(
        "_node_renamed_focus_out",
    ...
    ClassDB::bind_method(
        "_node_selected",
        &AnimationNodeBlendTreeEditor::_node_selected
    );
    ClassDB::bind_method(
        "_open_in_editor",
        &AnimationNodeBlendTreeEditor::_open_in_editor
    );
    ClassDB::bind_method(
        "_scroll_changed",
        &AnimationNodeBlendTreeEditor::_scroll_changed
    );
    ClassDB::bind_method(
        "_delete_request",
        &AnimationNodeBlendTreeEditor::_delete_request
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        "_edit_filters",
        &AnimationNodeBlendTreeEditor::_edit_filters
    );
    ClassDB::bind_method(
        "_update_filters",
        &AnimationNodeBlendTreeEditor::_update_filters
    );
    ClassDB::bind_method(
        "_filter_edited",
    ...
    ClassDB::bind_method(
        "_removed_from_graph",
        &AnimationNodeBlendTreeEditor::_removed_from_graph
    );
    ClassDB::bind_method(
        "_property_changed",
        &AnimationNodeBlendTreeEditor::_property_changed
    );
    ClassDB::bind_method(
        "_file_opened",
        &AnimationNodeBlendTreeEditor::_file_opened
    );
    ClassDB::bind_method(
        "_update_options_menu",
        &AnimationNodeBlendTreeEditor::_update_options_menu
    );
    ClassDB::bind_method(
...
void AnimationNodeBlendTreeEditor::_node_renamed(
    ...
    undo_redo
        ->add_undo_method(blend_tree.ptr(), "rename_node", name, prev_name);
    undo_redo->add_do_method(
        AnimationTreeEditor::get_singleton()->get_tree(),
        "rename_parameter",
        base_path + prev_name,
    ...
    for (int i = 0; i < visible_properties.size(); i++) {
        String pname =
            visible_properties[i]->get_edited_property().operator String();
        if (pname.begins_with(base_path + prev_name)) {
            String new_name2 =
                pname.replace_first(base_path + prev_name, base_path + name);
            visible_properties[i]->set_object_and_property(
                visible_properties[i]->get_edited_object(),
                new_name2
            );
        }
    }
    ...
    _update_graph(
    ); // Needed to update the signal connections with the new name.
}

void AnimationNodeBlendTreeEditor::_node_renamed_focus_out(
    Node* le,
    Ref<AnimationNode> p_node
) {
    if (le == nullptr) {
        return; // The text_submitted signal triggered the graph update and
                // freed the LineEdit.
    }
...
void AnimationNodeBlendTreeEditor::edit(const Ref<AnimationNode>& p_node) {
    if (blend_tree.is_valid()) {
        blend_tree
            ->disconnect("removed_from_graph", this, "_removed_from_graph");
    }
...
AnimationNodeBlendTreeEditor::AnimationNodeBlendTreeEditor() {
    ...
    add_node
        ->connect("about_to_show", this, "_update_options_menu", varray(false));
