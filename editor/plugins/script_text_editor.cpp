...
void ConnectionInfoDialog::popup_connections(
    String p_method,
    Vector<Node*> p_nodes
) {
    ...
    for (int i = 0; i < p_nodes.size(); i++) {
        ...
        for (List<Connection>::Element* E = all_connections.front(); E;
             E                            = E->next()) {
            ...
            node_item->set_text(
                0,
                Object::cast_to<Node>(connection.source)->get_name()
            );
            node_item->set_icon(
                0,
                EditorNode::get_singleton()
                    ->get_object_icon(connection.source, "Node")
            );
            ...
            node_item->set_icon(
                1,
                get_parent_control()->get_icon("Slot", "EditorIcons")
            );
            ...
            node_item->set_text(
                2,
                Object::cast_to<Node>(connection.target)->get_name()
            );
            node_item->set_icon(
                2,
                EditorNode::get_singleton()
                    ->get_object_icon(connection.target, "Node")
            );
...
Vector<String> ScriptTextEditor::get_functions() {
    ...
    if (script->get_language()
            ->validate(text, line, col, errortxt, script->get_path(), &fnc)) {
...
void ScriptTextEditor::_update_member_keywords() {
    ...
    Color member_variable_color =
        EDITOR_GET("text_editor/highlighting/member_variable_color");
    ...
    for (List<PropertyInfo>::Element* E = plist.front(); E; E = E->next()) {
        ...
        if (E->get().usage & PROPERTY_USAGE_CATEGORY
            || E->get().usage & PROPERTY_USAGE_GROUP) {
            continue;
        }
        if (name.find("/") != -1) {
            continue;
        ...
        code_editor->get_text_edit()->add_member_keyword(
            name,
...
void ScriptTextEditor::_load_theme_settings() {
    ...
    Color completion_background_color =
        EDITOR_GET("text_editor/highlighting/completion_background_color");
    Color completion_selected_color =
        EDITOR_GET("text_editor/highlighting/completion_selected_color");
    Color completion_existing_color =
        EDITOR_GET("text_editor/highlighting/completion_existing_color");
    Color completion_scroll_color =
        EDITOR_GET("text_editor/highlighting/completion_scroll_color");
    Color completion_font_color =
        EDITOR_GET("text_editor/highlighting/completion_font_color");
    Color text_color = EDITOR_GET("text_editor/highlighting/text_color");
    Color line_number_color =
        EDITOR_GET("text_editor/highlighting/line_number_color");
    Color safe_line_number_color =
        EDITOR_GET("text_editor/highlighting/safe_line_number_color");
    Color caret_color = EDITOR_GET("text_editor/highlighting/caret_color");
    Color caret_background_color =
        EDITOR_GET("text_editor/highlighting/caret_background_color");
    Color text_selected_color =
        EDITOR_GET("text_editor/highlighting/text_selected_color");
    Color selection_color =
        EDITOR_GET("text_editor/highlighting/selection_color");
    Color brace_mismatch_color =
        EDITOR_GET("text_editor/highlighting/brace_mismatch_color");
    Color current_line_color =
        EDITOR_GET("text_editor/highlighting/current_line_color");
    Color line_length_guideline_color =
        EDITOR_GET("text_editor/highlighting/line_length_guideline_color");
    Color word_highlighted_color =
        EDITOR_GET("text_editor/highlighting/word_highlighted_color");
    ...
    Color bookmark_color =
        EDITOR_GET("text_editor/highlighting/bookmark_color");
    Color breakpoint_color =
        EDITOR_GET("text_editor/highlighting/breakpoint_color");
    Color executing_line_color =
        EDITOR_GET("text_editor/highlighting/executing_line_color");
    Color code_folding_color =
        EDITOR_GET("text_editor/highlighting/code_folding_color");
    ...
    Color usertype_color =
        EDITOR_GET("text_editor/highlighting/user_type_color");
    ...
    text_edit->add_constant_override(
        "line_spacing",
...
void ScriptTextEditor::_set_theme_for_script() {
    ...
    for (List<String>::Element* E = keywords.front(); E; E = E->next()) {
        if (script->get_language()->is_control_flow_keyword(E->get())) {
            // Use a different color for control flow keywords to make them
            // easier to distinguish.
            text_edit->add_keyword_color(
                E->get(),
                colors_cache.control_flow_keyword_color
            );
        } else {
    ...
    for (List<String>::Element* E = comments.front(); E; E = E->next()) {
        ...
        String end     = comment.get_slice_count(" ") > 1
                           ? comment.get_slice(" ", 1)
                           : String();

        text_edit
            ->add_color_region(beg, end, colors_cache.comment_color, end == "");
    }
    ...
    for (List<String>::Element* E = strings.front(); E; E = E->next()) {
        ...
        String end = string.get_slice_count(" ") > 1 ? string.get_slice(" ", 1)
                                                     : String();
        text_edit
            ->add_color_region(beg, end, colors_cache.string_color, end == "");
    }
}
...
void ScriptTextEditor::_warning_clicked(Variant p_line) {
    if (p_line.get_type() == Variant::INT) {
        ...
        code_editor->get_text_edit()->insert_at(
            "# warning-ignore:" + meta["code"].operator String(),
            meta["line"].operator int64_t() - 1
        );
        _validate_script();
    }
}
...
void ScriptTextEditor::add_callback(
    ...
    if (pos == -1) {
        ...
        String func =
            script->get_language()->make_function("", p_function, p_args);
        // code=code+func;
        code_editor->get_text_edit()->cursor_set_line(pos + 1);
        code_editor->get_text_edit()->cursor_set_column(1000000
        ); // none shall be that big
...
bool ScriptTextEditor::is_unsaved() {
    return code_editor->get_text_edit()->get_version()
        != code_editor->get_text_edit()->get_saved_version();
}
...
void ScriptTextEditor::set_edit_state(const Variant& p_state) {
    ...
    if (state.has("syntax_highlighter")) {
        int idx =
            highlighter_menu->get_item_idx_from_text(state["syntax_highlighter"]
            );
        if (idx >= 0) {
...
Ref<Texture> ScriptTextEditor::get_icon() {
    if (get_parent_control()
        && get_parent_control()->has_icon(script->get_class(), "EditorIcons")) {
        return get_parent_control()->get_icon(
            script->get_class(),
            "EditorIcons"
        );
    }
...
void ScriptTextEditor::_validate_script() {
    ...
    if (!script->get_language()->validate(
            text,
            line,
            col,
            errortxt,
        ...
        String error_text =
            "error(" + itos(line) + "," + itos(col) + "): " + errortxt;
    ...
    if (GLOBAL_GET("debug/rebelscript/warnings/enable").booleanize()) {
        ...
        if (base && missing_connections.size() > 0) {
            ...
            for (List<Connection>::Element* E = missing_connections.front(); E;
                 E                            = E->next()) {
                Connection connection = E->get();
                ...
                String source_path =
                    base == connection.source
                        ...
                        : base_path + "/"
                              + base->get_path_to(
                                  Object::cast_to<Node>(connection.source)
                              );
                String target_path =
                    base == connection.target
                        ? base_path
                        : base_path + "/"
                              + base->get_path_to(
                                  Object::cast_to<Node>(connection.target)
                              );

                warnings_panel->push_cell();
                warnings_panel->push_color(
                    warnings_panel->get_color("warning_color", "Editor")
                );
                warnings_panel->add_text(vformat(
                    TTR("Missing connected method '%s' for signal '%s' from "
                        "node '%s' to node '%s'."),
                    connection.method,
                    connection.signal,
                    source_path,
    ...
    for (List<ScriptLanguage::Warning>::Element* E = warnings.front(); E;
         E                                         = E->next()) {
        ...
        warnings_panel->push_color(
            warnings_panel->get_color("accent_color", "Editor")
                .linear_interpolate(
                    warnings_panel->get_color("mono_color", "Editor"),
                    0.5
                )
        );
    ...
    line--;
    bool highlight_safe =
        EDITOR_DEF("text_editor/highlighting/highlight_type_safe_lines", true);
...
void ScriptTextEditor::_update_bookmark_list() {
    ...
    bookmarks_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/toggle_bookmark"),
        BOOKMARK_TOGGLE
    );
    bookmarks_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/remove_all_bookmarks"),
        BOOKMARK_REMOVE_ALL
    );
    bookmarks_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/goto_next_bookmark"),
        BOOKMARK_GOTO_NEXT
    );
    bookmarks_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/goto_previous_bookmark"),
        BOOKMARK_GOTO_PREV
    );
    ...
    for (int i = 0; i < bookmark_list.size(); i++) {
        ...
        // Also replace any tabs by spaces, since we can't print tabs in the
        // menu.
        String line = code_editor->get_text_edit()
                          ->get_line(bookmark_list[i])
                          .replace("\t", "  ")
                          .strip_edges();
        ...
        bookmarks_menu->add_item(
            String::num((int)bookmark_list[i] + 1) + " - `" + line + "`"
        );
        bookmarks_menu->set_item_metadata(
            bookmarks_menu->get_item_count() - 1,
            bookmark_list[i]
        );
    }
}
...
static Vector<Node*> _find_all_node_for_script(
    ...
    for (int i = 0; i < p_current->get_child_count(); i++) {
        Vector<Node*> found = _find_all_node_for_script(
            p_base,
...
static Node* _find_node_for_script(
    Node* p_base,
    Node* p_current,
    const Ref<Script>& p_script
) {
...
static void _find_changed_scripts_for_external_editor(
    Node* p_base,
    Node* p_current,
    Set<Ref<Script>>& r_scripts
) {
    if (p_current->get_owner() != p_base && p_base != p_current) {
        return;
    }
    ...
    for (int i = 0; i < p_current->get_child_count(); i++) {
        _find_changed_scripts_for_external_editor(
            p_base,
            p_current->get_child(i),
            r_scripts
        );
...
void ScriptTextEditor::_code_complete_script(
    const String& p_code,
    List<ScriptCodeCompletionOption>* r_options,
    bool& r_force
) {
    ...
    String hint;
    Error err = script->get_language()->complete_code(
        p_code,
        script->get_path(),
        base,
        r_options,
...
void ScriptTextEditor::_update_breakpoint_list() {
    ...
    breakpoints_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/toggle_breakpoint"),
        DEBUG_TOGGLE_BREAKPOINT
    );
    breakpoints_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/remove_all_breakpoints"),
        DEBUG_REMOVE_ALL_BREAKPOINTS
    );
    breakpoints_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/goto_next_breakpoint"),
        DEBUG_GOTO_NEXT_BREAKPOINT
    );
    breakpoints_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/goto_previous_breakpoint"),
        DEBUG_GOTO_PREV_BREAKPOINT
    );

    Array breakpoint_list =
        code_editor->get_text_edit()->get_breakpoints_array();
    ...
    for (int i = 0; i < breakpoint_list.size(); i++) {
        ...
        breakpoints_menu->add_item(
            String::num((int)breakpoint_list[i] + 1) + " - `" + line + "`"
        );
        breakpoints_menu->set_item_metadata(
            breakpoints_menu->get_item_count() - 1,
            breakpoint_list[i]
        );
    }
}
...
void ScriptTextEditor::_breakpoint_item_pressed(int p_idx) {
    if (p_idx < 4) { // Any item before the separator.
        ...
        code_editor->get_text_edit()->call_deferred("center_viewport_to_cursor"
        ); // Need to be deferred, because goto uses call_deferred().
    }
}

void ScriptTextEditor::_breakpoint_toggled(int p_row) {
    ScriptEditor::get_singleton()->get_debugger()->set_breakpoint(
        script->get_path(),
        p_row + 1,
        code_editor->get_text_edit()->is_line_set_as_breakpoint(p_row)
    );
}

void ScriptTextEditor::_lookup_symbol(
    ...
    if (ScriptServer::is_global_class(p_symbol)) {
        EditorNode::get_singleton()->load_resource(
            ScriptServer::get_global_class_path(p_symbol)
        );
        ...
        switch (result.type) {
            ...
            case ScriptLanguage::LookupResult::RESULT_CLASS_CONSTANT: {
                ...
                emit_signal(
                    "go_to_help",
                    "class_constant:" + result.class_name + ":"
                        + result.class_member
                );
            ...
            case ScriptLanguage::LookupResult::RESULT_CLASS_PROPERTY: {
                emit_signal(
                    "go_to_help",
                    "class_property:" + result.class_name + ":"
                        + result.class_member
                );
            ...
            case ScriptLanguage::LookupResult::RESULT_CLASS_METHOD: {
                ...
                emit_signal(
                    "go_to_help",
                    "class_method:" + result.class_name + ":"
                        + result.class_member
                );
            ...
            case ScriptLanguage::LookupResult::RESULT_CLASS_ENUM: {
                ...
                StringName success;
                while (true) {
                    success = ClassDB::get_integer_constant_enum(
                        cname,
                        result.class_member,
                        true
                    );
            ...
            case ScriptLanguage::LookupResult::RESULT_CLASS_TBD_GLOBALSCOPE: {
                emit_signal(
                    "go_to_help",
                    "class_global:" + result.class_name + ":"
                        + result.class_member
                );
            } break;
        }
        ...
        // Every symbol other than absolute path is relative path so keep this
        // condition at last.
...
void ScriptTextEditor::_update_connected_methods() {
    ...
    for (int i = 0; i < nodes.size(); i++) {
        ...
        for (List<Connection>::Element* E = connections.front(); E;
            ...
            // As deleted nodes are still accessible via the undo/redo system,
            // check if they're still on the tree.
            ...
            if (!ClassDB::has_method(
                ...
                for (int j = 0; j < functions.size(); j++) {
                    ...
                    if (name == connection.method) {
                        ...
                        text_edit->set_line_info_icon(
                            line - 1,
                            get_parent_control()
                                ->get_icon("Slot", "EditorIcons"),
                            connection.method
                        );
                ...
                // There is a chance that the method is inherited from another
                // script.
...
void ScriptTextEditor::_edit_option(int p_op) {
    ...
    switch (p_op) {
        ...
        case EDIT_EVALUATE: {
            ...
            Vector<String> lines =
                code_editor->get_text_edit()->get_selection_text().split("\n");
            PoolStringArray results;

            for (int i = 0; i < lines.size(); i++) {
                String line       = lines[i];
                String whitespace = line.substr(
                    0,
                    line.size() - line.strip_edges(true, false).size()
                ); // extract the whitespace at the beginning

                if (expression.parse(line) == OK) {
                    Variant result =
                        expression.execute(Array(), Variant(), false);
                    if (expression.get_error_text() == "") {
                        results.append(
                            whitespace + result.get_construct_string()
                        );
                    } else {
                        results.append(line);
                    }
            ...
            ); // prevents creating a two-step undo
            code_editor->get_text_edit()->insert_text_at_cursor(
                results.join("\n")
            );
        ...
        case SEARCH_IN_FILES: {
            String selected_text =
                code_editor->get_text_edit()->get_selection_text();

            // Yep, because it doesn't make sense to instance this dialog for
            // every single script open... So this will be delegated to the
            // ScriptEditor.
        ...
        case DEBUG_GOTO_NEXT_BREAKPOINT: {
            ...
            if (line >= bpoints[bpoints.size() - 1]) {
                ...
                for (List<int>::Element* E = bpoints.front(); E;
                     E                     = E->next()) {
                    int bline = E->get();
                    if (bline > line) {
...
void ScriptTextEditor::_edit_option_toggle_inline_comment() {
    ...
    for (List<String>::Element* E = comment_delimiters.front(); E;
         E                        = E->next()) {
...
void ScriptTextEditor::set_syntax_highlighter(SyntaxHighlighter* p_highlighter
    ...
    if (p_highlighter != nullptr) {
        highlighter_menu->set_item_checked(
            highlighter_menu->get_item_idx_from_text(p_highlighter->get_name()),
            true
        );
    } else {
        highlighter_menu->set_item_checked(
            highlighter_menu->get_item_idx_from_text(TTR("Standard")),
            true
        );
    }
}
...
void ScriptTextEditor::_change_syntax_highlighter(int p_idx) {
    ...
    while (el != nullptr) {
        highlighter_menu->set_item_checked(
            highlighter_menu->get_item_idx_from_text(el->key()),
            false
        );
        el = el->next();
    }
...
void ScriptTextEditor::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &ScriptTextEditor::_update_bookmark_list
    );
    ClassDB::bind_method(
        "_bookmark_item_pressed",
        &ScriptTextEditor::_bookmark_item_pressed
    );
    ClassDB::bind_method(
        "_load_theme_settings",
        &ScriptTextEditor::_load_theme_settings
    );
    ClassDB::bind_method(
        "_update_breakpoint_list",
        &ScriptTextEditor::_update_breakpoint_list
    );
    ClassDB::bind_method(
        "_breakpoint_item_pressed",
        &ScriptTextEditor::_breakpoint_item_pressed
    );
    ClassDB::bind_method(
        "_breakpoint_toggled",
        &ScriptTextEditor::_breakpoint_toggled
    );
    ClassDB::bind_method(
        "_lookup_connections",
        &ScriptTextEditor::_lookup_connections
    );
    ClassDB::bind_method(
        "_update_connected_methods",
        &ScriptTextEditor::_update_connected_methods
    );
    ClassDB::bind_method(
        "_change_syntax_highlighter",
        &ScriptTextEditor::_change_syntax_highlighter
    );
    ...
    ClassDB::bind_method(
        "_text_edit_gui_input",
        &ScriptTextEditor::_text_edit_gui_input
    );
    ClassDB::bind_method(
        "_show_warnings_panel",
        &ScriptTextEditor::_show_warnings_panel
    );
    ClassDB::bind_method(
        "_warning_clicked",
    ...
    ClassDB::bind_method(
        "_prepare_edit_menu",
        &ScriptTextEditor::_prepare_edit_menu
    );

    ClassDB::bind_method(
        "get_drag_data_fw",
        &ScriptTextEditor::get_drag_data_fw
    );
    ClassDB::bind_method(
        "can_drop_data_fw",
...
void ScriptTextEditor::reload(bool p_soft) {
    ...
    bool soft = p_soft
             || scr->get_instance_base_type()
                    == "EditorPlugin"; // always soft-reload editor plugins
...
void ScriptTextEditor::set_tooltip_request_func(
    String p_method,
    Object* p_obj
) {
    code_editor->get_text_edit()
        ->set_tooltip_request_func(p_obj, p_method, this);
}

void ScriptTextEditor::set_debugger_active(bool p_active) {}

Variant ScriptTextEditor::get_drag_data_fw(
...
bool ScriptTextEditor::can_drop_data_fw(
    const Point2& p_point,
...
void ScriptTextEditor::drop_data_fw(
    const Point2& p_point,
    const Variant& p_data,
    Control* p_from
) {
    ...
    if (d.has("type") && String(d["type"]) == "resource") {
        ...
        if (res->get_path().is_resource_file()) {
            EditorNode::get_singleton()->show_warning(
                TTR("Only resources from filesystem can be dropped.")
            );
            return;
        }
    ...
    if (d.has("type")
        ...
        for (int i = 0; i < files.size(); i++) {
            ...
            if (preload) {
                text_to_drop += "preload("
                              + String(files[i]).c_escape().quote(quote_style)
                              + ")";
            } else {
    ...
    if (d.has("type") && String(d["type"]) == "obj_property") {
        ...
        const String text_to_drop =
            String(d["property"]).c_escape().quote(quote_style);
...
void ScriptTextEditor::_text_edit_gui_input(const Ref<InputEvent>& ev) {
    ...
    TextEdit* tx = code_editor->get_text_edit();
    if (mb.is_valid() && mb->get_button_index() == BUTTON_RIGHT
        && mb->is_pressed()) {
    ...
    if (create_menu) {
        ...
        tx->set_right_click_moves_caret(EditorSettings::get_singleton()->get(
        ...
        if (word_at_pos.is_resource_file()) {
            ...
            ScriptLanguage::LookupResult result;
            if (script->get_language()->lookup_code(
                    code_editor->get_text_edit()
                        ->get_text_for_lookup_completion(),
                    word_at_pos,
                    script->get_path(),
                    base,
                    result
        ...
        if (has_color) {
            ...
            if (valid) {
                ...
                String stripped =
                    color_args.replace(" ", "").replace("(", "").replace(
                        ")",
                        ""
                    );
...
void ScriptTextEditor::_color_changed(const Color& p_color) {
    ...
    if (p_color.a == 1.0f) {
        new_args = String(
            "(" + rtos(p_color.r) + ", " + rtos(p_color.g) + ", "
            + rtos(p_color.b) + ")"
        );
    } else {
        new_args = String(
            "(" + rtos(p_color.r) + ", " + rtos(p_color.g) + ", "
            + rtos(p_color.b) + ", " + rtos(p_color.a) + ")"
        );
    }
    ...
    line_with_replaced_args =
        line_with_replaced_args.insert(color_args_pos, new_args);

    color_args = new_args;
    code_editor->get_text_edit()->begin_complex_operation();
    code_editor->get_text_edit()->set_line(
        color_position.x,
        line_with_replaced_args
    );
    code_editor->get_text_edit()->end_complex_operation();
...
void ScriptTextEditor::_make_context_menu(
    bool p_selection,
    bool p_color,
    bool p_foldable,
    bool p_open_docs,
    ...
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/undo"),
        EDIT_UNDO
    );
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/redo"),
        EDIT_REDO
    );

    context_menu->add_separator();
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/cut"),
        EDIT_CUT
    );
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/copy"),
        EDIT_COPY
    );
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/paste"),
        EDIT_PASTE
    );

    context_menu->add_separator();
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/select_all"),
        EDIT_SELECT_ALL
    );

    context_menu->add_separator();
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/indent_left"),
        EDIT_INDENT_LEFT
    );
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/indent_right"),
        EDIT_INDENT_RIGHT
    );
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/toggle_comment"),
        EDIT_TOGGLE_COMMENT
    );
    context_menu->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/toggle_bookmark"),
    ...
    if (p_selection) {
        context_menu->add_separator();
        context_menu->add_shortcut(
            ED_GET_SHORTCUT("script_text_editor/convert_to_uppercase"),
            EDIT_TO_UPPERCASE
        );
        context_menu->add_shortcut(
            ED_GET_SHORTCUT("script_text_editor/convert_to_lowercase"),
            EDIT_TO_LOWERCASE
        );
        context_menu->add_shortcut(
            ED_GET_SHORTCUT("script_text_editor/evaluate_selection"),
            EDIT_EVALUATE
        );
    }
    if (p_foldable) {
        context_menu->add_shortcut(
            ED_GET_SHORTCUT("script_text_editor/toggle_fold_line"),
    ...
    context_menu->set_item_disabled(
        context_menu->get_item_index(EDIT_UNDO),
        !tx->has_undo()
    );
    context_menu->set_item_disabled(
        context_menu->get_item_index(EDIT_REDO),
        !tx->has_redo()
    );
...
void ScriptTextEditor::_enable_code_editor() {
    ...
    code_editor->get_text_edit()
        ->connect("breakpoint_toggled", this, "_breakpoint_toggled");
    code_editor->get_text_edit()
        ->connect("symbol_lookup", this, "_lookup_symbol");
    code_editor->get_text_edit()
        ->connect("info_clicked", this, "_lookup_connections");
    code_editor->get_text_edit()
        ->connect("gui_input", this, "_text_edit_gui_input");
    ...
    int default_color_mode =
        EDITOR_GET("interface/inspector/default_color_picker_mode");
    ...
    search_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/find"),
        SEARCH_FIND
    );
    search_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/find_next"),
        SEARCH_FIND_NEXT
    );
    search_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/find_previous"),
        SEARCH_FIND_PREV
    );
    search_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/replace"),
        SEARCH_REPLACE
    );
    search_menu->get_popup()->add_separator();
    search_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/find_in_files"),
        SEARCH_IN_FILES
    );
    search_menu->get_popup()->add_separator();
    search_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/contextual_help"),
        HELP_CONTEXTUAL
    );
    ...
    edit_menu->get_popup()->add_separator();
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/move_up"),
        EDIT_MOVE_LINE_UP
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/move_down"),
        EDIT_MOVE_LINE_DOWN
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/indent_left"),
    ...
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/fold_all_lines"),
        EDIT_FOLD_ALL_LINES
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/unfold_all_lines"),
        EDIT_UNFOLD_ALL_LINES
    );
    edit_menu->get_popup()->add_separator();
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/duplicate_selection"),
        EDIT_DUPLICATE_SELECTION
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/complete_symbol"),
        EDIT_COMPLETE
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/evaluate_selection"),
    ...
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/trim_trailing_whitespace"),
        EDIT_TRIM_TRAILING_WHITESAPCE
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/convert_indent_to_spaces"),
        EDIT_CONVERT_INDENT_TO_SPACES
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/convert_indent_to_tabs"),
        EDIT_CONVERT_INDENT_TO_TABS
    );
    edit_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/auto_indent"),
        EDIT_AUTO_INDENT
    );
    ...
    convert_case->add_shortcut(
        ED_SHORTCUT(
            "script_text_editor/convert_to_uppercase",
            TTR("Uppercase"),
            KEY_MASK_SHIFT | KEY_F4
        ),
        EDIT_TO_UPPERCASE
    );
    convert_case->add_shortcut(
    ...
    goto_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/goto_function"),
        SEARCH_LOCATE_FUNCTION
    );
    goto_menu->get_popup()->add_shortcut(
        ED_GET_SHORTCUT("script_text_editor/goto_line"),
        SEARCH_GOTO_LINE
    );
    ...
    breakpoints_menu
        ->connect("index_pressed", this, "_breakpoint_item_pressed");
...
ScriptTextEditor::ScriptTextEditor() {
    ...
    code_editor->get_text_edit()->set_callhint_settings(
        ...
        EditorSettings::get_singleton()->get(
            "text_editor/completion/callhint_tooltip_offset"
        )
    );
...
ScriptTextEditor::~ScriptTextEditor() {
    for (const Map<String, SyntaxHighlighter*>::Element* E =
             highlighters.front();
         E;
         E = E->next()) {
        if (E->get() != NULL) {
...
void ScriptTextEditor::register_editor() {
    ...
    // Leave these at zero, same can be accomplished with tab/shift-tab,
    // including selection. The next/previous in history shortcut in this case
    ...
    ED_SHORTCUT(
        ...
        TTR("Fold/Unfold Line"),
        KEY_MASK_ALT | KEY_F
    );
    ED_SHORTCUT("script_text_editor/fold_all_lines", TTR("Fold All Lines"), 0);
    ED_SHORTCUT(
        "script_text_editor/unfold_all_lines",
    ...
    ED_SHORTCUT(
        ...
        KEY_MASK_SHIFT | KEY_MASK_CMD | KEY_C
    );
    ED_SHORTCUT(
        "script_text_editor/complete_symbol",
        TTR("Complete Symbol"),
        KEY_MASK_CTRL | KEY_SPACE
    );
#else
    ED_SHORTCUT(
        "script_text_editor/duplicate_selection",
    ...
    ED_SHORTCUT(
        "script_text_editor/complete_symbol",
        TTR("Complete Symbol"),
        KEY_MASK_CMD | KEY_SPACE
    );
#endif
    ED_SHORTCUT(
        "script_text_editor/evaluate_selection",
        TTR("Evaluate Selection"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_E
    );
    ED_SHORTCUT(
        "script_text_editor/trim_trailing_whitespace",
        TTR("Trim Trailing Whitespace"),
        KEY_MASK_CMD | KEY_MASK_ALT | KEY_T
    );
    ED_SHORTCUT(
        "script_text_editor/convert_indent_to_spaces",
        TTR("Convert Indent to Spaces"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_Y
    );
    ED_SHORTCUT(
        "script_text_editor/convert_indent_to_tabs",
        TTR("Convert Indent to Tabs"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_I
    );
    ED_SHORTCUT(
    ...
    ED_SHORTCUT(
        ...
        TTR("Find Previous"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_G
    );
    ED_SHORTCUT(
    ...
    ED_SHORTCUT(
        ...
        TTR("Contextual Help"),
        KEY_MASK_ALT | KEY_MASK_SHIFT | KEY_SPACE
    );
#else
    ED_SHORTCUT(
        "script_text_editor/contextual_help",
    ...
    ED_SHORTCUT(
        ...
        TTR("Remove All Breakpoints"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_F9
    );
    ED_SHORTCUT(
        "script_text_editor/goto_next_breakpoint",
        TTR("Go to Next Breakpoint"),
        KEY_MASK_CMD | KEY_PERIOD
    );
    ED_SHORTCUT(
        "script_text_editor/goto_previous_breakpoint",
        TTR("Go to Previous Breakpoint"),
