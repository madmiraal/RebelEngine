...
// Min and Max are power of two in order to play nicely with successive
// increment. That way, we can naturally reach a 100% zoom from boundaries.
...
public:
    ...
    void set_fields(
        const Point2 p_grid_offset,
        const Point2 p_grid_step,
        const int p_primary_grid_steps,
        const float p_rotation_offset,
        const float p_rotation_step,
        const float p_scale_step
    ) {
    ...
    void get_fields(
        Point2& p_grid_offset,
        Point2& p_grid_step,
        int& p_primary_grid_steps,
        float& p_rotation_offset,
        float& p_rotation_step,
        float& p_scale_step
    ) {
        p_grid_offset =
            Point2(grid_offset_x->get_value(), grid_offset_y->get_value());
        p_grid_step =
            Point2(grid_step_x->get_value(), grid_step_y->get_value());
...
bool CanvasItemEditor::_is_node_movable(
    const Node* p_node,
    bool p_popup_warning
) {
    if (_is_node_locked(p_node)) {
        return false;
    }
    if (Object::cast_to<Control>(p_node)
        && Object::cast_to<Container>(p_node->get_parent())) {
        if (p_popup_warning) {
...
void CanvasItemEditor::_snap_if_closer_float(
    ...
    if ((p_radius < 0 || dist < radius)
        && (r_current_snap_target == SNAP_TARGET_NONE
            || dist < Math::abs(r_current_snap - p_value))) {
...
void CanvasItemEditor::_snap_if_closer_point(
    Point2 p_value,
    Point2& r_current_snap,
    SnapTarget (&r_current_snap_target)[2],
...
void CanvasItemEditor::_snap_other_nodes(
    ...
    Point2& r_current_snap,
    SnapTarget (&r_current_snap_target)[2],
    const SnapTarget p_snap_target,
    List<const CanvasItem*> p_exceptions,
    ...
    for (List<const CanvasItem*>::Element* E = p_exceptions.front(); E;
         E                                   = E->next()) {
        if (E->get() == p_current) {
            exception = true;
            break;
    ...
    if (canvas_item && !exception) {
        Transform2D ci_transform =
            canvas_item->get_global_transform_with_canvas();
        if (fmod(
                ci_transform.get_rotation()
                    - p_transform_to_snap.get_rotation(),
                (real_t)360.0
            )
            == 0.0) {
            if (canvas_item->_edit_use_rect()) {
                Point2 begin = ci_transform.xform(
                    canvas_item->_edit_get_rect().get_position()
                );
                Point2 end = ci_transform.xform(
                    canvas_item->_edit_get_rect().get_position()
                    + canvas_item->_edit_get_rect().get_size()
                );

                _snap_if_closer_point(
                    p_value,
                    r_current_snap,
                    r_current_snap_target,
                    begin,
                    p_snap_target,
                    ci_transform.get_rotation()
                );
    ...
    for (int i = 0; i < p_current->get_child_count(); i++) {
        _snap_other_nodes(
            ...
            p_snap_target,
            p_exceptions,
            p_current->get_child(i)
        );
    }
}

Point2 CanvasItemEditor::snap_point(
    Point2 p_target,
    unsigned int p_modes,
    unsigned int p_forced_modes,
    const CanvasItem* p_self_canvas_item,
    List<CanvasItem*> p_other_nodes_exceptions
) {
    snap_target[0] = SNAP_TARGET_NONE;
    snap_target[1] = SNAP_TARGET_NONE;

    bool is_snap_active =
        smart_snap_active ^ Input::get_singleton()->is_key_pressed(KEY_CONTROL);
    ...
    if (p_self_canvas_item) {
        rotation =
            p_self_canvas_item->get_global_transform_with_canvas().get_rotation(
            );
        ...
        if ((is_snap_active && snap_node_parent && (p_modes & SNAP_NODE_PARENT))
            ...
            if (const Control* c =
                    Object::cast_to<Control>(p_self_canvas_item)) {
                Point2 begin =
                    p_self_canvas_item->get_global_transform_with_canvas()
                        .xform(_anchor_to_position(c, Point2(0, 0)));
                Point2 end =
                    p_self_canvas_item->get_global_transform_with_canvas()
                        .xform(_anchor_to_position(c, Point2(1, 1)));
                _snap_if_closer_point(
                    p_target,
                    output,
                    snap_target,
                ...
                if (parent_ci->_edit_use_rect()) {
                    Point2 begin =
                        p_self_canvas_item->get_transform()
                            .affine_inverse()
                            .xform(parent_ci->_edit_get_rect().get_position());
                    Point2 end =
                        p_self_canvas_item->get_transform()
                            .affine_inverse()
                            .xform(
                                parent_ci->_edit_get_rect().get_position()
                                + parent_ci->_edit_get_rect().get_size()
                            );
                    _snap_if_closer_point(
                        p_target,
                        output,
                    ...
                    Point2 position = p_self_canvas_item->get_transform()
                                          .affine_inverse()
                                          .xform(Point2());
                    _snap_if_closer_point(
                        p_target,
        ...
        // Self anchors
        if ((is_snap_active && snap_node_anchors
             && (p_modes & SNAP_NODE_ANCHORS))
            || (p_forced_modes & SNAP_NODE_ANCHORS)) {
            if (const Control* c =
                    Object::cast_to<Control>(p_self_canvas_item)) {
                Point2 begin =
                    p_self_canvas_item->get_global_transform_with_canvas()
                        .xform(_anchor_to_position(
                            c,
                            Point2(
                                c->get_anchor(MARGIN_LEFT),
                                c->get_anchor(MARGIN_TOP)
                            )
                        ));
                Point2 end =
                    p_self_canvas_item->get_global_transform_with_canvas()
                        .xform(_anchor_to_position(
                            c,
                            Point2(
                                c->get_anchor(MARGIN_RIGHT),
                                c->get_anchor(MARGIN_BOTTOM)
                            )
                        ));
                _snap_if_closer_point(
                    p_target,
        ...
        // Self sides
        if ((is_snap_active && snap_node_sides && (p_modes & SNAP_NODE_SIDES))
            || (p_forced_modes & SNAP_NODE_SIDES)) {
    ...
    // Other nodes sides
    if ((is_snap_active && snap_other_nodes && (p_modes & SNAP_OTHER_NODES))
        || (p_forced_modes & SNAP_OTHER_NODES)) {
        Transform2D to_snap_transform      = Transform2D();
        List<const CanvasItem*> exceptions = List<const CanvasItem*>();
        for (List<CanvasItem*>::Element* E = p_other_nodes_exceptions.front();
             E;
             E = E->next()) {
    ...
    if (((is_snap_active && snap_guides && (p_modes & SNAP_GUIDES))
         || (p_forced_modes & SNAP_GUIDES))
        && fmod(rotation, (real_t)360.0) == 0.0) {
        // Guides
        if (EditorNode::get_singleton()->get_edited_scene()
            && EditorNode::get_singleton()->get_edited_scene()->has_meta(
                "_edit_vertical_guides_"
            )) {
            Array vguides =
                EditorNode::get_singleton()->get_edited_scene()->get_meta(
                    "_edit_vertical_guides_"
                );
    ...
    if (((grid_snap_active && (p_modes & SNAP_GRID))
         || (p_forced_modes & SNAP_GRID))
        && fmod(rotation, (real_t)360.0) == 0.0) {
        // Grid
        ...
        if (snap_relative) {
            ...
            if (selection.size() == 1
                && Object::cast_to<Node2D>(selection[0])) {
                offset = Object::cast_to<Node2D>(selection[0])
                             ->get_global_position();
        ...
        Point2 grid_output;
        grid_output.x = Math::stepify(
                            p_target.x - offset.x,
                            grid_step.x * Math::pow(2.0, grid_step_multiplier)
                        )
                      + offset.x;
        grid_output.y = Math::stepify(
                            p_target.y - offset.y,
                            grid_step.y * Math::pow(2.0, grid_step_multiplier)
                        )
                      + offset.y;
        _snap_if_closer_point(
            p_target,
    ...
    if (((snap_pixel && (p_modes & SNAP_PIXEL)) || (p_forced_modes & SNAP_PIXEL)
        )
        && rotation == 0.0) {
        // Pixel
...
float CanvasItemEditor::snap_angle(float p_target, float p_start) const {
    return (((smart_snap_active || snap_rotation)
             ^ Input::get_singleton()->is_key_pressed(KEY_CONTROL))
            && snap_rotation_step != 0)
             ? Math::stepify(
                   p_target - snap_rotation_offset,
                   snap_rotation_step
...
void CanvasItemEditor::_unhandled_key_input(const Ref<InputEvent>& p_ev) {
    ...
    if (k->get_scancode() == KEY_CONTROL || k->get_scancode() == KEY_ALT
        || k->get_scancode() == KEY_SHIFT) {
        viewport->update();
    }

    if (k->is_pressed() && !k->get_control() && !k->is_echo()) {
        if ((grid_snap_active || show_grid)
            && multiply_grid_step_shortcut.is_valid()
            && multiply_grid_step_shortcut->is_shortcut(p_ev)) {
...
Rect2 CanvasItemEditor::_get_encompassing_rect_from_list(
    List<CanvasItem*> p_list
) {
    ...
    Rect2 rect              = Rect2(
        canvas_item->get_global_transform_with_canvas().xform(
            canvas_item->_edit_get_rect().position
            + canvas_item->_edit_get_rect().size / 2
        ),
        Size2()
    );
    ...
    for (List<CanvasItem*>::Element* E = p_list.front(); E; E = E->next()) {
        ...
        rect.expand_to(
            xform.xform(current_rect.position + Vector2(current_rect.size.x, 0))
        );
        rect.expand_to(xform.xform(current_rect.position + current_rect.size));
        rect.expand_to(
            xform.xform(current_rect.position + Vector2(0, current_rect.size.y))
        );
    }
...
void CanvasItemEditor::_expand_encompassing_rect_using_children(
    ...
    const Node* p_node,
    bool& r_first,
    const Transform2D& p_parent_xform,
    const Transform2D& p_canvas_xform,
    bool include_locked_nodes
) {
    if (!p_node) {
        return;
    }
    ...
    for (int i = p_node->get_child_count() - 1; i >= 0; i--) {
        if (canvas_item && !canvas_item->is_set_as_toplevel()) {
            _expand_encompassing_rect_using_children(
                ...
                p_node->get_child(i),
                r_first,
                p_parent_xform * canvas_item->get_transform(),
                p_canvas_xform
            );
        } else {
            const CanvasLayer* canvas_layer =
                Object::cast_to<CanvasLayer>(p_node);
            _expand_encompassing_rect_using_children(
                r_rect,
                p_node->get_child(i),
                r_first,
    ...
    if (canvas_item && canvas_item->is_visible_in_tree()
        && (include_locked_nodes || !_is_node_locked(canvas_item))) {
        Transform2D xform =
            p_parent_xform * p_canvas_xform * canvas_item->get_transform();
...
void CanvasItemEditor::_find_canvas_items_at_pos(
    const Point2& p_pos,
    Node* p_node,
    Vector<_SelectResult>& r_items,
    const Transform2D& p_parent_xform,
    const Transform2D& p_canvas_xform
) {
    if (!p_node) {
    ...
    for (int i = p_node->get_child_count() - 1; i >= 0; i--) {
        if (canvas_item) {
            ...
            _find_canvas_items_at_pos(
                ...
                r_items,
                Transform2D(),
                cl ? cl->get_transform() : p_canvas_xform
            );
        }
    }
    ...
    if (canvas_item && canvas_item->is_visible_in_tree()) {
        Transform2D xform =
            (p_parent_xform * p_canvas_xform * canvas_item->get_transform())
                .affine_inverse();
        const real_t local_grab_distance =
            xform.basis_xform(Vector2(grab_distance, 0)).length() / zoom;
        if (canvas_item->_edit_is_selected_on_click(
                xform.xform(p_pos),
                local_grab_distance
            )) {
...
void CanvasItemEditor::_get_canvas_items_at_pos(
    ...
    for (int i = 0; i < r_items.size(); i++) {
        ...
        // Check if the canvas item is already in the list (for groups or
        // scenes)
...
void CanvasItemEditor::_get_bones_at_pos(
    const Point2& p_pos,
    Vector<_SelectResult>& r_items
) {
    Point2 screen_pos = transform.xform(p_pos);

    for (Map<BoneKey, BoneList>::Element* E = bone_list.front(); E;
         E                                  = E->next()) {
        Node2D* from_node =
            Object::cast_to<Node2D>(ObjectDB::get_instance(E->key().from));
...
bool CanvasItemEditor::_get_bone_shape(
    ...
    Vector<Vector2>* outline_shape,
    Map<BoneKey, BoneList>::Element* bone
) {
    int bone_width =
        EditorSettings::get_singleton()->get("editors/2d/bone_width");
    int bone_outline_width =
        EditorSettings::get_singleton()->get("editors/2d/bone_outline_size");

    Node2D* from_node =
        Object::cast_to<Node2D>(ObjectDB::get_instance(bone->key().from));
    Node2D* to_node =
        Object::cast_to<Node2D>(ObjectDB::get_instance(bone->key().to));
    ...
    if (to_node) {
        ...
        to = transform.xform(from_node->get_global_transform().xform(
            Vector2(bone->get().length, 0)
        ));
    }
    ...
    if (outline_shape) {
        ...
        outline_shape->push_back(from + (-reln + reltn) * bone_outline_width);
        outline_shape->push_back(
            from + rel * 0.2f + relt + reltn * bone_outline_width
        );
...
void CanvasItemEditor::_find_canvas_items_in_rect(
    const Rect2& p_rect,
    Node* p_node,
    List<CanvasItem*>* r_items,
    const Transform2D& p_parent_xform,
    const Transform2D& p_canvas_xform
) {
    if (!p_node) {
    ...
    bool editable = p_node == scene || p_node->get_owner() == scene
                 || p_node == scene->get_deepest_editable_node(p_node);
    bool lock_children =
        ...
        for (int i = p_node->get_child_count() - 1; i >= 0; i--) {
            if (canvas_item) {
                ...
                _find_canvas_items_in_rect(
                    ...
                    r_items,
                    Transform2D(),
                    canvas_layer ? canvas_layer->get_transform()
                                 : p_canvas_xform
                );
            }
        }
    ...
    if (canvas_item && canvas_item->is_visible_in_tree() && !locked
        && editable) {
        Transform2D xform =
            p_parent_xform * p_canvas_xform * canvas_item->get_transform();
...
bool CanvasItemEditor::_select_click_on_item(
    CanvasItem* item,
    Point2 p_click_pos,
    bool p_append
) {
...
List<CanvasItem*> CanvasItemEditor::_get_edited_canvas_items(
    bool retreive_locked,
    bool remove_canvas_item_if_parent_in_selection
) {
    List<CanvasItem*> selection;
    for (Map<Node*, Object*>::Element* E =
             editor_selection->get_selection().front();
         E;
         E = E->next()) {
        CanvasItem* canvas_item = Object::cast_to<CanvasItem>(E->key());
        if (canvas_item && canvas_item->is_visible_in_tree()
            && canvas_item->get_viewport()
                   == EditorNode::get_singleton()->get_scene_root()
            && (retreive_locked || !_is_node_locked(canvas_item))) {
            CanvasItemEditorSelectedItem* se =
                editor_selection
                    ->get_node_editor_data<CanvasItemEditorSelectedItem>(
                        canvas_item
                    );
            if (se) {
...
Vector2 CanvasItemEditor::_anchor_to_position(
    ...
    return parent_transform.xform(
        ...
        + Vector2(parent_rect.size.x * anchor.x, parent_rect.size.y * anchor.y)
    );
}

Vector2 CanvasItemEditor::_position_to_anchor(
    const Control* p_control,
    Vector2 position
) {
    ERR_FAIL_COND_V(!p_control, Vector2());
    ...
    output.x       = (parent_rect.size.x == 0)
                       ? 0.0
                       : (p_control->get_transform().xform(position).x
                    - parent_rect.position.x)
                       / parent_rect.size.x;
    output.y       = (parent_rect.size.y == 0)
                       ? 0.0
                       : (p_control->get_transform().xform(position).y
...
void CanvasItemEditor::_save_canvas_item_ik_chain(
    ...
    if (bone && bone->has_meta("_edit_bone_")) {
        ...
        if (ik_found) {
            ...
            for (List<const Node2D*>::Element* bone_E = bone_ik_list.front();
                 bone_E;
                 bone_E = bone_E->next()) {
                bone_xform =
                    bone_xform * bone->get_transform().affine_inverse();
                const Node2D* parent_bone = bone_E->get();
                if (p_bones_length) {
                    p_bones_length->push_back(
                        parent_bone->get_global_transform()
                            .get_origin()
                            .distance_to(bone->get_global_position())
                    );
                }
                if (p_bones_state) {
...
void CanvasItemEditor::_save_canvas_item_state(
    List<CanvasItem*> p_canvas_items,
    bool save_bones
) {
    for (List<CanvasItem*>::Element* E = p_canvas_items.front(); E;
         E                             = E->next()) {
        CanvasItem* canvas_item = E->get();
        CanvasItemEditorSelectedItem* se =
            editor_selection
                ->get_node_editor_data<CanvasItemEditorSelectedItem>(canvas_item
                );
        if (se) {
            ...
            _save_canvas_item_ik_chain(
                canvas_item,
                &(se->pre_drag_bones_length),
                &(se->pre_drag_bones_undo_state)
            );
        }
    }
}
...
void CanvasItemEditor::_restore_canvas_item_ik_chain(
    CanvasItem* p_canvas_item,
    const List<Dictionary>* p_bones_state
) {
    CanvasItem* canvas_item = p_canvas_item;
    for (const List<Dictionary>::Element* E = p_bones_state->front(); E;
         E                                  = E->next()) {
...
void CanvasItemEditor::_restore_canvas_item_state(
    List<CanvasItem*> p_canvas_items,
    bool restore_bones
) {
    for (List<CanvasItem*>::Element* E = drag_selection.front(); E;
         E                             = E->next()) {
        CanvasItem* canvas_item = E->get();
        CanvasItemEditorSelectedItem* se =
            editor_selection
...
void CanvasItemEditor::_commit_canvas_item_state(
    ...
    for (List<CanvasItem*>::Element* E = modified_canvas_items.front(); E;
         E                             = E->next()) {
        CanvasItem* canvas_item = E->get();
        CanvasItemEditorSelectedItem* se =
            editor_selection
        ...
        if (se) {
            ...
            if (commit_bones) {
                for (List<Dictionary>::Element* F =
                         se->pre_drag_bones_undo_state.front();
                     F;
                     F = F->next()) {
                    canvas_item =
                        Object::cast_to<CanvasItem>(canvas_item->get_parent());
                    undo_redo->add_do_method(
                        canvas_item,
                        "_edit_set_state",
                        canvas_item->_edit_get_state()
...
void CanvasItemEditor::_selection_result_pressed(int p_result) {
    ...
    if (item) {
        _select_click_on_item(
            item,
            Point2(),
            selection_menu_additive_selection
        );
    }
}
...
void CanvasItemEditor::_node_created(Node* p_node) {
    ...
    if (c) {
        Transform2D xform =
            c->get_global_transform_with_canvas().affine_inverse()
            * c->get_transform();
        c->_edit_set_position(xform.xform(node_create_position));
    }

    call_deferred("_reset_create_position"
    ); // Defer the call in case more than one node is added.
}
...
bool CanvasItemEditor::_gui_input_rulers_and_guides(
    ...
    if (drag_type == DRAG_NONE) {
        if (show_guides && show_rulers
            ...
            Transform2D xform =
                viewport_scrollable->get_transform() * transform;
            // Retrieve the guide lists
            Array vguides;
            if (EditorNode::get_singleton()->get_edited_scene()->has_meta(
                    "_edit_vertical_guides_"
                )) {
                vguides =
                    EditorNode::get_singleton()->get_edited_scene()->get_meta(
            ...
            if (m.is_valid() && m->get_position().x < RULER_WIDTH) {
                ...
                for (int i = 0; i < hguides.size(); i++) {
                    if (ABS(xform.xform(Point2(0, hguides[i])).y
                            - m->get_position().y)
                        < MIN(minimum, 8)) {
                ...
                for (int i = 0; i < vguides.size(); i++) {
                    if (ABS(xform.xform(Point2(vguides[i], 0)).x
                            - m->get_position().x)
                        < MIN(minimum, 8)) {
    ...
    if (drag_type == DRAG_DOUBLE_GUIDE || drag_type == DRAG_V_GUIDE
        ...
        if (m.is_valid()) {
            ...
            dragged_guide_pos = xform.xform(
                snap_point(drag_to, SNAP_GRID | SNAP_PIXEL | SNAP_OTHER_NODES)
            );
            viewport->update();
            return true;
        }
            ...
            if (show_guides
                ...
                if (EditorNode::get_singleton()->get_edited_scene()->has_meta(
                    ...
                    vguides = EditorNode::get_singleton()
                                  ->get_edited_scene()
                                  ->get_meta("_edit_vertical_guides_");
                }
                Array hguides;
                if (EditorNode::get_singleton()->get_edited_scene()->has_meta(
                        "_edit_horizontal_guides_"
                    )) {
                    hguides = EditorNode::get_singleton()
                                  ->get_edited_scene()
                                  ->get_meta("_edit_horizontal_guides_");
                }

                Point2 edited = snap_point(
                    xform.affine_inverse().xform(b->get_position()),
                ...
                if (drag_type == DRAG_V_GUIDE) {
                    ...
                    if (b->get_position().x > RULER_WIDTH
                        ...
                        hguides.push_back(edited.y);
                        undo_redo->create_action(
                            TTR("Create Horizontal and Vertical Guides")
                        );
                        undo_redo->add_do_method(
                            EditorNode::get_singleton()->get_edited_scene(),
                            "set_meta",
...
bool CanvasItemEditor::_gui_input_zoom_or_pan(
    const Ref<InputEvent>& p_event,
    bool p_already_accepted
) {
    Ref<InputEventMouseButton> b = p_event;
    if (b.is_valid() && !p_already_accepted) {
        const bool pan_on_scroll =
            bool(EditorSettings::get_singleton()->get("editors/2d/scroll_to_pan"
            ))
            && !b->get_control();

        if (pan_on_scroll) {
            // Perform horizontal scrolling first so we can check for Shift
            // being held.
            if (b->is_pressed()
                && (b->get_button_index() == BUTTON_WHEEL_LEFT
                    || (b->get_shift()
                        && b->get_button_index() == BUTTON_WHEEL_UP))) {
                // Pan left
                view_offset.x -= int(EditorSettings::get_singleton()->get(
            ...
            if (b->is_pressed()
                && (b->get_button_index() == BUTTON_WHEEL_RIGHT
                    || (b->get_shift()
        ...
        if (b->is_pressed() && b->get_button_index() == BUTTON_WHEEL_DOWN) {
            // Scroll or pan down
            if (pan_on_scroll) {
                view_offset.y += int(EditorSettings::get_singleton()->get(
                                     "editors/2d/pan_speed"
                                 ))
                               / zoom * b->get_factor();
                update_viewport();
                ...
                if (!Math::is_equal_approx(b->get_factor(), 1.0f)) {
                    ...
                    new_zoom =
                        zoom
                        * ((new_zoom / zoom - 1.f) * b->get_factor() + 1.f);
                }
        ...
        if (panning) {
            if (!b->is_pressed()
                && (pan_on_scroll
                    || (b->get_button_index() != BUTTON_WHEEL_DOWN
                        && b->get_button_index() != BUTTON_WHEEL_UP))) {
                // Stop panning the viewport (for any mouse button press except
                // zooming)
                panning = false;
            }
        }
    ...
    if (k.is_valid()) {
        if (k->is_pressed()) {
            if (ED_GET_SHORTCUT("canvas_item_editor/zoom_3.125_percent")
                    ->is_shortcut(p_event)) {
                _shortcut_zoom_set(1.0 / 32.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_6.25_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(1.0 / 16.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_12.5_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(1.0 / 8.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_25_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(1.0 / 4.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_50_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(1.0 / 2.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_100_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(1.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_200_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(2.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_400_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(4.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_800_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(8.0);
            } else if (ED_GET_SHORTCUT("canvas_item_editor/zoom_1600_percent")
                           ->is_shortcut(p_event)) {
                _shortcut_zoom_set(16.0);
            }
        }

        bool is_pan_key = pan_view_shortcut.is_valid()
                       && pan_view_shortcut->is_shortcut(p_event);

        if (is_pan_key
            && (EditorSettings::get_singleton()->get("editors/2d/simple_panning"
                )
                || drag_type != DRAG_NONE)) {
            if (!panning) {
    ...
    if (m.is_valid()) {
        if (panning) {
            ...
            if (bool(EditorSettings::get_singleton()->get(
                    "editors/2d/warped_mouse_panning"
                ))) {
    ...
    if (magnify_gesture.is_valid() && !p_already_accepted) {
        // Zoom gesture
        _zoom_on_position(
            zoom * magnify_gesture->get_factor(),
            magnify_gesture->get_position()
        );
        return true;
    }
    ...
    if (pan_gesture.is_valid() && !p_already_accepted) {
        ...
        // Pan gesture
        const Vector2 delta =
            (int(EditorSettings::get_singleton()->get("editors/2d/pan_speed"))
             / zoom)
...
bool CanvasItemEditor::_gui_input_pivot(const Ref<InputEvent>& p_event) {
    ...
    if (drag_type == DRAG_NONE) {
        if ((b.is_valid() && b->is_pressed()
             && b->get_button_index() == BUTTON_LEFT && tool == TOOL_EDIT_PIVOT)
            || (k.is_valid() && k->is_pressed() && !k->is_echo()
                && k->get_scancode() == KEY_V)) {
            List<CanvasItem*> selection = _get_edited_canvas_items();
            ...
            if (drag_selection.size() > 0) {
                ...
                if (drag_selection.size() == 1) {
                    new_pos = snap_point(
                        drag_from,
                        SNAP_NODE_SIDES | SNAP_NODE_CENTER | SNAP_NODE_ANCHORS
                            | SNAP_OTHER_NODES | SNAP_GRID | SNAP_PIXEL,
                        0,
                        drag_selection[0]
                    );
                } else {
    ...
    if (drag_type == DRAG_PIVOT) {
        ...
        if (drag_selection.size() >= 1
            && ((b.is_valid() && !b->is_pressed()
                 && b->get_button_index() == BUTTON_LEFT
                 && tool == TOOL_EDIT_PIVOT)
                || (k.is_valid() && !k->is_pressed()
                    && k->get_scancode() == KEY_V))) {
        ...
        // Cancel a drag
        if (b.is_valid() && b->get_button_index() == BUTTON_RIGHT
            && b->is_pressed()) {
            _restore_canvas_item_state(drag_selection);
            drag_type = DRAG_NONE;
            viewport->update();
...
void CanvasItemEditor::_solve_IK(Node2D* leaf_node, Point2 target_position) {
    CanvasItemEditorSelectedItem* se =
        editor_selection->get_node_editor_data<CanvasItemEditorSelectedItem>(
            leaf_node
        );
    if (se) {
        ...
        if (nb_bones > 0) {
            ...
            Transform2D joint_transform =
                leaf_node->get_global_transform_with_canvas();
            for (int i = 0; i < nb_bones + 1; i++) {
                joints_list.push_back(joint);
                joints_pos.push_back(joint_transform.get_origin());
                joint_transform =
                    joint_transform * joint->get_transform().affine_inverse();
                joint = Object::cast_to<Node2D>(joint->get_parent());
            }
            Point2 root_pos = joints_list.back()
                                  ->get()
            ...
            float total_len = 0;
            for (List<float>::Element* E = se->pre_drag_bones_length.front(); E;
                 E                       = E->next()) {
                total_len += E->get();
            }
            ...
            for (int i = 0; i < solver_iterations; i++) {
                ...
                int node_id = 0;
                for (List<float>::Element* E =
                         se->pre_drag_bones_length.front();
                     E;
                     E = E->next()) {
                    Vector2 direction =
                        (joints_pos[node_id + 1] - joints_pos[node_id])
                            .normalized();
                    int len = E->get();
                    if (E == se->pre_drag_bones_length.front()) {
                        joints_pos[1] = joints_pos[1].linear_interpolate(
                            joints_pos[0] + len * direction,
                            solver_k
                        );
                    } else if (E == se->pre_drag_bones_length.back()) {
                        joints_pos[node_id] =
                            joints_pos[node_id].linear_interpolate(
                                joints_pos[node_id + 1] - len * direction,
                                solver_k
                            );
                    } else {
                        Vector2 center =
                        ...
                        joints_pos[node_id] =
                            joints_pos[node_id].linear_interpolate(
                                center - (direction * len) / 2.0,
                                solver_k
                            );
                        joints_pos[node_id + 1] =
                            joints_pos[node_id + 1].linear_interpolate(
                                center + (direction * len) / 2.0,
                                solver_k
                            );
                    }
            ...
            for (int node_id = joints_list.size() - 1; node_id > 0; node_id--) {
                Point2 current =
                    (joints_list[node_id - 1]->get_global_position()
                     - joints_list[node_id]->get_global_position())
                        .normalized();
                Point2 target = (joints_pos[node_id - 1]
                                 - joints_list[node_id]->get_global_position())
                                    .normalized();
                float rot = current.angle_to(target);
                if (joints_list[node_id]
...
bool CanvasItemEditor::_gui_input_rotate(const Ref<InputEvent>& p_event) {
    ...
    if (drag_type == DRAG_NONE) {
        if (b.is_valid() && b->get_button_index() == BUTTON_LEFT
            && b->is_pressed()) {
            if ((b->get_command() && !b->get_alt() && tool == TOOL_SELECT)
                || tool == TOOL_ROTATE) {
                List<CanvasItem*> selection = _get_edited_canvas_items();
                ...
                if (drag_selection.size() > 0) {
                    ...
                    drag_from =
                        transform.affine_inverse().xform(b->get_position());
                    CanvasItem* canvas_item = drag_selection[0];
                    if (canvas_item->_edit_use_pivot()) {
                        drag_rotation_center =
                            canvas_item->get_global_transform_with_canvas()
                                .xform(canvas_item->_edit_get_pivot());
                    } else {
                        drag_rotation_center =
                            canvas_item->get_global_transform_with_canvas()
                                .get_origin();
    ...
    if (drag_type == DRAG_ROTATE) {
        ...
        if (m.is_valid()) {
            ...
            for (List<CanvasItem*>::Element* E = drag_selection.front(); E;
                ...
                CanvasItem* canvas_item = E->get();
                drag_to = transform.affine_inverse().xform(m->get_position());
                // Rotate the opposite way if the canvas item's compounded scale
                // has an uneven number of negative elements
                bool opposite =
                    (canvas_item->get_global_transform().get_scale().sign().dot(
                         canvas_item->get_transform().get_scale().sign()
                     )
                     == 0);
                canvas_item->_edit_set_rotation(snap_angle(
                    canvas_item->_edit_get_rotation()
                        + (opposite ? -1 : 1)
                              * (drag_from - drag_rotation_center)
        ...
        if (b.is_valid() && b->get_button_index() == BUTTON_LEFT
            ...
            if (drag_selection.size() != 1) {
                ...
                _commit_canvas_item_state(
                    drag_selection,
                    vformat(
                        TTR("Rotate CanvasItem \"%s\" to %d degrees"),
                        drag_selection[0]->get_name(),
...
bool CanvasItemEditor::_gui_input_open_scene_on_double_click(
    const Ref<InputEvent>& p_event
) {
    Ref<InputEventMouseButton> b = p_event;
    ...
    if (b.is_valid() && b->get_button_index() == BUTTON_LEFT && b->is_pressed()
        && b->is_doubleclick() && tool == TOOL_SELECT) {
        List<CanvasItem*> selection = _get_edited_canvas_items();
        if (selection.size() == 1) {
            CanvasItem* canvas_item = selection[0];
            if (canvas_item->get_filename() != ""
                && canvas_item != editor->get_edited_scene()) {
...
bool CanvasItemEditor::_gui_input_anchors(const Ref<InputEvent>& p_event) {
    ...
    if (drag_type == DRAG_NONE) {
        if (b.is_valid() && b->get_button_index() == BUTTON_LEFT
            && b->is_pressed() && tool == TOOL_SELECT) {
            List<CanvasItem*> selection = _get_edited_canvas_items();
            if (selection.size() == 1) {
                Control* control = Object::cast_to<Control>(selection[0]);
                if (control && _is_node_movable(control)) {
                    Vector2 anchor_pos[4];
                    anchor_pos[0] = Vector2(
                        control->get_anchor(MARGIN_LEFT),
                        control->get_anchor(MARGIN_TOP)
                    );
                    anchor_pos[1] = Vector2(
                        control->get_anchor(MARGIN_RIGHT),
                        control->get_anchor(MARGIN_TOP)
                    );
                    anchor_pos[2] = Vector2(
                        control->get_anchor(MARGIN_RIGHT),
                        control->get_anchor(MARGIN_BOTTOM)
                    );
                    anchor_pos[3] = Vector2(
                        control->get_anchor(MARGIN_LEFT),
                        control->get_anchor(MARGIN_BOTTOM)
                    );
                    ...
                    for (int i = 0; i < 4; i++) {
                        anchor_pos[i] =
                             ...
                             * control->get_global_transform_with_canvas())
                                .xform(
                                    _anchor_to_position(control, anchor_pos[i])
                                );
                        anchor_rects[i] =
                            Rect2(anchor_pos[i], anchor_handle->get_size());
                        anchor_rects[i].position -=
                            anchor_handle->get_size()
                            * Vector2(float(i == 0 || i == 3), float(i <= 1));
                    }
                        ...
                        if (anchor_rects[i].has_point(b->get_position())) {
                            if ((anchor_pos[0] == anchor_pos[2])
                                && (anchor_pos[0].distance_to(b->get_position())
                                    < anchor_handle->get_size().length() / 3.0
                                )) {
    ...
    if (drag_type == DRAG_ANCHOR_TOP_LEFT || drag_type == DRAG_ANCHOR_TOP_RIGHT
        || drag_type == DRAG_ANCHOR_BOTTOM_RIGHT
        || drag_type == DRAG_ANCHOR_BOTTOM_LEFT
        || drag_type == DRAG_ANCHOR_ALL) {
        // Drag the anchor
        if (m.is_valid()) {
            _restore_canvas_item_state(drag_selection);
            ...
            previous_anchor.x = (drag_type == DRAG_ANCHOR_TOP_LEFT
                                 || drag_type == DRAG_ANCHOR_BOTTOM_LEFT)
                                  ? control->get_anchor(MARGIN_LEFT)
                                  : control->get_anchor(MARGIN_RIGHT);
            previous_anchor.y = (drag_type == DRAG_ANCHOR_TOP_LEFT
                                 || drag_type == DRAG_ANCHOR_TOP_RIGHT)
                                  ? control->get_anchor(MARGIN_TOP)
                                  : control->get_anchor(MARGIN_BOTTOM);
            previous_anchor   = xform.affine_inverse().xform(
                _anchor_to_position(control, previous_anchor)
            );

            Vector2 new_anchor = xform.xform(snap_point(
                previous_anchor + (drag_to - drag_from),
                SNAP_GRID | SNAP_OTHER_NODES,
                SNAP_NODE_PARENT | SNAP_NODE_SIDES | SNAP_NODE_CENTER,
                control
            ));
            new_anchor         = _position_to_anchor(control, new_anchor)
                             .snapped(Vector2(0.001f, 0.001f));
            ...
            switch (drag_type) {
                ...
                case DRAG_ANCHOR_ALL:
                    ...
                    if (!use_single_axis || use_y) {
                        control
                            ->set_anchor(MARGIN_TOP, new_anchor.y, false, true);
                        control->set_anchor(
                            MARGIN_BOTTOM,
                            new_anchor.y,
                            false,
        ...
        if (drag_selection.size() >= 1 && b.is_valid()
            && b->get_button_index() == BUTTON_LEFT && !b->is_pressed()) {
            _commit_canvas_item_state(
                drag_selection,
                vformat(
                    TTR("Move CanvasItem \"%s\" Anchor"),
                    drag_selection[0]->get_name()
                )
            );
            drag_type = DRAG_NONE;
...
bool CanvasItemEditor::_gui_input_resize(const Ref<InputEvent>& p_event) {
            ...
            if (selection.size() == 1) {
                ...
                if (canvas_item->_edit_use_rect()
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        ofs += (endpoints[next] - endpoints[i])
                                   .tangent()
                                   .normalized()
    ...
    if (drag_type == DRAG_LEFT || drag_type == DRAG_RIGHT
        || drag_type == DRAG_TOP || drag_type == DRAG_BOTTOM
        ...
        if (m.is_valid()) {
            ...
            Point2 current_end =
                local_rect.get_position() + local_rect.get_size();
            Point2 max_begin =
                (symmetric)
                    ? (current_begin + current_end
                       - canvas_item->_edit_get_minimum_size())
                          / 2.0
            ...
            Transform2D xform =
                canvas_item->get_global_transform_with_canvas().affine_inverse(
                );
            ...
            if (drag_type == DRAG_LEFT || drag_type == DRAG_TOP
                || drag_type == DRAG_TOP_LEFT) {
                drag_to_snapped_end = snap_point(
                    xform.affine_inverse().xform(current_end)
                        + (drag_to - drag_from),
                    SNAP_NODE_ANCHORS | SNAP_NODE_PARENT | SNAP_OTHER_NODES
                        | SNAP_GRID | SNAP_PIXEL,
                    0,
                    canvas_item
                );
                drag_to_snapped_begin = snap_point(
                    xform.affine_inverse().xform(current_begin)
                        + (drag_to - drag_from),
                    SNAP_NODE_ANCHORS | SNAP_NODE_PARENT | SNAP_OTHER_NODES
                        | SNAP_GRID | SNAP_PIXEL,
                    0,
            ...
            if (uniform) {
                if (drag_type == DRAG_LEFT || drag_type == DRAG_RIGHT) {
                    current_end.y = current_begin.y
                                  + aspect * (current_end.x - current_begin.x);
                } else if (drag_type == DRAG_TOP || drag_type == DRAG_BOTTOM) {
                    current_end.x = current_begin.x
                                  + (current_end.y - current_begin.y) / aspect;
                } else {
                    if (aspect >= 1.0) {
            ...
            canvas_item->_edit_set_rect(
                Rect2(current_begin, current_end - current_begin)
            );
            return true;
        }
        ...
        if (drag_selection.size() >= 1 && b.is_valid()
            ...
            if (node2d) {
                // Extends from Node2D.
                // Node2D doesn't have an actual stored rect size, unlike
                // Controls.
                _commit_canvas_item_state(
                    drag_selection,
                    vformat(
                        ...
                        drag_selection[0]->get_name(),
                        Math::stepify(
                            drag_selection[0]->_edit_get_scale().x,
                            0.01
                        ),
                        Math::stepify(
                            drag_selection[0]->_edit_get_scale().y,
                            0.01
                        )
                    ),
                    true
...
bool CanvasItemEditor::_gui_input_scale(const Ref<InputEvent>& p_event) {
            ...
            if (selection.size() == 1) {
                ...
                if (_is_node_movable(canvas_item)) {
                    ...
                    Transform2D unscaled_transform =
                        (xform * canvas_item->get_transform().affine_inverse()
                         * canvas_item->_edit_get_transform())
                            .orthonormalized();
                    Transform2D simple_xform =
                        viewport->get_transform() * unscaled_transform;

                    drag_type = DRAG_SCALE_BOTH;

                    Size2 scale_factor =
                        Size2(SCALE_HANDLE_DISTANCE, SCALE_HANDLE_DISTANCE);
                    Rect2 x_handle_rect = Rect2(
                        scale_factor.x * EDSCALE,
                        -5 * EDSCALE,
                        10 * EDSCALE,
                        10 * EDSCALE
                    );
                    if (x_handle_rect.has_point(
                            simple_xform.affine_inverse().xform(b->get_position(
                            ))
                        )) {
                        drag_type = DRAG_SCALE_X;
                    }
                    Rect2 y_handle_rect = Rect2(
                        -5 * EDSCALE,
                        -(scale_factor.y + 10) * EDSCALE,
                        10 * EDSCALE,
                        10 * EDSCALE
                    );
                    if (y_handle_rect.has_point(
                            simple_xform.affine_inverse().xform(b->get_position(
                            ))
                        )) {
    ...
    if (drag_type == DRAG_SCALE_BOTH || drag_type == DRAG_SCALE_X
        || drag_type == DRAG_SCALE_Y) {
        // Resize the node
        if (m.is_valid()) {
            _restore_canvas_item_state(drag_selection);
            ...
            Transform2D parent_xform =
                canvas_item->get_global_transform_with_canvas()
                * canvas_item->get_transform().affine_inverse();
            Transform2D unscaled_transform =
                (transform * parent_xform * canvas_item->_edit_get_transform())
                    .orthonormalized();
            Transform2D simple_xform =
                (viewport->get_transform() * unscaled_transform)
                    .affine_inverse()
            ...
            if (drag_type == DRAG_SCALE_BOTH) {
                ...
                Size2 scale_factor =
                    Vector2(offset.x, -offset.y) / SCALE_HANDLE_DISTANCE;
                Size2 parent_scale = parent_xform.get_scale();
                scale_factor *=
                    Vector2(1.0 / parent_scale.x, 1.0 / parent_scale.y);
...
bool CanvasItemEditor::_gui_input_move(const Ref<InputEvent>& p_event) {
    ...
    if (drag_type == DRAG_MOVE) {
        ...
        if (m.is_valid()) {
            ...
            if (!drag_selection.empty()) {
                if (drag_selection.size() == 1) {
                    Transform2D xform =
                        drag_selection[0]->get_global_transform_with_canvas()
                        * drag_selection[0]->get_transform().affine_inverse();
                    previous_pos =
                        xform.xform(drag_selection[0]->_edit_get_position());
                } else {
                    previous_pos =
                        _get_encompassing_rect_from_list(drag_selection)
                            .position;
                }
            }
            Point2 new_pos = snap_point(
                previous_pos + (drag_to - drag_from),
                SNAP_GRID | SNAP_GUIDES | SNAP_PIXEL | SNAP_NODE_PARENT
                    | SNAP_NODE_ANCHORS | SNAP_OTHER_NODES,
                0,
                nullptr,
                drag_selection
            );
            ...
            if (single_axis) {
                if (ABS(new_pos.x - previous_pos.x)
                    > ABS(new_pos.y - previous_pos.y)) {
            ...
            for (List<CanvasItem*>::Element* E = drag_selection.front(); E;
                ...
                if (se) {
                    ...
                    if (node2d && se->pre_drag_bones_undo_state.size() > 0
                        && !force_no_IK) {
                        real_t initial_leaf_node_rotation =
                            node2d->get_global_transform_with_canvas()
                                .get_rotation();
                        _restore_canvas_item_ik_chain(
                            node2d,
                            &(all_bones_ik_states[index])
                        );
                        real_t final_leaf_node_rotation =
                            node2d->get_global_transform_with_canvas()
                                .get_rotation();
                        node2d->rotate(
                            initial_leaf_node_rotation
                            - final_leaf_node_rotation
                        );
                        _solve_IK(node2d, new_pos);
                    } else {
                        canvas_item->_edit_set_position(
                            canvas_item->_edit_get_position()
                            + xform.xform(new_pos) - xform.xform(previous_pos)
                        );
                    }
                }
                index++;
        ...
        if (b.is_valid() && !b->is_pressed()
            && b->get_button_index() == BUTTON_LEFT) {
            if (transform.affine_inverse().xform(b->get_position())
                != drag_from) {
                if (drag_selection.size() != 1) {
                    _commit_canvas_item_state(
                        drag_selection,
                        vformat(
                            TTR("Move %d CanvasItems"),
                            drag_selection.size()
                        ),
                        true
                    );
    ...
    if (k.is_valid() && k->is_pressed()
        && (tool == TOOL_SELECT || tool == TOOL_MOVE)
        && (k->get_scancode() == KEY_UP || k->get_scancode() == KEY_DOWN
            || k->get_scancode() == KEY_LEFT
            || k->get_scancode() == KEY_RIGHT)) {
        if (!k->is_echo()) {
        ...
        if (drag_selection.size() > 0) {
            ...
            drag_to += dir;
            if (k->get_shift()) {
                drag_to = drag_to.snapped(
                    grid_step * Math::pow(2.0, grid_step_multiplier)
                );
            }
            ...
            if (drag_selection.size() == 1) {
                ...
                previous_pos =
                    _get_encompassing_rect_from_list(drag_selection).position;
            }
    ...
    if (k.is_valid() && !k->is_pressed() && drag_type == DRAG_KEY_MOVE
        && (tool == TOOL_SELECT || tool == TOOL_MOVE)
        && (k->get_scancode() == KEY_UP || k->get_scancode() == KEY_DOWN
            || k->get_scancode() == KEY_LEFT
            || k->get_scancode() == KEY_RIGHT)) {
    ...
    return (
        k.is_valid()
        && (k->get_scancode() == KEY_UP || k->get_scancode() == KEY_DOWN
            || k->get_scancode() == KEY_LEFT || k->get_scancode() == KEY_RIGHT)
...
bool CanvasItemEditor::_gui_input_select(const Ref<InputEvent>& p_event) {
    ...
    if (drag_type == DRAG_NONE) {
        if (b.is_valid()
            && ((b->get_button_index() == BUTTON_RIGHT && b->get_alt()
                 && tool == TOOL_SELECT)
                || (b->get_button_index() == BUTTON_LEFT
                    && tool == TOOL_LIST_SELECT))) {
            ...
            _get_canvas_items_at_pos(
                click,
                selection_results,
                b->get_alt() && tool != TOOL_LIST_SELECT
            );
            ...
            if (selection_results.size() == 1) {
                ...
                NodePath root_path =
                    get_tree()->get_edited_scene_root()->get_path();
                StringName root_name =
                    root_path.get_name(root_path.get_name_count() - 1);

                for (int i = 0; i < selection_results.size(); i++) {
                    CanvasItem* item = selection_results[i].item;

                    Ref<Texture> icon = EditorNode::get_singleton()
                                            ->get_object_icon(item, "Node");
                    String node_path = "/" + root_name + "/"
                                     + root_path.rel_path_to(item->get_path());
                    ...
                    if (_is_node_locked(item)) {
                        ...
                        while (node && node != scene->get_parent()) {
                            CanvasItem* canvas_item_tmp =
                                Object::cast_to<CanvasItem>(node);
                            if (canvas_item_tmp
                                && node->has_meta("_edit_group_")) {
                                locked = 2;
                            }
                            node = node->get_parent();
                        }
        ...
        if (b.is_valid() && b->is_pressed()
            && b->get_button_index() == BUTTON_RIGHT && b->get_control()) {
            add_node_menu->set_position(
                get_global_transform().xform(get_local_mouse_position())
            );
            add_node_menu->set_size(Vector2(1, 1));
            add_node_menu->popup();
            node_create_position =
                transform.affine_inverse().xform((get_local_mouse_position()));
            return true;
        }
        ...
        if (b.is_valid() && b->get_button_index() == BUTTON_LEFT
            ...
            if (!canvas_item) {
                ...
                bool still_selected =
                    _select_click_on_item(canvas_item, click, b->get_shift());
                // Start dragging
                if (still_selected) {
    ...
    if (drag_type == DRAG_QUEUED) {
        ...
        if (m.is_valid()) {
            ...
            // Scale movement threshold with zoom (which itself is set relative
            // to the editor scale).
            bool movement_threshold_passed =
                drag_start_origin.distance_to(click)
                > (8 * MAX(1, EDSCALE)) / zoom;
    ...
    if (drag_type == DRAG_BOX_SELECTION) {
        if (b.is_valid() && !b->is_pressed()
            ...
            if (scene) {
                ...
                if (selitems.size() == 1
                    && editor_selection->get_selected_node_list().empty()) {
                    editor->push_item(selitems[0]);
                }
                for (List<CanvasItem*>::Element* E = selitems.front(); E;
                     E                             = E->next()) {
        ...
        if (m.is_valid()) {
            ...
            box_selecting_to =
                transform.affine_inverse().xform(m->get_position());
            viewport->update();
            return true;
        }
    }

    if (k.is_valid() && k->is_pressed() && k->get_scancode() == KEY_ESCAPE
        && drag_type == DRAG_NONE && tool == TOOL_SELECT) {
        // Unselect everything
        editor_selection->clear();
        viewport->update();
    }
...
bool CanvasItemEditor::_gui_input_ruler_tool(const Ref<InputEvent>& p_event) {
    ...
    if (!ruler_tool_active) {
        ruler_tool_origin = snap_point(
            viewport->get_local_mouse_position() / zoom + view_offset
        );
    }
    ...
    if (m.is_valid()
        && (ruler_tool_active
            || (grid_snap_active && previous_origin != ruler_tool_origin))) {
        viewport->update();
        return true;
    }
...
bool CanvasItemEditor::_gui_input_hover(const Ref<InputEvent>& p_event) {
    ...
    if (m.is_valid()) {
        ...
        for (int i = 0; i < hovering_results_items.size(); i++) {
            ...
            hover_result.position =
                canvas_item->get_global_transform_with_canvas().get_origin();
            hover_result.icon =
                EditorNode::get_singleton()->get_object_icon(canvas_item);
        ...
        if (hovering_results_tmp.size() == hovering_results.size()) {
            for (int i = 0; i < hovering_results_tmp.size(); i++) {
                ...
                if (a.icon != b.icon || a.name != b.name
...
void CanvasItemEditor::_gui_input_viewport(const Ref<InputEvent>& p_event) {
    ...
    if (EditorSettings::get_singleton()->get("editors/2d/simple_panning")
        || !pan_pressed) {
        if ((accepted = _gui_input_rulers_and_guides(p_event))) {
            // printf("Rulers and guides\n");
        } else if ((accepted = editor->get_editor_plugins_over()
                                   ->forward_gui_input(p_event))) {
            // printf("Plugin\n");
        } else if ((accepted =
    ...
    CursorType cursor_type = CURSOR_ARROW;
    switch (drag_type) {
        case DRAG_NONE:
            switch (tool) {
                case TOOL_MOVE:
    ...
    if (!viewport->has_focus()
        && (!get_focus_owner() || !get_focus_owner()->is_text_field())) {
...
void CanvasItemEditor::_draw_margin_at_position(
    int p_value,
    Point2 p_position,
    Margin p_side
) {
...
void CanvasItemEditor::_draw_percentage_at_position(
    float p_value,
    Point2 p_position,
    Margin p_side
) {
...
void CanvasItemEditor::_draw_focus() {
    ...
    if (viewport->has_focus()) {
        get_stylebox("Focus", "EditorStyles")
            ->draw(
                viewport->get_canvas_item(),
                Rect2(Point2(), viewport->get_size())
            );
    }
}
...
void CanvasItemEditor::_draw_guides() {
    Color guide_color =
        EditorSettings::get_singleton()->get("editors/2d/guides_color");
    ...
    if (drag_type == DRAG_DOUBLE_GUIDE || drag_type == DRAG_V_GUIDE) {
        String str = vformat(
            "%d px",
            Math::round(xform.affine_inverse().xform(dragged_guide_pos).x)
        );
        Ref<Font> font  = get_font("font", "Label");
        ...
        viewport->draw_string(
            font,
            Point2(
                dragged_guide_pos.x + 10,
                RULER_WIDTH + text_size.y / 2 + 10
            ),
            str,
            text_color
        );
        viewport->draw_line(
            Point2(dragged_guide_pos.x, 0),
            Point2(dragged_guide_pos.x, viewport->get_size().y),
            guide_color,
            Math::round(EDSCALE)
        );
    }
    if (drag_type == DRAG_DOUBLE_GUIDE || drag_type == DRAG_H_GUIDE) {
        String str = vformat(
            "%d px",
            Math::round(xform.affine_inverse().xform(dragged_guide_pos).y)
        );
        Ref<Font> font  = get_font("font", "Label");
        Size2 text_size = font->get_string_size(str);
        viewport->draw_string(
            ...
            Point2(
                RULER_WIDTH + 10,
                dragged_guide_pos.y + text_size.y / 2 + 10
            ),
            str,
            text_color
        );
        viewport->draw_line(
            Point2(0, dragged_guide_pos.y),
            Point2(viewport->get_size().x, dragged_guide_pos.y),
            guide_color,
            Math::round(EDSCALE)
        );
    }
...
void CanvasItemEditor::_draw_smart_snapping() {
    Color line_color = EditorSettings::get_singleton()->get(
        "editors/2d/smart_snapping_line_color"
    );
    if (snap_target[0] != SNAP_TARGET_NONE
        && snap_target[0] != SNAP_TARGET_GRID) {
        viewport->draw_set_transform_matrix(
            viewport->get_transform() * transform * snap_transform
        );
        viewport
            ->draw_line(Point2(0, -1.0e+10F), Point2(0, 1.0e+10F), line_color);
        viewport->draw_set_transform_matrix(viewport->get_transform());
    }
    if (snap_target[1] != SNAP_TARGET_NONE
        && snap_target[1] != SNAP_TARGET_GRID) {
        viewport->draw_set_transform_matrix(
            viewport->get_transform() * transform * snap_transform
        );
        viewport
            ->draw_line(Point2(-1.0e+10F, 0), Point2(1.0e+10F, 0), line_color);
        viewport->draw_set_transform_matrix(viewport->get_transform());
    }
}
...
void CanvasItemEditor::_draw_rulers() {
    ...
    Color graduation_color =
        get_color("font_color", "Editor").linear_interpolate(bg_color, 0.5);
    ...
    if (show_grid || grid_snap_active) {
        ...
        if (snap_relative && selection.size() > 0) {
            ruler_transform.translate(
                _get_encompassing_rect_from_list(selection).position
            );
            ruler_transform.scale_basis(
                grid_step * Math::pow(2.0, grid_step_multiplier)
            );
        } else {
        ...
        while ((transform * ruler_transform).get_scale().x < 50
               || (transform * ruler_transform).get_scale().y < 50) {
    ...
    major_subdivide.scale(
        Size2(1.0 / major_subdivision, 1.0 / major_subdivision)
    );

    int minor_subdivision       = 5;
    Transform2D minor_subdivide = Transform2D();
    minor_subdivide.scale(
        Size2(1.0 / minor_subdivision, 1.0 / minor_subdivision)
    );
    ...
    viewport->draw_rect(
        Rect2(
            Point2(RULER_WIDTH, 0),
            Size2(viewport->get_size().x, RULER_WIDTH)
        ),
        bg_color
    );
    for (int i = Math::ceil(first.x); i < last.x; i++) {
        ...
        if (i % (major_subdivision * minor_subdivision) == 0) {
            viewport->draw_line(
                Point2(position.x, 0),
                Point2(position.x, RULER_WIDTH),
                graduation_color,
                Math::round(EDSCALE)
            );
            float val = (ruler_transform * major_subdivide * minor_subdivide)
                            .xform(Point2(i, 0))
                            .x;
            viewport->draw_string(
                font,
                Point2(position.x + 2, font->get_height()),
                vformat(((int)val == val) ? "%d" : "%.1f", val),
                font_color
            );
        } else {
            if (i % minor_subdivision == 0) {
                viewport->draw_line(
                    Point2(position.x, RULER_WIDTH * 0.33),
                    Point2(position.x, RULER_WIDTH),
                    graduation_color,
                    Math::round(EDSCALE)
                );
                ...
                viewport->draw_line(
                    Point2(position.x, RULER_WIDTH * 0.75),
                    Point2(position.x, RULER_WIDTH),
                    graduation_color,
                    Math::round(EDSCALE)
                );
    ...
    viewport->draw_rect(
        Rect2(
            Point2(0, RULER_WIDTH),
            Size2(RULER_WIDTH, viewport->get_size().y)
        ),
        bg_color
    );
    for (int i = Math::ceil(first.y); i < last.y; i++) {
        ...
        if (i % (major_subdivision * minor_subdivision) == 0) {
            ...
            Transform2D text_xform = Transform2D(
                -static_cast<real_t>(Math_PI) / 2,
                Point2(font->get_height(), position.y - 2)
            );
            viewport->draw_set_transform_matrix(
                viewport->get_transform() * text_xform
            );
            viewport->draw_string(
                font,
                Point2(),
            ...
            if (i % minor_subdivision == 0) {
                viewport->draw_line(
                    Point2(RULER_WIDTH * 0.33, position.y),
                    Point2(RULER_WIDTH, position.y),
                    graduation_color,
                    Math::round(EDSCALE)
                );
                ...
                viewport->draw_line(
                    Point2(RULER_WIDTH * 0.75, position.y),
                    Point2(RULER_WIDTH, position.y),
                    graduation_color,
                    Math::round(EDSCALE)
                );
    ...
    viewport->draw_rect(
        Rect2(Point2(), Size2(RULER_WIDTH, RULER_WIDTH)),
        graduation_color
    );
}
    ...
    if (show_grid || grid_snap_active) {
        ...
        if (snap_relative && selection.size() > 0) {
            const Vector2 topleft =
                _get_encompassing_rect_from_list(selection).position;
            real_grid_offset.x = fmod(
                topleft.x,
                grid_step.x * (real_t)Math::pow(2.0, grid_step_multiplier)
            );
            real_grid_offset.y = fmod(
                topleft.y,
                grid_step.y * (real_t)Math::pow(2.0, grid_step_multiplier)
            );
        } else {
            real_grid_offset = grid_offset;
        }

        // Draw a "primary" line every several lines to make measurements
        // easier. The step is configurable in the Configure Snap dialog.
        const Color secondary_grid_color =
            EditorSettings::get_singleton()->get("editors/2d/grid_color");
        const Color primary_grid_color = Color(
            secondary_grid_color.r,
            secondary_grid_color.g,
            secondary_grid_color.b,
            secondary_grid_color.a * 2.5
        );
        ...
        if (grid_step.x != 0) {
            for (int i = 0; i < viewport_size.width; i++) {
                const int cell = Math::fast_ftoi(Math::floor(
                    (xform.xform(Vector2(i, 0)).x - real_grid_offset.x)
                    / (grid_step.x * Math::pow(2.0, grid_step_multiplier))
                ));
                ...
                if (last_cell != cell) {
                    ...
                    if (primary_grid_steps == 0) {
                        ...
                        grid_color = cell % primary_grid_steps == 0
                                       ? primary_grid_color
        ...
        if (grid_step.y != 0) {
            for (int i = 0; i < viewport_size.height; i++) {
                const int cell = Math::fast_ftoi(Math::floor(
                    (xform.xform(Vector2(0, i)).y - real_grid_offset.y)
                    / (grid_step.y * Math::pow(2.0, grid_step_multiplier))
                ));
...
void CanvasItemEditor::_draw_ruler_tool() {
    ...
    if (ruler_tool_active) {
        ...
        bool draw_secondary_lines =
            !(Math::is_equal_approx(begin.y, corner.y)
              || Math::is_equal_approx(end.x, corner.x));

        viewport->draw_line(
            begin,
            end,
            ruler_primary_color,
            Math::round(EDSCALE * 3),
            true
        );
        ...
        Point2 text_pos =
            (begin + end) / 2 - Vector2(text_width / 2, text_height / 2);
        text_pos.x = CLAMP(
            text_pos.x,
            text_width / 2,
            viewport->get_rect().size.x - text_width * 1.5
        );
        text_pos.y = CLAMP(
            text_pos.y,
            text_height * 1.5,
            viewport->get_rect().size.y - text_height * 1.5
        );
        viewport->draw_string(
            font,
            text_pos,
            vformat("%.1f px", length_vector.length()),
            font_color
        );

        if (draw_secondary_lines) {
            const float horizontal_angle_rad =
                atan2(length_vector.y, length_vector.x);
            const float vertical_angle_rad =
                Math_PI / 2.0 - horizontal_angle_rad;
            const int horizontal_angle =
                round(180 * horizontal_angle_rad / Math_PI);
            const int vertical_angle =
            ...
            viewport->draw_string(
                font,
                text_pos2,
                vformat("%.1f px", length_vector.y),
                font_secondary_color
            );

            Point2 v_angle_text_pos = Point2();
            v_angle_text_pos.x      = CLAMP(
                begin.x - angle_text_width / 2,
                angle_text_width / 2,
            ...
            viewport->draw_string(
                font,
                v_angle_text_pos,
                vformat(String::utf8("%d°"), vertical_angle),
                font_secondary_color
            );
            ...
            if (begin.y < end.y) {
                ...
                if (ABS(text_pos2.x - h_angle_text_pos.x) < text_width) {
                    ...
                    h_angle_text_pos.y =
                        MAX(text_pos.y + height_multiplier * text_height,
                            MAX(end.y + text_height * 1.5,
                ...
                if (ABS(text_pos2.x - h_angle_text_pos.x) < text_width) {
                    ...
                    h_angle_text_pos.y =
                        MIN(text_pos.y - height_multiplier * text_height,
                            MIN(end.y - text_height * 0.5,
                                text_pos2.y - height_multiplier * text_height));
                }
            }
            viewport->draw_string(
                font,
                h_angle_text_pos,
                vformat(String::utf8("%d°"), horizontal_angle),
                font_secondary_color
            );

            // Angle arcs
            int arc_point_count                 = 8;
            float arc_radius_max_length_percent = 0.1f;
            float ruler_length                  = length_vector.length() * zoom;
            float arc_max_radius                = 50;
            float arc_line_width                = 2;
            ...
            float arc_1_start_angle =
                end_to_begin.x < 0
                    ? (end_to_begin.y < 0
                           ? 3.0 * Math_PI / 2.0 - vertical_angle_rad
                           : Math_PI / 2.0)
                    : (end_to_begin.y < 0 ? 3.0 * Math_PI / 2.0
                                          : Math_PI / 2.0 - vertical_angle_rad);
            float arc_1_end_angle = arc_1_start_angle + vertical_angle_rad;
            // Constrain arc to triangle height & max size
            float arc_1_radius =
                MIN(MIN(arc_radius_max_length_percent * ruler_length,
                        ABS(end_to_begin.y)),
                    arc_max_radius);
            ...
            float arc_2_start_angle =
                end_to_begin.x < 0
                    ...
                    : (end_to_begin.y < 0 ? Math_PI - horizontal_angle_rad
                                          : Math_PI);
            ...
            viewport->draw_arc(
                ...
                arc_point_count,
                ruler_primary_color,
                Math::round(EDSCALE * arc_line_width)
            );
            viewport->draw_arc(
                end,
                arc_2_radius,
        ...
        if (grid_snap_active) {
            ...
            text_pos.y = CLAMP(
                text_pos.y,
                text_height * 2.5,
                viewport->get_rect().size.y - text_height / 2
            );
            ...
            if (draw_secondary_lines) {
                viewport->draw_string(
                    font,
                    text_pos,
                    vformat("%.2f units", (length_vector / grid_step).length()),
                    font_color
                );
                ...
                viewport->draw_string(
                    font,
                    text_pos2,
                    vformat("%d units", roundf(length_vector.y / grid_step.y)),
                    font_secondary_color
                );
                ...
                viewport->draw_string(
                    font,
                    text_pos2,
                    vformat("%d units", roundf(length_vector.x / grid_step.x)),
                    font_secondary_color
                );
            } else {
                viewport->draw_string(
                    ...
                    vformat(
                        "%d units",
                        roundf((length_vector / grid_step).length())
                    ),
                    font_color
                );
            }
        ...
        if (grid_snap_active) {
            Ref<Texture> position_icon =
                get_icon("EditorPosition", "EditorIcons");
            viewport->draw_texture(
                get_icon("EditorPosition", "EditorIcons"),
                (ruler_tool_origin - view_offset) * zoom
                    - position_icon->get_size() / 2
            );
        }
    }
}
...
void CanvasItemEditor::_draw_control_anchors(Control* control) {
    ...
    if (tool == TOOL_SELECT
        && !Object::cast_to<Container>(control->get_parent())) {
        // Compute the anchors
        ...
        for (int i = 0; i < 4; i++) {
            Vector2 value = Vector2(
                (i % 2 == 0) ? anchors_values[i] : anchors_values[(i + 1) % 4],
        ...
        Rect2 anchor_rects[4];
        anchor_rects[0] = Rect2(
            anchors_pos[0] - anchor_handle->get_size(),
            anchor_handle->get_size()
        );
        anchor_rects[1] = Rect2(
            anchors_pos[1] - Vector2(0.0, anchor_handle->get_size().y),
            Point2(-anchor_handle->get_size().x, anchor_handle->get_size().y)
        );
        anchor_rects[2] = Rect2(anchors_pos[2], -anchor_handle->get_size());
        anchor_rects[3] = Rect2(
            anchors_pos[3] - Vector2(anchor_handle->get_size().x, 0.0),
            Point2(anchor_handle->get_size().x, -anchor_handle->get_size().y)
        );
...
void CanvasItemEditor::_draw_control_helpers(Control* control) {
    Transform2D xform = transform * control->get_global_transform_with_canvas();
    if (tool == TOOL_SELECT && show_helpers
        && !Object::cast_to<Container>(control->get_parent())) {
        // Draw the helpers
        ...
        for (int i = 0; i < 4; i++) {
            ...
            anchors_pos[i] =
                xform.xform(_anchor_to_position(control, anchors[i]));
        }
        ...
        if (dragged_anchor >= 0) {
            ...
            bool anchor_snapped;
            Color color_snapped = Color(0.64f, 0.93f, 0.67f, 0.5f);

            Vector2 corners_pos[4];
            for (int i = 0; i < 4; i++) {
                corners_pos[i] = xform.xform(_anchor_to_position(
                    control,
                    Vector2(
                        (i == 0 || i == 3) ? ANCHOR_BEGIN : ANCHOR_END,
                        (i <= 1) ? ANCHOR_BEGIN : ANCHOR_END
                    )
                ));
            }
            ...
            Vector2 line_ends[4];
            for (int i = 0; i < 4; i++) {
                float anchor_val = (i >= 2) ? ANCHOR_END - anchors_values[i]
                                            : anchors_values[i];
                line_starts[i]   = Vector2::linear_interpolate(
                    corners_pos[i],
                    corners_pos[(i + 1) % 4],
                    anchor_val
                );
                line_ends[i] = Vector2::linear_interpolate(
                    corners_pos[(i + 3) % 4],
                    corners_pos[(i + 2) % 4],
                    anchor_val
                );
                anchor_snapped = anchors_values[i] == 0.0
                              || anchors_values[i] == 0.5
                              || anchors_values[i] == 1.0;
                viewport->draw_line(
                    line_starts[i],
                    line_ends[i],
                    anchor_snapped ? color_snapped : color_base,
                    (i == dragged_anchor || (i + 3) % 4 == dragged_anchor) ? 2
                                                                           : 1
                );
            }
            ...
            float percent_val;
            percent_val = anchors_values[(dragged_anchor + 2) % 4]
                        - anchors_values[dragged_anchor];
            percent_val = (dragged_anchor >= 2) ? -percent_val : percent_val;
            _draw_percentage_at_position(
                percent_val,
                (anchors_pos[dragged_anchor]
                 + anchors_pos[(dragged_anchor + 1) % 4])
                    / 2,
                (Margin)((dragged_anchor + 1) % 4)
            );

            percent_val = anchors_values[(dragged_anchor + 3) % 4]
                        - anchors_values[(dragged_anchor + 1) % 4];
            percent_val =
                ((dragged_anchor + 1) % 4 >= 2) ? -percent_val : percent_val;
            _draw_percentage_at_position(
                percent_val,
                (anchors_pos[dragged_anchor]
                 + anchors_pos[(dragged_anchor + 3) % 4])
                    / 2,
                (Margin)(dragged_anchor)
            );

            percent_val = anchors_values[(dragged_anchor + 1) % 4];
            percent_val = ((dragged_anchor + 1) % 4 >= 2)
                            ? ANCHOR_END - percent_val
                            : percent_val;
            _draw_percentage_at_position(
                percent_val,
                (line_starts[dragged_anchor] + anchors_pos[dragged_anchor]) / 2,
                (Margin)(dragged_anchor)
            );

            percent_val = anchors_values[dragged_anchor];
            percent_val =
                (dragged_anchor >= 2) ? ANCHOR_END - percent_val : percent_val;
            _draw_percentage_at_position(
                percent_val,
                (line_ends[(dragged_anchor + 1) % 4]
                 + anchors_pos[dragged_anchor])
                    / 2,
                (Margin)((dragged_anchor + 1) % 4)
            );
        ...
        // Draw the margin values and the node width/height when dragging
        // control side
        float ratio = 0.33f;
        Transform2D parent_transform =
            xform * control->get_transform().affine_inverse();
        float node_pos_in_parent[4];

        Rect2 parent_rect = control->get_parent_anchorable_rect();

        node_pos_in_parent[0] =
            control->get_anchor(MARGIN_LEFT) * parent_rect.size.width
            + control->get_margin(MARGIN_LEFT) + parent_rect.position.x;
        node_pos_in_parent[1] =
            control->get_anchor(MARGIN_TOP) * parent_rect.size.height
            + control->get_margin(MARGIN_TOP) + parent_rect.position.y;
        node_pos_in_parent[2] =
            control->get_anchor(MARGIN_RIGHT) * parent_rect.size.width
            + control->get_margin(MARGIN_RIGHT) + parent_rect.position.x;
        node_pos_in_parent[3] =
            control->get_anchor(MARGIN_BOTTOM) * parent_rect.size.height
            + control->get_margin(MARGIN_BOTTOM) + parent_rect.position.y;
        ...
        switch (drag_type) {
            ...
            case DRAG_BOTTOM_LEFT:
                _draw_margin_at_position(
                    control->get_size().width,
                    parent_transform.xform(Vector2(
                        (node_pos_in_parent[0] + node_pos_in_parent[2]) / 2,
                        node_pos_in_parent[3]
                    )) + Vector2(0, 5),
                    MARGIN_BOTTOM
                );
            ...
            case DRAG_MOVE:
                ...
                _draw_margin_at_position(
                    control->get_margin(MARGIN_LEFT),
                    parent_transform.xform((start + end) / 2),
                    MARGIN_TOP
                );
                viewport->draw_line(
                    parent_transform.xform(start),
                    parent_transform.xform(end),
                    color_base,
                    Math::round(EDSCALE)
                );
                break;
        ...
        switch (drag_type) {
            ...
            case DRAG_MOVE:
                ...
                _draw_margin_at_position(
                    control->get_margin(MARGIN_RIGHT),
                    parent_transform.xform((start + end) / 2),
                    MARGIN_BOTTOM
                );
                viewport->draw_line(
        ...
        switch (drag_type) {
            ...
            case DRAG_TOP_RIGHT:
                _draw_margin_at_position(
                    control->get_size().height,
                    parent_transform.xform(Vector2(
                        node_pos_in_parent[2],
                        (node_pos_in_parent[1] + node_pos_in_parent[3]) / 2
                    )) + Vector2(5, 0),
                    MARGIN_RIGHT
                );
                FALLTHROUGH;
        ...
        switch (drag_type) {
            ...
            case DRAG_MOVE:
                ...
                _draw_margin_at_position(
                    control->get_margin(MARGIN_BOTTOM),
                    parent_transform.xform((start + end) / 2),
                    MARGIN_RIGHT
                );
                viewport->draw_line(
        ...
        switch (drag_type) {
            ...
            case DRAG_BOTTOM_LEFT:
            case DRAG_MOVE:
                if (control->get_rotation() != 0.0
                    || control->get_scale() != Vector2(1, 1)) {
                    Rect2 rect = Rect2(
                        Vector2(node_pos_in_parent[0], node_pos_in_parent[1]),
                        control->get_size()
                    );
                    viewport->draw_rect(
                        parent_transform.xform(rect),
                        color_base,
                        false,
                        Math::round(EDSCALE)
                    );
...
void CanvasItemEditor::_draw_selection() {
    ...
    Ref<Texture> previous_position_icon =
        get_icon("EditorPositionPrevious", "EditorIcons");
    ...
    for (List<CanvasItem*>::Element* E = selection.front(); E; E = E->next()) {
        ...
        if (show_helpers
            && (drag_type == DRAG_MOVE || drag_type == DRAG_ROTATE
                || drag_type == DRAG_LEFT || drag_type == DRAG_RIGHT
                || drag_type == DRAG_TOP || drag_type == DRAG_BOTTOM
                || drag_type == DRAG_TOP_LEFT || drag_type == DRAG_TOP_RIGHT
                || drag_type == DRAG_BOTTOM_LEFT
                || drag_type == DRAG_BOTTOM_RIGHT)) {
            ...
            if (canvas_item->_edit_use_rect()) {
                Vector2 pre_drag_endpoints[4] = {
                    ...
                    pre_drag_xform.xform(
                        se->pre_drag_rect.position
                        + Vector2(se->pre_drag_rect.size.x, 0)
                    ),
                    pre_drag_xform.xform(
                        se->pre_drag_rect.position + se->pre_drag_rect.size
                    ),
                    pre_drag_xform.xform(
                        se->pre_drag_rect.position
                        + Vector2(0, se->pre_drag_rect.size.y)
                    )
                };

                for (int i = 0; i < 4; i++) {
                    viewport->draw_line(
                        pre_drag_endpoints[i],
                        pre_drag_endpoints[(i + 1) % 4],
                        pre_drag_color,
                ...
                viewport->draw_texture(
                    previous_position_icon,
                    (pre_drag_xform.xform(Point2())
                     - (previous_position_icon->get_size() / 2))
                        .floor()
                );
            }
        }
        ...
        Transform2D xform =
            transform * canvas_item->get_global_transform_with_canvas();
        ...
        if (single && !item_locked
            && (tool == TOOL_SELECT || tool == TOOL_MOVE || tool == TOOL_SCALE
                || tool == TOOL_ROTATE
            ...
            if (tool == TOOL_SELECT && canvas_item->_edit_use_rect()
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    select_handle->draw(
                        ci,
                        (endpoints[i] + ofs - (select_handle->get_size() / 2))
                            .floor()
                    );

                    ofs = (endpoints[i] + endpoints[next]) / 2;
                    ofs +=
                        (endpoints[next] - endpoints[i]).tangent().normalized()
                        * (select_handle->get_size().width / 2);

                    select_handle->draw(
                        ci,
                        (ofs - (select_handle->get_size() / 2)).floor()
                    );
                }
            }
            ...
            if ((is_alt && is_ctrl) || tool == TOOL_SCALE
                || drag_type == DRAG_SCALE_X || drag_type == DRAG_SCALE_Y) {
                if (_is_node_movable(canvas_item)) {
                    Transform2D unscaled_transform =
                        (xform * canvas_item->get_transform().affine_inverse()
                         * canvas_item->_edit_get_transform())
                    ...
                    Point2 offset =
                        (simple_xform.affine_inverse().xform(drag_to)
                    ...
                    viewport->draw_rect(
                        x_handle_rect,
                        get_color("axis_x_color", "Editor")
                    );
                    viewport->draw_line(
                        Point2(),
                        Point2(scale_factor.x * EDSCALE, 0),
                        get_color("axis_x_color", "Editor"),
                        Math::round(EDSCALE),
                        true
                    );
                    ...
                    viewport->draw_rect(
                        y_handle_rect,
                        get_color("axis_y_color", "Editor")
                    );
                    viewport->draw_line(
                        Point2(),
                        Point2(0, -scale_factor.y * EDSCALE),
                        get_color("axis_y_color", "Editor"),
                        Math::round(EDSCALE),
                        true
                    );
...
void CanvasItemEditor::_draw_straight_line(
    ...
    if (to.x == from.x) {
        ...
        float y_for_viewport_x =
            ((to.y - from.y) * (viewport_size.x - from.x)) / (to.x - from.x)
            + from.y;
        float x_for_viewport_y =
            ((to.x - from.x) * (viewport_size.y - from.y)) / (to.y - from.y)
            + from.x; // faux
    ...
    if (points.size() >= 2) {
        VisualServer::get_singleton()
            ->canvas_item_add_line(ci, points[0], points[1], p_color);
    }
}
...
void CanvasItemEditor::_draw_axis() {
    if (show_origin) {
        _draw_straight_line(
            Point2(),
            Point2(1, 0),
            get_color("axis_x_color", "Editor") * Color(1, 1, 1, 0.75)
        );
        _draw_straight_line(
            Point2(),
            Point2(0, 1),
            get_color("axis_y_color", "Editor") * Color(1, 1, 1, 0.75)
        );
    }
    ...
    if (show_viewport) {
        ...
        Color area_axis_color = EditorSettings::get_singleton()->get(
...
void CanvasItemEditor::_draw_bones() {
    ...
    if (skeleton_show_bones) {
        Color bone_color1 =
            EditorSettings::get_singleton()->get("editors/2d/bone_color1");
        Color bone_color2 =
            EditorSettings::get_singleton()->get("editors/2d/bone_color2");
        Color bone_ik_color =
            EditorSettings::get_singleton()->get("editors/2d/bone_ik_color");
        Color bone_outline_color =
            EditorSettings::get_singleton()->get("editors/2d/bone_outline_color"
            );
        Color bone_selected_color = EditorSettings::get_singleton()->get(
...
void CanvasItemEditor::_draw_invisible_nodes_positions(
    ...
    Node* scene = editor->get_edited_scene();
    if (p_node != scene && p_node->get_owner() != scene
        && !scene->is_editable_instance(p_node->get_owner())) {
        return;
    }
    ...
    for (int i = p_node->get_child_count() - 1; i >= 0; i--) {
        _draw_invisible_nodes_positions(
            p_node->get_child(i),
            parent_xform,
            canvas_xform
        );
    ...
    if (canvas_item && !canvas_item->_edit_use_rect()
        && (!editor_selection->is_selected(canvas_item)
            || _is_node_locked(canvas_item))) {
        Transform2D xform = transform * canvas_xform * parent_xform;

        // Draw the node's position
        Ref<Texture> position_icon =
            get_icon("EditorPositionUnselected", "EditorIcons");
        Transform2D unscaled_transform =
            (xform * canvas_item->get_transform().affine_inverse()
             * canvas_item->_edit_get_transform())
                .orthonormalized();
...
void CanvasItemEditor::_draw_hover() {
    ...
    for (int i = 0; i < hovering_results.size(); i++) {
        ...
        Size2 item_size      = Size2(
            node_icon->get_size().x + 4 + node_name_size.x,
            MAX(node_icon->get_size().y, node_name_size.y - 3)
        );

        Point2 pos =
            transform.xform(hovering_results[i].position)
            - Point2(0, item_size.y)
            + (Point2(node_icon->get_size().x, -node_icon->get_size().y) / 4);
        // Rectify the position to avoid overlapping items
        for (List<Rect2>::Element* E = previous_rects.front(); E;
             E                       = E->next()) {
        ...
        viewport->draw_string(
            font,
            pos + Point2(node_icon->get_size().x + 4, item_size.y - 3),
            node_name,
            Color(1.0, 1.0, 1.0, 0.5)
        );
    }
...
void CanvasItemEditor::_draw_locks_and_groups(
    ...
    if (canvas_item) {
        ...
        if (p_node->has_meta("_edit_lock_") && show_edit_locks) {
            lock->draw(
                viewport_canvas_item,
                (transform * canvas_xform * parent_xform).xform(Point2(0, 0))
                    + Point2(offset, 0)
            );
...
void CanvasItemEditor::_draw_viewport() {
    ...
    if (selection.empty()) {
        ...
        for (List<Node*>::Element* E = selection.front(); E; E = E->next()) {
            if (Object::cast_to<CanvasItem>(E->get())
                && !Object::cast_to<CanvasItem>(E->get())->has_meta(
                    "_edit_lock_"
                )) {
                all_locked = false;
                break;
    ...
    EditorPluginList* force_over_plugin_list =
        editor->get_editor_plugins_force_over();
...
void CanvasItemEditor::_notification(int p_what) {
    if (p_what == NOTIFICATION_PHYSICS_PROCESS) {
        EditorNode::get_singleton()
            ...
            ->set_snap_controls_to_pixels(
                GLOBAL_GET("gui/common/snap_controls_to_pixels")
            );
        ...
        for (List<CanvasItem*>::Element* E = selection.front(); E;
            ...
            if (control) {
                ...
                if (pivot != se->prev_pivot
                    || anchors[MARGIN_LEFT] != se->prev_anchors[MARGIN_LEFT]
                    || anchors[MARGIN_RIGHT] != se->prev_anchors[MARGIN_RIGHT]
                    || anchors[MARGIN_TOP] != se->prev_anchors[MARGIN_TOP]
                    || anchors[MARGIN_BOTTOM]
                           != se->prev_anchors[MARGIN_BOTTOM]) {
                    se->prev_pivot                  = pivot;
        ...
        if (nb_control > 0 && nb_control == selection.size()) {
            ...
            if (has_container_parents) {
                ...
                presets_menu->set_tooltip(
                    TTR("Children of containers have their anchors and margins "
                        "values overridden by their parent.")
                );
                ...
                presets_menu->set_tooltip(
                    TTR("Presets for the anchors and margins values of a "
                        "Control node.")
                );
                anchor_mode_button->set_disabled(false);
                anchor_mode_button->set_tooltip(
                    TTR("When active, moving Control nodes changes their "
                        "anchors instead of their margins.")
                );
            }
        } else {
    ...
    if (p_what == NOTIFICATION_ENTER_TREE) {
        ...
        AnimationPlayerEditor::singleton->get_track_editor()
            ->connect("visibility_changed", this, "_keying_changed");
        _keying_changed();
    ...
    if (p_what == NOTIFICATION_ENTER_TREE
        || p_what == EditorSettings::NOTIFICATION_EDITOR_SETTINGS_CHANGED) {
        ...
        const Color key_auto_color =
            EditorSettings::get_singleton()->is_dark_theme()
                ? Color(1, 1, 1)
                : Color(4.25, 4.25, 4.25);
        key_auto_insert_button->add_color_override(
            "icon_color_pressed",
            key_auto_color.linear_interpolate(Color(1, 0, 0), 0.55f)
        );
        ...
        p->clear();
        p->add_icon_item(
            get_icon("ControlAlignTopLeft", "EditorIcons"),
            TTR("Top Left"),
            ANCHORS_AND_MARGINS_PRESET_TOP_LEFT
        );
        p->add_icon_item(
            get_icon("ControlAlignTopRight", "EditorIcons"),
            TTR("Top Right"),
            ANCHORS_AND_MARGINS_PRESET_TOP_RIGHT
        );
        p->add_icon_item(
            get_icon("ControlAlignBottomRight", "EditorIcons"),
            TTR("Bottom Right"),
            ANCHORS_AND_MARGINS_PRESET_BOTTOM_RIGHT
        );
        p->add_icon_item(
            get_icon("ControlAlignBottomLeft", "EditorIcons"),
            TTR("Bottom Left"),
            ANCHORS_AND_MARGINS_PRESET_BOTTOM_LEFT
        );
        p->add_separator();
        p->add_icon_item(
            get_icon("ControlAlignLeftCenter", "EditorIcons"),
            TTR("Center Left"),
            ANCHORS_AND_MARGINS_PRESET_CENTER_LEFT
        );
        p->add_icon_item(
            get_icon("ControlAlignTopCenter", "EditorIcons"),
            TTR("Center Top"),
            ANCHORS_AND_MARGINS_PRESET_CENTER_TOP
        );
        p->add_icon_item(
            get_icon("ControlAlignRightCenter", "EditorIcons"),
            TTR("Center Right"),
            ANCHORS_AND_MARGINS_PRESET_CENTER_RIGHT
        );
        p->add_icon_item(
            get_icon("ControlAlignBottomCenter", "EditorIcons"),
            TTR("Center Bottom"),
            ANCHORS_AND_MARGINS_PRESET_CENTER_BOTTOM
        );
        p->add_icon_item(
            get_icon("ControlAlignCenter", "EditorIcons"),
            TTR("Center"),
            ANCHORS_AND_MARGINS_PRESET_CENTER
        );
        p->add_separator();
        p->add_icon_item(
            get_icon("ControlAlignLeftWide", "EditorIcons"),
            TTR("Left Wide"),
            ANCHORS_AND_MARGINS_PRESET_LEFT_WIDE
        );
        p->add_icon_item(
            get_icon("ControlAlignTopWide", "EditorIcons"),
            TTR("Top Wide"),
            ANCHORS_AND_MARGINS_PRESET_TOP_WIDE
        );
        p->add_icon_item(
            get_icon("ControlAlignRightWide", "EditorIcons"),
            TTR("Right Wide"),
            ANCHORS_AND_MARGINS_PRESET_RIGHT_WIDE
        );
        p->add_icon_item(
            get_icon("ControlAlignBottomWide", "EditorIcons"),
            TTR("Bottom Wide"),
            ANCHORS_AND_MARGINS_PRESET_BOTTOM_WIDE
        );
        p->add_icon_item(
            get_icon("ControlVcenterWide", "EditorIcons"),
            TTR("VCenter Wide"),
            ANCHORS_AND_MARGINS_PRESET_VCENTER_WIDE
        );
        p->add_icon_item(
            get_icon("ControlHcenterWide", "EditorIcons"),
            TTR("HCenter Wide"),
            ANCHORS_AND_MARGINS_PRESET_HCENTER_WIDE
        );
        p->add_separator();
        p->add_icon_item(
            get_icon("ControlAlignWide", "EditorIcons"),
            TTR("Full Rect"),
            ANCHORS_AND_MARGINS_PRESET_WIDE
        );
        p->add_icon_item(
            get_icon("Anchor", "EditorIcons"),
            TTR("Keep Ratio"),
        ...
        info_overlay->get_theme()->set_stylebox(
            "normal",
            "Label",
            get_stylebox("CanvasItemInfoOverlay", "EditorStyles")
        );
        warning_child_of_container->add_color_override(
            "font_color",
            get_color("warning_color", "Editor")
        );
        warning_child_of_container->add_font_override(
            "font",
...
void CanvasItemEditor::_selection_changed() {
    ...
    for (List<Node*>::Element* E = selection.front(); E; E = E->next()) {
        ...
        nbValidControls++;
        if (control->has_meta("_edit_use_anchors_")
            && control->get_meta("_edit_use_anchors_")) {
            nbAnchorsMode++;
        }
    }
...
void CanvasItemEditor::_update_bone_list() {
    ...
    for (Map<BoneKey, BoneList>::Element* E = bone_list.front(); E;
        ...
        Node* node =
            Object::cast_to<Node>(ObjectDB::get_instance(E->key().from));
...
void CanvasItemEditor::_update_context_menu_stylebox() {
    // This must be called when the theme changes to follow the new accent
    // color.
    Ref<StyleBoxFlat> context_menu_stylebox = memnew(StyleBoxFlat);
    const Color accent_color =
        EditorNode::get_singleton()->get_gui_base()->get_color(
            "accent_color",
            "Editor"
        );
    ...
    context_menu_stylebox->set_border_width(
        MARGIN_BOTTOM,
        Math::round(2 * EDSCALE)
    );
...
void CanvasItemEditor::_update_scrollbars() {
    ...
    controls_vb_begin +=
        (show_rulers) ? Point2(RULER_WIDTH, RULER_WIDTH) : Point2();
    ...
    Point2 end   = canvas_item_rect.position + canvas_item_rect.size
               - local_rect.size / zoom;
    bool constrain_editor_view = bool(
        EditorSettings::get_singleton()->get("editors/2d/constrain_editor_view")
    );

    if (canvas_item_rect.size.height <= (local_rect.size.y / zoom)) {
        float centered = -(size.y / 2) / zoom + screen_rect.y / 2;
        if (constrain_editor_view
            && ABS(centered - previous_update_view_offset.y)
                   < ABS(centered - view_offset.y)) {
            view_offset.y = previous_update_view_offset.y;
        }

        v_scroll->hide();
    } else {
        if (constrain_editor_view && view_offset.y > end.y
            && view_offset.y > previous_update_view_offset.y) {
            view_offset.y = MAX(end.y, previous_update_view_offset.y);
        }
        if (constrain_editor_view && view_offset.y < begin.y
            && view_offset.y < previous_update_view_offset.y) {
    ...
    if (canvas_item_rect.size.width <= (local_rect.size.x / zoom)) {
        ...
        if (constrain_editor_view
            && ABS(centered - previous_update_view_offset.x)
                   < ABS(centered - view_offset.x)) {
            view_offset.x = previous_update_view_offset.x;
        }

        h_scroll->hide();
    } else {
        if (constrain_editor_view && view_offset.x > end.x
            && view_offset.x > previous_update_view_offset.x) {
            view_offset.x = MAX(end.x, previous_update_view_offset.x);
        }
        if (constrain_editor_view && view_offset.x < begin.x
            && view_offset.x < previous_update_view_offset.x) {
    ...
    v_scroll->set_begin(
        Point2(size.width - vmin.width, (show_rulers) ? RULER_WIDTH : 0)
    );
    v_scroll->set_end(Point2(
        size.width,
        size.height - (h_scroll->is_visible() ? hmin.height : 0)
    ));
    h_scroll->set_begin(
        Point2((show_rulers) ? RULER_WIDTH : 0, size.height - hmin.height)
    );
    h_scroll->set_end(Point2(
        size.width - (v_scroll->is_visible() ? vmin.width : 0),
        size.height
    ));
...
void CanvasItemEditor::_popup_warning_temporarily(
    Control* p_control,
...
void CanvasItemEditor::_set_anchors_and_margins_preset(
    Control::LayoutPreset p_preset
) {
    List<Node*> selection = editor_selection->get_selected_node_list();
...
void CanvasItemEditor::_set_anchors_and_margins_to_keep_ratio() {
    ...
    for (List<Node*>::Element* E = selection.front(); E; E = E->next()) {
        ...
        if (control) {
            ...
            Point2 bottom_right_anchor =
                _position_to_anchor(control, control->get_size());
            undo_redo->add_do_method(
                control,
                "set_anchor",
                MARGIN_LEFT,
            ...
            const bool use_anchors = control->has_meta("_edit_use_anchors_")
                                  && control->get_meta("_edit_use_anchors_");
            undo_redo->add_undo_method(
                control,
                "_edit_set_state",
...
float CanvasItemEditor::_get_next_zoom_value(
    int p_increment_count,
    bool p_integer_only
) const {
    ...
    if (p_integer_only) {
        // Only visit integer scaling factors above 100%, and fractions with an
        // integer denominator below 100% (1/2 = 50%, 1/3 = 33.33%, 1/4 = 25%,
        // …). This is useful when working on pixel art projects to avoid
        // distortion. This algorithm is designed to handle fractional start
        // zoom values correctly (e.g. 190% will zoom up to 200% and down to
        // 100%).
        if (zoom_noscale + p_increment_count * 0.001 >= 1.0 - CMP_EPSILON) {
            // New zoom is certain to be above 100%.
            if (p_increment_count >= 1) {
                // Zooming.
                return Math::floor(zoom_noscale + p_increment_count)
                     * MAX(1, EDSCALE);
            } else {
                // Dezooming.
                return Math::ceil(zoom_noscale + p_increment_count)
                     * MAX(1, EDSCALE);
            }
        } else {
            if (p_increment_count >= 1) {
                ...
                float new_zoom =
                    1.0 / Math::ceil(1.0 / zoom_noscale - p_increment_count);
                if (Math::is_equal_approx(zoom_noscale, new_zoom)) {
                    ...
                    new_zoom = 1.0
                             / Math::ceil(
                                   1.0 / zoom_noscale - p_increment_count - 1
                             );
                }
                ...
                float new_zoom =
                    1.0 / Math::floor(1.0 / zoom_noscale - p_increment_count);
                if (Math::is_equal_approx(zoom_noscale, new_zoom)) {
                    // New zoom is identical to the old zoom, so try again.
                    // This can happen due to floating-point precision issues.
                    new_zoom = 1.0
        ...
        // This allow a smooth geometric evolution of the zoom, with the
        // advantage of visiting all integer power of two scale factors. note:
        // this is analogous to the 'semitones' interval in the music world In
        // order to avoid numerical imprecisions, we compute and edit a zoom
        // index with the following relation: zoom = 2 ^ (index / 12)
        ...
        float closest_zoom_index =
            Math::round(Math::log(zoom_noscale) * 12.f / Math::log(2.f));
...
void CanvasItemEditor::_zoom_on_position(float p_zoom, Point2 p_position) {
    ...
    // We want to align in-scene pixels to screen pixels, this prevents blurry
    // rendering in small details (texts, lines). This correction adds a jitter
    // movement when zooming, so we correct only when the zoom factor is an
    // integer. (in the other cases, all pixels won't be aligned anyway)
    ...
    if (Math::is_zero_approx(zoom - closest_zoom_factor)) {
        ...
        view_offset              = view_offset_int
                    + (view_offset_frac * closest_zoom_factor).round()
                          / closest_zoom_factor;
    }
...
void CanvasItemEditor::_update_zoom_label() {
    ...
    // (like in most image editors). Its lower bound is clamped to 1 as some
    // people lower the editor scale to increase the available real estate, even
    // if their display doesn't have a particularly low DPI.
    if (zoom >= 10) {
        // Don't show a decimal when the zoom level is higher than 1000 %.
        zoom_text = rtos(Math::round((zoom / MAX(1, EDSCALE)) * 100)) + " %";
    } else {
        zoom_text =
            rtos(Math::stepify((zoom / MAX(1, EDSCALE)) * 100, 0.1)) + " %";
    }
...
void CanvasItemEditor::_button_zoom_minus() {
    if (Input::get_singleton()->is_key_pressed(KEY_ALT)) {
        _zoom_on_position(
            _get_next_zoom_value(-1, true),
            viewport_scrollable->get_size() / 2.0
        );
    } else {
        _zoom_on_position(
            _get_next_zoom_value(-6),
            viewport_scrollable->get_size() / 2.0
        );
    }
}
        ...
        if (Object::cast_to<Node2D>(canvas_item)) {
            ...
            if (n2d->has_meta("_edit_bone_") && n2d->get_parent_item()) {
                ...
                if (has_chain && ik_chain.size()) {
                    for (List<Node2D*>::Element* F = ik_chain.front(); F;
                         F                         = F->next()) {
                        if (key_pos) {
                            AnimationPlayerEditor::singleton->get_track_editor()
                                ->insert_node_value_key(
                        ...
                        if (key_rot) {
                            AnimationPlayerEditor::singleton->get_track_editor()
                                ->insert_node_value_key(
                                    F->get(),
                                    "rotation_degrees",
                                    Math::rad2deg(F->get()->get_rotation()),
                                    p_on_existing
                                );
                        }
                        if (key_scale) {
...
void CanvasItemEditor::_update_override_camera_button(bool p_game_running) {
    if (p_game_running) {
        ...
        override_camera_button->set_tooltip(
            TTR("Project Camera Override\nOverrides the running project's "
                "camera with the editor viewport camera.")
        );
    } else {
        override_camera_button->set_disabled(true);
        override_camera_button->set_pressed(false);
        override_camera_button->set_tooltip(
            TTR("Project Camera Override\nNo project instance running. Run the "
                "project from the editor to use this feature.")
        );
    }
}
...
void CanvasItemEditor::_popup_callback(int p_op) {
    ...
    switch (p_op) {
        ...
        case SNAP_USE_NODE_PARENT: {
            ...
            int idx =
                smartsnap_config_popup->get_item_index(SNAP_USE_NODE_PARENT);
        ...
        case SNAP_USE_ROTATION: {
            ...
            int idx =
                snap_config_menu->get_popup()->get_item_index(SNAP_USE_ROTATION
                );
        ...
        case SNAP_USE_SCALE: {
            ...
            int idx =
                snap_config_menu->get_popup()->get_item_index(SNAP_USE_SCALE);
            snap_config_menu->get_popup()->set_item_checked(idx, snap_scale);
        } break;
        case SNAP_RELATIVE: {
            snap_relative = !snap_relative;
            int idx =
                snap_config_menu->get_popup()->get_item_index(SNAP_RELATIVE);
        ...
        case SNAP_USE_PIXEL: {
            ...
            int idx =
                snap_config_menu->get_popup()->get_item_index(SNAP_USE_PIXEL);
        ...
        case SKELETON_SHOW_BONES: {
            ...
            int idx =
                skeleton_menu->get_popup()->get_item_index(SKELETON_SHOW_BONES);
            skeleton_menu->get_popup()->set_item_checked(
                idx,
                skeleton_show_bones
            );
            viewport->update();
        ...
        case UNLOCK_SELECTED: {
            ...
            for (List<Node*>::Element* E = selection.front(); E;
                ...
                undo_redo
                    ->add_do_method(canvas_item, "remove_meta", "_edit_lock_");
                undo_redo->add_undo_method(
                    canvas_item,
                    "set_meta",
                    "_edit_lock_",
        ...
        case UNGROUP_SELECTED: {
            ...
            for (List<Node*>::Element* E = selection.front(); E;
                ...
                undo_redo
                    ->add_do_method(canvas_item, "remove_meta", "_edit_group_");
                undo_redo->add_undo_method(
                    canvas_item,
                    "set_meta",
                    "_edit_group_",
        ...
        case ANIM_PASTE_POSE: {
            ...
            for (List<PoseClipboard>::Element* E = pose_clipboard.front(); E;
                 E                               = E->next()) {
                Node2D* n2d =
                    Object::cast_to<Node2D>(ObjectDB::get_instance(E->get().id)
                    );
                if (!n2d) {
                    continue;
                }
                ...
                undo_redo
                    ->add_undo_method(n2d, "set_position", n2d->get_position());
                undo_redo
                    ->add_undo_method(n2d, "set_rotation", n2d->get_rotation());
        ...
        case CLEAR_GUIDES: {
            ...
            if (root
                && (root->has_meta("_edit_horizontal_guides_")
        ...
        case SKELETON_SET_IK_CHAIN: {
            ...
            for (List<Node*>::Element* E = selection.front(); E;
                ...
                if (canvas_item->has_meta("_edit_ik_")
                    && canvas_item->get_meta("_edit_ik_")) {
                    continue;
                }

                undo_redo
                    ->add_do_method(canvas_item, "set_meta", "_edit_ik_", true);
                undo_redo
                    ->add_undo_method(canvas_item, "remove_meta", "_edit_ik_");
            }
            undo_redo->add_do_method(viewport, "update");
            undo_redo->add_undo_method(viewport, "update");
            undo_redo->commit_action();
...
void CanvasItemEditor::_focus_selection(int p_op) {
    ...
    for (Map<Node*, Object*>::Element* E = selection.front(); E;
        ...
        Rect2 canvas_item_rect(
            pos + scale * item_rect.position,
            scale * item_rect.size
        );
        if (count == 1) {
    ...
    if (p_op == VIEW_CENTER_TO_SELECTION) {
        ...
        Vector2 offset =
            viewport->get_size() / 2
            - editor->get_scene_root()->get_global_canvas_transform().xform(
                center
            );
...
void CanvasItemEditor::_bind_methods() {
    ClassDB::bind_method(
        "_button_zoom_minus",
        &CanvasItemEditor::_button_zoom_minus
    );
    ClassDB::bind_method(
        "_button_zoom_reset",
        &CanvasItemEditor::_button_zoom_reset
    );
    ClassDB::bind_method(
        "_button_zoom_plus",
        &CanvasItemEditor::_button_zoom_plus
    );
    ClassDB::bind_method(
        "_button_toggle_smart_snap",
        &CanvasItemEditor::_button_toggle_smart_snap
    );
    ClassDB::bind_method(
        "_button_toggle_grid_snap",
        &CanvasItemEditor::_button_toggle_grid_snap
    );
    ClassDB::bind_method(
        D_METHOD("_button_override_camera", "pressed"),
        &CanvasItemEditor::_button_override_camera
    );
    ClassDB::bind_method(
        D_METHOD("_update_override_camera_button", "game_running"),
        &CanvasItemEditor::_update_override_camera_button
    );
    ClassDB::bind_method(
        "_button_toggle_anchor_mode",
        &CanvasItemEditor::_button_toggle_anchor_mode
    );
    ClassDB::bind_method("_update_scroll", &CanvasItemEditor::_update_scroll);
    ClassDB::bind_method(
        "_update_scrollbars",
        &CanvasItemEditor::_update_scrollbars
    );
    ClassDB::bind_method("_popup_callback", &CanvasItemEditor::_popup_callback);
    ClassDB::bind_method(
        "_get_editor_data",
        &CanvasItemEditor::_get_editor_data
    );
    ClassDB::bind_method(
        "_button_tool_select",
        &CanvasItemEditor::_button_tool_select
    );
    ClassDB::bind_method("_keying_changed", &CanvasItemEditor::_keying_changed);
    ClassDB::bind_method(
        "_unhandled_key_input",
        &CanvasItemEditor::_unhandled_key_input
    );
    ...
    ClassDB::bind_method(
        "_queue_update_bone_list",
        &CanvasItemEditor::_update_bone_list
    );
    ClassDB::bind_method(
        "_update_bone_list",
        &CanvasItemEditor::_update_bone_list
    ...
    ClassDB::bind_method(
        "_selection_changed",
        &CanvasItemEditor::_selection_changed
    );
    ClassDB::bind_method(
        "_popup_warning_depop",
        &CanvasItemEditor::_popup_warning_depop
    );
    ClassDB::bind_method(
        "_add_node_pressed",
        &CanvasItemEditor::_add_node_pressed
    );
    ClassDB::bind_method("_node_created", &CanvasItemEditor::_node_created);
    ClassDB::bind_method(
        "_reset_create_position",
        &CanvasItemEditor::_reset_create_position
    );
    ClassDB::bind_method(
        D_METHOD("_selection_result_pressed"),
        &CanvasItemEditor::_selection_result_pressed
    );
    ClassDB::bind_method(
        D_METHOD("_selection_menu_hide"),
        &CanvasItemEditor::_selection_menu_hide
    );
    ClassDB::bind_method(D_METHOD("get_state"), &CanvasItemEditor::get_state);
    ClassDB::bind_method(D_METHOD("set_state"), &CanvasItemEditor::set_state);
    ClassDB::bind_method(
        D_METHOD("update_viewport"),
        &CanvasItemEditor::update_viewport
    );
    ClassDB::bind_method(
        D_METHOD("_zoom_on_position"),
        &CanvasItemEditor::_zoom_on_position
    );
...
void CanvasItemEditor::set_state(const Dictionary& p_state) {
    ...
    if (state.has("zoom")) {
        ...
        // and the zoom level will still be the same (relative to the editor
        // scale).
...
void CanvasItemEditor::add_control_to_info_overlay(Control* p_control) {
    ...
    p_control->set_h_size_flags(
        p_control->get_h_size_flags() & ~Control::SIZE_EXPAND_FILL
    );
...
CanvasItemEditor::CanvasItemEditor(EditorNode* p_editor) {
    ...
    previous_update_view_offset =
        view_offset; // Moves the view a little bit to the left so that (0,0) is
                     // visible. The values a relative to a 16/10 screen
    grid_offset = Point2();
    ...
    snap_rotation_step   = 15 / (180 / static_cast<float>(Math_PI));
    ...
    snap_scale           = false;
    snap_relative        = false;
    // Enable pixel snapping even if pixel snap rendering is disabled in the
    // Project Settings. This results in crisper visuals by preventing 2D nodes
    // from being placed at subpixel coordinates.
    snap_pixel           = true;
    snap_target[0]       = SNAP_TARGET_NONE;
    snap_target[1]       = SNAP_TARGET_NONE;
    ...
    editor->get_scene_tree_dock()
        ->connect("node_created", this, "_node_created");
    editor->get_scene_tree_dock()
        ->connect("add_node_used", this, "_reset_create_position");

    editor->call_deferred(
        "connect",
        "play_pressed",
        this,
    ...
    warning_child_of_container->set_text(
        TTR("Warning: Children of a container get their position and size "
            "determined only by their parent.")
    );
    ...
    zoom_minus->set_shortcut(ED_SHORTCUT(
        "canvas_item_editor/zoom_minus",
        TTR("Zoom Out"),
        KEY_MASK_CMD | KEY_MINUS
    ));
    ...
    zoom_reset->set_shortcut(ED_SHORTCUT(
    ...
    zoom_plus->set_shortcut(ED_SHORTCUT(
        "canvas_item_editor/zoom_plus",
        TTR("Zoom In"),
        KEY_MASK_CMD | KEY_EQUAL
    )); // Usually direct access key for PLUS
    ...
    select_button->connect(
        ...
        this,
        "_button_tool_select",
        make_binds(TOOL_SELECT)
    );
    select_button->set_pressed(true);
    select_button->set_shortcut(
        ED_SHORTCUT("canvas_item_editor/select_mode", TTR("Select Mode"), KEY_Q)
    );
    select_button->set_tooltip(
        keycode_get_string(KEY_MASK_CMD)
        + TTR("Drag: Rotate selected node around pivot.") + "\n"
        + TTR("Alt+Drag: Move selected node.") + "\n"
        + TTR("V: Set selected node's pivot position.") + "\n"
        + TTR("Alt+RMB: Show list of all nodes at position clicked, including "
              "locked.")
        + "\n" + keycode_get_string(KEY_MASK_CMD)
    ...
    move_button->connect(
        ...
        this,
        "_button_tool_select",
        make_binds(TOOL_MOVE)
    );
    move_button->set_shortcut(
        ED_SHORTCUT("canvas_item_editor/move_mode", TTR("Move Mode"), KEY_W)
    );
    ...
    rotate_button->connect(
        "pressed",
        this,
        "_button_tool_select",
        make_binds(TOOL_ROTATE)
    );
    rotate_button->set_shortcut(
        ED_SHORTCUT("canvas_item_editor/rotate_mode", TTR("Rotate Mode"), KEY_E)
    );
    ...
    scale_button->connect(
        "pressed",
        this,
        "_button_tool_select",
        make_binds(TOOL_SCALE)
    );
    scale_button->set_shortcut(
        ED_SHORTCUT("canvas_item_editor/scale_mode", TTR("Scale Mode"), KEY_S)
    );
    ...
    list_select_button->set_tooltip(
        TTR("Show a list of all objects at the position clicked\n(same as "
            "Alt+RMB in select mode).")
    );

    pivot_button = memnew(ToolButton);
    hb->add_child(pivot_button);
    pivot_button->set_toggle_mode(true);
    pivot_button->connect(
        "pressed",
        this,
        "_button_tool_select",
        make_binds(TOOL_EDIT_PIVOT)
    );
    ...
    pan_button
        ->connect("pressed", this, "_button_tool_select", make_binds(TOOL_PAN));
    pan_button->set_shortcut(
        ED_SHORTCUT("canvas_item_editor/pan_mode", TTR("Pan Mode"), KEY_G)
    );
    ...
    ruler_button->set_shortcut(
        ED_SHORTCUT("canvas_item_editor/ruler_mode", TTR("Ruler Mode"), KEY_R)
    );
    ...
    smart_snap_button->set_shortcut(ED_SHORTCUT(
        "canvas_item_editor/use_smart_snap",
        TTR("Use Smart Snap"),
        KEY_MASK_SHIFT | KEY_S
    ));
    ...
    grid_snap_button->set_shortcut(ED_SHORTCUT(
        "canvas_item_editor/use_grid_snap",
        TTR("Use Grid Snap"),
        KEY_MASK_SHIFT | KEY_G
    ));
    ...
    p->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/use_rotation_snap",
            TTR("Use Rotation Snap")
        ),
        SNAP_USE_ROTATION
    );
    p->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/use_scale_snap", TTR("Use Scale Snap")),
        SNAP_USE_SCALE
    );
    p->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/snap_relative", TTR("Snap Relative")),
        SNAP_RELATIVE
    );
    p->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/use_pixel_snap", TTR("Use Pixel Snap")),
        SNAP_USE_PIXEL
    );
    ...
    smartsnap_config_popup->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/snap_node_parent",
            TTR("Snap to Parent")
        ),
    ...
    smartsnap_config_popup->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/snap_node_anchors",
            TTR("Snap to Node Anchor")
        ),
        SNAP_USE_NODE_ANCHORS
    );
    smartsnap_config_popup->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/snap_node_sides",
            TTR("Snap to Node Sides")
        ),
        SNAP_USE_NODE_SIDES
    );
    smartsnap_config_popup->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/snap_node_center",
            TTR("Snap to Node Center")
        ),
        SNAP_USE_NODE_CENTER
    );
    smartsnap_config_popup->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/snap_other_nodes",
            TTR("Snap to Other Nodes")
        ),
        SNAP_USE_OTHER_NODES
    );
    smartsnap_config_popup->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/snap_guides", TTR("Snap to Guides")),
        SNAP_USE_GUIDES
    );
    ...
    lock_button
        ->connect("pressed", this, "_popup_callback", varray(LOCK_SELECTED));
    lock_button->set_tooltip(
        TTR("Lock the selected object in place (can't be moved).")
    );

    unlock_button = memnew(ToolButton);
    hb->add_child(unlock_button);
    unlock_button
        ->connect("pressed", this, "_popup_callback", varray(UNLOCK_SELECTED));
    ...
    group_button
        ->connect("pressed", this, "_popup_callback", varray(GROUP_SELECTED));
    group_button->set_tooltip(
        TTR("Makes sure the object's children are not selectable.")
    );

    ungroup_button = memnew(ToolButton);
    hb->add_child(ungroup_button);
    ungroup_button
        ->connect("pressed", this, "_popup_callback", varray(UNGROUP_SELECTED));
    ungroup_button->set_tooltip(
        TTR("Restores the object's children's ability to be selected.")
    );
    ...
    p->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/skeleton_show_bones",
            TTR("Show Bones")
        ),
        SKELETON_SHOW_BONES
    );
    ...
    p->add_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/skeleton_set_ik_chain",
            TTR("Make IK Chain")
        ),
    ...
    p->add_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/skeleton_clear_ik_chain",
            TTR("Clear IK Chain")
        ),
    ...
    p->add_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/skeleton_clear_bones",
            TTR("Clear Custom Bones")
        ),
    ...
    p->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/show_rulers", TTR("Show Rulers")),
        SHOW_RULERS
    );
    p->add_check_shortcut(
        ED_SHORTCUT(
    ...
    p->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/show_origin", TTR("Show Origin")),
        SHOW_ORIGIN
    );
    p->add_check_shortcut(
        ED_SHORTCUT("canvas_item_editor/show_viewport", TTR("Show Viewport")),
        SHOW_VIEWPORT
    );
    p->add_check_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/show_edit_locks",
            TTR("Show Group And Lock Icons")
        ),
        SHOW_EDIT_LOCKS
    );
    ...
    p->add_shortcut(
        ED_SHORTCUT(
            ...
            TTR("Frame Selection"),
            KEY_MASK_SHIFT | KEY_F
        ),
        VIEW_FRAME_TO_SELECTION
    );
    p->add_shortcut(
        ED_SHORTCUT("canvas_item_editor/clear_guides", TTR("Clear Guides")),
        CLEAR_GUIDES
    );
    p->add_separator();
    p->add_check_shortcut(
    ...
    // Use a custom stylebox to make contextual menu items stand out from the
    // rest. This helps with editor usability as contextual menu items change
    // when selecting nodes, even though it may not be immediately obvious at
    // first.
    ...
    key_loc_button
        ->connect("pressed", this, "_popup_callback", varray(ANIM_INSERT_POS));
    ...
    key_rot_button
        ->connect("pressed", this, "_popup_callback", varray(ANIM_INSERT_ROT));
    ...
    key_scale_button->connect(
        "pressed",
        this,
        "_popup_callback",
        varray(ANIM_INSERT_SCALE)
    );
    ...
    key_insert_button
        ->connect("pressed", this, "_popup_callback", varray(ANIM_INSERT_KEY));
    key_insert_button->set_tooltip(TTR("Insert keys (based on mask)."));
    key_insert_button->set_shortcut(ED_SHORTCUT(
    ...
    // key_auto_insert_button->connect("pressed", this, "_popup_callback",
    // varray(ANIM_INSERT_KEY));
    key_auto_insert_button->set_tooltip(
        TTR("Auto insert keys when objects are translated, rotated or scaled "
            "(based on mask).\nKeys are only added to existing tracks, no new "
            "tracks will be created.\nKeys must be inserted manually for the "
            "first time.")
    );
    key_auto_insert_button->set_shortcut(ED_SHORTCUT(
        "canvas_item_editor/anim_auto_insert_key",
        TTR("Auto Insert Key")
    ));
    ...
    p->add_shortcut(
        ED_GET_SHORTCUT("canvas_item_editor/anim_insert_key"),
        ANIM_INSERT_KEY
    );
    p->add_shortcut(
        ED_SHORTCUT(
            "canvas_item_editor/anim_insert_key_existing_tracks",
            TTR("Insert Key (Existing Tracks)"),
            KEY_MASK_CMD + KEY_INSERT
        ),
        ANIM_INSERT_KEY_EXISTING
    );
    p->add_separator();
    p->add_shortcut(
        ED_SHORTCUT("canvas_item_editor/anim_copy_pose", TTR("Copy Pose")),
        ANIM_COPY_POSE
    );
    p->add_shortcut(
        ED_SHORTCUT("canvas_item_editor/anim_paste_pose", TTR("Paste Pose")),
        ANIM_PASTE_POSE
    );
    p->add_shortcut(
        ED_SHORTCUT(
    ...
    add_node_menu->add_icon_item(
        editor->get_scene_tree_dock()->get_icon("Add", "EditorIcons"),
        TTR("Add Node Here")
    );
    add_node_menu->add_icon_item(
        editor->get_scene_tree_dock()->get_icon("Instance", "EditorIcons"),
        TTR("Instance Scene Here")
    );
    add_node_menu->connect("id_pressed", this, "_add_node_pressed");

    multiply_grid_step_shortcut = ED_SHORTCUT(
        "canvas_item_editor/multiply_grid_step",
        TTR("Multiply grid step by 2"),
        KEY_KP_MULTIPLY
    );
    divide_grid_step_shortcut = ED_SHORTCUT(
        "canvas_item_editor/divide_grid_step",
        TTR("Divide grid step by 2"),
        KEY_KP_DIVIDE
    );
    pan_view_shortcut =
        ED_SHORTCUT("canvas_item_editor/pan_view", TTR("Pan View"), KEY_SPACE);
    ...
    // To ensure that scripts can parse the list of shortcuts correctly, we have
    // to define those shortcuts one by one. Resetting zoom to 100% is a
    // duplicate shortcut of `canvas_item_editor/reset_zoom`, but it ensures
    // both 1 and Ctrl + 0 can be used to reset zoom.
    ED_SHORTCUT(
        "canvas_item_editor/zoom_3.125_percent",
        TTR("Zoom to 3.125%"),
        KEY_MASK_SHIFT | KEY_5
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_6.25_percent",
        TTR("Zoom to 6.25%"),
        KEY_MASK_SHIFT | KEY_4
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_12.5_percent",
        TTR("Zoom to 12.5%"),
        KEY_MASK_SHIFT | KEY_3
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_25_percent",
        TTR("Zoom to 25%"),
        KEY_MASK_SHIFT | KEY_2
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_50_percent",
        TTR("Zoom to 50%"),
        KEY_MASK_SHIFT | KEY_1
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_100_percent",
        TTR("Zoom to 100%"),
        KEY_1
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_200_percent",
        TTR("Zoom to 200%"),
        KEY_2
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_400_percent",
        TTR("Zoom to 400%"),
        KEY_3
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_800_percent",
        TTR("Zoom to 800%"),
        KEY_4
    );
    ED_SHORTCUT(
        "canvas_item_editor/zoom_1600_percent",
        TTR("Zoom to 1600%"),
        KEY_5
    );
...
void CanvasItemEditorPlugin::make_visible(bool p_visible) {
    if (p_visible) {
        ...
        VisualServer::get_singleton()->viewport_set_hide_canvas(
            editor->get_scene_root()->get_viewport_rid(),
            false
        );
...
void CanvasItemEditorViewport::_on_change_type_confirmed() {
    ...
    CheckBox* check =
        Object::cast_to<CheckBox>(button_group->get_pressed_button());
...
void CanvasItemEditorViewport::_create_preview(const Vector<String>& files
) const {
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        Ref<PackedScene> scene =
            Ref<PackedScene>(Object::cast_to<PackedScene>(*res));
...
void CanvasItemEditorViewport::_create_nodes(
    Node* parent,
    Node* child,
    String& path,
    ...
    Ref<Texture> texture =
        Ref<Texture>(Object::cast_to<Texture>(ResourceCache::get(path)));
    Size2 texture_size = texture->get_size();

    if (parent) {
        editor_data->get_undo_redo().add_do_method(parent, "add_child", child);
        editor_data->get_undo_redo()
            .add_do_method(child, "set_owner", editor->get_edited_scene());
        editor_data->get_undo_redo().add_do_reference(child);
        editor_data->get_undo_redo()
            .add_undo_method(parent, "remove_child", child);
    } else { // if we haven't parent, lets try to make a child as a parent.
        editor_data->get_undo_redo()
            .add_do_method(editor, "set_edited_scene", child);
        editor_data->get_undo_redo()
            .add_do_method(child, "set_owner", editor->get_edited_scene());
        editor_data->get_undo_redo().add_do_reference(child);
        editor_data->get_undo_redo()
            .add_undo_method(editor, "set_edited_scene", (Object*)nullptr);
    }

    if (parent) {
        String new_name = parent->validate_child_name(child);
        ScriptEditorDebugger* sed =
            ScriptEditor::get_singleton()->get_debugger();
        editor_data->get_undo_redo().add_do_method(
            sed,
            "live_debug_create_node",
            editor->get_edited_scene()->get_path_to(parent),
            child->get_class(),
            new_name
        );
        editor_data->get_undo_redo().add_undo_method(
            sed,
            "live_debug_remove_node",
            NodePath(
                String(editor->get_edited_scene()->get_path_to(parent)) + "/"
                + new_name
            )
        );
    }
    ...
    if (default_type == "NinePatchRect") {
        editor_data->get_undo_redo()
            .add_do_property(child, "rect/size", texture_size);
    ...
    // there's nothing to be used as source position so snapping will work as
    // absolute if enabled
    target_position = canvas_item_editor->snap_point(target_position);
    editor_data->get_undo_redo()
        .add_do_method(child, "set_global_position", target_position);
}
...
bool CanvasItemEditorViewport::_create_instance(
    ...
    Node* instanced_scene =
        sdata->instance(PackedScene::GEN_EDIT_STATE_INSTANCE);
    if (!instanced_scene) { // error on instancing
        return false;
    }

    if (editor->get_edited_scene()->get_filename()
        != "") { // cyclical instancing
        if (_cyclical_dependency_exists(
                editor->get_edited_scene()->get_filename(),
                instanced_scene
            )) {
    ...
    editor_data->get_undo_redo()
        .add_do_method(parent, "add_child", instanced_scene);
    editor_data->get_undo_redo().add_do_method(
        instanced_scene,
        "set_owner",
        editor->get_edited_scene()
    );
    editor_data->get_undo_redo().add_do_reference(instanced_scene);
    editor_data->get_undo_redo()
        .add_undo_method(parent, "remove_child", instanced_scene);

    String new_name           = parent->validate_child_name(instanced_scene);
    ScriptEditorDebugger* sed = ScriptEditor::get_singleton()->get_debugger();
    editor_data->get_undo_redo().add_do_method(
        sed,
        "live_debug_instance_node",
        editor->get_edited_scene()->get_path_to(parent),
        path,
        new_name
    );
    ...
    if (parent_ci) {
        Vector2 target_pos =
            canvas_item_editor->get_canvas_transform().affine_inverse().xform(
                p_point
            );
        target_pos = canvas_item_editor->snap_point(target_pos);
        target_pos = parent_ci->get_global_transform_with_canvas()
                         .affine_inverse()
                         .xform(target_pos);
        ...
        editor_data->get_undo_redo()
            .add_do_method(instanced_scene, "set_position", target_pos);
    }
...
void CanvasItemEditorViewport::_perform_drop_data() {
    ...
    for (int i = 0; i < selected_files.size(); i++) {
        ...
        if (scene != nullptr && scene.is_valid()) {
            if (!target_node) {
                ...
                Error err =
                    EditorNode::get_singleton()->load_scene(path, false, true);
                if (err != OK) {
    ...
    if (error_files.size() > 0) {
        ...
        accept->set_text(
            vformat(TTR("Error instancing scene from %s"), files_str.c_str())
        );
        accept->popup_centered_minsize();
    }
}

bool CanvasItemEditorViewport::can_drop_data(
    const Point2& p_point,
    ...
    if (d.has("type")) {
        if (String(d["type"]) == "files") {
            ...
            for (int i = 0; i < files.size();
                 i++) { // check if dragged files contain resource or scene can
                        // be created at least once
...
bool CanvasItemEditorViewport::_only_packed_scenes_selected() const {
    for (int i = 0; i < selected_files.size(); ++i) {
        if (ResourceLoader::load(selected_files[i])->get_class()
            != "PackedScene") {
            return false;
        }
    }
...
void CanvasItemEditorViewport::drop_data(
    const Point2& p_point,
    const Variant& p_data
) {
...
void CanvasItemEditorViewport::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_on_select_type"),
        &CanvasItemEditorViewport::_on_select_type
    );
    ClassDB::bind_method(
        D_METHOD("_on_change_type_confirmed"),
        &CanvasItemEditorViewport::_on_change_type_confirmed
    );
    ClassDB::bind_method(
        D_METHOD("_on_change_type_closed"),
        &CanvasItemEditorViewport::_on_change_type_closed
    );
    ClassDB::bind_method(
        D_METHOD("_on_mouse_exit"),
        &CanvasItemEditorViewport::_on_mouse_exit
    );
}
...
CanvasItemEditorViewport::CanvasItemEditorViewport(
    ...
    label_desc->set_text(
        TTR("Drag & drop + Shift : Add node as sibling\nDrag & drop + Alt : "
            "Change node type")
    );
    label_desc->add_color_override("font_color", Color(0.6f, 0.6f, 0.6f, 1));
    label_desc->add_color_override(
        "font_color_shadow",
