...
void draw_margin_line(Control* edit_draw, Vector2 from, Vector2 to) {
    ...
    // Draw a translucent background line to make the foreground line visible on
    // any background.
    edit_draw->draw_line(
        from,
        to,
    ...
    while ((to - from).length_squared() > 200) {
        edit_draw->draw_line(
            from,
            from + line,
            EditorNode::get_singleton()->get_theme_base()->get_color(
                "mono_color",
                "Editor"
            ),
            Math::round(2 * EDSCALE)
...
void TextureRegionEditor::_region_draw() {
        ...
        if (snap_step.x != 0) {
            if (snap_separation.x == 0) {
                for (int i = 0; i < s.width; i++) {
                    int cell = Math::fast_ftoi(Math::floor(
                        (mtx.affine_inverse().xform(Vector2(i, 0)).x
                         - snap_offset.x)
                        / snap_step.x
                    ));
                    if (i == 0) {
                ...
                for (int i = 0; i < s.width; i++) {
                    ...
                    if (last_cell != cell) {
                        edit_draw->draw_rect(
                            Rect2(
                                i - snap_separation.x * draw_zoom,
                                0,
                                snap_separation.x * draw_zoom,
                                s.height
                            ),
                            grid_color
                        );
        ...
        if (snap_step.y != 0) {
            if (snap_separation.y == 0) {
                for (int i = 0; i < s.height; i++) {
                    int cell = Math::fast_ftoi(Math::floor(
                        (mtx.affine_inverse().xform(Vector2(0, i)).y
                         - snap_offset.y)
                        / snap_step.y
                    ));
                    if (i == 0) {
                ...
                for (int i = 0; i < s.height; i++) {
                    ...
                    if (last_cell != cell) {
                        edit_draw->draw_rect(
                            Rect2(
                                0,
                                i - snap_separation.y * draw_zoom,
                                s.width,
                                snap_separation.y * draw_zoom
                            ),
                            grid_color
                        );
                    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (snap_mode != SNAP_AUTOSLICE) {
            edit_draw->draw_texture(
                select_handle,
                (endpoints[i] + ofs - (select_handle->get_size() / 2)).floor()
                    - draw_ofs * draw_zoom
            );
        }
    ...
    if (ABS(scroll_rect.position.x
            - (scroll_rect.position.x + scroll_rect.size.x))
        <= scroll_margin.x) {
        hscroll->hide();
    } else {
        hscroll->show();
    ...
    if (ABS(scroll_rect.position.y
            - (scroll_rect.position.y + scroll_rect.size.y))
        <= scroll_margin.y) {
        vscroll->hide();
    ...
    hscroll->set_anchor_and_margin(
        MARGIN_RIGHT,
        ANCHOR_END,
        vscroll->is_visible() ? -vmin.width : 0
    );
    vscroll->set_anchor_and_margin(
        MARGIN_BOTTOM,
        ANCHOR_END,
        hscroll->is_visible() ? -hmin.height : 0
    );
    ...
    if (node_ninepatch || obj_styleBox.is_valid()) {
        ...
        Vector2 pos[4] = {
            mtx.basis_xform(Vector2(0, margins[0]))
                + Vector2(0, endpoints[0].y - draw_ofs.y * draw_zoom),
            -mtx.basis_xform(Vector2(0, margins[1]))
                + Vector2(0, endpoints[2].y - draw_ofs.y * draw_zoom),
            mtx.basis_xform(Vector2(margins[2], 0))
                + Vector2(endpoints[0].x - draw_ofs.x * draw_zoom, 0),
            -mtx.basis_xform(Vector2(margins[3], 0))
                + Vector2(endpoints[2].x - draw_ofs.x * draw_zoom, 0)
        };

        draw_margin_line(
            edit_draw,
            pos[0],
            pos[0] + Vector2(edit_draw->get_size().x, 0)
        );
        draw_margin_line(
            edit_draw,
            pos[1],
            pos[1] + Vector2(edit_draw->get_size().x, 0)
        );
        draw_margin_line(
            edit_draw,
            pos[2],
            pos[2] + Vector2(0, edit_draw->get_size().y)
        );
        draw_margin_line(
            edit_draw,
            pos[3],
            pos[3] + Vector2(0, edit_draw->get_size().y)
        );
    }
}
...
void TextureRegionEditor::_region_input(const Ref<InputEvent>& p_input) {
    ...
    const Vector2 endpoints[8] = {
        ...
        mtx.xform(rect.position + Vector2(rect.size.x / 2, 0))
            + Vector2(0, -handle_offset),
        mtx.xform(rect.position + Vector2(rect.size.x, 0))
            + Vector2(handle_offset, -handle_offset),
        mtx.xform(rect.position + Vector2(rect.size.x, rect.size.y / 2))
            + Vector2(handle_offset, 0),
        mtx.xform(rect.position + rect.size)
            + Vector2(handle_offset, handle_offset),
        mtx.xform(rect.position + Vector2(rect.size.x / 2, rect.size.y))
            + Vector2(0, handle_offset),
        mtx.xform(rect.position + Vector2(0, rect.size.y))
            + Vector2(-handle_offset, handle_offset),
        mtx.xform(rect.position + Vector2(0, rect.size.y / 2))
            + Vector2(-handle_offset, 0)
    };
        ...
        if (mb->get_button_index() == BUTTON_LEFT) {
            if (mb->is_pressed()) {
                if (node_ninepatch || obj_styleBox.is_valid()) {
                    ...
                    if (node_ninepatch) {
                        margins[0] =
                            node_ninepatch->get_patch_margin(MARGIN_TOP);
                        margins[1] =
                            node_ninepatch->get_patch_margin(MARGIN_BOTTOM);
                        margins[2] =
                            node_ninepatch->get_patch_margin(MARGIN_LEFT);
                        margins[3] =
                            node_ninepatch->get_patch_margin(MARGIN_RIGHT);
                    } else if (obj_styleBox.is_valid()) {
                        margins[0] = obj_styleBox->get_margin_size(MARGIN_TOP);
                        margins[1] =
                            obj_styleBox->get_margin_size(MARGIN_BOTTOM);
                        margins[2] = obj_styleBox->get_margin_size(MARGIN_LEFT);
                        margins[3] =
                            obj_styleBox->get_margin_size(MARGIN_RIGHT);
                    }

                    Vector2 pos[4] = {
                        mtx.basis_xform(rect.position + Vector2(0, margins[0]))
                            - draw_ofs * draw_zoom,
                        mtx.basis_xform(
                            rect.position + rect.size - Vector2(0, margins[1])
                        ) - draw_ofs * draw_zoom,
                        mtx.basis_xform(rect.position + Vector2(margins[2], 0))
                            - draw_ofs * draw_zoom,
                        mtx.basis_xform(
                ...
                if (edited_margin < 0 && snap_mode == SNAP_AUTOSLICE) {
                    Vector2 point = mtx.affine_inverse().xform(
                        Vector2(mb->get_position().x, mb->get_position().y)
                    );
                    for (List<Rect2>::Element* E = autoslice_cache.front(); E;
                         E                       = E->next()) {
                        if (E->get().has_point(point)) {
                            ...
                            if (Input::get_singleton()->is_key_pressed(
                                ...
                                && !(Input::get_singleton()->is_key_pressed(
                                    KEY_SHIFT | KEY_ALT
                                ))) {
                                Rect2 r;
                                if (node_sprite) {
                            ...
                            if (node_sprite) {
                                ...
                                undo_redo->add_undo_method(
                                    node_ninepatch,
                                    "set_region_rect",
                                    node_ninepatch->get_region_rect()
                                );
                            } else if (obj_styleBox.is_valid()) {
                                undo_redo->add_do_method(
                                    obj_styleBox.ptr(),
                    ...
                    drag_from = mtx.affine_inverse().xform(
                        Vector2(mb->get_position().x, mb->get_position().y)
                    );
                ...
                if (edited_margin >= 0) {
                    ...
                    if (node_ninepatch) {
                        undo_redo->add_do_method(
                            ...
                            "set_patch_margin",
                            m[edited_margin],
                            node_ninepatch->get_patch_margin(m[edited_margin])
                        );
                        undo_redo->add_undo_method(
                            node_ninepatch,
                            "set_patch_margin",
                        ...
                        undo_redo->add_do_method(
                            ...
                            "set_margin_size",
                            m[edited_margin],
                            obj_styleBox->get_margin_size(m[edited_margin])
                        );
                        undo_redo->add_undo_method(
                            obj_styleBox.ptr(),
                            "set_margin_size",
            ...
            if (drag) {
                ...
                if (edited_margin >= 0) {
                    static Margin m[4] =
                        {MARGIN_TOP, MARGIN_BOTTOM, MARGIN_LEFT, MARGIN_RIGHT};
                    if (node_ninepatch) {
                        node_ninepatch->set_patch_margin(
                            m[edited_margin],
                            prev_margin
                        );
                    }
                    if (obj_styleBox.is_valid()) {
                        obj_styleBox->set_margin_size(
                            m[edited_margin],
                            prev_margin
                        );
                    }
                   ...
                   && mb->is_pressed()) {
            _zoom_on_position(
                draw_zoom * ((0.95 + (0.05 * mb->get_factor())) / 0.95),
                mb->get_position()
            );
        } else if (mb->get_button_index() == BUTTON_WHEEL_DOWN
                   && mb->is_pressed()) {
            _zoom_on_position(
                draw_zoom * (1 - (0.05 * mb->get_factor())),
                mb->get_position()
            );
        }
    }
    ...
    if (mm.is_valid()) {
        if (mm->get_button_mask() & BUTTON_MASK_MIDDLE
            ...
            Vector2 dragged(
                mm->get_relative().x / draw_zoom,
                mm->get_relative().y / draw_zoom
            );
            ...
            if (edited_margin >= 0) {
                ...
                if (snap_mode != SNAP_GRID) {
                    if (edited_margin == 0) {
                        new_margin =
                            prev_margin
                            + (mm->get_position().y - drag_from.y) / draw_zoom;
                        ...
                        new_margin =
                            prev_margin
                            + (mm->get_position().x - drag_from.x) / draw_zoom;
                    ...
                    Vector2 pos_snapped =
                        snap_point(mtx.affine_inverse().xform(mm->get_position()
                        ));
                    Rect2 rect_rounded =
                        Rect2(rect.position.round(), rect.size.round());

                    if (edited_margin == 0) {
                        new_margin = pos_snapped.y - rect_rounded.position.y;
                    } else if (edited_margin == 1) {
                        new_margin = rect_rounded.size.y
                                   + rect_rounded.position.y - pos_snapped.y;
                    } else if (edited_margin == 2) {
                        new_margin = pos_snapped.x - rect_rounded.position.x;
                    } else if (edited_margin == 3) {
                        new_margin = rect_rounded.size.x
                                   + rect_rounded.position.x - pos_snapped.x;
                    } else {
                        ERR_PRINT("Unexpected edited_margin");
                    }
                }
                ...
                Vector2 new_pos =
                    mtx.affine_inverse().xform(mm->get_position());
                if (snap_mode == SNAP_PIXEL) {
                ...
                switch (drag_index) {
                    ...
                    case 1: {
                        Vector2 p =
                            rect_prev.position + Vector2(0, rect_prev.size.y);
                    ...
                    case 6: {
                        Vector2 p =
                            rect_prev.position + Vector2(rect_prev.size.x, 0);
                        rect = Rect2(p, Size2());
                        rect.expand_to(new_pos);
                        apply_rect(rect);
                    } break;
    ...
    if (magnify_gesture.is_valid()) {
        _zoom_on_position(
            draw_zoom * magnify_gesture->get_factor(),
    ...
    if (pan_gesture.is_valid()) {
        hscroll->set_value(
            hscroll->get_value()
            + hscroll->get_page() * pan_gesture->get_delta().x / 8
        );
        vscroll->set_value(
            vscroll->get_value()
            + vscroll->get_page() * pan_gesture->get_delta().y / 8
        );
    }
}
                            ...
                            for (List<Rect2>::Element* F =
                                ...
                                if (E->get().grow(1).intersects(F->get())) {
                                    ...
                                    if (F->prev()) {
                                        ...
                                        queue_erase = true;
                                        // Can't delete the first rect in the
                                        // list.
                                    }
                                    merged = true;
                                }
...
void TextureRegionEditor::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_VISIBILITY_CHANGED: {
            if (snap_mode == SNAP_AUTOSLICE && is_visible()
                && autoslice_is_dirty) {
                _update_autoslice();
            }
        } break;
...
void TextureRegionEditor::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_edit_region"),
        &TextureRegionEditor::_edit_region
    );
    ClassDB::bind_method(
        D_METHOD("_region_draw"),
        &TextureRegionEditor::_region_draw
    );
    ClassDB::bind_method(
        D_METHOD("_region_input"),
        &TextureRegionEditor::_region_input
    );
    ClassDB::bind_method(
        D_METHOD("_scroll_changed"),
        &TextureRegionEditor::_scroll_changed
    );
    ClassDB::bind_method(
        D_METHOD("_node_removed"),
        &TextureRegionEditor::_node_removed
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_mode"),
        &TextureRegionEditor::_set_snap_mode
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_off_x"),
        &TextureRegionEditor::_set_snap_off_x
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_off_y"),
        &TextureRegionEditor::_set_snap_off_y
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_step_x"),
        &TextureRegionEditor::_set_snap_step_x
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_step_y"),
        &TextureRegionEditor::_set_snap_step_y
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_sep_x"),
        &TextureRegionEditor::_set_snap_sep_x
    );
    ClassDB::bind_method(
        D_METHOD("_set_snap_sep_y"),
        &TextureRegionEditor::_set_snap_sep_y
    );
    ClassDB::bind_method(
        D_METHOD("_zoom_on_position"),
        &TextureRegionEditor::_zoom_on_position
    );
    ClassDB::bind_method(D_METHOD("_zoom_in"), &TextureRegionEditor::_zoom_in);
    ClassDB::bind_method(
        D_METHOD("_zoom_reset"),
        &TextureRegionEditor::_zoom_reset
    );
    ClassDB::bind_method(
        D_METHOD("_zoom_out"),
        &TextureRegionEditor::_zoom_out
    );
    ClassDB::bind_method(
        D_METHOD("_update_rect"),
        &TextureRegionEditor::_update_rect
    );
}
...
void TextureRegionEditor::edit(Object* p_obj) {
    ...
    if (p_obj) {
        ...
        if (Object::cast_to<StyleBoxTexture>(p_obj)) {
            obj_styleBox =
                Ref<StyleBoxTexture>(Object::cast_to<StyleBoxTexture>(p_obj));
        }
    ...
    if ((node_sprite && !node_sprite->is_region())
        || (node_sprite_3d && !node_sprite_3d->is_region())) {
        set_process(true);
    }
    if (!p_obj) {
        set_process(false);
...
void TextureRegionEditor::_changed_callback(
    Object* p_changed,
    const char* p_prop
) {
    if (!is_visible()) {
...
Vector2 TextureRegionEditor::snap_point(Vector2 p_target) const {
    if (snap_mode == SNAP_GRID) {
        p_target.x = Math::snap_scalar_separation(
            snap_offset.x,
            snap_step.x,
            p_target.x,
            snap_separation.x
        );
        p_target.y = Math::snap_scalar_separation(
            snap_offset.y,
            snap_step.y,
            p_target.y,
            snap_separation.y
...
bool TextureRegionEditorPlugin::handles(Object* p_object) const {
    return p_object->is_class("Sprite") || p_object->is_class("Sprite3D")
        || p_object->is_class("NinePatchRect")
        || p_object->is_class("StyleBoxTexture")
        || p_object->is_class("AtlasTexture");
}
...
void TextureRegionEditorPlugin::make_visible(bool p_visible) {
    if (p_visible) {
        ...
        bool is_node_configured =
            region_editor->is_stylebox() || region_editor->is_atlas_texture()
            || region_editor->is_ninepatch()
            || (region_editor->get_sprite()
                && region_editor->get_sprite()->is_region())
            || (region_editor->get_sprite_3d()
                && region_editor->get_sprite_3d()->is_region());
        if ((is_node_configured && !manually_hidden)
            || texture_region_button->is_pressed()) {
...
void TextureRegionEditorPlugin::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_editor_visiblity_changed"),
        &TextureRegionEditorPlugin::_editor_visiblity_changed
    );
}
...
TextureRegionEditorPlugin::TextureRegionEditorPlugin(EditorNode* p_node) {
    ...
    region_editor->hide();
    region_editor
        ->connect("visibility_changed", this, "_editor_visiblity_changed");
