...
Control* VisualShaderNodePlugin::create_editor(
    const Ref<Resource>& p_parent_resource,
    const Ref<VisualShaderNode>& p_node
) {
    if (get_script_instance()) {
        return get_script_instance()
            ->call("create_editor", p_parent_resource, p_node);
    }
    return nullptr;
}
...
void VisualShaderEditor::remove_plugin(
    const Ref<VisualShaderNodePlugin>& p_plugin
) {
...
void VisualShaderEditor::add_custom_type(
    const String& p_name,
    const Ref<Script>& p_script,
    const String& p_description,
    int p_return_icon_type,
    const String& p_category,
    const String& p_subcategory
) {
...
void VisualShaderEditor::update_custom_nodes() {
    ...
    for (int i = 0; i < class_list.size(); i++) {
        if (ScriptServer::get_global_class_native_base(class_list[i])
            == "VisualShaderNodeCustom") {
            String script_path =
                ScriptServer::get_global_class_path(class_list[i]);
...
String VisualShaderEditor::_get_description(int p_idx) {
    if (add_options[p_idx].highend) {
        return TTR("(GLES3 only)") + " "
             + add_options[p_idx].description; // TODO: change it to (Vulkan
                                               // Only) when its ready
    } else {
...
void VisualShaderEditor::_update_options_menu() {
    ...
    for (int i = 0; i < add_options.size() + 1; i++) {
        ...
        if (!use_filter || add_options[i].name.findn(filter) != -1) {
            if ((add_options[i].func != current_func
                 && add_options[i].func != -1)
                || !_is_available(add_options[i].mode)) {
                continue;
            }
                ...
                switch (add_options[i].return_type) {
                    ...
                    case VisualShaderNode::PORT_TYPE_TRANSFORM:
                        item->set_icon(
                            ...
                            EditorNode::get_singleton()
                                ->get_gui_base()
                                ->get_icon("Transform", "EditorIcons")
                        );
                        break;
                    case VisualShaderNode::PORT_TYPE_SAMPLER:
                        item->set_icon(
                            ...
                            EditorNode::get_singleton()
                                ->get_gui_base()
                                ->get_icon("ImageTexture", "EditorIcons")
                        );
                        break;
                    default:
                        break;
...
void VisualShaderEditor::_update_graph() {
    ...
    for (int t = 0; t < VisualShader::TYPE_MAX; t++) {
        Vector<int> tnodes =
            visual_shader->get_node_list((VisualShader::Type)t);
        for (int i = 0; i < tnodes.size(); i++) {
            Ref<VisualShaderNode> vsnode =
                visual_shader->get_node((VisualShader::Type)t, tnodes[i]);
            ...
            if (uniform.is_valid()) {
                ...
                Ref<VisualShaderNodeTransformUniform> transform_uniform =
                    vsnode;

                VisualShaderNodeUniformRef::UniformType uniform_type;
                if (scalar_uniform.is_valid()) {
                    uniform_type = VisualShaderNodeUniformRef::UniformType::
                        UNIFORM_TYPE_SCALAR;
                ...
                VisualShaderNodeUniformRef::add_uniform(
                    uniform->get_uniform_name(),
                    uniform_type
                );
            }
        }
    ...
    for (int n_i = 0; n_i < nodes.size(); n_i++) {
        ...
        Ref<VisualShaderNode> vsnode =
            visual_shader->get_node(type, nodes[n_i]);

        Ref<VisualShaderNodeGroupBase> group_node =
            Object::cast_to<VisualShaderNodeGroupBase>(vsnode.ptr());
        bool is_group = !group_node.is_null();
        Size2 size    = Size2(0, 0);

        VisualShaderNodeCustom* custom =
            Object::cast_to<VisualShaderNodeCustom>(vsnode.ptr());
        if (custom) {
            custom->_set_initialized(true);
        }

        Ref<VisualShaderNodeExpression> expression_node =
            Object::cast_to<VisualShaderNodeExpression>(group_node.ptr());
        ...
        /*if (!vsnode->is_connected("changed", this, "_node_changed")) {
            vsnode->connect("changed", this, "_node_changed",
        varray(vsnode->get_instance_id()), CONNECT_DEFERRED);
        }*/
        ...
        if (uniform.is_valid()) {
            ...
            uniform_name->connect(
                "text_entered",
                this,
                "_line_edit_changed",
                varray(uniform_name, nodes[n_i])
            );
            uniform_name->connect(
                "focus_exited",
                this,
                "_line_edit_focus_out",
                varray(uniform_name, nodes[n_i])
            );

            if (vsnode->get_input_port_count() == 0
                && vsnode->get_output_port_count() == 1
                && vsnode->get_output_port_name(0) == "") {
                // shortcut
                VisualShaderNode::PortType port_right =
                    vsnode->get_output_port_type(0);
                node->set_slot(
                    0,
                    false,
        ...
        for (int i = 0; i < plugins.size(); i++) {
            custom_editor =
                plugins.write[i]->create_editor(visual_shader, vsnode);
            if (custom_editor) {
                break;
            }
        }

        if (custom_editor && vsnode->get_output_port_count() > 0
            && vsnode->get_output_port_name(0) == ""
            && (vsnode->get_input_port_count() == 0
                || vsnode->get_input_port_name(0) == "")) {
        ...
        if (is_group) {
            ...
            if (group_node->is_editable()) {
                ...
                add_input_btn->connect(
                    "pressed",
                    this,
                    "_add_input_port",
                    varray(
                        nodes[n_i],
                        group_node->get_free_input_port_id(),
                        VisualShaderNode::PORT_TYPE_VECTOR,
                        "input" + itos(group_node->get_free_input_port_id())
                    ),
                    CONNECT_DEFERRED
                );
                ...
                add_output_btn->connect(
                    "pressed",
                    this,
                    "_add_output_port",
                    varray(
                        nodes[n_i],
                        group_node->get_free_output_port_id(),
                        VisualShaderNode::PORT_TYPE_VECTOR,
                        "output" + itos(group_node->get_free_output_port_id())
                    ),
                    CONNECT_DEFERRED
                );
        ...
        for (int i = 0; i < MAX(
                            vsnode->get_input_port_count(),
                            vsnode->get_output_port_count()
                        );
             i++) {
            ...
            VisualShaderNode::PortType port_left =
                VisualShaderNode::PORT_TYPE_SCALAR;
            ...
            if (valid_left) {
                ...
                for (List<VisualShader::Connection>::Element* E =
                         connections.front();
                     E;
                     E = E->next()) {
                    if (E->get().to_node == nodes[n_i]
                        && E->get().to_port == i) {
                        port_left_used = true;
                    }
                }
            ...
            if (default_value.get_type() != Variant::NIL) { // only a label
                ...
                button->connect(
                    "pressed",
                    this,
                    "_edit_port_default_input",
                    varray(button, nodes[n_i], i)
                );
                ...
                switch (default_value.get_type()) {
                    case Variant::COLOR: {
                        ...
                        button->connect(
                            ...
                            varray(button, default_value)
                        );
                    } break;
                    case Variant::BOOL: {
                        button->set_text(
                            ((bool)default_value) ? "true" : "false"
                        );
                    } break;
                    case Variant::INT:
                    case Variant::REAL: {
                    ...
                    case Variant::VECTOR3: {
                        ...
                        button->set_text(
                            String::num(v.x, 3) + "," + String::num(v.y, 3)
                            + "," + String::num(v.z, 3)
                        );
                    } break;
                    default: {
            ...
            if (i == 0 && custom_editor) {
                ...
                if (valid_left) {
                    if (is_group) {
                        ...
                        type_box->set_custom_minimum_size(
                            Size2(100 * EDSCALE, 0)
                        );
                        type_box->connect(
                            "item_selected",
                            this,
                            "_change_input_port_type",
                            varray(nodes[n_i], i),
                            CONNECT_DEFERRED
                        );
                        ...
                        name_box->connect(
                            "text_entered",
                            this,
                            "_change_input_port_name",
                            varray(name_box, nodes[n_i], i)
                        );
                        name_box->connect(
                            "focus_exited",
                            this,
                            "_port_name_focus_out",
                            varray(name_box, nodes[n_i], i, false)
                        );
                        ...
                        remove_btn->set_tooltip(
                            TTR("Remove") + " " + name_left
                        );
                        remove_btn->connect(
                            "pressed",
                            this,
                        ...
                        if (vsnode->get_input_port_default_hint(i) != ""
                            && !port_left_used) {
                            Label* hint_label = memnew(Label);
                            hint_label->set_text(
                                "[" + vsnode->get_input_port_default_hint(i)
                                + "]"
                            );
                            hint_label->add_color_override(
                                "font_color",
                                get_color("font_color_readonly", "TextEdit")
                            );
                            hint_label->add_style_override(
                                "normal",
                                label_style
                            );
                ...
                if (valid_right) {
                    if (is_group) {
                        ...
                        name_box->connect(
                            ...
                            this,
                            "_port_name_focus_out",
                            varray(name_box, nodes[n_i], i, true)
                        );
            ...
            if (valid_right
                && edit_type->get_selected() == VisualShader::TYPE_FRAGMENT
                && port_right != VisualShaderNode::PORT_TYPE_TRANSFORM
                && port_right != VisualShaderNode::PORT_TYPE_SAMPLER) {
                TextureButton* preview = memnew(TextureButton);
                preview->set_toggle_mode(true);
                preview->set_normal_texture(
                    get_icon("GuiVisibilityHidden", "EditorIcons")
                );
                preview->set_pressed_texture(
        ...
        if (vsnode->get_output_port_for_preview() >= 0) {
            int port_type = vsnode->get_output_port_type(
                vsnode->get_output_port_for_preview()
            );

            if (port_type != VisualShaderNode::PORT_TYPE_TRANSFORM
                && port_type != VisualShaderNode::PORT_TYPE_SAMPLER) {
                offset = memnew(Control);
                offset->set_custom_minimum_size(Size2(0, 5 * EDSCALE));
                node->add_child(offset);

                VisualShaderNodePortPreview* port_preview =
                    memnew(VisualShaderNodePortPreview);
                port_preview->setup(
                    visual_shader,
                    type,
                    nodes[n_i],
        ...
        if (is_expression) {
            ...
            Color background_color =
                EDITOR_GET("text_editor/highlighting/background_color");
            Color text_color =
                EDITOR_GET("text_editor/highlighting/text_color");
            Color keyword_color =
                EDITOR_GET("text_editor/highlighting/keyword_color");
            Color control_flow_keyword_color =
                EDITOR_GET("text_editor/highlighting/control_flow_keyword_color"
                );
            Color comment_color =
                EDITOR_GET("text_editor/highlighting/comment_color");
            Color symbol_color =
                EDITOR_GET("text_editor/highlighting/symbol_color");

            expression_box->set_syntax_coloring(true);
            expression_box->add_color_override(
                "background_color",
                background_color
            );

            for (List<String>::Element* E = keyword_list.front(); E;
                 E                        = E->next()) {
                if (ShaderLanguage::is_control_flow_keyword(E->get())) {
                    expression_box->add_keyword_color(
                        E->get(),
            ...
            expression_box->add_font_override(
                "font",
                get_font("expression", "EditorFonts")
            );
            ...
            expression_box->connect(
                "focus_exited",
                this,
                "_expression_focus_out",
                varray(expression_box, nodes[n_i])
            );
        }
    ...
    for (List<VisualShader::Connection>::Element* E = connections.front(); E;
         E                                          = E->next()) {
...
void VisualShaderEditor::_add_input_port(
    int p_node,
    int p_port,
    int p_port_type,
    const String& p_name
    ...
    VisualShader::Type type = VisualShader::Type(edit_type->get_selected());
    Ref<VisualShaderNodeExpression> node =
        visual_shader->get_node(type, p_node);
    if (node.is_null()) {
        return;
    }
...
void VisualShaderEditor::_add_output_port(
    int p_node,
    int p_port,
    int p_port_type,
    const String& p_name
...
void VisualShaderEditor::_change_input_port_type(
    int p_type,
    int p_node,
    int p_port
) {
    ...
    undo_redo->add_undo_method(
        ...
        "set_input_port_type",
        p_port,
        node->get_input_port_type(p_port)
    );
    undo_redo->add_do_method(this, "_update_graph");
    undo_redo->add_undo_method(this, "_update_graph");
    undo_redo->add_do_method(this, "_rebuild");
...
void VisualShaderEditor::_change_output_port_type(
    int p_type,
    int p_node,
    int p_port
) {
    ...
    undo_redo
        ->add_do_method(node.ptr(), "set_output_port_type", p_port, p_type);
    undo_redo->add_undo_method(
        node.ptr(),
        "set_output_port_type",
        p_port,
        node->get_output_port_type(p_port)
    );
    undo_redo->add_do_method(this, "_update_graph");
    undo_redo->add_undo_method(this, "_update_graph");
    undo_redo->add_do_method(this, "_rebuild");
...
void VisualShaderEditor::_change_input_port_name(
    const String& p_text,
    Object* line_edit,
    int p_node_id,
    int p_port_id
    ...
    Ref<VisualShaderNodeGroupBase> node =
        visual_shader->get_node(type, p_node_id);
    ERR_FAIL_COND(!node.is_valid());

    undo_redo->create_action(TTR("Change input port name"));
    undo_redo
        ->add_do_method(node.ptr(), "set_input_port_name", p_port_id, p_text);
    undo_redo->add_undo_method(
        node.ptr(),
        "set_input_port_name",
        p_port_id,
        node->get_input_port_name(p_port_id)
    );
    undo_redo->add_do_method(this, "_rebuild");
    undo_redo->add_undo_method(this, "_rebuild");
    undo_redo->commit_action();
...
void VisualShaderEditor::_change_output_port_name(
    const String& p_text,
    Object* line_edit,
    int p_node_id,
    int p_port_id
    ...
    undo_redo
        ->add_do_method(node.ptr(), "set_output_port_name", p_port_id, p_text);
    undo_redo->add_undo_method(
        node.ptr(),
        "set_output_port_name",
        p_port_id,
        node->get_output_port_name(p_port_id)
    );
    undo_redo->add_do_method(this, "_rebuild");
    undo_redo->add_undo_method(this, "_rebuild");
    undo_redo->commit_action();
...
void VisualShaderEditor::_remove_input_port(int p_node, int p_port) {
    ...
    for (List<VisualShader::Connection>::Element* E = conns.front(); E;
         E                                          = E->next()) {
    ...
    undo_redo->add_undo_method(
        ...
        "add_input_port",
        p_port,
        (int)node->get_input_port_type(p_port),
        node->get_input_port_name(p_port)
    );
...
void VisualShaderEditor::_remove_output_port(int p_node, int p_port) {
    ...
    undo_redo->add_undo_method(
        ...
        "add_output_port",
        p_port,
        (int)node->get_output_port_type(p_port),
        node->get_output_port_name(p_port)
    );
...
void VisualShaderEditor::_expression_focus_out(Object* text_edit, int p_node) {
    ...
    undo_redo->add_do_method(
        node.ptr(),
        "set_expression",
        expression_box->get_text()
    );
    undo_redo
        ->add_undo_method(node.ptr(), "set_expression", node->get_expression());
    undo_redo->add_do_method(this, "_rebuild");
    undo_redo->add_undo_method(this, "_rebuild");
    undo_redo->commit_action();
}
...
void VisualShaderEditor::_set_node_size(
    int p_type,
    int p_node,
    ...
    Ref<VisualShaderNodeGroupBase> group_node =
        Object::cast_to<VisualShaderNodeGroupBase>(node.ptr());
    ...
    if (edit_type->get_selected()
        == p_type) { // check - otherwise the error will be emitted
    ...
    Ref<VisualShaderNodeExpression> expression_node =
        Object::cast_to<VisualShaderNodeExpression>(node.ptr());
...
void VisualShaderEditor::_node_resized(
    const Vector2& p_new_size,
    int p_type,
    int p_node
) {
    VisualShader::Type type             = VisualShader::Type(p_type);
    ...
    undo_redo->create_action(
        TTR("Resize VisualShader node"),
        UndoRedo::MERGE_ENDS
    );
    undo_redo
        ->add_do_method(this, "_set_node_size", p_type, p_node, p_new_size);
    undo_redo->add_undo_method(
        this,
        "_set_node_size",
        p_type,
...
void VisualShaderEditor::_preview_select_port(int p_node, int p_port) {
    ...
    undo_redo->add_undo_method(
        ...
        node->get_output_port_for_preview()
    );
    undo_redo->add_do_method(this, "_update_graph");
    undo_redo->add_undo_method(this, "_update_graph");
    undo_redo->commit_action();
...
void VisualShaderEditor::_line_edit_changed(
    const String& p_text,
    Object* line_edit,
    int p_node_id
) {
    ...
    Ref<VisualShaderNodeUniform> node =
        visual_shader->get_node(type, p_node_id);
    ERR_FAIL_COND(!node.is_valid());
...
void VisualShaderEditor::_line_edit_focus_out(
    Object* line_edit,
    int p_node_id
) {
...
void VisualShaderEditor::_port_name_focus_out(
    Object* line_edit,
    int p_node_id,
    int p_port_id,
    bool p_output
    ...
    String validated_name =
        visual_shader->validate_port_name(text, input_names, output_names);
    if (validated_name == "") {
        if (!p_output) {
            Object::cast_to<LineEdit>(line_edit)->set_text(
                node->get_input_port_name(p_port_id)
            );
        } else {
            Object::cast_to<LineEdit>(line_edit)->set_text(
...
void VisualShaderEditor::_port_edited() {
    ...
    Ref<VisualShaderNodeCustom> custom =
        Object::cast_to<VisualShaderNodeCustom>(vsn.ptr());
    if (custom.is_valid()) {
        undo_redo->add_do_method(
            custom.ptr(),
        ...
        undo_redo->add_undo_method(
            ...
            "_set_input_port_default_value",
            editing_port,
            vsn->get_input_port_default_value(editing_port)
        );
    } else {
        undo_redo->add_do_method(
            vsn.ptr(),
...
void VisualShaderEditor::_edit_port_default_input(
    Object* p_button,
    int p_node,
    int p_port
) {
    ...
    property_editor->set_global_position(
        button->get_global_position() + Vector2(0, button->get_size().height)
    );
...
void VisualShaderEditor::_add_texture_node(const String& p_path) {
    VisualShaderNodeTexture* texture =
        (VisualShaderNodeTexture*)_add_node(texture_node_option_idx, -1);
...
VisualShaderNode* VisualShaderEditor::_add_node(int p_idx, int p_op_idx) {
    ...
    if (!is_custom && add_options[p_idx].type != String()) {
        ...
        if (p_op_idx != -1) {
            VisualShaderNodeInput* input =
                Object::cast_to<VisualShaderNodeInput>(vsn);
            ...
            VisualShaderNodeCompare* cmp =
                Object::cast_to<VisualShaderNodeCompare>(vsn);

            if (cmp) {
                cmp->set_function((VisualShaderNodeCompare::Function)p_op_idx);
            }

            VisualShaderNodeColorOp* colorOp =
                Object::cast_to<VisualShaderNodeColorOp>(vsn);

            if (colorOp) {
                colorOp->set_operator((VisualShaderNodeColorOp::Operator
                )p_op_idx);
            }

            VisualShaderNodeColorFunc* colorFunc =
                Object::cast_to<VisualShaderNodeColorFunc>(vsn);

            if (colorFunc) {
                colorFunc->set_function((VisualShaderNodeColorFunc::Function
                )p_op_idx);
            }

            VisualShaderNodeScalarOp* scalarOp =
                Object::cast_to<VisualShaderNodeScalarOp>(vsn);

            if (scalarOp) {
                scalarOp->set_operator((VisualShaderNodeScalarOp::Operator
                )p_op_idx);
            }

            VisualShaderNodeScalarFunc* scalarFunc =
                Object::cast_to<VisualShaderNodeScalarFunc>(vsn);

            if (scalarFunc) {
                scalarFunc->set_function((VisualShaderNodeScalarFunc::Function
                )p_op_idx);
            }

            VisualShaderNodeVectorOp* vecOp =
                Object::cast_to<VisualShaderNodeVectorOp>(vsn);
            ...
            VisualShaderNodeVectorFunc* vecFunc =
                Object::cast_to<VisualShaderNodeVectorFunc>(vsn);

            if (vecFunc) {
                vecFunc->set_function((VisualShaderNodeVectorFunc::Function
                )p_op_idx);
            }

            VisualShaderNodeTransformFunc* matFunc =
                Object::cast_to<VisualShaderNodeTransformFunc>(vsn);

            if (matFunc) {
                matFunc->set_function((VisualShaderNodeTransformFunc::Function
                )p_op_idx);
            }

            VisualShaderNodeScalarDerivativeFunc* sderFunc =
                Object::cast_to<VisualShaderNodeScalarDerivativeFunc>(vsn);

            if (sderFunc) {
                sderFunc->set_function(
                    (VisualShaderNodeScalarDerivativeFunc::Function)p_op_idx
                );
            }

            VisualShaderNodeVectorDerivativeFunc* vderFunc =
                Object::cast_to<VisualShaderNodeVectorDerivativeFunc>(vsn);

            if (vderFunc) {
                vderFunc->set_function(
                    (VisualShaderNodeVectorDerivativeFunc::Function)p_op_idx
                );
            }
        }
        ...
        VisualShaderNode* vsn =
            Object::cast_to<VisualShaderNode>(ClassDB::instance(base_type));
        ERR_FAIL_COND_V(!vsn, nullptr);
        vsnode = Ref<VisualShaderNode>(vsn);
    ...
    undo_redo
        ->add_undo_method(visual_shader.ptr(), "remove_node", type, id_to_use);

    VisualShaderNodeExpression* expr =
        Object::cast_to<VisualShaderNodeExpression>(vsnode.ptr());
    if (expr) {
        undo_redo->add_do_method(
            expr,
            "set_size",
            Size2(250 * EDSCALE, 150 * EDSCALE)
        );
    }
    ...
    if (to_node != -1 && to_slot != -1) {
        if (vsnode->get_output_port_count() > 0) {
            ...
            if (visual_shader->is_port_types_compatible(
                    vsnode->get_output_port_type(_from_slot),
                    visual_shader->get_node(type, to_node)
                        ->get_input_port_type(to_slot)
                )) {
                undo_redo->add_do_method(
                    visual_shader.ptr(),
                    "connect_nodes",
        ...
        if (vsnode->get_input_port_count() > 0) {
            ...
            if (visual_shader->is_port_types_compatible(
                    visual_shader->get_node(type, from_node)
                        ->get_output_port_type(from_slot),
                    vsnode->get_input_port_type(_to_slot)
                )) {
                undo_redo->add_do_method(
                    visual_shader.ptr(),
                    "connect_nodes",
...
void VisualShaderEditor::_node_dragged(
...
void VisualShaderEditor::_connection_request(
    const String& p_from,
    int p_from_index,
    const String& p_to,
    int p_to_index
    ...
    if (!visual_shader
             ->can_connect_nodes(type, from, p_from_index, to, p_to_index)) {
        return;
    }
...
void VisualShaderEditor::_disconnection_request(
    const String& p_from,
    int p_from_index,
    const String& p_to,
    int p_to_index
...
void VisualShaderEditor::_connection_to_empty(
    const String& p_from,
    int p_from_slot,
    const Vector2& p_release_position
) {
...
void VisualShaderEditor::_delete_request(int which) {
    ...
    Ref<VisualShaderNode> node =
        Ref<VisualShaderNode>(visual_shader->get_node(type, which));
    ...
    undo_redo->add_undo_method(
        ...
        type,
        node,
        visual_shader->get_node_position(type, which),
        which
    );
    ...
    VisualShaderNodeGroupBase* group =
        Object::cast_to<VisualShaderNodeGroupBase>(node.ptr());
    if (group) {
...
void VisualShaderEditor::_show_members_dialog(bool at_mouse_pos) {
    if (at_mouse_pos) {
        ...
        members_dialog->popup();
        saved_node_pos_dirty = false;
        members_dialog->set_position(
            graph->get_global_position() + Point2(5 * EDSCALE, 65 * EDSCALE)
        );
    }
    ...
    if (dialog_rect.position.y + dialog_rect.size.y > window_size.y) {
        int difference =
            dialog_rect.position.y + dialog_rect.size.y - window_size.y;
        members_dialog->set_position(
            members_dialog->get_position() - Point2(0, difference)
        );
    }
    if (dialog_rect.position.x + dialog_rect.size.x > window_size.x) {
        int difference =
            dialog_rect.position.x + dialog_rect.size.x - window_size.x;
        members_dialog->set_position(
            members_dialog->get_position() - Point2(difference, 0)
        );
    }
...
void VisualShaderEditor::_sbox_input(const Ref<InputEvent>& p_ie) {
    ...
    if (ie.is_valid()
        && (ie->get_scancode() == KEY_UP || ie->get_scancode() == KEY_DOWN
...
void VisualShaderEditor::_notification(int p_what) {
    ...
    if (p_what == NOTIFICATION_ENTER_TREE
...

        {
            ...
            preview_text->add_color_override(
                "background_color",
                background_color
            );
            ...
            for (List<String>::Element* E = keyword_list.front(); E;
                 E                        = E->next()) {
                if (ShaderLanguage::is_control_flow_keyword(E->get())) {
                    preview_text->add_keyword_color(
                        E->get(),
                        control_flow_keyword_color
                    );
                } else {
            ...
            preview_text->add_font_override(
                "font",
                get_font("expression", "EditorFonts")
            );
            ...
            error_text->add_font_override(
                "font",
                get_font("status_source", "EditorFonts")
            );
            error_text->add_color_override(
                "font_color",
                get_color("error_color", "Editor")
            );
        }

        tools->set_icon(EditorNode::get_singleton()->get_gui_base()->get_icon(
            "Tools",
            "EditorIcons"
        ));
...
void VisualShaderEditor::_dup_update_excluded(
    int p_type,
    Set<int>& r_excluded
) {
    r_excluded.clear();
...
void VisualShaderEditor::_dup_copy_nodes(
    int p_type,
    List<int>& r_nodes,
    Set<int>& r_excluded
) {
...
void VisualShaderEditor::_dup_paste_nodes(
    ...
    for (List<int>::Element* E = r_nodes.front(); E; E = E->next()) {
        ...
        Ref<VisualShaderNode> node =
            visual_shader->get_node(pasted_type, E->get());
        ...
        undo_redo->add_do_method(
            ...
            type,
            dupli,
            visual_shader->get_node_position(pasted_type, E->get()) + p_offset,
            id_from
        );
        undo_redo->add_undo_method(
            visual_shader.ptr(),
        ...
        Ref<VisualShaderNodeGroupBase> group =
            Object::cast_to<VisualShaderNodeGroupBase>(node.ptr());
        if (!group.is_null()) {
            undo_redo
                ->add_do_method(dupli.ptr(), "set_size", group->get_size());
            undo_redo
                ->add_do_method(dupli.ptr(), "set_inputs", group->get_inputs());
            undo_redo->add_do_method(
                dupli.ptr(),
                "set_outputs",
                group->get_outputs()
    ...
    for (List<VisualShader::Connection>::Element* E = conns.front(); E;
         E                                          = E->next()) {
        if (unsupported_set.has(E->get().from_node)
            || unsupported_set.has(E->get().to_node)) {
            continue;
        }
        if (connection_remap.has(E->get().from_node)
            && connection_remap.has(E->get().to_node)) {
            undo_redo->add_do_method(
                visual_shader.ptr(),
                "connect_nodes_forced",
                type,
                connection_remap[E->get().from_node],
                E->get().from_port,
                connection_remap[E->get().to_node],
                E->get().to_port
            );
        }
    }
...
void VisualShaderEditor::_paste_nodes() {
    ...
    _dup_paste_nodes(
        ...
        copy_type,
        copy_nodes_buffer,
        copy_nodes_excluded_buffer,
        (graph->get_scroll_ofs() / scale
         + graph->get_local_mouse_position() / scale - selection_center),
        false
    );

    _dup_update_excluded(
        type,
        copy_nodes_excluded_buffer
    ); // to prevent selection of previous copies at new paste
}
...
void VisualShaderEditor::_on_nodes_delete() {
    ...
    for (List<int>::Element* F = to_erase.front(); F; F = F->next()) {
        ...
        undo_redo
            ->add_do_method(visual_shader.ptr(), "remove_node", type, F->get());
        undo_redo->add_undo_method(
            visual_shader.ptr(),
            "add_node",
            type,
            node,
            visual_shader->get_node_position(type, F->get()),
            F->get()
        );
        ...
        if (group) {
            undo_redo->add_undo_method(group, "set_size", group->get_size());
            undo_redo
                ->add_undo_method(group, "set_inputs", group->get_inputs());
            undo_redo
                ->add_undo_method(group, "set_outputs", group->get_outputs());
        }
    ...
    for (List<int>::Element* F = to_erase.front(); F; F = F->next()) {
        for (List<VisualShader::Connection>::Element* E = conns.front(); E;
             E                                          = E->next()) {
            if (E->get().from_node == F->get()
                || E->get().to_node == F->get()) {
                bool cancel = false;
                for (List<VisualShader::Connection>::Element* R =
                         used_conns.front();
                     R;
                     R = R->next()) {
                    if (R->get().from_node == E->get().from_node
                        && R->get().from_port == E->get().from_port
                        && R->get().to_node == E->get().to_node
                        && R->get().to_port == E->get().to_port) {
...
void VisualShaderEditor::_input_select_item(
    ...
    bool type_changed = input->get_input_type_by_name(name)
                     != input->get_input_type_by_name(prev_name);
...
void VisualShaderEditor::_uniform_select_item(
    Ref<VisualShaderNodeUniformRef> p_uniform_ref,
    String p_name
) {
    ...
    bool type_changed = p_uniform_ref->get_uniform_type_by_name(p_name)
                     != p_uniform_ref->get_uniform_type_by_name(prev_name);
    ...
    undo_redo
        ->add_undo_method(p_uniform_ref.ptr(), "set_uniform_name", prev_name);
...
Variant VisualShaderEditor::get_drag_data_fw(
...
bool VisualShaderEditor::can_drop_data_fw(
    const Point2& p_point,
                ...
                for (int i = 0; i < arr.size(); i++) {
                    ...
                    if (type == "RebelScript") {
                        ...
                        if (script->get_instance_base_type()
                            == "VisualShaderNodeCustom") {
                            saved_node_pos =
                                p_point + Vector2(0, j * 210 * EDSCALE);
                            saved_node_pos_dirty = true;
...
void VisualShaderEditor::_update_preview() {
    ...
    Error err = sl.compile(
        code,
        ShaderTypes::get_singleton()->get_functions(
            VisualServer::ShaderMode(visual_shader->get_mode())
        ),
        ShaderTypes::get_singleton()->get_modes(
            VisualServer::ShaderMode(visual_shader->get_mode())
        ),
    ...
    if (err != OK) {
        ...
        String text =
            "error(" + itos(sl.get_error_line()) + "): " + sl.get_error_text();
...
void VisualShaderEditor::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_update_options_menu",
        &VisualShaderEditor::_update_options_menu
    );
    ClassDB::bind_method(
        "_expression_focus_out",
        &VisualShaderEditor::_expression_focus_out
    );
    ClassDB::bind_method("_add_node", &VisualShaderEditor::_add_node);
    ClassDB::bind_method("_node_dragged", &VisualShaderEditor::_node_dragged);
    ClassDB::bind_method(
        "_connection_request",
        &VisualShaderEditor::_connection_request
    );
    ClassDB::bind_method(
        "_disconnection_request",
        &VisualShaderEditor::_disconnection_request
    );
    ClassDB::bind_method("_node_selected", &VisualShaderEditor::_node_selected);
    ClassDB::bind_method(
        "_scroll_changed",
        &VisualShaderEditor::_scroll_changed
    );
    ClassDB::bind_method(
        "_delete_request",
        &VisualShaderEditor::_delete_request
    );
    ClassDB::bind_method(
        "_on_nodes_delete",
        &VisualShaderEditor::_on_nodes_delete
    );
    ClassDB::bind_method("_node_changed", &VisualShaderEditor::_node_changed);
    ClassDB::bind_method(
        "_edit_port_default_input",
        &VisualShaderEditor::_edit_port_default_input
    );
    ClassDB::bind_method("_port_edited", &VisualShaderEditor::_port_edited);
    ClassDB::bind_method(
        "_connection_to_empty",
        &VisualShaderEditor::_connection_to_empty
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        "_line_edit_focus_out",
        &VisualShaderEditor::_line_edit_focus_out
    );
    ClassDB::bind_method(
        "_line_edit_changed",
        &VisualShaderEditor::_line_edit_changed
    );
    ClassDB::bind_method(
        "_port_name_focus_out",
    ...
    ClassDB::bind_method(
        "_duplicate_nodes",
        &VisualShaderEditor::_duplicate_nodes
    );
    ...
    ClassDB::bind_method(
        "_input_select_item",
        &VisualShaderEditor::_input_select_item
    );
    ClassDB::bind_method(
        "_uniform_select_item",
        &VisualShaderEditor::_uniform_select_item
    );
    ClassDB::bind_method(
        "_preview_select_port",
        &VisualShaderEditor::_preview_select_port
    );
    ClassDB::bind_method(
        "_graph_gui_input",
        &VisualShaderEditor::_graph_gui_input
    );
    ClassDB::bind_method(
        "_add_input_port",
        &VisualShaderEditor::_add_input_port
    );
    ClassDB::bind_method(
        "_change_input_port_type",
        &VisualShaderEditor::_change_input_port_type
    );
    ClassDB::bind_method(
        "_change_input_port_name",
        &VisualShaderEditor::_change_input_port_name
    );
    ClassDB::bind_method(
        "_remove_input_port",
        &VisualShaderEditor::_remove_input_port
    );
    ClassDB::bind_method(
        "_add_output_port",
        &VisualShaderEditor::_add_output_port
    );
    ClassDB::bind_method(
        "_change_output_port_type",
        &VisualShaderEditor::_change_output_port_type
    );
    ClassDB::bind_method(
        "_change_output_port_name",
        &VisualShaderEditor::_change_output_port_name
    );
    ClassDB::bind_method(
        "_remove_output_port",
        &VisualShaderEditor::_remove_output_port
    );
    ...
    ClassDB::bind_method(
        "_show_preview_text",
        &VisualShaderEditor::_show_preview_text
    );
    ClassDB::bind_method(
        "_update_preview",
        &VisualShaderEditor::_update_preview
    );
    ClassDB::bind_method("_nodes_dragged", &VisualShaderEditor::_nodes_dragged);

    ClassDB::bind_method(
        D_METHOD("get_drag_data_fw"),
        &VisualShaderEditor::get_drag_data_fw
    );
    ClassDB::bind_method(
        D_METHOD("can_drop_data_fw"),
        &VisualShaderEditor::can_drop_data_fw
    );
    ClassDB::bind_method(
        D_METHOD("drop_data_fw"),
        &VisualShaderEditor::drop_data_fw
    );

    ClassDB::bind_method("_is_available", &VisualShaderEditor::_is_available);
    ClassDB::bind_method(
        "_tools_menu_option",
        &VisualShaderEditor::_tools_menu_option
    );
    ClassDB::bind_method(
        "_show_members_dialog",
        &VisualShaderEditor::_show_members_dialog
    );
    ClassDB::bind_method("_sbox_input", &VisualShaderEditor::_sbox_input);
    ClassDB::bind_method(
        "_member_filter_changed",
        &VisualShaderEditor::_member_filter_changed
    );
    ClassDB::bind_method(
        "_member_selected",
        &VisualShaderEditor::_member_selected
    );
    ClassDB::bind_method(
        "_member_unselected",
        &VisualShaderEditor::_member_unselected
    );
...
VisualShaderEditor::VisualShaderEditor() {
    ...
    graph->add_valid_connection_type(
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShaderNode::PORT_TYPE_SCALAR
    );
    graph->add_valid_connection_type(
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeColorFunc",
        TTR("Grayscale function."),
        VisualShaderNodeColorFunc::FUNC_GRAYSCALE,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "HSV2RGB",
        ...
        TTR("Converts HSV vector to RGB equivalent."),
        VisualShaderNodeVectorFunc::FUNC_HSV2RGB,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "RGB2HSV",
        ...
        "Functions",
        "VisualShaderNodeVectorFunc",
        TTR("Converts RGB vector to HSV equivalent."),
        VisualShaderNodeVectorFunc::FUNC_RGB2HSV,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Sepia",
        ...
        "VisualShaderNodeColorFunc",
        TTR("Sepia function."),
        VisualShaderNodeColorFunc::FUNC_SEPIA,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeColorOp",
        TTR("Burn operator."),
        VisualShaderNodeColorOp::OP_BURN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Darken",
        ...
        "VisualShaderNodeColorOp",
        TTR("Darken operator."),
        VisualShaderNodeColorOp::OP_DARKEN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Difference",
        ...
        "Operators",
        "VisualShaderNodeColorOp",
        TTR("Difference operator."),
        VisualShaderNodeColorOp::OP_DIFFERENCE,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Dodge",
        ...
        "VisualShaderNodeColorOp",
        TTR("Dodge operator."),
        VisualShaderNodeColorOp::OP_DODGE,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "HardLight",
        ...
        "Operators",
        "VisualShaderNodeColorOp",
        TTR("HardLight operator."),
        VisualShaderNodeColorOp::OP_HARD_LIGHT,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Lighten",
        ...
        "Operators",
        "VisualShaderNodeColorOp",
        TTR("Lighten operator."),
        VisualShaderNodeColorOp::OP_LIGHTEN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Overlay",
        ...
        "Operators",
        "VisualShaderNodeColorOp",
        TTR("Overlay operator."),
        VisualShaderNodeColorOp::OP_OVERLAY,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Screen",
        ...
        "VisualShaderNodeColorOp",
        TTR("Screen operator."),
        VisualShaderNodeColorOp::OP_SCREEN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "SoftLight",
        ...
        "Operators",
        "VisualShaderNodeColorOp",
        TTR("SoftLight operator."),
        VisualShaderNodeColorOp::OP_SOFT_LIGHT,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    ...
    const String& compare_func_desc =
        TTR("Returns the boolean result of the %s comparison between two "
            "parameters.");
    ...
    add_options.push_back(AddOption(
        ...
        vformat(compare_func_desc, TTR("Equal (==)")),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeCompare",
        vformat(compare_func_desc, TTR("Greater Than (>)")),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeCompare",
        vformat(compare_func_desc, TTR("Greater Than or Equal (>=)")),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeIf",
        TTR("Returns an associated vector if the provided scalars are equal, "
            "greater or less."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeIs",
        TTR("Returns the boolean result of the comparison between INF and a "
            "scalar parameter."),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeIs",
        TTR("Returns the boolean result of the comparison between NaN and a "
            "scalar parameter."),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeCompare",
        vformat(compare_func_desc, TTR("Less Than (<)")),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeCompare",
        vformat(compare_func_desc, TTR("Less Than or Equal (<=)")),
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeCompare",
        vformat(compare_func_desc, TTR("Not Equal (!=)")),
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Returns an associated vector if the provided boolean value is "
            "true or false."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Returns an associated scalar if the provided boolean value is "
            "true or false."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        "Common",
        "VisualShaderNodeCompare",
        TTR("Returns the boolean result of the comparison between two "
            "parameters."),
        -1,
        VisualShaderNode::PORT_TYPE_BOOLEAN
    ));
    add_options.push_back(AddOption(
        ...
        "Common",
        "VisualShaderNodeIs",
        TTR("Returns the boolean result of the comparison between INF (or NaN) "
    ...
    // SPATIAL-FOR-ALL
    const String input_param_shader_modes =
        TTR("'%s' input parameter for all shader modes.");
    add_options.push_back(AddOption(
        "Camera",
        "Input",
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "camera"),
        "camera",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "inv_camera"),
        "inv_camera",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "inv_projection"),
        "inv_projection",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "normal"),
        "normal",
        VisualShaderNode::PORT_TYPE_VECTOR,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "output_is_srgb"),
        "output_is_srgb",
        VisualShaderNode::PORT_TYPE_BOOLEAN,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "time"),
        "time",
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "viewport_size"),
        "viewport_size",
        VisualShaderNode::PORT_TYPE_VECTOR,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "world"),
        "world",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        -1,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "alpha"),
        "alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "color"),
        "color",
        VisualShaderNode::PORT_TYPE_VECTOR,
        -1,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "modulate_alpha"),
        "modulate_alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "modulate_color"),
        "modulate_color",
        VisualShaderNode::PORT_TYPE_VECTOR,
        -1,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "texture_pixel_size"),
        "texture_pixel_size",
        VisualShaderNode::PORT_TYPE_VECTOR,
        -1,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "All",
        "VisualShaderNodeInput",
        vformat(input_param_shader_modes, "uv"),
        "uv",
        VisualShaderNode::PORT_TYPE_VECTOR,
        -1,
        Shader::MODE_CANVAS_ITEM
    ...
    const String input_param_for_vertex_and_fragment_shader_modes =
        TTR("'%s' input parameter for vertex and fragment shader modes.");
    const String input_param_for_fragment_and_light_shader_modes =
        TTR("'%s' input parameter for fragment and light shader modes.");
    const String input_param_for_fragment_shader_mode =
        TTR("'%s' input parameter for fragment shader mode.");
    const String input_param_for_light_shader_mode =
        TTR("'%s' input parameter for light shader mode.");
    const String input_param_for_vertex_shader_mode =
        TTR("'%s' input parameter for vertex shader mode.");
    const String input_param_for_vertex_and_fragment_shader_mode =
        TTR("'%s' input parameter for vertex and fragment shader mode.");
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "alpha"),
        "alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "binormal"),
        "binormal",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "color"),
        "color",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_and_light_shader_modes, "fragcoord"),
        "fragcoord",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "front_facing"),
        "front_facing",
        VisualShaderNode::PORT_TYPE_BOOLEAN,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "point_coord"),
        "point_coord",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "screen_texture"),
        "screen_texture",
        VisualShaderNode::PORT_TYPE_SAMPLER,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "screen_uv"),
        "screen_uv",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "side"),
        "side",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "tangent"),
        "tangent",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "uv"),
        "uv",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "uv2"),
        "uv2",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "vertex"),
        "vertex",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_and_light_shader_modes, "view"),
        "view",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "albedo"),
        "albedo",
        VisualShaderNode::PORT_TYPE_VECTOR,
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "attenuation"),
        "attenuation",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "diffuse"),
        "diffuse",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "light"),
        "light",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "light_color"),
        "light_color",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "metallic"),
        "metallic",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "roughness"),
        "roughness",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "specular"),
        "specular",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "transmission"),
        "transmission",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "modelview"),
        "modelview",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "point_size"),
        "point_size",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_mode, "tangent"),
        "tangent",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_SPATIAL
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_and_fragment_shader_modes, "light_pass"),
        "light_pass",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "normal_texture"),
        "normal_texture",
        VisualShaderNode::PORT_TYPE_SAMPLER,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_and_light_shader_modes, "point_coord"),
        "point_coord",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_shader_mode, "screen_pixel_size"),
        "screen_pixel_size",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_and_light_shader_modes, "screen_uv"),
        "screen_uv",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Fragment",
        "VisualShaderNodeInput",
        vformat(input_param_for_fragment_and_light_shader_modes, "texture"),
        "texture",
        VisualShaderNode::PORT_TYPE_SAMPLER,
        VisualShader::TYPE_FRAGMENT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "light_alpha"),
        "light_alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "light_height"),
        "light_height",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "light_uv"),
        "light_uv",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "light_vec"),
        "light_vec",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "normal"),
        "normal",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "shadow_alpha"),
        "shadow_alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "shadow_color"),
        "shadow_color",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Light",
        "VisualShaderNodeInput",
        vformat(input_param_for_light_shader_mode, "shadow_vec"),
        "shadow_vec",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_LIGHT,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "extra"),
        "extra",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "projection"),
        "projection",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "vertex"),
        "vertex",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "world"),
        "world",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_CANVAS_ITEM
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "active"),
        "active",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "alpha"),
        "alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "color"),
        "color",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "custom"),
        "custom",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "custom_alpha"),
        "custom_alpha",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "delta"),
        "delta",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "emission_transform"),
        "emission_transform",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "index"),
        "index",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "lifetime"),
        "lifetime",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "restart"),
        "restart",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "time"),
        "time",
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "transform"),
        "transform",
        VisualShaderNode::PORT_TYPE_TRANSFORM,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        "Vertex",
        "VisualShaderNodeInput",
        vformat(input_param_for_vertex_shader_mode, "velocity"),
        "velocity",
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_VERTEX,
        Shader::MODE_PARTICLES
    ...
    add_options.push_back(AddOption(
        ...
        TTR("E constant (2.718282). Represents the base of the natural "
            "logarithm."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Epsilon constant (0.00001). Smallest possible scalar number."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Phi constant (1.618034). Golden ratio."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Pi/4 constant (0.785398) or 45 degrees."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Pi/2 constant (1.570796) or 90 degrees."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Pi constant (3.141593) or 180 degrees."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Tau constant (6.283185) or 360 degrees."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Constants",
        "VisualShaderNodeScalarConstant",
        TTR("Sqrt2 constant (1.414214). Square root of 2."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the absolute value of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ABS,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ACos",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the arc-cosine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ACOS,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ACosH",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the inverse hyperbolic cosine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ACOSH,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ASin",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the arc-sine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ASIN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ASinH",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the inverse hyperbolic sine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ASINH,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ATan",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the arc-tangent of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ATAN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ATan2",
        ...
        "Functions",
        "VisualShaderNodeScalarOp",
        TTR("Returns the arc-tangent of the parameters."),
        VisualShaderNodeScalarOp::OP_ATAN2,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "ATanH",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the inverse hyperbolic tangent of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ATANH,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Ceil",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Finds the nearest integer that is greater than or equal to the "
            "parameter."),
        VisualShaderNodeScalarFunc::FUNC_CEIL,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Clamp",
        ...
        TTR("Constrains a value to lie between two further values."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the cosine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_COS,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "CosH",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the hyperbolic cosine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_COSH,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Degrees",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Converts a quantity in radians to degrees."),
        VisualShaderNodeScalarFunc::FUNC_DEGREES,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Exp",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Base-e Exponential."),
        VisualShaderNodeScalarFunc::FUNC_EXP,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Exp2",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Base-2 Exponential."),
        VisualShaderNodeScalarFunc::FUNC_EXP2,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Floor",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Finds the nearest integer less than or equal to the parameter."),
        VisualShaderNodeScalarFunc::FUNC_FLOOR,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Fract",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Computes the fractional part of the argument."),
        VisualShaderNodeScalarFunc::FUNC_FRAC,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "InverseSqrt",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the inverse of the square root of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_INVERSE_SQRT,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Log",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Natural logarithm."),
        VisualShaderNodeScalarFunc::FUNC_LOG,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Log2",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Base-2 logarithm."),
        VisualShaderNodeScalarFunc::FUNC_LOG2,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Max",
        ...
        "Functions",
        "VisualShaderNodeScalarOp",
        TTR("Returns the greater of two values."),
        VisualShaderNodeScalarOp::OP_MAX,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Min",
        ...
        "Functions",
        "VisualShaderNodeScalarOp",
        TTR("Returns the lesser of two values."),
        VisualShaderNodeScalarOp::OP_MIN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Mix",
        ...
        TTR("Linear interpolation between two scalars."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the opposite value of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_NEGATE,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "OneMinus",
        ...
        "VisualShaderNodeScalarFunc",
        TTR("1.0 - scalar"),
        VisualShaderNodeScalarFunc::FUNC_ONEMINUS,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Pow",
        ...
        "Functions",
        "VisualShaderNodeScalarOp",
        TTR("Returns the value of the first parameter raised to the power of "
            "the second."),
        VisualShaderNodeScalarOp::OP_POW,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Radians",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Converts a quantity in degrees to radians."),
        VisualShaderNodeScalarFunc::FUNC_RADIANS,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Reciprocal",
        ...
        "VisualShaderNodeScalarFunc",
        TTR("1.0 / scalar"),
        VisualShaderNodeScalarFunc::FUNC_RECIPROCAL,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Round",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Finds the nearest integer to the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ROUND,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "RoundEven",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Finds the nearest even integer to the parameter."),
        VisualShaderNodeScalarFunc::FUNC_ROUNDEVEN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Saturate",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Clamps the value between 0.0 and 1.0."),
        VisualShaderNodeScalarFunc::FUNC_SATURATE,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Sign",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Extracts the sign of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_SIGN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Sin",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the sine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_SIN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "SinH",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the hyperbolic sine of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_SINH,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Sqrt",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the square root of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_SQRT,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "SmoothStep",
        ...
        TTR("SmoothStep function( scalar(edge0), scalar(edge1), scalar(x) "
            ").\n\nReturns 0.0 if 'x' is smaller than 'edge0' and 1.0 if x is "
            "larger than 'edge1'. Otherwise the return value is interpolated "
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeScalarOp",
        TTR("Step function( scalar(edge), scalar(x) ).\n\nReturns 0.0 if 'x' "
            "is smaller than 'edge' and otherwise 1.0."),
        VisualShaderNodeScalarOp::OP_STEP,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Tan",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the tangent of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_TAN,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "TanH",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Returns the hyperbolic tangent of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_TANH,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Trunc",
        ...
        "Functions",
        "VisualShaderNodeScalarFunc",
        TTR("Finds the truncated value of the parameter."),
        VisualShaderNodeScalarFunc::FUNC_TRUNC,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        "Operators",
        "VisualShaderNodeScalarOp",
        TTR("Adds scalar to scalar."),
        VisualShaderNodeScalarOp::OP_ADD,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Divide",
        ...
        "Operators",
        "VisualShaderNodeScalarOp",
        TTR("Divides scalar by scalar."),
        VisualShaderNodeScalarOp::OP_DIV,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Multiply",
        ...
        "Operators",
        "VisualShaderNodeScalarOp",
        TTR("Multiplies scalar by scalar."),
        VisualShaderNodeScalarOp::OP_MUL,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Remainder",
        ...
        "Operators",
        "VisualShaderNodeScalarOp",
        TTR("Returns the remainder of the two scalars."),
        VisualShaderNodeScalarOp::OP_MOD,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        "Subtract",
        ...
        "Operators",
        "VisualShaderNodeScalarOp",
        TTR("Subtracts scalar from scalar."),
        VisualShaderNodeScalarOp::OP_SUB,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Perform the cubic texture lookup."),
        -1,
        -1
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Perform the texture lookup."),
        -1,
        -1
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Cubic texture uniform lookup."),
        -1,
        -1
    ));
    add_options.push_back(AddOption(
    ...
    add_options.push_back(AddOption(
        ...
        TTR("2D texture uniform lookup with triplanar."),
        -1,
        -1,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        Shader::MODE_SPATIAL
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Calculate the outer product of a pair of vectors.\n\nOuterProduct "
            "treats the first parameter 'c' as a column vector (matrix with "
            "one column) and the second parameter 'r' as a row vector (matrix "
            "with one row) and does a linear algebraic matrix multiply 'c * "
            "r', yielding a matrix whose number of rows is the number of "
            "components in 'c' and whose number of columns is the number of "
            "components in 'r'."),
        -1,
        VisualShaderNode::PORT_TYPE_TRANSFORM
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Composes transform from four vectors."),
        -1,
        VisualShaderNode::PORT_TYPE_TRANSFORM
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Decomposes transform to four vectors.")
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Calculates the determinant of a transform."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeTransformFunc",
        TTR("Calculates the inverse of a transform."),
        VisualShaderNodeTransformFunc::FUNC_INVERSE,
        VisualShaderNode::PORT_TYPE_TRANSFORM
    ));
    add_options.push_back(AddOption(
        "Transpose",
        ...
        "Functions",
        "VisualShaderNodeTransformFunc",
        TTR("Calculates the transpose of a transform."),
        VisualShaderNodeTransformFunc::FUNC_TRANSPOSE,
        VisualShaderNode::PORT_TYPE_TRANSFORM
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Multiplies transform by transform."),
        -1,
        VisualShaderNode::PORT_TYPE_TRANSFORM
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Multiplies vector by transform."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Composes vector from three scalars."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Decomposes vector to three scalars.")
    ));
    ...
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the absolute value of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ABS,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ACos",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the arc-cosine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ACOS,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ACosH",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the inverse hyperbolic cosine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ACOSH,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ASin",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the arc-sine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ASIN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ASinH",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the inverse hyperbolic sine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ASINH,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ATan",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the arc-tangent of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ATAN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ATan2",
        ...
        "VisualShaderNodeVectorOp",
        TTR("Returns the arc-tangent of the parameters."),
        VisualShaderNodeVectorOp::OP_ATAN2,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "ATanH",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the inverse hyperbolic tangent of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ATANH,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Ceil",
        ...
        TTR("Finds the nearest integer that is greater than or equal to the "
            "parameter."),
        VisualShaderNodeVectorFunc::FUNC_CEIL,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Clamp",
    ...
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the cosine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_COS,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "CosH",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the hyperbolic cosine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_COSH,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Cross",
        ...
        "Functions",
        "VisualShaderNodeVectorOp",
        TTR("Calculates the cross product of two vectors."),
        VisualShaderNodeVectorOp::OP_CROSS,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Degrees",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Converts a quantity in radians to degrees."),
        VisualShaderNodeVectorFunc::FUNC_DEGREES,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Distance",
        ...
        TTR("Returns the distance between two points."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Calculates the dot product of two vectors."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Base-e Exponential."),
        VisualShaderNodeVectorFunc::FUNC_EXP,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Exp2",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Base-2 Exponential."),
        VisualShaderNodeVectorFunc::FUNC_EXP2,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "FaceForward",
        ...
        TTR("Returns the vector that points in the same direction as a "
            "reference vector. The function has three vector parameters : N, "
            "the vector to orient, I, the incident vector, and Nref, the "
            "reference vector. If the dot product of I and Nref is smaller "
            "than zero the return value is N. Otherwise -N is returned."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Finds the nearest integer less than or equal to the parameter."),
        VisualShaderNodeVectorFunc::FUNC_FLOOR,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Fract",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Computes the fractional part of the argument."),
        VisualShaderNodeVectorFunc::FUNC_FRAC,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "InverseSqrt",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the inverse of the square root of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_INVERSE_SQRT,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Length",
        ...
        TTR("Calculates the length of a vector."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Natural logarithm."),
        VisualShaderNodeVectorFunc::FUNC_LOG,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Log2",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Base-2 logarithm."),
        VisualShaderNodeVectorFunc::FUNC_LOG2,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Max",
        ...
        "VisualShaderNodeVectorOp",
        TTR("Returns the greater of two values."),
        VisualShaderNodeVectorOp::OP_MAX,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Min",
        ...
        "VisualShaderNodeVectorOp",
        TTR("Returns the lesser of two values."),
        VisualShaderNodeVectorOp::OP_MIN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Mix",
        ...
        TTR("Linear interpolation between two vectors."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        TTR("Linear interpolation between two vectors using scalar."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the opposite value of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_NEGATE,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Normalize",
        ...
        "Functions",
        "VisualShaderNodeVectorFunc",
        TTR("Calculates the normalize product of vector."),
        VisualShaderNodeVectorFunc::FUNC_NORMALIZE,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "OneMinus",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("1.0 - vector"),
        VisualShaderNodeVectorFunc::FUNC_ONEMINUS,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Pow",
        ...
        TTR("Returns the value of the first parameter raised to the power of "
            "the second."),
        VisualShaderNodeVectorOp::OP_POW,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Radians",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Converts a quantity in degrees to radians."),
        VisualShaderNodeVectorFunc::FUNC_RADIANS,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Reciprocal",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("1.0 / vector"),
        VisualShaderNodeVectorFunc::FUNC_RECIPROCAL,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Reflect",
        ...
        "Functions",
        "VisualShaderNodeVectorOp",
        TTR("Returns the vector that points in the direction of reflection ( a "
            ": incident vector, b : normal vector )."),
        VisualShaderNodeVectorOp::OP_REFLECT,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Refract",
        ...
        TTR("Returns the vector that points in the direction of refraction."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Finds the nearest integer to the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ROUND,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "RoundEven",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Finds the nearest even integer to the parameter."),
        VisualShaderNodeVectorFunc::FUNC_ROUNDEVEN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Saturate",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Clamps the value between 0.0 and 1.0."),
        VisualShaderNodeVectorFunc::FUNC_SATURATE,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Sign",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Extracts the sign of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_SIGN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Sin",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the sine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_SIN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "SinH",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the hyperbolic sine of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_SINH,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Sqrt",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the square root of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_SQRT,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "SmoothStep",
        ...
        TTR("SmoothStep function( vector(edge0), vector(edge1), vector(x) "
            ").\n\nReturns 0.0 if 'x' is smaller than 'edge0' and 1.0 if 'x' "
            "is larger than 'edge1'. Otherwise the return value is "
            "interpolated between 0.0 and 1.0 using Hermite polynomials."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        TTR("SmoothStep function( scalar(edge0), scalar(edge1), vector(x) "
            ").\n\nReturns 0.0 if 'x' is smaller than 'edge0' and 1.0 if 'x' "
            "is larger than 'edge1'. Otherwise the return value is "
            "interpolated between 0.0 and 1.0 using Hermite polynomials."),
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Functions",
        "VisualShaderNodeVectorOp",
        TTR("Step function( vector(edge), vector(x) ).\n\nReturns 0.0 if 'x' "
            "is smaller than 'edge' and otherwise 1.0."),
        VisualShaderNodeVectorOp::OP_STEP,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "StepS",
        ...
        TTR("Step function( scalar(edge), vector(x) ).\n\nReturns 0.0 if 'x' "
            "is smaller than 'edge' and otherwise 1.0."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the tangent of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_TAN,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "TanH",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Returns the hyperbolic tangent of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_TANH,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Trunc",
        ...
        "VisualShaderNodeVectorFunc",
        TTR("Finds the truncated value of the parameter."),
        VisualShaderNodeVectorFunc::FUNC_TRUNC,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        "Operators",
        "VisualShaderNodeVectorOp",
        TTR("Adds vector to vector."),
        VisualShaderNodeVectorOp::OP_ADD,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Divide",
        ...
        "Operators",
        "VisualShaderNodeVectorOp",
        TTR("Divides vector by vector."),
        VisualShaderNodeVectorOp::OP_DIV,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Multiply",
        ...
        "Operators",
        "VisualShaderNodeVectorOp",
        TTR("Multiplies vector by vector."),
        VisualShaderNodeVectorOp::OP_MUL,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Remainder",
        ...
        "Operators",
        "VisualShaderNodeVectorOp",
        TTR("Returns the remainder of the two vectors."),
        VisualShaderNodeVectorOp::OP_MOD,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    add_options.push_back(AddOption(
        "Subtract",
        ...
        "Operators",
        "VisualShaderNodeVectorOp",
        TTR("Subtracts vector from vector."),
        VisualShaderNodeVectorOp::OP_SUB,
        VisualShaderNode::PORT_TYPE_VECTOR
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("Returns falloff based on the dot product of surface normal and "
            "view direction of camera (pass associated inputs to it)."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR
    ));
    add_options.push_back(AddOption(
    ...
    add_options.push_back(AddOption(
        ...
        TTR("A reference to an existing uniform.")
    ));
    ...
    add_options.push_back(AddOption(
        ...
        TTR("(Fragment/Light mode only) Scalar derivative function."),
        -1,
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        TTR("(Fragment/Light mode only) Vector derivative function."),
        -1,
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Derivative",
        "VisualShaderNodeVectorDerivativeFunc",
        TTR("(Fragment/Light mode only) (Vector) Derivative in 'x' using local "
            "differencing."),
        VisualShaderNodeVectorDerivativeFunc::FUNC_X,
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Derivative",
        "VisualShaderNodeScalarDerivativeFunc",
        TTR("(Fragment/Light mode only) (Scalar) Derivative in 'x' using local "
            "differencing."),
        VisualShaderNodeScalarDerivativeFunc::FUNC_X,
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Derivative",
        "VisualShaderNodeVectorDerivativeFunc",
        TTR("(Fragment/Light mode only) (Vector) Derivative in 'y' using local "
            "differencing."),
        VisualShaderNodeVectorDerivativeFunc::FUNC_Y,
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Derivative",
        "VisualShaderNodeScalarDerivativeFunc",
        TTR("(Fragment/Light mode only) (Scalar) Derivative in 'y' using local "
            "differencing."),
        VisualShaderNodeScalarDerivativeFunc::FUNC_Y,
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Derivative",
        "VisualShaderNodeVectorDerivativeFunc",
        TTR("(Fragment/Light mode only) (Vector) Sum of absolute derivative in "
            "'x' and 'y'."),
        VisualShaderNodeVectorDerivativeFunc::FUNC_SUM,
        VisualShaderNode::PORT_TYPE_VECTOR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
        -1,
    ...
    add_options.push_back(AddOption(
        ...
        "Derivative",
        "VisualShaderNodeScalarDerivativeFunc",
        TTR("(Fragment/Light mode only) (Scalar) Sum of absolute derivative in "
            "'x' and 'y'."),
        VisualShaderNodeScalarDerivativeFunc::FUNC_SUM,
        VisualShaderNode::PORT_TYPE_SCALAR,
        VisualShader::TYPE_FRAGMENT | VisualShader::TYPE_LIGHT,
        -1,
        -1,
...
protected:
    static void _bind_methods() {
        ClassDB::bind_method(
            "_item_selected",
            &VisualShaderNodePluginInputEditor::_item_selected
        );
    }
...
public:
    ...
    void _item_selected(int p_item) {
        VisualShaderEditor::get_singleton()
            ->call_deferred("_input_select_item", input, get_item_text(p_item));
    }

    void setup(const Ref<VisualShaderNodeInput>& p_input) {
        input                     = p_input;
        Ref<Texture> type_icon[5] = {
            EditorNode::get_singleton()->get_gui_base()->get_icon(
                "float",
                "EditorIcons"
            ),
            EditorNode::get_singleton()->get_gui_base()->get_icon(
        ...
        for (int i = 0; i < input->get_input_index_count(); i++) {
            ...
            add_icon_item(
                type_icon[input->get_input_index_type(i)],
                input->get_input_index_name(i)
            );
        }
...
protected:
    static void _bind_methods() {
        ClassDB::bind_method(
            "_item_selected",
            &VisualShaderNodePluginUniformRefEditor::_item_selected
        );
    }
...
public:
    ...
    void setup(const Ref<VisualShaderNodeUniformRef>& p_uniform_ref) {
        ...
        for (int i = 0; i < p_uniform_ref->get_uniforms_count(); i++) {
            if (p_uniform_ref->get_uniform_name()
                == p_uniform_ref->get_uniform_name_by_index(i)) {
                to_select = i + 1;
            }
            add_icon_item(
                type_icon[p_uniform_ref->get_uniform_type_by_index(i)],
                p_uniform_ref->get_uniform_name_by_index(i)
            );
        }
...
public:
    ...
    void setup(
        Ref<Resource> p_parent_resource,
        Vector<EditorProperty*> p_properties,
        const Vector<StringName>& p_names,
        Ref<VisualShaderNode> p_node
    ) {
        ...
        for (int i = 0; i < p_properties.size(); i++) {
            ...
            bool res_prop =
                Object::cast_to<EditorPropertyResource>(p_properties[i]);
            if (res_prop) {
                p_properties[i]
            ...
            properties[i]
                ->connect("property_changed", this, "_property_changed");
    ...
    static void _bind_methods() {
        ClassDB::bind_method(
            "_property_changed",
            &VisualShaderNodePluginDefaultEditor::_property_changed,
            DEFVAL(String()),
            DEFVAL(false)
        );
        ClassDB::bind_method(
            "_node_changed",
            &VisualShaderNodePluginDefaultEditor::_node_changed
        );
        ClassDB::bind_method(
            "_refresh_request",
            &VisualShaderNodePluginDefaultEditor::_refresh_request
        );
        ClassDB::bind_method(
            "_resource_selected",
            &VisualShaderNodePluginDefaultEditor::_resource_selected
        );
        ClassDB::bind_method(
            "_open_inspector",
            &VisualShaderNodePluginDefaultEditor::_open_inspector
        );
    }
};

Control* VisualShaderNodePluginDefault::create_editor(
    const Ref<Resource>& p_parent_resource,
    const Ref<VisualShaderNode>& p_node
) {
    if (p_node->is_class("VisualShaderNodeUniformRef")) {
        // create input
        VisualShaderNodePluginUniformRefEditor* uniform_editor =
            memnew(VisualShaderNodePluginUniformRefEditor);
    ...
    if (p_node->is_class("VisualShaderNodeInput")) {
        // create input
        VisualShaderNodePluginInputEditor* input_editor =
            memnew(VisualShaderNodePluginInputEditor);
    ...
    for (int i = 0; i < pinfo.size(); i++) {
        EditorProperty* prop = EditorInspector::instantiate_property_editor(
            node.ptr(),
            pinfo[i].type,
            pinfo[i].name,
            pinfo[i].hint,
        ...
        if (Object::cast_to<EditorPropertyResource>(prop)) {
            ...
            prop->set_custom_minimum_size(Size2(100 * EDSCALE, 0));
            Object::cast_to<EditorPropertyEnum>(prop)->set_option_button_clip(
                false
            );
        }
    ...
    VisualShaderNodePluginDefaultEditor* editor =
        memnew(VisualShaderNodePluginDefaultEditor);
...
void EditorPropertyShaderMode::_option_selected(int p_which) {
    ...
    Ref<VisualShader> visual_shader(
        Object::cast_to<VisualShader>(get_edited_object())
    );
    ...
    for (int i = 0; i < VisualShader::TYPE_MAX; i++) {
        ...
        for (int j = 0; j < nodes.size(); j++) {
            Ref<VisualShaderNodeInput> input =
                visual_shader->get_node(type, nodes[j]);
            if (!input.is_valid()) {
                continue;
            }
    ...
    for (List<PropertyInfo>::Element* E = props.front(); E; E = E->next()) {
        if (E->get().name.begins_with("flags/")
            || E->get().name.begins_with("modes/")) {
            undo_redo->add_undo_property(
                visual_shader.ptr(),
                E->get().name,
                visual_shader->get(E->get().name)
            );
        }
    }
...
void EditorPropertyShaderMode::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_option_selected"),
        &EditorPropertyShaderMode::_option_selected
    );
}
...
bool EditorInspectorShaderModePlugin::parse_property(
    Object* p_object,
    Variant::Type p_type,
    const String& p_path,
    PropertyHint p_hint,
    ...
    if (p_path == "mode" && p_object->is_class("VisualShader")
        && p_type == Variant::INT) {
    ...
    return false; // can be overridden, although it will most likely be last
                  // anyway
}
...
void VisualShaderNodePortPreview::_shader_changed() {
    ...
    String shader_code =
        shader->generate_preview_shader(type, node, port, default_textures);

    Ref<Shader> preview_shader;
    preview_shader.instance();
    preview_shader->set_code(shader_code);
    for (int i = 0; i < default_textures.size(); i++) {
        preview_shader->set_default_texture_param(
            default_textures[i].name,
            default_textures[i].param
        );
    }
    ...
    for (int i =
             EditorNode::get_singleton()->get_editor_history()->get_path_size()
             - 1;
         i >= 0;
         i--) {
        Object* object = ObjectDB::get_instance(
            EditorNode::get_singleton()->get_editor_history()->get_path_object(i
            )
        );
        ...
        if (object->has_method("get_material_override"
            )) { // trying getting material from MeshInstance
            src_mat = Object::cast_to<ShaderMaterial>(
                object->call("get_material_override")
            );
        } else if (object->has_method("get_material"
                   )) { // from CanvasItem/Node2D
            src_mat =
                Object::cast_to<ShaderMaterial>(object->call("get_material"));
        } else {
        ...
        if (src_mat && src_mat->get_shader().is_valid()) {
            ...
            for (List<PropertyInfo>::Element* E = params.front(); E;
                 E                              = E->next()) {
...
void VisualShaderNodePortPreview::setup(
...
void VisualShaderNodePortPreview::_bind_methods() {
    ClassDB::bind_method(
        "_shader_changed",
        &VisualShaderNodePortPreview::_shader_changed
    );
}
...
bool VisualShaderConversionPlugin::handles(const Ref<Resource>& p_resource
) const {
    Ref<VisualShader> vshader = p_resource;
    return vshader.is_valid();
}

Ref<Resource> VisualShaderConversionPlugin::convert(
