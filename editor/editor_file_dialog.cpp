    ...
    if (p_what == NOTIFICATION_ENTER_TREE) {
        ...
        if (preview_waiting) {
            ...
            if (preview_wheel_timeout <= 0) {
                ...
                Ref<Texture> frame = get_icon(
                    "Progress" + itos(preview_wheel_index + 1),
                    "EditorIcons"
                );
        ...
        bool is_showing_hidden = EditorSettings::get_singleton()->get(
        ...
        fav_up->set_icon(get_icon("MoveUp", "EditorIcons"));
        fav_down->set_icon(get_icon("MoveDown", "EditorIcons"));
        // DO NOT CALL UPDATE FILE LIST HERE, ALL HUNDREDS OF HIDDEN DIALOGS
        // WILL RESPOND, CALL INVALIDATE INSTEAD
        invalidate();
    }
}
...
void EditorFileDialog::set_enable_multiple_selection(bool p_enable) {
    item_list->set_select_mode(
        p_enable ? ItemList::SELECT_MULTI : ItemList::SELECT_SINGLE
    );
};
...
void EditorFileDialog::_post_popup() {
    ...
    if (is_visible_in_tree()) {
        ...
        Vector<String> recentd =
            EditorSettings::get_singleton()->get_recent_dirs();
        for (int i = 0; i < recentd.size(); i++) {
            ...
            if (!exists) {
                ...
                recent->set_item_metadata(
                    recent->get_item_count() - 1,
                    recentd[i]
                );
                recent->set_item_icon_modulate(
                    recent->get_item_count() - 1,
                    folder_color
                );
            }
...
void EditorFileDialog::_thumbnail_result(
    const String& p_path,
...
void EditorFileDialog::_thumbnail_done(
    const String& p_path,
    const Ref<Texture>& p_preview,
    const Ref<Texture>& p_small_preview,
    const Variant& p_udata
...
void EditorFileDialog::_action_pressed() {
    ...
    if ((mode == MODE_OPEN_ANY || mode == MODE_OPEN_FILE)
        && dir_access->file_exists(f)) {
        _save_to_recent();
        hide();
        emit_signal("file_selected", f);
...
void EditorFileDialog::_push_history() {
    ...
    if (local_history.size() == 0
        || new_path != local_history[local_history_pos]) {
...
void EditorFileDialog::_item_list_item_rmb_selected(
    int p_item,
    const Vector2& p_pos
) {
...
void EditorFileDialog::_item_list_rmb_clicked(const Vector2& p_pos) {
    // Right click on folder background. Deselect all files so that actions are
    // applied on the current folder.
    for (int i = 0; i < item_list->get_item_count(); i++) {
...
void EditorFileDialog::_item_menu_id_pressed(int p_option) {
    switch (p_option) {
        case ITEM_MENU_COPY_PATH: {
            Dictionary item_meta =
                item_list->get_item_metadata(item_list->get_current());
...
bool EditorFileDialog::_is_open_should_be_disabled() {
    ...
    if (items.size() == 0) {
        return mode != MODE_OPEN_DIR; // In "Open folder" mode, having nothing
                                      // selected picks the current folder.
    }

    for (int i = 0; i < items.size(); i++) {
        Dictionary d = item_list->get_item_metadata(items.get(i));

        if (((mode == MODE_OPEN_FILE || mode == MODE_OPEN_FILES) && d["dir"])
...
void EditorFileDialog::update_file_name() {
    ...
    if ((idx == -1 && filter->get_item_count() == 2)
        || (filter->get_item_count() > 2 && idx >= 0
            && idx < filter->get_item_count() - 2)) {
        if (idx == -1) {
            idx += 1;
        }
        ...
        if (filter_substr.size() >= 2) {
            file_str = base_name + "."
                     + filter_substr[0].strip_edges().lstrip("*.").to_lower();
        } else {
            file_str = base_name + "."
                     + filter_str.get_extension().strip_edges().to_lower();
        }
...
void EditorFileDialog::update_file_list() {
    int thumbnail_size = EditorSettings::get_singleton()->get(
    ...
    while (!files.empty()) {
        ...
        if (match) {
            ...
            if (get_icon_func) {
                Ref<Texture> icon =
                    get_icon_func(cdir.plus_file(files.front()->get()));
                if (display_mode == DISPLAY_THUMBNAILS) {
                    item_list->set_item_icon(
                        item_list->get_item_count() - 1,
                        file_thumbnail
                    );
                    item_list->set_item_tag_icon(
                        item_list->get_item_count() - 1,
                        icon
                    );
                } else {
    ...
    for (int i = 0; i < favorites->get_item_count(); i++) {
        if (favorites->get_item_metadata(i) == cdir
            || favorites->get_item_metadata(i) == cdir + "/") {
            favorites->select(i);
...
void EditorFileDialog::_make_dir_confirm() {
    ...
    if (err == OK) {
        ...
        ); // we created a dir, so rescan changes
    } else {
...
void EditorFileDialog::_delete_items() {
    // Collect the selected folders and files to delete and check them in the
    // deletion dependency dialog.
...
void EditorFileDialog::_update_drives() {
    ...
    if (dc == 0 || access != ACCESS_FILESYSTEM) {
        ...
        dp = dir_access->drives_are_shortcuts() ? shortcuts_container
                                                : drives_container;
        dp->add_child(drives);
        drives->show();
...
void EditorFileDialog::_favorite_move_up() {
    ...
    if (current > 0 && current < favorites->get_item_count()) {
        Vector<String> favorited =
            EditorSettings::get_singleton()->get_favorites();

        int a_idx =
            favorited.find(String(favorites->get_item_metadata(current - 1)));
        int b_idx =
            favorited.find(String(favorites->get_item_metadata(current)));
...
void EditorFileDialog::_update_favorites() {
    ...
    for (int i = 0; i < favorited.size(); i++) {
        ...
        favorites->set_item_metadata(
            favorites->get_item_count() - 1,
            favorited[i]
        );
        favorites->set_item_icon_modulate(
            favorites->get_item_count() - 1,
            folder_color
        );
...
void EditorFileDialog::_bind_methods() {
    ClassDB::bind_method(
        ...
        &EditorFileDialog::_unhandled_input
    );

    ClassDB::bind_method(
        D_METHOD("_item_selected"),
        &EditorFileDialog::_item_selected
    );
    ClassDB::bind_method(
        D_METHOD("_multi_selected"),
        &EditorFileDialog::_multi_selected
    );
    ClassDB::bind_method(
        D_METHOD("_items_clear_selection"),
        &EditorFileDialog::_items_clear_selection
    );
    ClassDB::bind_method(
        D_METHOD("_item_list_item_rmb_selected"),
        &EditorFileDialog::_item_list_item_rmb_selected
    );
    ClassDB::bind_method(
        D_METHOD("_item_list_rmb_clicked"),
        &EditorFileDialog::_item_list_rmb_clicked
    );
    ClassDB::bind_method(
        D_METHOD("_item_menu_id_pressed"),
        &EditorFileDialog::_item_menu_id_pressed
    );
    ClassDB::bind_method(
        D_METHOD("_item_db_selected"),
        &EditorFileDialog::_item_dc_selected
    );
    ClassDB::bind_method(
        D_METHOD("_dir_entered"),
        &EditorFileDialog::_dir_entered
    );
    ClassDB::bind_method(
        D_METHOD("_file_entered"),
        &EditorFileDialog::_file_entered
    );
    ClassDB::bind_method(
        D_METHOD("_action_pressed"),
        &EditorFileDialog::_action_pressed
    );
    ClassDB::bind_method(
        D_METHOD("_cancel_pressed"),
        &EditorFileDialog::_cancel_pressed
    );
    ClassDB::bind_method(
        D_METHOD("_filter_selected"),
        &EditorFileDialog::_filter_selected
    );
    ClassDB::bind_method(
        D_METHOD("_save_confirm_pressed"),
        &EditorFileDialog::_save_confirm_pressed
    );

    ClassDB::bind_method(
        D_METHOD("clear_filters"),
        &EditorFileDialog::clear_filters
    );
    ClassDB::bind_method(
        D_METHOD("add_filter", "filter"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_current_dir", "dir"),
        &EditorFileDialog::set_current_dir
    );
    ClassDB::bind_method(
        D_METHOD("set_current_file", "file"),
        &EditorFileDialog::set_current_file
    );
    ClassDB::bind_method(
        D_METHOD("set_current_path", "path"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_access", "access"),
        &EditorFileDialog::set_access
    );
    ClassDB::bind_method(D_METHOD("get_access"), &EditorFileDialog::get_access);
    ClassDB::bind_method(
        D_METHOD("set_show_hidden_files", "show"),
        &EditorFileDialog::set_show_hidden_files
    );
    ClassDB::bind_method(
        D_METHOD("is_showing_hidden_files"),
        &EditorFileDialog::is_showing_hidden_files
    );
    ClassDB::bind_method(
        D_METHOD("_select_drive"),
        &EditorFileDialog::_select_drive
    );
    ClassDB::bind_method(D_METHOD("_make_dir"), &EditorFileDialog::_make_dir);
    ClassDB::bind_method(
        D_METHOD("_make_dir_confirm"),
        &EditorFileDialog::_make_dir_confirm
    );
    ClassDB::bind_method(
        D_METHOD("_update_file_name"),
        &EditorFileDialog::update_file_name
    );
    ClassDB::bind_method(
        D_METHOD("_update_file_list"),
        &EditorFileDialog::update_file_list
    );
    ClassDB::bind_method(
        D_METHOD("_update_dir"),
        &EditorFileDialog::update_dir
    );
    ClassDB::bind_method(
        D_METHOD("_thumbnail_done"),
        &EditorFileDialog::_thumbnail_done
    );
    ClassDB::bind_method(
        D_METHOD("set_display_mode", "mode"),
        &EditorFileDialog::set_display_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_display_mode"),
        &EditorFileDialog::get_display_mode
    );
    ClassDB::bind_method(
        D_METHOD("_thumbnail_result"),
        &EditorFileDialog::_thumbnail_result
    );
    ClassDB::bind_method(
        D_METHOD("set_disable_overwrite_warning", "disable"),
        &EditorFileDialog::set_disable_overwrite_warning
    );
    ClassDB::bind_method(
        D_METHOD("is_overwrite_warning_disabled"),
        &EditorFileDialog::is_overwrite_warning_disabled
    );

    ClassDB::bind_method(
        D_METHOD("_recent_selected"),
        &EditorFileDialog::_recent_selected
    );
    ClassDB::bind_method(D_METHOD("_go_back"), &EditorFileDialog::_go_back);
    ClassDB::bind_method(
        D_METHOD("_go_forward"),
        &EditorFileDialog::_go_forward
    );
    ClassDB::bind_method(D_METHOD("_go_up"), &EditorFileDialog::_go_up);

    ClassDB::bind_method(
        D_METHOD("_favorite_pressed"),
        &EditorFileDialog::_favorite_pressed
    );
    ClassDB::bind_method(
        D_METHOD("_favorite_selected"),
        &EditorFileDialog::_favorite_selected
    );
    ClassDB::bind_method(
        D_METHOD("_favorite_move_up"),
        &EditorFileDialog::_favorite_move_up
    );
    ClassDB::bind_method(
        D_METHOD("_favorite_move_down"),
        &EditorFileDialog::_favorite_move_down
    );

    ClassDB::bind_method(D_METHOD("invalidate"), &EditorFileDialog::invalidate);

    ADD_SIGNAL(
        MethodInfo("file_selected", PropertyInfo(Variant::STRING, "path"))
    );
    ADD_SIGNAL(MethodInfo(
        "files_selected",
        PropertyInfo(Variant::POOL_STRING_ARRAY, "paths")
    ));
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "mode",
            PROPERTY_HINT_ENUM,
            "Open one,Open many,Open folder,Open any,Save"
        ),
        "set_mode",
        "get_mode"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "current_dir", PROPERTY_HINT_DIR),
        "set_current_dir",
        "get_current_dir"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "current_file", PROPERTY_HINT_FILE, "*"),
        "set_current_file",
        "get_current_file"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "current_path"),
        "set_current_path",
        "get_current_path"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "show_hidden_files"),
        "set_show_hidden_files",
        "is_showing_hidden_files"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "disable_overwrite_warning"),
...
EditorFileDialog::EditorFileDialog() {
    ...
    ED_SHORTCUT(
        ...
        TTR("Move Favorite Up"),
        KEY_MASK_CMD | KEY_UP
    );
    ED_SHORTCUT(
        "file_dialog/move_favorite_down",
        TTR("Move Favorite Down"),
    ...
    mode_thumbnails->connect(
        "pressed",
        this,
        "set_display_mode",
        varray(DISPLAY_THUMBNAILS)
    );
    ...
    mode_list
        ->connect("pressed", this, "set_display_mode", varray(DISPLAY_LIST));
    ...
    item_list
        ->connect("item_rmb_selected", this, "_item_list_item_rmb_selected");
...
void EditorLineEditFileChooser::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_browse"),
        &EditorLineEditFileChooser::_browse
    );
    ClassDB::bind_method(
        D_METHOD("_chosen"),
        &EditorLineEditFileChooser::_chosen
    );
    ClassDB::bind_method(
        D_METHOD("get_button"),
        &EditorLineEditFileChooser::get_button
    );
    ClassDB::bind_method(
        D_METHOD("get_line_edit"),
        &EditorLineEditFileChooser::get_line_edit
    );
    ClassDB::bind_method(
        D_METHOD("get_file_dialog"),
        &EditorLineEditFileChooser::get_file_dialog
    );
}
