...
void SceneTreeDock::_nodes_drag_begin() {
    if (restore_script_editor_on_drag) {
        EditorNode::get_singleton()->set_visible_editor(
...
void SceneTreeDock::_quick_open() {
    instance_scenes(
        quick_open->get_selected_files(),
        scene_tree->get_selected()
    );
}
...
void SceneTreeDock::instance_scenes(
...
void SceneTreeDock::_perform_instance_scenes(
    const Vector<String>& p_files,
    Node* parent,
    int p_pos
) {
    ...
    for (int i = 0; i < p_files.size(); i++) {
        ...
        if (!sdata.is_valid()) {
            current_option = -1;
            accept->set_text(
                vformat(TTR("Error loading scene from %s"), p_files[i])
            );
            accept->popup_centered_minsize();
            error = true;
            break;
        ...
        if (!instanced_scene) {
            current_option = -1;
            accept->set_text(
                vformat(TTR("Error instancing scene from %s"), p_files[i])
            );
            accept->popup_centered_minsize();
            error = true;
            break;
        ...
        if (edited_scene->get_filename() != "") {
            if (_cyclical_dependency_exists(
                    edited_scene->get_filename(),
                    instanced_scene
                )) {
                accept->set_text(vformat(
                    TTR("Cannot instance the scene '%s' because the current "
                        "scene exists within one of its nodes."),
                    p_files[i]
                ));
                accept->popup_centered_minsize();
                error = true;
    ...
    for (int i = 0; i < instances.size(); i++) {
        ...
        editor_data->get_undo_redo()
            .add_do_method(instanced_scene, "set_owner", edited_scene);
        editor_data->get_undo_redo().add_do_method(editor_selection, "clear");
        editor_data->get_undo_redo()
            .add_do_method(editor_selection, "add_node", instanced_scene);
        ...
        editor_data->get_undo_redo().add_do_method(
            ...
            edited_scene->get_path_to(parent),
            p_files[i],
            new_name
        );
        editor_data->get_undo_redo().add_undo_method(
            ...
            NodePath(
                String(edited_scene->get_path_to(parent)).plus_file(new_name)
            )
        );
    }
...
void SceneTreeDock::_replace_with_branch_scene(
    const String& p_file,
    Node* base
) {
...
bool SceneTreeDock::_track_inherit(
    const String& p_target_scene_path,
    Node* p_desired_node
) {
...
void SceneTreeDock::_tool_selected(int p_tool, bool p_confirm_override) {
    ...
    switch (p_tool) {
        ...
        case TOOL_REPARENT_TO_NEW_NODE: {
            ...
            Node* current_edited_scene_root =
            ...
            if (current_edited_scene_root) {
                ...
                for (int i = 0; i < preferred_types.size(); i++) {
                    if (ClassDB::is_parent_class(
                        ...
                        create_dialog->set_preferred_search_result_type(
                            preferred_types[i]
                        );
                        break;
                    }
        ...
        case TOOL_PASTE: {
            ...
            if (has_cycle) {
                current_option = -1;
                accept->set_text(
                    TTR("Can't paste root node into the same scene.")
                );
            ...
            for (List<Node*>::Element* E = node_clipboard.front(); E;
                ...
                Node* dup =
                    node->duplicate_from_editor(duplimap, resource_remap);

                ERR_CONTINUE(!dup);

                editor_data->get_undo_redo()
                    .add_do_method(paste_parent, "add_child", dup);

                for (Map<const Node*, Node*>::Element* E2 = duplimap.front();
                     E2;
                     E2 = E2->next()) {
                    Node* d = E2->value();
                    editor_data->get_undo_redo()
                        .add_do_method(d, "set_owner", owner);
                }

                editor_data->get_undo_redo()
                    .add_do_method(dup, "set_owner", owner);
                editor_data->get_undo_redo()
                    .add_do_method(editor_selection, "add_node", dup);
                editor_data->get_undo_redo()
                    .add_undo_method(paste_parent, "remove_child", dup);
                editor_data->get_undo_redo().add_do_reference(dup);

                if (node_clipboard.size() == 1) {
                    editor_data->get_undo_redo()
                        .add_do_method(editor, "push_item", dup);
                }
            }
        ...
        case TOOL_REPLACE: {
            ...
            if (!selected
                ...
                selected =
                    editor_selection->get_selected_node_list().front()->get();
            }
        ...
        case TOOL_DETACH_SCRIPT: {
            ...
            for (int i = 0; i < selection.size(); i++) {
                ...
                if (existing != empty) {
                    editor_data->get_undo_redo()
                        .add_do_method(n, "set_script", empty);
                    editor_data->get_undo_redo()
        ...
        case TOOL_MOVE_DOWN: {
            ...
            if (scene_tree->get_selected() == edited_scene) {
                current_option = -1;
                accept->set_text(
                    TTR("This operation can't be done on the tree root.")
                );
                accept->popup_centered_minsize();
                break;
            }
            ...
            if (!common_parent
                || (MOVING_DOWN
                    && highest_id
                           >= common_parent->get_child_count() - MOVING_DOWN)
                || (MOVING_UP && lowest_id == 0)) {
                break; // one or more nodes can not be moved
            }

            if (selection.size() == 1) {
                editor_data->get_undo_redo().create_action(
                    TTR("Move Node In Parent")
                );
            }
            if (selection.size() > 1) {
                editor_data->get_undo_redo().create_action(
                    TTR("Move Nodes In Parent")
                );
            }
            ...
            for (int i = 0; i < selection.size(); i++) {
                ...
                int top_node_pos_next =
                    top_node->get_index() + (MOVING_DOWN ? 1 : -1);
        ...
        case TOOL_DUPLICATE: {
            ...
            for (List<Node*>::Element* E = selection.front(); E;
                ...
                for (List<Node*>::Element* F = owned.front(); F;
                    ...
                    editor_data->get_undo_redo()
                        .add_do_method(d, "set_owner", node->get_owner());
                }
                editor_data->get_undo_redo()
                    .add_do_method(editor_selection, "add_node", dup);
                editor_data->get_undo_redo()
                    .add_undo_method(parent, "remove_child", dup);
                editor_data->get_undo_redo().add_do_reference(dup);
                ...
                editor_data->get_undo_redo().add_do_method(
                    ...
                    edited_scene->get_path_to(node),
                    dup->get_name()
                );
                editor_data->get_undo_redo().add_undo_method(
                    sed,
                    "live_debug_remove_node",
                    NodePath(String(edited_scene->get_path_to(parent))
                                 .plus_file(dup->get_name()))
                );
        ...
        case TOOL_MAKE_ROOT: {
            ...
            if (root->get_scene_inherited_state().is_valid()) {
                accept->set_text(
                    TTR("Can't reparent nodes in inherited scenes, order of "
                        "nodes can't change.")
                );
                accept->popup_centered_minsize();
                return;
            ...
            if (node->get_owner() != root) {
                accept->set_text(
                    TTR("Node must belong to the edited scene to become root.")
                );
                accept->popup_centered_minsize();
                return;
            }
            ...
            editor_data->get_undo_redo()
                .add_do_method(node->get_parent(), "remove_child", node);
            editor_data->get_undo_redo()
                .add_do_method(editor, "set_edited_scene", node);
            editor_data->get_undo_redo().add_do_method(node, "add_child", root);
            editor_data->get_undo_redo()
                .add_do_method(node, "set_filename", root->get_filename());
            editor_data->get_undo_redo()
                .add_do_method(root, "set_filename", String());
            editor_data->get_undo_redo()
                .add_do_method(node, "set_owner", (Object*)nullptr);
            editor_data->get_undo_redo().add_do_method(root, "set_owner", node);
            _node_replace_owner(root, root, node, MODE_DO);

            editor_data->get_undo_redo()
                .add_undo_method(root, "set_filename", root->get_filename());
            editor_data->get_undo_redo()
                .add_undo_method(node, "set_filename", String());
            editor_data->get_undo_redo()
                .add_undo_method(node, "remove_child", root);
            editor_data->get_undo_redo()
                .add_undo_method(editor, "set_edited_scene", root);
            editor_data->get_undo_redo()
                .add_undo_method(node->get_parent(), "add_child", node);
            editor_data->get_undo_redo().add_undo_method(
                node->get_parent(),
                "move_child",
                node,
            ...
            editor_data->get_undo_redo()
                .add_undo_method(root, "set_owner", (Object*)nullptr);
            editor_data->get_undo_redo()
                .add_undo_method(node, "set_owner", root);
        ...
        case TOOL_MULTI_EDIT: {
            ...
            for (const Map<Node*, Object*>::Element* E =
        ...
        case TOOL_ERASE: {
            ...
            if (p_confirm_override) {
                ...
                if (remove_list.size() > 1) {
                    ...
                    for (int i = 0; !any_children && i < remove_list.size();
                         i++) {
                        any_children = remove_list[i]->get_child_count() > 0;
                    }

                    msg = vformat(
                        any_children ? TTR("Delete %d nodes and any children?")
                                     : TTR("Delete %d nodes?"),
                        remove_list.size()
                    );
                } else {
        ...
        case TOOL_NEW_SCENE_FROM: {
            ...
            if (!scene) {
                accept->set_text(
                    TTR("Saving the branch as a scene requires having a scene "
                        "open in the editor.")
                );
                accept->popup_centered_minsize();
                break;
            ...
            if (selection.size() != 1) {
                accept->set_text(vformat(
                    TTR("Saving the branch as a scene requires selecting only "
                        "one node, but you have selected %d nodes."),
                    selection.size()
                ));
                accept->popup_centered_minsize();
                break;
            ...
            if (tocopy->get_owner() != scene) {
                accept->set_text(
                    TTR("Can't save a branch which is a child of an already "
                        "instantiated scene.\nTo save this branch into its own "
                        "scene, open the original scene, right click on this "
            ...
            if (scene->get_scene_inherited_state().is_valid()
                && scene->get_scene_inherited_state()->find_node_by_path(
                       scene->get_path_to(tocopy)
                   ) >= 0) {
                accept->set_text(
                    TTR("Can't save a branch which is part of an inherited "
                        "scene.\nTo save this branch into its own scene, open "
                        "the original scene, right click on this branch, and "
            ...
            for (int i = 0; i < extensions.size(); i++) {
                new_scene_from_dialog->add_filter(
            ...
            if (e) {
                ...
                if (node) {
                    Node* root =
                        EditorNode::get_singleton()->get_edited_scene();
                    NodePath path =
                        root->get_path().rel_path_to(node->get_path());
        ...
        case TOOL_OPEN_DOCUMENTATION: {
            List<Node*> selection = editor_selection->get_selected_node_list();
            for (int i = 0; i < selection.size(); i++) {
                ScriptEditor::get_singleton()->goto_help(
                    "class_name:" + selection[i]->get_class()
                );
            }
            EditorNode::get_singleton()->set_visible_editor(
                EditorNode::EDITOR_SCRIPT
                ...
                if (node) {
                    ...
                    if (editable) {
                        editable_instance_remove_dialog->set_text(
                            TTR("Disabling \"editable_instance\" will cause "
                                "all properties of the node to be reverted to "
                                "their default.")
                        );
            ...
            if (e) {
                ...
                if (node) {
                    ...
                    if (editable && !placeholder) {
                        placeholder_editable_instance_remove_dialog->set_text(
                            TTR("Enabling \"Load As Placeholder\" will disable "
                                "\"Editable Children\" and cause all "
                                "properties of the node to be reverted to "
                                "their default.")
                        );
                    ...
                    if (placeholder) {
                        EditorNode::get_singleton()
                            ->get_edited_scene()
                            ->set_editable_instance(node, false);
                    }
        ...
        case TOOL_SCENE_OPEN_INHERITED: {
            ...
            if (e) {
                ...
                if (node && node->get_scene_inherited_state().is_valid()) {
                    scene_tree->emit_signal(
                        "open",
                        node->get_scene_inherited_state()->get_path()
                    );
                }
            }
        } break;
        ...
        case TOOL_CREATE_FAVORITE: {
            ...
            if (TOOL_CREATE_FAVORITE == p_tool) {
                ...
                if (ScriptServer::is_global_class(name)) {
                    new_node = Object::cast_to<Node>(ClassDB::instance(
                        ScriptServer::get_global_class_native_base(name)
                    ));
                    Ref<Script> script = ResourceLoader::load(
                    ...
                    new_node = Object::cast_to<Node>(
                        ClassDB::instance(selected_favorite_root)
                    );
                }
                ...
                if (!new_node) {
                    ...
                    ERR_PRINT(
                        "Creating root from favorite '" + selected_favorite_root
            ...
            editor_data->get_undo_redo()
                .add_do_method(editor, "set_edited_scene", new_node);
            editor_data->get_undo_redo().add_do_method(
                scene_tree,
                "update_tree"
            );
...
void SceneTreeDock::_property_selected(int p_idx) {
    ...
    _perform_property_drop(
        property_drop_node,
        menu_properties->get_item_metadata(p_idx),
        ResourceLoader::load(resource_drop_path)
    );
    property_drop_node = nullptr;
}

void SceneTreeDock::_perform_property_drop(
    Node* p_node,
    String p_property,
    RES p_res
) {
    editor_data->get_undo_redo().create_action(
        vformat(TTR("Set %s"), p_property)
    );
    ...
    editor_data->get_undo_redo()
        .add_undo_property(p_node, p_property, p_node->get(p_property));
    editor_data->get_undo_redo().add_undo_method(
        p_node,
        "property_list_changed_notify"
    );
    ...
    switch (p_what) {
        case NOTIFICATION_READY: {
            ...
            CanvasItemEditorPlugin* canvas_item_plugin =
                Object::cast_to<CanvasItemEditorPlugin>(
                    editor_data->get_editor("2D")
                );
            if (canvas_item_plugin) {
                ...
                scene_tree->connect(
                    "node_changed",
                    canvas_item_plugin->get_canvas_item_editor()
                        ->get_viewport_control(),
                    "update"
                );
            }

            SpatialEditorPlugin* spatial_editor_plugin =
                Object::cast_to<SpatialEditorPlugin>(
                    editor_data->get_editor("3D")
                );
            ...
            button_create_script->set_icon(
                get_icon("ScriptCreate", "EditorIcons")
            );
            button_detach_script->set_icon(
                get_icon("ScriptRemove", "EditorIcons")
            );
            ...
            scene_tree->get_scene_tree()
                ->connect("item_collapsed", this, "_node_collapsed");
            ...
            node_shortcuts_toggle->set_pressed(
                EDITOR_GET("_use_favorites_root_selection")
            );
            node_shortcuts_toggle->set_anchors_and_margins_preset(
                Control::PRESET_CENTER_RIGHT
            );
            node_shortcuts_toggle
                ->connect("pressed", this, "_update_create_root_dialog");
            ...
            button_2d->connect(
                ...
                this,
                "_tool_selected",
                make_binds(TOOL_CREATE_2D_SCENE, false)
            );
            ...
            button_3d->connect(
                ...
                this,
                "_tool_selected",
                make_binds(TOOL_CREATE_3D_SCENE, false)
            );
            ...
            button_ui->connect(
                ...
                this,
                "_tool_selected",
                make_binds(TOOL_CREATE_USER_INTERFACE, false)
            );
            ...
            button_custom->connect(
                "pressed",
                this,
                "_tool_selected",
                make_binds(TOOL_NEW, false)
            );
        ...
        case NOTIFICATION_ENTER_TREE: {
            clear_inherit_confirm->connect(
                "confirmed",
                this,
                "_tool_selected",
                varray(TOOL_SCENE_CLEAR_INHERITANCE_CONFIRM)
            );
        } break;

        case NOTIFICATION_EXIT_TREE: {
            clear_inherit_confirm
                ->disconnect("confirmed", this, "_tool_selected");
        } break;
...
void SceneTreeDock::_fill_path_renames(
...
bool SceneTreeDock::_update_node_path(
    ...
    Map<Node*, NodePath>::Element* found_node_path =
        p_renames->find(target_node);
    if (found_node_path) {
        Map<Node*, NodePath>::Element* found_root_path =
            p_renames->find(p_root_node);
        NodePath root_path_new =
            found_root_path ? found_root_path->get() : p_root_node->get_path();
    ...
    if (found_root_path) {
        ...
        if (!root_path_new.is_empty()) {
            NodePath old_abs_path =
                NodePath(String(p_root_node->get_path()).plus_file(r_node_path)
                );
...
bool SceneTreeDock::_check_node_path_recursive(
    Node* p_root_node,
    Variant& r_variant,
    Map<Node*, NodePath>* p_renames
) const {
    switch (r_variant.get_type()) {
        case Variant::NODE_PATH: {
            ...
            if (!node_path.is_empty()
                && _update_node_path(p_root_node, node_path, p_renames)) {
                r_variant = node_path;
                return true;
            }
        } break;
        ...
        case Variant::ARRAY: {
            ...
            for (int i = 0; i < a.size(); i++) {
                ...
                if (_check_node_path_recursive(p_root_node, value, p_renames)) {
                    if (!updated) {
                        a = a.duplicate(
                        ); // Need to duplicate for undo-redo to work.
                        updated = true;
                    }
                    a[i] = value;
                }
...
void SceneTreeDock::perform_node_renames(
    Node* p_base,
    Map<Node*, NodePath>* p_renames,
    Map<Ref<Animation>, Set<int>>* r_rem_anims
) {
    ...
    for (List<PropertyInfo>::Element* E = properties.front(); E;
        ...
        if (_check_node_path_recursive(p_base, updated_variant, p_renames)) {
            editor_data->get_undo_redo()
                .add_do_property(p_base, propertyname, updated_variant);
            editor_data->get_undo_redo()
                .add_undo_property(p_base, propertyname, old_variant);
            p_base->set(propertyname, updated_variant);
        }
    }

    bool autorename_animation_tracks =
        bool(EDITOR_DEF("editors/animation/autorename_animation_tracks", true));

    if (autorename_animation_tracks
        && Object::cast_to<AnimationPlayer>(p_base)) {
        ...
        if (root) {
            Map<Node*, NodePath>::Element* found_root_path =
                p_renames->find(root);
            NodePath new_root_path =
                found_root_path ? found_root_path->get() : root->get_path();
            if (!new_root_path.is_empty(
                ...
                for (List<StringName>::Element* E = anims.front(); E;
                    ...
                    for (int i = 0; i < anim->get_track_count(); i++) {
                        ...
                        Map<Node*, NodePath>::Element* found_path =
                            p_renames->find(n);
                        if (found_path) {
                            if (found_path->get() == NodePath()) {
                                ...
                                for (int j = 0;
                                    ...
                                    editor_data->get_undo_redo()
                                        .add_undo_method(
                                            ...
                                            "track_insert_key",
                                            idx,
                                            anim->track_get_key_time(i, j),
                                            anim->track_get_key_value(i, j),
                                ...
                                // will be renamed
                                NodePath rel_path =
                                    new_root_path.rel_path_to(found_path->get()
                                    );

                                NodePath new_path = NodePath(
                                    rel_path.get_names(),
                                    track_np.get_subnames(),
                                    false
                                );
...
bool SceneTreeDock::_validate_no_foreign() {
    ...
    for (List<Node*>::Element* E = selection.front(); E; E = E->next()) {
        ...
        // When edited_scene inherits from another one the root Node will be the
        // parent Scene, we don't want to consider that Node a foreign one
        // otherwise we would not be able to delete it.
        ...
        if (edited_scene->get_scene_inherited_state().is_valid()
            && edited_scene->get_scene_inherited_state()->find_node_by_path(
                   edited_scene->get_path_to(E->get())
               ) >= 0) {
            accept->set_text(
                TTR("Can't operate on nodes the current scene inherits from!")
            );
            accept->popup_centered_minsize();
            return false;
        }
...
bool SceneTreeDock::_validate_no_instance() {
    ...
    for (List<Node*>::Element* E = selection.front(); E; E = E->next()) {
        if (E->get() != edited_scene && E->get()->get_filename() != "") {
            accept->set_text(
                TTR("This operation can't be done on instanced scenes.")
            );
            accept->popup_centered();
            return false;
        }
...
void SceneTreeDock::_do_reparent(
    ...
    for (int ni = 0; ni < p_nodes.size(); ni++) {
        ...
        if (p_nodes[ni]->get_parent() != p_new_parent
            || p_position_in_parent + ni
                   != p_nodes[ni]->get_position_in_parent()) {
            no_change = false;
        }
    }
    ...
    while (validate) {
        ERR_FAIL_COND_MSG(
            p_nodes.find(validate) != -1,
            "Selection changed at some point. Can't reparent."
        );
    ...
    for (int ni = 0; ni < p_nodes.size(); ni++) {
        ...
        if (new_parent == node->get_parent()
            && node->get_index() < p_position_in_parent + ni) {
        ...
        editor_data->get_undo_redo()
            .add_do_method(node->get_parent(), "remove_child", node);
        editor_data->get_undo_redo()
            .add_do_method(new_parent, "add_child", node);
        ...
        editor_data->get_undo_redo().add_do_method(
            ...
            edited_scene->get_path_to(node),
            edited_scene->get_path_to(new_parent),
            new_name,
            p_position_in_parent + inc
        );
        editor_data->get_undo_redo().add_undo_method(
            sed,
            "live_debug_reparent_node",
            NodePath(String(edited_scene->get_path_to(new_parent))
                         .plus_file(new_name)),
            edited_scene->get_path_to(node->get_parent()),
            node->get_name(),
            node->get_index()
        );
        ...
        if (p_keep_global_xform) {
            if (Object::cast_to<Node2D>(node)) {
                editor_data->get_undo_redo().add_do_method(
                    ...
                    Object::cast_to<Node2D>(node)->get_global_transform()
                );
            }
            if (Object::cast_to<Spatial>(node)) {
                editor_data->get_undo_redo().add_do_method(
                    node,
                    "set_global_transform",
                    Object::cast_to<Spatial>(node)->get_global_transform()
                );
            }
            if (Object::cast_to<Control>(node)) {
                editor_data->get_undo_redo().add_do_method(
                    node,
                    "set_global_position",
                    Object::cast_to<Control>(node)->get_global_position()
                );
            }
        }

        editor_data->get_undo_redo()
            .add_do_method(this, "_set_owners", edited_scene, owners);

        if (AnimationPlayerEditor::singleton->get_track_editor()->get_root()
            == node) {
            editor_data->get_undo_redo().add_do_method(
        ...
        editor_data->get_undo_redo()
            .add_undo_method(new_parent, "remove_child", node);
        editor_data->get_undo_redo()
            .add_undo_method(node, "set_name", former_names[ni]);
    ...
    for (int ni = 0; ni < p_nodes.size(); ni++) {
        ...
        editor_data->get_undo_redo()
            .add_undo_method(node->get_parent(), "add_child", node);
        editor_data->get_undo_redo()
            .add_undo_method(node->get_parent(), "move_child", node, child_pos);
        editor_data->get_undo_redo()
            .add_undo_method(this, "_set_owners", edited_scene, owners);
        if (AnimationPlayerEditor::singleton->get_track_editor()->get_root()
            == node) {
            editor_data->get_undo_redo().add_undo_method(
                AnimationPlayerEditor::singleton->get_track_editor(),
        ...
        if (p_keep_global_xform) {
            if (Object::cast_to<Node2D>(node)) {
                editor_data->get_undo_redo().add_undo_method(
                    node,
                    "set_transform",
                    Object::cast_to<Node2D>(node)->get_transform()
                );
            }
            if (Object::cast_to<Spatial>(node)) {
                editor_data->get_undo_redo().add_undo_method(
                    node,
                    "set_transform",
                    Object::cast_to<Spatial>(node)->get_transform()
                );
            }
            if (Object::cast_to<Control>(node)) {
                editor_data->get_undo_redo().add_undo_method(
                    node,
                    "set_position",
                    Object::cast_to<Control>(node)->get_position()
                );
            }
        }
    }
...
void SceneTreeDock::_set_collapsed_recursive(
    TreeItem* p_item,
    bool p_collapsed
) {
...
void SceneTreeDock::_script_created(Ref<Script> p_script) {
    ...
    for (List<Node*>::Element* E = selected.front(); E; E = E->next()) {
        ...
        editor_data->get_undo_redo()
            .add_do_method(E->get(), "set_script", p_script.get_ref_ptr());
        editor_data->get_undo_redo()
            .add_undo_method(E->get(), "set_script", existing);
        editor_data->get_undo_redo().add_do_method(
            this,
            "_update_script_button"
        );
...
void SceneTreeDock::_toggle_editable_children(Node* p_node) {
    if (p_node) {
        bool editable = !EditorNode::get_singleton()
                             ->get_edited_scene()
                             ->is_editable_instance(p_node);
        EditorNode::get_singleton()->get_edited_scene()->set_editable_instance(
            p_node,
            editable
        );
        if (editable) {
...
void SceneTreeDock::_delete_confirm(bool p_cut) {
    ...
    if (entire_scene) {
        editor_data->get_undo_redo()
            .add_do_method(editor, "set_edited_scene", (Object*)nullptr);
        editor_data->get_undo_redo()
            .add_undo_method(editor, "set_edited_scene", edited_scene);
        editor_data->get_undo_redo().add_undo_method(
            edited_scene,
            "set_owner",
            edited_scene->get_owner()
        );
        ...
        remove_list.sort_custom<Node::Comparator>(
        ); // sort nodes to keep positions
        Map<Node*, NodePath> path_renames;
        ...
        for (List<Node*>::Element* E = remove_list.front(); E; E = E->next()) {
            ...
            editor_data->get_undo_redo()
                .add_do_method(n->get_parent(), "remove_child", n);
            editor_data->get_undo_redo()
                .add_undo_method(n->get_parent(), "add_child", n);
            editor_data->get_undo_redo().add_undo_method(
                n->get_parent(),
                "move_child",
                n,
            ...
            editor_data->get_undo_redo().add_undo_method(
                ...
                n->get_instance_id(),
                edited_scene->get_path_to(n->get_parent()),
                n->get_index()
            );
        }
    }
    ...
    EditorHistory* editor_history =
        EditorNode::get_singleton()->get_editor_history();
...
Node* SceneTreeDock::_get_selection_group_tail(
    Node* p_node,
    List<Node*> p_list
) {
    Node* tail   = p_node;
...
void SceneTreeDock::_do_create(Node* p_parent) {
    ...
    if (edited_scene) {
        editor_data->get_undo_redo()
            .add_do_method(p_parent, "add_child", child);
        editor_data->get_undo_redo()
            .add_do_method(child, "set_owner", edited_scene);
        editor_data->get_undo_redo().add_do_method(editor_selection, "clear");
        editor_data->get_undo_redo()
            .add_do_method(editor_selection, "add_node", child);
        editor_data->get_undo_redo().add_do_reference(child);
        editor_data->get_undo_redo()
            .add_undo_method(p_parent, "remove_child", child);
        ...
        editor_data->get_undo_redo().add_do_method(
            ...
            edited_scene->get_path_to(p_parent),
            child->get_class(),
            new_name
        );
        editor_data->get_undo_redo().add_undo_method(
            ...
            "live_debug_remove_node",
            NodePath(
                String(edited_scene->get_path_to(p_parent)).plus_file(new_name)
            )
        );
    ...
    if (Object::cast_to<Control>(c)) {
        // make editor more comfortable, so some controls don't appear super
        // shrunk
...
void SceneTreeDock::_create() {
    if (current_option == TOOL_NEW) {
        ...
        ERR_FAIL_COND(!first);
        int smaller_path_to_top =
            first->get_path_to(scene_root).get_name_count();
        Node* top_node = first;
        ...
        // This works because editor_selection was cleared and populated with
        // last created node in _do_create()
        Node* last_created =
            editor_selection->get_selected_node_list().front()->get();
...
void SceneTreeDock::replace_node(
    Node* p_node,
    Node* p_by_node,
    ...
    if (p_keep_properties) {
        Node* default_oldnode =
            Object::cast_to<Node>(ClassDB::instance(n->get_class()));
            ...
            if (E->get().name == "__meta__") {
                ...
                if (metadata.has("_editor_description_")) {
                    newnode->set_meta(
                        ...
                        metadata["_editor_description_"]
                    );
                }

                if (Object::cast_to<CanvasItem>(newnode)
                    || Object::cast_to<Spatial>(newnode)) {
                    if (metadata.has("_edit_group_")
                        && metadata["_edit_group_"]) {
                        newnode->set_meta("_edit_group_", true);
                    }
                    if (metadata.has("_edit_lock_")
    ...
    for (int i = 0; i < n->get_child_count(); i++) {
        if (n->get_child(i)->get_owner() == nullptr
            && n->is_owned_by_parent()) {
...
void SceneTreeDock::_new_scene_from(String p_file) {
    ...
    if (copy) {
        ...
        if (err != OK) {
            accept->set_text(
                TTR("Couldn't save new scene. Likely dependencies (instances) "
...
static bool _is_node_visible(Node* p_node) {
    ...
    if (p_node->get_owner() != EditorNode::get_singleton()->get_edited_scene()
        && !EditorNode::get_singleton()
                ->get_edited_scene()
                ->is_editable_instance(p_node->get_owner())) {
        return false;
    }
...
void SceneTreeDock::_normalize_drop(Node*& to_node, int& to_pos, int p_type) {
    ...
    if (p_type == -1) {
        ...
        if (_has_visible_children(to_node)) {
            ...
            for (int i = to_node->get_index() + 1;
                 i < to_node->get_parent()->get_child_count();
                 i++) {
...
void SceneTreeDock::_files_dropped(
    ...
    if (scene_tree->get_scene_tree()->get_drop_mode_flags()
        & Tree::DROP_MODE_INBETWEEN) {
        ...
        StringName res_type =
            EditorFileSystem::get_singleton()->get_file_type(res_path);
        ...
        for (List<PropertyInfo>::Element* E = pinfo.front(); E; E = E->next()) {
            ...
            for (int i = 0; i < valid_types.size(); i++) {
                ...
                if (res_type == prop_type
                    || ClassDB::is_parent_class(res_type, prop_type)
                    || EditorNode::get_editor_data()
                           .script_class_is_parent(res_type, prop_type)) {
        ...
        if (valid_properties.size() > 1) {
            ...
            for (List<String>::Element* E = valid_properties.front(); E;
                ...
                menu_properties->set_item_metadata(
                    menu_properties->get_item_count() - 1,
                    p
                );
            }
...
void SceneTreeDock::_script_dropped(String p_file, NodePath p_to) {
    ...
    if (n) {
        ...
        editor_data->get_undo_redo()
            .add_undo_method(n, "set_script", n->get_script());
        editor_data->get_undo_redo().add_do_method(
            this,
            "_update_script_button"
        );
...
void SceneTreeDock::_add_children_to_popup(Object* p_obj, int p_depth) {
    ...
    for (List<PropertyInfo>::Element* E = pinfo.front(); E; E = E->next()) {
        ...
        menu_subresources->add_icon_item(
            icon,
            E->get().name.capitalize(),
            EDIT_SUBRESOURCE_BASE + subresources.size()
        );
...
void SceneTreeDock::_tree_rmb(const Vector2& p_menu_pos) {
    if (!EditorNode::get_singleton()->get_edited_scene()) {
        ...
        if (profile_allow_editing) {
            menu->add_icon_shortcut(
                ...
                ED_GET_SHORTCUT("scene_tree/add_child_node"),
                TOOL_NEW
            );
            menu->add_icon_shortcut(
                get_icon("Instance", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/instance_scene"),
                TOOL_INSTANCE
            );
        }
    ...
    if (selection.size() == 1) {
        ...
        menu->add_icon_shortcut(
            get_icon("Collapse", "EditorIcons"),
            ED_GET_SHORTCUT("scene_tree/expand_collapse_all"),
        ...
        if (EditorNode::get_singleton()->get_object_custom_type_base(selected)
            == existing_script) {
    ...
    if (profile_allow_editing) {
        ...
        if (selection.size() == 1 && !node_clipboard.empty()) {
            menu->add_shortcut(
                ED_GET_SHORTCUT("scene_tree/paste_node"),
                TOOL_PASTE
            );
        }
        menu->add_separator();
    ...
    if (profile_allow_script_editing) {
        ...
        if (full_selection.size() == 1) {
            ...
            menu->add_icon_shortcut(
                get_icon("ScriptCreate", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/attach_script"),
                TOOL_ATTACH_SCRIPT
            );
            if (existing_script.is_valid()) {
                menu->add_icon_shortcut(
                    get_icon("ScriptExtend", "EditorIcons"),
                    ED_GET_SHORTCUT("scene_tree/extend_script"),
                    TOOL_EXTEND_SCRIPT
                );
            }
        }
        if (existing_script.is_valid() && exisiting_script_removable) {
            ...
            menu->add_icon_shortcut(
                get_icon("ScriptRemove", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/detach_script"),
                TOOL_DETACH_SCRIPT
            );
    ...
    if (profile_allow_editing) {
        if (full_selection.size() == 1) {
            menu->add_icon_shortcut(
                get_icon("Rename", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/rename"),
                TOOL_RENAME
            );
        }
        ...
        if (can_replace) {
            menu->add_icon_shortcut(
                ...
                ED_GET_SHORTCUT("scene_tree/change_node_type"),
                TOOL_REPLACE
            );
        }
        ...
        if (scene_tree->get_selected() != edited_scene) {
            ...
            menu->add_icon_shortcut(
                ...
                ED_GET_SHORTCUT("scene_tree/move_up"),
                TOOL_MOVE_UP
            );
            menu->add_icon_shortcut(
                get_icon("MoveDown", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/move_down"),
                TOOL_MOVE_DOWN
            );
            menu->add_icon_shortcut(
                get_icon("Duplicate", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/duplicate"),
                TOOL_DUPLICATE
            );
            menu->add_icon_shortcut(
                get_icon("Reparent", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/reparent"),
                TOOL_REPARENT
            );
            menu->add_icon_shortcut(
                get_icon("ReparentToNewNode", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/reparent_to_new_node"),
                TOOL_REPARENT_TO_NEW_NODE
            );
            if (selection.size() == 1) {
                menu->add_icon_shortcut(
                    get_icon("NewRoot", "EditorIcons"),
                    ED_GET_SHORTCUT("scene_tree/make_root"),
                    TOOL_MAKE_ROOT
                );
            }
        }
    ...
    if (selection.size() == 1) {
        if (profile_allow_editing) {
            menu->add_separator();
            menu->add_icon_shortcut(
                get_icon("Blend", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/merge_from_scene"),
                TOOL_MERGE_FROM_SCENE
            );
            menu->add_icon_shortcut(
                get_icon("CreateNewSceneFrom", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/save_branch_as_scene"),
                TOOL_NEW_SCENE_FROM
            );
        }
        if (full_selection.size() == 1) {
            menu->add_separator();
            menu->add_icon_shortcut(
                get_icon("CopyNodePath", "EditorIcons"),
                ED_GET_SHORTCUT("scene_tree/copy_node_path"),
                TOOL_COPY_NODE_PATH
            );
        }
        ...
        if (is_external) {
            ...
            if (is_inherited && is_top_level) {
                ...
                bool editable = EditorNode::get_singleton()
                                    ->get_edited_scene()
                                    ->is_editable_instance(selection[0]);
                bool placeholder =
                    selection[0]->get_scene_instance_load_placeholder();
                if (profile_allow_editing) {
                    menu->add_check_item(
                        TTR("Editable Children"),
                        TOOL_SCENE_EDITABLE_CHILDREN
                    );
                    menu->add_check_item(
                        TTR("Load As Placeholder"),
                ...
                if (profile_allow_editing) {
                    menu->set_item_checked(
                        menu->get_item_idx_from_text(TTR("Editable Children")),
                        editable
                    );
                    menu->set_item_checked(
                        menu->get_item_idx_from_text(TTR("Load As Placeholder")
                        ),
                        placeholder
                    );
                }
    ...
    if (profile_allow_editing && selection.size() > 1) {
        // this is not a commonly used action, it makes no sense for it to be
        // where it was nor always present.
        menu->add_separator();
        menu->add_icon_shortcut(
            get_icon("Rename", "EditorIcons"),
            ED_GET_SHORTCUT("scene_tree/batch_rename"),
            TOOL_BATCH_RENAME
        );
    }
#endif // MODULE_REGEX_ENABLED
    menu->add_separator();
    menu->add_icon_item(
        get_icon("Help", "EditorIcons"),
        TTR("Open Documentation"),
...
void SceneTreeDock::_focus_node() {
    ...
    if (node->is_class("CanvasItem")) {
        CanvasItemEditorPlugin* editor =
            Object::cast_to<CanvasItemEditorPlugin>(editor_data->get_editor("2D"
            ));
        editor->get_canvas_item_editor()->focus_selection();
    } else {
        SpatialEditorPlugin* editor =
            Object::cast_to<SpatialEditorPlugin>(editor_data->get_editor("3D"));
...
void SceneTreeDock::attach_script_to_selected(bool p_extend) {
    if (ScriptServer::get_language_count() == 0) {
        EditorNode::get_singleton()->show_warning(
            TTR("Cannot attach a script: there are no languages "
                "registered.\nThis is probably because this editor was built "
                "with all language modules disabled.")
        );
        return;
    }
...
void SceneTreeDock::_local_tree_selected() {
    if (!bool(EDITOR_GET("interface/editors/show_scene_tree_root_selection"))
        || get_tree()->get_edited_scene_root() != nullptr) {
        scene_tree->show();
    }
    if (remote_tree) {
...
void SceneTreeDock::_update_create_root_dialog() {
    BaseButton* toggle =
        Object::cast_to<BaseButton>(create_root_dialog->get_node(
            String("NodeShortcutsTopRow/NodeShortcutsToggle")
        ));
    Node* node_shortcuts =
        create_root_dialog->get_node(String("NodeShortcuts"));

    if (!toggle || !node_shortcuts) {
        return;
    }

    Control* beginner_nodes = Object::cast_to<Control>(
        node_shortcuts->get_node(String("BeginnerNodeShortcuts"))
    );
    Control* favorite_nodes = Object::cast_to<Control>(
        node_shortcuts->get_node(String("FavoriteNodeShortcuts"))
    );
        ...
        if (f) {
            while (!f->eof_reached()) {
                ...
                if (l != String()) {
                    ...
                    button->set_icon(
                        EditorNode::get_singleton()->get_class_icon(name)
                    );
                    button->connect(
                        "pressed",
                        this,
                        "_favorite_root_selected",
                        make_binds(l)
                    );
                }
            }
...
void SceneTreeDock::_feature_profile_changed() {
    ...
    if (profile.is_valid()) {
        profile_allow_editing = !profile->is_feature_disabled(
    ...
    for (List<PropertyInfo>::Element* E = props.front(); E; E = E->next()) {
        ...
        if (v.is_ref()) {
            ...
            if (res.is_valid()) {
                if (is_instanced) {
                    Variant orig;
                    if (EditorPropertyRevert::
                            get_instanced_node_original_property(
                                p_node,
                                E->get().name,
                                orig
                            )) {
                        if (!EditorPropertyRevert::is_node_property_different(
                                p_node,
                                v,
                                orig
                            )) {
                ...
                if ((res->get_path() == "" || res->get_path().find("::") > -1)
                    && !r_remap.has(res)) {
...
void SceneTreeDock::_create_remap_for_resource(
    RES p_resource,
    Map<RES, RES>& r_remap
) {
...
void SceneTreeDock::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_tool_selected"),
        &SceneTreeDock::_tool_selected,
        DEFVAL(false)
    );
    ClassDB::bind_method(D_METHOD("_create"), &SceneTreeDock::_create);
    ClassDB::bind_method(
        D_METHOD("_node_reparent"),
        &SceneTreeDock::_node_reparent
    );
    ClassDB::bind_method(D_METHOD("_set_owners"), &SceneTreeDock::_set_owners);
    ClassDB::bind_method(
        D_METHOD("_node_selected"),
        &SceneTreeDock::_node_selected
    );
    ClassDB::bind_method(
        D_METHOD("_node_renamed"),
        &SceneTreeDock::_node_renamed
    );
    ClassDB::bind_method(
        D_METHOD("_script_created"),
        &SceneTreeDock::_script_created
    );
    ClassDB::bind_method(
        D_METHOD("_script_creation_closed"),
        &SceneTreeDock::_script_creation_closed
    );
    ClassDB::bind_method(
        D_METHOD("_load_request"),
        &SceneTreeDock::_load_request
    );
    ClassDB::bind_method(
        D_METHOD("_script_open_request"),
        &SceneTreeDock::_script_open_request
    );
    ClassDB::bind_method(
        D_METHOD("_unhandled_key_input"),
        &SceneTreeDock::_unhandled_key_input
    );
    ClassDB::bind_method(D_METHOD("_input"), &SceneTreeDock::_input);
    ClassDB::bind_method(
        D_METHOD("_nodes_drag_begin"),
        &SceneTreeDock::_nodes_drag_begin
    );
    ClassDB::bind_method(
        D_METHOD("_delete_confirm"),
        &SceneTreeDock::_delete_confirm
    );
    ClassDB::bind_method(
        D_METHOD("_toggle_editable_children_from_selection"),
        &SceneTreeDock::_toggle_editable_children_from_selection
    );
    ClassDB::bind_method(
        D_METHOD("_toggle_placeholder_from_selection"),
        &SceneTreeDock::_toggle_placeholder_from_selection
    );
    ClassDB::bind_method(
        D_METHOD("_node_prerenamed"),
        &SceneTreeDock::_node_prerenamed
    );
    ClassDB::bind_method(
        D_METHOD("_import_subscene"),
        &SceneTreeDock::_import_subscene
    );
    ClassDB::bind_method(
        D_METHOD("_selection_changed"),
        &SceneTreeDock::_selection_changed
    );
    ClassDB::bind_method(
        D_METHOD("_node_collapsed"),
        &SceneTreeDock::_node_collapsed
    );
    ClassDB::bind_method(
        D_METHOD("_new_scene_from"),
        &SceneTreeDock::_new_scene_from
    );
    ClassDB::bind_method(
        D_METHOD("_nodes_dragged"),
        &SceneTreeDock::_nodes_dragged
    );
    ClassDB::bind_method(
        D_METHOD("_files_dropped"),
        &SceneTreeDock::_files_dropped
    );
    ClassDB::bind_method(
        D_METHOD("_property_selected"),
        &SceneTreeDock::_property_selected
    );
    ClassDB::bind_method(D_METHOD("_quick_open"), &SceneTreeDock::_quick_open);
    ClassDB::bind_method(
        D_METHOD("_script_dropped"),
        &SceneTreeDock::_script_dropped
    );
    ClassDB::bind_method(D_METHOD("_tree_rmb"), &SceneTreeDock::_tree_rmb);
    ClassDB::bind_method(
        D_METHOD("_filter_changed"),
        &SceneTreeDock::_filter_changed
    );
    ClassDB::bind_method(D_METHOD("_focus_node"), &SceneTreeDock::_focus_node);
    ClassDB::bind_method(
        D_METHOD("_remote_tree_selected"),
        &SceneTreeDock::_remote_tree_selected
    );
    ClassDB::bind_method(
        D_METHOD("_local_tree_selected"),
        &SceneTreeDock::_local_tree_selected
    );
    ClassDB::bind_method(
        D_METHOD("_update_script_button"),
        &SceneTreeDock::_update_script_button
    );
    ClassDB::bind_method(
        D_METHOD("_favorite_root_selected"),
        &SceneTreeDock::_favorite_root_selected
    );
    ClassDB::bind_method(
        D_METHOD("_update_create_root_dialog"),
        &SceneTreeDock::_update_create_root_dialog
    );
    ClassDB::bind_method(
        D_METHOD("_feature_profile_changed"),
        &SceneTreeDock::_feature_profile_changed
    );

    ClassDB::bind_method(D_METHOD("instance"), &SceneTreeDock::instance);
    ClassDB::bind_method(
        D_METHOD("get_tree_editor"),
        &SceneTreeDock::get_tree_editor
    );
    ClassDB::bind_method(
        D_METHOD("replace_node"),
        &SceneTreeDock::replace_node
    );
...
SceneTreeDock::SceneTreeDock(
    ...
    ED_SHORTCUT(
        "scene_tree/instance_scene",
        TTR("Instance Child Scene"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_A
    );
    ...
    ED_SHORTCUT(
        ...
        TTR("Copy Node Path"),
        KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_C
    );
    ED_SHORTCUT(
        "scene_tree/delete_no_confirm",
        TTR("Delete (No Confirm)"),
        KEY_MASK_SHIFT | KEY_DELETE
    );
    ...
    button_instance->connect(
        "pressed",
        this,
        "_tool_selected",
        make_binds(TOOL_INSTANCE, false)
    );
    button_instance->set_tooltip(
        TTR("Instance a scene file as a Node. Creates an inherited scene if no "
            "root node exists.")
    );
    ...
    button_create_script->connect(
        ...
        this,
        "_tool_selected",
        make_binds(TOOL_ATTACH_SCRIPT, false)
    );
    button_create_script->set_tooltip(
        TTR("Attach a new or existing script to the selected node.")
    );
    button_create_script->set_shortcut(
        ED_GET_SHORTCUT("scene_tree/attach_script")
    );
    ...
    button_detach_script->connect(
        ...
        this,
        "_tool_selected",
        make_binds(TOOL_DETACH_SCRIPT, false)
    );
    button_detach_script->set_tooltip(
        TTR("Detach the script from the selected node.")
    );
    button_detach_script->set_shortcut(
        ED_GET_SHORTCUT("scene_tree/detach_script")
    );
    ...
    edit_remote->set_tooltip(
        TTR("If selected, the Remote scene tree dock will cause the project to "
            "stutter every time it updates.\nSwitch back to the Local scene "
            "tree dock to improve performance.")
    );
    ...
    scene_tree->get_scene_tree()
        ->connect("item_double_clicked", this, "_focus_node");
    ...
    create_dialog
        ->connect("favorites_updated", this, "_update_create_root_dialog");

#ifdef MODULE_REGEX_ENABLED
    rename_dialog =
        memnew(RenameDialog(scene_tree, &editor_data->get_undo_redo()));
    ...
    placeholder_editable_instance_remove_dialog
        ->connect("confirmed", this, "_toggle_placeholder_from_selection");

    import_subscene_dialog = memnew(EditorSubScene);
    add_child(import_subscene_dialog);
    import_subscene_dialog
        ->connect("subscene_selected", this, "_import_subscene");
