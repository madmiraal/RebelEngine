...
public:
    ...
    // this is a bit excessive boilerplatewise but can be removed if we decide
    // to stick with one method

    // note this is actually the BVH id +1, so that visual server can test
    // against zero for validity to maintain compatibility with octree (where 0
    // indicates invalid)
    ...
    static void
    _instance_unpair(void* p_self, SpatialPartitionID, Instance* p_A, int, SpatialPartitionID, Instance* p_B, int, void*);
    ...
    struct Instance : RasterizerScene::InstanceBase {
        ...
        // rooms & portals
        OcclusionHandle
            occlusion_handle; // handle of instance in occlusion system (or 0)
        ...
        AABB* custom_aabb; // <Zylann> would using aabb directly with a bool be
                           // better?
        float extra_margin;
        uint32_t object_id;
        ...
        uint64_t
            version; // changes to this, and changes to base increase version
    ...
    void _instance_queue_update(
        Instance* p_instance,
        bool p_update_aabb,
        bool p_update_materials = false
    );
    ...
    struct InstanceGIProbeData : public InstanceBaseData {
        ...
        struct LightCache {
            ...
            bool operator==(const LightCache& p_cache) {
                return (
                    type == p_cache.type && transform == p_cache.transform
                    && color == p_cache.color && energy == p_cache.energy
        ...
        struct LocalData {
            uint16_t pos[3];
            uint16_t
                energy[3]; // using 0..1024 for float range 0..1. integer is
                           // needed for deterministic add/remove of lights
        };
    ...
    Instance*
        instance_shadow_cull_result[MAX_INSTANCE_CULL]; // used for generating
                                                        // shadowmaps
    ...
    bool _instance_get_transformed_aabb_for_occlusion(
        VSInstance* p_instance,
        AABB& r_aabb
    ) const {
        r_aabb = ((Instance*)p_instance)->transformed_aabb;
        return ((Instance*)p_instance)->portal_mode
            != VisualServer::INSTANCE_PORTAL_MODE_GLOBAL;
    }

    void* _instance_get_from_rid(RID p_instance);

    bool _instance_cull_check(VSInstance* p_instance, uint32_t p_cull_mask)
        const {
...
public:
    ...
    _FORCE_INLINE_ bool _light_instance_update_shadow(
        Instance* p_instance,
    ...
    void _gi_probe_fill_local_data(
        int p_idx,
        int p_level,
        int p_x,
        int p_y,
    ...
    _FORCE_INLINE_ uint32_t _gi_bake_find_cell(
