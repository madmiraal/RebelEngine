...
public:
    ...
    ) const = 0; // get gravity vector working on this body space/area
    virtual float get_total_linear_damp(
    ) const = 0; // get density of this body space/area
    virtual float get_total_angular_damp(
    ) const = 0; // get density of this body space/area

    virtual float get_inverse_mass() const = 0; // get the mass
    virtual real_t get_inverse_inertia(
    ...
    virtual Vector2 get_velocity_at_local_position(const Vector2& p_position
    ) const = 0;
    ...
    virtual Variant get_contact_collider_shape_metadata(int p_contact_idx
    ) const                                                         = 0;
    virtual Vector2 get_contact_collider_velocity_at_position(int p_contact_idx
    ) const                                                         = 0;
...
class Physics2DDirectSpaceState : public Object {
    ...
    Array _intersect_point_on_canvas(
        const Vector2& p_point,
    ...
    Array _intersect_point_impl(
        const Vector2& p_point,
        int p_max_results,
        const Vector<RID>& p_exclud,
        uint32_t p_layers,
        bool p_collide_with_bodies,
        bool p_collide_with_areas,
        bool p_filter_by_canvas       = false,
        ObjectID p_canvas_instance_id = 0
    );
...
public:
    ...
    virtual bool intersect_ray(
        ...
        uint32_t p_collision_layer = 0xFFFFFFFF,
        bool p_collide_with_bodies = true,
        bool p_collide_with_areas  = false
    ) = 0;
...
public:
    ...
    enum ShapeType {
        ...
        SHAPE_CONCAVE_POLYGON, ///< Vector2 array:"triangles" , or Dictionary
    ...
    enum AreaSpaceOverrideMode {
        ...
        AREA_SPACE_OVERRIDE_COMBINE_REPLACE, // Combines, then discards all
                                             // subsequent calculations
        AREA_SPACE_OVERRIDE_REPLACE,
        AREA_SPACE_OVERRIDE_REPLACE_COMBINE // Discards all previous
                                            // calculations, then keeps
                                            // combining
    };
    ...
    virtual Transform2D area_get_shape_transform(RID p_area, int p_shape_idx)
        const = 0;
    ...
    virtual void body_set_shape_metadata(
        RID p_body,
        int p_shape_idx,
    ...
    virtual Transform2D body_get_shape_transform(RID p_body, int p_shape_idx)
        const = 0;
    virtual Variant body_get_shape_metadata(RID p_body, int p_shape_idx)
        const = 0;
    ...
    virtual void body_set_shape_as_one_way_collision(
        RID p_body,
        int p_shape,
        bool p_enabled,
        float p_margin = 0
    ...
    virtual CCDMode body_get_continuous_collision_detection_mode(RID p_body
    ) const = 0;
    ...
    virtual bool body_collide_shape(
        RID p_body,
        int p_body_shape,
        RID p_shape,
        const Transform2D& p_shape_xform,
    ...
    virtual RID pin_joint_create(
        const Vector2& p_anchor,
        RID p_body_a,
        RID p_body_b = RID()
    ) = 0;
    virtual RID groove_joint_create(
    ...
    virtual RID damped_spring_joint_create(
    ...
    virtual real_t pin_joint_get_param(RID p_joint, PinJointParam p_param)
        const = 0;
    ...
    virtual void damped_string_joint_set_param(
        RID p_joint,
        DampedStringParam p_param,
        real_t p_value
    ) = 0;
    virtual real_t damped_string_joint_get_param(
        RID p_joint,
        DampedStringParam p_param
    ) const = 0;
...
public:
    Physics2DServer::MotionResult* get_result_ptr() const {
        return const_cast<Physics2DServer::MotionResult*>(&result);
    }
...
class Physics2DServerManager {
    struct ClassInfo {
        ...
        ClassInfo(
            String p_name,
            CreatePhysics2DServerCallback p_create_callback
        ) :
            name(p_name),
