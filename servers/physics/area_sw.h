...
public:
    //_FORCE_INLINE_ const Transform& get_inverse_transform() const { return
    // inverse_transform; } _FORCE_INLINE_ SpaceSW* get_owner() { return owner;
    // }

    void set_monitor_callback(ObjectID p_id, const StringName& p_method);

    _FORCE_INLINE_ bool has_monitor_callback() const {
        return monitor_callback_id;
    }

    void set_area_monitor_callback(ObjectID p_id, const StringName& p_method);

    _FORCE_INLINE_ bool has_area_monitor_callback() const {
        return area_monitor_callback_id;
    }
    ...
    _FORCE_INLINE_ void remove_body_from_query(
        BodySW* p_body,
        uint32_t p_body_shape,
        uint32_t p_area_shape
    );

    _FORCE_INLINE_ void add_area_to_query(
        AreaSW* p_area,
        uint32_t p_area_shape,
        uint32_t p_self_shape
    );
    _FORCE_INLINE_ void remove_area_from_query(
        AreaSW* p_area,
        uint32_t p_area_shape,
        uint32_t p_self_shape
    );
    ...
    PhysicsServer::AreaSpaceOverrideMode get_space_override_mode() const {
    ...
    _FORCE_INLINE_ void add_constraint(ConstraintSW* p_constraint) {
    ...
    _FORCE_INLINE_ const Set<ConstraintSW*>& get_constraints() const {
        return constraints;
    }
...
void AreaSW::remove_body_from_query(
    BodySW* p_body,
    uint32_t p_body_shape,
    uint32_t p_area_shape
) {
...
void AreaSW::remove_area_from_query(
    AreaSW* p_area,
    uint32_t p_area_shape,
    uint32_t p_self_shape
) {
