...
class BodySW : public CollisionObjectSW {
    ...
    friend class PhysicsDirectBodyStateSW; // i give up, too many functions to
                                           // expose
...
public:
    ...
    _FORCE_INLINE_ void add_contact(
        const Vector3& p_local_pos,
        const Vector3& p_local_normal,
        real_t p_depth,
        int p_local_shape,
        const Vector3& p_collider_pos,
        int p_collider_shape,
        ObjectID p_collider_instance_id,
        const RID& p_collider,
        const Vector3& p_collider_velocity_at_pos
    );
    ...
    _FORCE_INLINE_ BodySW* get_island_next() const {
        return island_next;
    }

    _FORCE_INLINE_ void set_island_next(BodySW* p_next) {
        island_next = p_next;
    }

    _FORCE_INLINE_ BodySW* get_island_list_next() const {
        return island_list_next;
    }

    _FORCE_INLINE_ void set_island_list_next(BodySW* p_next) {
        island_list_next = p_next;
    }

    _FORCE_INLINE_ void add_constraint(ConstraintSW* p_constraint, int p_pos) {
    ...
    const Map<ConstraintSW*, int>& get_constraint_map() const {
    ...
    _FORCE_INLINE_ Vector3 xform_local_to_principal(const Vector3& p_pos
    ) const {
    ...
    _FORCE_INLINE_ void apply_impulse(
        const Vector3& p_pos,
        const Vector3& p_j
    ) {
        linear_velocity += p_j * _inv_mass;
        angular_velocity +=
            _inv_inertia_tensor.xform((p_pos - center_of_mass).cross(p_j));
    }
    ...
    _FORCE_INLINE_ void apply_bias_impulse(
        const Vector3& p_pos,
        const Vector3& p_j,
        real_t p_max_delta_av = -1.0
    ) {
    ...
    _FORCE_INLINE_ void wakeup() {
        if ((!get_space()) || mode == PhysicsServer::BODY_MODE_STATIC
    ...
    _FORCE_INLINE_ Vector3 get_velocity_in_local_point(const Vector3& rel_pos
    ) const {
        return linear_velocity
             + angular_velocity.cross(rel_pos - center_of_mass);
    }

    _FORCE_INLINE_ real_t compute_impulse_denominator(
        const Vector3& p_pos,
    ...
    _FORCE_INLINE_ real_t
    compute_angular_impulse_denominator(const Vector3& p_axis) const {
...
public:
    ...
    void apply_central_impulse(const Vector3& p_j) override {
        body->wakeup();
