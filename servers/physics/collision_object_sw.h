...
protected:
    ...
    _FORCE_INLINE_ void _set_transform(
        ...
        ERR_FAIL_COND_MSG(
            p_transform.origin.length_squared() > MAX_OBJECT_DISTANCE_X2,
            "Object went too far away (more than '" + itos(MAX_OBJECT_DISTANCE)
                + "' units from origin)."
        );
#endif
...
public:
    ...
    _FORCE_INLINE_ SpaceSW* get_space() const {
        return space;
    }
    ...
    ~CollisionObjectSW() override = default;
};
