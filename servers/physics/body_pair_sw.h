...
class BodyPairSW : public ConstraintSW {
    ...
    struct Contact {
        ...
        real_t acc_bias_impulse; // accumulated normal impulse for position bias
                                 // (Pnb)
        real_t
            acc_bias_impulse_center_of_mass; // accumulated normal impulse for
                                             // position bias applied to com
        real_t mass_normal;
        real_t bias;
        real_t bounce;
        ...
        bool active;
        Vector3 rA,
            rB; // Offset in world orientation with respect to center of mass
    };

    Vector3 offset_B; // use local A coordinates to avoid numerical issues on
                      // collision detection
    ...
    static void _contact_added_callback(
        const Vector3& p_point_A,
        const Vector3& p_point_B,
        void* p_userdata
    );

    void contact_added_callback(
        const Vector3& p_point_A,
        const Vector3& p_point_B
    );
