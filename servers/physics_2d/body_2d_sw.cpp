...
void Body2DSW::update_inertias() {
    ...
    switch (mode) {
        case Physics2DServer::BODY_MODE_RIGID: {
            ...
            // update tensor for allshapes, not the best way but should be
            ...
            for (int i = 0; i < get_shape_count(); i++) {
                ...
                inertia         += shape->get_moment_of_inertia(mass, scale)
                         + mass * mtx.get_origin().length_squared();
            }
...
void Body2DSW::set_param(
    Physics2DServer::BodyParameter p_param,
    real_t p_value
) {
    switch (p_param) {
...
void Body2DSW::set_mode(Physics2DServer::BodyMode p_mode) {
    ...
    switch (p_mode) {
        ...
        case Physics2DServer::BODY_MODE_KINEMATIC: {
            ...
            set_active(
                p_mode == Physics2DServer::BODY_MODE_KINEMATIC
                && contacts.size()
            );
...
void Body2DSW::set_state(
    ...
    switch (p_state) {
        ...
        case Physics2DServer::BODY_STATE_CAN_SLEEP: {
            can_sleep = p_variant;
            if (mode == Physics2DServer::BODY_MODE_RIGID && !active
                && !can_sleep) {
                set_active(true);
            }
...
void Body2DSW::integrate_forces(real_t p_step) {
    ...
    if (ac) {
        ...
        for (int i = ac - 1; i >= 0 && !stopped; i--) {
            Physics2DServer::AreaSpaceOverrideMode mode =
            ...
            switch (mode) {
                ...
                case Physics2DServer::AREA_SPACE_OVERRIDE_REPLACE_COMBINE: {
                    ...
                    _compute_area_gravity_and_dampenings(aa[i].area);
                    stopped =
                        mode == Physics2DServer::AREA_SPACE_OVERRIDE_REPLACE;
                } break;
                default: {
                }
            }
    ...
    */
    ...
    if (mode == Physics2DServer::BODY_MODE_KINEMATIC) {
        ...
        real_t rot =
            new_transform.get_rotation() - get_transform().get_rotation();
...
void Body2DSW::integrate_velocities(real_t p_step) {
    ...
    if (mode == Physics2DServer::BODY_MODE_KINEMATIC) {
        ...
        if (contacts.size() == 0 && linear_velocity == Vector2()
            && angular_velocity == 0) {
    ...
    real_t angle =
        get_transform().get_rotation() + total_angular_velocity * p_step;
...
void Body2DSW::wakeup_neighbours() {
    for (Map<Constraint2DSW*, int>::Element* E = constraint_map.front(); E;
         E                                     = E->next()) {
...
bool Body2DSW::sleep_test(real_t p_step) {
    if (mode == Physics2DServer::BODY_MODE_STATIC
        ...
        return !active; // characters and kinematic bodies don't sleep unless
                        // asked to sleep
    } else if (!can_sleep) {
        return false;
    }

    if (Math::abs(angular_velocity)
...
void Body2DSW::set_force_integration_callback(
    ObjectID p_id,
...
Variant Physics2DDirectBodyStateSW::get_contact_collider_shape_metadata(
    ...
    if (!Physics2DServerSW::singletonsw->body_owner.owns(
            body->contacts[p_contact_idx].collider
        )) {
        return Variant();
    }
    Body2DSW* other = Physics2DServerSW::singletonsw->body_owner.get(
        body->contacts[p_contact_idx].collider
    );
