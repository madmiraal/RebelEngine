...
class Body2DSW : public CollisionObject2DSW {
    ...
    _FORCE_INLINE_ void _compute_area_gravity_and_dampenings(
        const Area2DSW* p_area
    );

    friend class Physics2DDirectBodyStateSW; // i give up, too many functions to
                                             // expose
...
public:
    ...
    _FORCE_INLINE_ void add_contact(
        ...
        const Vector2& p_local_normal,
        real_t p_depth,
        int p_local_shape,
        const Vector2& p_collider_pos,
    ...
    _FORCE_INLINE_ void add_constraint(
        Constraint2DSW* p_constraint,
        int p_pos
    ) {
    ...
    _FORCE_INLINE_ void wakeup() {
        if ((!get_space()) || mode == Physics2DServer::BODY_MODE_STATIC
    ...
    void set_state(
        Physics2DServer::BodyState p_state,
    ...
    _FORCE_INLINE_ void set_continuous_collision_detection_mode(
        Physics2DServer::CCDMode p_mode
    ) {
        continuous_cd_mode = p_mode;
    }

    _FORCE_INLINE_ Physics2DServer::CCDMode
    get_continuous_collision_detection_mode() const {
    ...
    _FORCE_INLINE_ Vector2 get_velocity_in_local_point(const Vector2& rel_pos
    ) const {
        return linear_velocity
             + Vector2(
                   -angular_velocity * rel_pos.y,
    ...
    _FORCE_INLINE_ Vector2 get_motion() const {
        if (mode > Physics2DServer::BODY_MODE_KINEMATIC) {
            ...
            return get_transform().get_origin()
                 - new_transform.get_origin(); // kinematic simulates forward
        }
        return Vector2();
    }
...
public:
    ...
    virtual real_t get_inverse_inertia() const {
    ...
    virtual real_t get_angular_velocity() const {
        return body->get_angular_velocity();
    }

    virtual void set_transform(const Transform2D& p_transform) {
        body->set_state(Physics2DServer::BODY_STATE_TRANSFORM, p_transform);
    }
