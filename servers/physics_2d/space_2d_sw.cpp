...
_FORCE_INLINE_ static bool _can_collide_with(
    ...
    if (p_object->get_type() == CollisionObject2DSW::TYPE_BODY
...
int Physics2DDirectSpaceStateSW::_intersect_point_impl(
    ...
    int amount = space->broadphase->cull_aabb(
        aabb,
        space->intersection_query_results,
        Space2DSW::INTERSECTION_QUERY_MAX,
    ...
    for (int i = 0; i < amount; i++) {
        if (!_can_collide_with(
                space->intersection_query_results[i],
                p_collision_mask,
                p_collide_with_bodies,
                p_collide_with_areas
            )) {
        ...
        const CollisionObject2DSW* col_obj =
            space->intersection_query_results[i];
        ...
        if (p_filter_by_canvas
            && col_obj->get_canvas_instance_id() != p_canvas_instance_id) {
            continue;
        }
        ...
        Vector2 local_point =
            (col_obj->get_transform() * col_obj->get_shape_transform(shape_idx))
                .affine_inverse()
                .xform(p_point);
        ...
        if (r_results[cc].collider_id != 0) {
            r_results[cc].collider =
                ObjectDB::get_instance(r_results[cc].collider_id);
        }
...
int Physics2DDirectSpaceStateSW::intersect_point(
    const Vector2& p_point,
    ShapeResult* r_results,
    int p_result_max,
    const Set<RID>& p_exclude,
...
int Physics2DDirectSpaceStateSW::intersect_point_on_canvas(
    const Vector2& p_point,
    ObjectID p_canvas_instance_id,
    ShapeResult* r_results,
    int p_result_max,
...
bool Physics2DDirectSpaceStateSW::intersect_ray(
    ...
    int amount = space->broadphase->cull_segment(
        begin,
        end,
        space->intersection_query_results,
        Space2DSW::INTERSECTION_QUERY_MAX,
    ...
    // todo, create another array that references results, compute AABBs and
    // check closest point to ray origin, sort, and stop evaluating results when
    // beyond first collision
    ...
    for (int i = 0; i < amount; i++) {
        ...
        int shape_idx         = space->intersection_query_subindex_results[i];
        Transform2D inv_xform = col_obj->get_shape_inv_transform(shape_idx)
                              * col_obj->get_inv_transform();
        ...
        local_from =
        col_obj->get_shape_inv_transform(shape_idx).xform(local_from);

        local_to = col_obj->get_inv_transform().xform(end);
        local_to =
        col_obj->get_shape_inv_transform(shape_idx).xform(local_to);*/

        const Shape2DSW* shape = col_obj->get_shape(shape_idx);

        Vector2 shape_point, shape_normal;

        if (shape->intersect_segment(
                local_from,
                local_to,
                shape_point,
                shape_normal
            )) {
            Transform2D xform = col_obj->get_transform()
                              * col_obj->get_shape_transform(shape_idx);
            ...
            if (ld < min_d) {
                ...
                res_normal =
                    inv_xform.basis_xform_inv(shape_normal).normalized();
                res_shape = shape_idx;
                res_obj   = col_obj;
                collided  = true;
            }
...
int Physics2DDirectSpaceStateSW::intersect_shape(
    const RID& p_shape,
    ...
    for (int i = 0; i < amount; i++) {
        ...
        if (!CollisionSolver2DSW::solve(
                shape,
                p_xform,
                p_motion,
                col_obj->get_shape(shape_idx),
                col_obj->get_transform()
                    * col_obj->get_shape_transform(shape_idx),
                Vector2(),
                nullptr,
                nullptr,
...
bool Physics2DDirectSpaceStateSW::cast_motion(
    const RID& p_shape,
    const Transform2D& p_xform,
    const Vector2& p_motion,
    real_t p_margin,
    ...
    for (int i = 0; i < amount; i++) {
        ...
        for (int j = 0; j < 8; j++) { // steps should be customizable..
            ...
            Vector2 sep =
                mnormal; // important optimization for this to work fast enough
            bool collided = CollisionSolver2DSW::solve(
                shape,
                p_xform,
                p_motion * fraction,
                col_obj->get_shape(shape_idx),
            ...
            if (collided) {
                ...
                if ((j == 0) || (low > 0.0)) { // Did it not collide before?
                    ...
                    // fraction for more accurate results with long motions that
                    // collide near the start.
                    fraction_coeff = 0.25;
                }
            } else {
                low = fraction;
                if ((j == 0) || (hi < 1.0)) { // Did it collide before?
                    ...
                    // When not colliding again, converge faster towards high
                    // fraction for more accurate results with long motions that
                    // collide near the end.
                    fraction_coeff = 0.75;
                }
            }
        }
...
bool Physics2DDirectSpaceStateSW::collide_shape(
    RID p_shape,
    ...
    cbk.passed = 0;
    cbk.ptr    = r_results;
    CollisionSolver2DSW::CallbackResult cbkres =
        Physics2DServerSW::_shape_col_cbk;
...
bool Physics2DDirectSpaceStateSW::rest_info(
    RID p_shape,
    const Transform2D& p_shape_xform,
    const Vector2& p_motion,
    real_t p_margin,
    ...
    for (int i = 0; i < amount; i++) {
        ...
        rcd.shape       = shape_idx;
        rcd.local_shape = 0;
        bool sc         = CollisionSolver2DSW::solve(
            shape,
            p_shape_xform,
            p_motion,
            col_obj->get_shape(shape_idx),
...
int Space2DSW::_cull_aabb_for_body(Body2DSW* p_body, const Rect2& p_aabb) {
    int amount = broadphase->cull_aabb(
        p_aabb,
        intersection_query_results,
    ...
    for (int i = 0; i < amount; i++) {
        ...
        if (!keep) {
            if (i < amount - 1) {
                SWAP(
                    intersection_query_results[i],
                    intersection_query_results[amount - 1]
                );
                SWAP(
                    intersection_query_subindex_results[i],
...
int Space2DSW::test_body_ray_separation(
    Body2DSW* p_body,
    const Transform2D& p_transform,
    bool p_infinite_inertia,
    Vector2& r_recover_motion,
    Physics2DServer::SeparationResult* r_results,
    int p_result_max,
    real_t p_margin
) {
    Rect2 body_aabb;
    ...
    // Undo the currently transform the physics server is aware of and apply the
    // provided one
...

    {
        ...
        do {
            ...
            for (int j = 0; j < p_body->get_shape_count(); j++) {
                ...
                Transform2D body_shape_xform =
                    body_transform * p_body->get_shape_transform(j);
                ...
                for (int i = 0; i < amount; i++) {
                    ...
                    if (CollisionObject2DSW::TYPE_BODY == col_obj->get_type()) {
                        const Body2DSW* b =
                            static_cast<const Body2DSW*>(col_obj);
                        if (p_infinite_inertia
                            && Physics2DServer::BODY_MODE_STATIC
                                   != b->get_mode()
                            && Physics2DServer::BODY_MODE_KINEMATIC
                                   != b->get_mode()) {
                            continue;
                        }
                    }

                    Transform2D col_obj_shape_xform =
                        col_obj->get_transform()
                        * col_obj->get_shape_transform(shape_idx);

                    /*
 * There is no point in supporting one way collisions with ray shapes, as they
 will always collide in the desired
 * direction. Use a short ray shape if you want to achieve a similar effect.
 *
                    if (col_obj->is_shape_set_as_one_way_collision(shape_idx)) {

                        cbk.valid_dir =
 col_obj_shape_xform.get_axis(1).normalized(); cbk.valid_depth = p_margin;
 //only valid depth is the collision margin cbk.invalid_by_dir = 0;
                    ...
                    */
                    ...
                    if (CollisionSolver2DSW::solve(
                        ...
                        for (int k = 0; k < rays_found; k++) {
                            if (r_results[ray_index].collision_local_shape
                                == j) {
                                ray_index = k;
                            }
                        }
                        ...
                        if (ray_index != -1) {
                            Physics2DServer::SeparationResult& result =
                                r_results[ray_index];
                            ...
                            for (int k = 0; k < cbk.amount; k++) {
                                ...
                                if (depth > result.collision_depth) {
                                    ...
                                    result.collider_id =
                                        col_obj->get_instance_id();
                                    result.collider_metadata =
                                        col_obj->get_shape_metadata(shape_idx);
                                    if (col_obj->get_type()
                                        ...
                                        Vector2 rel_vec =
                                            b
                                            - body->get_transform().get_origin(
                                            );
                                        result.collider_velocity =
                                            Vector2(
...
bool Space2DSW::test_body_motion(
    Body2DSW* p_body,
    const Transform2D& p_from,
    const Vector2& p_motion,
    bool p_infinite_inertia,
    real_t p_margin,
    Physics2DServer::MotionResult* r_result,
    ...
    for (int i = 0; i < p_body->get_shape_count(); i++) {
        ...
        if (p_exclude_raycast_shapes
            && p_body->get_shape(i)->get_type() == Physics2DServer::SHAPE_RAY) {
            continue;
        }
...

    {
        ...
        do {
            ...
            for (int j = 0; j < p_body->get_shape_count(); j++) {
                ...
                Shape2DSW* body_shape = p_body->get_shape(j);
                if (p_exclude_raycast_shapes
                    && body_shape->get_type() == Physics2DServer::SHAPE_RAY) {
                    continue;
                }
                ...
                for (int i = 0; i < amount; i++) {
                    ...
                    if (col_obj->is_shape_set_as_one_way_collision(shape_idx)) {
                        ...
                        float owc_margin =
                            col_obj->get_shape_one_way_collision_margin(
                                shape_idx
                            );
                        cbk.valid_depth =
                            MAX(owc_margin, p_margin
                            ); // user specified, but never less than actual
                               // margin or it won't work
                        cbk.invalid_by_dir = 0;
                        ...
                        if (col_obj->get_type()
                            ...
                            if (b->get_mode()
                                ...
                                // fix for moving platforms (kinematic and
                                // dynamic), margin is increased by how much it
                                // moved in the given direction
                                Vector2 lv = b->get_linear_velocity();
                                // compute displacement from linear velocity
                                Vector2 motion =
                                    lv
                                    * Physics2DDirectBodyStateSW::singleton
                                          ->step;
                                float motion_len = motion.length();
                                motion.normalize();
                                cbk.valid_depth +=
                                    motion_len
                                    * MAX(motion.dot(-cbk.valid_dir), 0.0);
                            }
                        }
                    } else {
                        cbk.valid_dir      = Vector2();
                    ...
                    int current_passed =
                        cbk.passed; // save how many points passed collision
                    ...
                    if (CollisionSolver2DSW::solve(
                        ...
                        did_collide =
                            cbk.passed
                            > current_passed; // more passed, so collision
                                              // actually existed
                    }

                    if (!did_collide && cbk.invalid_by_dir > 0) {
                        // this shape must be excluded
                        if (excluded_shape_pair_count
                            < max_excluded_shape_pairs) {
                            ExcludedShapeSW esp;
...

    {
        ...
        for (int body_shape_idx = 0; body_shape_idx < p_body->get_shape_count();
             body_shape_idx++) {
            ...
            Transform2D body_shape_xform =
                body_transform * p_body->get_shape_transform(body_shape_idx);
            ...
            for (int i = 0; i < amount; i++) {
                ...
                if (CollisionObject2DSW::TYPE_BODY == col_obj->get_type()) {
                    ...
                    if (p_infinite_inertia
                        && Physics2DServer::BODY_MODE_STATIC != b->get_mode()
                        && Physics2DServer::BODY_MODE_KINEMATIC
                               != b->get_mode()) {
                        continue;
                    }
                ...
                for (int k = 0; k < excluded_shape_pair_count; k++) {
                    if (excluded_shape_pairs[k].local_shape == body_shape
                        && excluded_shape_pairs[k].against_object == col_obj
                        && excluded_shape_pairs[k].against_shape_index
                               == col_shape_idx) {
                        excluded = true;
                        break;
                    }
                ...
                Transform2D col_obj_shape_xform =
                    col_obj->get_transform()
                    * col_obj->get_shape_transform(col_shape_idx);
                // test initial overlap, does it collide if going all the way?
                if (!CollisionSolver2DSW::solve(
                        body_shape,
                        body_shape_xform,
                ...
                for (int k = 0; k < 8; k++) { // steps should be customizable..
                    ...
                    if (collided) {
                        hi = fraction;
                        if ((k == 0)
                            || (low > 0.0)) { // Did it not collide before?
                            // When alternating or first iteration, use
                            // dichotomy.
                            fraction_coeff = 0.5;
                        } else {
                            // When colliding again, converge faster towards low
                            // fraction for more accurate results with long
                            // motions that collide near the start.
                            fraction_coeff = 0.25;
                        }
                    } else {
                        low = fraction;
                ...
                if (col_obj->is_shape_set_as_one_way_collision(col_shape_idx)) {
                    ...
                    bool collided = CollisionSolver2DSW::solve(
                        body_shape,
                        body_shape_xform,
                        p_motion * (hi + contact_max_allowed_penetration),
                        col_obj->get_shape(col_shape_idx),
                        col_obj_shape_xform,
                        Vector2(),
                        Physics2DServerSW::_shape_col_cbk,
                        &cbk,
    ...
    if (recovered || (safe < 1)) {
        ...
        // Allowed depth can't be lower than motion length, in order to handle
        // contacts at low speed.
        ...
        int to_shape =
            best_shape != -1 ? best_shape + 1 : p_body->get_shape_count();
        ...
        for (int j = from_shape; j < to_shape; j++) {
            ...
            for (int i = 0; i < amount; i++) {
                ...
                if (col_obj->is_shape_set_as_one_way_collision(shape_idx)) {
                    ...
                    float owc_margin =
                        col_obj->get_shape_one_way_collision_margin(shape_idx);
                    rcd.valid_depth =
                        MAX(owc_margin,
                            p_margin); // user specified, but never less than
                                       // actual margin or it won't work
                    ...
                    if (col_obj->get_type() == CollisionObject2DSW::TYPE_BODY) {
                        ...
                        if (b->get_mode()
                            ...
                            || b->get_mode()
                                   == Physics2DServer::BODY_MODE_RIGID) {
                            // fix for moving platforms (kinematic and dynamic),
                            // margin is increased by how much it moved in the
                            // given direction
                            Vector2 lv = b->get_linear_velocity();
                            // compute displacement from linear velocity
                            Vector2 motion =
                                lv
                                * Physics2DDirectBodyStateSW::singleton->step;
                            float motion_len = motion.length();
                            motion.normalize();
                            rcd.valid_depth +=
                                motion_len
                                * MAX(motion.dot(-rcd.valid_dir), 0.0);
                        }
                    }
                } else {
                    rcd.valid_dir   = Vector2();
        ...
        if (rcd.best_len != 0) {
            if (r_result) {
                ...
                r_result->collider_metadata =
                    rcd.best_object->get_shape_metadata(rcd.best_shape);

                const Body2DSW* body =
                    static_cast<const Body2DSW*>(rcd.best_object);
                Vector2 rel_vec = r_result->collision_point
                                - body->get_transform().get_origin();
                ...
                r_result->motion +=
                    (body_transform.get_origin() - p_from.get_origin());
            }
...
void* Space2DSW::_broadphase_pair(
    ...
    if (type_A == CollisionObject2DSW::TYPE_AREA) {
        ...
        if (type_B == CollisionObject2DSW::TYPE_AREA) {
            ...
            Area2Pair2DSW* area2_pair =
                memnew(Area2Pair2DSW(area_b, p_subindex_B, area_a, p_subindex_A)
                );
            return area2_pair;
        } else {
            Body2DSW* body_b = static_cast<Body2DSW*>(p_object_B);
            AreaPair2DSW* area_pair =
                memnew(AreaPair2DSW(body_b, p_subindex_B, area_a, p_subindex_A)
                );
            return area_pair;
        }
        ...
        Body2DSW* body_b = static_cast<Body2DSW*>(p_object_B);
        BodyPair2DSW* body_pair =
            memnew(BodyPair2DSW(body_a, p_subindex_A, body_b, p_subindex_B));
        return body_pair;
    }
...
void Space2DSW::_broadphase_unpair(
    CollisionObject2DSW* p_object_A,
    int p_subindex_A,
    CollisionObject2DSW* p_object_B,
    int p_subindex_B,
...
void Space2DSW::set_param(
    Physics2DServer::SpaceParameter p_param,
    real_t p_value
) {
    switch (p_param) {
