...
#define FLUSH_QUERY_CHECK(m_object)                                            \
    ERR_FAIL_COND_MSG(                                                         \
        m_object->get_space() && flushing_queries,                             \
...
void Physics2DServerSW::shape_set_custom_solver_bias(
    RID p_shape,
    real_t p_bias
) {
    Shape2DSW* shape = shape_owner.get(p_shape);
...
Physics2DServer::ShapeType Physics2DServerSW::shape_get_type(RID p_shape
) const {
...
void Physics2DServerSW::_shape_col_cbk(
    ...
    if (cbk->valid_dir != Vector2()) {
        if (cbk->valid_depth < 10e20) {
            if (rel_length2 > cbk->valid_depth * cbk->valid_depth
                || (rel_length2 > CMP_EPSILON
                    && cbk->valid_dir.dot(rel_dir.normalized()) < CMP_EPSILON
                )) {
                cbk->invalid_by_dir++;
                return;
            }
...
bool Physics2DServerSW::shape_collide(
    RID p_shape_A,
    ...
    if (p_result_max == 0) {
        return CollisionSolver2DSW::solve(
            shape_A,
            p_xform_A,
            p_motion_A,
            shape_B,
    ...
    bool res = CollisionSolver2DSW::solve(
        shape_A,
        p_xform_A,
        p_motion_A,
        shape_B,
...
void Physics2DServerSW::space_set_param(
    RID p_space,
...
real_t Physics2DServerSW::space_get_param(RID p_space, SpaceParameter p_param)
    const {
    const Space2DSW* space = space_owner.get(p_space);
...
void Physics2DServerSW::space_set_debug_contacts(
    RID p_space,
    int p_max_contacts
) {
    Space2DSW* space = space_owner.get(p_space);
...
Physics2DDirectSpaceState* Physics2DServerSW::space_get_direct_state(RID p_space
    ...
    ERR_FAIL_COND_V_MSG(
        (using_threads && !doing_sync) || space->is_locked(),
        nullptr,
...
Physics2DServer::AreaSpaceOverrideMode Physics2DServerSW::
...
void Physics2DServerSW::area_add_shape(
    RID p_area,
    RID p_shape,
...
void Physics2DServerSW::area_set_shape(
    RID p_area,
    int p_shape_idx,
    RID p_shape
) {
...
void Physics2DServerSW::area_set_shape_transform(
    RID p_area,
    int p_shape_idx,
    const Transform2D& p_transform
) {
...
void Physics2DServerSW::area_set_shape_disabled(
    RID p_area,
    int p_shape,
    bool p_disabled
) {
...
Transform2D Physics2DServerSW::area_get_shape_transform(
    RID p_area,
    int p_shape_idx
) const {
    Area2DSW* area = area_owner.get(p_area);
...
void Physics2DServerSW::area_attach_object_instance_id(
    RID p_area,
    ObjectID p_id
) {
...
void Physics2DServerSW::area_set_transform(
    RID p_area,
    const Transform2D& p_transform
) {
    Area2DSW* area = area_owner.get(p_area);
...
Variant Physics2DServerSW::area_get_param(RID p_area, AreaParameter p_param)
    const {
    if (space_owner.owns(p_area)) {
        Space2DSW* space = space_owner.get(p_area);
        p_area           = space->get_default_area()->get_self();
...
void Physics2DServerSW::area_set_monitor_callback(
    RID p_area,
    Object* p_receiver,
    ...
    area->set_monitor_callback(
        p_receiver ? p_receiver->get_instance_id() : 0,
        p_method
    );
}

void Physics2DServerSW::area_set_area_monitor_callback(
    RID p_area,
    Object* p_receiver,
    const StringName& p_method
) {
...
void Physics2DServerSW::body_add_shape(
    RID p_body,
    RID p_shape,
    const Transform2D& p_transform,
    bool p_disabled
...
void Physics2DServerSW::body_set_shape(
    RID p_body,
    int p_shape_idx,
    RID p_shape
) {
...
void Physics2DServerSW::body_set_shape_transform(
    RID p_body,
    int p_shape_idx,
    const Transform2D& p_transform
) {
...
void Physics2DServerSW::body_set_shape_metadata(
    RID p_body,
    int p_shape_idx,
...
Variant Physics2DServerSW::body_get_shape_metadata(RID p_body, int p_shape_idx)
    const {
    Body2DSW* body = body_owner.get(p_body);
...
Transform2D Physics2DServerSW::body_get_shape_transform(
    RID p_body,
    int p_shape_idx
) const {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_set_shape_disabled(
    RID p_body,
    int p_shape_idx,
    bool p_disabled
) {
...
void Physics2DServerSW::body_set_shape_as_one_way_collision(
    RID p_body,
    int p_shape_idx,
    bool p_enable,
    float p_margin
...
void Physics2DServerSW::body_set_continuous_collision_detection_mode(
    RID p_body,
    CCDMode p_mode
) {
    Body2DSW* body = body_owner.get(p_body);
...
Physics2DServerSW::CCDMode Physics2DServerSW::
    body_get_continuous_collision_detection_mode(RID p_body) const {
...
void Physics2DServerSW::body_attach_object_instance_id(
    RID p_body,
    uint32_t p_id
) {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_attach_canvas_instance_id(
    RID p_body,
    uint32_t p_id
) {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_set_param(
    RID p_body,
...
real_t Physics2DServerSW::body_get_param(RID p_body, BodyParameter p_param)
    const {
    Body2DSW* body = body_owner.get(p_body);
    ERR_FAIL_COND_V(!body, 0);
...
void Physics2DServerSW::body_set_state(
    RID p_body,
    BodyState p_state,
...
void Physics2DServerSW::body_set_applied_force(
    RID p_body,
...
void Physics2DServerSW::body_apply_central_impulse(
    RID p_body,
...
void Physics2DServerSW::body_apply_impulse(
    RID p_body,
    const Vector2& p_pos,
    const Vector2& p_impulse
) {
...
void Physics2DServerSW::body_add_central_force(
    RID p_body,
    const Vector2& p_force
) {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_add_force(
    RID p_body,
...
void Physics2DServerSW::body_set_axis_velocity(
    RID p_body,
    const Vector2& p_axis_velocity
) {
    Body2DSW* body = body_owner.get(p_body);
    ERR_FAIL_COND(!body);
...
void Physics2DServerSW::body_remove_collision_exception(
    RID p_body,
    RID p_body_b
) {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_get_collision_exceptions(
    RID p_body,
    List<RID>* p_exceptions
) {
    Body2DSW* body = body_owner.get(p_body);
    ERR_FAIL_COND(!body);
...
void Physics2DServerSW::body_set_contacts_reported_depth_threshold(
    RID p_body,
    real_t p_threshold
) {
    Body2DSW* body = body_owner.get(p_body);
...
real_t Physics2DServerSW::body_get_contacts_reported_depth_threshold(RID p_body
) const {
    Body2DSW* body = body_owner.get(p_body);
    ERR_FAIL_COND_V(!body, 0);
    return 0;
...
void Physics2DServerSW::body_set_omit_force_integration(
    RID p_body,
    bool p_omit
) {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_set_max_contacts_reported(
    RID p_body,
    int p_contacts
) {
    Body2DSW* body = body_owner.get(p_body);
...
void Physics2DServerSW::body_set_force_integration_callback(
    RID p_body,
    Object* p_receiver,
    const StringName& p_method,
...
bool Physics2DServerSW::body_collide_shape(
    RID p_body,
    int p_body_shape,
    RID p_shape,
    ...
    return shape_collide(
        body->get_shape(p_body_shape)->get_self(),
        body->get_transform() * body->get_shape_transform(p_body_shape),
        Vector2(),
        p_shape,
        p_shape_xform,
        p_motion,
...
bool Physics2DServerSW::body_test_motion(
    RID p_body,
    const Transform2D& p_from,
    const Vector2& p_motion,
    bool p_infinite_inertia,
    real_t p_margin,
    MotionResult* r_result,
    ...
    return body->get_space()->test_body_motion(
        body,
        p_from,
        p_motion,
        p_infinite_inertia,
...
int Physics2DServerSW::body_test_ray_separation(
    RID p_body,
    const Transform2D& p_transform,
    bool p_infinite_inertia,
    Vector2& r_recover_motion,
    SeparationResult* r_results,
    int p_result_max,
    float p_margin
) {
    Body2DSW* body = body_owner.get(p_body);
    ...
    return body->get_space()->test_body_ray_separation(
        body,
        p_transform,
        p_infinite_inertia,
        r_recover_motion,
...
Physics2DDirectBodyState* Physics2DServerSW::body_get_direct_state(RID p_body) {
    ERR_FAIL_COND_V_MSG(
        (using_threads && !doing_sync),
        nullptr,
        "Body state is inaccessible right now, wait for iteration or physics "
        "process notification."
    );
...
void Physics2DServerSW::joint_set_param(
    RID p_joint,
    JointParam p_param,
    real_t p_value
) {
...
real_t Physics2DServerSW::joint_get_param(RID p_joint, JointParam p_param)
    const {
...
void Physics2DServerSW::joint_disable_collisions_between_bodies(
    RID p_joint,
    const bool p_disable
) {
    Joint2DSW* joint = joint_owner.get(p_joint);
    ERR_FAIL_COND(!joint);
...
bool Physics2DServerSW::joint_is_disabled_collisions_between_bodies(RID p_joint
) const {
    const Joint2DSW* joint = joint_owner.get(p_joint);
...
RID Physics2DServerSW::pin_joint_create(
    const Vector2& p_pos,
    RID p_body_a,
    RID p_body_b
) {
...
RID Physics2DServerSW::groove_joint_create(
    const Vector2& p_a_groove1,
    const Vector2& p_a_groove2,
    const Vector2& p_b_anchor,
    RID p_body_a,
    RID p_body_b
) {
    Body2DSW* A = body_owner.get(p_body_a);
    ...
    Joint2DSW* joint =
        memnew(GrooveJoint2DSW(p_a_groove1, p_a_groove2, p_b_anchor, A, B));
    RID self = joint_owner.make_rid(joint);
    joint->set_self(self);
    return self;
}

RID Physics2DServerSW::damped_spring_joint_create(
    const Vector2& p_anchor_a,
    const Vector2& p_anchor_b,
    RID p_body_a,
    RID p_body_b
) {
    Body2DSW* A = body_owner.get(p_body_a);
    ...
    Joint2DSW* joint =
        memnew(DampedSpringJoint2DSW(p_anchor_a, p_anchor_b, A, B));
    RID self = joint_owner.make_rid(joint);
    joint->set_self(self);
    return self;
}

void Physics2DServerSW::pin_joint_set_param(
    RID p_joint,
...
real_t Physics2DServerSW::pin_joint_get_param(
    RID p_joint,
    PinJointParam p_param
) const {
    Joint2DSW* j = joint_owner.get(p_joint);
...
void Physics2DServerSW::damped_string_joint_set_param(
    RID p_joint,
    DampedStringParam p_param,
    real_t p_value
) {
    Joint2DSW* j = joint_owner.get(p_joint);
    ERR_FAIL_COND(!j);
...
real_t Physics2DServerSW::damped_string_joint_get_param(
    RID p_joint,
    DampedStringParam p_param
) const {
    Joint2DSW* j = joint_owner.get(p_joint);
...
Physics2DServer::JointType Physics2DServerSW::joint_get_type(RID p_joint
) const {
    Joint2DSW* joint = joint_owner.get(p_joint);
    ...
    if (shape_owner.owns(p_rid)) {
        ...
        */
        ...
        while (space->get_objects().size()) {
            CollisionObject2DSW* co =
                (CollisionObject2DSW*)space->get_objects().front()->get();
...
void Physics2DServerSW::step(real_t p_step) {
    ...
    active_objects                              = 0;
    collision_pairs                             = 0;
    for (Set<const Space2DSW*>::Element* E = active_spaces.front(); E;
         E                                 = E->next()) {
...
void Physics2DServerSW::flush_queries() {
    ...
    if (ScriptDebugger::get_singleton()
        ...
        for (Set<const Space2DSW*>::Element* E = active_spaces.front(); E;
            ...
            for (int i = 0; i < Space2DSW::ELAPSED_TIME_MAX; i++) {
                total_time[i] +=
                    E->get()->get_elapsed_time(Space2DSW::ElapsedTime(i));
            }
        }
        ...
        values.push_back(
            USEC_TO_SEC(OS::get_singleton()->get_ticks_usec() - time_beg)
        );
