...
#define FUNC0R(m_r, m_type)                                                    \
    m_r m_type() override {                                                    \
        if (Thread::get_caller_id() != server_thread) {                        \
            m_r ret;                                                           \
            command_queue                                                      \
                .push_and_ret(server_name, &ServerName::m_type, &ret);         \
            SYNC_DEBUG                                                         \
            return ret;                                                        \
        } else {                                                               \
        ...
        if (Thread::get_caller_id() != server_thread) {                        \
            ...
            if (m_type##_id_pool.size() == 0) {                                \
                ...
                command_queue.push_and_ret(                                    \
                    this,                                                      \
                    &ServerNameWrapMT::m_type##allocn,                         \
                    &ret                                                       \
                );                                                             \
                SYNC_DEBUG                                                     \
            }                                                                  \
...
#define FUNC5RID(m_type, m_arg1, m_arg2, m_arg3, m_arg4, m_arg5)               \
    ...
    int m_type##allocn(                                                        \
        ...
        for (int i = 0; i < pool_max_size; i++) {                              \
            m_type##_id_pool.push_back(                                        \
                server_name->m_type##_create(p1, p2, p3, p4, p5)               \
            );                                                                 \
        }                                                                      \
        return 0;                                                              \
    }                                                                          \
...
#define FUNC1R(m_r, m_type, m_arg1)                                            \
    m_r m_type(m_arg1 p1) override {                                           \
        if (Thread::get_caller_id() != server_thread) {                        \
            m_r ret;                                                           \
            command_queue                                                      \
                .push_and_ret(server_name, &ServerName::m_type, p1, &ret);     \
            SYNC_DEBUG                                                         \
            return ret;                                                        \
        } else {                                                               \
...
#define FUNC2R(m_r, m_type, m_arg1, m_arg2)                                    \
    m_r m_type(m_arg1 p1, m_arg2 p2) override {                                \
        if (Thread::get_caller_id() != server_thread) {                        \
            m_r ret;                                                           \
            command_queue                                                      \
                .push_and_ret(server_name, &ServerName::m_type, p1, p2, &ret); \
            SYNC_DEBUG                                                         \
            return ret;                                                        \
        } else {                                                               \
...
#define FUNC12(                                                                \
    ...
    void m_type(                                                               \
        ...
        if (Thread::get_caller_id() != server_thread) {                        \
            ...
            server_name                                                        \
                ->m_type(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12);   \
        }                                                                      \
    }
