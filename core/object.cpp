...
static Array convert_property_list(const List<PropertyInfo>* list) {
    Array array;
...
MethodInfo::MethodInfo() : flags(METHOD_FLAG_NORMAL), id(0) {}
...
MethodInfo::MethodInfo(Variant::Type ret) : flags(METHOD_FLAG_NORMAL), id(0) {
    return_val.type = ret;
}
...
bool Object::_predelete() {
    ...
    if (_predelete_ok) {
        _class_ptr = nullptr; // must restore so destructors can access class
                              // ptr correctly
    }
...
void Object::set_indexed(
    ...
    for (int i = 1; i < p_names.size() - 1; i++) {
        value_stack.push_back(
            value_stack.back()->get().get_named(p_names[i], r_valid)
        );
    ...
    for (int i = p_names.size() - 1; i > 0; i--) {
        value_stack.back()->prev()->get().set_named(
            p_names[i],
            value_stack.back()->get(),
            r_valid
        );
...
Variant Object::get_indexed(const Vector<StringName>& p_names, bool* r_valid)
    const {
    if (p_names.empty()) {
        if (r_valid) {
            *r_valid = false;
...
void Object::get_property_list(List<PropertyInfo>* p_list, bool p_reversed)
    const {
...
Variant Object::_call_deferred_bind(
    const Variant** p_args,
    int p_argcount,
    Variant::CallError& r_error
) {
...
#ifdef DEBUG_ENABLED
static void _test_call_error(
    const StringName& p_func,
    const Variant::CallError& error
) {
    switch (error.error) {
        ...
        case Variant::CallError::CALL_ERROR_INVALID_ARGUMENT: {
            ERR_FAIL_MSG(
                "Error calling function: " + String(p_func)
                + " - Invalid type for argument " + itos(error.argument)
                + ", expected " + Variant::get_type_name(error.expected) + "."
            );
            break;
        }
        case Variant::CallError::CALL_ERROR_TOO_MANY_ARGUMENTS: {
            ERR_FAIL_MSG(
                "Error calling function: " + String(p_func)
                + " - Too many arguments, expected " + itos(error.argument)
                + "."
            );
            break;
        }
        case Variant::CallError::CALL_ERROR_TOO_FEW_ARGUMENTS: {
            ERR_FAIL_MSG(
                "Error calling function: " + String(p_func)
                + " - Too few arguments, expected " + itos(error.argument) + "."
            );
            break;
        }
...
#ifdef DEBUG_ENABLED
        ERR_FAIL_COND_MSG(
            Object::cast_to<Reference>(this),
            "Can't 'free' a reference."
        );
...
void Object::call_multilevel_reversed(
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
Variant Object::callv(const StringName& p_method, const Array& p_args) {
    ...
    if (ce.error != Variant::CallError::CALL_OK) {
        ERR_FAIL_V_MSG(
            Variant(),
            "Error calling method from 'callv': "
...
ObjectRC* Object::_use_rc() {
    ...
    // that way, there's no need to allocate and release it at all if this
    // Object is not being referred by any Variant at all.
    ...
    // Spin-wait until we know it's created (or just return if it's already
    // created)
    for (;;) {
...
void Object::set_script_and_instance(
...
void Object::add_user_signal(const MethodInfo& p_signal) {
    ...
    ERR_FAIL_COND_MSG(
        ClassDB::has_signal(get_class_name(), p_signal.name),
        "User signal's name conflicts with a built-in signal of '"
            + get_class_name() + "'."
    );
    ERR_FAIL_COND_MSG(
        signal_map.has(p_signal.name),
...
Error Object::emit_signal(
    ...
    if (!s) {
        ...
        // check in script
        ERR_FAIL_COND_V_MSG(
            !signal_is_valid && !script.is_null()
                && !Ref<Script>(script)->has_script_signal(p_name),
            ERR_UNAVAILABLE,
    ...
    // copy on write will ensure that disconnecting the signal or even deleting
    // the object will not affect the signal calling. this happens automatically
    // and will not change the performance of calling. awesome, isn't it?
    ...
    for (int i = 0; i < ssize; i++) {
        const Connection& c = slot_map.get_value_at(i).connection;

        Object* target = ObjectDB::get_instance(slot_map.get_key_at(i).id);
        if (!target) {
...
#endif
                if (ce.error == Variant::CallError::CALL_ERROR_INVALID_METHOD
                    && !ClassDB::class_exists(target->get_class_name())) {
                    // most likely object is not initialized yet, do not throw
                    // error.
                } else {
                    ERR_PRINT(
                        "Error calling method from signal '" + String(p_name)
                        + "': "
                        + Variant::get_call_error_text(
                            target,
                            c.method,
...
#ifdef TOOLS_ENABLED
        if (disconnect && (c.flags & CONNECT_PERSIST)
            && Engine::get_singleton()->is_editor_hint()) {
            // this signal was connected from the editor, and is being edited.
            // just don't disconnect for now
            disconnect = false;
        }
#endif
        if (disconnect) {
...
void Object::_add_user_signal(const String& p_name, const Array& p_args) {
    // this version of add_user_signal is meant to be used from scripts or
    // external apis without access to ADD_SIGNAL in bind_methods added events
    // are per instance, as opposed to the other ones, which are global
...
void Object::get_all_signal_connections(List<Connection>* p_connections) const {
    ...
    while ((S = signal_map.next(S))) {
        ...
        for (int i = 0; i < s->slot_map.size(); i++) {
            p_connections->push_back(s->slot_map.get_value_at(i).connection);
        }
    }
}

void Object::get_signal_connection_list(
...
int Object::get_persistent_signal_connection_count() const {
    ...
    while ((S = signal_map.next(S))) {
        ...
        for (int i = 0; i < s->slot_map.size(); i++) {
            if (s->slot_map.get_value_at(i).connection.flags & CONNECT_PERSIST) {
                count += 1;
            }
        }
    }
...
Error Object::connect(
    const StringName& p_signal,
    Object* p_to_object,
    const StringName& p_to_method,
    const Vector<Variant>& p_binds,
    uint32_t p_flags
) {
    ...
    if (!s) {
        ...
        if (!signal_is_valid && !script.is_null()) {
            if (Ref<Script>(script)->has_script_signal(p_signal)) {
                ...
                // allow connecting signals anyway if script is invalid, see
                // issue #17070
    ...
    if (s->slot_map.has(target)) {
        if (p_flags & CONNECT_REFERENCE_COUNTED) {
            ...
            ERR_FAIL_V_MSG(
                ERR_INVALID_PARAMETER,
                "Signal '" + p_signal
                    + "' is already connected to given method '" + p_to_method
                    + "' in that object."
            );
        }
    }
...
bool Object::is_connected(
    ...
    if (!s) {
        ...
        if (!script.is_null()
            && Ref<Script>(script)->has_script_signal(p_signal)) {
            return false;
        }
    ...
    // const Map<Signal::Target,Signal::Slot>::Element *E =
    // s->slot_map.find(target); return (E!=NULL);
}
...
void Object::_disconnect(
    ...
    if (!s) {
        bool signal_is_valid =
            ClassDB::has_signal(get_class_name(), p_signal)
            || (!script.is_null()
                && Ref<Script>(script)->has_script_signal(p_signal));
        ERR_FAIL_COND_MSG(
            signal_is_valid,
            vformat(
                "Attempt to disconnect a nonexistent connection to signal '%s' "
                "in %s, with target '%s' in %s.",
                p_signal,
                to_string(),
                p_to_method,
                p_to_object->to_string()
            )
        );
    }
    ERR_FAIL_COND_MSG(
        !s,
        vformat(
            "Disconnecting nonexistent signal '%s' in %s.",
            p_signal,
            to_string()
        )
    );
    ...
    if (!p_force) {
        slot->reference_count--; // by default is zero, if it was not referenced
                                 // it will go below it
...
#ifdef TOOLS_ENABLED
void Object::editor_set_section_unfold(
...
void Object::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set", "property", "value"),
        &Object::_set_bind
    );
    ClassDB::bind_method(D_METHOD("get", "property"), &Object::_get_bind);
    ClassDB::bind_method(
        D_METHOD("set_indexed", "property", "value"),
        &Object::_set_indexed_bind
    );
    ClassDB::bind_method(
        D_METHOD("get_indexed", "property"),
        &Object::_get_indexed_bind
    );
    ClassDB::bind_method(
        D_METHOD("get_property_list"),
        &Object::_get_property_list_bind
    );
    ClassDB::bind_method(
        D_METHOD("get_method_list"),
        &Object::_get_method_list_bind
    );
    ClassDB::bind_method(
        D_METHOD("notification", "what", "reversed"),
        &Object::notification,
        DEFVAL(false)
    );
    ...
    ClassDB::bind_method(
        D_METHOD("get_meta_list"),
        &Object::_get_meta_list_bind
    );

    ClassDB::bind_method(
        D_METHOD("add_user_signal", "signal", "arguments"),
        &Object::_add_user_signal,
        DEFVAL(Array())
    );
    ClassDB::bind_method(
        D_METHOD("has_user_signal", "signal"),
        &Object::_has_user_signal
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_deferred", "property", "value"),
        &Object::set_deferred
    );

    ClassDB::bind_method(
        D_METHOD("callv", "method", "arg_array"),
        &Object::callv
    );
    ...
    ClassDB::bind_method(
        D_METHOD("get_signal_list"),
        &Object::_get_signal_list
    );
    ClassDB::bind_method(
        D_METHOD("get_signal_connection_list", "signal"),
        &Object::_get_signal_connection_list
    );
    ClassDB::bind_method(
        D_METHOD("get_incoming_connections"),
        &Object::_get_incoming_connections
    );

    ClassDB::bind_method(
        D_METHOD("connect", "signal", "target", "method", "binds", "flags"),
        &Object::connect,
        DEFVAL(Array()),
        DEFVAL(0)
    );
    ClassDB::bind_method(
        D_METHOD("disconnect", "signal", "target", "method"),
        &Object::disconnect
    );
    ClassDB::bind_method(
        D_METHOD("is_connected", "signal", "target", "method"),
        &Object::is_connected
    );

    ClassDB::bind_method(
        D_METHOD("set_block_signals", "enable"),
        &Object::set_block_signals
    );
    ClassDB::bind_method(
        D_METHOD("is_blocking_signals"),
        &Object::is_blocking_signals
    );
    ClassDB::bind_method(
        D_METHOD("property_list_changed_notify"),
        &Object::property_list_changed_notify
    );

    ClassDB::bind_method(
        D_METHOD("set_message_translation", "enable"),
        &Object::set_message_translation
    );
    ClassDB::bind_method(
        D_METHOD("can_translate_messages"),
        &Object::can_translate_messages
    );
    ClassDB::bind_method(D_METHOD("tr", "message"), &Object::tr);

    ClassDB::bind_method(
        D_METHOD("is_queued_for_deletion"),
...
Variant::Type Object::get_static_property_type(
    const StringName& p_property,
    bool* r_valid
) const {
    bool valid;
    Variant::Type t =
        ClassDB::get_property_type(get_class_name(), p_property, &valid);
    if (valid) {
        if (r_valid) {
            *r_valid = true;
        }
...
Variant::Type Object::get_static_property_type_indexed(
    ...
    for (int i = 1; i < p_path.size(); i++) {
        if (check.get_type() == Variant::OBJECT
            || check.get_type() == Variant::DICTIONARY
            || check.get_type() == Variant::ARRAY) {
            // We cannot be sure about the type of properties this types can
            // have
            if (r_valid) {
                *r_valid = false;
            }
...
void* Object::get_script_instance_binding(int p_script_language_index) {
    ...
    ERR_FAIL_INDEX_V(
        ...
        MAX_SCRIPT_INSTANCE_BINDINGS,
        nullptr
    );
#endif

    // it's up to the script language to make this thread safe, if the function
    // is called twice due to threads being out of syncro just return the same
    // pointer. if you want to put a big lock in the entire function and keep
    // allocated pointers in a map or something, feel free to do it as it should
    // not really affect performance much (won't be called too often), as in far
    // most caes the condition below will be false afterwards

    if (!_script_instance_bindings[p_script_language_index]) {
        void* script_data = ScriptServer::get_language(p_script_language_index)
                                ->alloc_instance_binding_data(this);
        if (script_data) {
...
void Object::set_script_instance_binding(
...
Object::Object() {
    ...
    memset(
        ...
        0,
        sizeof(void*) * MAX_SCRIPT_INSTANCE_BINDINGS
    );
    script_instance = nullptr;
...
Object::~Object() {
    ...
    if (_emitting) {
        //@todo this may need to actually reach the debugger prioritarily
        // somehow because it may crash before
    ...
    while ((S = signal_map.next(nullptr))) {
        ...
        const VMap<Signal::Target, Signal::Slot>::Pair* slot_list =
    ...
    if (!ScriptServer::are_languages_finished()) {
        for (int i = 0; i < MAX_SCRIPT_INSTANCE_BINDINGS; i++) {
            if (_script_instance_bindings[i]) {
                ScriptServer::get_language(i)->free_instance_binding_data(
                    _script_instance_bindings[i]
                );
            }
        }
    }
...
void ObjectDB::cleanup() {
    ...
    if (instances.size()) {
        WARN_PRINT(
            "ObjectDB instances leaked at exit (run with --verbose for "
            "details)."
        );
        if (OS::get_singleton()->is_stdout_verbose()) {
            // Ensure calling the native classes because if a leaked instance
            // has a script that overrides any of those methods, it'd not be OK
            // to call them at this point, now the scripting languages have
            // already been terminated.
            MethodBind* node_get_name = ClassDB::get_method("Node", "get_name");
            MethodBind* resource_get_path =
                ClassDB::get_method("Resource", "get_path");
            ...
            while ((K = instances.next(K))) {
                ...
                if (instances[*K]->is_class("Node")) {
                    extra_info = " - Node name: "
                               + String(node_get_name->call(
                                   instances[*K],
                                   nullptr,
                                   0,
                                   call_error
                ...
                print_line(
                    "Leaked instance: " + String(instances[*K]->get_class())
                    + ":" + itos(*K) + extra_info
                );
            }
            print_line(
                "Hint: Leaked instances typically happen when nodes are "
                "removed from the scene tree (with `remove_child()`) but not "
                "freed (with `free()` or `queue_free()`)."
            );
        }
    }
    instances.clear();
