...
#define _VC(m_idx)                                                             \
    (VariantCaster<P##m_idx>::cast(                                            \
        (m_idx - 1) >= p_arg_count ? get_default_argument(m_idx - 1)           \
                                   : *p_args[m_idx - 1]                        \
    ))
...
public:
    ...
    _FORCE_INLINE_ Variant::Type get_argument_type(int p_argument) const {
        ERR_FAIL_COND_V(
            p_argument < -1 || p_argument > argument_count,
            Variant::NIL
        );
    ...
    uint32_t get_hint_flags() const {
        return hint_flags | (is_const() ? METHOD_FLAG_CONST : 0)
             | (is_vararg() ? METHOD_FLAG_VARARG : 0);
    }
...
#ifdef DEBUG_METHODS_ENABLED
        Variant::Type* at =
            memnew_arr(Variant::Type, p_info.arguments.size() + 1);
...
MethodBind* create_vararg_method_bind(
    Variant (T::*p_method)(const Variant**, int, Variant::CallError&),
    const MethodInfo& p_info,
    bool p_return_nil_is_variant
) {
