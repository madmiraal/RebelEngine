...
#define _REF_OBJ_PTR(m_variant)                                                \
    (reinterpret_cast<Ref<Reference>*>((m_variant)._get_obj().ref.get_data())  \
         ->ptr())
#define _OBJ_PTR(m_variant)                                                    \
    ((m_variant)._get_obj().rc ? (m_variant)._get_obj().rc->get_ptr()          \
                               : _REF_OBJ_PTR(m_variant))
// _UNSAFE_OBJ_PROXY_PTR is needed for comparing an object Variant against NIL
// or compare two object Variants. It's guaranteed to be unique per object, in
// contrast to the pointer stored in the RC structure, which is set to null when
// the object is destroyed.
#define _UNSAFE_OBJ_PROXY_PTR(m_variant)                                       \
    ((m_variant)._get_obj().rc                                                 \
         ? reinterpret_cast<uint8_t*>((m_variant)._get_obj().rc)               \
         : reinterpret_cast<uint8_t*>(_REF_OBJ_PTR(m_variant)))
...
private:
    ...
    struct ObjData {
        // Will be null for every type deriving from Reference as they have
        // their own reference count mechanism
        ObjectRC* rc;
    ...
    union {
        ...
        uint8_t _mem
            [sizeof(ObjData) > (sizeof(real_t) * 4) ? sizeof(ObjData)
                                                    : (sizeof(real_t) * 4)];
    } _data GCC_ALIGNED_8;
...
// Variant(long unsigned int p_long);
    ...
    evaluate(const Operator& p_op, const Variant& p_a, const Variant& p_b) {
        bool valid = true;
        Variant res;
    ...
    static String get_call_error_text(
        Object* p_base,
        const StringName& p_method,
        const Variant** p_argptrs,
        int p_argcount,
        const Variant::CallError& ce
    );
    ...
    static Vector<Variant::Type> get_method_argument_types(
        Variant::Type p_type,
        const StringName& p_method
    );
    static Vector<Variant> get_method_default_arguments(
        Variant::Type p_type,
        const StringName& p_method
    );
    static Variant::Type get_method_return_type(
        Variant::Type p_type,
        const StringName& p_method,
        bool* r_has_return = nullptr
    );
    static Vector<StringName> get_method_argument_names(
        Variant::Type p_type,
        const StringName& p_method
    );
    static bool is_method_const(
        Variant::Type p_type,
        const StringName& p_method
    );
    ...
    static void get_constructor_list(
        Variant::Type p_type,
        List<MethodInfo>* p_list
    );
    static void get_constants_for_type(
        Variant::Type p_type,
        List<StringName>* p_constants
    );
    static bool has_constant(Variant::Type p_type, const StringName& p_value);
    static Variant get_constant_value(
        Variant::Type p_type,
    ...
    static void construct_from_string(
    ...
    ); // only this is enough for all the other types
...
Vector<Variant> varray(
    const Variant& p_arg1,
    const Variant& p_arg2,
    const Variant& p_arg3
);
Vector<Variant> varray(
    const Variant& p_arg1,
    const Variant& p_arg2,
    const Variant& p_arg3,
    const Variant& p_arg4
);
Vector<Variant> varray(
    const Variant& p_arg1,
    const Variant& p_arg2,
...
struct VariantComparator {
    static _FORCE_INLINE_ bool compare(
        const Variant& p_lhs,
        const Variant& p_rhs
    ) {
