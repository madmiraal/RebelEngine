...
#define DECL_CMD_RET(N)                                                        \
    template <                                                                 \
        class T,                                                               \
        class M,                                                               \
        COMMA_SEP_LIST(TYPE_PARAM, N) COMMA(N) class R>                        \
...
#define CMD_RET_TYPE(N)                                                        \
    CommandRet##N<T, M, COMMA_SEP_LIST(TYPE_ARG, N) COMMA(N) R>
...
#define CMD_SYNC_TYPE(N)                                                       \
    CommandSync##N<T, M COMMA(N) COMMA_SEP_LIST(TYPE_ARG, N)>
...
class CommandQueueMT {
    ...
    T* allocate() {
        ...
        ERR_FAIL_COND_V(
            alloc_size * 2 + sizeof(uint32_t) > command_mem_size,
            nullptr
        );
    ...
    tryagain:
        ...
        if (write_ptr < dealloc_ptr) {
            ...
            if ((command_mem_size - write_ptr)
                < alloc_size + sizeof(uint32_t)) {
                // no room at the end, wrap down;

                if (dealloc_ptr
                    == 0) { // don't want write_ptr to become dealloc_ptr
                ...
                *p          = 1;
                ...
                // space while we wait. This is required if alloc_size * 2 + 4 >
                // COMMAND_MEM_SIZE
                if (sync) {
                    sync->post();
                }
    ...
    tryagain:
        ...
        CommandBase* cmd =
            reinterpret_cast<CommandBase*>(&command_mem[read_ptr]);
