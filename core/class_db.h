...
#ifdef DEBUG_METHODS_ENABLED
    static MethodBind* bind_methodfi(
        uint32_t p_flags,
        MethodBind* p_bind,
...
public:
    ...
    static MethodBind* bind_method(N p_method_name, M p_method) {
        ...
        return bind_methodfi(
            METHOD_FLAGS_DEFAULT,
            bind,
            p_method_name,
            nullptr,
            0
        ); // use static function, much smaller binary usage
    }

    template <class N, class M>
    static MethodBind* bind_method(
        N p_method_name,
        M p_method,
    ...
    static MethodBind* bind_method(
        ...
        const Variant& p_def4,
        const Variant& p_def5,
        const Variant& p_def6
    ) {
        MethodBind* bind = create_method_bind(p_method);
        const Variant* ptr[6] =
            {&p_def1, &p_def2, &p_def3, &p_def4, &p_def5, &p_def6};
    ...
    static MethodBind* bind_method(
        ...
        MethodBind* bind = create_method_bind(p_method);
        const Variant* ptr[7] =
            {&p_def1, &p_def2, &p_def3, &p_def4, &p_def5, &p_def6, &p_def7};
    ...
    template <class M>
    static MethodBind* bind_vararg_method(
        uint32_t p_flags,
        StringName p_name,
        M p_method,
        ...
        MethodBind* bind = create_vararg_method_bind(
            p_method,
            p_info,
        ...
        if (type->method_map.has(p_name)) {
            ...
            ERR_FAIL_V_MSG(
                nullptr,
                "Method already bound: " + instance_type + "::" + p_name + "."
            );
        }
        type->method_map[p_name] = bind;
#ifdef DEBUG_METHODS_ENABLED
        // FIXME: <reduz> set_return_type is no longer in MethodBind, so I guess
        // it should be moved to vararg method bind
    ...
    static void get_signal_list(
        StringName p_class,
        List<MethodInfo>* p_signals,
        bool p_no_inheritance = false
    );

    static void add_property_group(
        StringName p_class,
        const String& p_name,
    ...
    static StringName get_property_setter(
        StringName p_class,
        const StringName& p_property
    );
    static StringName get_property_getter(
        StringName p_class,
        const StringName& p_property
    );
    ...
    static void add_virtual_method(
        const StringName& p_class,
    ...
    static void get_enum_list(
        const StringName& p_class,
        List<StringName>* p_enums,
        bool p_no_inheritance = false
    );
    static void get_enum_constants(
        const StringName& p_class,
        const StringName& p_enum,
    ...
    static void get_extensions_for_type(
        const StringName& p_class,
        List<String>* p_extensions
    );

    static void add_compatibility_class(
        const StringName& p_class,
