...
public:
    ...
    static void (*_image_compress_bc_func)(
        Image*,
        float,
        CompressSource p_source
    );
    static void (*_image_compress_bptc_func)(
        Image*,
    ...
    static void (*_image_compress_etc2_func)(
        Image*,
        float,
        CompressSource p_source
    );
    ...
    static PoolVector<uint8_t> (*webp_lossy_packer)(
...
private:
    ...
    _FORCE_INLINE_ void _get_mipmap_offset_and_size(
        ...
        int& r_width,
        int& r_height
    ) const; // get where the mipmap begins in data

    static int _get_dst_image_size(
        int p_width,
        int p_height,
        Format p_format,
        int& r_mipmaps,
    ...
    _FORCE_INLINE_ void _put_pixelb(
        int p_x,
        int p_y,
    ...
    _FORCE_INLINE_ void _get_pixelb(
        int p_x,
        int p_y,
        uint32_t p_pixelsize,
    ...
    Error _load_from_buffer(
        const PoolVector<uint8_t>& p_array,
        ImageMemLoadFunc p_loader
    );
...
public:
    ...
    void get_mipmap_offset_and_size(int p_mipmap, int& r_ofs, int& r_size)
        const; // get where the mipmap begins in data
    void get_mipmap_offset_size_and_dimensions(
        int p_mipmap,
        int& r_ofs,
        int& r_size,
        int& w,
    ...
    /**
     * Crop the image to a specific size, if larger, then the image is filled by
     * black
     */
    ...
    /**
     * Generate a mipmap to an image (creates an image 1/4 the size, with
     * averaging of 4->1)
     */
    ...
    static int get_image_data_size(
        int p_width,
        int p_height,
        Format p_format,
        bool p_mipmaps = false
    );
    static int get_image_required_mipmaps(
        int p_width,
        int p_height,
        Format p_format
    );
    static int get_image_mipmap_offset(
        int p_width,
        int p_height,
        Format p_format,
        int p_mipmap
    ...
    void blit_rect(
        const Ref<Image>& p_src,
        const Rect2& p_src_rect,
        const Point2& p_dest
    );
    void blit_rect_mask(
        const Ref<Image>& p_src,
        const Ref<Image>& p_mask,
        const Rect2& p_src_rect,
        const Point2& p_dest
    );
    void blend_rect(
    ...
    static void set_compress_bc_func(
        void (*p_compress_func)(Image*, float, CompressSource)
    );
    static void set_compress_bptc_func(
        void (*p_compress_func)(Image*, float, CompressSource)
    );
