...
bool ResourceFormatSaver::recognize(const RES& p_resource) const {
    if (get_script_instance()
        && get_script_instance()->has_method("recognize")) {
...
void ResourceFormatSaver::get_recognized_extensions(
    ...
    if (get_script_instance()
        ...
        PoolStringArray exts = get_script_instance()->call(
...
void ResourceFormatSaver::_bind_methods() {
    {
        ...
        PropertyInfo arg1 = PropertyInfo(
            Variant::OBJECT,
            "resource",
        ...
        ClassDB::add_virtual_method(
            get_class_static(),
            MethodInfo(Variant::INT, "save", arg0, arg1, arg2)
        );
    }
...
void ResourceSaver::add_resource_format_saver(
    ...
    ERR_FAIL_COND_MSG(
        p_format_saver.is_null(),
        "It's not a reference to a valid ResourceFormatSaver object."
    );
...
void ResourceSaver::remove_resource_format_saver(
    Ref<ResourceFormatSaver> p_format_saver
) {
    ERR_FAIL_COND_MSG(
        p_format_saver.is_null(),
...
Ref<ResourceFormatSaver> ResourceSaver::_find_custom_resource_format_saver(
    String path
) {
    for (int i = 0; i < saver_count; ++i) {
        if (saver[i]->get_script_instance()
            && saver[i]->get_script_instance()->get_script()->get_path()
                   == path) {
            return saver[i];
        }
    }
...
bool ResourceSaver::add_custom_resource_format_saver(String script_path) {
    ...
    ERR_FAIL_COND_V_MSG(
        !valid_type,
        false,
        "Script does not inherit a CustomResourceSaver: " + script_path + "."
    );
    ...
    ERR_FAIL_COND_V_MSG(
        obj == nullptr,
        false,
        "Cannot instance script as custom resource saver, expected "
        "'ResourceFormatSaver' inheritance, got: "
            + String(ibt) + "."
    );
...
void ResourceSaver::remove_custom_resource_format_saver(String script_path) {
    Ref<ResourceFormatSaver> custom_saver =
        _find_custom_resource_format_saver(script_path);
