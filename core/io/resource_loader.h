...
class ResourceLoader {
    ...
    static String _path_remap(
        const String& p_path,
        bool* r_translation_remapped = nullptr
    );
    ...
    static Ref<ResourceFormatLoader> _find_custom_resource_format_loader(
        String path
    );
    ...
    struct LoadingMapKeyHasher {
        static _FORCE_INLINE_ uint32_t hash(const LoadingMapKey& p_key) {
            return p_key.path.hash() + HashMapHasherDefault::hash(p_key.thread);
        }
    };
    ...
    static void _remove_from_loading_map_and_thread(
        const String& p_path,
        Thread::ID p_thread
    );
...
public:
    ...
    static void get_recognized_extensions_for_type(
        const String& p_type,
        List<String>* p_extensions
    );
    static void add_resource_format_loader(
        Ref<ResourceFormatLoader> p_format_loader,
        bool p_at_front = false
    );
    static void remove_resource_format_loader(
        Ref<ResourceFormatLoader> p_format_loader
    );
    ...
    static Error rename_dependencies(
        const String& p_path,
        const Map<String, String>& p_map
    );
    ...
    static void set_error_notify_func(
        void* p_ud,
    ...
    static void notify_dependency_error(
        const String& p_path,
    ...
    static void set_dependency_error_notify_func(
        void* p_ud,
