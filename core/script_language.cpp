...
void Script::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "source_code", PROPERTY_HINT_NONE, "", 0),
        "set_source_code",
        "get_source_code"
    );
}
    ...
    { // load global classes
        ...
        if (Global::ProjectSettings().has_setting("_global_script_classes")) {
            ...
            for (int i = 0; i < script_classes.size(); i++) {
                ...
                if (!c.has("class") || !c.has("language") || !c.has("path")
                    || !c.has("base")) {
                    continue;
                }
                add_global_class(
...
HashMap<StringName, ScriptServer::GlobalScriptClass>
...
void ScriptServer::add_global_class(
    ...
    const StringName& p_language,
    const String& p_path
) {
    ERR_FAIL_COND_MSG(
        p_class == p_base
            || (global_classes.has(p_base)
                && get_global_class_native_base(p_base) == p_class),
...
void ScriptInstance::call_multilevel(
    ...
    call(
        ...
        p_argcount,
        ce
    ); // script may not support multilevel calls
}

void ScriptInstance::call_multilevel_reversed(
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
void ScriptInstance::property_set_fallback(
    const StringName&,
    const Variant&,
    bool* r_valid
) {
...
Variant ScriptInstance::property_get_fallback(
    const StringName&,
    bool* r_valid
) {
    if (r_valid) {
...
bool PlaceHolderScriptInstance::set(
...
void PlaceHolderScriptInstance::get_property_list(
...
Variant::Type PlaceHolderScriptInstance::get_property_type(
    const StringName& p_name,
    bool* r_is_valid
) const {
    if (values.has(p_name)) {
...
void PlaceHolderScriptInstance::update(
...
void PlaceHolderScriptInstance::property_set_fallback(
    const StringName& p_name,
    const Variant& p_value,
    bool* r_valid
) {
    if (script->is_placeholder_fallback_enabled()) {
        ...
        if (!found) {
            properties.push_back(PropertyInfo(
                ...
                PROPERTY_HINT_NONE,
                "",
                PROPERTY_USAGE_NOEDITOR | PROPERTY_USAGE_SCRIPT_VARIABLE
            ));
        }
    }
...
Variant PlaceHolderScriptInstance::property_get_fallback(
    const StringName& p_name,
    bool* r_valid
) {
    if (script->is_placeholder_fallback_enabled()) {
