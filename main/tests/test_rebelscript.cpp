...
#include "modules/rebelscript/rebelscript_compiler.h"
#include "modules/rebelscript/rebelscript_parser.h"
#include "modules/rebelscript/rebelscript_tokenizer.h"
...
static String _parser_extends(const RebelScriptParser::ClassNode* p_class) {
    String txt = "extends ";
...
static String _parser_expr(const RebelScriptParser::Node* p_expr) {
    String txt;
    switch (p_expr->type) {
        case RebelScriptParser::Node::TYPE_IDENTIFIER: {
            const RebelScriptParser::IdentifierNode* id_node =
                static_cast<const RebelScriptParser::IdentifierNode*>(p_expr);
            txt = id_node->name;
        } break;
        case RebelScriptParser::Node::TYPE_CONSTANT: {
            const RebelScriptParser::ConstantNode* c_node =
                static_cast<const RebelScriptParser::ConstantNode*>(p_expr);
        ...
        case RebelScriptParser::Node::TYPE_ARRAY: {
            const RebelScriptParser::ArrayNode* arr_node =
                static_cast<const RebelScriptParser::ArrayNode*>(p_expr);
            txt += "[";
        ...
        case RebelScriptParser::Node::TYPE_DICTIONARY: {
            const RebelScriptParser::DictionaryNode* dict_node =
                static_cast<const RebelScriptParser::DictionaryNode*>(p_expr);
            txt += "{";
            for (int i = 0; i < dict_node->elements.size(); i++) {
                ...
                const RebelScriptParser::DictionaryNode::Pair& p =
                    dict_node->elements[i];
        ...
        case RebelScriptParser::Node::TYPE_OPERATOR: {
            const RebelScriptParser::OperatorNode* c_node =
                static_cast<const RebelScriptParser::OperatorNode*>(p_expr);
            switch (c_node->op) {
                ...
                case RebelScriptParser::OperatorNode::OP_CALL: {
                    ...
                    String func_name;
                    const RebelScriptParser::Node* nfunc = c_node->arguments[0];
                    int arg_ofs                          = 0;
                    if (nfunc->type
                        == RebelScriptParser::Node::TYPE_BUILT_IN_FUNCTION) {
                        const RebelScriptParser::BuiltInFunctionNode* bif_node =
                            static_cast<
                                const RebelScriptParser::BuiltInFunctionNode*>(
                                nfunc
                            );
                        func_name = RebelScriptFunctions::get_func_name(
                            bif_node->function
                        );
                        arg_ofs = 1;
                    } else if (nfunc->type == RebelScriptParser::Node::TYPE_TYPE) {
                        const RebelScriptParser::TypeNode* t_node =
                        ...
                        if (c_node->arguments[0]->type
                            != RebelScriptParser::Node::TYPE_SELF) {
                            func_name =
                                _parser_expr(c_node->arguments[0]) + ".";
                        }
                    ...
                    for (int i = arg_ofs; i < c_node->arguments.size(); i++) {
                        const RebelScriptParser::Node* arg =
                            c_node->arguments[i];
                        if (i > arg_ofs) {
                            txt += ", ";
                        }
                ...
                case RebelScriptParser::OperatorNode::OP_INDEX: {
                    ...
                    txt = _parser_expr(c_node->arguments[0]) + "["
                        + _parser_expr(c_node->arguments[1]) + "]";
                ...
                case RebelScriptParser::OperatorNode::OP_IN: {
                    txt = _parser_expr(c_node->arguments[0]) + " in "
                        + _parser_expr(c_node->arguments[1]);
                } break;
                ...
                case RebelScriptParser::OperatorNode::OP_AND: {
                    txt = _parser_expr(c_node->arguments[0]) + " and "
                        + _parser_expr(c_node->arguments[1]);
                } break;
                case RebelScriptParser::OperatorNode::OP_OR: {
                    txt = _parser_expr(c_node->arguments[0]) + " or "
                        + _parser_expr(c_node->arguments[1]);
                } break;
        ...
        case RebelScriptParser::Node::TYPE_CAST: {
            const RebelScriptParser::CastNode* cast_node =
                static_cast<const RebelScriptParser::CastNode*>(p_expr);
            txt = _parser_expr(cast_node->source_node) + " as "
                + cast_node->cast_type.to_string();
        ...
        default: {
            ERR_FAIL_V_MSG(
                "",
                "Parser bug at " + itos(p_expr->line)
                    + ", invalid expression type: " + itos(p_expr->type)
            );
        }
    }
...
static void _parser_show_block(
    const RebelScriptParser::BlockNode* p_block,
    int p_indent
) {
    for (int i = 0; i < p_block->statements.size(); i++) {
        const RebelScriptParser::Node* statement = p_block->statements[i];

        switch (statement->type) {
            case RebelScriptParser::Node::TYPE_CONTROL_FLOW: {
                const RebelScriptParser::ControlFlowNode* cf_node =
                ...
                switch (cf_node->cf_type) {
                    ...
                    case RebelScriptParser::ControlFlowNode::CF_RETURN: {
                        if (cf_node->arguments.size()) {
                            _print_indent(
                                p_indent,
                                "return " + _parser_expr(cf_node->arguments[0])
                            );
                        } else {
            ...
            case RebelScriptParser::Node::TYPE_LOCAL_VAR: {
                const RebelScriptParser::LocalVarNode* lv_node =
                    static_cast<const RebelScriptParser::LocalVarNode*>(
                        statement
                    );
...
static void _parser_show_function(
    const RebelScriptParser::FunctionNode* p_func,
    int p_indent,
    RebelScriptParser::BlockNode* p_initializer = nullptr
) {
    String txt;
    if (p_func->_static) {
        txt = "static ";
    ...
    for (int i = 0; i < p_func->arguments.size(); i++) {
        ...
        if (i >= (p_func->arguments.size() - p_func->default_values.size())) {
            int defarg =
                i - (p_func->arguments.size() - p_func->default_values.size());
            txt += "=";
...
static void _parser_show_class(
    const RebelScriptParser::ClassNode* p_class,
    int p_indent,
    const Vector<String>& p_code
) {
    if (p_indent == 0
        && (String(p_class->extends_file) != "" || p_class->extends_class.size()
        )) {
    ...
    for (int i = 0; i < p_class->subclasses.size(); i++) {
        const RebelScriptParser::ClassNode* subclass = p_class->subclasses[i];
    ...
    for (Map<StringName, RebelScriptParser::ClassNode::Constant>::Element* E =
         ...
         E;
         E = E->next()) {
        const RebelScriptParser::ClassNode::Constant& constant = E->get();
        _print_indent(
            p_indent,
            "const " + String(E->key()) + "="
                + _parser_expr(constant.expression)
        );
    }

    for (int i = 0; i < p_class->variables.size(); i++) {
        const RebelScriptParser::ClassNode::Member& m = p_class->variables[i];
...
static String _disassemble_addr(
    const Ref<RebelScript>& p_script,
    const RebelScriptFunction& func,
    int p_addr
) {
    int addr = p_addr & RebelScriptFunction::ADDR_MASK;

    switch (p_addr >> RebelScriptFunction::ADDR_BITS) {
        case RebelScriptFunction::ADDR_TYPE_SELF: {
            return "self";
        } break;
        case RebelScriptFunction::ADDR_TYPE_CLASS: {
            return "class";
        } break;
        case RebelScriptFunction::ADDR_TYPE_MEMBER: {
            return "member(" + p_script->debug_get_member_by_index(addr) + ")";
        } break;
        case RebelScriptFunction::ADDR_TYPE_CLASS_CONSTANT: {
            return "class_const(" + func.get_global_name(addr) + ")";
        } break;
        case RebelScriptFunction::ADDR_TYPE_LOCAL_CONSTANT: {
            Variant v = func.get_constant(addr);
            String txt;
            if (v.get_type() == Variant::STRING
                || v.get_type() == Variant::NODE_PATH) {
                txt = "\"" + String(v) + "\"";
            } else {
                txt = v;
            }
        ...
        case RebelScriptFunction::ADDR_TYPE_STACK: {
            return "stack(" + itos(addr) + ")";
        } break;
        case RebelScriptFunction::ADDR_TYPE_STACK_VARIABLE: {
            return "var_stack(" + itos(addr) + ")";
        } break;
        case RebelScriptFunction::ADDR_TYPE_GLOBAL: {
            return "global(" + func.get_global_name(addr) + ")";
        } break;
        case RebelScriptFunction::ADDR_TYPE_NIL: {
            return "nil";
        } break;
    }
...
static void _disassemble_class(
    const Ref<RebelScript>& p_class,
    const Vector<String>& p_code
) {
    const Map<StringName, RebelScriptFunction*>& mf =
        p_class->debug_get_member_functions();

    for (const Map<StringName, RebelScriptFunction*>::Element* E = mf.front();
         E;
         E = E->next()) {
        const RebelScriptFunction& func = *E->get();
        ...
        print_line(
            "== function " + String(func.get_name()) + "() :: stack size: "
            + itos(func.get_max_stack_size()) + " " + defargs + "=="
        );
...
#define DADDR(m_ip) (_disassemble_addr(p_class, func, code[ip + m_ip]))
        ...
        for (int ip = 0; ip < codelen;) {
            ...
            switch (code[ip]) {
                case RebelScriptFunction::OPCODE_OPERATOR: {
                    int op  = code[ip + 1];
                    txt    += " op ";
                ...
                case RebelScriptFunction::OPCODE_SET: {
                    txt  += "set ";
                    txt  += DADDR(1);
                    txt  += "[";
                    txt  += DADDR(2);
                ...
                case RebelScriptFunction::OPCODE_GET: {
                    txt  += " get ";
                    txt  += DADDR(3);
                    txt  += "=";
                    txt  += DADDR(1);
                ...
                case RebelScriptFunction::OPCODE_SET_NAMED: {
                    txt  += " set_named ";
                    txt  += DADDR(1);
                    txt  += "[\"";
                ...
                case RebelScriptFunction::OPCODE_GET_NAMED: {
                    txt  += " get_named ";
                    txt  += DADDR(3);
                    txt  += "=";
                    txt  += DADDR(1);
                ...
                case RebelScriptFunction::OPCODE_SET_MEMBER: {
                    txt  += " set_member ";
                    txt  += "[\"";
                ...
                case RebelScriptFunction::OPCODE_GET_MEMBER: {
                    txt  += " get_member ";
                    txt  += DADDR(2);
                    txt  += "=";
                    txt  += "[\"";
                ...
                case RebelScriptFunction::OPCODE_ASSIGN: {
                    txt  += " assign ";
                    txt  += DADDR(1);
                    txt  += "=";
                    txt  += DADDR(2);
                ...
                case RebelScriptFunction::OPCODE_ASSIGN_TRUE: {
                    txt  += " assign ";
                    txt  += DADDR(1);
                    txt  += "= true";
                    incr += 2;
                ...
                case RebelScriptFunction::OPCODE_ASSIGN_FALSE: {
                    txt  += " assign ";
                    txt  += DADDR(1);
                    txt  += "= false";
                    incr += 2;
                ...
                case RebelScriptFunction::OPCODE_ASSIGN_TYPED_BUILTIN: {
                ...
                case RebelScriptFunction::OPCODE_ASSIGN_TYPED_NATIVE: {
                    Variant className = func.get_constant(code[ip + 1]);
                    RebelScriptNativeClass* nc =
                        Object::cast_to<RebelScriptNativeClass>(
                ...
                case RebelScriptFunction::OPCODE_CAST_TO_SCRIPT: {
                    txt  += " cast ";
                    txt  += DADDR(3);
                    txt  += "=";
                    txt  += DADDR(1);
                ...
                case RebelScriptFunction::OPCODE_CONSTRUCT: {
                ...
                case RebelScriptFunction::OPCODE_CONSTRUCT_ARRAY: {
                    int argc  = code[ip + 1];
                    txt      += " make_array ";
                    txt      += DADDR(2 + argc);
                    txt      += " = [ ";
                ...
                case RebelScriptFunction::OPCODE_CONSTRUCT_DICTIONARY: {
                    int argc  = code[ip + 1];
                    txt      += " make_dict ";
                    txt      += DADDR(2 + argc * 2);
                    txt      += " = { ";
                ...
                case RebelScriptFunction::OPCODE_CALL:
                case RebelScriptFunction::OPCODE_CALL_RETURN: {
                    bool ret =
                        code[ip] == RebelScriptFunction::OPCODE_CALL_RETURN;
                ...
                case RebelScriptFunction::OPCODE_CALL_BUILT_IN: {
                    txt += " call-built-in ";

                    int argc  = code[ip + 2];
                    txt      += DADDR(3 + argc) + "=";

                    txt += RebelScriptFunctions::get_func_name(
                        RebelScriptFunctions::Function(code[ip + 1])
                    );
                    txt += "(";
                ...
                case RebelScriptFunction::OPCODE_CALL_SELF_BASE: {
                    txt += " call-self-base ";
                ...
                case RebelScriptFunction::OPCODE_YIELD: {
                    txt  += " yield ";
                    incr  = 1;

                } break;
                case RebelScriptFunction::OPCODE_YIELD_SIGNAL: {
                    txt  += " yield_signal ";
                    txt  += DADDR(1);
                    txt  += ",";
                    txt  += DADDR(2);
                ...
                case RebelScriptFunction::OPCODE_YIELD_RESUME: {
                    txt  += " yield resume: ";
                    txt  += DADDR(1);
                    incr  = 2;
                } break;
                case RebelScriptFunction::OPCODE_JUMP: {
                    txt += " jump ";
                    txt += itos(code[ip + 1]);
                ...
                case RebelScriptFunction::OPCODE_JUMP_IF: {
                    txt += " jump-if ";
                    txt += DADDR(1);
                    txt += " to ";
                    txt += itos(code[ip + 2]);
                ...
                case RebelScriptFunction::OPCODE_JUMP_IF_NOT: {
                    txt += " jump-if-not ";
                    txt += DADDR(1);
                    txt += " to ";
                    txt += itos(code[ip + 2]);
                ...
                case RebelScriptFunction::OPCODE_JUMP_TO_DEF_ARGUMENT: {
                    txt  += " jump-to-default-argument ";
                    incr  = 1;
                } break;
                case RebelScriptFunction::OPCODE_RETURN: {
                    txt += " return ";
                    txt += DADDR(1);
                ...
                case RebelScriptFunction::OPCODE_ITERATE_BEGIN: {
                    txt += " for-init " + DADDR(4) + " in " + DADDR(2)
                         + " counter " + DADDR(1) + " end "
                         + itos(code[ip + 3]);
                    incr += 5;

                } break;
                case RebelScriptFunction::OPCODE_ITERATE: {
                    txt += " for-loop " + DADDR(4) + " in " + DADDR(2)
                         + " counter " + DADDR(1) + " end "
                         + itos(code[ip + 3]);
                    incr += 5;

                } break;
                case RebelScriptFunction::OPCODE_LINE: {
                ...
                case RebelScriptFunction::OPCODE_END: {
                    txt  += " end";
                    incr += 1;
                } break;
                case RebelScriptFunction::OPCODE_ASSERT: {
                    txt  += " assert ";
                    txt  += DADDR(1);
                    incr += 2;
...
MainLoop* test(TestType p_type) {
    ...
    if (!test.ends_with(".gd") && !test.ends_with(".gdc")) {
        print_line(
            "This test expects a path to a RebelScript file as its last "
            "parameter. Got: "
            + test
        );
        return nullptr;
    ...
    if (p_type == TEST_TOKENIZER) {
        RebelScriptTokenizerText tk;
        tk.set_code(code);
        int line = -1;
        while (tk.get_token() != RebelScriptTokenizer::TK_EOF) {
            String text;
            if (tk.get_token() == RebelScriptTokenizer::TK_IDENTIFIER) {
                text = "'" + tk.get_token_identifier() + "' (identifier)";
            } else if (tk.get_token() == RebelScriptTokenizer::TK_CONSTANT) {
                ...
                text = text + " (" + Variant::get_type_name(c.get_type())
                     + " constant)";
            } else if (tk.get_token() == RebelScriptTokenizer::TK_ERROR) {
                text = "ERROR: " + tk.get_token_error();
            } else if (tk.get_token() == RebelScriptTokenizer::TK_NEWLINE) {
                text = "newline (" + itos(tk.get_token_line())
                     + ") + indent: " + itos(tk.get_token_line_indent());
            } else if (tk.get_token() == RebelScriptTokenizer::TK_BUILT_IN_FUNC) {
                text = "'"
                     + String(RebelScriptFunctions::get_func_name(
                         tk.get_token_built_in_func()
                     ))
                     + "' (built-in function)";
            } else {
    ...
    if (p_type == TEST_PARSER) {
        ...
        if (err) {
            print_line(
                "Parse Error:\n" + itos(parser.get_error_line()) + ":"
                + itos(parser.get_error_column()) + ":" + parser.get_error()
            );
            memdelete(fa);
            return nullptr;
        }
        ...
        const RebelScriptParser::ClassNode* cnode =
    ...
    if (p_type == TEST_COMPILER) {
        ...
        RebelScriptCompiler gdc;
        err = gdc.compile(&parser, gds.ptr());
        if (err) {
            print_line(
                "Compile Error:\n" + itos(gdc.get_error_line()) + ":"
                + itos(gdc.get_error_column()) + ":" + gdc.get_error()
            );
            return nullptr;
        }

        Ref<RebelScript> current = gds;
        ...
        Vector<uint8_t> buf2 =
            RebelScriptTokenizerBuffer::parse_code_string(code);
...
MainLoop* test(TestType p_type) {
    ERR_PRINT(
        "The RebelScript module is disabled, therefore RebelScript tests "
        "cannot be used."
    );
    return NULL;
}
