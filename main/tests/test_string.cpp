...
bool test_16() {
    ...
    OS::get_singleton()->print(
        "\t\"Revenge of the Monster Truck\" is at %i pos.\n",
...
bool test_17() {
    ...
    OS::get_singleton()->print(
        "\t\"Revenge of the Monster SawFish\" is at %i pos.\n",
...
bool test_19() {
    ...
    OS::get_singleton()->print(
        "\tReplaced Birthday/Halloween: %ls.\n",
        s.c_str()
    );
...
bool test_22() {
    ...
    for (int i = 0; i < 4; i++) {
        OS::get_singleton()->print(
            "\tString: \"%s\" as Int is %i\n",
            nums[i],
            String(nums[i]).to_int()
        );
...
bool test_23() {
    ...
    static const char* nums[4] =
        {"-12348298412.2", "0.05", "2.0002", " -0.0001"};
    static const double num[4] = {-12348298412.2, 0.05, 2.0002, -0.0001};

    for (int i = 0; i < 4; i++) {
        OS::get_singleton()->print(
            "\tString: \"%s\" as Float is %f\n",
            nums[i],
            String(nums[i]).to_double()
        );
...
bool test_28() {
    ...
    output = format.sprintf(args, &error);
    success =
        (output == "not all arguments converted during string formatting"
         && error);
    OS::get_singleton()->print(
        output_format,
        format.c_str(),
    ...
    output = format.sprintf(args, &error);
    success =
        (output == "%c requires number or single-character string" && error);
    OS::get_singleton()->print(
        output_format,
        format.c_str(),
        output.c_str(),
...
bool test_30() {
    ...
    state   = state && success;
    OS::get_singleton()->print(
        "Capitalize %ls (existing incorrect behavior): %ls, %s\n",
        input.c_str(),
        output.c_str(),
        success ? "OK" : "FAIL"
    );
...
#define STRIP_TEST(x)                                                          \
    ...
    STRIP_TEST(
        String::utf8("¿").lstrip(String::utf8("µÿ")) == String::utf8("¿")
    );
    STRIP_TEST(
        String::utf8("¿").rstrip(String::utf8("µÿ")) == String::utf8("¿")
    );
    STRIP_TEST(
        String::utf8("µ¿ÿ").lstrip(String::utf8("µÿ")) == String::utf8("¿ÿ")
    );
    STRIP_TEST(
        String::utf8("µ¿ÿ").rstrip(String::utf8("µÿ")) == String::utf8("µ¿")
    );
    ...
    // and the same second as "ÿ" (\u00ff)
    STRIP_TEST(
        String::utf8("¿").lstrip(String::utf8("qwaµÿjkl")) == String::utf8("¿")
    );
    STRIP_TEST(
        String::utf8("¿").rstrip(String::utf8("qwaµÿjkl")) == String::utf8("¿")
    );
    STRIP_TEST(
        String::utf8("µ¿ÿ").lstrip(String::utf8("qwaµÿjkl"))
        == String::utf8("¿ÿ")
    );
    STRIP_TEST(
        String::utf8("µ¿ÿ").rstrip(String::utf8("qwaµÿjkl"))
        == String::utf8("µ¿")
    );
...
TestFunc test_funcs[] = {
    ...
    test_33, test_34, test_35, test_36, nullptr
