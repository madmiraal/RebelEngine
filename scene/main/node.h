...
private:
    ...
    struct Data {
        ...
        int pos;
        int depth;
        int blocked; // safeguard that throws an error when attempting to modify
                     // the tree in a harmful way while being traversed.
        StringName name;
        SceneTree* tree;
        bool inside_tree;
        bool ready_notified; // this is a small hack, so if a node is added
                             // during _ready() to the tree, it correctly gets
                             // the _ready() notification
        bool ready_first;
#ifdef TOOLS_ENABLED
        NodePath import_path; // path used when imported, used by scene editors
                              // to keep tracking
#endif
        ...
        // variables used to properly sort the node when processing, ignored
        // otherwise
    ...
    void _validate_child_name(
        Node* p_child,
        bool p_force_human_readable = false
    );
    void _generate_serial_child_name(const Node* p_child, StringName& name)
        const;
    ...
    void _duplicate_and_reown(
        Node* p_new_parent,
        const Map<Node*, Node*>& p_reown_map
    ) const;
    Node* _duplicate(int p_flags, Map<const Node*, Node*>* r_duplimap = nullptr)
        const;
    ...
    Variant _rpc_unreliable_bind(
        const Variant** p_args,
        int p_argcount,
        Variant::CallError& r_error
    );
    Variant _rpc_id_bind(
        const Variant** p_args,
        int p_argcount,
        Variant::CallError& r_error
    );
    Variant _rpc_unreliable_id_bind(
        const Variant** p_args,
        int p_argcount,
        Variant::CallError& r_error
    );
...
public:
    ...
    void add_child_below_node(
        Node* p_node,
        Node* p_child,
        bool p_legible_unique_name = false
    );
    ...
    Node* duplicate(
        int p_flags = DUPLICATE_GROUPS | DUPLICATE_SIGNALS | DUPLICATE_SCRIPTS
    ) const;
    Node* duplicate_and_reown(const Map<Node*, Node*>& p_reown_map) const;
#ifdef TOOLS_ENABLED
    Node* duplicate_from_editor(Map<const Node*, Node*>& r_duplimap) const;
    Node* duplicate_from_editor(
        Map<const Node*, Node*>& r_duplimap,
        const Map<RES, RES>& p_resource_remap
    ) const;
    void remap_node_resources(
        Node* p_node,
        const Map<RES, RES>& p_resource_remap
    ) const;
    void remap_nested_resources(
        RES p_resource,
        const Map<RES, RES>& p_resource_remap
    ) const;
#endif
    ...
    void set_import_path(const NodePath& p_import_path
    ...
    const Map<StringName, MultiplayerAPI::RPCMode>::Element* get_node_rpc_mode(
        const StringName& p_method
    );
    const Map<StringName, MultiplayerAPI::RPCMode>::Element* get_node_rset_mode(
        const StringName& p_property
    );
