...
public:
    void add_node(
        NodeType p_type,
        const StringName& p_node
    ); // nodes must be >0 node 0 is built-in (exit)
    ...
    StringName node_get_input_source(const StringName& p_node, int p_input)
        const;

    String get_configuration_warning() const override;

    /* ANIMATION NODE */
    void animation_node_set_animation(
        const StringName& p_node,
    ...
    void animation_node_set_get_filtered_paths(
        const StringName& p_node,
        List<NodePath>* r_paths
    ) const;
    bool animation_node_is_path_filtered(
        const StringName& p_node,
    ...
    void oneshot_node_set_autorestart_delay(
        const StringName& p_node,
        float p_time
    );
    ...
    float oneshot_node_get_autorestart_random_delay(const StringName& p_node
    ) const;
    ...
    void oneshot_node_set_filter_path(
        const StringName& p_node,
        const NodePath& p_filter,
        bool p_enable
    );
    void oneshot_node_set_get_filtered_paths(
        const StringName& p_node,
        List<NodePath>* r_paths
    ) const;
    bool oneshot_node_is_path_filtered(
        const StringName& p_node,
        const NodePath& p_path
    ) const;
    ...
    void blend2_node_set_get_filtered_paths(
        const StringName& p_node,
        List<NodePath>* r_paths
    ) const;
    bool blend2_node_is_path_filtered(
        const StringName& p_node,
        const NodePath& p_path
    ) const;
    ...
    void transition_node_set_input_count(
        const StringName& p_node,
        int p_inputs
    ); // used for transition node
    ...
    void transition_node_set_input_auto_advance(
        const StringName& p_node,
        int p_input,
        bool p_auto_advance
    ); // used for transition node
    bool transition_node_has_input_auto_advance(
        const StringName& p_node,
        int p_input
    ) const;

    void transition_node_set_xfade_time(
        const StringName& p_node,
        float p_time
    ); // used for transition node
