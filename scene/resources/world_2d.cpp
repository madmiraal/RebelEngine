...
struct SpatialIndexer2D {
    ...
    void _notifier_update(
        VisibilityNotifier2D* p_notifier,
        const Rect2& p_rect
    ) {
        Map<VisibilityNotifier2D*, Rect2>::Element* E =
            notifiers.find(p_notifier);
        ERR_FAIL_COND(!E);
        if (E->get() == p_rect) {
            return;
        }
    ...
    void _notifier_remove(VisibilityNotifier2D* p_notifier) {
        ...
        for (Map<Viewport*, ViewportData>::Element* F = viewports.front(); F;
             F                                        = F->next()) {
            Map<VisibilityNotifier2D*, uint64_t>::Element* G =
    ...
    void _remove_viewport(Viewport* p_viewport) {
        ...
        for (Map<VisibilityNotifier2D*, uint64_t>::Element* E =
                 viewports[p_viewport].notifiers.front();
             E;
             E = E->next()) {
    ...
    void _update() {
        ...
        for (Map<Viewport*, ViewportData>::Element* E = viewports.front(); E;
             E                                        = E->next()) {
            ...
            uint64_t visible_cells =
                (uint64_t)(end.x - begin.x) * (uint64_t)(end.y - begin.y);

            if (visible_cells > 10000) {
                // well you zoomed out a lot, it's your problem. To avoid
                // freezing in the for loops below, we'll manually check cell by
                // cell

                for (Map<CellKey, CellData>::Element* F = cells.front(); F;
                     F                                  = F->next()) {
                    ...
                    // notifiers in cell
                    for (Map<VisibilityNotifier2D*, CellRef>::Element* G =
                             F->get().notifiers.front();
                         G;
                         G = G->next()) {
                        Map<VisibilityNotifier2D*, uint64_t>::Element* H =
                            E->get().notifiers.find(G->key());
                        if (!H) {
            ...
            for (Map<VisibilityNotifier2D*, uint64_t>::Element* F =
                     E->get().notifiers.front();
                 F;
                 F = F->next()) {
                if (F->get() != pass) {
                    removed.push_back(F->key());
...
void World2D::_register_notifier(
    VisibilityNotifier2D* p_notifier,
    const Rect2& p_rect
) {
...
void World2D::get_viewport_list(List<Viewport*>* r_viewports) {
    for (Map<Viewport*, SpatialIndexer2D::ViewportData>::Element* E =
             indexer->viewports.front();
         E;
         E = E->next()) {
...
void World2D::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("get_direct_space_state"),
        &World2D::get_direct_space_state
    );

    ADD_PROPERTY(
        PropertyInfo(Variant::_RID, "canvas", PROPERTY_HINT_NONE, "", 0),
        "",
        "get_canvas"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::_RID, "space", PROPERTY_HINT_NONE, "", 0),
        "",
        "get_space"
    );
    ADD_PROPERTY(
...
World2D::World2D() {
    ...
    // set space2D to be more friendly with pixels than meters, by adjusting
    // some constants
    Physics2DServer::get_singleton()->space_set_active(space, true);
    Physics2DServer::get_singleton()->area_set_param(
        space,
        Physics2DServer::AREA_PARAM_GRAVITY,
        GLOBAL_DEF("physics/2d/default_gravity", 98)
    );
    Physics2DServer::get_singleton()->area_set_param(
        space,
    ...
    Physics2DServer::get_singleton()->area_set_param(
        space,
        Physics2DServer::AREA_PARAM_LINEAR_DAMP,
        GLOBAL_DEF("physics/2d/default_linear_damp", 0.1)
    );
    ...
    Physics2DServer::get_singleton()->area_set_param(
        space,
        Physics2DServer::AREA_PARAM_ANGULAR_DAMP,
        GLOBAL_DEF("physics/2d/default_angular_damp", 1.0)
    );
    Global::ProjectSettings().set_custom_property_info(
