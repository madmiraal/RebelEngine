...
float DynamicFont::get_ascent() const {
    ...
    return font_at_size->get_ascent() + static_cast<float>(top_spacing);
}
...
float DynamicFont::get_descent() const {
    ...
    return font_at_size->get_descent() + static_cast<float>(bottom_spacing);
}
...
float DynamicFont::get_height() const {
    ...
    return font_at_size->get_height()
         + static_cast<float>(top_spacing + bottom_spacing);
}
...
Size2 DynamicFont::get_char_size(
    ...
    if (character == ' ') {
        character_size.width +=
            static_cast<float>(space_spacing + character_spacing);
    } else if (next_character) {
        character_size.width += static_cast<float>(character_spacing);
    }
...
void DynamicFont::set_spacing(const int spacing_type, const int new_value) {
    if (spacing_type == SPACING_TOP) {
...
bool DynamicFont::_get(const StringName& name, Variant& result) const {
    ...
    if (string.begins_with("fallback/")) {
        const int index = string.get_slicec('/', 1).to_int();
...
bool DynamicFont::_set(const StringName& name, const Variant& new_value) {
    ...
    if (string.begins_with("fallback/")) {
        const int index = string.get_slicec('/', 1).to_int();
        const Ref<DynamicFontData> new_font_data = new_value;
