...
Map<ParticlesMaterial::MaterialKey, ParticlesMaterial::ShaderData>
    ParticlesMaterial::shader_map;
...
void ParticlesMaterial::_update_shader() {
    ...
    if (emission_shape == EMISSION_SHAPE_POINTS
        ...
        code +=
            "	int point = min(emission_texture_point_count - 1, "
            "int(rand_from_seed(alt_seed) * "
            "float(emission_texture_point_count)));\n";
        code +=
            "	ivec2 emission_tex_size = textureSize(emission_texture_points, "
            "0);\n";
        code +=
            "	ivec2 emission_tex_ofs = ivec2(point % emission_tex_size.x, "
            "point / emission_tex_size.x);\n";
    }
    ...
    code +=
        "		uint alt_restart_seed = hash(base_number + uint(301184) + "
        "RANDOM_SEED);\n";

    if (tex_parameters[PARAM_INITIAL_LINEAR_VELOCITY].is_valid()) {
        code +=
            "		float tex_linear_velocity = "
            "textureLod(linear_velocity_texture, vec2(0.0, 0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_ANGLE].is_valid()) {
        code +=
            "		float tex_angle = textureLod(angle_texture, vec2(0.0, "
            "0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_ANIM_OFFSET].is_valid()) {
        code +=
            "		float tex_anim_offset = textureLod(anim_offset_texture, "
            "vec2(0.0, 0.0), 0.0).r;\n";
    } else {
    ...
    if (flags[FLAG_DISABLE_Z]) {
        ...
        code +=
            "			float angle1_rad = "
            "rand_from_seed_m1_p1(alt_restart_seed) * spread_rad;\n";
        code +=
            "			angle1_rad += direction.x != 0.0 ? atan(direction.y, "
            "direction.x) : sign(direction.y) * (pi / 2.0);\n";
        code +=
            "			vec3 rot = vec3(cos(angle1_rad), sin(angle1_rad), "
            "0.0);\n";
        code +=
            "			VELOCITY = rot * initial_linear_velocity * mix(1.0, "
            "rand_from_seed(alt_restart_seed), "
            "initial_linear_velocity_random);\n";
        code += "		}\n";
        ...
        code +=
            "			float angle2_rad = "
            "rand_from_seed_m1_p1(alt_restart_seed) * spread_rad * (1.0 - "
            "flatness);\n";
        code +=
            "			vec3 direction_xz = vec3(sin(angle1_rad), 0.0, "
            "cos(angle1_rad));\n";
        code +=
            "			vec3 direction_yz = vec3(0.0, sin(angle2_rad), "
            "cos(angle2_rad));\n";
        code +=
            "			direction_yz.z = direction_yz.z / "
            "max(0.0001,sqrt(abs(direction_yz.z))); // better uniform "
            "distribution\n";
        code +=
            "			vec3 spread_direction = vec3(direction_xz.x * "
            "direction_yz.z, direction_yz.y, direction_xz.z * "
            "direction_yz.z);\n";
        code +=
            "			vec3 direction_nrm = length(direction) > 0.0 ? "
            "normalize(direction) : vec3(0.0, 0.0, 1.0);\n";
        code += "			// rotate spread to direction\n";
        code +=
            "			vec3 binormal = cross(vec3(0.0, 1.0, 0.0), "
            "direction_nrm);\n";
        code += "			if (length(binormal) < 0.0001) {\n";
        code +=
            "				// direction is parallel to Y. Choose Z as the "
            "binormal.\n";
        ...
        code +=
            ...
            "normal * spread_direction.y + direction_nrm * "
            "spread_direction.z;\n";
        code +=
            "			VELOCITY = spread_direction * initial_linear_velocity "
            "* mix(1.0, rand_from_seed(alt_restart_seed), "
            "initial_linear_velocity_random);\n";
        code += "		}\n";
    }

    code +=
        "		float base_angle = (initial_angle + tex_angle) * mix(1.0, "
        "angle_rand, initial_angle_random);\n";
    code += "		CUSTOM.x = base_angle * degree_to_rad;\n"; // angle
    code += "		CUSTOM.y = 0.0;\n";                        // phase
    code +=
        "		CUSTOM.w = (1.0 - lifetime_randomness * "
        "rand_from_seed(alt_restart_seed));\n";
    code +=
        "		CUSTOM.z = (anim_offset + tex_anim_offset) * mix(1.0, "
        "anim_offset_rand, anim_offset_random);\n"; // animation offset (0-1)
    ...
    switch (emission_shape) {
        ...
        case EMISSION_SHAPE_SPHERE: {
            code +=
                "		float s = rand_from_seed(alt_restart_seed) * 2.0 - "
                "1.0;\n";
            code +=
                "		float t = rand_from_seed(alt_restart_seed) * 2.0 * "
                "pi;\n";
            code +=
                "		float radius = emission_sphere_radius * sqrt(1.0 - s * "
                "s);\n";
            code +=
                "		TRANSFORM[3].xyz = vec3(radius * cos(t), radius * "
                "sin(t), emission_sphere_radius * s);\n";
        } break;
        case EMISSION_SHAPE_BOX: {
            code +=
                "		TRANSFORM[3].xyz = "
                "vec3(rand_from_seed(alt_restart_seed) * 2.0 - 1.0, "
                "rand_from_seed(alt_restart_seed) * 2.0 - 1.0, "
                "rand_from_seed(alt_restart_seed) * 2.0 - 1.0) * "
        ...
        case EMISSION_SHAPE_DIRECTED_POINTS: {
            code +=
                "		TRANSFORM[3].xyz = texelFetch(emission_texture_points, "
                "emission_tex_ofs, 0).xyz;\n";
            ...
            if (emission_shape == EMISSION_SHAPE_DIRECTED_POINTS) {
                if (flags[FLAG_DISABLE_Z]) {
                    ...
                    code +=
                        "			rotm[0] = "
                        "texelFetch(emission_texture_normal, emission_tex_ofs, "
                        "0).xy;\n";
                    code +=
                    ...
                    code +=
                        ...
                        "normal));\n";
                    code +=
                        "			VELOCITY = mat3(tangent, bitangent, "
                        "normal) * VELOCITY;\n";
                    code += "		}\n";
                }
            }
        ...
        case EMISSION_SHAPE_RING: {
            code +=
                "		float ring_spawn_angle = "
                "rand_from_seed(alt_restart_seed) * 2.0 * pi;\n";
            code +=
                "		float ring_random_radius = "
                "rand_from_seed(alt_restart_seed) * (ring_radius - "
            ...
            code +=
                "			vec3(c + axis.x * axis.x * oc, axis.x * axis.y * "
                "oc - axis.z * s, axis.x * axis.z *oc + axis.y * s),\n";
            code +=
                "			vec3(axis.x * axis.y * oc + s * axis.z, c + axis.y "
                "* axis.y * oc, axis.y * axis.z * oc - axis.x * s),\n";
            code +=
                "			vec3(axis.z * axis.x * oc - axis.y * s, axis.z * "
                "axis.y * oc + axis.x * s, c + axis.z * axis.z * oc)\n";
            code += "			) * ortho_axis;\n";
            code += "		ortho_axis = normalize(ortho_axis);\n";
            code +=
                "		TRANSFORM[3].xyz = ortho_axis * ring_random_radius + "
                "(rand_from_seed(alt_restart_seed) * ring_height - ring_height "
                "/ 2.0) * axis;\n";
        } break;
        case EMISSION_SHAPE_MAX: { // Max value for validity check.
            break;
    ...
    if (tex_parameters[PARAM_INITIAL_LINEAR_VELOCITY].is_valid()) {
        code +=
            "		float tex_linear_velocity = "
            "textureLod(linear_velocity_texture, vec2(tv, 0.0), 0.0).r;\n";
    } else {
        code += "		float tex_linear_velocity = 0.0;\n";
    }
    ...
    if (flags[FLAG_DISABLE_Z]) {
        if (tex_parameters[PARAM_ORBIT_VELOCITY].is_valid()) {
            code +=
                "		float tex_orbit_velocity = "
                "textureLod(orbit_velocity_texture, vec2(tv, 0.0), 0.0).r;\n";
        } else {
    ...
    if (tex_parameters[PARAM_ANGULAR_VELOCITY].is_valid()) {
        code +=
            "		float tex_angular_velocity = "
            "textureLod(angular_velocity_texture, vec2(tv, 0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_LINEAR_ACCEL].is_valid()) {
        code +=
            "		float tex_linear_accel = textureLod(linear_accel_texture, "
            "vec2(tv, 0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_RADIAL_ACCEL].is_valid()) {
        code +=
            "		float tex_radial_accel = textureLod(radial_accel_texture, "
            "vec2(tv, 0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_TANGENTIAL_ACCEL].is_valid()) {
        code +=
            "		float tex_tangent_accel = "
            "textureLod(tangent_accel_texture, vec2(tv, 0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_DAMPING].is_valid()) {
        code +=
            "		float tex_damping = textureLod(damping_texture, vec2(tv, "
            "0.0), 0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_ANGLE].is_valid()) {
        code +=
            "		float tex_angle = textureLod(angle_texture, vec2(tv, 0.0), "
            "0.0).r;\n";
    } else {
        code += "		float tex_angle = 0.0;\n";
    }
    ...
    if (tex_parameters[PARAM_ANIM_SPEED].is_valid()) {
        code +=
            "		float tex_anim_speed = textureLod(anim_speed_texture, "
            "vec2(tv, 0.0), 0.0).r;\n";
    } else {
    ...
    code +=
        "		force += length(VELOCITY) > 0.0 ? normalize(VELOCITY) * "
        "(linear_accel + tex_linear_accel) * mix(1.0, "
        "rand_from_seed(alt_seed), linear_accel_random) : vec3(0.0);\n";
    code += "		// apply radial acceleration\n";
    code += "		vec3 org = EMISSION_TRANSFORM[3].xyz;\n";
    code += "		vec3 diff = pos - org;\n";
    code +=
        "		force += length(diff) > 0.0 ? normalize(diff) * (radial_accel "
        "+ tex_radial_accel) * mix(1.0, rand_from_seed(alt_seed), "
        "radial_accel_random) : vec3(0.0);\n";
    code += "		// apply tangential acceleration;\n";
    if (flags[FLAG_DISABLE_Z]) {
        code +=
            "		force += length(diff.yx) > 0.0 ? vec3(normalize(diff.yx * "
            "vec2(-1.0, 1.0)), 0.0) * ((tangent_accel + tex_tangent_accel) * "
            "mix(1.0, rand_from_seed(alt_seed), tangent_accel_random)) : "
            "vec3(0.0);\n";

    } else {
        code +=
            "		vec3 crossDiff = cross(normalize(diff), "
            "normalize(gravity));\n";
        code +=
            "		force += length(crossDiff) > 0.0 ? normalize(crossDiff) * "
            "((tangent_accel + tex_tangent_accel) * mix(1.0, "
            "rand_from_seed(alt_seed), tangent_accel_random)) : vec3(0.0);\n";
    }
    ...
    if (flags[FLAG_DISABLE_Z]) {
        code +=
            "		float orbit_amount = (orbit_velocity + tex_orbit_velocity) "
            "* mix(1.0, rand_from_seed(alt_seed), orbit_velocity_random);\n";
        code += "		if (orbit_amount != 0.0) {\n";
        code += "		     float ang = orbit_amount * DELTA * pi * 2.0;\n";
        code +=
            "		     mat2 rot = mat2(vec2(cos(ang), -sin(ang)), "
    ...
    if (tex_parameters[PARAM_INITIAL_LINEAR_VELOCITY].is_valid()) {
        code +=
            "		VELOCITY = normalize(VELOCITY) * tex_linear_velocity;\n";
    }
    code += "		if (damping + tex_damping > 0.0) {\n";
    code += "			float v = length(VELOCITY);\n";
    code +=
        "			float damp = (damping + tex_damping) * mix(1.0, "
        "rand_from_seed(alt_seed), damping_random);\n";
    code += "			v -= damp * DELTA;\n";
    code += "			if (v < 0.0) {\n";
    ...
    code +=
        ...
        "angle_rand, initial_angle_random);\n";
    code +=
        "		base_angle += CUSTOM.y * LIFETIME * (angular_velocity + "
        "tex_angular_velocity) * mix(1.0, rand_from_seed(alt_seed) * 2.0 - "
        "1.0, angular_velocity_random);\n";
    code += "		CUSTOM.x = base_angle * degree_to_rad;\n"; // angle
    code +=
        "		CUSTOM.z = (anim_offset + tex_anim_offset) * mix(1.0, "
        "anim_offset_rand, anim_offset_random) + CUSTOM.y * (anim_speed + "
        "tex_anim_speed) * mix(1.0, rand_from_seed(alt_seed), "
        "anim_speed_random);\n"; // angle
    code += "	}\n";
    // apply color
    // apply hue rotation
    if (tex_parameters[PARAM_SCALE].is_valid()) {
        code +=
            "	float tex_scale = textureLod(scale_texture, vec2(tv, 0.0), "
            "0.0).r;\n";
    } else {
    ...
    if (tex_parameters[PARAM_HUE_VARIATION].is_valid()) {
        code +=
            "	float tex_hue_variation = textureLod(hue_variation_texture, "
            "vec2(tv, 0.0), 0.0).r;\n";
    } else {
    ...
    code +=
        "	float hue_rot_angle = (hue_variation + tex_hue_variation) * pi * "
        "2.0 * mix(1.0, hue_rot_rand * 2.0 - 1.0, hue_variation_random);\n";
    ...
    if (color_ramp.is_valid()) {
        code +=
            "	COLOR = hue_rot_mat * textureLod(color_ramp, vec2(tv, 0.0), "
            "0.0) * color_value;\n";
    } else {
    ...
    if (emission_color_texture.is_valid()
        && (emission_shape == EMISSION_SHAPE_POINTS
            || emission_shape == EMISSION_SHAPE_DIRECTED_POINTS)) {
        code +=
            "	COLOR *= texelFetch(emission_texture_color, emission_tex_ofs, "
            "0);\n";
    }
    if (trail_color_modifier.is_valid()) {
        code += "	if (trail_divisor > 1) {\n";
        code +=
            "		COLOR *= textureLod(trail_color_modifier, "
            "vec2(float(int(NUMBER) % trail_divisor) / float(trail_divisor - "
            "1), 0.0), 0.0);\n";
        code += "	}\n";
    }
    code += "\n";
    ...
    if (flags[FLAG_DISABLE_Z]) {
        if (flags[FLAG_ALIGN_Y_TO_VELOCITY]) {
            ...
            code += "	}\n";
            code +=
                "	TRANSFORM[0].xyz = normalize(cross(TRANSFORM[1].xyz, "
                "TRANSFORM[2].xyz));\n";
            code += "	TRANSFORM[2] = vec4(0.0, 0.0, 1.0, 0.0);\n";
        } else {
            code +=
                "	TRANSFORM[0] = vec4(cos(CUSTOM.x), -sin(CUSTOM.x), 0.0, "
                "0.0);\n";
            code +=
                "	TRANSFORM[1] = vec4(sin(CUSTOM.x), cos(CUSTOM.x), 0.0, "
                "0.0);\n";
            code += "	TRANSFORM[2] = vec4(0.0, 0.0, 1.0, 0.0);\n";
        }
        ...
        if (flags[FLAG_ALIGN_Y_TO_VELOCITY]) {
            ...
            code +=
                "		TRANSFORM[0].xyz = "
                "normalize(cross(normalize(TRANSFORM[1].xyz), "
                "normalize(TRANSFORM[2].xyz)));\n";
            code +=
                "		TRANSFORM[2].xyz = "
                "normalize(cross(normalize(TRANSFORM[0].xyz), "
        ...
        if (flags[FLAG_ROTATE_Y]) {
            code +=
                "	TRANSFORM = mat4(vec4(cos(CUSTOM.x), 0.0, -sin(CUSTOM.x), "
                "0.0), vec4(0.0, 1.0, 0.0, 0.0), vec4(sin(CUSTOM.x), 0.0, "
                "cos(CUSTOM.x), 0.0), TRANSFORM[3]);\n";
        }
    }
    // scale by scale
    code +=
        "	float base_scale = tex_scale * mix(scale, 1.0, scale_random * "
        "scale_rand);\n";
    ...
    if (trail_size_modifier.is_valid()) {
        code += "	if (trail_divisor > 1) {\n";
        code +=
            "		base_scale *= textureLod(trail_size_modifier, "
            "vec2(float(int(NUMBER) % trail_divisor) / float(trail_divisor - "
            "1), 0.0), 0.0).r;\n";
        code += "	}\n";
    }
...
void ParticlesMaterial::set_direction(Vector3 p_direction) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->direction,
        direction
    );
}
...
void ParticlesMaterial::set_spread(float p_spread) {
    ...
    VisualServer::get_singleton()
        ->material_set_param(_get_material(), shader_names->spread, p_spread);
}
...
void ParticlesMaterial::set_param(Parameter p_param, float p_value) {
    ...
    switch (p_param) {
        case PARAM_INITIAL_LINEAR_VELOCITY: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->initial_linear_velocity,
                p_value
            );
        } break;
...
void ParticlesMaterial::set_param_randomness(Parameter p_param, float p_value) {
    ...
    switch (p_param) {
        ...
        case PARAM_ANGULAR_VELOCITY: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->angular_velocity_random,
                p_value
            );
        } break;
        case PARAM_ORBIT_VELOCITY: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->orbit_velocity_random,
                p_value
            );
        } break;
        case PARAM_LINEAR_ACCEL: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->linear_accel_random,
                p_value
            );
        } break;
        case PARAM_RADIAL_ACCEL: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->radial_accel_random,
                p_value
            );
        } break;
        case PARAM_TANGENTIAL_ACCEL: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->tangent_accel_random,
                p_value
            );
        } break;
        case PARAM_DAMPING: {
        ...
        case PARAM_ANGLE: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->initial_angle_random,
                p_value
            );
        } break;
        case PARAM_SCALE: {
        ...
        case PARAM_HUE_VARIATION: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->hue_variation_random,
                p_value
            );
        } break;
        case PARAM_ANIM_SPEED: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->anim_speed_random,
                p_value
            );
        } break;
        case PARAM_ANIM_OFFSET: {
...
static void _adjust_curve_range(
...
void ParticlesMaterial::set_param_texture(
    Parameter p_param,
    const Ref<Texture>& p_texture
) {
    ERR_FAIL_INDEX(p_param, PARAM_MAX);
    ...
    switch (p_param) {
        ...
        case PARAM_ANGULAR_VELOCITY: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->angular_velocity_texture,
                p_texture
            );
        ...
        case PARAM_ORBIT_VELOCITY: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->orbit_velocity_texture,
                p_texture
            );
        ...
        case PARAM_LINEAR_ACCEL: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->linear_accel_texture,
                p_texture
            );
        ...
        case PARAM_RADIAL_ACCEL: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->radial_accel_texture,
                p_texture
            );
            _adjust_curve_range(p_texture, -200, 200);
        } break;
        case PARAM_TANGENTIAL_ACCEL: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->tangent_accel_texture,
                p_texture
            );
            _adjust_curve_range(p_texture, -200, 200);
        } break;
        ...
        case PARAM_HUE_VARIATION: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->hue_variation_texture,
                p_texture
            );
        ...
        case PARAM_ANIM_SPEED: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->anim_speed_texture,
                p_texture
            );
        ...
        case PARAM_ANIM_OFFSET: {
            VisualServer::get_singleton()->material_set_param(
                _get_material(),
                shader_names->anim_offset_texture,
                p_texture
            );
        } break;
        case PARAM_MAX:
...
void ParticlesMaterial::set_color(const Color& p_color) {
    VisualServer::get_singleton()
        ->material_set_param(_get_material(), shader_names->color, p_color);
    color = p_color;
}
...
void ParticlesMaterial::set_emission_sphere_radius(float p_radius) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_sphere_radius,
        p_radius
    );
}
...
void ParticlesMaterial::set_emission_box_extents(Vector3 p_extents) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_box_extents,
        p_extents
    );
}
...
void ParticlesMaterial::set_emission_point_texture(const Ref<Texture>& p_points
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_texture_points,
        p_points
    );
}

void ParticlesMaterial::set_emission_normal_texture(
    const Ref<Texture>& p_normals
) {
    emission_normal_texture = p_normals;
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_texture_normal,
        p_normals
    );
}
...
void ParticlesMaterial::set_emission_color_texture(const Ref<Texture>& p_colors
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_texture_color,
        p_colors
    );
    _queue_shader_change();
}
...
void ParticlesMaterial::set_emission_point_count(int p_count) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_texture_point_count,
        p_count
    );
}
...
void ParticlesMaterial::set_emission_ring_height(float p_height) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_ring_height,
        p_height
    );
}
...
void ParticlesMaterial::set_emission_ring_radius(float p_radius) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_ring_radius,
        p_radius
    );
}
...
void ParticlesMaterial::set_emission_ring_inner_radius(float p_offset) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_ring_inner_radius,
        p_offset
    );
}
...
void ParticlesMaterial::set_emission_ring_axis(Vector3 p_axis) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->emission_ring_axis,
        p_axis
    );
}
...
void ParticlesMaterial::set_trail_size_modifier(
    const Ref<CurveTexture>& p_trail_size_modifier
) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->trail_size_modifier,
        curve
    );
    _queue_shader_change();
}
...
void ParticlesMaterial::set_trail_color_modifier(
    const Ref<GradientTexture>& p_trail_color_modifier
) {
    trail_color_modifier = p_trail_color_modifier;
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->trail_color_modifier,
        p_trail_color_modifier
    );
    _queue_shader_change();
}
...
void ParticlesMaterial::set_gravity(const Vector3& p_gravity) {
    ...
    VisualServer::get_singleton()
        ->material_set_param(_get_material(), shader_names->gravity, gset);
}
...
void ParticlesMaterial::set_lifetime_randomness(float p_lifetime) {
    ...
    VisualServer::get_singleton()->material_set_param(
        _get_material(),
        shader_names->lifetime_randomness,
        lifetime_randomness
    );
}
...
void ParticlesMaterial::_validate_property(PropertyInfo& property) const {
    if (property.name == "emission_sphere_radius"
        && emission_shape != EMISSION_SHAPE_SPHERE) {
        property.usage = 0;
    }

    if (property.name == "emission_box_extents"
        && emission_shape != EMISSION_SHAPE_BOX) {
        property.usage = 0;
    }

    if ((property.name == "emission_point_texture"
         || property.name == "emission_color_texture")
        && (emission_shape != EMISSION_SHAPE_POINTS)
        && emission_shape != EMISSION_SHAPE_DIRECTED_POINTS) {
        property.usage = 0;
    }

    if (property.name == "emission_normal_texture"
        && emission_shape != EMISSION_SHAPE_DIRECTED_POINTS) {
        property.usage = 0;
    }

    if (property.name == "emission_point_count"
        && (emission_shape != EMISSION_SHAPE_POINTS
            && emission_shape != EMISSION_SHAPE_DIRECTED_POINTS)) {
        property.usage = 0;
    }

    if (property.name.begins_with("emission_ring_")
...
void ParticlesMaterial::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_direction", "degrees"),
        &ParticlesMaterial::set_direction
    );
    ClassDB::bind_method(
        D_METHOD("get_direction"),
        &ParticlesMaterial::get_direction
    );

    ClassDB::bind_method(
        D_METHOD("set_spread", "degrees"),
        &ParticlesMaterial::set_spread
    );
    ClassDB::bind_method(
        D_METHOD("get_spread"),
        &ParticlesMaterial::get_spread
    );

    ClassDB::bind_method(
        D_METHOD("set_flatness", "amount"),
        &ParticlesMaterial::set_flatness
    );
    ClassDB::bind_method(
        D_METHOD("get_flatness"),
        &ParticlesMaterial::get_flatness
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_param_randomness", "param", "randomness"),
        &ParticlesMaterial::set_param_randomness
    );
    ClassDB::bind_method(
        D_METHOD("get_param_randomness", "param"),
        &ParticlesMaterial::get_param_randomness
    );

    ClassDB::bind_method(
        D_METHOD("set_param_texture", "param", "texture"),
        &ParticlesMaterial::set_param_texture
    );
    ClassDB::bind_method(
        D_METHOD("get_param_texture", "param"),
        &ParticlesMaterial::get_param_texture
    );

    ClassDB::bind_method(
        D_METHOD("set_color", "color"),
        &ParticlesMaterial::set_color
    );
    ClassDB::bind_method(D_METHOD("get_color"), &ParticlesMaterial::get_color);

    ClassDB::bind_method(
        D_METHOD("set_color_ramp", "ramp"),
        &ParticlesMaterial::set_color_ramp
    );
    ClassDB::bind_method(
        D_METHOD("get_color_ramp"),
        &ParticlesMaterial::get_color_ramp
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_emission_shape", "shape"),
        &ParticlesMaterial::set_emission_shape
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_shape"),
        &ParticlesMaterial::get_emission_shape
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_sphere_radius", "radius"),
        &ParticlesMaterial::set_emission_sphere_radius
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_sphere_radius"),
        &ParticlesMaterial::get_emission_sphere_radius
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_box_extents", "extents"),
        &ParticlesMaterial::set_emission_box_extents
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_box_extents"),
        &ParticlesMaterial::get_emission_box_extents
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_point_texture", "texture"),
        &ParticlesMaterial::set_emission_point_texture
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_point_texture"),
        &ParticlesMaterial::get_emission_point_texture
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_normal_texture", "texture"),
        &ParticlesMaterial::set_emission_normal_texture
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_normal_texture"),
        &ParticlesMaterial::get_emission_normal_texture
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_color_texture", "texture"),
        &ParticlesMaterial::set_emission_color_texture
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_color_texture"),
        &ParticlesMaterial::get_emission_color_texture
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_point_count", "point_count"),
        &ParticlesMaterial::set_emission_point_count
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_point_count"),
        &ParticlesMaterial::get_emission_point_count
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_radius", "radius"),
        &ParticlesMaterial::set_emission_ring_radius
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_radius"),
        &ParticlesMaterial::get_emission_ring_radius
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_inner_radius", "offset"),
        &ParticlesMaterial::set_emission_ring_inner_radius
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_inner_radius"),
        &ParticlesMaterial::get_emission_ring_inner_radius
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_height", "height"),
        &ParticlesMaterial::set_emission_ring_height
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_height"),
        &ParticlesMaterial::get_emission_ring_height
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_axis", "axis"),
        &ParticlesMaterial::set_emission_ring_axis
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_axis"),
        &ParticlesMaterial::get_emission_ring_axis
    );

    ClassDB::bind_method(
        D_METHOD("set_trail_divisor", "divisor"),
        &ParticlesMaterial::set_trail_divisor
    );
    ClassDB::bind_method(
        D_METHOD("get_trail_divisor"),
        &ParticlesMaterial::get_trail_divisor
    );

    ClassDB::bind_method(
        D_METHOD("set_trail_size_modifier", "texture"),
        &ParticlesMaterial::set_trail_size_modifier
    );
    ClassDB::bind_method(
        D_METHOD("get_trail_size_modifier"),
        &ParticlesMaterial::get_trail_size_modifier
    );

    ClassDB::bind_method(
        D_METHOD("set_trail_color_modifier", "texture"),
        &ParticlesMaterial::set_trail_color_modifier
    );
    ClassDB::bind_method(
        D_METHOD("get_trail_color_modifier"),
        &ParticlesMaterial::get_trail_color_modifier
    );

    ClassDB::bind_method(
        D_METHOD("get_gravity"),
        &ParticlesMaterial::get_gravity
    );
    ClassDB::bind_method(
        D_METHOD("set_gravity", "accel_vec"),
        &ParticlesMaterial::set_gravity
    );

    ClassDB::bind_method(
        D_METHOD("set_lifetime_randomness", "randomness"),
        &ParticlesMaterial::set_lifetime_randomness
    );
    ClassDB::bind_method(
        D_METHOD("get_lifetime_randomness"),
        &ParticlesMaterial::get_lifetime_randomness
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            PROPERTY_HINT_RANGE,
            "0.01,128,0.01,or_greater"
        ),
        "set_emission_sphere_radius",
        "get_emission_sphere_radius"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "emission_box_extents"),
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            PROPERTY_HINT_RANGE,
            "0.0,100,0.01,or_greater"
        ),
        "set_emission_ring_height",
        "get_emission_ring_height"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "emission_ring_axis"),
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flag_align_y"),
        "set_flag",
        "get_flag",
        FLAG_ALIGN_Y_TO_VELOCITY
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flag_rotate_y"),
        "set_flag",
        "get_flag",
        FLAG_ROTATE_Y
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flag_disable_z"),
        "set_flag",
        "get_flag",
        FLAG_DISABLE_Z
    );
    ...
    ADD_GROUP("Gravity", "");
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "gravity"),
        "set_gravity",
        "get_gravity"
    );
    ...
    ADD_PROPERTYI(
        PropertyInfo(
            ...
            "initial_velocity",
            PROPERTY_HINT_RANGE,
            "0,1000,0.01,or_lesser,or_greater"
        ),
        "set_param",
        "get_param",
    ...
    ADD_PROPERTYI(
        PropertyInfo(
            ...
            "angle",
            PROPERTY_HINT_RANGE,
            "-720,720,0.1,or_lesser,or_greater"
        ),
        "set_param",
        "get_param",
        PARAM_ANGLE
