...
private:
    ...
    _FORCE_INLINE_ Animation::TransformKey _interpolate(
    ...
    _interpolate(const Vector3& p_a, const Vector3& p_b, float p_c) const;
    _FORCE_INLINE_ Quat
    _interpolate(const Quat& p_a, const Quat& p_b, float p_c) const;
    _FORCE_INLINE_ Variant
    _interpolate(const Variant& p_a, const Variant& p_b, float p_c) const;
    _FORCE_INLINE_ float _interpolate(
        const float& p_a,
        const float& p_b,
        float p_c
    ) const;

    _FORCE_INLINE_ Animation::TransformKey _cubic_interpolate(
    ...
    _FORCE_INLINE_ Vector3 _cubic_interpolate(
    ...
    _FORCE_INLINE_ Quat _cubic_interpolate(
        const Quat& p_pre_a,
        const Quat& p_a,
        const Quat& p_b,
        const Quat& p_post_b,
    ...
    _FORCE_INLINE_ Variant _cubic_interpolate(
        const Variant& p_pre_a,
        const Variant& p_a,
        const Variant& p_b,
        const Variant& p_post_b,
        float p_c
    ) const;
    _FORCE_INLINE_ float _cubic_interpolate(
        const float& p_pre_a,
        const float& p_a,
        const float& p_b,
    ...
    template <class T>
    _FORCE_INLINE_ void _track_get_key_indices_in_range(
        const Vector<T>& p_array,
        float from_time,
        float to_time,
        List<int>* p_indices
    ) const;

    _FORCE_INLINE_ void _value_track_get_key_indices_in_range(
        const ValueTrack* vt,
        float from_time,
        float to_time,
        List<int>* p_indices
    ) const;
    _FORCE_INLINE_ void _method_track_get_key_indices_in_range(
...
private:
    ...
    PoolVector<int> _value_track_get_key_indices(
        int p_track,
        float p_time,
        float p_delta
    ) const {
    ...
    PoolVector<int> _method_track_get_key_indices(
        int p_track,
        float p_time,
        float p_delta
    ) const {
    ...
    bool _transform_track_optimize_key(
...
public:
    ...
    void track_insert_key(
        int p_track,
        float p_time,
        const Variant& p_key,
        float p_transition = 1
    );
    void track_set_key_transition(
        int p_track,
        int p_key_idx,
        float p_transition
    );
    void track_set_key_value(
        int p_track,
        int p_key_idx,
    ...
    int transform_track_insert_key(
        int p_track,
        float p_time,
        const Vector3& p_loc,
    ...
    int bezier_track_insert_key(
        int p_track,
        float p_time,
        float p_value,
        const Vector2& p_in_handle,
        const Vector2& p_out_handle
    );
    void bezier_track_set_key_value(int p_track, int p_index, float p_value);
    void bezier_track_set_key_in_handle(
        int p_track,
        int p_index,
        const Vector2& p_handle
    );
    void bezier_track_set_key_out_handle(
        int p_track,
        int p_index,
        const Vector2& p_handle
    );
    ...
    void audio_track_set_key_start_offset(
        int p_track,
        int p_key,
        float p_offset
    );
    ...
    int animation_track_insert_key(
        int p_track,
        float p_time,
    ...
    Error transform_track_interpolate(
        int p_track,
        float p_time,
        Vector3* r_loc,
        Quat* r_rot,
    ...
    void value_track_get_key_indices(
        int p_track,
        float p_time,
        float p_delta,
        List<int>* p_indices
    ...
    void method_track_get_key_indices(
        int p_track,
        float p_time,
        float p_delta,
        List<int>* p_indices
    ...
    void track_get_key_indices_in_range(
        int p_track,
        float p_time,
        float p_delta,
        List<int>* p_indices
