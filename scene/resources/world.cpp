...
struct SpatialIndexer {
    ...
    void _notifier_update(VisibilityNotifier* p_notifier, const AABB& p_rect) {
        Map<VisibilityNotifier*, NotifierData>::Element* E =
    ...
    void _remove_camera(Camera* p_camera) {
        ...
        for (Map<VisibilityNotifier*, uint64_t>::Element* E =
                 cameras[p_camera].notifiers.front();
             E;
             E = E->next()) {
    ...
    void _update(uint64_t p_frame) {
        ...
        for (Map<Camera*, CameraData>::Element* E = cameras.front(); E;
            ...
            for (int i = 0; i < culled; i++) {
                ...
                Map<VisibilityNotifier*, uint64_t>::Element* H =
                    E->get().notifiers.find(ptr[i]);
                if (!H) {
            ...
            for (Map<VisibilityNotifier*, uint64_t>::Element* F =
...
void World::_register_notifier(
    VisibilityNotifier* p_notifier,
    const AABB& p_rect
) {
#ifndef _3D_DISABLED
...
void World::set_environment(const Ref<Environment>& p_environment) {
    ...
    if (environment.is_valid()) {
        VS::get_singleton()->scenario_set_environment(
            scenario,
            environment->get_rid()
        );
    } else {
...
void World::set_fallback_environment(const Ref<Environment>& p_environment) {
    ...
    if (fallback_environment.is_valid()) {
        VS::get_singleton()->scenario_set_fallback_environment(
            scenario,
            p_environment->get_rid()
        );
    } else {
...
void World::get_camera_list(List<Camera*>* r_cameras) {
    for (Map<Camera*, SpatialIndexer::CameraData>::Element* E =
             indexer->cameras.front();
         E;
         E = E->next()) {
...
void World::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_fallback_environment", "env"),
        &World::set_fallback_environment
    );
    ClassDB::bind_method(
        D_METHOD("get_fallback_environment"),
        &World::get_fallback_environment
    );
    ClassDB::bind_method(
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::_RID, "scenario", PROPERTY_HINT_NONE, "", 0),
        "",
        "get_scenario"
    );
    ADD_PROPERTY(
...
World::World() {
    ...
    PhysicsServer::get_singleton()->area_set_param(
        space,
        PhysicsServer::AREA_PARAM_GRAVITY,
        GLOBAL_DEF("physics/3d/default_gravity", 9.8)
    );
    PhysicsServer::get_singleton()->area_set_param(
        space,
        PhysicsServer::AREA_PARAM_GRAVITY_VECTOR,
        GLOBAL_DEF("physics/3d/default_gravity_vector", Vector3(0, -1, 0))
    );
    PhysicsServer::get_singleton()->area_set_param(
        space,
