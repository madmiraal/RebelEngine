...
uint32_t SurfaceTool::VertexHasher::hash(const Vertex& p_vtx) {
    uint32_t h =
        hash_djb2_buffer((const uint8_t*)&p_vtx.vertex, sizeof(real_t) * 3);
    h = hash_djb2_buffer((const uint8_t*)&p_vtx.normal, sizeof(real_t) * 3, h);
    h = hash_djb2_buffer(
        (const uint8_t*)&p_vtx.binormal,
        sizeof(real_t) * 3,
        h
    );
    ...
    h = hash_djb2_buffer(
        (const uint8_t*)p_vtx.bones.ptr(),
        p_vtx.bones.size() * sizeof(int),
        h
    );
    h = hash_djb2_buffer(
        (const uint8_t*)p_vtx.weights.ptr(),
        p_vtx.weights.size() * sizeof(float),
        h
    );
    return h;
}
...
void SurfaceTool::add_vertex(const Vector3& p_vertex) {
    ...
    vtx.binormal =
        last_normal.cross(last_tangent.normal).normalized() * last_tangent.d;

    const int expected_vertices = 4;

    if ((format & Mesh::ARRAY_FORMAT_WEIGHTS
         || format & Mesh::ARRAY_FORMAT_BONES)
        && (vtx.weights.size() != expected_vertices
            || vtx.bones.size() != expected_vertices)) {
...
void SurfaceTool::add_triangle_fan(
    const Vector<Vector3>& p_vertices,
    const Vector<Vector2>& p_uvs,
    const Vector<Color>& p_colors,
    const Vector<Vector2>& p_uv2s,
    const Vector<Vector3>& p_normals,
    const Vector<Plane>& p_tangents
) {
    ERR_FAIL_COND(!begun);
...
#define ADD_POINT(n)                                                           \
    {                                                                          \
        if (p_colors.size() > n) add_color(p_colors[n]);                       \
        if (p_uvs.size() > n) add_uv(p_uvs[n]);                                \
        if (p_uv2s.size() > n) add_uv2(p_uv2s[n]);                             \
        if (p_normals.size() > n) add_normal(p_normals[n]);                    \
        if (p_tangents.size() > n) add_tangent(p_tangents[n]);                 \
...
Ref<ArrayMesh> SurfaceTool::commit(
    const Ref<ArrayMesh>& p_existing,
    uint32_t p_flags
) {
    Ref<ArrayMesh> mesh;
...
void SurfaceTool::_create_list(
    ...
    ERR_FAIL_COND_MSG(
        p_existing.is_null(),
        "First argument in SurfaceTool::_create_list() must be a valid object "
        "of type Mesh"
    );
...
Vector<SurfaceTool::Vertex> SurfaceTool::
    create_vertex_array_from_triangle_arrays(const Array& p_arrays) {
...
void SurfaceTool::_create_list_from_arrays(
    Array arr,
    List<Vertex>* r_vertex,
    List<int>* r_index,
    int& lformat
...
void SurfaceTool::create_from(const Ref<Mesh>& p_existing, int p_surface) {
    ERR_FAIL_COND_MSG(
        p_existing.is_null(),
        "First argument in SurfaceTool::create_from() must be a valid object "
        "of type Mesh"
    );
    clear();
...
void SurfaceTool::create_from_blend_shape(
    const Ref<Mesh>& p_existing,
    int p_surface,
    ...
    ERR_FAIL_COND_MSG(
        p_existing.is_null(),
        "First argument in SurfaceTool::create_from_blend_shape() must be a "
        "valid object of type Mesh"
    );
    clear();
    primitive = p_existing->surface_get_primitive_type(p_surface);
...
void SurfaceTool::append_from(
    ...
    ERR_FAIL_COND_MSG(
        p_existing.is_null(),
        "First argument in SurfaceTool::append_from() must be a valid object "
        "of type Mesh"
    );
...
int SurfaceTool::mikktGetNumFaces(const SMikkTSpaceContext* pContext) {
    TangentGenerationContextUserData& triangle_data =
        *reinterpret_cast<TangentGenerationContextUserData*>(
...
int SurfaceTool::mikktGetNumVerticesOfFace(
...
void SurfaceTool::mikktGetNormal(
    const SMikkTSpaceContext* pContext,
    float fvNormOut[],
    const int iFace,
    const int iVert
...
void SurfaceTool::mikktSetTSpaceDefault(
    const SMikkTSpaceContext* pContext,
    ...
    if (vtx != nullptr) {
        ...
        // These are reversed, in the Rebel Engine coordinate system.
        vtx->binormal =
...
void SurfaceTool::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("add_vertex", "vertex"),
        &SurfaceTool::add_vertex
    );
    ClassDB::bind_method(
        D_METHOD("add_color", "color"),
        &SurfaceTool::add_color
    );
    ClassDB::bind_method(
        D_METHOD("add_normal", "normal"),
        &SurfaceTool::add_normal
    );
    ClassDB::bind_method(
        D_METHOD("add_tangent", "tangent"),
    ...
    ClassDB::bind_method(
        D_METHOD("add_bones", "bones"),
        &SurfaceTool::add_bones
    );
    ClassDB::bind_method(
        D_METHOD("add_weights", "weights"),
        &SurfaceTool::add_weights
    );
    ClassDB::bind_method(
        D_METHOD("add_smooth_group", "smooth"),
    ...
    ClassDB::bind_method(
        ...
        DEFVAL(Vector<Vector2>()),
        DEFVAL(Vector<Color>()),
        DEFVAL(Vector<Vector2>()),
        DEFVAL(Vector<Vector3>()),
        DEFVAL(Vector<Plane>())
    );

    ClassDB::bind_method(
        D_METHOD("add_index", "index"),
    ...
    ClassDB::bind_method(
        D_METHOD("generate_normals", "flip"),
        &SurfaceTool::generate_normals,
        DEFVAL(false)
    );
    ClassDB::bind_method(
        D_METHOD("generate_tangents"),
    ...
    ClassDB::bind_method(
        D_METHOD("create_from", "existing", "surface"),
    ...
    ClassDB::bind_method(
        D_METHOD("append_from", "existing", "surface", "transform"),
        &SurfaceTool::append_from
    );
    ClassDB::bind_method(
        D_METHOD("commit", "existing", "flags"),
        &SurfaceTool::commit,
        DEFVAL(Variant()),
        DEFVAL(Mesh::ARRAY_COMPRESS_DEFAULT)
    );
    ClassDB::bind_method(
        D_METHOD("commit_to_arrays"),
