...
void Material::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_next_pass", "next_pass"),
        &Material::set_next_pass
    );
    ClassDB::bind_method(D_METHOD("get_next_pass"), &Material::get_next_pass);

    ClassDB::bind_method(
        D_METHOD("set_render_priority", "priority"),
        &Material::set_render_priority
    );
    ClassDB::bind_method(
        D_METHOD("get_render_priority"),
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "render_priority",
            PROPERTY_HINT_RANGE,
            itos(RENDER_PRIORITY_MIN) + "," + itos(RENDER_PRIORITY_MAX) + ",1"
        ),
        "set_render_priority",
        "get_render_priority"
    );
    ...
    if (shader.is_valid()) {
        ...
        if (pr) {
            VisualServer::get_singleton()
                ->material_set_param(_get_material(), pr, p_value);
            return true;
        }
    }
...
bool ShaderMaterial::_get(const StringName& p_name, Variant& r_ret) const {
    if (shader.is_valid()) {
        ...
        if (pr) {
            r_ret = VisualServer::get_singleton()->material_get_param(
                _get_material(),
                pr
            );
            return true;
    ...
    if (shader.is_valid()) {
        ...
        if (pr) {
            Variant default_value =
                VisualServer::get_singleton()->material_get_param_default(
                    _get_material(),
                    pr
                );
            Variant current_value;
            _get(p_name, current_value);
            return default_value.get_type() != Variant::NIL
...
Variant ShaderMaterial::property_get_revert(const String& p_name) {
    ...
    if (shader.is_valid()) {
        StringName pr = shader->remap_param(p_name);
        if (pr) {
            r_ret = VisualServer::get_singleton()->material_get_param_default(
                _get_material(),
                pr
            );
        }
...
void ShaderMaterial::set_shader(const Ref<Shader>& p_shader) {
    ...
    // This can be a slow operation, and `_change_notify()` (which is called by
    // `_shader_changed()`) does nothing in non-editor builds anyway. See
    // GH-34741 for details.
...
void ShaderMaterial::set_shader_param(
...
void ShaderMaterial::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_shader", "shader"),
        &ShaderMaterial::set_shader
    );
    ClassDB::bind_method(D_METHOD("get_shader"), &ShaderMaterial::get_shader);
    ClassDB::bind_method(
        D_METHOD("set_shader_param", "param", "value"),
        &ShaderMaterial::set_shader_param
    );
    ClassDB::bind_method(
        D_METHOD("get_shader_param", "param"),
        &ShaderMaterial::get_shader_param
    );
    ClassDB::bind_method(
        D_METHOD("_shader_changed"),
        &ShaderMaterial::_shader_changed
    );
    ClassDB::bind_method(
...
Map<SpatialMaterial::MaterialKey, SpatialMaterial::ShaderData>
    SpatialMaterial::shader_map;
...
Ref<SpatialMaterial>
    SpatialMaterial::materials_for_2d[SpatialMaterial::MAX_MATERIALS_FOR_2D];
...
void SpatialMaterial::_update_shader() {
    ...
    if (shader_map.has(mk)) {
        VS::get_singleton()->material_set_shader(
            _get_material(),
            shader_map[mk].shader
        );
        shader_map[mk].users++;
        return;
    }
    ...
    if (flags[FLAG_SRGB_VERTEX_COLOR]) {
        ...
        code +=
            "\t\tCOLOR.rgb = mix(pow((COLOR.rgb + vec3(0.055)) * (1.0 / (1.0 + "
            "0.055)), vec3(2.4)), COLOR.rgb * (1.0 / 12.92), "
    ...
    switch (billboard_mode) {
        ...
        case BILLBOARD_ENABLED: {
            code +=
                ...
                "mat4(CAMERA_MATRIX[0],CAMERA_MATRIX[1],CAMERA_MATRIX[2],WORLD_"
                "MATRIX[3]);\n";

            if (flags[FLAG_BILLBOARD_KEEP_SCALE]) {
                code +=
                    "\tMODELVIEW_MATRIX = MODELVIEW_MATRIX * "
                    "mat4(vec4(length(WORLD_MATRIX[0].xyz), 0.0, 0.0, "
                    "0.0),vec4(0.0, length(WORLD_MATRIX[1].xyz), 0.0, "
                    "0.0),vec4(0.0, 0.0, length(WORLD_MATRIX[2].xyz), "
        ...
        case BILLBOARD_FIXED_Y: {
            code +=
                "\tMODELVIEW_MATRIX = INV_CAMERA_MATRIX * "
                "mat4(vec4(normalize(cross(vec3(0.0, 1.0, 0.0), "
                "CAMERA_MATRIX[2].xyz)),0.0),vec4(0.0, 1.0, 0.0, "
                "0.0),vec4(normalize(cross(CAMERA_MATRIX[0].xyz, vec3(0.0, "
                "1.0, 0.0))),0.0),WORLD_MATRIX[3]);\n";
        ...
        case BILLBOARD_PARTICLES: {
            ...
            code +=
                "\tmat4 mat_world = "
                "mat4(normalize(CAMERA_MATRIX[0])*length(WORLD_MATRIX[0]),"
                "normalize(CAMERA_MATRIX[1])*length(WORLD_MATRIX[0]),normalize("
                "CAMERA_MATRIX[2])*length(WORLD_MATRIX[2]),WORLD_MATRIX[3]);\n";
            // rotate by rotation
            code +=
                "\tmat_world = mat_world * mat4( "
                "vec4(cos(INSTANCE_CUSTOM.x),-sin(INSTANCE_CUSTOM.x), 0.0, "
                "0.0), vec4(sin(INSTANCE_CUSTOM.x), cos(INSTANCE_CUSTOM.x), "
                "0.0, 0.0),vec4(0.0, 0.0, 1.0, 0.0),vec4(0.0, 0.0, 0.0, "
                "1.0));\n";
            // set modelview
            ...
            code +=
                "\tfloat particle_frame = floor(INSTANCE_CUSTOM.z * "
                "float(particle_total_frames));\n";
            code += "\tif (!particles_anim_loop) {\n";
            code +=
                "\t\tparticle_frame = clamp(particle_frame, 0.0, "
                "particle_total_frames - 1.0);\n";
            code += "\t} else {\n";
            code +=
                "\t\tparticle_frame = mod(particle_frame, "
                "particle_total_frames);\n";
            code += "\t}";
    ...
    if (flags[FLAG_UV1_USE_TRIPLANAR] || flags[FLAG_UV2_USE_TRIPLANAR]) {
        code +=
            "vec4 triplanar_texture(sampler2D p_sampler,vec3 p_weights,vec3 "
            "p_triplanar_pos) {\n";
        ...
        code +=
            "\tsamp+= texture(p_sampler,p_triplanar_pos.zy * vec2(-1.0,1.0)) * "
            "p_weights.x;\n";
        code += "\treturn samp;\n";
        code += "}\n";
    }
    ...
    if (!VisualServer::get_singleton()->is_low_end()
        && features[FEATURE_DEPTH_MAPPING]
        && !flags[FLAG_UV1_USE_TRIPLANAR]) { // depthmap not supported with
                                             // triplanar
        code += "\t{\n";
        code +=
            "\t\tvec3 view_dir = "
            "normalize(normalize(-VERTEX)*mat3(TANGENT*depth_flip.x,-BINORMAL*"
            "depth_flip.y,NORMAL));\n"; // binormal is negative due to
                                        // mikktspace, flip 'unflips' it ;-)

        if (deep_parallax) {
            code +=
                "\t\tfloat num_layers = "
                "mix(float(depth_max_layers),float(depth_min_layers), "
            ...
            code +=
                "\t\tfloat before_depth = textureLod(texture_depth, prev_ofs, "
                "0.0).r - current_depth + layer_depth;\n";
            code +=
                "\t\tfloat weight = after_depth / (after_depth - "
                "before_depth);\n";
            ...
            code +=
                "\t\tvec2 ofs = base_uv - view_dir.xy / view_dir.z * (depth * "
                "depth_scale);\n";
        }
    ...
    if (flags[FLAG_USE_POINT_SIZE]) {
        ...
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tvec4 albedo_tex = "
                "triplanar_texture(texture_albedo,uv1_power_normal,uv1_"
                "triplanar_pos);\n";
        } else {
    ...
    if (flags[FLAG_ALBEDO_TEXTURE_FORCE_SRGB]) {
        code +=
            "\talbedo_tex.rgb = mix(pow((albedo_tex.rgb + vec3(0.055)) * (1.0 "
            "/ (1.0 + 0.055)),vec3(2.4)),albedo_tex.rgb.rgb * (1.0 / "
            "12.92),lessThan(albedo_tex.rgb,vec3(0.04045)));\n";
    }
    ...
    if (textures[TEXTURE_METALLIC] != nullptr) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tfloat metallic_tex = "
                "dot(triplanar_texture(texture_metallic,uv1_power_normal,uv1_"
                "triplanar_pos),metallic_texture_channel);\n";
        } else {
            code +=
                "\tfloat metallic_tex = "
                "dot(texture(texture_metallic,base_uv),metallic_texture_"
                "channel);\n";
        }
    ...
    if (textures[TEXTURE_ROUGHNESS] != nullptr) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tfloat roughness_tex = "
                "dot(triplanar_texture(texture_roughness,uv1_power_normal,uv1_"
                "triplanar_pos),roughness_texture_channel);\n";
        } else {
            code +=
                "\tfloat roughness_tex = "
                "dot(texture(texture_roughness,base_uv),roughness_texture_"
                "channel);\n";
        }
    ...
    if (features[FEATURE_NORMAL_MAPPING]) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tNORMALMAP = "
                "triplanar_texture(texture_normal,uv1_power_normal,uv1_"
                "triplanar_pos).rgb;\n";
        } else {
    ...
    if (features[FEATURE_EMISSION]) {
        if (flags[FLAG_EMISSION_ON_UV2]) {
            if (flags[FLAG_UV2_USE_TRIPLANAR]) {
                code +=
                    "\tvec3 emission_tex = "
                    "triplanar_texture(texture_emission,uv2_power_normal,uv2_"
                    "triplanar_pos).rgb;\n";
            } else {
                code +=
                    "\tvec3 emission_tex = "
                    "texture(texture_emission,base_uv2).rgb;\n";
            }
        } else {
            if (flags[FLAG_UV1_USE_TRIPLANAR]) {
                code +=
                    "\tvec3 emission_tex = "
                    "triplanar_texture(texture_emission,uv1_power_normal,uv1_"
                    "triplanar_pos).rgb;\n";
            } else {
                code +=
                    "\tvec3 emission_tex = "
    ...
    if (features[FEATURE_REFRACTION]) {
        if (features[FEATURE_NORMAL_MAPPING]) {
            ...
            code +=
                "\tunpacked_normal.z = sqrt(max(0.0, 1.0 - "
                "dot(unpacked_normal.xy, unpacked_normal.xy)));\n";
            code +=
                "\tvec3 ref_normal = normalize( mix(NORMAL,TANGENT * "
                "unpacked_normal.x + BINORMAL * unpacked_normal.y + NORMAL * "
                "unpacked_normal.z,NORMALMAP_DEPTH) );\n";
        } else {
        ...
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                ...
                "dot(triplanar_texture(texture_refraction,uv1_power_normal,uv1_"
                "triplanar_pos),refraction_texture_channel) * refraction;\n";
        } else {
            code +=
                "\tvec2 ref_ofs = SCREEN_UV - ref_normal.xy * "
                "dot(texture(texture_refraction,base_uv),refraction_texture_"
                "channel) * refraction;\n";
        }
        code += "\tfloat ref_amount = 1.0 - albedo.a * albedo_tex.a;\n";
        code +=
            "\tEMISSION += textureLod(SCREEN_TEXTURE,ref_ofs,ROUGHNESS * "
            "8.0).rgb * ref_amount;\n";
    ...
    if (proximity_fade_enabled) {
        ...
        code +=
            ...
            "vec4(SCREEN_UV*2.0-1.0,depth_tex*2.0-1.0,1.0);\n";
        code += "\tworld_pos.xyz/=world_pos.w;\n";
        code +=
            "\tALPHA*=clamp(1.0-smoothstep(world_pos.z+proximity_fade_distance,"
    ...
    if (distance_fade != DISTANCE_FADE_DISABLED) {
        if ((distance_fade == DISTANCE_FADE_OBJECT_DITHER
            ...
            if (!VisualServer::get_singleton()->is_low_end()) {
                ...
                if (distance_fade == DISTANCE_FADE_OBJECT_DITHER) {
                    code +=
                        "\t\tfloat fade_distance = abs((INV_CAMERA_MATRIX * "
                        "WORLD_MATRIX[3]).z);\n";
                ...
                code +=
                    "\t\tfloat "
                    "fade=clamp(smoothstep(distance_fade_min,distance_fade_max,"
                    "fade_distance),0.0,1.0);\n";
            ...
            code +=
                "\tALPHA*=clamp(smoothstep(distance_fade_min,distance_fade_max,"
                "-VERTEX.z),0.0,1.0);\n";
        }
    }
    ...
    if (features[FEATURE_RIM]) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tvec2 rim_tex = "
                "triplanar_texture(texture_rim,uv1_power_normal,uv1_triplanar_"
                "pos).xy;\n";
        } else {
    ...
    if (features[FEATURE_CLEARCOAT]) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tvec2 clearcoat_tex = "
                "triplanar_texture(texture_clearcoat,uv1_power_normal,uv1_"
                "triplanar_pos).xy;\n";
        } else {
            code +=
                "\tvec2 clearcoat_tex = "
                "texture(texture_clearcoat,base_uv).xy;\n";
        }
    ...
    if (features[FEATURE_ANISOTROPY]) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tvec3 anisotropy_tex = "
                "triplanar_texture(texture_flowmap,uv1_power_normal,uv1_"
                "triplanar_pos).rga;\n";
        } else {
            code +=
                "\tvec3 anisotropy_tex = "
                "texture(texture_flowmap,base_uv).rga;\n";
        }
    ...
    if (features[FEATURE_AMBIENT_OCCLUSION]) {
        if (flags[FLAG_AO_ON_UV2]) {
            if (flags[FLAG_UV2_USE_TRIPLANAR]) {
                code +=
                    "\tAO = "
                    "dot(triplanar_texture(texture_ambient_occlusion,uv2_power_"
                    "normal,uv2_triplanar_pos),ao_texture_channel);\n";
            } else {
                code +=
                    "\tAO = "
                    "dot(texture(texture_ambient_occlusion,base_uv2),ao_"
                    "texture_channel);\n";
            }
        } else {
            if (flags[FLAG_UV1_USE_TRIPLANAR]) {
                code +=
                    "\tAO = "
                    "dot(triplanar_texture(texture_ambient_occlusion,uv1_power_"
                    "normal,uv1_triplanar_pos),ao_texture_channel);\n";
            } else {
                code +=
                    "\tAO = "
                    "dot(texture(texture_ambient_occlusion,base_uv),ao_texture_"
                    "channel);\n";
            }
        }
    ...
    if (features[FEATURE_SUBSURACE_SCATTERING]) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tfloat sss_tex = "
                "triplanar_texture(texture_subsurface_scattering,uv1_power_"
                "normal,uv1_triplanar_pos).r;\n";
        } else {
            code +=
                "\tfloat sss_tex = "
                "texture(texture_subsurface_scattering,base_uv).r;\n";
        }
    ...
    if (features[FEATURE_TRANSMISSION]) {
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                "\tvec3 transmission_tex = "
                "triplanar_texture(texture_transmission,uv1_power_normal,uv1_"
                "triplanar_pos).rgb;\n";
        } else {
            code +=
                "\tvec3 transmission_tex = "
                "texture(texture_transmission,base_uv).rgb;\n";
        }
    ...
    if (features[FEATURE_DETAIL]) {
        bool triplanar =
            (flags[FLAG_UV1_USE_TRIPLANAR] && detail_uv == DETAIL_UV_1)
        ...
        if (triplanar) {
            ...
            code +=
                ...
                + tp_uv + "_power_normal," + tp_uv + "_triplanar_pos);\n";
            code +=
                "\tvec4 detail_norm_tex = "
                "triplanar_texture(texture_detail_normal,"
                + tp_uv + "_power_normal," + tp_uv + "_triplanar_pos);\n";

        } else {
            String det_uv = detail_uv == DETAIL_UV_1 ? "base_uv" : "base_uv2";
            code += "\tvec4 detail_tex = texture(texture_detail_albedo,"
                  + det_uv + ");\n";
            code += "\tvec4 detail_norm_tex = texture(texture_detail_normal,"
                  + det_uv + ");\n";
        }
        ...
        if (flags[FLAG_UV1_USE_TRIPLANAR]) {
            code +=
                ...
                "triplanar_texture(texture_detail_mask,uv1_power_normal,uv1_"
                "triplanar_pos);\n";
        } else {
            code +=
                "\tvec4 detail_mask_tex = "
                "texture(texture_detail_mask,base_uv);\n";
        }
        ...
        switch (detail_blend_mode) {
            case BLEND_MODE_MIX: {
                code +=
                    "\tvec3 detail = "
                    "mix(ALBEDO.rgb,detail_tex.rgb,detail_tex.a);\n";
            } break;
            case BLEND_MODE_ADD: {
                code +=
                    "\tvec3 detail = "
                    "mix(ALBEDO.rgb,ALBEDO.rgb+detail_tex.rgb,detail_tex.a);\n";
            } break;
            case BLEND_MODE_SUB: {
                code +=
                    "\tvec3 detail = "
        ...
        code +=
            "\tvec3 detail_norm = "
            "mix(NORMALMAP,detail_norm_tex.rgb,detail_tex.a);\n";
...
void SpatialMaterial::set_albedo(const Color& p_albedo) {
    ...
    VS::get_singleton()
        ->material_set_param(_get_material(), shader_names->albedo, p_albedo);
}
...
void SpatialMaterial::set_rim(float p_rim) {
    rim = p_rim;
    VS::get_singleton()
        ->material_set_param(_get_material(), shader_names->rim, p_rim);
}
...
void SpatialMaterial::set_subsurface_scattering_strength(
...
void SpatialMaterial::set_texture(
    TextureParam p_param,
...
void SpatialMaterial::_validate_feature(
    const String& text,
    Feature feature,
    PropertyInfo& property
) const {
    if (property.name.begins_with(text) && property.name != text + "_enabled"
        && !features[feature]) {
        property.usage = 0;
    }
}

void SpatialMaterial::_validate_high_end(
    const String& text,
    PropertyInfo& property
) const {
...
void SpatialMaterial::_validate_property(PropertyInfo& property) const {
    ...
    if (property.name.begins_with("particles_anim_")
        && billboard_mode != BILLBOARD_PARTICLES) {
        property.usage = 0;
    }
    ...
    if ((property.name == "distance_fade_max_distance"
         || property.name == "distance_fade_min_distance")
        && distance_fade == DISTANCE_FADE_DISABLED) {
        property.usage = 0;
    }

    if (property.name == "params_alpha_scissor_threshold"
        && !flags[FLAG_USE_ALPHA_SCISSOR]) {
        property.usage = 0;
    }

    if ((property.name == "depth_min_layers"
         || property.name == "depth_max_layers")
        && !deep_parallax) {
        property.usage = 0;
    }
...
void SpatialMaterial::set_uv1_scale(const Vector3& p_scale) {
    uv1_scale = p_scale;
    VS::get_singleton()
        ->material_set_param(_get_material(), shader_names->uv1_scale, p_scale);
}
...
void SpatialMaterial::set_uv2_scale(const Vector3& p_scale) {
    uv2_scale = p_scale;
    VS::get_singleton()
        ->material_set_param(_get_material(), shader_names->uv2_scale, p_scale);
}
...
void SpatialMaterial::set_uv2_triplanar_blend_sharpness(float p_sharpness) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->uv2_blend_sharpness,
        p_sharpness
    );
}
...
void SpatialMaterial::set_particles_anim_h_frames(int p_frames) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->particles_anim_h_frames,
        p_frames
    );
}
...
void SpatialMaterial::set_particles_anim_v_frames(int p_frames) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->particles_anim_v_frames,
        p_frames
    );
}
...
void SpatialMaterial::set_particles_anim_loop(bool p_loop) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->particles_anim_loop,
        particles_anim_loop
    );
}
...
void SpatialMaterial::set_depth_deep_parallax_flip_tangent(bool p_flip) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->depth_flip,
        Vector2(
            depth_parallax_flip_tangent ? -1 : 1,
            depth_parallax_flip_binormal ? -1 : 1
        )
    );
}
...
void SpatialMaterial::set_alpha_scissor_threshold(float p_threshold) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->alpha_scissor_threshold,
        p_threshold
    );
}
...
void SpatialMaterial::set_grow(float p_grow) {
    grow = p_grow;
    VS::get_singleton()
        ->material_set_param(_get_material(), shader_names->grow, p_grow);
}
...
void SpatialMaterial::set_metallic_texture_channel(TextureChannel p_channel) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->metallic_texture_channel,
        _get_texture_mask(p_channel)
    );
}
...
void SpatialMaterial::set_roughness_texture_channel(TextureChannel p_channel) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->roughness_texture_channel,
        _get_texture_mask(p_channel)
    );
}
...
void SpatialMaterial::set_ao_texture_channel(TextureChannel p_channel) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->ao_texture_channel,
        _get_texture_mask(p_channel)
    );
}
...
void SpatialMaterial::set_refraction_texture_channel(TextureChannel p_channel) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->refraction_texture_channel,
        _get_texture_mask(p_channel)
    );
}
...
RID SpatialMaterial::get_material_rid_for_2d(
    bool p_shaded,
    bool p_transparent,
    bool p_double_sided,
    bool p_cut_alpha,
    ...
    material->set_depth_draw_mode(
        p_opaque_prepass ? DEPTH_DRAW_ALPHA_OPAQUE_PREPASS
    ...
    if (p_billboard || p_billboard_y) {
        ...
        material->set_billboard_mode(
            p_billboard_y ? BILLBOARD_FIXED_Y : BILLBOARD_ENABLED
        );
    }
...
void SpatialMaterial::set_proximity_fade_distance(float p_distance) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->proximity_fade_distance,
        p_distance
    );
}
...
void SpatialMaterial::set_distance_fade_max_distance(float p_distance) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->distance_fade_max,
...
void SpatialMaterial::set_distance_fade_min_distance(float p_distance) {
    ...
    VS::get_singleton()->material_set_param(
        _get_material(),
        shader_names->distance_fade_min,
...
void SpatialMaterial::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_albedo", "albedo"),
        &SpatialMaterial::set_albedo
    );
    ClassDB::bind_method(D_METHOD("get_albedo"), &SpatialMaterial::get_albedo);

    ClassDB::bind_method(
        D_METHOD("set_specular", "specular"),
        &SpatialMaterial::set_specular
    );
    ClassDB::bind_method(
        D_METHOD("get_specular"),
        &SpatialMaterial::get_specular
    );

    ClassDB::bind_method(
        D_METHOD("set_metallic", "metallic"),
        &SpatialMaterial::set_metallic
    );
    ClassDB::bind_method(
        D_METHOD("get_metallic"),
        &SpatialMaterial::get_metallic
    );

    ClassDB::bind_method(
        D_METHOD("set_roughness", "roughness"),
        &SpatialMaterial::set_roughness
    );
    ClassDB::bind_method(
        D_METHOD("get_roughness"),
        &SpatialMaterial::get_roughness
    );

    ClassDB::bind_method(
        D_METHOD("set_emission", "emission"),
        &SpatialMaterial::set_emission
    );
    ClassDB::bind_method(
        D_METHOD("get_emission"),
        &SpatialMaterial::get_emission
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_energy", "emission_energy"),
        &SpatialMaterial::set_emission_energy
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_energy"),
        &SpatialMaterial::get_emission_energy
    );

    ClassDB::bind_method(
        D_METHOD("set_normal_scale", "normal_scale"),
        &SpatialMaterial::set_normal_scale
    );
    ClassDB::bind_method(
        D_METHOD("get_normal_scale"),
        &SpatialMaterial::get_normal_scale
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_rim_tint", "rim_tint"),
        &SpatialMaterial::set_rim_tint
    );
    ClassDB::bind_method(
        D_METHOD("get_rim_tint"),
        &SpatialMaterial::get_rim_tint
    );

    ClassDB::bind_method(
        D_METHOD("set_clearcoat", "clearcoat"),
        &SpatialMaterial::set_clearcoat
    );
    ClassDB::bind_method(
        D_METHOD("get_clearcoat"),
        &SpatialMaterial::get_clearcoat
    );

    ClassDB::bind_method(
        D_METHOD("set_clearcoat_gloss", "clearcoat_gloss"),
        &SpatialMaterial::set_clearcoat_gloss
    );
    ClassDB::bind_method(
        D_METHOD("get_clearcoat_gloss"),
        &SpatialMaterial::get_clearcoat_gloss
    );

    ClassDB::bind_method(
        D_METHOD("set_anisotropy", "anisotropy"),
        &SpatialMaterial::set_anisotropy
    );
    ClassDB::bind_method(
        D_METHOD("get_anisotropy"),
        &SpatialMaterial::get_anisotropy
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_scale", "depth_scale"),
        &SpatialMaterial::set_depth_scale
    );
    ClassDB::bind_method(
        D_METHOD("get_depth_scale"),
        &SpatialMaterial::get_depth_scale
    );

    ClassDB::bind_method(
        D_METHOD("set_subsurface_scattering_strength", "strength"),
        &SpatialMaterial::set_subsurface_scattering_strength
    );
    ClassDB::bind_method(
        D_METHOD("get_subsurface_scattering_strength"),
        &SpatialMaterial::get_subsurface_scattering_strength
    );

    ClassDB::bind_method(
        D_METHOD("set_transmission", "transmission"),
        &SpatialMaterial::set_transmission
    );
    ClassDB::bind_method(
        D_METHOD("get_transmission"),
        &SpatialMaterial::get_transmission
    );

    ClassDB::bind_method(
        D_METHOD("set_refraction", "refraction"),
        &SpatialMaterial::set_refraction
    );
    ClassDB::bind_method(
        D_METHOD("get_refraction"),
        &SpatialMaterial::get_refraction
    );

    ClassDB::bind_method(
        D_METHOD("set_line_width", "line_width"),
        &SpatialMaterial::set_line_width
    );
    ClassDB::bind_method(
        D_METHOD("get_line_width"),
        &SpatialMaterial::get_line_width
    );

    ClassDB::bind_method(
        D_METHOD("set_point_size", "point_size"),
        &SpatialMaterial::set_point_size
    );
    ClassDB::bind_method(
        D_METHOD("get_point_size"),
        &SpatialMaterial::get_point_size
    );

    ClassDB::bind_method(
        D_METHOD("set_detail_uv", "detail_uv"),
        &SpatialMaterial::set_detail_uv
    );
    ClassDB::bind_method(
        D_METHOD("get_detail_uv"),
        &SpatialMaterial::get_detail_uv
    );

    ClassDB::bind_method(
        D_METHOD("set_blend_mode", "blend_mode"),
        &SpatialMaterial::set_blend_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_blend_mode"),
        &SpatialMaterial::get_blend_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_draw_mode", "depth_draw_mode"),
        &SpatialMaterial::set_depth_draw_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_depth_draw_mode"),
        &SpatialMaterial::get_depth_draw_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_cull_mode", "cull_mode"),
        &SpatialMaterial::set_cull_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_cull_mode"),
        &SpatialMaterial::get_cull_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_diffuse_mode", "diffuse_mode"),
        &SpatialMaterial::set_diffuse_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_diffuse_mode"),
        &SpatialMaterial::get_diffuse_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_specular_mode", "specular_mode"),
        &SpatialMaterial::set_specular_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_specular_mode"),
        &SpatialMaterial::get_specular_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_flag", "flag", "enable"),
        &SpatialMaterial::set_flag
    );
    ClassDB::bind_method(
        D_METHOD("get_flag", "flag"),
        &SpatialMaterial::get_flag
    );

    ClassDB::bind_method(
        D_METHOD("set_feature", "feature", "enable"),
        &SpatialMaterial::set_feature
    );
    ClassDB::bind_method(
        D_METHOD("get_feature", "feature"),
        &SpatialMaterial::get_feature
    );

    ClassDB::bind_method(
        D_METHOD("set_texture", "param", "texture"),
        &SpatialMaterial::set_texture
    );
    ClassDB::bind_method(
        D_METHOD("get_texture", "param"),
        &SpatialMaterial::get_texture
    );

    ClassDB::bind_method(
        D_METHOD("set_detail_blend_mode", "detail_blend_mode"),
        &SpatialMaterial::set_detail_blend_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_detail_blend_mode"),
        &SpatialMaterial::get_detail_blend_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_uv1_scale", "scale"),
        &SpatialMaterial::set_uv1_scale
    );
    ClassDB::bind_method(
        D_METHOD("get_uv1_scale"),
        &SpatialMaterial::get_uv1_scale
    );

    ClassDB::bind_method(
        D_METHOD("set_uv1_offset", "offset"),
        &SpatialMaterial::set_uv1_offset
    );
    ClassDB::bind_method(
        D_METHOD("get_uv1_offset"),
        &SpatialMaterial::get_uv1_offset
    );

    ClassDB::bind_method(
        D_METHOD("set_uv1_triplanar_blend_sharpness", "sharpness"),
        &SpatialMaterial::set_uv1_triplanar_blend_sharpness
    );
    ClassDB::bind_method(
        D_METHOD("get_uv1_triplanar_blend_sharpness"),
        &SpatialMaterial::get_uv1_triplanar_blend_sharpness
    );

    ClassDB::bind_method(
        D_METHOD("set_uv2_scale", "scale"),
        &SpatialMaterial::set_uv2_scale
    );
    ClassDB::bind_method(
        D_METHOD("get_uv2_scale"),
        &SpatialMaterial::get_uv2_scale
    );

    ClassDB::bind_method(
        D_METHOD("set_uv2_offset", "offset"),
        &SpatialMaterial::set_uv2_offset
    );
    ClassDB::bind_method(
        D_METHOD("get_uv2_offset"),
        &SpatialMaterial::get_uv2_offset
    );

    ClassDB::bind_method(
        D_METHOD("set_uv2_triplanar_blend_sharpness", "sharpness"),
        &SpatialMaterial::set_uv2_triplanar_blend_sharpness
    );
    ClassDB::bind_method(
        D_METHOD("get_uv2_triplanar_blend_sharpness"),
        &SpatialMaterial::get_uv2_triplanar_blend_sharpness
    );

    ClassDB::bind_method(
        D_METHOD("set_billboard_mode", "mode"),
        &SpatialMaterial::set_billboard_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_billboard_mode"),
        &SpatialMaterial::get_billboard_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_particles_anim_h_frames", "frames"),
        &SpatialMaterial::set_particles_anim_h_frames
    );
    ClassDB::bind_method(
        D_METHOD("get_particles_anim_h_frames"),
        &SpatialMaterial::get_particles_anim_h_frames
    );

    ClassDB::bind_method(
        D_METHOD("set_particles_anim_v_frames", "frames"),
        &SpatialMaterial::set_particles_anim_v_frames
    );
    ClassDB::bind_method(
        D_METHOD("get_particles_anim_v_frames"),
        &SpatialMaterial::get_particles_anim_v_frames
    );

    ClassDB::bind_method(
        D_METHOD("set_particles_anim_loop", "loop"),
        &SpatialMaterial::set_particles_anim_loop
    );
    ClassDB::bind_method(
        D_METHOD("get_particles_anim_loop"),
        &SpatialMaterial::get_particles_anim_loop
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_deep_parallax", "enable"),
        &SpatialMaterial::set_depth_deep_parallax
    );
    ClassDB::bind_method(
        D_METHOD("is_depth_deep_parallax_enabled"),
        &SpatialMaterial::is_depth_deep_parallax_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_deep_parallax_min_layers", "layer"),
        &SpatialMaterial::set_depth_deep_parallax_min_layers
    );
    ClassDB::bind_method(
        D_METHOD("get_depth_deep_parallax_min_layers"),
        &SpatialMaterial::get_depth_deep_parallax_min_layers
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_deep_parallax_max_layers", "layer"),
        &SpatialMaterial::set_depth_deep_parallax_max_layers
    );
    ClassDB::bind_method(
        D_METHOD("get_depth_deep_parallax_max_layers"),
        &SpatialMaterial::get_depth_deep_parallax_max_layers
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_deep_parallax_flip_tangent", "flip"),
        &SpatialMaterial::set_depth_deep_parallax_flip_tangent
    );
    ClassDB::bind_method(
        D_METHOD("get_depth_deep_parallax_flip_tangent"),
        &SpatialMaterial::get_depth_deep_parallax_flip_tangent
    );

    ClassDB::bind_method(
        D_METHOD("set_depth_deep_parallax_flip_binormal", "flip"),
        &SpatialMaterial::set_depth_deep_parallax_flip_binormal
    );
    ClassDB::bind_method(
        D_METHOD("get_depth_deep_parallax_flip_binormal"),
        &SpatialMaterial::get_depth_deep_parallax_flip_binormal
    );

    ClassDB::bind_method(
        D_METHOD("set_grow", "amount"),
        &SpatialMaterial::set_grow
    );
    ClassDB::bind_method(D_METHOD("get_grow"), &SpatialMaterial::get_grow);

    ClassDB::bind_method(
        D_METHOD("set_emission_operator", "operator"),
        &SpatialMaterial::set_emission_operator
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_operator"),
        &SpatialMaterial::get_emission_operator
    );

    ClassDB::bind_method(
        D_METHOD("set_ao_light_affect", "amount"),
        &SpatialMaterial::set_ao_light_affect
    );
    ClassDB::bind_method(
        D_METHOD("get_ao_light_affect"),
        &SpatialMaterial::get_ao_light_affect
    );

    ClassDB::bind_method(
        D_METHOD("set_alpha_scissor_threshold", "threshold"),
        &SpatialMaterial::set_alpha_scissor_threshold
    );
    ClassDB::bind_method(
        D_METHOD("get_alpha_scissor_threshold"),
        &SpatialMaterial::get_alpha_scissor_threshold
    );

    ClassDB::bind_method(
        D_METHOD("set_grow_enabled", "enable"),
        &SpatialMaterial::set_grow_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_grow_enabled"),
        &SpatialMaterial::is_grow_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_metallic_texture_channel", "channel"),
        &SpatialMaterial::set_metallic_texture_channel
    );
    ClassDB::bind_method(
        D_METHOD("get_metallic_texture_channel"),
        &SpatialMaterial::get_metallic_texture_channel
    );

    ClassDB::bind_method(
        D_METHOD("set_roughness_texture_channel", "channel"),
        &SpatialMaterial::set_roughness_texture_channel
    );
    ClassDB::bind_method(
        D_METHOD("get_roughness_texture_channel"),
        &SpatialMaterial::get_roughness_texture_channel
    );

    ClassDB::bind_method(
        D_METHOD("set_ao_texture_channel", "channel"),
        &SpatialMaterial::set_ao_texture_channel
    );
    ClassDB::bind_method(
        D_METHOD("get_ao_texture_channel"),
        &SpatialMaterial::get_ao_texture_channel
    );

    ClassDB::bind_method(
        D_METHOD("set_refraction_texture_channel", "channel"),
        &SpatialMaterial::set_refraction_texture_channel
    );
    ClassDB::bind_method(
        D_METHOD("get_refraction_texture_channel"),
        &SpatialMaterial::get_refraction_texture_channel
    );

    ClassDB::bind_method(
        D_METHOD("set_proximity_fade", "enabled"),
        &SpatialMaterial::set_proximity_fade
    );
    ClassDB::bind_method(
        D_METHOD("is_proximity_fade_enabled"),
        &SpatialMaterial::is_proximity_fade_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_proximity_fade_distance", "distance"),
        &SpatialMaterial::set_proximity_fade_distance
    );
    ClassDB::bind_method(
        D_METHOD("get_proximity_fade_distance"),
        &SpatialMaterial::get_proximity_fade_distance
    );

    ClassDB::bind_method(
        D_METHOD("set_distance_fade", "mode"),
        &SpatialMaterial::set_distance_fade
    );
    ClassDB::bind_method(
        D_METHOD("get_distance_fade"),
        &SpatialMaterial::get_distance_fade
    );

    ClassDB::bind_method(
        D_METHOD("set_distance_fade_max_distance", "distance"),
        &SpatialMaterial::set_distance_fade_max_distance
    );
    ClassDB::bind_method(
        D_METHOD("get_distance_fade_max_distance"),
        &SpatialMaterial::get_distance_fade_max_distance
    );

    ClassDB::bind_method(
        D_METHOD("set_distance_fade_min_distance", "distance"),
        &SpatialMaterial::set_distance_fade_min_distance
    );
    ClassDB::bind_method(
        D_METHOD("get_distance_fade_min_distance"),
        &SpatialMaterial::get_distance_fade_min_distance
    );

    ADD_GROUP("Flags", "flags_");
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_transparent"),
        "set_feature",
        "get_feature",
        FEATURE_TRANSPARENT
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_use_shadow_to_opacity"),
        "set_flag",
        "get_flag",
        FLAG_USE_SHADOW_TO_OPACITY
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_unshaded"),
        "set_flag",
        "get_flag",
        FLAG_UNSHADED
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_vertex_lighting"),
        "set_flag",
        "get_flag",
        FLAG_USE_VERTEX_LIGHTING
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_no_depth_test"),
        "set_flag",
        "get_flag",
        FLAG_DISABLE_DEPTH_TEST
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_use_point_size"),
        "set_flag",
        "get_flag",
        FLAG_USE_POINT_SIZE
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_world_triplanar"),
        "set_flag",
        "get_flag",
        FLAG_TRIPLANAR_USE_WORLD
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_fixed_size"),
        "set_flag",
        "get_flag",
        FLAG_FIXED_SIZE
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_albedo_tex_force_srgb"),
        "set_flag",
        "get_flag",
        FLAG_ALBEDO_TEXTURE_FORCE_SRGB
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_do_not_receive_shadows"),
        "set_flag",
        "get_flag",
        FLAG_DONT_RECEIVE_SHADOWS
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_disable_ambient_light"),
        "set_flag",
        "get_flag",
        FLAG_DISABLE_AMBIENT_LIGHT
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "flags_ensure_correct_normals"),
        "set_flag",
        "get_flag",
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "vertex_color_use_as_albedo"),
        "set_flag",
        "get_flag",
        FLAG_ALBEDO_FROM_VERTEX_COLOR
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "vertex_color_is_srgb"),
        "set_flag",
        "get_flag",
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "params_diffuse_mode",
            PROPERTY_HINT_ENUM,
            "Burley,Lambert,Lambert Wrap,Oren Nayar,Toon"
        ),
        "set_diffuse_mode",
        "get_diffuse_mode"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            PROPERTY_HINT_ENUM,
            "Opaque Only,Always,Never,Opaque Pre-Pass"
        ),
        "set_depth_draw_mode",
        "get_depth_draw_mode"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            PROPERTY_HINT_ENUM,
            "Disabled,Enabled,Y-Billboard,Particle Billboard"
        ),
        "set_billboard_mode",
        "get_billboard_mode"
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "params_billboard_keep_scale"),
        "set_flag",
        "get_flag",
        FLAG_BILLBOARD_KEEP_SCALE
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "params_grow"),
        "set_grow_enabled",
        "is_grow_enabled"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "params_use_alpha_scissor"),
        "set_flag",
        "get_flag",
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "particles_anim_loop"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::COLOR, "albedo_color"),
        "set_albedo",
        "get_albedo"
    );
    ADD_PROPERTYI(
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "emission_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_EMISSION
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::COLOR, "emission", PROPERTY_HINT_COLOR_NO_ALPHA),
        "set_emission",
        "get_emission"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "emission_on_uv2"),
        "set_flag",
        "get_flag",
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "normal_enabled"),
        "set_feature",
        "get_feature",
    ...
    ADD_GROUP("Rim", "rim_");
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "rim_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_RIM
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "rim", PROPERTY_HINT_RANGE, "0,1,0.01"),
        "set_rim",
        "get_rim"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "clearcoat_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_CLEARCOAT
    );
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "anisotropy_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_ANISOTROPY
    );
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "ao_enabled"),
        "set_feature",
        "get_feature",
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "ao_on_uv2"),
        "set_flag",
        "get_flag",
        FLAG_AO_ON_UV2
    );
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "depth_enabled"),
        "set_feature",
        "get_feature",
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "depth_deep_parallax"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "depth_flip_tangent"),
        "set_depth_deep_parallax_flip_tangent",
        "get_depth_deep_parallax_flip_tangent"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "depth_flip_binormal"),
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "subsurf_scatter_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_SUBSURACE_SCATTERING
    );
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "transmission_enabled"),
        "set_feature",
        "get_feature",
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "refraction_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_REFRACTION
    );
    ...
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "detail_enabled"),
        "set_feature",
        "get_feature",
        FEATURE_DETAIL
    );
    ...
    ADD_GROUP("UV1", "uv1_");
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "uv1_scale"),
        "set_uv1_scale",
        "get_uv1_scale"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "uv1_offset"),
        "set_uv1_offset",
        "get_uv1_offset"
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "uv1_triplanar"),
        "set_flag",
        "get_flag",
    ...
    ADD_GROUP("UV2", "uv2_");
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "uv2_scale"),
        "set_uv2_scale",
        "get_uv2_scale"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "uv2_offset"),
        "set_uv2_offset",
        "get_uv2_offset"
    );
    ADD_PROPERTYI(
        PropertyInfo(Variant::BOOL, "uv2_triplanar"),
        "set_flag",
        "get_flag",
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "proximity_fade_enable"),
        "set_proximity_fade",
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "distance_fade_mode",
            PROPERTY_HINT_ENUM,
            "Disabled,PixelAlpha,PixelDither,ObjectDither"
        ),
        "set_distance_fade",
        "get_distance_fade"
    );
