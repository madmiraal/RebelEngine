...
private:
    ...
    Error _write_node(
        ...
        StringBuilder& code,
        Vector<DefaultTextureParam>& def_tex_params,
        const VMap<ConnectionKey, const List<Connection>::Element*>&
            input_connections,
        const VMap<ConnectionKey, const List<Connection>::Element*>&
            output_connections,
        int node,
...
public:
    ...
    bool is_nodes_connected_relatively(
        const Graph* p_graph,
        int p_node,
        int p_target
    ) const;
    bool can_connect_nodes(
        Type p_type,
        int p_from_node,
        int p_from_port,
        int p_to_node,
    ...
    void rebuild();
    void get_node_connections(Type p_type, List<Connection>* r_connections)
        const;
    ...
    String generate_preview_shader(
        Type p_type,
        int p_node,
        int p_port,
        Vector<DefaultTextureParam>& r_default_tex_params
    ) const;

    String validate_port_name(
        const String& p_name,
        const List<String>& p_input_ports,
        const List<String>& p_output_ports
    ) const;
    String validate_uniform_name(
        const String& p_name,
        const Ref<VisualShaderNodeUniform>& p_uniform
    ) const;
...
public:
    ...
    virtual Variant get_input_port_default_value(int p_port
    ) const; // if NIL (default if node does not set anything) is returned, it
             // means no default value is wanted if disconnected, thus no input
    ...
    ) const = 0; // if no output is connected, the output var passed will be
                 // empty. if no input is connected and input is NIL, the input
                 // var passed will be empty
...
public:
    ...
    PortType get_input_port_type(int p_port) const override;
    String get_input_port_name(int p_port) const override;
    Variant get_input_port_default_value(int p_port) const override;
...
public:
    ...
    String get_input_port_name(int p_id) const override;
    ...
    String get_output_port_name(int p_id) const override;
