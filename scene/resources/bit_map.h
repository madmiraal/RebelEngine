...
class BitMap : public Resource {
    ...
    Vector<Vector2> _march_square(const Rect2i& rect, const Point2i& start)
        const;
...
public:
    ...
    void create_from_image_alpha(
        const Ref<Image>& p_image,
        float p_threshold = 0.1
    );
    ...
    Vector<Vector<Vector2>> clip_opaque_to_polygons(
        const Rect2& p_rect,
        float p_epsilon = 2.0
    ) const;
