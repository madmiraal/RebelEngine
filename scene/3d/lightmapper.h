...
public:
    ...
    struct __aligned(16) Ray {
        const static unsigned int INVALID_GEOMETRY_ID =
            ((unsigned int)-1); // from rtcore_common.h

        /*! Default construction does nothing. */
        _FORCE_INLINE_ Ray() : geomID(INVALID_GEOMETRY_ID) {}
...
public:
    typedef bool (*BakeStepFunc)(
        float,
        const String&,
        void*,
        bool
    ); // progress, step description, userdata, force refresh
