...
protected:
    ...
    bool _get_property_pinned_points(
        int p_item,
        const String& p_what,
        Variant& r_ret
    ) const;
...
public:
    ...
    void set_pinned_points_indices(
        PoolVector<PinnedPoint> p_pinned_points_indices
    );
    ...
    void pin_point(
        int p_point_index,
        bool pin,
        const NodePath& p_spatial_attachment_path = NodePath()
    );
...
private:
    ...
    void _add_pinned_point(
        int p_point_index,
        const NodePath& p_spatial_attachment_path
    );
