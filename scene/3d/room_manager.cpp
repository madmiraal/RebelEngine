...
static void merge_rooms_meshes(
...
void RoomManager::rooms_convert() {
    ...
    auto* rooms_root_node = cast_to<Spatial>(get_node(room_list_node_path));
    if (!rooms_root_node) {
    ...
    const bool all_rooms_found =
        add_portal_links(rooms, portals, rooms_root_node);
    if (merge_meshes) {
        merge_rooms_meshes(rooms, debug_logging, remove_danglers);
    }
    add_rooms_bounds(rooms, portals, default_simplify_info);
    const bool all_auto_links_successful =
        auto_link_portals(rooms, portals, debug_logging);
    if (debug_logging) {
        check_rooms_portals_links(rooms, portals);
    }
    finalize_rooms(rooms, portals, default_simplify_info, debug_logging);
    auto_place_objects(rooms_root_node, rooms, debug_logging);
...
#ifdef TOOLS_ENABLED
    bool rooms_overlap =
        create_room_overlap_zones(rooms, overlap_warning_threshold);
#endif // TOOLS_ENABLED
    ...
    if (!all_rooms_found) {
        show_alert(TTR("Imported portal link room not found."));
    }

    if (!all_auto_links_successful) {
        show_alert(
            TTR("Portal auto link failed!\n"
...
bool RoomManager::add_portal_links(
    ...
    return all_rooms_found;
}

bool RoomManager::add_imported_portal_portal_link(
    Portal* portal,
    const Spatial* rooms_root_node
) {
...
void RoomManager::add_nodes(
    ...
    if (cast_to<RoomGroup>(node) || node_name_ends_with(node, "-roomgroup")) {
        room_group =
            add_room_group(node, room_groups, conversion_count, debug_logging);
    }
    for (int index = 0; index < node->get_child_count(); index++) {
    ...
    add_rooms_to_room_groups(rooms, room_groups);
}
...
void RoomManager::add_room(
    ...
    add_portals(room, room->_room_ID, portals, conversion_count);
}
...
void RoomManager::add_portals(
    ...
    if (cast_to<Portal>(node)
        ...
        add_portal(node, room_id, portals, conversion_count);
    }
    for (int index = 0; index < node->get_child_count(); index++) {
        auto* child = cast_to<Spatial>(node->get_child(index));
        if (child) {
            add_portals(child, room_id, portals, conversion_count);
        }
    }
}
...
void RoomManager::add_rooms_bounds(
    LocalVector<Room*, int32_t>& rooms,
...
bool RoomManager::room_has_point(Room* room, const Vector3& point) {
    if (!room->_aabb.has_point(point)) {
        return false;
    }
    for (int plane_index = 0; plane_index < room->_preliminary_planes.size();
         plane_index++) {
        const Plane& plane = room->_preliminary_planes[plane_index];
...
int RoomManager::find_best_destination_room_id_at_point(
    LocalVector<Room*, int32_t>& rooms,
    const int source_room_id,
    const Vector3& point
) {
    int best_destination_room_id = -1;
    int best_priority            = -1000;
    for (int room_index = 0; room_index < rooms.size(); room_index++) {
        Room* room = rooms[room_index];
        if (room->_room_ID == source_room_id) {
            continue;
        }
        if (!room_has_point(room, point)) {
            continue;
        }
        // A destination room found.
        // Use room with the highest priority in case there are internal rooms.
        if (room->_room_priority > best_priority) {
            best_destination_room_id = room_index;
            best_priority            = room->_room_priority;
        }
    }
    return best_destination_room_id;
}

int RoomManager::find_portal_best_destination_room_id(
    const Portal* portal,
    LocalVector<Room*, int32_t>& rooms
) {
    for (int attempt = 0; attempt < 4; attempt++) {
        // These numbers are arbitrary.
        // TODO: Investigate using portal margins.
        real_t dist = 0.01f;
        switch (attempt) {
            case 0: {
                dist = 0.01f;
        ...
        const int destination_room_id = find_best_destination_room_id_at_point(
            rooms,
...
bool RoomManager::auto_link_portals(
    LocalVector<Room*, int32_t>& rooms,
    LocalVector<Portal*>& portals,
    const bool debug_logging
) {
    bool all_auto_links_successful = true;
    for (unsigned int portal_index = 0; portal_index < portals.size();
         portal_index++) {
        Portal* portal           = portals[portal_index];
        const int source_room_id = portal->_linkedroom_ID[0];
        // All portals must have a source room.
        DEV_ASSERT(source_room_id != -1);
        const Room* source_room = rooms[source_room_id];
        if (portal->_linkedroom_ID[1] != -1) {
            // Already manually linked.
            continue;
        }

        const int destination_room_id =
            find_portal_best_destination_room_id(portal, rooms);
        if (destination_room_id == -1) {
            WARN_PRINT(
                "Portal AUTOLINK failed for " + portal->get_name() + " from "
                + source_room->get_name()
            );
            all_auto_links_successful = false;
#ifdef TOOLS_ENABLED
        ...
        Room* destination_room = rooms[destination_room_id];
        if (debug_logging) {
            print_line(
                "\t\tAUTOLINK OK from " + source_room->get_name() + " to "
                + destination_room->get_name()
            );
        }
        portal->_linkedroom_ID[1] = destination_room_id;
        destination_room->_portals.push_back(portal_index);
        VisualServer::get_singleton()->portal_link(
            portal->_portal_rid,
            source_room->_room_rid,
            destination_room->_room_rid,
            portal->_settings_two_way
        );
        portal->_internal =
            source_room->_room_priority > destination_room->_room_priority;
    }
    return all_auto_links_successful;
}

void RoomManager::create_simplified_bound(
    const Room* room,
    Geometry::MeshData& mesh_data,
    LocalVector<Plane, int32_t>& planes,
    const Room::SimplifyInfo& default_simplify_info,
    const int portal_planes_count
) {
    if (planes.empty()) {
        return;
    }
    ...
    const Vector<Vector3> points =
        Geometry::compute_convex_mesh_points(&planes[0], planes.size(), 0.001f);
    const Error error = build_room_convex_hull(
        room,
        points,
        mesh_data,
        default_simplify_info._plane_simplify
    ...
    // If the number of faces is less than the number of planes,
    // we can use this simplified version to reduce the number of planes.
    if (mesh_data.faces.size() < planes.size()) {
        // Always include the portal planes.
        planes.resize(portal_planes_count);
        for (int face_index = 0; face_index < mesh_data.faces.size();
             face_index++) {
            add_plane_if_unique(
                mesh_data.faces[face_index].plane,
                planes,
                room,
                default_simplify_info
            );
...
void RoomManager::check_portal_link(
    const Portal* portal,
    LocalVector<Room*, int32_t>& rooms,
    const bool portal_links_out
) {
    const int linked_room_id = portal_links_out ? portal->_linkedroom_ID[1]
                                                : portal->_linkedroom_ID[0];
    ERR_FAIL_INDEX_MSG(
        linked_room_id,
        rooms.size(),
        "Linked room id is out of range"
    );
    const String in_or_out  = portal_links_out ? "POUT" : "PIN ";
    const Room* linked_room = rooms[linked_room_id];
    const String portal_link_room_name =
        remove_suffix(linked_room->get_name(), "-room", true);
    print_line("\t\t" + in_or_out + "\t" + portal_link_room_name);
}

void RoomManager::check_rooms_portals_links(
    LocalVector<Room*, int32_t>& rooms,
    const LocalVector<Portal*>& portals
) {
    for (int room_index = 0; room_index < rooms.size(); room_index++) {
        Room* room             = rooms[room_index];
        String room_short_name = remove_suffix(room->get_name(), "-room", true);
        print_line("ROOM\t" + room_short_name);
        for (int portal_index = 0; portal_index < room->_portals.size();
             portal_index++) {
            const Portal* portal = portals[room->_portals[portal_index]];
            check_portal_link(
                portal,
                rooms,
...
void RoomManager::auto_place_objects(
    Spatial* node,
    LocalVector<Room*, int32_t>& rooms,
    const bool debug_logging
) {
    ...
    if (cast_to<Room>(node)) {
        // All objects inside rooms have already been added.
        return;
    }

    auto* visual_instance = cast_to<VisualInstance>(node);
    if (visual_instance) {
        const CullInstance::PortalMode mode =
            visual_instance->get_portal_mode();
        if (mode == CullInstance::PORTAL_MODE_STATIC
            || mode == CullInstance::PORTAL_MODE_DYNAMIC) {
            // We only place objects that are in static or dynamic mode.
            auto_place_object(visual_instance, rooms, debug_logging);
        }
    }

    for (int n = 0; n < node->get_child_count(); n++) {
        auto* child = cast_to<Spatial>(node->get_child(n));
        if (child) {
            auto_place_objects(child, rooms, debug_logging);
        }
    }
}

void RoomManager::auto_place_object(
    VisualInstance* visual_instance,
    LocalVector<Room*, int32_t>& rooms,
    const bool debug_logging
) {
    const AABB aabb      = visual_instance->get_transformed_aabb();
    const Vector3 centre = aabb.get_center();
    const int instance_auto_place_priority =
        visual_instance->get_portal_autoplace_priority();
    const Room* best_room = nullptr;
    for (int room_index = 0; room_index < rooms.size(); room_index++) {
        const Room* room = rooms[room_index];
        if (room->contains_point(centre)) {
            // If the instance auto priority is set, we place the object in the
            // room with the same priority.
            if (instance_auto_place_priority != 0
                && room->_room_priority == instance_auto_place_priority) {
                best_room = room;
                break;
            }
            // Else, place the object in the room with the highest priority.
            if (!best_room
                || room->_room_priority > best_room->_room_priority) {
                best_room = room;
            }
        }
    }
...
void RoomManager::rooms_flip_portals() {
    ...
    flip_portals(rooms_root_node);
}

void RoomManager::flip_portals(Spatial* node) {
    auto* portal = cast_to<Portal>(node);
    if (portal) {
        portal->flip();
    }
...
#ifdef TOOLS_ENABLED
bool RoomManager::create_room_overlap_zones(
    LocalVector<Room*, int32_t>& rooms,
    const real_t rooms_overlap_volume_threshold
) {
    bool rooms_overlap = false;
    for (int first_room_index = 0; first_room_index < rooms.size();
         first_room_index++) {
        Room* first_room = rooms[first_room_index];
        if (first_room->_planes.empty()) {
            continue;
        }
        for (int second_room_index = first_room_index + 1;
             second_room_index < rooms.size();
             second_room_index++) {
            if (second_room_index == first_room_index) {
                continue;
            }
            Room* second_room = rooms[second_room_index];
            if (second_room->_planes.empty()
                || !first_room->_aabb.intersects(second_room->_aabb)) {
                continue;
            }
            // If one room is internal to the other, they can overlap.
            if (first_room->_room_priority != second_room->_room_priority) {
                continue;
            }

            LocalVector<Plane, int32_t> combined_planes;
            combined_planes.resize(
                first_room->_planes.size() + second_room->_planes.size()
            );
            Plane* dest = combined_planes.ptr();
            memcpy(
                dest,
                &first_room->_planes[0],
                first_room->_planes.size() * sizeof(Plane)
            );
            dest += first_room->_planes.size();
            memcpy(
                dest,
                &second_room->_planes[0],
                second_room->_planes.size() * sizeof(Plane)
            );

            Vector<Vector3> overlap_points =
                Geometry::compute_convex_mesh_points(
                    combined_planes.ptr(),
                    combined_planes.size()
                );
            if (overlap_points.size() < 4) {
                continue;
            }

            // There is an overlap, create a mesh from the overlap points.
            Geometry::MeshData mesh_data;
            const Error error = build_quick_hull(overlap_points, mesh_data);
            ERR_CONTINUE_MSG(
                error != OK,
                "QuickHull failed building room overlap hull"
            );
            // only if the volume is more than some threshold
            real_t mesh_volume =
                Geometry::calculate_convex_hull_volume(mesh_data);
            if (mesh_volume > rooms_overlap_volume_threshold) {
                WARN_PRINT(
                    "Room overlap of " + String(Variant(mesh_volume))
                    + " detected between " + first_room->get_name() + " and "
                    + second_room->get_name()
                );
                first_room->_gizmo_overlap_zones.push_back(mesh_data);
                rooms_overlap = true;
            }
        }
    }
...
void RoomManager::add_rooms_to_room_groups(
    LocalVector<Room*, int32_t>& rooms,
    const LocalVector<RoomGroup*>& room_groups
) {
    for (int index = 0; index < rooms.size(); index++) {
...
bool RoomManager::create_initial_room_hull(
    Room* room,
    const Vector<Vector3>& room_points,
    const LocalVector<Portal*>& portals,
    const Room::SimplifyInfo& default_simplify_info
...
bool RoomManager::create_final_room_hull(
    Room* room,
    const LocalVector<Portal*>& portals,
    const Room::SimplifyInfo& default_simplify_info,
    const bool debug_logging
    ...
    Vector<Vector3> vertices_including_portals =
        room->_bound_mesh_data.vertices;
    // Add the portals planes first, because these will trump any other existing
    // planes further out.
    int num_portals_added = 0;
    for (int portal_index = 0; portal_index < room->_portals.size();
         portal_index++) {
        const int portal_id  = room->_portals[portal_index];
        const Portal* portal = portals[portal_id];
        // Don't add portals to the world bound that are internal to this room!
        if (portal->is_portal_internal(room->_room_ID)) {
            continue;
        }
        Plane plane = portal->_plane;
        // Reverse plane for incoming portals.
        if (portal->_linkedroom_ID[1] == room->_room_ID) {
            plane = -plane;
        }
        ...
        for (int point_index = 0; point_index < portal->_pts_world.size();
             point_index++) {
            const Vector3& point = portal->_pts_world[point_index];
            vertices_including_portals.push_back(point);
            room->_aabb.expand_to(point);
        }
    }
    ...
    for (int face_index = 0; face_index < mesh_data.faces.size();
         face_index++) {
        const Plane& plane = mesh_data.faces[face_index].plane;
        add_plane_if_unique(plane, room->_planes, room, default_simplify_info);
    }

    // Recreate the points within the new simplified bound.
    // Then recreate the convex hull by running quickhull a second time.
    // This enables the gizmo to accurately show the simplified hull.
    const int num_planes_before_simplification = room->_planes.size();
    Geometry::MeshData simplified_mesh_data;
    create_simplified_bound(
        room,
        simplified_mesh_data,
        room->_planes,
        default_simplify_info,
        num_portals_added
    );

    if (num_planes_before_simplification != room->_planes.size()) {
        if (debug_logging) {
            print_line(
                "\t\t\tcontained " + itos(num_planes_before_simplification)
                + " planes before simplification, " + itos(room->_planes.size())
                + " planes after."
            );
        }
    }
    ...
    // TODO: Investigate whether this be avoided in release builds.
    room->_bound_mesh_data = simplified_mesh_data;
...
bool RoomManager::get_room_points(
    ...
    for (int index = 0; index < room->get_child_count(); index++) {
        ...
        if (mesh_instance && node_name_ends_with(child, "-bound")) {
            ...
            return create_initial_room_hull(
                room,
                mesh_instance_points,
                portals,
                default_simplify_info
