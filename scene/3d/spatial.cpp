...
/*
 ...
 2) If a node sets a GLOBAL, it is converted to LOCAL (and forces validation of
everything pending below)

 drawback: setting/reading globals is useful and used very very often, and using
affine inverses is slow
...
void Spatial::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_ENTER_TREE: {
            ...
            if (data.toplevel && !Engine::get_singleton()->is_editor_hint()) {
                if (data.parent) {
                    data.local_transform =
                        data.parent->get_global_transform() * get_transform();
                    data.dirty = DIRTY_VECTORS; // global is always dirty upon
                                                // entering a scene
                }
            ...
            data.dirty |=
                DIRTY_GLOBAL; // global is always dirty upon entering a scene
            _notify_dirty();
        ...
        case NOTIFICATION_ENTER_WORLD: {
            ...
            if (get_script_instance()) {
                get_script_instance()->call_multilevel(
                    SceneStringNames::get_singleton()->_enter_world,
                    nullptr,
                    0
                );
            }
            ...
            if (Engine::get_singleton()->is_editor_hint()
                ...
                get_tree()->call_group_flags(
                    0,
                    SceneStringNames::get_singleton()->_spatial_editor_group,
                    SceneStringNames::get_singleton()->_request_gizmo,
                    this
                );
        ...
        case NOTIFICATION_EXIT_WORLD: {
            ...
            if (get_script_instance()) {
                get_script_instance()->call_multilevel(
                    SceneStringNames::get_singleton()->_exit_world,
                    nullptr,
                    0
                );
            }
    ...
    if (data.dirty & DIRTY_GLOBAL) {
        ...
        if (data.parent && !data.toplevel_active) {
            data.global_transform =
                data.parent->get_global_transform() * data.local_transform;
        } else {
...
void Spatial::set_as_toplevel(bool p_enabled) {
    ...
    if (is_inside_tree() && !Engine::get_singleton()->is_editor_hint()) {
        if (p_enabled) {
            ...
            set_transform(
                data.parent->get_global_transform().affine_inverse()
...
void Spatial::look_at_from_position(
    const Vector3& p_pos,
    ...
    ERR_FAIL_COND_MSG(
        p_up == Vector3(),
        "The up vector can't be zero, look_at() failed."
    );
    ERR_FAIL_COND_MSG(
...
void Spatial::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("get_parent_spatial"),
        &Spatial::get_parent_spatial
    );
    ClassDB::bind_method(
        D_METHOD("set_ignore_transform_notification", "enabled"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_disable_scale", "disable"),
        &Spatial::set_disable_scale
    );
    ClassDB::bind_method(
        D_METHOD("is_scale_disabled"),
    ...
    ClassDB::bind_method(
        D_METHOD("global_rotate", "axis", "angle"),
        &Spatial::global_rotate
    );
    ClassDB::bind_method(
        D_METHOD("global_scale", "scale"),
        &Spatial::global_scale
    );
    ClassDB::bind_method(
        D_METHOD("global_translate", "offset"),
        &Spatial::global_translate
    );
    ClassDB::bind_method(
        D_METHOD("rotate_object_local", "axis", "angle"),
        &Spatial::rotate_object_local
    );
    ClassDB::bind_method(
        D_METHOD("scale_object_local", "scale"),
        &Spatial::scale_object_local
    );
    ClassDB::bind_method(
        D_METHOD("translate_object_local", "offset"),
    ...
    ClassDB::bind_method(
        D_METHOD("look_at", "target", "up"),
        &Spatial::look_at
    );
    ClassDB::bind_method(
        D_METHOD("look_at_from_position", "position", "target", "up"),
        &Spatial::look_at_from_position
    );

    ClassDB::bind_method(
        D_METHOD("to_local", "global_point"),
        &Spatial::to_local
    );
    ClassDB::bind_method(
        D_METHOD("to_global", "local_point"),
        &Spatial::to_global
    );
    ...
    // ADD_PROPERTY(
    // PropertyInfo(Variant::TRANSFORM,"transform/global",PROPERTY_HINT_NONE,
    // "", PROPERTY_USAGE_EDITOR ), "set_global_transform",
    // "get_global_transform") ;
    ADD_GROUP("Transform", "");
    ADD_PROPERTY(
        PropertyInfo(
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR3, "rotation", PROPERTY_HINT_NONE, "", 0),
        "set_rotation",
        "get_rotation"
    );
    ADD_PROPERTY(
    ...
    ADD_GROUP("Matrix", "");
    ADD_PROPERTY(
        PropertyInfo(Variant::TRANSFORM, "transform", PROPERTY_HINT_NONE, ""),
        "set_transform",
        "get_transform"
    );
