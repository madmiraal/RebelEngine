...
void SplitContainer::_resort() {
    ...
    bool first_expanded =
        (vertical ? first->get_v_size_flags() : first->get_h_size_flags())
        & SIZE_EXPAND;
    bool second_expanded =
        (vertical ? second->get_v_size_flags() : second->get_h_size_flags())
        & SIZE_EXPAND;
    ...
    float ratio = first->get_stretch_ratio()
                / (first->get_stretch_ratio() + second->get_stretch_ratio());
    ...
    if (!collapsed) {
        int clamped_split_offset = CLAMP(
            split_offset,
            ms_first[axis] - no_offset_middle_sep,
            (get_size()[axis] - ms_second[axis] - sep) - no_offset_middle_sep
        );
    ...
    if (vertical) {
        fit_child_in_rect(
            first,
            Rect2(Point2(0, 0), Size2(get_size().width, middle_sep))
        );
        ...
        fit_child_in_rect(
            ...
            Rect2(
                Point2(0, sofs),
                Size2(get_size().width, get_size().height - sofs)
            )
        );
    } else {
        fit_child_in_rect(
            first,
            Rect2(Point2(0, 0), Size2(middle_sep, get_size().height))
        );
        int sofs = middle_sep + sep;
        fit_child_in_rect(
            second,
            Rect2(
                Point2(sofs, 0),
                Size2(get_size().width - sofs, get_size().height)
            )
        );
    }
...
void SplitContainer::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_DRAW: {
            ...
            if (collapsed
                || (!dragging && !mouse_inside && get_constant("autohide"))) {
                return;
            }
            ...
            if (vertical) {
                draw_texture(
                    tex,
                    Point2i(
                        (size.x - tex->get_width()) / 2,
                        middle_sep + (sep - tex->get_height()) / 2
                    )
                );
            } else {
                draw_texture(
                    tex,
                    Point2i(
                        middle_sep + (sep - tex->get_width()) / 2,
                        (size.y - tex->get_height()) / 2
                    )
                );
            }
        } break;
...
void SplitContainer::_gui_input(const Ref<InputEvent>& p_event) {
    ...
    if (mm.is_valid()) {
        ...
        if (vertical) {
            mouse_inside_state = mm->get_position().y > middle_sep
                              && mm->get_position().y
                                     < middle_sep + get_constant("separation");
        } else {
            mouse_inside_state = mm->get_position().x > middle_sep
                              && mm->get_position().x
                                     < middle_sep + get_constant("separation");
        }
        ...
        split_offset = drag_ofs
                     + ((vertical ? mm->get_position().y : mm->get_position().x)
                        - drag_from);
...
Control::CursorType SplitContainer::get_cursor_type(const Point2& p_pos
) const {
    if (dragging) {
...
void SplitContainer::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_split_offset", "offset"),
        &SplitContainer::set_split_offset
    );
    ClassDB::bind_method(
        D_METHOD("get_split_offset"),
        &SplitContainer::get_split_offset
    );
    ClassDB::bind_method(
        D_METHOD("clamp_split_offset"),
        &SplitContainer::clamp_split_offset
    );

    ClassDB::bind_method(
        D_METHOD("set_collapsed", "collapsed"),
        &SplitContainer::set_collapsed
    );
    ClassDB::bind_method(
        D_METHOD("is_collapsed"),
        &SplitContainer::is_collapsed
    );

    ClassDB::bind_method(
        D_METHOD("set_dragger_visibility", "mode"),
        &SplitContainer::set_dragger_visibility
    );
    ClassDB::bind_method(
        D_METHOD("get_dragger_visibility"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "split_offset"),
        "set_split_offset",
        "get_split_offset"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "dragger_visibility",
            PROPERTY_HINT_ENUM,
            "Visible,Hidden,Hidden & Collapsed"
        ),
