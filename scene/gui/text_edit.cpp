...
const Map<int, TextEdit::Text::ColorRegionInfo>& TextEdit::Text::
    get_color_region_info(int p_line) const {
...
void TextEdit::_update_scrollbars() {
    ...
    v_scroll->set_begin(Point2(
        size.width - vmin.width,
        cache.style_normal->get_margin(MARGIN_TOP)
    ));
    v_scroll->set_end(Point2(
        size.width,
        size.height - cache.style_normal->get_margin(MARGIN_TOP)
            - cache.style_normal->get_margin(MARGIN_BOTTOM)
    ));
    ...
    int visible_width =
        size.width - cache.style_normal->get_minimum_size().width;
...
void TextEdit::_click_selection_held() {
    // Warning: is_mouse_button_pressed(BUTTON_LEFT) returns false for double+
    // clicks, so this doesn't work for MODE_WORD and MODE_LINE. However, moving
    // the mouse triggers _gui_input, which calls these functions too, so that's
    // not a huge problem. I'm unsure if there's an actual fix that doesn't have
...
void TextEdit::_update_selection_mode_word() {
    ...
    // If its the first selection and on whitespace make sure we grab the word
    // instead.
    if (!selection.active) {
    ...
    while (beg > 0 && line[beg - 1] > 32
           && (symbol == _is_symbol(line[beg - 1]))) {
        beg--;
    }
    while (end < line.length() && line[end + 1] > 32
           && (symbol == _is_symbol(line[end + 1]))) {
        end++;
    }
    if (end < line.length()) {
        end += 1;
    ...
    if (!selection.active) {
        ...
        if ((col <= selection.selected_word_origin
            ...
            select(
                row,
                beg,
                selection.selecting_line,
                selection.selected_word_end
            );
...
void TextEdit::_update_minimap_hover() {
    ...
    const bool hovering_sidebar =
        mp.x > xmargin_end - minimap_width && mp.x < xmargin_end;
    if (!hovering_sidebar) {
        if (hovering_minimap) {
    ...
    const bool new_hovering_minimap =
        row >= get_first_visible_line() && row <= get_last_full_visible_line();
...
void TextEdit::_update_minimap_click() {
    ...
    int xmargin_end =
        get_size().width - cache.style_normal->get_margin(MARGIN_RIGHT);
    if (!dragging_minimap
        && (mp.x < xmargin_end - minimap_width || mp.y > xmargin_end)) {
    ...
    int wi;
    int first_line =
        row - num_lines_from_rows(row, 0, -get_visible_rows() / 2, wi) + 1;
...
void TextEdit::_update_minimap_drag() {
    ...
    int scroll_height =
        v_scroll->get_max() * (minimap_char_size.y + minimap_line_spacing);
    ...
    switch (p_what) {
        ...
        case NOTIFICATION_INTERNAL_PHYSICS_PROCESS: {
            if (scrolling && get_v_scroll() != target_v_scroll) {
                ...
                double vel      = ((target_y / dist)
                              * ((minimap_clicked) ? 3000 : v_scroll_speed))
        ...
        case NOTIFICATION_DRAW: {
            if (first_draw) {
                // Size may not be the final one, so attempts to ensure cursor
                // was visible may have failed.
...

            {
                ...
                if (line_numbers) {
                    ...
                    cache.line_number_w    = (cache.line_number_w + 1)
                                        * cache.font->get_char_size('0').width;
                } else {
                    cache.line_number_w = 0;
                }
            }
            ...
            VisualServer::get_singleton()->canvas_item_set_clip(
                get_canvas_item(),
                true
            );
            int xmargin_beg = cache.style_normal->get_margin(MARGIN_LEFT)
                            + cache.line_number_w
                            + cache.breakpoint_gutter_width
                            + cache.fold_gutter_width + cache.info_gutter_width;

            int xmargin_end = size.width
                            - cache.style_normal->get_margin(MARGIN_RIGHT)
                            - cache.minimap_width;
            ...
            Color color =
                readonly ? cache.font_color_readonly : cache.font_color;
            ...
            if (line_length_guidelines) {
                const int hard_x = xmargin_beg
                                 + (int)cache.font->get_char_size('0').width
                                       * line_length_guideline_hard_col
                                 - cursor.x_ofs;
                if (hard_x > xmargin_beg && hard_x < xmargin_end) {
                    VisualServer::get_singleton()->canvas_item_add_line(
                        ci,
                        Point2(hard_x, 0),
                        Point2(hard_x, size.height),
                        cache.line_length_guideline_color
                    );
                }

                // Draw a "Soft" line length guideline, less visible than the
                // hard line length guideline. It's usually set to a lower
                // column compared to the hard line length guideline. Only drawn
                // if its column differs from the hard line length guideline.
                const int soft_x = xmargin_beg
                                 + (int)cache.font->get_char_size('0').width
                                       * line_length_guideline_soft_col
                                 - cursor.x_ofs;
                if (hard_x != soft_x && soft_x > xmargin_beg
                    && soft_x < xmargin_end) {
                    VisualServer::get_singleton()->canvas_item_add_line(
                        ci,
                        Point2(soft_x, 0),
                        Point2(soft_x, size.height),
                        cache.line_length_guideline_color * Color(1, 1, 1, 0.5)
                    );
                }
            }
            ...
            if (brace_matching_enabled && cursor.line >= 0
                && cursor.line < text.size() && cursor.column >= 0) {
                            ...
                            for (int j = from; j < text[i].length(); j++) {
                                ...
                                if (cc == '"' || cc == '\'') {
                                    ...
                                    do {
                                        ...
                                        cc = text[i][j];
                                        // Skip over escaped quotation marks
                                        // inside strings.
                                        if (cc == '\\') {
                                            bool escaped = true;
                                            while (j + 1 < text[i].length()
                                                   && text[i][j + 1] == '\\') {
                                                escaped = !escaped;
                                                j++;
                                            }
                ...
                if (cursor.column > 0) {
                    ...
                    if (closec != 0) {
                        ...
                        for (int i = cursor.line; i >= 0; i--) {
                            int from = i == cursor.line ? cursor.column - 2
                                                        : text[i].length() - 1;
            ...
            // Check if highlighted words contains only whitespaces (tabs or
            // spaces).
            bool only_whitespaces_highlighted =
                highlighted_text.strip_edges() == String();
            ...
            if (draw_minimap) {
                ...
                int minimap_line_height =
                    (minimap_char_size.y + minimap_line_spacing);
                ...
                int viewport_offset_y =
                    round(
                        get_scroll_pos_for_line(first_visible_line + 1)
                        * control_height
                    )
                    / ((v_scroll->get_max() <= minimap_visible_lines)
                           ? (minimap_visible_lines - draw_amount)
                           : (v_scroll->get_max() - draw_amount));

                // calculate the first line.
                int num_lines_before =
                    round((viewport_offset_y) / minimap_line_height);
                int wi;
                int minimap_line =
                    (v_scroll->get_max() <= minimap_visible_lines)
                        ? -1
                        : first_visible_line;
                if (minimap_line >= 0) {
                    minimap_line -= num_lines_from_rows(
                        first_visible_line,
                        0,
                        -num_lines_before,
                        wi
                    ...
                    minimap_line -=
                        (minimap_line > 0 && smooth_scroll_enabled ? 1 : 0);
                }
                int minimap_draw_amount =
                    minimap_visible_lines + times_line_wraps(minimap_line + 1);

                // Draw the minimap.

                // Add visual feedback when dragging or hovering the the visible
                // area rectangle.
                ...
                const Color viewport_color =
                    (cache.background_color.get_v() < 0.5)
                ...
                for (int i = 0; i < minimap_draw_amount; i++) {
                    ...
                    for (int line_wrap_index = 0;
                         line_wrap_index < line_wrap_amount + 1;
                         line_wrap_index++) {
                        ...
                        if (line_wrap_index > 0) {
                            last_wrap_column +=
                                wrap_rows[line_wrap_index - 1].length();
                        }

                        if (minimap_line == cursor.line
                        ...
                        for (int j = 0; j < str.length(); j++) {
                            if (syntax_coloring) {
                                if (color_map.has(last_wrap_column + j)) {
                                    current_color =
                                        color_map[last_wrap_column + j].color;
                                    if (readonly) {
                                        current_color.a =
                                            cache.font_color_readonly.a;
                                    }
                                }
                                color = current_color;
                            }
                            ...
                            int xpos = indent_px
                                     + ((xmargin_end + minimap_char_size.x)
                                        + (minimap_char_size.x * j))
                                     + tabs;
                            bool out_of_bounds =
                                (xpos >= xmargin_end + cache.minimap_width);
                            ...
                            if (!is_whitespace) {
                                ...
                                // If we've changed colour we are at the start
                                // of a new section, therefore we need to go
                                // back to the end of the previous section to
                                // draw it, we'll also add the character back
                                // on.
            ...
            if (readonly) {
                ...
                bottom_limit_y -=
                    cache.style_readonly->get_margin(MARGIN_BOTTOM);
            } else {
            ...
            for (int i = 0; i < draw_amount; i++) {
                ...
                for (int line_wrap_index = 0;
                    ...
                    int indent_px     = line_wrap_index != 0
                                        ...
                                        ) * cache.font->get_char_size(' ').width
                                          : 0;
                    if (indent_px >= wrap_at) {
                        indent_px = 0;
                    }
                    ...
                    if (ofs_y + get_row_height() < top_limit_y) {
                        ...
                        // Still need to go through the process to prepare color
                        // changes for next lines.
                        clipped = true;
                    }
                    ...
                    if (!search_text.empty()) {
                        search_text_col = _get_column_pos_of_word(
                            search_text,
                            str,
                            search_flags,
                            0
                    ...
                    if (highlighted_text.length() != 0
                        && highlighted_text != search_text) {
                        highlighted_text_col = _get_column_pos_of_word(
                            highlighted_text,
                            str,
                            SEARCH_MATCH_CASE | SEARCH_WHOLE_WORDS,
                            0
                        );
                    }

                    if (select_identifiers_enabled
                        && highlighted_word.length() != 0) {
                        if (_is_char(highlighted_word[0])
                            || highlighted_word[0] == '.') {
                            highlighted_word_col = _get_column_pos_of_word(
                                highlighted_word,
                                fullstr,
                                SEARCH_MATCH_CASE | SEARCH_WHOLE_WORDS,
                                0
                    ...
                    if (str.length() == 0) {
                        // Draw line background if empty as we won't loop at at
                        // all.
                        if (line == cursor.line
                            && cursor_wrap_index == line_wrap_index
                            && highlight_current_line) {
                        ...
                        if (selection.active && line >= selection.from_line
                            && line <= selection.to_line
                            && char_margin >= xmargin_beg) {
                            ...
                            VisualServer::get_singleton()->canvas_item_add_rect(
                                ...
                                cache.selection_color
                            );
                        }
                    } else {
                        // If it has text, then draw current line marker in the
                        // margin, as line number etc will draw over it, draw
                        // the rest of line marker later.
                        if (line == cursor.line
                            && cursor_wrap_index == line_wrap_index
                            && highlight_current_line) {
                            VisualServer::get_singleton()->canvas_item_add_rect(
                    ...
                    if (line_wrap_index == 0) {
                        // Only do these if we are on the first wrapped part of
                        // a line.
                        ...
                        if (text.is_breakpoint(line)
                            ...
                            VisualServer::get_singleton()->canvas_item_add_rect(
                                ...
                                Rect2(
                                    xmargin_beg + ofs_x,
                                    ofs_y + get_row_height() - EDSCALE,
                                    xmargin_end - xmargin_beg,
                                    EDSCALE
                                ),
                                cache.breakpoint_color
                            );
#else
                            VisualServer::get_singleton()->canvas_item_add_rect(
                                ci,
                        ...
                        if (text.is_bookmark(line)) {
                            if (draw_bookmark_gutter) {
                                int vertical_gap =
                                    (get_row_height() * 40) / 100;
                                int horizontal_gap =
                                    (cache.breakpoint_gutter_width * 30) / 100;
                                int marker_radius =
                                    get_row_height() - (vertical_gap * 2);
                                VisualServer::get_singleton()
                                    ->canvas_item_add_circle(
                                        ci,
                                        Point2(
                                            cache.style_normal->get_margin(
                                                MARGIN_LEFT
                                            ) + horizontal_gap
                                                - 2 + marker_radius / 2,
                                            ofs_y + vertical_gap
                                        ...
                                        marker_radius,
                                        Color(
                                            cache.bookmark_color.r,
                                            cache.bookmark_color.g,
                                            cache.bookmark_color.b
                                        )
                                    );
                            }
                        }
                        ...
                        if (text.is_breakpoint(line)) {
                            if (draw_breakpoint_gutter) {
                                ...
                                int marker_height =
                                    get_row_height() - (vertical_gap * 2);
                                int marker_width = cache.breakpoint_gutter_width
                                                 - (horizontal_gap * 2);
                                ...
                                VisualServer::get_singleton()
                                    ->canvas_item_add_rect(
                                        ...
                                        Color(
                                            cache.breakpoint_color.r,
                                            cache.breakpoint_color.g,
                                            cache.breakpoint_color.b
                                        )
                                    );
                            }
                        }
                        ...
                        if (draw_info_gutter && text.has_info_icon(line)) {
                            ...
                            int horizontal_gap =
                                (cache.info_gutter_width * 30) / 100;
                            int gutter_left =
                                cache.style_normal->get_margin(MARGIN_LEFT)
                                + cache.breakpoint_gutter_width;
                            ...
                            if (icon_size.width
                                > cache.info_gutter_width - horizontal_gap) {
                                icon_size.width =
                                    cache.info_gutter_width - horizontal_gap;
                            }
                            if (icon_size.height
                                > get_row_height() - horizontal_gap) {
                                icon_size.height =
                                    get_row_height() - horizontal_gap;
                            }
                            ...
                            Size2i icon_pos;
                            int xofs =
                                horizontal_gap - (info_icon->get_width() / 4);
                            int yofs =
                                vertical_gap - (info_icon->get_height() / 4);
                            icon_pos.x = gutter_left + xofs + ofs_x;
                            icon_pos.y = ofs_y + yofs;

                            draw_texture_rect(
                                info_icon,
                                Rect2(icon_pos, icon_size)
                            );
                        }
                        ...
                        if (executing_line == line) {
                            if (draw_breakpoint_gutter) {
                                ...
                                int horizontal_gap =
                                    (cache.breakpoint_gutter_width * 30) / 100;
                                int marker_height = get_row_height()
                                                  - (vertical_gap * 2)
                                                  + icon_extra_size;
                                int marker_width = cache.breakpoint_gutter_width
                                                 - (horizontal_gap * 2)
                                ...
                                cache.executing_icon->draw_rect(
                                    ...
                                    false,
                                    Color(
                                        cache.executing_line_color.r,
                                        cache.executing_line_color.g,
                                        cache.executing_line_color.b
                                    )
                                );
                            } else {
#ifdef TOOLS_ENABLED
                        ...
                        if (draw_fold_gutter) {
                            int horizontal_gap =
                                (cache.fold_gutter_width * 30) / 100;
                            int gutter_left =
                                cache.style_normal->get_margin(MARGIN_LEFT)
                                + cache.breakpoint_gutter_width
                                + cache.line_number_w + cache.info_gutter_width;
                            if (is_folded(line)) {
                                int xofs =
                                    horizontal_gap
                                    - (cache.can_fold_icon->get_width()) / 2;
                                int yofs = (get_row_height()
                                            - cache.folded_icon->get_height())
                                         / 2;
                                cache.folded_icon->draw(
                                    ci,
                                    Point2(
                                        gutter_left + xofs + ofs_x,
                                        ofs_y + yofs
                                    ),
                                    cache.code_folding_color
                                );
                            } else if (can_fold(line)) {
                                int xofs = -cache.can_fold_icon->get_width() / 2
                                         - horizontal_gap + 3;
                                int yofs = (get_row_height()
                                            - cache.can_fold_icon->get_height())
                                         / 2;
                                cache.can_fold_icon->draw(
                                    ci,
                                    Point2(
                                        gutter_left + xofs + ofs_x,
                                        ofs_y + yofs
                        ...
                        if (cache.line_number_w) {
                            int yofs =
                                ofs_y
                                + (get_row_height() - cache.font->get_height())
                                      / 2;
                            ...
                            cache.font->draw(
                                ...
                                Point2(
                                    cache.style_normal->get_margin(MARGIN_LEFT)
                                        + cache.breakpoint_gutter_width
                                        + cache.info_gutter_width + ofs_x,
                                    yofs + cache.font->get_ascent()
                                ),
                                fc,
                                text.is_safe(line)
                    ...
                    for (; j < str.length(); j++) {
                        ...
                        // Handle tabulator.
                        char_w =
                            text.get_char_width(str[j], str[j + 1], char_ofs);
                        ...
                        if (search_text_col != -1) {
                            // If we are at the end check for new search result
                            // on same line.
                            ...
                            in_search_result =
                                j >= search_text_col
                                && j < search_text_col + search_text.length();
                        ...
                        bool in_selection =
                            (selection.active && line >= selection.from_line
                             && line <= selection.to_line
                             && (line > selection.from_line
                                 || last_wrap_column + j
                                        >= selection.from_column)
                             && (line < selection.to_line
                                 || last_wrap_column + j < selection.to_column)
                            );

                        if (!clipped && line == cursor.line
                            && cursor_wrap_index == line_wrap_index
                            && highlight_current_line) {
                        ...
                        if (!clipped && in_search_result) {
                            Color border_color =
                                (line == search_result_line
                                 && j >= search_result_col
                                 && j < search_result_col + search_text.length()
                                )
                                    ? cache.font_color
                                    : cache.search_result_border_color;
                            ...
                            if (j
                                == search_text_col + search_text.length() - 1) {
                                VisualServer::get_singleton()
                                    ->canvas_item_add_rect(
                                        ci,
                                        Rect2(
                        ...
                        if (!clipped && highlight_all_occurrences
                            && !only_whitespaces_highlighted) {
                            if (highlighted_text_col != -1) {
                                // If we are at the end check for new word on
                                // same line.
                                if (j > highlighted_text_col
                                ...
                                bool in_highlighted_word =
                                    (j >= highlighted_text_col
                                     && j < highlighted_text_col
                                                + highlighted_text.length());

                                // If this is the original highlighted text we
                                // don't want to highlight it again.
                                if (cursor.line == line
                                    && cursor_wrap_index == line_wrap_index
                                    && (cursor.column >= highlighted_text_col
                                        && cursor.column
                                               <= highlighted_text_col
                                                      + highlighted_text.length(
                                                      ))) {
                        ...
                        if (highlighted_word_col != -1) {
                            if (j + last_wrap_column
                                > highlighted_word_col
                                      + highlighted_word.length()) {
                                highlighted_word_col = _get_column_pos_of_word(
                                    highlighted_word,
                                    fullstr,
                                    SEARCH_MATCH_CASE | SEARCH_WHOLE_WORDS,
                            ...
                            underlined =
                                (j + last_wrap_column >= highlighted_word_col
                                 && j + last_wrap_column
                                        < highlighted_word_col
                                              + highlighted_word.length());
                        }
                        ...
                        if (brace_matching_enabled) {
                            int yofs =
                                ...
                                + (get_row_height() - cache.font->get_height())
                                      / 2;
                            if ((brace_open_match_line == line
                                 && brace_open_match_column
                                        == last_wrap_column + j)
                                || (cursor.column == last_wrap_column + j
                                    && cursor.line == line
                                    && cursor_wrap_index == line_wrap_index
                                    && (brace_open_matching
                                        || brace_open_mismatch))) {
                                ...
                                drawer.draw_char(
                                    ...
                                    in_selection && override_selected_font_color
                                        ? cache.font_color_selected
                                        : color
                                );
                            }

                            if ((brace_close_match_line == line
                                 && brace_close_match_column
                                        == last_wrap_column + j)
                                || (cursor.column == last_wrap_column + j + 1
                                    && cursor.line == line
                                    && cursor_wrap_index == line_wrap_index
                                    && (brace_close_matching
                                        || brace_close_mismatch))) {
                        ...
                        if (!clipped && cursor.column == last_wrap_column + j
                            && cursor.line == line
                            && cursor_wrap_index == line_wrap_index) {
                            is_cursor_visible = true;
                            cursor_pos =
                                Point2i(char_ofs + char_margin + ofs_x, ofs_y);
                            cursor_pos.y +=
                                (get_row_height() - cache.font->get_height())
                                / 2;

                            if (insert_mode) {
                                cursor_insert_offset_y =
                                    (cache.font->get_height() - 3);
                            ...
                            if (ime_text.length() > 0) {
                                ...
                                while (true) {
                                    ...
                                    int im_char_width =
                                        cache.font->get_char_size(cchar, next)
                                            .width;

                                    if ((char_ofs + char_margin + im_char_width)
                                        >= xmargin_end) {
                                        break;
                                    }

                                    bool selected = ofs >= ime_selection.x
                                                 && ofs < ime_selection.x
                                                              + ime_selection.y;
                                    if (selected) {
                                        VisualServer::get_singleton()
                                    ...
                                    if (insert_mode) {
                                        ...
                                        VisualServer::get_singleton()
                                            ->canvas_item_add_rect(
                                                ...
                                                Rect2(
                                                    cursor_pos,
                                                    Size2i(caret_w, caret_h)
                                                ),
                                                cache.caret_color
                                            );
                                    } else {
#ifdef TOOLS_ENABLED
                                        caret_w = (block_caret) ? caret_w
                                                                : 2 * EDSCALE;
#else
                        ...
                        if (!clipped) {
                            if (cursor.column == last_wrap_column + j
                                && cursor.line == line
                                && cursor_wrap_index == line_wrap_index
                                && block_caret && draw_caret && !insert_mode) {
                                color = cache.caret_background_color;
                            } else if (!syntax_coloring && block_caret) {
                                color = readonly ? cache.font_color_readonly
                                                 : cache.font_color;
                            }
                            ...
                            if (draw_spaces && str[j] == ' ') {
                                int yofs = (get_row_height()
                                            - cache.space_icon->get_height())
                                         / 2;
                                cache.space_icon->draw(
                                    ci,
                                    Point2(
                                        char_ofs + char_margin + ofs_x,
                                        ofs_y + yofs
                        ...
                        if (line_wrap_index == line_wrap_amount
                            && j == str.length() - 1 && is_folded(line)) {
                            int yofs = (get_row_height()
                                        - cache.folded_eol_icon->get_height())
                                     / 2;
                            int xofs = cache.folded_eol_icon->get_width() / 2;
                            Color eol_color = cache.code_folding_color;
                            eol_color.a     = 1;
                            cache.folded_eol_icon->draw(
                                ci,
                                Point2(
                                    char_ofs + char_margin + xofs + ofs_x,
                                    ofs_y + yofs
                                ),
                                eol_color
                            );
                    ...
                    if (!clipped && cursor.column == (last_wrap_column + j)
                        && cursor.line == line
                        && cursor_wrap_index == line_wrap_index
                        && (char_ofs + char_margin) >= xmargin_beg) {
                        is_cursor_visible = true;
                        cursor_pos =
                            Point2i(char_ofs + char_margin + ofs_x, ofs_y);
                        cursor_pos.y +=
                            (get_row_height() - cache.font->get_height()) / 2;
                        ...
                        if (ime_text.length() > 0) {
                            ...
                            while (true) {
                                ...
                                bool selected =
                                    ofs >= ime_selection.x
                                    && ofs < ime_selection.x + ime_selection.y;
                                if (selected) {
                                    VisualServer::get_singleton()
                                        ->canvas_item_add_rect(
                                            ci,
                    ...
                    cache_entry.first_visible_char.push_back(
                        wrap_column_offset + first_visible_char
                    );
                    cache_entry.last_visible_char.push_back(
                        wrap_column_offset + last_visible_char
                    );
            ...
            if (completion_active && is_cursor_visible
                && completion_options.size() > 0) {
                // Completion panel

                const Ref<StyleBox> csb = get_stylebox("completion");
                const int maxlines      = get_constant("completion_lines");
                const int cmax_width    = get_constant("completion_max_width")
                                     * cache.font->get_char_size('x').x;
                ...
                const int completion_base_width =
                    cache.font->get_string_size(completion_base).width;

                int scroll_rectangle_width =
                    get_constant("completion_scroll_width");
                int width = 0;

                // Compute max width of the panel based on the longest
                // completion option. Limit the number of results for automatic
                // width calculation to avoid freezing while showing results.
                if (completion_options_size < 1000) {
                    for (int i = 0; i < completion_options_size; i++) {
                        int line_width = MIN(
                            cache.font
                                ->get_string_size(completion_options[i].display)
                                .x,
                            cmax_width
                        );
                        if (line_width > width) {
                ...
                const int line_from = CLAMP(
                    completion_index - row_count / 2,
                    0,
                    completion_options_size - row_count
                );

                for (int i = 0; i < row_count; i++) {
                    int l = line_from + i;
                    ERR_CONTINUE(l < 0 || l >= completion_options_size);
                    if (completion_options[l].default_value.get_type()
                        == Variant::COLOR) {
                ...
                const int total_height =
                    completion_rect.size.height + csb->get_minimum_size().y;
                const int ajdusted_cursor_y =
                    cursor_pos.y - cursor_insert_offset_y
                    - (get_row_height() - cache.font->get_height()) / 2;

                completion_rect.position.x = cursor_pos.x
                                           - completion_base_width
                                           - icon_area_width - csb_offset.x;

                if (ajdusted_cursor_y + row_height + total_height
                        > get_size().height
                    && ajdusted_cursor_y > total_height) {
                    // Completion panel above the cursor line
                    completion_rect.position.y =
                        ajdusted_cursor_y - total_height;
                } else {
                ...
                draw_style_box(
                    csb,
                    Rect2(
                        completion_rect.position - csb_offset,
                        completion_rect.size + csb->get_minimum_size()
                ...
                VisualServer::get_singleton()->canvas_item_add_rect(
                    ...
                    Rect2(
                        Point2(
                            completion_rect.position.x,
                            completion_rect.position.y
                                + (completion_index - line_from)
                                      * get_row_height()
                        ),
                        Size2(completion_rect.size.width, get_row_height())
                    ),
                    cache.completion_selected_color
                );
                draw_rect(
                    Rect2(
                        completion_rect.position
                            + Vector2(icon_area_size.x + icon_hsep, 0),
                        Size2(
                            MIN(completion_base_width,
                                completion_rect.size.width
                                    - (icon_area_size.x + icon_hsep)),
                            completion_rect.size.height
                        )
                    ),
                ...
                for (int i = 0; i < row_count; i++) {
                    ...
                    for (int j = 0; j < color_regions.size(); j++) {
                        if (completion_options[l].insert_text.begins_with(
                                color_regions[j].begin_key
                            )) {
                    ...
                    Point2 title_pos(
                        completion_rect.position.x,
                        completion_rect.position.y + i * get_row_height()
                            + cache.font->get_ascent() + yofs
                    );
                    ...
                    if (icon.is_valid()) {
                        ...
                        real_t scale =
                            MIN(side / icon->get_width(),
                                side / icon->get_height());
                        Size2 icon_size = icon->get_size() * scale;
                        draw_texture_rect(
                            icon,
                            Rect2(
                                icon_area.position
                                    + (icon_area.size - icon_size) / 2,
                                icon_size
                            )
                        );
                    }
                    ...
                    title_pos.x =
                        icon_area.position.x + icon_area.size.width + icon_hsep;
                    ...
                    if (completion_options[l].default_value.get_type()
                        ...
                        draw_rect(
                            ...
                            (Color)completion_options[l].default_value
                        );
                    }
                ...
                if (scroll_rectangle_width) {
                    // Draw a small scroll rectangle to show a position in the
                    // options.
                    ...
                    draw_rect(
                        Rect2(
                            completion_rect.position.x
                                + completion_rect.size.width,
                            completion_rect.position.y
                                + o * completion_rect.size.y,
                            scroll_rectangle_width,
                            completion_rect.size.y * r
                        ),
                        scrollc
                    );
                }
            ...
            if (show_hint) {
                    ...
                    if (i == 0) {
                        offset =
                            font->get_string_size(
                                    l.substr(0, l.find(String::chr(0xFFFF)))
                            )
                                .x;
                    } else {
                ...
                for (int i = 0; i < sc; i++) {
                    ...
                    if (l.find(String::chr(0xFFFF)) != -1) {
                        begin = font->get_string_size(
                                        l.substr(0, l.find(String::chr(0xFFFF)))
                        )
                                    .x;
                        end = font->get_string_size(
                                      l.substr(0, l.rfind(String::chr(0xFFFF)))
                        )
                                  .x;
                    }

                    Point2 round_ofs =
                        hint_ofs + sb->get_offset()
                        + Vector2(
                            0,
                            font->get_ascent() + font->get_height() * i
                                + spacing
                        );
                    ...
                    if (end > 0) {
                        Vector2 b =
                            ...
                            + Vector2(
                                begin,
                                font->get_height() + font->get_height() * i
                                    + spacing - 1
                            );
            ...
            if (has_focus()) {
                ...
                OS::get_singleton()->set_ime_position(
                    get_global_position() + cursor_pos
                    + Point2(0, get_row_height())
                );
            }
        } break;
        case NOTIFICATION_FOCUS_ENTER: {
            ...
            OS::get_singleton()->set_ime_active(true);
            Point2 cursor_pos = Point2(cursor_get_column(), cursor_get_line())
                              * get_row_height();
            OS::get_singleton()->set_ime_position(
                get_global_position() + cursor_pos
            );
            ...
            if (OS::get_singleton()->has_virtual_keyboard()
                ...
                if (!selection.active) {
                    String full_text =
                        _base_get_text(0, 0, cursor.line, cursor.column);

                    cursor_start = full_text.length();
                } else {
                    String pre_text = _base_get_text(
                        0,
                        0,
                        selection.from_line,
                        selection.from_column
                    );
                    String post_text = _base_get_text(
                        selection.from_line,
                        selection.from_column,
                        selection.to_line,
                        selection.to_column
...
void TextEdit::_consume_backspace_for_pair_symbol(
    int prev_line,
    int prev_column
) {
...
void TextEdit::backspace_at_cursor() {
    ...
    int prev_column =
        cursor.column ? (cursor.column - 1) : (text[cursor.line - 1].length());
    ...
    if (text.has_info_icon(cursor.line)) {
        set_line_info_icon(
            prev_line,
            text.get_info_icon(cursor.line),
            text.get_info(cursor.line)
        );
    }

    if (auto_brace_completion_enabled && cursor.column > 0
        && _is_pair_left_symbol(text[cursor.line][cursor.column - 1])) {
        _consume_backspace_for_pair_symbol(prev_line, prev_column);
    } else {
        // Handle space indentation.
        if (cursor.column != 0 && indent_using_spaces) {
            // Check if there are no other chars before cursor, just
            // indentation.
            bool unindent = true;
            int i         = 0;
            ...
            if (unindent) {
                // We want to remove spaces up to closest indent, or whole
                // indent if cursor is pointing at it.
                int spaces_to_delete =
                    _calculate_spaces_till_next_left_indent(cursor.column);
...
void TextEdit::indent_right() {
    ...
    // This value informs us by how much we changed selection position by
    // indenting right. Default is 1 for tab indentation.
    ...
    for (int i = start_line; i <= end_line; i++) {
        ...
        if (indent_using_spaces) {
            // We don't really care where selection is - we just need to know
            // indentation level at the beginning of the line.
            ...
            // Since we will add this much spaces we want move whole selection
            // and cursor by this much.
    ...
    if (is_selection_active()) {
        select(
            selection.from_line,
            selection.from_column + selection_offset,
            selection.to_line,
            selection.to_column + selection_offset
        );
    }
...
void TextEdit::indent_left() {
    ...
    // changing content of line can move cursor and selection on it's own (if
    // new line ends before previous position of either), therefore we just
    // remember initial values and at the end of the operation offset them by
    ...
    for (int i = start_line; i <= end_line; i++) {
        ...
        if (line_text.begins_with("\t")) {
            ...
            // When unindenting we aim to remove spaces before line that has
            // selection no matter what is selected, so we start of by finding
            // first non whitespace character of line
            int left = _find_first_non_whitespace_column_of_line(line_text);

            // Here we remove only enough spaces to align text to nearest full
            // multiple of indentation_size. In case where selection begins at
            // the start of indentation_size multiple we remove whole
            // indentation level.
            int spaces_to_remove =
                _calculate_spaces_till_next_left_indent(left);
    ...
    if (is_selection_active()) {
        ...
        if (first_line_text != get_line(start_line)) {
            select(
                selection.from_line,
                selection.from_column - removed_characters,
                selection.to_line,
...
void TextEdit::_get_mouse_pos(const Point2i& p_mouse, int& r_row, int& r_col)
    const {
    ...
    if (row >= text.size()) {
        ...
        int colx = p_mouse.x
                 - (cache.style_normal->get_margin(MARGIN_LEFT)
                    + cache.line_number_w + cache.breakpoint_gutter_width
                    + cache.fold_gutter_width + cache.info_gutter_width);
        colx += cursor.x_ofs;
...
Vector2i TextEdit::_get_cursor_pixel_pos() {
    ...
    int y =
        (row - get_v_scroll_offset() + 1 /*Bottom of line*/) * get_row_height();
    int x = cache.style_normal->get_margin(MARGIN_LEFT) + cache.line_number_w
          + cache.breakpoint_gutter_width + cache.fold_gutter_width
          + cache.info_gutter_width - cursor.x_ofs;
    int ix = 0;
...
void TextEdit::_get_minimap_mouse_row(const Point2i& p_mouse, int& r_row)
    const {
    float rows  = p_mouse.y;
    rows       -= cache.style_normal->get_margin(MARGIN_TOP);
    ...
    int control_height  = _get_control_height() - viewport_height;
    int viewport_offset_y =
        round(get_scroll_pos_for_line(first_visible_line) * control_height)
        / ((v_scroll->get_max() <= minimap_visible_lines)
               ? (minimap_visible_lines - draw_amount)
               : (v_scroll->get_max() - draw_amount));
    ...
    int wi;
    int minimap_line = (v_scroll->get_max() <= minimap_visible_lines)
                         ? -1
                         : first_visible_line;
    if (first_visible_line > 0 && minimap_line >= 0) {
        minimap_line -=
            num_lines_from_rows(first_visible_line, 0, -num_lines_before, wi);
...
void TextEdit::_gui_input(const Ref<InputEvent>& p_gui_input) {
    ...
    if (mb.is_valid()) {
        if (completion_active
            && completion_rect.has_point(mb->get_position())) {
            if (!mb->is_pressed()) {
                return;
            }
            ...
            if (mb->get_button_index() == BUTTON_LEFT) {
                completion_index = CLAMP(
                    completion_line_ofs
                        + (mb->get_position().y - completion_rect.position.y)
                              / get_row_height(),
                    0,
                    completion_options.size() - 1
                );
        ...
        if (mb->is_pressed()) {
            if (mb->get_button_index() == BUTTON_WHEEL_UP
                ...
                if (mb->get_shift()) {
                    h_scroll->set_value(
                        h_scroll->get_value() - (100 * mb->get_factor())
                    );
                } else if (mb->get_alt()) {
                    // Scroll 5 times as fast as normal (like in Visual Studio
                    // Code).
            ...
            if (mb->get_button_index() == BUTTON_LEFT) {
                ...
                int row, col;
                _get_mouse_pos(
                    Point2i(mb->get_position().x, mb->get_position().y),
                    row,
                    col
                );
                ...
                if (draw_breakpoint_gutter) {
                    ...
                    if (mb->get_position().x > gutter - 6
                        && mb->get_position().x
                               <= gutter + cache.breakpoint_gutter_width - 3) {
                        set_line_as_breakpoint(
                            row,
                            !is_line_set_as_breakpoint(row)
                        );
                ...
                if (draw_info_gutter && text.has_info_icon(row)) {
                    int left_margin =
                        cache.style_normal->get_margin(MARGIN_LEFT);
                    int gutter_left =
                        left_margin + cache.breakpoint_gutter_width;
                    if (mb->get_position().x > gutter_left - 6
                        && mb->get_position().x
                               <= gutter_left + cache.info_gutter_width - 3) {
                ...
                if (draw_fold_gutter) {
                    ...
                    if (mb->get_position().x > gutter_left - 6
                        && mb->get_position().x
                               <= gutter_left + cache.fold_gutter_width - 3) {
                        if (is_folded(row)) {
                            unfold_line(row);
                ...
                if (is_folded(row)) {
                    ...
                    line_width += cache.style_normal->get_margin(MARGIN_LEFT)
                                + cache.line_number_w
                                + cache.breakpoint_gutter_width
                                + cache.info_gutter_width
                                + cache.fold_gutter_width - cursor.x_ofs;
                    if (mb->get_position().x > line_width - 3
                        && mb->get_position().x
                               <= line_width
                                      + cache.folded_eol_icon->get_width()
                                      + 3) {
                        unfold_line(row);
                        return;
                    }
                ...
                if (mb->get_shift()
                    && (cursor.column != prev_col || cursor.line != prev_line
                    )) {
                    if (!selection.active) {
                        ...
                        if (selection.from_line > selection.to_line
                            || (selection.from_line == selection.to_line
                                && selection.from_column > selection.to_column
                            )) {
                        ...
                        if (cursor.line < selection.selecting_line
                            || (cursor.line == selection.selecting_line
                                && cursor.column < selection.selecting_column
                            )) {
                ...
                if (!mb->is_doubleclick()
                    && (OS::get_singleton()->get_ticks_msec() - last_dblclk)
                           < triple_click_timeout
                    && mb->get_position().distance_to(last_dblclk_pos)
            ...
            if (mb->get_button_index() == BUTTON_RIGHT
                ...
                if (!readonly) {
                    menu->set_item_disabled(
                        menu->get_item_index(MENU_UNDO),
                        !has_undo()
                    );
                    menu->set_item_disabled(
                        menu->get_item_index(MENU_REDO),
                        !has_redo()
                    );
                }
    ...
    if (pan_gesture.is_valid()) {
        ...
        h_scroll->set_value(
            h_scroll->get_value() + pan_gesture->get_delta().x * 100
        );
        if (v_scroll->get_value() != prev_v_scroll
            || h_scroll->get_value() != prev_h_scroll) {
    ...
    if (mm.is_valid()) {
        if (select_identifiers_enabled) {
            if (!dragging_minimap && !dragging_selection && mm->get_command()
                && mm->get_button_mask() == 0) {
    ...
    if (k.is_valid()) {
...
#endif
            if (select_identifiers_enabled) {
                if (k->is_pressed() && !dragging_minimap
                    && !dragging_selection) {
                    highlighted_word =
                        get_word_at_pos(get_local_mouse_position());
                    update();
            ...
            if (valid) {
                if (!k->get_alt()) {
                    if (k->get_scancode() == KEY_UP) {
                        ...
                        completion_current =
                            completion_options[completion_index];
                        update();
                    ...
                    if (k->get_scancode() == KEY_KP_ENTER
                        || k->get_scancode() == KEY_ENTER
                        || k->get_scancode() == KEY_TAB) {
                        _confirm_completion();
                        accept_event();
                        return;
                    }
        ...
        if (k->get_command() && !k->get_shift() && !k->get_alt()
            && !k->get_metakey() && k->get_scancode() == KEY_INSERT) {
        ...
        // Save here for insert mode as well as arrow navigation, just in case
        // it is cleared in the following section.
        ...
        if (!readonly && selection.active) {
            ...
            switch (k->get_scancode()) {
                ...
                case KEY_C:
                    ...
                    clear =
                        (!k->get_command() || k->get_shift() || k->get_alt());
                    break;
                case KEY_DELETE:
                    if (!k->get_shift()) {
                        accept_event();
                ...
                case KEY_HOME:
                case KEY_END:
                    // Ignore arrows if any modifiers are held (shift =
                    // selecting, others may be used for editor hotkeys).
                    if (k->get_command() || k->get_shift() || k->get_alt()) {
                        break;
                    }
                    unselect = true;
                ...
                default:
                    ...
                    if (auto_brace_completion_enabled
                        && _is_pair_left_symbol(k->get_unicode())) {
                        clear = false;
                    }
            }
        ...
        switch (k->get_scancode()) {
            ...
            case KEY_ENTER: {
                ...
                if (auto_indent && !(k->get_command() && k->get_shift())) {
                    ...
                    // ':','{','[','(' and the line is not a comment (i.e.
                    // colon/brace precedes current cursor position).
                    if (cursor.column > 0) {
                        const Map<int, Text::ColorRegionInfo>& cri_map =
                            text.get_color_region_info(cursor.line);
                        ...
                        for (int i = 0; i < cursor.column; i++) {
                            ...
                            if (indent_char_found && cri_map.has(i)
                                && (color_regions[cri_map[i].region].begin_key
                                        == "#"
                                    || color_regions[cri_map[i].region]
                                               .begin_key
                                           == "//")) {
                                should_indent = true;
                                break;
                        ...
                        if (!is_line_comment(cursor.line) && should_indent) {
                            ...
                            // No need to move the brace below if we are not
                            // taking the text with us.
                            char closing_char =
                                _get_right_pair_symbol(indent_char);
                            if ((closing_char != 0)
                                && (closing_char
            ...
            case KEY_TAB: {
                ...
                if (is_selection_active()) {
                    ...
                    if (k->get_shift()) {
                        ...
                        int left =
                            _find_first_non_whitespace_column_of_line(line);
                        cc = MIN(cc, left);
                        ...
                        if (cc > 0 && cc <= text[cursor.line].length()) {
                            if (text[cursor.line][cc - 1] == '\t') {
                                ...
                                int spaces_to_remove =
                                    _calculate_spaces_till_next_left_indent(cc);
                                if (spaces_to_remove > 0) {
                                    ...
                                    if (cursor.column
                                        > left - spaces_to_remove) { // Inside
                                                                     // text?
                                        cursor_set_column(MAX(
                                            0,
                                            cursor.column - spaces_to_remove
                                        ));
                                    }
                                    update();
                                }
                        ...
                        // Simple indent.
                        if (indent_using_spaces) {
                            // Insert only as much spaces as needed till next
                            // indentation level.
                            int spaces_to_add =
                                _calculate_spaces_till_next_right_indent(
                                    cursor.column
                                );
            ...
            case KEY_BACKSPACE: {
                ...
                if (k->get_alt()) {
                    ...
                    // Check if we are removing a single whitespace, if so
                    // remove it and the next char type, else we just remove the
                    // whitespace.
                    bool only_whitespace = false;
                    if (_is_whitespace(text[line][column - 1])
                        && _is_whitespace(text[line][column - 2])) {
                    ...
                    bool only_char =
                        (_is_text_char(text[line][column - 1])
                         && !only_whitespace);
                    ...
                    while (column > 0) {
                        bool is_whitespace =
                            _is_whitespace(text[line][column - 1]);
                        bool is_text_char =
                            _is_text_char(text[line][column - 1]);
            ...
            case KEY_LEFT: {
...
#ifdef APPLE_STYLE_KEYS
                if (k->get_command()) {
                    // Start at first column (it's slightly faster that way) and
                    ...
                    if (new_cursor_pos == cursor.column) {
                        // We're already at the first text character, so move to
                        // the very beginning of the line.
                        cursor_set_column(0);
                    } else {
                        // We're somewhere to the right of the first text
                        // character; move to the first one.
                ...
                if (k->get_alt()) {
                    ...
                    if (cc == 0 && cursor.line > 0) {
                        ...
                        while (cc > 0) {
                            bool ischar =
                                _is_text_char(text[cursor.line][cc - 1]);
                    ...
                    if (cursor.line > 0) {
                        cursor_set_line(
                            cursor.line
                            - num_lines_from(
                                CLAMP(cursor.line - 1, 0, text.size() - 1),
                                -1
                            )
                        );
                        cursor_set_column(text[cursor.line].length());
            ...
            case KEY_UP: {
...
#endif
                {
                    ...
                    if (cur_wrap_index > 0) {
                        ...
                        int new_line =
                            cursor.line - num_lines_from(cursor.line - 1, -1);
                        if (line_wraps(new_line)) {
                            cursor_set_line(
                                new_line,
                                true,
                                false,
                                times_line_wraps(new_line)
                            );
                        } else {
            ...
            case KEY_DELETE: {
                ...
                if (cursor.line == text.size() - 1
                    && cursor.column == curline_len) {
                    break; // Nothing to do.
                }

                int next_line =
                    cursor.column < curline_len ? cursor.line : cursor.line + 1;
                int next_column;
                ...
                if (k->get_alt()) {
                    ...
                    // whitespace.
                    bool only_whitespace = false;
                    if (_is_whitespace(text[line][column])
                        && _is_whitespace(text[line][column + 1])) {
                        only_whitespace = true;
                    ...
                    // Check if its a text char.
                    bool only_char =
                        (_is_text_char(text[line][column]) && !only_whitespace);
                    ...
                    next_column =
                        cursor.column < curline_len ? (cursor.column + 1) : 0;
                }
            ...
            case KEY_HOME: {
                ...
                if (k->get_command()) {
                    ...
                    // Move cursor column to start of wrapped row and then to
                    // start of text.
                    ...
                    if (cursor.column == row_start_col || wi == 0) {
                        ...
                        while (current_line_whitespace_len
                               < text[cursor.line].length()) {
                            CharType c =
                                text[cursor.line][current_line_whitespace_len];
                            if (c != '\t' && c != ' ') {
                                break;
                            }
            ...
            case KEY_END: {
                ...
                // Move cursor column to end of wrapped row and then to end of
                // text.
                Vector<String> rows = get_wrap_rows_text(cursor.line);
                int wi              = get_cursor_wrap_index();
                int row_end_col     = -1;
            ...
            case KEY_PAGEUP: {
                ...
                int wi;
                int n_line = cursor.line
                           - num_lines_from_rows(
                                 cursor.line,
                                 get_cursor_wrap_index(),
                                 -get_visible_rows(),
                                 wi
                           )
                           + 1;
            ...
            case KEY_MENU: {
                if (context_menu_enabled) {
                    ...
                    menu->set_position(
                        get_global_transform().xform(_get_cursor_pixel_pos())
                    );
                    menu->set_size(Vector2(1, 1));
                    menu->set_scale(get_global_transform().get_scale());
                    menu->popup();
...
void TextEdit::_scroll_up(real_t p_delta) {
    if (scrolling && smooth_scroll_enabled
        && SGN(target_v_scroll - v_scroll->get_value()) != SGN(-p_delta)) {
        scrolling       = false;
        minimap_clicked = false;
    }
...
void TextEdit::_base_insert_text(
    ...
    if (shift_first_line) {
        ...
        if (text.has_info_icon(p_line)) {
            text.set_info_icon(
                p_line + 1,
                text.get_info_icon(p_line),
                text.get_info(p_line)
            );
        }
...
String TextEdit::_base_get_text(
    ...
    ERR_FAIL_COND_V(
        p_to_line == p_from_line && p_to_column < p_from_column,
        String()
    ); // 'from > to'.
...
void TextEdit::_base_remove_text(
    int p_from_line,
    int p_from_column,
    int p_to_line,
    int p_to_column
    ...
    String post_text =
        text[p_to_line].substr(p_to_column, text[p_to_line].length());
    ...
    for (int i = p_from_line + 1; i < text.size(); i++) {
        if (text.is_breakpoint(i)) {
            ...
            if (i > p_to_line
                && (i - lines < 0 || !text.is_breakpoint(i - lines))) {
                emit_signal("breakpoint_toggled", i - lines);
            }
        }
    }
...
void TextEdit::_remove_text(
    ...
    if (undo_enabled) {
        _clear_redo();
        text =
            _base_get_text(p_from_line, p_from_column, p_to_line, p_to_column);
    }
    ...
    // See if it can be merged.
    if (current_op.from_line == p_to_line
        && current_op.from_column == p_to_column) {
...
void TextEdit::_generate_context_menu() {
    ...
    if (!readonly) {
        menu->add_item(
            RTR("Cut"),
            MENU_CUT,
            is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_X : 0
        );
    }
    menu->add_item(
        RTR("Copy"),
        MENU_COPY,
        is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_C : 0
    );
    if (!readonly) {
        menu->add_item(
            RTR("Paste"),
            MENU_PASTE,
            is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_V : 0
        );
    }
    menu->add_separator();
    if (is_selecting_enabled()) {
        menu->add_item(
            RTR("Select All"),
            MENU_SELECT_ALL,
            is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_A : 0
        );
    }
    if (!readonly) {
        ...
        menu->add_item(
            RTR("Undo"),
            MENU_UNDO,
            is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_Z : 0
        );
        menu->add_item(
            RTR("Redo"),
            MENU_REDO,
            is_shortcut_keys_enabled() ? KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_Z
                                       : 0
        );
    }
}
...
void TextEdit::_update_wrap_at() {
    wrap_at = get_size().width - cache.style_normal->get_minimum_size().width
            - cache.line_number_w - cache.breakpoint_gutter_width
            - cache.fold_gutter_width - cache.info_gutter_width
            - cache.minimap_width - wrap_right_offset;
...
void TextEdit::adjust_viewport_to_cursor() {
    ...
    int visible_width = get_size().width
                      - cache.style_normal->get_minimum_size().width
                      - cache.line_number_w - cache.breakpoint_gutter_width
                      - cache.fold_gutter_width - cache.info_gutter_width
                      - cache.minimap_width;
...
void TextEdit::update_cursor_wrap_offset() {
    ...
    if (line_wraps(first_vis_line)) {
        cursor.wrap_ofs =
            MIN(cursor.wrap_ofs, times_line_wraps(first_vis_line));
    } else {
        cursor.wrap_ofs = 0;
    }
...
Vector<String> TextEdit::get_wrap_rows_text(int p_line) const {
    ...
    int tab_offset_px =
        get_indent_level(p_line) * cache.font->get_char_size(' ').width;
...
void TextEdit::cursor_set_column(int p_col, bool p_adjust_viewport) {
    ...
    cursor.last_fit_x =
        get_column_x_offset_for_line(cursor.column, cursor.line);
...
void TextEdit::cursor_set_line(
    int p_row,
    bool p_adjust_viewport,
    bool p_can_be_hidden,
    int p_wrap_index
) {
    if (setting_row) {
        return;
    ...
    if (!p_can_be_hidden) {
        if (is_line_hidden(CLAMP(p_row, 0, text.size() - 1))) {
            ...
            if (p_row + move_down <= text.size() - 1
                && !is_line_hidden(p_row + move_down)) {
                p_row += move_down;
            } else {
                ...
                if (p_row - move_up > 0 && !is_line_hidden(p_row - move_up)) {
                    ...
                    WARN_PRINT(
                        ("Cursor set to hidden line " + itos(p_row)
    ...
    if (n_col != 0 && is_wrap_enabled()
        && p_wrap_index < times_line_wraps(p_row)) {
...
Rect2 TextEdit::get_rect_at_line_column(int p_line, int p_column) const {
    ...
    if (line_drawing_cache.size() == 0 || !line_drawing_cache.has(p_line)) {
        // Line is not in the cache, which means it's outside of the viewing
        // area.
    ...
    if (p_column < first_visible_char || p_column > last_visible_char) {
        // Character is outside of the viewing area, no point calculating its
        // position.
        return Rect2i(-1, -1, 0, 0);
    }
    ...
    pos.x = get_total_gutter_width()
          + cache.style_normal->get_margin(MARGIN_LEFT) - get_h_scroll();
...
int TextEdit::get_char_pos_for_line(int p_px, int p_line, int p_wrap_index)
    const {
    ERR_FAIL_INDEX_V(p_line, text.size(), 0);
...
Input::CursorType TextEdit::get_cursor_type(const Point2& p_pos) const {
    ...
    if (p_pos.x < gutter) {
        ...
        // Fold icon.
        if (draw_fold_gutter && p_pos.x > gutter_left + cache.line_number_w - 6
...
void TextEdit::set_text(String p_text) {
    ...
    if (undo_enabled) {
        ...
        _remove_text(
            0,
            0,
            MAX(0, get_line_count() - 1),
            MAX(get_line(MAX(get_line_count() - 1, 0)).size() - 1, 0)
        );
        _insert_text_at_cursor(p_text);
        end_complex_operation();
        selection.active = false;
...
String TextEdit::get_text_for_lookup_completion() {
    ...
    for (int i = 0; i < len; i++) {
        if (i == row) {
            ...
            longthing +=
                String::chr(0xFFFF); // Not unicode, represents the cursor.
...
void TextEdit::set_readonly(bool p_readonly) {
    ...
    if (!readonly) {
        ...
        menu->add_item(
            RTR("Redo"),
            MENU_REDO,
            KEY_MASK_CMD | KEY_MASK_SHIFT | KEY_Z
        );
    }
...
void TextEdit::_update_caches() {
    ...
    cache.completion_background_color =
        get_color("completion_background_color");
    ...
    cache.line_length_guideline_color =
        get_color("line_length_guideline_color");
...
int TextEdit::_is_line_in_region(int p_line) {
    ...
    for (int i = previous_line; i < p_line; i++) {
        const Map<int, Text::ColorRegionInfo>& cri_map =
            _get_line_color_region_info(i);
        for (const Map<int, Text::ColorRegionInfo>::Element* E =
                 cri_map.front();
             E;
             E = E->next()) {
...
Map<int, TextEdit::Text::ColorRegionInfo> TextEdit::_get_line_color_region_info(
    int p_line
) const {
...
void TextEdit::add_keyword_color(
...
void TextEdit::add_color_region(
    const String& p_begin_key,
    const String& p_end_key,
    const Color& p_color,
    bool p_line_only
) {
    color_regions.push_back(
        ColorRegion(p_begin_key, p_end_key, p_color, p_line_only)
    );
    syntax_highlighting_cache.clear();
    text.clear_width_cache();
    update();
...
void TextEdit::cut() {
    if (!selection.active) {
        ...
        String clipboard = _base_get_text(
            selection.from_line,
            selection.from_column,
            selection.to_line,
            selection.to_column
        ...
        cursor_set_line(
            selection.from_line,
            false
        ); // Set afterwards else it causes the view to be offset.
        cursor_set_column(selection.from_column);
...
bool TextEdit::search(
    ...
    for (int i = 0; i < text.size() + 1; i++) {
        ...
        int pos_from =
            (p_search_flags & SEARCH_BACKWARDS) ? text_line.length() : 0;
        int last_pos = -1;

        while (true) {
            if (p_search_flags & SEARCH_BACKWARDS) {
                while ((last_pos = (p_search_flags & SEARCH_MATCH_CASE)
                                     ? text_line.rfind(p_key, pos_from)
                                     : text_line.rfindn(p_key, pos_from))
                       != -1) {
                ...
                while ((last_pos = (p_search_flags & SEARCH_MATCH_CASE)
                                     ? text_line.find(p_key, pos_from)
                                     : text_line.findn(p_key, pos_from))
                       != -1) {
                    if (last_pos >= from_column) {
                        pos = last_pos;
                        break;
...
void TextEdit::remove_breakpoints() {
    for (int i = 0; i < text.size(); i++) {
        if (text.is_breakpoint(i)) {
            /* Should "breakpoint_toggled" be fired when breakpoints are removed
             * this way? */
...
void TextEdit::set_line_info_icon(
    int p_line,
    Ref<Texture> p_icon,
    String p_info
) {
...
int TextEdit::num_lines_from(int p_line_from, int visible_amount) const {
    // Returns the number of lines (hidden and unhidden) from p_line_from to
    // (p_line_from + visible_amount of unhidden lines).
...
int TextEdit::num_lines_from_rows(
    int p_line_from,
    int p_wrap_index_from,
    int visible_amount,
    int& wrap_index
) const {
    // Returns the number of lines (hidden and unhidden) from (p_line_from +
    // p_wrap_index_from) row to (p_line_from + visible_amount of unhidden and
    // wrapped rows). Wrap index is set to the wrap index of the last line.
    wrap_index = 0;
    ERR_FAIL_INDEX_V(p_line_from, text.size(), ABS(visible_amount));
    ...
    if (visible_amount == 0) {
        ...
        wrap_index = times_line_wraps(MIN(i, text.size() - 1))
                   - (num_visible - visible_amount);
    } else {
        visible_amount = ABS(visible_amount);
        int i;
...
bool TextEdit::is_line_comment(int p_line) const {
    ...
    const Map<int, Text::ColorRegionInfo>& cri_map =
        text.get_color_region_info(p_line);

    int line_length = text[p_line].size();
    for (int i = 0; i < line_length - 1; i++) {
        if (_is_symbol(text[p_line][i]) && cri_map.has(i)) {
            const Text::ColorRegionInfo& cri = cri_map[i];
            return color_regions[cri.region].begin_key == "#"
                || color_regions[cri.region].begin_key == "//";
...
void TextEdit::fold_line(int p_line) {
    ...
    // Fix selection.
    if (is_selection_active()) {
        if (is_line_hidden(selection.from_line)
            && is_line_hidden(selection.to_line)) {
            deselect();
...
void TextEdit::undo() {
    ...
    _do_text_op(op, true);
    if (op.type != TextOperation::TYPE_INSERT
        && (op.from_line != op.to_line || op.to_column != op.from_column + 1)) {
...
void TextEdit::set_override_selected_font_color(
...
void TextEdit::set_line_as_center_visible(int p_line, int p_wrap_index) {
    ...
    int first_line =
        p_line
        - num_lines_from_rows(p_line, p_wrap_index, -visible_rows / 2, wi) + 1;
...
void TextEdit::set_line_as_last_visible(int p_line, int p_wrap_index) {
    ...
    int first_line =
        p_line
        - num_lines_from_rows(p_line, p_wrap_index, -get_visible_rows() - 1, wi)
        + 1;

    set_v_scroll(
        get_scroll_pos_for_line(first_line, wi) + get_visible_rows_offset()
    );
}
...
void TextEdit::_confirm_completion() {
    ...
    _remove_text(
        cursor.line,
        cursor.column - completion_base.length(),
        cursor.line,
        cursor.column
    );
    ...
    // When inserted into the middle of an existing string/method, don't add an
    // unnecessary quote/bracket.
    String line        = text[cursor.line];
    CharType next_char = line[cursor.column];
    CharType last_completion_char =
        completion_current
            .insert_text[completion_current.insert_text.length() - 1];
    CharType last_completion_char_display =
        completion_current.display[completion_current.display.length() - 1];

    if ((last_completion_char == '"' || last_completion_char == '\'')
        && (last_completion_char == next_char
            || last_completion_char_display == next_char)) {
        _remove_text(
            cursor.line,
            cursor.column,
            cursor.line,
...
void TextEdit::_update_completion_candidates() {
    ...
    if (!inquote && first_quote == cofs - 1) {
        ...
        while (cofs > 0 && l[cofs - 1] > 32
               && (l[cofs - 1] == '/' || _is_completable(l[cofs - 1]))) {
    ...
    if (cursor.column > 0 && l[cursor.column - 1] == '(' && !pre_keyword
        && !completion_forced) {
        cancel = true;
    }
    ...
    if (cofs > 1 && l[cofs - 1] == ' '
        && completion_prefixes.has(String::chr(l[cofs - 2]))) {
        prev_is_prefix = true;
    }
    ...
    for (List<ScriptCodeCompletionOption>::Element* E =
             completion_sources.front();
         E;
         E = E->next()) {
...
void TextEdit::query_code_comple() {
    ...
    if (ignored) {
        ...
        ignored = ignored
               && (kind == ScriptCodeCompletionOption::KIND_FILE_PATH
...
void TextEdit::code_complete(
    const List<ScriptCodeCompletionOption>& p_strings,
    bool p_forced
) {
...
void TextEdit::set_line(int line, String new_text) {
    ...
    if (is_selection_active() && line == selection.to_line
...
int TextEdit::get_total_gutter_width() const {
    return cache.line_number_w + cache.breakpoint_gutter_width
         + cache.fold_gutter_width + cache.info_gutter_width;
}
...
void TextEdit::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("_cursor_changed_emit"),
        &TextEdit::_cursor_changed_emit
    );
    ClassDB::bind_method(
        D_METHOD("_text_changed_emit"),
        &TextEdit::_text_changed_emit
    );
    ClassDB::bind_method(
        D_METHOD("_push_current_op"),
        &TextEdit::_push_current_op
    );
    ClassDB::bind_method(
        D_METHOD("_click_selection_held"),
        &TextEdit::_click_selection_held
    );
    ClassDB::bind_method(
        D_METHOD("_toggle_draw_caret"),
        &TextEdit::_toggle_draw_caret
    );
    ClassDB::bind_method(
        D_METHOD("_v_scroll_input"),
        &TextEdit::_v_scroll_input
    );
    ClassDB::bind_method(
        D_METHOD("_update_wrap_at"),
        &TextEdit::_update_wrap_at
    );
...
*/
    ...
    ClassDB::bind_method(
        D_METHOD("insert_text_at_cursor", "text"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_line", "line", "new_text"),
        &TextEdit::set_line
    );
    ClassDB::bind_method(
        D_METHOD("get_line_wrapped_text", "line"),
        &TextEdit::get_wrap_rows_text
    );

    ClassDB::bind_method(
        D_METHOD("get_line_width", "line", "wrap_index"),
        &TextEdit::get_line_width,
        DEFVAL(-1)
    );
    ClassDB::bind_method(
        D_METHOD("get_line_height"),
        &TextEdit::get_line_height
    );

    ClassDB::bind_method(
        D_METHOD("is_line_wrapped", "line"),
        &TextEdit::line_wraps
    );
    ClassDB::bind_method(
        D_METHOD("get_line_wrap_count", "line"),
        &TextEdit::times_line_wraps
    );

    ClassDB::bind_method(
        D_METHOD("center_viewport_to_cursor"),
        &TextEdit::center_viewport_to_cursor
    );
    ClassDB::bind_method(
        D_METHOD("cursor_set_column", "column", "adjust_viewport"),
    ...
    ClassDB::bind_method(
        D_METHOD("cursor_get_column"),
        &TextEdit::cursor_get_column
    );
    ClassDB::bind_method(
        D_METHOD("cursor_get_line"),
        &TextEdit::cursor_get_line
    );
    ClassDB::bind_method(
        D_METHOD("cursor_set_blink_enabled", "enable"),
        &TextEdit::cursor_set_blink_enabled
    );
    ClassDB::bind_method(
        D_METHOD("cursor_get_blink_enabled"),
        &TextEdit::cursor_get_blink_enabled
    );
    ClassDB::bind_method(
        D_METHOD("cursor_set_blink_speed", "blink_speed"),
        &TextEdit::cursor_set_blink_speed
    );
    ClassDB::bind_method(
        D_METHOD("cursor_get_blink_speed"),
        &TextEdit::cursor_get_blink_speed
    );
    ClassDB::bind_method(
        D_METHOD("cursor_set_block_mode", "enable"),
        &TextEdit::cursor_set_block_mode
    );
    ClassDB::bind_method(
        D_METHOD("cursor_is_block_mode"),
        &TextEdit::cursor_is_block_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_right_click_moves_caret", "enable"),
        &TextEdit::set_right_click_moves_caret
    );
    ClassDB::bind_method(
        D_METHOD("is_right_click_moving_caret"),
        &TextEdit::is_right_click_moving_caret
    );

    /* Line and character position. */
    ClassDB::bind_method(
        D_METHOD("get_pos_at_line_column", "line", "column"),
        &TextEdit::get_pos_at_line_column
    );
    ClassDB::bind_method(
        D_METHOD("get_rect_at_line_column", "line", "column"),
        &TextEdit::get_rect_at_line_column
    );
    ClassDB::bind_method(
        D_METHOD("get_line_column_at_pos", "position"),
        &TextEdit::get_line_column_at_pos
    );

    ClassDB::bind_method(
        D_METHOD("set_readonly", "enable"),
        &TextEdit::set_readonly
    );
    ClassDB::bind_method(D_METHOD("is_readonly"), &TextEdit::is_readonly);

    ClassDB::bind_method(
        D_METHOD("set_wrap_enabled", "enable"),
        &TextEdit::set_wrap_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_wrap_enabled"),
        &TextEdit::is_wrap_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_context_menu_enabled", "enable"),
        &TextEdit::set_context_menu_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_context_menu_enabled"),
        &TextEdit::is_context_menu_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_shortcut_keys_enabled", "enable"),
        &TextEdit::set_shortcut_keys_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_shortcut_keys_enabled"),
        &TextEdit::is_shortcut_keys_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_virtual_keyboard_enabled", "enable"),
        &TextEdit::set_virtual_keyboard_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_virtual_keyboard_enabled"),
        &TextEdit::is_virtual_keyboard_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_selecting_enabled", "enable"),
        &TextEdit::set_selecting_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_selecting_enabled"),
        &TextEdit::is_selecting_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_line_set_as_safe", "line"),
        &TextEdit::is_line_set_as_safe
    );
    ClassDB::bind_method(
        D_METHOD("set_line_as_safe", "line", "safe"),
        &TextEdit::set_line_as_safe
    );
    ClassDB::bind_method(
        D_METHOD("is_line_set_as_bookmark", "line"),
        &TextEdit::is_line_set_as_bookmark
    );
    ClassDB::bind_method(
        D_METHOD("set_line_as_bookmark", "line", "bookmark"),
        &TextEdit::set_line_as_bookmark
    );
    ClassDB::bind_method(
        D_METHOD("set_line_as_breakpoint", "line", "breakpoint"),
        &TextEdit::set_line_as_breakpoint
    );
    ClassDB::bind_method(
        D_METHOD("is_line_set_as_breakpoint", "line"),
        &TextEdit::is_line_set_as_breakpoint
    );
    ...
    ClassDB::bind_method(
        D_METHOD("select", "from_line", "from_column", "to_line", "to_column"),
        &TextEdit::select
    );
    ...
    ClassDB::bind_method(
        D_METHOD("is_selection_active"),
        &TextEdit::is_selection_active
    );
    ClassDB::bind_method(
        D_METHOD("get_selection_from_line"),
        &TextEdit::get_selection_from_line
    );
    ClassDB::bind_method(
        D_METHOD("get_selection_from_column"),
        &TextEdit::get_selection_from_column
    );
    ClassDB::bind_method(
        D_METHOD("get_selection_to_line"),
        &TextEdit::get_selection_to_line
    );
    ClassDB::bind_method(
        D_METHOD("get_selection_to_column"),
        &TextEdit::get_selection_to_column
    );
    ClassDB::bind_method(
        D_METHOD("get_selection_text"),
        &TextEdit::get_selection_text
    );
    ClassDB::bind_method(
        D_METHOD("get_word_under_cursor"),
        &TextEdit::get_word_under_cursor
    );
    ClassDB::bind_method(
        D_METHOD("search", "key", "flags", "from_line", "from_column"),
        &TextEdit::_search_bind
    );
    ...
    ClassDB::bind_method(
        D_METHOD("clear_undo_history"),
        &TextEdit::clear_undo_history
    );

    ClassDB::bind_method(
        D_METHOD("set_show_line_numbers", "enable"),
        &TextEdit::set_show_line_numbers
    );
    ClassDB::bind_method(
        D_METHOD("is_show_line_numbers_enabled"),
        &TextEdit::is_show_line_numbers_enabled
    );
    ClassDB::bind_method(D_METHOD("set_draw_tabs"), &TextEdit::set_draw_tabs);
    ClassDB::bind_method(
        D_METHOD("is_drawing_tabs"),
        &TextEdit::is_drawing_tabs
    );
    ClassDB::bind_method(
        D_METHOD("set_draw_spaces"),
        &TextEdit::set_draw_spaces
    );
    ClassDB::bind_method(
        D_METHOD("is_drawing_spaces"),
        &TextEdit::is_drawing_spaces
    );
    ClassDB::bind_method(
        D_METHOD("set_bookmark_gutter_enabled", "enable"),
        &TextEdit::set_bookmark_gutter_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_bookmark_gutter_enabled"),
        &TextEdit::is_bookmark_gutter_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_breakpoint_gutter_enabled", "enable"),
        &TextEdit::set_breakpoint_gutter_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_breakpoint_gutter_enabled"),
        &TextEdit::is_breakpoint_gutter_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_draw_fold_gutter"),
        &TextEdit::set_draw_fold_gutter
    );
    ClassDB::bind_method(
        D_METHOD("is_drawing_fold_gutter"),
        &TextEdit::is_drawing_fold_gutter
    );
    ClassDB::bind_method(
        D_METHOD("get_total_gutter_width"),
        &TextEdit::get_total_gutter_width
    );

    ClassDB::bind_method(
        D_METHOD("set_hiding_enabled", "enable"),
        &TextEdit::set_hiding_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_hiding_enabled"),
        &TextEdit::is_hiding_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_line_as_hidden", "line", "enable"),
        &TextEdit::set_line_as_hidden
    );
    ClassDB::bind_method(
        D_METHOD("is_line_hidden", "line"),
        &TextEdit::is_line_hidden
    );
    ClassDB::bind_method(D_METHOD("fold_all_lines"), &TextEdit::fold_all_lines);
    ClassDB::bind_method(
        D_METHOD("unhide_all_lines"),
        &TextEdit::unhide_all_lines
    );
    ClassDB::bind_method(D_METHOD("fold_line", "line"), &TextEdit::fold_line);
    ClassDB::bind_method(
        D_METHOD("unfold_line", "line"),
        &TextEdit::unfold_line
    );
    ClassDB::bind_method(
        D_METHOD("toggle_fold_line", "line"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_highlight_all_occurrences", "enable"),
        &TextEdit::set_highlight_all_occurrences
    );
    ClassDB::bind_method(
        D_METHOD("is_highlight_all_occurrences_enabled"),
        &TextEdit::is_highlight_all_occurrences_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_override_selected_font_color", "override"),
        &TextEdit::set_override_selected_font_color
    );
    ClassDB::bind_method(
        D_METHOD("is_overriding_selected_font_color"),
        &TextEdit::is_overriding_selected_font_color
    );

    ClassDB::bind_method(
        D_METHOD("set_syntax_coloring", "enable"),
        &TextEdit::set_syntax_coloring
    );
    ClassDB::bind_method(
        D_METHOD("is_syntax_coloring_enabled"),
        &TextEdit::is_syntax_coloring_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_highlight_current_line", "enabled"),
        &TextEdit::set_highlight_current_line
    );
    ClassDB::bind_method(
        D_METHOD("is_highlight_current_line_enabled"),
        &TextEdit::is_highlight_current_line_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_smooth_scroll_enable", "enable"),
        &TextEdit::set_smooth_scroll_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_smooth_scroll_enabled"),
        &TextEdit::is_smooth_scroll_enabled
    );
    ClassDB::bind_method(
        D_METHOD("set_v_scroll_speed", "speed"),
        &TextEdit::set_v_scroll_speed
    );
    ClassDB::bind_method(
        D_METHOD("get_v_scroll_speed"),
        &TextEdit::get_v_scroll_speed
    );
    ClassDB::bind_method(
        D_METHOD("set_v_scroll", "value"),
        &TextEdit::set_v_scroll
    );
    ClassDB::bind_method(D_METHOD("get_v_scroll"), &TextEdit::get_v_scroll);
    ClassDB::bind_method(
        D_METHOD("set_h_scroll", "value"),
        &TextEdit::set_h_scroll
    );
    ClassDB::bind_method(D_METHOD("get_h_scroll"), &TextEdit::get_h_scroll);

    ClassDB::bind_method(
        D_METHOD("add_keyword_color", "keyword", "color"),
        &TextEdit::add_keyword_color
    );
    ClassDB::bind_method(
        D_METHOD("has_keyword_color", "keyword"),
        &TextEdit::has_keyword_color
    );
    ClassDB::bind_method(
        D_METHOD("get_keyword_color", "keyword"),
    ...
    ClassDB::bind_method(
        D_METHOD("menu_option", "option"),
        &TextEdit::menu_option
    );
    ClassDB::bind_method(D_METHOD("get_menu"), &TextEdit::get_menu);

    ClassDB::bind_method(
        D_METHOD("get_breakpoints"),
        &TextEdit::get_breakpoints_array
    );
    ClassDB::bind_method(
        D_METHOD("remove_breakpoints"),
        &TextEdit::remove_breakpoints
    );

    ClassDB::bind_method(
        D_METHOD("draw_minimap", "draw"),
        &TextEdit::set_draw_minimap
    );
    ClassDB::bind_method(
        D_METHOD("is_drawing_minimap"),
        &TextEdit::is_drawing_minimap
    );
    ClassDB::bind_method(
        D_METHOD("set_minimap_width", "width"),
        &TextEdit::set_minimap_width
    );
    ClassDB::bind_method(
        D_METHOD("get_minimap_width"),
        &TextEdit::get_minimap_width
    );

    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "text", PROPERTY_HINT_MULTILINE_TEXT),
        "set_text",
        "get_text"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "readonly"),
        "set_readonly",
        "is_readonly"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "highlight_current_line"),
        "set_highlight_current_line",
        "is_highlight_current_line_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "syntax_highlighting"),
        "set_syntax_coloring",
        "is_syntax_coloring_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "show_line_numbers"),
        "set_show_line_numbers",
        "is_show_line_numbers_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "draw_tabs"),
        "set_draw_tabs",
        "is_drawing_tabs"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "draw_spaces"),
        "set_draw_spaces",
        "is_drawing_spaces"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "bookmark_gutter"),
        "set_bookmark_gutter_enabled",
        "is_bookmark_gutter_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "breakpoint_gutter"),
        "set_breakpoint_gutter_enabled",
        "is_breakpoint_gutter_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "fold_gutter"),
        "set_draw_fold_gutter",
        "is_drawing_fold_gutter"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "highlight_all_occurrences"),
        "set_highlight_all_occurrences",
        "is_highlight_all_occurrences_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "override_selected_font_color"),
        "set_override_selected_font_color",
        "is_overriding_selected_font_color"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "context_menu_enabled"),
        "set_context_menu_enabled",
        "is_context_menu_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "shortcut_keys_enabled"),
        "set_shortcut_keys_enabled",
        "is_shortcut_keys_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "virtual_keyboard_enabled"),
        "set_virtual_keyboard_enabled",
        "is_virtual_keyboard_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "selecting_enabled"),
        "set_selecting_enabled",
        "is_selecting_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "smooth_scrolling"),
        "set_smooth_scroll_enable",
        "is_smooth_scroll_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "v_scroll_speed"),
        "set_v_scroll_speed",
        "get_v_scroll_speed"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "hiding_enabled"),
        "set_hiding_enabled",
        "is_hiding_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "wrap_enabled"),
        "set_wrap_enabled",
        "is_wrap_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "scroll_vertical"),
        "set_v_scroll",
        "get_v_scroll"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "scroll_horizontal"),
        "set_h_scroll",
        "get_h_scroll"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "minimap_draw"),
        "draw_minimap",
        "is_drawing_minimap"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "minimap_width"),
        "set_minimap_width",
        "get_minimap_width"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "caret_block_mode"),
        "cursor_set_block_mode",
        "cursor_is_block_mode"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "caret_blink"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "caret_moving_by_right_click"),
    ...
    ADD_SIGNAL(
        MethodInfo("breakpoint_toggled", PropertyInfo(Variant::INT, "row"))
    );
    ADD_SIGNAL(MethodInfo(
        "symbol_lookup",
        PropertyInfo(Variant::STRING, "symbol"),
...
TextEdit::TextEdit() {
    ...
    add_child(menu);
    readonly = true; // Initialise to opposite first, so we get past the
                     // early-out in set_readonly.
    set_readonly(false);
...
Map<int, TextEdit::HighlighterInfo> TextEdit::_get_line_syntax_highlighting(
    ...
    if (syntax_highlighter != nullptr) {
        Map<int, HighlighterInfo> color_map =
            syntax_highlighter->_get_line_syntax_highlighting(p_line);
    ...
    for (int j = 0; j < str.length(); j++) {
        ...
        // Check for dot or underscore or 'x' for hex notation in floating point
        // number or 'e' for scientific notation.
        if ((str[j] == '.' || str[j] == 'x' || str[j] == '_' || str[j] == 'f'
             || str[j] == 'e')
        ...
        if (!in_function_name && in_word && !in_keyword) {
            int k = j;
            while (k < str.length() && !_is_symbol(str[k]) && str[k] != '\t'
                   && str[k] != ' ') {
                k++;
            }
        ...
        if (!in_function_name && !in_member_variable && !in_keyword
            && !is_number && in_word) {
            int k = j;
            while (k > 0 && !_is_symbol(str[k]) && str[k] != '\t'
                   && str[k] != ' ') {
                k--;
            }
