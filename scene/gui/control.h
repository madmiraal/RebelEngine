...
private:
    ...
    void _compute_margins(
        Rect2 p_rect,
        const float p_anchors[4],
        float (&r_margins)[4]
    );
    void _compute_anchors(
        Rect2 p_rect,
...
public:
    ...
    void add_shader_override(
        const StringName& p_name,
        const Ref<Shader>& p_shader
    );
    void add_style_override(
        const StringName& p_name,
    ...
    Ref<Texture> get_icon(
        const StringName& p_name,
        const StringName& p_theme_type = StringName()
    ) const;
