...
void ItemList::set_item_custom_bg_color(
    int p_idx,
...
void ItemList::set_item_custom_fg_color(
    int p_idx,
    const Color& p_custom_fg_color
) {
    ERR_FAIL_INDEX(p_idx, items.size());
...
void ItemList::_gui_input(const Ref<InputEvent>& p_event) {
    ...
    if (defer_select_single >= 0 && mb.is_valid()
    ...
    if (mb.is_valid()
        ...
        if (closest != -1) {
            ...
            if (select_mode == SELECT_MULTI && items[i].selected
                && mb->get_command()) {
                unselect(i);
                ...
                if (!mb->is_doubleclick() && !mb->get_command()
                    && select_mode == SELECT_MULTI && items[i].selectable
                    && !items[i].disabled && items[i].selected
                    && mb->get_button_index() == BUTTON_LEFT) {
                ...
                if (items[i].selected
                    ...
                    select(
                        i,
                        select_mode == SELECT_SINGLE || !mb->get_command()
                    );
                    ...
                    } else if (/*select_mode==SELECT_SINGLE &&*/ mb
                                   ->is_doubleclick()) {
        ...
        // Since closest is null, more likely we clicked on empty space, so send
        // signal to interested controls. Allows, for example, implement items
        // deselecting.
    ...
    if (mb.is_valid() && mb->get_button_index() == BUTTON_WHEEL_UP
        ...
        scroll_bar->set_value(
            scroll_bar->get_value()
            - scroll_bar->get_page() * mb->get_factor() / 8
        );
    }
    ...
    if (pan_gesture.is_valid()) {
        scroll_bar->set_value(
            scroll_bar->get_value()
            + scroll_bar->get_page() * pan_gesture->get_delta().y / 8
        );
    }
...
void ItemList::_notification(int p_what) {
    ...
    if (p_what == NOTIFICATION_DRAW) {
        ...
        scroll_bar->set_anchor_and_margin(
            MARGIN_TOP,
            ANCHOR_BEGIN,
        ...
        if (shape_changed) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                if (items[i].text != "") {
                    ...
                    if (icon_mode == ICON_MODE_TOP) {
                        ...
                        if (max_text_lines > 0) {
                            minsize.y += (font_height + line_separation)
                                       * max_text_lines;
                        } else {
                            minsize.y += s.height;
                        }
            ...
            while (true) {
                ...
                if (all_fit) {
                    float page =
                        MAX(0, size.height - bg->get_minimum_size().height);
                    float max = MAX(page, ofs.y + max_h);
                    if (auto_height) {
                        auto_height_value =
                            ofs.y + max_h + bg->get_minimum_size().height;
                    }
        ...
        const Rect2 clip(
            -base_ofs,
            size
        ); // visible frame, don't need to draw outside of there

        int first_item_visible;
        {
            // do a binary search to find the first item whose rect reaches
            // below clip.position.y
            int lo = 0;
            int hi = items.size();
            while (lo < hi) {
            ...
            // we might have ended up with column 2, or 3, ..., so let's find
            // the first column
            while (lo > 0
                   && items[lo - 1].rect_cache.position.y
                          == items[lo].rect_cache.position.y) {
                lo -= 1;
            }
        ...
        for (int i = first_item_visible; i < items.size(); i++) {
            ...
            if (items[i].icon.is_valid()) {
                Size2 icon_size;
                //=
                //_adjust_to_max_size(items[i].get_icon_size(),fixed_icon_size)
                //* icon_scale;
                ...
                if (icon_mode == ICON_MODE_TOP) {
                    pos.x += Math::floor(
                        (items[i].rect_cache.size.width - icon_size.width) / 2
                    );
                    pos.y += MIN(
                        Math::floor(
                            (items[i].rect_cache.size.height - icon_size.height)
                            / 2
                        ),
                    ...
                    pos.y += Math::floor(
                        (items[i].rect_cache.size.height - icon_size.height) / 2
                    );
                ...
                if (fixed_icon_size.x > 0 && fixed_icon_size.y > 0) {
                    Rect2 adj = _adjust_to_max_size(
                        items[i].get_icon_size() * icon_scale,
                        icon_size
                    );
                ...
                Rect2 region = (items[i].icon_region.size.x == 0
                                || items[i].icon_region.size.y == 0)
                                 ? Rect2(Vector2(), items[i].icon->get_size())
                                 : Rect2(items[i].icon_region);
            ...
            if (items[i].tag_icon.is_valid()) {
                draw_texture(
                    items[i].tag_icon,
                    items[i].rect_cache.position + base_ofs
                );
            }
            ...
            if (items[i].text != "") {
                ...
                Color modulate =
                    items[i].selected
                        ? font_color_selected
                        : (items[i].custom_fg != Color() ? items[i].custom_fg
                                                         : font_color);
                if (items[i].disabled) {
                    modulate.a *= 0.5;
                }
                ...
                if (icon_mode == ICON_MODE_TOP && max_text_lines > 0) {
                    ...
                    for (int j = 0; j < ss; j++) {
                        ...
                        ofs += drawer.draw_char(
                            get_canvas_item(),
                            text_ofs
                                + Vector2(
                                      ofs
                                          + (max_len - line_size_cache[line])
                                                / 2,
                                      line * (font_height + line_separation)
                                )
                                      .floor(),
                            items[i].text[j],
                            items[i].text[j + 1],
                    ...
                    if (icon_mode == ICON_MODE_TOP) {
                        text_ofs.x +=
                            (items[i].rect_cache.size.width - size2.x) / 2;
                    } else {
                        text_ofs.y +=
                            (items[i].rect_cache.size.height - size2.y) / 2;
                    }
        ...
        {
            // do a binary search to find the first separator that is below
            // clip_position.y
            int lo = 0;
        ...
        for (int i = first_visible_separator; i < separators.size(); i++) {
            ...
            draw_line(
                Vector2(bg->get_margin(MARGIN_LEFT), y),
                Vector2(width, y),
                guide_color
            );
        }
...
int ItemList::get_item_at_position(const Point2& p_pos, bool p_exact) const {
    ...
    for (int i = 0; i < items.size(); i++) {
        ...
        if (i % current_columns == current_columns - 1) {
            rc.size.width =
                get_size().width
                - rc.position.x; // make sure you can still select the last item
                                 // when clicking past the column
        }
...
void ItemList::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("add_item", "text", "icon", "selectable"),
        &ItemList::add_item,
        DEFVAL(Variant()),
        DEFVAL(true)
    );
    ClassDB::bind_method(
        D_METHOD("add_icon_item", "icon", "selectable"),
        &ItemList::add_icon_item,
        DEFVAL(true)
    );

    ClassDB::bind_method(
        D_METHOD("set_item_text", "idx", "text"),
        &ItemList::set_item_text
    );
    ClassDB::bind_method(
        D_METHOD("get_item_text", "idx"),
        &ItemList::get_item_text
    );

    ClassDB::bind_method(
        D_METHOD("set_item_icon", "idx", "icon"),
        &ItemList::set_item_icon
    );
    ClassDB::bind_method(
        D_METHOD("get_item_icon", "idx"),
        &ItemList::get_item_icon
    );

    ClassDB::bind_method(
        D_METHOD("set_item_icon_transposed", "idx", "transposed"),
        &ItemList::set_item_icon_transposed
    );
    ClassDB::bind_method(
        D_METHOD("is_item_icon_transposed", "idx"),
        &ItemList::is_item_icon_transposed
    );

    ClassDB::bind_method(
        D_METHOD("set_item_icon_region", "idx", "rect"),
        &ItemList::set_item_icon_region
    );
    ClassDB::bind_method(
        D_METHOD("get_item_icon_region", "idx"),
        &ItemList::get_item_icon_region
    );

    ClassDB::bind_method(
        D_METHOD("set_item_icon_modulate", "idx", "modulate"),
        &ItemList::set_item_icon_modulate
    );
    ClassDB::bind_method(
        D_METHOD("get_item_icon_modulate", "idx"),
        &ItemList::get_item_icon_modulate
    );

    ClassDB::bind_method(
        D_METHOD("set_item_selectable", "idx", "selectable"),
        &ItemList::set_item_selectable
    );
    ClassDB::bind_method(
        D_METHOD("is_item_selectable", "idx"),
        &ItemList::is_item_selectable
    );

    ClassDB::bind_method(
        D_METHOD("set_item_disabled", "idx", "disabled"),
        &ItemList::set_item_disabled
    );
    ClassDB::bind_method(
        D_METHOD("is_item_disabled", "idx"),
        &ItemList::is_item_disabled
    );

    ClassDB::bind_method(
        D_METHOD("set_item_metadata", "idx", "metadata"),
        &ItemList::set_item_metadata
    );
    ClassDB::bind_method(
        D_METHOD("get_item_metadata", "idx"),
        &ItemList::get_item_metadata
    );

    ClassDB::bind_method(
        D_METHOD("set_item_custom_bg_color", "idx", "custom_bg_color"),
        &ItemList::set_item_custom_bg_color
    );
    ClassDB::bind_method(
        D_METHOD("get_item_custom_bg_color", "idx"),
        &ItemList::get_item_custom_bg_color
    );

    ClassDB::bind_method(
        D_METHOD("set_item_custom_fg_color", "idx", "custom_fg_color"),
        &ItemList::set_item_custom_fg_color
    );
    ClassDB::bind_method(
        D_METHOD("get_item_custom_fg_color", "idx"),
        &ItemList::get_item_custom_fg_color
    );

    ClassDB::bind_method(
        D_METHOD("set_item_tooltip_enabled", "idx", "enable"),
        &ItemList::set_item_tooltip_enabled
    );
    ClassDB::bind_method(
        D_METHOD("is_item_tooltip_enabled", "idx"),
        &ItemList::is_item_tooltip_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_item_tooltip", "idx", "tooltip"),
        &ItemList::set_item_tooltip
    );
    ClassDB::bind_method(
        D_METHOD("get_item_tooltip", "idx"),
        &ItemList::get_item_tooltip
    );

    ClassDB::bind_method(
        D_METHOD("select", "idx", "single"),
        &ItemList::select,
        DEFVAL(true)
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_auto_height", "enable"),
        &ItemList::set_auto_height
    );
    ClassDB::bind_method(
        D_METHOD("has_auto_height"),
        &ItemList::has_auto_height
    );

    ClassDB::bind_method(
        D_METHOD("is_anything_selected"),
        &ItemList::is_anything_selected
    );

    ClassDB::bind_method(
        D_METHOD("get_item_at_position", "position", "exact"),
        &ItemList::get_item_at_position,
        DEFVAL(false)
    );

    ClassDB::bind_method(
        D_METHOD("ensure_current_is_visible"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "auto_height"),
        "set_auto_height",
        "has_auto_height"
    );
    ADD_GROUP("Columns", "");
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "same_column_width"),
    ...
    ADD_GROUP("Icon", "");
    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "icon_mode", PROPERTY_HINT_ENUM, "Top,Left"),
        "set_icon_mode",
        "get_icon_mode"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "icon_scale"),
        "set_icon_scale",
        "get_icon_scale"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR2, "fixed_icon_size"),
        "set_fixed_icon_size",
        "get_fixed_icon_size"
    );
    ...
    ADD_SIGNAL(
        MethodInfo("rmb_clicked", PropertyInfo(Variant::VECTOR2, "at_position"))
    );
