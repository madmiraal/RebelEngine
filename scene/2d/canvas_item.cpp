...
SelfList<CanvasItemMaterial>::List* CanvasItemMaterial::dirty_materials =
    nullptr;
Map<CanvasItemMaterial::MaterialKey, CanvasItemMaterial::ShaderData>
...
void CanvasItemMaterial::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_light_mode", "light_mode"),
        &CanvasItemMaterial::set_light_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_light_mode"),
        &CanvasItemMaterial::get_light_mode
    );

    ClassDB::bind_method(
        D_METHOD("set_particles_animation", "particles_anim"),
        &CanvasItemMaterial::set_particles_animation
    );
    ClassDB::bind_method(
        D_METHOD("get_particles_animation"),
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "light_mode",
            PROPERTY_HINT_ENUM,
            "Normal,Unshaded,Light Only"
        ),
        "set_light_mode",
        "get_light_mode"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "particles_animation"),
...
Transform2D CanvasItem::_edit_get_transform() const {
    return Transform2D(
        _edit_get_rotation(),
        _edit_get_position() + _edit_get_pivot()
    );
}
#endif
...
void CanvasItem::_propagate_visibility_changed(bool p_visible) {
    ...
    for (int i = 0; i < get_child_count(); i++) {
        ...
        if (c && c->visible) { // should the toplevels stop propagation? i think
                               // so but..
    ...
    if (is_visible_in_tree()) { // todo optimize this!!
        ...
        if (get_script_instance()) {
            get_script_instance()->call_multilevel_reversed(
                SceneStringNames::get_singleton()->_draw,
                nullptr,
                0
            );
        }
...
void CanvasItem::_toplevel_raise_self() {
    ...
    if (canvas_layer) {
        VisualServer::get_singleton()->canvas_item_set_draw_index(
            canvas_item,
            canvas_layer->get_sort_index()
        );
    } else {
        VisualServer::get_singleton()->canvas_item_set_draw_index(
            canvas_item,
            get_viewport()->gui_get_canvas_sort_index()
        );
    }
}
...
void CanvasItem::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_EXIT_TREE: {
            ...
            _exit_canvas();
            if (C) {
                Object::cast_to<CanvasItem>(get_parent())
                    ->children_items.erase(C);
                C = nullptr;
            }
            global_invalid = true;
        } break;
...
void CanvasItem::set_modulate(const Color& p_modulate) {
    ...
    modulate = p_modulate;
    VisualServer::get_singleton()->canvas_item_set_modulate(
        canvas_item,
        modulate
    );
}
...
void CanvasItem::set_self_modulate(const Color& p_self_modulate) {
    ...
    VisualServer::get_singleton()->canvas_item_set_self_modulate(
        canvas_item,
        self_modulate
    );
}
...
void CanvasItem::draw_line(
    ...
    ERR_FAIL_COND_MSG(
        !drawing,
        "Drawing is only allowed inside NOTIFICATION_DRAW, _draw() function or "
        "'draw' signal."
    );
...
void CanvasItem::draw_polyline_colors(
    const Vector<Point2>& p_points,
...
void CanvasItem::draw_arc(
    ...
    for (int i = 0; i < p_point_count; i++) {
        float theta =
            (i / (p_point_count - 1.0f)) * delta_angle + p_start_angle;
        points.set(
            i,
            p_center + Vector2(Math::cos(theta), Math::sin(theta)) * p_radius
        );
    }
...
void CanvasItem::draw_multiline(
    ...
    Vector<Color> colors;
    colors.push_back(p_color);
    VisualServer::get_singleton()->canvas_item_add_multiline(
        canvas_item,
        p_points,
        colors,
        p_width,
...
void CanvasItem::draw_multiline_colors(
    const Vector<Point2>& p_points,
    const Vector<Color>& p_colors,
    float p_width,
    bool p_antialiased
...
void CanvasItem::draw_rect(
    ...
    if (p_filled) {
        if (p_width != 1.0) {
            WARN_PRINT(
                "The draw_rect() \"width\" argument has no effect when "
                "\"filled\" is \"true\"."
            );
        }
        ...
        if (p_antialiased) {
            WARN_PRINT(
                "The draw_rect() \"antialiased\" argument has no effect when "
                "\"filled\" is \"true\"."
            );
        }

        VisualServer::get_singleton()
            ->canvas_item_add_rect(canvas_item, p_rect, p_color);
    } else {
        // Thick lines are offset depending on their width to avoid partial
        // overlapping. Thin lines don't require an offset, so don't apply one
        // in this case
        float offset;
        if (p_width >= 2) {
            offset = p_width / 2.0;
        ...
        VisualServer::get_singleton()->canvas_item_add_line(
            ...
            p_rect.position
                + Size2(p_rect.size.width, p_rect.size.height - offset),
            p_color,
            p_width,
            p_antialiased
        );
        VisualServer::get_singleton()->canvas_item_add_line(
            canvas_item,
            p_rect.position
                + Size2(p_rect.size.width + offset, p_rect.size.height),
...
void CanvasItem::draw_circle(
    ...
    VisualServer::get_singleton()
        ->canvas_item_add_circle(canvas_item, p_pos, p_radius, p_color);
}
...
void CanvasItem::draw_texture_rect(
    const Ref<Texture>& p_texture,
    const Rect2& p_rect,
    bool p_tile,
    const Color& p_modulate,
...
void CanvasItem::draw_texture_rect_region(
    const Ref<Texture>& p_texture,
    const Rect2& p_rect,
...
void CanvasItem::draw_style_box(
    const Ref<StyleBox>& p_style_box,
    const Rect2& p_rect
) {
    ERR_FAIL_COND_MSG(
        !drawing,
...
void CanvasItem::draw_primitive(
    const Vector<Point2>& p_points,
    const Vector<Color>& p_colors,
...
void CanvasItem::draw_set_transform(
...
void CanvasItem::draw_colored_polygon(
    const Vector<Point2>& p_points,
    const Color& p_color,
    const Vector<Point2>& p_uvs,
    Ref<Texture> p_texture,
...
void CanvasItem::draw_multimesh(
    ...
    RID normal_map_rid =
        p_normal_map.is_valid() ? p_normal_map->get_rid() : RID();
    VisualServer::get_singleton()->canvas_item_add_multimesh(
        canvas_item,
        p_multimesh->get_rid(),
        texture_rid,
        normal_map_rid
...
void CanvasItem::set_draw_behind_parent(bool p_enable) {
    ...
    behind = p_enable;
    VisualServer::get_singleton()->canvas_item_set_draw_behind_parent(
        canvas_item,
        behind
    );
}
...
Vector2 CanvasItem::make_canvas_position_local(const Vector2& screen_point
) const {
    ERR_FAIL_COND_V(!is_inside_tree(), screen_point);

    Transform2D local_matrix =
        (get_canvas_transform() * get_global_transform()).affine_inverse();

    return local_matrix.xform(screen_point);
}

Ref<InputEvent> CanvasItem::make_input_local(const Ref<InputEvent>& p_event
) const {
    ERR_FAIL_COND_V(p_event.is_null(), p_event);
    ERR_FAIL_COND_V(!is_inside_tree(), p_event);

    return p_event->xformed_by(
        (get_canvas_transform() * get_global_transform()).affine_inverse()
    );
}
...
Vector2 CanvasItem::get_global_mouse_position() const {
    ...
    return get_canvas_transform().affine_inverse().xform(
...
Vector2 CanvasItem::get_local_mouse_position() const {
    ...
    return get_global_transform().affine_inverse().xform(
...
void CanvasItem::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_toplevel_raise_self"),
        &CanvasItem::_toplevel_raise_self
    );
    ClassDB::bind_method(
        D_METHOD("_update_callback"),
        &CanvasItem::_update_callback
    );

#ifdef TOOLS_ENABLED
    ClassDB::bind_method(
        D_METHOD("_edit_set_state", "state"),
        &CanvasItem::_edit_set_state
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_state"),
        &CanvasItem::_edit_get_state
    );
    ClassDB::bind_method(
        D_METHOD("_edit_set_position", "position"),
        &CanvasItem::_edit_set_position
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_position"),
        &CanvasItem::_edit_get_position
    );
    ClassDB::bind_method(
        D_METHOD("_edit_set_scale", "scale"),
        &CanvasItem::_edit_set_scale
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_scale"),
        &CanvasItem::_edit_get_scale
    );
    ClassDB::bind_method(
        D_METHOD("_edit_set_rect", "rect"),
        &CanvasItem::_edit_set_rect
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_rect"),
        &CanvasItem::_edit_get_rect
    );
    ClassDB::bind_method(
        D_METHOD("_edit_use_rect"),
        &CanvasItem::_edit_use_rect
    );
    ClassDB::bind_method(
        D_METHOD("_edit_set_rotation", "degrees"),
        &CanvasItem::_edit_set_rotation
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_rotation"),
        &CanvasItem::_edit_get_rotation
    );
    ClassDB::bind_method(
        D_METHOD("_edit_use_rotation"),
        &CanvasItem::_edit_use_rotation
    );
    ClassDB::bind_method(
        D_METHOD("_edit_set_pivot", "pivot"),
        &CanvasItem::_edit_set_pivot
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_pivot"),
        &CanvasItem::_edit_get_pivot
    );
    ClassDB::bind_method(
        D_METHOD("_edit_use_pivot"),
        &CanvasItem::_edit_use_pivot
    );
    ClassDB::bind_method(
        D_METHOD("_edit_get_transform"),
        &CanvasItem::_edit_get_transform
    );
#endif

    ClassDB::bind_method(
        D_METHOD("get_canvas_item"),
        &CanvasItem::get_canvas_item
    );

    ClassDB::bind_method(
        D_METHOD("set_visible", "visible"),
        &CanvasItem::set_visible
    );
    ClassDB::bind_method(D_METHOD("is_visible"), &CanvasItem::is_visible);
    ClassDB::bind_method(
        D_METHOD("is_visible_in_tree"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_as_toplevel", "enable"),
        &CanvasItem::set_as_toplevel
    );
    ClassDB::bind_method(
        D_METHOD("is_set_as_toplevel"),
        &CanvasItem::is_set_as_toplevel
    );

    ClassDB::bind_method(
        D_METHOD("set_light_mask", "light_mask"),
        &CanvasItem::set_light_mask
    );
    ClassDB::bind_method(
        D_METHOD("get_light_mask"),
        &CanvasItem::get_light_mask
    );

    ClassDB::bind_method(
        D_METHOD("set_modulate", "modulate"),
        &CanvasItem::set_modulate
    );
    ClassDB::bind_method(D_METHOD("get_modulate"), &CanvasItem::get_modulate);
    ClassDB::bind_method(
        D_METHOD("set_self_modulate", "self_modulate"),
        &CanvasItem::set_self_modulate
    );
    ClassDB::bind_method(
        D_METHOD("get_self_modulate"),
        &CanvasItem::get_self_modulate
    );

    ClassDB::bind_method(
        D_METHOD("set_draw_behind_parent", "enable"),
        &CanvasItem::set_draw_behind_parent
    );
    ClassDB::bind_method(
        D_METHOD("is_draw_behind_parent_enabled"),
        &CanvasItem::is_draw_behind_parent_enabled
    );

    ClassDB::bind_method(
        D_METHOD("_set_on_top", "on_top"),
        &CanvasItem::_set_on_top
    );
    ClassDB::bind_method(D_METHOD("_is_on_top"), &CanvasItem::_is_on_top);
    // ClassDB::bind_method(D_METHOD("get_transform"),&CanvasItem::get_transform);

    ClassDB::bind_method(
        D_METHOD("draw_line", "from", "to", "color", "width", "antialiased"),
        &CanvasItem::draw_line,
        DEFVAL(1.0),
        DEFVAL(false)
    );
    ClassDB::bind_method(
        D_METHOD("draw_polyline", "points", "color", "width", "antialiased"),
    ...
    ClassDB::bind_method(
        D_METHOD("draw_multiline", "points", "color", "width", "antialiased"),
    ...
    ClassDB::bind_method(
        D_METHOD("draw_circle", "position", "radius", "color"),
    ...
    ClassDB::bind_method(
        D_METHOD("draw_style_box", "style_box", "rect"),
    ...
    ClassDB::bind_method(
        D_METHOD("draw_char", "font", "position", "char", "next", "modulate"),
    ...
    ClassDB::bind_method(
        D_METHOD("draw_multimesh", "multimesh", "texture", "normal_map"),
        &CanvasItem::draw_multimesh,
        DEFVAL(Ref<Texture>())
    );

    ClassDB::bind_method(
        D_METHOD("draw_set_transform", "position", "rotation", "scale"),
        &CanvasItem::draw_set_transform
    );
    ClassDB::bind_method(
        D_METHOD("draw_set_transform_matrix", "xform"),
        &CanvasItem::draw_set_transform_matrix
    );
    ClassDB::bind_method(D_METHOD("get_transform"), &CanvasItem::get_transform);
    ClassDB::bind_method(
        D_METHOD("get_global_transform"),
        &CanvasItem::get_global_transform
    );
    ClassDB::bind_method(
        D_METHOD("get_global_transform_with_canvas"),
        &CanvasItem::get_global_transform_with_canvas
    );
    ClassDB::bind_method(
        D_METHOD("get_viewport_transform"),
        &CanvasItem::get_viewport_transform
    );
    ClassDB::bind_method(
        D_METHOD("get_viewport_rect"),
    ...
    ClassDB::bind_method(
        D_METHOD("get_local_mouse_position"),
        &CanvasItem::get_local_mouse_position
    );
    ClassDB::bind_method(
        D_METHOD("get_global_mouse_position"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_use_parent_material", "enable"),
        &CanvasItem::set_use_parent_material
    );
    ClassDB::bind_method(
        D_METHOD("get_use_parent_material"),
        &CanvasItem::get_use_parent_material
    );

    ClassDB::bind_method(
        D_METHOD("set_notify_local_transform", "enable"),
        &CanvasItem::set_notify_local_transform
    );
    ClassDB::bind_method(
        D_METHOD("is_local_transform_notification_enabled"),
        &CanvasItem::is_local_transform_notification_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_notify_transform", "enable"),
        &CanvasItem::set_notify_transform
    );
    ClassDB::bind_method(
        D_METHOD("is_transform_notification_enabled"),
        &CanvasItem::is_transform_notification_enabled
    );

    ClassDB::bind_method(
        D_METHOD("force_update_transform"),
        &CanvasItem::force_update_transform
    );

    ClassDB::bind_method(
        D_METHOD("make_canvas_position_local", "screen_point"),
        &CanvasItem::make_canvas_position_local
    );
    ClassDB::bind_method(
        D_METHOD("make_input_local", "event"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::COLOR, "modulate"),
        "set_modulate",
        "get_modulate"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::COLOR, "self_modulate"),
        "set_self_modulate",
        "get_self_modulate"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "show_behind_parent"),
        "set_draw_behind_parent",
        "is_draw_behind_parent_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "show_on_top", PROPERTY_HINT_NONE, "", 0),
        "_set_on_top",
        "_is_on_top"
    ); // compatibility
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "use_parent_material"),
        "set_use_parent_material",
        "get_use_parent_material"
    );
    // exporting these things doesn't really make much sense i think
    //  ADD_PROPERTY(PropertyInfo(Variant::BOOL, "toplevel", PROPERTY_HINT_NONE,
    //  "", PROPERTY_USAGE_NOEDITOR), "set_as_toplevel", "is_set_as_toplevel");
...
void CanvasItem::set_notify_transform(bool p_enable) {
    ...
    if (notify_transform && is_inside_tree()) {
        // this ensures that invalid globals get resolved, so notifications can
        // be received
