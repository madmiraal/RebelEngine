...
TScriptInstance* cast_script_instance(ScriptInstance* p_inst) {
    ...
    return p_inst->get_language() == TScriptLanguage::get_singleton()
             ? static_cast<TScriptInstance*>(p_inst)
             : NULL;
}
#else
template <typename TScriptInstance, typename TScriptLanguage>
...
#define CAST_CSHARP_INSTANCE(m_inst)                                           \
...
class CSharpScript : public Script {
    ...
    struct StateBackup {
        // TODO
        // Replace with buffer containing the serialized state of managed
        // scripts. Keep variant state backup to use only with script instance
        // placeholders.
    ...
    Map<StringName, Variant>
        exported_members_defval_cache; // member_default_values_cache
...
#ifdef TOOLS_ENABLED
    static int _try_get_member_export_hint(
        IMonoClassMember* p_member,
        ManagedType p_type,
    ...
    CSharpInstance* _create_instance(
    ...
    static Ref<CSharpScript> create_for_managed_type(
        GDMonoClass* p_class,
        GDMonoClass* p_native
    );
    static void initialize_for_managed_type(
...
class CSharpInstance : public ScriptInstance {
    ...
    /*
     * If true is returned, the caller must memdelete the script instance's
     * owner.
     */
    bool _unreference_owner_unsafe();

    /*
     * If NULL is returned, the caller must destroy the script instance by
     * removing it from its owner.
     */
    MonoObject* _internal_new_managed();

    // Do not use unless you know what you are doing
    friend void GDMonoInternals::tie_managed_to_unmanaged(MonoObject*, Object*);
    static CSharpInstance* create_for_managed_type(
        Object* p_owner,
        CSharpScript* p_script,
        const Ref<MonoGCHandle>& p_gchandle
    );

    void _call_multilevel(
    ...
    MultiplayerAPI::RPCMode _member_get_rpc_mode(IMonoClassMember* p_member
    ) const;

    void get_properties_state_for_reloading(
...
public:
    ...
    /*
     * If 'r_delete_owner' is set to true, the caller must memdelete the script
     * instance's owner. Otherwise, if 'r_remove_script_instance' is set to
     * true, the caller must destroy the script instance by removing it from its
     * owner.
     */
    void mono_object_disposed_baseref(
        MonoObject* p_obj,
        bool p_is_finalizer,
        bool& r_delete_owner,
...
public:
    ...
    static void release_script_gchandle(
    ...
    Map<Object*, CSharpScriptBinding>::Element* insert_script_binding(
        Object* p_object,
        const CSharpScriptBinding& p_script_binding
    );
    bool setup_csharp_script_binding(
