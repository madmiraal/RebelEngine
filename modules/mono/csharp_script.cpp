...
#define CACHED_STRING_NAME(m_var)                                              \
    (CSharpLanguage::get_singleton()->get_string_names().m_var)
...
void CSharpLanguage::init() {
    ...
    // Generate bindings here, before loading assemblies.
    // 'initialize_load_assemblies' aborts the applications if the api
    // assemblies or the main tools assembly is missing, but this is not a
    // problem for BindingsGenerator as it only needs the tools project editor
    // assembly.
...
void CSharpLanguage::finish() {
    ...
    // Make sure all script binding gchandles are released before finalizing
    // GDMono
    for (Map<Object*, CSharpScriptBinding>::Element* E =
             script_bindings.front();
         E;
         E = E->next()) {
    ...
    // Clear here, after finalizing all domains to make sure there is nothing
    // else referencing the elements.
    script_bindings.clear();

#ifdef DEBUG_ENABLED
    for (Map<ObjectID, int>::Element* E = unsafe_object_references.front(); E;
         E                              = E->next()) {
...
void CSharpLanguage::get_reserved_words(List<String>* p_words) const {
    static const char* _reserved_words[] = {
        ...
        // Contextual keywords. Not reserved words, but I guess we should
        // include
...
bool CSharpLanguage::is_control_flow_keyword(String p_keyword) const {
    return p_keyword == "break" || p_keyword == "case" || p_keyword == "catch"
        || p_keyword == "continue" || p_keyword == "default"
        || p_keyword == "do" || p_keyword == "else" || p_keyword == "finally"
        || p_keyword == "for" || p_keyword == "foreach" || p_keyword == "goto"
        || p_keyword == "if" || p_keyword == "return" || p_keyword == "switch"
...
void CSharpLanguage::get_string_delimiters(List<String>* p_delimiters) const {
    ...
    // Verbatim string literals (`@" "`) don't render correctly, so don't
    // highlight them. Generic string highlighting suffices as a workaround for
    // now.
}

static String get_base_class_name(
...
Ref<Script> CSharpLanguage::get_template(
    const String& p_class_name,
    const String& p_base_class_name
) const {
    ...
    String base_class_name =
        get_base_class_name(p_base_class_name, class_name_no_spaces);
...
void CSharpLanguage::make_template(
    const String& p_class_name,
    const String& p_base_class_name,
    Ref<Script>& p_script
) {
...
static String variant_type_to_managed_name(const String& p_var_type_name) {
    ...
    if (p_var_type_name
        == Variant::get_type_name(Variant::POOL_VECTOR2_ARRAY)) {
        return "Vector2[]";
    }
    if (p_var_type_name
        == Variant::get_type_name(Variant::POOL_VECTOR3_ARRAY)) {
        return "Vector3[]";
    }
...
String CSharpLanguage::make_function(
    ...
    for (int i = 0; i < p_args.size(); i++) {
        ...
        s += variant_type_to_managed_name(arg.get_slice(":", 1)) + " "
           + escape_csharp_keyword(arg.get_slice(":", 0));
    }
...
#else
String CSharpLanguage::
    make_function(const String&, const String&, const PoolStringArray&) const {
    return String();
}
#endif
...
Vector<ScriptLanguage::StackInfo> CSharpLanguage::debug_get_current_stack_info(
    ...
    MonoObject* stack_trace = mono_object_new(
        mono_domain_get(),
        CACHED_CLASS(System_Diagnostics_StackTrace)->get_mono_ptr()
    );

    MonoBoolean need_file_info = true;
    void* ctor_args[1]         = {&need_file_info};

    CACHED_METHOD(System_Diagnostics_StackTrace, ctor_bool)
        ->invoke_raw(stack_trace, ctor_args);
...
Vector<ScriptLanguage::StackInfo> CSharpLanguage::stack_trace_get_info(
    ...
    MonoArray* frames =
        CACHED_METHOD_THUNK(System_Diagnostics_StackTrace, GetFrames)
            .invoke(p_stack_trace, &exc);
    ...
    for (int i = 0; i < frame_count; i++) {
        ...
        CACHED_METHOD_THUNK(DebuggingUtils, GetStackFrameInfo)
            .invoke(frame, &file_name, &file_line_num, &method_decl, &exc);
        ...
        // TODO
        // what if the StackFrame method is null (method_decl is empty). should
...
void CSharpLanguage::frame() {
    if (gdmono && gdmono->is_runtime_initialized()
        && gdmono->get_core_api_assembly() != NULL) {
        const Ref<MonoGCHandle>& task_scheduler_handle =
            GDMonoCache::cached_data.task_scheduler_handle;
...
struct CSharpScriptDepSort {
    // must support sorting so inheritance works properly (parent must be
    // reloaded first)
    bool operator()(const Ref<CSharpScript>& A, const Ref<CSharpScript>& B)
        const {
        if (A == B) {
...
void CSharpLanguage::reload_tool_script(
...
bool CSharpLanguage::is_assembly_reloading_needed() {
    ...
    if (proj_assembly) {
        ...
        if (FileAccess::get_modified_time(proj_asm_path)
...
void CSharpLanguage::reload_assemblies(bool p_soft_reload) {
    ...
    for (List<Ref<CSharpScript>>::Element* E = scripts.front(); E;
         E                                   = E->next()) {
        Ref<CSharpScript>& script = E->get();
        // If someone removes a script from a node, deletes the script, builds,
        // adds a script to the same node, then builds again, the script might
        // have no path and also no script_class. In that case, we can't (and
        ...
        if (script->get_path().empty()) {
            script->tied_class_name_for_reload =
                script->script_class->get_name_for_lookup();
            script->tied_class_namespace_for_reload =
                script->script_class->get_namespace();
        }

        // Script::instances are deleted during managed object disposal, which
        // happens on domain finalize. Only placeholders are kept. Therefore we
        // need to keep a copy before that happens.

        for (Set<Object*>::Element* F = script->instances.front(); F;
             F                        = F->next()) {
            Object* obj = F->get();
...
#ifdef TOOLS_ENABLED
        for (Set<PlaceHolderScriptInstance*>::Element* F =
                 script->placeholders.front();
             F;
             F = F->next()) {
        ...
        Map<ObjectID, CSharpScript::StateBackup>& owners_map =
            script->pending_reload_state;
        ...
        for (Set<Object*>::Element* F = script->instances.front(); F;
            ...
            CSharpInstance* csi =
                static_cast<CSharpInstance*>(obj->get_script_instance());

            // Call OnBeforeSerialize
            if (csi->script->script_class->implements_interface(
                    CACHED_CLASS(ISerializationListener)
                )) {
                obj->get_script_instance()->call_multilevel(
                    string_names.on_before_serialize
                );
            }
            ...
            // TODO: Proper state backup (Not only variants, serialize managed
            // state of scripts)
    ...
    // After the state of all instances is saved, clear scripts and script
    // instances
    for (List<Ref<CSharpScript>>::Element* E = scripts.front(); E;
         E                                   = E->next()) {
        Ref<CSharpScript>& script = E->get();
        ...
        while (script->instances.front()) {
            ...
            ); // Remove script and existing script instances (placeholder are
               // not removed before domain reload)
        }
    ...
    if (gdmono->reload_scripts_domain() != OK) {
        ...
        for (List<Ref<CSharpScript>>::Element* E = to_reload.front(); E;
             E                                   = E->next()) {
            Ref<CSharpScript> scr = E->get();

            for (const Map<ObjectID, CSharpScript::StateBackup>::Element* F =
                     scr->pending_reload_state.front();
                 F;
                 F = F->next()) {
                ...
                // Use a placeholder for now to avoid losing the state when
                // saving a scene

                obj->set_script(scr.get_ref_ptr());

                PlaceHolderScriptInstance* placeholder =
                    scr->placeholder_instance_create(obj);
                obj->set_script_instance(placeholder);

#ifdef TOOLS_ENABLED
                // Even though build didn't fail, this tells the placeholder to
                // keep properties and it allows using property_set_fallback for
                // restoring the state without a valid script.
                scr->placeholder_fallback_enabled = true;
#endif

                // Restore Variant properties state, it will be kept by the
                // placeholder until the next script reloading
                for (List<Pair<StringName, Variant>>::Element* G =
                         scr->pending_reload_state[obj_id].properties.front();
                     G;
                     G = G->next()) {
                    placeholder->property_set_fallback(
                        G->get().first,
                        G->get().second,
                        NULL
                    );
    ...
    for (List<Ref<CSharpScript>>::Element* E = to_reload.front(); E;
        ...
        if (!script->get_path().empty()) {
            ...
            const StringName& class_namespace =
                script->tied_class_namespace_for_reload;
            const StringName& class_name = script->tied_class_name_for_reload;
            GDMonoAssembly* project_assembly = gdmono->get_project_assembly();

            // Search in project and tools assemblies first as those are the
            // most likely to have the class
            GDMonoClass* script_class =
                (project_assembly
                     ? project_assembly->get_class(class_namespace, class_name)
                     : NULL);
            ...
            CSharpScript::initialize_for_managed_type(
                script,
                script_class,
                native
            );
        ...
        {
            for (Set<ObjectID>::Element* F =
                     script->pending_reload_instances.front();
                 F;
                 F = F->next()) {
                ...
                if (si) {
                    ...
                    if (script->is_tool()
                        ...
                        CSharpScript::StateBackup& state_backup =
                            script->pending_reload_state[obj_id];

                        // Backup placeholder script instance state before
                        // replacing it with a script instance
                        si->get_property_state(state_backup.properties);

                        ScriptInstance* script_instance =
                            script->instance_create(obj);

                        if (script_instance) {
                            script->placeholders.erase(
                ...
                ); // will create the script instance as well
            }
        }
    ...
    for (List<Ref<CSharpScript>>::Element* E = to_reload_state.front(); E;
         E                                   = E->next()) {
        Ref<CSharpScript> script = E->get();
        ...
        for (Set<ObjectID>::Element* F =
            ...
            for (List<Pair<StringName, Variant>>::Element* G =
                     state_backup.properties.front();
                 G;
                 G = G->next()) {
                obj->get_script_instance()->set(
                    G->get().first,
                    G->get().second
                );
            }
            ...
            CSharpInstance* csi =
                CAST_CSHARP_INSTANCE(obj->get_script_instance());
            if (csi
                && csi->script->script_class->implements_interface(
                    CACHED_CLASS(ISerializationListener)
                )) {
                obj->get_script_instance()->call_multilevel(
                    string_names.on_after_deserialize
                );
            }
        }
...
#ifdef TOOLS_ENABLED
    // FIXME: Hack to refresh editor in order to display new properties and
    // signals. See if there is a better alternative.
    if (Engine::get_singleton()->is_editor_hint()) {
...
void CSharpLanguage::_load_scripts_metadata() {
    ...
    scripts_metadata_filename +=
        Engine::get_singleton()->is_editor_hint() ? "editor" : "editor_player";
#else
#ifdef DEBUG_ENABLED
    ...
    if (FileAccess::exists(scripts_metadata_path)) {
        ...
        if (json_err != OK) {
            ERR_PRINT(
                "Failed to parse metadata file: '" + err_str + "' ("
                + String::num_int64(err_line) + ")."
            );
            return;
        }
...
void CSharpLanguage::get_recognized_extensions(List<String>* p_extensions
) const {
    p_extensions->push_back("cs");
}

#ifdef TOOLS_ENABLED
Error CSharpLanguage::open_in_external_editor(
    const Ref<Script>& p_script,
    int p_line,
    int p_col
) {
...
bool CSharpLanguage::debug_break_parse(
    const String& p_file,
    int p_line,
    const String& p_error
) {
    // Not a parser error in our case, but it's still used for other type of
    // errors
...
void CSharpLanguage::_editor_init_callback() {
    ...
    MonoObject* mono_object =
        mono_object_new(mono_domain_get(), editor_klass->get_mono_ptr());
...
void CSharpLanguage::release_script_gchandle(
    MonoObject* p_expected_obj,
    Ref<MonoGCHandle>& p_gchandle
) {
    uint32_t pinned_gchandle =
        MonoGCHandle::new_strong_handle_pinned(p_expected_obj
        ); // We might lock after this, so pin it
    ...
    if (!p_gchandle->is_released()) { // Do not lock unnecessarily
        ...
        // We release the gchandle if it points to the MonoObject* we expect
        // (otherwise it was already released and could have been replaced) or
        // if we can't get its target MonoObject* (which doesn't necessarily
        // mean it was released, and we want it released in order to avoid
        // locking other threads unnecessarily).
...
bool CSharpLanguage::setup_csharp_script_binding(
    CSharpScriptBinding& r_script_binding,
    Object* p_object
) {
#ifdef DEBUG_ENABLED
    // I don't trust you
    if (p_object->get_script_instance()) {
        CSharpInstance* csharp_instance =
            CAST_CSHARP_INSTANCE(p_object->get_script_instance());
        CRASH_COND(
            csharp_instance != NULL
            && !csharp_instance->is_destructing_script_instance()
        );
    }
#endif
...
void* CSharpLanguage::alloc_instance_binding_data(Object* p_object) {
    ...
    Map<Object*, CSharpScriptBinding>::Element* match =
        script_bindings.find(p_object);
    if (match) {
        return (void*)match;
    }
...
Map<Object*, CSharpScriptBinding>::Element* CSharpLanguage::
    insert_script_binding(
        Object* p_object,
        const CSharpScriptBinding& p_script_binding
    ) {
...
void CSharpLanguage::free_instance_binding_data(void* p_data) {
...
#endif
        // Mono runtime finalized, all the gchandle bindings were already
        // released
        return;
    }

    if (finalizing) {
        return; // inside CSharpLanguage::finish(), all the gchandle bindings
                // are released there
    }
...

    {
        ...
        Map<Object*, CSharpScriptBinding>::Element* data =
            (Map<Object*, CSharpScriptBinding>::Element*)p_data;

        CSharpScriptBinding& script_binding = data->value();

        if (script_binding.inited) {
            // Set the native instance field to IntPtr.Zero, if not yet garbage
            // collected. This is done to avoid trying to dispose the native
...
void CSharpLanguage::refcount_incremented_instance_binding(Object* p_object) {
    ...
    if (ref_owner->reference_get_count() > 1
        && gchandle->is_weak(
        )) { // The managed side also holds a reference, hence 1 instead of 0
        GD_MONO_SCOPE_THREAD_ATTACH;

        // The reference count was increased after the managed side was the only
        // one referencing our owner. This means the owner is being referenced
        // again by the unmanaged side, so the owner must hold the managed side
        // alive again to avoid it from being GCed.

        MonoObject* target = gchandle->get_target();
        if (!target) {
            return; // Called after the managed side was collected, so nothing
                    // to do here
        }
...
bool CSharpLanguage::refcount_decremented_instance_binding(Object* p_object) {
    ...
    if (refcount == 1 && gchandle.is_valid()
        && !gchandle->is_weak(
        )) { // The managed side also holds a reference, hence 1 instead of 0
        GD_MONO_SCOPE_THREAD_ATTACH;

        // If owner owner is no longer referenced by the unmanaged side,
        // the managed instance takes responsibility of deleting the owner when
        // GCed.

        MonoObject* target = gchandle->get_target();
        if (!target) {
            return refcount == 0; // Called after the managed side was
                                  // collected, so nothing to do here
        }
...
CSharpInstance* CSharpInstance::create_for_managed_type(
    Object* p_owner,
    CSharpScript* p_script,
    const Ref<MonoGCHandle>& p_gchandle
) {
...
bool CSharpInstance::set(const StringName& p_name, const Variant& p_value) {
    ...
    while (top && top != script->native) {
        ...
        if (property) {
            property->set_value(
                mono_object,
                GDMonoMarshal::variant_to_mono_object(
                    p_value,
                    property->get_type()
                )
            );
...
void CSharpInstance::get_properties_state_for_reloading(
    List<Pair<StringName, Variant>>& r_state
) {
    ...
    for (List<PropertyInfo>::Element* E = pinfo.front(); E; E = E->next()) {
        ...
        if (!field) {
            continue; // Properties ignored. We get the property baking fields
                      // instead.
        }

        managedType = field->get_type();

        if (GDMonoMarshal::managed_to_variant_type(managedType)
            != Variant::NIL) { // If we can marshal it
...
void CSharpInstance::get_property_list(List<PropertyInfo>* p_properties) const {
    ...
    for (OrderedHashMap<StringName, PropertyInfo>::ConstElement E =
             script->member_info.front();
         E;
         E = E.next()) {
    ...
    while (top && top != script->native) {
        GDMonoMethod* method =
            top->get_method(CACHED_STRING_NAME(_get_property_list), 0);
...
Variant::Type CSharpInstance::get_property_type(
...
void CSharpInstance::get_method_list(List<MethodInfo>* p_list) const {
    ...
    // TODO: We're filtering out constructors but there may be other methods
    // unsuitable for explicit calls.
    GDMonoClass* top = script->script_class;
...
void CSharpInstance::call_multilevel(
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
void CSharpInstance::_call_multilevel(
    MonoObject* p_mono_object,
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
void CSharpInstance::call_multilevel_reversed(
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
bool CSharpInstance::_reference_owner_unsafe() {
    ...
    // Unsafe refcount increment. The managed instance also counts as a
    // reference. This way if the unmanaged world has no references to our owner
...
bool CSharpInstance::_unreference_owner_unsafe() {
    ...
    // reference. See: _reference_owner_unsafe()

    // Destroying the owner here means self destructing, so we defer the owner
...
MonoObject* CSharpInstance::_internal_new_managed() {
    ...
    // Search the constructor first, to fail with an error if it's not found
    // before allocating anything else.
    GDMonoMethod* ctor =
        script->script_class->get_method(CACHED_STRING_NAME(dotctor), 0);
    ERR_FAIL_NULL_V_MSG(
        ctor,
        NULL,
        "Cannot create script instance because the class does not define a "
    ...
    MonoObject* mono_object = mono_object_new(
        mono_domain_get(),
        script->script_class->get_mono_ptr()
    );
    ...
    if (!mono_object) {
        ...
        // Not ok for the owner to die here. If there is a situation where this
        // can happen, it will be considered a bug.
        CRASH_COND(die);
    ...
    if (base_ref) {
        _reference_owner_unsafe(); // Here, after assigning the gchandle (for
...
void CSharpInstance::mono_object_disposed_baseref(
    MonoObject* p_obj,
    bool p_is_finalizer,
    bool& r_delete_owner,
    bool& r_remove_script_instance
) {
#ifdef DEBUG_ENABLED
    CRASH_COND(!base_ref);
    CRASH_COND(gchandle.is_null());
    ...
    if (_unreference_owner_unsafe()) {
        // Safe to self destruct here with memdelete(owner), but it's deferred
        // to the caller to prevent future mistakes.
        r_delete_owner = true;
    } else {
        r_delete_owner = false;
        CSharpLanguage::get_singleton()->release_script_gchandle(
            p_obj,
            gchandle
        );
        ...
        if (!p_is_finalizer) {
            ...
            // If the native instance is still alive and this is called from the
            // finalizer, then it was referenced from another thread before the
            // finalizer could unreference and delete it, so we want to keep it.
            // GC.ReRegisterForFinalize(this) is not safe because the objects
            // referenced by 'this' could have already been collected. Instead
            // we will create a new managed instance here.
...
void CSharpInstance::refcount_incremented() {
    ...
    if (ref_owner->reference_get_count() > 1
        ...
        // Release the current weak handle and replace it with a strong handle.
        uint32_t strong_gchandle =
            MonoGCHandle::new_strong_handle(gchandle->get_target());
        gchandle->release();
        gchandle->set_handle(strong_gchandle, MonoGCHandle::STRONG_HANDLE);
    }
}
...
bool CSharpInstance::refcount_decremented() {
    ...
    if (refcount == 1
        ...
        uint32_t weak_gchandle =
            MonoGCHandle::new_weak_handle(gchandle->get_target());
        gchandle->release();
        gchandle->set_handle(weak_gchandle, MonoGCHandle::WEAK_HANDLE);
...
MultiplayerAPI::RPCMode CSharpInstance::_member_get_rpc_mode(
    IMonoClassMember* p_member
) const {
...
MultiplayerAPI::RPCMode CSharpInstance::get_rpc_mode(const StringName& p_method
) const {
    GD_MONO_SCOPE_THREAD_ATTACH;
...
MultiplayerAPI::RPCMode CSharpInstance::get_rset_mode(
...
void CSharpInstance::notification(int p_notification) {
    ...
    if (p_notification == Object::NOTIFICATION_PREDELETE) {
        // When NOTIFICATION_PREDELETE is sent, we also take the chance to call
        // Dispose(). It's safe to call Dispose() multiple times and
        // NOTIFICATION_PREDELETE is guaranteed to be sent at least once, which
        // happens right before the call to the destructor.

        predelete_notified = true;

        if (base_ref) {
            // It's not safe to proceed if the owner derives Reference and the
            // refcount reached 0. At this point, Dispose() was already called
            // (manually or from the finalizer) so that's not a problem. The
...
CSharpInstance::~CSharpInstance() {
    ...
    if (gchandle.is_valid()) {
        if (!predelete_notified && !ref_dying) {
            ...
            // This could be being called from the owner's set_script_instance
            // method, meaning this script is being replaced with another one.
            // If this is the case, we must call Dispose here, because Dispose
            // calls owner->set_script_instance(NULL) and that would mess up
            // with the new script instance if called later.
    ...
    // If not being called from the owner's destructor, and we still hold a
    ...
    if (base_ref && !ref_dying && owner && unsafe_referenced) {
        ...
        // We will unreference the owner before referencing it again, so we need
        // to keep it alive
        Ref<Reference> scope_keep_owner_alive(ref_owner);
        (void)scope_keep_owner_alive;

        // Unreference the owner here, before the new "instance binding"
        // references it. Otherwise, the unsafe reference debug checks will
        // incorrectly detect a bug.
        bool die = _unreference_owner_unsafe();
        CRASH_COND(die
        ); // `owner_keep_alive` holds a reference, so it can't die

        void* data = owner->get_script_instance_binding(
        ...
        if (!script_binding.inited) {
            MutexLock lock(
                CSharpLanguage::get_singleton()->get_language_bind_mutex()
            );

            if (!script_binding.inited) { // Other thread may have set it up
                // Already had a binding that needs to be setup
                CSharpLanguage::get_singleton()->setup_csharp_script_binding(
                    script_binding,
                    owner
                );
...
#ifdef DEBUG_ENABLED
        // The "instance binding" holds a reference so the refcount should be at
        // least 2 before `scope_keep_owner_alive` goes out of scope
...
#ifdef DEBUG_ENABLED
        // CSharpInstance must not be created unless it's going to be added to
        // the list for sure
...
void CSharpScript::_update_exports_values(
    ...
    for (Map<StringName, Variant>::Element* E =
             exported_members_defval_cache.front();
         E;
         E = E->next()) {
        values[E->key()] = E->get();
    }

    for (List<PropertyInfo>::Element* E = exported_members_cache.front(); E;
         E                              = E->next()) {
    ...
    if (exports_invalidated) {
        ...
        while (top && top != native) {
            ...
            for (int i = fields.size() - 1; i >= 0; i--) {
                ...
                if (_get_member_export(
                        field,
                        /* inspect export: */ false,
                        prop_info,
                        exported
                    )) {
            ...
            const Vector<GDMonoProperty*>& properties =
                top->get_all_properties();
...
bool CSharpScript::_update_exports(
    PlaceHolderScriptInstance* p_instance_to_update
) {
#ifdef TOOLS_ENABLED
...
#endif
    {
        ...
        if (is_editor) {
            ...
            // Here we create a temporary managed instance of the class to get
            // the initial values
            tmp_object = mono_object_new(
                mono_domain_get(),
                script_class->get_mono_ptr()
            );
            ...
            tmp_pinned_gchandle =
                MonoGCHandle::new_strong_handle_pinned(tmp_object
                ); // pin it (not sure if needed)
            ...
            ERR_FAIL_NULL_V_MSG(
                ctor,
                NULL,
                "Cannot construct temporary MonoObject because the class does "
                "not define a parameterless constructor: '"
                    + get_path() + "'."
            );
            ...
            if (ctor_exc) {
                // TODO: Should we free 'tmp_native' if the exception was thrown
                // after its creation?
                ...
                ERR_PRINT(
                    "Exception thrown from constructor of temporary MonoObject:"
                );
                ...
                if (_get_member_export(
                    ...
                    if (exported) {
                        ...
                        if (is_editor) {
                            ...
                            if (tmp_object) {
                                exported_members_defval_cache[member_name] =
                                    GDMonoMarshal::mono_object_to_variant(
                                        field->get_value(tmp_object)
                                    );
                            }
                        }
#endif
                    ...
                    if (exported) {
                        ...
                        if (is_editor) {
                            ...
                            if (tmp_object) {
                                ...
                                MonoObject* ret =
                                    property->get_value(tmp_object, &exc);
                                if (exc) {
                                    exported_members_defval_cache[member_name] =
                                        Variant();
                                    GDMonoUtils::
                                        debug_print_unhandled_exception(exc);
                                } else {
                                    exported_members_defval_cache[member_name] =
                                        GDMonoMarshal::mono_object_to_variant(
                                            ret
        ...
        if (is_editor) {
            ...
            if (exc) {
                ERR_PRINT(
                    "Exception thrown from method Dispose() of temporary "
                    "MonoObject:"
                );
                GDMonoUtils::debug_print_unhandled_exception(exc);
            }
    ...
    if (is_editor) {
        ...
        if ((changed || p_instance_to_update) && placeholders.size()) {
            ...
            if (changed) {
                for (Set<PlaceHolderScriptInstance*>::Element* E =
                         placeholders.front();
                     E;
                     E = E->next()) {
...
void CSharpScript::load_script_signals(
    GDMonoClass* p_class,
    GDMonoClass* p_native_class
) {
...
bool CSharpScript::_get_signal(
    GDMonoClass* p_class,
    GDMonoClass* p_delegate,
            ...
            if (names.size() == types.size()) {
                for (int i = 0; i < names.size(); ++i) {
                    ...
                    if (arg.type == Variant::NIL) {
                        ERR_PRINT(
                            "Unknown type of signal parameter: '" + arg.name
                            + "' in '" + p_class->get_full_name() + "'."
                        );
                        return false;
                    }
...
bool CSharpScript::_get_member_export(
    IMonoClassMember* p_member,
    bool p_inspect_export,
    PropertyInfo& r_prop_info,
    bool& r_exported
) {
    GD_MONO_ASSERT_THREAD_ATTACHED;
...
#define MEMBER_FULL_QUALIFIED_NAME(m_member)                                   \
    (m_member->get_enclosing_class()->get_full_name() + "."                    \
     + (String)m_member->get_name())
    ...
    if (p_member->get_member_type() == IMonoClassMember::MEMBER_TYPE_PROPERTY) {
        ...
        if (!property->has_getter()) {
            ...
            if (exported) {
                ERR_PRINT(
                    "Cannot export a property without a getter: '"
                    + MEMBER_FULL_QUALIFIED_NAME(p_member) + "'."
                );
            }
#endif
        ...
        if (!property->has_setter()) {
            ...
            if (exported) {
                ERR_PRINT(
                    "Cannot export a property without a setter: '"
                    + MEMBER_FULL_QUALIFIED_NAME(p_member) + "'."
                );
            }
#endif
    ...
    if (!p_inspect_export || !exported) {
        r_prop_info = PropertyInfo(
            variant_type,
            (String)p_member->get_name(),
            PROPERTY_HINT_NONE,
            "",
    ...
    ERR_FAIL_COND_V_MSG(
        hint_res == -1,
        false,
        "Error while trying to determine information about the exported "
        "member: '"
            + MEMBER_FULL_QUALIFIED_NAME(p_member) + "'."
    );
    ...
    if (hint_res == 0) {
        hint = PropertyHint(
            CACHED_FIELD(ExportAttribute, hint)->get_int_value(attr)
        );
        hint_string =
...
#ifdef TOOLS_ENABLED
int CSharpScript::_try_get_member_export_hint(
    IMonoClassMember* p_member,
    ManagedType p_type,
    Variant::Type p_variant_type,
    bool p_allow_generics,
    PropertyHint& r_hint,
    String& r_hint_string
) {
    GD_MONO_ASSERT_THREAD_ATTACHED;

    if (p_variant_type == Variant::INT
        && p_type.type_encoding == MONO_TYPE_VALUETYPE
        ...
        MonoType* enum_basetype =
            mono_class_enum_basetype(p_type.type_class->get_mono_ptr());
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            // TODO:
            // Instead of using mono_field_get_value_object, we can do this
            // without boxing. Check the internal mono functions:
            // ves_icall_System_Enum_GetEnumValuesAndNames and the
            // get_enum_field.

            MonoObject* val_obj =
                mono_field_get_value_object(mono_domain_get(), field, NULL);

            ERR_FAIL_NULL_V_MSG(
                val_obj,
                -1,
                "Failed to get '" + enum_field_name + "' constant enum value."
            );

            bool r_error;
            uint64_t val =
                GDMonoUtils::unbox_enum_value(val_obj, enum_basetype, r_error);
            ERR_FAIL_COND_V_MSG(
                r_error,
                -1,
                "Failed to unbox '" + enum_field_name + "' constant enum value."
            );
        ...
        if (uses_default_values) {
            ...
            // That's annoying if the user is not using custom values for the
            // enum constants. This may not be needed in the future if the
        ...
        Variant::Type elem_variant_type =
            GDMonoMarshal::managed_to_variant_type(elem_type);

        PropertyHint elem_hint = PROPERTY_HINT_NONE;
        String elem_hint_string;

        ERR_FAIL_COND_V_MSG(
            elem_variant_type == Variant::NIL,
            -1,
        ...
        if (elem_variant_type == Variant::STRING) {
            MonoObject* attr =
                p_member->get_attribute(CACHED_CLASS(ExportAttribute));
            if (PropertyHint(
                    CACHED_FIELD(ExportAttribute, hint)->get_int_value(attr)
                )
                == PROPERTY_HINT_ENUM) {
                r_hint_string = itos(elem_variant_type) + "/"
                              + itos(PROPERTY_HINT_ENUM) + ":"
                              + CACHED_FIELD(ExportAttribute, hintString)
        ...
        if (!preset_hint) {
            ...
            ERR_FAIL_COND_V_MSG(
                hint_res == -1,
                -1,
                "Error while trying to determine information about the array "
                "element type."
            );

            // Format: type/hint:hint_string
            r_hint_string = itos(elem_variant_type) + "/" + itos(elem_hint)
                          + ":" + elem_hint_string;
        }
...
void CSharpScript::_get_property_list(List<PropertyInfo>* p_properties) const {
    p_properties->push_back(PropertyInfo(
        Variant::STRING,
        CSharpLanguage::singleton->string_names._script_source,
        PROPERTY_HINT_NONE,
        "",
...
Ref<CSharpScript> CSharpScript::create_for_managed_type(
    GDMonoClass* p_class,
    GDMonoClass* p_native
) {
    // This method should not fail, only assertions allowed

    CRASH_COND(p_class == NULL);

    // TODO OPTIMIZE: Cache the 'CSharpScript' associated with this 'p_class'
...
void CSharpScript::initialize_for_managed_type(
    Ref<CSharpScript> p_script,
    GDMonoClass* p_class,
    GDMonoClass* p_native
) {
    // This method should not fail, only assertions allowed
    ...
    p_script->valid = true;
    p_script->tool =
        p_script->script_class->has_attribute(CACHED_CLASS(ToolAttribute));
    p_script->reload_invalidated = false;

    if (!p_script->tool) {
        GDMonoClass* nesting_class =
            p_script->script_class->get_nesting_class();
        p_script->tool =
            nesting_class
            && nesting_class->has_attribute(CACHED_CLASS(ToolAttribute));
    }

#if TOOLS_ENABLED
    if (!p_script->tool) {
        p_script->tool = p_script->script_class->get_assembly()
                      == GDMono::get_singleton()->get_tools_assembly();
    }
#endif
...
bool CSharpScript::can_instance() const {
    ...
    // For tool scripts, this will never fire if the class is not found. That's
    // because we don't know if it's a tool script if we can't find the class to
    ...
    if (extra_cond && !script_class) {
        if (GDMono::get_singleton()->get_project_assembly() == NULL) {
            ...
            ERR_FAIL_V_MSG(
                NULL,
                "Cannot instance script because the project assembly is not "
                "loaded. Script: '"
                    + get_path() + "'."
            );
        } else {
            ...
            ERR_FAIL_V_MSG(
                NULL,
                "Cannot instance script because the class '" + name
                    + "' could not be found. Script: '" + get_path() + "'."
            );
        }
    }
...
CSharpInstance* CSharpScript::_create_instance(
    const Variant** p_args,
    int p_argcount,
    Object* p_owner,
    bool p_isref,
    ...
    // before allocating anything else.
    GDMonoMethod* ctor =
        script_class->get_method(CACHED_STRING_NAME(dotctor), p_argcount);
    if (ctor == NULL) {
        ERR_FAIL_COND_V_MSG(
            p_argcount == 0,
            NULL,
            "Cannot create script instance. The class '"
                + script_class->get_full_name()
                + "' does not define a parameterless constructor."
                + (get_path().empty() ? String()
                                      : " Path: '" + get_path() + "'.")
        );
    ...
    Ref<Reference> ref;
    if (p_isref) {
        // Hold it alive. Important if we have to dispose a script instance
        // binding before creating the CSharpInstance.
        ref = Ref<Reference>(static_cast<Reference*>(p_owner));
    }

    // If the object had a script instance binding, dispose it before adding the
    // CSharpInstance
    if (p_owner->has_script_instance_binding(
            CSharpLanguage::get_singleton()->get_language_index()
        )) {
        void* data = p_owner->get_script_instance_binding(
            CSharpLanguage::get_singleton()->get_language_index()
        );
        CRASH_COND(data == NULL);
    ...
    MonoObject* mono_object =
        mono_object_new(mono_domain_get(), script_class->get_mono_ptr());
    ...
    if (instance->base_ref) {
        instance->_reference_owner_unsafe(
        ); // Here, after assigning the gchandle (for the refcount_incremented
           // callback)
    }
...
Variant CSharpScript::_new(
    ...
    CSharpInstance* instance =
        _create_instance(p_args, p_argcount, owner, r != NULL, r_error);
    if (!instance) {
        if (ref.is_null()) {
...
ScriptInstance* CSharpScript::instance_create(Object* p_this) {
    ...
    if (native) {
        ...
        if (!ClassDB::is_parent_class(p_this->get_class_name(), native_name)) {
            if (ScriptDebugger::get_singleton()) {
                CSharpLanguage::get_singleton()->debug_break_parse(
                    get_path(),
                    0,
    ...
    return _create_instance(
        ...
        0,
        p_this,
        Object::cast_to<Reference>(p_this) != NULL,
        unchecked_error
    );
}

PlaceHolderScriptInstance* CSharpScript::placeholder_instance_create(
    Object* p_this
) {
#ifdef TOOLS_ENABLED
    PlaceHolderScriptInstance* si = memnew(PlaceHolderScriptInstance(
        CSharpLanguage::get_singleton(),
        Ref<Script>(this),
        p_this
    ));
...
Error CSharpScript::reload(bool p_keep_state) {
    ...
    GDMonoAssembly* project_assembly =
        GDMono::get_singleton()->get_project_assembly();

    if (project_assembly) {
        const Variant* script_metadata_var =
            CSharpLanguage::get_singleton()->get_scripts_metadata().getptr(
                get_path()
            );
        if (script_metadata_var) {
            Dictionary script_metadata =
                script_metadata_var->operator Dictionary()["class"];
            ...
            GDMonoClass* klass = project_assembly->get_class(
...
#ifdef DEBUG_ENABLED
            print_verbose(
                "Found class " + script_class->get_full_name() + " for script "
                + get_path()
            );
#endif
...
#if TOOLS_ENABLED
            if (!tool) {
                tool = script_class->get_assembly()
                    == GDMono::get_singleton()->get_tools_assembly();
            }
#endif
...
#ifdef DEBUG_ENABLED
            // For debug builds, we must fetch from all native base methods as
            // well. Native base methods must be fetched before the current
            // class. Not needed if the script class itself is a native class.
...
bool CSharpScript::get_property_default_value(
    const StringName& p_property,
    Variant& r_value
) const {
#ifdef TOOLS_ENABLED

    const Map<StringName, Variant>::Element* E =
        exported_members_defval_cache.find(p_property);
    if (E) {
        r_value = E->get();
        return true;
    }
...
void CSharpScript::get_script_signal_list(List<MethodInfo>* r_signals) const {
    for (const Map<StringName, Vector<Argument>>::Element* E = _signals.front();
         E;
         E = E->next()) {
        MethodInfo mi;
...
#if defined(TOOLS_ENABLED) || defined(DEBUG_ENABLED)
    if (p_members) {
        for (Set<StringName>::Element* E = exported_members_names.front(); E;
             E                           = E->next()) {
...
RES ResourceFormatLoaderCSharpScript::load(
    const String& p_path,
    ...
    ERR_FAIL_COND_V_MSG(
        err != OK,
        RES(),
        "Cannot load C# script file '" + p_path + "'."
    );
#endif
...
void ResourceFormatLoaderCSharpScript::get_recognized_extensions(
    List<String>* p_extensions
) const {
    p_extensions->push_back("cs");
}

bool ResourceFormatLoaderCSharpScript::handles_type(const String& p_type
) const {
    return p_type == "Script"
        || p_type == CSharpLanguage::get_singleton()->get_type();
}

String ResourceFormatLoaderCSharpScript::get_resource_type(const String& p_path
) const {
    return p_path.get_extension().to_lower() == "cs"
             ? CSharpLanguage::get_singleton()->get_type()
             : "";
}

Error ResourceFormatSaverCSharpScript::save(
    const String& p_path,
    ...
    if (!FileAccess::exists(p_path)) {
        // The file does not yet exists, let's assume the user just created this
        // script
        ...
        if (_create_project_solution_if_needed()) {
            ...
            ERR_PRINT(
                "C# project could not be created; cannot add file: '" + p_path
                + "'."
            );
        }
    }
    ...
    ERR_FAIL_COND_V_MSG(
        err != OK,
        err,
        "Cannot save C# script file '" + p_path + "'."
    );
...
void ResourceFormatSaverCSharpScript::get_recognized_extensions(
    const RES& p_resource,
    List<String>* p_extensions
) const {
