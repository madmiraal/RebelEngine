...
struct Version {
    ...
    static Version get_from_loaded_assembly(
...
public:
...
#ifdef TOOLS_ENABLED
    bool copy_prebuilt_api_assembly(
    ...
    GD_NORETURN static void unhandled_exception_hook(
        MonoObject* p_exc,
        void* p_user_data
    );
    ...
    _FORCE_INLINE_ bool is_runtime_initialized() const {
        return runtime_initialized
            ...
            ) /* stays true after shutdown finished */;
    }
...
public:
    ScopeExitDomainUnload(MonoDomain* p_domain) : domain(p_domain) {}
...
#define _GDMONO_SCOPE_EXIT_DOMAIN_UNLOAD_(m_mono_domain)                       \
    gdmono::ScopeExitDomainUnload __gdmono__scope__exit__domain__unload__(     \
        m_mono_domain                                                          \
    );                                                                         \
