...
String determine_app_native_lib_dir() {
    ...
    ScopedLocalRef<jobject> applicationInfo(
        env,
        env->CallObjectMethod(activity, getApplicationInfo)
    );
    jfieldID nativeLibraryDirField = env->GetFieldID(
        env->GetObjectClass(applicationInfo),
        "nativeLibraryDir",
        "Ljava/lang/String;"
    );
    ScopedLocalRef<jstring> nativeLibraryDir(
        env,
        (jstring)env->GetObjectField(applicationInfo, nativeLibraryDirField)
    );

    String result;

    const char* const nativeLibraryDirUtf8 =
        env->GetStringUTFChars(nativeLibraryDir, NULL);
...
void* _try_dlopen_file_path(const String& p_so_path, int p_flags) {
    ...
    if (!handle) {
        if (OS::get_singleton()->is_stdout_verbose()) {
            OS::get_singleton()->print(
                "Failed to open shared library: '%s'. Error: '%s'\n",
                p_so_path.utf8().get_data(),
                dlerror()
            );
        }
    ...
    if (OS::get_singleton()->is_stdout_verbose()) {
        OS::get_singleton()->print(
            "Successfully loaded shared library: '%s'\n",
            p_so_path.utf8().get_data()
        );
    }
    ...
    if (p_name == NULL) {
        ...
        if (!mono_dl_handle) {
            ...
            ERR_FAIL_COND_V_MSG(
                !mono_dl_handle,
                nullptr,
                "Failed to load Mono native library from path"
            );
        }
...
void* gd_mono_android_dlclose(void* p_handle, void* p_user_data) {
    ...
    // Not sure if this ever happens. Does Mono close the handle for the main
    // module?
...
int32_t get_build_version_sdk_int() {
    ...
    if (build_version_sdk_int == 0) {
        ...
        build_version_sdk_int =
            (int32_t)env->GetStaticIntField(versionClass, sdkIntField);
    }
...
MonoBoolean _gd_mono_init_cert_store() {
    ...
    ScopedLocalRef<jclass> keyStoreClass(
        env,
        env->FindClass("java/security/KeyStore")
    );
    ...
    ScopedLocalRef<jstring> androidCAStoreString(
        env,
        env->NewStringUTF("AndroidCAStore")
    );

    ScopedLocalRef<jobject> certStoreLocal(
        env,
        env->CallStaticObjectMethod(
            keyStoreClass,
            getInstance,
            androidCAStoreString.get()
        )
    );
...
MonoArray* _gd_mono_android_cert_store_lookup(MonoString* p_alias) {
    ...
    if (!mono_error_ok(&mono_error)) {
        ERR_PRINT(
            String() + "Failed to convert MonoString* to UTF-8: '"
            + mono_error_get_message(&mono_error) + "'."
        );
    ...
    jmethodID getEncoded =
        env->GetMethodID(certificateClass, "getEncoded", "()[B");
    ERR_FAIL_NULL_V(getEncoded, NULL);

    ScopedLocalRef<jobject> certificate(
        env,
        env->CallObjectMethod(certStore, getCertificate, js_alias.get())
    );
    ...
    ScopedLocalRef<jbyteArray> encoded(
        env,
        (jbyteArray)env->CallObjectMethod(certificate, getEncoded)
    );
    jsize encodedLength = env->GetArrayLength(encoded);

    MonoArray* encoded_ret = mono_array_new(
        mono_domain_get(),
        CACHED_CLASS_RAW(uint8_t),
        encodedLength
    );
...
void register_internal_calls() {
    GDMonoUtils::add_internal_call(
        "Android.Runtime.AndroidEnvironment::_gd_mono_init_cert_store",
        _gd_mono_init_cert_store
    );
    GDMonoUtils::add_internal_call(
        "Android.Runtime.AndroidEnvironment::_gd_mono_android_cert_store_"
        "lookup",
        _gd_mono_android_cert_store_lookup
    );
}

void initialize() {
    // We need to set this environment variable to make the monodroid BCL use
    // btls instead of legacy as the default provider
    OS::get_singleton()->set_environment("XA_TLS_PROVIDER", "btls");

    mono_dl_fallback_register(
        gd_mono_android_dlopen,
        gd_mono_android_dlsym,
        gd_mono_android_dlclose,
        NULL
    );
...
// The following are P/Invoke functions required by the monodroid profile of the
// BCL. These are P/Invoke functions and not internal calls, hence why they use
...
GD_PINVOKE_EXPORT int32_t
monodroid_get_system_property(const char* p_name, char** r_value) {
...
GD_PINVOKE_EXPORT mono_bool _monodroid_get_network_interface_up_state(
    const char* p_ifname,
    mono_bool* r_is_up
) {
    // The JNI code is the equivalent of:
    ...
    *r_is_up = 0;
    ...
    jmethodID getByName = env->GetStaticMethodID(
    ...
    ScopedLocalRef<jobject> networkInterface(
        env,
        env->CallStaticObjectMethod(
            networkInterfaceClass,
            getByName,
...
GD_PINVOKE_EXPORT mono_bool _monodroid_get_network_interface_supports_multicast(
    const char* p_ifname,
    mono_bool* r_supports_multicast
) {
    // The JNI code is the equivalent of:
    //
    ...
    *r_supports_multicast = 0;
    ...
    jmethodID supportsMulticast =
        env->GetMethodID(networkInterfaceClass, "supportsMulticast", "()Z");
    ...
    *r_supports_multicast =
        (mono_bool)env->CallBooleanMethod(networkInterface, supportsMulticast);
...
static void interop_get_active_network_dns_servers(
    ...
    // connectivityManager.getLinkProperties(activeNerwork); List<String>
    // dnsServers = linkProperties.getDnsServers().stream()
    // .map(inetAddress ->
    // inetAddress.getHostAddress()).collect(Collectors.toList());
    ...
    ScopedLocalRef<jobject> applicationContext(
        env,
    ...
    jfieldID connectivityServiceField = env->GetStaticFieldID(
        contextClass,
        "CONNECTIVITY_SERVICE",
        "Ljava/lang/String;"
    );
    ScopedLocalRef<jstring> connectivityServiceString(
        env,
        (jstring
        )env->GetStaticObjectField(contextClass, connectivityServiceField)
    );
    ...
    ScopedLocalRef<jobject> connectivityManager(
        env,
        env->CallObjectMethod(
            applicationContext,
            getSystemService,
            connectivityServiceString.get()
        )
    );
    ...
    ScopedLocalRef<jclass> connectivityManagerClass(
        env,
        env->FindClass("android/net/ConnectivityManager")
    );
    ...
    ScopedLocalRef<jobject> activeNetwork(
        env,
        env->CallObjectMethod(connectivityManager, getActiveNetwork)
    );
    ...
    jmethodID getLinkProperties = env->GetMethodID(
        connectivityManagerClass,
        "getLinkProperties",
    ...
    ScopedLocalRef<jobject> linkProperties(
        env,
        env->CallObjectMethod(
            connectivityManager,
            getLinkProperties,
    ...
    ScopedLocalRef<jclass> linkPropertiesClass(
        env,
        env->FindClass("android/net/LinkProperties")
    );
    ERR_FAIL_NULL(linkPropertiesClass);

    jmethodID getDnsServers = env->GetMethodID(
        linkPropertiesClass,
        "getDnsServers",
        "()Ljava/util/List;"
    );
    ...
    ScopedLocalRef<jobject> dnsServers(
        env,
        env->CallObjectMethod(linkProperties, getDnsServers)
    );
    ...
    ScopedLocalRef<jclass> inetAddressClass(
        env,
        env->FindClass("java/net/InetAddress")
    );
    ERR_FAIL_NULL(inetAddressClass);

    jmethodID getHostAddress = env->GetMethodID(
        inetAddressClass,
        "getHostAddress",
        "()Ljava/lang/String;"
    );
    ...
    for (int i = 0; i < dnsServersCount; i++) {
        ScopedLocalRef<jobject> dnsServer(
            env,
            env->CallObjectMethod(dnsServers, listGet, (jint)i)
        );
        if (!dnsServer) {
            continue;
        }

        ScopedLocalRef<jstring> hostAddress(
            env,
            (jstring)env->CallObjectMethod(dnsServer, getHostAddress)
        );
...
GD_PINVOKE_EXPORT int32_t _monodroid_get_dns_servers(void** r_dns_servers_array
    ...
    *r_dns_servers_array = NULL;
    ...
    if (_monodroid_get_android_api_level() < 26) {
        // The 'net.dns*' system properties are no longer available in
        // Android 8.0 (API level 26) and greater:
        ...
        for (int i = 0; i < dns_servers_len; i++) {
            ...
            if (len > 0) {
                dns_servers[dns_servers_count] =
                    strndup(prop_value, (size_t)len); // freed by the BCL
                dns_servers_count++;
                free(prop_value);
            }
        }
...
GD_PINVOKE_EXPORT const char* _monodroid_timezone_get_default_id() {
    ...
    ScopedLocalRef<jclass> timeZoneClass(
        env,
        env->FindClass("java/util/TimeZone")
    );
    ...
    jmethodID getID =
        env->GetMethodID(timeZoneClass, "getID", "()Ljava/lang/String;");
    ERR_FAIL_NULL_V(getID, NULL);

    ScopedLocalRef<jobject> defaultTimeZone(
        env,
        env->CallStaticObjectMethod(timeZoneClass, getDefault)
    );
    ...
    ScopedLocalRef<jstring> defaultTimeZoneID(
        env,
        (jstring)env->CallObjectMethod(defaultTimeZone, getID)
    );
    ...
    const char* default_time_zone_id =
        env->GetStringUTFChars(defaultTimeZoneID, 0);
