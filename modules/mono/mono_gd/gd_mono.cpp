...
// This will no longer be the case if we replace appdomains with
// AssemblyLoadContext
...
// TODO:
// This has turn into a gigantic mess. There's too much going on here. Too much
// #ifdef as well. It's just painful to read... It needs to be re-structured.
// Please, clean this up, future me.
...
void gd_mono_setup_runtime_main_args() {
    ...
    for (List<String>::Element* E = cmdline_args.front(); E; E = E->next()) {
        CharString& stored =
            cmdline_args_utf8.push_back(E->get().utf8())->get();
...
void gd_mono_profiler_init() {
    String profiler_args = GLOBAL_DEF(
        "mono/profiler/args",
    ...
    if (OS::get_singleton()->has_environment(env_var_name)) {
        const auto mono_env_ops =
            OS::get_singleton()->get_environment(env_var_name);
        // Usually MONO_ENV_OPTIONS looks like:
        // --profile=jb:prof=timeline,ctl=remote,host=127.0.0.1:55467
        const String prefix = "--profile=";
        if (mono_env_ops.begins_with(prefix)) {
            const auto ops =
                mono_env_ops.substr(prefix.length(), mono_env_ops.length());
...
void gd_mono_debug_init() {
    ...
    if (da_args.length()) {
        ...
        // Try with command line arguments. This is useful on platforms where
        // it's difficult to pass environment variables. The command line
        // arguments can be specified in the export options.
...
#ifdef TOOLS_ENABLED
    // Define debugger_agent settings even if we end up ignoring the current
    // values. This makes sure they show up in Editor Settings.
    ...
    if (da_args.length() == 0) {
        ...
        da_args = String(
                      ...
                      + itos(da_port) + ",embedding=1,server=y,suspend="
                      + (da_suspend ? "y,timeout=" + itos(da_timeout) : "n")
        )
                      .utf8();
    }
#else
    if (da_args.length() == 0) {
        return; // Exported games don't use the project settings to setup the
                // debugger agent
    }
#endif
    ...
    // --debugger-agent=help
    const char* options[] = {"--soft-breakpoints", da_args.get_data()};
...
void GDMono::add_mono_shared_libs_dir_to_path() {
    ...
    // By default Mono seems to search shared libraries in the following
    // directories: Current working directory, @executable_path@ and PATH The
    // parent directory of the image file (assembly where the dllimport method
    // is declared)
    // @executable_path@/../lib
    // @executable_path@/../Libraries (__MACH__ only)

    // This does not work when embedding Mono unless we use the same directory
    // structure. To fix this we append the directory containing our shared
    // libraries to PATH.
    ...
    if (DirAccess::exists(bundled_lib_dir)) {
        ...
        // TODO: Do we need to add the lib dir when using the system installed
...
void GDMono::determine_mono_dirs(
    ...
    if (mono_reg_info.assembly_dir.length()
        && DirAccess::exists(mono_reg_info.assembly_dir)) {
        r_assembly_rootdir = mono_reg_info.assembly_dir;
    }

    if (mono_reg_info.config_dir.length()
    ...
    if (r_assembly_rootdir.empty() || r_config_dir.empty()) {
        ...
        // Assertion: if they are not set, then they weren't found in the
        // registry
        CRASH_COND(
            mono_reg_info.assembly_dir.length() > 0
            || mono_reg_info.config_dir.length() > 0
        );
    }
#endif // WINDOWS_ENABLED
...
void GDMono::initialize() {
    ...
    mono_set_dirs(
        assembly_rootdir.length() ? assembly_rootdir.utf8().get_data() : NULL,
    ...
    // Exported games that don't use C# must still work. They likely don't ship
    // with mscorlib. We only initialize the Mono runtime if we can find
    // mscorlib. Otherwise it would crash.
    if (GDMonoAssembly::find_assembly("mscorlib.dll").empty()) {
        print_verbose(
            "Mono: Skipping runtime initialization because 'mscorlib.dll' "
            "could not be found"
        );
        return;
    }
    ...
    // FIXME: Temporary workaround. See:
    // https://github.com/godotengine/godot/issues/29812
    ...
    // NOTE: Internal calls must be registered after the Mono runtime
    // initialization. Otherwise registration fails with the error: 'assertion
    // 'hash != NULL' failed'.
    ...
    ERR_FAIL_COND_MSG(
        !corlib_loaded,
        "Mono: Failed to load mscorlib assembly."
    );
    ...
    ERR_FAIL_COND_MSG(
        domain_load_err != OK,
        "Mono: Failed to load scripts domain."
    );
#else
...
void GDMono::initialize_load_assemblies() {
    ...
    CRASH_NOW_MSG(
        "Mono: This binary was built with 'mono_glue=no'; cannot load "
        "assemblies."
    );
#endif
    ...
    // Load the project's main assembly. This doesn't necessarily need to
    // succeed. The game may not be using .NET at all, or if the project does
    // use .NET and we're running in the editor, it may just happen to be it
    // wasn't built yet.
...
bool GDMono::_are_api_assemblies_out_of_sync() {
    ...
    if (!out_of_sync) {
        out_of_sync =
            editor_api_assembly.assembly && editor_api_assembly.out_of_sync;
    }
#endif
    return out_of_sync;
}
...
void GDMono::_init_exception_policy() {
    PropertyInfo exc_policy_prop = PropertyInfo(
        Variant::INT,
        "mono/unhandled_exception_policy",
        PROPERTY_HINT_ENUM,
        vformat(
            "Terminate Application:%s,Log Error:%s",
            (int)POLICY_TERMINATE_APP,
            (int)POLICY_LOG_ERROR
        )
    );
...
bool GDMono::load_assembly(
    const String& p_name,
    GDMonoAssembly** r_assembly,
    bool p_refonly
) {
#ifdef DEBUG_ENABLED
...
bool GDMono::load_assembly(
    ...
    return load_assembly(
        ...
        r_assembly,
        p_refonly,
        GDMonoAssembly::get_default_search_dirs()
    );
}
...
bool GDMono::load_assembly(
    ...
    print_verbose(
        "Mono: Loading assembly " + p_name + (p_refonly ? " (refonly)" : "")
        + "..."
    );

    GDMonoAssembly* assembly =
        GDMonoAssembly::load(p_name, p_aname, p_refonly, p_search_dirs);
    ...
    *r_assembly = assembly;
    ...
    print_verbose(
        "Mono: Assembly " + p_name + (p_refonly ? " (refonly)" : "")
        + " loaded from path: " + (*r_assembly)->get_path()
    );

    return true;
}

bool GDMono::load_assembly_from(
    const String& p_name,
    const String& p_path,
    GDMonoAssembly** r_assembly,
    bool p_refonly
    ...
    GDMonoAssembly* assembly =
        GDMonoAssembly::load_from(p_name, p_path, p_refonly);
...
ApiAssemblyInfo::Version ApiAssemblyInfo::Version::get_from_loaded_assembly(
    GDMonoAssembly* p_api_assembly,
    ApiAssemblyInfo::Type p_api_type
) {
    ...
    GDMonoClass* nativecalls_klass =
        p_api_assembly->get_class(BINDINGS_NAMESPACE, nativecalls_name);
    ...
    if (nativecalls_klass) {
        ...
        GDMonoField* binds_ver_field =
            nativecalls_klass->get_field("bindings_version");
        if (binds_ver_field) {
            api_assembly_version.bindings_version =
                GDMonoMarshal::unbox<uint32_t>(binds_ver_field->get_value(NULL)
                );
        }

        GDMonoField* cs_glue_ver_field =
            nativecalls_klass->get_field("cs_glue_version");
        if (cs_glue_ver_field) {
            api_assembly_version.cs_glue_version =
                GDMonoMarshal::unbox<uint32_t>(cs_glue_ver_field->get_value(NULL
                ));
        }
    }
...
#ifdef TOOLS_ENABLED
bool GDMono::copy_prebuilt_api_assembly(
    ApiAssemblyInfo::Type p_api_type,
    ...
    if (da->copy(src_dir.plus_file(xml_file), dst_dir.plus_file(xml_file))
        != OK) {
        WARN_PRINT("Failed to copy '" + xml_file + "'.");
    }

    String pdb_file = assembly_name + ".pdb";
    if (da->copy(src_dir.plus_file(pdb_file), dst_dir.plus_file(pdb_file))
        != OK) {
    ...
    if (da->copy(
            src_dir.plus_file(assembly_file),
            dst_dir.plus_file(assembly_file)
        )
        != OK) {
...
static bool try_get_cached_api_hash_for(
    const String& p_api_assemblies_dir,
    bool& r_out_of_sync
) {
    String core_api_assembly_path =
        p_api_assemblies_dir.plus_file(CORE_API_ASSEMBLY_NAME ".dll");
    String editor_api_assembly_path =
        p_api_assemblies_dir.plus_file(EDITOR_API_ASSEMBLY_NAME ".dll");

    if (!FileAccess::exists(core_api_assembly_path)
        || !FileAccess::exists(editor_api_assembly_path)) {
        return false;
    }

    String cached_api_hash_path =
        p_api_assemblies_dir.plus_file("api_hash_cache.cfg");
    ...
    if (FileAccess::get_modified_time(core_api_assembly_path)
            != (uint64_t)cfg->get_value("core", "modified_time")
        || FileAccess::get_modified_time(editor_api_assembly_path)
               != (uint64_t)cfg->get_value("editor", "modified_time")) {
        return false;
    }
...
static void create_cached_api_hash_for(const String& p_api_assemblies_dir) {
    ...
    cfg->set_value(
        "core",
        "modified_time",
        FileAccess::get_modified_time(core_api_assembly_path)
    );
    cfg->set_value(
        "editor",
        "modified_time",
...
bool GDMono::_temp_domain_load_are_assemblies_out_of_sync(const String& p_config
    ...
    if (!_try_load_api_assemblies(
            temp_core_api_assembly,
            temp_editor_api_assembly,
            p_config,
            /* refonly: */ true,
            /* loaded_callback: */ NULL
        )) {
        return temp_core_api_assembly.out_of_sync
            || temp_editor_api_assembly.out_of_sync;
    }
...
String GDMono::update_api_assemblies_from_prebuilt(
    const String& p_config,
    const bool* p_core_api_out_of_sync,
    const bool* p_editor_api_out_of_sync
) {
#define FAIL_REASON(m_out_of_sync, m_prebuilt_exists)                          \
    ((m_out_of_sync ? String("The assembly is invalidated ")                   \
    ...
    String editor_assembly_path =
        dst_assemblies_dir.plus_file(EDITOR_API_ASSEMBLY_NAME ".dll");
    ...
    if (p_core_api_out_of_sync && p_editor_api_out_of_sync) {
        ...
        if (!try_get_cached_api_hash_for(
            ...
            api_assemblies_out_of_sync =
                _temp_domain_load_are_assemblies_out_of_sync(p_config);
        }
    }

    // Note: Even if only one of the assemblies if missing or out of sync, we
    // update both

    if (!api_assemblies_out_of_sync && FileAccess::exists(core_assembly_path)
    ...
    String prebuilt_editor_dll_path =
        prebuilt_api_dir.plus_file(EDITOR_API_ASSEMBLY_NAME ".dll");

    if (!FileAccess::exists(prebuilt_core_dll_path)
        || !FileAccess::exists(prebuilt_editor_dll_path)) {
        return FAIL_REASON(
            api_assemblies_out_of_sync,
...
bool GDMono::_load_core_api_assembly(
    LoadedApiAssembly& r_loaded_api_assembly,
    const String& p_config,
    bool p_refonly
) {
    ...
    bool success = FileAccess::exists(assembly_path)
                && load_assembly_from(
                       CORE_API_ASSEMBLY_NAME,
                       assembly_path,
                       &r_loaded_api_assembly.assembly,
                       p_refonly
                );
#else
    bool success = load_assembly(
        CORE_API_ASSEMBLY_NAME,
        &r_loaded_api_assembly.assembly,
        p_refonly
    );
    ...
    if (success) {
        ApiAssemblyInfo::Version api_assembly_ver =
            ApiAssemblyInfo::Version::get_from_loaded_assembly(
                r_loaded_api_assembly.assembly,
...
bool GDMono::_try_load_api_assemblies(
    LoadedApiAssembly& r_core_api_assembly,
    LoadedApiAssembly& r_editor_api_assembly,
    const String& p_config,
    bool p_refonly,
    CoreApiAssemblyLoadedCallback p_callback
) {
    ...
    // Check if the core API assembly is out of sync only after trying to load
    // the editor API assembly. Otherwise, if both assemblies are out of sync,
    // we would only update the former as we won't know the latter also needs to
    // be updated.
...
bool GDMono::_try_load_api_assemblies_preset() {
    return _try_load_api_assemblies(
        core_api_assembly,
        editor_api_assembly,
        get_expected_api_build_config(),
        /* refonly: */ false,
...
void GDMono::_load_api_assemblies() {
    ...
    if (!api_assemblies_loaded) {
        // The API assemblies are out of sync or some other error happened.
        ...
        CRASH_COND_MSG(
            domain_unload_err != OK,
            "Mono: Failed to unload scripts domain."
        );
        ...
        String update_error = update_api_assemblies_from_prebuilt(
            "Debug",
            &core_api_assembly.out_of_sync,
            &editor_api_assembly.out_of_sync
        );
    ...
    if (!api_assemblies_loaded) {
        ...
        if (_are_api_assemblies_out_of_sync()) {
            ...
            if (editor_api_assembly.out_of_sync) {
                ERR_PRINT("The assembly '" EDITOR_API_ASSEMBLY_NAME
                          "' is out of sync.");
            }
#endif
...
void GDMono::_install_trace_listener() {
    ...
    GDMonoClass* debug_utils = get_core_api_assembly()->get_class(
        BINDINGS_NAMESPACE,
        "DebuggingUtils"
    );
    GDMonoMethod* install_func =
        debug_utils->get_method("InstallTraceListener");
...
Error GDMono::_load_scripts_domain() {
    ...
    ERR_FAIL_NULL_V_MSG(
        scripts_domain,
        ERR_CANT_CREATE,
        "Mono: Could not create scripts app domain."
    );
...
Error GDMono::reload_scripts_domain() {
    ...
    // However, unlike the API and tools assemblies, the application can
    // continue working.
    if (!_load_project_assembly()) {
        print_error("Mono: Failed to load project assembly");
...
Error GDMono::finalize_and_unload_domain(MonoDomain* p_domain) {
    ...
    CRASH_COND(
        p_domain == GDMono::get_singleton()->get_scripts_domain()
    ...
    if (exc) {
        ERR_PRINT(
            "Exception thrown when unloading domain '" + domain_name + "'."
        );
        GDMonoUtils::debug_print_unhandled_exception(exc);
        return FAILED;
    }
...
GDMonoClass* GDMono::get_class(
    const StringName& p_namespace,
...
void GDMono::_domain_assemblies_cleanup(uint32_t p_domain_id) {
    HashMap<String, GDMonoAssembly*>& domain_assemblies =
        assemblies[p_domain_id];
...
void GDMono::unhandled_exception_hook(MonoObject* p_exc, void*) {
    // This method will be called by the runtime when a thrown exception is not
    // handled. It won't be called when we manually treat a thrown exception as
    // unhandled. We assume the exception was already printed before calling
    // this hook.
...
bool _RebelSharp::is_domain_finalizing_for_unload(MonoDomain* p_domain) {
    ...
    if (p_domain == GDMono::get_singleton()->get_scripts_domain()
        && GDMono::get_singleton()->is_finalizing_scripts_domain()) {
        return true;
    }
