...
void GDMonoAssembly::fill_search_dirs(
    Vector<String>& r_search_dirs,
    const String& p_custom_config,
    const String& p_custom_bcl_dir
) {
    ...
    if (!p_custom_bcl_dir.empty()) {
        ...
        framework_dir = String::utf8(mono_assembly_getrootdir())
                            .plus_file("mono")
                            .plus_file("4.5");
    }
...
//
// - The 'search' hook checks if the assembly has already been loaded, to avoid
// loading again.
...
// - The 'load' hook is called after the assembly has been loaded. Its job is to
// add the
//   assembly to the list of loaded assemblies so that the 'search' hook can
//   look it up.

void GDMonoAssembly::assembly_load_hook(
    ...
    GDMono::get_singleton()->add_assembly(
        domain ? mono_domain_get_id(domain) : 0,
        gdassembly
    );
}

MonoAssembly* GDMonoAssembly::assembly_search_hook(
...
MonoAssembly* GDMonoAssembly::assembly_refonly_search_hook(
    MonoAssemblyName* aname,
    void* user_data
) {
...
MonoAssembly* GDMonoAssembly::assembly_preload_hook(
    MonoAssemblyName* aname,
    char** assemblies_path,
    void* user_data
) {
    return GDMonoAssembly::_preload_hook(
        aname,
        assemblies_path,
        user_data,
        false
...
MonoAssembly* GDMonoAssembly::assembly_refonly_preload_hook(
    MonoAssemblyName* aname,
    char** assemblies_path,
    void* user_data
) {
...
MonoAssembly* GDMonoAssembly::_search_hook(
    MonoAssemblyName* aname,
    void* user_data,
    bool refonly
) {
    ...
    GDMonoAssembly* loaded_asm = GDMono::get_singleton()->get_loaded_assembly(
        has_extension ? name.get_basename() : name
    );
    if (loaded_asm) {
...
MonoAssembly* GDMonoAssembly::_preload_hook(
    MonoAssemblyName* aname,
    char**,
    void* user_data,
    bool refonly
...
MonoAssembly* GDMonoAssembly::_load_assembly_search(
    const String& p_name,
...
MonoAssembly* GDMonoAssembly::_real_load_assembly_from(
    const String& p_path,
    bool p_refonly,
    MonoAssemblyName* p_aname
) {
    ...
    ERR_FAIL_COND_V_MSG(
        data.empty(),
        NULL,
        "Could read the assembly in the specified location"
    );
    ...
    ERR_FAIL_COND_V_MSG(
        ...
        NULL,
        "Failed to open assembly image from memory: '" + p_path + "'."
    );

    if (p_aname != nullptr) {
        // Check assembly version
        const MonoTableInfo* table =
            mono_image_get_table_info(image, MONO_TABLE_ASSEMBLY);
        ...
        if (mono_table_info_get_rows(table)) {
            ...
            // Not sure about .NET's policy. We will only ensure major and minor
            // are equal, and ignore build and revision.
            uint16_t major = cols[MONO_ASSEMBLY_MAJOR_VERSION];
            uint16_t minor = cols[MONO_ASSEMBLY_MINOR_VERSION];

            uint16_t required_minor;
            uint16_t required_major = mono_assembly_name_get_version(
                p_aname,
                &required_minor,
                nullptr,
                nullptr
...
no_pdb:
    ...
    bool need_manual_load_hook =
        mono_image_get_assembly(image)
        != nullptr; // Re-using an existing image with an assembly loaded
    ...
    ERR_FAIL_COND_V_MSG(
        status != MONO_IMAGE_OK || !assembly,
        nullptr,
        "Failed to load assembly for image"
    );
    ...
    if (need_manual_load_hook) {
        ...
        bool has_extension = name.ends_with(".dll") || name.ends_with(".exe");
        GDMonoAssembly* loaded_asm =
            GDMono::get_singleton()->get_loaded_assembly(
                has_extension ? name.get_basename() : name
            );
        if (!loaded_asm) {
    ...
    // Decrement refcount which was previously incremented by
...
GDMonoClass* GDMonoAssembly::get_class(
    ...
    MonoClass* mono_class = mono_class_from_name(
        image,
        String(p_namespace).utf8(),
        String(p_name).utf8()
    );
...
GDMonoClass* GDMonoAssembly::get_class(MonoClass* p_mono_class) {
    ...
    Map<MonoClass*, GDMonoClass*>::Element* match =
        cached_raw.find(p_mono_class);
    ...
    StringName namespace_name =
        String::utf8(mono_class_get_namespace(p_mono_class));
...
GDMonoClass* GDMonoAssembly::get_object_derived_class(const StringName& p_class
    ...
    if (gdobject_class_cache_updated) {
        Map<StringName, GDMonoClass*>::Element* result =
            gdobject_class_cache.find(p_class);
        ...
        for (int i = 1; i < rows; i++) {
            ...
            while (!nested_classes.empty()) {
                ...
                while (true) {
                    MonoClass* raw_nested = mono_class_get_nested_types(
                        current_nested->get_mono_ptr(),
                        &iter
                    );
                    ...
                    if (nested_class) {
                        gdobject_class_cache.insert(
                            nested_class->get_name(),
                            nested_class
                        );
...
GDMonoAssembly* GDMonoAssembly::load(
    const String& p_name,
    MonoAssemblyName* p_aname,
    bool p_refonly,
    const Vector<String>& p_search_dirs
) {
    if (GDMono::get_singleton()->get_corlib_assembly()
        && (p_name == "mscorlib" || p_name == "mscorlib.dll")) {
    ...
    if (!assembly) {
        assembly =
            _load_assembly_search(p_name, p_aname, p_refonly, p_search_dirs);
        if (!assembly) {
            return nullptr;
        }
    }
    ...
    GDMonoAssembly* loaded_asm =
        GDMono::get_singleton()->get_loaded_assembly(p_name);
    ERR_FAIL_NULL_V_MSG(
        loaded_asm,
        nullptr,
        "Loaded assembly missing from table. Did we not receive the load hook?"
    );
...
GDMonoAssembly* GDMonoAssembly::load_from(
    const String& p_name,
    const String& p_path,
    bool p_refonly
) {
