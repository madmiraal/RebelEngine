...
using Mono.Cecil;
using Directory = RebelTools.Utils.Directory;
using File = RebelTools.Utils.File;
using OS = RebelTools.Utils.OS;
using Path = System.IO.Path;

namespace RebelTools.Export
{
    ...
    public static class AotBuilder
    {
        public static void CompileAssemblies(ExportPlugin exporter, AotOptions aotOpts, string[] features, string platform, bool isDebug, string bclDir, string outputDir, string outputDataDir, IDictionary<string, string> assemblies)
        {
            ...
            string aotTempDir = Path.Combine(Path.GetTempPath(), $"rebel-aot-{Process.GetCurrentProcess().Id}");
        ...
        public static void CompileAssembliesForAndroid(ExportPlugin exporter, bool isDebug, string[] abis, AotOptions aotOpts, string aotTempDir, IDictionary<string, string> assemblies, string bclDir)
        {
            foreach (var assembly in assemblies)
            {
                ...
                // Not sure if the 'lib' prefix is an Android thing or just Rebel being picky,
                ...
                foreach (string abi in abis)
                {
                    ...
                    string compilerDirPath = Path.Combine(RebelSharpDirs.DataEditorToolsDir, "aot-compilers", $"{OS.Platforms.Android}-{abi}");

                    ExecuteCompiler(FindCrossCompiler(compilerDirPath), compilerArgs, bclDir);

                    // The Rebel exporter expects us to pass the abi in the tags parameter
        ...
        public static void CompileAssembliesForiOS(ExportPlugin exporter, bool isDebug, string[] architectures, AotOptions aotOpts, string aotTempDir, IDictionary<string, string> assemblies, string bclDir)
        {
            ...
            string projectAssemblyName = RebelSharpEditor.ProjectAssemblyName;
            ...
            string compilerDirPath = Path.Combine(RebelSharpDirs.DataEditorToolsDir, "aot-compilers",
                $"{OS.Platforms.iOS}-arm64");
            string crossCompiler = FindCrossCompiler(compilerDirPath);

            string aotCacheDir = Path.Combine(ProjectSettings.GlobalizePath(RebelSharpDirs.ResTempDir),
                "obj", isDebug ? "ExportDebug" : "ExportRelease", "rebel-aot-cache");
...
MONO_AOT_MODE_LAST = 1000,
            ...
            cppCode.AppendLine("__attribute__((used)) __attribute__((optnone)) static void __rebel_symbol_referencer() {");
            ...
            cppCode.AppendLine("} // __rebel_symbol_referencer");
        ...
        private static IEnumerable<string> GetAotCompilerArgs(string platform, bool isDebug, string target, AotOptions aotOpts, string assemblyPath, string outputFilePath)
        {
            ...
            if (platform == OS.Platforms.Android)
            {
                ...
                {
                    androidToolchain = Path.Combine(RebelSharpDirs.DataEditorToolsDir, "android-toolchains", $"{abi}"); // TODO: $"{abi}-{apiLevel}{(clang?"clang":"")}"
