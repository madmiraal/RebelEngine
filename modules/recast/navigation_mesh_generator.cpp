...
EditorNavigationMeshGenerator* EditorNavigationMeshGenerator::singleton =
    nullptr;

void EditorNavigationMeshGenerator::_add_vertex(
...
void EditorNavigationMeshGenerator::_add_mesh(
...
void EditorNavigationMeshGenerator::_add_faces(
...
void EditorNavigationMeshGenerator::_parse_geometry(
    ...
    if (Object::cast_to<MeshInstance>(p_node)
        ...
        if (mesh.is_valid()) {
            _add_mesh(
                mesh,
                p_accumulated_transform * mesh_instance->get_transform(),
                p_verticies,
                p_indices
            );
        }
...
#ifdef MODULE_CSG_ENABLED
    if (Object::cast_to<CSGShape>(p_node)
        && p_generate_from
               != NavigationMesh::PARSED_GEOMETRY_STATIC_COLLIDERS) {
        ...
        if (!meshes.empty()) {
            ...
            if (mesh.is_valid()) {
                _add_mesh(
                    mesh,
                    p_accumulated_transform * csg_shape->get_transform(),
                    p_verticies,
                    p_indices
                );
            }
    ...
    if (Object::cast_to<StaticBody>(p_node)
        && p_generate_from != NavigationMesh::PARSED_GEOMETRY_MESH_INSTANCES) {
            ...
            for (int i = 0; i < p_node->get_child_count(); ++i) {
                ...
                if (Object::cast_to<CollisionShape>(child)) {
                    CollisionShape* col_shape =
                        Object::cast_to<CollisionShape>(child);

                    Transform transform = p_accumulated_transform
                                        * static_body->get_transform()
                                        * col_shape->get_transform();
                    ...
                    CylinderShape* cylinder =
                        Object::cast_to<CylinderShape>(*s);
                    if (cylinder) {
                    ...
                    ConcavePolygonShape* concave_polygon =
                        Object::cast_to<ConcavePolygonShape>(*s);
                    if (concave_polygon) {
                        _add_faces(
                            concave_polygon->get_faces(),
                            transform,
                            p_verticies,
                            p_indices
                        );
                    ...
                    ConvexPolygonShape* convex_polygon =
                        Object::cast_to<ConvexPolygonShape>(*s);
                    if (convex_polygon) {
                        Vector<Vector3> varr =
                            Variant(convex_polygon->get_points());
...
#ifdef MODULE_GRIDMAP_ENABLED
    if (Object::cast_to<GridMap>(p_node)
        && p_generate_from
               != NavigationMesh::PARSED_GEOMETRY_STATIC_COLLIDERS) {
        ...
        for (int i = 0; i < meshes.size(); i += 2) {
            ...
            if (mesh.is_valid()) {
                _add_mesh(
                    mesh,
                    p_accumulated_transform * xform * meshes[i],
                    p_verticies,
                    p_indices
                );
            }
    ...
    if (Object::cast_to<Spatial>(p_node)) {
        ...
        p_accumulated_transform =
            p_accumulated_transform * spatial->get_transform();
    }
...
void EditorNavigationMeshGenerator::
    _convert_detail_mesh_to_native_navigation_mesh(
...
void EditorNavigationMeshGenerator::_build_recast_navigation_mesh(
    Ref<NavigationMesh> p_nav_mesh,
    EditorProgress* ep,
    rcHeightfield* hf,
    ...
    cfg.walkableClimb =
        (int)Math::floor(p_nav_mesh->get_agent_max_climb() / cfg.ch);
    cfg.walkableRadius =
        (int)Math::ceil(p_nav_mesh->get_agent_radius() / cfg.cs);
    cfg.maxEdgeLen =
    ...
    ERR_FAIL_COND(!hf);
    ERR_FAIL_COND(!rcCreateHeightfield(
        &ctx,
        *hf,
        cfg.width,
        cfg.height,
    ...
    ep->step(TTR("Marking walkable triangles..."), 4);
    {
        ...
        ERR_FAIL_COND(!rcRasterizeTriangles(
            &ctx,
            verts,
            nverts,
            tris,
    ...
    ERR_FAIL_COND(!chf);
    ERR_FAIL_COND(!rcBuildCompactHeightfield(
        &ctx,
        cfg.walkableHeight,
        cfg.walkableClimb,
        *hf,
    ...
    if (p_nav_mesh->get_sample_partition_type()
        == NavigationMesh::SAMPLE_PARTITION_WATERSHED) {
        ERR_FAIL_COND(!rcBuildDistanceField(&ctx, *chf));
        ERR_FAIL_COND(!rcBuildRegions(
            &ctx,
            *chf,
            0,
            cfg.minRegionArea,
    ...
    ERR_FAIL_COND(!rcBuildPolyMeshDetail(
        &ctx,
        *poly_mesh,
        *chf,
...
void EditorNavigationMeshGenerator::bake(
    Ref<NavigationMesh> p_nav_mesh,
    Node* p_node
) {
    ...
    if (p_nav_mesh->get_source_geometry_mode()
        == NavigationMesh::SOURCE_GEOMETRY_NAVMESH_CHILDREN) {
        parse_nodes.push_back(p_node);
    } else {
        p_node->get_tree()->get_nodes_in_group(
            p_nav_mesh->get_source_group_name(),
            &parse_nodes
        );
    }
...
void EditorNavigationMeshGenerator::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("bake", "nav_mesh", "root_node"),
        &EditorNavigationMeshGenerator::bake
    );
    ClassDB::bind_method(
        D_METHOD("clear", "nav_mesh"),
