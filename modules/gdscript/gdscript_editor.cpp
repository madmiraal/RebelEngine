...
void GDScriptLanguage::get_comment_delimiters(List<String>* p_delimiters
) const {
...
String GDScriptLanguage::_get_processed_template(
...
Ref<Script> GDScriptLanguage::get_template(
...
void GDScriptLanguage::make_template(
    const String& p_class_name,
    const String& p_base_class_name,
    Ref<Script>& p_script
) {
...
bool GDScriptLanguage::validate(
...
#ifdef DEBUG_ENABLED
    if (r_warnings) {
        for (const List<GDScriptWarning>::Element* E =
    ...
    if (err) {
        ...
        const GDScriptParser::ClassNode* cl =
            static_cast<const GDScriptParser::ClassNode*>(root);
...
int GDScriptLanguage::find_function(
    ...
    int indent = 0;
    while (tokenizer.get_token() != GDScriptTokenizer::TK_EOF
...
bool GDScriptLanguage::debug_break_parse(
    const String& p_file,
    int p_line,
...
bool GDScriptLanguage::debug_break(
    const String& p_error,
...
void GDScriptLanguage::debug_get_stack_level_locals(
    int p_level,
...
void GDScriptLanguage::debug_get_stack_level_members(
    int p_level,
    ...
    const Map<StringName, GDScript::MemberInfo>& mi =
        script->debug_get_member_indices();

    for (const Map<StringName, GDScript::MemberInfo>::Element* E = mi.front();
         E;
         E = E->next()) {
...
void GDScriptLanguage::debug_get_globals(
...
void GDScriptLanguage::get_recognized_extensions(List<String>* p_extensions
) const {
    p_extensions->push_back("gd");
}

void GDScriptLanguage::get_public_functions(List<MethodInfo>* p_functions
) const {
    for (int i = 0; i < GDScriptFunctions::FUNC_MAX; i++) {
        p_functions->push_back(
            GDScriptFunctions::get_info(GDScriptFunctions::Function(i))
        );
    }
...
void GDScriptLanguage::get_public_constants(
...
String GDScriptLanguage::make_function(
...
struct GDScriptCompletionIdentifier {
    ...
    GDScriptCompletionIdentifier() : assigned_expression(nullptr) {}
};
...
static GDScriptCompletionIdentifier _type_from_property(
...
static GDScriptCompletionIdentifier _type_from_gdtype(
...
static bool _guess_identifier_type(
    GDScriptCompletionContext& p_context,
    const StringName& p_identifier,
    GDScriptCompletionIdentifier& r_type
);
static bool _guess_identifier_type_from_base(
    GDScriptCompletionContext& p_context,
    const GDScriptCompletionIdentifier& p_base,
    const StringName& p_identifier,
    GDScriptCompletionIdentifier& r_type
);
    ...
    switch (p_expression->type) {
        case GDScriptParser::Node::TYPE_CONSTANT: {
            const GDScriptParser::ConstantNode* cn =
                static_cast<const GDScriptParser::ConstantNode*>(p_expression);
        ...
        case GDScriptParser::Node::TYPE_SELF: {
            if (p_context._class) {
                ...
                r_type.type.class_type =
                    const_cast<GDScriptParser::ClassNode*>(p_context._class);
        ...
        case GDScriptParser::Node::TYPE_DICTIONARY: {
            ...
            const GDScriptParser::DictionaryNode* dn =
                static_cast<const GDScriptParser::DictionaryNode*>(p_expression
                );
            Dictionary d;
            bool full = true;
        ...
        case GDScriptParser::Node::TYPE_ARRAY: {
            ...
            const GDScriptParser::ArrayNode* an =
                static_cast<const GDScriptParser::ArrayNode*>(p_expression);
            Array a;
            bool full = true;
        ...
        case GDScriptParser::Node::TYPE_CAST: {
            const GDScriptParser::CastNode* cn =
                static_cast<const GDScriptParser::CastNode*>(p_expression);
            GDScriptCompletionIdentifier value;
        ...
        case GDScriptParser::Node::TYPE_OPERATOR: {
            const GDScriptParser::OperatorNode* op =
            ...
            switch (op->op) {
                case GDScriptParser::OperatorNode::OP_CALL: {
                    if (op->arguments[0]->type
                        ...
                        const GDScriptParser::TypeNode* tn =
                            static_cast<const GDScriptParser::TypeNode*>(
                                op->arguments[0]
                            );
                        r_type.type.has_type = true;
                        r_type.type.kind = GDScriptParser::DataType::BUILTIN;
                        ...
                        MethodInfo mi =
                            GDScriptFunctions::get_info(bin->function);
                        ...
                        if (base.type.is_constant
                            ...
                            while (native_type.kind
                                   == GDScriptParser::DataType::CLASS) {
                                native_type = native_type.class_type->base_type;
                            }

                            while (native_type.kind
                                       == GDScriptParser::DataType::GDSCRIPT
                                   || native_type.kind
                                          == GDScriptParser::DataType::SCRIPT) {
                                if (native_type.script_type.is_valid()) {
                                    ...
                                    if (parent.is_valid()) {
                                        ...
                                        native_type.kind =
                                            GDScriptParser::DataType::NATIVE;
                ...
                case GDScriptParser::OperatorNode::OP_INDEX_NAMED: {
                    ...
                    if (op->arguments[0]->type
                        == GDScriptParser::Node::TYPE_DICTIONARY) {
    ...
    while (blk) {
        if (blk->variables.has(p_identifier)) {
            ...
            if (!last_assigned_expression
                ...
                && blk->variables[p_identifier]->assign->type
                       == GDScriptParser::Node::TYPE_OPERATOR) {
                const GDScriptParser::OperatorNode* op =
                    static_cast<const GDScriptParser::OperatorNode*>(
                        blk->variables[p_identifier]->assign
                    );
        ...
        for (int z = 0; z < blk->statements.size(); z++) {
            ...
            if (expr->line > p_context.line
                || expr->type != GDScriptParser::Node::TYPE_OPERATOR) {
                continue;
            }

            const GDScriptParser::OperatorNode* op =
                static_cast<const GDScriptParser::OperatorNode*>(expr);
            if (op->op != GDScriptParser::OperatorNode::OP_ASSIGN
        ...
        if (blk->if_condition
            && blk->if_condition->type == GDScriptParser::Node::TYPE_OPERATOR
            && static_cast<const GDScriptParser::OperatorNode*>(
                   blk->if_condition
               )
                       ->op
                   == GDScriptParser::OperatorNode::OP_IS) {
            ...
            switch (base_type.kind) {
                case GDScriptParser::DataType::GDSCRIPT: {
                    ...
                    if (gds.is_valid()
                        ...
                        GDScriptFunction* func = gds->get_member_functions(
        ...
        if (ClassDB::class_exists(target_id)) {
            ...
            if (Engine::get_singleton()->has_singleton(target_id)) {
                ...
                const Map<StringName, int>::Element* target_elem =
                    GDScriptLanguage::get_singleton()->get_global_map().find(
                        target_id
                    );
                // Check because classes like EditorNode are in ClassDB by now,
                // but unknown to GDScript
                if (!target_elem) {
                    return false;
                }
        ...
        switch (base_type.kind) {
            case GDScriptParser::DataType::CLASS: {
                if (base_type.class_type->constant_expressions.has(p_identifier
                    ...
                    GDScriptParser::ClassNode::Constant c =
                    ...
                    if (c.expression->type
                        ...
                        r_type.value =
                            static_cast<const GDScriptParser::ConstantNode*>(
                                c.expression
                            )
                                ->value;
                    }
                ...
                if (!_static) {
                    for (int i = 0; i < base_type.class_type->variables.size();
                         i++) {
                        GDScriptParser::ClassNode::Member m =
...
static bool _find_last_return_in_block(
    const GDScriptCompletionContext& p_context,
    int& r_last_return_line,
    const GDScriptParser::Node** r_last_returned_value
) {
    if (!p_context.block) {
        return false;
    }
    ...
    for (int i = 0; i < p_context.block->statements.size(); i++) {
        ...
        if (p_context.block->statements[i]->type
            != GDScriptParser::Node::TYPE_CONTROL_FLOW) {
            continue;
        }

        const GDScriptParser::ControlFlowNode* cf =
            static_cast<const GDScriptParser::ControlFlowNode*>(
                p_context.block->statements[i]
            );
        if (cf->cf_type == GDScriptParser::ControlFlowNode::CF_RETURN
            && cf->arguments.size() > 0) {
            ...
            case GDScriptParser::DataType::CLASS: {
                ...
                for (int i = 0;
                    ...
                    if (base_type.class_type->static_functions[i]->name
                        ...
                        const GDScriptParser::Node* last_returned_value =
                            nullptr;
                        GDScriptCompletionContext c = p_context;
        ...
        if (i - def_args >= 0) {
            ...
            if (p_function->default_values[i - def_args]
                && p_function->default_values[i - def_args]->type
                       == GDScriptParser::Node::TYPE_OPERATOR) {
                const GDScriptParser::OperatorNode* assign =
                    static_cast<const GDScriptParser::OperatorNode*>(
...
static void _find_identifiers_in_block(
    ...
    for (Map<StringName, GDScriptParser::LocalVarNode*>::Element* E =
...
static void _find_identifiers_in_class(
    ...
    if (!p_parent_only) {
        ...
        if (!p_only_functions) {
            for (Map<StringName, GDScriptParser::ClassNode::Constant>::Element*
                     E = p_context._class->constant_expressions.front();
                 E;
                 E = E->next()) {
                ScriptCodeCompletionOption option(
                    E->key(),
                ...
                if (E->get().expression
                    && E->get().expression->type
                           == GDScriptParser::Node::TYPE_CONSTANT) {
                    GDScriptParser::ConstantNode* cnode =
                        (GDScriptParser::ConstantNode*)E->get().expression;
                    if (cnode) {
        ...
        switch (base_type.kind) {
            ...
            case GDScriptParser::DataType::GDSCRIPT: {
                ...
                if (script.is_valid()) {
                    ...
                    for (const Map<StringName, GDScriptFunction*>::Element* E =
                             script->get_member_functions().front();
                         E;
                         E = E->next()) {
                    ...
                    if (!p_only_functions) {
                        for (const Map<StringName, Ref<GDScript>>::Element* E =
                                 script->get_subclasses().front();
                             E;
                             E = E->next()) {
                            ScriptCodeCompletionOption option(
                                E->key().operator String(),
...
static void _find_identifiers(
    ...
    for (int i = 0; i < GDScriptFunctions::FUNC_MAX; i++) {
        ...
        ScriptCodeCompletionOption option(
            String(
                GDScriptFunctions::get_func_name(GDScriptFunctions::Function(i))
            ),
            ScriptCodeCompletionOption::KIND_FUNCTION
        );
    ...
    for (const Map<StringName, int>::Element* E =
             GDScriptLanguage::get_singleton()->get_global_map().front();
         E;
         E = E->next()) {
        ScriptCodeCompletionOption option(
            E->key().operator String(),
...
static void _find_call_arguments(
    ...
    bool _static = false;
    const GDScriptParser::OperatorNode* op =
        static_cast<const GDScriptParser::OperatorNode*>(p_node);

    GDScriptCompletionIdentifier connect_base;

    if (op->op != GDScriptParser::OperatorNode::OP_CALL
        && op->op != GDScriptParser::OperatorNode::OP_PARENT_CALL) {
        return;
    }
    ...
    if (op->op == GDScriptParser::OperatorNode::OP_CALL) {
        if (op->arguments[0]->type
            ...
            const GDScriptParser::BuiltInFunctionNode* fn =
                static_cast<const GDScriptParser::BuiltInFunctionNode*>(
                    op->arguments[0]
                );
            ...
            const GDScriptParser::TypeNode* tn =
                static_cast<const GDScriptParser::TypeNode*>(op->arguments[0]);
        ...
        function =
            static_cast<const GDScriptParser::IdentifierNode*>(op->arguments[0])
                ->name;
...
Error GDScriptLanguage::complete_code(
    const String& p_code,
    const String& p_path,
    Object* p_owner,
    ...
    switch (parser.get_completion_type()) {
        ...
        case GDScriptParser::COMPLETION_INDEX: {
            ...
            const GDScriptParser::OperatorNode* op =
                static_cast<const GDScriptParser::OperatorNode*>(node);
        ...
        case GDScriptParser::COMPLETION_TYPE_HINT: {
            ...
            while (clss) {
                for (Map<StringName,
                         GDScriptParser::ClassNode::Constant>::Element* E =
            ...
            while (base_type.has_type) {
                switch (base_type.kind) {
                    case GDScriptParser::DataType::CLASS: {
                        if (base_type.class_type) {
                            for (Map<StringName,
                                     GDScriptParser::ClassNode::Constant>::
                                     Element* E =
                                         base_type.class_type
...
void GDScriptLanguage::auto_indent_code(
    String& p_code,
    int p_from_line,
    int p_to_line
) const {
...
static Error _lookup_symbol_from_base(
    ...
    bool p_is_function,
    GDScriptLanguage::LookupResult& r_result
) {
...
Error GDScriptLanguage::lookup_code(
    const String& p_code,
    const String& p_symbol,
    const String& p_path,
    Object* p_owner,
        ...
        case GDScriptParser::COMPLETION_IDENTIFIER: {
            ...
            if (context._class) {
                if (parser.get_completion_type()
                    != GDScriptParser::COMPLETION_PARENT_FUNCTION) {
                    base_type.has_type = true;
                    base_type.kind     = GDScriptParser::DataType::CLASS;
                    base_type.class_type =
                        const_cast<GDScriptParser::ClassNode*>(context._class);
                } else {
                    ...
                    if (value.get_type() == Variant::OBJECT) {
                        ...
                        if (obj) {
                            if (Object::cast_to<GDScriptNativeClass>(obj)) {
                                ...
                                r_result.class_name =
                                    Object::cast_to<GDScriptNativeClass>(obj)
                                        ->get_name();
                            } else {
                                r_result.type =
                                    ScriptLanguage::LookupResult::RESULT_CLASS;
    ...
    for (int i = 0; i < GDScriptFunctions::FUNC_MAX; i++) {
        ...
        if (GDScriptFunctions::get_func_name(GDScriptFunctions::Function(i))
            == p_symbol) {
