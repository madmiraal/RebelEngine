...
struct _GDScriptMemberSort {
    ...
    _FORCE_INLINE_ bool operator<(const _GDScriptMemberSort& p_member) const {
...
MethodInfo GDScript::get_method_info(const StringName& p_method) const {
    const Map<StringName, GDScriptFunction*>::Element* E =
...
bool GDScript::get_property_default_value(
...
#ifdef TOOLS_ENABLED
void GDScript::_update_exports_values(
...
bool GDScript::_update_exports(
    ...
    if (source_changed_cache) {
        ...
        if (err == OK) {
            ...
            ERR_FAIL_COND_V(
                root->type != GDScriptParser::Node::TYPE_CLASS,
                false
            );

            const GDScriptParser::ClassNode* c =
...
void GDScript::_set_subclass_path(Ref<GDScript>& p_sc, const String& p_path) {
    p_sc->path = p_path;
    for (Map<StringName, Ref<GDScript>>::Element* E = p_sc->subclasses.front();
         E;
         E = E->next()) {
...
Error GDScript::reload(bool p_keep_state) {
    ...
    for (Map<StringName, Ref<GDScript>>::Element* E = subclasses.front(); E;
         E                                          = E->next()) {
...
Variant GDScript::call(
    ...
    GDScript* top = this;
    while (top) {
        Map<StringName, GDScriptFunction*>::Element* E =
...
bool GDScript::_get(const StringName& p_name, Variant& r_ret) const {
    {
        ...
        while (top) {
            ...
            {
                const Map<StringName, Ref<GDScript>>::Element* E =
        ...
        if (p_name
            == GDScriptLanguage::get_singleton()->strings._script_source) {
...
void GDScript::_save_orphaned_subclasses() {
    ...
    for (int i = 0; i < weak_subclasses.size(); i++) {
        ...
        GDScriptLanguage::get_singleton()->add_orphan_subclass(
...
GDScript::~GDScript() {
    ...
    for (Map<StringName, GDScriptFunction*>::Element* E =
...
bool GDScriptInstance::set(const StringName& p_name, const Variant& p_value) {
    // member
    {
        const Map<StringName, GDScript::MemberInfo>::Element* E =
    ...
    while (sptr) {
        Map<StringName, GDScriptFunction*>::Element* E =
            sptr->member_functions.find(
                GDScriptLanguage::get_singleton()->strings._set
            );
        if (E) {
            Variant name           = p_name;
    ...
    while (sptr) {
        {
            ...
            if (E) {
                if (E->get().getter) {
                    Variant::CallError err;
                    r_ret = const_cast<GDScriptInstance*>(this)
...

        {
            const Map<StringName, GDScriptFunction*>::Element* E =
                sptr->member_functions.find(
                    GDScriptLanguage::get_singleton()->strings._get
                );
            if (E) {
                Variant name           = p_name;
                ...
                Variant::CallError err;
                Variant ret = const_cast<GDScriptFunction*>(E->get())->call(
                    const_cast<GDScriptInstance*>(this),
                    (const Variant**)args,
                    1,
                    err
                );
...
Variant::Type GDScriptInstance::get_property_type(
...
void GDScriptInstance::get_property_list(List<PropertyInfo>* p_properties
) const {
    ...
    while (sptr) {
        const Map<StringName, GDScriptFunction*>::Element* E =
            sptr->member_functions.find(
                GDScriptLanguage::get_singleton()->strings._get_property_list
            );
        if (E) {
            Variant::CallError err;
            Variant ret = const_cast<GDScriptFunction*>(E->get())->call(
...
void GDScriptInstance::call_multilevel(
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
void GDScriptInstance::_ml_call_reversed(
    GDScript* sptr,
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
...
void GDScriptInstance::call_multilevel_reversed(
    const StringName& p_method,
    const Variant** p_args,
    int p_argcount
) {
...
void GDScriptInstance::notification(int p_notification) {
    ...
    while (sptr) {
        Map<StringName, GDScriptFunction*>::Element* E =
            sptr->member_functions.find(
                GDScriptLanguage::get_singleton()->strings._notification
            );
        if (E) {
            Variant::CallError err;
...
MultiplayerAPI::RPCMode GDScriptInstance::get_rpc_mode(
    const StringName& p_method
) const {
...
MultiplayerAPI::RPCMode GDScriptInstance::get_rset_mode(
...
void GDScriptInstance::reload_members() {
    ...
    for (Map<StringName, GDScript::MemberInfo>::Element* E =
...
void GDScriptLanguage::_add_global(
    const StringName& p_name,
...
void GDScriptLanguage::add_global_constant(
    const StringName& p_variable,
    const Variant& p_value
) {
...
void GDScriptLanguage::add_named_global_constant(
    const StringName& p_name,
    const Variant& p_value
) {
...
int GDScriptLanguage::profiling_get_accumulated_data(
...
int GDScriptLanguage::profiling_get_frame_data(
    ProfilingInfo* p_info_arr,
    int p_info_max
) {
    int current = 0;
...
void GDScriptLanguage::reload_all_scripts() {
    ...
    scripts.sort_custom<GDScriptDepSort>(
...
void GDScriptLanguage::reload_tool_script(
    ...
    Map<Ref<GDScript>, Map<ObjectID, List<Pair<StringName, Variant>>>>
        to_reload;
...
String GDScriptLanguage::get_global_class_name(
    ...
    if (parser.get_parse_tree()
        && parser.get_parse_tree()->type == GDScriptParser::Node::TYPE_CLASS) {
        const GDScriptParser::ClassNode* c =
            static_cast<const GDScriptParser::ClassNode*>(parser.get_parse_tree(
            ));
        if (r_icon_path) {
                        ...
                        if (subclass->extends_class.size() == 0) {
                            ...
                            if (!subparser.get_parse_tree()
                                || subparser.get_parse_tree()->type
                                       != GDScriptParser::Node::TYPE_CLASS) {
                                break;
                            }
                            subclass =
                                static_cast<const GDScriptParser::ClassNode*>(
                            ...
                            while (extend_classes.size() > 0) {
                                ...
                                for (int i = 0; i < subclass->subclasses.size();
                                     i++) {
                                    const GDScriptParser::ClassNode*
...
#define CHECK_SYMBOLS(m_amount)                                                \
    ...
    switch (code) {
        ...
        case FUNCTION_MAY_YIELD: {
            ...
            return "Assigned variable is typed but the function '" + symbols[0]
                 + "()' may yield and return a GDScriptFunctionState instead.";
        } break;
    ...
    ERR_FAIL_V_MSG(
        String(),
        "Invalid GDScript warning code: " + get_name_from_code(code) + "."
    );
...
GDScriptLanguage::GDScriptLanguage() {
    ...
    for (int i = 0; i < (int)GDScriptWarning::WARNING_MAX; i++) {
        String warning =
            GDScriptWarning::get_name_from_code((GDScriptWarning::Code)i)
                .to_lower();
...
void GDScriptLanguage::add_orphan_subclass(
...
Ref<GDScript> GDScriptLanguage::get_orphan_subclass(
    const String& p_qualified_name
) {
...
RES ResourceFormatLoaderGDScript::load(
    const String& p_path,
...
void ResourceFormatLoaderGDScript::get_recognized_extensions(
...
String ResourceFormatLoaderGDScript::get_resource_type(const String& p_path
) const {
...
Error ResourceFormatSaverGDScript::save(
    const String& p_path,
    ...
    ERR_FAIL_COND_V_MSG(
        err,
        err,
        "Cannot save GDScript file '" + p_path + "'."
    );
    ...
    if (ScriptServer::is_reload_scripts_on_save_enabled()) {
        GDScriptLanguage::get_singleton()->reload_tool_script(
            p_resource,
            false
        );
    }
...
void ResourceFormatSaverGDScript::get_recognized_extensions(
    const RES& p_resource,
    List<String>* p_extensions
) const {
