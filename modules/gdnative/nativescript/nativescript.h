...
private:
    ...
    SafeFlag has_objects_to_register; // so that we don't lock mutex every frame
                                      // - it's rarely needed
...
inline NativeScriptDesc* NativeScript::get_script_desc() const {
    Map<StringName, NativeScriptDesc>::Element* E =
        NativeScriptLanguage::singleton->library_classes[lib_path].find(
            class_name
        );
