...
bool VideoStreamPlaybackGDNative::open_file(const String& p_file) {
    ...
    if (file_opened) {
        ...
        if (num_channels > 0) {
            pcm = (float*)memalloc(
                num_channels * AUX_BUFFER_SIZE * sizeof(float)
            );
...
void VideoStreamPlaybackGDNative::update(float p_delta) {
    ...
    if (mix_callback && num_channels > 0) {
        if (pcm_write_idx >= 0) {
            // Previous remains
            int mixed = mix_callback(
                mix_udata,
                pcm + pcm_write_idx * num_channels,
                samples_decoded
            );
        ...
        if (pcm_write_idx < 0) {
            samples_decoded =
                interface->get_audioframe(data_struct, pcm, AUX_BUFFER_SIZE);
...
void VideoStreamPlaybackGDNative::set_mix_callback(
...
Ref<VideoStreamPlayback> VideoStreamGDNative::instance_playback() {
    ...
    VideoDecoderGDNative* decoder =
        decoder_server.get_decoder(file.get_extension().to_lower());
    if (decoder == nullptr) {
        return nullptr;
    }
...
RES ResourceFormatLoaderVideoStreamGDNative::load(
    const String& p_path,
...
void ResourceFormatLoaderVideoStreamGDNative::get_recognized_extensions(
    List<String>* p_extensions
) const {
    Map<String, int>::Element* el =
        VideoDecoderServer::get_instance()->get_extensions().front();
    while (el) {
...
bool ResourceFormatLoaderVideoStreamGDNative::handles_type(const String& p_type
) const {
    return ClassDB::is_parent_class(p_type, "VideoStream");
}

String ResourceFormatLoaderVideoStreamGDNative::get_resource_type(
    const String& p_path
) const {
