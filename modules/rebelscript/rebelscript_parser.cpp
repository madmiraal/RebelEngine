...
template <class T>
T* RebelScriptParser::alloc_node() {
    T* t = memnew(T);
...
#ifdef DEBUG_ENABLED
static String _find_function_name(const RebelScriptParser::OperatorNode* p_call
);
#endif // DEBUG_ENABLED

bool RebelScriptParser::_end_statement() {
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_SEMICOLON) {
        tokenizer->advance();
        return true; // handle next
    } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_NEWLINE || tokenizer->get_token() == RebelScriptTokenizer::TK_EOF) {
...
void RebelScriptParser::_set_end_statement_error(String p_name) {
    String error_msg;
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_IDENTIFIER) {
        error_msg = vformat(
            "Expected end of statement (\"%s\"), got %s (\"%s\") instead.",
            p_name,
            tokenizer->get_token_name(tokenizer->get_token()),
            tokenizer->get_token_identifier()
        );
    } else {
        error_msg = vformat(
            "Expected end of statement (\"%s\"), got %s instead.",
            p_name,
            tokenizer->get_token_name(tokenizer->get_token())
        );
    }
...
bool RebelScriptParser::_enter_indent_block(BlockNode* p_block) {
    if (tokenizer->get_token() != RebelScriptTokenizer::TK_COLON) {
    ...
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_EOF) {
        return false;
    }

    if (tokenizer->get_token() != RebelScriptTokenizer::TK_NEWLINE) {
        // be more python-like
    ...
    while (true) {
        if (tokenizer->get_token() != RebelScriptTokenizer::TK_NEWLINE) {
            return false; // wtf
        } else if (tokenizer->get_token(1) == RebelScriptTokenizer::TK_EOF) {
            return false;
        } else if (tokenizer->get_token(1) != RebelScriptTokenizer::TK_NEWLINE) {
...
bool RebelScriptParser::_parse_arguments(
    Node* p_parent,
    Vector<Node*>& p_args,
    bool p_static,
    bool p_can_codecomplete,
    bool p_parsing_constant
) {
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_PARENTHESIS_CLOSE) {
        tokenizer->advance();
    } else {
        parenthesis++;
        int argidx = 0;
        ...
        while (true) {
            if (tokenizer->get_token() == RebelScriptTokenizer::TK_CURSOR) {
                _make_completable_call(argidx);
                completion_node = p_parent;
            } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONSTANT && tokenizer->get_token_constant().get_type() == Variant::STRING && tokenizer->get_token(1) == RebelScriptTokenizer::TK_CURSOR) {
            ...
            if (tokenizer->get_token()
                == RebelScriptTokenizer::TK_PARENTHESIS_CLOSE) {
                tokenizer->advance();
                break;

            } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_COMMA) {
                if (tokenizer->get_token(1)
                    == RebelScriptTokenizer::TK_PARENTHESIS_CLOSE) {
...
void RebelScriptParser::_make_completable_call(int p_arg) {
...
bool RebelScriptParser::_get_completable_identifier(
    CompletionType p_type,
    ...
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_CURSOR) {
        ...
        if (tokenizer->is_token_literal()) {
            identifier = identifier.operator String()
                       + tokenizer->get_token_literal().operator String();
            tokenizer->advance();
        }

        if (tokenizer->get_token()
            == RebelScriptTokenizer::TK_PARENTHESIS_OPEN) {
...
RebelScriptParser::Node* RebelScriptParser::_parse_expression(
    Node* p_parent,
    bool p_static,
    bool p_allow_assign,
    bool p_parsing_constant
    ...
    int op_line = tokenizer->get_token_line(
    ); // when operators are created at the bottom, the line might have been
       // changed (\n found)
    ...
    while (true) {
        ...
        if (parenthesis > 0) {
            ...
            while (tokenizer->get_token() == RebelScriptTokenizer::TK_NEWLINE) {
                tokenizer->advance();
            }
        }

        // Check that the next token is not TK_CURSOR and if it is, the offset
        // should be incremented.
        int next_valid_offset = 1;
        if (tokenizer->get_token(next_valid_offset)
            == RebelScriptTokenizer::TK_CURSOR) {
            next_valid_offset++;
            // There is a chunk of the identifier that also needs to be ignored
            // (not always there!)
            if (tokenizer->get_token(next_valid_offset)
                == RebelScriptTokenizer::TK_IDENTIFIER) {
                next_valid_offset++;
            }
        }
        ...
        if (tokenizer->get_token()
            ...
            tokenizer->advance();
            parenthesis++;
            Node* subexpr = _parse_expression(
                p_parent,
                p_static,
                p_allow_assign,
                p_parsing_constant
            ...
            tokenizer->advance();
            expr = subexpr;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_DOLLAR) {
            tokenizer->advance();
            ...
            while (!done) {
                switch (tokenizer->get_token()) {
                    case RebelScriptTokenizer::TK_CURSOR: {
                    ...
                    case RebelScriptTokenizer::TK_CONSTANT: {
                        if (!need_identifier) {
                            done = true;
                            break;
                        }

                        if (tokenizer->get_token_constant().get_type()
                            != Variant::STRING) {
                            _set_error(
                                "Expected string constant or identifier after "
                                "'$' or '/'."
                            );
                            return nullptr;
                        }
                    ...
                    case RebelScriptTokenizer::TK_OP_DIV: {
                        if (need_identifier) {
                            done = true;
                            break;
                        }
                    ...
                    default: {
                        // Instead of checking for TK_IDENTIFIER, we check with
                        // is_token_literal, as this allows us to use
                        // match/sync/etc. as a name
        ...
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CURSOR) {
            tokenizer->advance();
            continue; // no point in cursor in the middle of expression

        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONSTANT) {
            ...
            tokenizer->advance();
            expr = constant;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONST_PI) {
            // constant defined by tokenizer
            ConstantNode* constant = alloc_node<ConstantNode>();
            ...
            tokenizer->advance();
            expr = constant;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONST_TAU) {
            // constant defined by tokenizer
            ConstantNode* constant = alloc_node<ConstantNode>();
            ...
            tokenizer->advance();
            expr = constant;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONST_INF) {
            // constant defined by tokenizer
            ConstantNode* constant = alloc_node<ConstantNode>();
            ...
            tokenizer->advance();
            expr = constant;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONST_NAN) {
            // constant defined by tokenizer
            ConstantNode* constant = alloc_node<ConstantNode>();
            ...
            tokenizer->advance();
            expr = constant;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_PR_PRELOAD) {
            // constant defined by tokenizer
            tokenizer->advance();
            if (tokenizer->get_token()
                != RebelScriptTokenizer::TK_PARENTHESIS_OPEN) {
            ...
            if (subexpr) {
                ...
                if (subexpr->type == Node::TYPE_IDENTIFIER) {
                    ...
                    if (current_class->constant_expressions.has(in->name)) {
                        Node* cn_exp =
                            current_class->constant_expressions[in->name]
                                .expression;
            ...
            Ref<RebelScript> gds = res;
            if (gds.is_valid() && !gds->is_valid()) {
                _set_error(
                    "Couldn't fully preload the script, possible cyclic "
                    "reference or compilation error. Use \"load()\" instead if "
                    "a cyclic reference is intended."
                );
                return nullptr;
            }
            ...
            expr = constant;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_PR_YIELD) {
            if (!current_function) {
                _set_error(
            ...
            if (tokenizer->get_token()
                ...
                if (tokenizer->get_token() != RebelScriptTokenizer::TK_COMMA) {
                    _set_error(
        ...
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_SELF) {
            if (p_static) {
                _set_error(
                    "\"self\" isn't allowed in a static function or constant "
                    "expression."
                );
                return nullptr;
            }
            ...
            tokenizer->advance();
            expr = self;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_BUILT_IN_TYPE && tokenizer->get_token(1) == RebelScriptTokenizer::TK_PERIOD) {
            ...
            if (!Variant::has_constant(bi_type, identifier)) {
                if (tokenizer->get_token()
                    ...
                    && Variant::get_method_return_type(bi_type, identifier)
                           == bi_type) {
                    tokenizer->advance();
                    ...
                    if (bi_type == Variant::OBJECT) {
                        int object_constant = ClassDB::get_integer_constant(
                            "Object",
                            identifier,
                            &valid
                        );
                    ...
                    if (!valid) {
                        _set_error(
                            "Static constant  '" + identifier.operator String()
                            + "' not present in built-in type "
                            + Variant::get_type_name(bi_type) + "."
                        );
                        return nullptr;
                    }
                }
        ...
        } else if (tokenizer->get_token(next_valid_offset) == RebelScriptTokenizer::TK_PARENTHESIS_OPEN && tokenizer->is_token_literal()) {
            // We check with is_token_literal, as this allows us to use
            // match/sync/etc. as a name
            ...
            // Do a quick Array and Dictionary Check.  Replace if either require
            // no arguments.
            bool replaced = false;

            if (tokenizer->get_token()
                == RebelScriptTokenizer::TK_BUILT_IN_TYPE) {
                Variant::Type ct = tokenizer->get_token_type();
                if (!p_parsing_constant) {
                    if (ct == Variant::ARRAY) {
                        if (tokenizer->get_token(2)
                            == RebelScriptTokenizer::TK_PARENTHESIS_CLOSE) {
            ...
            } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_BUILT_IN_FUNC) {
            ...
            while (!bfn && b) {
                if (b->variables.has(identifier)) {
                    ...
                    switch (tokenizer->get_token()) {
                        case RebelScriptTokenizer::TK_OP_ASSIGN_ADD:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_BIT_AND:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_BIT_OR:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_BIT_XOR:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_DIV:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_MOD:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_MUL:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_SHIFT_LEFT:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_SHIFT_RIGHT:
                        case RebelScriptTokenizer::TK_OP_ASSIGN_SUB: {
                            if (lv->assignments == 0) {
                                if (!lv->datatype.has_type) {
                                    _set_error(
                                        "Using assignment with operation on a "
                        ...
                        case RebelScriptTokenizer::TK_OP_ASSIGN: {
                            lv->assignments += 1;
            ...
            if (!bfn && p_parsing_constant) {
                if (cln->constant_expressions.has(identifier)) {
                    ...
                    bfn  = true;
                } else if (RebelScriptLanguage::get_singleton()
                               ->get_global_map()
                               .has(identifier)) {
                    // check from constants
                    ConstantNode* constant = alloc_node<ConstantNode>();
                    constant->value =
                        RebelScriptLanguage::get_singleton()->get_global_array(
                ...
                if (!bfn
                    ...
                    ConstantNode* constant = alloc_node<ConstantNode>();
                    constant->value = RebelScriptLanguage::get_singleton()
                                          ->get_named_globals_map()[identifier];
                    expr = constant;
                    bfn  = true;
                }
                ...
                if (!dependencies_only) {
                    ...
                    if (!bfn) {
                        // Using current_class instead of cln here, since cln is
                        // const*
                        _determine_inheritance(current_class, false);
                        if (cln->base_type.has_type
                            && cln->base_type.kind == DataType::REBELSCRIPT
                            && cln->base_type.script_type->is_valid()) {
                            Map<StringName, Variant> parent_constants;
                            current_class->base_type.script_type->get_constants(
                                &parent_constants
                            );
                            if (parent_constants.has(identifier)) {
                                ConstantNode* constant =
                                    alloc_node<ConstantNode>();
                ...
                if (current_function) {
                    ...
                    if (arg_idx != -1) {
                        switch (tokenizer->get_token()) {
                            ...
                            default: {
                                current_function->arguments_usage
                                    .write[arg_idx] =
        ...
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_OP_ADD || tokenizer->get_token() == RebelScriptTokenizer::TK_OP_SUB || tokenizer->get_token() == RebelScriptTokenizer::TK_OP_NOT || tokenizer->get_token() == RebelScriptTokenizer::TK_OP_BIT_INVERT) {
            ...
            switch (tokenizer->get_token()) {
                case RebelScriptTokenizer::TK_OP_ADD:
                    e.op = OperatorNode::OP_POS;
                    break;
                case RebelScriptTokenizer::TK_OP_SUB:
                    e.op = OperatorNode::OP_NEG;
                    break;
                case RebelScriptTokenizer::TK_OP_NOT:
                    e.op = OperatorNode::OP_NOT;
                    break;
                case RebelScriptTokenizer::TK_OP_BIT_INVERT:
            ...
            if (e.op != OperatorNode::OP_NOT
                && tokenizer->get_token() == RebelScriptTokenizer::TK_OP_NOT) {
        ...
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_PR_IS && tokenizer->get_token(1) == RebelScriptTokenizer::TK_BUILT_IN_TYPE) {
            ...
            expr = op;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_BRACKET_OPEN) {
            // array
            tokenizer->advance();
            ...
            while (true) {
                ...
                } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_BRACKET_CLOSE) {
                    tokenizer->advance();
                    break;
                } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_NEWLINE) {
            ...
            expr = arr;
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CURLY_BRACKET_OPEN) {
            // array
            tokenizer->advance();
            ...
            while (true) {
                if (tokenizer->get_token() == RebelScriptTokenizer::TK_EOF) {
                ...
                } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CURLY_BRACKET_CLOSE) {
                ...
                } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_COLON) {
                    if (expecting == DICT_EXPECT_KEY) {
                        _set_error("key or '}' expected");
                        return nullptr;
                    }
                    ...
                    if (expecting == DICT_EXPECT_KEY) {
                        if (tokenizer->is_token_literal()
                            && tokenizer->get_token(1)
                                   == RebelScriptTokenizer::TK_OP_ASSIGN) {
                            // We check with is_token_literal, as this allows us
                            // to use match/sync/etc. as a name
                    ...
                    if (expecting == DICT_EXPECT_VALUE) {
                        ...
                        if (key->type
                            ...
                            Variant const& keyName =
                                static_cast<
                                    const RebelScriptParser::ConstantNode*>(key)
                                    ->value;
        ...
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_PERIOD && (tokenizer->is_token_literal(1) || tokenizer->get_token(1) == RebelScriptTokenizer::TK_CURSOR)) {
            // We check with is_token_literal, as this allows us to use
            // match/sync/etc. as a name parent call
            ...
            forbidden for now */
            StringName identifier;
            bool is_completion = _get_completable_identifier(
        ...
        } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_BUILT_IN_TYPE && expression.size() > 0 && expression[expression.size() - 1].is_op && expression[expression.size() - 1].op == OperatorNode::OP_IS) {
            ...
            _set_error(
                ...
                + String(tokenizer->get_token_name(tokenizer->get_token()))
            );
        ...
        ERR_FAIL_COND_V_MSG(
            !expr,
            nullptr,
            "RebelScriptParser bug, couldn't figure out what expression is."
        );
        ...
        while (true) {
            ...
            if (tokenizer->get_token() == RebelScriptTokenizer::TK_PERIOD) {
                // indexing using "."

                if (tokenizer->get_token(1) != RebelScriptTokenizer::TK_CURSOR
                    && !tokenizer->is_token_literal(1)) {
                    // We check with is_token_literal, as this allows us to use
                    // match/sync/etc. as a name
                    _set_error("Expected identifier as member");
                    return nullptr;
                } else if (tokenizer->get_token(2) == RebelScriptTokenizer::TK_PARENTHESIS_OPEN) {
                    // call!!
                    OperatorNode* op = alloc_node<OperatorNode>();
                    op->op           = OperatorNode::OP_CALL;
                ...
                if (tokenizer->get_token()
                    != RebelScriptTokenizer::TK_BRACKET_CLOSE) {
                    _set_error("Expected ']'");
                    return nullptr;
                }
        ...
        if (tokenizer->get_token() == RebelScriptTokenizer::TK_PR_AS) {
            if (has_casting) {
...
#define _VALIDATE_ASSIGN                                                       \
    ...
    p_allow_assign = false;
        ...
        switch (tokenizer->get_token()) { // see operator
            ...
            case RebelScriptTokenizer::TK_OP_IN:
                op = OperatorNode::OP_IN;
                break;
            case RebelScriptTokenizer::TK_OP_EQUAL:
                op = OperatorNode::OP_EQUAL;
                break;
            case RebelScriptTokenizer::TK_OP_NOT_EQUAL:
                op = OperatorNode::OP_NOT_EQUAL;
                break;
            case RebelScriptTokenizer::TK_OP_LESS:
                op = OperatorNode::OP_LESS;
                break;
            case RebelScriptTokenizer::TK_OP_LESS_EQUAL:
                op = OperatorNode::OP_LESS_EQUAL;
                break;
            case RebelScriptTokenizer::TK_OP_GREATER:
                op = OperatorNode::OP_GREATER;
                break;
            case RebelScriptTokenizer::TK_OP_GREATER_EQUAL:
                op = OperatorNode::OP_GREATER_EQUAL;
                break;
            case RebelScriptTokenizer::TK_OP_AND:
                op = OperatorNode::OP_AND;
                break;
            case RebelScriptTokenizer::TK_OP_OR:
            ...
            case RebelScriptTokenizer::TK_OP_SUB:
                ...
                break;
            case RebelScriptTokenizer::TK_OP_MUL:
                op = OperatorNode::OP_MUL;
                break;
            case RebelScriptTokenizer::TK_OP_DIV:
                op = OperatorNode::OP_DIV;
                break;
            case RebelScriptTokenizer::TK_OP_MOD:
                op = OperatorNode::OP_MOD;
                break;
            // case RebelScriptTokenizer::TK_OP_NEG: op=OperatorNode::OP_NEG ;
            // break;
            case RebelScriptTokenizer::TK_OP_SHIFT_LEFT:
                op = OperatorNode::OP_SHIFT_LEFT;
                break;
            case RebelScriptTokenizer::TK_OP_SHIFT_RIGHT:
            ...
            case RebelScriptTokenizer::TK_OP_ASSIGN_BIT_XOR:
                ...
                break;
            case RebelScriptTokenizer::TK_OP_BIT_AND:
                op = OperatorNode::OP_BIT_AND;
                break;
            case RebelScriptTokenizer::TK_OP_BIT_OR:
                op = OperatorNode::OP_BIT_OR;
                break;
            case RebelScriptTokenizer::TK_OP_BIT_XOR:
                op = OperatorNode::OP_BIT_XOR;
                break;
            case RebelScriptTokenizer::TK_PR_IS:
                op = OperatorNode::OP_IS;
                break;
            case RebelScriptTokenizer::TK_CF_IF:
                op = OperatorNode::OP_TERNARY_IF;
                break;
            case RebelScriptTokenizer::TK_CF_ELSE:
    ...
    /* Reduce the set set of expressions and place them in an operator tree,
    ...
    while (expression.size() > 1) {
        ...
        for (int i = 0; i < expression.size(); i++) {
            ...
            switch (expression[i].op) {
                ...
                case OperatorNode::OP_TERNARY_ELSE:
                    priority = 14;
                    error    = true;
                    // Rigth-to-left should be false in this case, otherwise it
                    // would always error.
                    break;
                ...
                default: {
                    _set_error(
                        "RebelScriptParser bug, invalid operator in "
                        "expression: "
                        + itos(expression[i].op)
                    );
                    return nullptr;
                }
            }
            ...
            if (priority < min_priority
                || (right_to_left && priority == min_priority)) {
        ...
        if (is_unary) {
            ...
            if (next_op >= (expression.size() - 2)
                || expression[next_op + 2].op
                       != OperatorNode::OP_TERNARY_ELSE) {
            ...
            if (expression[next_op + 1].is_op) {
                ...
                // due to how precedence works, unaries will always disappear
                // first
            ...
            ); // next expression goes as first
            op->arguments.push_back(expression[next_op - 1].node
            ); // left expression goes as when-true
            op->arguments.push_back(expression[next_op + 3].node
            ); // expression after next goes as when-false
            ...
            op->arguments.push_back(expression[next_op - 1].node
            ); // expression goes as left
            op->arguments.push_back(expression[next_op + 1].node
            ); // next expression goes as right
...
RebelScriptParser::Node* RebelScriptParser::_reduce_expression(
    Node* p_node,
    bool p_to_const
) {
    switch (p_node->type) {
        ...
        case Node::TYPE_ARRAY: {
            ...
            for (int i = 0; i < an->elements.size(); i++) {
                an->elements.write[i] =
                    _reduce_expression(an->elements[i], p_to_const);
            ...
            if (all_constants && p_to_const) {
                ...
                for (int i = 0; i < an->elements.size(); i++) {
                    ConstantNode* acn =
                        static_cast<ConstantNode*>(an->elements[i]);
                    arr[i] = acn->value;
                }
                cn->value    = arr;
                cn->datatype = _type_from_variant(cn->value);
        ...
        case Node::TYPE_DICTIONARY: {
            ...
            for (int i = 0; i < dn->elements.size(); i++) {
                dn->elements.write[i].key =
                    _reduce_expression(dn->elements[i].key, p_to_const);
                if (dn->elements[i].key->type != Node::TYPE_CONSTANT) {
                    all_constants = false;
                }
                dn->elements.write[i].value =
                    _reduce_expression(dn->elements[i].value, p_to_const);
            ...
            if (all_constants && p_to_const) {
                ...
                for (int i = 0; i < dn->elements.size(); i++) {
                    ConstantNode* key_c =
                        static_cast<ConstantNode*>(dn->elements[i].key);
                    ConstantNode* value_c =
                        static_cast<ConstantNode*>(dn->elements[i].value);
        ...
        case Node::TYPE_OPERATOR: {
            ...
            for (int i = 0; i < op->arguments.size(); i++) {
                op->arguments.write[i] =
                    _reduce_expression(op->arguments[i], p_to_const);
            ...
            if (op->op == OperatorNode::OP_PARENT_CALL) {
                ...
                if ((op->arguments[0]->type == Node::TYPE_TYPE
                     || (op->arguments[0]->type == Node::TYPE_BUILT_IN_FUNCTION
                         && RebelScriptFunctions::is_deterministic(
                             static_cast<BuiltInFunctionNode*>(op->arguments[0])
                                 ->function
                         )))
                    && last_not_constant == 0) {
                    ...
                    if (op->arguments.size() > 1) {
                        ...
                        for (int i = 0; i < ptrs.size(); i++) {
                            ConstantNode* cn =
                                static_cast<ConstantNode*>(op->arguments[i + 1]
                                );
                    ...
                    if (op->arguments[0]->type == Node::TYPE_TYPE) {
                        ...
                        RebelScriptFunctions::Function func =
                            static_cast<BuiltInFunctionNode*>(op->arguments[0])
                                ->function;
                        RebelScriptFunctions::call(
                            func,
                            vptr,
                            ptrs.size(),
                            v,
                    ...
                    if (ce.error != Variant::CallError::CALL_OK) {
                        ...
                        if (op->arguments[0]->type == Node::TYPE_TYPE) {
                            TypeNode* tn =
                                static_cast<TypeNode*>(op->arguments[0]);
                            errwhere = "'" + Variant::get_type_name(tn->vtype)
                                     + "' constructor";

                        } else {
                            RebelScriptFunctions::Function func =
                                static_cast<BuiltInFunctionNode*>(
                                    op->arguments[0]
                                )
                                    ->function;
                            errwhere = String("'")
                                     + RebelScriptFunctions::get_func_name(func)
                                     + "' intrinsic function";
                        }

                        switch (ce.error) {
                            case Variant::CallError::
                                CALL_ERROR_INVALID_ARGUMENT: {
                                _set_error(
                                    "Invalid argument (#"
                                    + itos(ce.argument + 1) + ") for "
                                    + errwhere + "."
                                );
                            ...
                            case Variant::CallError::
                                ...
                                _set_error(
                                    "Too many arguments for " + errwhere + "."
                                );
                            } break;
                            case Variant::CallError::
                                CALL_ERROR_TOO_FEW_ARGUMENTS: {
                                _set_error(
                                    "Too few arguments for " + errwhere + "."
                                );
                            } break;
                            default: {
                                _set_error(
                                    "Invalid arguments for " + errwhere + "."
                                );
                ...
                if (all_constants) {
                    ConstantNode* ca =
                        static_cast<ConstantNode*>(op->arguments[0]);
                    ConstantNode* cb =
                        static_cast<ConstantNode*>(op->arguments[1]);
                ...
                if (op->arguments[0]->type == Node::TYPE_CONSTANT
                    && op->arguments[1]->type == Node::TYPE_IDENTIFIER) {
                    ConstantNode* ca =
                        static_cast<ConstantNode*>(op->arguments[0]);
                    IdentifierNode* ib =
                        static_cast<IdentifierNode*>(op->arguments[1]);

                    bool valid;
                    Variant v = ca->value.get_named(ib->name, &valid);
                    if (!valid) {
                        _set_error(
                            "invalid index '" + String(ib->name)
            ...
            switch (op->op) {
                ...
                case OperatorNode::OP_ASSIGN_BIT_XOR: {
                    ...
                    if (op->arguments[0]->type == Node::TYPE_OPERATOR) {
                        OperatorNode* on =
                            static_cast<OperatorNode*>(op->arguments[0]);
                        if (on->op != OperatorNode::OP_INDEX
                            && on->op != OperatorNode::OP_INDEX_NAMED) {
                            _set_error(
...
#define _REDUCE_UNARY(m_vop)                                                   \
    ...
    Variant::evaluate(                                                         \
        m_vop,                                                                 \
        static_cast<ConstantNode*>(op->arguments[0])->value,                   \
        Variant(),                                                             \
        res,                                                                   \
        valid                                                                  \
    );                                                                         \
...
#define _REDUCE_BINARY(m_vop)                                                  \
    ...
    Variant::evaluate(                                                         \
        m_vop,                                                                 \
        static_cast<ConstantNode*>(op->arguments[0])->value,                   \
        static_cast<ConstantNode*>(op->arguments[1])->value,                   \
        res,                                                                   \
        valid                                                                  \
    );                                                                         \
    if (!valid) {                                                              \
    ...
    return cn;
            ...
            switch (op->op) {
                ...
                case OperatorNode::OP_TERNARY_IF: {
                    if (static_cast<ConstantNode*>(op->arguments[0])
                            ->value.booleanize()) {
...
RebelScriptParser::Node* RebelScriptParser::_parse_and_reduce_expression(
    Node* p_parent,
    bool p_static,
    bool p_reduce_const,
    bool p_allow_assign
    ...
    Node* expr =
        _parse_expression(p_parent, p_static, p_allow_assign, p_reduce_const);
    if (!expr || error_set) {
        return nullptr;
    }
...
bool RebelScriptParser::_reduce_export_var_type(Variant& p_value, int p_line) {
    ...
    _set_error(
        "Invalid export type. Only built-in and native resource types can be "
        "exported.",
        p_line
    );
    return false;
...
bool RebelScriptParser::_recover_from_completion() {
    if (!completion_found) {
        return false; // can't recover if no completion
    }
    // skip stuff until newline
    while (tokenizer->get_token() != RebelScriptTokenizer::TK_NEWLINE
           && tokenizer->get_token() != RebelScriptTokenizer::TK_EOF
           && tokenizer->get_token() != RebelScriptTokenizer::TK_ERROR) {
        tokenizer->advance();
    }
    completion_found = false;
    error_set        = false;
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_ERROR) {
        error_set = true;
    }
...
RebelScriptParser::PatternNode* RebelScriptParser::_parse_pattern(bool p_static
) {
    PatternNode* pattern = alloc_node<PatternNode>();

    RebelScriptTokenizer::Token token = tokenizer->get_token();
    if (error_set) {
        return nullptr;
    }

    if (token == RebelScriptTokenizer::TK_EOF) {
        return nullptr;
    }

    switch (token) {
        // array
        case RebelScriptTokenizer::TK_BRACKET_OPEN: {
            tokenizer->advance();
            pattern->pt_type = RebelScriptParser::PatternNode::PT_ARRAY;
            while (true) {
                if (tokenizer->get_token()
                    == RebelScriptTokenizer::TK_BRACKET_CLOSE) {
                    tokenizer->advance();
                ...
                if (tokenizer->get_token() == RebelScriptTokenizer::TK_PERIOD
                    && tokenizer->get_token(1)
                           == RebelScriptTokenizer::TK_PERIOD) {
                    // match everything
                    tokenizer->advance(2);
                    PatternNode* sub_pattern = alloc_node<PatternNode>();
                    sub_pattern->pt_type =
                        RebelScriptParser::PatternNode::PT_IGNORE_REST;
                    ...
                    if (tokenizer->get_token() == RebelScriptTokenizer::TK_COMMA
                        ...
                        _set_error(
                            "'..' pattern only allowed at the end of an array "
                            "pattern"
                        );
                        return nullptr;
                    }
        ...
        // bind
        case RebelScriptTokenizer::TK_PR_VAR: {
            tokenizer->advance();
            if (!tokenizer->is_token_literal()) {
                _set_error("Expected identifier for binding variable name.");
                return nullptr;
            }
            pattern->pt_type = RebelScriptParser::PatternNode::PT_BIND;
            ...
            while (bl) {
                if (bl->variables.has(pattern->bind)) {
                    _set_error(
                        "Binding name of '" + pattern->bind.operator String()
        ...
        // dictionary
        case RebelScriptTokenizer::TK_CURLY_BRACKET_OPEN: {
            tokenizer->advance();
            pattern->pt_type = RebelScriptParser::PatternNode::PT_DICTIONARY;
            while (true) {
                if (tokenizer->get_token()
                    == RebelScriptTokenizer::TK_CURLY_BRACKET_CLOSE) {
                    tokenizer->advance();
                    break;
                }
                ...
                if (tokenizer->get_token() == RebelScriptTokenizer::TK_PERIOD
                    ...
                    if (tokenizer->get_token() == RebelScriptTokenizer::TK_COMMA
                        ...
                        _set_error(
                            "'..' pattern only allowed at the end of a "
                            "dictionary pattern"
                        );
                        return nullptr;
                    }
                ...
                if (key->type != RebelScriptParser::Node::TYPE_CONSTANT) {
                ...
                if (tokenizer->get_token() == RebelScriptTokenizer::TK_COLON) {
                    ...
                    pattern->dictionary.insert(
                        static_cast<ConstantNode*>(key),
                        value
                    );
                } else {
                    pattern->dictionary.insert(
        ...
        case RebelScriptTokenizer::TK_WILDCARD: {
            tokenizer->advance();
            ...
            if (value->type == Node::TYPE_OPERATOR) {
                ...
                while (current_value->type == Node::TYPE_OPERATOR) {
                    OperatorNode* op_node =
                        static_cast<OperatorNode*>(current_value);

                    if (op_node->op != OperatorNode::OP_INDEX_NAMED) {
                        _set_error(
                            "Invalid operator in pattern. Only index (`A.B`) "
                            "is allowed"
                        );
                        return nullptr;
                    }
                ...
                if (current_value->type != Node::TYPE_IDENTIFIER) {
                    _set_error(
                        "Only constant expression or variables allowed in a "
                        "pattern"
                    );
                    return nullptr;
                }
                ...
                _set_error(
                    "Only constant expressions or variables allowed in a "
                    "pattern"
                );
                return nullptr;
            }
...
void RebelScriptParser::_parse_pattern_block(
    BlockNode* p_block,
    Vector<PatternBranchNode*>& p_branches,
    bool p_static
) {
    IndentLevel current_level = indent_level.back()->get();
    ...
    while (true) {
        ...
        bool catch_all =
            has_binding
            || branch->patterns[0]->pt_type == PatternNode::PT_WILDCARD;

#ifdef DEBUG_ENABLED
        // Branches after a wildcard or binding are unreachable
        if (catch_all_appeared && !current_function->has_unreachable_code) {
            _add_warning(
                RebelScriptWarning::UNREACHABLE_CODE,
                -1,
                current_function->name.operator String()
            );
            current_function->has_unreachable_code = true;
        }
#endif

        while (tokenizer->get_token() == RebelScriptTokenizer::TK_COMMA) {
            tokenizer->advance();
            branch->patterns.push_back(_parse_pattern(p_static));
            ...
            PatternNode::PatternType pt =
                branch->patterns[branch->patterns.size() - 1]->pt_type;
...
void RebelScriptParser::_generate_pattern(
    PatternNode* p_pattern,
    Node* p_node_to_match,
    Node*& p_resulting_node,
    Map<StringName, Node*>& p_bindings
) {
    ...
    switch (p_pattern->pt_type) {
        case PatternNode::PT_CONSTANT: {
            ...
            if (pattern_type.has_type && to_match_type.has_type) {
                if (!_is_type_compatible(to_match_type, pattern_type)
                    && !_is_type_compatible(pattern_type, to_match_type)) {
                ...
                // runtime typecheck
                BuiltInFunctionNode* typeof_node =
                    alloc_node<BuiltInFunctionNode>();
                typeof_node->function = RebelScriptFunctions::TYPE_OF;
        ...
        case PatternNode::PT_ARRAY: {
            ...
            if (p_pattern->array.size() > 0) {
                if (p_pattern->array[p_pattern->array.size() - 1]->pt_type
                    == PatternNode::PT_IGNORE_REST) {
                    open_ended = true;
                }
            }

            // typeof(value_to_match) == TYPE_ARRAY && value_to_match.size() >=
            // length typeof(value_to_match) == TYPE_ARRAY &&
            // value_to_match.size() == length

            {
                ...
                if (to_match_type.has_type) {
                    // must be an array
                    if (to_match_type.kind != DataType::BUILTIN
                        || to_match_type.builtin_type != Variant::ARRAY) {
                        _set_error(
                            "Cannot match an array pattern with a non-array "
                            "expression.",
                            p_pattern->line
                        );
                        return;
                    ...
                    OperatorNode* typeof_match_value =
                        alloc_node<OperatorNode>();
                    typeof_match_value->op = OperatorNode::OP_CALL;
                    typeof_match_value->arguments.push_back(typeof_node);
                    typeof_match_value->arguments.push_back(p_node_to_match);
                ...
                length->value        = Variant(
                    open_ended ? p_pattern->array.size() - 1
                               : p_pattern->array.size()
                );
        ...
        case PatternNode::PT_DICTIONARY: {
            ...
            // typeof(value_to_match) == TYPE_DICTIONARY &&
            // value_to_match.size() >= length typeof(value_to_match) ==
            // TYPE_DICTIONARY && value_to_match.size() == length

            {
                ...
                if (to_match_type.has_type) {
                    ...
                    if (to_match_type.kind != DataType::BUILTIN
                        || to_match_type.builtin_type != Variant::DICTIONARY) {
                        _set_error(
                            "Cannot match an dictionary pattern with a "
                            "non-dictionary expression.",
                            p_pattern->line
                        );
                        return;
                    }
                    ...
                    IdentifierNode* typeof_dictionary =
                        alloc_node<IdentifierNode>();
                ...
                ConstantNode* length = alloc_node<ConstantNode>();
                length->value        = Variant(
                    open_ended ? p_pattern->dictionary.size() - 1
                               : p_pattern->dictionary.size()
                );
            ...
            for (Map<ConstantNode*, PatternNode*>::Element* e =
                     p_pattern->dictionary.front();
                 e;
                 e = e->next()) {
                Node* condition = nullptr;
...
void RebelScriptParser::_transform_match_statment(MatchNode* p_match_statement
) {
    IdentifierNode* id = alloc_node<IdentifierNode>();
    id->name           = "#match_value";
    ...
    for (int i = 0; i < p_match_statement->branches.size(); i++) {
        ...
        for (Map<StringName, Node*>::Element* e = binding.front(); e;
             e                                  = e->next()) {
...
void RebelScriptParser::_parse_block(BlockNode* p_block, bool p_static) {
    IndentLevel current_level = indent_level.back()->get();
    ...
    while (true) {
        if (!is_first_line && indent_level.back()->prev()
            && indent_level.back()->prev()->get().indent
...
#ifdef DEBUG_ENABLED
        switch (token) {
            case RebelScriptTokenizer::TK_EOF:
            case RebelScriptTokenizer::TK_ERROR:
            case RebelScriptTokenizer::TK_NEWLINE:
            case RebelScriptTokenizer::TK_CF_PASS: {
                // will check later
            } break;
            default: {
                if (p_block->has_return
                    && !current_function->has_unreachable_code) {
                    _add_warning(
                        RebelScriptWarning::UNREACHABLE_CODE,
                        -1,
                        current_function->name.operator String()
        ...
        switch (token) {
            ...
            case RebelScriptTokenizer::TK_CF_PASS: {
                if (tokenizer->get_token(1)
                        != RebelScriptTokenizer::TK_SEMICOLON
                    && tokenizer->get_token(1)
                           != RebelScriptTokenizer::TK_NEWLINE
                    && tokenizer->get_token(1)
                           != RebelScriptTokenizer::TK_EOF) {
            ...
            case RebelScriptTokenizer::TK_PR_VAR: {
                ...
                if (current_function) {
                    for (int i = 0; i < current_function->arguments.size();
                         i++) {
                        if (n == current_function->arguments[i]) {
                            _set_error(
                                "Variable \"" + String(n)
                                + "\" already defined in the scope (at line "
                                + itos(current_function->line) + ")."
                            );
                            return;
                        }
                    }
                ...
                while (check_block) {
                    if (check_block->variables.has(n)) {
                        _set_error(
                            "Variable \"" + String(n)
                            + "\" already defined in the scope (at line "
                            + itos(check_block->variables[n]->line) + ")."
                        );
                        return;
                    }
                ...
                if (tokenizer->get_token()
                    ...
                    tokenizer->advance();
                    Node* subexpr =
                        _parse_and_reduce_expression(p_block, p_static);
                    if (!subexpr) {
                    ...
                    assigned =
                        _get_default_value_for_type(lv->datatype, var_line);
                }
            ...
            case RebelScriptTokenizer::TK_CF_IF: {
                ...
                while (true) {
                    while (tokenizer->get_token()
                               == RebelScriptTokenizer::TK_NEWLINE
                           && _parse_newline()) {
                        ;
                    }

                    if (indent_level.back()->get().indent
                        < current_level.indent) { // not at current indent level
                        p_block->end_line = tokenizer->get_token_line();
                        return;
                    }

                    if (tokenizer->get_token()
                        == RebelScriptTokenizer::TK_CF_ELIF) {
                        if (indent_level.back()->get().indent
                            > current_level.indent) {
                        ...
                        ControlFlowNode* cf_else =
                            alloc_node<ControlFlowNode>();
                        ...
                        all_have_return =
                            all_have_return && cf_else->body->has_return;

                    } else if (tokenizer->get_token() == RebelScriptTokenizer::TK_CF_ELSE) {
                        if (indent_level.back()->get().indent
                            > current_level.indent) {
                            _set_error("Invalid indentation.");
                            return;
                        ...
                        all_have_return =
                            all_have_return && cf_if->body_else->has_return;
                        have_else = true;
                ...
                // If there's no else block, path out of the if might not have a
                // return
                p_block->has_return     = all_have_return && have_else;

            } break;
            case RebelScriptTokenizer::TK_CF_WHILE: {
                tokenizer->advance();
                Node* condition2 =
                    _parse_and_reduce_expression(p_block, p_static);
                if (!condition2) {
            ...
            case RebelScriptTokenizer::TK_CF_FOR: {
                tokenizer->advance();
                ...
                for (int j = 0; j < current_class->variables.size(); j++) {
                    if (current_class->variables[j].identifier == id->name) {
                        _add_warning(
                            RebelScriptWarning::SHADOWED_VARIABLE,
                            id->line,
                            id->name,
                            itos(current_class->variables[j].line)
                        );
                    }
                }
#endif // DEBUG_ENABLED
                ...
                while (check_block) {
                    if (check_block->variables.has(id->name)) {
                        _set_error(
                            "Variable \"" + String(id->name)
                            + "\" already defined in the scope (at line "
                            + itos(check_block->variables[id->name]->line)
                            + ")."
                        );
                        return;
                    }
                ...
                if (tokenizer->get_token() != RebelScriptTokenizer::TK_OP_IN) {
                ...
                if (container->type == Node::TYPE_OPERATOR) {
                    ...
                    if (op->op == OperatorNode::OP_CALL
                        && op->arguments[0]->type
                               == Node::TYPE_BUILT_IN_FUNCTION
                        && static_cast<BuiltInFunctionNode*>(op->arguments[0])
                                   ->function
                               == RebelScriptFunctions::GEN_RANGE) {
                        // iterating a range, so see if range() can be optimized
                        // without allocating memory, by replacing it by vectors
                        // (which can work as iterable too!)
                        ...
                        for (int i = 1; i < op->arguments.size(); i++) {
                            ...
                            if (op->arguments[i]->type == Node::TYPE_CONSTANT) {
                                ConstantNode* c =
                                    static_cast<ConstantNode*>(op->arguments[i]
                                    );
                                if (c->value.get_type() == Variant::REAL
                                    || c->value.get_type() == Variant::INT) {
                        ...
                        if (args.size() > 0 && args.size() < 4) {
                            if (constant) {
                                ...
                                switch (args.size()) {
                                    ...
                                    case 2:
                                        cn->value =
                                            Vector2(constants[0], constants[1]);
                                        break;
                                    case 3:
                                        cn->value = Vector3(
                                            constants[0],
            ...
            case RebelScriptTokenizer::TK_CF_CONTINUE: {
            ...
            case RebelScriptTokenizer::TK_CF_BREAK: {
                BlockNode* upper_block = p_block;
            ...
            case RebelScriptTokenizer::TK_CF_RETURN: {
                tokenizer->advance();
            ...
            case RebelScriptTokenizer::TK_CF_MATCH: {
                tokenizer->advance();
                ...
                p_block->has_return =
                    p_block->has_return || compiled_branches->has_return;
                p_block->statements.push_back(match_cf_node);

                _end_statement();
            } break;
            case RebelScriptTokenizer::TK_PR_ASSERT: {
                tokenizer->advance();
            ...
            case RebelScriptTokenizer::TK_PR_BREAKPOINT: {
                tokenizer->advance();
            ...
            default: {
                Node* expression = _parse_and_reduce_expression(
                    p_block,
                    p_static,
                    false,
                    true
                ...
                if (!_end_statement()) {
                    // Attempt to guess a better error message if the user
                    // "retypes" a variable
                    if (tokenizer->get_token() == RebelScriptTokenizer::TK_COLON
                        && tokenizer->get_token(1)
                               == RebelScriptTokenizer::TK_OP_ASSIGN) {
                        _set_error(
                            "Unexpected ':=', use '=' instead. Expected end of "
                            "statement after expression."
                        );
                    } else {
                        _set_error(vformat(
                            "Expected end of statement after expression, got "
                            "%s instead.",
                            tokenizer->get_token_name(tokenizer->get_token())
                        ));
                    }
...
bool RebelScriptParser::_parse_newline() {
    if (tokenizer->get_token(1) != RebelScriptTokenizer::TK_EOF
        && tokenizer->get_token(1) != RebelScriptTokenizer::TK_NEWLINE) {
        IndentLevel current_level = indent_level.back()->get();
        int indent                = tokenizer->get_token_line_indent();
        int tabs                  = tokenizer->get_token_line_tab_indent();
...
void RebelScriptParser::_parse_extends(ClassNode* p_class) {
    if (p_class->extends_used) {
        _set_error("\"extends\" can only be present once per script.");
        return;
    }

    if (!p_class->constant_expressions.empty() || !p_class->subclasses.empty()
        || !p_class->functions.empty() || !p_class->variables.empty()) {
    ...
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_BUILT_IN_TYPE
        && tokenizer->get_token_type() == Variant::OBJECT) {
    ...
    if (tokenizer->get_token() == RebelScriptTokenizer::TK_CONSTANT) {
    ...
    while (true) {
        switch (tokenizer->get_token()) {
            case RebelScriptTokenizer::TK_IDENTIFIER: {
                StringName identifier = tokenizer->get_token_identifier();
                p_class->extends_class.push_back(identifier);
            } break;

            case RebelScriptTokenizer::TK_CURSOR:
            case RebelScriptTokenizer::TK_PERIOD:
                break;

            default: {
                _set_error(
                    "Invalid \"extends\" syntax, expected string constant "
                    "(path) and/or identifier (parent class)."
                );
                return;
            }
        }
...
void RebelScriptParser::_parse_class(ClassNode* p_class) {
    IndentLevel current_level = indent_level.back()->get();
    ...
    while (true) {
        ...
        switch (token) {
            ...
            case RebelScriptTokenizer::TK_PR_EXTENDS: {
                _mark_line_as_safe(tokenizer->get_token_line());
            ...
            case RebelScriptTokenizer::TK_PR_CLASS_NAME: {
                _mark_line_as_safe(tokenizer->get_token_line());
                if (p_class->owner) {
                    _set_error(
                        "\"class_name\" is only valid for the main class "
                        "namespace."
                    );
                    return;
                }
                if (self_path.begins_with("res://")
                    && self_path.find("::") != -1) {
                    _set_error(
                ...
                if (self_path != String()
                    && ScriptServer::is_global_class(p_class->name)
                    && ScriptServer::get_global_class_path(p_class->name)
                           != self_path) {
                    _set_error(
                        "Unique global class \"" + p_class->name
                ...
                if (tokenizer->get_token() == RebelScriptTokenizer::TK_COMMA) {
                    ...
                    if ((tokenizer->get_token()
                             == RebelScriptTokenizer::TK_CONSTANT
                         && tokenizer->get_token_constant().get_type()
                                == Variant::STRING)) {
#ifdef TOOLS_ENABLED
                        if (Engine::get_singleton()->is_editor_hint()) {
                            ...
                            String abs_icon_path = icon_path.is_rel_path()
                                                     ? self_path.get_base_dir()
                                                           .plus_file(icon_path)
                                                           .simplify_path()
                                                     : icon_path;
                            if (!FileAccess::exists(abs_icon_path)) {
                                _set_error(
                                    "No class icon found at: " + abs_icon_path
                                );
                                return;
                            }
                        ...
                        _set_error(
                            "The optional parameter after \"class_name\" must "
                            "be a string constant file path to an icon."
                        );
                        return;
                    }
            ...
            case RebelScriptTokenizer::TK_PR_TOOL: {
                if (p_class->tool) {
                    _set_error(
                        "The \"tool\" keyword can only be present once per "
                        "script."
                    );
                    return;
                }
            ...
            case RebelScriptTokenizer::TK_PR_CLASS: {
                ...
                if (tokenizer->get_token(1)
                    != RebelScriptTokenizer::TK_IDENTIFIER) {
                    _set_error(
                        "\"class\" syntax: \"class <Name>:\" or \"class <Name> "
                        "extends <BaseClass>:\""
                    );
                    return;
                }
                ...
                if (ClassDB::class_exists(name)
                    || ClassDB::class_exists("_" + name.operator String())) {
                    _set_error(
                ...
                if (ScriptServer::is_global_class(name)) {
                    _set_error(
                        "Can't override name of the unique global class \""
                        + name + "\". It already exists at: "
                        + ScriptServer::get_global_class_path(p_class->name)
                    );
                    return;
                }
                ...
                while (outer_class) {
                    for (int i = 0; i < outer_class->subclasses.size(); i++) {
                        if (outer_class->subclasses[i]->name == name) {
                            _set_error(
                                "Another class named \"" + String(name)
                                + "\" already exists in this scope (at line "
                                + itos(outer_class->subclasses[i]->line) + ")."
                            );
                            return;
                        }
                    }
                ...
                if (tokenizer->get_token()
                    == RebelScriptTokenizer::TK_PR_EXTENDS) {
            ...
            */
            case RebelScriptTokenizer::TK_PR_STATIC: {
                tokenizer->advance();
                if (tokenizer->get_token()
                    != RebelScriptTokenizer::TK_PR_FUNCTION) {
            ...
            case RebelScriptTokenizer::TK_PR_FUNCTION: {
                bool _static    = false;
                pending_newline = -1;

                if (tokenizer->get_token(-1)
                    == RebelScriptTokenizer::TK_PR_STATIC) {
                    _static = true;
                }
                ...
                if (name == StringName()) {
                    _set_error(
                        "Expected an identifier after \"func\" (syntax: \"func "
                ...
                for (int i = 0; i < p_class->static_functions.size(); i++) {
                    if (p_class->static_functions[i]->name == name) {
                        _set_error(
                            "The function \"" + String(name)
                            + "\" already exists in this class (at line "
                            + itos(p_class->static_functions[i]->line) + ")."
                        );
                    }
                }
                ...
                if (p_class->constant_expressions.has(name)) {
                    _add_warning(
                        RebelScriptWarning::FUNCTION_CONFLICTS_CONSTANT,
                        -1,
                        name
                    );
                }
                for (int i = 0; i < p_class->variables.size(); i++) {
                    if (p_class->variables[i].identifier == name) {
                        _add_warning(
                            RebelScriptWarning::FUNCTION_CONFLICTS_VARIABLE,
                            -1,
                            name
                        );
                    }
                ...
                if (tokenizer->get_token()
                    != RebelScriptTokenizer::TK_PARENTHESIS_OPEN) {
                    _set_error(
                        "Expected \"(\" after the identifier (syntax: \"func "
                        "<identifier>([arguments]):\" )."
                    );
                    return;
                }
                ...
                if (tokenizer->get_token()
                    ...
                    while (true) {
                        ...
                        if (tokenizer->get_token()
                            == RebelScriptTokenizer::TK_PR_VAR) {
                            tokenizer->advance(
                            ); // var before the identifier is allowed
                        }
                        ...
                        for (int i = 0; i < arguments.size(); i++) {
                            if (arguments[i] == argname) {
                                _set_error(
                                    "The argument name \"" + String(argname)
                        ...
                        if (tokenizer->get_token()
                            ...
                            tokenizer->advance(1);
                            Node* defval =
                                _parse_and_reduce_expression(p_class, _static);
                            if (!defval || error_set) {
                                return;
                            }
                        ...
                        if (tokenizer->get_token()
                            == RebelScriptTokenizer::TK_COMMA) {
                            tokenizer->advance();
                            continue;
                        } else if (tokenizer->get_token() != RebelScriptTokenizer::TK_PARENTHESIS_CLOSE) {
                            _set_error("Expected \",\" or \")\".");
                            return;
                        }
                    ...
                    if (p_class->extends_used) {
                        ...
                        if (tokenizer->get_token()
                            ...
                            if (tokenizer->get_token()
                                != RebelScriptTokenizer::TK_PARENTHESIS_OPEN) {
                                _set_error(
                                    "Expected \"(\" for parent constructor "
                                    "arguments."
                                );
                                return;
                            }
                            ...
                            if (tokenizer->get_token()
                                ...
                                while (true) {
                                    ...
                                    Node* arg = _parse_and_reduce_expression(
                                        p_class,
                                        _static
                                    );
                                    if (!arg) {
                        ...
                        if (tokenizer->get_token()
                            == RebelScriptTokenizer::TK_PERIOD) {
                            _set_error(
                                "Parent constructor call found for a class "
                                "without inheritance."
                            );
                            return;
                        }
                ...
                if (tokenizer->get_token()
                    == RebelScriptTokenizer::TK_FORWARD_ARROW) {
                ...
                if (!_enter_indent_block(block)) {
                    _set_error(vformat(
                        "Indented block expected after declaration of \"%s\" "
                        "function.",
                        function->name
                    ));
                    return;
            ...
            case RebelScriptTokenizer::TK_PR_SIGNAL: {
                _mark_line_as_safe(tokenizer->get_token_line());
                tokenizer->advance();
                ...
                for (int i = 0; i < current_class->_signals.size(); i++) {
                    if (current_class->_signals[i].name == sig.name) {
                        _set_error(
                            "The signal \"" + sig.name
                            + "\" already exists in this class (at line: "
                            + itos(current_class->_signals[i].line) + ")."
                        );
                        return;
                    }
                }
                ...
                if (tokenizer->get_token()
                    ...
                    while (true) {
                        ...
                        if (!tokenizer->is_token_literal(0, true)) {
                            _set_error(
                                "Expected an identifier in a \"signal\" "
                                "argument."
                            );
                            return;
                        }
            ...
            case RebelScriptTokenizer::TK_PR_EXPORT: {
                tokenizer->advance();
    ...
    do {                                                                       \
        tokenizer->advance();                                                  \
                    ...
                    if (tokenizer->get_token()
                            ...
                            switch (type) {
                                case Variant::INT: {
                                    if (tokenizer->get_token(
                                        ...
                                        if (tokenizer->get_token()
                                            == RebelScriptTokenizer::
                                                TK_PARENTHESIS_CLOSE) {
                                            WARN_DEPRECATED_MSG(
                                                "Exporting bit flags hint "
                                                "requires string constants."
                                            );
                                            break;
                                        }
                                        if (tokenizer->get_token()
                                ...
                                case Variant::REAL: {
                                    ...
                                    if (tokenizer->get_token()
                                        == RebelScriptTokenizer::TK_OP_SUB) {
                                        sign = -1;
                                        _ADVANCE_AND_CONSUME_NEWLINES;
                                    }
                                    if (tokenizer->get_token()
                                        ...
                                        current_export = PropertyInfo();
                                        _set_error(
                                            "Expected a range in the numeric "
                                            "hint."
                                        );
                                        return;
                                    }

                                    current_export.hint_string = rtos(
                                        sign
                                        * double(tokenizer->get_token_constant()
                                        )
                                    );
                                    _ADVANCE_AND_CONSUME_NEWLINES;
                                    ...
                                    if (tokenizer->get_token()
                                        ...
                                        current_export = PropertyInfo();
                                        _set_error(
                                            "Expected a number as upper bound "
                                            "in the numeric range hint."
                                        );
                                        return;
                                    }
                                    ...
                                    if (tokenizer->get_token()
                                        ...
                                        current_export = PropertyInfo();
                                        _set_error(
                                            "Expected a number as step in the "
                                            "numeric range hint."
                                        );
                                        return;
                                    }
                                    ...
                                    if (tokenizer->get_token(
                                        ...
                                        if (tokenizer->get_token()
                                            ...
                                            if (tokenizer->get_token()
                                                ...
                                                _set_error(
                                                    "Expected \"GLOBAL\" after "
                                                    "comma in the directory "
                                                    "hint."
                                                );
                                                return;
                                            }
                                            if (!p_class->tool) {
                                                _set_error(
                                                    "Global filesystem hints "
                                                    "may only be used in tool "
                                                    "scripts."
                                                );
                                                return;
                                            }
                                ...
                                case Variant::COLOR: {
                                    if (tokenizer->get_token()
                                        ...
                                        current_export = PropertyInfo();
                                        _set_error(
                                            "Color type hint expects RGB or "
                                            "RGBA as hints."
                                        );
                                        return;
                                    }
                        ...
                        parenthesis++;
                        Node* subexpr =
                            _parse_and_reduce_expression(p_class, true, true);
                        if (!subexpr) {
                            if (_recover_from_completion()) {
                                break;
                            }
                        ...
                        Variant constant =
                            static_cast<ConstantNode*>(subexpr)->value;

                        if (constant.get_type() == Variant::OBJECT) {
                            RebelScriptNativeClass* native_class =
                                Object::cast_to<RebelScriptNativeClass>(constant
                                );

                            if (native_class
                                && ClassDB::is_parent_class(
                                    native_class->get_name(),
                                    "Resource"
                                )) {
                                current_export.type = Variant::OBJECT;
                                current_export.hint =
                                    PROPERTY_HINT_RESOURCE_TYPE;
                                current_export.usage |=
                                    PROPERTY_USAGE_SCRIPT_VARIABLE;

                                current_export.hint_string =
                                    native_class->get_name();
                                current_export.class_name =
                                    native_class->get_name();
                            ...
                            current_export.hint = is_flags ? PROPERTY_HINT_FLAGS
                                                           : PROPERTY_HINT_ENUM;
                            current_export.usage |=
                                PROPERTY_USAGE_SCRIPT_VARIABLE;
                            ...
                            bool first = true;
                            for (List<Variant>::Element* E = keys.front(); E;
                                 E                         = E->next()) {
                                if (enum_values[E->get()].get_type()
                                    == Variant::INT) {
                                    if (!first) {
                                        current_export.hint_string += ",";
                                    } else {
                    ...
                    if (is_arrayed) {
                        ...
                        current_export.hint_string =
                            hint_prefix + ":" + current_export.hint_string;
...
#undef _ADVANCE_AND_CONSUME_NEWLINES
                ...
                if (tokenizer->get_token() != RebelScriptTokenizer::TK_PR_VAR
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_ONREADY
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_REMOTE
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_MASTER
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_PUPPET
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_SYNC
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_REMOTESYNC
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_MASTERSYNC
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_PUPPETSYNC
                    && tokenizer->get_token()
                           != RebelScriptTokenizer::TK_PR_SLAVE) {
                    current_export = PropertyInfo();
                    _set_error(
                        "Expected \"var\", \"onready\", \"remote\", "
                        "\"master\", \"puppet\", \"sync\", \"remotesync\", "
            ...
            case RebelScriptTokenizer::TK_PR_ONREADY: {
            ...
            case RebelScriptTokenizer::TK_PR_REMOTE: {
                // may be fallthrough from export, ignore if so
                tokenizer->advance();
            ...
            case RebelScriptTokenizer::TK_PR_MASTER: {
                // may be fallthrough from export, ignore if so
                tokenizer->advance();
                if (current_export.type) {
                    if (tokenizer->get_token()
            ...
            case RebelScriptTokenizer::TK_PR_SLAVE:
#ifdef DEBUG_ENABLED
                _add_warning(
                    RebelScriptWarning::DEPRECATED_KEYWORD,
                    tokenizer->get_token_line(),
                    "slave",
                    "puppet"
                );
#endif
                FALLTHROUGH;
            case RebelScriptTokenizer::TK_PR_PUPPET: {
                // may be fallthrough from export, ignore if so
                tokenizer->advance();
                if (current_export.type) {
                    if (tokenizer->get_token()
            ...
            case RebelScriptTokenizer::TK_PR_REMOTESYNC:
            case RebelScriptTokenizer::TK_PR_SYNC: {
                // may be fallthrough from export, ignore if so
                tokenizer->advance();
                if (tokenizer->get_token() != RebelScriptTokenizer::TK_PR_VAR
                    && tokenizer->get_token()
            ...
            case RebelScriptTokenizer::TK_PR_MASTERSYNC: {
                // may be fallthrough from export, ignore if so
                tokenizer->advance();
                if (tokenizer->get_token() != RebelScriptTokenizer::TK_PR_VAR
                    && tokenizer->get_token()
            ...
            case RebelScriptTokenizer::TK_PR_PUPPETSYNC: {
                // may be fallthrough from export, ignore if so
                tokenizer->advance();
                if (tokenizer->get_token() != RebelScriptTokenizer::TK_PR_VAR
                    && tokenizer->get_token()
            ...
            case RebelScriptTokenizer::TK_PR_VAR: {
                ...
                bool autoexport = tokenizer->get_token(-1)
                               == RebelScriptTokenizer::TK_PR_EXPORT;
                if (current_export.type != Variant::NIL) {
                    member._export = current_export;
                    current_export = PropertyInfo();
                }

                bool onready = tokenizer->get_token(-1)
                            == RebelScriptTokenizer::TK_PR_ONREADY;
                ...
                if (current_class->constant_expressions.has(member.identifier
                    ...
                    _set_error(
                        "A constant named \"" + String(member.identifier)
                        + "\" already exists in this class (at line: "
                        + itos(current_class
                                   ->constant_expressions[member.identifier]
                                   .expression->line)
                        + ")."
                    );
                    return;
                ...
                for (int i = 0; i < current_class->variables.size(); i++) {
                    if (current_class->variables[i].identifier
                        == member.identifier) {
                        _set_error(
                            "Variable \"" + String(member.identifier)
                            + "\" already exists in this class (at line: "
                ...
                for (int i = 0; i < current_class->subclasses.size(); i++) {
                    if (current_class->subclasses[i]->name
                        == member.identifier) {
                        _set_error(
                            "A class named \"" + String(member.identifier)
                            + "\" already exists in this class (at line "
                            + itos(current_class->subclasses[i]->line) + ")."
                        );
                        return;
                    }
                }
                ...
                for (int i = 0; i < current_class->functions.size(); i++) {
                    if (current_class->functions[i]->name
                        == member.identifier) {
                        _add_warning(
                            RebelScriptWarning::VARIABLE_CONFLICTS_FUNCTION,
                            member.line,
                            member.identifier
                        );
                        break;
                ...
                for (int i = 0; i < current_class->static_functions.size();
                     i++) {
                    if (current_class->static_functions[i]->name
                        == member.identifier) {
                        _add_warning(
                            RebelScriptWarning::VARIABLE_CONFLICTS_FUNCTION,
                            member.line,
                ...
                if (autoexport && member.data_type.has_type) {
                    if (member.data_type.kind == DataType::BUILTIN) {
                        ...
                        if (ClassDB::is_parent_class(
                            ...
                            member._export.usage |=
                                PROPERTY_USAGE_SCRIPT_VARIABLE;
                            member._export.hint_string =
                                member.data_type.native_type;
                            member._export.class_name =
                                member.data_type.native_type;
                        } else {
                            _set_error(
                                "Invalid export type. Only built-in and native "
                                "resource types can be exported.",
                                member.line
                            );
                            return;
                        }
                ...
                member.default_value =
                    Variant::construct(member._export.type, nullptr, 0, ce);
#endif
                ...
                if (tokenizer->get_token()
                    ...
                    Node* subexpr = _parse_and_reduce_expression(
                        p_class,
                        false,
                        autoexport || member._export.type != Variant::NIL
                    );
                    if (!subexpr) {
                        if (_recover_from_completion()) {
                            break;
                    ...
                    if (!onready && subexpr->type == Node::TYPE_OPERATOR) {
                        ...
                        if (op->op == OperatorNode::OP_CALL
                            && op->arguments[0]->type == Node::TYPE_SELF
                            && op->arguments[1]->type
                    ...
                    if (autoexport && !member.data_type.has_type) {
                        if (subexpr->type != Node::TYPE_CONSTANT) {
                            _set_error(
                                "Type-less export needs a constant expression "
                                "assigned to infer type."
                            );
                            return;
                        }
                        ...
                        if (cn->value.get_type() == Variant::NIL) {
                            _set_error(
                                "Can't accept a null constant expression for "
                                "inferring export type."
                            );
                            return;
                        }
                        ...
                        if (cn->value.get_type() == Variant::OBJECT) {
                            ...
                            if (res == nullptr) {
                                _set_error(
                                    "The exported constant isn't a type or "
                                    "resource."
                                );
                                return;
                            }
...
#ifdef TOOLS_ENABLED
                    if (subexpr->type == Node::TYPE_CONSTANT
                        && (member._export.type != Variant::NIL
                            || member.data_type.has_type)) {
                        ConstantNode* cn = static_cast<ConstantNode*>(subexpr);
                        if (cn->value.get_type() != Variant::NIL) {
                            if (member._export.type != Variant::NIL
                                ...
                                if (Variant::can_convert(
                                    ...
                                    _set_error(
                                        "Can't convert the provided value to "
                                        "the export type."
                                    );
                                    return;
                                }
                ...
                if (tokenizer->get_token()
                    == RebelScriptTokenizer::TK_PR_SETGET) {
                    tokenizer->advance();
                    ...
                    if (tokenizer->get_token()
                        ...
                        if (!tokenizer->is_token_literal()) {
                            _set_error(
                                "Expected an identifier for the setter "
                                "function after \"setget\"."
                            );
                        }
                    ...
                    if (tokenizer->get_token()
                        ...
                        if (!tokenizer->is_token_literal()) {
                            _set_error(
                                "Expected an identifier for the getter "
                                "function after \",\"."
                            );
                        }
            ...
            case RebelScriptTokenizer::TK_PR_CONST: {
                ...
                if (current_class->constant_expressions.has(const_id)) {
                    _set_error(
                        ...
                        + "\" already exists in this class (at line "
                        + itos(current_class->constant_expressions[const_id]
                                   .expression->line)
                        + ")."
                    );
                    return;
                ...
                for (int i = 0; i < current_class->variables.size(); i++) {
                    if (current_class->variables[i].identifier == const_id) {
                        _set_error(
                            "A variable named \"" + String(const_id)
                            + "\" already exists in this class (at line "
                            + itos(current_class->variables[i].line) + ")."
                        );
                        return;
            ...
            case RebelScriptTokenizer::TK_PR_ENUM: {
                // multiple constant declarations..

                int last_assign =
                    -1; // Incremented by 1 right before the assignment.
                String enum_name;
                ...
                if (tokenizer->is_token_literal(0, true)) {
                    ...
                    if (current_class->constant_expressions.has(enum_name)) {
                        _set_error(
                            "A constant named \"" + String(enum_name)
                            + "\" already exists in this class (at line "
                            + itos(current_class
                                       ->constant_expressions[enum_name]
                                       .expression->line)
                            + ")."
                        );
                        return;
                    ...
                    for (int i = 0; i < current_class->variables.size(); i++) {
                        if (current_class->variables[i].identifier
                            == enum_name) {
                            _set_error(
                                "A variable named \"" + String(enum_name)
                                + "\" already exists in this class (at line "
                                + itos(current_class->variables[i].line) + ")."
                            );
                            return;
                    ...
                    for (int i = 0; i < current_class->subclasses.size(); i++) {
                        if (current_class->subclasses[i]->name == enum_name) {
                            _set_error(
                                "A class named \"" + String(enum_name)
                                + "\" already exists in this class (at line "
                                + itos(current_class->subclasses[i]->line)
                                + ")."
                            );
                ...
                if (tokenizer->get_token()
                    != RebelScriptTokenizer::TK_CURLY_BRACKET_OPEN) {
                ...
                while (true) {
                    if (tokenizer->get_token()
                        ...
                        if (tokenizer->get_token()
                            ...
                            _set_error(
                                String("Unexpected ")
                                + RebelScriptTokenizer::get_token_name(
                                    tokenizer->get_token()
                                )
                        ...
                        if (tokenizer->get_token()
                            ...
                            enum_value_expr =
                                static_cast<ConstantNode*>(subexpr);

                            if (enum_value_expr->value.get_type()
                                != Variant::INT) {
                                _set_error(
                            ...
                            enum_value_expr->datatype =
                                _type_from_variant(enum_value_expr->value);
                        }
                        ...
                        if (enum_name != "") {
                            ...
                            if (current_class->constant_expressions.has(const_id
                                ...
                                _set_error(
                                    "A constant named \"" + String(const_id)
                                    + "\" already exists in this class (at "
                                      "line "
                                    + itos(current_class
                                               ->constant_expressions[const_id]
                                               .expression->line)
                                    + ")."
                                );
                                return;
                            ...
                            for (int i = 0; i < current_class->variables.size();
                                 i++) {
                                if (current_class->variables[i].identifier
                                    == const_id) {
                            ...
                            for (int i = 0;
                                 i < current_class->subclasses.size();
                                 i++) {
                                if (current_class->subclasses[i]->name
                                    == const_id) {
                            ...
                            p_class->constant_expressions.insert(
                                const_id,
                                constant
                            );
                        }
            ...
            case RebelScriptTokenizer::TK_CONSTANT: {
                if (tokenizer->get_token_constant().get_type()
                    ...
                    _set_error(
                        String() + "Unexpected constant of type: "
            ...
            default: {
                if (token == RebelScriptTokenizer::TK_IDENTIFIER) {
...
void RebelScriptParser::_determine_inheritance(
    ClassNode* p_class,
    bool p_recursive
) {
    if (p_class->base_type.has_type) {
        ...
        if (path != "") {
            ...
            if (path.is_rel_path()) {
                ...
                if (base == "" || base.is_rel_path()) {
                    _set_error(
                        "Couldn't resolve relative path for the parent class: "
                            + path,
                        p_class->line
                    );
                    return;
            ...
            if (p_class->extends_class.size()) {
                for (int i = 0; i < p_class->extends_class.size(); i++) {
                    ...
                    if (script->get_subclasses().has(sub)) {
                        Ref<Script> subclass = script->get_subclasses(
                        )[sub]; // avoid reference from disappearing
                        script = subclass;
                    } else {
                        _set_error(
                            "Couldn't find the subclass: " + sub,
                            p_class->line
                        );
                        return;
                    }
            ...
            Ref<RebelScript> base_script;

            if (ScriptServer::is_global_class(base)) {
                base_script = ResourceLoader::load(
                    ScriptServer::get_global_class_path(base)
                );
            ...
            while (p) {
                ...
                if (p->constant_expressions.has(base)) {
                    if (p->constant_expressions[base].expression->type
                        != Node::TYPE_CONSTANT) {
                        _set_error(
                            "Couldn't resolve the constant \"" + base + "\".",
                            p_class->line
                        );
                        return;
                    }
                    const ConstantNode* cn = static_cast<const ConstantNode*>(
                        p->constant_expressions[base].expression
                    );
            ...
            if (base_script.is_valid()) {
                String ident                   = base;
                Ref<RebelScript> find_subclass = base_script;

                for (int i = extend_iter; i < p_class->extends_class.size();
                     i++) {
                    ...
                    if (find_subclass->get_subclasses().has(subclass)) {
                        find_subclass =
                            find_subclass->get_subclasses()[subclass];
                    } else if (find_subclass->get_constants().has(subclass)) {
                        Ref<RebelScript> new_base_class =
                            find_subclass->get_constants()[subclass];
                        if (new_base_class.is_null()) {
                            _set_error(
                                "Constant isn't a class: " + ident,
                                p_class->line
                            );
                            return;
                        }
                        ...
                        _set_error(
                            "Couldn't find the subclass: " + ident,
                            p_class->line
                        );
                        return;
                    }
        ...
        if (base_class) {
            ...
            p_class->base_type.has_type    = true;
            p_class->base_type.kind        = DataType::REBELSCRIPT;
...
String RebelScriptParser::DataType::to_string() const {
    if (!has_type) {
        return "var";
    }
    switch (kind) {
        ...
        case NATIVE: {
            if (is_meta_type) {
                return "RebelScriptNativeClass";
            }
            return native_type.operator String();
        } break;

        case REBELSCRIPT: {
            Ref<RebelScript> gds    = script_type;
...
bool RebelScriptParser::_parse_type(DataType& r_type, bool p_can_be_void) {
    tokenizer->advance();
    ...
    switch (tokenizer->get_token()) {
        case RebelScriptTokenizer::TK_PR_VOID: {
            if (!p_can_be_void) {
                return false;
            }
        ...
        case RebelScriptTokenizer::TK_BUILT_IN_TYPE: {
        ...
        case RebelScriptTokenizer::TK_IDENTIFIER: {
            ...
            if (ClassDB::class_exists(r_type.native_type)
                || ClassDB::class_exists(
                    "_" + r_type.native_type.operator String()
                )) {
                r_type.kind = DataType::NATIVE;
            } else {
    ...
    if (can_index) {
        while (!finished) {
            switch (tokenizer->get_token()) {
                ...
                case RebelScriptTokenizer::TK_IDENTIFIER: {
                    ...
                    StringName id;
                    bool has_completion = _get_completable_identifier(
        ...
        if (tokenizer->get_token(-1) == RebelScriptTokenizer::TK_PERIOD) {
...
RebelScriptParser::DataType RebelScriptParser::_resolve_type(
    const DataType& p_source,
    int p_line
) {
    ...
    Vector<String> full_name =
        p_source.native_type.operator String().split(".", false);
    int name_part = 0;
    ...
    while (name_part < full_name.size()) {
        ...
        if (name_part == 0) {
            ...
            if (!singleton_path.empty()) {
                ...
                if (gds.is_valid()) {
                    ...
                    _set_error(
                        "Couldn't fully load singleton script '" + id
        ...
        while (p) {
            if (p->constant_expressions.has(id)) {
                if (p->constant_expressions[id].expression->type
                    != Node::TYPE_CONSTANT) {
                    _set_error("Parser bug: unresolved constant.", p_line);
                    ERR_FAIL_V(result);
                }
                const ConstantNode* cn = static_cast<const ConstantNode*>(
                    p->constant_expressions[id].expression
                );
                Ref<RebelScript> gds = cn->value;
                if (gds.is_valid()) {
                    result.kind        = DataType::REBELSCRIPT;
                    result.script_type = gds;
                    found              = true;
        ...
        if (!found
            && (base_type.kind == DataType::REBELSCRIPT
                || base_type.kind == DataType::SCRIPT)) {
            ...
            while (scr.is_valid()) {
                ...
                if (constants.has(id)) {
                    Ref<RebelScript> gds = constants[id];
...
RebelScriptParser::DataType RebelScriptParser::_type_from_variant(
...
RebelScriptParser::DataType RebelScriptParser::_type_from_property(
    const PropertyInfo& p_property,
    bool p_nil_is_variant
) const {
    DataType ret;
    ...
    if (p_property.type == Variant::OBJECT) {
        ...
        ret.native_type = p_property.class_name == StringName()
                            ? "Object"
...
RebelScriptParser::DataType RebelScriptParser::_type_from_gdtype(
    const RebelScriptDataType& p_gdtype
) const {
    DataType result;
    ...
    switch (p_gdtype.kind) {
        case RebelScriptDataType::UNINITIALIZED: {
...
RebelScriptParser::DataType RebelScriptParser::_get_operation_type(
    const Variant::Operator p_op,
    const DataType& p_a,
    const DataType& p_b,
    bool& r_valid
) const {
    ...
    Variant::Type a_type =
        p_a.kind == DataType::BUILTIN ? p_a.builtin_type : Variant::OBJECT;
    Variant::Type b_type =
        p_b.kind == DataType::BUILTIN ? p_b.builtin_type : Variant::OBJECT;
...
Variant::Operator RebelScriptParser::_get_variant_operation(
    const OperatorNode::Operator& p_op
) const {
    switch (p_op) {
        case OperatorNode::OP_NEG: {
...
bool RebelScriptParser::_is_type_compatible(
    const DataType& p_container,
    const DataType& p_expression,
    bool p_allow_implicit_conversion
) const {
    ...
    if (p_container.kind == DataType::BUILTIN
        ...
        if (p_allow_implicit_conversion) {
            valid = valid
                 || Variant::can_convert_strict(
                        p_expression.builtin_type,
                        p_container.builtin_type
                 );
        }
        return valid;
    }
    ...
    if (p_container.kind == DataType::BUILTIN
        && p_container.builtin_type == Variant::OBJECT) {
        // Object built-in is a special case, it's compatible with any object
        // and with null
    ...
    if (p_container.kind == DataType::BUILTIN
        || (p_expression.kind == DataType::BUILTIN
            && p_expression.builtin_type != Variant::NIL)) {
    ...
    switch (p_expression.kind) {
        case DataType::NATIVE: {
            ...
            if (p_expression.is_meta_type) {
                expr_native = RebelScriptNativeClass::get_class_static();
            } else {
                expr_native = p_expression.native_type;
            }
        } break;
        case DataType::SCRIPT:
        case DataType::REBELSCRIPT: {
            if (p_container.kind == DataType::CLASS) {
                // This cannot be resolved without cyclic dependencies, so just
                // bail out
                return false;
            }
            if (p_expression.is_meta_type) {
        ...
        case DataType::CLASS: {
            if (p_expression.is_meta_type) {
                expr_native = RebelScript::get_class_static();
            } else {
    ...
    switch (p_container.kind) {
        case DataType::NATIVE: {
            if (p_container.is_meta_type) {
                return ClassDB::is_parent_class(
                    expr_native,
                    RebelScriptNativeClass::get_class_static()
                );
            } else {
                StringName container_native =
                    ClassDB::class_exists(p_container.native_type)
                        ? p_container.native_type
                        : StringName("_" + p_container.native_type);
        ...
        case DataType::REBELSCRIPT: {
            if (p_container.is_meta_type) {
                return ClassDB::is_parent_class(
                    expr_native,
                    RebelScript::get_class_static()
                );
            }
            if (expr_class == head
                && p_container.script_type->get_path() == self_path) {
        ...
        case DataType::CLASS: {
            ...
            if (p_container.class_type == head && expr_script.is_valid()
                && expr_script->get_path() == self_path) {
...
RebelScriptParser::Node* RebelScriptParser::_get_default_value_for_type(
    const DataType& p_type,
    int p_line
) {
    Node* result;

    if (p_type.has_type && p_type.kind == DataType::BUILTIN
...
RebelScriptParser::DataType RebelScriptParser::_reduce_node_type(Node* p_node) {
#ifdef DEBUG_ENABLED
    if (p_node->get_datatype().has_type && p_node->type != Node::TYPE_ARRAY
        && p_node->type != Node::TYPE_DICTIONARY) {
#else
    ...
    switch (p_node->type) {
        case Node::TYPE_CONSTANT: {
            node_type =
                _type_from_variant(static_cast<ConstantNode*>(p_node)->value);
        } break;
        case Node::TYPE_TYPE: {
        ...
        case Node::TYPE_IDENTIFIER: {
            ...
            if (id->declared_block) {
                node_type =
                    id->declared_block->variables[id->name]->get_datatype();
                ...
                node_type =
                    _reduce_identifier_type(nullptr, id->name, id->line, false);
            }
        } break;
        case Node::TYPE_CAST: {
            ...
            if (source_type.has_type) {
                bool valid = false;
                if (check_types) {
                    if (cn->cast_type.kind == DataType::BUILTIN
                        && source_type.kind == DataType::BUILTIN) {
                        valid = Variant::can_convert(
                            source_type.builtin_type,
                            cn->cast_type.builtin_type
                        );
                    }
                    if (cn->cast_type.kind != DataType::BUILTIN
                        && source_type.kind != DataType::BUILTIN) {
                        valid = _is_type_compatible(cn->cast_type, source_type)
                             || _is_type_compatible(source_type, cn->cast_type);
                    }

                    if (!valid) {
                        _set_error(
                            "Invalid cast. Cannot convert from \""
                                + source_type.to_string() + "\" to \""
                                + cn->cast_type.to_string() + "\".",
                            cn->line
                        );
                        return DataType();
                    }
...
#ifdef DEBUG_ENABLED
                _add_warning(
                    RebelScriptWarning::UNSAFE_CAST,
                    cn->line,
                    cn->cast_type.to_string()
                );
#endif // DEBUG_ENABLED
            ...
            switch (op->op) {
                ...
                case OperatorNode::OP_YIELD: {
                    if (op->arguments.size() == 2) {
                        ...
                        DataType signal_type =
                            _reduce_node_type(op->arguments[1]);
                        // TODO: Check if signal exists when it's a constant
                        if (base_type.has_type
                            && base_type.kind == DataType::BUILTIN
                            && base_type.builtin_type != Variant::NIL
                            && base_type.builtin_type != Variant::OBJECT) {
                            _set_error(
                                "The first argument of \"yield()\" must be an "
                                "object.",
                                op->line
                            );
                            return DataType();
                        }
                        if (signal_type.has_type
                            && (signal_type.kind != DataType::BUILTIN
                                || signal_type.builtin_type != Variant::STRING
                            )) {
                            _set_error(
                                "The second argument of \"yield()\" must be a "
                                "string.",
                                op->line
                            );
                            return DataType();
                ...
                case OperatorNode::OP_IS_BUILTIN: {
                    ...
                    if (check_types && type_type.has_type) {
                        if (!type_type.is_meta_type
                            && (type_type.kind != DataType::NATIVE
                                || !ClassDB::is_parent_class(
                                    type_type.native_type,
                                    "Script"
                                ))) {
                            _set_error(
                                "Invalid \"is\" test: the right operand isn't "
                                "a type (neither a native type nor a script).",
                                op->line
                            );
                            return DataType();
                        }
                        type_type.is_meta_type = false; // Test the actual type
                        if (!_is_type_compatible(type_type, value_type)
                            && !_is_type_compatible(value_type, type_type)) {
                            if (op->op == OperatorNode::OP_IS) {
                                _set_error(
                                    "A value of type \""
                                        + value_type.to_string()
                                        + "\" will never be an instance of \""
                                        + type_type.to_string() + "\".",
                                    op->line
                                );
                            } else {
                    ...
                    if (check_types && !valid) {
                        _set_error(
                            "Invalid operand type (\""
                                ...
                                + "\") to unary operator \""
                                + Variant::get_operator_name(var_op) + "\".",
                            op->line,
                            op->column
                        );
                        return DataType();
                ...
                case OperatorNode::OP_BIT_XOR: {
                    ...
                    DataType argument_a_type =
                        _reduce_node_type(op->arguments[0]);
                    DataType argument_b_type =
                        _reduce_node_type(op->arguments[1]);
                    if (!argument_a_type.has_type
                        || !argument_b_type.has_type) {
                    ...
                    if (check_types && !valid) {
                        _set_error(
                            "Invalid operand types (\""
                                + argument_a_type.to_string() + "\" and \""
                                + argument_b_type.to_string()
                                + "\") to operator \""
                                + Variant::get_operator_name(var_op) + "\".",
                            op->line,
                            op->column
...
#ifdef DEBUG_ENABLED
                    if (var_op == Variant::OP_DIVIDE
                        && argument_a_type.kind == DataType::BUILTIN
                        && argument_a_type.builtin_type == Variant::INT
                        && argument_b_type.kind == DataType::BUILTIN
                        && argument_b_type.builtin_type == Variant::INT) {
                        _add_warning(
                            RebelScriptWarning::INTEGER_DIVISION,
                            op->line
                        );
                    }
#endif // DEBUG_ENABLED
                ...
                case OperatorNode::OP_TERNARY_IF: {
                    ...
                    // If types are equal, then the expression is of the same
                    // type If they are compatible, return the broader type
                    if (true_type == false_type
                        || _is_type_compatible(true_type, false_type)) {
                        node_type = true_type;
...
#ifdef DEBUG_ENABLED
                        _add_warning(
                            RebelScriptWarning::INCOMPATIBLE_TERNARY,
                            op->line
                        );
#endif // DEBUG_ENABLED
                    }
                ...
                case OperatorNode::OP_INIT_ASSIGN: {
                    _set_error(
                        "Assignment inside an expression isn't allowed (parser "
                        "bug?).",
                        op->line
                    );
                    return DataType();
                ...
                case OperatorNode::OP_INDEX_NAMED: {
                    ...
                    if (op->arguments[1]->type != Node::TYPE_IDENTIFIER) {
                        _set_error(
                            "Parser bug: named index without identifier "
                            "argument.",
                            op->line
                        );
                        ERR_FAIL_V(DataType());
                    ...
                    if (base_type.has_type) {
                        if (check_types
                            ...
                            switch (base_type.builtin_type) {
                                ...
                                default: {
                                    Variant::CallError err;
                                    Variant temp = Variant::construct(
                                    ...
                                    bool valid  = false;
                                    Variant res = temp.get(
                                        member_id->name.operator String(),
                                        &valid
                                    );
                            ...
                            node_type = _reduce_identifier_type(
                                &base_type,
                                member_id->name,
                                op->line,
                                true
                            ...
                            if (!node_type.has_type) {
                                _mark_line_as_unsafe(op->line);
                                _add_warning(
                                    RebelScriptWarning::UNSAFE_PROPERTY_ACCESS,
                                    op->line,
                                    member_id->name.operator String(),
                                    base_type.to_string()
                                );
                ...
                case OperatorNode::OP_INDEX: {
                    ...
                    if (check_types && index_type.has_type) {
                        if (base_type.kind == DataType::BUILTIN) {
                            ...
                            if (op->arguments[1]->type
                                ...
                                switch (base_type.builtin_type) {
                                    // Arrays/string have variable indexing,
                                    // can't test directly
                                    case Variant::STRING:
                                    case Variant::ARRAY:
                                    case Variant::DICTIONARY:
                            ...
                            _set_error(
                                "Only strings can be used as an index in the "
                                "base type \""
                                    + base_type.to_string() + "\".",
                                op->line
                            );
                    ...
                    if (check_types && !node_type.has_type
                        ...
                        switch (base_type.builtin_type) {
                            ...
                            case Variant::_RID: {
                                _set_error(
                                    "Can't index on a value of type \""
                                        + base_type.to_string() + "\".",
                                    op->line
                                );
                                return DataType();
...
bool RebelScriptParser::_get_function_signature(
    DataType& p_base_type,
    const StringName& p_function,
    DataType& r_return_type,
    List<DataType>& r_arg_types,
    ...
    Ref<RebelScript> base_rebelscript;
    Ref<Script> base_script;
    StringName native;
    if (p_base_type.kind == DataType::REBELSCRIPT) {
        base_rebelscript = p_base_type.script_type;
        if (base_rebelscript.is_null() || !base_rebelscript->is_valid()) {
            // RebelScript wasn't properly compíled, don't bother trying
            return false;
        }
    ...
    while (base_rebelscript.is_valid()) {
        native = base_rebelscript->get_instance_base_type();

        Map<StringName, RebelScriptFunction*> funcs =
            base_rebelscript->get_member_functions();

        if (funcs.has(p_function)) {
            RebelScriptFunction* f = funcs[p_function];
        ...
        base_rebelscript = base_rebelscript->get_base_script();
    }
    ...
    while (base_script.is_valid()) {
        ...
        if (!(mi == MethodInfo())) {
            ...
            for (List<PropertyInfo>::Element* E = mi.arguments.front(); E;
                 E                              = E->next()) {
    ...
    if (!ClassDB::class_exists(native)) {
        ...
        ERR_FAIL_V_MSG(
            false,
            "Parser bug: Class '" + String(native) + "' not found."
        );
    }
    ...
    if (!method) {
        ...
        for (const List<MethodInfo>::Element* E = virtuals.front(); E;
             E                                  = E->next()) {
            const MethodInfo& mi = E->get();
            if (mi.name == p_function) {
                r_default_arg_count = mi.default_arguments.size();
                for (const List<PropertyInfo>::Element* pi =
                         mi.arguments.front();
                     pi;
                     pi = pi->next()) {
        ...
        // If the base is a script, it might be trying to access members of the
        // Script class itself
        if (original_type.is_meta_type && !(p_function == "new")
            && (original_type.kind == DataType::SCRIPT
                || original_type.kind == DataType::REBELSCRIPT)) {
            method = ClassDB::get_method(
                original_type.script_type->get_class_name(),
                p_function
            );
            ...
            if (method) {
                ...
                for (const List<MethodInfo>::Element* E = virtuals.front(); E;
                    ...
                    if (mi.name == p_function) {
                        ...
                        r_return_type =
                            _type_from_property(mi.return_val, false);
                        r_static = true;
                        r_vararg = mi.flags & METHOD_FLAG_VARARG;
                        return true;
                    }
...
RebelScriptParser::DataType RebelScriptParser::_reduce_function_call_type(
    const OperatorNode* p_call
) {
    ...
    switch (p_call->arguments[0]->type) {
        case RebelScriptParser::Node::TYPE_TYPE: {
            ...
            // Special case: check copy constructor. Those are defined
            // implicitly in Variant.
            if (par_types.size() == 1) {
                if (!par_types[0].has_type
                    || (par_types[0].kind == DataType::BUILTIN
                        && par_types[0].builtin_type == tn->vtype)) {
                    DataType result;
                    result.has_type     = true;
                    result.kind         = DataType::BUILTIN;
            ...
            for (List<MethodInfo>::Element* E = constructors.front(); E;
                ...
                if (p_call->arguments.size() - 1
                    < mi.arguments.size() - mi.default_arguments.size()) {
                    continue;
                }
                ...
                for (int i = 0; i < par_types.size(); i++) {
                    ...
                    if (mi.arguments[i].type != Variant::NIL) {
                        ...
                        arg_type.kind = mi.arguments[i].type == Variant::OBJECT
                                          ? DataType::NATIVE
                                          : DataType::BUILTIN;
...
#ifdef DEBUG_ENABLED
                        if (arg_type.kind == DataType::BUILTIN
                            && arg_type.builtin_type == Variant::INT
                            && par_types[i].kind == DataType::BUILTIN
                            && par_types[i].builtin_type == Variant::REAL) {
                            _add_warning(
                                RebelScriptWarning::NARROWING_CONVERSION,
                                p_call->line,
                                Variant::get_type_name(tn->vtype)
                            );
                        }
                        if (par_types[i].may_yield
                            && p_call->arguments[i + 1]->type
                                   == Node::TYPE_OPERATOR) {
                            _add_warning(
                                RebelScriptWarning::FUNCTION_MAY_YIELD,
                                p_call->line,
                                _find_function_name(static_cast<OperatorNode*>(
                                    p_call->arguments[i + 1]
                                ))
                            );
                        }
#endif // DEBUG_ENABLED
        ...
        case RebelScriptParser::Node::TYPE_BUILT_IN_FUNCTION: {
            BuiltInFunctionNode* func =
                static_cast<BuiltInFunctionNode*>(p_call->arguments[0]);
            MethodInfo mi = RebelScriptFunctions::get_info(func->function);
        ...
        default: {
            if (p_call->op == OperatorNode::OP_CALL
                && p_call->arguments.size() < 2) {
                _set_error(
            ...
            IdentifierNode* func_id =
                static_cast<IdentifierNode*>(p_call->arguments[arg_id]);
            ...
            if (base_type.kind == DataType::BUILTIN) {
                Variant::CallError err;
                Variant tmp =
                    Variant::construct(base_type.builtin_type, nullptr, 0, err);

                if (check_types) {
                    if (!tmp.has_method(callee_name)) {
                        _set_error(
                            "The method \"" + callee_name
                    ...
                    default_args_count = Variant::get_method_default_arguments(
                                             base_type.builtin_type,
                                             callee_name
                    )
                                             .size();
                    const Vector<Variant::Type>& var_arg_types =
                        Variant::get_method_argument_types(
                            base_type.builtin_type,
                            callee_name
                        );
                ...
                return_type.builtin_type = Variant::get_method_return_type(
                    base_type.builtin_type,
                    callee_name,
                    &rets
                );
                // If the method returns, but it might return any type,
                // (Variant::NIL), pretend we don't know the type. At least make
                // sure we know that it returns
            ...
            bool is_get_script = p_call->arguments[0]->type == Node::TYPE_SELF
            ...
            if (is_initializer && original_type.is_meta_type) {
                ...
                valid = true; // There's always an initializer, we can assume
                              // this is true
            }
            ...
            if (!valid) {
                ...
                DataType tmp_type;
                valid =
                    _get_member_type(original_type, func_id->name, tmp_type);
                if (valid) {
                    if (tmp_type.is_constant) {
                        _add_warning(
                            RebelScriptWarning::CONSTANT_USED_AS_FUNCTION,
                            p_call->line,
                            callee_name,
                        ...
                        _add_warning(
                            RebelScriptWarning::PROPERTY_USED_AS_FUNCTION,
                            p_call->line,
                            callee_name,
                            original_type.to_string()
                        );
                ...
                _add_warning(
                    RebelScriptWarning::UNSAFE_METHOD_ACCESS,
                    p_call->line,
                    callee_name,
                    original_type.to_string()
                );
...
#ifdef DEBUG_ENABLED
            if (current_function && !for_completion && !is_static
            ...
            if (check_types && !is_static && !is_initializer
                && base_type.is_meta_type) {
                _set_error(
                    "Non-static function \"" + String(callee_name)
                        + "\" can only be called from an instance.",
                    p_call->line
                );
                return DataType();
            }
            ...
            if (callee_name == "emit_signal"
                && p_call->op == OperatorNode::OP_CALL
                && p_call->arguments[0]->type == Node::TYPE_SELF
                && p_call->arguments.size() >= 3
                && p_call->arguments[2]->type == Node::TYPE_CONSTANT) {
                ConstantNode* sig =
                    static_cast<ConstantNode*>(p_call->arguments[2]);
                String emitted = sig->value.get_type() == Variant::STRING
                                   ? sig->value.operator String()
                                   : "";
                for (int i = 0; i < current_class->_signals.size(); i++) {
    ...
    if (arg_count < arg_types.size() - default_args_count) {
        _set_error(
            "Too few arguments for \"" + callee_name
                + "()\" call. Expected at least "
                + itos(arg_types.size() - default_args_count) + ".",
            p_call->line
        );
        return return_type;
    }
    if (!is_vararg && arg_count > arg_types.size()) {
        _set_error(
            "Too many arguments for \"" + callee_name
                + "()\" call. Expected at most " + itos(arg_types.size()) + ".",
            p_call->line
        );
        return return_type;
    }
    ...
    for (int i = arg_diff; i < p_call->arguments.size(); i++) {
        ...
        if (!par_type.has_type) {
            _mark_line_as_unsafe(p_call->line);
            if (par_type.may_yield
                && p_call->arguments[i]->type == Node::TYPE_OPERATOR) {
                _add_warning(
                    RebelScriptWarning::FUNCTION_MAY_YIELD,
                    p_call->line,
                    _find_function_name(
                        static_cast<OperatorNode*>(p_call->arguments[i])
                    )
                );
            }
        } else if (!_is_type_compatible(
                       arg_types[i - arg_diff],
                       par_type,
                       true
                   )) {
            ...
            if (arg_type.kind == DataType::BUILTIN
                && arg_type.builtin_type == Variant::INT
                && par_type.kind == DataType::BUILTIN
                && par_type.builtin_type == Variant::REAL) {
                _add_warning(
                    RebelScriptWarning::NARROWING_CONVERSION,
                    p_call->line,
                    callee_name
...
bool RebelScriptParser::_get_member_type(
    const DataType& p_base_type,
    const StringName& p_member,
    DataType& r_member_type,
    bool* r_is_const
) const {
    ...
    while (base) {
        if (base->constant_expressions.has(p_member)) {
            ...
            r_member_type =
                base->constant_expressions[p_member].expression->get_datatype();
            return true;
        }
    ...
    Ref<RebelScript> gds;
    if (base_type.kind == DataType::REBELSCRIPT) {
    ...
    while (gds.is_valid()) {
        ...
        if (!base_type.is_meta_type) {
            if (gds->get_members().has(p_member)) {
                r_member_type =
                    _type_from_gdtype(gds->get_member_type(p_member));
                return true;
            }
        }
...
#define IS_USAGE_MEMBER(m_usage)                                               \
    (!(m_usage & (PROPERTY_USAGE_GROUP | PROPERTY_USAGE_CATEGORY)))
        ...
        for (List<PropertyInfo>::Element* E = properties.front(); E;
            ...
            if (E->get().name == p_member && IS_USAGE_MEMBER(E->get().usage)) {
                ...
                StringName getter_name =
                    ClassDB::get_property_getter(native, p_member);
                if (getter_name != StringName()) {
                    // Use the getter return type
#ifdef DEBUG_METHODS_ENABLED
                    MethodBind* getter_method =
                        ClassDB::get_method(native, getter_name);
                    if (getter_method) {
                        r_member_type =
                            _type_from_property(getter_method->get_return_info()
                            );
                    } else {
    ...
    if (p_base_type.is_meta_type
        && (p_base_type.kind == DataType::SCRIPT
            || p_base_type.kind == DataType::REBELSCRIPT)) {
...
RebelScriptParser::DataType RebelScriptParser::_reduce_identifier_type(
    const DataType* p_base_type,
    ...
    if (_get_member_type(base_type, p_identifier, member_type, &is_const)) {
        if (!p_base_type && current_function && current_function->_static
            && !is_const) {
            _set_error(
                "Can't access member variable (\""
                    + p_identifier.operator String()
    ...
    if (!p_base_type) {
        ...
        if (ClassDB::class_exists(p_identifier)
            || ClassDB::class_exists("_" + p_identifier.operator String())) {
            DataType result;
            result.has_type     = true;
            result.is_constant  = true;
            result.is_meta_type = true;
            if (Engine::get_singleton()->has_singleton(p_identifier)
                || Engine::get_singleton()->has_singleton(
                    "_" + p_identifier.operator String()
                )) {
                result.is_meta_type = false;
            }
            result.kind        = DataType::NATIVE;
        ...
        if (ScriptServer::is_global_class(p_identifier)) {
            Ref<Script> scr = ResourceLoader::load(
                ScriptServer::get_global_class_path(p_identifier)
            );
            if (scr.is_valid()) {
                DataType result;
                result.has_type      = true;
        ...
        if (RebelScriptLanguage::get_singleton()->get_global_map().has(
            ...
            Variant g =
                RebelScriptLanguage::get_singleton()->get_global_array()[idx];
            return _type_from_variant(g);
        }

        if (RebelScriptLanguage::get_singleton()->get_named_globals_map().has(
                p_identifier
            )) {
            Variant g =
                RebelScriptLanguage::get_singleton()->get_named_globals_map(
                )[p_identifier];
            return _type_from_variant(g);
        }
        ...
        _set_error(
            "The identifier \"" + p_identifier.operator String()
...
#ifdef DEBUG_ENABLED
    {
        ...
        if (_get_function_signature(
            ...
            _add_warning(
                RebelScriptWarning::FUNCTION_USED_AS_PROPERTY,
                p_line,
                p_identifier.operator String(),
                base_type.to_string()
            );
...
void RebelScriptParser::_check_class_level_types(ClassNode* p_class) {
    // Names of internal object properties that we check to avoid overriding
    // them.
    // "__meta__" could also be in here, but since it doesn't really affect
    // object metadata, it is okay to override it on script.
    StringName script_name = CoreStringNames::get_singleton()->_script;

    _mark_line_as_safe(p_class->line);

    // Constants
    for (Map<StringName, ClassNode::Constant>::Element* E =
        ...
        DataType expr =
            _resolve_type(c.expression->get_datatype(), c.expression->line);

        if (check_types && !_is_type_compatible(cont, expr)) {
            _set_error(
                "The constant value type (" + expr.to_string()
                    + ") isn't compatible with declared type ("
                    + cont.to_string() + ").",
                c.expression->line
            );
            return;
        ...
        if (constant_name == script_name
            || _get_member_type(p_class->base_type, constant_name, tmp)) {
            _set_error(
                "The member \"" + String(constant_name)
    ...
    for (int i = 0; i < p_class->variables.size(); i++) {
        ...
        DataType tmp;
        if (v.identifier == script_name
            || _get_member_type(p_class->base_type, v.identifier, tmp)) {
            _set_error(
                "The member \"" + String(v.identifier)
                    + "\" already exists in a parent class.",
                v.line
            );
            return;
        ...
        if (v.expression) {
            ...
            if (check_types && !_is_type_compatible(v.data_type, expr_type)) {
                ...
                if (_is_type_compatible(expr_type, v.data_type)) {
                    ...
                    if (v.data_type.kind != DataType::BUILTIN
                        || !_is_type_compatible(v.data_type, expr_type, true)) {
                        _set_error(
                            "The assigned expression's type ("
                                + expr_type.to_string()
                                + ") doesn't match the variable's type ("
                    ...
                    BuiltInFunctionNode* convert =
                        alloc_node<BuiltInFunctionNode>();
                    convert->line     = v.line;
                    convert->function = RebelScriptFunctions::TYPE_CONVERT;
            ...
            if (v.data_type.infer_type) {
                if (!expr_type.has_type) {
                    _set_error(
                        "The assigned value doesn't have a set type; the "
                        "variable type can't be inferred.",
                        v.line
                    );
                    return;
                }
                if (expr_type.kind == DataType::BUILTIN
                    && expr_type.builtin_type == Variant::NIL) {
                    _set_error(
                        "The variable type cannot be inferred because its "
                        "value is \"null\".",
                        v.line
                    );
                    return;
        ...
        if (v.data_type.has_type && v._export.type != Variant::NIL) {
            ...
            if (!_is_type_compatible(v.data_type, export_type, true)) {
                _set_error(
                    "The export hint's type (" + export_type.to_string()
                        + ") doesn't match the variable's type ("
                        + v.data_type.to_string() + ").",
                    v.line
                );
        ...
        for (int j = 0; j < p_class->functions.size(); j++) {
            if (v.setter == p_class->functions[j]->name) {
                ...
                if (setter->get_required_argument_count() != 1
                    && !(
                        setter->get_required_argument_count() == 0
                        && setter->default_values.size() > 0
                    )) {
                    _set_error(
                        "The setter function needs to receive exactly 1 "
                        "argument. See \""
                            + setter->name + "()\" definition at line "
                            + itos(setter->line) + ".",
                        v.line
                    );
                    return;
                }
                if (!_is_type_compatible(
                        v.data_type,
                        setter->argument_types[0]
                    )) {
                    _set_error(
                        "The setter argument's type ("
                            + setter->argument_types[0].to_string()
                            + ") doesn't match the variable's type ("
                            + v.data_type.to_string() + "). See \""
                            + setter->name + "()\" definition at line "
                            + itos(setter->line) + ".",
                        v.line
                    );
            ...
            if (v.getter == p_class->functions[j]->name) {
                ...
                if (getter->get_required_argument_count() != 0) {
                    _set_error(
                        "The getter function can't receive arguments. See \""
                            + getter->name + "()\" definition at line "
                            + itos(getter->line) + ".",
                        v.line
                    );
                    return;
        ...
        if ((found_getter || v.getter == StringName())
            && (found_setter || v.setter == StringName())) {
            continue;
        }
        ...
        for (int j = 0; j < p_class->static_functions.size(); j++) {
            if (v.setter == p_class->static_functions[j]->name) {
                ...
                _set_error(
                    "The setter can't be a static function. See \""
                        + setter->name + "()\" definition at line "
                        + itos(setter->line) + ".",
                    v.line
                );
            ...
            if (v.getter == p_class->static_functions[j]->name) {
                ...
                _set_error(
                    "The getter can't be a static function. See \""
                        + getter->name + "()\" definition at line "
                        + itos(getter->line) + ".",
                    v.line
                );
        ...
        for (int i = 0; i < p_class->_signals.size(); i++) {
            for (int j = 0; j < base_class->_signals.size(); j++) {
                if (p_class->_signals[i].name == base_class->_signals[j].name) {
                    _set_error(
                        "The signal \"" + p_class->_signals[i].name
                            + "\" already exists in a parent class.",
                        p_class->_signals[i].line
                    );
                    return;
                }
            }
    ...
    StringName native;
    if (base.kind == DataType::REBELSCRIPT || base.kind == DataType::SCRIPT) {
...
void RebelScriptParser::_check_function_types(FunctionNode* p_function) {
    p_function->return_type =
        _resolve_type(p_function->return_type, p_function->line);

    // Arguments
    int defaults_ofs =
        p_function->arguments.size() - p_function->default_values.size();
    for (int i = 0; i < p_function->arguments.size(); i++) {
        if (i < defaults_ofs) {
            p_function->argument_types.write[i] =
                _resolve_type(p_function->argument_types[i], p_function->line);
        } else {
            if (p_function->default_values[i - defaults_ofs]->type
                != Node::TYPE_OPERATOR) {
                _set_error(
            ...
            OperatorNode* op = static_cast<OperatorNode*>(
                p_function->default_values[i - defaults_ofs]
            );
            ...
            if (p_function->argument_types[i].infer_type) {
                ...
                p_function->argument_types.write[i] = _resolve_type(
                    p_function->argument_types[i],
                    p_function->line
                );
                ...
                if (!_is_type_compatible(
                    ...
                    _set_error(
                        "Value type (" + def_type.to_string()
                            + ") doesn't match the type of argument '"
                            + arg_name + "' ("
                            + p_function->argument_types[i].to_string() + ").",
                        p_function->line
                    );
                }
            }
        ...
        if (p_function->arguments_usage[i] == 0
            && !p_function->arguments[i].operator String().begins_with("_")) {
            _add_warning(
                RebelScriptWarning::UNUSED_ARGUMENT,
                p_function->line,
                p_function->name,
                p_function->arguments[i].operator String()
            );
        }
        for (int j = 0; j < current_class->variables.size(); j++) {
            if (current_class->variables[j].identifier
                == p_function->arguments[i]) {
                _add_warning(
                    RebelScriptWarning::SHADOWED_VARIABLE,
                    p_function->line,
                    p_function->arguments[i],
    ...
    if (!(p_function->name == "_init")) {
        ...
        if (_get_function_signature(
            ...
            valid            = valid
                 && p_function->default_values.size()
                        >= default_arg_count + argsize_diff;
            int i = 0;
            for (List<DataType>::Element* E = arg_types.front(); valid && E;
                 E                          = E->next()) {
                valid = valid && E->get() == p_function->argument_types[i++];
            }

            if (!valid) {
                String parent_signature =
                    return_type.has_type ? return_type.to_string() : "Variant";
                ...
                if (arg_types.size()) {
                    int j = 0;
                    for (List<DataType>::Element* E = arg_types.front(); E;
                         E                          = E->next()) {
                ...
                parent_signature += ")";
                _set_error(
                    "The function signature doesn't match the parent. Parent "
                    "signature is: \""
                        + parent_signature + "\".",
                    p_function->line
                );
        ...
        if (p_function->return_type.has_type
            && (p_function->return_type.kind != DataType::BUILTIN
                || p_function->return_type.builtin_type != Variant::NIL)) {
            _set_error(
    ...
    if (p_function->return_type.has_type
        ...
        if (!p_function->body->has_return) {
            _set_error(
                "A non-void function must return a value in all possible "
                "paths.",
                p_function->line
            );
            return;
    ...
    if (p_function->has_yield) {
        // yield() will make the function return a RebelScriptFunctionState, so
...
void RebelScriptParser::_check_class_blocks_types(ClassNode* p_class) {
    // Function blocks
    for (int i = 0; i < p_class->static_functions.size(); i++) {
    ...
    for (int i = 0; i < p_class->variables.size(); i++) {
        if (p_class->variables[i].usages == 0) {
            _add_warning(
                RebelScriptWarning::UNUSED_CLASS_VARIABLE,
                p_class->variables[i].line,
                p_class->variables[i].identifier
            );
        }
    }
    for (int i = 0; i < p_class->_signals.size(); i++) {
        if (p_class->_signals[i].emissions == 0) {
            _add_warning(
                RebelScriptWarning::UNUSED_SIGNAL,
                p_class->_signals[i].line,
                p_class->_signals[i].name
            );
        }
    }
#endif // DEBUG_ENABLED
...
static String _find_function_name(const RebelScriptParser::OperatorNode* p_call
    ...
    switch (p_call->arguments[0]->type) {
        case RebelScriptParser::Node::TYPE_TYPE: {
            return Variant::get_type_name(
                static_cast<RebelScriptParser::TypeNode*>(p_call->arguments[0])
                    ->vtype
            );
        } break;
        case RebelScriptParser::Node::TYPE_BUILT_IN_FUNCTION: {
            return RebelScriptFunctions::get_func_name(
                static_cast<RebelScriptParser::BuiltInFunctionNode*>(
                    p_call->arguments[0]
                )
                    ->function
            );
        ...
        default: {
            int id_index =
                p_call->op == RebelScriptParser::OperatorNode::OP_PARENT_CALL
                    ? 0
                    : 1;
            if (p_call->arguments.size() > id_index
                && p_call->arguments[id_index]->type
                       == RebelScriptParser::Node::TYPE_IDENTIFIER) {
                return static_cast<RebelScriptParser::IdentifierNode*>(
                           p_call->arguments[id_index]
                )
                    ->name;
            }
        } break;
...
void RebelScriptParser::_check_block_types(BlockNode* p_block) {
            ...
            case Node::TYPE_LOCAL_VAR: {
                ...
                if (lv->assign) {
                    ...
                    if (assign_type.has_type
                        && assign_type.kind == DataType::BUILTIN
                        && assign_type.builtin_type == Variant::NIL) {
                        if (lv->assign->type == Node::TYPE_OPERATOR) {
                            OperatorNode* call =
                                static_cast<OperatorNode*>(lv->assign);
                            if (call->op == OperatorNode::OP_CALL
                                || call->op == OperatorNode::OP_PARENT_CALL) {
                                _add_warning(
                                    RebelScriptWarning::VOID_ASSIGNMENT,
                                    lv->line,
                                    _find_function_name(call)
                                );
                            }
                        }
                    }
                    if (lv->datatype.has_type && assign_type.may_yield
                        && lv->assign->type == Node::TYPE_OPERATOR) {
                        _add_warning(
                            RebelScriptWarning::FUNCTION_MAY_YIELD,
                            lv->line,
                            _find_function_name(
...
#ifdef DEBUG_ENABLED
                            if (lv->datatype.builtin_type == Variant::INT
                                && assign_type.builtin_type == Variant::REAL) {
                                _add_warning(
                                    RebelScriptWarning::NARROWING_CONVERSION,
                                    lv->line
                                );
                    ...
                    if (lv->datatype.infer_type) {
                        if (!assign_type.has_type) {
                            _set_error(
                                "The assigned value doesn't have a set type; "
                                "the variable type can't be inferred.",
                                lv->line
                            );
                            return;
                        }
                        if (assign_type.kind == DataType::BUILTIN
                            && assign_type.builtin_type == Variant::NIL) {
                            _set_error(
                                "The variable type cannot be inferred because "
                                "its value is \"null\".",
                                lv->line
                            );
                            return;
            ...
            case Node::TYPE_OPERATOR: {
                ...
                switch (op->op) {
                    ...
                    case OperatorNode::OP_ASSIGN_BIT_XOR: {
                        if (op->arguments.size() < 2) {
                            _set_error(
                                "Parser bug: operation without enough "
                                "arguments.",
                                op->line,
                                op->column
                            );
                        ...
                        if (op->op != OperatorNode::OP_ASSIGN) {
                            ...
                            Variant::Operator oper =
                                _get_variant_operation(op->op);
                            bool valid = false;
                            rh_type    = _get_operation_type(
                                oper,
                                lh_type,
                                arg_type,
                                valid
                            ...
                            if (check_types && !valid) {
                                _set_error(
                                    "Invalid operand types (\""
                                        + lh_type.to_string() + "\" and \""
                                        + arg_type.to_string()
                                        + "\") to assignment operator \""
                                        + Variant::get_operator_name(oper)
                                        + "\".",
                                    op->line
                                );
                                return;
...
#ifdef DEBUG_ENABLED
                        if (rh_type.has_type
                            && rh_type.kind == DataType::BUILTIN
                            && rh_type.builtin_type == Variant::NIL) {
                            if (op->arguments[1]->type == Node::TYPE_OPERATOR) {
                                OperatorNode* call =
                                    static_cast<OperatorNode*>(op->arguments[1]
                                    );
                                if (call->op == OperatorNode::OP_CALL
                                    || call->op
                                           == OperatorNode::OP_PARENT_CALL) {
                                    _add_warning(
                                        RebelScriptWarning::VOID_ASSIGNMENT,
                                        op->line,
                                        _find_function_name(call)
                        ...
                        if (lh_type.has_type && rh_type.may_yield
                            && op->arguments[1]->type == Node::TYPE_OPERATOR) {
                            _add_warning(
                                RebelScriptWarning::FUNCTION_MAY_YIELD,
                                op->line,
                                _find_function_name(
                        ...
                        if (check_types
                            && !_is_type_compatible(lh_type, rh_type)) {
                            type_match = false;
                            ...
                            if (_is_type_compatible(rh_type, lh_type)) {
                                ...
                                if (op->op == OperatorNode::OP_ASSIGN) {
                                    // Replace assignment with implicit
                                    // conversion
                                    BuiltInFunctionNode* convert =
                                        alloc_node<BuiltInFunctionNode>();
                                    ...
                                    convert_call->op   = OperatorNode::OP_CALL;
                                    convert_call->arguments.push_back(convert);
                                    convert_call->arguments.push_back(
                                        op->arguments[1]
                                    );
                                    convert_call->arguments.push_back(tgt_type);
                                    ...
                                    type_match =
                                        true; // Since we are converting, the
                                              // type is matching
                                }
#ifdef DEBUG_ENABLED
                                if (lh_type.builtin_type == Variant::INT
                                    && rh_type.builtin_type == Variant::REAL) {
                                    _add_warning(
                                        RebelScriptWarning::
                        ...
                        if (!rh_type.has_type
                            && (op->op != OperatorNode::OP_ASSIGN
                                || lh_type.has_type
                                || op->arguments[0]->type == Node::TYPE_OPERATOR
                            )) {
                            _mark_line_as_unsafe(op->line);
                        }
#endif // DEBUG_ENABLED
...
#ifdef DEBUG_ENABLED
                        if (func_type.has_type
                            && (func_type.kind != DataType::BUILTIN
                                || func_type.builtin_type != Variant::NIL)) {
                            ...
                            _add_warning(
                                RebelScriptWarning::RETURN_VALUE_DISCARDED,
                                op->line,
                                func_name
                            );
                        }
                    ...
                    default: {
                        ...
                        if (op->op == OperatorNode::OP_TERNARY_IF) {
                            _add_warning(
                                RebelScriptWarning::STANDALONE_TERNARY,
                                statement->line
                            );
                        } else {
                            _add_warning(
                                RebelScriptWarning::STANDALONE_EXPRESSION,
                                statement->line
                            );
                        }
#endif // DEBUG_ENABLED
            ...
            case Node::TYPE_CONTROL_FLOW: {
                ...
                switch (cf->cf_type) {
                    case ControlFlowNode::CF_RETURN: {
                        ...
                        if (function_type.kind == DataType::BUILTIN
                            && function_type.builtin_type == Variant::NIL) {
...
#ifdef DEBUG_ENABLED
    // Warnings check
    for (Map<StringName, LocalVarNode*>::Element* E =
             p_block->variables.front();
         E;
         E = E->next()) {
        ...
        if (!lv->name.operator String().begins_with("_")) {
            if (lv->usages == 0) {
                _add_warning(
                    RebelScriptWarning::UNUSED_VARIABLE,
                    lv->line,
                    lv->name
                );
            } else if (lv->assignments == 0) {
                _add_warning(
                    RebelScriptWarning::UNASSIGNED_VARIABLE,
                    lv->line,
                    lv->name
                );
            }
...
void RebelScriptParser::_set_error(
...
#ifdef DEBUG_ENABLED
void RebelScriptParser::_add_warning(
    int p_code,
    int p_line,
    const String& p_symbol1,
    const String& p_symbol2,
    const String& p_symbol3,
    const String& p_symbol4
) {
...
void RebelScriptParser::_add_warning(
    ...
    if (GLOBAL_GET("debug/rebelscript/warnings/exclude_addons").booleanize()
        && base_path.begins_with("res://addons/")) {
        return;
    }
    if (tokenizer->is_ignoring_warnings()
        || !GLOBAL_GET("debug/rebelscript/warnings/enable").booleanize()) {
        return;
    }
    String warn_name =
        RebelScriptWarning::get_name_from_code((RebelScriptWarning::Code)p_code)
            .to_lower();
    if (tokenizer->get_warning_global_skips().has(warn_name)) {
        return;
    }
    if (!GLOBAL_GET("debug/rebelscript/warnings/" + warn_name)) {
        return;
    }

    RebelScriptWarning warn;
    warn.code    = (RebelScriptWarning::Code)p_code;
    warn.symbols = p_symbols;
    warn.line    = p_line == -1 ? tokenizer->get_token_line() : p_line;

    List<RebelScriptWarning>::Element* before = nullptr;
    for (List<RebelScriptWarning>::Element* E = warnings.front(); E;
         E                                    = E->next()) {
...
String RebelScriptParser::get_error() const {
    return error;
}

int RebelScriptParser::get_error_line() const {
    return error_line;
}

int RebelScriptParser::get_error_column() const {
    return error_column;
}

bool RebelScriptParser::has_error() const {
    return error_set;
}

Error RebelScriptParser::_parse(const String& p_base_path) {
    ...
    bool warning_is_error =
        GLOBAL_GET("debug/rebelscript/warnings/treat_warnings_as_errors")
            .booleanize();
    for (List<RebelScriptWarning>::Element* E = warnings.front(); E;) {
        RebelScriptWarning& w = E->get();
        int skip_index        = -1;
        ...
        List<RebelScriptWarning>::Element* next = E->next();
        bool erase                              = false;
        if (skip_index != -1) {
            if (warning_skips[skip_index].second
                == RebelScriptWarning::get_name_from_code(w.code).to_lower()) {
                erase = true;
            }
...
Error RebelScriptParser::parse_bytecode(
    ...
    RebelScriptTokenizerBuffer* tb = memnew(RebelScriptTokenizerBuffer);
...
Error RebelScriptParser::parse(
    const String& p_code,
    const String& p_base_path,
    bool p_just_validate,
    const String& p_self_path,
    bool p_for_completion,
    Set<int>* r_safe_lines,
    bool p_dependencies_only
) {
    clear();

    self_path                    = p_self_path;
    RebelScriptTokenizerText* tt = memnew(RebelScriptTokenizerText);
    tt->set_code(p_code);
...
bool RebelScriptParser::is_tool_script() const {
    return (
        head && head->type == Node::TYPE_CLASS
        && static_cast<const ClassNode*>(head)->tool
    );
}

const RebelScriptParser::Node* RebelScriptParser::get_parse_tree() const {
    return head;
}

void RebelScriptParser::clear() {
    while (list) {
        Node* l = list;
        list    = list->next;
        memdelete(l);
...
RebelScriptParser::CompletionType RebelScriptParser::get_completion_type() {
    return completion_type;
}

StringName RebelScriptParser::get_completion_cursor() {
    return completion_cursor;
}

int RebelScriptParser::get_completion_line() {
    return completion_line;
}

Variant::Type RebelScriptParser::get_completion_built_in_constant() {
    return completion_built_in_constant;
}

RebelScriptParser::Node* RebelScriptParser::get_completion_node() {
    return completion_node;
}

RebelScriptParser::BlockNode* RebelScriptParser::get_completion_block() {
    return completion_block;
}

RebelScriptParser::ClassNode* RebelScriptParser::get_completion_class() {
    return completion_class;
}

RebelScriptParser::FunctionNode* RebelScriptParser::get_completion_function() {
    return completion_function;
}

int RebelScriptParser::get_completion_argument_index() {
    return completion_argument;
}

int RebelScriptParser::get_completion_identifier_is_function() {
    return completion_ident_is_call;
}

RebelScriptParser::RebelScriptParser() {
    head            = nullptr;
    list            = nullptr;
    tokenizer       = nullptr;
    pending_newline = -1;
