...
#include "rebelscript_functions.h"

const char* RebelScriptTokenizer::token_names[TK_MAX] = {
    "Empty",
    "Identifier",
    "Constant",
    "Self",
...
struct _kws {
    RebelScriptTokenizer::Token token;
    const char* text;
};

static const _kws _keyword_list[] = {
  // ops
    {RebelScriptTokenizer::TK_OP_IN,         "in"        },
    {RebelScriptTokenizer::TK_OP_NOT,        "not"       },
    {RebelScriptTokenizer::TK_OP_OR,         "or"        },
    {RebelScriptTokenizer::TK_OP_AND,        "and"       },
 // func
    {RebelScriptTokenizer::TK_PR_FUNCTION,   "func"      },
    {RebelScriptTokenizer::TK_PR_CLASS,      "class"     },
    {RebelScriptTokenizer::TK_PR_CLASS_NAME, "class_name"},
    {RebelScriptTokenizer::TK_PR_EXTENDS,    "extends"   },
    {RebelScriptTokenizer::TK_PR_IS,         "is"        },
    {RebelScriptTokenizer::TK_PR_ONREADY,    "onready"   },
    {RebelScriptTokenizer::TK_PR_TOOL,       "tool"      },
    {RebelScriptTokenizer::TK_PR_STATIC,     "static"    },
    {RebelScriptTokenizer::TK_PR_EXPORT,     "export"    },
    {RebelScriptTokenizer::TK_PR_SETGET,     "setget"    },
    {RebelScriptTokenizer::TK_PR_VAR,        "var"       },
    {RebelScriptTokenizer::TK_PR_AS,         "as"        },
    {RebelScriptTokenizer::TK_PR_VOID,       "void"      },
    {RebelScriptTokenizer::TK_PR_PRELOAD,    "preload"   },
    {RebelScriptTokenizer::TK_PR_ASSERT,     "assert"    },
    {RebelScriptTokenizer::TK_PR_YIELD,      "yield"     },
    {RebelScriptTokenizer::TK_PR_SIGNAL,     "signal"    },
    {RebelScriptTokenizer::TK_PR_BREAKPOINT, "breakpoint"},
    {RebelScriptTokenizer::TK_PR_REMOTE,     "remote"    },
    {RebelScriptTokenizer::TK_PR_MASTER,     "master"    },
    {RebelScriptTokenizer::TK_PR_SLAVE,      "slave"     },
    {RebelScriptTokenizer::TK_PR_PUPPET,     "puppet"    },
    {RebelScriptTokenizer::TK_PR_SYNC,       "sync"      },
    {RebelScriptTokenizer::TK_PR_REMOTESYNC, "remotesync"},
    {RebelScriptTokenizer::TK_PR_MASTERSYNC, "mastersync"},
    {RebelScriptTokenizer::TK_PR_PUPPETSYNC, "puppetsync"},
    {RebelScriptTokenizer::TK_PR_CONST,      "const"     },
    {RebelScriptTokenizer::TK_PR_ENUM,       "enum"      },
 // controlflow
    {RebelScriptTokenizer::TK_CF_IF,         "if"        },
    {RebelScriptTokenizer::TK_CF_ELIF,       "elif"      },
    {RebelScriptTokenizer::TK_CF_ELSE,       "else"      },
    {RebelScriptTokenizer::TK_CF_FOR,        "for"       },
    {RebelScriptTokenizer::TK_CF_WHILE,      "while"     },
    {RebelScriptTokenizer::TK_CF_BREAK,      "break"     },
    {RebelScriptTokenizer::TK_CF_CONTINUE,   "continue"  },
    {RebelScriptTokenizer::TK_CF_RETURN,     "return"    },
    {RebelScriptTokenizer::TK_CF_MATCH,      "match"     },
    {RebelScriptTokenizer::TK_CF_PASS,       "pass"      },
    {RebelScriptTokenizer::TK_SELF,          "self"      },
    {RebelScriptTokenizer::TK_CONST_PI,      "PI"        },
    {RebelScriptTokenizer::TK_CONST_TAU,     "TAU"       },
    {RebelScriptTokenizer::TK_WILDCARD,      "_"         },
    {RebelScriptTokenizer::TK_CONST_INF,     "INF"       },
    {RebelScriptTokenizer::TK_CONST_NAN,     "NAN"       },
    {RebelScriptTokenizer::TK_ERROR,         nullptr     }
};

const char* RebelScriptTokenizer::get_token_name(Token p_token) {
    ERR_FAIL_INDEX_V(p_token, TK_MAX, "<error>");
    return token_names[p_token];
}

bool RebelScriptTokenizer::is_token_literal(int p_offset, bool variable_safe)
    const {
...
StringName RebelScriptTokenizer::get_token_literal(int p_offset) const {
    ...
    switch (token) {
        ...
        case TK_BUILT_IN_FUNC:
            return RebelScriptFunctions::get_func_name(
                get_token_built_in_func(p_offset)
            );
        case TK_CONSTANT: {
...
void RebelScriptTokenizerText::_make_token(Token p_type) {
...
void RebelScriptTokenizerText::_make_identifier(const StringName& p_identifier
) {
    TokenData& tk = tk_rb[tk_rb_pos];
...
void RebelScriptTokenizerText::_make_built_in_func(
    RebelScriptFunctions::Function p_func
) {
    TokenData& tk = tk_rb[tk_rb_pos];
...
void RebelScriptTokenizerText::_make_constant(const Variant& p_constant) {
    TokenData& tk = tk_rb[tk_rb_pos];
...
void RebelScriptTokenizerText::_make_type(const Variant::Type& p_type) {
    TokenData& tk = tk_rb[tk_rb_pos];
...
void RebelScriptTokenizerText::_make_error(const String& p_error) {
    error_flag = true;
    last_error = p_error;
...
void RebelScriptTokenizerText::_make_newline(int p_indentation, int p_tabs) {
    TokenData& tk = tk_rb[tk_rb_pos];
    tk.type       = TK_NEWLINE;
...
void RebelScriptTokenizerText::_advance() {
    if (error_flag) {
        // parser broke
        ...
        switch (GETCHAR(0)) {
            ...
            case '#': { // line comment skip
                ...
                String comment_content =
                    comment.trim_prefix("#").trim_prefix(" ");
                if (comment_content.begins_with("warning-ignore:")) {
                    String code = comment_content.get_slice(":", 1);
                    warning_skips.push_back(
                        Pair<int, String>(line, code.strip_edges().to_lower())
                    );
            ...
            default: {
                if (_is_number(GETCHAR(0))
                    ...
                    while (true) {
                        if (GETCHAR(i) == '.') {
                            if (period_found || exponent_found) {
                                ...
                                _make_error(
                                    "Invalid hexadecimal constant at '.'"
                                );
                                return;
                            }
                            period_found = true;
                        } else if (GETCHAR(i) == 'x') {
                            if (hexa_found || bin_found || str.length() != 1
                                || !(
                                    (i == 1 && str[0] == '0')
                                    || (i == 2 && str[1] == '0' && str[0] == '-'
                                    )
                            ...
                            i++;
                            continue; // Included for readability, shouldn't be
                    ...
                    if (!(_is_number(str[str.length() - 1])
                          || (hexa_found && _is_hex(str[str.length() - 1])))) {
                ...
                if (_is_text_char(GETCHAR(0))) {
                    ...
                    if (str == "null") {
                        ...
                        if (!found) {
                            ...
                            for (int j = 0; j < RebelScriptFunctions::FUNC_MAX;
                                 j++) {
                                if (str
                                    == RebelScriptFunctions::get_func_name(
                                        RebelScriptFunctions::Function(j)
                                    )) {
                                    _make_built_in_func(
                                        RebelScriptFunctions::Function(j)
                                    );
...
void RebelScriptTokenizerText::set_code(const String& p_code) {
    code = p_code;
    len  = p_code.length();
    if (len) {
        _code = &code[0];
...
RebelScriptTokenizerText::Token RebelScriptTokenizerText::get_token(int p_offset
) const {
    ERR_FAIL_COND_V(p_offset <= -MAX_LOOKAHEAD, TK_ERROR);
    ERR_FAIL_COND_V(p_offset >= MAX_LOOKAHEAD, TK_ERROR);

    int ofs =
        (TK_RB_SIZE + tk_rb_pos + p_offset - MAX_LOOKAHEAD - 1) % TK_RB_SIZE;
    return tk_rb[ofs].type;
}

int RebelScriptTokenizerText::get_token_line(int p_offset) const {
...
int RebelScriptTokenizerText::get_token_column(int p_offset) const {
    ERR_FAIL_COND_V(p_offset <= -MAX_LOOKAHEAD, -1);
    ERR_FAIL_COND_V(p_offset >= MAX_LOOKAHEAD, -1);
...
const Variant& RebelScriptTokenizerText::get_token_constant(int p_offset
) const {
...
StringName RebelScriptTokenizerText::get_token_identifier(int p_offset) const {
...
RebelScriptFunctions::Function RebelScriptTokenizerText::
    get_token_built_in_func(int p_offset) const {
    ERR_FAIL_COND_V(p_offset <= -MAX_LOOKAHEAD, RebelScriptFunctions::FUNC_MAX);
    ERR_FAIL_COND_V(p_offset >= MAX_LOOKAHEAD, RebelScriptFunctions::FUNC_MAX);

    int ofs =
        (TK_RB_SIZE + tk_rb_pos + p_offset - MAX_LOOKAHEAD - 1) % TK_RB_SIZE;
    ERR_FAIL_COND_V(
        tk_rb[ofs].type != TK_BUILT_IN_FUNC,
        RebelScriptFunctions::FUNC_MAX
    );
    return tk_rb[ofs].func;
}

Variant::Type RebelScriptTokenizerText::get_token_type(int p_offset) const {
...
int RebelScriptTokenizerText::get_token_line_indent(int p_offset) const {
...
int RebelScriptTokenizerText::get_token_line_tab_indent(int p_offset) const {
    ERR_FAIL_COND_V(p_offset <= -MAX_LOOKAHEAD, 0);
    ERR_FAIL_COND_V(p_offset >= MAX_LOOKAHEAD, 0);
...
String RebelScriptTokenizerText::get_token_error(int p_offset) const {
...
void RebelScriptTokenizerText::advance(int p_amount) {
...
Error RebelScriptTokenizerBuffer::set_code_buffer(
    ...
    ERR_FAIL_COND_V_MSG(
        ...
        ERR_INVALID_DATA,
        "Bytecode is too recent! Please use a newer engine version."
    );
...
Vector<uint8_t> RebelScriptTokenizerBuffer::parse_code_string(
    const String& p_code
) {
    Vector<uint8_t> buf;
...
RebelScriptTokenizerBuffer::Token RebelScriptTokenizerBuffer::get_token(
    int p_offset
) const {
    ...
    return RebelScriptTokenizerBuffer::Token(tokens[offset] & TOKEN_MASK);
}

StringName RebelScriptTokenizerBuffer::get_token_identifier(int p_offset
) const {
    int offset = token + p_offset;
...
RebelScriptFunctions::Function RebelScriptTokenizerBuffer::
    get_token_built_in_func(int p_offset) const {
    int offset = token + p_offset;
    ERR_FAIL_INDEX_V(offset, tokens.size(), RebelScriptFunctions::FUNC_MAX);
    return RebelScriptFunctions::Function(tokens[offset] >> TOKEN_BITS);
}

Variant::Type RebelScriptTokenizerBuffer::get_token_type(int p_offset) const {
    int offset = token + p_offset;
...
int RebelScriptTokenizerBuffer::get_token_line(int p_offset) const {
    int offset = token + p_offset;
...
int RebelScriptTokenizerBuffer::get_token_column(int p_offset) const {
    int offset = token + p_offset;
    int pos    = lines.find_nearest(offset);
    if (pos < 0) {
        return -1;
...
int RebelScriptTokenizerBuffer::get_token_line_indent(int p_offset) const {
    int offset = token + p_offset;
    ERR_FAIL_INDEX_V(offset, tokens.size(), 0);
    return tokens[offset] >> TOKEN_BITS;
}

const Variant& RebelScriptTokenizerBuffer::get_token_constant(int p_offset
) const {
    int offset = token + p_offset;
...
String RebelScriptTokenizerBuffer::get_token_error(int p_offset) const {
    ERR_FAIL_V(String());
}

void RebelScriptTokenizerBuffer::advance(int p_amount) {
    ERR_FAIL_INDEX(p_amount + token, tokens.size());
    token += p_amount;
}

RebelScriptTokenizerBuffer::RebelScriptTokenizerBuffer() {
    token = 0;
}
