...
#include "rebelscript_highlighter.h"
...
Map<int, TextEdit::HighlighterInfo> RebelScriptSyntaxHighlighter::
    ...
    for (int j = 0; j < str.length(); j++) {
        ...
        if ((str[j] == '.' || str[j] == 'x' || str[j] == 'b' || str[j] == '_'
             || str[j] == 'e')
        ...
        if (is_symbol && cri_map.has(j)) {
            ...
            if (in_region == -1) {
                ...
                TextEdit::ColorRegion cr =
                    text_editor->_get_color_region(cri.region);
                if (in_region == cri.region
                    && !cr.line_only) { // ignore otherwise
                    if (cri.end || cr.eq) {
                        deregion =
                            cr.eq ? cr.begin_key.length() : cr.end_key.length();
                    }
                }
            }
        }
        ...
        if (!in_function_name && in_word && !in_keyword) {
            ...
            } else if (previous_text == RebelScriptTokenizer::get_token_name(RebelScriptTokenizer::TK_PR_VAR)) {
        ...
        if (next_type != current_type) {
            if (current_type == NONE) {
                ...
                if (previous_type == REGION) {
                    ...
                    String text =
                        str.substr(previous_column, j - previous_column)
                            .strip_edges();
                    previous_column = j;
...
String RebelScriptSyntaxHighlighter::get_name() const {
    return "RebelScript";
}

List<String> RebelScriptSyntaxHighlighter::get_supported_languages() {
    List<String> languages;
    languages.push_back("RebelScript");
    return languages;
}

void RebelScriptSyntaxHighlighter::_update_cache() {
    ...
    EDITOR_DEF(
        "text_editor/highlighting/rebelscript/function_definition_color",
        function_definition_color
    );
    EDITOR_DEF(
        "text_editor/highlighting/rebelscript/node_path_color",
        node_path_color
    );
    ...
    function_definition_color = EDITOR_GET(
        "text_editor/highlighting/rebelscript/function_definition_color"
    );
    node_path_color =
        EDITOR_GET("text_editor/highlighting/rebelscript/node_path_color");
    type_color = EDITOR_GET("text_editor/highlighting/base_type_color");
}

SyntaxHighlighter* RebelScriptSyntaxHighlighter::create() {
    return memnew(RebelScriptSyntaxHighlighter);
}
