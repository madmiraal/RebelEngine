...
#ifndef REBELSCRIPT_PARSER_H
#define REBELSCRIPT_PARSER_H
...
struct RebelScriptDataType;
struct RebelScriptWarning;

class RebelScriptParser {
public:
    struct ClassNode;
    ...
    struct BuiltInFunctionNode : public Node {
        RebelScriptFunctions::Function function;
    ...
    struct IdentifierNode : public Node {
        StringName name;
        BlockNode* declared_block; // Simplify lookup by checking if it is
                                   // declared locally
        DataType datatype;
    ...
    struct AssertNode : public Node {
        ...
        AssertNode() : condition(nullptr), message(nullptr) {
            type = TYPE_ASSERT;
        }
    };
...
private:
    RebelScriptTokenizer* tokenizer;
...
#ifdef DEBUG_ENABLED
    List<RebelScriptWarning> warnings;
#endif // DEBUG_ENABLED
    ...
    void _add_warning(
        ...
        const String& p_symbol4 = String()
    );
    void _add_warning(int p_code, int p_line, const Vector<String>& p_symbols);
#endif // DEBUG_ENABLED
    bool _recover_from_completion();

    bool _parse_arguments(
        Node* p_parent,
        Vector<Node*>& p_args,
        bool p_static,
        bool p_can_codecomplete = false,
    ...
    DataType _type_from_property(
        const PropertyInfo& p_property,
        bool p_nil_is_variant = true
    ) const;
    DataType _type_from_gdtype(const RebelScriptDataType& p_gdtype) const;
    DataType _get_operation_type(
        const Variant::Operator p_op,
        const DataType& p_a,
        const DataType& p_b,
        bool& r_valid
    ) const;
    Variant::Operator _get_variant_operation(const OperatorNode::Operator& p_op
    ) const;
    bool _get_function_signature(
    ...
    DataType _reduce_identifier_type(
        const DataType* p_base_type,
...
#ifdef DEBUG_ENABLED
    const List<RebelScriptWarning>& get_warnings() const {
        return warnings;
    }
#endif // DEBUG_ENABLED
    Error parse(
