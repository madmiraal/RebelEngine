...
#include "rebelscript_workspace.h"
...
void RebelScriptWorkspace::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("didDeleteFiles"),
        &RebelScriptWorkspace::did_delete_files
    );
    ClassDB::bind_method(D_METHOD("symbol"), &RebelScriptWorkspace::symbol);
    ClassDB::bind_method(
        D_METHOD("parse_script", "path", "content"),
        &RebelScriptWorkspace::parse_script
    );
    ClassDB::bind_method(
        D_METHOD("parse_local_script", "path"),
        &RebelScriptWorkspace::parse_local_script
    );
    ClassDB::bind_method(
        D_METHOD("get_file_path", "uri"),
        &RebelScriptWorkspace::get_file_path
    );
    ClassDB::bind_method(
        D_METHOD("get_file_uri", "path"),
        &RebelScriptWorkspace::get_file_uri
    );
    ClassDB::bind_method(
        D_METHOD("publish_diagnostics", "path"),
        &RebelScriptWorkspace::publish_diagnostics
    );
    ClassDB::bind_method(
        D_METHOD("generate_script_api", "path"),
        &RebelScriptWorkspace::generate_script_api
    );
    ClassDB::bind_method(
        D_METHOD("apply_new_signal", "obj", "function", "args"),
        &RebelScriptWorkspace::apply_new_signal
    );
}

void RebelScriptWorkspace::apply_new_signal(
    Object* obj,
    String function,
    ...
    if (script->get_language()->get_name() != "RebelScript") {
        return;
    }
    ...
    function_body += ")";
    if (EditorSettings::get_singleton()->get_setting(
    ...
    RebelScriptLanguageProtocol::get_singleton()->request_client(
        "workspace/applyEdit",
        params.to_json()
    );
}

void RebelScriptWorkspace::did_delete_files(const Dictionary& p_params) {
...
void RebelScriptWorkspace::remove_cache_parser(const String& p_path) {
    Map<String, ExtendRebelScriptParser*>::Element* parser =
        parse_results.find(p_path);
    Map<String, ExtendRebelScriptParser*>::Element* script =
        scripts.find(p_path);
    if (parser && script) {
...
const lsp::DocumentSymbol* RebelScriptWorkspace::get_native_symbol(
    ...
    while (class_name != empty) {
        if (const Map<StringName, lsp::DocumentSymbol>::Element* E =
                native_symbols.find(class_name)) {
...
const lsp::DocumentSymbol* RebelScriptWorkspace::get_script_symbol(
    const String& p_path
) const {
    const Map<String, ExtendRebelScriptParser*>::Element* S =
        scripts.find(p_path);
    if (S) {
...
const lsp::DocumentSymbol* RebelScriptWorkspace::get_parameter_symbol(
    ...
    for (int i = 0; i < p_parent->children.size(); ++i) {
        ...
        if (!parameter_symbol->detail.empty()
            && parameter_symbol->name == symbol_identifier) {
...
const lsp::DocumentSymbol* RebelScriptWorkspace::get_local_symbol(
    const ExtendRebelScriptParser* p_parser,
    const String& p_symbol_identifier
) {
    const lsp::DocumentSymbol* class_symbol = &p_parser->get_symbols();

    for (int i = 0; i < class_symbol->children.size(); ++i) {
        if (class_symbol->children[i].kind == lsp::SymbolKind::Function
            || class_symbol->children[i].kind == lsp::SymbolKind::Class) {
            const lsp::DocumentSymbol* function_symbol =
                &class_symbol->children[i];

            for (int l = 0; l < function_symbol->children.size(); ++l) {
                const lsp::DocumentSymbol* local =
                    &function_symbol->children[l];
                if (!local->detail.empty()
                    && local->name == p_symbol_identifier) {
                    return local;
                }
            }
        }
...
void RebelScriptWorkspace::reload_all_workspace_scripts() {
    List<String> paths;
    ...
    for (List<String>::Element* E = paths.front(); E; E = E->next()) {
        ...
        if (err != OK) {
            Map<String, ExtendRebelScriptParser*>::Element* S =
                parse_results.find(path);
...
void RebelScriptWorkspace::list_script_files(
    const String& p_root_dir,
    List<String>& r_files
) {
    Error err;
...
ExtendRebelScriptParser* RebelScriptWorkspace::get_parse_successed_script(
    const String& p_path
) {
    const Map<String, ExtendRebelScriptParser*>::Element* S =
        scripts.find(p_path);
...
ExtendRebelScriptParser* RebelScriptWorkspace::get_parse_result(
    const String& p_path
) {
    const Map<String, ExtendRebelScriptParser*>::Element* S =
        parse_results.find(p_path);
...
Array RebelScriptWorkspace::symbol(const Dictionary& p_params) {
    ...
    if (!query.empty()) {
        for (Map<String, ExtendRebelScriptParser*>::Element* E =
            ...
            E->get()->get_symbols().symbol_tree_as_list(
                E->key(),
                script_symbols
            );
...
Error RebelScriptWorkspace::initialize() {
    if (initialized) {
        return OK;
    }
    ...
    for (Map<String, DocsData::ClassDoc>::Element* E =
        ...
        for (int i = 0; i < class_data.theme_properties.size(); i++) {
            ...
            symbol.native_class = class_name;
            symbol.kind         = lsp::SymbolKind::Property;
            symbol.detail = "<Theme> var " + class_name + "." + data.name + ": "
                          + data.type;
            symbol.documentation = data.description;
            class_symbol.children.push_back(symbol);
        }
        ...
        for (int i = 0; i < methods_signals.size(); i++) {
            ...
            symbol.name         = data.name;
            symbol.native_class = class_name;
            symbol.kind         = i >= signal_start_idx ? lsp::SymbolKind::Event
            ...
            symbol.detail = "func " + class_name + "." + data.name + "("
                          + params + ") -> " + return_type;
            symbol.documentation = data.description;
            class_symbol.children.push_back(symbol);
        }
    ...
    if (RebelScriptLanguageProtocol::get_singleton()->is_smart_resolve_enabled(
        ...
        for (Map<StringName, lsp::DocumentSymbol>::Element* E =
                 native_symbols.front();
             E;
             E = E->next()) {
        ...
        for (Map<String, ExtendRebelScriptParser*>::Element* S =
                 scripts.front();
             S;
             S = S->next()) {
    ...
    editor_node
        ->connect("script_add_function_request", this, "apply_new_signal");

    return OK;
}

Error RebelScriptWorkspace::parse_script(
    const String& p_path,
    const String& p_content
) {
    ExtendRebelScriptParser* parser = memnew(ExtendRebelScriptParser);
    Error err                       = parser->parse(p_content, p_path);
    Map<String, ExtendRebelScriptParser*>::Element* last_parser =
        parse_results.find(p_path);
    Map<String, ExtendRebelScriptParser*>::Element* last_script =
        scripts.find(p_path);
    ...
    if (err == OK) {
        ...
        if (last_parser && last_script
            && last_parser->get() != last_script->get()) {
...
Dictionary RebelScriptWorkspace::rename(
    ...
    if (reference_symbol) {
        ...
        for (List<String>::Element* PE = paths.front(); PE; PE = PE->next()) {
            Vector<String> content =
                FileAccess::get_file_as_string(PE->get(), &err).split("\n");
            for (int i = 0; i < content.size(); ++i) {
                ...
                while (character > -1) {
                    ...
                    const lsp::DocumentSymbol* other_symbol =
...
Error RebelScriptWorkspace::parse_local_script(const String& p_path) {
    Error err;
...
String RebelScriptWorkspace::get_file_path(const String& p_uri) const {
    String path = p_uri;
...
String RebelScriptWorkspace::get_file_uri(const String& p_path) const {
    String uri = p_path;
    uri        = uri.replace("res://", root_uri + "/");
    return uri;
}

void RebelScriptWorkspace::publish_diagnostics(const String& p_path) {
    Dictionary params;
    Array errors;
    const Map<String, ExtendRebelScriptParser*>::Element* ele =
        parse_results.find(p_path);
    if (ele) {
...
void RebelScriptWorkspace::_get_owners(
...
Node* RebelScriptWorkspace::_get_owner_scene_node(String p_path) {
    ...
    for (int i = 0; i < owners.size(); i++) {
        ...
        if (Object::cast_to<PackedScene>(owner_res.ptr())) {
            Ref<PackedScene> owner_packed_scene =
                Ref<PackedScene>(Object::cast_to<PackedScene>(*owner_res));
...
void RebelScriptWorkspace::completion(
    ...
    if (const ExtendRebelScriptParser* parser = get_parse_result(path)) {
        ...
        if (owner_scene_node) {
            ...
            while (!stack.empty()) {
                ...
                Ref<RebelScript> script = current->get_script();
        ...
        RebelScriptLanguage::get_singleton()
            ->complete_code(code, path, current, r_options, forced, call_hint);
        if (owner_scene_node) {
...
const lsp::DocumentSymbol* RebelScriptWorkspace::resolve_symbol(
    const lsp::TextDocumentPositionParams& p_doc_pos,
    ...
    if (const ExtendRebelScriptParser* parser = get_parse_result(path)) {
        ...
        if (symbol_identifier.empty()) {
            Vector2i offset;
            symbol_identifier = parser->get_identifier_under_position(
                p_doc_pos.position,
                offset
            );
        ...
        if (!symbol_identifier.empty()) {
            if (ScriptServer::is_global_class(symbol_identifier)) {
                String class_path =
                    ScriptServer::get_global_class_path(symbol_identifier);
                symbol = get_script_symbol(class_path);

            } else {
                ScriptLanguage::LookupResult ret;
                if (symbol_identifier == "new"
                    && parser->get_lines()[p_doc_pos.position.line]
                               .replace(" ", "")
                               .replace("\t", "")
                               .find("new(")
                           > -1) {
                ...
                if (OK
                    == RebelScriptLanguage::get_singleton()->lookup_code(
                        parser->get_text_for_lookup_symbol(
                            pos,
                            symbol_identifier,
                            p_func_required
                        ),
                    ...
                    if (ret.type
                        ...
                        if (const ExtendRebelScriptParser* target_parser =
                                get_parse_result(target_script_path)) {
                            symbol = target_parser->get_symbol_defined_at_line(
                                LINE_NUMBER_TO_INDEX(ret.location)
                            );

                            if (symbol
                                && symbol->kind == lsp::SymbolKind::Function
                                && symbol->name != symbol_identifier) {
                        ...
                        if (member.empty()
                            && symbol_identifier != ret.class_name) {
...
void RebelScriptWorkspace::resolve_related_symbols(
    const lsp::TextDocumentPositionParams& p_doc_pos,
    ...
    if (const ExtendRebelScriptParser* parser = get_parse_result(path)) {
        ...
        Vector2i offset;
        symbol_identifier =
            parser->get_identifier_under_position(p_doc_pos.position, offset);

        const StringName* class_ptr = native_members.next(nullptr);
        while (class_ptr) {
            const ClassMembers& members = native_members.get(*class_ptr);
            if (const lsp::DocumentSymbol* const* symbol =
                    members.getptr(symbol_identifier)) {
        ...
        for (Map<String, ExtendRebelScriptParser*>::Element* E =
             ...
             E;
             E = E->next()) {
            const ExtendRebelScriptParser* script = E->get();
            ...
            const HashMap<String, ClassMembers>& inner_classes =
                script->get_inner_classes();
            const String* _class = inner_classes.next(nullptr);
            while (_class) {
                const ClassMembers* inner_class = inner_classes.getptr(*_class);
                if (const lsp::DocumentSymbol* const* symbol =
                        inner_class->getptr(symbol_identifier)) {
                    r_list.push_back(*symbol);
                }
...
const lsp::DocumentSymbol* RebelScriptWorkspace::resolve_native_symbol(
    const lsp::NativeSymbolInspectParams& p_params
) {
    if (Map<StringName, lsp::DocumentSymbol>::Element* E =
            native_symbols.find(p_params.native_class)) {
        const lsp::DocumentSymbol& symbol = E->get();
        if (p_params.symbol_name.empty()
            || p_params.symbol_name == symbol.name) {
            return &symbol;
        }
...
void RebelScriptWorkspace::resolve_document_links(
    const String& p_uri,
    List<lsp::DocumentLink>& r_list
) {
    if (const ExtendRebelScriptParser* parser =
            get_parse_successed_script(get_file_path(p_uri))) {
...
Dictionary RebelScriptWorkspace::generate_script_api(const String& p_path) {
    Dictionary api;
    if (const ExtendRebelScriptParser* parser =
            get_parse_successed_script(p_path)) {
        api = parser->generate_api();
    }
    return api;
}

Error RebelScriptWorkspace::resolve_signature(
    const lsp::TextDocumentPositionParams& p_doc_pos,
    lsp::SignatureHelp& r_signature
) {
    if (const ExtendRebelScriptParser* parser =
            get_parse_result(get_file_path(p_doc_pos.textDocument.uri))) {
        lsp::TextDocumentPositionParams text_pos;
        text_pos.textDocument = p_doc_pos.textDocument;

        if (parser->get_left_function_call(
                p_doc_pos.position,
                text_pos.position,
                r_signature.activeParameter
            )
            == OK) {
            ...
            if (const lsp::DocumentSymbol* symbol = resolve_symbol(text_pos)) {
                ...
                RebelScriptLanguageProtocol::get_singleton()
                    ->get_workspace()
                    ->resolve_related_symbols(text_pos, symbols);
            }

            for (List<const lsp::DocumentSymbol*>::Element* E = symbols.front();
                 E;
                 E = E->next()) {
                const lsp::DocumentSymbol* symbol = E->get();
                if (symbol->kind == lsp::SymbolKind::Method
                    || symbol->kind == lsp::SymbolKind::Function) {
...
RebelScriptWorkspace::RebelScriptWorkspace() {
...
RebelScriptWorkspace::~RebelScriptWorkspace() {
    ...
    for (Map<String, ExtendRebelScriptParser*>::Element* E = scripts.front(); E;
         E                                                 = E->next()) {
        cached_parsers.insert(E->key());
    }
