...
void ExtendRebelScriptParser::update_diagnostics() {
    diagnostics.clear();
    ...
    const List<RebelScriptWarning>& warnings = get_warnings();
    for (const List<RebelScriptWarning>::Element* E = warnings.front(); E;
         E                                          = E->next()) {
...
void ExtendRebelScriptParser::update_symbols() {
    members.clear();

    const RebelScriptParser::Node* head = get_parse_tree();
    if (const RebelScriptParser::ClassNode* gdclass =
            dynamic_cast<const RebelScriptParser::ClassNode*>(head)) {
...
void ExtendRebelScriptParser::update_document_links(const String& p_code) {
    ...
    while (true) {
        RebelScriptTokenizerText::Token token = tokenizer.get_token();
        if (token == RebelScriptTokenizer::TK_EOF
            || token == RebelScriptTokenizer::TK_ERROR) {
            break;
        } else if (token == RebelScriptTokenizer::TK_CONSTANT) {
            ...
            if (const_val.get_type() == Variant::STRING) {
                ...
                if (exists) {
                    ...
                    link.target = RebelScriptLanguageProtocol::get_singleton()
                                      ->get_workspace()
                                      ->get_file_uri(path);
                    link.range.start.line =
                        LINE_NUMBER_TO_INDEX(tokenizer.get_token_line());
                    link.range.end.line = link.range.start.line;
                    link.range.end.character =
                        LINE_NUMBER_TO_INDEX(tokenizer.get_token_column());
                    link.range.start.character =
                        link.range.end.character - value.length();
...
void ExtendRebelScriptParser::parse_class_symbol(
    ...
    r_symbol.documentation             = parse_documentation(
        is_root_class ? 0 : LINE_NUMBER_TO_INDEX(p_class->line),
        is_root_class
    );
    ...
    for (int i = 0; i < p_class->variables.size(); ++i) {
        ...
        symbol.range.start.character =
            lines[line].length()
            - lines[line].strip_edges(true, false).length();
        ...
        if (m.data_type.kind != RebelScriptParser::DataType::UNRESOLVED) {
    ...
    for (int i = 0; i < p_class->_signals.size(); ++i) {
        const RebelScriptParser::ClassNode::Signal& signal =
            p_class->_signals[i];
    ...
    for (Map<StringName, RebelScriptParser::ClassNode::Constant>::Element* E =
         ...
         E = E->next()) {
        lsp::DocumentSymbol symbol;
        const RebelScriptParser::ClassNode::Constant& c = E->value();
        const RebelScriptParser::ConstantNode* node =
            dynamic_cast<const RebelScriptParser::ConstantNode*>(c.expression);
        ERR_FAIL_COND(!node);
        symbol.name       = E->key();
        ...
        if (c.type.kind != RebelScriptParser::DataType::UNRESOLVED) {
        ...
        if (node->value.get_type() == Variant::OBJECT) {
            ...
            if (res.is_valid() && !res->get_path().empty()) {
                ...
                if (symbol.documentation.empty()) {
                    if (Map<String, ExtendRebelScriptParser*>::Element* S =
                            RebelScriptLanguageProtocol::get_singleton()
                                ->get_workspace()
                                ->scripts.find(res->get_path())) {
                        symbol.documentation =
                            S->get()->class_symbol.documentation;
                    }
                }
            } else {
    ...
    for (int i = 0; i < p_class->functions.size(); ++i) {
        const RebelScriptParser::FunctionNode* func = p_class->functions[i];
        lsp::DocumentSymbol symbol;
    ...
    for (int i = 0; i < p_class->static_functions.size(); ++i) {
        const RebelScriptParser::FunctionNode* func =
            p_class->static_functions[i];
        lsp::DocumentSymbol symbol;
        parse_function_symbol(func, symbol);
        r_symbol.children.push_back(symbol);
    }
...
void ExtendRebelScriptParser::parse_function_symbol(
    ...
    r_symbol.kind =
        p_func->_static ? lsp::SymbolKind::Function : lsp::SymbolKind::Method;
    ...
    r_symbol.range.end.line =
        MAX(p_func->body->end_line - 2, r_symbol.range.start.line);
    ...
    for (int i = 0; i < p_func->arguments.size(); i++) {
        ...
        if (p_func->argument_types[i].kind
            != RebelScriptParser::DataType::UNRESOLVED) {
        ...
        if (default_value_idx >= 0) {
            const RebelScriptParser::ConstantNode* const_node =
                dynamic_cast<const RebelScriptParser::ConstantNode*>(
                    p_func->default_values[default_value_idx]
                );
            if (const_node == nullptr) {
                const RebelScriptParser::OperatorNode* operator_node =
                    dynamic_cast<const RebelScriptParser::OperatorNode*>(
                        p_func->default_values[default_value_idx]
                    );
                if (operator_node) {
                    const_node =
    ...
    if (p_func->return_type.kind != RebelScriptParser::DataType::UNRESOLVED) {
        r_symbol.detail += " -> " + p_func->return_type.to_string();
    }

    List<RebelScriptParser::BlockNode*> function_blocks;
    List<RebelScriptParser::BlockNode*> block_stack;
    block_stack.push_back(p_func->body);

    while (!block_stack.empty()) {
        RebelScriptParser::BlockNode* block = block_stack[0];
        block_stack.pop_front();

        function_blocks.push_back(block);
        for (const List<RebelScriptParser::BlockNode*>::Element* E =
                 block->sub_blocks.front();
             E;
             E = E->next()) {
    ...
    for (const List<RebelScriptParser::BlockNode*>::Element* B =
             function_blocks.front();
         B;
         B = B->next()) {
        for (const Map<StringName, LocalVarNode*>::Element* E =
                 B->get()->variables.front();
             E;
             E = E->next()) {
            lsp::DocumentSymbol symbol;
            const RebelScriptParser::LocalVarNode* var = E->value();
            symbol.name                                = E->key();
            symbol.kind                  = lsp::SymbolKind::Variable;
            ...
            if (var->datatype.kind != RebelScriptParser::DataType::UNRESOLVED) {
...
String ExtendRebelScriptParser::parse_documentation(
    int p_line,
    bool p_docs_down
) {
    ...
    if (!p_docs_down) { // inline comment
        ...
        if (comment_start != -1) {
            inline_comment =
                inline_comment.substr(comment_start, inline_comment.length())
                    .strip_edges();
            if (inline_comment.length() > 1) {
                doc_lines.push_back(
                    inline_comment.substr(1, inline_comment.length())
                );
            }
        }
    }
...
String ExtendRebelScriptParser::get_text_for_completion(
...
String ExtendRebelScriptParser::get_text_for_lookup_symbol(
    const lsp::Position& p_cursor,
    ...
    for (int i = 0; i < len; i++) {
        if (i == p_cursor.line) {
            ...
            String last_part =
                line.substr(p_cursor.character + 1, lines[i].length());
            if (!p_symbol.empty()) {
...
String ExtendRebelScriptParser::get_identifier_under_position(
    ...
    for (int c = p_position.character; c >= 0; c--) {
        start_pos       = c;
        CharType ch     = line[c];
        bool valid_char = (ch >= '0' && ch <= '9') || (ch >= 'a' && ch <= 'z')
                       || (ch >= 'A' && ch <= 'Z') || ch == '_';
        if (!valid_char) {
            break;
        }
...
String ExtendRebelScriptParser::get_uri() const {
    return RebelScriptLanguageProtocol::get_singleton()
        ->get_workspace()
        ->get_file_uri(path);
}

const lsp::DocumentSymbol* ExtendRebelScriptParser::
    search_symbol_defined_at_line(
        int p_line,
...
Error ExtendRebelScriptParser::get_left_function_call(
    const lsp::Position& p_position,
    lsp::Position& r_func_pos,
    int& r_arg_index
) const {
...
const lsp::DocumentSymbol* ExtendRebelScriptParser::get_symbol_defined_at_line(
    int p_line
) const {
    if (p_line <= 0) {
        return &class_symbol;
...
const lsp::DocumentSymbol* ExtendRebelScriptParser::get_member_symbol(
    const String& p_name,
    const String& p_subclass
) const {
    if (p_subclass.empty()) {
...
const List<lsp::DocumentLink>& ExtendRebelScriptParser::get_document_links(
) const {
    return document_links;
}

const Array& ExtendRebelScriptParser::get_member_completions() {
        ...
        while (_class) {
            ...
            while (member_name) {
                const lsp::DocumentSymbol* symbol =
                    inner_class->get(*member_name);
                lsp::CompletionItem item = symbol->make_completion_item();
                item.data =
                    JOIN_SYMBOLS(path, JOIN_SYMBOLS(*_class, *member_name));
                member_completions.push_back(item.to_json());
...
Dictionary ExtendRebelScriptParser::dump_function_api(
    const RebelScriptParser::FunctionNode* p_func
) const {
    Dictionary func;
    ...
    if (const lsp::DocumentSymbol* symbol =
            get_symbol_defined_at_line(LINE_NUMBER_TO_INDEX(p_func->line))) {
...
Dictionary ExtendRebelScriptParser::dump_class_api(
    const RebelScriptParser::ClassNode* p_class
) const {
    ...
    for (Map<StringName, RebelScriptParser::ClassNode::Constant>::Element* E =
        ...
        if (const lsp::DocumentSymbol* symbol =
                get_symbol_defined_at_line(LINE_NUMBER_TO_INDEX(node->line))) {
    ...
    for (int i = 0; i < p_class->variables.size(); ++i) {
        ...
        if (const lsp::DocumentSymbol* symbol =
                get_symbol_defined_at_line(LINE_NUMBER_TO_INDEX(m.line))) {
            api["signature"]   = symbol->detail;
            api["description"] = symbol->documentation;
        }
    ...
    for (int i = 0; i < p_class->_signals.size(); ++i) {
        ...
        api["arguments"] = args;
        if (const lsp::DocumentSymbol* symbol =
                get_symbol_defined_at_line(LINE_NUMBER_TO_INDEX(signal.line))) {
            api["signature"]   = symbol->detail;
            api["description"] = symbol->documentation;
        }
...
Dictionary ExtendRebelScriptParser::generate_api() const {
    Dictionary api;
    const RebelScriptParser::Node* head = get_parse_tree();
    if (const RebelScriptParser::ClassNode* gdclass =
            dynamic_cast<const RebelScriptParser::ClassNode*>(head)) {
...
Error ExtendRebelScriptParser::parse(
    const String& p_code,
    const String& p_path
) {
    path  = p_path;
    ...
    Error err = RebelScriptParser::parse(
        p_code,
        p_path.get_base_dir(),
        false,
        p_path,
