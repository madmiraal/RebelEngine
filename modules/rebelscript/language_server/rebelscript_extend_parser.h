...
#ifndef REBELSCRIPT_EXTEND_PARSER_H
#define REBELSCRIPT_EXTEND_PARSER_H
...
class ExtendRebelScriptParser : public RebelScriptParser {
    String path;
    Vector<String> lines;
    ...
    void parse_class_symbol(
        const RebelScriptParser::ClassNode* p_class,
        lsp::DocumentSymbol& r_symbol
    );
    void parse_function_symbol(
        const RebelScriptParser::FunctionNode* p_func,
        lsp::DocumentSymbol& r_symbol
    );

    Dictionary dump_function_api(const RebelScriptParser::FunctionNode* p_func
    ) const;
    Dictionary dump_class_api(const RebelScriptParser::ClassNode* p_class
    ) const;

    String parse_documentation(int p_line, bool p_docs_down = false);
    const lsp::DocumentSymbol* search_symbol_defined_at_line(
        int p_line,
...
public:
    ...
    _FORCE_INLINE_ const HashMap<String, ClassMembers>& get_inner_classes(
    ) const {
        return inner_classes;
    }

    Error get_left_function_call(
