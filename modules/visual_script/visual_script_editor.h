...
class VisualScriptEditor : public ScriptEditorBase {
    ...
    int _create_new_node_from_name(
        const String& p_text,
        const Vector2& p_point,
        const StringName& p_func = StringName()
    );
    void _selected_new_virtual_method(
        const String& p_text,
        const String& p_category,
    ...
    void _graph_connected(
        const String& p_from,
        int p_from_slot,
        const String& p_to,
        int p_to_slot
    );
    void _graph_disconnected(
        const String& p_from,
        int p_from_slot,
        const String& p_to,
        int p_to_slot
    );
    void _graph_connect_to_empty(
        const String& p_from,
        int p_from_slot,
    ...
    void _button_resource_previewed(
        const String& p_path,
    ...
    VisualScriptNode::TypeGuess _guess_output_type(
        int p_port_action_node,
        int p_port_action_output,
        Set<int>& p_visited_nodes
    );
