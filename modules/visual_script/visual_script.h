...
public:
    ...
    enum {
        ...
        FLOW_STACK_PUSHED_BIT = 1
                             << 30, // in flow stack, means bit was pushed (must
                                    // go back here if end of sequence)
    ...
    virtual int step(
        ...
        String& r_error_str
    ) = 0; // do a step, return which sequence port to go out
...
public:
    ...
    void sequence_connect(
        const StringName& p_func,
        int p_from_node,
        int p_from_output,
        int p_to_node
    );
    void sequence_disconnect(
        const StringName& p_func,
        int p_from_node,
        int p_from_output,
        int p_to_node
    );
    bool has_sequence_connection(
        const StringName& p_func,
        int p_from_node,
        int p_from_output,
        int p_to_node
    ) const;
    void get_sequence_connection_list(
        const StringName& p_func,
    ...
    void get_data_connection_list(
        const StringName& p_func,
        List<DataConnection>* r_connection
    ) const;
    bool is_input_value_port_connected(
        const StringName& p_func,
        int p_node,
        int p_port
    ) const;
    bool get_input_value_port_connection_source(
        const StringName& p_func,
        int p_node,
        int p_port,
        int* r_node,
    ...
    void custom_signal_add_argument(
        const StringName& p_func,
        Variant::Type p_type,
        const String& p_name,
        int p_index = -1
    ...
    Variant::Type custom_signal_get_argument_type(
        const StringName& p_func,
        int p_argidx
    ) const;
    ...
    String custom_signal_get_argument_name(
        const StringName& p_func,
        int p_argidx
    ) const;
    ...
    void custom_signal_swap_argument(
        const StringName& p_func,
        int p_argidx,
        int p_with_argidx
    );
    void remove_custom_signal(const StringName& p_name);
    void rename_custom_signal(
        const StringName& p_name,
        const StringName& p_new_name
    );
    Set<int> get_output_sequence_ports_connected(
...
#ifdef TOOLS_ENABLED
    bool are_subnodes_edited() const;
#endif
