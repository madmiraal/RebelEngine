...
void VisualScriptNode::set_default_input_value(
    int p_port,
...
void VisualScriptNode::validate_input_default_values() {
    default_input_values.resize(
        MAX(default_input_values.size(), get_input_value_port_count())
    ...
    for (int i = 0; i < get_input_value_port_count(); i++) {
        ...
        if (expected == Variant::NIL
            ...
            if (ce.error != Variant::CallError::CALL_OK) {
                ...
                default_input_values[i] =
                    Variant::construct(expected, nullptr, 0, ce, false);
            }
        }
    }
}
...
void VisualScriptNode::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("get_visual_script"),
        &VisualScriptNode::get_visual_script
    );
    ClassDB::bind_method(
        D_METHOD("set_default_input_value", "port_idx", "value"),
        &VisualScriptNode::set_default_input_value
    );
    ClassDB::bind_method(
        D_METHOD("get_default_input_value", "port_idx"),
        &VisualScriptNode::get_default_input_value
    );
    ClassDB::bind_method(
        D_METHOD("ports_changed_notify"),
        &VisualScriptNode::ports_changed_notify
    );
    ClassDB::bind_method(
        D_METHOD("_set_default_input_values", "values"),
        &VisualScriptNode::_set_default_input_values
    );
    ClassDB::bind_method(
        D_METHOD("_get_default_input_values"),
...
void VisualScript::remove_function(const StringName& p_name) {
    ...
    for (Map<int, Function::NodeData>::Element* E =
             functions[p_name].nodes.front();
         E;
         E = E->next()) {
...
void VisualScript::rename_function(
...
void VisualScript::set_function_scroll(
    const StringName& p_name,
    const Vector2& p_scroll
) {
    ERR_FAIL_COND(!functions.has(p_name));
...
void VisualScript::_node_ports_changed(int p_id) {
...

    {
        ...
        for (Set<SequenceConnection>::Element* E =
                 func.sequence_connections.front();
             E;
             E = E->next()) {
...

    {
        ...
        for (Set<DataConnection>::Element* E = func.data_connections.front(); E;
            ...
            if (E->get().to_node == p_id
                && E->get().to_port >= vsn->get_input_value_port_count()) {
                to_remove.push_back(E->get());
            }
        }
...
void VisualScript::add_node(
    ...
    if (Object::cast_to<VisualScriptFunction>(*p_node)) {
        ...
        ERR_FAIL_COND_MSG(
            func.function_id >= 0,
            "A function node has already been set here."
        );
...
void VisualScript::remove_node(const StringName& p_func, int p_id) {
    ...
    func.nodes[p_id]
        .node->disconnect("ports_changed", this, "_node_ports_changed");
...
Ref<VisualScriptNode> VisualScript::get_node(const StringName& p_func, int p_id)
    const {
...
void VisualScript::set_node_position(
    const StringName& p_func,
    int p_id,
    const Point2& p_pos
) {
...
Point2 VisualScript::get_node_position(const StringName& p_func, int p_id)
    const {
...
void VisualScript::get_node_list(const StringName& p_func, List<int>* r_nodes)
    const {
    ERR_FAIL_COND(!functions.has(p_func));
    const Function& func = functions[p_func];
...
void VisualScript::sequence_connect(
    const StringName& p_func,
    int p_from_node,
    int p_from_output,
    int p_to_node
...
void VisualScript::sequence_disconnect(
    const StringName& p_func,
    int p_from_node,
    int p_from_output,
    int p_to_node
...
bool VisualScript::has_sequence_connection(
    const StringName& p_func,
    int p_from_node,
    int p_from_output,
    int p_to_node
...
void VisualScript::get_sequence_connection_list(
    const StringName& p_func,
    List<SequenceConnection>* r_connection
) const {
    ERR_FAIL_COND(!functions.has(p_func));
    const Function& func = functions[p_func];

    for (const Set<SequenceConnection>::Element* E =
             func.sequence_connections.front();
         E;
         E = E->next()) {
...
void VisualScript::data_connect(
    const StringName& p_func,
    int p_from_node,
    int p_from_port,
    int p_to_node,
...
void VisualScript::data_disconnect(
    const StringName& p_func,
    int p_from_node,
    int p_from_port,
    int p_to_node,
...
bool VisualScript::has_data_connection(
    const StringName& p_func,
    int p_from_node,
    int p_from_port,
    int p_to_node,
...
bool VisualScript::is_input_value_port_connected(
    const StringName& p_func,
    int p_node,
    int p_port
) const {
    ...
    for (const Set<DataConnection>::Element* E = func.data_connections.front();
         E;
         E = E->next()) {
...
bool VisualScript::get_input_value_port_connection_source(
    const StringName& p_func,
    int p_node,
    int p_port,
    int* r_node,
...
void VisualScript::get_data_connection_list(
    const StringName& p_func,
    List<DataConnection>* r_connection
) const {
    ERR_FAIL_COND(!functions.has(p_func));
    const Function& func = functions[p_func];
...
void VisualScript::add_variable(
    const StringName& p_name,
...
void VisualScript::set_variable_default_value(
    const StringName& p_name,
    const Variant& p_value
) {
    ERR_FAIL_COND(!variables.has(p_name));
...
Variant VisualScript::get_variable_default_value(const StringName& p_name
) const {
    ERR_FAIL_COND_V(!variables.has(p_name), Variant());
    return variables[p_name].default_value;
}

void VisualScript::set_variable_info(
    const StringName& p_name,
...
void VisualScript::set_variable_export(
    const StringName& p_name,
    bool p_export
) {
    ERR_FAIL_COND(!variables.has(p_name));
...
void VisualScript::_set_variable_info(
    const StringName& p_name,
...
void VisualScript::rename_variable(
    const StringName& p_name,
    const StringName& p_new_name
) {
    ERR_FAIL_COND(instances.size());
...
void VisualScript::custom_signal_add_argument(
    const StringName& p_func,
    Variant::Type p_type,
    const String& p_name,
    int p_index
...
Variant::Type VisualScript::custom_signal_get_argument_type(
    const StringName& p_func,
    int p_argidx
) const {
...
void VisualScript::custom_signal_set_argument_name(
    const StringName& p_func,
    int p_argidx,
    const String& p_name
) {
...
String VisualScript::custom_signal_get_argument_name(
    const StringName& p_func,
    int p_argidx
) const {
...
void VisualScript::custom_signal_remove_argument(
    const StringName& p_func,
    int p_argidx
) {
    ERR_FAIL_COND(instances.size());
...
int VisualScript::custom_signal_get_argument_count(const StringName& p_func
) const {
    ERR_FAIL_COND_V(!custom_signals.has(p_func), 0);
    return custom_signals[p_func].size();
}

void VisualScript::custom_signal_swap_argument(
    const StringName& p_func,
    int p_argidx,
    int p_with_argidx
) {
    ...
    SWAP(
        custom_signals[p_func].write[p_argidx],
...
void VisualScript::rename_custom_signal(
    const StringName& p_name,
    const StringName& p_new_name
) {
    ERR_FAIL_COND(instances.size());
...
void VisualScript::get_custom_signal_list(List<StringName>* r_custom_signals
) const {
    for (const Map<StringName, Vector<Argument>>::Element* E =
             custom_signals.front();
         E;
         E = E->next()) {
...
bool VisualScript::get_property_default_value(
    ...
    for (Map<StringName, Function>::Element* E = functions.front(); E;
        ...
        if (E->get().function_id >= 0) {
            Ref<VisualScriptFunction> func =
                E->get().nodes[E->get().function_id].node;
            if (func.is_valid()) {
...
int VisualScript::get_member_line(const StringName& p_member) const {
    ...
    if (has_function(p_member)) {
        for (Map<int, Function::NodeData>::Element* E =
                 functions[p_member].nodes.front();
             E;
             E = E->next()) {
...
void VisualScript::_set_data(const Dictionary& p_data) {
    ...
    Vector2 last_pos = Vector2(
        -100 * funcs.size(),
        -100 * funcs.size()
    ); // this is the center of the last fn box
    ...
    for (int i = 0; i < funcs.size(); i++) {
        ...
        if (!d.has("vs_unify") && nodes.size() > 0) {
            ...
            Vector2 offset = last_pos + (last_size / 2.0)
            ...
            for (int j = 0; j < nodes.size(); j += 3) {
                add_node(
                    ...
                    nodes[j + 2],
                    offset + nodes[j + 1]
                ); // also add an additional buffer if you want to
            }
        ...
        for (int j = 0; j < sequence_connections.size(); j += 3) {
            sequence_connect(
                name,
                sequence_connections[j + 0],
                sequence_connections[j + 1],
                sequence_connections[j + 2]
            );
        }
        ...
        for (int j = 0; j < data_connections.size(); j += 4) {
            data_connect(
                name,
                data_connections[j + 0],
                data_connections[j + 1],
                data_connections[j + 2],
                data_connections[j + 3]
            );
        }
    }
...
Dictionary VisualScript::_get_data() const {
    ...
    for (const Map<StringName, Function>::Element* E = functions.front(); E;
        ...
        for (const Set<SequenceConnection>::Element* F =
                 E->get().sequence_connections.front();
             F;
             F = F->next()) {
        ...
        for (const Set<DataConnection>::Element* F =
                 E->get().data_connections.front();
             F;
             F = F->next()) {
...
void VisualScript::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("_node_ports_changed"),
        &VisualScript::_node_ports_changed
    );

    ClassDB::bind_method(
        D_METHOD("add_function", "name"),
        &VisualScript::add_function
    );
    ClassDB::bind_method(
        D_METHOD("has_function", "name"),
        &VisualScript::has_function
    );
    ClassDB::bind_method(
        D_METHOD("remove_function", "name"),
        &VisualScript::remove_function
    );
    ClassDB::bind_method(
        D_METHOD("rename_function", "name", "new_name"),
        &VisualScript::rename_function
    );
    ClassDB::bind_method(
        D_METHOD("set_function_scroll", "name", "ofs"),
        &VisualScript::set_function_scroll
    );
    ClassDB::bind_method(
        D_METHOD("get_function_scroll", "name"),
        &VisualScript::get_function_scroll
    );

    ClassDB::bind_method(
        D_METHOD("add_node", "func", "id", "node", "position"),
        &VisualScript::add_node,
        DEFVAL(Point2())
    );
    ClassDB::bind_method(
        D_METHOD("remove_node", "func", "id"),
        &VisualScript::remove_node
    );
    ClassDB::bind_method(
        D_METHOD("get_function_node_id", "name"),
        &VisualScript::get_function_node_id
    );

    ClassDB::bind_method(
        D_METHOD("get_node", "func", "id"),
        &VisualScript::get_node
    );
    ClassDB::bind_method(
        D_METHOD("has_node", "func", "id"),
        &VisualScript::has_node
    );
    ClassDB::bind_method(
        D_METHOD("set_node_position", "func", "id", "position"),
        &VisualScript::set_node_position
    );
    ClassDB::bind_method(
        D_METHOD("get_node_position", "func", "id"),
    ...
    ClassDB::bind_method(
        D_METHOD("has_variable", "name"),
        &VisualScript::has_variable
    );
    ClassDB::bind_method(
        D_METHOD("remove_variable", "name"),
        &VisualScript::remove_variable
    );
    ClassDB::bind_method(
        D_METHOD("set_variable_default_value", "name", "value"),
        &VisualScript::set_variable_default_value
    );
    ClassDB::bind_method(
        D_METHOD("get_variable_default_value", "name"),
        &VisualScript::get_variable_default_value
    );
    ClassDB::bind_method(
        D_METHOD("set_variable_info", "name", "value"),
        &VisualScript::_set_variable_info
    );
    ClassDB::bind_method(
        D_METHOD("get_variable_info", "name"),
        &VisualScript::_get_variable_info
    );
    ClassDB::bind_method(
        D_METHOD("set_variable_export", "name", "enable"),
        &VisualScript::set_variable_export
    );
    ClassDB::bind_method(
        D_METHOD("get_variable_export", "name"),
        &VisualScript::get_variable_export
    );
    ClassDB::bind_method(
        D_METHOD("rename_variable", "name", "new_name"),
        &VisualScript::rename_variable
    );

    ClassDB::bind_method(
        D_METHOD("add_custom_signal", "name"),
        &VisualScript::add_custom_signal
    );
    ClassDB::bind_method(
        D_METHOD("has_custom_signal", "name"),
    ...
    ClassDB::bind_method(
        D_METHOD("custom_signal_get_argument_count", "name"),
    ...
    ClassDB::bind_method(
        D_METHOD("remove_custom_signal", "name"),
        &VisualScript::remove_custom_signal
    );
    ClassDB::bind_method(
        D_METHOD("rename_custom_signal", "name", "new_name"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_instance_base_type", "type"),
        &VisualScript::set_instance_base_type
    );

    ClassDB::bind_method(
        D_METHOD("_set_data", "data"),
        &VisualScript::_set_data
    );
...
bool VisualScript::inherits_script(const Ref<Script>& p_script) const {
    return this
        == p_script.ptr(
        ); // there is no inheritance in visual scripts, so this is enough
}
...
Set<int> VisualScript::get_output_sequence_ports_connected(
    const String& edited_func,
    int from_node
) {
...
void VisualScriptInstance::get_property_list(List<PropertyInfo>* p_properties
) const {
    for (const Map<StringName, VisualScript::Variable>::Element* E =
             script->variables.front();
         E;
         E = E->next()) {
        if (!E->get()._export) {
            continue;
...
Variant::Type VisualScriptInstance::get_property_type(
    const StringName& p_name,
    bool* r_is_valid
) const {
    const Map<StringName, VisualScript::Variable>::Element* E =
        script->variables.find(p_name);
    if (!E) {
        if (r_is_valid) {
            *r_is_valid = false;
        }
...
void VisualScriptInstance::get_method_list(List<MethodInfo>* p_list) const {
    for (const Map<StringName, VisualScript::Function>::Element* E =
             script->functions.front();
         E;
         E = E->next()) {
        ...
        mi.name = E->key();
        if (E->get().function_id >= 0
            && E->get().nodes.has(E->get().function_id)) {
            Ref<VisualScriptFunction> vsf =
                E->get().nodes[E->get().function_id].node;
            if (vsf.is_valid()) {
...
void VisualScriptInstance::_dependency_step(
    ...
    node->step(
        input_args,
        output_args,
        VisualScriptNodeInstance::START_MODE_BEGIN_SEQUENCE,
        working_mem,
        r_error,
        error_str
    );
...
Variant VisualScriptInstance::_call_internal(
    const StringName& p_method,
    void* p_stack,
    int p_stack_size,
    VisualScriptNodeInstance* p_node,
    int p_flow_stack_pos,
    int p_pass,
    ...
    while (true) {
        ...
        if (current_node_id == f->node) {
            // if function node, set up function arguments from beginning of
            // stack
            ...
            if (!error) {
                ...
                for (int i = 0; i < node->input_port_count; i++) {
                    int index = node->input_ports[i]
                              & VisualScriptNodeInstance::INPUT_MASK;
        ...
        for (int i = 0; i < node->output_port_count; i++) {
            output_args[i] = &variant_stack[node->output_ports[i]];
            VSDEBUG(
                "PORT " + itos(i) + " AT STACK " + itos(node->output_ports[i])
            );
        }
        ...
        if (ret & VisualScriptNodeInstance::STEP_YIELD_BIT) {
            ...
            if (node->get_working_memory_size() == 0) {
                ...
                error_str =
                    RTR("A node yielded without working memory, please read "
                ...
                if (!state.is_valid()) {
                    ...
                    error_str =
                        RTR("Node yielded, but did not return a function state "
        ...
        if (ScriptDebugger::get_singleton()) {
            ...
            if (ScriptDebugger::get_singleton()->get_lines_left() > 0) {
                if (ScriptDebugger::get_singleton()->get_depth() <= 0) {
                    ScriptDebugger::get_singleton()->set_lines_left(
                        ScriptDebugger::get_singleton()->get_lines_left() - 1
                    );
                }
                if (ScriptDebugger::get_singleton()->get_lines_left() <= 0) {
                    do_break = true;
            ...
            if (ScriptDebugger::get_singleton()
                    ->is_breakpoint(current_node_id, source)) {
                do_break = true;
            }

            if (do_break) {
                VisualScriptLanguage::singleton->debug_break(
                    "Breakpoint",
                    true
                );
            }
        ...
        if (ret & VisualScriptNodeInstance::STEP_EXIT_FUNCTION_BIT) {
            if (node->get_working_memory_size() == 0) {
                r_error.error = Variant::CallError::CALL_ERROR_INVALID_METHOD;
                error_str =
                    RTR("Return value must be assigned to first element of "
        ...
        if (flow_stack) {
            ...
            if (ret & VisualScriptNodeInstance::STEP_FLAG_PUSH_STACK_BIT) {
                ...
                sequence_bits[node->sequence_index] =
                    true; // remember sequence bit
                VSDEBUG("NEXT SEQ - FLAG BIT");
            } else {
                sequence_bits[node->sequence_index] =
                    false; // forget sequence bit
            ...
            if (ret & VisualScriptNodeInstance::STEP_FLAG_GO_BACK_BIT) {
                ...
                if (sequence_bits[next->sequence_index]) {
                    // what happened here is that we are entering a node that is
                    // in the middle of doing a sequence (pushed stack) from the
                    // front because each node has a working memory, we can't
                    // really do a sub-sequence as a result, the sequence will
                    // be restarted and the stack will roll back to find where
                    // this node started the sequence
                    ...
                    if (!found) {
                        r_error.error =
                            Variant::CallError::CALL_ERROR_INVALID_METHOD;
                        error_str =
                            RTR("Found sequence bit but not the node in the "
                                "stack, report bug!");
                        error = true;
                        break;
                    }
                    ...
                    node = next;
                    VSDEBUG(
                        "RE-ENTERED A LOOP, RETURNED STACK POS TO - "
                        + itos(flow_stack_pos)
                    );
                    ...
                    if (flow_stack_pos + 1 >= flow_max) {
                        r_error.error =
                            Variant::CallError::CALL_ERROR_INVALID_METHOD;
                        error_str = RTR("Stack overflow with stack depth: ")
                                  + itos(output);
                        error = true;
                        break;
                    }
    ...
    if (error) {
        ...
        if (!VisualScriptLanguage::singleton->debug_break(error_str, false)) {
            _err_print_error(
                ...
                err_file.utf8().get_data(),
                err_line,
                error_str.utf8().get_data(),
                ERR_HANDLER_SCRIPT
            );
        }
...
Variant VisualScriptInstance::call(
    const StringName& p_method,
    ...
    VSDEBUG(
        "ARGUMENTS: " + itos(f->argument_count) =
...
void VisualScriptInstance::notification(int p_notification) {
    ...
    Variant::CallError ce;
    call(
        VisualScriptLanguage::singleton->notification,
        &whatp,
        1,
        ce
    ); // do as call
...
MultiplayerAPI::RPCMode VisualScriptInstance::get_rpc_mode(
    const StringName& p_method
) const {
    if (p_method == script->get_default_func()) {
    ...
    const Map<StringName, VisualScript::Function>::Element* E =
        script->functions.find(p_method);
    if (!E) {
        return MultiplayerAPI::RPC_MODE_DISABLED;
    }
...
MultiplayerAPI::RPCMode VisualScriptInstance::get_rset_mode(
    const StringName& p_variable
) const {
    return MultiplayerAPI::RPC_MODE_DISABLED;
}

void VisualScriptInstance::create(
    const Ref<VisualScript>& p_script,
    Object* p_owner
) {
    script = p_script;
    owner  = p_owner;
    ...
    for (const Map<StringName, VisualScript::Function>::Element* E =
...

        {
            Ref<VisualScriptFunction> func_node =
                script->get_node(E->key(), E->get().function_id);
        ...
        for (const Map<int, VisualScript::Function::NodeData>::Element* F =
                 E->get().nodes.front();
             F;
             F = F->next()) {
            ...
            VisualScriptNodeInstance* instance =
                node->instance(this); // create instance
            ...
            if (instance->input_port_count) {
                instance->input_ports =
                    memnew_arr(int, instance->input_port_count);
            ...
            if (instance->output_port_count) {
                instance->output_ports =
                    memnew_arr(int, instance->output_port_count);
            ...
            if (Object::cast_to<VisualScriptLocalVar>(node.ptr())
                ...
                if (Object::cast_to<VisualScriptLocalVar>(*node)) {
                    var_name =
                        String(Object::cast_to<VisualScriptLocalVar>(*node)
                                   ->get_var_name())
                            .strip_edges();
                } else {
                    var_name =
                        String(Object::cast_to<VisualScriptLocalVarSet>(*node)
                                   ->get_var_name())
                            .strip_edges();
                }
        ...
        for (const Set<VisualScript::DataConnection>::Element* F =
                 E->get().data_connections.front();
             F;
             F = F->next()) {
            ...
            if (from->get_sequence_output_count() == 0
                ...
                // if the node we are reading from has no output sequence, we
                // must call step() before reading from it.
                if (from->pass_idx == -1) {
            ...
            to->input_ports[dc.to_port] =
                from->output_ports[dc.from_port]; // read from wherever the
                                                  // stack is
        }

        // third pass, do sequence connections

        for (const Set<VisualScript::SequenceConnection>::Element* F =
                 E->get().sequence_connections.front();
             F;
             F = F->next()) {
...
Variant VisualScriptFunctionState::_signal_callback(
    ...
    ERR_FAIL_COND_V_MSG(
        instance_id && !ObjectDB::get_instance(instance_id),
        Variant(),
        "Resumed after yield, but class instance is gone."
    );
    ERR_FAIL_COND_V_MSG(
        script_id && !ObjectDB::get_instance(script_id),
        Variant(),
        "Resumed after yield, but script is gone."
    );
    ...
    *working_mem = args; // arguments go to working mem.
    ...
    Variant ret = instance->_call_internal(
        function,
        stack.ptrw(),
        stack.size(),
        node,
...
void VisualScriptFunctionState::connect_to_signal(
    Object* p_obj,
    ...
    binds.push_back(Ref<VisualScriptFunctionState>(this)
    ); // add myself on the back to avoid dying from unreferencing
...
void VisualScriptFunctionState::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("connect_to_signal", "obj", "signals", "args"),
        &VisualScriptFunctionState::connect_to_signal
    );
    ClassDB::bind_method(
        D_METHOD("resume", "args"),
...
void VisualScriptLanguage::get_comment_delimiters(List<String>* p_delimiters
) const {}

void VisualScriptLanguage::get_string_delimiters(List<String>* p_delimiters
) const {}

Ref<Script> VisualScriptLanguage::get_template(
...
void VisualScriptLanguage::make_template(
    const String& p_class_name,
    const String& p_base_class_name,
    Ref<Script>& p_script
) {
...
bool VisualScriptLanguage::validate(
...
int VisualScriptLanguage::find_function(
...
String VisualScriptLanguage::make_function(
...
void VisualScriptLanguage::auto_indent_code(
    String& p_code,
    int p_from_line,
    int p_to_line
) const {}

void VisualScriptLanguage::add_global_constant(
    const StringName& p_variable,
    const Variant& p_value
) {}
...
bool VisualScriptLanguage::debug_break_parse(
    const String& p_file,
    int p_node,
...
void VisualScriptLanguage::debug_get_stack_level_locals(
    int p_level,
    ...
    for (int i = 0; i < node->input_port_count; i++) {
        ...
        if (in_from & VisualScriptNodeInstance::INPUT_DEFAULT_VALUE_BIT) {
            p_values->push_back(
                _call_stack[l].instance->default_values[in_value]
            );
        } else {
...
void VisualScriptLanguage::debug_get_globals(
    List<String>* p_locals,
    List<Variant>* p_values,
    int p_max_subitems,
    int p_max_depth
...
void VisualScriptLanguage::reload_tool_script(
...
void VisualScriptLanguage::get_recognized_extensions(List<String>* p_extensions
) const {
    p_extensions->push_back("vs");
}

void VisualScriptLanguage::get_public_functions(List<MethodInfo>* p_functions
) const {}
...
int VisualScriptLanguage::profiling_get_frame_data(
    ProfilingInfo* p_info_arr,
    int p_info_max
) {
    return 0;
...
Ref<VisualScriptNode> VisualScriptLanguage::create_node_from_name(
    const String& p_name
) {
...
void VisualScriptLanguage::get_registered_node_names(List<String>* r_names) {
    for (Map<String, VisualScriptNodeRegisterFunc>::Element* E =
             register_funcs.front();
         E;
         E = E->next()) {
