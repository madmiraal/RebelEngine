...
int VisualScriptFunctionCall::get_output_sequence_port_count() const {
    if ((method_cache.flags & METHOD_FLAG_CONST
        ...
        || (call_mode == CALL_MODE_BASIC_TYPE
            && Variant::is_method_const(basic_type, function))) {
        return 0;
    } else {
        return 1;
    }
...
int VisualScriptFunctionCall::get_input_value_port_count() const {
    if (call_mode == CALL_MODE_BASIC_TYPE) {
        Vector<Variant::Type> types =
            Variant::get_method_argument_types(basic_type, function);
        ...
        if (mb) {
            ...
            return mb->get_argument_count()
                 ...
                 + (rpc_call_mode >= RPC_RELIABLE_TO_ID ? 1 : 0)
                 - defaulted_args;
        }
...
String VisualScriptFunctionCall::get_output_sequence_port_text(int p_port
) const {
    return String();
}

PropertyInfo VisualScriptFunctionCall::get_input_value_port_info(int p_idx
) const {
    ...
    if (call_mode == CALL_MODE_BASIC_TYPE) {
        Vector<StringName> names =
            Variant::get_method_argument_names(basic_type, function);
        Vector<Variant::Type> types =
            Variant::get_method_argument_types(basic_type, function);
...
PropertyInfo VisualScriptFunctionCall::get_output_value_port_info(int p_idx
) const {
#ifdef DEBUG_METHODS_ENABLED

    if (call_mode == CALL_MODE_BASIC_TYPE) {
        return PropertyInfo(
            Variant::get_method_return_type(basic_type, function),
            ""
        );
    } else {
        if (call_mode == CALL_MODE_INSTANCE) {
...
void VisualScriptFunctionCall::_update_method_cache() {
    ...
    if (call_mode == CALL_MODE_NODE_PATH) {
        ...
        if (base_script != String()) {
            if (!ResourceCache::has(base_script)
                ...
                ScriptServer::edit_request_func(base_script
    ...
    if (mb) {
        ...
        if (mb->is_vararg()) {
            // for vararg just give it 10 arguments (should be enough for most
            // use cases)
            for (int i = 0; i < 10; i++) {
...
void VisualScriptFunctionCall::set_function(const StringName& p_type) {
    ...
    if (call_mode == CALL_MODE_BASIC_TYPE) {
        use_default_args =
            Variant::get_method_default_arguments(basic_type, function).size();
    } else {
        // update all caches
...
VisualScriptFunctionCall::CallMode VisualScriptFunctionCall::get_call_mode(
) const {
    return call_mode;
}
...
void VisualScriptFunctionCall::_validate_property(PropertyInfo& property
) const {
    ...
    if (property.name == "singleton") {
        if (call_mode != CALL_MODE_SINGLETON) {
            ...
            String sl;
            for (List<Engine::Singleton>::Element* E = names.front(); E;
                 E                                   = E->next()) {
                if (sl != String()) {
                    sl += ",";
                }
        ...
        if (call_mode != CALL_MODE_NODE_PATH) {
            ...
            if (bnode) {
                property.hint_string =
                    bnode->get_path(); // convert to loong string
            }
        }
    }
    ...
    if (property.name == "function") {
        if (call_mode == CALL_MODE_BASIC_TYPE) {
            ...
            if (base_script != String()) {
                ...
                if (ResourceCache::has(base_script)) {
                    Ref<Script> script =
                        Ref<Resource>(ResourceCache::get(base_script));
                    if (script.is_valid()) {
                        property.hint        = PROPERTY_HINT_METHOD_OF_SCRIPT;
    ...
    if (property.name == "use_default_args") {
        ...
        if (call_mode == CALL_MODE_BASIC_TYPE) {
            mc = Variant::get_method_default_arguments(basic_type, function)
                     .size();
        } else {
            MethodBind* mb = ClassDB::get_method(_get_base_type(), function);
            if (mb) {
...
void VisualScriptFunctionCall::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_base_type", "base_type"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_base_script", "base_script"),
        &VisualScriptFunctionCall::set_base_script
    );
    ClassDB::bind_method(
        D_METHOD("get_base_script"),
        &VisualScriptFunctionCall::get_base_script
    );

    ClassDB::bind_method(
        D_METHOD("set_basic_type", "basic_type"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_singleton", "singleton"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "singleton"),
        "set_singleton",
        "get_singleton"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "basic_type", PROPERTY_HINT_ENUM, bt),
        "set_basic_type",
        "get_basic_type"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        ...
        "set_function",
        "get_function"
    ); // when set, if loaded properly, will override argument count.
    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "use_default_args"),
        "set_use_default_args",
        "get_use_default_args"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "validate"),
        "set_validate",
        "get_validate"
    );
    ADD_PROPERTY(
        ...
        switch (call_mode) {
            ...
            case VisualScriptFunctionCall::CALL_MODE_BASIC_TYPE: {
                ...
                if (rpc_mode) {
                    ...
                    if (call_mode
                        == VisualScriptFunctionCall::CALL_MODE_INSTANCE) {
                        if (returns >= 2) {
                            *p_outputs[1] = v.call(
                                function,
                                p_inputs + 1,
                            ...
                            r_error.error =
                                Variant::CallError::CALL_ERROR_INVALID_METHOD;
                            r_error_str =
                                "Invalid returns count for call_mode == "
                                "CALL_MODE_INSTANCE";
                            return 0;
                        }
                    } else {
            ...
            case VisualScriptFunctionCall::CALL_MODE_SINGLETON: {
                ...
                if (!object) {
                    r_error.error =
                        Variant::CallError::CALL_ERROR_INVALID_METHOD;
                    r_error_str =
                        "Invalid singleton name: '" + String(singleton) + "'";
                    return 0;
                }
...
VisualScriptNodeInstance* VisualScriptFunctionCall::instance(
...
VisualScriptFunctionCall::TypeGuess VisualScriptFunctionCall::guess_output_type(
    TypeGuess* p_inputs,
    int p_output
) const {
...
int VisualScriptPropertySet::get_output_sequence_port_count() const {
    ...
    // return 1; //Adding a sequence port to set_basic_type breaks backwards
    // compatibility
}
...
String VisualScriptPropertySet::get_output_sequence_port_text(int p_port
) const {
    return String();
}
...
PropertyInfo VisualScriptPropertySet::get_input_value_port_info(int p_idx
) const {
    if (call_mode == CALL_MODE_INSTANCE || call_mode == CALL_MODE_BASIC_TYPE) {
        if (p_idx == 0) {
            PropertyInfo pi;
...
PropertyInfo VisualScriptPropertySet::get_output_value_port_info(int p_idx
) const {
    if (call_mode == CALL_MODE_BASIC_TYPE) {
...
void VisualScriptPropertySet::_update_cache() {
    ...
    if (!Engine::get_singleton()->is_editor_hint(
        )) { // only update cache if editor exists, it's pointless otherwise
        return;
    }
...
VisualScriptPropertySet::CallMode VisualScriptPropertySet::get_call_mode(
) const {
    return call_mode;
}
...
VisualScriptPropertySet::AssignOp VisualScriptPropertySet::get_assign_op(
) const {
    return assign_op;
}
...
void VisualScriptPropertySet::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_assign_op", "assign_op"),
        &VisualScriptPropertySet::set_assign_op
    );
    ClassDB::bind_method(
        D_METHOD("get_assign_op"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "index"),
        "set_index",
        "get_index"
    );
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "assign_op",
            PROPERTY_HINT_ENUM,
            "Assign,Add,Sub,Mul,Div,Mod,ShiftLeft,ShiftRight,BitAnd,BitOr,"
            "Bitxor"
        ),
        "set_assign_op",
        "get_assign_op"
...
public:
    ...
    _FORCE_INLINE_ void _process_get(
        Variant& source,
        const Variant& p_argument,
        bool& valid
    ) {
        if (index != StringName()
            ...
            switch (assign_op) {
                ...
                case VisualScriptPropertySet::ASSIGN_OP_ADD: {
                    value =
                        Variant::evaluate(Variant::OP_ADD, value, p_argument);
                } break;
                case VisualScriptPropertySet::ASSIGN_OP_SUB: {
                    value = Variant::evaluate(
                        Variant::OP_SUBTRACT,
                        value,
                        p_argument
                    );
            ...
            case VisualScriptPropertySet::CALL_MODE_SELF: {
                ...
                if (!valid) {
                    ...
                    r_error_str = "Invalid set value '" + String(*p_inputs[0])
                                + "' on property '" + String(property)
                                + "' of type " + object->get_class();
                }
            } break;
            ...
            case VisualScriptPropertySet::CALL_MODE_BASIC_TYPE: {
                ...
                if (!valid) {
                    r_error.error =
                        Variant::CallError::CALL_ERROR_INVALID_METHOD;
                    r_error_str =
                        "Invalid set value '" + String(*p_inputs[1]) + "' ("
...
VisualScriptNodeInstance* VisualScriptPropertySet::instance(
    VisualScriptInstance* p_instance
) {
...
VisualScriptPropertySet::TypeGuess VisualScriptPropertySet::guess_output_type(
    TypeGuess* p_inputs,
    int p_output
) const {
    if (p_output == 0 && call_mode == CALL_MODE_INSTANCE) {
...
String VisualScriptPropertyGet::get_output_sequence_port_text(int p_port
) const {
    return String();
}

PropertyInfo VisualScriptPropertyGet::get_input_value_port_info(int p_idx
) const {
    if (call_mode == CALL_MODE_INSTANCE || call_mode == CALL_MODE_BASIC_TYPE) {
        if (p_idx == 0) {
            PropertyInfo pi;
...
PropertyInfo VisualScriptPropertyGet::get_output_value_port_info(int p_idx
) const {
    // port 'pass' not backported to 3.x to keep script backwards compatibility
    List<PropertyInfo> props;
    ClassDB::get_property_list(_get_base_type(), &props, false);
...
VisualScriptPropertyGet::CallMode VisualScriptPropertyGet::get_call_mode(
) const {
    return call_mode;
}
...
void VisualScriptPropertyGet::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "index", PROPERTY_HINT_ENUM),
        "set_index",
        "get_index"
    );
    ...
    virtual int step(
        ...
        switch (call_mode) {
                ...
                *p_outputs[0] = another->get(property, &valid);
                ...
                if (!valid) {
                    r_error.error =
                        Variant::CallError::CALL_ERROR_INVALID_METHOD;
                    r_error_str = vformat(
                        RTR("Invalid index property name '%s' in node %s."),
                        String(property),
                        another->get_name()
                    );
                    return 0;
            ...
            default: {
                ...
                // port 'pass' not backported to 3.x to keep script backwards
                // compatibility
...
VisualScriptNodeInstance* VisualScriptPropertyGet::instance(
    VisualScriptInstance* p_instance
) {
...
PropertyInfo VisualScriptEmitSignal::get_input_value_port_info(int p_idx
) const {
    Ref<VisualScript> vs = get_visual_script();
    if (vs.is_valid()) {
        if (!vs->has_custom_signal(name)) {
        ...
        return PropertyInfo(
            vs->custom_signal_get_argument_type(name, p_idx),
...
PropertyInfo VisualScriptEmitSignal::get_output_value_port_info(int p_idx
) const {
    return PropertyInfo();
}
...
void VisualScriptEmitSignal::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "signal"),
        "set_signal",
        "get_signal"
    );
}
...
VisualScriptNodeInstance* VisualScriptEmitSignal::instance(
    VisualScriptInstance* p_instance
) {
...
void register_visual_script_func_nodes() {
    VisualScriptLanguage::singleton
        ->add_register_func("functions/call", create_node_generic<VisualScriptFunctionCall>);
    VisualScriptLanguage::singleton
        ->add_register_func("functions/set", create_node_generic<VisualScriptPropertySet>);
    VisualScriptLanguage::singleton
        ->add_register_func("functions/get", create_node_generic<VisualScriptPropertyGet>);
    ...
    VisualScriptLanguage::singleton
        ->add_register_func("functions/emit_signal", create_node_generic<VisualScriptEmitSignal>);
    ...
    for (int i = 0; i < Variant::VARIANT_MAX; i++) {
        ...
        for (List<MethodInfo>::Element* E = ml.front(); E; E = E->next()) {
            VisualScriptLanguage::singleton->add_register_func(
                "functions/by_type/" + type_name + "/" + E->get().name,
