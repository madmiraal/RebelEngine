...
public:
    ...
    struct ShapeWrapper {
        ...
        ShapeWrapper() : shape(nullptr), bt_shape(nullptr), active(true) {}
...
protected:
    ...
    /// New area is added when overlap with new area (AreaBullet::addOverlap),
    /// then is removed when it exit (CollisionObjectBullet::onExitArea) This
    /// array is used mainly to know which area hold the pointer of this object
...
public:
    ...
    void add_collision_exception(
        const CollisionObjectBullet* p_ignoreCollisionObject
    );
    void remove_collision_exception(
        const CollisionObjectBullet* p_ignoreCollisionObject
    );
    bool has_collision_exception(
        const CollisionObjectBullet* p_otherCollisionObject
    ) const;
    ...
    _FORCE_INLINE_ bool test_collision_mask(CollisionObjectBullet* p_other
    ) const {
        return collisionLayer & p_other->collisionMask
...
class RigidCollisionObjectBullet :
    public CollisionObjectBullet,
    public ShapeOwnerBullet {
protected:
...
public:
    ...
    _FORCE_INLINE_ const Vector<ShapeWrapper>& get_shapes_wrappers() const {
        return shapes;
    }
