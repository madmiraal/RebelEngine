...
int BulletPhysicsDirectSpaceState::intersect_point(
    ...
    collision_object_point.setWorldTransform(
        btTransform(btQuaternion::getIdentity(), bt_point)
    );

    // Setup query
    AllContactResultCallback btResult(
    ...
    // The result is already populated by AllConvexResultCallback
    return btResult.m_count;
}

bool BulletPhysicsDirectSpaceState::intersect_ray(
    ...
    R_TO_B(p_from, btVec_from);
    R_TO_B(p_to, btVec_to);

    // setup query
    ClosestRayResultCallback btResult(
        btVec_from,
        btVec_to,
        &p_exclude,
        p_collide_with_bodies,
    ...
    if (btResult.hasHit()) {
        B_TO_R(btResult.m_hitPointWorld, r_result.position);
        B_TO_R(btResult.m_hitNormalWorld.normalize(), r_result.normal);
        CollisionObjectBullet* gObj = static_cast<CollisionObjectBullet*>(
            btResult.m_collisionObject->getUserPointer()
        );
        if (gObj) {
            ...
            r_result.collider =
                0 == r_result.collider_id
                    ? nullptr
                    : ObjectDB::get_instance(r_result.collider_id);
        } else {
            WARN_PRINT(
                "The raycast performed has hit a collision object that is not "
                "part of the scene, please check it."
            );
        }
        return true;
    } else {
...
int BulletPhysicsDirectSpaceState::intersect_shape(
    const RID& p_shape,
    ...
    ShapeBullet* shape =
        space->get_physics_server()->get_shape_owner()->get(p_shape);
    ERR_FAIL_COND_V(!shape, 0);

    btCollisionShape* btShape =
        shape->create_bt_shape(p_xform.basis.get_scale_abs(), p_margin);
    if (!btShape->isConvex()) {
        bulletdelete(btShape);
        ERR_PRINT(
            "The shape is not a convex shape, then is not supported: shape "
            "type: "
            + itos(shape->get_type())
        );
        return 0;
    ...
    AllContactResultCallback btQuery(
        &collision_object,
        r_results,
        p_result_max,
        &p_exclude,
...
bool BulletPhysicsDirectSpaceState::cast_motion(
    const RID& p_shape,
    const Transform& p_xform,
    ...
    btCollisionShape* btShape =
        shape->create_bt_shape(p_xform.basis.get_scale(), p_margin);
    if (!btShape->isConvex()) {
        bulletdelete(btShape);
        ERR_PRINT(
            "The shape is not a convex shape, then is not supported: shape "
    ...
    ClosestConvexResultCallback btResult(
        bt_xform_from.getOrigin(),
        bt_xform_to.getOrigin(),
        &p_exclude,
        p_collide_with_bodies,
        p_collide_with_areas
    );
    ...
    space->dynamicsWorld->convexSweepTest(
        ...
        bt_xform_to,
        btResult,
        space->dynamicsWorld->getDispatchInfo().m_allowedCcdPenetration
    );
    ...
    if (btResult.hasHit()) {
        ...
        if (r_info) {
            if (btCollisionObject::CO_RIGID_BODY
                == btResult.m_hitCollisionObject->getInternalType()) {
                B_TO_R(
                    static_cast<const btRigidBody*>(
                        btResult.m_hitCollisionObject
                    )
                        ->getVelocityInLocalPoint(btResult.m_hitPointWorld),
                    r_info->linear_velocity
                );
            }
            CollisionObjectBullet* collision_object =
                static_cast<CollisionObjectBullet*>(
                    btResult.m_hitCollisionObject->getUserPointer()
                );
            B_TO_R(btResult.m_hitPointWorld, r_info->point);
            B_TO_R(btResult.m_hitNormalWorld, r_info->normal);
...
/// contact
bool BulletPhysicsDirectSpaceState::collide_shape(
    RID p_shape,
    ...
    btCollisionShape* btShape =
        shape->create_bt_shape(p_shape_xform.basis.get_scale_abs(), p_margin);
    if (!btShape->isConvex()) {
        bulletdelete(btShape);
        ERR_PRINT(
            "The shape is not a convex shape, then is not supported: shape "
    ...
    ContactPairContactResultCallback btQuery(
        &collision_object,
        r_results,
        p_result_max,
        &p_exclude,
...
bool BulletPhysicsDirectSpaceState::rest_info(
    RID p_shape,
    const Transform& p_shape_xform,
    float p_margin,
    ShapeRestInfo* r_info,
    ...
    RestInfoContactResultCallback btQuery(
        &collision_object,
        r_info,
        &p_exclude,
        p_collide_with_bodies,
    ...
    if (btQuery.m_collided) {
        if (btCollisionObject::CO_RIGID_BODY
            == btQuery.m_rest_info_collision_object->getInternalType()) {
            B_TO_R(
                static_cast<const btRigidBody*>(
                    btQuery.m_rest_info_collision_object
                )
                    ->getVelocityInLocalPoint(btQuery.m_rest_info_bt_point),
                r_info->linear_velocity
            );
        }
        B_TO_R(btQuery.m_rest_info_bt_point, r_info->point);
    }

    return btQuery.m_collided;
}

Vector3 BulletPhysicsDirectSpaceState::get_closest_point_to_object_volume(
    RID p_object,
    const Vector3 p_point
) const {
    RigidCollisionObjectBullet* rigid_object =
        space->get_physics_server()->get_rigid_collision_object(p_object);
    ...
    btTransform body_transform(
        rigid_object->get_bt_collision_object()->getWorldTransform()
    );
    ...
    for (int i = rigid_object->get_shape_count() - 1; 0 <= i; --i) {
        ...
        if (shape->isConvex()) {
            ...
            btGjkPairDetector gjk_pair_detector(
                &point_shape,
                convex_shape,
                space->gjk_simplex_solver,
                space->gjk_epa_pen_solver
            );
...
void SpaceBullet::flush_queries() {
    const btCollisionObjectArray& colObjArray =
        dynamicsWorld->getCollisionObjectArray();
    for (int i = colObjArray.size() - 1; 0 <= i; --i) {
        static_cast<CollisionObjectBullet*>(colObjArray[i]->getUserPointer())
            ->dispatch_callbacks();
    }
}
...
void SpaceBullet::set_param(
    ...
    switch (p_param) {
        ...
        default:
            WARN_PRINT(
                "This set parameter (" + itos(p_param)
                + ") is ignored, the SpaceBullet doesn't support it."
            );
            break;
    }
}
...
Variant SpaceBullet::get_param(PhysicsServer::AreaParameter p_param) {
    switch (p_param) {
        ...
        default:
            WARN_PRINT(
                "This get parameter (" + itos(p_param)
                + ") is ignored, the SpaceBullet doesn't support it."
            );
            return Variant();
    }
...
void SpaceBullet::set_param(
    PhysicsServer::SpaceParameter p_param,
    real_t p_value
) {
    switch (p_param) {
...
real_t SpaceBullet::get_param(PhysicsServer::SpaceParameter p_param) {
    switch (p_param) {
        ...
        default:
            WARN_PRINT(
                "The SpaceBullet doesn't support this get parameter ("
                + itos(p_param) + "), 0 is returned."
            );
            return 0.f;
    }
}
...
void SpaceBullet::add_area(AreaBullet* p_area) {
    ...
    dynamicsWorld->addCollisionObject(
        p_area->get_bt_ghost(),
        p_area->get_collision_layer(),
        p_area->get_collision_mask()
    );
}
...
void SpaceBullet::add_soft_body(SoftBodyBullet* p_body) {
    if (is_using_soft_world()) {
        if (p_body->get_bt_soft_body()) {
            p_body->get_bt_soft_body()->m_worldInfo =
                get_soft_body_world_info();
            static_cast<btSoftRigidDynamicsWorld*>(dynamicsWorld)
                ->addSoftBody(
                    p_body->get_bt_soft_body(),
                    p_body->get_collision_layer(),
                    p_body->get_collision_mask()
                );
        }
...
void SpaceBullet::remove_soft_body(SoftBodyBullet* p_body) {
    if (is_using_soft_world()) {
        if (p_body->get_bt_soft_body()) {
            static_cast<btSoftRigidDynamicsWorld*>(dynamicsWorld)
                ->removeSoftBody(p_body->get_bt_soft_body());
...
void SpaceBullet::add_constraint(
    ConstraintBullet* p_constraint,
...
void SpaceBullet::remove_all_collision_objects() {
    for (int i = dynamicsWorld->getNumCollisionObjects() - 1; 0 <= i; --i) {
        ...
        CollisionObjectBullet* colObj =
            static_cast<CollisionObjectBullet*>(btObj->getUserPointer());
        colObj->set_space(nullptr);
    }
}

void onBulletPreTickCallback(
    btDynamicsWorld* p_dynamicsWorld,
    btScalar timeStep
) {
    static_cast<SpaceBullet*>(p_dynamicsWorld->getWorldUserInfo())
        ->flush_queries();
}

void onBulletTickCallback(btDynamicsWorld* p_dynamicsWorld, btScalar timeStep) {
    const btCollisionObjectArray& colObjArray =
        p_dynamicsWorld->getCollisionObjectArray();

    // Notify all Collision objects the collision checker is started
    for (int i = colObjArray.size() - 1; 0 <= i; --i) {
        static_cast<CollisionObjectBullet*>(colObjArray[i]->getUserPointer())
            ->on_collision_checker_start();
    }
...
btScalar calculateCombinedRestitution(
...
btScalar calculateCombinedFriction(
    const btCollisionObject* body0,
    const btCollisionObject* body1
) {
...
void SpaceBullet::create_empty_world(bool p_create_soft_world) {
    ...
    if (p_create_soft_world) {
        collisionConfiguration = bulletnew(SoftCollisionConfiguration(
            static_cast<btDiscreteDynamicsWorld*>(world_mem)
        ));
    } else {
        collisionConfiguration = bulletnew(CollisionConfiguration(
            static_cast<btDiscreteDynamicsWorld*>(world_mem)
        ));
    }

    dispatcher = bulletnew(CollisionDispatcher(collisionConfiguration));
    broadphase = bulletnew(btDbvtBroadphase);
    solver     = bulletnew(btSequentialImpulseConstraintSolver);

    if (p_create_soft_world) {
        dynamicsWorld = new (world_mem) btSoftRigidDynamicsWorld(
            dispatcher,
            broadphase,
            solver,
        ...
        dynamicsWorld = new (world_mem) btDiscreteDynamicsWorld(
            dispatcher,
            broadphase,
            solver,
            collisionConfiguration
    ...
    filterCallback                        = bulletnew(FilterCallback);
    gCalculateCombinedRestitutionCallback = &calculateCombinedRestitution;
    gCalculateCombinedFrictionCallback    = &calculateCombinedFriction;
    ...
    dynamicsWorld->getBroadphase()
        ...
        ->setInternalGhostPairCallback(ghostPairCallback); // Setup ghost check
...
void SpaceBullet::destroy_world() {
    // The world elements (Collision Objects, Constraints, Shapes, etc.) are
    // managed by Rebel.
...
void SpaceBullet::check_ghost_overlaps() {
    ...
    for (x = areas.size() - 1; 0 <= x; --x) {
        ...
        for (i = area->overlappingObjects.size() - 1; 0 <= i; --i) {
            AreaBullet::OverlappingObjectData& otherObj =
                area->overlappingObjects.write[i];
        ...
        const btAlignedObjectArray<btCollisionObject*> ghostOverlaps =
            area->get_bt_ghost()->getOverlappingPairs();
        ...
        for (i = ghostOverlaps.size() - 1; 0 <= i; --i) {
            ...
            RigidCollisionObjectBullet* otherObject =
                static_cast<RigidCollisionObjectBullet*>(
                    overlapped_bt_co->getUserPointer()
                );
            btVector3 other_body_scale(otherObject->get_bt_body_scale());

            if (!area->is_transform_changed()
                && !otherObject->is_transform_changed()) {
                hasOverlap = -1 != area->find_overlapping_object(otherObject);
                goto collision_found;
            }

            if (overlapped_bt_co->getUserIndex()
                == CollisionObjectBullet::TYPE_AREA) {
                if (!static_cast<AreaBullet*>(overlapped_bt_co->getUserPointer()
                    )
                         ->is_monitorable()) {
                    continue;
                }
            ...
            for (y = area->get_shape_count() - 1; 0 <= y; --y) {
                ...
                for (z = otherObject->get_shape_count() - 1; 0 <= z; --z) {
                    other_body_shape = static_cast<btCollisionShape*>(
                        otherObject->get_bt_shape(z)
                    );

                    btTransform other_shape_transform(
                        otherObject->get_bt_shape_transform(z)
                    );
                    ...
                    if (other_body_shape->isConvex()) {
                        ...
                        gjk_pair_detector
                            .getClosestPoints(gjk_input, result, nullptr);
                        ...
                        DeepPenetrationContactResultCallback contactPointResult(
                            &obA,
                            &obB
                        );
        ...
        for (i = area->overlappingObjects.size() - 1; 0 <= i; --i) {
            // If the overlap has DIRTY state it means that it's no more
            // overlapping
            if (area->overlappingObjects[i].state
                == AreaBullet::OVERLAP_STATE_DIRTY) {
...
void SpaceBullet::check_body_collision() {
    ...
    for (int i = 0; i < numManifolds; ++i) {
        ...
        // I know this static cast is a bit risky. But I'm checking its type
        // just after it. This allow me to avoid a lot of other cast and checks
        RigidBodyBullet* bodyA = static_cast<RigidBodyBullet*>(
            contactManifold->getBody0()->getUserPointer()
        );
        RigidBodyBullet* bodyB = static_cast<RigidBodyBullet*>(
            contactManifold->getBody1()->getUserPointer()
        );

        if (CollisionObjectBullet::TYPE_RIGID_BODY == bodyA->getType()
            && CollisionObjectBullet::TYPE_RIGID_BODY == bodyB->getType()) {
...
#endif
                if (pt.getDistance() <= 0.0 || bodyA->was_colliding(bodyB)
                    || bodyB->was_colliding(bodyA)) {
                    Vector3 collisionWorldPosition;
                    Vector3 collisionLocalPosition;
                    Vector3 normalOnB;
                    float appliedImpulse = pt.m_appliedImpulse;
                    B_TO_R(pt.m_normalWorldOnB, normalOnB);

                    // The pt.m_index only contains the shape index when more
                    // than one collision shape is used and only if the
                    // collision shape is not a concave collision shape. A value
                    // of -1 in pt.m_partId indicates the pt.m_index is a shape
                    // index.
                    int shape_index_a = 0;
                    ...
                    if (bodyA->can_add_collision()) {
                        ...
                        /// pt.m_localPointB Doesn't report the exact point in
                        /// local space
                        B_TO_R(
                            pt.getPositionWorldOnB()
                                - contactManifold->getBody1()
                                      ->getWorldTransform()
                                      .getOrigin(),
                            collisionLocalPosition
                        );
                        bodyA->add_collision_object(
                            bodyB,
                            collisionWorldPosition,
                            collisionLocalPosition,
                            normalOnB,
                    ...
                    if (bodyB->can_add_collision()) {
                        ...
                        /// pt.m_localPointA Doesn't report the exact point in
                        /// local space
                        B_TO_R(
                            pt.getPositionWorldOnA()
...
void SpaceBullet::update_gravity() {
    btVector3 btGravity;
    R_TO_B(gravityDirection * gravityMagnitude, btGravity);
...
bool SpaceBullet::test_body_motion(
    RigidBodyBullet* p_body,
    const Transform& p_from,
    const Vector3& p_motion,
    bool p_infinite_inertia,
    PhysicsServer::MotionResult* r_result,
    bool p_exclude_raycast_shapes,
    const Set<RID>& p_exclude
) {
#if debug_test_motion
    ...
    real_t safe_fraction   = 1.0;
    {
        ...
        for (int shIndex = 0; shIndex < shape_count; ++shIndex) {
            ...
            if (p_exclude_raycast_shapes
                && p_body->get_bt_shape(shIndex)->getShapeType()
                       == CUSTOM_CONVEX_SHAPE_TYPE) {
                // Skip rayshape in order to implement custom separation process
                continue;
            }

            btConvexShape* convex_shape_test(
                static_cast<btConvexShape*>(p_body->get_bt_shape(shIndex))
            );

            btTransform shape_world_from =
                body_transform
                * p_body->get_kinematic_utilities()->shapes[shIndex].transform;

            btTransform shape_world_to(shape_world_from);
            shape_world_to.getOrigin() += motion;

            if ((shape_world_to.getOrigin() - shape_world_from.getOrigin())
                    .fuzzyZero()) {
                motion = btVector3(0, 0, 0);
                break;
            }

            KinClosestConvexResultCallback btResult(
                shape_world_from.getOrigin(),
                shape_world_to.getOrigin(),
                p_body,
                p_infinite_inertia,
                &p_exclude
            );
            ...
            dynamicsWorld->convexSweepTest(
                ...
                shape_world_to,
                btResult,
                dynamicsWorld->getDispatchInfo().m_allowedCcdPenetration
            );

            if (btResult.hasHit()) {
                if (total_length > CMP_EPSILON) {
                    real_t hit_fraction = btResult.m_closestHitFraction
                                        * motion.length() / total_length;
                    if (hit_fraction < unsafe_fraction) {
                        unsafe_fraction = hit_fraction;
                        real_t margin =
                            p_body->get_kinematic_utilities()->safe_margin;
                        safe_fraction = MAX(
                            hit_fraction
                                - (1 - ((total_length - margin) / total_length)
                                ),
                            0
                        );
                    }
                ...
                /// Since for each sweep test I fix the motion of new shapes in
                /// base the recover result, if another shape will hit something
                /// it means that has a deepest penetration respect the previous
                /// shape
    ...
    { /// Phase three - contact test with margin
        ...
        has_penetration = recover_from_penetration(
            p_body,
            body_transform,
            1,
            p_infinite_inertia,
        ...
        // Parse results
        if (r_result) {
            B_TO_R(motion + initial_recover_motion + __rec, r_result->motion);

            if (has_penetration) {
                const btRigidBody* btRigid = static_cast<const btRigidBody*>(
                    r_recover_result.other_collision_object
                );
                CollisionObjectBullet* collisionObject =
                    static_cast<CollisionObjectBullet*>(btRigid->getUserPointer(
                    ));

                B_TO_R(
                    motion,
                    r_result->remainder
                ); // is the remaining movements
                r_result->remainder = p_motion - r_result->remainder;

                B_TO_R(r_recover_result.pointWorld, r_result->collision_point);
                B_TO_R(r_recover_result.normal, r_result->collision_normal);
                // Calculates the velocity at a point.
                B_TO_R(
                    btRigid->getVelocityInLocalPoint(
                        r_recover_result.pointWorld
                        - btRigid->getWorldTransform().getOrigin()
                    ),
                    r_result->collider_velocity
                );
                r_result->collider    = collisionObject->get_self();
                r_result->collider_id = collisionObject->get_instance_id();
                r_result->collider_shape =
                    r_recover_result.other_compound_shape_index;
                r_result->collision_local_shape =
                    r_recover_result.local_shape_most_recovered;
                r_result->collision_depth =
                    Math::abs(r_recover_result.penetration_distance);
                ...
                normalLine->add_vertex(
                    r_result->collision_point + r_result->collision_normal * 10
                );
                normalLine->end();
#endif
            } else {
...
int SpaceBullet::test_ray_separation(
    RigidBodyBullet* p_body,
    const Transform& p_transform,
    bool p_infinite_inertia,
    Vector3& r_recover_motion,
    PhysicsServer::SeparationResult* r_results,
    int p_result_max,
    float p_margin
) {
    btTransform body_transform;
    R_TO_B(p_transform, body_transform);
    UNSCALE_BT_BASIS(body_transform);
    ...
    for (int t(RECOVERING_MOVEMENT_CYCLES); 0 < t; --t) {
        ...
        rays_found_this_round = recover_from_penetration_ray(
            p_body,
            body_transform,
            RECOVERING_MOVEMENT_SCALE,
            p_infinite_inertia,
            p_result_max - rays_found,
            recover_motion,
            next_results
        );
    ...
    B_TO_R(recover_motion, r_recover_motion);
    return rays_found;
}
...
private:
    ...
    public:
        CompoundLeafCallback(
            RecoverPenetrationBroadPhaseCallback* p_parent_callback,
            btCollisionObject* p_collision_object
        ) :
...
public:
    RecoverPenetrationBroadPhaseCallback(
        const btCollisionObject* p_self_collision_object,
        uint32_t p_collision_layer,
        uint32_t p_collision_mask,
        btVector3 p_aabb_min,
        btVector3 p_aabb_max
    ) :
    ...
    bool process(const btBroadphaseProxy* proxy) override {
        ...
        if (co->getInternalType() <= btCollisionObject::CO_RIGID_BODY) {
            if (self_collision_object != proxy->m_clientObject
                && FilterCallback::test_collision_filters(
                    collision_layer,
                    collision_mask,
                ...
                if (co->getCollisionShape()->isCompound()) {
                    const btCompoundShape* cs =
                        static_cast<btCompoundShape*>(co->getCollisionShape());
                    ...
                    if (cs->getNumChildShapes() > 1) {
                        ...
                        const btTransform other_in_compound_space =
                            co->getWorldTransform().inverse();
                        const btMatrix3x3 abs_b =
                            other_in_compound_space.getBasis().absolute();
                        const btVector3 local_center =
                            other_in_compound_space(bounds.Center());
                        const btVector3 local_extent =
                            bounds.Extents().dot3(abs_b[0], abs_b[1], abs_b[2]);
                        const btVector3 local_aabb_min =
                            local_center - local_extent;
                        const btVector3 local_aabb_max =
                            local_center + local_extent;
                        const btDbvtVolume local_bounds = btDbvtVolume::FromMM(
                            local_aabb_min,
                            local_aabb_max
                        );

                        // Test collision against compound child shapes using
                        // its AABB tree
                        ...
                        // If there's only a single child shape then there's no
                        // need to search any more, we know which child overlaps
                        BroadphaseResult result;
...
bool SpaceBullet::recover_from_penetration(
    RigidBodyBullet* p_body,
    const btTransform& p_body_position,
    btScalar p_recover_movement_scale,
    bool p_infinite_inertia,
    ...
    for (int kinIndex = p_body->get_kinematic_utilities()->shapes.size() - 1;
         0 <= kinIndex;
         --kinIndex) {
        const RigidBodyBullet::KinematicShape& kin_shape(
            p_body->get_kinematic_utilities()->shapes[kinIndex]
        );
        ...
        kin_shape.shape
            ->getAabb(shape_transform, shape_aabb_min, shape_aabb_max);
        ...
        if (!shapes_found) {
            ...
            aabb_min.setX(
                (aabb_min.x() < shape_aabb_min.x()) ? aabb_min.x()
                                                    : shape_aabb_min.x()
            );
            aabb_min.setY(
                (aabb_min.y() < shape_aabb_min.y()) ? aabb_min.y()
                                                    : shape_aabb_min.y()
            );
            aabb_min.setZ(
                (aabb_min.z() < shape_aabb_min.z()) ? aabb_min.z()
                                                    : shape_aabb_min.z()
            );

            aabb_max.setX(
                (aabb_max.x() > shape_aabb_max.x()) ? aabb_max.x()
                                                    : shape_aabb_max.x()
            );
            aabb_max.setY(
                (aabb_max.y() > shape_aabb_max.y()) ? aabb_max.y()
                                                    : shape_aabb_max.y()
            );
            aabb_max.setZ(
                (aabb_max.z() > shape_aabb_max.z()) ? aabb_max.z()
                                                    : shape_aabb_max.z()
            );
        }
    }
    ...
    RecoverPenetrationBroadPhaseCallback recover_broad_result(
    ...
    dynamicsWorld->getBroadphase()
        ->aabbTest(aabb_min, aabb_max, recover_broad_result);
    ...
    for (int kinIndex = p_body->get_kinematic_utilities()->shapes.size() - 1;
        ...
        for (int i = recover_broad_result.results.size() - 1; 0 <= i; --i) {
            btCollisionObject* otherObject =
                recover_broad_result.results[i].collision_object;
            ...
            if (p_infinite_inertia
                && !otherObject->isStaticOrKinematicObject()) {
            ...
            if (otherObject->getCollisionShape()->isCompound()) {
                const btCompoundShape* cs = static_cast<const btCompoundShape*>(
                    otherObject->getCollisionShape()
                );
                if (cs->getNumChildShapes() == 0) {
                    continue; // No shapes to depenetrate from.
                }
                int shape_idx =
                    recover_broad_result.results[i].compound_child_index;
                ERR_FAIL_COND_V(
                    shape_idx < 0 || shape_idx >= cs->getNumChildShapes(),
                    false
                );

                if (cs->getChildShape(shape_idx)->isConvex()) {
                    if (RFP_convex_convex_test(
                            kin_shape.shape,
                            static_cast<const btConvexShape*>(
                                cs->getChildShape(shape_idx)
                            ),
                            otherObject,
                            kinIndex,
                            shape_idx,
                            shape_transform,
                            otherObject->getWorldTransform()
                                * cs->getChildTransform(shape_idx),
...
bool SpaceBullet::RFP_convex_convex_test(
    const btConvexShape* p_shapeA,
    const btConvexShape* p_shapeB,
    ...
    if (0 > result.m_distance) {
        // Has penetration
        r_delta_recover_movement +=
            result.m_normalOnBInWorld
            * (result.m_distance * -1 * p_recover_movement_scale);
...
bool SpaceBullet::RFP_convex_world_test(
    ...
    btCollisionObjectWrapper
        obA(nullptr, p_shapeA, p_objectA, tA, -1, p_shapeId_A);
    btCollisionObjectWrapper
    ...
    if (algorithm) {
        ...
        if (contactPointResult.hasHit()) {
            r_delta_recover_movement +=
                contactPointResult.m_pointNormalWorld
                * (contactPointResult.m_penetration_distance * -1
                   * p_recover_movement_scale);
            if (r_recover_result) {
                if (contactPointResult.m_penetration_distance
                    ...
                    r_recover_result->other_compound_shape_index = p_shapeId_B;
                    r_recover_result->penetration_distance =
                        contactPointResult.m_penetration_distance;
                    r_recover_result->pointWorld =
                        contactPointResult.m_pointWorld;
                    r_recover_result->normal =
                        contactPointResult.m_pointNormalWorld;
                }
            }
            return true;
...
int SpaceBullet::add_separation_result(
    PhysicsServer::SeparationResult* r_result,
    const SpaceBullet::RecoverResult& p_recover_result,
    int p_shape_id,
    const btCollisionObject* p_other_object
) const {
    // optimize results (ignore non-colliding)
    if (p_recover_result.penetration_distance < 0.0) {
        const btRigidBody* btRigid =
            static_cast<const btRigidBody*>(p_other_object);
        CollisionObjectBullet* collisionObject =
            static_cast<CollisionObjectBullet*>(p_other_object->getUserPointer()
            );

        r_result->collision_depth = p_recover_result.penetration_distance;
        B_TO_R(p_recover_result.pointWorld, r_result->collision_point);
        B_TO_R(p_recover_result.normal, r_result->collision_normal);
        B_TO_R(
            btRigid->getVelocityInLocalPoint(
                p_recover_result.pointWorld
                - btRigid->getWorldTransform().getOrigin()
            ),
            r_result->collider_velocity
        );
...
int SpaceBullet::recover_from_penetration_ray(
    RigidBodyBullet* p_body,
    const btTransform& p_body_position,
    btScalar p_recover_movement_scale,
    bool p_infinite_inertia,
        ...
        for (int i = recover_broad_result.results.size() - 1; 0 <= i; --i) {
            ...
            if (otherObject->getCollisionShape()->isCompound()) {
                ...
                if (RFP_convex_world_test(
                    ...
                    ray_count = add_separation_result(
                        &r_results[ray_count],
                        recover_result,
                        kinIndex,
                        otherObject
