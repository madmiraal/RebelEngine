...
void EditorSceneImporterFBX::_register_project_setting_import(
...
Node* EditorSceneImporterFBX::import_scene(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps,
    ...
    // done for performance when re-importing lots of files when testing
    // importer in verbose only!
...

    {
        ...
        if (fbx_header_string.find("Kaydara FBX Binary", 0) != -1) {
            ...
            FBXDocParser::TokenizeBinary(
                tokens,
                (const char*)data.write().ptr(),
                (size_t)data.size()
            );
        } else {
            print_verbose("[doc] is ascii");
            FBXDocParser::Tokenize(
                tokens,
                (const char*)data.write().ptr(),
                (size_t)data.size()
            );
        ...
        // 3. This creates a problem: shared_ptr ownership, should Elements
        // later 'take ownership'
        // 4. No, it shouldn't so we should either a.) use weak ref for
        // elements; but this is not correct.
        ...
        if (doc.IsSafeToImport()) {
            ...
            // const FBXDocParser::PropertyPtr app_vendor =
            // p_document->GlobalSettingsPtr()->Props() p_document->Creator()
            const FBXDocParser::PropertyTable* import_props =
                doc.GetMetadataProperties();
            const FBXDocParser::PropertyPtr app_name =
                import_props->Get("Original|ApplicationName");
            const FBXDocParser::PropertyPtr app_vendor =
                import_props->Get("Original|ApplicationVendor");
            const FBXDocParser::PropertyPtr app_version =
                import_props->Get("Original|ApplicationVersion");
            //
            if (app_name) {
                const FBXDocParser::TypedProperty<std::string>*
                    app_name_string = dynamic_cast<
                        const FBXDocParser::TypedProperty<std::string>*>(
                        app_name
                    );
                if (app_name_string) {
                    print_verbose(
                        "FBX App Name: "
                        + String(app_name_string->Value().c_str())
                    );
                }
            }
            ...
            if (app_vendor) {
                const FBXDocParser::TypedProperty<std::string>*
                    app_vendor_string = dynamic_cast<
                        const FBXDocParser::TypedProperty<std::string>*>(
                        app_vendor
                    );
                if (app_vendor_string) {
                    print_verbose(
                        "FBX App Vendor: "
                        + String(app_vendor_string->Value().c_str())
                    );
                    is_blender_fbx = app_vendor_string->Value().find("Blender")
                                  != std::string::npos;
                }
            }
            ...
            if (app_version) {
                const FBXDocParser::TypedProperty<std::string>*
                    app_version_string = dynamic_cast<
                        const FBXDocParser::TypedProperty<std::string>*>(
                        app_version
                    );
                if (app_version_string) {
                    print_verbose(
                        "FBX App Version: "
                        + String(app_version_string->Value().c_str())
                    );
                }
            }
            ...
            if (is_blender_fbx) {
                WARN_PRINT(
                    "We don't officially support Blender FBX animations yet, "
                    "due to issues with upstream Blender,\n"
                    "so please wait for us to work around remaining issues. We "
                    "will continue to import the file but it may be broken.\n"
                    "For minimal breakage, please export FBX from Blender with "
                    "-Z forward, and Y up."
                );
            }

            Spatial* spatial = _generate_scene(
                p_path,
                &doc,
                p_flags,
                p_bake_fps,
            ...
            // todo: move to document shutdown (will need to be validated after
            // moving; this code has been validated already)
...
template <class T>
T EditorSceneImporterFBX::_interpolate_track(
...
void set_owner_recursive(Node* root, Node* current_node) {
    ...
    for (int child_id = 0; child_id < current_node->get_child_count();
         child_id++) {
...
Spatial* EditorSceneImporterFBX::_generate_scene(
    ...
    const real_t fbx_unit_scale =
        p_document->GlobalSettingsPtr()->UnitScaleFactor();
    ...
    // make sure fake noFBXDocParser::PropertyPtr ptrde always has a transform
    // too ;)
    ...
    for (uint64_t skin_id : p_document->GetSkinIDs()) {
        ...
        ERR_CONTINUE_MSG(
            lazy_skin == nullptr,
            "invalid lazy object [serious parser bug]"
        );
        ...
        ERR_CONTINUE_MSG(
            skin == nullptr,
            "invalid skin added to skin list [parser bug]"
        );

        const std::vector<const FBXDocParser::Connection*>
        ...
        const std::vector<const FBXDocParser::Cluster*> clusters =
            skin->Clusters();

        // NOTE: this will ONLY work on skinned bones (it is by design.)
        // A cluster is a skinned bone so SKINS won't contain unskinned bones so
        // we need to pre-add all bones and parent them in a step beforehand.
        for (const FBXDocParser::Cluster* cluster : clusters) {
            ...
            std::vector<const FBXDocParser::Connection*> connections =
                p_document->GetConnectionsByDestinationSequenced(deformer_id);

            // Weight data always has a node in the scene lets grab the limb's
            // node in the scene :) (reverse set to true since it's the opposite
            // way around)
            const FBXDocParser::ModelLimbNode* limb_node =
                ProcessDOMConnection<FBXDocParser::ModelLimbNode>(
                    p_document,
                    deformer_id,
                    true
                );
            ...
            ERR_CONTINUE_MSG(
                limb_node == nullptr,
                "unable to resolve model for skinned bone"
            );
            ...
            ERR_CONTINUE_MSG(
                !state.fbx_bone_map.has(model_id),
                "missing LimbNode detected"
            );
            ...
            ERR_CONTINUE_MSG(
                indexes.size() != weights.size(),
                "[doc] error mismatch between weight info"
            );
            ...
            for (size_t idx = 0; idx < indexes.size(); idx++) {
                ...
                VertexWeightMapping& vm =
                    mesh_vertex_data->vertex_weights[vertex_index];
                vm.weights.push_back(influence_weight);
                vm.bones.push_back(0
                ); // bone id is pushed on here during sanitization phase
                vm.bones_ref.push_back(bone_element);
            }

            for (const int* vertex_index =
                     mesh_vertex_data->vertex_weights.next(nullptr);
                 vertex_index != nullptr;
                 vertex_index =
                     mesh_vertex_data->vertex_weights.next(vertex_index)) {
                VertexWeightMapping* vm =
            ...
            if (mesh_vertex_data->max_weight_count > 4) {
                if (mesh_vertex_data->max_weight_count > 8) {
                    ERR_PRINT(
                        "[doc] Serious: maximum bone influences is 8 in this "
                        "branch."
                    );
                }
                ...
                print_verbose(
                    "[doc] Using 8 vertex bone influences configuration."
                );
            } else {
                mesh_vertex_data->max_weight_count = 4;
                print_verbose(
                    "[doc] Using 4 vertex bone influences configuration."
                );
            }
        }
    }
    ...
    if (state.enable_material_import) {
        ...
        for (uint64_t material_id : materials) {
            FBXDocParser::LazyObject* lazy_material =
                p_document->GetObject(material_id);
            FBXDocParser::Material* mat =
                (FBXDocParser::Material*)
                    lazy_material->Get<FBXDocParser::Material>();
            ERR_CONTINUE_MSG(
                !mat,
                "Could not convert fbx material by id: " + itos(material_id)
            );
    ...
    print_verbose(
        "[doc] Skeleton Bone count: " + itos(state.fbx_bone_map.size())
    );

    // Importing bones using document based method from FBX directly
    // We do not use the assimp bone format to determine this information
    // anymore.
    if (state.fbx_bone_map.size() > 0) {
        ...
        // In order to handle the FBX skeleton we must also inverse any parent
        // transforms on the bones just to rule out any parent node transforms
        // in the bone data this is trivial to do and allows us to use the
        // single skeleton method and merge them this means that the nodes from
        // maya kLocators will be preserved as bones in the same rig without
        // having to match this across skeletons and merge by detection we can
        // just merge and undo any parent transforms
        for (Map<uint64_t, Ref<FBXBone>>::Element* bone_element =
                 state.fbx_bone_map.front();
             bone_element;
            ...
            //// populate bone skeleton - since fbx has no DOM for the skeleton
            /// just a node.
            ...
            CRASH_COND_MSG(
                !state.fbx_target_map.has(armature_id),
                "invalid armature [serious]"
            );
            ...
            CRASH_COND_MSG(
                node->pivot_transform.is_null(),
                "invalid pivot transform [serious]"
            );
            fbx_skeleton_inst->fbx_node = node;

            ERR_CONTINUE_MSG(
                fbx_skeleton_inst->fbx_node.is_null(),
                "invalid skeleton node [serious]"
            );

            // we need to have a valid armature id and the model configured for
            // the bone to be assigned fully. happens once per skeleton

            if (state.fbx_target_map.has(armature_id)
                && !fbx_skeleton_inst->fbx_node->has_model()) {
                print_verbose(
                    "allocated fbx skeleton primary / armature node for the "
                    "level: "
                ...
                // this means bone will be removed and not used, which is safe
                // actually and no skeleton will be created.
            }
        }

        // setup skeleton instances if required :)
        for (Map<uint64_t, Ref<FBXSkeleton>>::Element* skeleton_node =
                 state.skeleton_map.front();
             skeleton_node;
             skeleton_node = skeleton_node->next()) {
            Ref<FBXSkeleton>& skeleton = skeleton_node->value();
            skeleton->init_skeleton(state);

            ERR_CONTINUE_MSG(
                skeleton->fbx_node.is_null(),
                "invalid fbx target map, missing skeleton"
            );
        }

        // This list is not populated
        for (Map<uint64_t, Ref<FBXNode>>::Element* skin_mesh =
                 state.MeshNodes.front();
             skin_mesh;
    ...
    if (state.fbx_node_list.size() > 0) {
        for (List<Ref<FBXNode>>::Element* node_element =
            ...
            if (fbx_node->fbx_model == nullptr) {
                print_error(
                    "[doc] fundamental flaw, submit bug immediately with full "
                    "import log with verbose logging on"
                );
            } else {
                const std::vector<const FBXDocParser::Geometry*>& geometry =
                    fbx_node->fbx_model->GetGeometry();
                for (const FBXDocParser::Geometry* mesh : geometry) {
                    print_verbose(
                        "[doc] [" + itos(mesh->ID())
                        + "] mesh: " + fbx_node->node_name
                    );
                    ...
                    const FBXDocParser::MeshGeometry* mesh_geometry =
                        dynamic_cast<const FBXDocParser::MeshGeometry*>(mesh);
                    if (mesh_geometry) {
                        uint64_t mesh_id = mesh_geometry->ID();

                        // this data will pre-exist if vertex weight information
                        // is found
                        if (state.renderer_mesh_data.has(mesh_id)) {
                            mesh_data_precached =
                                state.renderer_mesh_data[mesh_id];
                        } else {
                            mesh_data_precached.instance();
                            state.renderer_mesh_data.insert(
                                mesh_id,
                                mesh_data_precached
                            );
                        }
                        ...
                        // mesh node, mesh id
                        mesh_node = mesh_data_precached->create_fbx_mesh(
                            state,
                            mesh_geometry,
                            fbx_node->fbx_model,
                            p_compress_flags
                    ...
                    const FBXDocParser::ShapeGeometry* shape_geometry =
                        dynamic_cast<const FBXDocParser::ShapeGeometry*>(mesh);
    ...
    for (Map<uint64_t, Ref<FBXMeshData>>::Element* mesh_data =
        ...
        //
        // ERR_CONTINUE_MSG(!state.fbx_target_map.has(mesh_id), "invalid xform
        // for the skin pose: " + itos(mesh_id)); Ref<FBXNode>
        // mesh_node_xform_data = state.fbx_target_map[mesh_id];
        ...
        for (const FBXDocParser::Cluster* cluster : mesh_skin->Clusters()) {
            // node or bone this cluster targets (in theory will only be a bone
            // target)
            uint64_t skin_target_id = cluster->TargetNode()->ID();

            print_verbose(
                "adding cluster [" + itos(cluster->ID()) + "] "
                + String(cluster->Name().c_str()) + " for target: ["
                + itos(skin_target_id) + "] "
                + String(cluster->TargetNode()->Name().c_str())
            );
            ERR_CONTINUE_MSG(
                !state.fbx_bone_map.has(skin_target_id),
                "no bone found by that ID? locator"
            );
            ...
            skin->add_named_bind(
                ...
                get_unscaled_transform(
                    skeleton_node->pivot_transform->GlobalTransform
                            .affine_inverse()
                        * cluster->TransformLink().affine_inverse(),
                    state.scale
                )
            );
        }
        ...
        print_verbose(
            "cluster name / id: " + String(mesh_skin->Name().c_str()) + " ["
            + itos(mesh_skin->ID()) + "]"
        );
        print_verbose(
            "skeleton has " + itos(state.fbx_bone_map.size()) + " binds"
        );
        print_verbose(
            "fbx skin has " + itos(mesh_skin->Clusters().size()) + " binds"
        );
    }
    ...
    for (Map<uint64_t, Ref<FBXMeshData>>::Element* mesh_data =
        ...
        if (state.skeleton_map.has(armature)) {
            ...
            print_verbose(
                "[doc] armature mesh to skeleton mapping has been allocated"
            );
        } else {
        ...
        ERR_CONTINUE_MSG(
            !mesh_instance,
            "[doc] invalid mesh mapping for skeleton assignment"
        );
        ERR_CONTINUE_MSG(
            skeleton.is_null(),
            "[doc] unable to resolve the correct skeleton but we have weights!"
        );

        mesh_instance->set_skeleton_path(
            mesh_instance->get_path_to(skeleton->skeleton)
        );
        print_verbose(
            "[doc] allocated skeleton to mesh " + mesh_instance->get_name()
        );

        // do we have a mesh skin for this mesh
        ERR_CONTINUE_MSG(
            !state.MeshSkins.has(mesh_id),
            "no skin found for mesh"
        );
        ...
        print_verbose(
            "[doc] allocated skin to mesh " + mesh_instance->get_name()
        );
    ...
    const FBXDocParser::FileGlobalSettings* FBXSettings =
        p_document->GlobalSettingsPtr();

    // Configure constraints
    // NOTE: constraints won't be added quite yet, we don't have a real need for
    // them *yet*. (they can be supported later on) const std::vector<uint64_t>
    // fbx_constraints = p_document->GetConstraintStackIDs();
    ...
    if (state.enable_animation_import && (p_flags & IMPORT_ANIMATION)) {
        // document animation stack list - get by ID so we can unload any non
        // used animation stack
        const std::vector<uint64_t> animation_stack =
            p_document->GetAnimationStackIDs();

        for (uint64_t anim_id : animation_stack) {
            FBXDocParser::LazyObject* lazyObject =
                p_document->GetObject(anim_id);
            const FBXDocParser::AnimationStack* stack =
                lazyObject->Get<FBXDocParser::AnimationStack>();

            if (stack != nullptr) {
                String animation_name =
                    ImportUtils::FBXNodeToName(stack->Name());
                print_verbose(
                ...
                print_verbose(
                    "start_time" + rtos(start_time) + " end_time "
                    + rtos(end_time)
                );
                ...
                print_verbose(
                    "Animation length: " + rtos(animation->get_length())
                    + " seconds"
                );

                // i think assimp was duplicating things, this lets me know to
                // just reference or ignore this to prevent duplicate
                // information in tracks this would mean that we would be doing
                // three times as much work per track if my theory is correct.
                // this was not the case but this is a good sanity check for the
                // animation handler from the document. it also lets us know if
                // the FBX specification massively changes the animation system,
                // in theory such a change would make this show an fbx
                // specification error, so best keep it in the overhead is tiny.
                Map<uint64_t, const FBXDocParser::AnimationCurve*>
                    CheckForDuplication;

                const std::vector<const FBXDocParser::AnimationLayer*>& layers =
                    stack->Layers();
                print_verbose("FBX Animation layers: " + itos(layers.size()));
                for (const FBXDocParser::AnimationLayer* layer : layers) {
                    std::vector<const FBXDocParser::AnimationCurveNode*>
                        node_list = layer->Nodes();
                    print_verbose(
                        "Layer: " + ImportUtils::FBXNodeToName(layer->Name())
                        + ", " + " AnimCurveNode count "
                        + itos(node_list.size())
                    );
                    ...
                    // 	// visible, T, R, S
                    // 	Map<StringName, Map<uint64_t, Vector3> >
                    // animation_track;
                    // };
                    ...
                    // we need the target ID and the target name for the track
                    // reduction.

                    FBXDocParser::Model::RotOrder quat_rotation_order =
                        FBXDocParser::Model::RotOrder_EulerXYZ;

                    // T:: R:: S:: Visible:: Custom::
                    for (const FBXDocParser::AnimationCurveNode* curve_node :
                         node_list) {
                        // when Curves() is called the curves are actually read,
                        // we could replace this with our own
                        // ProcessDomConnection code here if required. We may
                        // need to do this but ideally we use Curves note: when
                        // you call this there might be a delay in opening it
                        // uses mutable type to 'cache' the response until the
                        // AnimationCurveNode is cleaned up.
                        std::map<
                            std::string,
                            const FBXDocParser::AnimationCurve*>
                            curves = curve_node->Curves();
                        const FBXDocParser::Object* object =
                            curve_node->Target();
                        const FBXDocParser::Model* target =
                            curve_node->TargetAsModel();
                        if (target == nullptr) {
                            if (object != nullptr) {
                                print_error(
                                    "[doc] warning failed to find a target "
                                    "Model for curve: "
                                    + String(object->Name().c_str())
                                );
                            } else {
                                // print_error("[doc] failed to resolve
                                // object");
                                continue;
                            }
                            ...
                            // print_verbose("[doc] applied rotation order: " +
                            // itos(target->RotationOrder()));
                            quat_rotation_order = target->RotationOrder();
                        }

                        uint64_t target_id = target->ID();
                        String target_name =
                            ImportUtils::FBXNodeToName(target->Name());

                        const FBXDocParser::PropertyTable* properties =
                            curve_node->Props();
                        bool got_x = false, got_y = false, got_z = false;
                        float offset_x = FBXDocParser::PropertyGet<float>(
                            properties,
                            "d|X",
                            got_x
                        );
                        float offset_y = FBXDocParser::PropertyGet<float>(
                            properties,
                            "d|Y",
                            got_y
                        );
                        float offset_z = FBXDocParser::PropertyGet<float>(
                            properties,
                            "d|Z",
                            got_z
                        );

                        String curve_node_name =
                            ImportUtils::FBXNodeToName(curve_node->Name());

                        // Reduce all curves for this node into a single
                        // container T, R, S is what we expect, although other
                        // tracks are possible like for example visibility
                        // tracks.
                        ...
                        // another format :)
                        // Map<String, Vector<const Assimp::FBX::AnimationCurve
                        // *> > unordered_track;
                        ...
                        // We build this as a lookup, this is essentially our
                        // 'animation track'
                        // AnimCurveNodes.insert(curve_node_name, Map<uint64_t,
                        // Vector3>());

                        // create the animation curve information with the
                        // target id so the point of this makes a track with the
                        // name "T" for example the target ID is also set here,
                        // this means we don't need to do anything extra when we
                        // are in the 'create all animation tracks' step
                        FBXTrack& keyframe_map =
                            AnimCurveNodes[target_id]
                                          [StringName(curve_node_name)];
                        ...
                        // target id, [ track name, [time index, vector] ]
                        // Map<uint64_t, Map<StringName, Map<uint64_t, Vector3>
                        // > > AnimCurveNodes;

                        // we probably need the target id here.
                        // so map[uint64_t map]...
                        // Map<uint64_t, Vector3D> translation_keys,
                        ...
                        for (std::pair<
                             ...
                             curves) {
                            const String curve_element =
                                ImportUtils::FBXNodeToName(kvp.first);
                            const FBXDocParser::AnimationCurve* curve =
                                kvp.second;
                            String curve_name =
                                ImportUtils::FBXNodeToName(curve->Name());
                            ...
                            // FBX has no name for AnimCurveNode::, most of the
                            // time, not seen any with valid name here.
                            const std::map<int64_t, float>& track_time =
                                curve->GetValueTimeTrack();

                            if (track_time.size() > 0) {
                                for (std::pair<int64_t, float> keyframe :
                                     track_time) {
                                    if (curve_element == "d|X") {
                                        keyframe_map.keyframes[keyframe.first]
                                            .x = keyframe.second;
                                        ...
                                        // print_error("FBX Unsupported element:
                                        // " + curve_element);
                                    }

                                    // print_verbose("[" + itos(target_id) + "]
                                    // Keyframe added:  " +
                                    // itos(keyframe_map.size()));

                                    // print_verbose("Keyframe t:" +
                                    // rtos(animation_track_time) + " v: " +
                                    // rtos(keyframe.second));
                                }
                            }
                        }
                    }
                    ...
                    // Map<uint64_t, Map<StringName, Map<uint64_t, Vector3> > >
                    // AnimCurveNodes; add this animation track here

                    // target id, [ track name, [time index, vector] ]
                    // std::map<uint64_t, std::map<StringName, FBXTrack > >
                    // AnimCurveNodes;
                    for (Map<uint64_t, Map<StringName, FBXTrack>>::Element*
                             track = AnimCurveNodes.front();
                         track;
                         track = track->next()) {
                        // 5 tracks
                        // current track index
                        ...
                        // note we must not run the below code if the entry
                        // doesn't exist, it will create dummy entries which is
                        // very bad. remember that state.fbx_bone_map[target_id]
                        // will create a new entry EVEN if you only read. this
                        // would break node animation targets, so if you change
                        // this be warned. :)
                        ...
                        if (state.fbx_target_map.has(target_id)) {
                            Ref<FBXNode> node_ref =
                                state.fbx_target_map[target_id];
                            target_transform =
                                node_ref->pivot_transform->GlobalTransform;
                            // print_verbose("[doc] allocated animation node
                            // transform");
                        }

                        // int size_targets = state.fbx_target_map.size();
                        // print_verbose("Target ID map: " +
                        // itos(size_targets)); print_verbose("[doc] debug bone
                        // map size: " + itos(state.fbx_bone_map.size()));

                        // if this is a skeleton mapped track we can just set
                        // the path for the track. todo: implement node paths
                        // here at some
                        if (state.fbx_bone_map.size() > 0
                            && state.fbx_bone_map.has(target_id)) {
                            if (bone->fbx_skeleton.is_valid()
                                && bone.is_valid()) {
                                Ref<FBXSkeleton> fbx_skeleton =
                                    bone->fbx_skeleton;
                                String bone_path = state.root->get_path_to(
                            ...
                            // print_verbose("[doc] we have a valid target for a
                            // node animation");
                            ...
                            if (target_node.is_valid()
                                ...
                                animation->track_set_path(track_idx, path);
                                // print_verbose("[doc] node animation path: " +
                                // node_path);
                            }
                        } else {
                            // note: this could actually be unsafe this means we
                            // should be careful about continuing here, if we
                            // see bizarre effects later we should disable this.
                            // I am not sure if this is unsafe or not, testing
                            // will tell us this.
                            print_error(
                                "[doc] invalid fbx target detected for this "
                                "track"
                            );
                            continue;
                        }

                        // everything in FBX and Maya is a node therefore if
                        // this happens something is seriously broken.
                        if (!state.fbx_target_map.has(target_id)) {
                            print_error(
                                "unable to resolve this to an FBX object."
                            );
                            continue;
                        }
                        ...
                        const FBXDocParser::Model* model =
                            target_node->fbx_model;
                        const FBXDocParser::PropertyTable* props =
                            model->Props();
                        ...
                        for (std::pair<int64_t, Vector3> position_key :
                            ...
                            pos_values.push_back(
                                position_key.second * state.scale
                            );
                            double animation_track_time =
                                CONVERT_FBX_TIME(position_key.first);

                            if (animation_track_time > max_duration) {
                                max_duration = animation_track_time;
                            }

                            // print_verbose("pos keyframe: t:" +
                            // rtos(animation_track_time) + " value " +
                            // position_key.second);
                            pos_times.push_back(animation_track_time);
                        }

                        for (std::pair<int64_t, Vector3> scale_key :
                             scale_keys.keyframes) {
                            scale_values.push_back(scale_key.second);
                            double animation_track_time =
                                CONVERT_FBX_TIME(scale_key.first);

                            if (animation_track_time > max_duration) {
                                max_duration = animation_track_time;
                            }
                            // print_verbose("scale keyframe t:" +
                            // rtos(animation_track_time));
                            scale_times.push_back(animation_track_time);
                        }

                        //
                        // Pre and Post keyframe rotation handler
                        // -- Required because Maya and Autodesk <3 the pain
                        ...
                        // Rotation matrix
                        const Vector3& PreRotation =
                            FBXDocParser::PropertyGet<Vector3>(
                                props,
                                "PreRotation",
                                got_pre
                            );
                        const Vector3& PostRotation =
                            FBXDocParser::PropertyGet<Vector3>(
                                props,
                                "PostRotation",
                                got_post
                        ...
                        FBXDocParser::Model::RotOrder rot_order =
                            model->RotationOrder();
                        if (got_pre) {
                            pre_rotation = ImportUtils::EulerToQuaternion(
                                rot_order,
                                ImportUtils::deg2rad(PreRotation)
                            );
                        }
                        if (got_post) {
                            post_rotation = ImportUtils::EulerToQuaternion(
                                rot_order,
                                ImportUtils::deg2rad(PostRotation)
                            );
                        }

                        Quat lastQuat = Quat();

                        for (std::pair<int64_t, Vector3> rotation_key :
                             rotation_keys.keyframes) {
                            double animation_track_time =
                                CONVERT_FBX_TIME(rotation_key.first);

                            // print_verbose("euler rotation key: " +
                            // rotation_key.second);
                            Quat rot_key_value = ImportUtils::EulerToQuaternion(
                                quat_rotation_order,
                                ImportUtils::deg2rad(rotation_key.second)
                            );

                            if (lastQuat != Quat()
                                && rot_key_value.dot(lastQuat) < 0) {
                            ...
                            Quat final_rotation =
                                pre_rotation * rot_key_value * post_rotation;
                        ...
                        const Vector3 def_pos =
                            translation_keys.has_default
                                ? (translation_keys.default_value * state.scale)
                                : bone_rest.origin;
                        const Quat def_rot =
                        ...
                        const Vector3 def_scale =
                            scale_keys.has_default
                                ? scale_keys.default_value
                                : bone_rest.basis.get_scale();
                        print_verbose(
                            "track defaults: p(" + def_pos + ") s(" + def_scale
                            + ") r(" + def_rot + ")"
                        );
                        ...
                        while (true) {
                            ...
                            if (pos_values.size()) {
                                pos = _interpolate_track<Vector3>(
                                    pos_times,
                                    pos_values,
                                    time,
                            ...
                            if (scale_values.size()) {
                                scale = _interpolate_track<Vector3>(
                                    scale_times,
                                    scale_values,
                                    time,
                                    AssetImportAnimation::INTERP_LINEAR
                            ...
                            if (skeleton_bone >= 0) {
                                ...
                                xform.origin = pos;
                                const Transform t =
                                    bone_rest.affine_inverse() * xform;
                ...
                state.animation_player->add_animation(
                    animation_name,
                    animation
                );
            }
        ...
        // AnimLayer is the current active layer of the animation (multiple
        // layers can be active we only support 1) AnimCurveNode has a OP link
        // back to the model which is the real node. AnimCurveNode has a direct
        // link to AnimationCurve (of which it may have more than one)
        ...
        // AnimStack:: <-- (OO) --> AnimLayer:: <-- (OO) --> AnimCurveNode::
        // (which can OP resolve) to Model::
    }

    //
    // Cleanup operations - explicit to prevent errors on shutdown - found that
    // ref to ref does behave badly sometimes.
    //
    ...
    for (Map<uint64_t, Ref<FBXBone>>::Element* element =
             state.fbx_bone_map.front();
         element;
    ...
    for (Map<uint64_t, Ref<FBXSkeleton>>::Element* element =
             state.skeleton_map.front();
         element;
         element = element->next()) {
...
void EditorSceneImporterFBX::BuildDocumentBones(
    ...
    const std::vector<const FBXDocParser::Connection*>& conns =
        p_doc->GetConnectionsByDestinationSequenced(p_id, "Model");
    // FBX can do an join like this
    // Model -> SubDeformer (bone) -> Deformer (skin pose)
    // This is important because we need to somehow link skin back to bone id in
    // skeleton :) The rules are: A subdeformer will exist if 'limbnode' class
    // tag present The subdeformer will not necessarily have a deformer as
    // joints do not have one
    for (const FBXDocParser::Connection* con : conns) {
        // goto: bone creation
        // print_verbose("con: " + String(con->PropertyName().c_str()));

        // ignore object-property links we want the object to object links
        // nothing else
        ...
        const FBXDocParser::Model* const model =
            dynamic_cast<const FBXDocParser::Model*>(object);

        // declare our bone element reference (invalid, unless we create a bone
        // in this step) this lets us pass valid armature information into
        // children objects and this is why we moved this up here previously
        // this was created .instanced() on the same line.
        Ref<FBXBone> bone_element;

        if (model != nullptr) {
            // model marked with limb node / casted.
            const FBXDocParser::ModelLimbNode* const limb_node =
                dynamic_cast<const FBXDocParser::ModelLimbNode*>(model);
            if (limb_node != nullptr) {
                // Write bone into bone list for FBX

                ERR_FAIL_COND_MSG(
                    state.fbx_bone_map.has(limb_node->ID()),
                    "[serious] duplicate LimbNode detected"
                );
                ...
                if (!parent_is_bone) {
                    ...
                    if (p_parent_bone->valid_armature_id) {
                        ...
                        print_verbose(
                            "[doc] bone has valid armature id:"
                            + itos(bone_element->armature_id)
                        );
                    } else {
                        print_error(
                            "[doc] unassigned armature id: "
                            + String(limb_node->Name().c_str())
                        );
                    }
                } else {
                    print_error(
                        "[doc] error is this a bone? "
                        + String(limb_node->Name().c_str())
                    );
                }
                ...
                if (!parent_is_bone) {
                    print_verbose(
                        "[doc] Root bone: " + bone_element->bone_name
                    );
                }
...
void EditorSceneImporterFBX::BuildDocumentNodes(
    ...
    const std::vector<const FBXDocParser::Connection*>& conns =
        p_doc->GetConnectionsByDestinationSequenced(id, "Model");
    ...
    for (const FBXDocParser::Connection* con : conns) {
        ...
        if (con->PropertyName().length()) {
            ...
            print_verbose(
                "ignoring property link - no docs on why this is ignored"
            );
            continue;
        }

        // convert connection source object into Object base class
        // Source objects can exist with 'null connections' this means that we
        // only for sure know the source exists.
        ...
        // This detects if we can cast the object into this model structure.
        const FBXDocParser::Model* const model =
            dynamic_cast<const FBXDocParser::Model*>(source_object);
        ...
        if (nullptr != model) {
            ...
            CRASH_COND_MSG(
                new_node->pivot_transform.is_null(),
                "invalid fbx target map pivot transform [serious]"
            );
