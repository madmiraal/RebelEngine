...
void RasterizerSceneGLES2::shadow_atlas_set_size(RID p_atlas, int p_size) {
    ...
    if (shadow_atlas->size) {
        ...
        if (shadow_atlas->size > storage->config.max_viewport_dimensions[0]
            || shadow_atlas->size
                   > storage->config.max_viewport_dimensions[1]) {
            WARN_PRINT(
                "Cannot set shadow atlas size larger than maximum hardware "
                "supported size of ("
                + itos(storage->config.max_viewport_dimensions[0]) + ", "
                + itos(storage->config.max_viewport_dimensions[1])
                + "). Setting size to maximum."
            );
            shadow_atlas->size =
                MIN(shadow_atlas->size,
                    storage->config.max_viewport_dimensions[0]);
            shadow_atlas->size =
                MIN(shadow_atlas->size,
                    storage->config.max_viewport_dimensions[1]);
        }
        ...
        if (storage->config.use_rgba_3d_shadows) {
            ...
            glRenderbufferStorage(
                GL_RENDERBUFFER,
                storage->config.depth_buffer_internalformat,
                shadow_atlas->size,
                shadow_atlas->size
            );
            ...
            glTexImage2D(
                GL_TEXTURE_2D,
                0,
                storage->config.depth_internalformat,
                shadow_atlas->size,
                shadow_atlas->size,
                0,
                GL_DEPTH_COMPONENT,
                storage->config.depth_type,
                nullptr
            );
...
void RasterizerSceneGLES2::shadow_atlas_set_quadrant_subdivision(
    RID p_atlas,
    int p_quadrant,
    int p_subdivision
) {
...
bool RasterizerSceneGLES2::_shadow_atlas_find_shadow(
    ...
    for (int i = p_quadrant_count - 1; i >= 0; i--) {
        ...
        uint64_t min_pass =
            0; // pass of the existing one, try to use the least recently
...
bool RasterizerSceneGLES2::shadow_atlas_update_light(
    RID p_atlas,
    RID p_light_intance,
    float p_coverage,
...
void RasterizerSceneGLES2::reflection_atlas_set_size(
    RID p_ref_atlas,
    int p_size
) {}

void RasterizerSceneGLES2::reflection_atlas_set_subdivision(
    RID p_ref_atlas,
    int p_subdiv
) {}
...
RID RasterizerSceneGLES2::reflection_probe_instance_create(RID p_probe) {
    RasterizerStorageGLES2::ReflectionProbe* probe =
...
void RasterizerSceneGLES2::reflection_probe_instance_set_transform(
    RID p_instance,
...
bool RasterizerSceneGLES2::reflection_probe_instance_needs_redraw(RID p_instance
    ...
    const ReflectionProbeInstance* rpi =
        reflection_probe_instance_owner.getornull(p_instance);
    ERR_FAIL_COND_V(!rpi, false);

    bool need_redraw =
        rpi->probe_ptr->resolution != rpi->current_resolution || rpi->dirty
        || rpi->probe_ptr->update_mode == VS::REFLECTION_PROBE_UPDATE_ALWAYS;
    rpi->dirty = false;
    return need_redraw;
}

bool RasterizerSceneGLES2::reflection_probe_instance_has_reflection(
    RID p_instance
) {
    return true;
}

bool RasterizerSceneGLES2::reflection_probe_instance_begin_render(
    RID p_instance,
    ...
    if (rpi->probe_ptr->resolution != rpi->current_resolution) {
        ...
        if (size > storage->config.max_viewport_dimensions[0]
            || size > storage->config.max_viewport_dimensions[1]) {
            WARN_PRINT_ONCE(
                "Cannot set reflection probe resolution larger than maximum "
                "hardware supported size of ("
                + itos(storage->config.max_viewport_dimensions[0]) + ", "
                + itos(storage->config.max_viewport_dimensions[1])
                + "). Setting size to maximum."
            );
...
bool RasterizerSceneGLES2::reflection_probe_instance_postprocess_step(
    RID p_instance
) {
    ReflectionProbeInstance* rpi =
        reflection_probe_instance_owner.getornull(p_instance);
    ...
    glTexParameterf(
        ...
        GL_TEXTURE_MIN_FILTER,
        GL_LINEAR
    ); // use linear, no mipmaps so it does not read from what is being written
       // to
    ...
    glBindTexture(
        GL_TEXTURE_2D,
        storage->resources.radical_inverse_vdc_cache_tex
    );
    ...
    while (size >= 1) {
        ...
        glFramebufferTexture2D(
            ...
            GL_COLOR_ATTACHMENT0,
            GL_TEXTURE_2D,
            storage->resources.mipmap_blur_color,
            0
        );
        glViewport(0, 0, size, size);
        glActiveTexture(GL_TEXTURE0);
        ...
        for (int i = 0; i < 6; i++) {
            ...
            storage->shaders.cubemap_filter.set_uniform(
                CubemapFilterShaderGLES2::FACE_ID,
                i
            );
            ...
            storage->shaders.cubemap_filter.set_uniform(
                CubemapFilterShaderGLES2::Z_FLIP,
                false
            );
...
void RasterizerSceneGLES2::environment_set_background(
    RID p_env,
...
void RasterizerSceneGLES2::environment_set_sky_custom_fov(
    RID p_env,
    float p_scale
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES2::environment_set_sky_orientation(
    RID p_env,
...
void RasterizerSceneGLES2::environment_set_bg_color(
    RID p_env,
    const Color& p_color
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES2::environment_set_bg_energy(
    RID p_env,
    float p_energy
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES2::environment_set_canvas_max_layer(
    RID p_env,
    int p_max_layer
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES2::environment_set_ambient_light(
    RID p_env,
    const Color& p_color,
    float p_energy,
...
void RasterizerSceneGLES2::environment_set_camera_feed_id(
    RID p_env,
...
void RasterizerSceneGLES2::environment_set_dof_blur_far(
    RID p_env,
    bool p_enable,
    float p_distance,
    float p_transition,
...
void RasterizerSceneGLES2::environment_set_fog(
    RID p_env,
    bool p_enable,
    float p_begin,
    float p_end,
...
void RasterizerSceneGLES2::environment_set_ssr(
    RID p_env,
    bool p_enable,
    int p_max_steps,
    float p_fade_in,
...
void RasterizerSceneGLES2::environment_set_ssao(
    RID p_env,
    bool p_enable,
    float p_radius,
    float p_intensity,
...
void RasterizerSceneGLES2::environment_set_tonemap(
    RID p_env,
...
void RasterizerSceneGLES2::environment_set_adjustment(
    RID p_env,
    bool p_enable,
    float p_brightness,
    float p_contrast,
...
void RasterizerSceneGLES2::environment_set_fog_depth(
    RID p_env,
    bool p_enable,
    float p_depth_begin,
    float p_depth_end,
...
void RasterizerSceneGLES2::environment_set_fog_height(
    RID p_env,
    bool p_enable,
    float p_min_height,
    float p_max_height,
...
void RasterizerSceneGLES2::light_instance_set_transform(
...
void RasterizerSceneGLES2::light_instance_set_shadow_transform(
    RID p_light_instance,
...
void RasterizerSceneGLES2::gi_probe_instance_set_light_data(
    RID p_probe,
    RID p_base,
    RID p_data
) {}

void RasterizerSceneGLES2::gi_probe_instance_set_transform_to_data(
    RID p_probe,
...
void RasterizerSceneGLES2::_add_geometry(
...
void RasterizerSceneGLES2::_add_geometry_with_material(
    RasterizerStorageGLES2::Geometry* p_geometry,
    InstanceBase* p_instance,
    RasterizerStorageGLES2::GeometryOwner* p_owner,
    ...
    bool has_blend_alpha =
        ...
        != RasterizerStorageGLES2::Shader::Spatial::BLEND_MODE_MIX;
        ...
        if (has_blend_alpha || p_material->shader->spatial.uses_depth_texture
            || (has_base_alpha
                && p_material->shader->spatial.depth_draw_mode
                       != RasterizerStorageGLES2::Shader::Spatial::
    ...
    e->refprobe_0_index =
        RenderList::MAX_REFLECTION_PROBES; // refprobe disabled by default
    e->refprobe_1_index =
        RenderList::MAX_REFLECTION_PROBES; // refprobe disabled by default
    ...
    if (!p_depth_pass) {
        ...
        if (rpsize > 0) {
            ...
            rpsize     = MIN(
                rpsize,
                2
            ); // more than 2 per object are not supported, this keeps it stable

            for (int i = 0; i < rpsize; i++) {
                ReflectionProbeInstance* rpi =
                    reflection_probe_instance_owner.getornull(
                        e->instance->reflection_probe_instances[i]
                    );
            ...
            /*	if (e->refprobe_0_index > e->refprobe_1_index) { //if both are
            valid, swap them to keep order as best as possible uint64_t tmp =
            e->refprobe_0_index; e->refprobe_0_index = e->refprobe_1_index;
        ...
        if (p_material->shader->spatial.unshaded) {
            ...
            for (int i = 0; i < render_directional_lights; i++) {
                if (copy) {
                    RenderList::Element* e2 =
                        has_alpha ? render_list.add_alpha_element()
                                  : render_list.add_element();
                    if (!e2) {
                        break;
                    }
            ...
            for (int i = 0; i < e->instance->light_instances.size(); i++) {
                LightInstance* li = light_instance_owner.getornull(
                    e->instance->light_instances[i]
                );

                if (!li || li->light_index >= render_light_instance_count
                    || render_light_instances[li->light_index] != li) {
                    continue; // too many or light_index did not correspond to
                              // the light instances to be rendered
                }
                ...
                e->light_type1 = 1;
                e->light_type2 =
                    li->light_ptr->type == VisualServer::LIGHT_OMNI ? 0 : 1;
...
void RasterizerSceneGLES2::_copy_texture_to_buffer(
    GLuint p_texture,
    GLuint p_buffer
) {
...
void RasterizerSceneGLES2::_fill_render_list(
    ...
    for (int i = 0; i < p_cull_count; i++) {
        ...
        switch (instance->base_type) {
            case VS::INSTANCE_MESH: {
                RasterizerStorageGLES2::Mesh* mesh =
                    storage->mesh_owner.getornull(instance->base);
                ERR_CONTINUE(!mesh);
                ...
                for (int j = 0; j < num_surfaces; j++) {
                    int material_index =
                        instance->materials[j].is_valid() ? j : -1;

                    RasterizerStorageGLES2::Surface* surface =
                        mesh->surfaces[j];
            ...
            case VS::INSTANCE_MULTIMESH: {
                RasterizerStorageGLES2::MultiMesh* multi_mesh =
                    storage->multimesh_owner.getptr(instance->base);
            ...
            case VS::INSTANCE_IMMEDIATE: {
                RasterizerStorageGLES2::Immediate* im =
                    storage->immediate_owner.getptr(instance->base);
                ERR_CONTINUE(!im);
...
void RasterizerSceneGLES2::_set_cull(
    bool p_front,
    bool p_disabled,
    bool p_reverse_cull
) {
...
bool RasterizerSceneGLES2::_setup_material(
    RasterizerStorageGLES2::Material* p_material,
    bool p_alpha_pass,
    Size2i p_skeleton_tex_size
) {
    // material parameters
    ...
    if (p_material->shader->spatial.uses_screen_texture
        ...
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->copy_screen_effect.color
        );
    }
    ...
    switch (p_material->shader->spatial.depth_draw_mode) {
        ...
        case RasterizerStorageGLES2::Shader::Spatial::DEPTH_DRAW_OPAQUE: {
            glDepthMask(
                !p_alpha_pass && !p_material->shader->spatial.uses_depth_texture
            );
        } break;
        case RasterizerStorageGLES2::Shader::Spatial::DEPTH_DRAW_ALWAYS: {
            glDepthMask(
                GL_TRUE && !p_material->shader->spatial.uses_depth_texture
            );
        } break;
    ...
    const ShaderLanguage::ShaderNode::Uniform::Hint* texture_hints =
        p_material->shader->texture_hints.ptr();

    state.scene_shader.set_uniform(
        SceneShaderGLES2::SKELETON_TEXTURE_SIZE,
        p_skeleton_tex_size
    );
    ...
    for (int i = 0; i < tc; i++) {
        ...
        RasterizerStorageGLES2::Texture* t =
            storage->texture_owner.getornull(textures[i].second);
        ...
        if (t->redraw_if_visible) { // must check before proxy because this is
...
void RasterizerSceneGLES2::_setup_geometry(
    ...
    switch (p_element->instance->base_type) {
        case VS::INSTANCE_MESH: {
            RasterizerStorageGLES2::Surface* s =
                static_cast<RasterizerStorageGLES2::Surface*>(
                    p_element->geometry
                );
            ...
            for (int i = 0; i < VS::ARRAY_MAX - 1; i++) {
                if (s->attribs[i].enabled) {
                    ...
                    if (!s->blend_shape_data.empty() && i != VS::ARRAY_BONES
                        && s->blend_shape_buffer_size > 0) {
                        glBindBuffer(GL_ARRAY_BUFFER, s->blend_shape_buffer_id);
                        // When using octahedral compression (2 component
                        // normal/tangent) decompression changes the component
                        // count to 3/4
                        int size;
                        switch (i) {
                        ...
                        glVertexAttribPointer(
                            ...
                            GL_FALSE,
                            8 * 4 * sizeof(float),
                            CAST_INT_TO_UCHAR_PTR(i * 4 * sizeof(float))
                        );
                        ...
                        glVertexAttribPointer(
                            ...
                            s->attribs[i].size,
                            s->attribs[i].type,
                            s->attribs[i].normalized,
                            s->attribs[i].stride,
                            CAST_INT_TO_UCHAR_PTR(s->attribs[i].offset)
                        );
                    }
                } else {
                    glDisableVertexAttribArray(i);
            ...
            if (p_skeleton) {
                if (!storage->config.use_skeleton_software) {
                    ...
                    glActiveTexture(
                        GL_TEXTURE0 + storage->config.max_texture_image_units
                        - 1
                    );
                    ...
                    PoolVector<float>& transform_buffer =
                        storage->resources.skeleton_transform_cpu_buffer;

                    if (!s->attribs[VS::ARRAY_BONES].enabled
                        || !s->attribs[VS::ARRAY_WEIGHTS].enabled) {
                        break; // the whole instance has a skeleton, but this
                               // surface is not affected by it.
                    }
                    ...
                    const size_t bones_offset =
                        s->attribs[VS::ARRAY_BONES].offset;
                    const size_t bones_stride =
                        s->attribs[VS::ARRAY_BONES].stride;
                    const size_t bone_weight_offset =
                        s->attribs[VS::ARRAY_WEIGHTS].offset;
                    const size_t bone_weight_stride =
                        s->attribs[VS::ARRAY_WEIGHTS].stride;

                    {
                        PoolVector<float>::Write write =
                            transform_buffer.write();
                        float* buffer = write.ptr();

                        PoolVector<uint8_t>::Read vertex_array_read =
                            s->data.read();
                        ...
                        for (int i = 0; i < s->array_len; i++) {
                            ...
                            if (s->attribs[VS::ARRAY_BONES].type
                                == GL_UNSIGNED_BYTE) {
                                // read as byte
                                const uint8_t* bones_ptr = vertex_data
                                                         + bones_offset
                                                         + (i * bones_stride);
                                bones[0] = bones_ptr[0];
                                bones[1] = bones_ptr[1];
                                ...
                                // read as short
                                const uint16_t* bones_ptr =
                                    (const uint16_t*)(vertex_data + bones_offset
                                                      + (i * bones_stride));
                                bones[0] = bones_ptr[0];
                                bones[1] = bones_ptr[1];
                                bones[2] = bones_ptr[2];
                            ...
                            if (s->attribs[VS::ARRAY_WEIGHTS].type
                                == GL_FLOAT) {
                                // read as float
                                const float* weight_ptr =
                                    (const float*)(vertex_data
                                                   + bone_weight_offset
                                                   + (i * bone_weight_stride));
                                ...
                                const uint16_t* weight_ptr =
                                    (const uint16_t*)(vertex_data
                                                      + bone_weight_offset
                                                      + (i * bone_weight_stride)
                                    );
                                bone_weight[0] =
                                    (weight_ptr[0] / (float)0xFFFF);
                                bone_weight[1] =
                                    (weight_ptr[1] / (float)0xFFFF);
                                bone_weight[2] =
                                    (weight_ptr[2] / (float)0xFFFF);
                                bone_weight[3] =
                                    (weight_ptr[3] / (float)0xFFFF);
                            }

                            Transform transform;

                            Transform bone_transforms[4] = {
                                storage->skeleton_bone_get_transform(
                                    p_element->instance->skeleton,
                                    bones[0]
                                ),
                                storage->skeleton_bone_get_transform(
                                    p_element->instance->skeleton,
                    ...
                    storage->_update_skeleton_transform_buffer(
                        transform_buffer,
                        s->array_len * 12
                    );
                    ...
                    glBindBuffer(
                        GL_ARRAY_BUFFER,
                        storage->resources.skeleton_transform_buffer
                    );
                    ...
                    glVertexAttribPointer(
                        INSTANCE_BONE_BASE + 0,
                        4,
                        GL_FLOAT,
                        GL_FALSE,
                        sizeof(float) * 12,
                        (const void*)(sizeof(float) * 4 * 0)
                    );
                    glVertexAttribPointer(
                        INSTANCE_BONE_BASE + 1,
                        4,
                        GL_FLOAT,
                        GL_FALSE,
                        sizeof(float) * 12,
                        (const void*)(sizeof(float) * 4 * 1)
                    );
                    glVertexAttribPointer(
                        INSTANCE_BONE_BASE + 2,
                        4,
                        GL_FLOAT,
                        GL_FALSE,
                        sizeof(float) * 12,
                        (const void*)(sizeof(float) * 4 * 2)
                    );
...
void RasterizerSceneGLES2::_render_geometry(RenderList::Element* p_element) {
    switch (p_element->instance->base_type) {
        case VS::INSTANCE_MESH: {
            ...
            s->attribs[VS::ARRAY_BONES].enabled &&
            s->attribs[VS::ARRAY_WEIGHTS].enabled) {
...
*/
        ...
        case VS::INSTANCE_MULTIMESH: {
            RasterizerStorageGLES2::MultiMesh* multi_mesh =
                static_cast<RasterizerStorageGLES2::MultiMesh*>(p_element->owner
                );
            RasterizerStorageGLES2::Surface* s =
                static_cast<RasterizerStorageGLES2::Surface*>(
                    p_element->geometry
            ...
            int stride = multi_mesh->color_floats
            ...
            for (int i = 0; i < amount; i++) {
                ...
                if (multi_mesh->custom_data_floats) {
                    if (multi_mesh->custom_data_format
                        == VS::MULTIMESH_CUSTOM_DATA_8BIT) {
                        uint8_t* custom_data =
                            (uint8_t*)(buffer + custom_data_ofs);
                        glVertexAttrib4f(
                            INSTANCE_ATTRIB_BASE + 4,
                            custom_data[0] / 255.0,
                            custom_data[1] / 255.0,
                            custom_data[2] / 255.0,
                            custom_data[3] / 255.0
        ...
        case VS::INSTANCE_IMMEDIATE: {
            const RasterizerStorageGLES2::Immediate* im =
                static_cast<const RasterizerStorageGLES2::Immediate*>(
                    p_element->geometry
                );
            ...
            for (const List<RasterizerStorageGLES2::Immediate::Chunk>::Element*
                     E = im->chunks.front();
                 E;
                 E = E->next()) {
...
void RasterizerSceneGLES2::_setup_light_type(
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES2::SHADOW_MODE_PCF_5,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::SHADOW_MODE_PCF_13,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::LIGHT_MODE_DIRECTIONAL,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::LIGHT_MODE_OMNI,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::LIGHT_MODE_SPOT,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::LIGHT_USE_PSSM2,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::LIGHT_USE_PSSM4,
        false
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES2::LIGHT_USE_PSSM_BLEND,
        false
    );
    ...
    switch (p_light->light_ptr->type) {
        case VS::LIGHT_DIRECTIONAL: {
            ...
            state.scene_shader.set_conditional(
                SceneShaderGLES2::LIGHT_USE_PSSM_BLEND,
                p_light->light_ptr->directional_blend_splits
            );
            if (!state.render_no_shadows && p_light->light_ptr->shadow) {
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::USE_SHADOW,
                    true
                );
                glActiveTexture(
        ...
        case VS::LIGHT_OMNI: {
            ...
            if (!state.render_no_shadows && shadow_atlas
                && p_light->light_ptr->shadow) {
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::USE_SHADOW,
                    true
                );
...
void RasterizerSceneGLES2::_setup_light(
    LightInstance* light,
    ShadowAtlas* shadow_atlas,
    ...
    float sign     = (light_ptr->negative && !accum_pass)
                       ? -1
    ...
    state.scene_shader.set_uniform(
        SceneShaderGLES2::SHADOW_COLOR,
        light_ptr->shadow_color
    );
    ...
    switch (light_ptr->type) {
        case VS::LIGHT_DIRECTIONAL: {
            // not using inverse for performance, view should be normalized
            // anyway
            Vector3 direction =
                p_view_transform.basis
                    .xform_inv(light->transform.basis.xform(Vector3(0, 0, -1)))
                    .normalized();
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_DIRECTION,
                direction
            );

            CameraMatrix matrices[4];

            if (!state.render_no_shadows && light_ptr->shadow
                ...
                for (int k = 0; k < shadow_count; k++) {
                    ...
                    if (light_ptr->directional_shadow_mode
                    ...
                    Transform modelview =
                        (p_view_transform.inverse()
                         * light->shadow_transform[k].transform)
                            .affine_inverse();
                    ...
                    CameraMatrix shadow_mtx = rectm * bias
                                            * light->shadow_transform[k].camera
                                            * modelview;
                ...
                //	state.scene_shader.set_uniform(SceneShaderGLES2::LIGHT_CLAMP,
                // light_clamp);
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::SHADOW_PIXEL_SIZE,
                    Size2(
                ...
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_SPLIT_OFFSETS,
                    split_offsets
                );
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_SHADOW_MATRIX,
                    matrices[0]
                );
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_SHADOW_MATRIX2,
                    matrices[1]
                );
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_SHADOW_MATRIX3,
                    matrices[2]
                );
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_SHADOW_MATRIX4,
                    matrices[3]
                );
            }
        } break;
        case VS::LIGHT_OMNI: {
            Vector3 position =
                p_view_transform.xform_inv(light->transform.origin);

            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_POSITION,
                position
            );

            float range = light_ptr->param[VS::LIGHT_PARAM_RANGE];
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_RANGE,
                range
            );

            float attenuation = light_ptr->param[VS::LIGHT_PARAM_ATTENUATION];
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_ATTENUATION,
                attenuation
            );

            if (!state.render_no_shadows && light_ptr->shadow && shadow_atlas
                && shadow_atlas->shadow_owners.has(light->self)) {
                ...
                ERR_BREAK(
                    shadow
                    >= (uint32_t)shadow_atlas->quadrants[quadrant].shadows.size(
                    )
                );
                ...
                if (light->light_ptr->omni_shadow_detail
                ...
                Transform proj =
                    (p_view_transform.inverse() * light->transform).inverse();
                ...
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::SHADOW_PIXEL_SIZE,
                    Size2(1.0 / shadow_atlas->size, 1.0 / shadow_atlas->size)
                );
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_SHADOW_MATRIX,
                    proj
        ...
        case VS::LIGHT_SPOT: {
            ...
            Vector3 direction =
                p_view_transform.inverse()
                    .basis
                    .xform(light->transform.basis.xform(Vector3(0, 0, -1)))
                    .normalized();
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_DIRECTION,
                direction
            ...
            float spot_attenuation =
                light_ptr->param[VS::LIGHT_PARAM_SPOT_ATTENUATION];
            ...
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_SPOT_ATTENUATION,
                spot_attenuation
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_SPOT_RANGE,
                spot_attenuation
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_SPOT_ANGLE,
                angle
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHT_RANGE,
            ...
            if (!state.render_no_shadows && light->light_ptr->shadow
                && shadow_atlas
                && shadow_atlas->shadow_owners.has(light->self)) {
                uint32_t key = shadow_atlas->shadow_owners[light->self];
                ...
                CameraMatrix shadow_matrix = rectm * bias
                                           * light->shadow_transform[0].camera
                                           * modelview;
...
void RasterizerSceneGLES2::_setup_refprobes(
    ...
    if (p_refprobe1) {
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_USE_BOX_PROJECT,
            p_refprobe1->probe_ptr->box_projection
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_BOX_EXTENTS,
            p_refprobe1->probe_ptr->extents
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_BOX_OFFSET,
            p_refprobe1->probe_ptr->origin_offset
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_EXTERIOR,
            !p_refprobe1->probe_ptr->interior
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_INTENSITY,
            p_refprobe1->probe_ptr->intensity
        );

        Color ambient;
        if (p_refprobe1->probe_ptr->interior) {
            ambient = p_refprobe1->probe_ptr->interior_ambient
                    * p_refprobe1->probe_ptr->interior_ambient_energy;
        ...
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_AMBIENT,
            ambient
        );

        Transform proj = (p_view_transform.inverse() * p_refprobe1->transform)
                             .affine_inverse();

        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE1_LOCAL_MATRIX,
            proj
        );
    }
    ...
    if (p_refprobe2) {
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_USE_BOX_PROJECT,
            p_refprobe2->probe_ptr->box_projection
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_BOX_EXTENTS,
            p_refprobe2->probe_ptr->extents
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_BOX_OFFSET,
            p_refprobe2->probe_ptr->origin_offset
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_EXTERIOR,
            !p_refprobe2->probe_ptr->interior
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_INTENSITY,
            p_refprobe2->probe_ptr->intensity
        );

        Color ambient;
        if (p_refprobe2->probe_ptr->interior) {
            ambient = p_refprobe2->probe_ptr->interior_ambient
                    * p_refprobe2->probe_ptr->interior_ambient_energy;
        ...
        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_AMBIENT,
            ambient
        );

        Transform proj = (p_view_transform.inverse() * p_refprobe2->transform)
                             .affine_inverse();

        state.scene_shader.set_uniform(
            SceneShaderGLES2::REFPROBE2_LOCAL_MATRIX,
            proj
        );
    }
}

void RasterizerSceneGLES2::_render_render_list(
    ...
    if (!p_shadow && p_base_env) {
        ...
        state.scene_shader.set_conditional(
            SceneShaderGLES2::USE_RADIANCE_MAP,
            true
        ); // since prev unshaded is false, this needs to be true if exists
    }
    ...
    if (p_env && !p_shadow && p_env->fog_enabled
        && (p_env->fog_depth_enabled || p_env->fog_height_enabled)) {
        state.scene_shader.set_conditional(
            SceneShaderGLES2::FOG_DEPTH_ENABLED,
            p_env->fog_depth_enabled
        );
        state.scene_shader.set_conditional(
            SceneShaderGLES2::FOG_HEIGHT_ENABLED,
            p_env->fog_height_enabled
        );
    ...
    for (int i = 0; i < p_element_count; i++) {
        ...
        *e->use_accum_ptr    = true; // set to accum for next time this is found
        ...
        if (!p_shadow && material->shader) {
            ...
            bool base_pass =
                !accum_pass && !unshaded; // conditions for a base pass

            if (base_pass != prev_base_pass) {
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::BASE_PASS,
                    base_pass
                );
                rebind         = true;
            ...
            if (!unshaded && e->light_index < RenderList::MAX_LIGHTS) {
                ...
                if ((e->instance->baked_light
                     && light->light_ptr->bake_mode == VS::LIGHT_BAKE_ALL)
                    || (e->instance->layer_mask & light->light_ptr->cull_mask)
                           == 0) {
                    light = nullptr; // Don't use this light, it is culled or
                                     // already included in the lightmap
                }
            }
            ...
            int blend_mode = p_alpha_pass ? material->shader->spatial.blend_mode
                                          : -1; // -1 no blend, no mix

            if (accum_pass) { // accum pass force pass
                blend_mode =
                    RasterizerStorageGLES2::Shader::Spatial::BLEND_MODE_ADD;
                if (light && light->light_ptr->negative) {
                    blend_mode =
                        RasterizerStorageGLES2::Shader::Spatial::BLEND_MODE_SUB;
                }
            }
            ...
            bool vertex_lit = (material->shader->spatial.uses_vertex_lighting
                               || storage->config.force_vertex_shading)
                           && ((!unshaded && light) || using_fog
                           ); // fog forces vertex lighting because it still
                              // applies even if unshaded or no fog

            if (vertex_lit != prev_vertex_lit) {
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::USE_VERTEX_LIGHTING,
                    vertex_lit
                );
            ...
            if (!unshaded && !accum_pass
                && e->refprobe_0_index != RenderList::MAX_REFLECTION_PROBES) {
                ERR_FAIL_INDEX(e->refprobe_0_index, reflection_probe_count);
                refprobe_1 = reflection_probe_instances[e->refprobe_0_index];
            }
            if (!unshaded && !accum_pass
                && e->refprobe_1_index != RenderList::MAX_REFLECTION_PROBES) {
                ERR_FAIL_INDEX(e->refprobe_1_index, reflection_probe_count);
                refprobe_2 = reflection_probe_instances[e->refprobe_1_index];
            }

            if (refprobe_1 != prev_refprobe_1
                || refprobe_2 != prev_refprobe_2) {
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::USE_REFLECTION_PROBE1,
                    refprobe_1 != nullptr
                );
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::USE_REFLECTION_PROBE2,
                    refprobe_2 != nullptr
                );
            ...
            use_lightmap_capture = !unshaded && !accum_pass
                                && !e->instance->lightmap_capture_data.empty();

            if (use_lightmap_capture != prev_use_lightmap_capture) {
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::USE_LIGHTMAP_CAPTURE,
                    use_lightmap_capture
                );
                rebind = true;
            }
            ...
            if (!unshaded && !accum_pass && e->instance->lightmap.is_valid()) {
                ...
                lightmap_energy = 1.0;
                if (lightmap) {
                    RasterizerStorageGLES2::LightmapCapture* capture =
        ...
        if (depth_prepass != prev_depth_prepass) {
            state.scene_shader.set_conditional(
                SceneShaderGLES2::USE_DEPTH_PREPASS,
                depth_prepass
            );
        ...
        if (instancing != prev_instancing) {
            state.scene_shader.set_conditional(
                SceneShaderGLES2::USE_INSTANCING,
                instancing
            );
            rebind = true;
        }
        ...
        RasterizerStorageGLES2::Skeleton* skeleton =
            storage->skeleton_owner.getornull(e->instance->skeleton);

        if (skeleton != prev_skeleton) {
            if ((prev_skeleton == nullptr) != (skeleton == nullptr)) {
                if (skeleton) {
                    state.scene_shader.set_conditional(
                        SceneShaderGLES2::USE_SKELETON,
                        true
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES2::USE_SKELETON_SOFTWARE,
                        storage->config.use_skeleton_software
                    );
                } else {
                    state.scene_shader.set_conditional(
                        SceneShaderGLES2::USE_SKELETON,
        ...
        if (rebind || material != prev_material) {
            ...
            shader_rebind = _setup_material(
                material,
                p_alpha_pass,
                Size2i(skeleton ? skeleton->size * 3 : 0, 0)
            );
            if (shader_rebind) {
        ...
        _set_cull(
            e->front_facing,
            material->shader->spatial.cull_mode
                == RasterizerStorageGLES2::Shader::Spatial::CULL_MODE_DISABLED,
            p_reverse_cull
        );
        ...
        if (i == 0 || shader_rebind) { // first time must rebind
            ...
            if (p_shadow) {
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_BIAS,
                    p_shadow_bias
                );
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHT_NORMAL_BIAS,
                    p_shadow_normal_bias
                );
                if (state.shadow_is_dual_parabolloid) {
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::SHADOW_DUAL_PARABOLOID_RENDER_SIDE,
                        state.dual_parbolloid_direction
                    );
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::SHADOW_DUAL_PARABOLOID_RENDER_ZFAR,
                        state.dual_parbolloid_zfar
                    );
                }
            } else {
                if (use_radiance_map) {
                    if (p_env) {
                        Transform sky_orientation(
                            p_env->sky_orientation,
                            Vector3(0.0, 0.0, 0.0)
                        );
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::RADIANCE_INVERSE_XFORM,
                            sky_orientation.affine_inverse() * p_view_transform
                        );
                    } else {
                ...
                if (p_env) {
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::BG_ENERGY,
                        p_env->bg_energy
                    );
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::BG_COLOR,
                        p_env->bg_color
                    );
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::AMBIENT_SKY_CONTRIBUTION,
                        p_env->ambient_sky_contribution
                    );

                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::AMBIENT_COLOR,
                        p_env->ambient_color
                    );
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::AMBIENT_ENERGY,
                        p_env->ambient_energy
                    );
                ...
                if (using_fog) {
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::FOG_COLOR_BASE,
                        p_env->fog_color
                    );
                    ...
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::FOG_SUN_COLOR_AMOUNT,
                        sun_color_amount
                    );
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::FOG_TRANSMIT_ENABLED,
                        p_env->fog_transmit_enabled
                    );
                    state.scene_shader.set_uniform(
                        SceneShaderGLES2::FOG_TRANSMIT_CURVE,
                        p_env->fog_transmit_curve
                    );

                    if (p_env->fog_depth_enabled) {
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_DEPTH_BEGIN,
                            p_env->fog_depth_begin
                        );
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_DEPTH_CURVE,
                            p_env->fog_depth_curve
                        );
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_MAX_DISTANCE,
                            fog_max_distance
                        );
                    }
                    ...
                    if (p_env->fog_height_enabled) {
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_HEIGHT_MIN,
                            p_env->fog_height_min
                        );
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_HEIGHT_MAX,
                            p_env->fog_height_max
                        );
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_HEIGHT_MAX,
                            p_env->fog_height_max
                        ...
                        state.scene_shader.set_uniform(
                            SceneShaderGLES2::FOG_HEIGHT_CURVE,
                            p_env->fog_height_curve
                        );
                    }
                }
            }
            ...
            state.scene_shader.set_uniform(
                SceneShaderGLES2::CAMERA_MATRIX,
                p_view_transform
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::CAMERA_INVERSE_MATRIX,
                view_transform_inverse
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::PROJECTION_MATRIX,
                p_projection
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::PROJECTION_INVERSE_MATRIX,
                projection_inverse
            );

            state.scene_shader.set_uniform(
                SceneShaderGLES2::TIME,
                storage->frame.time[0]
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES2::VIEW_INDEX,
                p_eye == 2 ? 1 : 0
            );

            state.scene_shader.set_uniform(
                SceneShaderGLES2::VIEWPORT_SIZE,
                viewport_size
            );

            state.scene_shader.set_uniform(
                SceneShaderGLES2::SCREEN_PIXEL_SIZE,
                screen_pixel_size
            );
        }
        ...
        if (rebind_lightmap && lightmap) {
            state.scene_shader.set_uniform(
                SceneShaderGLES2::LIGHTMAP_ENERGY,
                lightmap_energy
            );
            if (storage->config.use_lightmap_filter_bicubic) {
                state.scene_shader.set_uniform(
                    SceneShaderGLES2::LIGHTMAP_TEXTURE_SIZE,
        ...
        state.scene_shader.set_uniform(
            SceneShaderGLES2::WORLD_TRANSFORM,
            e->instance->transform
        );

        if (use_lightmap_capture) { // this is per instance, must be set always
                                    // if present
            glUniform4fv(
                state.scene_shader.get_uniform_location(
                    SceneShaderGLES2::LIGHTMAP_CAPTURES
                ),
                12,
...
void RasterizerSceneGLES2::_draw_sky(
    ...
    storage->shaders.copy.set_conditional(
        CopyShaderGLES2::USE_ASYM_PANO,
        asymmetrical
    );
    storage->shaders.copy.set_conditional(
        CopyShaderGLES2::USE_PANORAMA,
        !asymmetrical
    );
    storage->shaders.copy.set_conditional(
        CopyShaderGLES2::USE_MULTIPLIER,
        true
    );
    storage->shaders.copy.set_conditional(CopyShaderGLES2::USE_CUBEMAP, false);
    storage->shaders.copy.set_conditional(
        CopyShaderGLES2::USE_COPY_SECTION,
        false
    );
    storage->shaders.copy.set_conditional(
        CopyShaderGLES2::USE_CUSTOM_ALPHA,
        false
    );
    if (storage->frame.current_rt) {
        storage->shaders.copy.set_conditional(
            CopyShaderGLES2::OUTPUT_LINEAR,
            storage->frame.current_rt
    ...
    if (asymmetrical) {
        ...
        /// better here.
        storage->shaders.copy.set_uniform(
            CopyShaderGLES2::PANO_TRANSFORM,
            p_transform
        );
    }
...
void RasterizerSceneGLES2::_post_process(
    Environment* env,
    ...
    bool use_post_process =
        env
        && !storage->frame.current_rt
                ->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT];
    use_post_process = use_post_process && storage->frame.current_rt->width >= 4
                    && storage->frame.current_rt->height >= 4;
    use_post_process =
        use_post_process && storage->frame.current_rt->mip_maps_allocated;

    if (env) {
        use_post_process =
            use_post_process
            && (env->adjustments_enabled || env->glow_enabled
                || env->dof_blur_far_enabled || env->dof_blur_near_enabled);
    }
    use_post_process = use_post_process || storage->frame.current_rt->use_fxaa;

    // If using multisample buffer, resolve to post_process_effect buffer or to
    // front buffer
    if (storage->frame.current_rt
        && storage->frame.current_rt->multisample_active) {
        GLuint next_buffer;
        if (use_post_process) {
            ...
            // set next_buffer to front buffer so multisample blit can happen if
            // needed
        ...
        glBindFramebuffer(
            GL_READ_FRAMEBUFFER,
            storage->frame.current_rt->multisample_fbo
        );
        ...
        // In GLES2 Android Blit is not available, so just copy color texture
        // manually
        _copy_texture_to_buffer(
            storage->frame.current_rt->multisample_color,
            next_buffer
        );
#else
        // TODO: any other platform not supported? this will fail.. maybe we
        // should just call _copy_texture_to_buffer here as well?
        (void)next_buffer; // Silence warning as it's unused.
#endif
    } else if (use_post_process) {
        if (storage->frame.current_rt->external.fbo != 0) {
            _copy_texture_to_buffer(
                storage->frame.current_rt->external.color,
                storage->frame.current_rt->mip_maps[0].sizes[0].fbo
            );
        } else {
            _copy_texture_to_buffer(
    ...
    // Order of operation
    // 1) DOF Blur (first blur, then copy to buffer applying the blur) //only on
    // desktop 2) FXAA 3) Bloom (Glow) //only on desktop 4) Adjustments
    ...
    if (env && env->dof_blur_far_enabled) {
        ...
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::USE_ORTHOGONAL_PROJECTION,
            p_cam_projection.is_orthogonal()
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::DOF_FAR_BLUR,
            true
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::DOF_QUALITY_LOW,
            env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_LOW
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::DOF_QUALITY_MEDIUM,
            env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_MEDIUM
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::DOF_QUALITY_HIGH,
        ...
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::DOF_BEGIN,
            env->dof_blur_far_distance
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::DOF_END,
            env->dof_blur_far_distance + env->dof_blur_far_transition
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::DOF_DIR,
            Vector2(1, 0)
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::DOF_RADIUS,
            radius
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::PIXEL_SIZE,
            Vector2(1.0 / vp_w, 1.0 / vp_h)
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::CAMERA_Z_NEAR,
            p_cam_projection.get_z_near()
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES2::CAMERA_Z_FAR,
        ...
        if (storage->frame.current_rt->mip_maps[0].color) {
            glBindTexture(
                GL_TEXTURE_2D,
                storage->frame.current_rt->mip_maps[0].color
            );
        } else {
            glBindTexture(
                GL_TEXTURE_2D,
                storage->frame.current_rt->mip_maps[0].sizes[0].color
            );
        }
    ...
    if (env && env->dof_blur_near_enabled) {
        ...
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::DOF_NEAR_BLUR,
            true
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES2::DOF_NEAR_FIRST_TAP,
            true
        );
    ...
    if (env && env->glow_enabled) {
        for (int i = 0; i < VS::MAX_GLOW_LEVELS; i++) {
            if (env->glow_levels & (1 << i)) {
                if (i >= storage->frame.current_rt->mip_maps[1].sizes.size()) {
                    max_glow_level =
                        storage->frame.current_rt->mip_maps[1].sizes.size() - 1;
        ...
        // If max_texture_image_units is 8, our max glow level is 5, which
        // allows 6 layers of glow
        max_glow_level =
            MIN(max_glow_level, storage->config.max_texture_image_units - 3);
        ...
        for (int i = 0; i < (max_glow_level + 1); i++) {
            ...
            if (i == 0) {
                state.effect_blur_shader.set_conditional(
                    EffectBlurShaderGLES2::GLOW_FIRST_PASS,
                    true
                );
            }

            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES2::GLOW_GAUSSIAN_HORIZONTAL,
                true
            );
            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES2::USE_GLOW_HIGH_QUALITY,
            ...
            state.effect_blur_shader.set_uniform(
                EffectBlurShaderGLES2::LOD,
                storage->frame.current_rt->mip_maps[0].color ? float(i) : 0.0
            );
            state.effect_blur_shader.set_uniform(
                EffectBlurShaderGLES2::GLOW_STRENGTH,
                env->glow_strength
            );
            state.effect_blur_shader.set_uniform(
                EffectBlurShaderGLES2::LUMINANCE_CAP,
            ...
            if (storage->frame.current_rt->mip_maps[0].color) {
                ...
                glBindTexture(
                    GL_TEXTURE_2D,
                    storage->frame.current_rt->mip_maps[0].sizes[i].color
                );
            }

            if (i == 0) {
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES2::GLOW_BLOOM,
                    env->glow_bloom
                );
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES2::GLOW_HDR_THRESHOLD,
                    env->glow_hdr_bleed_threshold
                );
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES2::GLOW_HDR_SCALE,
            ...
            glBindFramebuffer(
                GL_FRAMEBUFFER,
                storage->frame.current_rt->mip_maps[1].sizes[i].fbo
            );
            storage->_copy_screen();
            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES2::GLOW_GAUSSIAN_HORIZONTAL,
            ...
            // vertical pass
            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES2::GLOW_GAUSSIAN_VERTICAL,
                true
            );
            state.effect_blur_shader.bind();
            state.effect_blur_shader.set_uniform(
            ...
            if (storage->frame.current_rt->mip_maps[0].color) {
                glBindTexture(
                    GL_TEXTURE_2D,
                    storage->frame.current_rt->mip_maps[1].color
                );
            } else {
                glBindTexture(
                    GL_TEXTURE_2D,
                    storage->frame.current_rt->mip_maps[1].sizes[i].color
                );
            }

            glBindFramebuffer(
                GL_FRAMEBUFFER,
                storage->frame.current_rt->mip_maps[0].sizes[i + 1].fbo
    ...
    state.tonemap_shader.set_conditional(
        TonemapShaderGLES2::USE_FXAA,
        storage->frame.current_rt->use_fxaa
    );

    if (env) {
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES2::USE_GLOW_FILTER_BICUBIC,
        ...
        if (max_glow_level >= 0) {
            if (storage->frame.current_rt->mip_maps[0].color) {
                for (int i = 0; i < (max_glow_level + 1); i++) {
                    if (glow_mask & (1 << i)) {
                        if (i == 0) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL1,
                                true
                            );
                        }
                        if (i == 1) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL2,
                                true
                            );
                        }
                        if (i == 2) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL3,
                                true
                            );
                        }
                        if (i == 3) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL4,
                                true
                            );
                        }
                        if (i == 4) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL5,
                                true
                            );
                        }
                        if (i == 5) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL6,
                                true
                            );
                        }
                        if (i == 6) {
                            state.tonemap_shader.set_conditional(
                                TonemapShaderGLES2::USE_GLOW_LEVEL7,
                                true
                            );
                        }
                    }
                ...
                state.tonemap_shader.set_conditional(
                    TonemapShaderGLES2::USE_MULTI_TEXTURE_GLOW,
                    true
                );
                ...
                for (int i = 0; i < (max_glow_level + 1); i++) {
                    if (glow_mask & (1 << i)) {
                        ...
                        glBindTexture(
                            GL_TEXTURE_2D,
                            storage->frame.current_rt->mip_maps[0]
                                .sizes[i + 1]
                                .color
                        );
            ...
            state.tonemap_shader.set_conditional(
                TonemapShaderGLES2::USE_GLOW_SCREEN,
                env->glow_blend_mode == VS::GLOW_BLEND_MODE_SCREEN
            );
            state.tonemap_shader.set_conditional(
                TonemapShaderGLES2::USE_GLOW_SOFTLIGHT,
                env->glow_blend_mode == VS::GLOW_BLEND_MODE_SOFTLIGHT
            );
            state.tonemap_shader.set_conditional(
                TonemapShaderGLES2::USE_GLOW_REPLACE,
    ...
    if (env && env->adjustments_enabled) {
        ...
        if (tex) {
            state.tonemap_shader.set_conditional(
                TonemapShaderGLES2::USE_COLOR_CORRECTION,
                true
            );
            glActiveTexture(GL_TEXTURE1);
            glBindTexture(tex->target, tex->tex_id);
    ...
    if (env) {
        if (max_glow_level >= 0) {
            state.tonemap_shader.set_uniform(
                TonemapShaderGLES2::GLOW_INTENSITY,
                env->glow_intensity
            );
            int ss[2] = {
                storage->frame.current_rt->width,
            ...
            glUniform2iv(
                state.tonemap_shader.get_uniform(
                    TonemapShaderGLES2::GLOW_TEXTURE_SIZE
                ),
                1,
                ss
            );
...
void RasterizerSceneGLES2::render_scene(
    ...
    if (p_light_cull_count) {
        ...
        render_light_instance_count =
            MIN(RenderList::MAX_LIGHTS, p_light_cull_count);
        render_light_instances = (LightInstance**)alloca(
            sizeof(LightInstance*) * render_light_instance_count
        );
        render_directional_lights = 0;

        // doing this because directional lights are at the end, put them at the
        // beginning
        int index = 0;
        for (int i = render_light_instance_count - 1; i >= 0; i--) {
            ...
            if (light->light_ptr->type == VS::LIGHT_DIRECTIONAL) {
                ...
                // as going in reverse, directional lights are always first
                // anyway
            }
        ...
        // for fog transmission, we want some kind of consistent ordering of
        // lights add any more conditions here in which we need consistent light
        // ordering (perhaps we always should have it, but don't know yet)
        if (env && env->fog_transmit_enabled) {
            ...
            int num_lights_to_sort =
                render_light_instance_count - render_directional_lights;

            if (num_lights_to_sort) {
                SortArray<LightInstance*, _LightSort> sorter;
                sorter.sort(
                    &render_light_instances[render_directional_lights],
                    num_lights_to_sort
                );
                // rejig indices
                for (int i = render_directional_lights;
                     i < render_light_instance_count;
                     i++) {
    ...
    if (p_reflection_probe_cull_count) {
        reflection_probe_instances = (ReflectionProbeInstance**)alloca(
            sizeof(ReflectionProbeInstance*) * p_reflection_probe_cull_count
        );
    ...
    if (env && env->bg_mode == VS::ENV_BG_CANVAS) {
        ...
        // TODO: When GLES2 renders to current_rt->mip_maps[], this copy will no
        // longer be needed
        _copy_texture_to_buffer(
            storage->frame.current_rt->color,
            storage->frame.current_rt->copy_screen_effect.fbo
        );
    }
    ...
    if (!env || env->bg_mode != VS::ENV_BG_KEEP) {
        if (storage->frame.current_rt
            ...
            // leave clear_color in sRGB as most of the render pipeline remains
            // in sRGB color space until writing out to frag_color
        } else {
            glClearColor(
                clear_color.r,
                clear_color.g,
    ...
    state.default_ambient =
        Color(clear_color.r, clear_color.g, clear_color.b, 1.0);
        ...
        switch (env->bg_mode) {
            ...
            case VS::ENV_BG_CAMERA_FEED: {
                if (feed.is_valid() && (feed->get_base_width() > 0)
                    ...
                    storage->shaders.copy.set_conditional(
                        CopyShaderGLES2::USE_NO_ALPHA,
                        true
                    );
                    storage->shaders.copy.set_conditional(
                        CopyShaderGLES2::USE_DISPLAY_TRANSFORM,
                        true
                    );
                    ...
                    if (feed->get_datatype() == CameraFeed::FEED_RGB) {
                        ...
                        storage->shaders.copy.set_conditional(
                            CopyShaderGLES2::SEP_CBCR_TEXTURE,
                            true
                        );
                        storage->shaders.copy.set_conditional(
                            CopyShaderGLES2::YCBCR_TO_RGB,
                    ...
                    storage->shaders.copy.bind();
                    storage->shaders.copy.set_uniform(
                        CopyShaderGLES2::DISPLAY_TRANSFORM,
                        feed->get_transform()
                    );
    ...
    if (probe_interior) {
        ...
        state.default_ambient =
            Color(0, 0, 0, 1); // black as default ambient for interior
        state.default_bg =
            Color(0, 0, 0, 1); // black as default background for interior
    }

    // make sure we set our output mode correctly
    if (storage->frame.current_rt) {
        state.scene_shader.set_conditional(
            SceneShaderGLES2::OUTPUT_LINEAR,
            storage->frame.current_rt
                ->flags[RasterizerStorage::RENDER_TARGET_KEEP_3D_LINEAR]
        );
    } else {
    ...
    if (storage->frame.current_rt && state.used_screen_texture) {
        ...
        if (storage->frame.current_rt->multisample_active) {
            ...
            // In GLES2 AndroidBlit is not available, so just copy color texture
            // manually
            _copy_texture_to_buffer(
                storage->frame.current_rt->multisample_color,
                storage->frame.current_rt->fbo
...
void RasterizerSceneGLES2::render_shadow(
    RID p_light,
    RID p_shadow_atlas,
    int p_pass,
    InstanceBase** p_cull_result,
    ...
    if (light->type == VS::LIGHT_DIRECTIONAL) {
        ...
        if (directional_shadow.light_count == 1) {
            light_instance->directional_rect =
                Rect2(0, 0, directional_shadow.size, directional_shadow.size);
        ...
        if (light->type == VS::LIGHT_OMNI) {
            ...
            if (light->omni_shadow_mode == VS::LIGHT_OMNI_SHADOW_CUBE
                && storage->config.support_shadow_cubemaps) {
                ...
                state.scene_shader.set_conditional(
                    SceneShaderGLES2::RENDER_DEPTH_DUAL_PARABOLOID,
                    true
                );
            }
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES2::OUTPUT_LINEAR,
        false
    ); // just in case, should be false already
    ...
    if (light->type == VS::LIGHT_OMNI
        ...
        for (int i = 0; i < 2; i++) {
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES2::Z_FLIP,
                i == 1
            );
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES2::Z_NEAR,
                light_projection.get_z_near()
            );
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES2::Z_FAR,
                light_projection.get_z_far()
            );
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES2::BIAS,
...
void RasterizerSceneGLES2::set_debug_draw_mode(
...
void RasterizerSceneGLES2::initialize() {
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES2::USE_RGBA_SHADOWS,
...

    {
        ...
        if (directional_shadow.size > storage->config.max_viewport_dimensions[0]
            || directional_shadow.size
                   > storage->config.max_viewport_dimensions[1]) {
            WARN_PRINT(
                "Cannot set directional shadow size larger than maximum "
                "hardware supported size of ("
                + itos(storage->config.max_viewport_dimensions[0]) + ", "
                + itos(storage->config.max_viewport_dimensions[1])
                + "). Setting size to maximum."
