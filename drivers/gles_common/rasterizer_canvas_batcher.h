...
// data / call funcs in the derived rasterizers from the base without writing
// and maintaining a boatload of virtual functions. In addition it assures that
// vtable will not be used and the function calls can be optimized, because it
// gives compile time static polymorphism.

// These macros makes it simpler and less verbose to define (and redefine) the
// inline functions template preamble
...
public:
    ...
    // pod versions of vector and color and RID, need to be 32 bit for vertex
    // format
    struct BatchVector2 {
        float x, y;
    ...
    // be needed with custom shaders in order to have the data accessible in the
    // shader.
    ...
    // last resort, specially for custom shader, we put everything possible into
    // a huge FVF not very efficient, but better than no batching at all.
    struct BatchVertexLarge : public BatchVertexModulated {
        // must be pod
        BatchTransform transform;
    };

    // Batch should be as small as possible, and ideally nicely aligned (is 32
    // bytes at the moment)
    struct Batch {
        ...
        // the color if a handled batch, or the parent item if a default batch,
        // so we can reference the correct originating command
        union {
            BatchColor color;
    ...
    struct BItemJoined {
        ...
        // note the z_index  may only be correct for the first of the joined
        // item references this has implications for light culling with z ranged
        // lights.
        int16_t z_index;
        ...
        bool is_single_item() const {
            return (num_item_refs == 1);
        }

        bool use_attrib_transform() const {
            return flags & RasterizerStorageCommon::USE_LARGE_FVF;
        }
    };
    ...
    struct BLightRegion {
        ...
        bool too_many_lights; // we can only do light region optimization if
                              // there are 64 or less lights
    };
    ...
    struct BatchData {
        ...
        // extra data which can be stored during prefilling, for later
        // translation to larger FVFs
        RasterizerArray<float> light_angles;
        RasterizerArray<BatchColor>
            vertex_colors; // these aren't usually used, but are for polys
        ...
        RasterizerArray<Batch>
            batches_temp; // used for translating to colored vertex batches
        RasterizerArray_non_pod<BatchTex>
            batch_textures; // the only reason this is non-POD is because of
                            // RIDs
        ...
        // if the shader is using MODULATE, we prevent baking color so the
        // final_modulate can be read in the shader. if the shader is reading
        // VERTEX, we prevent baking vertex positions with extra matrices etc to
        // prevent the read position being incorrect. These flags are defined in
        // RasterizerStorageCommon::BatchFlags
        ...
        // diagnose this frame, every nTh frame when settings_diagnose_frame is
        // on
        bool diagnose_frame;
        String frame_string;
        ...
        // whether to join items across z_indices - this can interfere with z
        // ranged lights, so has to be disabled in some circumstances
        bool join_across_z_indices;

        // global settings
        bool settings_use_batching; // the current use_batching (affected by
                                    // flash)
        bool settings_use_batching_original_choice; // the choice entered in
                                                    // project settings
        bool settings_flash_batching; // for regression testing, flash between
                                      // non-batched and batched renderer
        bool settings_diagnose_frame; // print out batches to help optimize /
                                      // regression test
    ...
    struct FillState {
        ...
        // using flags allows quicker rejection of sequences with different
        // batch types
        ...
        bool extra_matrix_sent; // whether sent on this item (in which case
                                // software transform can't be used untl end of
                                // item)
        int transform_extra_command_number_p1;   // plus one to allow fast
    ...
    struct RenderItemState {
        ...
        // we need some logic to prevent joining items that have vastly
        // different batch types these are defined in
        // RasterizerStorageCommon::BatchTypeFlags
...
private:
    // curiously recurring template pattern - allows access to functions in the
    // DERIVED class this is kind of like using virtual functions but more
    // efficient as they are resolved at compile time
...
protected:
    ...
    // joining logic
    bool _disallow_item_join_if_batch_types_too_different(
        RenderItemState& r_ris,
        uint32_t btf_allowed
    );
    bool _detect_item_batch_break(
        RenderItemState& r_ris,
        RasterizerCanvas::Item* p_ci,
        bool& r_batch_break
    );

    // drives the loop filling batches and flushing
    void render_joined_item_commands(
        const BItemJoined& p_bij,
        RasterizerCanvas::Item* p_current_clip,
        bool& r_reclip,
        typename T_STORAGE::Material* p_material,
        bool p_lit,
        const RenderItemState& p_ris
    );

private:
    // flush once full or end of joined item
    void flush_render_batches(
        RasterizerCanvas::Item* p_first_item,
        RasterizerCanvas::Item* p_current_clip,
        bool& r_reclip,
        typename T_STORAGE::Material* p_material,
        uint32_t p_sequence_batch_type_flags
    );

    // a single joined item can contain multiple itemrefs, and thus create lots
    // of batches
    bool prefill_joined_item(
        FillState& r_fill_state,
        int& r_command_start,
        RasterizerCanvas::Item* p_item,
        RasterizerCanvas::Item* p_current_clip,
        bool& r_reclip,
        typename T_STORAGE::Material* p_material
    );
    ...
    // default batch is an 'unhandled' legacy type batch that will be drawn with
    // the legacy path, all other batches are accelerated.
    void _prefill_default_batch(
        FillState& r_fill_state,
        int p_command_num,
        const RasterizerCanvas::Item& p_item
    );

    // accelerated batches
    bool _prefill_line(
        RasterizerCanvas::Item::CommandLine* p_line,
        FillState& r_fill_state,
        int& r_command_start,
        int command_num,
        int command_count,
        RasterizerCanvas::Item* p_item,
        bool multiply_final_modulate
    );
    template <bool SEND_LIGHT_ANGLES>
    bool _prefill_ninepatch(
        RasterizerCanvas::Item::CommandNinePatch* p_np,
        FillState& r_fill_state,
        int& r_command_start,
        int command_num,
        int command_count,
    ...
    template <bool SEND_LIGHT_ANGLES>
    bool _prefill_polygon(
        RasterizerCanvas::Item::CommandPolygon* p_poly,
        FillState& r_fill_state,
        int& r_command_start,
        int command_num,
        int command_count,
    ...
    template <bool SEND_LIGHT_ANGLES>
    bool _prefill_rect(
        RasterizerCanvas::Item::CommandRect* rect,
        FillState& r_fill_state,
        int& r_command_start,
        int command_num,
        int command_count,
        RasterizerCanvas::Item::Command* const* commands,
        RasterizerCanvas::Item* p_item,
        bool multiply_final_modulate
    );

    // dealing with textures
    int _batch_find_or_create_tex(
        const RID& p_texture,
        const RID& p_normal,
        bool p_tile,
        int p_previous_match
    );

protected:
    // legacy support for non batched mode
    void _legacy_canvas_item_render_commands(
        RasterizerCanvas::Item* p_item,
        RasterizerCanvas::Item* p_current_clip,
        bool& r_reclip,
        typename T_STORAGE::Material* p_material
    ...
    // light scissoring
    bool _light_scissor_begin(
        const Rect2& p_item_rect,
        const Transform2D& p_light_xform,
        const Rect2& p_light_rect
    ) const;
    bool _light_find_intersection(
        const Rect2& p_item_rect,
        const Transform2D& p_light_xform,
        const Rect2& p_light_rect,
        Rect2& r_cliprect
...
private:
    ...
    template <
        class BATCH_VERTEX_TYPE,
        bool INCLUDE_LIGHT_ANGLES,
        bool INCLUDE_MODULATE,
        bool INCLUDE_LARGE>
    void _translate_batches_to_larger_FVF(uint32_t p_sequence_batch_type_flags);

protected:
    // accessory funcs
    void _software_transform_vertex(BatchVector2& r_v, const Transform2D& p_tr)
        const;
    void _software_transform_vertex(Vector2& r_v, const Transform2D& p_tr)
        const;
    ...
    TransformMode _find_transform_mode(const Transform2D& p_tr) const {
        ...
        if ((p_tr.elements[0].x == 1.0f) && (p_tr.elements[0].y == 0.0f)
            && (p_tr.elements[1].x == 0.0f) && (p_tr.elements[1].y == 1.0f)) {
            return TM_TRANSLATE;
        }
    ...
    bool _software_skin_poly(
        RasterizerCanvas::Item::CommandPolygon* p_poly,
        RasterizerCanvas::Item* p_item,
        BatchVertex* bvs,
        BatchColor* vertex_colors,
        const FillState& p_fill_state,
        const BatchColor* p_precalced_colors
    );

    typename T_STORAGE::Texture* _get_canvas_texture(const RID& p_texture
    ) const {
        if (p_texture.is_valid()) {
            typename T_STORAGE::Texture* texture =
                get_storage()->texture_owner.getornull(p_texture);
                ...
                if (texture->proxy) {
                    ...
                    while (texture->proxy) {
                        ...
                        ERR_FAIL_COND_V_MSG(
                            count == 16,
                            nullptr,
                            "Texture proxy infinite loop detected."
                        );
                    }
                }
...
protected:
    // no need to compile these in in release, they are unneeded outside the
    // editor and only add to executable size
...
PREAMBLE(void)::batch_canvas_begin() {
    // diagnose_frame?
    bdata.frame_string = ""; // just in case, always set this as we don't want a
                             // string leak in release...
#if defined(TOOLS_ENABLED) && defined(DEBUG_ENABLED)
...
PREAMBLE(void)::batch_canvas_end() {
    ...
    if (bdata.diagnose_frame) {
        ...
        if (bdata.stats_items_sorted) {
            bdata.frame_string +=
                "\titems reordered: " + itos(bdata.stats_items_sorted) + "\n";
        }
        if (bdata.stats_light_items_joined) {
            bdata.frame_string += "\tlight items joined: "
                                + itos(bdata.stats_light_items_joined) + "\n";
        }
...
PREAMBLE(void)::batch_canvas_render_items_begin(
    const Color& p_modulate,
    RasterizerCanvas::Light* p_light,
    const Transform2D& p_base_transform
) {
    // if we are debugging, flash each frame between batching renderer and old
    // version to compare for regressions
    ...
    // whether to join across z indices depends on whether there are z ranged
    // lights. joined z_index items can be wrongly classified with z ranged
    // lights.
    ...
    while (p_light) {
        ...
        if ((p_light->z_min != VS::CANVAS_ITEM_Z_MIN)
            || (p_light->z_max != VS::CANVAS_ITEM_Z_MAX)) {
            // prevent joining across z indices. This would have caused visual
            // regressions
...
PREAMBLE(void)::batch_canvas_render_items_end() {
    ...
    // batching render is deferred until after going through all the z_indices,
    // joining all the items
    get_this()->canvas_render_items_implementation(
        nullptr,
        0,
        _render_item_state.item_group_modulate,
...
PREAMBLE(void)::batch_canvas_render_items(
    RasterizerCanvas::Item* p_item_list,
    int p_z,
    const Color& p_modulate,
    RasterizerCanvas::Light* p_light,
    ...
    // stage 1 : join similar items, so that their state changes are not
    // repeated, and commands from joined items can be batched together
    if (bdata.settings_use_batching) {
        record_items(p_item_list, p_z);
        return;
    }

    // only legacy renders at this stage, batched renderer doesn't render until
...
// EXCEPT IN THE CASE OF SINGLE RECTS (and this may well not occur, check the
// logic in prefill_join_item TYPE_RECT) but can occur where transform commands
// have been sent during hardware batch
PREAMBLE(void)::_prefill_default_batch(
    FillState& r_fill_state,
    int p_command_num,
    const RasterizerCanvas::Item& p_item
) {
    if (r_fill_state.curr_batch->type == RasterizerStorageCommon::BT_DEFAULT) {
        ...
        if (!r_fill_state.transform_extra_command_number_p1) {
            ...
            // Note this is getting hit, needs investigation as to whether this
            // is a bug or a false flag
            // DEV_CHECK_ONCE(r_fill_state.curr_batch->num_commands <=
            // p_command_num);
        } else {
#if defined(TOOLS_ENABLED) && defined(DEBUG_ENABLED)
            if (r_fill_state.transform_extra_command_number_p1
                != p_command_num) {
                WARN_PRINT_ONCE(
                    "_prefill_default_batch : "
                    "transform_extra_command_number_p1 != p_command_num"
                );
            }
#endif
            // if the first member of the batch is a transform we have to be
            // careful
            if (!r_fill_state.curr_batch->num_commands) {
                // there can be leading useless extra transforms (sometimes
                // happens with debug collision polys) we need to rejig the
                // first_command for the first useful transform
                r_fill_state.curr_batch->first_command +=
                    r_fill_state.transform_extra_command_number_p1 - 1;
            }

            // we do have a pending extra transform command to flush
            // either the extra transform is in the prior command, or not, in
            // which case we need 2 batches
        ...
        if (r_fill_state.transform_extra_command_number_p1) {
            // get which command the extra is in, and blank all the records as
            // it no longer is stored CPU side
            ...
            // reset the original transform if we are going back to software
            // mode, because the extra is now done on the GPU... (any subsequent
            // extras are sent directly to the GPU, no deferring)
            ...
            // this is more efficient than having an extra batch especially for
            // the extra
...
PREAMBLE(int)::_batch_find_or_create_tex(
    const RID& p_texture,
    const RID& p_normal,
    bool p_tile,
    int p_previous_match
) {
    // optimization .. in 99% cases the last matched value will be the same, so
    // no need to traverse the list
    if (p_previous_match
        > 0) // if it is zero, it will get hit first in the linear search anyway
    {
        const BatchTex& batch_texture = bdata.batch_textures[p_previous_match];

        // note for future reference, if RID implementation changes, this could
        // become more expensive
        if ((batch_texture.RID_texture == p_texture)
            && (batch_texture.RID_normal == p_normal)) {
    ...
    // not the previous match .. we will do a linear search ... slower, but
    // should happen not very often except with non-batchable runs, which are
    // going to be slow anyway n.b. could possibly be replaced later by a fast
    // hash table
    ...
    // pushing back from local variable .. not ideal but has to use a Vector
    ...
    if (p_tile) {
        if (texture) {
            ...
            if (!get_storage()->config.support_npot_repeat_mipmap) {
                if (next_power_of_2(texture->alloc_width)
                        != (unsigned int)texture->alloc_width
                    && next_power_of_2(texture->alloc_height)
                           != (unsigned int)texture->alloc_height) {
...
#ifdef GLES_OVER_GL
    use_nvidia_rect_workaround =
        GLOBAL_GET("rendering/2d/options/use_nvidia_rect_flicker_workaround");
#else
...
PREAMBLE(void)::batch_initialize() {
    bdata.settings_use_batching =
        GLOBAL_GET("rendering/batching/options/use_batching");
    bdata.settings_max_join_item_commands =
        GLOBAL_GET("rendering/batching/parameters/max_join_item_commands");
    bdata.settings_colored_vertex_format_threshold = GLOBAL_GET(
        "rendering/batching/parameters/colored_vertex_format_threshold"
    );
    bdata.settings_item_reordering_lookahead =
        GLOBAL_GET("rendering/batching/parameters/item_reordering_lookahead");
    bdata.settings_light_max_join_items =
        GLOBAL_GET("rendering/batching/lights/max_join_items");
    bdata.settings_use_single_rect_fallback =
        GLOBAL_GET("rendering/batching/options/single_rect_fallback");
    bdata.settings_use_software_skinning =
        GLOBAL_GET("rendering/2d/options/use_software_skinning");
    bdata.settings_ninepatch_mode =
        GLOBAL_GET("rendering/2d/options/ninepatch_mode");
    ...
    bdata.settings_uv_contract =
        GLOBAL_GET("rendering/batching/precision/uv_contract");
    bdata.settings_uv_contract_amount =
        (float)GLOBAL_GET("rendering/batching/precision/uv_contract_amount")
        / 1000000.0f;

    // we can use the threshold to determine whether to turn scissoring off or
    // on
    bdata.settings_scissor_threshold =
        GLOBAL_GET("rendering/batching/lights/scissor_area_threshold");
    if (bdata.settings_scissor_threshold > 0.999f) {
        bdata.settings_scissor_lights = false;
    } else {
        bdata.settings_scissor_lights = true;

        // apply power of 4 relationship for the area, as most of the important
        // changes will be happening at low values of scissor threshold
    ...
    // and this is the default. This saves memory too so we will use it for now,
    // needs testing to see whether this varies according to device / platform.
    bdata.settings_batch_buffer_num_verts =
        GLOBAL_GET("rendering/batching/parameters/batch_buffer_size");

    // override the use_batching setting in the editor
    // (note that if the editor can't start, you can't change the use_batching
    // project setting!)
    if (Engine::get_singleton()->is_editor_hint()) {
        bool use_in_editor =
            GLOBAL_GET("rendering/batching/options/use_batching_in_editor");
        bdata.settings_use_batching = use_in_editor;

        // fix some settings in the editor, as the performance not worth the
        // risk
        bdata.settings_use_single_rect_fallback = false;
    }

    // if we are using batching, we will purposefully disable the nvidia
    // workaround. This is because the only reason to use the single rect
    // fallback is the approx 2x speed of the uniform drawing technique. If we
    // used nvidia workaround, speed would be approx equal to the batcher
    // drawing technique (indexed primitive + VB).
    if (bdata.settings_use_batching) {
        use_nvidia_rect_workaround = false;
    }

    // For debugging, if flash is set in project settings, it will flash on
    // alternate frames between the non-batched renderer and the batched
    // renderer, in order to find regressions. This should not be used except
    // during development. make a note of the original choice in case we are
    // flashing on and off the batching
    bdata.settings_use_batching_original_choice = bdata.settings_use_batching;
    bdata.settings_flash_batching =
        GLOBAL_GET("rendering/batching/debug/flash_batching");
    if (!bdata.settings_use_batching) {
    ...
    if (!Engine::get_singleton()->is_editor_hint()
        && bdata.settings_use_batching) {
        //	{
        bdata.settings_diagnose_frame =
            GLOBAL_GET("rendering/batching/debug/diagnose_frame");
    }

    // the maximum num quads in a batch is limited by GLES2. We can have only 16
    // bit indices, which means we can address a vertex buffer of max size
    // 65535. 4 vertices are needed per quad.

    // Note this determines the memory use by the vertex buffer vector. max
    // quads (65536/4)-1 but can be reduced to save memory if really required
    // (will result in more batches though)
    ...
    bdata.settings_max_join_item_commands =
        CLAMP(bdata.settings_max_join_item_commands, 0, 65535);
    bdata.settings_colored_vertex_format_threshold =
        CLAMP(bdata.settings_colored_vertex_format_threshold, 0.0f, 1.0f);
    bdata.settings_scissor_threshold =
        CLAMP(bdata.settings_scissor_threshold, 0.0f, 1.0f);
    bdata.settings_light_max_join_items =
        CLAMP(bdata.settings_light_max_join_items, 0, 65535);
    bdata.settings_item_reordering_lookahead =
        CLAMP(bdata.settings_item_reordering_lookahead, 0, 65535);
    ...
    if (bdata.settings_use_batching) {
        ...
        if (OS::get_singleton()->is_stdout_verbose()) {
            ...
            batching_options_string +=
                ...
                + itos(bdata.settings_max_join_item_commands) + "\n";
            batching_options_string +=
                "\tcolored_vertex_format_threshold "
                + String(Variant(bdata.settings_colored_vertex_format_threshold)
                )
                + "\n";
            batching_options_string +=
                "\tbatch_buffer_size "
                + itos(bdata.settings_batch_buffer_num_verts) + "\n";
            batching_options_string +=
                "\tlight_scissor_area_threshold "
                + String(Variant(bdata.settings_scissor_threshold)) + "\n";

            batching_options_string +=
                "\titem_reordering_lookahead "
                + itos(bdata.settings_item_reordering_lookahead) + "\n";
            batching_options_string += "\tlight_max_join_items "
                                     + itos(bdata.settings_light_max_join_items)
                                     + "\n";
            batching_options_string +=
                "\tsingle_rect_fallback "
                + String(Variant(bdata.settings_use_single_rect_fallback))
                + "\n";

            batching_options_string +=
                "\tdebug_flash "
                + String(Variant(bdata.settings_flash_batching)) + "\n";
            batching_options_string +=
                "\tdiagnose_frame "
                + String(Variant(bdata.settings_diagnose_frame));
        }
    ...
    // the index buffer can be longer than 65535, only the indices need to be
    // within this range
    bdata.index_buffer_size_units = max_quads * 6;

    const int max_verts = bdata.vertex_buffer_size_units;

    // this comes out at approx 64K for non-colored vertex buffer, and 128K for
    // colored vertex buffer
    bdata.vertex_buffer_size_bytes = max_verts * sizeof_batch_vert;
    bdata.index_buffer_size_bytes =
        bdata.index_buffer_size_units * 2; // 16 bit inds

    // create equal number of normal and (max) unit sized verts (as the normal
    // may need to be translated to a larger FVF)
...
PREAMBLE(bool)::_light_scissor_begin(
    const Rect2& p_item_rect,
    const Transform2D& p_light_xform,
    const Rect2& p_light_rect
) const {
    float area_item =
        p_item_rect.size.x
        * p_item_rect.size.y; // double check these are always positive

    // quick reject .. the area of pixels saved can never be more than the area
    // of the item
...
PREAMBLE(bool)::_light_find_intersection(
    const Rect2& p_item_rect,
    const Transform2D& p_light_xform,
    const Rect2& p_light_rect,
    Rect2& r_cliprect
) const {
    // transform light to world space (note this is done in the earlier
    // intersection test, so could be made more efficient)
    Vector2 pts[4] = {
        p_light_xform.xform(p_light_rect.position),
        p_light_xform.xform(Vector2(
            p_light_rect.position.x + p_light_rect.size.x,
            p_light_rect.position.y
        )),
        p_light_xform.xform(Vector2(
            p_light_rect.position.x,
            p_light_rect.position.y + p_light_rect.size.y
        )),
        p_light_xform.xform(Vector2(
            p_light_rect.position.x + p_light_rect.size.x,
            p_light_rect.position.y + p_light_rect.size.y
    ...
    // they should probably always intersect, because of earlier check, but just
    // in case...
    if (!p_item_rect.intersects(lrect)) {
        return false;
    }

    // note this does almost the same as Rect2.clip but slightly more efficient
    // for our use case
    ...
    r_cliprect.size.y =
        MIN(item_rect_end.y, lrect_end.y) - r_cliprect.position.y;
...
PREAMBLE(void)::_calculate_scissor_threshold_area() {
    ...
    if (bdata.settings_scissor_threshold < 0.0001f) {
        ...
        bdata.scissor_threshold_area =
            bdata.settings_scissor_threshold * screen_area;
    }
}

PREAMBLE(bool)::_prefill_line(
    RasterizerCanvas::Item::CommandLine* p_line,
    FillState& r_fill_state,
    int& r_command_start,
    int command_num,
    ...
    RasterizerStorageCommon::BatchType line_batch_type =
        RasterizerStorageCommon::BT_LINE;
    ...
    if (!bvs) {
        // run out of space in the vertex buffer .. finish this function and
        // draw what we have so far return where we got to
        r_command_start = command_num;
        return true;
    }
...
// unsigned int
// _ninepatch_apply_tiling_modes(RasterizerCanvas::Item::CommandNinePatch *p_np,
// Rect2 &r_source) { 	unsigned int rect_flags = 0;
...
T_PREAMBLE
template <bool SEND_LIGHT_ANGLES>
bool C_PREAMBLE::_prefill_ninepatch(
    RasterizerCanvas::Item::CommandNinePatch* p_np,
    FillState& r_fill_state,
    int& r_command_start,
    int command_num,
    ...
    if (r_fill_state.curr_batch->type != RasterizerStorageCommon::BT_RECT) {
        // don't allow joining to a different sequence type
        if (r_fill_state.sequence_batch_type_flags
            & (~RasterizerStorageCommon::BTF_RECT)) {
            // don't allow joining to a different sequence type
            r_command_start = command_num;
            return true;
        }
    ...
    if ((bdata.settings_ninepatch_mode == 1) && (source.size.x != 0)
        && (source.size.y != 0)) {
        screen_scale =
            MIN(p_np->rect.size.x / source.size.x,
                p_np->rect.size.y / source.size.y);
    ...
    // Some protection for the use of ninepatches with rect size smaller than
    // margin size. Note these cannot be produced by the UI, only
    // programmatically, and the results are somewhat undefined, because the
    // margins overlap. Ninepatch get_minimum_size()	forces minimum size to
    // be the sum of the margins. So this should occur very rarely if ever.
    // Consider commenting these 4 lines out for higher speed in ninepatches.
    x[1] = MIN(x[1], x[3]);
    x[2] = MIN(x[2], x[3]);
    y[1] = MIN(y[1], y[3]);
    y[2] = MIN(y[2], y[3]);
...
T_PREAMBLE
template <bool SEND_LIGHT_ANGLES>
bool C_PREAMBLE::_prefill_polygon(
    RasterizerCanvas::Item::CommandPolygon* p_poly,
    FillState& r_fill_state,
    int& r_command_start,
    int command_num,
    ...
    if (r_fill_state.curr_batch->type != RasterizerStorageCommon::BT_POLY) {
        // don't allow joining to a different sequence type
        if (r_fill_state.sequence_batch_type_flags
            & (~RasterizerStorageCommon::BTF_POLY)) {
            // don't allow joining to a different sequence type
            r_command_start = command_num;
            return true;
        }
        r_fill_state.sequence_batch_type_flags |=
            RasterizerStorageCommon::BTF_POLY;
    ...
    // we aren't using indices, so will transform verts more than once .. less
    // efficient. could be done with a temporary vertex buffer
    BatchVertex* bvs = bdata.vertices.request(num_inds);
    if (!bvs) {
        // run out of space in the vertex buffer
        // check for special case where the batching buffer is simply not big
        // enough to fit this primitive.
        if (!bdata.vertices.size()) {
            // can't draw, ignore the primitive, otherwise we would enter an
            // infinite loop
            WARN_PRINT_ONCE(
                "poly has too many indices to draw, increase batch buffer size"
            );
            return false;
        }
    ...
    if (use_modulate) {
        ...
        // we store the modulate as an attribute in the fvf rather than a
        // uniform
    ...
    if (use_large_verts) {
        ...
        // we store the transform as an attribute in the fvf rather than a
        // uniform
    ...
    if (change_batch) {
        // put the tex pixel size  in a local (less verbose and can be a
        // register)
        const BatchTex& batchtex =
            bdata.batch_textures[r_fill_state.batch_tex_id];
        batchtex.tex_pixel_size.to(r_fill_state.texpixel_size);

        if (bdata.settings_uv_contract) {
            r_fill_state.contract_uvs =
                (batchtex.flags & VS::TEXTURE_FLAG_FILTER) == 0;
        }
    ...
    // PRECALCULATE THE COLORS (as there may be less colors than there are
    // indices in either hardware or software paths)
    BatchColor vcol;
    int num_verts = p_poly->points.size();

    // in special cases, only 1 color is specified by convention, so we want to
    // preset this to use in all verts.
    ...
    BatchColor* precalced_colors =
        (BatchColor*)alloca(num_verts * sizeof(BatchColor));
    ...
    if (!_software_skin_poly(
        ...
        bool software_transform =
            (r_fill_state.transform_mode != TM_NONE) && (!use_large_verts);
        ...
        for (int n = 0; n < num_inds; n++) {
            ...
            // recover at runtime from invalid polys (the editor may send
            // invalid polys)
            if ((unsigned int)ind >= (unsigned int)num_verts) {
                // will recover as long as there is at least one vertex.
                // if there are no verts, we will have quick rejected earlier in
                // this function
                ind = 0;
            }
            ...
            if (software_transform) {
                ...
                _software_transform_vertex(
                    pos,
                    r_fill_state.transform_combined
                );
            ...
            if (use_large_verts) {
                // reuse precalced transform (same for each vertex within
                // polygon)
                pBT[n] = pBT[0];
            }
        }
        ...
        // not sure if this will produce garbage if software skinning is
        // changing vertex pos in the shader, but is included for completeness
        if (use_large_verts) {
            for (int n = 0; n < num_inds; n++) {
                pBT[n] = pBT[0];
            }
...
PREAMBLE(bool)::_software_skin_poly(
    RasterizerCanvas::Item::CommandPolygon* p_poly,
    RasterizerCanvas::Item* p_item,
    BatchVertex* bvs,
    BatchColor* vertex_colors,
    ...
    int bone_count =
        RasterizerStorage::base_singleton->skeleton_get_bone_count(skeleton);
    ...
    const Transform2D& skel_trans_inv =
        p_fill_state.skeleton_base_inverse_xform;
    // we can't get this from the state, because more than one skeleton item may
    // have been joined together.. we need to handle the base skeleton on a per
    // item basis as the joined item is rendered. const Transform2D &skel_trans
    // = get_this()->state.skeleton_transform; const Transform2D &skel_trans_inv
    // = get_this()->state.skeleton_transform_inverse;

    // get the bone transforms.
    // this is not ideal because we don't know in advance which bones are needed
    // for any particular poly, but depends how cheap the
    // skeleton_bone_get_transform_2d call is
    Transform2D* bone_transforms =
        (Transform2D*)alloca(bone_count * sizeof(Transform2D));
    for (int b = 0; b < bone_count; b++) {
        bone_transforms[b] = RasterizerStorage::base_singleton
                                 ->skeleton_bone_get_transform_2d(skeleton, b);
    }

    if (num_verts && (p_poly->bones.size() == num_verts * 4)
        && (p_poly->weights.size() == p_poly->bones.size())) {
        ...
        for (int n = 0; n < num_verts; n++) {
            ...
            // concatenated with the bone transforms to save extra transforms -
            // not sure yet
            ...
            // but it seemed to work for the example project ... watch for
            // regressions
    ...
    if (p_fill_state.transform_mode != TM_NONE) {
        for (int n = 0; n < num_verts; n++) {
            ...
            _software_transform_vertex(
                dst_pos,
                p_fill_state.transform_combined
            );
        }
    }
    ...
    for (int n = 0; n < num_inds; n++) {
        ...
        // recover at runtime from invalid polys (the editor may send invalid
        // polys)
        if ((unsigned int)ind >= (unsigned int)num_verts) {
            // will recover as long as there is at least one vertex.
            // if there are no verts, we will have quick rejected earlier in
...
bool C_PREAMBLE::_prefill_rect(
    ...
    if (r_fill_state.curr_batch->type != RasterizerStorageCommon::BT_RECT) {
        ...
        // check for special case if there is only a single or small number of
        // rects, in which case we will use the legacy default rect renderer
        // because it is faster for single rects

        // we only want to do this if not a joined item with more than 1 item,
        // because joined items with more than 1, the command * will be
        // incorrect NOTE - this is assuming that use_hardware_transform means
        // that it is a non-joined item!! If that assumption is incorrect this
        ...
        if (bdata.settings_use_single_rect_fallback
            ...
            if (command_num_next < command_count) {
                RasterizerCanvas::Item::Command* command_next =
                    commands[command_num_next];
                if ((command_next->type
                     != RasterizerCanvas::Item::Command::TYPE_RECT)
                    && (command_next->type
                        != RasterizerCanvas::Item::Command::TYPE_TRANSFORM)) {
                    is_single_rect = true;
                }
            } else {
                is_single_rect = true;
            }
            // if it is a rect on its own, do exactly the same as the default
            // routine
            if (is_single_rect) {
    ...
    // use_modulate and use_large_verts should have been checked in the calling
    // prefill_item function. we don't want to apply the modulate on the CPU if
    // it is stored in the vertex format, it will be applied in the shader
    if (multiply_final_modulate) {
        col *= r_fill_state.final_modulate;
    }
    ...
    // possibility of applying flips here for normal mapping .. but they don't
    // seem to be used
    ...
    if (rect->flags & RasterizerCanvas::CANVAS_RECT_REGION) {
        ...
        // nudge offset for the maximum to prevent precision error on GPU
        // reading into line outside the source rect this is very difficult to
        // get right.
    ...
    if (SEND_LIGHT_ANGLES) {
        // we can either keep the light angles in sync with the verts when
        // writing, or sync them up during translation. We are syncing in
        // translation. N.B. There may be batches that don't require
        // light_angles between batches that do.
        ...
        if (r_fill_state.transform_mode != TM_NONE) {
            ...
            // apply to an x axis
            // the x axis and y axis can be taken directly from the transform
            // (no need to xform identity vectors)
            Vector2 x_axis(tr.elements[0][0], tr.elements[0][1]);

            // have to do a y axis to check for scaling flips
            // this is hassle and extra slowness. We could only allow flips via
            // the flags.
            ...
            // passing an angle is smaller than a vector, it can be
            // reconstructed in the shader
            angle = x_axis.angle();

            // we don't want negative angles, as negative is used to encode
            // flips. This moves range from -PI to PI to 0 to TWO_PI
            if (angle < 0.0f) {
                angle += TWO_PI;
            }
        ...
        // light angle must be sent for each vert, instead as a single uniform
        // in the uniform draw method this has the benefit of enabling batching
        // with light angles.
        for (int n = 0; n < 4; n++) {
            angles[n] = angle;
        }
...
// This function may be called MULTIPLE TIMES for each item, so needs to record
// how far it has got
PREAMBLE(bool)::prefill_joined_item(
    FillState& r_fill_state,
    int& r_command_start,
    RasterizerCanvas::Item* p_item,
    RasterizerCanvas::Item* p_current_clip,
    ...
    if (r_fill_state.is_single_item || bdata.use_modulate
        || bdata.use_large_verts) {
    ...
    for (command_num = r_command_start; command_num < command_count;
         command_num++) {
        ...
        switch (command->type) {
            ...
            case RasterizerCanvas::Item::Command::TYPE_TRANSFORM: {
                ...
                // break this extra matrix software path (as we don't want to
                // unset it on the GPU etc)
                if (r_fill_state.extra_matrix_sent) {
                    _prefill_default_batch(r_fill_state, command_num, *p_item);

                    // keep track of the combined matrix on the CPU in parallel,
                    // in case we use large vertex format
                    RasterizerCanvas::Item::CommandTransform* transform =
                        static_cast<RasterizerCanvas::Item::CommandTransform*>(
                            command
                        );
                    const Transform2D& extra_matrix = transform->xform;
                    r_fill_state.transform_combined =
                        p_item->final_transform * extra_matrix;
                } else {
                    // Extra matrix fast path.
                    // Instead of sending the command immediately, we store the
                    // modified transform (in combined) for software transform,
                    // and only flush this transform command if we NEED to (i.e.
                    // we want to render some default commands)
                    RasterizerCanvas::Item::CommandTransform* transform =
                        static_cast<RasterizerCanvas::Item::CommandTransform*>(
                            command
                        );
                    ...
                    if (r_fill_state.is_single_item
                        && !r_fill_state.use_attrib_transform) {
                        // if we are using hardware transform mode, we have
                        // already sent the final transform, so we only want to
                        // software transform the extra matrix
                        r_fill_state.transform_combined = extra_matrix;
                    } else {
                        r_fill_state.transform_combined =
                            p_item->final_transform * extra_matrix;
                    }
                    // after a transform command, always use some form of
                    // software transform (either the combined final + extra, or
                    // just the extra) until we flush this dirty extra matrix
                    // because we need to render default commands.
                    r_fill_state.transform_mode =
                        _find_transform_mode(r_fill_state.transform_combined);

                    // make a note of which command the dirty extra matrix is
                    // store in, so we can send it later if necessary
                    r_fill_state.transform_extra_command_number_p1 =
                        command_num + 1; // plus 1 so we can test against zero
                }
            } break;
            case RasterizerCanvas::Item::Command::TYPE_RECT: {
                RasterizerCanvas::Item::CommandRect* rect =
                    static_cast<RasterizerCanvas::Item::CommandRect*>(command);

                // unoptimized - could this be done once per batch / batch
                // texture?
                ...
                if (send_light_angles) {
                    buffer_full = _prefill_rect<true>(
                        rect,
                        r_fill_state,
                        r_command_start,
                        command_num,
                    ...
                    buffer_full = _prefill_rect<false>(
                        rect,
                        r_fill_state,
                        r_command_start,
                        command_num,
            ...
            case RasterizerCanvas::Item::Command::TYPE_NINEPATCH: {
                RasterizerCanvas::Item::CommandNinePatch* np =
                    static_cast<RasterizerCanvas::Item::CommandNinePatch*>(
                        command
                    );

                if ((np->axis_x != VisualServer::NINE_PATCH_STRETCH)
                    || (np->axis_y != VisualServer::NINE_PATCH_STRETCH)) {
                    // not accelerated
                    _prefill_default_batch(r_fill_state, command_num, *p_item);
                    continue;
                }
                ...
                if (send_light_angles) {
                    buffer_full = _prefill_ninepatch<true>(
                        np,
                        r_fill_state,
                        r_command_start,
                        command_num,
                    ...
                    buffer_full = _prefill_ninepatch<false>(
                        np,
                        r_fill_state,
                        r_command_start,
                        command_num,
            ...
            case RasterizerCanvas::Item::Command::TYPE_LINE: {
                RasterizerCanvas::Item::CommandLine* line =
                    static_cast<RasterizerCanvas::Item::CommandLine*>(command);

                if (line->width <= 1) {
                    bool buffer_full = _prefill_line(
                        line,
                        r_fill_state,
                        r_command_start,
                        command_num,
            ...
            case RasterizerCanvas::Item::Command::TYPE_POLYGON: {
                RasterizerCanvas::Item::CommandPolygon* polygon =
                    static_cast<RasterizerCanvas::Item::CommandPolygon*>(command
                    );
#ifdef GLES_OVER_GL
                // anti aliasing not accelerated .. it is problematic because it
                // requires a 2nd line drawn around the outside of each poly,
                // which would require either a second list of indices or a
                // second list of vertices for this step
                bool use_legacy_path = false;

                if (polygon->antialiased) {
                    // anti aliasing is also not supported for software skinned
                    // meshes. we can't easily revert, so we force software
                    // skinned meshes to run through batching path with no AA.
                    use_legacy_path = !bdata.settings_use_software_skinning
                                   || p_item->skeleton == RID();
                }
                ...
                if (use_legacy_path) {
                    ...
                    if (!bdata.settings_use_software_skinning
                        ...
                        // unoptimized - could this be done once per batch /
                        // batch texture?
                        ...
                        if (send_light_angles) {
                            ...
                            // for batching .. this means software skinned with
                            // light angles won't work
                            _prefill_default_batch(
                                r_fill_state,
                                command_num,
                                *p_item
                            ...
                            buffer_full = _prefill_polygon<false>(
                                polygon,
                                r_fill_state,
                                r_command_start,
                                command_num,
    ...
    // VERY IMPORTANT to return where we got to, because this func may be called
    // multiple times per item. Don't miss out on this step by calling return
    // earlier in the function without setting r_command_start.
    r_command_start = command_num;

    return false;
}

PREAMBLE(void)::flush_render_batches(
    RasterizerCanvas::Item* p_first_item,
    RasterizerCanvas::Item* p_current_clip,
    bool& r_reclip,
    typename T_STORAGE::Material* p_material,
    ...
    // lines contain no color (this is stored in the batch), and polys contain
    // vertex and color only.
    if (p_sequence_batch_type_flags
        & (RasterizerStorageCommon::BTF_LINE
           | RasterizerStorageCommon::BTF_LINE_AA)) {
        ...
        if (bdata.fvf == RasterizerStorageCommon::FVF_REGULAR) {
            // only check whether to convert if there are quads (prevent divide
            // by zero) and we haven't decided to prevent color baking (due to
            // e.g. MODULATE being used in a shader)
            if (bdata.total_quads
                && !(
                    bdata.joined_item_batch_flags
                    & RasterizerStorageCommon::PREVENT_COLOR_BAKING
                )) {
                // minus 1 to prevent single primitives (ratio 1.0) always being
                // converted to colored.. in that case it is slightly cheaper to
                // just have the color as part of the batch
                float ratio = (float)(bdata.total_color_changes - 1)
                            / (float)bdata.total_quads;

                // use bigger than or equal so that 0.0 threshold can force
                // always using colored verts
    ...
    switch (bdata.fvf) {
        ...
        case RasterizerStorageCommon::FVF_COLOR: {
            ...
            if (!bdata.vertex_colors.size()) {
                _translate_batches_to_larger_FVF<
                    BatchVertexColored,
                    false,
                    false,
                    false>(p_sequence_batch_type_flags);
            } else {
        ...
        case RasterizerStorageCommon::FVF_LARGE:
            _translate_batches_to_larger_FVF<
                ...
                true,
                true,
                true>(p_sequence_batch_type_flags);
            break;
    }
...
PREAMBLE(void)::render_joined_item_commands(
    const BItemJoined& p_bij,
    RasterizerCanvas::Item* p_current_clip,
    bool& r_reclip,
    typename T_STORAGE::Material* p_material,
    ...
    RasterizerCanvas::Item* first_item =
        bdata.item_refs[p_bij.first_item_ref].item;

    // fill_state and bdata have once off setup per joined item, and a smaller
    // reset on flush
    FillState fill_state;
    fill_state.reset_joined_item(
        p_bij.is_single_item(),
        p_bij.use_attrib_transform()
    );
    ...
    // indirectly in a couple of places from the state rather than from the
    // joined item. we could alternatively make sure to only read directly from
    // the joined item during the render, but it is probably more bug future
    // proof to make sure both are up to date.
    bdata.joined_item_batch_flags = p_bij.flags;

    // in the special case of custom shaders that read from VERTEX (i.e. vertex
    // position) we want to disable software transform of extra matrix
    if (bdata.joined_item_batch_flags
    ...
    for (unsigned int i = 0; i < p_bij.num_item_refs; i++) {
        ...
        // ONCE OFF fill state setup, that will be retained over multiple calls
        // to prefill_joined_item()
        fill_state.transform_combined = item->final_transform;

        // calculate skeleton base inverse transform if required for software
        // skinning put in the fill state as this is readily accessible from the
        // software skinner
        if (item->skeleton.is_valid() && bdata.settings_use_software_skinning
            && get_storage()->skeleton_owner.owns(item->skeleton)) {
            typename T_STORAGE::Skeleton* skeleton = nullptr;
            skeleton = get_storage()->skeleton_owner.get(item->skeleton);

            if (skeleton->use_2d) {
                // with software skinning we still need to know the skeleton
                // inverse transform, the other two aren't needed but are left
                // in for simplicity here
                Transform2D skeleton_transform = p_ris.item_group_base_transform
                                               * skeleton->base_transform_2d;
                fill_state.skeleton_base_inverse_xform =
                    skeleton_transform.affine_inverse();
            }
        }
        ...
        if (fill_state.use_software_transform) {
            fill_state.transform_mode =
                _find_transform_mode(fill_state.transform_combined);
        } else {
        ...
        while (command_start < command_count) {
            // fill as many batches as possible (until all done, or the vertex
            // buffer is full)
            bool bFull = get_this()->prefill_joined_item(
                fill_state,
                command_start,
                item,
                p_current_clip,
            ...
            if (bFull) {
                // always pass first item (commands for default are always first
                // item)
                flush_render_batches(
                    ...
                    r_reclip,
                    p_material,
                    fill_state.sequence_batch_type_flags
                );
...
PREAMBLE(void)::_legacy_canvas_item_render_commands(
    RasterizerCanvas::Item* p_item,
    RasterizerCanvas::Item* p_current_clip,
    bool& r_reclip,
    typename T_STORAGE::Material* p_material
...
PREAMBLE(void)::join_sorted_items() {
    ...
    for (int s = 0; s < bdata.sort_items.size(); s++) {
        ...
        if (si.z_index != z) {
            ...
            // if z ranged lights are present, sometimes we have to disable
            // joining over z_indices. we do this here. Note this restriction
            // may be able to be relaxed with light bitfields, investigate!
        ...
        if (_render_item_state.join_batch_break) {
            ...
            // could be another batch break (i.e. prevent NEXT item from joining
            // this) so we still need to run try_join_item even though we know
            // join is false. also we need to run try_join_item for every item
            // because it keeps the state up to date, if we didn't run it the
            // state would be out of date.
            get_this()->try_join_item(
                ci,
                _render_item_state,
                _render_item_state.join_batch_break
            );
        } else {
            join = get_this()->try_join_item(
                ci,
                _render_item_state,
                _render_item_state.join_batch_break
            );
        ...
        if (!join) {
            _render_item_state.joined_item =
                bdata.items_joined.request_with_grow();
            _render_item_state.joined_item->first_item_ref =
                bdata.item_refs.size();
            _render_item_state.joined_item->num_item_refs = 1;
            _render_item_state.joined_item->bounding_rect =
                ci->global_rect_cache;
            _render_item_state.joined_item->z_index = z;
            _render_item_state.joined_item->flags =
                bdata.joined_item_batch_flags;

            // we need some logic to prevent joining items that have vastly
            // different batch types
            _render_item_state.joined_item_batch_type_flags_prev =
            ...
            // reference, but it is stupidly complex to calculate later, which
            ...
            _render_item_state.joined_item->num_item_refs += 1;
            _render_item_state.joined_item->bounding_rect =
                _render_item_state.joined_item->bounding_rect.merge(
                    ci->global_rect_cache
                );
...
PREAMBLE(bool
)::_sort_items_match(const BSortItem& p_a, const BSortItem& p_b) const {
    ...
    const RasterizerCanvas::Item::CommandRect* rect_a =
        static_cast<const RasterizerCanvas::Item::CommandRect*>(&ca);
    const RasterizerCanvas::Item::CommandRect* rect_b =
        static_cast<const RasterizerCanvas::Item::CommandRect*>(&cb);
...
case RasterizerCanvas::Item::Command::TYPE_RECT:
    {
        const RasterizerCanvas::Item::CommandRect *comm_a = static_cast<const
RasterizerCanvas::Item::CommandRect *>(&ca); const
RasterizerCanvas::Item::CommandRect *comm_b = static_cast<const
RasterizerCanvas::Item::CommandRect *>(&cb); if (comm_a->texture !=
comm_b->texture) return false;
    }
    break;
case RasterizerCanvas::Item::Command::TYPE_POLYGON:
    {
        const RasterizerCanvas::Item::CommandPolygon *comm_a = static_cast<const
RasterizerCanvas::Item::CommandPolygon *>(&ca); const
RasterizerCanvas::Item::CommandPolygon *comm_b = static_cast<const
RasterizerCanvas::Item::CommandPolygon *>(&cb); if (comm_a->texture !=
comm_b->texture) return false;
    }
    break;
}
...
PREAMBLE(bool)::sort_items_from(int p_start) {
    ...
    const RasterizerCanvas::Item::Command& command_start =
        *start.item->commands[0];
    ...
    for (int test = 2; test < test_last; test++) {
        ...
        // do they match?
        if (!_sort_items_match(
                start,
                *test_sort_item
            )) // order is crucial, start first
        {
            continue;
        }

        // we can only swap if there are no AABB overlaps with sandwiched
        // neighbours
        bool ok = true;

        // start from 2, no need to check 1 as the second has already been
        // checked against this item in the intersection test above
        for (int sn = 2; sn < test; sn++) {
            BSortItem* sandwich_neighbour = &bdata.sort_items[p_start + sn];
            if (test_item->global_rect_cache.intersects(
                    sandwich_neighbour->item->global_rect_cache
                )) {
                ok = false;
                break;
            }
...
PREAMBLE(void
)::_software_transform_vertex(BatchVector2& r_v, const Transform2D& p_tr)
    const {
...
PREAMBLE(void
)::_software_transform_vertex(Vector2& r_v, const Transform2D& p_tr) const {
    r_v = p_tr.xform(r_v);
}
...
// there comes a point where it is better to just use colored verts to avoid
// lots of batches. In addition this can optionally add light angles to the FVF,
// necessary for normal mapping.
T_PREAMBLE
template <
    class BATCH_VERTEX_TYPE,
    bool INCLUDE_LIGHT_ANGLES,
...
void C_PREAMBLE::_translate_batches_to_larger_FVF(
    ...
    if (p_sequence_batch_type_flags & RasterizerStorageCommon::BTF_POLY) {
        include_poly_color =
            INCLUDE_LIGHT_ANGLES | INCLUDE_MODULATE | INCLUDE_LARGE;
    }
    ...
    // As the vertices_colored and batches_temp are 'mirrors' of the non-colored
    // version, the sizes should be equal, and allocations should never fail.
    // Hence the use of debug asserts to check program flow, these should not
    // occur at runtime unless the allocation code has been altered.
    ...
    for (int n = 0; n < bdata.batches.size(); n++) {
        ...
        const BatchTex& btex =
            bdata.batch_textures[source_batch.batch_texture_id];
        ...
        if (dest_batch) {
            if (dest_batch->type == source_batch.type) {
                if (source_batch.type == RasterizerStorageCommon::BT_RECT) {
                    if (dest_batch->batch_texture_id
                        ...
                        for (int v = first_vert; v < end_vert; v++) {
                            ...
                            if (INCLUDE_LIGHT_ANGLES) {
                                ...
                                // this is required to allow compilation with
                                // non light angle vertex. it should be compiled
                                // out.
                                BatchVertexLightAngled* lv =
                                    (BatchVertexLightAngled*)cv;
                                if (source_batch_uses_light_angles) {
                                    lv->light_angle = *source_light_angles++;
                                } else {
                                    lv->light_angle =
                                        0.0f; // dummy, unused in vertex shader
                                              // (could possibly be left
                                              // uninitialized, but probably bad
                                              // idea)
                                }
                            } // if including light angles

                            if (INCLUDE_MODULATE) {
                                RAST_DEV_DEBUG_ASSERT(
                                    bdata.vertex_modulates.size()
                                );
                                BatchVertexModulated* mv =
                                    (BatchVertexModulated*)cv;
                                mv->modulate = *source_vertex_modulates++;
                            } // including modulate

                            if (INCLUDE_LARGE) {
                                RAST_DEV_DEBUG_ASSERT(
                                    bdata.vertex_transforms.size()
                                );
                    ...
                    // does this ever happen? not sure at this stage, but left
                    // for future expansion
                    uint32_t source_last_command =
            ...
            *dest_batch = source_batch;
            ...
            if (source_batch.type != RasterizerStorageCommon::BT_DEFAULT) {
                ...
                for (int v = first_vert; v < end_vert; v++) {
                    ...
                    if (INCLUDE_LIGHT_ANGLES) {
                        RAST_DEV_DEBUG_ASSERT(bdata.light_angles.size());
                        // this is required to allow compilation with non light
                        // angle vertex. it should be compiled out.
                        BatchVertexLightAngled* lv =
                            (BatchVertexLightAngled*)cv;
                        if (source_batch_uses_light_angles) {
                            lv->light_angle = *source_light_angles++;
                            ...
                            lv->light_angle =
                                0.0f; // dummy, unused in vertex shader (could
                                      // possibly be left uninitialized, but
                                      // probably bad idea)
                        }
    ...
    // copy the temporary batches to the master batch list (this could be
    // avoided but it makes the code cleaner)
    bdata.batches.copy_from(bdata.batches_temp);
}

PREAMBLE(bool)::_disallow_item_join_if_batch_types_too_different(
    RenderItemState& r_ris,
    uint32_t btf_allowed
) {
...
PREAMBLE(bool)::_detect_item_batch_break(
    RenderItemState& r_ris,
    RasterizerCanvas::Item* p_ci,
    bool& r_batch_break
) {
    ...
    // (i.e. not handled by software transform and the batching renderer) should
    // not be joined.

    // ALSO batched types that differ in what the vertex format is needed to be
    // should not be joined.

    // In order to work this out, it does a lookahead through the commands,
    // which could potentially be very expensive. As such it makes sense to put
    // a limit on this to some small number, which will catch nearly all cases
    // which need joining, but not be overly expensive in the case of items with
    // large numbers of commands.

    // It is hard to know what this number should be, empirically,
    // and this has not been fully investigated. It works to join single sprite
    // items when set to 1 or above. Note that there is a cost to increasing
    // this because it has to look in advance through the commands. On the other
    // hand joining items where possible will usually be better up to a certain
    // number where the cost of software transform is higher than separate
    // drawcalls with hardware transform.
    ...
    // This is set to quite a conservative (low) number until investigated
    // properly. const int MAX_JOIN_ITEM_COMMANDS = 16;
        ...
        for (int command_num = 0; command_num < command_count; command_num++) {
            ...
            switch (command->type) {
                ...
                case RasterizerCanvas::Item::Command::TYPE_LINE: {
                    ...
                    RasterizerCanvas::Item::CommandLine* line =
                        static_cast<RasterizerCanvas::Item::CommandLine*>(
                            command
                        );
                    ...
                    if (_disallow_item_join_if_batch_types_too_different(
                            r_ris,
                            RasterizerStorageCommon::BTF_LINE
                                | RasterizerStorageCommon::BTF_LINE_AA
                        )) {
                ...
                case RasterizerCanvas::Item::Command::TYPE_POLYGON: {
                    ...
                    RasterizerCanvas::Item::CommandPolygon* poly =
                        static_cast<RasterizerCanvas::Item::CommandPolygon*>(
                            command
                        );
                    ...
                    if (!get_this()->bdata.settings_use_software_skinning
                        && poly->bones.size()) {
                        return true;
                    }
                ...
                case RasterizerCanvas::Item::Command::TYPE_NINEPATCH: {
                    // do not handle tiled ninepatches, these can't be batched
                    // and need to use legacy method
                    RasterizerCanvas::Item::CommandNinePatch* np =
                        static_cast<RasterizerCanvas::Item::CommandNinePatch*>(
                            command
                        );
