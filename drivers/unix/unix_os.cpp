...
Error UnixOS::execute(
    ...
    if (p_blocking && r_pipe) {
        ...
        ERR_FAIL_COND_V_MSG(
            !f,
            ERR_CANT_OPEN,
            "Cannot pipe stream from process running with following arguments '"
                + argss + "'."
        );
...
Error UnixOS::open_dynamic_library(
    ...
    if (FileAccess::exists(path) && path.is_rel_path()) {
        // dlopen expects a slash, in this case a leading ./ for it to be
        // interpreted as a relative path,
        //  otherwise it will end up searching various system directories for
        //  the lib instead and finally failing.
        path = "./" + path;
    }

    if (!FileAccess::exists(path)) {
        // this code exists so gdnative can load .so files from within the
        // executable path
        path =
    ...
    if (!FileAccess::exists(path)) {
        // this code exists so gdnative can load .so files from a standard unix
        // location
        path =
            get_executable_path().get_base_dir().plus_file("../lib").plus_file(
                p_path.get_file()
            );
    }
...
String UnixOS::get_executable_path() const {
    ...
    if (b == "") {
        WARN_PRINT(
            "Couldn't get executable path from /proc/self/exe, using argv[0]"
        );
...
#else
    ERR_PRINT(
        "Warning, don't know how to obtain executable path on this OS! Please "
        "override this function properly."
    );
    return OS::get_executable_path();
#endif
}

void UnixTerminalLogger::log_error(
