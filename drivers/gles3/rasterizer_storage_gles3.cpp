...
Ref<Image> RasterizerStorageGLES3::_get_gl_image_and_format(
    ...
    uint32_t p_flags,
    Image::Format& r_real_format,
    GLenum& r_gl_format,
    GLenum& r_gl_internal_format,
    GLenum& r_gl_type,
    bool& r_compressed,
    bool& r_srgb,
        ...
        case Image::FORMAT_DXT3: {
            if (config.s3tc_supported) {
                r_gl_internal_format =
                    (config.srgb_decode_supported
                     || (p_flags & VS::TEXTURE_FLAG_CONVERT_TO_LINEAR))
                        ? _EXT_COMPRESSED_SRGB_ALPHA_S3TC_DXT3_NV
        ...
        case Image::FORMAT_PVRTC2: {
            if (config.pvrtc_supported) {
                r_gl_internal_format =
                    (config.srgb_decode_supported
                     || (p_flags & VS::TEXTURE_FLAG_CONVERT_TO_LINEAR))
                        ? _EXT_COMPRESSED_SRGB_PVRTC_2BPPV1_EXT
                        : _EXT_COMPRESSED_RGB_PVRTC_2BPPV1_IMG;
                r_gl_format  = GL_RGBA;
                r_gl_type    = GL_UNSIGNED_BYTE;
                r_compressed = true;
                r_srgb       = true;
        ...
        case Image::FORMAT_PVRTC4: {
            if (config.pvrtc_supported) {
                r_gl_internal_format =
                    (config.srgb_decode_supported
                     || (p_flags & VS::TEXTURE_FLAG_CONVERT_TO_LINEAR))
                        ? _EXT_COMPRESSED_SRGB_PVRTC_4BPPV1_EXT
                        : _EXT_COMPRESSED_RGB_PVRTC_4BPPV1_IMG;
                r_gl_format  = GL_RGBA;
                r_gl_type    = GL_UNSIGNED_BYTE;
                r_compressed = true;
                r_srgb       = true;
        ...
        case Image::FORMAT_PVRTC4A: {
            if (config.pvrtc_supported) {
                r_gl_internal_format =
                    (config.srgb_decode_supported
                     || (p_flags & VS::TEXTURE_FLAG_CONVERT_TO_LINEAR))
                        ? _EXT_COMPRESSED_SRGB_ALPHA_PVRTC_4BPPV1_EXT
                        : _EXT_COMPRESSED_RGBA_PVRTC_4BPPV1_IMG;
                r_gl_format  = GL_RGBA;
                r_gl_type    = GL_UNSIGNED_BYTE;
                r_compressed = true;
                r_srgb       = true;
...
void RasterizerStorageGLES3::texture_allocate(
    RID p_texture,
    int p_width,
    int p_height,
    int p_depth_3d,
    Image::Format p_format,
    VisualServer::TextureType p_type,
    uint32_t p_flags
) {
    GLenum format;
    ...
    switch (p_format) {
        ...
        case Image::FORMAT_RGBAH: {
            if (!config.texture_float_linear_supported) {
                // disable linear texture filtering when not supported for float
                // format on some devices (issue #24295)
    ...
    if (p_type != VS::TEXTURE_TYPE_EXTERNAL) {
        ...
        // decompressing them to work it around on WebGL 2.0 at a performance
        // cost (GH-33058).
        ...
        if (!is_po2
            && (p_flags & VS::TEXTURE_FLAG_REPEAT
    ...
    if (p_type == VS::TEXTURE_TYPE_EXTERNAL) {
        ...
        while (width > 0 || height > 0
               || (p_type == VS::TEXTURE_TYPE_3D && depth > 0)) {
            width  = MAX(1, width);
            height = MAX(1, height);
            depth  = MAX(1, depth);
...
void RasterizerStorageGLES3::texture_set_data(
    RID p_texture,
    const Ref<Image>& p_image,
    int p_layer
) {
    ...
    if (config.keep_original_textures
        && !(texture->flags & VS::TEXTURE_FLAG_USED_FOR_STREAMING)) {
        texture->images.write[p_layer] = p_image;
    }

    Image::Format real_format;
    Ref<Image> img = _get_gl_image_and_format(
        p_image,
        p_image->get_format(),
        texture->flags,
        real_format,
    ...
    if ((texture->flags & VS::TEXTURE_FLAG_MIPMAPS)
        && !texture->ignore_mipmaps) {
        if (texture->flags & VS::TEXTURE_FLAG_FILTER) {
            glTexParameteri(
                texture->target,
                GL_TEXTURE_MIN_FILTER,
                config.use_fast_texture_filter ? GL_LINEAR_MIPMAP_NEAREST
            ...
            glTexParameteri(
                texture->target,
                GL_TEXTURE_MIN_FILTER,
                config.use_fast_texture_filter ? GL_NEAREST_MIPMAP_NEAREST
...
// set swizle for older format compatibility
    ...
    int mipmaps =
        ((texture->flags & VS::TEXTURE_FLAG_MIPMAPS) && img->has_mipmaps())
            ? img->get_mipmap_count() + 1
            : 1;
    ...
    for (int i = 0; i < mipmaps; i++) {
        ...
        if (texture->type == VS::TEXTURE_TYPE_2D
            || texture->type == VS::TEXTURE_TYPE_CUBEMAP) {
    ...
    // printf("texture: %i x %i - size: %i - total:
    ...
    if ((texture->type == VS::TEXTURE_TYPE_2D
         || texture->type == VS::TEXTURE_TYPE_CUBEMAP)
        && (texture->flags & VS::TEXTURE_FLAG_MIPMAPS) && mipmaps == 1
        && !texture->ignore_mipmaps
        && (texture->type != VS::TEXTURE_TYPE_CUBEMAP
            || texture->stored_cube_sides == (1 << 6) - 1)) {
        // generate mipmaps if they were requested and the image does not
        // contain them
...
// The texture pixels must have been allocated before, because most features
// seen in texture_set_data() make no sense in a partial update.
// TODO If we want this to be usable without pre-filling pixels with a full
// image, we have to call glTexImage2D() with null data.
void RasterizerStorageGLES3::texture_set_data_partial(
    RID p_texture,
    const Ref<Image>& p_image,
    int src_x,
    int src_y,
    ...
    ERR_FAIL_COND(
        dst_x < 0 || dst_y < 0 || dst_x + src_w > texture->alloc_width
        || dst_y + src_h > texture->alloc_height
    );
    ...
    // Because OpenGL wants data as a dense array, we have to extract the
    // sub-image if the source rect isn't the full image
    Ref<Image> p_sub_img = p_image;
    if (src_x > 0 || src_y > 0 || src_w != p_image->get_width()
        || src_h != p_image->get_height()) {
    ...
    if (texture->type == VS::TEXTURE_TYPE_2D
        ...
        if (texture->compressed) {
            ...
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
            // `format` has to match the internal_format used when the texture
            // was created
            glTexSubImage2D(
                blit_target,
                p_dst_mip,
...
Ref<Image> RasterizerStorageGLES3::texture_get_data(RID p_texture, int p_layer)
    const {
    Texture* texture = texture_owner.get(p_texture);
    ...
    ERR_FAIL_COND_V(
        texture->data_size == 0 && !texture->render_target,
        Ref<Image>()
    );

    if (texture->type == VS::TEXTURE_TYPE_CUBEMAP && p_layer < 6
        && !texture->images[p_layer].is_null()) {
        return texture->images[p_layer];
    }

    // 3D textures and 2D texture arrays need special treatment, as the
    // glGetTexImage reads **the whole** texture to host-memory. 3D textures and
    // 2D texture arrays are potentially very big, so reading everything just to
    // throw everything but one layer away is A Bad Idea.
    //
    // Unfortunately, to solve this, the copy shader has to read the data out
    // via a shader and store it in a temporary framebuffer. The data from the
    // framebuffer can then be read using glReadPixels.
    if (texture->type == VS::TEXTURE_TYPE_2D_ARRAY
        ...
        int data_size = Image::get_image_data_size(
            texture->alloc_width,
            texture->alloc_height,
            Image::FORMAT_RGBA8,
            false
        ...
        data.resize(
            data_size * 2
        ); // add some more memory at the end, just in case for buggy drivers
        ...
        // now bring the OpenGL context into the correct state
        {
            ...
            shaders.copy.set_conditional(
                CopyShaderGLES3::LINEAR_TO_SRGB,
                !srgb
            );
            shaders.copy.set_conditional(
                CopyShaderGLES3::USE_TEXTURE3D,
                texture->type == VS::TEXTURE_TYPE_3D
            );
            shaders.copy.set_conditional(
                CopyShaderGLES3::USE_TEXTURE2DARRAY,
                texture->type == VS::TEXTURE_TYPE_2D_ARRAY
            );
            shaders.copy.bind();
    ...
    data.resize(
        data_size * 2
    ); // add some memory at the end, just in case for buggy drivers
    PoolVector<uint8_t>::Write wb = data.write();
    ...
    for (int i = 0; i < texture->mipmaps; i++) {
        int ofs = Image::get_image_mipmap_offset(
            texture->alloc_width,
            texture->alloc_height,
            real_format,
            i
    ...
    // convert special case RGB10_A2 to RGBA8 because it's not a supported image
    // format
    if (texture->gl_internal_format_cache == GL_RGB10_A2) {
        ...
        for (uint32_t ofs = 0; ofs < num_pixels; ofs++) {
            ...
            ptr[ofs] = (px >> 2 & 0xFF) | (px >> 12 & 0xFF) << 8
                     | (px >> 22 & 0xFF) << 16
                     | (a | a << 2 | a << 4 | a << 6) << 24;
        }
    } else {
...
void RasterizerStorageGLES3::texture_set_flags(
    RID p_texture,
    uint32_t p_flags
) {
    Texture* texture = texture_owner.get(p_texture);
    ...
    if (texture->render_target) {
        // only allow filter and repeat flags for render target (ie. viewport)
        // textures
...
VisualServer::TextureType RasterizerStorageGLES3::texture_get_type(RID p_texture
) const {
    Texture* texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES3::texture_bind(
    RID p_texture,
    uint32_t p_texture_no
) {
...
void RasterizerStorageGLES3::texture_set_size_override(
    RID p_texture,
    int p_width,
    int p_height,
    int p_depth
...
void RasterizerStorageGLES3::texture_set_path(
    RID p_texture,
    const String& p_path
) {
    Texture* texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES3::texture_set_detect_3d_callback(
    RID p_texture,
...
void RasterizerStorageGLES3::texture_set_detect_srgb_callback(
    RID p_texture,
    VisualServer::TextureDetectCallback p_callback,
    void* p_userdata
) {
...
void RasterizerStorageGLES3::texture_set_detect_normal_callback(
    RID p_texture,
    VisualServer::TextureDetectCallback p_callback,
    void* p_userdata
) {
...
RID RasterizerStorageGLES3::texture_create_radiance_cubemap(
    RID p_source,
    int p_resolution
) const {
    ...
    while (mm_level) {
        for (int i = 0; i < 6; i++) {
            ...
            shaders.cubemap_filter.set_uniform(
                CubemapFilterShaderGLES3::FACE_ID,
                i
            );
            shaders.cubemap_filter.set_uniform(
...
void RasterizerStorageGLES3::texture_set_force_redraw_if_visible(
    RID p_texture,
    bool p_enable
) {
    Texture* texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES3::sky_set_texture(
    RID p_sky,
    RID p_panorama,
    int p_radiance_size
) {
    ...
    glTexParameteri(
        ...
        int(Math::floor(Math::log(float(texture->width)) / Math::log(2.0f)))
    );
    glGenerateMipmap(texture->target);
#else
    glTexParameteri(texture->target, GL_TEXTURE_MAX_LEVEL, 0);
...

    {
        ...
        int upscale_size =
            MIN(int(previous_power_of_2(irradiance_size)), p_radiance_size);

        GLuint tmp_fb2;
        GLuint tmp_tex;
        {
            // generate another one for rendering, as can't read and write from
            // a single texarray it seems
        ...
        shaders.cubemap_filter.set_conditional(
            CubemapFilterShaderGLES3::USE_SOURCE_PANORAMA,
            true
        );
        shaders.cubemap_filter.set_conditional(
            CubemapFilterShaderGLES3::COMPUTE_IRRADIANCE,
            true
        );
        shaders.cubemap_filter.bind();

        // Very large Panoramas require way too much effort to compute
        // irradiance so use a mipmap level that corresponds to a panorama of
        // 1024x512
        shaders.cubemap_filter.set_uniform(
            CubemapFilterShaderGLES3::SOURCE_MIP_LEVEL,
            MAX(Math::floor(Math::log(float(texture->width)) / Math::log(2.0f))
                    - 10.0f,
                0.0f)
        );

        // Compute Irradiance for a large texture, specified by radiance size
        // and then pull out a low mipmap corresponding to 32x32
        for (int i = 0; i < 2; i++) {
        ...
        shaders.copy.bind();
        shaders.copy.set_uniform(
            CopyShaderGLES3::MIP_LEVEL,
            MAX(Math::floor(Math::log(float(upscale_size)) / Math::log(2.0f))
                    - 5.0f,
                0.0f)
        ); // Mip level that corresponds to a 32x32 texture
    ...
    if (config.use_texture_array_environment) {
        ...
        for (int j = 0; j < array_level; j++) {
            ...
            if (j == 0) {
                ...
                shaders.cubemap_filter.set_conditional(
                    CubemapFilterShaderGLES3::USE_SOURCE_DUAL_PARABOLOID_ARRAY,
                    false
                );
                shaders.cubemap_filter.bind();
                glActiveTexture(GL_TEXTURE0);
                glBindTexture(texture->target, texture->tex_id);
                shaders.cubemap_filter.set_uniform(
                    CubemapFilterShaderGLES3::SOURCE_RESOLUTION,
                ...
                glBindTexture(GL_TEXTURE_2D_ARRAY, sky->radiance);
                shaders.cubemap_filter.set_uniform(
                    CubemapFilterShaderGLES3::SOURCE_ARRAY_INDEX,
                    j - 1
                ); // read from previous to ensure better blur
            }
        ...
        GLuint tmp_tex;
        {
            // Need a temporary framebuffer for rendering so we can read from
            // previous iterations
            glGenFramebuffers(1, &tmp_fb2);
            glBindFramebuffer(GL_FRAMEBUFFER, tmp_fb2);
            glGenTextures(1, &tmp_tex);
        ...
        while (mm_level) {
            ...
            if (lod == 0) {
                ...
                shaders.cubemap_filter.set_conditional(
                    CubemapFilterShaderGLES3::USE_SOURCE_DUAL_PARABOLOID,
                    false
                );
                shaders.cubemap_filter.bind();
                glActiveTexture(GL_TEXTURE0);
...
void RasterizerStorageGLES3::shader_set_code(
    RID p_shader,
    const String& p_code
) {
...
void RasterizerStorageGLES3::_update_shader(Shader* p_shader) const {
    ...
    switch (p_shader->mode) {
        case VS::SHADER_CANVAS_ITEM: {
            p_shader->canvas_item.light_mode =
                Shader::CanvasItem::LIGHT_MODE_NORMAL;
            p_shader->canvas_item.blend_mode =
            ...
            shaders.actions_canvas.render_mode_values["blend_add"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.blend_mode,
                    Shader::CanvasItem::BLEND_MODE_ADD
                );
            shaders.actions_canvas.render_mode_values["blend_mix"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.blend_mode,
                    Shader::CanvasItem::BLEND_MODE_MIX
                );
            shaders.actions_canvas.render_mode_values["blend_sub"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.blend_mode,
                    Shader::CanvasItem::BLEND_MODE_SUB
                );
            shaders.actions_canvas.render_mode_values["blend_mul"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.blend_mode,
                    Shader::CanvasItem::BLEND_MODE_MUL
                );
            shaders.actions_canvas.render_mode_values["blend_premul_alpha"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.blend_mode,
                    Shader::CanvasItem::BLEND_MODE_PMALPHA
                );
            shaders.actions_canvas.render_mode_values["blend_disabled"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.blend_mode,
                    Shader::CanvasItem::BLEND_MODE_DISABLED
                );

            shaders.actions_canvas.render_mode_values["unshaded"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.light_mode,
                    Shader::CanvasItem::LIGHT_MODE_UNSHADED
                );
            shaders.actions_canvas.render_mode_values["light_only"] =
                Pair<int*, int>(
                    &p_shader->canvas_item.light_mode,
                    Shader::CanvasItem::LIGHT_MODE_LIGHT_ONLY
                );

            shaders.actions_canvas.usage_flag_pointers["SCREEN_UV"] =
                &p_shader->canvas_item.uses_screen_uv;
            shaders.actions_canvas.usage_flag_pointers["SCREEN_PIXEL_SIZE"] =
                &p_shader->canvas_item.uses_screen_uv;
            shaders.actions_canvas.usage_flag_pointers["SCREEN_TEXTURE"] =
                &p_shader->canvas_item.uses_screen_texture;
            shaders.actions_canvas.usage_flag_pointers["TIME"] =
                &p_shader->canvas_item.uses_time;

            shaders.actions_canvas.usage_flag_pointers["MODULATE"] =
                &p_shader->canvas_item.uses_modulate;
            shaders.actions_canvas.usage_flag_pointers["COLOR"] =
                &p_shader->canvas_item.uses_color;
            shaders.actions_canvas.usage_flag_pointers["VERTEX"] =
                &p_shader->canvas_item.uses_vertex;

            shaders.actions_canvas.usage_flag_pointers["WORLD_MATRIX"] =
                &p_shader->canvas_item.uses_world_matrix;
            shaders.actions_canvas.usage_flag_pointers["EXTRA_MATRIX"] =
                &p_shader->canvas_item.uses_extra_matrix;
            shaders.actions_canvas.usage_flag_pointers["PROJECTION_MATRIX"] =
                &p_shader->canvas_item.uses_projection_matrix;
        ...
        case VS::SHADER_SPATIAL: {
            ...
            shaders.actions_scene.render_mode_values["blend_add"] =
                Pair<int*, int>(
                    &p_shader->spatial.blend_mode,
                    Shader::Spatial::BLEND_MODE_ADD
                );
            shaders.actions_scene.render_mode_values["blend_mix"] =
                Pair<int*, int>(
                    &p_shader->spatial.blend_mode,
                    Shader::Spatial::BLEND_MODE_MIX
                );
            shaders.actions_scene.render_mode_values["blend_sub"] =
                Pair<int*, int>(
                    &p_shader->spatial.blend_mode,
                    Shader::Spatial::BLEND_MODE_SUB
                );
            shaders.actions_scene.render_mode_values["blend_mul"] =
                Pair<int*, int>(
                    &p_shader->spatial.blend_mode,
                    Shader::Spatial::BLEND_MODE_MUL
                );

            shaders.actions_scene.render_mode_values["depth_draw_opaque"] =
                Pair<int*, int>(
                    &p_shader->spatial.depth_draw_mode,
                    Shader::Spatial::DEPTH_DRAW_OPAQUE
                );
            shaders.actions_scene.render_mode_values["depth_draw_always"] =
                Pair<int*, int>(
                    &p_shader->spatial.depth_draw_mode,
                    Shader::Spatial::DEPTH_DRAW_ALWAYS
                );
            shaders.actions_scene.render_mode_values["depth_draw_never"] =
                Pair<int*, int>(
                    &p_shader->spatial.depth_draw_mode,
                    Shader::Spatial::DEPTH_DRAW_NEVER
                );
            shaders.actions_scene
                .render_mode_values["depth_draw_alpha_prepass"] =
                Pair<int*, int>(
                    &p_shader->spatial.depth_draw_mode,
                    Shader::Spatial::DEPTH_DRAW_ALPHA_PREPASS
                );

            shaders.actions_scene.render_mode_values["cull_front"] =
                Pair<int*, int>(
                    &p_shader->spatial.cull_mode,
                    Shader::Spatial::CULL_MODE_FRONT
                );
            shaders.actions_scene.render_mode_values["cull_back"] =
                Pair<int*, int>(
                    &p_shader->spatial.cull_mode,
                    Shader::Spatial::CULL_MODE_BACK
                );
            shaders.actions_scene.render_mode_values["cull_disabled"] =
                Pair<int*, int>(
                    &p_shader->spatial.cull_mode,
                    Shader::Spatial::CULL_MODE_DISABLED
                );

            shaders.actions_scene.render_mode_flags["unshaded"] =
                &p_shader->spatial.unshaded;
            shaders.actions_scene.render_mode_flags["depth_test_disable"] =
                &p_shader->spatial.no_depth_test;

            shaders.actions_scene.render_mode_flags["vertex_lighting"] =
                &p_shader->spatial.uses_vertex_lighting;

            shaders.actions_scene.render_mode_flags["world_vertex_coords"] =
                &p_shader->spatial.uses_world_coordinates;

            shaders.actions_scene.render_mode_flags["ensure_correct_normals"] =
                &p_shader->spatial.uses_ensure_correct_normals;

            shaders.actions_scene.usage_flag_pointers["ALPHA"] =
                &p_shader->spatial.uses_alpha;
            shaders.actions_scene.usage_flag_pointers["ALPHA_SCISSOR"] =
                &p_shader->spatial.uses_alpha_scissor;

            shaders.actions_scene.usage_flag_pointers["SSS_STRENGTH"] =
                &p_shader->spatial.uses_sss;
            shaders.actions_scene.usage_flag_pointers["DISCARD"] =
                &p_shader->spatial.uses_discard;
            shaders.actions_scene.usage_flag_pointers["SCREEN_TEXTURE"] =
                &p_shader->spatial.uses_screen_texture;
            shaders.actions_scene.usage_flag_pointers["DEPTH_TEXTURE"] =
                &p_shader->spatial.uses_depth_texture;
            shaders.actions_scene.usage_flag_pointers["TIME"] =
                &p_shader->spatial.uses_time;

            // Use of any of these BUILTINS indicate the need for transformed
            // tangents. This is needed to know when to transform tangents in
            // software skinning.
            shaders.actions_scene.usage_flag_pointers["TANGENT"] =
                &p_shader->spatial.uses_tangent;
            shaders.actions_scene.usage_flag_pointers["NORMALMAP"] =
                &p_shader->spatial.uses_tangent;

            shaders.actions_scene.write_flag_pointers["MODELVIEW_MATRIX"] =
                &p_shader->spatial.writes_modelview_or_projection;
            shaders.actions_scene.write_flag_pointers["PROJECTION_MATRIX"] =
                &p_shader->spatial.writes_modelview_or_projection;
    ...
    Error err = shaders.compiler.compile(
        p_shader->mode,
        p_shader->code,
        actions,
        p_shader->path,
    ...
    p_shader->shader->set_custom_shader_code(
        p_shader->custom_code_id,
        gen_code.vertex,
    ...
    if (p_shader->mode == VS::SHADER_CANVAS_ITEM) {
        if (p_shader->canvas_item.uses_modulate
            | p_shader->canvas_item.uses_color) {
            p_shader->canvas_item.batch_flags |=
                RasterizerStorageCommon::PREVENT_COLOR_BAKING;
        }
        if (p_shader->canvas_item.uses_vertex) {
            p_shader->canvas_item.batch_flags |=
                RasterizerStorageCommon::PREVENT_VERTEX_BAKING;
        }
        if (p_shader->canvas_item.uses_world_matrix
            | p_shader->canvas_item.uses_extra_matrix
            | p_shader->canvas_item.uses_projection_matrix
            | p_shader->canvas_item.uses_instance_custom) {
            p_shader->canvas_item.batch_flags |=
                RasterizerStorageCommon::PREVENT_ITEM_JOINING;
        }
    }
...
void RasterizerStorageGLES3::shader_get_param_list(
    RID p_shader,
    List<PropertyInfo>* p_param_list
) const {
    Shader* shader = shader_owner.get(p_shader);
    ERR_FAIL_COND(!shader);
    ...
    for (Map<StringName, ShaderLanguage::ShaderNode::Uniform>::Element* E =
             shader->uniforms.front();
         E;
         E = E->next()) {
...
void RasterizerStorageGLES3::shader_set_default_texture_param(
    RID p_shader,
...
RID RasterizerStorageGLES3::shader_get_default_texture_param(
    RID p_shader,
    const StringName& p_name
) const {
    const Shader* shader = shader_owner.get(p_shader);
    ...
    const Map<StringName, RID>::Element* E =
        shader->default_textures.find(p_name);
    if (!E) {
        return RID();
    }
    return E->get();
...
void RasterizerStorageGLES3::shader_add_custom_define(
    RID p_shader,
    const String& p_define
) {
    Shader* shader = shader_owner.get(p_shader);
    ERR_FAIL_COND(!shader);
...
void RasterizerStorageGLES3::shader_get_custom_defines(
    RID p_shader,
    Vector<String>* p_defines
) const {
    Shader* shader = shader_owner.get(p_shader);
    ERR_FAIL_COND(!shader);
...
void RasterizerStorageGLES3::shader_remove_custom_define(
    RID p_shader,
    const String& p_define
) {
    Shader* shader = shader_owner.get(p_shader);
...
void RasterizerStorageGLES3::material_set_param(
    RID p_material,
...
Variant RasterizerStorageGLES3::material_get_param(
    RID p_material,
    const StringName& p_param
) const {
    const Material* material = material_owner.get(p_material);
...
Variant RasterizerStorageGLES3::material_get_param_default(
    RID p_material,
    const StringName& p_param
) const {
    const Material* material = material_owner.get(p_material);
    ...
    if (material->shader) {
        if (material->shader->uniforms.has(p_param)) {
            ShaderLanguage::ShaderNode::Uniform uniform =
                material->shader->uniforms[p_param];
            Vector<ShaderLanguage::ConstantNode::Value> default_value =
...
void RasterizerStorageGLES3::material_set_line_width(
    RID p_material,
    float p_width
) {
    Material* material = material_owner.get(p_material);
...
void RasterizerStorageGLES3::material_set_next_pass(
    RID p_material,
    RID p_next_material
) {
    Material* material = material_owner.get(p_material);
...
void RasterizerStorageGLES3::material_add_instance_owner(
    RID p_material,
    ...
    Map<RasterizerScene::InstanceBase*, int>::Element* E =
        material->instance_owners.find(p_instance);
    if (E) {
        E->get()++;
    } else {
...
void RasterizerStorageGLES3::material_remove_instance_owner(
    RID p_material,
    RasterizerScene::InstanceBase* p_instance
) {
    Material* material = material_owner.get(p_material);
...
void RasterizerStorageGLES3::material_set_render_priority(
    RID p_material,
    int priority
) {
...
_FORCE_INLINE_ static void _fill_std140_ubo_value(
    ShaderLanguage::DataType type,
    const Vector<ShaderLanguage::ConstantNode::Value>& value,
    uint8_t* data
) {
    switch (type) {
        case ShaderLanguage::TYPE_BOOL: {
...
void RasterizerStorageGLES3::_update_material(Material* material) {
...

    {
        ...
        if (material->shader && material->shader->mode == VS::SHADER_SPATIAL) {
            if (material->shader->spatial.blend_mode
                    == Shader::Spatial::BLEND_MODE_MIX
                && (!(material->shader->spatial.uses_alpha
                      && !material->shader->spatial.uses_alpha_scissor)
                    || material->shader->spatial.depth_draw_mode
                           == Shader::Spatial::DEPTH_DRAW_ALPHA_PREPASS)) {
                can_cast_shadow = true;
            }

            if (material->shader->spatial.uses_discard
                && material->shader->uses_fragment_time) {
                is_animated = true;
            }

            if (material->shader->spatial.uses_vertex
                && material->shader->uses_vertex_time) {
                is_animated = true;
            }

            if (can_cast_shadow != material->can_cast_shadow_cache
                || is_animated != material->is_animated_cache) {
                material->can_cast_shadow_cache = can_cast_shadow;
                material->is_animated_cache     = is_animated;

                for (Map<Geometry*, int>::Element* E =
                         material->geometry_owners.front();
                     E;
                     E = E->next()) {
                    E->key()->material_changed_notify();
                }

                for (Map<RasterizerScene::InstanceBase*, int>::Element* E =
                         material->instance_owners.front();
                     E;
                     E = E->next()) {
    ...
    if (material->ubo_size) {
        if (!material->shader
            || material->shader->ubo_size != material->ubo_size) {
            // by by ubo
    ...
    if (material->ubo_size == 0 && material->shader
        && material->shader->ubo_size) {
    ...
    if (material->shader && material->ubo_size) {
        ...
        for (Map<StringName, ShaderLanguage::ShaderNode::Uniform>::Element* E =
                 material->shader->uniforms.front();
             E;
             E = E->next()) {
            if (E->get().order < 0) {
            ...
            // regular uniform
            uint8_t* data =
                &local_ubo[material->shader->ubo_offsets[E->get().order]];

            Map<StringName, Variant>::Element* V =
                material->params.find(E->key());
            ...
            if (V) {
                ...
                _fill_std140_variant_ubo_value(
                    ...
                    V->get(),
                    data,
                    material->shader->mode == VS::SHADER_SPATIAL
                );
                ...
                if (E->get().type == ShaderLanguage::TYPE_VEC4
                    && E->get().hint
    ...
    if (material->shader && material->shader->texture_count) {
        ...
        for (Map<StringName, ShaderLanguage::ShaderNode::Uniform>::Element* E =
            ...
            switch (E->get().type) {
                case ShaderLanguage::TYPE_SAMPLER3D:
                case ShaderLanguage::TYPE_SAMPLER2DARRAY: {
                    material->texture_is_3d.write[E->get().texture_order] =
                        true;
                } break;
                default: {
                    material->texture_is_3d.write[E->get().texture_order] =
            ...
            if (!texture.is_valid()) {
                Map<StringName, RID>::Element* W =
                    material->shader->default_textures.find(E->key());
                if (W) {
                    texture = W->get();
                }
            }
...
void RasterizerStorageGLES3::_material_add_geometry(
    RID p_material,
    Geometry* p_geometry
) {
    ...
    Map<Geometry*, int>::Element* I =
        material->geometry_owners.find(p_geometry);
...
void RasterizerStorageGLES3::_material_remove_geometry(
    RID p_material,
    Geometry* p_geometry
) {
    Material* material = material_owner.getornull(p_material);
...
void RasterizerStorageGLES3::mesh_add_surface(
    RID p_mesh,
    uint32_t p_format,
    ...
    {
        uint32_t bones_weight =
            VS::ARRAY_FORMAT_BONES | VS::ARRAY_FORMAT_WEIGHTS;
        ERR_FAIL_COND_MSG(
            (p_format & bones_weight)
                && (p_format & bones_weight) != bones_weight,
            "Array must have both bones and weights in format or none."
        );
    }

    // bool has_morph = p_blend_shapes.size();
    bool use_split_stream =
        GLOBAL_GET("rendering/misc/mesh_storage/split_stream")
        ...
        switch (i) {
            case VS::ARRAY_VERTEX: {
                if (p_format & VS::ARRAY_FLAG_USE_2D_VERTICES) {
                    ...
                    attribs[i].size =
                        (p_format & VS::ARRAY_COMPRESS_VERTEX) ? 4 : 3;
                }
    ...
    for (int i = 0; i < surface->skeleton_bone_used.size(); i++) {
        if (surface->skeleton_bone_aabb[i].size.x < 0
            || surface->skeleton_bone_aabb[i].size.y < 0
            || surface->skeleton_bone_aabb[i].size.z < 0) {
...

    {
        ...
        glBufferData(
            ...
            array_size,
            vr.ptr(),
            (p_format & VS::ARRAY_FLAG_USE_DYNAMIC_UPDATE) ? GL_DYNAMIC_DRAW
                                                           : GL_STATIC_DRAW
        );
        ...
        if (config.generate_wireframes
            && p_primitive == VS::PRIMITIVE_TRIANGLES) {
            ...
            for (int ai = 0; ai < 2; ai++) {
                if (ai == 0) {
                    ...
                    glGenVertexArrays(
                        1,
                        &surface->instancing_array_wireframe_id
                    );
            ...
            for (int j = 0; j < VS::ARRAY_MAX - 1; j++) {
                ...
                if (attribs[j].integer) {
                    glVertexAttribIPointer(
                        ...
                        attribs[j].type,
                        attribs[j].stride,
                        CAST_INT_TO_UCHAR_PTR(attribs[j].offset)
                    );
                } else {
                    glVertexAttribPointer(
                        attribs[j].index,
...
void RasterizerStorageGLES3::mesh_set_blend_shape_count(
    RID p_mesh,
    int p_amount
) {
    Mesh* mesh = mesh_owner.getornull(p_mesh);
...
void RasterizerStorageGLES3::mesh_set_blend_shape_mode(
    RID p_mesh,
    VS::BlendShapeMode p_mode
) {
    Mesh* mesh = mesh_owner.getornull(p_mesh);
    ERR_FAIL_COND(!mesh);
...
VS::BlendShapeMode RasterizerStorageGLES3::mesh_get_blend_shape_mode(RID p_mesh
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
void RasterizerStorageGLES3::mesh_set_blend_shape_values(
    RID p_mesh,
    PoolVector<float> p_values
) {
    Mesh* mesh = mesh_owner.getornull(p_mesh);
    ERR_FAIL_COND(!mesh);
...
PoolVector<float> RasterizerStorageGLES3::mesh_get_blend_shape_values(RID p_mesh
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
void RasterizerStorageGLES3::mesh_surface_update_region(
    RID p_mesh,
    int p_surface,
    int p_offset,
    ...
    ERR_FAIL_COND(
        p_offset + total_size > mesh->surfaces[p_surface]->array_byte_size
    );
...
void RasterizerStorageGLES3::mesh_surface_set_material(
    RID p_mesh,
    int p_surface,
    RID p_material
) {
    ...
    if (mesh->surfaces[p_surface]->material.is_valid()) {
        _material_remove_geometry(
            mesh->surfaces[p_surface]->material,
            mesh->surfaces[p_surface]
        );
    }
...
RID RasterizerStorageGLES3::mesh_surface_get_material(RID p_mesh, int p_surface)
    const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
int RasterizerStorageGLES3::mesh_surface_get_array_len(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
int RasterizerStorageGLES3::mesh_surface_get_array_index_len(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
PoolVector<uint8_t> RasterizerStorageGLES3::mesh_surface_get_array(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
PoolVector<uint8_t> RasterizerStorageGLES3::mesh_surface_get_index_array(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
uint32_t RasterizerStorageGLES3::mesh_surface_get_format(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
VS::PrimitiveType RasterizerStorageGLES3::mesh_surface_get_primitive_type(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
AABB RasterizerStorageGLES3::mesh_surface_get_aabb(RID p_mesh, int p_surface)
    const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
Vector<PoolVector<uint8_t>> RasterizerStorageGLES3::
    mesh_surface_get_blend_shapes(RID p_mesh, int p_surface) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
    ...
    for (int i = 0; i < mesh->surfaces[p_surface]->blend_shapes.size(); i++) {
        ...
        glBindBuffer(
            GL_ELEMENT_ARRAY_BUFFER,
            mesh->surfaces[p_surface]->blend_shapes[i].vertex_id
        );
...
#if defined(GLES_OVER_GL) || defined(__EMSCRIPTEN__)
        {
            ...
            glGetBufferSubData(
                GL_ELEMENT_ARRAY_BUFFER,
                0,
                mesh->surfaces[p_surface]->array_byte_size,
                w.ptr()
            );
        }
#else
...
Vector<AABB> RasterizerStorageGLES3::mesh_surface_get_skeleton_aabb(
    RID p_mesh,
    int p_surface
) const {
    const Mesh* mesh = mesh_owner.getornull(p_mesh);
...
void RasterizerStorageGLES3::mesh_set_custom_aabb(
    RID p_mesh,
    const AABB& p_aabb
) {
    Mesh* mesh = mesh_owner.getornull(p_mesh);
...
AABB RasterizerStorageGLES3::mesh_get_aabb(RID p_mesh, RID p_skeleton) const {
    ...
    if (sk && sk->size != 0) {
        for (int i = 0; i < mesh->surfaces.size(); i++) {
            AABB laabb;
            if ((mesh->surfaces[i]->format & VS::ARRAY_FORMAT_BONES)
                && mesh->surfaces[i]->skeleton_bone_aabb.size()) {
                int bs = mesh->surfaces[i]->skeleton_bone_aabb.size();
                const AABB* skbones =
                    mesh->surfaces[i]->skeleton_bone_aabb.ptr();
                const bool* skused =
                    mesh->surfaces[i]->skeleton_bone_used.ptr();
...
void RasterizerStorageGLES3::mesh_render_blend_shapes(
    Surface* s,
    ...
    static const int sizes[VS::ARRAY_MAX - 1] =
        {3 * 4, 3 * 4, 4 * 4, 4 * 4, 2 * 4, 2 * 4, 4 * 4, 4 * 4};

    for (int i = 1; i < VS::ARRAY_MAX - 1; i++) {
        shaders.blend_shapes.set_conditional(
            cond[i],
            s->format & (1 << i)
        ); // enable conditional for format
        if (s->format & (1 << i)) {
            stride += sizes[i];
        }
    }
    ...
    shaders.blend_shapes.set_conditional(
        BlendShapeShaderGLES3::ENABLE_BLEND,
        false
    ); // first pass does not blend
    shaders.blend_shapes.set_conditional(
        BlendShapeShaderGLES3::USE_2D_VERTEX,
        s->format & VS::ARRAY_FLAG_USE_2D_VERTICES
    ); // use 2D vertices if needed
    shaders.blend_shapes.set_conditional(
        BlendShapeShaderGLES3::ENABLE_OCTAHEDRAL_COMPRESSION,
        s->format & VS::ARRAY_FLAG_USE_OCTAHEDRAL_COMPRESSION
    ); // use octahedral normal compression

    shaders.blend_shapes.bind();

    shaders.blend_shapes.set_uniform(
        BlendShapeShaderGLES3::BLEND_AMOUNT,
        base_weight
    );
...
void RasterizerStorageGLES3::multimesh_allocate(
    RID p_multimesh,
    int p_instances,
    VS::MultimeshTransformFormat p_transform_format,
    VS::MultimeshColorFormat p_color_format,
    VS::MultimeshCustomDataFormat p_data_format
) {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
    ERR_FAIL_COND(!multimesh);

    if (multimesh->size == p_instances
    ...
    if (multimesh->size) {
        ...
        for (int i = 0; i < p_instances * format_floats; i += format_floats) {
            ...
            if (multimesh->custom_data_format
                == VS::MULTIMESH_CUSTOM_DATA_NONE) {
                // none
        ...
        glBufferData(
            GL_ARRAY_BUFFER,
            multimesh->data.size() * sizeof(float),
            nullptr,
            GL_STATIC_DRAW
        );
        glBindBuffer(GL_ARRAY_BUFFER, 0);
...
int RasterizerStorageGLES3::multimesh_get_instance_count(RID p_multimesh
) const {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
...
void RasterizerStorageGLES3::multimesh_instance_set_transform(
    RID p_multimesh,
    int p_index,
...
void RasterizerStorageGLES3::multimesh_instance_set_transform_2d(
    RID p_multimesh,
    int p_index,
...
void RasterizerStorageGLES3::multimesh_instance_set_color(
    RID p_multimesh,
    int p_index,
    const Color& p_color
) {
    ...
    int stride = multimesh->color_floats + multimesh->xform_floats
               + multimesh->custom_data_floats;
    float* dataptr =
        &multimesh->data.write[stride * p_index + multimesh->xform_floats];
...
void RasterizerStorageGLES3::multimesh_instance_set_custom_data(
    RID p_multimesh,
    int p_index,
    const Color& p_custom_data
) {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
    ERR_FAIL_COND(!multimesh);
    ERR_FAIL_INDEX(p_index, multimesh->size);
    ERR_FAIL_COND(
        multimesh->custom_data_format == VS::MULTIMESH_CUSTOM_DATA_NONE
    );
    ERR_FAIL_INDEX(
        multimesh->custom_data_format,
        VS::MULTIMESH_CUSTOM_DATA_MAX
    );

    int stride = multimesh->color_floats + multimesh->xform_floats
               + multimesh->custom_data_floats;
    float* dataptr = &multimesh->data.write
                          [stride * p_index + multimesh->xform_floats
                           + multimesh->color_floats];
...
Transform RasterizerStorageGLES3::multimesh_instance_get_transform(
    RID p_multimesh,
    int p_index
) const {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
    ...
    ERR_FAIL_COND_V(
        multimesh->transform_format == VS::MULTIMESH_TRANSFORM_2D,
        Transform()
    );
...
Transform2D RasterizerStorageGLES3::multimesh_instance_get_transform_2d(
    RID p_multimesh,
    int p_index
) const {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
    ...
    ERR_FAIL_COND_V(
        multimesh->transform_format == VS::MULTIMESH_TRANSFORM_3D,
        Transform2D()
    );
...
Color RasterizerStorageGLES3::multimesh_instance_get_color(
    RID p_multimesh,
    int p_index
) const {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
    ...
    ERR_FAIL_COND_V(
        multimesh->color_format == VS::MULTIMESH_COLOR_NONE,
        Color()
    );
...
Color RasterizerStorageGLES3::multimesh_instance_get_custom_data(
    RID p_multimesh,
    int p_index
) const {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
...
void RasterizerStorageGLES3::multimesh_set_as_bulk_array(
    RID p_multimesh,
...
void RasterizerStorageGLES3::multimesh_set_visible_instances(
    RID p_multimesh,
    int p_visible
) {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
...
int RasterizerStorageGLES3::multimesh_get_visible_instances(RID p_multimesh
) const {
    MultiMesh* multimesh = multimesh_owner.getornull(p_multimesh);
...
AABB RasterizerStorageGLES3::multimesh_get_aabb(RID p_multimesh) const {
    ...
    const_cast<RasterizerStorageGLES3*>(this)->update_dirty_multimeshes(
...
void RasterizerStorageGLES3::update_dirty_multimeshes() {
    while (multimesh_update_list.first()) {
        ...
        if (multimesh->size && multimesh->dirty_data) {
            ...
            // this could potentially have a project setting for API options as
            // with 2d if (config.should_orphan) {
            glBufferData(
                GL_ARRAY_BUFFER,
                buffer_size,
                multimesh->data.ptr(),
...
void RasterizerStorageGLES3::immediate_begin(
    RID p_immediate,
    VS::PrimitiveType p_primitive,
    RID p_texture
) {
...
void RasterizerStorageGLES3::immediate_vertex(
    RID p_immediate,
...
void RasterizerStorageGLES3::immediate_normal(
    RID p_immediate,
...
void RasterizerStorageGLES3::immediate_tangent(
    RID p_immediate,
...
void RasterizerStorageGLES3::immediate_color(
    RID p_immediate,
    const Color& p_color
) {
    Immediate* im = immediate_owner.get(p_immediate);
...
void RasterizerStorageGLES3::immediate_uv(
    RID p_immediate,
...
void RasterizerStorageGLES3::immediate_uv2(
    RID p_immediate,
    const Vector2& tex_uv
) {
    Immediate* im = immediate_owner.get(p_immediate);
...
void RasterizerStorageGLES3::immediate_set_material(
    RID p_immediate,
    RID p_material
) {
    Immediate* im = immediate_owner.get(p_immediate);
...
void RasterizerStorageGLES3::skeleton_allocate(
    RID p_skeleton,
    int p_bones,
    bool p_2d_skeleton
) {
...
void RasterizerStorageGLES3::skeleton_bone_set_transform(
    RID p_skeleton,
    int p_bone,
    const Transform& p_transform
) {
...
Transform RasterizerStorageGLES3::skeleton_bone_get_transform(
    RID p_skeleton,
    int p_bone
) const {
    Skeleton* skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES3::skeleton_bone_set_transform_2d(
    RID p_skeleton,
    int p_bone,
    const Transform2D& p_transform
) {
...
Transform2D RasterizerStorageGLES3::skeleton_bone_get_transform_2d(
    RID p_skeleton,
    int p_bone
) const {
    Skeleton* skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES3::skeleton_set_base_transform_2d(
    RID p_skeleton,
    ...
    while (skeleton_update_list.first()) {
        ...
        if (skeleton->size) {
            ...
            glTexSubImage2D(
                ...
                0,
                256,
                height * (skeleton->use_2d ? 2 : 3),
                GL_RGBA,
                GL_FLOAT,
                skeleton->skel_texture.ptr()
            );
        }

        for (Set<RasterizerScene::InstanceBase*>::Element* E =
                 skeleton->instances.front();
             E;
             E = E->next()) {
...
void RasterizerStorageGLES3::light_set_color(
    RID p_light,
    const Color& p_color
) {
...
void RasterizerStorageGLES3::light_set_param(
    RID p_light,
    VS::LightParam p_param,
    float p_value
) {
    Light* light = light_owner.getornull(p_light);
    ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES3::light_set_shadow_color(
    RID p_light,
    const Color& p_color
) {
    Light* light = light_owner.getornull(p_light);
...
void RasterizerStorageGLES3::light_set_reverse_cull_face_mode(
    RID p_light,
    bool p_enabled
) {
    Light* light = light_owner.getornull(p_light);
...
void RasterizerStorageGLES3::light_set_use_gi(RID p_light, bool p_enabled) {
    WARN_DEPRECATED_MSG(
        "'VisualServer.light_set_use_gi' is deprecated and will be removed in "
        "a future version. Use 'VisualServer.light_set_bake_mode' instead."
    );
    light_set_bake_mode(
        p_light,
        p_enabled ? VS::LightBakeMode::LIGHT_BAKE_INDIRECT
                  : VS::LightBakeMode::LIGHT_BAKE_DISABLED
    );
}

void RasterizerStorageGLES3::light_set_bake_mode(
    RID p_light,
    VS::LightBakeMode p_bake_mode
) {
    Light* light = light_owner.getornull(p_light);
    ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES3::light_omni_set_shadow_mode(
    RID p_light,
    VS::LightOmniShadowMode p_mode
) {
    Light* light = light_owner.getornull(p_light);
    ERR_FAIL_COND(!light);
...
VS::LightOmniShadowMode RasterizerStorageGLES3::light_omni_get_shadow_mode(
    RID p_light
) {
...
void RasterizerStorageGLES3::light_omni_set_shadow_detail(
    RID p_light,
...
void RasterizerStorageGLES3::light_directional_set_shadow_mode(
    RID p_light,
...
void RasterizerStorageGLES3::light_directional_set_blend_splits(
    RID p_light,
    bool p_enable
) {
    Light* light = light_owner.getornull(p_light);
...
bool RasterizerStorageGLES3::light_directional_get_blend_splits(RID p_light
) const {
    const Light* light = light_owner.getornull(p_light);
...
VS::LightDirectionalShadowMode RasterizerStorageGLES3::
    light_directional_get_shadow_mode(RID p_light) {
    const Light* light = light_owner.getornull(p_light);
...
void RasterizerStorageGLES3::light_directional_set_shadow_depth_range_mode(
    RID p_light,
...
VS::LightDirectionalShadowDepthRangeMode RasterizerStorageGLES3::
    light_directional_get_shadow_depth_range_mode(RID p_light) const {
    const Light* light = light_owner.getornull(p_light);
...
float RasterizerStorageGLES3::light_get_param(
    RID p_light,
    VS::LightParam p_param
) {
    const Light* light = light_owner.getornull(p_light);
...
bool RasterizerStorageGLES3::light_get_use_gi(RID p_light) {
    return light_get_bake_mode(p_light)
        != VS::LightBakeMode::LIGHT_BAKE_DISABLED;
}
...
AABB RasterizerStorageGLES3::light_get_aabb(RID p_light) const {
    ...
    switch (light->type) {
        case VS::LIGHT_SPOT: {
            ...
            float size =
                Math::tan(Math::deg2rad(light->param[VS::LIGHT_PARAM_SPOT_ANGLE]
                ))
                * len;
            return AABB(
                Vector3(-size, -size, -len),
                Vector3(size * 2, size * 2, len)
            );
        };
        case VS::LIGHT_OMNI: {
...
void RasterizerStorageGLES3::reflection_probe_set_update_mode(
    RID p_probe,
    VS::ReflectionProbeUpdateMode p_mode
) {
    ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES3::reflection_probe_set_intensity(
    RID p_probe,
    float p_intensity
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_interior_ambient(
    RID p_probe,
...
void RasterizerStorageGLES3::reflection_probe_set_interior_ambient_energy(
    RID p_probe,
    float p_energy
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_max_distance(
    RID p_probe,
    float p_distance
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_extents(
    RID p_probe,
...
void RasterizerStorageGLES3::reflection_probe_set_origin_offset(
    RID p_probe,
...
void RasterizerStorageGLES3::reflection_probe_set_as_interior(
    RID p_probe,
    bool p_enable
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_enable_box_projection(
    RID p_probe,
    bool p_enable
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_enable_shadows(
    RID p_probe,
    bool p_enable
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_cull_mask(
    RID p_probe,
    uint32_t p_layers
) {
    ReflectionProbe* reflection_probe =
...
void RasterizerStorageGLES3::reflection_probe_set_resolution(
    RID p_probe,
    int p_resolution
) {}

AABB RasterizerStorageGLES3::reflection_probe_get_aabb(RID p_probe) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
VS::ReflectionProbeUpdateMode RasterizerStorageGLES3::
    reflection_probe_get_update_mode(RID p_probe) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
uint32_t RasterizerStorageGLES3::reflection_probe_get_cull_mask(RID p_probe
) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
Vector3 RasterizerStorageGLES3::reflection_probe_get_extents(RID p_probe
) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
Vector3 RasterizerStorageGLES3::reflection_probe_get_origin_offset(RID p_probe
) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
    ERR_FAIL_COND_V(!reflection_probe, Vector3());
...
bool RasterizerStorageGLES3::reflection_probe_renders_shadows(RID p_probe
) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
float RasterizerStorageGLES3::reflection_probe_get_origin_max_distance(
    RID p_probe
) const {
    const ReflectionProbe* reflection_probe =
        reflection_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES3::gi_probe_set_bounds(
    RID p_probe,
    const AABB& p_bounds
) {
...
void RasterizerStorageGLES3::gi_probe_set_to_cell_xform(
    RID p_probe,
...
Transform RasterizerStorageGLES3::gi_probe_get_to_cell_xform(RID p_probe
) const {
    const GIProbe* gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES3::gi_probe_set_dynamic_data(
    RID p_probe,
    const PoolVector<int>& p_data
) {
    GIProbe* gip = gi_probe_owner.getornull(p_probe);
    ERR_FAIL_COND(!gip);
...
PoolVector<int> RasterizerStorageGLES3::gi_probe_get_dynamic_data(RID p_probe
) const {
    const GIProbe* gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES3::gi_probe_set_dynamic_range(
    RID p_probe,
    int p_range
) {
    GIProbe* gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES3::gi_probe_set_normal_bias(
    RID p_probe,
    float p_range
) {
    GIProbe* gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES3::gi_probe_set_propagation(
    RID p_probe,
    float p_range
) {
    GIProbe* gip = gi_probe_owner.getornull(p_probe);
...
RID RasterizerStorageGLES3::gi_probe_dynamic_data_create(
    int p_width,
    int p_height,
    int p_depth,
    ...
    while (true) {
        ...
        if (p_width <= min_size || p_height <= min_size
            || p_depth <= min_size) {
            break;
        }
        p_width  >>= 1;
...
void RasterizerStorageGLES3::gi_probe_dynamic_data_update(
    RID p_gi_probe_data,
    int p_depth_slice,
    int p_slice_count,
    int p_mipmap,
    ...
    for(int i=0;i<(gipd->width>>p_mipmap);i++) {
        for(int j=0;j<(gipd->height>>p_mipmap);j++) {
            for(int k=0;k<(gipd->depth>>p_mipmap);k++) {
                ...
                int ofs = (k*(gipd->height>>p_mipmap)*(gipd->width>>p_mipmap)) +
    j *(gipd->width>>p_mipmap) + i; ofs*=4;
...
void RasterizerStorageGLES3::lightmap_capture_set_bounds(
    RID p_capture,
    const AABB& p_bounds
) {
...
void RasterizerStorageGLES3::lightmap_capture_set_octree(
    RID p_capture,
    ...
    ERR_FAIL_COND(
        p_octree.size() == 0
        || (p_octree.size() % sizeof(LightmapCaptureOctree)) != 0
    );
...
PoolVector<uint8_t> RasterizerStorageGLES3::lightmap_capture_get_octree(
    RID p_capture
) const {
    const LightmapCapture* capture =
        lightmap_capture_data_owner.getornull(p_capture);
...
void RasterizerStorageGLES3::lightmap_capture_set_octree_cell_transform(
    RID p_capture,
    const Transform& p_xform
) {
    LightmapCapture* capture = lightmap_capture_data_owner.getornull(p_capture);
...
Transform RasterizerStorageGLES3::lightmap_capture_get_octree_cell_transform(
    RID p_capture
) const {
    const LightmapCapture* capture =
        lightmap_capture_data_owner.getornull(p_capture);
...
void RasterizerStorageGLES3::lightmap_capture_set_octree_cell_subdiv(
    RID p_capture,
    int p_subdiv
) {
    LightmapCapture* capture = lightmap_capture_data_owner.getornull(p_capture);
...
int RasterizerStorageGLES3::lightmap_capture_get_octree_cell_subdiv(
    RID p_capture
) const {
    const LightmapCapture* capture =
        lightmap_capture_data_owner.getornull(p_capture);
...
void RasterizerStorageGLES3::lightmap_capture_set_energy(
    RID p_capture,
    float p_energy
) {
    LightmapCapture* capture = lightmap_capture_data_owner.getornull(p_capture);
...
void RasterizerStorageGLES3::lightmap_capture_set_interior(
    RID p_capture,
    bool p_interior
) {
    LightmapCapture* capture = lightmap_capture_data_owner.getornull(p_capture);
...
const PoolVector<RasterizerStorage::LightmapCaptureOctree>*
RasterizerStorageGLES3::lightmap_capture_get_octree_ptr(RID p_capture) const {
    const LightmapCapture* capture =
        lightmap_capture_data_owner.getornull(p_capture);
...
void RasterizerStorageGLES3::particles_set_emitting(
    RID p_particles,
    bool p_emitting
) {
...
void RasterizerStorageGLES3::particles_set_amount(
    RID p_particles,
    int p_amount
) {
    Particles* particles = particles_owner.getornull(p_particles);
    ...
    if (particles->histories_enabled) {
        for (int i = 0; i < 2; i++) {
            ...
            glBindBuffer(
                GL_ARRAY_BUFFER,
                particles->particle_buffer_histories[i]
            );
            glBufferData(
                GL_ARRAY_BUFFER,
                floats * sizeof(float),
...
void RasterizerStorageGLES3::particles_set_lifetime(
    RID p_particles,
    float p_lifetime
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_one_shot(
    RID p_particles,
    bool p_one_shot
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_pre_process_time(
    RID p_particles,
    float p_time
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_explosiveness_ratio(
    RID p_particles,
    float p_ratio
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_randomness_ratio(
    RID p_particles,
    float p_ratio
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::_particles_update_histories(Particles* particles) {
    bool needs_histories =
        particles->draw_order == VS::PARTICLES_DRAW_ORDER_VIEW_DEPTH;
...
void RasterizerStorageGLES3::particles_set_custom_aabb(
    RID p_particles,
    const AABB& p_aabb
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_speed_scale(
    RID p_particles,
    float p_scale
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_use_local_coordinates(
    RID p_particles,
    bool p_enable
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_fixed_fps(
    RID p_particles,
    int p_fps
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_fractional_delta(
    RID p_particles,
    bool p_enable
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_process_material(
    RID p_particles,
    RID p_material
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_draw_passes(
    RID p_particles,
    int p_passes
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::particles_set_draw_pass_mesh(
    RID p_particles,
    int p_pass,
    RID p_mesh
) {
...
void RasterizerStorageGLES3::particles_set_emission_transform(
    RID p_particles,
    const Transform& p_transform
) {
    Particles* particles = particles_owner.getornull(p_particles);
...
RID RasterizerStorageGLES3::particles_get_draw_pass_mesh(
    RID p_particles,
    int p_pass
) const {
    const Particles* particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES3::_particles_process(
    Particles* p_particles,
    float p_delta
) {
    float new_phase = Math::fmod(
        (float)p_particles->phase
            + (p_delta / p_particles->lifetime) * p_particles->speed_scale,
        (float)1.0
    );
    ...
    if (p_particles->clear) {
        ...
        if (p_particles->one_shot) {
            ...
            shaders.particles.set_uniform(
                ParticlesShaderGLES3::EMITTING,
                false
            );
        }
    ...
    shaders.particles.set_uniform(
        ParticlesShaderGLES3::SYSTEM_PHASE,
        new_phase
    );
    shaders.particles.set_uniform(
        ParticlesShaderGLES3::PREV_SYSTEM_PHASE,
        p_particles->phase
    );
    p_particles->phase = new_phase;

    shaders.particles.set_uniform(
        ParticlesShaderGLES3::DELTA,
        p_delta * p_particles->speed_scale
    );
    shaders.particles.set_uniform(
        ParticlesShaderGLES3::CLEAR,
        p_particles->clear
    );
    glUniform1ui(
        shaders.particles.get_uniform_location(ParticlesShaderGLES3::RANDOM_SEED
        ),
        p_particles->random_seed
    );

    if (p_particles->use_local_coords) {
        shaders.particles.set_uniform(
            ParticlesShaderGLES3::EMISSION_TRANSFORM,
            Transform()
        );
    } else {
        shaders.particles.set_uniform(
    ...
    glUniform1ui(
        shaders.particles.get_uniform(ParticlesShaderGLES3::CYCLE),
        p_particles->cycle_number
    );
    ...
    float *data = (float *)glMapBufferRange(GL_ARRAY_BUFFER, 0,
    p_particles->amount * 16 * 6, GL_MAP_READ_BIT); for (int i = 0; i <
    p_particles->amount; i++) { int ofs = i * 24; print_line(itos(i) + ":");
        print_line("\tColor: " + Color(data[ofs + 0], data[ofs + 1], data[ofs +
    2], data[ofs + 3])); print_line("\tVelocity: " + Vector3(data[ofs + 4],
    data[ofs + 5], data[ofs + 6])); print_line("\tActive: " + itos(data[ofs +
    7])); print_line("\tCustom: " + Color(data[ofs + 8], data[ofs + 9], data[ofs
    + 10], data[ofs + 11])); print_line("\tXF X: " + Color(data[ofs + 12],
    data[ofs + 13], data[ofs + 14], data[ofs + 15])); print_line("\tXF Y: " +
    Color(data[ofs + 16], data[ofs + 17], data[ofs + 18], data[ofs + 19]));
        print_line("\tXF Z: " + Color(data[ofs + 20], data[ofs + 21], data[ofs +
    22], data[ofs + 23]));
    }
...
void RasterizerStorageGLES3::update_particles() {
    ...
    while (particle_update_list.first()) {
        ...
        Material* material =
            material_owner.getornull(particles->process_material);
        if (!material || !material->shader
            || material->shader->mode != VS::SHADER_PARTICLES) {
            ...
            ShaderLanguage::ShaderNode::Uniform::Hint* texture_hints =
                material->shader->texture_hints.ptrw();
            ...
            for (int i = 0; i < tc; i++) {
                ...
                RasterizerStorageGLES3::Texture* t =
                    texture_owner.getornull(textures[i]);
        ...
        shaders.particles.set_conditional(
            ParticlesShaderGLES3::USE_FRACTIONAL_DELTA,
            particles->fractional_delta
        );

        shaders.particles.bind();

        shaders.particles.set_uniform(
            ParticlesShaderGLES3::TOTAL_PARTICLES,
            particles->amount
        );
        shaders.particles.set_uniform(
            ParticlesShaderGLES3::TIME,
            frame.time[0]
        );
        shaders.particles.set_uniform(
            ParticlesShaderGLES3::EXPLOSIVENESS,
            particles->explosiveness
        );
        shaders.particles.set_uniform(
            ParticlesShaderGLES3::LIFETIME,
            particles->lifetime
        );
        ...
        shaders.particles.set_uniform(
            ParticlesShaderGLES3::RANDOMNESS,
        ...
        if (particles->histories_enabled) {
            SWAP(
                particles->particle_buffer_histories[0],
                particles->particle_buffer_histories[1]
            );
            SWAP(
                particles->particle_vao_histories[0],
                particles->particle_vao_histories[1]
            );
            SWAP(
                particles->particle_valid_histories[0],
...
void RasterizerStorageGLES3::instance_add_skeleton(
    RID p_skeleton,
    RasterizerScene::InstanceBase* p_instance
) {
    Skeleton* skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES3::instance_remove_skeleton(
    RID p_skeleton,
    RasterizerScene::InstanceBase* p_instance
) {
    Skeleton* skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES3::instance_add_dependency(
    RID p_base,
    RasterizerScene::InstanceBase* p_instance
) {
...
void RasterizerStorageGLES3::instance_remove_dependency(
    RID p_base,
    RasterizerScene::InstanceBase* p_instance
) {
    Instantiable* inst = nullptr;
...
void RasterizerStorageGLES3::_render_target_allocate(RenderTarget* rt) {
    ...
    bool hdr =
        rt->flags[RENDER_TARGET_HDR] && config.framebuffer_half_float_supported;
    // hdr = false;

    if (!hdr || rt->flags[RENDER_TARGET_NO_3D]) {
        if (rt->flags[RENDER_TARGET_NO_3D_EFFECTS]
            && !rt->flags[RENDER_TARGET_TRANSPARENT]) {
    ...
    if (!rt->flags[RENDER_TARGET_NO_3D]
        ...
        if (msaa > max_samples) {
            WARN_PRINT(
                "MSAA must be <= GL_MAX_SAMPLES, falling-back to "
                "GL_MAX_SAMPLES = "
                + itos(max_samples)
            );
            msaa = max_samples;
        ...
        if (!rt->flags[RENDER_TARGET_NO_3D_EFFECTS]) {
            ...
            for (int i = 0; i < 4;
                 i++) { // 5, but 4 mips, base is read directly to save bw
    ...
    if (!rt->flags[RENDER_TARGET_NO_SAMPLING] && rt->width >= 2
        && rt->height >= 2) {
        for (int i = 0; i < 2; i++) {
            ...
            for (int j = 0; j < rt->effects.mip_maps[i].sizes.size(); j++) {
                RenderTarget::Effects::MipMaps::Size& mm =
                    rt->effects.mip_maps[i].sizes.write[j];
                ...
                glViewport(
                    0,
                    0,
                    rt->effects.mip_maps[i].sizes[j].width,
                    rt->effects.mip_maps[i].sizes[j].height
                );
...
void RasterizerStorageGLES3::render_target_set_position(
    RID p_render_target,
    int p_x,
    int p_y
) {
...
void RasterizerStorageGLES3::render_target_set_size(
    RID p_render_target,
    int p_width,
    int p_height
) {
...
uint32_t RasterizerStorageGLES3::render_target_get_depth_texture_id(
    RID p_render_target
) const {
    RenderTarget* rt = render_target_owner.getornull(p_render_target);
...
void RasterizerStorageGLES3::render_target_set_external_texture(
    RID p_render_target,
    ...
    if (p_texture_id == 0) {
        ...
        // is there a point to setting the internal formats? we don't know
        // them..
...
void RasterizerStorageGLES3::render_target_set_flag(
    RID p_render_target,
...
void RasterizerStorageGLES3::render_target_set_msaa(
    RID p_render_target,
    VS::ViewportMSAA p_msaa
) {
    RenderTarget* rt = render_target_owner.getornull(p_render_target);
    ERR_FAIL_COND(!rt);
...
void RasterizerStorageGLES3::render_target_set_use_fxaa(
    RID p_render_target,
    bool p_fxaa
) {
    RenderTarget* rt = render_target_owner.getornull(p_render_target);
...
void RasterizerStorageGLES3::render_target_set_use_debanding(
    RID p_render_target,
    bool p_debanding
) {
    RenderTarget* rt = render_target_owner.getornull(p_render_target);
...
void RasterizerStorageGLES3::render_target_set_sharpen_intensity(
    RID p_render_target,
    float p_intensity
) {
    RenderTarget* rt = render_target_owner.getornull(p_render_target);
...
void RasterizerStorageGLES3::canvas_light_occluder_set_polylines(
    RID p_occluder,
    ...
    if (p_lines.size()) {
        ...
        // if same buffer len is being set, just use BufferSubData to avoid a
        // pipeline flush
...
bool RasterizerStorageGLES3::free(RID p_rid) {
    if (render_target_owner.owns(p_rid)) {
        ...
        ERR_FAIL_COND_V(
            ...
            true
        ); // can't free the render target texture, dude
        info.texture_mem -= texture->total_data_size;
        ...
        multimesh_allocate(
            p_rid,
            0,
            VS::MULTIMESH_TRANSFORM_2D,
            VS::MULTIMESH_COLOR_NONE
        ); // frees multimesh
        ...
        LightmapCapture* lightmap_capture =
            lightmap_capture_data_owner.get(p_rid);
...
void RasterizerStorageGLES3::render_info_end_capture() {
    ...
    info.snap.draw_call_count =
        info.render.draw_call_count - info.snap.draw_call_count;
    info.snap.material_switch_count =
        info.render.material_switch_count - info.snap.material_switch_count;
    info.snap.surface_switch_count =
        info.render.surface_switch_count - info.snap.surface_switch_count;
    info.snap.shader_rebind_count =
        info.render.shader_rebind_count - info.snap.shader_rebind_count;
    info.snap.vertices_count =
        info.render.vertices_count - info.snap.vertices_count;
    info.snap._2d_item_count =
        info.render._2d_item_count - info.snap._2d_item_count;
    info.snap._2d_draw_call_count =
        info.render._2d_draw_call_count - info.snap._2d_draw_call_count;
}
...
void RasterizerStorageGLES3::initialize() {
    ...
    config.latc_supported =
        config.extensions.has("GL_EXT_texture_compression_latc");
    config.bptc_supported =
        config.extensions.has("GL_ARB_texture_compression_bptc");
#ifdef GLES_OVER_GL
    ...
    config.s3tc_supported =
        config.extensions.has("GL_EXT_texture_compression_dxt1")
        || config.extensions.has("GL_EXT_texture_compression_s3tc")
        || config.extensions.has("WEBGL_compressed_texture_s3tc");
    config.rgtc_supported =
        config.extensions.has("GL_EXT_texture_compression_rgtc")
        || config.extensions.has("GL_ARB_texture_compression_rgtc")
        || config.extensions.has("EXT_texture_compression_rgtc");
    config.texture_float_linear_supported =
        config.extensions.has("GL_OES_texture_float_linear");
    config.framebuffer_float_supported =
        config.extensions.has("GL_EXT_color_buffer_float");
    config.framebuffer_half_float_supported =
        config.extensions.has("GL_EXT_color_buffer_half_float")
        || config.framebuffer_float_supported;

    // If the desktop build is using S3TC, and you export / run from the IDE for
    // android, if the device supports S3TC it will crash trying to load these
    ...
    config.srgb_decode_supported =
        config.extensions.has("GL_EXT_texture_sRGB_decode");

    config.anisotropic_level = 1.0;
    config.use_anisotropic_filter =
        config.extensions.has("GL_EXT_texture_filter_anisotropic");
    if (config.use_anisotropic_filter) {
        glGetFloatv(
    ...
    glGetIntegerv(
        ...
        &config.max_cubemap_texture_size
    );
...

    {
        ...
        uint32_t xf_feedback_size = GLOBAL_DEF_RST(
        ...
        for (int i = 0; i < 2; i++) {
            ...
            glBindBuffer(
                GL_ARRAY_BUFFER,
                resources.transform_feedback_buffers[i]
            );
            glBufferData(
                GL_ARRAY_BUFFER,
                xf_feedback_size * 1024,
                nullptr,
                GL_STREAM_DRAW
            );
        }
    ...
    config.use_texture_array_environment =
        GLOBAL_GET("rendering/quality/reflections/texture_array_reflections");

    config.force_vertex_shading =
    ...
    config.use_lightmap_filter_bicubic =
        GLOBAL_GET("rendering/quality/lightmapping/use_bicubic_sampling");

    config.use_physical_light_attenuation =
        GLOBAL_GET("rendering/quality/shading/use_physical_light_attenuation");

    config.use_depth_prepass =
        bool(GLOBAL_GET("rendering/quality/depth_prepass/enable"));
    if (config.use_depth_prepass) {
        String vendors =
            GLOBAL_GET("rendering/quality/depth_prepass/disable_for_vendors");
