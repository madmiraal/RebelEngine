...
static _FORCE_INLINE_ void store_transform(
    const Transform& p_mtx,
    float* p_array
) {
...
static _FORCE_INLINE_ void store_camera(
    const CameraMatrix& p_mtx,
    float* p_array
) {
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
void RasterizerSceneGLES3::shadow_atlas_set_size(RID p_atlas, int p_size) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        shadow_atlas->quadrants[i].shadows.resize(
            1 << shadow_atlas->quadrants[i].subdivision
        );
    }

    // erase shadow atlas reference from lights
    for (Map<RID, uint32_t>::Element* E = shadow_atlas->shadow_owners.front();
         E;
         E = E->next()) {
...
void RasterizerSceneGLES3::shadow_atlas_set_quadrant_subdivision(
    RID p_atlas,
    int p_quadrant,
    int p_subdivision
) {
    ...
    for (int i = 0; i < shadow_atlas->quadrants[p_quadrant].shadows.size();
         i++) {
        if (shadow_atlas->quadrants[p_quadrant].shadows[i].owner.is_valid()) {
            shadow_atlas->shadow_owners.erase(
                shadow_atlas->quadrants[p_quadrant].shadows[i].owner
            );
            LightInstance* li = light_instance_owner.getornull(
                shadow_atlas->quadrants[p_quadrant].shadows[i].owner
            );
            ERR_CONTINUE(!li);
            li->shadow_atlases.erase(p_atlas);
    ...
    for (int i = 0; i < 4; i++) {
        if (shadow_atlas->quadrants[i].subdivision) {
            shadow_atlas->smallest_subdiv =
                MIN(shadow_atlas->smallest_subdiv,
                    shadow_atlas->quadrants[i].subdivision);
        }
    }
    ...
    do {
        ...
        for (int i = 0; i < 3; i++) {
            if (shadow_atlas->quadrants[shadow_atlas->size_order[i]].subdivision
                < shadow_atlas->quadrants[shadow_atlas->size_order[i + 1]]
                      .subdivision) {
                SWAP(
                    shadow_atlas->size_order[i],
                    shadow_atlas->size_order[i + 1]
                );
                swaps++;
            }
        }
...
bool RasterizerSceneGLES3::_shadow_atlas_find_shadow(
    ...
    for (int i = p_quadrant_count - 1; i >= 0; i--) {
        ...
        if (shadow_atlas->quadrants[qidx].subdivision
            == (uint32_t)p_current_subdiv) {
            return false;
        }

        // look for an empty space
        int sc = shadow_atlas->quadrants[qidx].shadows.size();
        ShadowAtlas::Quadrant::Shadow* sarr =
            shadow_atlas->quadrants[qidx].shadows.ptrw();

        int found_free_idx = -1; // found a free one
        int found_used_idx = -1; // found existing one, must steal it
        uint64_t min_pass = 0; // pass of the existing one, try to use the least
                               // recently used one (LRU fashion)
...
bool RasterizerSceneGLES3::shadow_atlas_update_light(
    ...
    int desired_fit =
        MIN(quad_size / shadow_atlas->smallest_subdiv,
            next_power_of_2(quad_size * p_coverage));
    ...
    // find the quadrants this fits into, and the best possible size it can fit
    // into
    for (int i = 0; i < 4; i++) {
    ...
    if (shadow_atlas->shadow_owners.has(p_light_intance)) {
        ...
        bool should_realloc =
            shadow_atlas->quadrants[q].subdivision != (uint32_t)best_subdiv
            && (shadow_atlas->quadrants[q].shadows[s].alloc_tick - tick
                > shadow_atlas_realloc_tolerance_msec);
        bool should_redraw =
            shadow_atlas->quadrants[q].shadows[s].version != p_light_version;

        if (!should_realloc) {
            shadow_atlas->quadrants[q].shadows.write[s].version =
                p_light_version;
        ...
        // find a better place
        if (_shadow_atlas_find_shadow(
                shadow_atlas,
                valid_quadrants,
                valid_quadrant_count,
                shadow_atlas->quadrants[q].subdivision,
                tick,
                new_quadrant,
                new_shadow
            )) {
            // found a better place!
            ShadowAtlas::Quadrant::Shadow* sh =
                &shadow_atlas->quadrants[new_quadrant]
                     .shadows.write[new_shadow];
    ...
    if (_shadow_atlas_find_shadow(
        ...
        // found a better place!
        ShadowAtlas::Quadrant::Shadow* sh =
            &shadow_atlas->quadrants[new_quadrant].shadows.write[new_shadow];
        if (sh->owner.is_valid()) {
            // is taken, but is invalid, erasing it
            shadow_atlas->shadow_owners.erase(sh->owner);
            LightInstance* sli = light_instance_owner.get(sh->owner);
...
int RasterizerSceneGLES3::get_directional_light_shadow_size(RID p_light_intance
    ...
    if (directional_shadow.light_count == 1) {
        ...
        shadow_size =
            directional_shadow.size / 2; // more than 4 not supported anyway
    }

    LightInstance* light_instance =
        light_instance_owner.getornull(p_light_intance);
...
void RasterizerSceneGLES3::reflection_atlas_set_size(
    RID p_ref_atlas,
    int p_size
) {
    ReflectionAtlas* reflection_atlas =
        reflection_atlas_owner.getornull(p_ref_atlas);
    ...
    for (int i = 0; i < reflection_atlas->reflections.size(); i++) {
        ...
        if (reflection_atlas->reflections[i].owner.is_valid()) {
            ReflectionProbeInstance* reflection_probe_instance =
                reflection_probe_instance_owner.getornull(
                    reflection_atlas->reflections[i].owner
                );
    ...
    if (reflection_atlas->size) {
        ...
        GLenum type =
            use_float ? GL_HALF_FLOAT : GL_UNSIGNED_INT_2_10_10_10_REV;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            ); // it needs to be cleared, to avoid generating garbage
...
void RasterizerSceneGLES3::reflection_atlas_set_subdivision(
    RID p_ref_atlas,
    int p_subdiv
) {
    ReflectionAtlas* reflection_atlas =
...
RID RasterizerSceneGLES3::reflection_probe_instance_create(RID p_probe) {
    RasterizerStorageGLES3::ReflectionProbe* probe =
        storage->reflection_probe_owner.getornull(p_probe);
...
void RasterizerSceneGLES3::reflection_probe_instance_set_transform(
    RID p_instance,
    const Transform& p_transform
) {
    ReflectionProbeInstance* rpi =
        reflection_probe_instance_owner.getornull(p_instance);
    ERR_FAIL_COND(!rpi);
...
void RasterizerSceneGLES3::reflection_probe_release_atlas_index(RID p_instance
    ...
    ReflectionAtlas* reflection_atlas =
        reflection_atlas_owner.getornull(rpi->atlas);
    ERR_FAIL_COND(!reflection_atlas);

    ERR_FAIL_INDEX(
        rpi->reflection_atlas_index,
        reflection_atlas->reflections.size()
    );

    ERR_FAIL_COND(
        reflection_atlas->reflections[rpi->reflection_atlas_index].owner
        != rpi->self
    );

    reflection_atlas->reflections.write[rpi->reflection_atlas_index].owner =
        RID();
...
bool RasterizerSceneGLES3::reflection_probe_instance_needs_redraw(RID p_instance
    ...
    return rpi->reflection_atlas_index == -1
        || rpi->probe_ptr->update_mode == VS::REFLECTION_PROBE_UPDATE_ALWAYS;
}

bool RasterizerSceneGLES3::reflection_probe_instance_has_reflection(
    RID p_instance
) {
    ReflectionProbeInstance* rpi =
        reflection_probe_instance_owner.getornull(p_instance);
...
bool RasterizerSceneGLES3::reflection_probe_instance_begin_render(
    RID p_instance,
    ...
    ReflectionAtlas* reflection_atlas =
        reflection_atlas_owner.getornull(p_reflection_atlas);
    ...
    for (int i = 0; i < reflection_atlas->reflections.size(); i++) {
        ...
        if (rpi->render_step < 0
            && reflection_atlas->reflections[i].last_frame
                   < storage->frame.count
            && (best_used == -1
                || reflection_atlas->reflections[i].last_frame < best_used_frame
            )) {
            best_used       = i;
    ...
    if (best_free == -1) {
        ...
        ReflectionProbeInstance* victim_rpi =
            reflection_probe_instance_owner.getornull(
                reflection_atlas->reflections[best_free].owner
            );
    ...
    reflection_atlas->reflections.write[best_free].last_frame =
        storage->frame.count;
...
bool RasterizerSceneGLES3::reflection_probe_instance_postprocess_step(
    RID p_instance
) {
    ReflectionProbeInstance* rpi =
        reflection_probe_instance_owner.getornull(p_instance);
    ...
    glBindTexture(
        GL_TEXTURE_CUBE_MAP,
        reflection_cubemaps[cubemap_index].cubemap
    );
    glDisable(GL_CULL_FACE);

    storage->shaders.cubemap_filter.set_conditional(
        CubemapFilterShaderGLES3::USE_DUAL_PARABOLOID,
        true
    );
    ...
    int x =
        (rpi->reflection_atlas_index % reflection_atlas->subdiv) * cell_size;
    int y =
        (rpi->reflection_atlas_index / reflection_atlas->subdiv) * cell_size;
    int width  = cell_size;
    ...
    storage->shaders.cubemap_filter.set_conditional(
        CubemapFilterShaderGLES3::USE_DIRECT_WRITE,
        rpi->render_step == 0
    );
    storage->shaders.cubemap_filter.set_conditional(
        CubemapFilterShaderGLES3::LOW_QUALITY,
        rpi->probe_ptr->update_mode == VS::REFLECTION_PROBE_UPDATE_ALWAYS
    );
    for (int i = 0; i < 2; i++) {
        storage->shaders.cubemap_filter.set_uniform(
            CubemapFilterShaderGLES3::Z_FLIP,
            i == 0
        );
        storage->shaders.cubemap_filter.set_uniform(
            CubemapFilterShaderGLES3::ROUGHNESS,
            rpi->render_step / 5.0
        );
...
void RasterizerSceneGLES3::environment_set_background(
    RID p_env,
    VS::EnvironmentBG p_bg
) {
...
void RasterizerSceneGLES3::environment_set_sky_custom_fov(
    RID p_env,
    float p_scale
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES3::environment_set_sky_orientation(
    RID p_env,
...
void RasterizerSceneGLES3::environment_set_bg_color(
    RID p_env,
    const Color& p_color
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES3::environment_set_bg_energy(
    RID p_env,
    float p_energy
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES3::environment_set_canvas_max_layer(
    RID p_env,
    int p_max_layer
) {
    Environment* env = environment_owner.getornull(p_env);
...
void RasterizerSceneGLES3::environment_set_ambient_light(
    RID p_env,
    const Color& p_color,
    float p_energy,
    float p_sky_contribution
) {
    Environment* env = environment_owner.getornull(p_env);
    ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES3::environment_set_camera_feed_id(
    RID p_env,
...
void RasterizerSceneGLES3::environment_set_dof_blur_far(
    RID p_env,
    bool p_enable,
    float p_distance,
    float p_transition,
...
void RasterizerSceneGLES3::environment_set_dof_blur_near(
    RID p_env,
    bool p_enable,
    float p_distance,
    float p_transition,
...
void RasterizerSceneGLES3::environment_set_glow(
    RID p_env,
    bool p_enable,
    int p_level_flags,
    float p_intensity,
...
void RasterizerSceneGLES3::environment_set_fog(
    RID p_env,
    bool p_enable,
    float p_begin,
    float p_end,
...
void RasterizerSceneGLES3::environment_set_ssr(
    RID p_env,
    bool p_enable,
    int p_max_steps,
    float p_fade_in,
...
void RasterizerSceneGLES3::environment_set_ssao(
    RID p_env,
    bool p_enable,
    float p_radius,
    float p_intensity,
    ...
    VisualServer::EnvironmentSSAOBlur p_blur,
...
void RasterizerSceneGLES3::environment_set_tonemap(
    RID p_env,
...
void RasterizerSceneGLES3::environment_set_adjustment(
    RID p_env,
    bool p_enable,
    float p_brightness,
    float p_contrast,
...
void RasterizerSceneGLES3::environment_set_fog_depth(
    RID p_env,
    bool p_enable,
    float p_depth_begin,
    float p_depth_end,
...
void RasterizerSceneGLES3::environment_set_fog_height(
    RID p_env,
    bool p_enable,
    float p_min_height,
    float p_max_height,
...
RID RasterizerSceneGLES3::light_instance_create(RID p_light) {
    ...
    if (!light_instance->light_ptr) {
        ...
        ERR_FAIL_V_MSG(
            RID(),
            "Condition ' !light_instance->light_ptr ' is true."
        );
    }
...
void RasterizerSceneGLES3::light_instance_set_transform(
    RID p_light_instance,
    const Transform& p_transform
) {
    LightInstance* light_instance =
        light_instance_owner.getornull(p_light_instance);
    ERR_FAIL_COND(!light_instance);

    light_instance->transform = p_transform;
}

void RasterizerSceneGLES3::light_instance_set_shadow_transform(
    RID p_light_instance,
...
void RasterizerSceneGLES3::gi_probe_instance_set_light_data(
    RID p_probe,
    RID p_base,
    RID p_data
) {
    ...
    if (p_data.is_valid()) {
        RasterizerStorageGLES3::GIProbeData* gipd =
            storage->gi_probe_data_owner.getornull(p_data);
        ERR_FAIL_COND(!gipd);
...
void RasterizerSceneGLES3::gi_probe_instance_set_transform_to_data(
    RID p_probe,
...
void RasterizerSceneGLES3::gi_probe_instance_set_bounds(
    RID p_probe,
...
bool RasterizerSceneGLES3::_setup_material(
    RasterizerStorageGLES3::Material* p_material,
    bool p_depth_pass,
    bool p_alpha_pass
) {
    ...
    if (state.current_depth_test
        != (!p_material->shader->spatial.no_depth_test)) {
    ...
    if (state.current_depth_draw
        != p_material->shader->spatial.depth_draw_mode) {
        switch (p_material->shader->spatial.depth_draw_mode) {
            case RasterizerStorageGLES3::Shader::Spatial::
                DEPTH_DRAW_ALPHA_PREPASS: {
                glDepthMask(p_depth_pass);
                // If some transparent objects write to depth, we need to
                // re-copy depth texture when we need it
    ...
    ShaderLanguage::ShaderNode::Uniform::Hint* texture_hints =
        p_material->shader->texture_hints.ptrw();
    const ShaderLanguage::DataType* texture_types =
        p_material->shader->texture_types.ptr();
    ...
    for (int i = 0; i < tc; i++) {
        ...
        RasterizerStorageGLES3::Texture* t =
            storage->texture_owner.getptr(textures[i]);

        if (t) {
            if (t->redraw_if_visible) { // must check before proxy because this
                                        // is often used with proxies
        ...
        if (t && storage->config.srgb_decode_supported) {
            // if SRGB decode extension is present, simply switch the texture to
            // whatever is needed
            bool must_srgb = false;

            if (t->srgb
                && (texture_hints[i]
                        == ShaderLanguage::ShaderNode::Uniform::HINT_ALBEDO
                    || texture_hints[i]
...
struct RasterizerGLES3ParticleSort {
    ...
    bool operator()(
        const RasterizerGLES3Particle& p_a,
        const RasterizerGLES3Particle& p_b
    ) const {
        return z_dir.dot(Vector3(p_a.xform_1[3], p_a.xform_2[3], p_a.xform_3[3])
               )
             < z_dir.dot(Vector3(p_b.xform_1[3], p_b.xform_2[3], p_b.xform_3[3])
             );
    }
};

void RasterizerSceneGLES3::_setup_geometry(
    RenderList::Element* e,
    const Transform& p_view_transform
) {
    switch (e->instance->base_type) {
        case VS::INSTANCE_MESH: {
            RasterizerStorageGLES3::Surface* s =
                static_cast<RasterizerStorageGLES3::Surface*>(e->geometry);

            if (s->blend_shapes.size() && e->instance->blend_values.size()) {
                // blend shapes, use transform feedback
                storage->mesh_render_blend_shapes(
                    s,
                    e->instance->blend_values.read().ptr()
                );
                // rebind shader
        ...
        case VS::INSTANCE_MULTIMESH: {
            RasterizerStorageGLES3::MultiMesh* multi_mesh =
                static_cast<RasterizerStorageGLES3::MultiMesh*>(e->owner);
            RasterizerStorageGLES3::Surface* s =
                static_cast<RasterizerStorageGLES3::Surface*>(e->geometry);
#ifdef DEBUG_ENABLED
            if (state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_WIREFRAME
                && s->instancing_array_wireframe_id) {
                glBindVertexArray(s->instancing_array_wireframe_id
                ); // use the instancing array ID
            } else
#endif
            {
            ...
            int stride = (multi_mesh->xform_floats + multi_mesh->color_floats
                          + multi_mesh->custom_data_floats)
                       * 4;
            ...
            switch (multi_mesh->color_format) {
                ...
                case VS::MULTIMESH_COLOR_8BIT: {
                    ...
                    glVertexAttribPointer(
                        ...
                        GL_TRUE,
                        stride,
                        CAST_INT_TO_UCHAR_PTR(color_ofs)
                    );
            ...
            switch (multi_mesh->custom_data_format) {
                ...
                case VS::MULTIMESH_CUSTOM_DATA_8BIT: {
                    ...
                    glVertexAttribPointer(
                        ...
                        GL_TRUE,
                        stride,
                        CAST_INT_TO_UCHAR_PTR(custom_data_ofs)
                    );
        ...
        case VS::INSTANCE_PARTICLES: {
            RasterizerStorageGLES3::Particles* particles =
                static_cast<RasterizerStorageGLES3::Particles*>(e->owner);
            RasterizerStorageGLES3::Surface* s =
                static_cast<RasterizerStorageGLES3::Surface*>(e->geometry);

            if (particles->draw_order == VS::PARTICLES_DRAW_ORDER_VIEW_DEPTH
                && particles->particle_valid_histories[1]) {
                glBindBuffer(
                    GL_ARRAY_BUFFER,
                    particles->particle_buffer_histories[1]
                ); // modify the buffer, this was used 2 frames ago so it should
                   // be good enough for flushing
                RasterizerGLES3Particle* particle_array;
#ifndef __EMSCRIPTEN__
                particle_array =
                    static_cast<RasterizerGLES3Particle*>(glMapBufferRange(
                        GL_ARRAY_BUFFER,
                        0,
                        particles->amount * 24 * sizeof(float),
                        GL_MAP_READ_BIT | GL_MAP_WRITE_BIT
                    ));
#else
                PoolVector<RasterizerGLES3Particle> particle_vector;
                particle_vector.resize(particles->amount);
                PoolVector<RasterizerGLES3Particle>::Write particle_writer =
                    particle_vector.write();
                particle_array = particle_writer.ptr();
                glGetBufferSubData(
                    GL_ARRAY_BUFFER,
                    0,
                    particles->amount * sizeof(RasterizerGLES3Particle),
                    particle_array
                );
#endif

                SortArray<RasterizerGLES3Particle, RasterizerGLES3ParticleSort>
                    sorter;

                if (particles->use_local_coords) {
                    sorter.compare.z_dir =
                        e->instance->transform.affine_inverse()
                            .xform(p_view_transform.basis.get_axis(2))
                            .normalized();
                } else {
                    sorter.compare.z_dir =
                        p_view_transform.basis.get_axis(2).normalized();
                }
                ...
                particle_array = NULL;
                {
                    PoolVector<RasterizerGLES3Particle>::Read r =
                        particle_vector.read();
                    glBufferSubData(
                        GL_ARRAY_BUFFER,
                        0,
                        particles->amount * sizeof(RasterizerGLES3Particle),
                ...
                if (state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_WIREFRAME
                    && s->instancing_array_wireframe_id) {
                    glBindVertexArray(s->instancing_array_wireframe_id
                    ); // use the wireframe instancing array ID
                } else
#endif
                {
            ...
            if (particles->draw_order != VS::PARTICLES_DRAW_ORDER_LIFETIME) {
                ...
                glVertexAttribPointer(
                    ...
                    GL_FALSE,
                    stride,
                    CAST_INT_TO_UCHAR_PTR(sizeof(float) * 4 * 3)
                );
                glVertexAttribDivisor(8, 1);
                ...
                glVertexAttribPointer(
                    ...
                    GL_FALSE,
                    stride,
                    CAST_INT_TO_UCHAR_PTR(sizeof(float) * 4 * 4)
                );
                glVertexAttribDivisor(9, 1);
                ...
                glVertexAttribPointer(
                    ...
                    GL_FALSE,
                    stride,
                    CAST_INT_TO_UCHAR_PTR(sizeof(float) * 4 * 5)
                );
                glVertexAttribDivisor(10, 1);
                ...
                glVertexAttribPointer(
                    ...
                    GL_FALSE,
                    stride,
                    CAST_INT_TO_UCHAR_PTR(sizeof(float) * 4 * 2)
                );
                glVertexAttribDivisor(12, 1);
            }
    ...
    switch (e->instance->base_type) {
        ...
        case VS::INSTANCE_MULTIMESH: {
            ...
            if (state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_WIREFRAME
                ...
                storage->info.render.vertices_count +=
                    s->index_array_len * amount;
            } else
#endif
                if (s->index_array_len > 0) {
        ...
        case VS::INSTANCE_IMMEDIATE: {
            ...
            const RasterizerStorageGLES3::Immediate* im =
                static_cast<const RasterizerStorageGLES3::Immediate*>(
                    e->geometry
                );
            ...
            for (const List<RasterizerStorageGLES3::Immediate::Chunk>::Element*
                     E = im->chunks.front();
                 E;
                 E = E->next()) {
                ...
                if (c.texture.is_valid()
                    && storage->texture_owner.owns(c.texture)) {
                    RasterizerStorageGLES3::Texture* t =
                        storage->texture_owner.get(c.texture);
        ...
        case VS::INSTANCE_PARTICLES: {
            ...
            if (!particles->use_local_coords) { // not using local coordinates?
                                                // then clear transform..
                state.scene_shader.set_uniform(
                    SceneShaderGLES3::WORLD_TRANSFORM,
                    Transform()
                );
            }
            ...
            if (particles->draw_order == VS::PARTICLES_DRAW_ORDER_LIFETIME) {
                ...
                int stride = sizeof(float) * 4 * 6;
                int split =
                    int(Math::ceil(particles->phase * particles->amount));
                ...
                if (amount - split > 0) {
                    ...
                    glVertexAttribPointer(
                        ...
                        GL_FALSE,
                        stride,
                        CAST_INT_TO_UCHAR_PTR(stride * split + 0)
                    );
                    glVertexAttribDivisor(11, 1);
                    glEnableVertexAttribArray(12); // custom
                    glVertexAttribPointer(
                    ...
                    if (state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_WIREFRAME
                        ...
                        storage->info.render.vertices_count +=
                            s->index_array_len * (amount - split);
                    } else
#endif
                        if (s->index_array_len > 0) {
...
void RasterizerSceneGLES3::_setup_light(
    RenderList::Element* e,
    const Transform& p_view_transform
) {
    ...
    if (omni_count) {
        glUniform1iv(
            state.scene_shader.get_uniform(SceneShaderGLES3::OMNI_LIGHT_INDICES
            ),
            omni_count,
            omni_indices
        );
    ...
    state.scene_shader.set_uniform(
        SceneShaderGLES3::SPOT_LIGHT_COUNT,
        spot_count
    );
    if (spot_count) {
        glUniform1iv(
            state.scene_shader.get_uniform(SceneShaderGLES3::SPOT_LIGHT_INDICES
            ),
            spot_count,
            spot_indices
        );
    ...
    if (rc) {
        ...
        for (int i = 0; i < rc; i++) {
            ReflectionProbeInstance* rpi =
                reflection_probe_instance_owner.getptr(reflections[i]);
    ...
    state.scene_shader.set_uniform(
        SceneShaderGLES3::REFLECTION_COUNT,
        reflection_count
    );
    if (reflection_count) {
        glUniform1iv(
            state.scene_shader.get_uniform(SceneShaderGLES3::REFLECTION_INDICES
            ),
            reflection_count,
            reflection_indices
        );
    ...
    if (gi_probe_count) {
        ...
        glActiveTexture(
            GL_TEXTURE0 + storage->config.max_texture_image_units - 9
        );
        glBindTexture(GL_TEXTURE_3D, gipi->tex_cache);
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_XFORM1,
            gipi->transform_to_data * p_view_transform
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_BOUNDS1,
            gipi->bounds
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_MULTIPLIER1,
            gipi->probe ? gipi->probe->dynamic_range * gipi->probe->energy : 0.0
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_BIAS1,
            gipi->probe ? gipi->probe->bias * bias_scale : 0.0
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_NORMAL_BIAS1,
            gipi->probe ? gipi->probe->normal_bias * bias_scale : 0.0
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_BLEND_AMBIENT1,
            gipi->probe ? !gipi->probe->interior : false
        );
        state.scene_shader.set_uniform(
            SceneShaderGLES3::GI_PROBE_CELL_SIZE1,
            gipi->cell_size_cache
        );
        if (gi_probe_count > 1) {
            GIProbeInstance* gipi2 = gi_probe_instance_owner.getptr(ridp[1]);

            glActiveTexture(
                GL_TEXTURE0 + storage->config.max_texture_image_units - 10
            );
            glBindTexture(GL_TEXTURE_3D, gipi2->tex_cache);
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_XFORM2,
                gipi2->transform_to_data * p_view_transform
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_BOUNDS2,
                gipi2->bounds
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_CELL_SIZE2,
                gipi2->cell_size_cache
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_MULTIPLIER2,
                gipi2->probe
                    ? gipi2->probe->dynamic_range * gipi2->probe->energy
                    : 0.0
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_BIAS2,
                gipi2->probe ? gipi2->probe->bias * bias_scale : 0.0
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_NORMAL_BIAS2,
                gipi2->probe ? gipi2->probe->normal_bias * bias_scale : 0.0
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE_BLEND_AMBIENT2,
                gipi2->probe ? !gipi2->probe->interior : false
            );
            state.scene_shader.set_uniform(
                SceneShaderGLES3::GI_PROBE2_ENABLED,
                true
            );
        } else {
            state.scene_shader.set_uniform(
        ...
        glUniform4fv(
            state.scene_shader.get_uniform_location(
                SceneShaderGLES3::LIGHTMAP_CAPTURES
            ),
            12,
            (const GLfloat*)e->instance->lightmap_capture_data.ptr()
        );

    } else if (e->instance->lightmap.is_valid()) {
        RasterizerStorageGLES3::Texture* lightmap =
            storage->texture_owner.getornull(e->instance->lightmap);
        RasterizerStorageGLES3::LightmapCapture* capture =
            storage->lightmap_capture_data_owner.getornull(
                e->instance->lightmap_capture->base
            );
        ...
        if (lightmap && capture) {
            ...
            if (e->instance->lightmap_slice == -1) {
                ...
                state.scene_shader.set_uniform(
                    SceneShaderGLES3::LIGHTMAP_LAYER,
                    e->instance->lightmap_slice
                );
            }
            const Rect2& uvr = e->instance->lightmap_uv_rect;
            state.scene_shader.set_uniform(
                SceneShaderGLES3::LIGHTMAP_UV_RECT,
                Color(
                    uvr.get_position().x,
                    uvr.get_position().y,
                    uvr.get_size().x,
            ...
            if (storage->config.use_lightmap_filter_bicubic) {
                state.scene_shader.set_uniform(
                    ...
                    Vector2(lightmap->width, lightmap->height)
                );
            }
            state.scene_shader.set_uniform(
                SceneShaderGLES3::LIGHTMAP_ENERGY,
                capture->energy
            );
        }
    }
...
void RasterizerSceneGLES3::_set_cull(
    bool p_front,
    bool p_disabled,
    bool p_reverse_cull
) {
...
void RasterizerSceneGLES3::_render_list(
    RenderList::Element** p_elements,
    int p_element_count,
    const Transform& p_view_transform,
    const CameraMatrix& p_projection,
    RasterizerStorageGLES3::Sky* p_sky,
    bool p_reverse_cull,
    bool p_alpha_pass,
    bool p_shadow,
    bool p_directional_add,
    bool p_directional_shadows
) {
    ...
    if (!p_shadow && !p_directional_add) {
        glBindBufferBase(
            GL_UNIFORM_BUFFER,
            2,
            state.env_radiance_ubo
        ); // bind environment radiance info

        if (p_sky != nullptr) {
            glActiveTexture(
                GL_TEXTURE0 + storage->config.max_texture_image_units - 2
            );
            ...
            glActiveTexture(
                GL_TEXTURE0 + storage->config.max_texture_image_units - 6
            );
            glBindTexture(GL_TEXTURE_2D, p_sky->irradiance);
            state.scene_shader.set_conditional(
                SceneShaderGLES3::USE_RADIANCE_MAP,
                true
            );
            state.scene_shader.set_conditional(
                SceneShaderGLES3::USE_RADIANCE_MAP_ARRAY,
                storage->config.use_texture_array_environment
            );
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES3::SHADELESS,
        true
    ); // by default unshaded (easier to set)
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES3::ENABLE_OCTAHEDRAL_COMPRESSION,
        false
    );
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES3::USE_OPAQUE_PREPASS,
        false
    );
    ...
    for (int i = 0; i < p_element_count; i++) {
        ...
        uint32_t shading = (e->sort_key >> RenderList::SORT_KEY_SHADING_SHIFT)
                         & RenderList::SORT_KEY_SHADING_MASK;

        if (!p_shadow) {
            bool use_directional = directional_light != nullptr;
            if (p_directional_add) {
                use_directional = use_directional
                               && !(e->instance->baked_light
                                    && directional_light->light_ptr->bake_mode
                                           == VS::LightBakeMode::LIGHT_BAKE_ALL
                               );
                use_directional = use_directional
                               && ((e->instance->layer_mask
                                    & directional_light->light_ptr->cull_mask)
                                   != 0);
                use_directional = use_directional
                               && ((e->sort_key & SORT_KEY_UNSHADED_FLAG) == 0);
                if (!use_directional) {
                    continue; // It's a directional-only pass and the
                              // directional light is disabled
                }
            } else {
                use_directional =
                    use_directional
                    && (e->sort_key & SORT_KEY_NO_DIRECTIONAL_FLAG) == 0;
            }
            ...
            if (shading != prev_shading) {
                if (e->sort_key & SORT_KEY_UNSHADED_FLAG) {
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_FORWARD_LIGHTING,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_VERTEX_LIGHTING,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_LIGHT_DIRECTIONAL,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::LIGHT_DIRECTIONAL_SHADOW,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::LIGHT_USE_PSSM4,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::LIGHT_USE_PSSM2,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::LIGHT_USE_PSSM_BLEND,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::LIGHT_USE_PSSM_BLEND,
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::SHADOW_MODE_PCF_5,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::SHADOW_MODE_PCF_13,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_GI_PROBES,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_LIGHTMAP_CAPTURE,
                        false
                    );
                    state.scene_shader.set_conditional(
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_LIGHTMAP_LAYERED,
                        false
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_RADIANCE_MAP,
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_CONTACT_SHADOWS,
                        false
                    );
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_GI_PROBES,
                        e->instance->gi_probe_instances.size() > 0
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_LIGHTMAP,
                        e->instance->lightmap.is_valid()
                            && e->instance->gi_probe_instances.size() == 0
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_LIGHTMAP_LAYERED,
                        e->instance->lightmap_slice != -1
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_LIGHTMAP_CAPTURE,
                        !e->instance->lightmap_capture_data.empty()
                            && !e->instance->lightmap.is_valid()
                            && e->instance->gi_probe_instances.size() == 0
                    );
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::SHADOW_MODE_PCF_13,
                        shadow_filter_mode == SHADOW_FILTER_PCF13
                    );
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_RADIANCE_MAP,
                        use_radiance_map
                    ...
                    state.scene_shader.set_conditional(
                        SceneShaderGLES3::USE_CONTACT_SHADOWS,
                        state.used_contact_shadows
                    );

                    if (use_directional) {
                        if (p_directional_shadows
                            && directional_light->light_ptr->shadow) {
                            state.scene_shader.set_conditional(
                                SceneShaderGLES3::LIGHT_DIRECTIONAL_SHADOW,
                                true
                            );

                            switch (directional_light->light_ptr
                                ...
                                case VS::
                                    ...
                                    state.scene_shader.set_conditional(
                                        SceneShaderGLES3::LIGHT_USE_PSSM_BLEND,
                                        directional_light->light_ptr
                                            ->directional_blend_splits
                                    );
                                    break;
                                case VS::
            ...
            if (p_alpha_pass || p_directional_add) {
                ...
                if (p_directional_add) {
                    desired_blend_mode =
                        RasterizerStorageGLES3::Shader::Spatial::BLEND_MODE_ADD;
                } else {
                ...
                if (desired_blend_mode != current_blend_mode) {
                    switch (desired_blend_mode) {
                        ...
                        case RasterizerStorageGLES3::Shader::Spatial::
                            ...
                            glBlendEquation(GL_FUNC_ADD);
                            glBlendFunc(
                                p_alpha_pass ? GL_SRC_ALPHA : GL_ONE,
                                GL_ONE
                            );
        ...
        bool use_instancing = e->instance->base_type == VS::INSTANCE_MULTIMESH
                           || e->instance->base_type == VS::INSTANCE_PARTICLES;
        ...
        if (prev_skeleton != skeleton) {
            ...
            if (skeleton) {
                glActiveTexture(
                    GL_TEXTURE0 + storage->config.max_texture_image_units - 1
                );
        ...
        state.scene_shader.set_conditional(
            SceneShaderGLES3::USE_PHYSICAL_LIGHT_ATTENUATION,
            storage->config.use_physical_light_attenuation
        );

        bool octahedral_compression =
            e->instance->base_type != VS::INSTANCE_IMMEDIATE
            && ((RasterizerStorageGLES3::Surface*)e->geometry)->format
                   & VisualServer::ArrayFormat::
                       ARRAY_FLAG_USE_OCTAHEDRAL_COMPRESSION
            && !(
                ((RasterizerStorageGLES3::Surface*)e->geometry)
                    ->blend_shapes.size()
                && e->instance->blend_values.size()
            );
        if (octahedral_compression != prev_octahedral_compression) {
            state.scene_shader.set_conditional(
                SceneShaderGLES3::ENABLE_OCTAHEDRAL_COMPRESSION,
                octahedral_compression
            );
            rebind = true;
        }
        ...
        if (material != prev_material || rebind) {
            ...
            rebind =
                _setup_material(material, use_opaque_prepass, p_alpha_pass);
        ...
        if (!(e->sort_key & SORT_KEY_UNSHADED_FLAG) && !p_directional_add
            && !p_shadow) {
            _setup_light(e, p_view_transform);
        }

        if (e->owner != prev_owner || prev_base_type != e->instance->base_type
...
void RasterizerSceneGLES3::_add_geometry(
    RasterizerStorageGLES3::Geometry* p_geometry,
    InstanceBase* p_instance,
    RasterizerStorageGLES3::GeometryOwner* p_owner,
    int p_material,
    bool p_depth_pass,
    bool p_shadow_pass
) {
    RasterizerStorageGLES3::Material* m = nullptr;
    RID m_src = p_instance->material_override.is_valid()
                  ? p_instance->material_override
                  : (p_material >= 0 ? p_instance->materials[p_material]
                                     : p_geometry->material);
...
void RasterizerSceneGLES3::_add_geometry_with_material(
    RasterizerStorageGLES3::Geometry* p_geometry,
    InstanceBase* p_instance,
    RasterizerStorageGLES3::GeometryOwner* p_owner,
    RasterizerStorageGLES3::Material* p_material,
    ...
    bool has_base_alpha = (p_material->shader->spatial.uses_alpha
                           && !p_material->shader->spatial.uses_alpha_scissor)
                       || p_material->shader->spatial.uses_screen_texture
                       || p_material->shader->spatial.uses_depth_texture;
    bool has_blend_alpha =
        p_material->shader->spatial.blend_mode
        != RasterizerStorageGLES3::Shader::Spatial::BLEND_MODE_MIX;
    ...
    if (p_depth_pass) {
        if (has_blend_alpha || p_material->shader->spatial.uses_depth_texture
            || ((has_base_alpha
                 || p_instance->cast_shadows == VS::SHADOW_CASTING_SETTING_OFF)
                && p_material->shader->spatial.depth_draw_mode
                       != RasterizerStorageGLES3::Shader::Spatial::
                           DEPTH_DRAW_ALPHA_PREPASS)
            || p_material->shader->spatial.depth_draw_mode
                   == RasterizerStorageGLES3::Shader::Spatial::DEPTH_DRAW_NEVER
            || p_material->shader->spatial.no_depth_test) {
            return; // bye
        }

        if (!p_material->shader->spatial.uses_alpha_scissor
            && !p_material->shader->spatial.writes_modelview_or_projection
            && !p_material->shader->spatial.uses_vertex
            && !p_material->shader->spatial.uses_discard
            && p_material->shader->spatial.depth_draw_mode
                   != RasterizerStorageGLES3::Shader::Spatial::
                       DEPTH_DRAW_ALPHA_PREPASS) {
            // shader does not use discard and does not write a vertex position,
            // use generic material
            if (p_instance->cast_shadows
                == VS::SHADOW_CASTING_SETTING_DOUBLE_SIDED) {
                p_material = storage->material_owner.getptr(
                    !p_shadow_pass
                            && p_material->shader->spatial
                                   .uses_world_coordinates
                        ? default_worldcoord_material_twosided
                        : default_material_twosided
                );
                no_cull = true;
                mirror  = false;
            } else {
    ...
    RenderList::Element* e =
        (has_alpha || p_material->shader->spatial.no_depth_test)
            ? render_list.add_alpha_element()
            : render_list.add_element();
    ...
    // We sort only by the first directional light. The rest of directional
    // lights will be drawn in additive passes that are skipped if disabled.
    if (first_directional_light.is_valid()
        && light_instance_owner.owns(first_directional_light)) {
        RasterizerStorageGLES3::Light* directional =
            light_instance_owner.getptr(first_directional_light)->light_ptr;
        if ((e->instance->layer_mask & directional->cull_mask) == 0
            || (e->instance->baked_light
                && directional->bake_mode == VS::LightBakeMode::LIGHT_BAKE_ALL
            )) {
            e->sort_key |= SORT_KEY_NO_DIRECTIONAL_FLAG;
        }
    }

    e->sort_key |= uint64_t(e->geometry->index)
                << RenderList::SORT_KEY_GEOMETRY_INDEX_SHIFT;
    e->sort_key |= uint64_t(e->instance->base_type)
                << RenderList::SORT_KEY_GEOMETRY_TYPE_SHIFT;
    ...
    e->sort_key |= uint64_t(e->material->index)
                << RenderList::SORT_KEY_MATERIAL_INDEX_SHIFT;
    e->sort_key |= uint64_t(e->instance->depth_layer)
                << RenderList::SORT_KEY_OPAQUE_DEPTH_LAYER_SHIFT;
    ...
    if (!p_depth_pass) {
        ...
        e->sort_key |= (uint64_t(p_material->render_priority) + 128)
                    << RenderList::SORT_KEY_PRIORITY_SHIFT;
    }

    /*
    if
    (e->geometry->type==RasterizerStorageGLES3::Geometry::GEOMETRY_MULTISURFACE)
    ...
    */
    ...
    if (p_depth_pass || p_material->shader->spatial.unshaded
        || state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_UNSHADED) {
    ...
    if (!p_depth_pass
        && (p_material->shader->spatial.uses_vertex_lighting
            || storage->config.force_vertex_shading)) {
        e->sort_key |= SORT_KEY_VERTEX_LIT_FLAG;
    }
...
void RasterizerSceneGLES3::_draw_sky(
    RasterizerStorageGLES3::Sky* p_sky,
    const CameraMatrix& p_projection,
    const Transform& p_transform,
    bool p_vflip,
    ...
    RasterizerStorageGLES3::Texture* tex =
        storage->texture_owner.getornull(p_sky->panorama);
    ...
    if (storage->config.srgb_decode_supported && tex->srgb
        && !tex->using_srgb) {
        ...
        if (!(tex->flags & VS::TEXTURE_FLAG_CONVERT_TO_LINEAR)) {
            ...
            // notify that texture must be set to linear beforehand, so it works
            // in other platforms when exported
        }
#endif
    }
    ...
    /*
        If matrix[2][0] or matrix[2][1] we're dealing with an asymmetrical
       projection matrix. This is the case for stereoscopic rendering (i.e. VR).
        To ensure the image rendered is perspective correct we need to move some
       logic into the shader. For this the USE_ASYM_PANO option is introduced.
        It also means the uv coordinates are ignored in this mode and we don't
       need our loop.
    */
    bool asymmetrical =
        ((camera.matrix[2][0] != 0.0) || (camera.matrix[2][1] != 0.0));
    ...
    storage->shaders.copy.set_conditional(
        CopyShaderGLES3::USE_ASYM_PANO,
        asymmetrical
    );
    storage->shaders.copy.set_conditional(
        CopyShaderGLES3::USE_PANORAMA,
        !asymmetrical
    );
    storage->shaders.copy.set_conditional(
        CopyShaderGLES3::USE_MULTIPLIER,
        true
    );
    ...
    // don't know why but I always have problems setting a uniform mat3, so
    // we're using a transform
    storage->shaders.copy.set_uniform(
        CopyShaderGLES3::SKY_TRANSFORM,
        Transform(p_sky_orientation, Vector3(0.0, 0.0, 0.0)).affine_inverse()
    );

    if (asymmetrical) {
        // pack the bits we need from our projection matrix
        storage->shaders.copy.set_uniform(
            CopyShaderGLES3::ASYM_PROJ,
            camera.matrix[2][0],
            camera.matrix[0][0],
            camera.matrix[2][1],
            camera.matrix[1][1]
        );
        ///@TODO I couldn't get mat3 + p_transform.basis to work, that would be
        /// better here.
        storage->shaders.copy.set_uniform(
            CopyShaderGLES3::PANO_TRANSFORM,
            p_transform
        );
    }
...
void RasterizerSceneGLES3::_setup_environment(
    Environment* env,
    ...
    store_camera(
        ...
        state.ubo_data.inv_projection_matrix
    );
    store_transform(p_cam_transform, state.ubo_data.camera_matrix);
    store_transform(
        p_cam_transform.affine_inverse(),
        state.ubo_data.camera_inverse_matrix
    );
    ...
    if (env) {
        ...
        // use the inverse of our sky_orientation, we may need to skip this if
        // we're using a reflection probe?
        sky_orientation =
            Transform(env->sky_orientation, Vector3(0.0, 0.0, 0.0))
                .affine_inverse();

        state.env_radiance_data.ambient_contribution =
            env->ambient_sky_contribution;
        state.ubo_data.ambient_occlusion_affect_light = env->ssao_light_affect;
        state.ubo_data.ambient_occlusion_affect_ssao =
            env->ssao_ao_channel_affect;
        ...
        state.ubo_data.fog_color_enabled[3] =
            (!p_no_fog && env->fog_enabled) ? 1.0 : 0.0;
        ...
        Color linear_ambient_color =
            storage->frame.clear_request_color.to_linear();
        state.ubo_data.ambient_light_color[0] = linear_ambient_color.r;
        state.ubo_data.ambient_light_color[1] = linear_ambient_color.g;
        state.ubo_data.ambient_light_color[2] = linear_ambient_color.b;
        state.ubo_data.ambient_light_color[3] = linear_ambient_color.a;
...

    {
        ...
        state.ubo_data.shadow_directional_pixel_size[0] =
            1.0 / directional_shadow.size;
        state.ubo_data.shadow_directional_pixel_size[1] =
            1.0 / directional_shadow.size;

        glActiveTexture(
            GL_TEXTURE0 + storage->config.max_texture_image_units - 4
        );
    ...
    store_transform(
        sky_orientation * p_cam_transform,
        state.env_radiance_data.transform
    );

    glBindBuffer(GL_UNIFORM_BUFFER, state.env_radiance_ubo);
    glBufferData(
        GL_UNIFORM_BUFFER,
        sizeof(State::EnvironmentRadianceUBO),
        &state.env_radiance_data,
        GL_DYNAMIC_DRAW
    );
    glBindBuffer(GL_UNIFORM_BUFFER, 0);
}

void RasterizerSceneGLES3::_setup_directional_light(
    int p_index,
    const Transform& p_camera_inverse_transform,
    bool p_use_shadows
) {
    ...
    ubo_data.light_color_energy[0] =
        linear_col.r * sign * li->light_ptr->param[VS::LIGHT_PARAM_ENERGY]
        * Math_PI;
    ubo_data.light_color_energy[1] =
        linear_col.g * sign * li->light_ptr->param[VS::LIGHT_PARAM_ENERGY]
        * Math_PI;
    ubo_data.light_color_energy[2] =
        linear_col.b * sign * li->light_ptr->param[VS::LIGHT_PARAM_ENERGY]
        * Math_PI;
    ...
    Vector3 direction = p_camera_inverse_transform.basis
                            .xform(li->transform.basis.xform(Vector3(0, 0, -1)))
                            .normalized();
    ...
    ubo_data.light_shadow_color_contact[3] =
        li->light_ptr->param[VS::LIGHT_PARAM_CONTACT_SHADOW_SIZE];
    ...
    if (p_use_shadows && li->light_ptr->shadow) {
        ...
        for (int j = 0; j < shadow_count; j++) {
            ...
            if (li->light_ptr->directional_shadow_mode
                == VS::LIGHT_DIRECTIONAL_SHADOW_PARALLEL_4_SPLITS) {
                width  /= 2;
                height /= 2;
            ...
            Transform modelview =
                (p_camera_inverse_transform * li->shadow_transform[j].transform)
                    .affine_inverse();

            CameraMatrix bias;
            bias.set_light_bias();
            CameraMatrix rectm;
            Rect2 atlas_rect = Rect2(
                float(x) / directional_shadow.size,
                float(y) / directional_shadow.size,
                float(width) / directional_shadow.size,
                float(height) / directional_shadow.size
            );
            rectm.set_light_atlas_rect(atlas_rect);

            CameraMatrix shadow_mtx =
                rectm * bias * li->shadow_transform[j].camera * modelview;
...
void RasterizerSceneGLES3::_setup_lights(
    RID* p_light_cull_result,
    int p_light_cull_count,
    const Transform& p_camera_inverse_transform,
    const CameraMatrix& p_camera_projection,
    RID p_shadow_atlas
) {
    ...
    for (int i = 0; i < p_light_cull_count; i++) {
        ...
        switch (li->light_ptr->type) {
            case VS::LIGHT_DIRECTIONAL: {
                ...
                if (state.directional_light_count
                    < RenderList::MAX_DIRECTIONAL_LIGHTS) {
            ...
            case VS::LIGHT_OMNI: {
                ...
                ubo_data.light_color_energy[0] =
                    linear_col.r * sign
                    * li->light_ptr->param[VS::LIGHT_PARAM_ENERGY] * Math_PI;
                ubo_data.light_color_energy[1] =
                    linear_col.g * sign
                    * li->light_ptr->param[VS::LIGHT_PARAM_ENERGY] * Math_PI;
                ubo_data.light_color_energy[2] =
                ...
                Vector3 pos =
                    p_camera_inverse_transform.xform(li->transform.origin);
                ...
                ubo_data.light_pos_inv_radius[3] =
                    1.0
                    / MAX(0.001, li->light_ptr->param[VS::LIGHT_PARAM_RANGE]);

                ubo_data.light_direction_attenuation[0] = 0;
                ubo_data.light_direction_attenuation[1] = 0;
                ubo_data.light_direction_attenuation[2] = 0;
                ubo_data.light_direction_attenuation[3] =
                    li->light_ptr->param[VS::LIGHT_PARAM_ATTENUATION];

                ubo_data.light_params[0] = 0;
                ubo_data.light_params[1] = 0;
                ubo_data.light_params[2] =
                    li->light_ptr->param[VS::LIGHT_PARAM_SPECULAR];
                ubo_data.light_params[3] = 0;
                ...
                if (li->light_ptr->shadow && shadow_atlas
                    ...
                    uint32_t quadrant =
                        (key >> ShadowAtlas::QUADRANT_SHIFT) & 0x3;
                    uint32_t shadow = key & ShadowAtlas::SHADOW_INDEX_MASK;

                    ERR_CONTINUE(
                        shadow >= (uint32_t)shadow_atlas->quadrants[quadrant]
                                      .shadows.size()
                    );
                    ...
                    uint32_t shadow_size =
                        (quadrant_size
                         / shadow_atlas->quadrants[quadrant].subdivision);
                    x +=
                        (shadow % shadow_atlas->quadrants[quadrant].subdivision)
                        * shadow_size;
                    y +=
                        (shadow / shadow_atlas->quadrants[quadrant].subdivision)
                        * shadow_size;
                    ...
                    if (li->light_ptr->omni_shadow_detail
                        == VS::LIGHT_OMNI_SHADOW_DETAIL_HORIZONTAL) {
                        height /= 2;
                    } else {
                        width /= 2;
                    }
                    ...
                    Transform proj =
                        (p_camera_inverse_transform * li->transform).inverse();
                ...
                memcpy(
                    &state.omni_array_tmp
                         [li->light_index * state.ubo_light_size],
                    &ubo_data,
                    state.ubo_light_size
                );
            ...
            case VS::LIGHT_SPOT: {
                ...
                Vector3 direction =
                    p_camera_inverse_transform.basis
                        .xform(li->transform.basis.xform(Vector3(0, 0, -1)))
                        .normalized();
                ubo_data.light_direction_attenuation[0] = direction.x;
                ubo_data.light_direction_attenuation[1] = direction.y;
                ...
                ubo_data.light_params[0] =
                    li->light_ptr->param[VS::LIGHT_PARAM_SPOT_ATTENUATION];
                ubo_data.light_params[1] = Math::cos(Math::deg2rad(
                    li->light_ptr->param[VS::LIGHT_PARAM_SPOT_ANGLE]
                ));
                ubo_data.light_params[2] =
                    li->light_ptr->param[VS::LIGHT_PARAM_SPECULAR];
                ubo_data.light_params[3] = 0;
                ...
                if (li->light_ptr->shadow && shadow_atlas
                    ...
                    Rect2 rect(
                        float(x) / atlas_size,
                        float(y) / atlas_size,
                        float(width) / atlas_size,
                        float(height) / atlas_size
                    );
                    ...
                    CameraMatrix shadow_mtx = rectm * bias
                                            * li->shadow_transform[0].camera
                                            * modelview;
    ...
    if (state.omni_light_count) {
        ...
        glBufferSubData(
            GL_UNIFORM_BUFFER,
            0,
            state.omni_light_count * state.ubo_light_size,
            state.omni_array_tmp
        );
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
    }
    ...
    if (state.spot_light_count) {
        ...
        glBufferSubData(
            GL_UNIFORM_BUFFER,
            0,
            state.spot_light_count * state.ubo_light_size,
            state.spot_array_tmp
        );
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
    }
...
void RasterizerSceneGLES3::_setup_reflections(
    ...
    for (int i = 0; i < p_reflection_probe_cull_count; i++) {
        ReflectionProbeInstance* rpi =
            reflection_probe_instance_owner.getornull(
                p_reflection_probe_cull_result[i]
            );
        ERR_CONTINUE(!rpi);
        ...
        if (rpi->probe_ptr->interior) {
            ...
            reflection_ubo.ambient[0] =
                ambient_linear.r * rpi->probe_ptr->interior_ambient_energy;
            reflection_ubo.ambient[1] =
                ambient_linear.g * rpi->probe_ptr->interior_ambient_energy;
            reflection_ubo.ambient[2] =
                ambient_linear.b * rpi->probe_ptr->interior_ambient_energy;
            reflection_ubo.ambient[3] =
                rpi->probe_ptr->interior_ambient_probe_contrib;
        } else {
            ...
            reflection_ubo.ambient[3] =
                0; // not used in exterior mode, since it just blends with
                   // regular ambient light
        }

        int cell_size = reflection_atlas->size / reflection_atlas->subdiv;
        int x         = (rpi->reflection_atlas_index % reflection_atlas->subdiv)
              * cell_size;
        int y = (rpi->reflection_atlas_index / reflection_atlas->subdiv)
              * cell_size;
        int width  = cell_size;
        int height = cell_size;
        ...
        Transform proj =
            (p_camera_inverse_transform * rpi->transform).inverse();
    ...
    if (state.reflection_probe_count) {
        ...
        glBufferSubData(
            GL_UNIFORM_BUFFER,
            0,
            state.reflection_probe_count * sizeof(ReflectionProbeDataUBO),
            state.reflection_array_tmp
        );
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
    }
...
void RasterizerSceneGLES3::_copy_texture_to_front_buffer(GLuint p_texture) {
    ...
    glViewport(
        0,
        0,
        storage->frame.current_rt->width * 0.5,
        storage->frame.current_rt->height * 0.5
    );
    ...
    storage->shaders.copy.set_conditional(
        CopyShaderGLES3::LINEAR_TO_SRGB,
        false
    );
    storage->shaders.copy.set_conditional(
        CopyShaderGLES3::DISABLE_ALPHA,
        false
    );
}

void RasterizerSceneGLES3::_fill_render_list(
        ...
        switch (inst->base_type) {
            case VS::INSTANCE_MESH: {
                RasterizerStorageGLES3::Mesh* mesh =
                    storage->mesh_owner.getptr(inst->base);
                ERR_CONTINUE(!mesh);
            ...
            case VS::INSTANCE_MULTIMESH: {
                RasterizerStorageGLES3::MultiMesh* multi_mesh =
                    storage->multimesh_owner.getptr(inst->base);
                ERR_CONTINUE(!multi_mesh);

                if (multi_mesh->size == 0
                    || multi_mesh->visible_instances == 0) {
                    continue;
                }

                RasterizerStorageGLES3::Mesh* mesh =
                    storage->mesh_owner.getptr(multi_mesh->mesh);
                if (!mesh) {
            ...
            case VS::INSTANCE_IMMEDIATE: {
                RasterizerStorageGLES3::Immediate* immediate =
                    storage->immediate_owner.getptr(inst->base);
            ...
            case VS::INSTANCE_PARTICLES: {
                RasterizerStorageGLES3::Particles* particles =
                    storage->particles_owner.getptr(inst->base);
...
void RasterizerSceneGLES3::_blur_effect_buffer() {
    ...
    for (int i = 0;
         i < storage->frame.current_rt->effects.mip_maps[1].sizes.size();
         i++) {
        int vp_w =
            storage->frame.current_rt->effects.mip_maps[1].sizes[i].width;
        int vp_h =
            storage->frame.current_rt->effects.mip_maps[1].sizes[i].height;
        glViewport(0, 0, vp_w, vp_h);
        // horizontal pass
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::GAUSSIAN_HORIZONTAL,
            true
        );
        state.effect_blur_shader.bind();
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::PIXEL_SIZE,
            Vector2(1.0 / vp_w, 1.0 / vp_h)
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::LOD,
            float(i)
        );
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->effects.mip_maps[0].color
        ); // previous level, since mipmaps[0] starts one level bigger
        glBindFramebuffer(
            GL_FRAMEBUFFER,
            storage->frame.current_rt->effects.mip_maps[1].sizes[i].fbo
        );
        _copy_screen(true);
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::GAUSSIAN_HORIZONTAL,
        ...
        // vertical pass
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::GAUSSIAN_VERTICAL,
            true
        );
        state.effect_blur_shader.bind();
        state.effect_blur_shader.set_uniform(
        ...
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->effects.mip_maps[1].color
        );
        glBindFramebuffer(
            GL_FRAMEBUFFER,
            storage->frame.current_rt->effects.mip_maps[0].sizes[i + 1].fbo
        ); // next level, since mipmaps[0] starts one level bigger
        _copy_screen(true);
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::GAUSSIAN_VERTICAL,
            false
...
void RasterizerSceneGLES3::_prepare_depth_texture() {
    if (!state.prepared_depth_texture) {
        ...
        glBindFramebuffer(
            GL_READ_FRAMEBUFFER,
            storage->frame.current_rt->buffers.fbo
        );
...
void RasterizerSceneGLES3::_bind_depth_texture() {
    if (!state.bound_depth_texture) {
        ...
        // bind depth for read
        glActiveTexture(
            GL_TEXTURE0 + storage->config.max_texture_image_units - 8
        );
...
void RasterizerSceneGLES3::_render_mrts(
    Environment* env,
    const CameraMatrix& p_cam_projection
) {
    glDepthMask(GL_FALSE);
    ...
    if (env->ssao_enabled || env->ssr_enabled) {
        ...
        glBindFramebuffer(
            GL_DRAW_FRAMEBUFFER,
            storage->frame.current_rt->buffers.effect_fbo
        );
        glBlitFramebuffer(
            0,
            0,
    ...
    if (env->ssao_enabled) {
        ...
        for (int i = 0;
             i
             < storage->frame.current_rt->effects.ssao.depth_mipmap_fbos.size();
             i++) {
            state.ssao_minify_shader.set_conditional(
                SsaoMinifyShaderGLES3::MINIFY_START,
                i == 0
            );
            state.ssao_minify_shader.set_conditional(
                SsaoMinifyShaderGLES3::USE_ORTHOGONAL_PROJECTION,
                p_cam_projection.is_orthogonal()
            );
            state.ssao_minify_shader.bind();
            state.ssao_minify_shader.set_uniform(
                SsaoMinifyShaderGLES3::CAMERA_Z_FAR,
                p_cam_projection.get_z_far()
            );
            state.ssao_minify_shader.set_uniform(
                SsaoMinifyShaderGLES3::CAMERA_Z_NEAR,
                p_cam_projection.get_z_near()
            );
            state.ssao_minify_shader.set_uniform(
                SsaoMinifyShaderGLES3::SOURCE_MIPMAP,
                MAX(0, i - 1)
            );
            glUniform2iv(
                state.ssao_minify_shader.get_uniform(
                    SsaoMinifyShaderGLES3::FROM_SIZE
                ),
                1,
                ss
            );
            ...
            if (i == 0) {
                ...
                glBindTexture(
                    GL_TEXTURE_2D,
                    storage->frame.current_rt->effects.ssao.linear_depth
                );
            }

            glBindFramebuffer(
                GL_FRAMEBUFFER,
                storage->frame.current_rt->effects.ssao.depth_mipmap_fbos[i]
            ); // copy to front first
        ...
        // do SSAO!
        state.ssao_shader.set_conditional(
            SsaoShaderGLES3::ENABLE_RADIUS2,
            env->ssao_radius2 > 0.001
        );
        state.ssao_shader.set_conditional(
            SsaoShaderGLES3::USE_ORTHOGONAL_PROJECTION,
            p_cam_projection.is_orthogonal()
        );
        state.ssao_shader.set_conditional(
            SsaoShaderGLES3::SSAO_QUALITY_LOW,
            env->ssao_quality == VS::ENV_SSAO_QUALITY_LOW
        );
        state.ssao_shader.set_conditional(
            SsaoShaderGLES3::SSAO_QUALITY_HIGH,
            env->ssao_quality == VS::ENV_SSAO_QUALITY_HIGH
        );
        state.ssao_shader.bind();
        state.ssao_shader.set_uniform(
            SsaoShaderGLES3::CAMERA_Z_FAR,
            p_cam_projection.get_z_far()
        );
        state.ssao_shader.set_uniform(
            SsaoShaderGLES3::CAMERA_Z_NEAR,
            p_cam_projection.get_z_near()
        );
        glUniform2iv(
            state.ssao_shader.get_uniform(SsaoShaderGLES3::SCREEN_SIZE),
            1,
            ss
        );
        ...
        state.ssao_shader.set_uniform(
            SsaoShaderGLES3::INTENSITY_DIV_R6,
            intensity / pow(radius, 6.0f)
        );
        ...
        if (env->ssao_radius2 > 0.001) {
            ...
            state.ssao_shader.set_uniform(
                SsaoShaderGLES3::INTENSITY_DIV_R62,
                intensity2 / pow(radius2, 6.0f)
            );
        }
        ...
        float proj_info[4] = {
            ...
            (1.0f - p_cam_projection.matrix[0][2])
                / p_cam_projection.matrix[0][0],
            (1.0f + p_cam_projection.matrix[1][2])
                / p_cam_projection.matrix[1][1]
        };

        glUniform4fv(
            state.ssao_shader.get_uniform(SsaoShaderGLES3::PROJ_INFO),
            1,
            proj_info
        );
        float pixels_per_meter =
            float(p_cam_projection.get_pixels_per_meter(ss[0]));

        state.ssao_shader.set_uniform(
            SsaoShaderGLES3::PROJ_SCALE,
            pixels_per_meter
        );
        ...
        glBindFramebuffer(
            GL_FRAMEBUFFER,
            storage->frame.current_rt->effects.ssao.blur_fbo[0]
        ); // copy to front first
        Color white(1, 1, 1, 1);
        ...
        if (env->ssao_filter) {
            for (int i = 0; i < 2; i++) {
                state.ssao_blur_shader.set_uniform(
                    SsaoBlurShaderGLES3::CAMERA_Z_FAR,
                    p_cam_projection.get_z_far()
                );
                state.ssao_blur_shader.set_uniform(
                    SsaoBlurShaderGLES3::CAMERA_Z_NEAR,
                    p_cam_projection.get_z_near()
                );
                state.ssao_blur_shader.set_uniform(
                    SsaoBlurShaderGLES3::EDGE_SHARPNESS,
                    env->ssao_bilateral_sharpness
                );
                state.ssao_blur_shader.set_uniform(
                    SsaoBlurShaderGLES3::FILTER_SCALE,
                    int(env->ssao_filter)
                );

                GLint axis[2] = {i, 1 - i};
                glUniform2iv(
                    state.ssao_blur_shader.get_uniform(SsaoBlurShaderGLES3::AXIS
                    ),
                    1,
                    axis
                );
                glUniform2iv(
                    state.ssao_blur_shader.get_uniform(
                        SsaoBlurShaderGLES3::SCREEN_SIZE
                    ),
                    1,
                    ss
                );
                ...
                glBindTexture(
                    GL_TEXTURE_2D,
                    storage->frame.current_rt->effects.ssao.blur_red[i]
                );
                glActiveTexture(GL_TEXTURE1);
                glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->depth);
                glActiveTexture(GL_TEXTURE2);
                glBindTexture(
                    GL_TEXTURE_2D,
                    storage->frame.current_rt->buffers.effect
                );
                glBindFramebuffer(
                    GL_FRAMEBUFFER,
                    storage->frame.current_rt->effects.ssao.blur_fbo[1 - i]
                );
                if (i == 0) {
                    glClearBufferfv(GL_COLOR, 0, white.components); // specular
                }
        ...
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::SSAO_MERGE,
            true
        );
        state.effect_blur_shader.bind();
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::SSAO_COLOR,
            env->ssao_color
        );
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->color
        ); // previous level, since mipmaps[0] starts one level bigger
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->effects.ssao.blur_red[0]
        ); // previous level, since mipmaps[0] starts one level bigger
        glBindFramebuffer(
            GL_FRAMEBUFFER,
            storage->frame.current_rt->effects.mip_maps[0].sizes[0].fbo
        ); // copy to base level
        _copy_screen(true);
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::SSAO_MERGE,
    ...
    if (state.used_sss) { // sss enabled
        ...
        state.sss_shader.set_conditional(
            SubsurfScatteringShaderGLES3::USE_ORTHOGONAL_PROJECTION,
            p_cam_projection.is_orthogonal()
        );
        state.sss_shader.set_conditional(
            SubsurfScatteringShaderGLES3::USE_11_SAMPLES,
            subsurface_scatter_quality == SSS_QUALITY_LOW
        );
        state.sss_shader.set_conditional(
            SubsurfScatteringShaderGLES3::USE_17_SAMPLES,
            subsurface_scatter_quality == SSS_QUALITY_MEDIUM
        );
        state.sss_shader.set_conditional(
            SubsurfScatteringShaderGLES3::USE_25_SAMPLES,
            subsurface_scatter_quality == SSS_QUALITY_HIGH
        );
        state.sss_shader.set_conditional(
            SubsurfScatteringShaderGLES3::ENABLE_FOLLOW_SURFACE,
            subsurface_scatter_follow_surface
        );
        state.sss_shader.set_conditional(
            SubsurfScatteringShaderGLES3::ENABLE_STRENGTH_WEIGHTING,
            subsurface_scatter_weight_samples
        );
        state.sss_shader.bind();
        state.sss_shader.set_uniform(
            SubsurfScatteringShaderGLES3::MAX_RADIUS,
            subsurface_scatter_size
        );
        state.sss_shader.set_uniform(
            SubsurfScatteringShaderGLES3::UNIT_SIZE,
            unit_size
        );
        state.sss_shader.set_uniform(
            SubsurfScatteringShaderGLES3::CAMERA_Z_NEAR,
            p_cam_projection.get_z_near()
        );
        state.sss_shader.set_uniform(
            SubsurfScatteringShaderGLES3::CAMERA_Z_FAR,
            p_cam_projection.get_z_far()
        );
        state.sss_shader.set_uniform(
            SubsurfScatteringShaderGLES3::DIR,
            Vector2(1, 0)
        );
        ...
        glTexParameteri(
            ...
            GL_NEAREST
        ); // disable filter (fixes bugs on AMD)
    ...
    if (env->ssr_enabled) {
        ...
        state.ssr_shader.set_conditional(
            ScreenSpaceReflectionShaderGLES3::REFLECT_ROUGHNESS,
            env->ssr_roughness
        );
        state.ssr_shader.set_conditional(
            ScreenSpaceReflectionShaderGLES3::USE_ORTHOGONAL_PROJECTION,
            p_cam_projection.is_orthogonal()
        );
        ...
        int ssr_w =
            storage->frame.current_rt->effects.mip_maps[1].sizes[0].width;
        int ssr_h =
            storage->frame.current_rt->effects.mip_maps[1].sizes[0].height;

        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::PIXEL_SIZE,
            Vector2(1.0 / (ssr_w * 0.5), 1.0 / (ssr_h * 0.5))
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::CAMERA_Z_NEAR,
            p_cam_projection.get_z_near()
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::CAMERA_Z_FAR,
            p_cam_projection.get_z_far()
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::PROJECTION,
            p_cam_projection
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::INVERSE_PROJECTION,
            p_cam_projection.inverse()
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::VIEWPORT_SIZE,
            Size2(ssr_w, ssr_h)
        );
        // state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES3::FRAME_INDEX,int(render_pass));
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::FILTER_MIPMAP_LEVELS,
            float(storage->frame.current_rt->effects.mip_maps[0].sizes.size())
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::NUM_STEPS,
            env->ssr_max_steps
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::DEPTH_TOLERANCE,
            env->ssr_depth_tolerance
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::DISTANCE_FADE,
            env->ssr_fade_out
        );
        state.ssr_shader.set_uniform(
            ScreenSpaceReflectionShaderGLES3::CURVE_FADE_IN,
            env->ssr_fade_in
        );
        ...
        glBindFramebuffer(
            GL_FRAMEBUFFER,
            storage->frame.current_rt->effects.mip_maps[1].sizes[0].fbo
        );
    ...
    state.resolve_shader.set_conditional(
        ResolveShaderGLES3::USE_SSR,
        env->ssr_enabled
    );
    state.resolve_shader.bind();
    state.resolve_shader.set_uniform(
        ResolveShaderGLES3::PIXEL_SIZE,
        Vector2(
            1.0 / storage->frame.current_rt->width,
            1.0 / storage->frame.current_rt->height
        )
    );
    ...
    glEnable(GL_BLEND);
    glBlendEquation(GL_FUNC_ADD);
    glBlendFunc(GL_ONE, GL_ONE); // use additive to accumulate one over the
                                 // other
    ...
    state.effect_blur_shader.set_conditional(
        EffectBlurShaderGLES3::SIMPLE_COPY,
        true
    );
    state.effect_blur_shader.bind();
...
void RasterizerSceneGLES3::_post_process(
    Environment* env,
    const CameraMatrix& p_cam_projection
) {
    // copy to front buffer
    ...
    if ((!env
         ...
         || storage->frame.current_rt->width < 4
         || storage->frame.current_rt->height < 4)
        && !storage->frame.current_rt->use_fxaa
        && !storage->frame.current_rt->use_debanding
        && storage->frame.current_rt->sharpen_intensity
               < 0.001) { // no post process on small render targets
        // no environment or transparent render, simply return and convert to
        // SRGB
        if (storage->frame.current_rt->external.fbo != 0) {
            glBindFramebuffer(
                GL_FRAMEBUFFER,
                storage->frame.current_rt->external.fbo
            );
        } else {
            glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->fbo);
        }
        ...
        storage->shaders.copy.set_conditional(
            CopyShaderGLES3::LINEAR_TO_SRGB,
            !storage->frame.current_rt
                 ->flags[RasterizerStorage::RENDER_TARGET_KEEP_3D_LINEAR]
        );
        storage->shaders.copy.set_conditional(
            CopyShaderGLES3::V_FLIP,
            storage->frame.current_rt
                ->flags[RasterizerStorage::RENDER_TARGET_VFLIP]
        );
        storage->shaders.copy.set_conditional(
            CopyShaderGLES3::DISABLE_ALPHA,
            !storage->frame.current_rt
    ...
    if (env && env->dof_blur_far_enabled) {
        ...
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::USE_ORTHOGONAL_PROJECTION,
            p_cam_projection.is_orthogonal()
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_FAR_BLUR,
            true
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_QUALITY_LOW,
            env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_LOW
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_QUALITY_MEDIUM,
            env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_MEDIUM
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_QUALITY_HIGH,
            env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_HIGH
        );

        state.effect_blur_shader.bind();
        int qsteps[3] = {4, 10, 20};

        float radius = (env->dof_blur_far_amount * env->dof_blur_far_amount)
                     / qsteps[env->dof_blur_far_quality];

        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_BEGIN,
            env->dof_blur_far_distance
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_END,
            env->dof_blur_far_distance + env->dof_blur_far_transition
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_DIR,
            Vector2(1, 0)
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_RADIUS,
            radius
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::PIXEL_SIZE,
        ...
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::CAMERA_Z_NEAR,
            p_cam_projection.get_z_near()
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::CAMERA_Z_FAR,
            p_cam_projection.get_z_far()
        );
    ...
    if (env && env->dof_blur_near_enabled) {
        ...
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_NEAR_BLUR,
            true
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_NEAR_FIRST_TAP,
            true
        );

        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_QUALITY_LOW,
            env->dof_blur_near_quality == VS::ENV_DOF_BLUR_QUALITY_LOW
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_QUALITY_MEDIUM,
            env->dof_blur_near_quality == VS::ENV_DOF_BLUR_QUALITY_MEDIUM
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_QUALITY_HIGH,
            env->dof_blur_near_quality == VS::ENV_DOF_BLUR_QUALITY_HIGH
        );

        state.effect_blur_shader.bind();
        int qsteps[3] = {4, 10, 20};

        float radius = (env->dof_blur_near_amount * env->dof_blur_near_amount)
                     / qsteps[env->dof_blur_near_quality];

        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_BEGIN,
            env->dof_blur_near_distance
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_END,
            env->dof_blur_near_distance - env->dof_blur_near_transition
        );
        state.effect_blur_shader.set_uniform(
            EffectBlurShaderGLES3::DOF_DIR,
            Vector2(1, 0)
        ...
        _copy_screen();
        // manually do the blend if this is the first operation resolving from
        // the diffuse buffer
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_NEAR_BLUR_MERGE,
            composite_from == storage->frame.current_rt->buffers.diffuse
        );
        state.effect_blur_shader.set_conditional(
            EffectBlurShaderGLES3::DOF_NEAR_FIRST_TAP,
            false
        ...
        if (composite_from != storage->frame.current_rt->buffers.diffuse) {
            ...
            glBindTexture(
                GL_TEXTURE_2D,
                storage->frame.current_rt->buffers.diffuse
            );
        }
    ...
    if (env && env->auto_exposure) {
        ...
        state.exposure_shader.set_conditional(
            ExposureShaderGLES3::EXPOSURE_BEGIN,
            true
        );
        ...
        glUniform2iv(
            state.exposure_shader.get_uniform(
                ExposureShaderGLES3::SOURCE_RENDER_SIZE
            ),
            1,
            ss
        );
        glUniform2iv(
            state.exposure_shader.get_uniform(ExposureShaderGLES3::TARGET_SIZE),
            1,
            ds
        );
        glActiveTexture(GL_TEXTURE0);
        ...
        // third step, shrink to 1x1 pixel taking in consideration the previous
        // exposure
        state.exposure_shader.set_conditional(
            ExposureShaderGLES3::EXPOSURE_END,
            true
        );

        uint64_t tick = OS::get_singleton()->get_ticks_usec();
        uint64_t tick_diff =
            storage->frame.current_rt->last_exposure_tick == 0
                ? 0
                : tick - storage->frame.current_rt->last_exposure_tick;
        storage->frame.current_rt->last_exposure_tick = tick;

        if (tick_diff == 0 || tick_diff > 1000000) {
            state.exposure_shader.set_conditional(
                ExposureShaderGLES3::EXPOSURE_FORCE_SET,
                true
            );
        }
        ...
        glBindFramebuffer(
            GL_FRAMEBUFFER,
            exposure_shrink[exposure_shrink.size() - 1].fbo
        );
        glViewport(0, 0, 1, 1);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(
            GL_TEXTURE_2D,
            exposure_shrink[exposure_shrink.size() - 2].color
        );
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->exposure.color
        ); // read from previous

        state.exposure_shader.set_uniform(
            ExposureShaderGLES3::EXPOSURE_ADJUST,
            env->auto_exposure_speed * (tick_diff / 1000000.0)
        );
        state.exposure_shader.set_uniform(
            ExposureShaderGLES3::MAX_LUMINANCE,
            env->auto_exposure_max
        );
        state.exposure_shader.set_uniform(
            ExposureShaderGLES3::MIN_LUMINANCE,
            env->auto_exposure_min
        );
        ...
        // last step, swap with the framebuffer exposure, so the right exposure
        // is kept int he framebuffer
        SWAP(
            exposure_shrink.write[exposure_shrink.size() - 1].fbo,
            storage->frame.current_rt->exposure.fbo
        );
        SWAP(
            exposure_shrink.write[exposure_shrink.size() - 1].color,
            storage->frame.current_rt->exposure.color
        );
        ...
        VisualServerRaster::redraw_request(
        ); // if using auto exposure, redraw must happen
    }
    ...
    if (env && env->glow_enabled) {
        for (int i = 0; i < VS::MAX_GLOW_LEVELS; i++) {
            if (env->glow_levels & (1 << i)) {
                if (i >= storage->frame.current_rt->effects.mip_maps[1]
                             .sizes.size()) {
                    max_glow_level =
                        storage->frame.current_rt->effects.mip_maps[1]
                            .sizes.size()
                        - 1;
        ...
        for (int i = 0; i < (max_glow_level + 1); i++) {
            ...
            if (i == 0) {
                state.effect_blur_shader.set_conditional(
                    EffectBlurShaderGLES3::GLOW_FIRST_PASS,
                    true
                );
                state.effect_blur_shader.set_conditional(
                    EffectBlurShaderGLES3::GLOW_USE_AUTO_EXPOSURE,
                    env->auto_exposure
                );
            }

            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES3::GLOW_GAUSSIAN_HORIZONTAL,
                true
            );
            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES3::USE_GLOW_HIGH_QUALITY,
                env->glow_high_quality
            );
            state.effect_blur_shader.bind();
            state.effect_blur_shader.set_uniform(
                EffectBlurShaderGLES3::PIXEL_SIZE,
            ...
            state.effect_blur_shader.set_uniform(
                EffectBlurShaderGLES3::GLOW_STRENGTH,
                env->glow_strength
            );
            state.effect_blur_shader.set_uniform(
                EffectBlurShaderGLES3::LUMINANCE_CAP,
                env->glow_hdr_luminance_cap
            );
            ...
            if (i == 0) {
                ...
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES3::EXPOSURE,
                    env->tone_mapper_exposure
                );
                if (env->auto_exposure) {
                    state.effect_blur_shader.set_uniform(
                        EffectBlurShaderGLES3::AUTO_EXPOSURE_GREY,
                        env->auto_exposure_grey
                    );
                }
                ...
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES3::GLOW_BLOOM,
                    env->glow_bloom
                );
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES3::GLOW_HDR_THRESHOLD,
                    env->glow_hdr_bleed_threshold
                );
                state.effect_blur_shader.set_uniform(
                    EffectBlurShaderGLES3::GLOW_HDR_SCALE,
                    env->glow_hdr_bleed_scale
                );
            ...
            // vertical pass
            state.effect_blur_shader.set_conditional(
                EffectBlurShaderGLES3::GLOW_GAUSSIAN_VERTICAL,
                true
            );
            state.effect_blur_shader.bind();
            state.effect_blur_shader.set_uniform(
    ...
    glBindTexture(GL_TEXTURE_2D, composite_from);
    if (env) {
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_FILMIC_TONEMAPPER,
            env->tone_mapper == VS::ENV_TONE_MAPPER_FILMIC
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_ACES_TONEMAPPER,
            env->tone_mapper == VS::ENV_TONE_MAPPER_ACES
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_ACES_FITTED_TONEMAPPER,
            env->tone_mapper == VS::ENV_TONE_MAPPER_ACES_FITTED
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_REINHARD_TONEMAPPER,
            env->tone_mapper == VS::ENV_TONE_MAPPER_REINHARD
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_AUTO_EXPOSURE,
            env->auto_exposure
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_GLOW_FILTER_BICUBIC,
            env->glow_bicubic_upscale
        );
    }

    state.tonemap_shader.set_conditional(
        TonemapShaderGLES3::KEEP_3D_LINEAR,
        storage->frame.current_rt
            ->flags[RasterizerStorage::RENDER_TARGET_KEEP_3D_LINEAR]
    );
    state.tonemap_shader.set_conditional(
        TonemapShaderGLES3::USE_FXAA,
        storage->frame.current_rt->use_fxaa
    );
    state.tonemap_shader.set_conditional(
        TonemapShaderGLES3::USE_DEBANDING,
        storage->frame.current_rt->use_debanding
    );
    state.tonemap_shader.set_conditional(
        TonemapShaderGLES3::USE_SHARPENING,
        storage->frame.current_rt->sharpen_intensity >= 0.001
    );
    ...
    if (env && max_glow_level >= 0) {
        for (int i = 0; i < (max_glow_level + 1); i++) {
            if (glow_mask & (1 << i)) {
                if (i == 0) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL1,
                        true
                    );
                }
                if (i == 1) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL2,
                        true
                    );
                }
                if (i == 2) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL3,
                        true
                    );
                }
                if (i == 3) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL4,
                        true
                    );
                }
                if (i == 4) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL5,
                        true
                    );
                }
                if (i == 5) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL6,
                        true
                    );
                }
                if (i == 6) {
                    state.tonemap_shader.set_conditional(
                        TonemapShaderGLES3::USE_GLOW_LEVEL7,
                        true
                    );
                }
            }
        ...
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_GLOW_SCREEN,
            env->glow_blend_mode == VS::GLOW_BLEND_MODE_SCREEN
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_GLOW_SOFTLIGHT,
            env->glow_blend_mode == VS::GLOW_BLEND_MODE_SOFTLIGHT
        );
        state.tonemap_shader.set_conditional(
            TonemapShaderGLES3::USE_GLOW_REPLACE,
            env->glow_blend_mode == VS::GLOW_BLEND_MODE_REPLACE
        );
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(
            GL_TEXTURE_2D,
    ...
    if (env && env->adjustments_enabled) {
        ...
        RasterizerStorageGLES3::Texture* tex =
            storage->texture_owner.getornull(env->color_correction);
        if (tex) {
            state.tonemap_shader.set_conditional(
                TonemapShaderGLES3::USE_COLOR_CORRECTION,
                true
            );
    ...
    state.tonemap_shader.set_conditional(
        TonemapShaderGLES3::V_FLIP,
        storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_VFLIP]
    );
    state.tonemap_shader.bind();

    if (env) {
        state.tonemap_shader.set_uniform(
            TonemapShaderGLES3::EXPOSURE,
            env->tone_mapper_exposure
        );
        state.tonemap_shader.set_uniform(
            TonemapShaderGLES3::WHITE,
            env->tone_mapper_exposure_white
        );

        if (max_glow_level >= 0) {
            state.tonemap_shader.set_uniform(
                TonemapShaderGLES3::GLOW_INTENSITY,
                env->glow_intensity
            );
            int ss[2] = {
                storage->frame.current_rt->width,
            ...
            glUniform2iv(
                state.tonemap_shader.get_uniform(
                    TonemapShaderGLES3::GLOW_TEXTURE_SIZE
                ),
                1,
                ss
            );
        ...
        if (env->auto_exposure) {
            ...
            state.tonemap_shader.set_uniform(
                TonemapShaderGLES3::AUTO_EXPOSURE_GREY,
                env->auto_exposure_grey
            );
        }
        ...
        if (env->adjustments_enabled) {
            state.tonemap_shader.set_uniform(
                TonemapShaderGLES3::BCS,
                Vector3(
                    env->adjustments_brightness,
                    env->adjustments_contrast,
                    env->adjustments_saturation
                )
            );
        }
    } else {
    ...
    if (storage->frame.current_rt->sharpen_intensity >= 0.001) {
        state.tonemap_shader.set_uniform(
            TonemapShaderGLES3::SHARPEN_INTENSITY,
            storage->frame.current_rt->sharpen_intensity
        );
    }
...
bool RasterizerSceneGLES3::_element_needs_directional_add(RenderList::Element* e
    ...
    for (int i = 0; i < state.directional_light_count; i++) {
        ...
        if (e->instance->baked_light
            && l->light_ptr->bake_mode == VS::LightBakeMode::LIGHT_BAKE_ALL) {
            continue;
        }
...
void RasterizerSceneGLES3::render_scene(
    ...
    state.scene_shader.set_conditional(
        SceneShaderGLES3::USE_SHADOW,
        use_shadows
    );

    if (use_shadows) {
        glActiveTexture(
            GL_TEXTURE0 + storage->config.max_texture_image_units - 5
        );
        glBindTexture(GL_TEXTURE_2D, shadow_atlas->depth);
        glTexParameteri(
            GL_TEXTURE_2D,
    ...
    if (reflection_atlas && reflection_atlas->size) {
        glActiveTexture(
            GL_TEXTURE0 + storage->config.max_texture_image_units - 3
        );
        glBindTexture(GL_TEXTURE_2D, reflection_atlas->color);
    }
    ...
    for (int i = 0; i < p_light_cull_count; i++) {
        ...
        LightInstance* li = light_instance_owner.getptr(p_light_cull_result[i]);
        if (li->light_ptr->param[VS::LIGHT_PARAM_CONTACT_SHADOW_SIZE]
            > CMP_EPSILON) {
    ...
    // If contact shadows are used then we need to do depth prepass even if it's
    // otherwise disabled
    ...
    use_depth_prepass =
        use_depth_prepass && storage->frame.current_rt
        && !storage->frame.current_rt
                ->flags[RasterizerStorage::RENDER_TARGET_NO_3D_EFFECTS];
    use_depth_prepass = use_depth_prepass
                     && state.debug_draw != VS::VIEWPORT_DEBUG_DRAW_OVERDRAW;
    ...
    if (use_depth_prepass) {
        ...
        state.scene_shader.set_conditional(
            SceneShaderGLES3::RENDER_DEPTH,
            true
        );
        _render_list(
            render_list.elements,
            render_list.element_count,
            p_cam_transform,
            p_cam_projection,
            nullptr,
            false,
    ...
    _setup_reflections(
        p_reflection_probe_cull_result,
        p_reflection_probe_cull_count,
        p_cam_transform.affine_inverse(),
        p_cam_projection,
        p_reflection_atlas,
        env
    ...
    ReflectionProbeInstance* probe =
        reflection_probe_instance_owner.getornull(p_reflection_probe);
    GLuint current_fbo;

    if (probe) {
        ReflectionAtlas* ref_atlas =
            reflection_atlas_owner.getptr(probe->atlas);
        ...
        current_fbo =
            reflection_cubemaps[cubemap_index].fbo_id[p_reflection_probe_pass];
        use_mrt = false;
        state.scene_shader.set_conditional(
            SceneShaderGLES3::USE_MULTIPLE_RENDER_TARGETS,
            false
        );
        ...
        glViewport(
            0,
            0,
            reflection_cubemaps[cubemap_index].size,
            reflection_cubemaps[cubemap_index].size
        );
        ...
        use_mrt = env
               && (state.used_sss || env->ssao_enabled || env->ssr_enabled
                   || env->dof_blur_far_enabled || env->dof_blur_near_enabled
               ); // only enable MRT rendering if any of these is enabled
        ...
        use_mrt = use_mrt
               && !storage->frame.current_rt
                       ->flags[RasterizerStorage::RENDER_TARGET_NO_3D_EFFECTS];
        use_mrt =
            use_mrt && state.debug_draw != VS::VIEWPORT_DEBUG_DRAW_OVERDRAW;
        use_mrt = use_mrt
               && (env->bg_mode != VS::ENV_BG_KEEP
        ...
        if (use_mrt) {
            ...
            if (storage->frame.current_rt->buffers.active) {
                ...
                if (storage->frame.current_rt->effects.mip_maps[0].sizes.size()
                    == 0) {
                    ERR_PRINT_ONCE(
                        "Can't use canvas background mode in a render target "
                        "configured without sampling"
                    );
                    return;
                }
                current_fbo =
    ...
    if (state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_OVERDRAW) {
        ...
        feed =
            CameraServer::get_singleton()->get_feed_by_id(env->camera_feed_id);
        storage->frame.clear_request = false;
    } else {
        storage->frame.clear_request = false;
    }
    ...
    VS::EnvironmentBG bg_mode =
        (!env || (probe && env->bg_mode == VS::ENV_BG_CANVAS))
            ? VS::ENV_BG_CLEAR_COLOR
            : env->bg_mode; // if no environment, or canvas while rendering a
                            // probe (invalid use case), use color.
    ...
    if (env) {
        switch (bg_mode) {
            ...
            case VS::ENV_BG_CANVAS:
                ...
                storage->shaders.copy.set_conditional(
                    CopyShaderGLES3::SRGB_TO_LINEAR,
                    true
                );
            ...
            case VS::ENV_BG_CAMERA_FEED:
                if (feed.is_valid() && (feed->get_base_width() > 0)
                    && (feed->get_base_height() > 0)) {
                    ...
                    storage->shaders.copy.set_conditional(
                        CopyShaderGLES3::USE_DISPLAY_TRANSFORM,
                        true
                    );
                    storage->shaders.copy.set_conditional(
                        CopyShaderGLES3::DISABLE_ALPHA,
                    ...
                    storage->shaders.copy.bind();
                    storage->shaders.copy.set_uniform(
                        CopyShaderGLES3::DISPLAY_TRANSFORM,
                        feed->get_transform()
                    );
    ...
    if (probe && probe->probe_ptr->interior) {
        sky = nullptr; // for rendering probe interiors, radiance must not be
                       // used.
    }
    ...
    if (env && env->bg_mode == VS::ENV_BG_SKY
        ...
        if (use_mrt) {
            glBindFramebuffer(GL_FRAMEBUFFER,storage->frame.current_rt->buffers.fbo);
        //switch to alpha fbo for sky, only diffuse/ambient matters
        */
    ...
    if (use_mrt) {
        ...
        // Here we have to do the blits/resolves that otherwise are done in the
        // MRT rendering, in particular
        // - prepare screen texture for any geometry that uses a shader with
        // screen texture
        // - prepare depth texture for any geometry that uses a shader with
        // depth texture

        bool framebuffer_dirty = false;

        if (storage->frame.current_rt
            && storage->frame.current_rt->buffers.active
            && state.used_screen_texture) {
            glBindFramebuffer(
                GL_READ_FRAMEBUFFER,
                storage->frame.current_rt->buffers.fbo
            );
    ...
    if (storage->frame.current_rt && state.used_depth_texture
        && storage->frame.current_rt->buffers.active) {
        _bind_depth_texture();
    }
    ...
    if (storage->frame.current_rt && state.used_screen_texture
        && storage->frame.current_rt->buffers.active) {
        glActiveTexture(
            GL_TEXTURE0 + storage->config.max_texture_image_units - 7
        );
        glBindTexture(
            GL_TEXTURE_2D,
            storage->frame.current_rt->effects.mip_maps[0].color
    ...
    if (state.directional_light_count <= 1) {
        ...
        _render_list(
            &render_list.elements
                 [render_list.max_elements - render_list.alpha_element_count],
        ...
        while (chunk_split < render_list.max_elements) {
            ...
            for (; chunk_split < render_list.max_elements; chunk_split++) {
                bool directional_add = _element_needs_directional_add(
                    render_list.elements[chunk_split]
                );
                uint32_t priority = uint32_t(
                    render_list.elements[chunk_split]->sort_key
                    >> RenderList::SORT_KEY_PRIORITY_SHIFT
                );
                if (first) {
            ...
            if (chunk_directional_add) {
                for (int i = 0; i < state.directional_light_count; i++) {
                    ...
                    _render_list(
                        &render_list.elements[chunk_start],
                        chunk_split - chunk_start,
                        p_cam_transform,
                        p_cam_projection,
                        sky,
                        false,
    ...
    if (env && (env->dof_blur_far_enabled || env->dof_blur_near_enabled)
        && storage->frame.current_rt
        && storage->frame.current_rt->buffers.active) {
        _prepare_depth_texture();
    }
    ...
    /*	if (shadow_atlas && storage->frame.current_rt) {
        ...
        glBindTexture(GL_TEXTURE_2D, shadow_atlas->depth);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_NONE);
        storage->canvas->draw_generic_textured_rect(Rect2(0, 0,
    storage->frame.current_rt->width / 2, storage->frame.current_rt->height /
    2), Rect2(0, 0, 1, 1));
    }
    ...
    if (storage->frame.current_rt) {
        ...
        storage->canvas->draw_generic_textured_rect(Rect2(0, 0,
    storage->frame.current_rt->width / 16, storage->frame.current_rt->height /
    16), Rect2(0, 0, 1, 1));
    }
    ...
    if ( env_radiance_tex) {
        ...
        storage->canvas->draw_generic_textured_rect(Rect2(0, 0,
    storage->frame.current_rt->width / 2, storage->frame.current_rt->height /
    2), Rect2(0, 0, 1, 1)); glTexParameteri(GL_TEXTURE_2D,
...
void RasterizerSceneGLES3::render_shadow(
    RID p_light,
    RID p_shadow_atlas,
    int p_pass,
    InstanceBase** p_cull_result,
    ...
    ERR_FAIL_COND(!light_instance);
    RasterizerStorageGLES3::Light* light =
        storage->light_owner.getornull(light_instance->light);
    ERR_FAIL_COND(!light);
    ...
    if (light->type == VS::LIGHT_DIRECTIONAL) {
        ...
        if (light_instance->last_scene_shadow_pass != scene_pass) {
            ...
            light_instance->light_directional_index =
                directional_shadow.current_light;
            light_instance->last_scene_shadow_pass = scene_pass;
            directional_shadow.current_light++;

            if (directional_shadow.light_count == 1) {
                light_instance->directional_rect = Rect2(
                    0,
                    0,
                    directional_shadow.size,
                    directional_shadow.size
                );
                ...
                light_instance->directional_rect = Rect2(
                    ...
                    0,
                    directional_shadow.size,
                    directional_shadow.size / 2
                );
                if (light_instance->light_directional_index == 1) {
                    light_instance->directional_rect.position.x +=
                        light_instance->directional_rect.size.x;
                }
            } else { // 3 and 4
                light_instance->directional_rect = Rect2(
                    0,
                ...
                if (light_instance->light_directional_index / 2) {
                    light_instance->directional_rect.position.y +=
                        light_instance->directional_rect.size.y;
                }
            }
        }
        ...
        float bias_mult = Math::lerp(
            1.0f,
            light_instance->shadow_transform[p_pass].bias_scale,
            light->param[VS::LIGHT_PARAM_SHADOW_BIAS_SPLIT_SCALE]
        );
        zfar = light->param[VS::LIGHT_PARAM_RANGE];
        bias = light->param[VS::LIGHT_PARAM_SHADOW_BIAS] * bias_mult;
        normal_bias =
            light->param[VS::LIGHT_PARAM_SHADOW_NORMAL_BIAS] * bias_mult;
        ...
        ShadowAtlas* shadow_atlas =
            shadow_atlas_owner.getornull(p_shadow_atlas);
        ERR_FAIL_COND(!shadow_atlas);
        ...
        ERR_FAIL_INDEX(
            (int)shadow,
            shadow_atlas->quadrants[quadrant].shadows.size()
        );
        ...
        uint32_t shadow_size =
            (quadrant_size / shadow_atlas->quadrants[quadrant].subdivision);
        x += (shadow % shadow_atlas->quadrants[quadrant].subdivision)
           * shadow_size;
        y += (shadow / shadow_atlas->quadrants[quadrant].subdivision)
           * shadow_size;
        ...
        if (light->type == VS::LIGHT_OMNI) {
            if (light->omni_shadow_mode == VS::LIGHT_OMNI_SHADOW_CUBE) {
                ...
                state.scene_shader.set_conditional(
                    SceneShaderGLES3::RENDER_DEPTH_DUAL_PARABOLOID,
                    true
                );
            }
    ...
    if (light->type == VS::LIGHT_OMNI
        && light->omni_shadow_mode == VS::LIGHT_OMNI_SHADOW_CUBE
        && p_pass == 5) {
        ...
        glBindTexture(
            GL_TEXTURE_CUBE_MAP,
            shadow_cubemaps[current_cubemap].cubemap
        );
        glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_COMPARE_MODE, GL_NONE);
        glDisable(GL_CULL_FACE);

        for (int i = 0; i < 2; i++) {
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES3::Z_FLIP,
                i == 1
            );
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES3::Z_NEAR,
                light_projection.get_z_near()
            );
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES3::Z_FAR,
                light_projection.get_z_far()
            );
            state.cube_to_dp_shader.set_uniform(
                CubeToDpShaderGLES3::BIAS,
                light->param[VS::LIGHT_PARAM_SHADOW_BIAS]
            );
...
bool RasterizerSceneGLES3::free(RID p_rid) {
    if (light_instance_owner.owns(p_rid)) {
        ...
        for (Set<RID>::Element* E = light_instance->shadow_atlases.front(); E;
             E                    = E->next()) {
        ...
        ReflectionProbeInstance* reflection_instance =
            reflection_probe_instance_owner.get(p_rid);
...
void RasterizerSceneGLES3::set_debug_draw_mode(
    VS::ViewportDebugDraw p_debug_draw
) {
...
void RasterizerSceneGLES3::initialize() {
...

    {
        ...
        storage->shader_set_code(
            default_shader_twosided,
            "shader_type spatial; render_mode cull_disabled;\n"
        );
        storage->material_set_shader(
            default_material_twosided,
            default_shader_twosided
        );
        ...
        storage->shader_set_code(
            default_worldcoord_shader_twosided,
            "shader_type spatial; render_mode "
    ...
    render_list.max_elements = GLOBAL_DEF_RST(
    ...
    render_list.max_lights = GLOBAL_DEF(
    ...
    render_list.max_reflections = GLOBAL_DEF(
    ...
    render_list.max_lights_per_object = GLOBAL_DEF_RST(
...

    {
        ...
        glVertexAttribPointer(
            ...
            GL_FALSE,
            sizeof(Vector3) * 2,
            CAST_INT_TO_UCHAR_PTR(sizeof(Vector3))
        );
        glEnableVertexAttribArray(VS::ARRAY_TEX_UV);
        glBindVertexArray(0);
        glBindBuffer(GL_ARRAY_BUFFER, 0); // unbind
    ...
    int max_shadow_cubemap_sampler_size =
        MIN(int(GLOBAL_GET("rendering/quality/shadow_atlas/cubemap_size")),
            storage->config.max_cubemap_texture_size);
...

    {
        ...
        directional_shadow.size        = next_power_of_2(
            GLOBAL_GET("rendering/quality/directional_shadow/size")
        );
...

    {
        ...
        state.max_ubo_lights =
            MIN(render_list.max_lights, max_ubo_size / ubo_light_size);

        state.spot_array_tmp =
            (uint8_t*)memalloc(ubo_light_size * state.max_ubo_lights);
        state.omni_array_tmp =
            (uint8_t*)memalloc(ubo_light_size * state.max_ubo_lights);
        ...
        glBufferData(
            GL_UNIFORM_BUFFER,
            ubo_light_size * state.max_ubo_lights,
            nullptr,
            GL_DYNAMIC_DRAW
        );
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
        ...
        state.max_forward_lights_per_object =
            MIN(state.max_ubo_lights, render_list.max_lights_per_object);

        state.scene_shader.add_custom_define(
            "#define MAX_LIGHT_DATA_STRUCTS " + itos(state.max_ubo_lights)
            + "\n"
        );
        state.scene_shader.add_custom_define(
            "#define MAX_FORWARD_LIGHTS "
            + itos(state.max_forward_lights_per_object) + "\n"
        );

        state.max_ubo_reflections =
            MIN(render_list.max_reflections,
                max_ubo_size / (int)sizeof(ReflectionProbeDataUBO));

        state.reflection_array_tmp = (uint8_t*)memalloc(
            sizeof(ReflectionProbeDataUBO) * state.max_ubo_reflections
        );
        ...
        state.scene_shader.add_custom_define(
            "#define MAX_REFLECTION_DATA_STRUCTS "
            + itos(state.max_ubo_reflections) + "\n"
        );

        state.max_skeleton_bones =
            MIN(2048, max_ubo_size / (12 * sizeof(float)));
        state.scene_shader.add_custom_define(
            "#define MAX_SKELETON_BONES " + itos(state.max_skeleton_bones)
            + "\n"
        );
    }
    ...
    {
        uint32_t immediate_buffer_size = GLOBAL_DEF(
...
void RasterizerSceneGLES3::iteration() {
    shadow_filter_mode =
        ShadowFilterMode(int(GLOBAL_GET("rendering/quality/shadows/filter_mode")
        ));
    subsurface_scatter_follow_surface =
        GLOBAL_GET("rendering/quality/subsurface_scattering/follow_surface");
    subsurface_scatter_weight_samples =
        GLOBAL_GET("rendering/quality/subsurface_scattering/weight_samples");
    subsurface_scatter_quality = SubSurfaceScatterQuality(
        int(GLOBAL_GET("rendering/quality/subsurface_scattering/quality"))
    );
    subsurface_scatter_size =
        GLOBAL_GET("rendering/quality/subsurface_scattering/scale");

    storage->config.use_lightmap_filter_bicubic =
        GLOBAL_GET("rendering/quality/lightmapping/use_bicubic_sampling");
    state.scene_shader.set_conditional(
        SceneShaderGLES3::USE_LIGHTMAP_FILTER_BICUBIC,
        storage->config.use_lightmap_filter_bicubic
    );
    state.scene_shader.set_conditional(
        SceneShaderGLES3::VCT_QUALITY_HIGH,
        GLOBAL_GET("rendering/quality/voxel_cone_tracing/high_quality")
    );
}
