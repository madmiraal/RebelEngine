...
public:
    ...
    struct State {
        ...
        struct SceneDataUBO {
            ...
            // Specification spec before doing any changes
        ...
        static_assert(
            sizeof(SceneDataUBO) % 16 == 0,
            "SceneDataUBO size must be a multiple of 16 bytes"
        );
    ...
    struct ReflectionProbeDataUBO {
        ...
        float atlas_clamp[4];
        float local_matrix[16]; // up to here for spot and omni, rest is for
                                // directional
        // notes: for ambientblend, use distance to edge to blend between
        // already existing global environment
    };
    ...
    struct Environment : public RID_Data {
        ...
        Environment() :
            ...
            auto_exposure(false),
            auto_exposure_speed(0.5f),
            auto_exposure_min(0.05f),
            auto_exposure_max(8),
            auto_exposure_grey(0.4f),
            ...
            fog_enabled(false),
            fog_color(Color(0.5f, 0.5f, 0.5f)),
            fog_sun_color(Color(0.8f, 0.8f, 0.0f)),
            fog_sun_amount(0),
    ...
    struct LightDataUBO {
        ...
        float light_params[4]; // spot attenuation, spot angle, specular, shadow
                               // enabled
        float light_clamp[4];
        ...
        union {
            struct {
                float matrix1[16]; // up to here for spot and omni, rest is for
                                   // directional
                float matrix2[16];
                float matrix3[16];
                float matrix4[16];
    ...
    struct LightInstance : public RID_Data {
        ...
        Set<RID> shadow_atlases; // shadow atlases where this light is
                                 // registered
    ...
    struct GIProbeInstance : public RID_Data {
        ...
        GIProbeInstance() : probe(nullptr), tex_cache(0) {}
    };
...
#define SORT_KEY_VERTEX_LIT_FLAG       (uint64_t(1) << 44)
        ...
        struct SortByReverseDepthAndPriority {
            _FORCE_INLINE_ bool operator()(const Element* A, const Element* B)
                const {
                uint32_t layer_A =
                    uint32_t(A->sort_key >> SORT_KEY_PRIORITY_SHIFT);
                uint32_t layer_B =
                    uint32_t(B->sort_key >> SORT_KEY_PRIORITY_SHIFT);
                if (layer_A == layer_B) {
                    return A->instance->depth > B->instance->depth;
                } else {
    ...
    _FORCE_INLINE_ void _add_geometry(
    ...
    _FORCE_INLINE_ void _add_geometry_with_material(
        RasterizerStorageGLES3::Geometry* p_geometry,
        InstanceBase* p_instance,
        RasterizerStorageGLES3::GeometryOwner* p_owner,
        RasterizerStorageGLES3::Material* p_material,
    ...
    void _copy_screen(
        bool p_invalidate_color = false,
        bool p_invalidate_depth = false
    );
